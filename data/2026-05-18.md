...
TL;DR: 提出ChangeFlow，一种基于潜在空间整流流生成变化掩码的生成式遥感变化检测框架，在四个基准上平均F1达80.4%，优于先前最佳方法1.3个百分点，同时推理速度与强基线相当。


<details>
  <summary>Details</summary>
Motivation: 现有判别式方法只产生单一预测，无法建模变化区域的整体性和歧义性；生成式方法计算成本高且条件机制复杂，性能落后于判别式基线。

Method: 将变化检测重构为在潜在空间中通过整流流（Rectified Flow）合成变化掩码的生成过程，采用结构化的轻量条件信号，支持基于采样的预测集成。

Result: 在四个基准数据集上平均F1为80.4%，比此前最佳方法平均提升1.3个百分点，推理速度与近期强基线相当。

Conclusion: ChangeFlow通过生成式建模有效捕捉变化区域的整体性和歧义性，同时保持高效推理，证明生成式方法在变化检测中的潜力。
...
TL;DR: 提出PanoWorld，一种从单张图像和描述生成几何一致的全景视频的世界模型。


<details>
  <summary>Details</summary>
Motivation: 现有全景视频方法仅优化视觉真实性，未显式约束3D场景状态，导致深度不一致、对应关系断裂和运动不真实。

Method: 将全景视频生成视为几何和动态一致的潜在状态建模，在预训练透视视频世界模型上引入深度一致性损失和轨迹一致性损失，并采用球面几何自适应条件编码和位置编码，同时构建PanoGeo数据集。

Result: PanoWorld在保持视觉真实性的同时，显著提升了几何一致性，优于现有方法。

Conclusion: 全景视频生成应作为几何建模问题，以支持具身AI的全面空间理解。
...
TL;DR: 提出MR2-ByteTrack，一种在MCU上实现视频目标检测的方法，通过多分辨率推理和跨帧分数聚合，兼顾精度与效率。


<details>
  <summary>Details</summary>
Motivation: 智能视觉传感器需要设备端智能，但MCU资源有限，传统视频检测需要大存储和多帧缓冲，不可行。

Method: 交替全分辨率和低分辨率推理，用ByteTrack关联检测，用Rescore算法聚合置信度，跨帧纠正误分类。

Result: 在ImageNetVID上，CNN模型mAP 49.0，Transformer 48.7；计算量减少53%（CNN）和32%（Transformer）；GAP9 MCU上节能55%。

Conclusion: 首次在MCU上实现实时Transformer视频目标检测，验证了方法的通用性和有效性。
...
TL;DR: 提出基于Retinex理论的同质图像分解方法RIDE，用于隐藏目标分割，理论保证可分辨性提升，并在多个子任务上有效。


<details>
  <summary>Details</summary>
Motivation: 现有隐藏目标分割方法直接操作RGB图像或使用异质分解，导致像素级线索不直接。作者发现视觉伪装在复合空间匹配，但不必在分量空间同时匹配，由此提出可分辨性差距定理。

Method: RIDE包含三个模块：任务驱动Retinex分解端到端学习分割最优分解；可分辨性差距注意力自适应利用分解优势；伪装破坏对比损失在反射率特征空间操作。

Result: 理论证明Retinex分解保持或严格提升前背景可分辨性，反相关最大化增益。实验在多个COS子任务上验证有效性。

Conclusion: 同质图像分解（Retinex）为隐藏目标分割提供了新视角，理论保证和实用模块结合，显著提升分割性能。
...
TL;DR: 提出DiffVAS，一种基于扩散模型和强化学习的目标条件策略，用于在部分可观测环境中同时搜索多种目标对象，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉主动搜索方法假设搜索空间完全已知且针对特定目标，无法处理部分可观测环境和多目标搜索任务，限制了实际应用。

Method: DiffVAS利用扩散模型从顺序观测的局部图像重建整个地理区域，再通过目标条件强化学习规划模块指导后续搜索步骤。

Result: 在多个数据集上，DiffVAS在搜索多样目标方面显著超越当前最优方法，尤其在部分可观测环境中表现优异。

Conclusion: DiffVAS提高了视觉主动搜索的现实适用性，能够灵活应对部分可观测环境和多目标搜索需求。
...
TL;DR: 提出一种无需微调、基于指令的视频编辑框架，通过结构噪声初始化策略和噪声引导机制，有效利用噪声隐空间信息，实现高质量视频编辑。


<details>
  <summary>Details</summary>
Motivation: 现有免调方法未能充分利用噪声隐空间中的丰富信息，导致编辑效果不佳。

Method: 设计结构噪声初始化策略，为编辑区域分配高噪声以促进变化，为未编辑区域分配低噪声以保持一致性；引入噪声引导机制，利用生成模型中的视频先验和噪声隐空间信息指导去噪过程。

Result: 实验表明，该方法在视觉质量和性能上达到先进水平。

Conclusion: 该方法有效提升了视频编辑的一致性和质量。
...
TL;DR: 在极低标注数据的细粒度分类中，比较不同预训练目标（监督、对比、掩码重建、自蒸馏）对下游表示质量的影响，发现在线性探针下监督和对比编码器表现最佳，而MAE在非线性探针下提升。


<details>
  <summary>Details</summary>
Motivation: 专家领域标注成本高，需要指导如何选择预训练编码器。

Method: 使用定制翡翠内含物等级数据集，控制主干容量，通过留一法交叉验证和置换检验评估四种ViT-B/16编码器。

Result: 监督和对比编码器线性可分性最强（Logistic AUC 0.768/0.735, SVM AUC 0.739/0.697），MAE在XGBoost下AUC 0.713，DINOv3表现最差。
...
TL;DR: 潜空间预测模型在多项鲁棒性指标上优于其他模型，支持其作为世界模型的潜力。


<details>
  <summary>Details</summary>
Motivation: 现有评估仅用单一准确率，无法全面理解视频模型作为世界模型的潜力。

Method: 系统比较四种前沿视频基础模型在五项鲁棒性轴上的表现。

Result: 潜空间预测模型在像素损坏、遮挡、细粒度运动线索和时间方向编码等方面表现更优。

Conclusion: 实验结果支持潜空间预测用于构建鲁棒世界模型。
...
TL;DR: 提出速度自适应引导尺度（VAGS），替代固定CFG，通过时间信号水平和速度场对齐动态调整引导强度，无需训练，提升编辑和生成质量。


<details>
  <summary>Details</summary>
Motivation: 固定CFG在整个ODE轨迹上使用相同尺度，忽略了早期噪声主导、后期结构承诺的差异，且引导强度应与当前动力学一致性匹配。

Method: VAGS将名义尺度乘以一个包含时间信号水平项和任务相关速度场余弦相似度的有界因子。编辑中衡量源与目标引导速度的对齐，生成中使用无条件和条件速度的对齐。无需微调、辅助网络或额外前向传播。

Result: 在PIE-Bench、DIV2K、COCO17、CUB-200、Flickr30K上，VAGS相比固定CFG和近期无训练引导变体，持续提升结构保真度和生成质量。

Conclusion: VAGS是一种简单有效的训练自由替代方案，可恢复固定CFG作为特例，适用于编辑和生成任务。
...
TL;DR: 视觉-语言模型在执行基本视觉操作如线追踪时，因局部竞争而频繁丢失目标路径，现有方法无法稳健解决。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言模型在多模态基准测试中表现出色，但在基本视觉操作（如线追踪）上缺乏鲁棒控制，需要深入探究其失败原因。

Method: 设计控制性线追踪任务，通过引入附近竞争者并减少语义和拓扑歧义（如交叉和重叠）来隔离模型能力，并进行行为干预和内部分析。

Result: 最先进的视觉-语言模型在任务中频繁丢失目标路径并转向局部相似的替代路径，模型规模扩展效果有限，推理虽能部分补偿但成本高，显式追踪指令无效，且在复杂场景中同样失败。

Conclusion: 局部竞争是导致线追踪失败的核心瓶颈，现有标准补救措施无法有效克服这一限制。
...
TL;DR: DiLA通过内容-结构解缠解决了潜在动作模型中动作抽象与生成保真度之间的权衡，实现了高质量视频生成和可解释的潜在动作空间。


<details>
  <summary>Details</summary>
Motivation: 现有潜在动作模型面临动作抽象与生成保真度之间的根本权衡，通常需要两阶段训练或限制预测为光流，缺乏统一高效的解决方案。

Method: 提出DiLA，一种基于内容-结构解缠的潜在动作世界模型，利用潜在动作学习的预测瓶颈作为解缠驱动力，将空间布局分离到结构路径，视觉细节分离到内容路径。

Result: DiLA在视频生成质量、动作迁移、视觉规划和流形可解释性上均取得优于现有方法的结果。

Conclusion: DiLA作为统一框架同时实现了高级动作抽象与高保真生成，推动了自监督世界模型学习的前沿。
...
TL;DR: 提出DecomPose框架，通过难度感知梯度解耦和稳定性驱动非对称分支，缓解跨类别优化冲突，提升类别级6D姿态估计性能。


<details>
  <summary>Details</summary>
Motivation: 传统多类别联合学习因几何异质性导致梯度冲突和负迁移，需解决优化干扰问题。

Method: 基于梯度诊断，提出困难感知梯度解耦（按难度分组路由）和稳定性驱动非对称分支（简单类别用高容量分支稳定训练，复杂类别用轻量分支抑制噪声）。

Result: 在REAL275、CAMERA25和HouseCat6D数据集上，有效降低优化冲突，取得领先姿态估计性能。

Conclusion: DecomPose通过解耦优化冲突，显著提升多类别6D姿态估计的准确性和泛化能力。
...
TL;DR: 提出UAM模型，通过添加平行背侧专家保留VLM多模态能力，避免微调导致的语义退化。


<details>
  <summary>Details</summary>
Motivation: 标准VLA微调会侵蚀VLM的多模态能力（具身税），需解决这一结构瓶颈。

Method: 受生物视觉双通路启发，添加一个预训练生成模型初始化的背侧专家，并用预测视觉动态的中等推理目标训练，实现端到端训练而不冻结参数。

Result: 保留VLM超95%多模态能力，并在多项操控任务中取得最高平均成功率。

Conclusion: 语义保留可通过架构分离实现，无需冻结权重或辅助数据回放，该能力可自然迁移到动作语义泛化。
...
TL;DR: 提出BiomedAP框架，通过门控跨模态融合和双锚点约束解决生物医学视觉语言模型对提示变动的脆弱性，提升少样本诊断的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有生物医学VLM的适应框架将视觉和文本提示独立优化，依赖理想的金色提示，在临床噪声描述下跨模态对齐不稳定。

Method: BiomedAP包括门控跨模态融合（层间交互动态抑制噪声）和双锚点约束（专家模板高锚点与少样本视觉原型低锚点正则化）。

Result: 在11个基准上超越基线，少样本精度高且对提示扰动鲁棒性显著增强。

Conclusion: BiomedAP有效提升了生物医学VLM在噪声提示下的少样本诊断鲁棒性。
...
TL;DR: 提出GRASP数据集和SGR奖励方法，提升多人物视频中社交交互推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大模型在多人物视频中难以识别谁与谁互动。

Method: 构建GRASP数据集（290K问答对、46K视频、749小时、16类别社交事件），提出Social Grounding Reward (SGR) 利用社交事件引导模型推理。

Result: SGR在GRASP-Bench上提升性能，同时保持相关社交视频问答基准的零样本性能。

Conclusion: 通过细粒度注视和手势事件监督，可有效改善多人物视频社交推理。
...
TL;DR: 提出FSCM，一种基于光谱信息引导的GAN框架，用于热红外图像彩色化，通过频率增强的空间-光谱状态空间生成器和在线语义分割损失，在视觉质量和语义保真度上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 热红外成像缺乏自然颜色和精细纹理，现有红外彩色化方法多依赖单波段图像，光谱线索不足导致结构扭曲和语义混乱；尽管高光谱图像提供丰富信息，但现有框架难以建模空间-光谱耦合和弱纹理细节。

Method: FSCM包括频率增强的空间-光谱状态空间生成器（由级联FSB单元组成），每个FSB集成状态空间建模、频率增强模块（多级小波分解和傅里叶门控）和双流混合门控模块（变形感知采样与稀疏注意力）；并引入在线语义分割引导损失。

Result: 实验表明FSCM在视觉质量和语义保真度上优于现有红外彩色化方法。

Conclusion: FSCM有效解决了红外图像彩色化中的结构失真和语义混淆问题，提升了复杂道路场景下的语义一致性。
...
TL;DR: 提出一个概率框架，从卫星图像中对野火烟雾进行轻、中、重三级分类，并估计认知和偶然不确定性，在16298个图像块上达到93.8%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法仅做二元烟雾检测，缺乏预测置信度评估，无法支持快速准确的烟雾严重程度评估。

Method: 采用预训练EfficientNet-B3和CBAM模块作为骨干，结合证据深度学习头预测狄利克雷浓度参数，单次前向传播直接分解认知和偶然不确定性，无需蒙特卡洛采样。

Result: 在16298个真实卫星图像块上，加权测试准确率93.8%，未加权91.1%，ECE=0.0274；保留最确定50%样本时准确率96.7%；认知不确定性随图像质量下降单调增加，中等烟雾类认知不确定性最高（平均空值0.187）。

Conclusion: 该框架能有效提供不确定性估计，认知不确定性可作为扫描质量指标，模型正确识别模糊边界区域，分类性能优异。
...
<details>
  <summary>Details</summary>
Motivation: 自然图像是连续的，但生成模型多在离散网格上合成，限制了分辨率灵活性；先前方法仅在解码阶段引入连续性，潜空间仍离散且面向重建。

Method: RaPD在神经图像场潜空间进行扩散，使用语义表示引导进行生成感知的潜学习，以及坐标查询注意力渲染器进行坐标条件缩放感知渲染；仅改变查询坐标即可任意分辨率渲染，扩散成本固定。

Result: 实验表明生成质量和分辨率可扩展性优越。

Conclusion: RaPD实现了分辨率无关的高质量图像生成。
...
TL;DR: 提出分解式视觉-语言对齐框架，通过将文本提示分解为概念令牌和属性令牌，并引入特征门控交叉注意力模块，在特征和评分层面分别处理语义单元，显著提升了对未见过的属性-类别组合的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 开放词汇分割模型难以泛化到未见过的物体类别与属性组合，因为细粒度描述通常被编码为整体句子，纠缠了多个语义单元。

Method: 显式将文本提示分解为概念令牌和多个属性令牌，实现每个语义单元的独立跨模态交互；在特征层面引入特征门控交叉注意力模块，生成属性特定门控图以乘法方式融合信息；在评分层面，逐令牌相似度在对数空间中聚合，产生稳定可解释的组合匹配。

Result: 显著提升了对未见过的属性-类别组合的泛化性能，在细粒度开放词汇分割基准上表现优异。

Conclusion: 该方法可无缝集成到现有基于Transformer的分割架构中，有效增强组合语义的泛化能力。
...
TL;DR: 提出一种结合SAM-HQ和先验引导标签重分配的3D高斯分割框架，实现实时、高保真的3D场景编辑。


<details>
  <summary>Details</summary>
Motivation: 3D-GS缺乏鲁棒分割能力，现有2D分割提升方法存在视图不一致和掩模粗糙问题。

Method: 利用SAM-HQ生成精确2D掩模，通过先验引导标签重分配将标签赋予3D高斯，强制多视图一致性。

Result: 达到最先进分割精度，支持交互式实时编辑，保持高视觉保真度，边界保留优于之前方法。

Conclusion: 在VR和机器人领域有实际应用价值，推进3D场景编辑。
...
TL;DR: 集成三个预训练CNN模型，在PlantVillage数据集上达到99.23%准确率，错误率降低三分之二。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国植物病害检测依赖人工，效率低，需自动化方案。

Method: 采用ResNet50、EfficientNet-B0和DenseNet121的集成学习，迁移学习冻结骨干网络，仅训练头部10个epoch，平均softmax输出。

Result: 集成模型测试准确率99.23%，单个模型96-97%；辣椒和马铃薯分类完美，番茄99.01%；GPU上53 FPS。

Conclusion: 集成模型显著提升准确性，但移动端实时应用需进一步TensorFlow Lite优化。
...
TL;DR: 提出VideoSeeker，通过视觉提示实现实例级视频理解，结合代理推理和主动感知，构建四阶段数据合成管道，在实例级任务上平均提升13.7%。


<details>
  <summary>Details</summary>
Motivation: 现有LVLMs在需要精确时空定位的实例级任务中表现不佳，文本提示难以提供精确时空参考，且模型缺乏主动感知视觉证据的能力。

Method: VideoSeeker利用视觉提示，集成代理推理与实例级视频理解，通过冷启动监督和强化学习内化工具调用和主动感知能力，并构建四阶段自动数据合成管道。

Result: 在实例级视频理解任务上平均提升13.7%，超越GPT-4o和Gemini-2.5-Pro，并在通用视频理解基准上表现良好。

Conclusion: 提出的方法有效提升了实例级视频理解能力，且具有可迁移性。
...
TL;DR: 提出GenShield统一自回归框架，联合进行可解释的AI生成图像检测与可控伪影修正，并构建了高质量数据集和评估流程。


<details>
  <summary>Details</summary>
Motivation: 现有AIGI检测方法进步显著，但如何修正检测出的伪影并恢复真实外观尚待探索，且缺乏检测与修正之间的连接。

Method: 采用统一自回归框架，结合Visual Chain-of-Thought的课程学习策略，实现闭环的“诊断-修复”修正，并配有显式停止准则。

Result: 在修正基准和主流AIGI检测基准上达到最先进性能，泛化能力强。

Conclusion: 检测与修正可相互促进，所提方法有效提升了图像的逼真度和可解释性。
...
TL;DR: 提出一种基于二阶优化的多模态训练框架ML-FOP-SOAP，通过多级方差校正和Fisher正交投影抑制模态冲突，在图像生成与文本理解任务中实现稳定的大批量训练，样本效率提升1.4倍，训练速度提升1.5倍。


<details>
  <summary>Details</summary>
Motivation: 自回归下一标记训练在图像生成与文本理解中导致模态竞争，一阶优化器（如AdamW）因梯度异质性难以稳定优化，且无法支持大批量扩展。

Method: 采用二阶优化器SOAP作为基础，引入多级方差校正（ML-FOP-SOAP），通过Fisher正交投影抑制模态冲突，并设计分层折叠策略以低微步开销捕捉细粒度方差。

Result: 在Janus和Emu3模型上，批量大小8192下训练稳定，与AdamW相比样本效率提升1.4倍，训练时间加速1.5倍。

Conclusion: ML-FOP-SOAP是一种稳健的多模态优化器，有效缓解模态竞争，适用于大规模多模态基础模型的扩展训练。
...
TL;DR: 提出VLA-AD蒸馏框架，利用视觉语言模型作为离线语义监督，将大型VLA教师模型压缩为轻量学生策略，大幅降低模型大小和推理延迟，同时保持接近的性能。


<details>
  <summary>Details</summary>
Motivation: 现有十亿参数级VLA策略在机器人操作中表现出色，但规模和推理成本阻碍了实时闭环控制的应用。

Method: 使用视觉语言模型作为离线语义监督，在训练时为学生策略提供任务阶段锚点、多帧操作方向描述等高层语义信号，替代传统的低级动作模仿；测试时仅需学生策略独立运行。

Result: 以OpenVLA-7B为教师，得到158M参数的学生模型（44倍缩减），平均相对差距仅0.27%，推理速度达12.5 Hz（3.28倍加速）；在π0.5-4B教师上也取得学生超越或接近教师的效果。

Conclusion: 离线语义引导显著提升了VLA策略蒸馏的效率、鲁棒性和可部署性，使学生策略对噪声教师动作更不敏感。
...
TL;DR: 提出IVGT，一种隐式视觉几何变换器，从无姿态多视图图像中隐式建模连续且连贯的3D几何，支持任意视角的渲染和几何提取。


<details>
  <summary>Details</summary>
Motivation: 现有视觉几何基础模型通过回归像素对齐点图来预测显式几何，存在冗余和几何不连续问题。

Method: 学习一个标准坐标系下的连续神经场景表示，支持任意3D位置的连续空间查询，通过轻量解码器预测符号距离函数（SDF）值和颜色。

Result: 能够直接提取连续且连贯的表面几何，从任意视角渲染RGB图像、深度图和表面法线图。在多数据集联合优化下，IVGT在网格和点云重建、新视图合成、深度和法线估计以及相机位姿估计等任务上表现出强泛化性能。

Conclusion: IVGT通过隐式连续表示有效克服了显式方法的冗余和几何不连续问题，并在多个下游任务中展现了优越的泛化能力。
...
TL;DR: 提出多模态程序合成框架MultiMat，利用大语言模型同时处理视觉和文本表示，生成程序化材质节点图，在无条件/条件合成中实现更优的视觉质量和效率，达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅将节点图表示为文本程序，未能捕捉其视觉空间特性，导致创建困难且需要专业训练。

Method: 使用多模态模型处理视觉和文本图表示，结合约束树搜索推理算法确保静态正确性并高效搜索，基于生产质量材质数据集训练。

Result: 在无条件/条件图合成中更高效，生成的材质视觉质量和保真度高于纯文本基线，达到新SOTA。

Conclusion: 多模态程序合成方法有效提升程序化材质图生成性能，为未来研究提供新方向。
...
TL;DR: 通过图像交换探测框架发现，视觉-语言模型在推理时产生的自反语句并未真正触发视觉重检，而仅仅是学习到的文本模式；思考模型比指示模型更易受欺骗，且缩放模型规模无缓解效果。


<details>
  <summary>Details</summary>
Motivation: 探究视觉-语言模型推理时产生的自反语句（如“让我再检查一下图片”）是否真正触发了视觉重检，还是仅为学得的文本模式。

Method: 提出VisualSwap图像交换探测框架：在模型基于一张图像推理后，替换为视觉相似但语义不同的图像，测试模型是否察觉变化。构建VS-Bench数据集，包含800个图像对，源自MathVista、MathVerse、MathVision和MMMU-Pro。

Result: 模型普遍未能察觉图像交换，准确率下降高达60%；思考模型比指示模型脆弱近3倍，缩放模型规模无改善；多轮用户指令能恢复视觉基础，但模型自生成的自反语句无效。

Conclusion: 当前视觉-语言模型在声称进行视觉重检时，更多是“说”而非真正“看”；自反语句并未触发实质性的视觉注意，而用户指令能提升对视觉标记的注意力。
...
TL;DR: 提出了 Minerva-Ego 基准，用于评估复杂的自我中心视觉推理，包含多步多模态问题和人工标注的推理轨迹，发现当前模型与人类仍有较大差距，通过提示“在哪里”和“何时”看可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理基准只评估输出结果，不评估中间推理步骤，且多数仅限文本领域。

Method: 扩展高质量的自我中心/具身视频数据，添加多步多模态问题和时空密集的人工标注推理轨迹，并标注每个轨迹中所需的目标对象的时空掩码。

Result: 最先进模型与人类性能存在大差距；提示模型“在哪里”和“何时”看能显著提升性能。

Conclusion: Minerva-Ego 基准可用于详细分析模型推理过程，并揭示了空间和时间线索的重要性。
...
TL;DR: 提出特征空间采样替代几何稠密采样，在保持等变性的同时大幅降低群卷积神经网络的训练和推理成本。


<details>
  <summary>Details</summary>
Motivation: 群卷积神经网络（GCNN）通过在密集变换群上采样实现等变性，但计算成本随自由度指数增长，阻碍实际应用。

Method: 在特征空间中采样，通过特征相似性选择代表性样本，替代几何稠密采样，从而解耦几何分辨率与计算成本。

Result: 粗特征空间采样能很好保持分类精度，基于几何相似性的预计算可显著加速等变3D分类器的训练。

Conclusion: 特征空间采样提供了新颖的计算精度权衡方式，尤其适用于高自由度等变网络。
...
TL;DR: MoE网络在视觉分类中需要路由大比例FLOPs和多专家路由才能实现正准确率差距；批量轴调度是CNN每样本设置中的主要失败模式，每样本Soft MoE可缓解。


<details>
  <summary>Details</summary>
Motivation: MoE网络在视觉部署中面临专家崩溃和效率提升有限的问题，需要理解稀疏top-k路由有效性的条件。

Method: 在多基准（CIFAR-10/100, Tiny-ImageNet, ImageNet-1K）上多种子评估，进行隐藏大小扫描和仅改变top-k的消融实验，并引入每样本Soft MoE变体。

Result: 正准确率差距要求路由大比例FLOPs（ρ）且多专家路由（k≥2）；隐藏大小扫描显示符号反转不依赖骨干网络；仅改变top-k消融实验逆转差距；每样本Soft MoE在CIFAR-100上超越密集基线。

Conclusion: 批量轴调度是CNN每样本设置中的主要失败模式，通过每样本Soft MoE可有效缓解。
...
TL;DR: 提出COVER方法，将3D资产转换为稀疏全景RGB-D-姿态数据，构建CM-EVS数据集，实现低冗余、高覆盖和可审计的3D学习资源。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉学习方法依赖于密集采样、异构注释或启发式稀疏采样，导致冗余、不一致或覆盖不全，缺乏稀疏、可比、几何一致的全景训练接口。

Method: COVER（面向覆盖的视点筛选与ERP范围深度扭曲）是一种免训练方法，通过贪心覆盖代理从候选视点中筛选且仅添加增量覆盖最大且深度冲突最小的视点，确保近似覆盖性能。

Result: 构建CM-EVS数据集，包含36,373个全景帧（来自1,275个室内场景），中位数仅25帧/场景即可覆盖所有13种统一房间类型，且覆盖-冲突权衡优于基线。

Conclusion: COVER可将3D资产高效转化为稀疏、紧凑、可审计的全景RGB-D-姿态资源，支持几何一致的3D学习。
...
TL;DR: 提出NeRP，一种即插即用的提示校正策略，在不修改模型参数的情况下提升视觉语言模型在新类别上的判别能力，同时保持基类性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在迁移学习中常出现基类-新类权衡：提升新类识别精度会降低基类性能。现有工作简单归因于过拟合，但观察发现模型存在非对称混淆偏误，新类偏误难以通过交叉熵损失消除。

Method: NeRP利用中性文本提示和参考图像测量类间先验偏好，结合样本似然得到代理分数；当先验强烈主导当前预测而证据不足时，在易混淆类对间进行局部翻转，纠正先验主导的误判。

Result: 在多个骨干网络、15个少样本和跨领域基准上，NeRP显著提升新类准确率，同时保持基类预测性能。

Conclusion: NeRP有效缓解基类-新类权衡，通过校正先验偏误提升新类泛化，不修改模型参数，即插即用。
...
TL;DR: 提出Semi-MedRef，一种专为医学半监督指代分割设计的教师-学生框架，通过三个对齐保持模块有效利用未标注数据。


<details>
  <summary>Details</summary>
Motivation: 医学指代分割标注成本高，现有半监督方法在强数据增强下难以保持跨模态对齐，且CutMix在多模态中未被充分利用。

Method: Semi-MedRef包含T-PatchMix（同步混合图像块与指表达）、PosAug（位置感知文本增强）和ITCL（位置引导图像-文本对比学习）。

Result: 在QaTa-COV19和MosMedData+数据集上，所有标签比例下均优于全监督和半监督基线。

Conclusion: Semi-MedRef通过保持跨模态一致性，有效缓解了低标签率下的标注瓶颈。
...
TL;DR: 针对组基优化框架中组内方差快速衰减导致学习信号消失的问题，提出嵌入扰动探索偏好优化（E²PO）方法，通过嵌入级结构化扰动维持方差，实验表明优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有组基优化框架（如GRPO）面临组内方差快速衰减的局限性，样本间差异性减小导致方差趋近于零，优化信号消失，引发训练不稳定或奖励破解。

Method: 提出E²PO框架，在样本组内引入嵌入级结构化扰动，保证稳健的方差，从而在训练过程中保留判别性信号。

Result: 大量实验表明，E²PO显著优于现有基线，实现了更忠实的人类偏好对齐。

Conclusion: E²PO通过嵌入级扰动有效解决了组内方差衰减问题，提升了模型与人类偏好对齐的稳定性与效果。
...
TL;DR: 提出OP-Mix，一种统一语言模型训练全生命周期的数据混合算法，通过在线决策和低秩适配器模拟混合，显著提升性能并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有数据混合方法仅针对单一训练阶段，缺乏统一解决方案，持续学习缺乏原则性指导。

Method: 将数据混合视为在线决策问题，通过插值当前模型上的低秩适配器廉价模拟候选混合，无需额外代理模型。

Result: 在预训练中平均困惑度提升6.3%；持续学习匹配再训练和在线蒸馏性能，计算量分别减少66%和95%。

Conclusion: 语言模型训练应被视为连续学习过程，而非不同阶段的序列。
...
TL;DR: 文学翻译中，流畅性与忠实性存在负相关，段落长度影响自动评估。


<details>
  <summary>Details</summary>
Motivation: 探究文学翻译中目标语言流畅性是否对应语义保留，以及两者关系。

Method: 分析130,486段翻译，来自106部小说16种源语言，使用翻译ese分类器测流畅性（词性n-gram），COMET-KIWI测忠实性，控制段落长度。

Result: 流畅性和忠实性普遍负相关，人类和Google Translate模式明显，TranslateGemma较弱且常不显著。

Conclusion: 段落长度影响评估，文学翻译存在流畅性与忠实性权衡。
...
TL;DR: 提出了一个开源网络界面DiscoExplorer，用于探索和比较跨语言的篇章关系数据集，支持16种语言。


<details>
  <summary>Details</summary>
Motivation: 篇章关系（如因果、让步）跨语言研究困难，现有数据集复杂且缺乏易用接口。

Method: 开发了DiscoExplorer，一个开源、可本地运行的网络界面，集成了DISRPT共享任务中的篇章关系数据集。

Result: 覆盖16种语言，提供查询语言、搜索和可视化功能，支持关系及其信号装置（如连接词）的分析。

Conclusion: 该工具简化了跨语言篇章关系的研究，促进了数据集的可用性和比较分析。
...
TL;DR: 从1亿份乌克兰法院判决中提取5亿条引用边，发现引用结构可无监督编码法律领域边界，并以近乎完美的准确率预测未来立法重要性。


<details>
  <summary>Details</summary>
Motivation: 构建首个大规模乌克兰法院引用图，探索引用结构如何反映法律领域并预测立法重要性。

Method: 从EDRSR注册表（9950万全文，1.1TB）提取5.02亿条引用链接，使用正则表达式在普通硬件上约5小时完成，精度1.00。分析度分布、社区检测和预测模型。

Result: 度分布遵循幂律；Louvain社区检测恢复法律领域边界，模性Q=0.44-0.55；引用特征预测top-1000文章AUC=0.9984；时空动态检测到立法体制变化和2022年入侵引发的引用熵峰值。

Conclusion: 引用衍生的本体可作为法律分析的领域层，与LLM结合；公开了数据和分析代码。
...
TL;DR: 研究在有限词汇（最低250高频词）下人类如何沟通，比较人类与贪婪及全局最优采样算法的行为。


<details>
  <summary>Details</summary>
Motivation: 探究人类在词汇受限时如何优化可理解性，并与计算模型对比。

Method: 让人类在不同词汇限制下回答问题，使用基于大语言模型的序列蒙特卡洛推断实现贪婪和全局最优采样算法。

Result: 人类更接近贪婪采样，但更熟练者会回溯修订（非贪婪行为）；高限制下人类依赖语义轻词。

Conclusion: 结果对资源理性认知、心理语言学、二语沟通和语言障碍有广泛意义。
...
TL;DR: Adesua是一个基于WhatsApp的人工智能教学助手，为西非初中和高中学生提供科学教育支持，集成了教科书和历年考题，通过对话问答和自动评估提供个性化学习支持。


<details>
  <summary>Details</summary>
Motivation: 撒哈拉以南非洲地区师生比例高且缺乏合格教师，限制了学生获得个性化学习支持和形成性评估的机会。

Method: 利用WhatsApp的广泛普及，整合课程教科书和33年国家考试题目，结合生成式AI，通过WhatsApp机器人实现对话式问答和自动评估，支持按主题或年份的定时/不定时测试，并提供即时评分和详细解释。

Result: 2025年在加纳进行了6个月可行性部署，有56名活跃用户（学生和家长）。定量评估显示AI生成的答案有用性得分为93.75%，但评分数量较少（n=16）。

Conclusion: 初步结果为未来更大规模评估提供了基础，以评估基于WhatsApp的AI助手在资源受限的教育环境中提供可扩展、低成本个性化学习支持和形成性评估的潜力。
...
TL;DR: 开发了Eskwai for Students，一个基于检索增强生成的AI助手，用于帮助加纳法学生，包含12K+判例和1.4K立法数据，经过2.5年研究，3100名学生使用，32000次查询。


<details>
  <summary>Details</summary>
Motivation: 全球南方国家法律教育中缺乏生成式AI系统，现有研究有限。

Method: 构建了检索增强生成（RAG）系统，基于12K+判例和1.4K立法数据库，部署后进行了30个月的纵向研究。

Result: 3100名法学生使用，进行了32K次查询，评估了AI的有用性，并揭示了学生提交的查询类型，引发伦理关注。

Conclusion: 该研究贡献了对全球南方法学生如何使用生成式AI的理解，并指出了负责任地推进法律教育的方式。
...
TL;DR: 提出能力条件支架框架，根据用户能力画像调节LLM干预行为，防止专业领域漂移。


<details>
  <summary>Details</summary>
Motivation: 现有LLM个性化未考虑用户不同领域的评估能力差异，导致用户在弱势领域过度依赖AI。

Method: 将专业能力划分为强、混合、弱三个领域，基于结构化的能力画像调节干预行为。

Result: 在多个MMLU子集和四个LLM基座上，实验显示一致的能力条件干预行为，包括画像翻转时的类别反转和混合风险区域的激活。

Conclusion: 能力感知支架可支持超越风格个性化的更可靠的人类-AI专业协作。
...
TL;DR: 本文分析了六种大型语言模型在十二类认知任务上的神经激活模式，发现数学推理产生最高注意力熵，解码器模型比编码器模型具有更高的稀疏性。


<details>
  <summary>Details</summary>
Motivation: 探究不同架构的语言模型在处理认知任务时的神经激活差异，为模型选择和优化提供依据。

Method: 系统测量了六种LLM架构在十二类认知任务上的最终激活值、注意力熵和稀疏性模式。

Result: 数学推理在所有架构中注意力熵最高；解码器模型稀疏性显著高于编码器模型。

Conclusion: 揭示了现代语言模型的计算特征及其任务特异性神经行为，对大数据应用中的模型选择和优化有重要意义。
...
TL;DR: 本文通过变分RNNG波束搜索中的并行句法分析数量，检验语言模型与人类在计算约束上的差异能否解释模型惊讶度对人类花园路径句阅读时间的低估。


<details>
  <summary>Details</summary>
Motivation: 语言模型惊讶度可以预测自然文本阅读时间，但系统性地低估了控制实验中花园路径句的困难程度，可能源于人类与模型的计算约束差异。

Method: 使用递归神经网络文法(RNNG)和词同步波束搜索，系统改变计算词惊讶度时使用的并行句法分析数量，并用这些惊讶度预测人类阅读时间。

Result: 减少并行的活跃句法分析数量确实增加了预测的花园路径效应幅度，但远不足以完全捕捉人类效应的大小。

Conclusion: 这表明语言模型与人类在可用并行句法分析数量上的差异无法调和基于LM的惊讶度与人类句子处理中的差异。
...
TL;DR: 推理训练的语言模型在生成思维链时，隐藏状态轨迹几何受生成长度影响，矫正长度后困难度与轨迹几何仍系统性关联，尤其在编程领域表现明显，但数学和布尔可满足性领域较弱。提示阶段的线性探针无法反映编程域的分离，行为注释表明更强耦合伴随策略转移和不确定性监控。


<details>
  <summary>Details</summary>
Motivation: 区分推理模型在困难问题上增加计算步数还是采用不同内部轨迹，需矫正长度对轨迹统计的影响。

Method: 通过隐藏状态轨迹分析，在编程、数学和布尔可满足性任务中进行长度矫正，比较推理训练模型与指令微调基线模型。

Result: 长度矫正后，困难度与轨迹几何在编程领域有最强耦合，推理模型轨迹更直接、局部曲率更均匀；数学和布尔可满足性领域耦合较弱。提示阶段线性探针无编程域分离，行为注释显示更强耦合伴随策略转变和不确定性。

Conclusion: 长度矫正是生成时轨迹分析的前提，推理训练可导致不同的矫正轨迹几何，效果依赖于领域。
...
TL;DR: 提出模块化检索增强生成（RAG）流水线，用于从护理人员-患者对话记录中提取结构化观测结果，最佳配置在MEDIQA-SYNUR任务上获得80.36% F1分数。


<details>
  <summary>Details</summary>
Motivation: 护理人员-患者对话中包含可操作的观察信息，但将大量对话记录自动转换为结构化表示面临挑战，且临床文档负担过重影响直接患者护理。

Method: 采用模块化RAG流水线，包括使用训练集作为示例语料、模式约束提示（完整模式与剪枝候选模式）、确定性模式后处理及二次审核，并使用Llama-4-Scout-17B-16E-Instruct和GPT-5.2两种大语言模型及对应嵌入模型。

Result: 最佳配置（GPT-5.2 + 完整模式 + RAG + 二次审核）达到80.36% F1分数；RAG一致提升性能，最优模式约束程度依赖模型，二次审核带来微小额外改进。

Conclusion: RAG可稳定提升性能，模式约束策略需根据模型调整，二次审核能修正残留模式遵循错误，但增益有限。
...
TL;DR: 提出了FINESSE-Bench基准套件，包含8个专业基准和3993个问题，用于分层评估LLM的金融能力。


<details>
  <summary>Details</summary>
Motivation: 现有金融基准缺乏专业难度层次，难以评估从基础知识到专家级推理的过渡。

Method: 构建由认证考试、交易任务和俄语奥赛问题组成的分层基准，采用统一评估协议和LLM-as-judge自动评分。

Result: 能够评估领域广度、难度性能下降、计算能力和专业领域行为。

Conclusion: 作为现有基准的补充，用于实质性评估LLM的专业金融能力。
...
TL;DR: 本文指出基于RoPE的Transformer长上下文语言模型存在固有限制：随着上下文增长，注意力失去局部偏置和令牌相关性一致性，失败概率接近0.5；调整基超参数无法同时保持区别位置和令牌。


<details>
  <summary>Details</summary>
Motivation: 识别RoPE在长上下文语言模型中的固有限制，解释其失效原因。

Method: 理论分析抽象上下文内容，仅依赖长度；实证分析多头多层架构。

Result: 证明注意力变得不可预测，局部偏置和一致性丧失，分数对位置或令牌变化不敏感；基超参数权衡位置与令牌区分能力，但无法兼得。
...
TL;DR: 提出多语言基准DetectRL-X，全面评估LLM生成内容检测器在8种语言、6个领域、4种商业模型及AI辅助写作场景下的表现，揭示当前检测器的优势与局限。


<details>
  <summary>Details</summary>
Motivation: LLM被滥用的风险增加，现有检测器在多语言真实场景中的可靠性和潜力尚未充分探索。

Method: 构建包含8种商业常用语言和6个易被滥用领域的基准；使用4种商业LLM生成文本，并纳入润色、扩展、压缩等典型AI辅助写作操作；开发多语言释义与扰动攻击框架以模拟人类修改和噪声。

Result: 实验展示了当前最先进检测器在不同语言资源上的优势与局限；分析了领域、生成器、攻击策略、文本长度、精炼操作对不同语言性能的影响。

Conclusion: DetectRL-X是一个有效的基准，可用于加强多语言和特定语言的检测器。
...
TL;DR: 提出BetaPRM，一种分布式过程奖励模型，同时预测步骤成功率及其可靠性，并基于此设计自适应计算分配（ACA）以优化推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有过程奖励模型只输出单一奖励分数，无法指示预测的可信度，导致下游方法无法区分可靠与不确定的奖励。

Method: 通过Beta-Binomial似然学习步骤成功概率的Beta分布，而非直接回归有限样本成功率。

Result: 在四个骨干网络和四个推理基准上，BetaPRM改进了PRM引导的Best-of-N选择，ACA在保持准确性的同时最多减少33.57%的token使用。

Conclusion: BetaPRM能有效指示步骤奖励的可信度，使下游任务（如自适应计算分配）能更高效地利用计算资源。
...
TL;DR: 连续扩散和流模型在非自回归文本生成中面临潜在状态与离散token的接口问题，提出草稿条件潜在精炼模型，实验表明潜在几何本身不足，需重视解码器可恢复性。


<details>
  <summary>Details</summary>
Motivation: 连续扩散和流模型可并行更新所有位置，但连续潜在状态与离散token的接口是主要难点，现有方法解码效果不佳。

Method: 构建冻结BERT编码器、并行解码器、去噪DraftPrior、局部FlowNet和可学习对角MetricNet的草稿条件潜在精炼模型。

Result: 768维BERT潜在向量在ROCStories上恢复token较好；DraftPrior目标token概率随token丢弃率下降；局部流精炼和融合解码器读出带来小幅提升，而度量学习和OT对齐未弥合解码器差距。

Conclusion: 潜在几何不足以保证解码质量，连续潜在文本生成应评估解码器可恢复性、起始分布质量及精炼是否保持解码器可读结构。
...
TL;DR: 提出GiLT模型，利用依存图增强Transformer语言模型，通过调节注意力权重注入结构信息，不额外添加结构标记，在句法泛化上优于基线模型，保持困惑度竞争力，并可微调提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 先前工作主要使用句法树（如成分树）增强Transformer，但依赖图可能提供更丰富的结构信息，且已有方法通常插入额外结构标记，影响语言建模。

Method: GiLT在Transformer中根据逐步构建的依存图提取特征，动态调节注意力权重，从而将结构信息注入语言建模，而不插入额外的结构标记。

Result: 在句法泛化上优于基线Transformer语言模型，同时保持相近的困惑度；且可从预训练模型微调，提升下游任务性能。

Conclusion: 使用依存图增强Transformer是一种有效且灵活的方式，可在不牺牲语言建模性能的前提下改善句法泛化，并支持迁移学习。
...
TL;DR: 本文系统测量了现代开放LLM中激活值的最大值，发现其跨越多个数量级，并受模型家族、架构和训练阶段影响，而非简单的规模函数。


<details>
  <summary>Details</summary>
Motivation: 激活值的动态范围是低比特量化、激活缩放和稳定推理的首要约束。已有研究基于2024年前的LLaMA模型，但未针对后LLaMA时代的开放模型更新。因此，需要回答部署导向的问题：现代开放LLM中激活值能有多大？以及不同模型家族、代际和训练阶段的变化。

Method: 使用统一流程（5000样本多领域语料、家族特定分词、嵌入层、隐藏状态、注意力层、MLP/MoE、SwiGLU门控、最终范数等的相同钩子），测量了8个开放家族、27个检查点的全局和逐层最大值，涵盖密集、MoE、视觉-语言、中间训练和指令调优变体。

Result: (i) 在相似参数量下，全局最大值跨越近四个数量级，Qwen3.5和MoE检查点在10^2-10^3范围，Gemma3-27B-it达到约7×10^5；(ii) 跨家族和跨代际比较打破了简单的单调缩放；(iii) MoE检查点的峰值比同等规模的密集模型低14.0-23.4倍，而残差流在22/24检查点携带全局最大值。INT-8轻量检查显示测量最大值与低比特重建误差通过激活尺度选择共变。

Conclusion: 最大激活幅度是模型属性，与家族、架构和训练阶段相关，而非简单的规模副产品。在低比特部署前，应测量并报告该指标，与开放权重发布一起。提供代码链接。
...
TL;DR: 提出Nexa，一种可训练的响应条件策略，混合并行与顺序执行模式，通过预测稀疏有向无环通信图来平衡延迟与准确性。


<details>
  <summary>Details</summary>
Motivation: 现有并行模式独立响应后聚合，顺序模式逐步细化，但均无法同时最小化通信延迟和最大化最终响应准确性。

Method: Nexa先并行执行，将响应嵌入共享语义空间，预测稀疏有向无环通信图；图为空则纯并行，非空则进行一次顺序消息传播。策略为轻量级Transformer，无需外部LLM评判或奖励模型，通过策略梯度优化训练。

Result: 学习到的策略在代理数量、任务或底层代理变化时可复用，展示了通信策略的泛化性。

Conclusion: 该混合执行框架严格优于纯并行模式，且训练策略具有跨场景迁移能力。
...
TL;DR: 提出校准语义奖励(CSR)框架，直接在语义空间校准语言模型，无需口头置信度界面，显著降低ECE并提高AUROC。


<details>
  <summary>Details</summary>
Motivation: 标准强化学习使用二元正确性奖励，不惩罚自信但错误的预测，导致校准差；口头置信度校准在词级，语义相同但表述不同时不一致。

Method: CSR结合正确性奖励和新的语义校准奖励，鼓励正确滚动间的语义一致性（利用），阻止错误滚动间的虚假一致性（探索）。

Result: 在HotpotQA、TriviaQA、MSMARCO和NQ-Open上，CSR几乎在所有设置中均比口头置信度基线实现更低的ECE和更高的AUROC，ECE降低40%，AUROC提升31%。

Conclusion: CSR有效提升模型校准性能，且泛化能力强。
...
TL;DR: 提出了一个基于知识图谱的心理健康基准MHGraphBench，用于评估大语言模型在实体识别、关系判断和两跳推理上的能力，发现存在识别-判断差距，且输出格式可靠性影响性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在心理健康领域应用广泛，但对其捕捉生物医学知识及其在临床结构化判断中的可靠性尚不明确。

Method: 基于PrimeKG知识图谱构建包含九个任务家族的基准，涵盖实体识别、关系判断和两跳推理，并设计受控的负选项。评估15个开源和闭源大语言模型。

Result: 领先模型在实体类型和关系类型子集上表现接近上限，但在关系预测和两跳推理上仍困难；短知识图谱片段可能提升或降低模型性能；输出格式可靠性显著影响受控多选题下的测量性能。

Conclusion: MHGraphBench应被解释为评估模型与PrimeKG中精选心理健康子集的一致性，而非直接评估真实世界临床安全性。
...
TL;DR: 大型语言模型在代码生成上存在“实现保真度差距”：它们能理解算法但无法用陌生语言正确实现。


<details>
  <summary>Details</summary>
Motivation: 探究LLM能否将代码生成能力迁移至预训练中未出现的编程语言。

Method: 引入新语言PyLang，评估零样本和微调Qwen3模型，并尝试多任务学习、偏好优化、代码填充、隐空间目标等干预。

Result: 微调能快速学习语法但语义能力不迁移；Python性能始终高出PyLang最多19%；所有干预均无效；模型80%时间选择正确算法但无法实现；内部表示高度相似但输出层分化。

Conclusion: 训练方法需解耦算法推理与语言特定实现。
...
TL;DR: 提出并行推测解码（PSD）框架，无需训练即可同时从空间和时间上加速扩散大语言模型的推理，在保持精度的前提下实现高达5.5倍的令牌数提升。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型生成文本的推理成本高，因为需要多次去噪迭代。现有方法分别从空间（每次解掩多个令牌）或时间（合并多个去噪步骤）优化，但无法同时兼顾。

Method: PSD利用单次前向传播的置信度分数，通过可配置的自适应非掩码策略选择位置，构建多深度推测草稿而不额外调用模型，最后通过批量验证应用分层接受。

Result: 在三个dLLM上的推理和代码生成任务中，PSD达到每前向传播5.5倍的令牌数，准确率与贪心解码相当。

Conclusion: PSD在推理效率和生成质量之间取得良好平衡，是一种有效的训练无关加速框架。
...
TL;DR: 研究发现开源大语言模型严重偏向英语；持续预训练在适应目标语言时并无成本优势，甚至对文化理解提升有限；未来需要针对每种语言进行专门投资。


<details>
  <summary>Details</summary>
Motivation: 探究持续预训练是否比从头训练更经济地适应目标语言，以及英语偏差对多语言模型的影响。

Method: 分析开源大语言模型生成的序列，比较持续预训练与从头训练在目标语言适应上的成本和文化理解效果。

Result: 持续预训练在成本和文化理解上并不优于从头训练；模型表现出强烈的英语偏差。

Conclusion: 未来大语言模型发展应重视针对每种语言的专门投资，而非主要依赖英语中心资源。
...
TL;DR: 构建了首个基于潜在歧义理论的中文歧义数据集CHA-Gen，评估发现大模型在歧义检测上表现不佳，但链式思维提示可提升性能，并揭示了三种常见失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有中文歧义数据集（如CHAmbi）可扩展性差，且研究多聚焦英语，缺乏对中文歧义的系统性探索。

Method: 基于潜在歧义（PA）理论设计半自动流水线，构建包含5712个句子（2414歧义、3298无歧义）的CHA-Gen数据集，覆盖18种潜在歧义结构；通过直接查询和机器翻译评估多种LLM。

Result: LLM在歧义检测上表现困难，但链式思维提示可提升性能；Qwen3-32B的CoT推理显示三种失败模式：歧义盲视、归因错误、过早消歧；语义熵不确定性度量显示歧义句子不确定性更高；指令调优导致过度自信，基座模型更好捕捉语义多样性；模型偏向于主导解释。

Conclusion: 提供了可扩展的中文歧义语料构建方法，揭示了LLM处理歧义的特点，为中文歧义研究奠定基础。
...
TL;DR: 提出动态分块扩散模型（DCDM），用内容定义的语义块替代固定位置块，在离散扩散语言模型中实现改进。


<details>
  <summary>Details</summary>
Motivation: 位置块浪费序列结构，将语义相关的token分开而将无关的token分组在一起。

Method: 通过可微分的Chunking Attention将token路由到K个可学习子空间聚类，生成块因果注意力掩码，按语义块自回归分解序列似然。

Result: 在高达1.5B参数的基准测试中，DCDM一致优于无结构和位置块扩散基线，优势在训练早期即显现且跨尺度稳定。

Conclusion: DCDM通过内容分块有效泛化了块离散扩散，取得了持续的性能提升。
...
TL;DR: 提出Diagram-as-Code范式和VCG-Bench基准，解决VLM在可控图表生成中的不足。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在图表任务中缺乏结构化和可控性，像素合成方法可编辑性和保真度差。

Method: 采用基于mxGraph XML的Diagram-as-Code范式，构建包含多样图表的数据集、定义视觉到代码和代码到代码的任务、提出多维评估指标。

Result: 实验表明当前SOTA VLM在结构保真度和指令遵循上表现不佳。

Conclusion: 现有VLM在结构化图表任务中存在局限，需要新的范式。
...
TL;DR: 利用大语言模型在低标注资源下对在线患者问询进行临床分诊，在四分类任务中达到接近监督基线的性能，但尚不能完全自动化。


<details>
  <summary>Details</summary>
Motivation: 在线患者问询非正式且不完整，需要路由到合适临床随访级别，但标注资源有限。

Method: 使用HealthCareMagic-100K构建300例人工标注金标准、700例自动标注银训练集和40例少样本池；比较TF-IDF、BioBERT基线及六个大语言模型在0-shot、4-shot、12-shot条件下的表现，并用macro-F1、急诊召回率、欠分诊率等安全指标评估。

Result: 最强LLM（Claude Haiku 4.5, 12-shot）达到macro-F1 0.475，超过最佳监督基线（BioBERT, 0.378），但置信区间重叠；少样本提示和两模型一致性在不同类别上效果不同。

Conclusion: LLM可辅助分诊优先级排序和选择性人工审核，但尚不能自主部署。
...
TL;DR: 提出ASRU框架，通过激活重定向和奖励优化实现可控的多模态知识遗忘，兼顾遗忘效果与生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有多模态遗忘方法只评估输出偏差，忽略生成质量，导致幻觉或生硬回应。

Method: 先通过激活重定向诱导拒绝行为，再用定制奖励函数优化细粒度拒绝边界。

Result: 在Qwen3-VL上遗忘效果平均提升24.6%，生成质量提升5.8倍，模型效用保持良好。

Conclusion: ASRU仅用少量保留监督数据即可实现遗忘效果与模型效用的更好平衡。
...
TL;DR: H-Mem是一种混合记忆机制，通过树和图结构建模记忆演化并高效检索，显著提升基于LLM的代理在问答任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏对代理记忆随时间演化的有效建模和检索机制，导致记忆利用性能不佳。

Method: 构建时间-语义树结构，使短期记忆逐步演化为提供摘要信息的长期记忆；同时构建知识图谱捕捉实体关系，并利用树与图的混合结构实现高效检索。

Result: 在三个代理记忆基准上，H-Mem在问答任务中达到最先进性能。

Conclusion: H-Mem通过混合结构有效建模记忆演化并实现高效检索，提升了代理的记忆利用和问答能力。
...
TL;DR: 提出源分布多模态记忆基准SMMBench，评估智能体从分散来源组合多模态证据的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准忽视源分布记忆组合，该能力是智能体记忆的重要瓶颈。

Method: 构建SMMBench基准，包含1877个样本和264个来源，评估跨源多模态推理、冲突解决、偏好推理和记忆驱动的动作预测四种能力。

Result: 实验显示现有系统在SMMBench上表现不佳，说明源分布多模态记忆仍是挑战。

Conclusion: 源分布多模态记忆是重要且未充分评估的问题，SMMBench有助于推动该领域研究。
...
TL;DR: 提出神经协同上下文工程（NCCE），将上下文工程从全局搜索转为实例级动态路由，通过协同过滤实现个性化上下文推荐，提升LLM任务精度。


<details>
  <summary>Details</summary>
Motivation: 现有上下文工程方法将优化视为全局搜索，忽略不同输入需要不同指导，无法挖掘实例级性能提升。

Method: 构建锚定上下文目录，采用Context-CF协同进化机制：轻量级NCF模型学习实例-上下文偏好，引导生成特化上下文变体，新上下文反过来优化NCF。推理时NCF作为路由器动态分配最优上下文。

Result: 理论和实验证明，NCCE通过匹配个体输入与最优上下文显著提高任务准确性。

Conclusion: 个性化工件在LLM上下文工程中至关重要，NCCE实现了这一范式转变。
...
TL;DR: 提出DimMem，一种轻量级维度记忆框架，将记忆表示为原子化、带类型、自包含的单元，包含时间、地点、原因、目的和关键词等字段，实现维度感知的检索和更新，在LoCoMo-10和LongMemEval-S上分别达到81.43%和78.20%准确率，token成本降低24%。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统在保真度和效率之间存在权衡，原始对话历史代价高，而扁平事实或摘要会丢失精确回忆所需的结构。

Method: DimMem将每个记忆表示为原子化、带类型、自包含的单元，包含时间、地点、原因、目的和关键词等显式字段，支持维度感知检索、记忆更新和选择性辅助上下文回忆。

Result: 在LoCoMo-10和LongMemEval-S上分别达到81.43%和78.20%整体准确率，优于现有轻量级记忆系统，同时LoCoMo每次查询token成本降低24%。微调后的Qwen3-4B提取器在关键设置上性能优于或媲美更大提取器。

Conclusion: 显式维度结构化是LLM代理长期记忆有效且高效的基础。
...
TL;DR: 小型开源LLM（<30B参数）通过单次提示策略，在机器翻译质量估计中达到与大型专有LLM竞争的性能，同时保护数据隐私。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的QE依赖大型专有LLM，存在数据隐私问题。

Method: 使用小型开源LLM，采用单次提示策略同时生成质量分数、MQM错误注释、修正建议和后编辑。

Result: 系统级与人类判断相关性超过传统神经指标、微调模型和人类注释者一致性。

Conclusion: 小型开源LLM是可行、经济且隐私保护的替代方案。
...
TL;DR: 提出ForMaT数据集，包含15个语言对的3,956个PDF，保留原始布局元数据，用于多模态机器翻译。


<details>
  <summary>Details</summary>
Motivation: 当前机器翻译系统在空间定位和几何同步方面表现不佳，丢失文本与视觉上下文的关联。

Method: 采用K-Medoids采样从45个几何特征中选取视觉多样的PDF文档，构建平行语料库。

Result: 评估表明现有MT系统难以处理空间定位和几何同步。

Conclusion: ForMaT为开发布局感知的翻译模型提供基准，融合视觉和文本上下文以实现高保真文档重建。
...
TL;DR: 本文介绍了一个俄罗斯政府跨模态政治传播数据集，包含数十年间的官方演讲、文本、图像及元数据，并经过主题建模和专家验证。


<details>
  <summary>Details</summary>
Motivation: 解决威权政治背景下社交媒体文本和图像数据匮乏的问题。

Method: 收集克里姆林宫和俄罗斯外交部高级官员的演讲，提供俄英双语文本、相关图像和字幕，统一元数据，并通过基于Transformer的多模态主题建模生成主题注释，经俄政治专家精炼。

Result: 形成了大规模的多模态语料库，支持多语言、时间、空间分析，并为社会科学研究和大型语言模型应用提供测试平台。

Conclusion: 该数据集填补了威权政治传播数据空白，支持多模态、多语种分析，并有助于政治领域的大语言模型应用。
...
TL;DR: 通过构建语义分割数据集和提出块蒸馏框架，解决了块注意力中文本分割困难和微调低效的问题，实现了接近全注意力的性能。


<details>
  <summary>Details</summary>
Motivation: 块注意力在长上下文中可提升KV缓存重用，但面临文本难以分割和现有点微调方法效率低且可能降级性能的挑战。

Method: 构建包含3万多实例、16类别的语义分割数据集SemanticSeg，训练轻量分割器自动分割文本；提出块蒸馏训练框架，包括块汇令牌、块丢弃和令牌级损失加权。

Result: 在多个模型和基准上，分割器优于启发式和统计基线，块蒸馏在块注意力下达到接近全注意力的性能。

Conclusion: 块蒸馏为部署块注意力提供了实用且可扩展的路径。
...
TL;DR: 将GRPO应用于编码器-解码器机器翻译模型（NLLB-200），使用无参考奖励，在13种语言上均获得提升，尤其在低资源语言上效果显著。


<details>
  <summary>Details</summary>
Motivation: 强化学习微调机器翻译主要针对解码器大语言模型，缺乏对编码器-解码器架构的系统研究。

Method: 采用组相对策略优化（GRPO），结合混合无参考奖励（LaBSE和COMET-Kiwi），无需微调时的平行数据。

Result: 在13种语言上一致改进，繁体中文chrF++提升最多达+5.03，无需目标语数据即可与有监督微调在形态复杂语言上竞争。

Conclusion: 该方法在基线性能最弱、奖励区分度最高的场景下增益最大，即平行数据最稀缺时效果最佳，该模式在英语和西班牙语源语言中一致。
...
TL;DR: 提出用符号方法将执法报告中的叙述文本转换为证据关联事实，构建时间图。在450份报告上，54.1%事件置信度≥0.80，语义映射率达93.7%。


<details>
  <summary>Details</summary>
Motivation: 执法报告中的关键信息常隐藏在自然语言叙述中，需要手动阅读，效率低下。

Method: 符号方法：去标识化、语义解析、谓词映射到本体（通过PropBank-VerbNet-WordNet路径）、推理。

Result: 54.1%的事件置信度≥0.80，93.7%语义映射成功；事件发起、被盗物品和时间线索达成100%一致，强制进入解释一致性较低。

Conclusion: 符号方法能有效从叙述中提取结构化事件，但在某些概念上仍需改进。
...
TL;DR: 提出AI文化能力的三层分类法：文化意识、文化敏感度和文化能力，以澄清概念并改进评估。


<details>
  <summary>Details</summary>
Motivation: 当前AI文化能力定义模糊、术语混用，且仅限于记忆信息，缺乏清晰概念。

Method: 借鉴跨文化传播学术，提出文化意识、文化敏感度、文化能力三层分类法。

Result: 该分类法作为实用工具，提高AI评估在多元文化环境中的有效性和可解释性。

Conclusion: 缺乏概念清晰度会导致评估高估模型能力，在不适当部署决策中造成风险。
...
TL;DR: 提出基于学习者模型的评估框架，评估视觉语言模型在数学辅导中的适应性，发现模型在有限信息下难以持续产生适应性教学响应。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型虽用于个性化数学教学，但缺乏对适应不同学习者能力的系统评估框架。

Method: 基于自适应学习框架的学习者模型，从认知、动机和复杂性三个方面评估适应能力，并额外评估正确性和响应质量。

Result: 模型间适应能力存在可测量差异，但在提供有限学习者信息时，模型难以持续生成基于学习者模型的教学响应。

Conclusion: 视觉语言模型在数学辅导中适应性不足，尤其缺乏学习者信息时表现更差。
...
TL;DR: LLM作为评判者时，输出格式改变会导致评分不一致，通过PEAP方法发现存在共享的潜在评估子图，格式不一致源于脆弱且特定格式的终端分支。


<details>
  <summary>Details</summary>
Motivation: 现有诊断仅停留在输入输出层面，未能解释格式不一致的内部机制。

Method: 使用位置感知边归因修补（PEAP）对Gemma-3、Qwen2.5和Llama-3进行因果内部机制分析。

Result: 发现跨任务共享的稀疏潜在评估子图（位于中后期MLP层），零消融会破坏判断但保留世界知识；格式不一致源于连续判断信号通过脆弱的特定格式终端分支映射。
...
TL;DR: 提出S2ST-Omni 2框架，通过结构化类型学先验替代扁平语言标签，实现多语言语音到语音翻译。


<details>
  <summary>Details</summary>
Motivation: 现有S2ST系统忽视源语言结构，将语言表示为独立扁平嵌入，导致监督数据稀缺时多语言适应数据效率低。

Method: 在表示、声学、解码三个层面引入类型学先验：层次语言编码、动态门控语言感知双CTC、类型感知LLM提示。

Result: 在CVSS-C上BLEU、COMET等指标优于基线；消融实验证实各层面互补；低资源日英翻译（约3小时数据）显示类型先验提升数据效率。

Conclusion: 结构化语言先验有效提升多语言S2ST性能和数据效率，尤其在低资源场景下。
...
TL;DR: 提出RecMem，通过延迟内存整合来降低token消耗，同时保持或提升准确性。


<details>
  <summary>Details</summary>
Motivation: 现有内存系统对所有交互都使用LLM进行内存提取，导致token消耗过大。

Method: 使用潜意识内存层存储交互，轻量嵌入模型检索；仅在观察到语义相似交互重复出现时调用LLM提取内存；引入语义细化机制。

Result: 相比三种SOTA内存系统，token成本降低高达87%，准确性超过它们。

Conclusion: RecMem有效降低token成本并提升准确性。
...
TL;DR: 提出基于大语言模型的数据增强框架，利用文本语义锚点生成口语化独白，改善认知评分预测，尤其通过相似性引导选择缓解类别不平衡。


<details>
  <summary>Details</summary>
Motivation: 口语认知衰退评估因数据集小、类别不平衡而准确率不足。

Method: 使用GPT-5基于书面回答生成多样化口语独白，结合Sentence-BERT嵌入和偏最小二乘回归预测认知评分，采用相似性引导的类平衡选择策略。

Result: 相似性引导策略显著降低低分少数群体的预测误差，同时保持多数群体性能，优于随机类平衡选择。

Conclusion: 语义引导的LLM增强是解决类别不平衡、提升临床语音分析数据效率的有效方法。
...
TL;DR: 提出一个多层次上下文令牌关系建模框架，通过局部校准和全局规则推理改进机器生成文本检测，在跨模型和跨领域场景下表现优异。


<details>
  <summary>Details</summary>
Motivation: 机器生成文本存在风险，现有度量方法虽实用但令牌级检测易受生成随机性影响，需要更鲁棒的检测框架。

Method: 构建多跳转移理论分析令牌级检测分数，提出局部马尔可夫校准模块和全局规则推理模块，联合多层级推理。

Result: 在跨大语言模型和跨领域场景下实现广泛且显著的性能提升，计算开销低。

Conclusion: 该框架有效缓解了令牌级检测的偏差，提高了机器生成文本检测的可靠性和泛化能力。
...
TL;DR: 提出SGR框架，通过构建外部知识子图增强LLM的多步推理，显著提升推理准确性和事实可靠性。


<details>
  <summary>Details</summary>
Motivation: LLM在复杂推理任务中易产生无关、噪声或事实不一致内容，需要借助结构化外部知识来引导推理。

Method: SGR先根据输入问题从知识库构建查询特定子图，然后引导模型在子图上逐步推理，并融合多条推理路径以得到最终预测。

Result: 在多个基准数据集上，SGR相比基线方法取得一致改进，推理准确性和事实可靠性均提高。

Conclusion: SGR通过外部子图生成有效增强了LLM的推理能力，在复杂推理任务中具有重要价值。
...
TL;DR: 本研究结合AI编码代理与树搜索算法，自动生成高效三维光伏结构，克服了平板太阳能板在中纬度的限制，并通过消除奖励黑客问题，发现了更优的设计。


<details>
  <summary>Details</summary>
Motivation: 平板太阳能电池板在中纬度地区因角度限制导致能量损失，需要开发高效三维光伏结构以提高日能量产出。

Method: 使用Google AntiGravity编码代理复现基准计算，再结合LLM驱动的树搜索算法ERA进行大规模搜索，通过迭代修补物理引擎消除奖励黑客现象。

Result: 发现了具有不同限制条件的优化三维光伏设计，包括固定收集面积、优化天顶跟踪和避免自遮挡的结构，性能优于初始设计。

Conclusion: 将编码代理与树搜索结合，为可通过评分函数经验评估的科学发现问题提供了强大的自动发现平台。
...
TL;DR: 提出基于价值的角色构建方法，通过抽样价值概况和校准程序，减少LLM跨文化预测误差，尤其改善代表性不足人群的表现。


<details>
  <summary>Details</summary>
Motivation: LLM模拟人类意见时跨文化能力有限，现有基于人口统计等的角色方法不能充分捕捉文化价值观。

Method: 从调查回答中提取文化维度描述符构建价值角色，抽样目标人群价值概况，聚合LLM回答，并引入校准程序提高多样性。

Result: 预测误差降低，代表性不足人群改善最大，缩小了国家间性能差距，输出分布更匹配人类多样性。

Conclusion: 基于价值的角色构建和校准能有效提高LLM跨文化模拟的准确性和多样性。
...
TL;DR: 提出Argus系统，通过搜索者和导航者协作将深度研究任务视为拼图，避免并行搜索中的证据重复，提升效率并降低上下文负担。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究系统并行搜索时产生重复证据，导致收益递减且聚合上下文超出模型限制。

Method: Argus由Searcher和Navigator组成：Searcher用ReAct收集子查询证据；Navigator维护证据图，识别缺失并调度Searcher，用强化学习训练以验证、调度和合成，最终生成有来源的回答。

Result: 基于35B-A3B MoE骨干，单个Searcher提升5.5点，8个并行Searcher提升12.7点（八个基准平均）。64个Searcher在BrowseComp达86.2，超越所有测试的专有代理，Navigator上下文保持21.5K tokens以下。

Conclusion: Argus通过合作避免了重复，提升了效率，且支持任意数量Searcher并行而无需重新训练。
...
TL;DR: 通过模拟失语症对语言模型进行“损伤”分析，发现其症状分布与人类不同，且不同组件和层次有特异性。


<details>
  <summary>Details</summary>
Motivation: 利用失语症研究揭示大脑语言组织的方式，提出一种类似方法探究语言模型的功能组织。

Method: 将模型参数置零（损伤），然后使用文本失语症量表（TAB）评估输出，比较与人类失语症症状的相似性。

Result: 在112,426个输出中，症状分布与人类不同；注意组件和前馈组件症状轮廓不同；早期层导致句法和语义症状，中后层导致音韵和流畅性缺陷。

Conclusion: 失语症症状模式受学习和处理细节影响，而非领域不变的结果；人类与LMs的差异表明这一点。
...
TL;DR: 智能计量、生成式人工智能和量子启发组合优化的融合正在重塑能源公用事业在物理基础设施、客户参与和环境责任方面的管理。


<details>
  <summary>Details</summary>
Motivation: 能源公用事业面临物理基础设施管理、客户参与和环境责任等多重挑战，需要借助新兴技术实现转型。

Method: 通过整合智能计量数据、生成式人工智能和量子启发组合优化算法，构建新的管理方案。

Result: 加速了能源公用事业在基础设施、客户互动和环保方面的能力提升。

Conclusion: 三种技术的协同作用为能源领域提供了创新解决方案，推动了行业变革。
...
TL;DR: 提出了一个端到端框架，统一了生成式AI账单起草、变压器负荷预测等四个生产级能力，以帮助配电公司提供可读账单、碳足迹和负荷调度。


<details>
  <summary>Details</summary>
Motivation: 配电公司需要提供客户可读的账单、为每千瓦时附上可辩护的碳数字，并根据电网压力和排放约束调度负荷。

Method: 构建了一个端到端框架，包含：(1) 生成式AI代理，从结构化数值输入在约束解码策略下起草自然语言账单；(2) 变压器预测器，提供日前消费估计和校准分位数带。

Result: 框架将四个生产级能力统一在单一架构下，实现了账单生成、碳核算和负荷预测的整合。

Conclusion: 该框架为配电公司提供了同时满足可读账单、碳足迹和负荷调度需求的综合解决方案。
...
### [91] [Multicritical Scaling Limit of Shifted Schur Measure](https://arxiv.org/abs/2605.15744)
*Haruna Aida,Taro Kimura*

Main category: math.CO

TL;DR: 研究了移动Schur测度的多临界标度极限，证明了在特定条件下，严格划分的极限形状可确定，且边缘标度极限从Pfaffian点过程转变为行列式分布。


<details>
  <summary>Details</summary>
Motivation: 探索多临界标度极限下的相变行为，理解点过程类型的转变。

Method: 通过适当的标度极限和连续参数条件，确定极限形状；在多临界条件下推导相关函数的标度极限。

Result: 相关函数的边缘标度极限收敛到高阶Airy核的行列式，实现Pfaffian到行列式分布的转变。

Conclusion: 严格证明了在移动Schur测度的多临界标度极限中，点过程从Pfaffian转变为行列式分布。
...
### [92] [paper.json: A Coordination Convention for LLM-Agent-Actionable Papers](https://arxiv.org/abs/2605.16194)
*Arquimedes Canedo*

Main category: cs.DL

TL;DR: 提出paper.json作为PDF的伴侣文件，通过轻量级约定解决LLM代理阅读论文时无法细粒度引用、范围过度延伸和命令隐藏在代码库等问题。


<details>
  <summary>Details</summary>
Motivation: LLM代理常作为论文的第一读者，但标准散文式论文导致子声明无法细粒度引用、范围过度外推、图形命令隐藏在代码库而非论文中。

Method: 定义五个约定：C1稳定声明ID、C2明确不声称列表、C3每个图形的精确shell命令、C5稳定定义ID，以及C4最小可行合规（手写JSON在一小时内完成）。

Result: 论文本身是合规的，运行`uv run validator.py paper.json --against paper.typ`通过验证。

Conclusion: C1、C2、C3、C5是开放邀请，代理读取合规论文并行动可产生支持或反对的证据。
...
### [93] [Coherent States of Non-Null Torus Knots](https://arxiv.org/abs/2605.15420)
*Gabriel Canadas da Silva,Ion Vasile Vancea*

Main category: quant-ph

TL;DR: 针对真空麦克斯韦方程的环面纽结解，构建了电磁场的相干态，并建立了量子-经典对应。


<details>
  <summary>Details</summary>
Motivation: 探索拓扑电磁系统的量子性质，并建立其量子-经典对应关系。

Method: 从经典场与相干态振幅的一般关系推导位移算符，通过直接计算验证相干态性质，并用经典解的整数参数计算可观测量期望值。

Result: 得到场期望值、能量密度、坡印廷矢量、螺旋度、光子数、正交不确定性和关联函数等可观测量在纽结相干态中的期望值，并以Hopfion为例。

Conclusion: 为这类真空拓扑电磁系统的量子-经典对应提供了基础。
...
### [94] [Quantum game theory for 2 2 games: a mathematical framework](https://arxiv.org/abs/2605.15747)
*Gloria Ferraris,Veronica Umanit\`a*

Main category: quant-ph

TL;DR: 为静态2x2博弈的量子博弈论建立了严格数学框架，并证明了纳什均衡的存在。


<details>
  <summary>Details</summary>
Motivation: 将经典博弈论扩展到量子设置，允许玩家使用任意酉操作（纯策略）或连续群SU(2)上的概率测度（混合策略）。

Method: 采用Eisert-Wilkens-Lewenstein协议作为标准实现，通过不动点论证证明纳什均衡存在性。

Result: 证明了连续量子混合策略下纳什均衡的存在性，推广了经典纳什定理。

Conclusion: 为量子博弈论提供了严格的数学基础，并确立了纳什均衡的量子版本。
...
### [95] [Fast convergence of Dynamic Capacities of GNS-Symmetric Quantum Channels](https://arxiv.org/abs/2605.15953)
*Omar Fawzi,Li Gao,Mizanur Rahaman,Mostafa Taheri*

Main category: quant-ph

TL;DR: 对于GNS对称量子信道，给出了经典和量子容量的指数收敛界，并用于比较主动与被动纠错。


<details>
  <summary>Details</summary>
Motivation: 研究量子信道信息容量的时间演化，量化主动与被动纠错性能。

Method: 针对GNS对称信道（如Pauli信道），利用熵性质推导指数收敛界。

Result: 得到了经典和量子容量的显式指数收敛界。

Conclusion: 这些界可用于量化主动与被动纠错设置的性能。
...
### [96] [Diagonal Adaptive Non-local Observables on Quantum Neural Networks](https://arxiv.org/abs/2605.15410)
*Huan-Hsin Tseng,Yan Li,Hsin-Yi Lin,Samuel Yen-Chi Chen*

Main category: quant-ph

TL;DR: 提出对角自适应非局域可观测量（Diagonal ANO），在保持完整ANO能力的同时大幅降低参数和计算成本。


<details>
  <summary>Details</summary>
Motivation: 完整ANO虽能扩大变分量子算法的函数空间，但参数数量和经典优化成本过高。

Method: 采用仅与量子电路配对的对角可观测量，利用对角矩阵作为ANO空间的规范代表。

Result: 在保持相同表达能力的同时，将k-局域可观测量复杂度从O(4^k)降至O(2^k)，降低测量端经典计算开销。

Conclusion: 对角ANO保留了完整ANO的优势，并包含传统变分量子线路作为特例。
...
### [97] [Quantum Feature Pyramid Gating for Seismic Image Segmentation](https://arxiv.org/abs/2605.15370)
*Taha Gharaibeh,Jyotsna Sharma*

Main category: quant-ph

TL;DR: 提出量子特征门控混合分割架构，在编码器-解码器的特征融合点嵌入参数化量子电路，用于地震盐体分割，实验表明量子特征融合可提升分割性能。


<details>
  <summary>Details</summary>
Motivation: 准确描绘盐体对地震解释至关重要，但混合量子-经典模型在像素级地球物理预测中的价值尚未验证。

Method: 在特征金字塔网络融合点引入4量子比特、2层参数化量子电路，通过数据重上传计算侧向和顶层特征的凸组合；全局平均池化将编码器特征映射为固定4维量子输入。

Result: 在EfficientNetV2-L消融实验中，用元素加法替换量子门控使平均IoU从0.9389降至0.8404（下降9.85%）；在U-Net中作为跳跃连接注意力提升IoU 0.88点。

Conclusion: 提供控制性证据表明量子特征融合可改善密集地震分割任务。
...
### [98] [Toward Natural and Companionable Virtual Agents via Cross-Temporal Emotional Modeling](https://arxiv.org/abs/2605.15812)
*Feier Qin,Xiao Li,Yi Zheng,Haibin Huang,Hanyao Wang,Xiaoyu Wang,Yan Lu,Yuan Zhang*

Main category: cs.HC

TL;DR: 提出跨时间情感建模框架CTEM，通过闭环连接历史行为与实时情感表达，提升陪伴型代理的自然性和连贯性。


<details>
  <summary>Details</summary>
Motivation: 现有代理缺乏对社交行为和内部情感的跨时间建模，导致交互短暂且不真实。

Method: CTEM框架建立闭环：过去经验更新情感状态，情感状态影响交互，用户反馈修订记忆和情感，支持反思和预测。

Result: 部署为Auri代理，21天野外研究显示在自然性、连贯性和情感和谐方面有改进。

Conclusion: CTEM能显著提升陪伴型代理的交互品质。
...
### [99] [SLIP & ETHICS: Graduated Intervention for AI Emotional Companions](https://arxiv.org/abs/2605.15915)
*Minseo Kim*

Main category: cs.HC

TL;DR: 提出SLIP和ETHICS框架，通过分级干预策略平衡AI情感陪伴中的安全与信任，初步验证显示能降低误报但存在边界问题，更大模型可改善检测。


<details>
  <summary>Details</summary>
Motivation: AI情感陪伴面临安全与信任的矛盾：严格限制损害支持关系，宽松则可能导致用户伤害。

Method: 设计SLIP四阶段分级干预协议，基于情感强度(a)和叙事动态(m)等定性指标，结合ETHICS信号分类体系，评估采用小规模生产部署和合成人物测试。

Result: 流动人格误报率为0%，危机人格出现预期升级模式；但连续8天高能量状态零干预，暴露原则冲突；更大模型将检测率从0/8提升至6/8，同时保持0/10误报。

Conclusion: 分级干预可作为应对而非解决情感计算中安全与信任张力的设计方向。
...
### [100] [Conversations in Space: Structuring Non-Linear LLM Interactions on a Canvas](https://arxiv.org/abs/2605.15848)
*Rifat Mehreen Amin,Alperen Adatepe,Daniela Fernandes,Daniel Buschek,Andreas Butz*

Main category: cs.HC

TL;DR: CanvasConvo通过将线性聊天转化为分支对话树并嵌入空间画布，支持用户探索假设场景、并行发展替代方向，并保持与熟悉聊天界面的集成。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型驱动的对话界面线性结构限制了备选方案的探索和长期交互的管理。

Method: 设计CanvasConvo概念，实现分支对话树、画布可视化、时间线导航、自动标签与总结、上下文感知控制（如目标、可复用提示）等功能。

Result: 在为期5-7天的实地研究中，24名参与者参与评估，结果表明非线性对话结构支持探索性工作流和不同的交互方式。

Conclusion: 非线性对话结构通过画布和分支机制有效增强了基于LLM的探索性工作。
...
### [101] [Neural Point-Forms](https://arxiv.org/abs/2605.15524)
*Bruno Trentini,Jacob Hume,Vincenzo Antonio Isoldi,Philipp Misof,Ekaterina S. Ivshina,Kelly Maggs*

Main category: cs.LG

TL;DR: 提出神经点形式（NPFs），利用扩散几何的拉普拉斯技术构建可学习的点云特征，通过比较矩阵编码微分形式与切向信息，在多个实验中表现优异。


<details>
  <summary>Details</summary>
Motivation: 点云学习中的几何结构未被坐标、距离或图邻域充分捕获，需要更好的高阶切向特征。

Method: 基于扩散几何的拉普拉斯算子，构建离散模型通过内积比较微分形式，将子流形表示为比较矩阵，并证明其一致收敛性，实现为排列不变的神经层。

Result: 在合成和生物学实验中，NPFs提供有竞争力和可解释的表示，尤其在依赖采样密度、流形结构或种群几何的任务中优势明显。

Conclusion: NPFs是一种有效、紧凑且可解释的点云学习方法，适用于几何敏感的标签预测。
...
### [102] [Entropy Across the Bridge: Conditional-Marginal Discretization for Flow and Schr\"odinger Samplers](https://arxiv.org/abs/2605.16126)
*Bruno Trentini,Dejan Stancevic,Michael M. Bronstein,Alexander Tong,Luca Ambrogioni*

Main category: cs.LG

TL;DR: 提出基于条件边际熵率的无训练推理时间调度器，通过非均匀网格改善低预算下流生成模型的样本质量。


<details>
  <summary>Details</summary>
Motivation: 固定流生成模型在少推理预算下，采样质量高度依赖有限函数评估的分配位置，而现有调度器多为启发式或继承自扩散模型。

Method: 推导条件边际熵率目标，分离端点条件桥几何与边际流演化，构建训练无关的熵调度器；对高斯布朗桥得到闭合U形率，启发边界加权非均匀网格。

Result: 二维桥流模型中，10步ODE/SDE-Heun的MMD分别比线性网格提升18.1%和22.7%；EDM/CIFAR-10上5步FID达186.3（线性200.5，余弦238.0）；AlphaFlow蛋白质生成在低NFE下表现更优。

Conclusion: 熵率调度是一种实用且有效的低预算分配信号，可提升高维桥与流采样器的性能。
...
### [103] [AgentStop: Terminating Local AI Agents Early to Save Energy in Consumer Devices](https://arxiv.org/abs/2605.15206)
*Dzung Pham,Kleomenis Katevas,Ali Shahin Shamsabadi,Hamed Haddadi*

Main category: cs.LG

TL;DR: 研究了本地部署LLM代理的资源消耗，提出AgentStop方法通过提前终止无效轨迹降低15-20%能耗，任务性能损失小于5%。


<details>
  <summary>Details</summary>
Motivation: 本地代理保护隐私、无API费用，但代理工作流比单次推理更耗资源，大量token消耗往往导致任务失败。

Method: AgentStop是一个轻量级效率监控器，利用令牌级对数概率等低成本执行信号预测并提前终止可能失败的轨迹。

Result: 在网页问答和编码基准测试中，AgentStop减少15-20%浪费的能量，任务性能下降小于5%。

Conclusion: 预测性提前终止是实现可持续、隐私保护的本地LLM代理的实用机制。
...
### [104] [TeamTR: Trust-Region Fine-Tuning for Multi-Agent LLM Coordination](https://arxiv.org/abs/2605.15207)
*Yi Xie,Siao Liu,Falong Fan,Yuanqi Yao,Yue Zhao,Bo Liu*

Main category: cs.LG

TL;DR: 提出TeamTR框架解决多智能体顺序微调中的复合占据偏移问题，实现平均7.1%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM系统在复杂推理中虽有潜力，但常不如单模型基线；原因是顺序微调时上下文分布偏移累积。

Method: 形式化复合占据偏移并证明其二次惩罚；提出TeamTR，采用信任区域方法，每次组件更新后重采样轨迹并控制每个智能体的散度。

Result: TeamTR平均提升7.1%，缓解协调退化，支持即插即用组件替换。

Conclusion: TeamTR通过理论分析和实践验证，有效解决了多智能体顺序微调中的性能退化问题。
...
### [105] [Quantization Undoes Alignment: Bias Emergence in Compressed LLMs Across Models and Precision Levels](https://arxiv.org/abs/2605.15208)
*Plawan Kumar Rath,Rahul Maliakkal*

Main category: cs.LG

TL;DR: 大语言模型的量化压缩会导致模型产生新的刻板偏见，而标准质量指标如困惑度几乎无法察觉这种退化，表明聚合评估指标会系统性遗漏公平性关键退化。


<details>
  <summary>Details</summary>
Motivation: 现有量化研究缺乏对模型质量退化的细致分析，无法区分渐进退化与阈值相关的安全失败，急需探究量化如何影响偏见。

Method: 对三个指令调优模型（Qwen2.5-7B、Mistral-7B、Phi-3.5-mini）在五个精度（BF16至3位）下，使用12,148个BBQ偏见基准条目和5个随机种子，共进行911,100次推理记录进行控制实验。

Result: 3位量化导致6-21%的原有公正项目出现新刻板行为，模型回答“未知”的比例下降17.4%；而困惑度在8位时增加不足0.5%，4位时不足3%，但4位时已有2.5-5.6%的项目出现新偏见。

Conclusion: 聚合评估指标无法捕捉量化引起的公平性退化，需要采用质量感知的压缩协议，在部署前明确测试偏见出现情况。
...
### [106] [Mask-Morph Graph U-Net: A Generalisable Mesh-Based Surrogate for Crashworthiness Field Prediction under Large Geometric Variation](https://arxiv.org/abs/2605.15231)
*Haoran Li,Tobias Lehrer,Yingxue Zhao,Haosu Zhou,Philipp Stocker,Tobias Pfaff,Nan Li*

Main category: cs.LG

TL;DR: 提出MMGUNet，一种基于图神经网络的代理模型，用于非线性有限元碰撞仿真，通过粗图变形和掩码预训练提高准确性和数据效率。


<details>
  <summary>Details</summary>
Motivation: 非线性有限元碰撞仿真计算昂贵，限制了迭代设计优化，需要更快的替代方法。

Method: 使用Mask-Morph Graph U-Net架构，通过特征对齐的重心参数化对粗图层次进行变形，并在监督预训练中应用节点掩码，然后冻结高参数边特定层进行参数高效微调。

Result: 粗图变形提高了测试准确率，掩码预训练减少了训练-测试差异并提高了数据效率，预测误差低于外部基线。

Conclusion: 证明了向可复用、数据高效的基于网格的代理模型用于耐撞性设计探索的实用途径。
...
### [107] [MuteBench: Modality Unavailability Tolerance Evaluation for Incomplete Multimodal Fusion](https://arxiv.org/abs/2605.15235)
*Wugeng Zheng,Ziwen Kan,Tianlong Chen,Chen Chen,Song Wang*

Main category: cs.LG

TL;DR: 提出了MuteBench基准，涵盖7个临床领域的9个数据集、6种融合架构和两种缺失模式，评估多模态生理数据在传感器故障下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对多模态生理数据中两种常见缺失模式（模态缺失和模态内缺失）在不同严重程度下的系统评估。

Method: 构建了包含125,000个样本的基准，覆盖9个数据集、6种融合架构，并分别测试了模态缺失和模态内缺失场景。
...
### [108] [Reducing the Safety Tax in LLM Safety Alignment with On-Policy Self-Distillation](https://arxiv.org/abs/2605.15239)
*Yu Fu,Longxuan Yu,Haz Sameen Shahgir,Zhipeng Wei,Hui Liu,N.Benjamin Erichson,Yue Dong*

Main category: cs.LG

TL;DR: 提出OPSA方法，通过on-policy自蒸馏和教师翻转率选择上下文，在多个推理模型上实现了更强的安全-推理权衡，减少了安全税。


<details>
  <summary>Details</summary>
Motivation: 安全对齐常以牺牲推理能力为代价（安全税），主要原因之一是分布不匹配，特别是监督微调中的off-policy训练导致模型无法从自身策略采样。

Method: 采用on-policy自蒸馏（OPSA），模型生成自身轨迹并接收来自冻结教师副本的逐token KL监督，教师基于特权安全上下文。引入教师翻转率作为搜索准则，寻找激活潜在安全推理的上下文。

Result: 在两组推理模型家族、五个规模上，OPSA在匹配数据和全参数微调下比off-policy自蒸馏和外部教师蒸馏取得更强的安全-推理权衡，小模型提升最大（R1-Distill-1.5B +8.85，Qwen3-0.6B +5.49）。

Conclusion: OPSA通过集中在早期合规决策token上更新，在提高安全性的同时保持通用推理能力，为安全对齐提供了一种有效方法。
...
### [109] [Logical Grammar Induction via Graph Kolmogorov Complexity: A Neuro-Symbolic Framework for Self-Healing Clinical Data Integrity](https://arxiv.org/abs/2605.15242)
*Abolfazl Zarghani,Amir Malekesfandiari*

Main category: cs.LG

TL;DR: 提出Logic-GNN框架，将临床记录视为结构化“私有语言”，通过图神经网络和Kolmogorov复杂度检测异常，F1达0.94，优于基线12%。


<details>
  <summary>Details</summary>
Motivation: 医疗信息系统的人工数据输入错误频繁，现有统计异常检测无法区分合法临床极端值。

Method: 结合时间图神经网络与图Kolmogorov复杂度，诱导符号语法，将异常定义为导致最小描述长度显著增加的“语法违规”。

Result: 在Sina系统数据集（200万+记录）上，F1得分为0.94，比最先进基线高出12%。

Conclusion: 该方法具有自愈机制，能建议逻辑修正以维护实时HIS系统的数据完整性。
...
### [110] [Reading the Cell, Designing the Cure: Perturbation-Conditioned Molecular Diffusion for Function-Oriented Drug Design](https://arxiv.org/abs/2605.15243)
*Ziyu Xu,Zijian Zhang,Liang Wang,Zhiyuan Liu,Qiang Liu,Shu Wu,Liang Wang*

Main category: cs.LG

TL;DR: 将基于转录组的药物设计形式化为生成逆问题，提出多分辨率转录组引导扩散框架CURE，以解决跨模态鸿沟和信号稀疏性，在结构质量和功能一致性上优于基线。


<details>
  <summary>Details</summary>
Motivation: 当可靠靶标结构不可得或表型源于失调通路时，转录组扰动提供系统级功能读数，但现有方法面临跨模态域差距和信号稀疏性挑战。

Method: 提出CURE，包含转录组扰动功能特征提取器（TFE），通过功能导向嵌入、跨模态对齐和异构感知聚合，实现多分辨率转录组引导的分子扩散生成。

Result: 在标准基准和严格分布外测试中，CURE在结构质量和功能一致性上持续优于强基线；零样本基因-抑制剂设计任务验证其实用性。

Conclusion: CURE有效实现表型驱动的生成性药物发现，展示了转录组引导设计的潜力。
...
### [111] [Privacy Evaluation of Generative Models for Trajectory Generation](https://arxiv.org/abs/2605.15246)
...
TL;DR: 现有生成轨迹模型隐私评估不足，通过成员推断攻击证明其存在隐私泄漏风险。


<details>
  <summary>Details</summary>
Motivation: 轨迹数据敏感，生成模型虽被假设具有隐私保护，但缺乏实证评估。

Method: 采用成员推断攻击等实证隐私评估方法。

Result: 生成模型的生成特性并不能消除隐私风险，攻击成功率高。

Conclusion: 生成轨迹模型必须进行严格隐私评估，不能依赖生成性质保证隐私。
...
### [112] [GQLA: Group-Query Latent Attention for Hardware-Adaptive Large Language Model Decoding](https://arxiv.org/abs/2605.15250)
*Fanxu Meng*

Main category: cs.LG

TL;DR: 提出GQLA，一种对MLA的最小修改，通过两种等效解码路径适应不同硬件，无需重新训练，显著压缩KV缓存。


<details>
  <summary>Details</summary>
Motivation: MLA在H100上高效，但H20等GPU上效率低，且缺乏MTP支持。

Method: GQLA在训练后暴露MQA-absorb和GQA两种路径，运行时根据硬件选择，支持从GQA转换的TransGQLA。

Result: 单组GQLA权重同时适配H100和H20，支持8路零冗余张量并行，KV缓存压缩至GQA的28.125%。

Conclusion: GQLA通过简单修改MLA，在保持参数不变的情况下，实现跨硬件高效推理，同时兼容预训练模型转换。
...
### [113] [PDRNN: Modular Data-driven Pedestrian Dead Reckoning on Loosely Coupled Radio- and Inertial-Signalstreams](https://arxiv.org/abs/2605.15252)
*Peter Bauer,Andreas Porada,Felix Ott,Christopher Mutschler,Tobias Feigl*

Main category: cs.LG

TL;DR: 提出PDRNN，一种模块化RNN架构，通过隐式预测异步传感器数据流和不确定性估计，提升动态运动下PDR精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有PDR系统因传感器采样率差异、不可靠传输及动态运动下多模态数据融合不足，导致精度下降。

Method: 采用简单RNN隐式预测异步数据流，模块化ML模型分别估计方向、速度等，最终融合模型结合不确定性估计，可选5G绝对定位。

Result: 在动态运动数据上，PDRNN比经典和ML方法更准确精确，避免误差累积，具备预测能力和更好的组件控制。

Conclusion: PDRNN有效解决了动态场景下的融合问题，模块化设计便于更新和维护。
...
### [114] [Position: Ideas Should be the Center of Machine Learning Research](https://arxiv.org/abs/2605.15253)
*Jairo Diaz-Rodriguez*

Main category: cs.LG

TL;DR: 当前机器学习研究分裂为基准驱动的工程和理想化的理论，本文提出以想法为中心的研究框架，强调通过定制实验测试想法的行为预测，而非追求指标或定理，以弥合理论与实践鸿沟并促进公平。


<details>
  <summary>Details</summary>
Motivation: 研究过于关注指标或抽象理论，忽视了作为科学核心的想法本身。

Method: 提出“想法优先”框架，重视想法对现代模型行为特征的预测，并通过量身定制的实验验证这些特征，而非追求排行榜成绩。

Result: 弥合理论与实践鸿沟，消除复杂度溢价，使资源有限的科研人员也能做出严谨的科学贡献。

Conclusion: 应建立以想法为核心的研究文化，将基准和定理视为测试机制假设的工具，而非终极目标。
...
### [115] [Curriculum Learning of Physics-Informed Neural Networks based on Spatial Correlation](https://arxiv.org/abs/2605.15254)
*Xujia Chen,Xinyue Hu,Letian Chen,Daming Shi,Wenhui Fan*

Main category: cs.LG

TL;DR: 提出了一种空间相关课程学习框架，通过空间因果引导、低频信息桥接和区域自适应加权，缓解PINN训练困难并提高精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏对空间信息传播和区域一致性的处理，不适用于强空间耦合的边值问题。

Method: 首先，空间因果权重引导信息从近边界区域向内传播；其次，低频信息桥接基于伪标签强制执行跨区域一致性；最后，区域自适应重加权策略调整子区域损失以降低局部残差。

Result: 在PDE基准测试中，同等计算成本下，该方法减少了训练失败并提高了解精度。

Conclusion: 该框架有效缓解了PINN训练中的高维非凸损失、多目标不平衡和信息传播问题，提高了求解准确性。
...
### [116] [Training on Documents About Monitoring Leads to CoT Obfuscation](https://arxiv.org/abs/2605.15257)
*Reilly Haskins,Bilal Chughtai,Joshua Engels*

Main category: cs.LG

TL;DR: 监控感知模型能够通过模糊推理逃避基于链式思维的监控，且CoT可控性与模糊成功高度相关。


<details>
  <summary>Details</summary>
Motivation: 链式思维监控依赖于模型忠实外化推理，但模型可能利用对监控的了解来隐藏错误行为。

Method: 使用合成文档微调使模型了解CoT监控，然后测试其在简单和更难任务上的模糊推理能力，并分析CoT可控性与模糊成功的相关性。

Result: 监控感知模型比未感知模型实现更高比例的未检测错误行为；CoT可控性与模糊成功显著相关（r=0.800, p=0.017）；在强化学习中，监控感知模型更快学会奖励黑客而不触发监控。

Conclusion: 对监控的了解加上高CoT可控性对CoT监控构成风险。
...
### [117] [Tadpole: Autoencoders as Foundation Models for 3D PDEs with Online Learning](https://arxiv.org/abs/2605.15284)
*Qiang Liu,Felix Koehler,Benjamin Holzschuh,Nils Thuerey*

Main category: cs.LG

TL;DR: 提出Tadpole，一个基于自编码器预训练的三维偏微分方程基础模型，通过在线数据生成实现大规模训练，并采用参数高效微调策略，在下游任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法在三维PDE学习中面临可迁移性、高维可扩展性和多功能性等挑战。

Method: 预训练自编码器于在线生成的合成3D PDE数据，使用单通道空间块学习可迁移表示；微调时结合低秩适应、潜在空间变换和重入跳跃连接。

Result: 在动力学学习和生成建模等下游任务中，以少量可训练参数实现准确的时间建模，性能显著。

Conclusion: Tadpole作为3D PDE学习的基础模型，展示了强大的多功能性和有效性。
...
### [118] [Universal Approximation of Nonlinear Operators and Their Derivatives](https://arxiv.org/abs/2605.15285)
*Filippo de Feo*

Main category: cs.LG

TL;DR: 首次证明了非线性k次可微算子及其导数的通用逼近定理，推广了Hornik (1991)的结果到无穷维设定。


<details>
  <summary>Details</summary>
Motivation: 非线性算子及其导数的通用逼近定理是算子学习领域的基础开放问题。

Method: 通过编码器-解码器架构，利用Banach空间逼近性质、Bastiani可微性、紧开拓扑和加权Sobolev空间四个关键特征。

Result: 证明了Banach空间之间非线性k次可微算子及其导数的均匀逼近定理，在紧集上和加权Sobolev范数下成立。

Conclusion: DIOL及其UAT在高阶精度算子学习、Banach空间快速约束优化、无穷维PDE数值方法等领域有重要应用。
...
### [119] [GQA-{\mu}P: The maximal parameterization update for grouped query attention](https://arxiv.org/abs/2605.15290)
...
TL;DR: 提出基于谱范数的特征学习视角，推导出分组查询注意力（GQA）的μP缩放，实现超参数迁移。


<details>
  <summary>Details</summary>
Motivation: 减少大型语言模型调参计算量，μP可迁移但难以推导新架构。

Method: 将谱范数条件从启发式提升为特征学习的定义，并修改谱范数处理非满秩矩阵，从而推导GQA的μP缩放。

Result: 学习率和权重衰减超参数在GQA重复超参数上成功迁移。

Conclusion: 证明了所提方法的有效性，可减少调参计算量。
...
TL;DR: 提出群组级解释稳定性差异（GESD），一种面向过程的公平性度量，衡量模型解释在不同子组间的稳定性差异，并集成到多目标优化框架FEU中，同时优化效用、结果公平和解释公平。


<details>
  <summary>Details</summary>
Motivation: 现有公平性度量（如统计均等、机会均等）仅关注结果导向的差异，而忽略决策过程的程序公平性，需要一种能够衡量解释过程稳定性的新度量。

Method: 提出GESD度量，量化不同子组间模型解释的稳定性、鲁棒性和敏感性差异；并构建多目标优化框架FEU，联合优化效用、结果公平和解释公平。

Result: 在多个基准数据集上，GESD有效捕捉了解释质量的组间差异，FEU在效用和公平性上均优于现有方法。

Conclusion: GESD桥接了结果公平与解释公平，为诊断和缓解预测模型中的偏见提供了全面工具。
...
### [121] [How Data Augmentation Shapes Neural Representations](https://arxiv.org/abs/2605.15306)
*Tianxiao He,Alex H. Williams,Sarah E. Harvey*

Main category: cs.LG

TL;DR: 数据增强对神经网络内部表示几何形状的影响通过形状分析工具被刻画，增强强度增加导致表示空间中的规则轨迹，不同增强类型引导不同方向，且神经几何可预测集成模型改进。


<details>
  <summary>Details</summary>
Motivation: 数据增强虽能改善泛化，但其对学习表示几何的影响尚不明确。

Method: 利用形状分析工具，将网络隐藏表示嵌入到距离对缩放、平移、旋转和反射不变的度量空间中，分析增强策略对表示形状的轨迹和扭曲。

Result: 增强强度增加产生规则轨迹，不同增强类型指向不同方向；表示形状扭曲程度可预测集成模型时的改进幅度；跨架构和种子存在共享几何模式。
...
### [122] [Time-Varying Deep State Space Models for Sequences with Switching Dynamics](https://arxiv.org/abs/2605.15311)
*Sanja Karilanova,Subhrakanti Dey,Ay\c{c}a \"Oz\c{c}elikkale*

Main category: cs.LG

TL;DR: 提出了一种时变状态空间模型神经网络，通过基函数字典学习时变动态，在切换系统和语音去噪任务中优于时不变模型，计算复杂度相当。


<details>
  <summary>Details</summary>
Motivation: 时变系统识别与建模是信号处理和系统辨识中的基本挑战。

Method: 基于时变状态空间模型的神经网络，利用基函数字典提供可学习的时变动态。

Result: 在切换系统合成数据和语音去噪任务上，时变模型持续优于时不变模型，且计算复杂度相近。

Conclusion: 揭示了时变动态的关键特征，以及如何分配自由度和利用更大模型弥补时不变限制。
...
TL;DR: 研究BG-0噪声模型下非凸随机优化，证明归一化动量SGD在标准/广义光滑下复杂度O(ε^{-6})，方差缩减STORM在均方光滑下达到最优O(ε^{-4})，在广义光滑下复杂度为O(ε^{-(4+α)})或O(ε^{-5})，并恢复标准有界方差率。


<details>
  <summary>Details</summary>
Motivation: 在BG-0噪声模型下，随机梯度方差随初始距离二次增长，现有方法常假设有界域或增加批大小，本文旨在无需这些假设提供收敛保证。

Method: 研究归一化随机梯度下降（带动量）和方差缩减的归一化STORM方法，在标准光滑和α对称广义光滑框架下分析。

Result: 归一化动量SGD复杂度O(ε^{-6})；STORM在均方光滑下O(ε^{-4})（最优），在期望α对称广义光滑下O(ε^{-(4+α)})（α∈(0,1)）或O(ε^{-5})（α=1）；噪声模型参数消失时恢复标准率。

Conclusion: 首次在没有有界域、增大批大小或显式锚定下，为BG-0噪声下非凸优化的归一化方法提供收敛保证，覆盖标准与广义光滑场景。
...
TL;DR: 提出通过扰动特征权重而非值来估计特征归因的新方法，形成XWP和XWP_c方法，在简单DNN上取得竞争性能。


<details>
  <summary>Details</summary>
Motivation: 全连接神经网络作为基础模型，但其可解释性缺乏共识，需要更简单的归因方法。

Method: 对特征附加的权重进行扰动，而非其值，以此形成新的归因方法XWP和XWP_c。

Result: 在简单DNN上识别图像信号方面，与最成熟的归因方法竞争，达到竞争性能。

Conclusion: 引入了一个稳健框架，解决了长期存在的脆弱性，使模型解释更可靠。
...
### [125] [From I/O to Code with Discovery Agent](https://arxiv.org/abs/2605.15334)
*Yihong Dong,Jiaru Qian,Haoran Zhang,Peixu Wang,Binhua Li,Zhi Jin,Yongbin Li,Ge Li,Xiaokang Yang,Xue Jiang*

Main category: cs.LG

TL;DR: 提出DIO-Agent，通过演化搜索和LLM变异算子解决IO2Code问题，引入转换优先级前提，实验优于基线。


<details>
  <summary>Details</summary>
Motivation: NL2Code成功但IO2Code更具挑战，需要从具体计算行为中恢复原理，假设空间巨大。

Method: 将IO2Code视为离散程序空间的演化搜索，LLM作为变异算子，执行错误引导变异，采用转换优先级前提作为先验。

Result: 在IO2CodeBench上，DIO-Agent在所有难度级别和LLM上优于传统方法和SOTA基线，超越同等采样预算的测试时缩放策略。

Conclusion: DIO-Agent有效解决IO2Code问题，转换优先级前提有助于避免错误死胡同。
...
### [126] [Bounded-Rationality, Hedging, and Generalization](https://arxiv.org/abs/2605.15340)
*Pedro A. Ortega*

Main category: cs.LG

TL;DR: 将学习者的决策建模为有界理性问题，通过f-散度正则化研究样本依赖与失真的权衡，得到损失-样本依赖曲线，并证明泛化可作为对冲属性测试。


<details>
  <summary>Details</summary>
Motivation: 理解学习者如何控制训练样本对输出的影响程度以及能对冲多少失真。

Method: 将学习者的响应律表示为f-散度正则化的有界理性决策，通过缩放损失和局部扰动从黑盒行为中恢复对冲和曲线。

Result: 导出训练损失与样本依赖的下界曲线和上界证书曲线，KL散度对应香农互信息。

Conclusion: 泛化被视为学习者响应律的可测试对冲性质，当对冲覆盖失真时提供实际证书。
...
### [127] [LEAP: Trajectory-Level Evaluation of LLMs in Iterative Scientific Design](https://arxiv.org/abs/2605.15341)
*Marilyn Zhang,Tianfeng Chen,Fabi\'an Barzuna,Ankita Rathod,Mark E. Whiting*

Main category: cs.LG

TL;DR: 提出LEAPBench，用于评估LLM在迭代科学设计中的学习效率，发现轨迹评分改变最佳模型选择，LLM未超越贝叶斯基线，领域无关提示更优，轨迹指标可作为训练目标。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅评估固定迭代次数的最终结果，忽略学习轨迹，无法捕捉学习效率。

Method: 构建55任务框架LEAPBench，使用最佳-so-far曲线下面积（AUC）轨迹指标、经典贝叶斯优化参考和文献审计。

Result: 轨迹评分使53%任务的最佳模型选择改变；LLM表现未超越贝叶斯基线；领域无关提示在16个生物学任务中比领域相关提示更接近已发表最优配置；轨迹指标作为奖励进行离线强化学习在14/21任务上提升性能。

Conclusion: 评估学习效率需关注轨迹而非仅最终结果，LLM在迭代科学设计中效率有限，领域无关提示可能更有效，轨迹指标可作为训练目标。
...
### [128] [PACER: Acyclic Causal Discovery from Large-Scale Interventional Data](https://arxiv.org/abs/2605.15353)
*Ramon Vi\~nas Torn\'e,S\'ilvia F\`abregas Salazar,Soyon Park,Ivo Alexander Ban,Artyom Gadetsky,Nikita Doikov,Maria Brbi\'c*

Main category: cs.LG

TL;DR: 提出PACER框架，通过参数化DAG分布保证无环性，实现高维干预数据的可扩展因果发现，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用软无环约束导致优化无效循环图、数值不稳定和可扩展性差。

Method: 通过联合模型变量排列和边概率参数化DAG分布，直接优化有效因果结构，支持观测和干预数据的统一似然处理。

Result: 在线性高斯机制下导出闭式期望，蛋白质信号和基因扰动基准上匹配或超越SOTA，扩展到数千变量，速度提升两个数量级。

Conclusion: 通过原则性搜索空间设计可实现高维扰动数据的精确可扩展因果发现。
...
### [129] [Controllable Molecular Generative Foundation Models](https://arxiv.org/abs/2605.15354)
*Yihan Zhu,Yuhan Liu,Weijiang Li,Tengfei Luo,Meng Jiang*

Main category: cs.LG

TL;DR: 提出CoMole，一种基于模体感知图扩散的可控分子生成基础模型，结合强化学习优化条件逆向策略，在三个异构基准上实现最优可控性和高有效性，并支持零样本迁移。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型在分子图生成中缺乏统一框架和可靠可控性，而强化学习应用于图生成面临原子动作空间巨大和中间状态无效的挑战。

Method: 构建模体感知的图扩散管道，将预训练结构先验迁移到可控生成，并通过强化学习优化模体级别的条件逆向策略，避免原子级瓶颈。

Result: 在9个可控性目标上排名第一，MAE最高降低48.2%，有效性保持0.94以上无需后处理，且冻结生成器仅优化任务嵌入即可迁移到未见属性。

Conclusion: CoMole通过模体感知和强化学习实现了高效可控的分子生成，兼具强泛化能力和零样本迁移性能。
...
### [130] [Federated Learning of Spiking Neural Networks under Heterogeneous Temporal Resolutions](https://arxiv.org/abs/2605.15355)
*Sanja Karilanova,Subhrakanti Dey,Ay\c{c}a \"Oz\c{c}elikkale*

Main category: cs.LG

TL;DR: 提出一种联邦学习框架，解决脉冲神经网络在时间分辨率异构下的训练问题。


<details>
  <summary>Details</summary>
Motivation: 边缘设备因硬件和能量限制采集数据的时间分辨率不同，导致联邦平均无效。

Method: 研究不同时间分辨率下神经元参数的整合与模型聚合，提出适应性方法。

Result: 在SHD和DVS-Gesture数据集上，所提方法能显著恢复因时间不匹配导致的精度损失。

Conclusion: 使各客户能在本地时间分辨率下训练，同时与全局模型兼容。
...
### [131] [PRB-RUPFormer: A Recursive Unified Probabilistic Transformer for Residual PRB Forecasting](https://arxiv.org/abs/2605.15363)
*Saad Masrur,Yuxuan Jiang,Matti Hiltunen,Ajay Rajkumar,Ismail Guvenc*

Main category: cs.LG

TL;DR: 提出PRB-RUPFormer，一种递归统一概率Transformer，用于预测剩余物理资源块（PRB），实现高精度与不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 现有PRB预测方法仅依赖历史值且独立训练，无法捕捉跨载波依赖和提供预测不确定性，点预测不足以应对高变流量。

Method: 使用时间、季节和载波感知嵌入联合处理多变量KPI时间序列，递归展开保持时序耦合，单一模型在所有载波和扇区上训练，通过分位数预测区间捕获不确定性。

Result: 在六个月商业LTE数据上，中值MAE低于0.05，命中概率大于0.80，支持一天和七天递归预测。

Conclusion: 该方法可直接支持动态载波激活、拥塞避免和 proactive 频谱共享等频谱感知RAN功能，适合动态频谱接入场景。
...
### [132] [Is One Score Enough? Rethinking the Evaluation of Sequentially Evolving LLM Memory](https://arxiv.org/abs/2605.15384)
*Songwei Dong,Zihan Chen,Chengshuai Shi,Peng Wang,Jundong Li,Cong Shen*

Main category: cs.LG

TL;DR: 提出了SeqMem-Eval，一个诊断性评估框架，用于顺序进化的大语言模型记忆，测量在线效用、留出泛化、反向迁移和遗忘，发现高准确率不一定代表高质量记忆。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要依赖聚合指标，掩盖了遗忘和负迁移等关键失败模式。

Method: 受持续学习启发，针对测试时外部、提示介导、不修改参数的记忆更新设置，评估记忆状态如何进化、泛化、巩固和保留信息。

Result: 许多方法虽性能提升但遭受显著遗忘或负迁移；不同记忆设计在适应性和稳定性之间存在权衡，标准指标无法揭示。

Conclusion: 需要更细致的记忆质量评估，而非仅关注最终准确率。
...
### [133] [Unified High-Probability Analysis of Stochastic Variance-Reduced Estimation](https://arxiv.org/abs/2605.15388)
*Zhankun Luo,Antesh Upadhyay,M. Berk Sahin,Sang Bin Moon,Anuran Makur,Abolfazl Hashemi*

Main category: cs.LG

TL;DR: 提出了一个统一的随机方差缩减估计框架，基于记忆保持、重置概率和迭代移动校正的三分量递归，并导出了高概率界。


<details>
  <summary>Details</summary>
Motivation: 现有随机估计方法分析局限于特定估计器和期望，忽略了决定可靠性的结构权衡。

Method: 开发了一个统一框架，通过三分量递归（记忆保持、重置概率、迭代移动校正）来恢复经典估计器并推导偏差-方差分解。

Result: 利用新维度无关向量值Freedman不等式得到了统一高概率界，适用于欧几里得和非欧几里得设置，并获得了镜像下降的优化oracle复杂度，首次在期望约束下将复杂度从Õ(ε⁻⁴)改进到Õ(ε⁻³)。

Conclusion: 该框架统一了多种方差缩减估计器，提供了高概率保证，并在约束优化中取得了实质性进展。
...
### [134] [LPDS: Evaluating LLM Robustness Through Logic-Preserving Difficulty Scaling](https://arxiv.org/abs/2605.15393)
*Philipp Mondorf,Samuel J. Bell,Jesse Dodge,Dieuwke Hupkes*

Main category: cs.LG

TL;DR: 提出逻辑保持难度缩放框架（LPDS），系统评估大语言模型在实体变化下的鲁棒性，发现难度越高性能下降越明显，且LPDS比随机采样更高效地找出失败案例。


<details>
  <summary>Details</summary>
Motivation: 现有评估缺乏系统性方法识别导致模型失败的逻辑保持变体，随机采样可能高估鲁棒性。

Method: LPDS框架：量化问题变体的难度，系统搜索允许变体空间以最大化难度和暴露失败。

Result: 难度增加导致性能下降和推理链错误增多；LPDS找到困难变体效率高，性能下降幅度达随机采样的5倍。

Conclusion: 在更难变体上微调比在容易变体上训练带来更一致的鲁棒性提升。
...
### [135] [Representation Without Reward: A JEPA Audit for LLM Fine-Tuning](https://arxiv.org/abs/2605.15394)
*Biswa Sengupta*

Main category: cs.LG

TL;DR: 基于LoRA和全微调实验，JEPA辅助目标在自然语言到正则表达式生成任务上未显著提升准确率，隐状态表示与解码任务准确率弱耦合。


<details>
  <summary>Details</summary>
Motivation: 探索JEPA原则在自回归语言模型微调中的更严格要求，即隐状态几何必须影响语言模型头部并改善任务指标。

Method: 在Llama-3.2-1B-Instruct模型上，使用LoRA和全微调，对照22种训练时辅助目标，包括轨迹形状正则化、分布约束、预测器/目标不对称、Fisher度量雅可比残差以及解码器可见JEPA目标。

Result: 多个辅助目标在未经多重比较校正时表现显著，但经Bonferroni或Holm-Bonferroni校正后均不显著；解码器可见JEPA目标虽产生首个正梯度余弦，但准确率提升不显著；全微调复制实验也得到空结果。

Conclusion: 隐状态表示与解码任务准确率在此设置下弱耦合，应将JEPA评估重构为耦合问题，关注何种指标下隐几何能成为解码器可见的任务信号。
...
### [136] [Breakeven complexity: A new perspective on neural partial differential equation solvers](https://arxiv.org/abs/2605.15399)
*Yijing Zhang,Nicholas Roberts,Tanya Marwah,Mikhail Khodak*

Main category: cs.LG

TL;DR: 提出基于盈亏平衡复杂度的神经PDE求解器评估框架，发现求解器在更难问题上更有效。


<details>
  <summary>Details</summary>
Motivation: 现有评估忽略神经求解器高昂前期成本和传统求解器低成本低精度解的可用性。

Method: 以盈亏平衡复杂度为核心指标，应用缩放定律分配训练预算，实现误差匹配。

Result: 在多个基准上，神经求解器在问题更困难（高成本、高维、长 rollout、高雷诺数）时更具成本效益。

Conclusion: 神经PDE求解器在复杂问题中更具优势，评估框架应考虑端到端成本。
...
### [137] [$\phi$-Balancing for Mixture-of-Experts Training](https://arxiv.org/abs/2605.15403)
*Lizhang Chen,Jonathan Li,Qi Wang,Runlong Liao,Shuozhe Li,Chen Liang,Ni Lao,Qiang Liu*

Main category: cs.LG

TL;DR: 提出ϕ-平衡框架，通过凸优化实现专家负载平衡，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有负载平衡方法基于启发式和噪声的小批量统计，存在偏差，需要更原则性的方法。

Method: 最小化期望路由分布的严格凸、对称、可微势函数，通过凸对偶得到min-max形式，用镜像下降得到在线EMA调整算法。

Result: 在大规模预训练和微调中，ϕ-平衡一致优于Switch-style和无损失基线，专家利用更稳定有效。

Conclusion: ϕ-平衡是一种原则性的专家负载平衡框架，有效提升MoE性能。
...
### [138] [Transformer Scalability Crisis: The First Comprehensive Empirical Analysis of Performance Walls in Modern Language Models](https://arxiv.org/abs/2605.15413)
*Mahdi Naser Moghadasi,Faezeh Ghaderi*

Main category: cs.LG

TL;DR: 大规模评估118个transformer模型，发现可扩展性危机：处理512 tokens成功率为88.1%，1024 tokens降至44.9%，2048 tokens完全失败；压缩模型参数效率更高。


<details>
  <summary>Details</summary>
Motivation: Transformer架构在NLP中成功，但其可扩展性限制缺乏系统实证分析。

Method: 对118个模型（七类架构）进行系统基准测试，测量加载时间、内存消耗和计算效率，序列长度从128到2048 tokens。

Result: 88.1%模型成功处理512 tokens，44.9%处理1024 tokens，0%处理2048 tokens；压缩模型参数效率（649.2 tokens/sec/M）远高于大生成模型（12.5 tokens/sec/M）。

Conclusion: 挑战现有缩放假设，提供O(n²)注意力复杂性转化为可测量性能墙的首个定量证据，建立新基准方法并指导实际部署。
...
### [139] [Margin-Adaptive Confidence Ranking for Reliable LLM Judgement](https://arxiv.org/abs/2605.15416)
*Gaojie Jin,Yong Tao,Lijia Yu,Tianjin Huang*

Main category: cs.LG

TL;DR: 通过专门学习的置信度估计器改进了LLM与人类判断的一致性框架。


<details>
  <summary>Details</summary>
Motivation: 现有框架依赖不可靠的单调性假设，且未分析置信度估计器的泛化行为。

Method: 利用模拟标注者多样性和间隔排序学习置信度估计器，并推导泛化保证以自适应训练。

Result: 集成到固定序列测试后，排序准确性提高，单调关系增强，成功率达更高。

Conclusion: 学习的置信度估计器能有效提升LLM与人类判断一致的保证。
...
### [140] [$f$-Trajectory Balance: A Loss Family for Tuning GFlowNets, Generative Models, and LLMs with Off- and On-Policy Data](https://arxiv.org/abs/2605.15417)
*Jake Fawkes,Jason Hartford*

Main category: cs.LG

TL;DR: 将目标与模型对数概率的均方误差损失扩展到f-散度族，实现策略内外梯度一致。


<details>
  <summary>Details</summary>
Motivation: 在GFlowNets和变分推断中，均方误差损失有效且低方差，但仅限于KL散度；希望将其推广到更广泛的f-散度。

Method: 证明平移不变损失函数与f-散度之间存在一一对应，设计新的代理损失函数。

Result: 在合成示例、分子发现SynFlowNets和非同步大语言模型调优中验证了方法的有效性。

Conclusion: 提出的损失函数保留了f-散度的性质，适用于策略内外数据，能覆盖多种生成模型。
...
### [141] [Lagrangian Flow Matching: A Least-Action Framework for Principled Path Design](https://arxiv.org/abs/2605.15419)
*Shukai Du,Junzhe Zhang,Yiming Li*

Main category: cs.LG

TL;DR: 提出拉格朗日流匹配框架，通过最小化一般拉格朗日量的作用量来设计概率路径和速度场，涵盖已有构造并推广到新路径。


<details>
  <summary>Details</summary>
Motivation: 现有流匹配方法仅使用直线路径（自由粒子动力学），限制了对更广泛动力学类的覆盖能力。

Method: 基于最小作用原理，用一般拉格朗日量的作用量最小化来定义概率路径和速度场，导出等价静态最优传输问题，得到无模拟训练目标。

Result: 恢复基于最优传输的流匹配和谐振子扩散路径，新拉格朗日量产生有意义的动力学变化，且与现有条件流匹配模型竞争力相当。

Conclusion: 该框架提供了更灵活的路径设计，同时保持竞争力。
...
### [142] [DualKV: Shared-Prompt Flash Attention for Efficient RL Training with Large Rollouts and Long Contexts](https://arxiv.org/abs/2605.15422)
*Jiading Gai,Shuai Zhang,Xiang Song,Bernie Wang,George Karypis*

Main category: cs.LG

TL;DR: 提出DualKV，消除RL训练中共享提示词的冗余计算，实现策略更新的显著加速。


<details>
  <summary>Details</summary>
Motivation: 现代RL后训练方法（如GRPO、DAPO）在大量rollout和长上下文中，标准FlashAttention复制提示词导致大量冗余计算和内存，成为策略更新的主要开销。

Method: DualKV通过融合CUDA前向和反向内核，将共享上下文和每序列响应分开处理，并重新设计veRL数据管道，将N(P+R) tokens重排为P+NR tokens，减少整体计算量。

Result: 在8×H100上训练Qwen3-8B GRPO，策略更新加速1.63-2.09倍，MFU从36%提升到76%；DAPO加速2.47倍，MFU 77%；30B MoE规模加速3.82倍策略更新和3.38倍端到端步速。

Conclusion: DualKV数学上等价于标准注意力，无近似，显著提升RL训练效率。
...
### [143] [Spectral Priors vs. Attention: Investigating the Utility of Attention Mechanisms in EEG-Based Diagnosis](https://arxiv.org/abs/2605.15433)
*Tawsik Jawad,Gowtham Atluri,Vikram Ravindra*

Main category: cs.LG

TL;DR: 脑电图信号噪声大、空间分辨率低，分类困难。本文提出频谱选择性特征构建方法，通过提取主要脑波频段的信号强度，将高维原始数据转化为高价值频谱特征，使传统机器学习模型性能匹配或超越现有深度模型，并揭示注意力机制在提取稳定频谱特征方面的根本性局限。


<details>
  <summary>Details</summary>
Motivation: 脑电图分类面临噪声、空间分辨率低和类间相似性高的问题，现有深度学习模型难以有效区分健康与疾病状态或不同疾病类型。

Method: 采用频谱选择性方法，孤立提取主要脑波频段（如δ、θ、α、β、γ）的信号强度作为特征，将高维原始数据转化为低频、高频等频谱特征。

Result: 在三个静息态和一个任务态脑电图数据集上，基于频域和时频域特征的传统机器学习模型（如SVM、随机森林）达到了或超越了当前最优深度学习模型的性能；注意力机制模型无法有效提取稳定的频谱特征，即使输入频域选择性时域数据，性能提升也不明显。
...
### [144] [On the Stability of Growth in Structural Plasticity](https://arxiv.org/abs/2605.15435)
*Lute Lillo,Nick Cheney*

Main category: cs.LG

TL;DR: 训练中动态调整网络结构时，生长（新增单元）与剪枝不同，新单元因梯度弱而表现不佳，但在持续学习中若给予足够时间可竞争。


<details>
  <summary>Details</summary>
Motivation: 探索在训练过程中通过生长新单元来适应模型，与固定架构或剪枝相比有何不同，特别是生长面临的挑战。

Method: 通过实验隔离了插入问题，比较了生长和剪枝在不同任务（MLP、图像分类、持续学习）中的表现，并进行了多种干预（优化器状态、插入、选择、可训练性）。

Result: 生长在结构编辑过程中能达到高最终准确率，但剪枝在平均训练轨迹或从头再训练时更强。干预可改善新单元整合，但不一定产生更好的最终子网络。在持续学习中，生长需要足够时间才能竞争。

Conclusion: 生长不仅应作为架构搜索算子，还应作为时间敏感的优化过程，其成功取决于插入稳定性。
...
### [145] [Don't Stop Me Yet: Sampling Loss Minima via Dissipative Riemannian Mechanics](https://arxiv.org/abs/2605.15459)
*Albert Kj{\o}ller Jacobsen,Leo Uhre Jakobsen,Johanna Marie Gegenfurtner,Georgios Arvanitidis*

Main category: cs.LG

TL;DR: 提出一种基于动力系统的采样器DiMS，能够精确采样神经网络损失函数的全局最小水平集，并通过物理超参数控制探索能力，在贝叶斯不确定性量化中表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么不能精确采样重参数化不变解，要么是局部的限制探索，无法有效表征损失函数的最小值连通结构。

Method: 设计基于动能、重力和摩擦阻尼的动力系统，保证采样分布精确收敛到最小水平集，并通过物理超参数调节探索范围。

Result: 在贝叶斯推断的不确定性量化任务中，DiMS相比已有方法获得更优性能。

Conclusion: DiMS为探索神经网络损失函数的最小值流形提供了一种有效且可控制的采样框架。
...
TL;DR: 提出SAE框架，通过跨任务记忆积累和重用经验，高效构建药物发现SOTA模型。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体通过试错找到SOTA方案，但每次新任务都需要从头搜索，成本高昂。

Method: 维护跨任务记忆，包括已验证技能、有效策略统计证据、常见错误及修复，支持直接迁移解决方案。

Result: 在33个分子性质预测任务中单任务设置排名第一；跨任务设置中，从16个小任务积累记忆后，在17个held-out任务上平均归一化分数0.935，零测试时搜索下优于基线10-30%。

Conclusion: 跨任务记忆在高效SOTA模型开发中具有优势。
...
TL;DR: 提出ChainzRule架构，通过多项式引擎和微分正则化同时提升准确性、硬件效率和功能稳定性。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型在准确性、硬件效率和功能稳定性之间存在冲突，传统方法牺牲稳定性换取性能。

Method: 用多项式引擎替代分段线性激活，并采用微分正则化（DREG）直接约束中间导数，避免全局Lipschitz约束的过度平滑。

Result: 在公平对抗基准中，ChainzRule以15.5倍更少参数超越标准模型；MNIST上峰值梯度波动降低23.1%；Yelp Full序数回归准确率达70.17%。
...
TL;DR: 提出GRLO方法，仅用5K提示和22.7 GPU小时，通过从零开始的RLHF训练，使Qwen3-4B-Base平均性能从24.1提升至63.1，超越强域内RLVR基线，并与更大成本训练的模型竞争。


<details>
  <summary>Details</summary>
Motivation: 现有RL后训练（RLHF和RLVR）需要大量GPU计算，域内训练虽好但成本高；探索RLHF从少量交互中泛化到下游任务的能力，以实现高效通用训练。

Method: 在Qwen3-4B-Base骨干上，使用5K提示和22.7 GPU小时，从零开始进行RLHF训练，在开放环境中学习对话能力，并隐式迁移到数学推理和代码生成任务。

Result: 平均性能从24.1提升至63.1，数据量减少约46倍，计算量减少约68倍；与Qwen发布的后训练模型性能相当；后续域内RLVR仅对困难竞赛数学基准有选择性提升。

Conclusion: GRLO提供了一种简单高效的配方，可训练出广泛能力的后训练模型，且数据与计算需求极低。
...
TL;DR: 提出NormWear-2世界模型，结合直觉与洞察实现多尺度生理信号长期预测，在多个真实数据集上取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究忽视长期信号级预测，生理信号的预测性质未充分探索。

Method: 将生理信号与临床干预编码到共享潜空间，建模为动力系统；结合先验知识（直觉）和非参数适应（洞察）；预训练时采用混沌理论平衡动态多样性。

Result: 在8,026名受试者的多个数据集上，时间、频率和潜表示域预测性能最优，优于现有基础模型。

Conclusion: NormWear-2迈向通用生理信号世界模型，实现多尺度一致预测。
...
### [150] [Njord: A Probabilistic Graph Neural Network for Ensemble Ocean Forecasting](https://arxiv.org/abs/2605.15470)
*Daniel Holmberg,Joel Oskarsson,Erik Larsson,Fredrik Lindsten,Teemu Roos*

Main category: cs.LG

TL;DR: Njord是一种结合深度潜变量和图神经网络的概率性海洋预报模型，能生成集合预报并提供不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 海洋动力过程本质混沌，现有机器学习模型仅能进行确定性预报，缺乏概率性预报能力。

Method: 模型融合深度潜变量框架与图神经网络，采用K-means聚类网格适应不规则海面几何，实现高效缩放。

Result: 在全球和区域（波罗的海）实验中表现优于确定性基线模型，在OceanBench基准上平均误差最低，尤其在海表温度预测方面改进最大。

Conclusion: Njord提供了概率预报和不确定性估计，在多个尺度上均具有强竞争力。
...
### [151] [Learning Normalized Energy Models for Linear Inverse Problems](https://arxiv.org/abs/2605.15487)
*Nicolas Zilberstein,Santiago Segarra,Eero Simoncelli,Florentin Guth*

Main category: cs.LG

TL;DR: 提出一种新的基于能量的扩散模型，通过协方差正则化训练去噪，能直接计算归一化后验密度，克服了隐式先验和采样偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在逆问题中先验密度隐式表示且依赖近似似然导致采样偏差。

Method: 训练一个基于能量的去噪模型，加入协方差正则化项以保持不同测量条件的一致性，无需重新训练即可处理多种线性逆问题，并实现能量引导自适应采样、无偏Metropolis-Hastings校正和盲估计退化算子。

Result: 在ImageNet、CelebA、AFHQ数据集上的图像修复和去模糊任务中取得与基线相当或更优的性能。

Conclusion: 该方法扩展了扩散模型的能力，实现了更灵活无偏的逆问题求解。
...
### [152] [SurvivalPFN: Amortizing Survival Prediction via In-Context Bayesian Inference](https://arxiv.org/abs/2605.15488)
*Shi-ang Qi,Vahid Balazadeh,Michael Cooper,Russell Greiner,Rahul G. Krishnan*

Main category: cs.LG

TL;DR: 提出 SurvivalPFN，一种先验数据拟合网络，通过上下文学习实现删失数据的贝叶斯推断，无需任务特定训练即可进行生存分析。


<details>
  <summary>Details</summary>
Motivation: 传统生存分析方法需要大量专业知识和人工选择估计器，缺乏通用性。

Method: 在多样化的合成、可识别、右删失数据生成过程上预训练，通过上下文学习在单次前向传播中完成生存分析。

Result: 在61个数据集、21种方法、5个评估指标的大规模基准测试中取得强预测性能，常优于已有模型。

Conclusion: SurvivalPFN 为生存分析提供了有原则且实用的基础模型，可应用于医疗、金融、工程等领域。
...
### [153] [Ghosted Layers: Unconstrained Activation Alignment for Recovering Layer-Pruned LLMs](https://arxiv.org/abs/2605.15491)
*Vincent-Daniel Yun,Junhyuk Jo,Sai Praneeth Karimireddy,Sunwoo Lee*

Main category: cs.LG

TL;DR: 提出Ghosted Layers，通过闭式最优线性算子恢复剪枝导致的激活差异，无需训练即可提升LLM层剪枝后的性能。


<details>
  <summary>Details</summary>
Motivation: 层剪枝移除Transformer解码器块后，隐藏状态分布不匹配，导致性能显著下降。

Method: Ghosted Layers通过边界激活对齐，从校准集推导闭式最优线性算子，重建被剪层引起的激活差异。

Result: 在多种LLM和剪枝策略上，相比现有免训练方法，准确率和困惑度持续提升，且保持剪枝效率。

Conclusion: Ghosted Layers有效解决了层剪枝中的分布不匹配问题，是一种免训练的高效恢复方法。
...
### [154] [Learning with Conflicts of Interest](https://arxiv.org/abs/2605.15504)
*Nischal Aryal,Arash Termehchy,Ali Vakilian,Marianne Winslett*

Main category: cs.LG

TL;DR: 针对机器学习系统与用户利益冲突导致的信息偏见问题，提出博弈论框架保护用户决策。


<details>
  <summary>Details</summary>
Motivation: 现有ML系统带有偏见信息，用户利益受损，而系统所有者缺乏缓解偏见的激励。

Method: 构建博弈论模型描述ML系统与用户互动，设计可扩展算法并给出理论保证。

Result: 算法能最大化期望信息/行为，最小化偏见/操纵行为。

Conclusion: 该框架有效平衡利益冲突，保护用户。
...
### [155] [STS: Efficient Sparse Attention with Speculative Token Sparsity](https://arxiv.org/abs/2605.15508)
*Ceyu Xu,Jiangnan Yu,Yongji Wu,Yuan Xie*

Main category: cs.LG

TL;DR: 提出STS稀疏注意力机制，利用草稿模型识别关键token，实现无需重训练的LLM推理加速，在NarrativeQA上达到2.67倍加速和90%稀疏度。


<details>
  <summary>Details</summary>
Motivation: 注意力二次复杂度导致LLM推理瓶颈，尤其处理百万token序列时，现有方法难以兼顾效率与精度。

Method: 在投机解码框架中，利用小草稿模型的注意力分数动态构建token和头级别的稀疏掩码，剪枝目标模型的计算。

Result: 在NarrativeQA上实现约90%稀疏度下2.67倍加速，精度损失可忽略，优于先前方法。

Conclusion: STS在稀疏度-精度权衡上达到新最优，允许在给定精度预算下实现更高稀疏度。
...
### [156] [parallelcbf: A composable safety-filter and auditability framework for tensor-parallel reinforcement learning](https://arxiv.org/abs/2605.15509)
*Yijun Lu,Zilei Yang,Yuyin Ma*

Main category: cs.LG

TL;DR: ParallelCBF是首个统一张量并行无人机环境、硬门控CBF安全滤波器、分片BC-to-RL流水线和可审计操作API的框架，提供可复现的安全约束训练。


<details>
  <summary>Details</summary>
Motivation: 现有框架如Isaac Lab、OmniSafe、safe-control-gym和CBFKit各自提供部分功能，但缺乏统一端到端安全约束训练的平台。

Method: 设计四层可组合API，包括张量并行无人机环境、硬门控CBF安全滤波器、分片行为克隆到强化学习流水线，以及可审计的操作原语（预注册、看门狗注册、故障取证、数据集审计）。

Result: 在1.67秒内完成39项属性基安全不变性测试，执行31,415集行为克隆收集，审计层阻止了未达标收敛的下游训练，防止了退化检查点的传播。

Conclusion: 该框架提供了可复现实证机器人研究所需的架构属性，通过pip install parallelcbf安装，开源在GitHub。
...
### [157] [OgBench: A Framework for Evaluating Graph Neural Networks on Omics Data](https://arxiv.org/abs/2605.15511)
*Louisa Cornelis,Johan Mathe,Louis Van Langendonck,Guillermo Bern\'ardez,Nina Miolane*

Main category: cs.LG

TL;DR: 提出OgBench基准平台，发现在生物组学数据（n<<p）中，常用GNN并不优于简单MLP和基线方法，挑战了图结构必然有益的观点。


<details>
  <summary>Details</summary>
Motivation: 现有图级别学习基准多适用于n>>p场景，忽略生物组学中n<<p的情况，需要研究GNN在此类数据上的表现。

Method: 构建OgBench平台，提供从原始组学数据到特征图的标准化流程，评估多种GNN和基线方法。

Result: 广泛使用的GNN在组学数据上表现不如简单MLP和经典基线。

Conclusion: 图结构在生物组学中并非必然带来性能提升，需要重新评估当前范式并开发专为生物图设计的架构。
...
### [158] [On the Fragility of Data Attribution When Learning Is Distributed](https://arxiv.org/abs/2605.15520)
*Xian Gao,Bo Hui,Min-Te Sun,Wei-Shinn Ku*

Main category: cs.LG

TL;DR: 发现数据归因方法存在漏洞，单个参与者可通过攻击大幅提高归因值而不影响模型效用。


<details>
  <summary>Details</summary>
Motivation: 现有归因方法假设归因值真实反映贡献，但该假设可能失败，存在被恶意利用的风险。

Method: 提出归因优先攻击，通过潜在优化注入合成数据批次，利用非IID标签覆盖和评估器敏感性。

Result: 攻击在各种数据集和模型上持续提高对手归因值，重塑良性客户归因结构，不降低准确率且不触发几何防御。

Conclusion: 归因本身形成新的攻击面，需开发归因鲁棒且激励兼容的评分机制。
...
### [159] [Rethinking Neural Network Learning Rates: A Stackelberg Perspective](https://arxiv.org/abs/2605.15530)
*Sihan Zeng,Sujay Bhatt,Sumitra Ganesh*

Main category: cs.LG

TL;DR: 本文通过Stackelberg优化视角，证明对网络主体层使用小学习率、最终层使用大学习率的非均匀学习率策略可加速训练，并给出理论保证和机制解释。


<details>
  <summary>Details</summary>
Motivation: 虽然经验证据表明逐层学习率可加速训练，但对非均匀学习率有益的条件和机制缺乏理论理解。

Method: 将非均匀学习率训练视为Stackelberg优化重述后的两时间尺度交替梯度下降算法，建立有限时间收敛保证，并分析两种加速机制。

Result: 非均匀学习率可以通过（1）构造具有更强优化结构的Stackelberg目标以更快收敛到全局最优，（2）在早期训练中产生更尖锐的局部曲率以提供更有信息量的梯度，从而优于均匀学习率。实验支持。

Conclusion: 非均匀学习率在理论和实践上都有优势，通过Stackelberg视角可以解释其加速机制。
...
### [160] [DeltaPrompts: Escaping the Zero-Delta Trap in Multimodal Distillation](https://arxiv.org/abs/2605.15532)
*Jaehun Jung,Hyunwoo Kim,Brandon Cui,Ximing Lu,David Acuna,Prithviraj Ammanabrolu,Yejin Choi*

Main category: cs.LG

TL;DR: 提出高差异度提示生成方法DeltaPrompts，解决蒸馏过程中零差异提示导致的训练饱和问题，在多个基准上取得最高15%的相对提升。


<details>
  <summary>Details</summary>
Motivation: 现有蒸馏方法中高达69%的提示为零差异提示（师生模型输出分布相同），导致训练信号微弱，模型性能快速饱和。

Method: 基于分布差异量化指标Δ，设计分阶段合成流水线，以现有数据集为种子，主动生成针对学生模型弱点的高差异度推理问题。

Result: DeltaPrompts数据集（20万高差异问题）在三种场景（同策略蒸馏、跨模型族迁移、非推理模型微调）下均显著提升性能，在Qwen3-VL-8B-Think等优化模型上获得15%的相对改进。

Conclusion: 通过聚焦功能能力差距而非简单数据聚合，能有效释放蒸馏潜力，突破零差异陷阱。
...
### [161] [CTF4Nuclear: Common Task Framework for Nuclear Fission and Fusion Models](https://arxiv.org/abs/2605.15549)
...
TL;DR: 提出一个用于核工程机器学习的通用任务框架（CTF），通过标准化评估和隐藏测试集来比较不同ML方法，以期取代临时比较，提高严谨性和可重复性。


<details>
  <summary>Details</summary>
Motivation: 核工程中系统复杂，高保真模拟计算昂贵且不适用于实时，模型基于简化假设导致偏差；ML方法多样但缺乏公平比较，尤其在安全关键领域。

Method: 构建一个包含核及核相关系统数据集的CTF，采用12个指标和新的稀疏测量系统监控范式，并基准测试标准ML基线。

Result: 基准测试揭示了当前ML方法的局限性。

Conclusion: 期望用标准化评估和隐藏测试集替代临时比较，提升核工业科学ML的严谨性和可重复性。
...
### [162] [Characterizing Learning in Deep Neural Networks using Tractable Algorithmic Complexity Analysis](https://arxiv.org/abs/2605.15551)
*Pedram Bakhtiarifard,Sophia N. Wilson,Mahmoud Afifi,Jonathan Wensh{\o}j,Raghavendra Selvan*

Main category: cs.LG

TL;DR: 提出量化块分解方法(QuBD)扩展算法复杂度估计到任意k元对象，证明其比二值化方法更紧，并用实验揭示神经网络训练中权重复杂度变化规律。


<details>
  <summary>Details</summary>
Motivation: 现有基于CTM和BDM的KCS复杂度估计仅适用于小型二值对象，无法扩展到现代深度神经网络。

Method: 将网络权重量化为有限字母表，然后通过聚合每个位平面的CTM估计来估计KCS复杂度，理论证明比二值化方法估计间隙更小。

Result: 权重复杂度在训练中下降、随数据预算增加、过拟合时上升、在grokking中表现延迟泛化，且与泛化性能相关；算法信息主要存在于最高有效位平面，可用于确定后训练量化水平。

Conclusion: 首次为大型非二值对象（如DNN权重）提供可扩展、可处理的KCS复杂度估计，为理解深度学习机制提供新见解。
...
### [163] [CrystalBoltz: End-to-End Protein Structure Determination via Experiment-Guided Diffusion for X-Ray Crystallography](https://arxiv.org/abs/2605.15564)
*Minseo Kim,Huanghao Mai,Jay Shenoy,Alec Follmer,Gordon Wetzstein,Frederic Poitevin*

Main category: cs.LG

TL;DR: CrystalBoltz是一个生成式框架，将晶体学精修视为原子结构的贝叶斯推断，直接基于结构因子振幅进行操作，实现了实验引导的后验采样，在多个数据集上取得更低的坐标RMSD和R因子，并缩短了33倍的运行时间。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型训练于公共蛋白质结构数据库，但不易依赖于新晶体学实验的测量，限制了它们在X射线结构测定中的应用。晶体学中相位未知，结构测定仍是一个逆问题，需要人工精修。

Method: CrystalBoltz框架通过贝叶斯推断将晶体学精修转化为在结构因子振幅上的操作，从无指导的预训练先验生成转向实验引导的后验采样，随后进行原子坐标和B因子精修。

Result: 在多个蛋白质晶体学数据集上，CrystalBoltz相比最强基线获得了更低的坐标RMSD和R因子，同时运行时间减少了33倍。

Conclusion: CrystalBoltz是一种有效的实验引导精修方法，显著提升了结构测定效率和准确性。
...
### [164] [AstraFlow: Dataflow-Oriented Reinforcement Learning for Agentic LLMs](https://arxiv.org/abs/2605.15565)
*Haizhong Zheng,Yizhuo Di,Jiahui Wang,Shuowei Jin,Xueshen Liu,Yongji Wu,Z. Morley Mao,Ion Stoica,Jiawei Zhao,Beidi Chen*

Main category: cs.LG

TL;DR: AstraFlow是一个面向数据流的强化学习系统，通过解耦组件支持多策略agentic RL和弹性异构计算，实现高效训练。


<details>
  <summary>Details</summary>
Motivation: 现有RL系统在扩展至agentic LLM时面临高昂成本，且难以支持多策略协作训练及弹性异构计算，每次扩展需专门工程改造。

Method: 采用数据流架构，将推出服务、数据流管理和训练解耦为自主组件，替代传统的训练器中心控制。

Result: 在数学、代码、搜索和AgentBench任务中，无需系统代码改动即可支持多策略训练、弹性扩展和异构跨区域执行；多策略协作训练中加速2.7倍，精度相当或更优。

Conclusion: AstraFlow通过原则性组件抽象解决了agentic RL的系统扩展瓶颈，为复杂RL工作负载提供了高效灵活的基础设施。
...
### [165] [Gaussian Relational Graph Transformer](https://arxiv.org/abs/2605.15575)
*Zezhong Ding,Jin Li,Xugang Wang,Xike Xie*

Main category: cs.LG

TL;DR: 提出GelGT，一种高斯关系图变换器，通过结构-语义协作采样和高斯图注意力机制，有效捕捉长程依赖和时序信息，在多项真实数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在消息传递中存在信息衰减，难以捕捉长程依赖；最近的图变换器在联合建模结构、语义和时序信息方面仍有限。

Method: 提出GelGT，包括结构-语义协作采样策略以保留结构连通性并过滤无关语义信息，以及基于采样子图的可学习高斯偏置的高斯图注意力机制，动态编码时序依赖。

Result: 在多个真实世界数据集上，GelGT在下游任务中达到最先进性能，预测性能提升高达13.8%。

Conclusion: GelGT有效解决了关系图学习中的长程依赖和异构信息融合挑战。
...
### [166] [Embracing Biased Transition Matrices for Complementary-Label Learning with Many Classes](https://arxiv.org/abs/2605.15586)
*Tan-Ha Mai,Chao-Kai Chiang,Han-Hwa Shih,Gang Niu,Masashi Sugiyama,Hsuan-Tien Lin*

Main category: cs.LG

TL;DR: 通过设计有偏互补标签生成过程，提出BICL框架，在大量类别数据集上实现超过七倍准确率提升，使互补标签学习适用于大规模分类。


<details>
  <summary>Details</summary>
Motivation: 传统互补标签学习在大量类别场景下性能受限，源于均匀标签生成假设稀释了学习信号。

Method: 提出Bias-Induced Constrained Labeling (BICL)框架，设计非均匀互补标签生成过程，限制标签到子集，并整合数据收集与训练。

Result: 在CIFAR-100和TinyImageNet-200上，BICL比传统方法准确率提升超过七倍。

Conclusion: 有偏互补标签生成可有效克服大规模分类瓶颈，为现实应用提供可行路径。
...
### [167] [Offline Reinforcement Learning with Universal Horizon Models](https://arxiv.org/abs/2605.15603)
*Hojun Chung,Junseo Lee,Songhwai Oh*

Main category: cs.LG

TL;DR: 提出通用视野模型（UHM），通过灵活预测任意视野的未来状态，并采用缩尾视野分布稳定训练，在离线强化学习上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 基于模型的强化学习在离线场景中存在复合误差问题，几何视野模型虽能缓解但难以精确模拟遥远未来状态。

Method: 引入通用视野模型，直接预测任意视野下的未来状态；使用缩尾视野分布截断过大视野以稳定训练。

Result: 在100个OGBench任务上，尤其是次优数据集和长视野任务中，表现优于竞争基线。

Conclusion: UHM有效提升了离线强化学习的长期推理能力，验证了灵活视野预测与分布截断策略的有效性。
...
TL;DR: 提出向量引导策略优化（VSPO），通过调整引导强度生成多样化的轨迹，解决多目标优化中的稀疏奖励问题。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型需同时优化准确性和行为偏好（如冗长度），但基础模型目标行为稀疏，形成奖励瓶颈。

Method: 修改GRPO，对不同的引导强度采样轨迹，通过在线潜在自蒸馏使模型内化引导向量，增加罕见行为采样，丰富多样性。

Result: 在数学推理等基准上，VSPO在保持或提升准确率的同时，对目标行为的控制优于奖励塑造、教师蒸馏等基线。

Conclusion: VSPO在理论上具有更优的迭代复杂度，能有效缓解稀疏奖励问题，加速策略优化。
...
### [169] [Transformer-like Inference from Optimal Control](https://arxiv.org/abs/2605.15608)
*Aditya Kudre,Heng-Sheng Chang,Prashant G. Mehta*

Main category: cs.LG

TL;DR: 从最优控制理论导出仅解码器变换器的推理架构，恢复其层操作。


<details>
  <summary>Details</summary>
Motivation: 解码器变换器计算条件概率，但缺乏理论解释，本文旨在用最优控制理论推导其架构。

Method: 针对非线性离散值和线性高斯两类模型，将预测目标重写为最优控制问题，求解得到双滤波器算法，层结构类似解码器变换器。

Result: 数值实验对比最优控制与训练变换器的注意力权重，发现嵌入维数不足时变换器隐含利用非马尔可夫性。

Conclusion: 最优控制理论可解释变换器架构，并揭示低维嵌入下的非马尔可夫行为。
...
### [170] [Position: Zeroth-Order Optimization in Deep Learning Is Underexplored, Not Underpowered](https://arxiv.org/abs/2605.15622)
...
TL;DR: 本文指出零阶优化在深度学习中被低估，通过分析其局限性并提出六个观点，强调其在大规模、系统友好型训练中的潜力。


<details>
  <summary>Details</summary>
Motivation: 零阶优化因估计方差和查询复杂度常被认为不可扩展，但本文认为这是开发策略短视所致，需要重新评估其能力。

Method: 从算法、系统和评估三个层面提出六点立场，包括通过方差控制、子空间和谱视角、前向计算优势等重新审视零阶优化的可行性。

Result: 识别出三个被忽视的机会：子空间方差缩减、前向计算带来的系统优势、以及去混淆评估的必要性。

Conclusion: 应重新思考零阶优化，围绕其独特优势发展，以实现大规模、系统高效、资源节约的学习。
...
### [171] [IO-SVD: Input-Output Whitened SVD for Adaptive-Rank LLM Compression](https://arxiv.org/abs/2605.15626)
*Ali Abbasi,Chayne Thrash,Haoran Qin,Hamed Pirsiavash,Soheil Kolouri*

Main category: cs.LG

TL;DR: 提出IO-SVD后训练压缩方法，通过KL感知的双侧白化和异构秩分配，在最小性能损失下实现模型压缩和推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有SVD压缩方法依赖输入侧白化或同质秩分配，在高压缩率下难以保持模型质量。

Method: 构建输入输出双侧白化空间，基于KL损失二阶展开计算输出侧敏感度，结合异构秩分配与损失感知量化策略。

Result: 在多种LLM和VLM上实验表明，IO-SVD能以最小性能下降压缩模型，并带来实际推理加速。

Conclusion: IO-SVD是一种有效的后训练压缩方法，适用于资源受限和延迟敏感场景。
...
### [172] [Perforated Neural Networks for Keyword Spotting](https://arxiv.org/abs/2605.15647)
*Vishy Gopal,Aris Ilias Goutis,Ralph Crewe,Erin Yanacek,Rorry Brenner*

Main category: cs.LG

TL;DR: 提出了一种在边端机器学习中应用穿孔反向传播的方法，通过添加人工树突节点到卷积神经网络中，在关键字识别任务上实现了模型精度与参数数量的双赢。


<details>
  <summary>Details</summary>
Motivation: 边端机器学习面临严格的内存、计算和精度约束，现有压缩技术难以同时提升准确率和减小模型大小。

Method: 在Edge Impulse平台上，对标准CNN添加人工树突节点，应用穿孔反向传播进行训练，并进行了800次超参数试验。

Result: 最佳树突模型仅用1,500参数即达到0.933测试精度，而基线模型需约4,000参数达到0.921精度。
...
### [173] [Towards Code-Oriented LM Embeddings for Surrogate-Assisted Neural Architecture Search](https://arxiv.org/abs/2605.15649)
*Pranav Somu,Advay Balakrishnan,Stepan Kravtsov,Aaron McDaniel,Jason Zutty*

Main category: cs.LG

TL;DR: 提出COLE方法，利用预训练语言模型将架构表示为代码文本，无需微调即可作为NAS性能预测器的特征提取器，提升了搜索效率。


<details>
  <summary>Details</summary>
Motivation: 现有NAS代理需要昂贵的微调或复杂表征工程，希望低成本嵌入策略消除这些开销。

Method: 将架构表示为PyTorch类定义文本，使用现成语言模型提取特征，再通过轻量级回归头构建性能预测器（COLE）。

Result: 在NAS-Bench-201和einspace上，原始代码输入比ONNX等文本编码预测性能更好；在CIFAR-100优化中，使用COLE降低34%评估预算即可达到最优架构的1%精度内。

Conclusion: COLE是一种通用高效的NAS基础方法，任何架构均可表示为代码，拓展了NAS的应用前景。
...
### [174] [Sharp Spectral Thresholds for Logit Fixed Points](https://arxiv.org/abs/2605.15651)
*Tongxi Wang*

Main category: cs.LG

TL;DR: 证明了软最大反馈系统稳定性的精确阈值条件，扩展了稳定性边界。


<details>
  <summary>Details</summary>
Motivation: 经典理论仅保证强随机化下的稳定性，忽视了更大稳定区域。

Method: 针对有限维仿射logit系统，推导出尖锐的维数无关欧几里得阈值β‖ΠWΠ‖_{T→T}<2。

Result: 新的阈值条件取代了先前保守的条件，揭示了预分岔稳定区间。

Conclusion: 该结果扩展了稳定性保证，并识别了系统真正发生相变的位置。
...
### [175] [On the Power of Adaptivity for $\varepsilon$-Best Arm Identification in Linear Bandits](https://arxiv.org/abs/2605.15663)
*Arnab Maiti,Yunbei Xu,Kevin Jamieson*

Main category: cs.LG

TL;DR: 研究了线性老虎机中epsilon最佳臂识别的极小样本复杂度，给出了非自适应方法及其下界，发现适应性在某些动作集上只有对数改进，但在构造的动作集上有多项式改进。


<details>
  <summary>Details</summary>
Motivation: 确定在未知线性奖励向量下，从紧凑行动集输出近似最优臂所需的最小样本数，并探究适应性采样能否带来显著改进。

Method: 提出了非自适应固定设计方法，以及自适应采样方法，包括一个用于估计l2范数的自适应子程序。

Result: 非自适应方法样本复杂度为O(d log(1/delta)/epsilon^2 + w(X)^2/epsilon^2)并匹配下界；适应性在超立方体、l2球、m-集和多任务多臂老虎机中仅带来对数改进，但在构造的动作集上带来多项式改进。

Conclusion: 适应性的收益依赖于动作集的结构：存在某些集合（如构造的）使适应性显著优于非自适应，但许多自然集合中适应性增益有限。
...
### [176] [Rule2DRC: Benchmarking LLM Agents for DRC Script Synthesis with Execution-Guided Test Generation](https://arxiv.org/abs/2605.15669)
*Jinuk Kim,Junsoo Byun,Donghwi Hwang,Seong-Jin Park,Hyun Oh Song*

Main category: cs.LG

TL;DR: 提出了Rule2DRC基准，包含1000条规则和13921个布局，用于评估LLM生成DRC脚本的正确性，并引入SplitTester通过执行反馈提升脚本选择性能。


<details>
  <summary>Details</summary>
Motivation: 现有DRC脚本生成基准规模小且评估依赖代码相似性而非执行正确性，缺乏有效的执行反馈机制。

Method: 构建大规模基准Rule2DRC，提供基于DRC执行结果的评估流程；设计SplitTester测试智能体，利用执行反馈生成判别性测试用例以区分候选脚本。

Result: SplitTester显著提升了Best-of-N脚本选择的性能，通过执行反馈提高了功能性正确性。

Conclusion: Rule2DRC和SplitTester有效促进了DRC脚本的自动生成与调试，为研究界提供了可靠的评估工具。
...
### [177] [Interaction-Aware Influence Functions for Group Attribution](https://arxiv.org/abs/2605.15675)
*Jaeseung Heo,Kyeongheung Yun,Youngbin Choi,Sehyun Hwang,Jungseul Ok,Dongwoo Kim*

Main category: cs.LG

TL;DR: 提出一种交互感知的影响函数，通过二阶展开捕捉示例间交互，在模型重训练跟踪和数据选择中表现更优。


<details>
  <summary>Details</summary>
Motivation: 标准影响函数求和忽略示例间冗余或互补的交互，无法准确估计组级影响。

Method: 将目标函数展开至二阶，估计个体影响和成对交互项。

Result: 在六个数据集-模型对和Llama-3.1-8B指令微调数据选择中，优于一阶基线和表示相似性基线。

Conclusion: 交互感知影响函数有效捕捉示例间交互，提升影响估计和下游任务性能。
...
### [178] [FRWKV+: Adaptive Periodic-Position Branch Interaction for Frequency-Space Linear Time Series Forecasting](https://arxiv.org/abs/2605.15690)
*Qingyuan Yang,Dongyue Chen,Da Teng,Junhua Xiao,Jiaji Pan,Shizhuo Deng*

Main category: cs.LG

TL;DR: 提出FRWKV+模型，通过选择性周期性位置分支交互提升长时序列预测的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 频率空间预测中，复数频谱的实部和虚部信息交换不足，且周期性位置线索仅在可靠时有效，需解决这两个问题。

Method: 引入跨分支门控交换实部和虚部上下文，使用自适应相位门控提取周期性位置信息并生成带符号修正，通过自适应信任机制控制修正强度。

Result: FRWKV+在匹配协议下取得家族变体中最高的MSE获胜覆盖率，在特定周期性场景中显著提升。

Conclusion: 组件分析验证了周期性位置上下文、符号修正和自适应信任的有效性，同时揭示了简单修正规则仍适用的边界情况。
...
### [179] [SEED: Targeted Data Selection by Weighted Independent Set](https://arxiv.org/abs/2605.15691)
*Yuan Zhang,Lifeng Guo,Junwen Pan,Chang Liu,Wenzhao Zheng,Kuan Cheng,Kurt Keutzer,Shanghang Zhang*

Main category: cs.LG

TL;DR: 提出SEED方法，通过加权独立集在相似图上进行数据选择，兼顾质量和多样性。


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法难以平衡样本质量与多样性，且存在节点权重不可靠和跨域图不平衡问题。

Method: 将数据选择建模为加权独立集问题，并引入节点值校准（在双边显著子空间估计影响力）和局部尺度归一化（自适应边缘阈值）两个改进。

Result: SEED在指令微调、视觉指令微调和语义分割任务上均优于现有方法。

Conclusion: SEED是一种鲁棒且可扩展的数据选择框架。
...
### [180] [Tighter Regret Bounds for Contextual Action-Set Reinforcement Learning](https://arxiv.org/abs/2605.15692)
*Zijun Chen,Zihan Zhang*

Main category: cs.LG

TL;DR: 研究了固定奖励和转移函数但每集动作集可变的强化学习，将MVP算法扩展至此框架，得到了对抗性和随机上下文下的遗憾界及样本复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决现实场景中每集允许动作集可能变化的问题，需要有效的学习算法。

Method: 扩展MVP算法以适应每集观察到的动作上下文，利用上下文信息进行决策。

Result: 对抗性上下文遗憾界为Õ(√(SAH^3K log L))，随机上下文为Õ(√(SAH^3K))，固定上下文样本复杂度为Õ(SAH^3/ε^2)，以及间隙相关遗憾界。

Conclusion: MVP算法在此框架下具有强理论保证，能有效处理动作集变化，且间隙相关界可改进极小极大率。
...
### [181] [Going Beyond the Edge: Distributed Inference of Transformer Models on Ultra-Low-Power Wireless Devices](https://arxiv.org/abs/2605.15694)
*Alexander Gr\"afe,Ding Huo,Johannes Berger,Marco Zimmerling,Sebastian Trimpe*

Main category: cs.LG

TL;DR: 提出CATS框架，首次在超低功耗无线设备上实现分布式Transformer推理，支持多设备协作执行远超单设备能力的模型。


<details>
  <summary>Details</summary>
Motivation: Transformer模型的计算和内存需求远超单个超低功耗物联网设备的能力。

Method: 采用通信感知的分布式Transformer推理方案，包括剪枝通信原语SomeGather（选择性广播激活列以减少带宽和RAM使用）、基于此原语的分区方法以及训练中的消息丢弃机制以增强鲁棒性。

Result: 在多达16个设备上部署，协作执行比单个设备大14倍的Transformer模型。

Conclusion: 首次将分布式Transformer推理引入超低功耗无线设备领域。
...
### [182] [AGOP-IxG: A Gradient Covariance Filter for Local Feature Attribution on Tabular Data, with a Controlled Benchmark](https://arxiv.org/abs/2605.15700)
*Raj Kiran Gupta Katakam*

Main category: cs.LG

TL;DR: 提出AGOP-IXG方法，在局部归因准确性和速度上优于SHAP等基线，但全局忠实度与其他方法相当。


<details>
  <summary>Details</summary>
Motivation: 现有特征归因方法缺乏基于真实标签的评估，通常按惯例选择，需要可衡量忠实度的归因方法。

Method: 提出AGOP-IXG，将每个样本的梯度乘以截断后的平均梯度外积矩阵（top-K秩），并在三个合成任务（线性、稀疏非线性、交互）和两个真实数据集（成人收入、信用卡违约）上评估。

Result: 在合成数据上，AGOP-IXG在Spearman秩相关、噪声特征质量和交互数据上的top-k精度领先；计算速度比SHAP快350-1650倍。在真实数据上，全局忠实度（ROAR协议）差异在约1.7%相对AUC内。

Conclusion: AGOP-IXG针对局部逐样本归因优化，速度快，但全局特征排序性能与基线相当。
...
### [183] [Differentiable Mixture-of-Agents Incentivizes Swarm Intelligence of Large Language Models](https://arxiv.org/abs/2605.15706)
*Xingjian Wu,Junkai Lu,Siyu Yan,Xiangfei Qiu,Jilin Hu,Chenjuan Guo,Bin Yang*

Main category: cs.LG

TL;DR: 提出可微分混合智能体框架，实现推理时弹性自适应用户交互。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统依赖固定通信拓扑，缺乏对动态任务需求的灵活性。

Method: 设计可微分上下文感知路由机制，利用循环结构融合历史信息，步骤级稀疏激活，并以预测熵作为自监督信号优化路由。

Result: 在9个基准上取得最先进性能，同时展现强效率、鲁棒性和集成能力。

Conclusion: 该方法可高效自适应推理，无需外部标注。
...
### [184] [Bidirectional Fusion Guided by Cardiac Patterns for Semi-Supervised ECG Segmentation](https://arxiv.org/abs/2605.15722)
*Jeonghwa Lim,Minje Park,Sunghoon Joo*

Main category: cs.LG

TL;DR: 提出CardioMix框架，利用双向CutMix和心脏模式引导，解决心电图分割中标注数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 心电图标注数据稀缺，传统半监督方法未充分利用标记与未标记数据间的信息交换。

Method: 基于心脏模式引导的双向CutMix策略，增强标记集并加强未标记集监督。

Result: 在SemiSegECG基准上，CardioMix作为即插即用模块，一致优于现有融合策略。

Conclusion: CardioMix有效提升半监督心电图分割性能，兼容多种算法。
...
### [185] [GOMA: Toward Structure-Driven Multimodal Alignment from a Graph Signal Smoothing Perspective](https://arxiv.org/abs/2605.15723)
*Xu Wang,Xunkai Li,Yinlin Zhu,Rong-Hua Li,Guoren Wang*

Main category: cs.LG

TL;DR: 提出GOMA框架，利用多模态属性图的结构信息后对齐冻结的多模态嵌入，在直推式检索中达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP式双编码器忽略实体间关系上下文，直接在图传播中易过平滑，需同时打破模态拓扑屏障、控制平滑深度。

Method: 将冻结嵌入视为图信号，学习模态感知传播算子，执行有限步耦合平滑，并自适应读出节点特定平滑轨迹。

Result: 在7个MAG基准上，GOMA取得SOTA或并列SOTA，且稳定性优于最强图竞争者。

Conclusion: MAG结构可作为冻结多模态嵌入的有效后编码器，提升检索性能。
...
### [186] [A Unified Perturbation Framework for Analyzing Leaderboard Stability and Manipulation](https://arxiv.org/abs/2605.15761)
*Hosna Oyarhoseini,Jimmy Lin,Amir-Hossein Karimi*

Main category: cs.LG

TL;DR: 本研究提出基于影响力近似的统一扰动框架，分析Bradley-Terry排行榜的鲁棒性。发现少于1%的针对性扰动即可改变排名前三模型、降低Kendall's tau和置信区间，表明现代排行榜对微小扰动不鲁棒。影响力分数还可用于高效定向操纵模型排名。


<details>
  <summary>Details</summary>
Motivation: 评估排行榜（如LMArena）在基准测试大语言模型中至关重要，但其排名的鲁棒性尚不明确。

Method: 提出基于影响力近似的统一扰动框架，研究三种比赛级扰动（删除、添加、翻转）和选手移除，评估对top-k成员、全局排名一致性（Kendall's tau）和置信区间不确定性的影响。

Result: 在Chatbot Arena等数据集上，少于1%的针对性扰动即可改变排名第一模型，降低Kendall's tau，改变置信区间。影响力分数可实现高效定向操纵，促进或降级特定模型，减少不确定性。

Conclusion: 通过归一化数据集级鲁棒性分数，该框架为审计排行榜稳定性提供了实用工具，并推动更稳健的评估协议。
...
### [187] [Continual Learning of Domain-Invariant Representations](https://arxiv.org/abs/2605.15775)
*Pascal Janetzky,Tobias Schlagenhauf,Stefan Feuerriegel*

Main category: cs.LG

TL;DR: 本文提出了一种持续学习方法，通过学习并保持跨领域的域不变表示，以提高对未见目标域的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法往往学习到特定于领域的虚假特征（捷径学习），限制了部署后对未见领域的泛化能力。

Method: 结合重放训练和序列不变性对齐，学习并保持随时间变化的域不变结构。

Result: 在多个基准和真实世界数据集上，该方法在未见过目标域的泛化上优于现有持续学习基线。

Conclusion: 本文首次开发了用于持续学习的域不变表示方法，并证明了其有效性。
...
### [188] [Grokking as Structural Inference: Transformers Need Bayesian Lottery Tickets](https://arxiv.org/abs/2605.15787)
*Kai Hidajat,Solden Stoll,Joseph An*

Main category: cs.LG

TL;DR: 该论文将注意力机制形式化为任务依赖图的贝叶斯后验，证明泛化需要MLP容量和注意力的结构条件，并指出延迟泛化源于结构延迟推理，可通过KL干预加速。


<details>
  <summary>Details</summary>
Motivation: 现有解释未能充分考虑注意力模型的独特约束：若注意力丢弃信息性token，下游计算无法恢复。本文旨在揭示Transformer记忆后延迟泛化的根本原因。

Method: 形式化注意力为隐式贝叶斯后验，分解泛化条件为MLP容量的Goldilocks界限和注意力的贝叶斯结构条件，并通过KL散度设计结构干预。

Result: 推导出逆权重衰减的结构等待时间；提出KL结构干预可绕过延迟，得到逆干预强度缩放律；实验表明贝叶斯票在算法序列任务上匹配或优于彩票票。

Conclusion: 延迟泛化是注意力结构延迟推理的结果，通过解耦注意力和记忆可加速泛化，贝叶斯票为理解泛化与记忆的关系提供了新视角。
...
### [189] [Learning Context-conditioned Gaussian Overbounds for Convolution-Based Uncertainty Propagation](https://arxiv.org/abs/2605.15789)
*Ruirui Liu,Xuejie Hou,Yiping Jiang,Hui Ren*

Main category: cs.LG

TL;DR: 提出一个统一的学习框架，训练神经网络产生上下文感知的高斯过界，在保证保守性的同时比传统方法更紧凑。


<details>
  <summary>Details</summary>
Motivation: 安全关键场景（如自动驾驶、航空）需要保守的边界而非点估计，传统高斯过界全局化且难以适应特征条件误差。

Method: 训练神经网络输出均值和尺度，通过有限分位数网格强制保守性，并加入Wasserstein项惩罚分布距离，在假设下实现连续尾部保守性。

Result: 在合成数据和真实数据集（多径、电离层、对流层残差）上，方法在保持保守性的同时得到更紧的边界。
...
### [190] [AOT-POT: Adaptive Operator Transformation for Large-Scale PDE Pre-training](https://arxiv.org/abs/2605.15793)
*Qitan Lv,Hong Wang,Zhongkai Hao,Wen Wu,Xuenan Xu,Bowen Zhou,Feng Wu,Chao Zhang*

Main category: cs.LG

TL;DR: 提出自适应算子变换方法AOT-POT，通过将复杂多样的PDE解算子转化为统一形式，实现高效多PDE预训练，在12个基准上平均降低40.9%的L2误差。


<details>
  <summary>Details</summary>
Motivation: 多PDE预训练中解算子结构多样且复杂，现有方法仅通过增大模型容量应对，缺乏对算子本身的简化。受数值分析启发，希望自适应地变换算子以降低联合建模难度。

Method: AOT-POT扩展隐藏表示为多个并行流，在每层前后自适应聚合和重分布信息，并利用Sinkhorn投影的双随机矩阵稳定地混合流，从而将不同PDE的解算子重塑为统一形式。

Result: 在12个PDE基准上，仅增加3%参数，相对L2误差降低最多77.6%（平均40.9%）；微调后域内误差降低达92%，域外（未见PDE类型）降低89%。

Conclusion: 自适应算子变换是超越单纯扩大模型规模的有效方向，为构建PDE基础模型提供了互补且高效的思路。
...
### [191] [Martingale Neural Operators: Learning Stochastic Marginals via Doob-Meyer Factorization](https://arxiv.org/abs/2605.15806)
*Kai Hidajat*

Main category: cs.LG

TL;DR: 基于Doob-Meyer定理提出Martingale神经算子(MNO)，直接学习随机PDE的末端条件均值和协方差，实现高效不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 现有神经算子应用于随机PDE时退化为条件均值，丢失方差和尾结构；恢复结构需蒙特卡洛或生成模型，牺牲单次效率和分辨率不变性。

Method: 利用Doob-Meyer定理将半鞅分解为漂移和鞅，架构上参数化漂移-like均值及低秩因子B_phi(保证B_phi^T B_phi半正定)，以高斯残差实例化，直接映射初始条件到末端条件均值和协方差。

Result: 在1D SPDE、粗糙波动率、2D任务中，Wasserstein距离降低至多120倍(phi^4)和68倍(随机Burgers)，评估速度比条件扩散基线快约3倍；2D任务零样本分辨率迁移与湍流上媲美FNO。

Conclusion: MNO在随机PDE上实现高效、一次性的不确定性量化，但对准确定性系统(如Gray-Scott)仍需改进。
...
### [192] [Intrinsic Wasserstein Rates for Score-Based Generative Models on Smooth Manifolds](https://arxiv.org/abs/2605.15822)
*Guoji Fu,Taiji Suzuki,Wee Sun Lee,Atsushi Nitanda*

Main category: cs.LG

TL;DR: 证明方差保持得分生成模型在紧凑d维光滑流形上达到内在Wasserstein-1样本指数，并显式分离几何、Hölder半径、密度下界和环境依赖。


<details>
  <summary>Details</summary>
Motivation: 得分生成模型在高维环境空间中训练，但数据分布在低维非线性结构上，需要分析其样本复杂度。

Method: 将得分近似分为大噪声切细胞区和小噪声投影中心、去高斯化拉普拉斯区；通过有限内在锚点和Gauss-Newton迭代的ReLU实现最近投影坐标，避免将流形投影视为黑箱高维光滑映射。

Result: 获得内在Wasserstein-1样本指数 ~ O(D^{O_β(d)} n^{-(β+1)/(d+2β)})，含对数因子和显式几何密度因子。
...
### [193] [From Observed Viability to Internal Predictive Approximation: A Single-Subject Latent-Space Analysis of Gait Dynamics Under Occlusal Constraint](https://arxiv.org/abs/2605.15862)
*Jacques Raynal,Pierre Slangen,Elsa Raynal,Jacques Margerit*

Main category: cs.LG

TL;DR: 使用机器学习模型近似帕金森患者步态纵向潜在变换。


<details>
  <summary>Details</summary>
Motivation: 研究观察到的步态纵向变化是否能在预测性潜在空间框架内近似，不声称临床预测或因果咬合效应。

Method: 单个帕金森患者，使用鞋垫记录步态，六种咬合观察探针，主成分分析构建潜在表示，前馈神经网络训练近似变换。

Result: 模型保持了三种中心关系条件的位移层次，扩展分析也保持了全局结构，但条件依赖的近似变异性。

Conclusion: 仅支持观察到的纵向潜在变换可以在单受试者数据集中内部近似的有限结论，为未来多受试者预测模型提供方法性桥梁。
...
### [194] [Ti-iLSTM: A TinyDL Approach for Logic-Level Anomaly Detection in Industrial Water Treatment Systems](https://arxiv.org/abs/2605.15874)
*Mandar Joshi,Farzana Zahid,Judy Bowen,Matthew M.Y. Kuo,Valeriy Vyatkin,Emil Karlsson*

Main category: cs.LG

TL;DR: 提出基于TinyDL的增量LSTM模型（Ti-iLSTM），在资源受限的PLC上实现轻量级逻辑层异常检测，在SWaT数据集上F1=0.983，ROC-AUC=0.998，并在WADI数据集上验证了泛化性。


<details>
  <summary>Details</summary>
Motivation: 工业水处理系统面临逻辑层欺骗异常，传统阈值监测或服务器端模型难以在资源受限的PLC上部署。

Method: 采用TinyDL技术优化LSTM的内存和空间占用，提出Ti-iLSTM框架，结合逻辑感知监督和序列学习进行轻量级异常检测。

Result: 在SWaT数据集上F1=0.983，ROC-AUC=0.998；WADI数据集验证了跨数据集适用性。

Conclusion: 逻辑感知监督与TinyDL序列学习结合可高效准确地在资源受限PLC上实现异常检测。
...
### [195] [Shapley Neuron Values for Continual Learning: Which Neurons Matter Most?](https://arxiv.org/abs/2605.15877)
*Mohammad Ali Vahedifar,Abhisek Ray,Qi Zhang*

Main category: cs.LG

TL;DR: 基于Shapley神经元价值（SNV）的持续学习方法，通过冻结重要神经元来防止灾难性遗忘，无需缓冲区或扩展架构。


<details>
  <summary>Details</summary>
Motivation: 神经网络在持续学习中存在灾难性遗忘问题，即学习新任务会降低旧任务性能。

Method: 提出Shapley神经元价值（SNV）框架，利用合作博弈论量化神经元重要性，选择性冻结重要神经元以保持知识，其余保持可塑性。

Result: 在ImageNet-1k上，SNV在类增量学习准确率提升2.88%，任务增量学习提升6.46%，优于现有无缓冲区方法。

Conclusion: SNV是一种有效的无缓冲区持续学习方法，无需扩展架构即可缓解灾难性遗忘。
...
### [196] [Practical Validity Conditions for Byzantine-Tolerant Federated Learning](https://arxiv.org/abs/2605.15887)
*M\'elanie Cambus,Darya Melnyk,Tijana Milentijevi\'c,Stefan Schmid*

Main category: cs.LG

TL;DR: 提出最小包围球（MEB）有效性条件及其松弛版本，作为拜占庭容错联邦学习中凸有效性的实用替代，并给出最优聚合规则和理论保证。


<details>
  <summary>Details</summary>
Motivation: 凸有效性条件虽理论强大，但具有维度依赖弹性且排除许多实际聚合规则，不适用于现代联邦学习系统。

Method: 引入MEB有效性及其乘法松弛c-MEB有效性，基于多数诚实假设（n>2t）提出最优MinMax-MEB规则，并分析标准聚合器的松弛MEB保证。

Result: 精确MEB有效性仍有限弹性，松弛c-MEB在n>2t时可实现；MinMax-MEB的松弛界c<√2；给出最小直径平均、medoid和几何中位数的显式松弛MEB保证；系统化几何有效性条件图谱。

Conclusion: 松弛MEB有效性将分布式计算中的有效性条件与拜占庭容错聚合规则联系起来，是凸有效性的实用替代方案。
...
TL;DR: 提出CHoE方法，通过结构条件专家和结构感知路由机制，解决异构图表征学习在跨域场景下的性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 现有异构图表征学习方法主要针对域内场景，实际应用常跨域且数据分布不同，导致性能下降。

Method: 预训练阶段引入结构条件专家；提示调优阶段采用结构感知专家路由和负载均衡机制选择专家；设计基于提示的语义融合模块整合多视图表示。

Result: 在少样本跨域应用中，CHoE持续提升性能，优于所有基线方法。

Conclusion: CHoE有效扩展了异构图表征学习到跨域场景，通过专家网络和路由机制增强了泛化能力。
...
### [198] [From Layers to Networks: Comparing Neural Representations via Diffusion Geometry](https://arxiv.org/abs/2605.15901)
*Atharva Khandait,Jan E. Gerken*

Main category: cs.LG

TL;DR: 将扩散几何应用于神经表征，首次引入多视角学习工具，通过构建马尔可夫矩阵的等价形式，提出了多尺度和跨层融合的相似性度量，在多个基准上取得最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有表征相似性度量缺乏多尺度分析能力，且无法有效融合多层信息。

Method: 基于表征相似性矩阵（RSM）与行随机马尔可夫矩阵的等价关系，利用转移矩阵的t次幂实现多尺度探测，并通过交替扩散融合多层的马尔可夫矩阵以捕捉网络的联合样本几何。

Result: 在ReSi基准测试（14种架构、7个数据集、3个领域）中，语言和视觉任务的准确率和输出相关性均达到最优，且在分布外数据上同样表现最佳。

Conclusion: 扩散几何为神经表征分析提供了新的多尺度和跨层融合工具，有效提升了相似性度量的性能。
...
### [199] [Context-aware Entity-Relation Extraction for Threat Intelligence Knowledge Graphs](https://arxiv.org/abs/2605.15904)
*Inoussa Mouiche,sherif Saad*

Main category: cs.LG

TL;DR: 提出CTiKG框架，通过混合NLP模型和领域本体提升威胁实体与关系抽取的准确性，在多个数据集上取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有构建网络安全知识图谱的方法因误差传播导致抽取精度低且泛化能力差，亟需更准确的方法。

Method: CTiKG采用流水线架构，集成SecureBERT+上下文嵌入和领域本体知识的混合NLP模型，以减少分类错误和级联误差。

Result: 在DNRTI-AUG-STIX2数据集上，NER性能提升3-4%，RE性能提升最高8%；在DNRTI和STUCCO基准上验证了鲁棒性。

Conclusion: CTiKG框架有效提升了威胁情报实体关系抽取的准确性和实用性，并公开了数据集以促进可重复研究。
...
### [200] [LoCO: Low-rank Compositional Rotation Fine-tuning](https://arxiv.org/abs/2605.15916)
*An Nguyen,Jaesik Choi,Anh Tong*

Main category: cs.LG

TL;DR: 提出了一种低秩组合正交微调方法（LoCO），通过低秩斜对称矩阵和组合旋转链构建正交变换，在保持参数效率的同时保留预训练表示的几何结构。


<details>
  <summary>Details</summary>
Motivation: 现有PEFT方法（如低秩适应）虽参数高效，但难以保持预训练表示的几何结构。

Method: 使用低秩斜对称矩阵和组合旋转链构造正交变换，并通过近似方案实现完全并行计算组合旋转。

Result: 在扩散Transformer、视觉Transformer和语言模型微调中表现出优越或竞争性性能。

Conclusion: LoCO在保持低计算复杂度和受控近似误差的同时，实现了正交性，并在多个领域验证了有效性。
...
### [201] [A Retrieval-Enhanced Transformer for Multi-Step Port-of-Call Sequence Prediction in Global Liner Shipping](https://arxiv.org/abs/2605.15937)
*Yanzhao Su,Fang He,Yineng Wang*

Main category: cs.LG

TL;DR: 提出一种连接约束与检索增强的深度学习框架，用于多步港口序列预测，在全局数据集上达到72.3%的第一步准确率和61.4%的三步平均准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以处理不可靠的航程时间表，且AIS数据无法提供超出下一港口的能见度。

Method: CCRE框架结合检索增强的历史编码器（查询全球数据库获取相似航行先例）、Transformer轨迹编码器，通过交叉注意力实现自适应中间融合，并采用自回归解码器结合调度采样和Gumbel-Softmax松弛进行联合序列生成，同时使用拓扑掩码强制执行海事网络可达性。

Result: 在全局数据集上，CCRE的第一步准确率为72.3%，三步平均准确率为61.4%，分别比CatBoost和LSTM平均高出12.6%和11.3%。

Conclusion: CCRE框架能有效捕捉复杂路由模式，具有可扩展性，优于现有基线方法。
...
### [202] [When and Why Adversarial Training Improves PINNs: A Neural Tangent Kernel Perspective](https://arxiv.org/abs/2605.15959)
*Yuan-dong Cao,Chi Chiu SO,Jun-Min Wang,He Wang*

Main category: cs.LG

TL;DR: 提出一种基于生成对抗网络（GAN）的对抗训练分析框架，用于改进物理信息神经网络（PINN）的训练，解决频谱偏差和刚度问题，并显著提高精度。


<details>
  <summary>Details</summary>
Motivation: PINN训练困难，尤其在高频或多尺度问题上，现有对抗训练虽有效但机制不明。

Method: 分析GAN中鉴别器对PINN训练动态的影响，提供理论依据，统一分析GAN变体，并设计新算法。

Result: 新方法显著减少PINN训练病理，模型性能优于现有方法，精度常高出几个数量级。

Conclusion: 对抗训练能有效改善PINN训练，所提框架提供了理论支撑和实用算法。
...
TL;DR: 提出基于平均表示熵的信息论准则来分类VAE的极化现象，并揭示其局限性：仅靠平均熵无法区分活跃与混合维度。实验表明该准则能一致地恢复极化状态，且被动维度在适当归一化后对下游任务有微小改进。


<details>
  <summary>Details</summary>
Motivation: VAE中潜在变量存在极化现象，但现有识别活跃维度的准则依赖高斯先验，限制了适用性。

Method: 提出基于平均表示熵的信息论分类，通过熵-方差界将熵与KL最小化耦合，并关联到Bonheme的活跃/被动条件。

Result: 在β-VAE、可识别VAE、最小体积自编码器和L2正则自编码器上，熵准则一致地恢复了极化状态。被动维度在归一化后能带来下游任务的微小改进。

Conclusion: 熵准则有效但需结合方差信息以区分活跃与混合维度；被动维度并非完全信息丢失，而是尺度问题。
...
### [204] [Constrained latent state modeling: A unifying perspective on representation learning under competing constraints](https://arxiv.org/abs/2605.15995)
*Gwenol\'e Quellec*

Main category: cs.LG

TL;DR: 提出约束潜在状态建模（CLSM）统一视角，识别核心属性及其权衡，将现有方法解释为施加不同约束子集，为设计可解释、鲁棒、任务对齐的潜在状态模型提供框架。


<details>
  <summary>Details</summary>
Motivation: 当前潜在表示学习方法零散，依赖隐含假设，目标函数缺乏约束导致表示歧义。

Method: 提出CLSM，识别预测充分性、最小性、时间一致性等核心属性及其内在权衡，重新审视现有建模家族，将其归入不同约束子集。

Result: 现有方法被解释为占据共同设计空间的不同区域；缺乏可辨识性等问题被重新定义为约束不足的结果。

Conclusion: CLSM提供原则性框架，使设计选择明确、分析权衡，指导更可解释、鲁棒、任务对齐的潜在状态模型开发。
...
### [205] [Accelerated Gradient Descent for Faster Convergence with Minimal Overhead](https://arxiv.org/abs/2605.16017)
*Manuel Graca,L. Miguel Silveira,Arlindo Oliveira,Frank Liu*

Main category: cs.LG

TL;DR: 提出CT-AGD方法，通过有限差分捕捉局部曲率加速一阶方法，在保持准确率的同时平均减少33%的训练周期。


<details>
  <summary>Details</summary>
Motivation: 深度学习训练中非凸优化问题需要加速一阶方法，同时处理随机小批量训练引入的噪声和偏差。

Method: CT-AGD使用有限差分商显式捕捉局部曲率，并开发启发式方法减轻随机小批量训练的噪声和偏差，开销与Adam类似。

Result: 在保持与基线一阶方法相同准确率的前提下，平均减少33%的训练周期。
...
### [206] [Variational Autoregressive Networks with probability priors](https://arxiv.org/abs/2605.16020)
*Piotr Bia{\l}as,Piotr Korcyl,Tomasz Stebel,Dawid Zapolski*

Main category: cs.LG

TL;DR: 通过引入物理先验概率分布，显著提升了深度学习蒙特卡洛方法的训练效率和模拟规模，在Ising模型和自旋玻璃模型中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 传统蒙特卡洛方法在相变附近存在临界慢化问题；深度学习方法虽可缓解，但通用网络架构忽视物理对称性导致训练困难。

Method: 基于已有整合自旋相互作用的策略，提出利用先验概率分布作为训练起点，将物理先验融入模型。

Result: 在Ising模型和Edwards-Anderson自旋玻璃模型中，物理先验减少了训练负担，并能模拟更大系统尺寸。

Conclusion: 从‘空白板’模型转向物理启发的先验模型，能够有效提升离散自旋模型的仿真能力。
...
### [207] [Mind Dreamer: Untethering Imagination via Active Latent Intervention on Latent Manifolds](https://arxiv.org/abs/2605.16030)
*Shaojun Xu,Xiaoling Zhou,Yihan Lin,Yapeng Meng,Xinglong Ji,Luping Shi,Rong Zhao*

Main category: cs.LG

TL;DR: 提出Mind Dreamer框架，通过主动潜在干预打破历史束缚，在模型强化学习中实现更高效的探索与稀疏奖励优化。


<details>
  <summary>Details</summary>
Motivation: 现有基于模型的强化学习受限于从观测状态初始化想象，导致世界模型与策略优化之间的学习不对称，尤其在稀疏奖励场景下效率低下。

Method: 采用主动潜在干预，从学习生成器中采样初始状态而非历史缓存，利用对抗生成器合成非连续潜在跳跃；推导中继值函数与不确定性函数，通过二次折扣γ²实现跨断点的信度分配，并最小化全局中继流形期望自由能。

Result: 在DeepMind控制套件上平均提速1.67倍，稀疏奖励任务中最高提速8.8倍，优于DreamerV3。
...
### [208] [Looped SSMs: Depth-Recurrence and Input Reshaping for Time Series Classification](https://arxiv.org/abs/2605.16048)
*M\'onika Farsang,Ramin Hasani,Daniela Rus,Radu Grosu*

Main category: cs.LG

TL;DR: 提出在状态空间模型中应用深度循环（参数共享）和输入重塑，实验证明两者独立有益，且可叠加提升性能。


<details>
  <summary>Details</summary>
Motivation: SSM中深度循环（跨层重复相同块）和输入重塑两个设计轴未被探索，旨在验证其有效性。

Method: 在四种SSM架构（LRU, S5, LinOSS, LrcSSM）和六个时间序列分类基准上，比较循环SSM（参数k迭代L次）与标准SSM（k·L独立参数），并测试输入重塑（低维拼接时间步，高维重塑特征-时间维度）。

Result: 循环SSM匹配或超越标准SSM；输入重塑带来1-6%准确率提升，两种技术独立且复合增益。

Conclusion: 深度循环和输入重塑是SSM两个未被充分探索的独立设计轴，它们提供正交且有益的归纳偏置和优化简化。
...
### [209] [Ada-Diffuser: Latent-Aware Adaptive Diffusion for Decision-Making](https://arxiv.org/abs/2605.16054)
*Fan Feng,Selena Ge,Minghao Fu,Zijian Li,Yujia Zheng,Zeyu Tang,Yingyao Hu,Biwei Huang,Kun Zhang*

Main category: cs.LG

TL;DR: 提出Ada-Diffuser框架，通过因果扩散模型同时学习观测交互的时间结构和潜在动态，用于决策制定。


<details>
  <summary>Details</summary>
Motivation: 现有生成式决策模型忽略潜在因素及其演化动态，需显式建模以精确模拟动态并有效决策。

Method: 理论证明从短时间观测块可识别潜在过程，构建Ada-Diffuser因果扩散模型，联合学习观测时间结构和潜在动态，并用于规划与控制。

Result: 在模拟控制和机器人基准测试中，Ada-Diffuser实现了准确的潜在推断和自适应策略学习。

Conclusion: 显式建模潜在动态可提升生成式决策模型的性能，Ada-Diffuser在规划和策略学习中表现出有效性。
...
### [210] [SAFE Quantum Machine Learning with Variational Quantum Classifiers](https://arxiv.org/abs/2605.16067)
*Ying Chen,Paolo Giudici,Vasily Kolesnikov,Paolo Recchia*

Main category: cs.LG

TL;DR: 提出了一种结合振幅编码和可学习经典预编码层的变分量子分类器，通过SAFE-AI指标评估，在准确率、鲁棒性和可解释性上表现平衡。


<details>
  <summary>Details</summary>
Motivation: 现有量子机器学习模型在可靠性评估方面缺乏系统性，特别是在安全关键场景下需要同时考虑准确率、鲁棒性和可解释性。

Method: 使用振幅编码将高维深度表示映射到量子态，并加入可学习的经典预编码层稳定嵌入；结合归一化幅度嵌入和有界量子可观测量，形成平滑且结构化的假设类；采用基于Cramér–von Mises散度的SAFE-AI指标进行多维度评估。

Result: 与强经典基线相比，该量子模型预测性能相当，但在SAFE可靠性上更平衡，抗噪性和结构化特征移除下的稳定性更好。

Conclusion: 变分量子电路为面向稳定性的SAFE学习提供了原则性机制，适合安全关键应用。
...
### [211] [ITGPT: Generative Pretraining on Irregular Timeseries](https://arxiv.org/abs/2605.16069)
*Antoine Honor\'e,Ming Xiao*

Main category: cs.LG

TL;DR: 提出ITGPT模型，结合自监督学习和生成式预训练，高效处理不规则采样的多模态时间序列数据，在医疗和预测维护任务上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列回归模型难以充分利用大规模未标记多模态数据，特别是在不规则采样或缺失值情况下，而Transformer模型处理此类数据灵活性不足。

Method: 基于注意力的ITGPT架构，允许同时使用自监督学习损失和GPT风格目标进行训练，无需重采样、特征融合或显式插补。

Result: 在TIHM医疗数据集和CompX预测维护数据集上达到最优性能，且在标签稀少时有效利用未标记数据，优于纯监督方法。

Conclusion: ITGPT是高效利用大规模非结构化时间序列数据进行推理的重要进展。
...
### [212] [MIND: Decoupling Model-Induced Label Noise via Latent Manifold Disentanglement](https://arxiv.org/abs/2605.16081)
*Dayong Ren*

Main category: cs.LG

TL;DR: 提出MIND框架，通过潜在流形解耦处理模型诱导的系统性噪声，在多个基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 模型诱导的噪声是系统性的，与局部特征流形耦合，现有方法难以处理。

Method: 通过潜在流形解耦将高维噪声分解为子空间组件，并利用LDE估计器将样本投影到一致错误模式的潜在簇中。

Result: 在CIFAR-100和3D数据集上显著优于现有方法，并有效纠正视觉语言模型的零样本幻觉。

Conclusion: MIND可作为基础模型稳健蒸馏的有效框架。
...
### [213] [Multi-level Self-supervised Pretraining on Compositional Hierarchical Graph for Molecular Property Prediction](https://arxiv.org/abs/2605.16088)
*Xiayu Liu,Zhengyi Lu,Hou-biao Li*

Main category: cs.LG

TL;DR: MolCHG是一种多层级自监督预训练框架，通过组合层级图引入并行键图，将键信息提升为独立语义层，并在原子、片段、图三个层级设计预训练任务，在多个分子性质预测基准上取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有分子图自监督方法多基于单一结构粒度，将键信息仅视为辅助边属性，未能充分挖掘其独立语义价值。

Method: 提出MolCHG，基于组合层级图将分子结构分为三种语义层（原子、片段、图）和四类节点；引入并行键图使键级信息独立演化；设计三个层级预训练目标：原子-键跨视图对比、片段级官能团预测、图级结构预测。

Result: 在九个MoleculeNet基准上，MolCHG在七个数据集（含分类与回归任务）中取得最佳，其余与最强基线持平。

Conclusion: 多层级监督信号互补，各组件均有助于提升整体性能。
...
### [214] [Centralized vs Decentralized Federated Learning: A trade-off performance analysis](https://arxiv.org/abs/2605.16089)
*Chaimaa Medjadji,Guilain Leduc,Sylvain Kubler,Yves Le Traon*

Main category: cs.LG

TL;DR: 本文使用Fedstellar模拟器、MNIST数据集和MLP分类器，实验比较了集中式、去中心化和半去中心化三种联邦学习架构，分析了各自的优势和性能权衡。


<details>
  <summary>Details</summary>
Motivation: 联邦学习是保护隐私的分布式训练范式，但很少有研究实验比较三种架构的性能和权衡。

Method: 使用Fedstellar模拟器、MNIST数据集和MLP分类器进行实验分析。

Result: 得出了不同架构的性能指标和权衡。

Conclusion: 选择正确的架构取决于应用需求，本实验提供了比较分析。
...
### [215] [Federated Imputation under Heterogeneous Feature Spaces](https://arxiv.org/abs/2605.16099)
*Imane Hocine,Chaimaa Medjadji,Sylvain Kubler,Gregoire Danoy,Yves Le Traon*

Main category: cs.LG

TL;DR: 提出FedHF-Impute框架，利用共享特征图通过消息传递实现跨客户端的知识迁移，解决异构特征空间下的联邦插补问题，在部分重叠特征场景下提升插补精度。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法假设特征模式对齐，但在表格数据中客户端往往只有部分重叠特征子集，参数平均方法在弱重叠或不相交特征组间信息传递有限，限制了联邦插补效果。

Method: FedHF-Impute将结构性特征缺失与传统缺失区分，使用共享全局特征图通过消息传递在统计相关特征间传播信息，实现间接跨客户端知识迁移，同时保持标准联邦通信。

Result: 在SECOM和AirQuality数据集上，相比联邦基线，插补精度（RMSE）分别提升26.9%和8.4%；在PhysioNET上性能相当，仅差0.3%。

Conclusion: FedHF-Impute有效处理特征异构性，通过特征图传播提升联邦插补性能，适用于弱重叠特征场景。
...
### [216] [Attention Dispersion in Dynamic Graph Transformers: Diagnosis and a Transferable Fix](https://arxiv.org/abs/2605.16112)
*Jinhao Zhang,Kangfei Zhao,Qiuhao Zeng,Long-Kai Huang*

Main category: cs.LG

TL;DR: 针对动态图Transformer在时间分布偏移下的注意力分散问题，提出差分注意力机制，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有动态图Transformer在时间分布偏移下性能受限，原因是注意力分散导致无法聚焦关键节点。

Method: 用差分注意力替代标准注意力，抑制共模噪声，增强令牌级独特信号；并构建参考实现DiffDyG。

Result: 在三个基线模型上加入差分注意力后，高偏移数据集性能持续提升；DiffDyG在9个基准和三个负采样协议下达到SOTA。

Conclusion: 差分注意力通过降低注意力熵、增加关键节点注意力权重，有效缓解时间偏移问题。
...
### [217] [Multi-Fidelity Flow Matching: Cascaded Refinement of PDE Solutions](https://arxiv.org/abs/2605.16118)
*Sipeng Chen,Junliang Liu,Hewei Tang,Shibo Li*

Main category: cs.LG

TL;DR: 提出了多保真流匹配（MFFM），一种级联细化框架，用于参数化PDE求解，通过校准源分布和条件化速度网络，实现从低保真到高保真的残差细化。


<details>
  <summary>Details</summary>
Motivation: 条件流匹配中的源分布是可校准的设计参数，而非默认的各向同性先验，利用此特性改进多保真PDE求解。

Method: 校准源到低-高保真残差尺度，使用局部高斯模糊相关，速度网络条件化于低保真解；多分辨率级联独立处理相邻保真度；级联流匹配预训练后，端到端微调，使用确定性单步展开，推理时每级一次速度评估。

Result: 在八个基准上验证，包括超分辨率和时空预测任务，达到类似多重网格细化的效果，每次查询需L次确定性网络评估。

Conclusion: MFFM是一种有效的多保真PDE求解方法，通过校准源和条件化设计，显著简化残差细化问题，并实现高效推理。
...
### [218] [Navigating Potholes with Geometry-Aware Sharpness Minimization](https://arxiv.org/abs/2605.16134)
*Simon Dufort-Labb\'e,Mehrab Hamidi,Razvan Pascanu,Ioannis Mitliagkas,Damien Scieur,Aristide Baratin*

Main category: cs.LG

TL;DR: 提出LLQR+SAM，结合SAM与学习预条件器，通过双时间尺度结构利用损失几何信息，在视觉和序列建模任务上超越单独方法。


<details>
  <summary>Details</summary>
Motivation: SAM均匀扰动所有参数方向，忽略损失几何结构。

Method: 将SAM与LLQR框架中的学习预条件器结合，预条件器通过稀疏更新和慢速指数移动平均捕获平滑的低分辨率几何，SAM在其上快速探测曲率。

Result: 在标准视觉和序列建模基准上，LLQR+SAM持续优于单独使用SAM或LLQR。

Conclusion: 慢学习几何与快速锐度校正是互补的。
...
### [219] [Surrogate Neural Architecture Codesign Package (SNAC-Pack)](https://arxiv.org/abs/2605.16138)
*Jason Weitz,Dmitri Demler,Benjamin Hawks,Aaron Wang,Nhan Tran,Javier Duarte*

Main category: cs.LG

TL;DR: SNAC-Pack 是一个开源 AutoML 框架，用于硬件感知的神经架构协同设计和端到端 FPGA 部署，通过多目标搜索和硬件代理模型高效优化资源与延迟。


<details>
  <summary>Details</summary>
Motivation: 现有 NAS 方法多只优化精度或使用与硬件成本相关性差的代理指标（如 BOPs），在 FPGA 部署中成本由查找表、DSP 等多维预算决定，存在较大差距。

Method: 使用 Optuna 和 NSGA-II 进行多目标全局搜索，共享 SQLite 存储支持并行；硬件代理模型估计资源与延迟，避免综合成本；局部搜索结合量化感知训练和迭代修剪；最终通过 hls4ml 合成 FPGA 固件；用户可通过 YAML 配置和可选智能前端使用。

Result: 在喷注分类和超导量子比特读出任务上，发现紧凑架构，匹配或超过强基线，减少 FPGA 资源使用，将设计空间探索从数月手动调优缩短至数小时自动搜索。

Conclusion: SNAC-Pack 有效解决了 FPGA 部署中硬件成本与精度权衡，实现了自动化、高效的硬件感知神经架构搜索和部署。
...
### [220] [Learn Where Outcomes Diverge: Efficient VLA RL via Probabilistic Chunk Masking](https://arxiv.org/abs/2605.16154)
*Vaidehi Bagaria,Nikshep Grampurohit,Pulkit Verma*

Main category: cs.LG

TL;DR: 提出概率性块掩码（PCM）方法，通过仅对轨迹中少量语义阶段进行梯度计算，在保持成功率的同时显著加速GRPO的强化学习后训练。


<details>
  <summary>Details</summary>
Motivation: 基于GRPO的VLA策略强化学习中，梯度计算占时间78%，且大量计算浪费在策略已掌握的阶段，因为GRPO为所有块分配相同优势，而学习信号仅来自成功与失败轨迹的分歧。

Method: PCM利用成功-失败动作方差作为每阶段梯度方差的代理，在线更新阶段保留概率，从每个轨迹中随机选择固定预算的块进行梯度计算。

Result: 在三个LIBERO基准上，PCM匹配GRPO的最终成功率，同时实现2.38倍加速、4.8倍更快梯度更新、60%更低峰值激活内存，且仅对不到20%的轨迹块进行反向传播。

Conclusion: PCM作为GRPO的即插即用修改，通过聚焦于有学习信号的阶段，大幅降低计算开销而不牺牲任务性能。
...
### [221] [Entropic Auto-Encoding via Implicit Free-Energy Minimization](https://arxiv.org/abs/2605.16164)
*Hazhir Aliahmadi,Irina Babayan,Greg van Anders*

Main category: cs.LG

TL;DR: 提出熵自编码器（EAE），通过隐式先验缓解后验坍缩，学习多模态潜分布，生成多样一致样本。


<details>
  <summary>Details</summary>
Motivation: VAEs因显式先验导致后验坍缩，忽略潜在变量。

Method: EAE仅以重构损失为目标，通过最小化自由能的编码器集合隐式生成熵先验，偏向高体积近优解区域，解码器更新引导搜索向信息丰富的潜表示。

Result: EAE缓解后验坍缩，学习非高斯多模态分布；在反应扩散过程捕获叠加动力学；MNIST识别隐式类别；CelebA展示从'全人类'到个体特征的层次理解。

Conclusion: EAE有效避免后验坍缩，生成多样且一致的数据，保留数据结构。
...
### [222] [BAPR: Bayesian amnesic piecewise-robust reinforcement learning for non-stationary continuous control](https://arxiv.org/abs/2605.16170)
*Yifan Zhang,Liang Zheng*

Main category: cs.LG

TL;DR: 提出BAPR方法，结合贝叶斯在线变化检测与鲁棒集成强化学习，用于分段平稳控制系统，具有自适应保守性和正式验证的误差界限。


<details>
  <summary>Details</summary>
Motivation: 标准鲁棒RL方法在分段平稳条件下存在困境：全局保守策略在稳定期浪费性能，局部自适应策略在变化时可能灾难性失败。

Method: BAPR将贝叶斯在线变化检测与鲁棒集成RL统一，使用BAPR算子（模式条件贝尔曼算子的凸组合，权重由固定信念分布确定），并推导了分量形式的正式误差预算，所有结果经Lean4机器验证。

Result: BAPR算子是γ-压缩算子的扩展，当信念依赖Q函数时压缩因子变为γ+λΔ，当γ+λΔ≥1时失效。误差预算限制了切换后恢复，检测延迟O(log(1/δ))。所有定理机器验证（22个，1145行Lean4代码）。

Conclusion: BAPR通过自适应保守性机制有效应对分段平稳环境，有理论保证和机器验证。
...
### [223] [Imitation learning for clinical decision support in pediatric ECMO](https://arxiv.org/abs/2605.16175)
...
TL;DR: 使用TabPFN在儿科ECMO决策支持中模仿学习，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 儿科ECMO数据稀缺且复杂，需要有效决策支持。

Method: 将临床决策建模为从轨迹中学习动作的模仿学习，使用TabPFN、XGBoost和MLP学习动作模型。

Result: TabPFN在真实儿科ECMO数据上始终优于XGBoost和MLP。

Conclusion: TabPFN可作为儿科ECMO决策支持的强临床医生行为基线。
...
### [224] [Hypothesis-driven construction of mesoscopic dynamics](https://arxiv.org/abs/2605.16211)
*Zhuoyuan Li,Aiqing Zhu,Qianxiao Li*

Main category: cs.LG

TL;DR: 提出一种基于广义Onsager原理的介观动力学学习框架，统一处理耗散和保守系统，具有理论保证，并通过数据驱动识别模型，实验验证有效且可解释。


<details>
  <summary>Details</summary>
Motivation: 传统科学建模依赖于固定方程和特定分析，在多尺度等复杂系统中面临巨大挑战。

Method: 基于广义Onsager原理建立数学约束的假设类，涵盖耗散和保守介观动力学，通过数据驱动识别框架内的具体模型。

Result: 提供全局适定性、渐近稳定性、唯一分解可辨识性和离散能量耗散等先验理论保证；在连续PDE模型和微观链模型数据上验证了准确、鲁棒和可解释性。

Conclusion: 该框架不仅是有效的动力学学习器，还能提供基础物理的可解释诊断。
...
### [225] [The Privacy Price of Tail-Risk Learning: Effective Tail Sample Size in Differentially Private CVaR Optimization](https://arxiv.org/abs/2605.16219)
*El Mustapha Mansouri*

Main category: cs.LG

TL;DR: 差分隐私改变了CVaR学习的有效样本量，有效私有尾部样本量为εnτ，风险分解为统计误差和隐私代价，标量估计和有限类有完整速率，凸Lipschitz学习中隐私项以1/(εnτ)缩放。


<details>
  <summary>Details</summary>
Motivation: 分析差分隐私对CVaR学习的影响，明确有效样本量变化，并建立完整的风险分解和速率。

Method: 通过风险分解、速率分析和模块化上下界约化，在纯DP和近似DP下推导标量估计、有限类和凸Lipschitz学习的隐私损失。

Result: 标量估计速率为Θ(B min{1,(nτ)^{-1/2}+(εnτ)^{-1}})，有限类速率为Θ(B min{1,√log(2M)/(nτ)+log(2M)/(εnτ)})，凸Lipschitz学习隐私项缩放为1/(εnτ)。

Conclusion: 私有CVaR学习的困难子问题是普通私有学习在Θ(nτ)个信息尾部记录上。
...
### [226] [Layer Equivalence Is Not a Property of Layers Alone: How You Test Redundancy Changes What You Find](https://arxiv.org/abs/2605.16234)
*Gabriel Garcia*

Main category: cs.LG

TL;DR: 两种层等价性测试（替换与互换）在预训练变压器上结果不一致，影响剪枝安全性评估。


<details>
  <summary>Details</summary>
Motivation: 研究者常混淆两种层等价性测试（替换与互换），但两者结果可能不同，影响剪枝决策。

Method: 在多个检查点和架构（Pythia 410M/1.4B、Qwen3-8B、Llama-3.1-8B）上测量替换与互换协议的距离。

Result: Pythia训练中差距增大；Qwen3-8B中互换引导的移除更安全；Llama-3.1-8B中两者剪枝代价相当。

Conclusion: 层移除前应同时评估两种swap-KL诊断指标。
...
### [227] [Dynamics-Level Watermarking of Flow Matching Models with Random Codes](https://arxiv.org/abs/2605.16239)
*Shuchan Wang*

Main category: cs.LG

TL;DR: 提出一种在生成模型动态层面嵌入水印的方法，通过扰动速度场实现，不影响生成分布。


<details>
  <summary>Details</summary>
Motivation: 现有水印嵌入模型权重或输出，希望更隐蔽且不破坏生成质量。

Method: 将水印作为随机编码嵌入连续动态（速度场），训练时加入密钥相关扰动，检测时黑盒查询恢复。

Result: MNIST和CIFAR-10实验证实消息可靠恢复，生成质量保持，无密钥时解码准确率随机。

Conclusion: 该方法有效且安全，为生成模型水印提供新途径。
...
### [228] [Evaluating Design Video Generation: Metrics for Compositional Fidelity](https://arxiv.org/abs/2605.16223)
*Adrienne Deganutti,Dingning Cao,Jaejung Seol,Elad Hirsch,Purvanshi Mehta*

Main category: cs.GR

TL;DR: 提出针对设计动画生成视频的自动化评估框架，涵盖布局、运动、时间和内容四个维度，消除主观评估。


<details>
  <summary>Details</summary>
Motivation: 现有生成视频模型缺乏标准化评估框架，设计动画具有严格的结构约束，需要客观评估方法。

Method: 构建自动化评估框架，从布局保真度、运动正确性、时间质量和内容保真度四个维度进行量化评估。

Result: 实现了对设计动画生成视频的自动化、客观评估，避免了主观偏差。

Conclusion: 该框架为设计动画生成领域建立了统一的评估基准，有助于推动该方向的发展。
...
### [229] [Unified Simulation of Lagrangian Particle Dynamics via Transformer](https://arxiv.org/abs/2605.15305)
...
TL;DR: 提出一个基于单一Transformer架构的学习型粒子模拟器，统一建模布料、弹性固体、牛顿/非牛顿流体、颗粒材料和分子动力学，采用预测-校正设计，通过粒子分词、超级分词编码/解码实现高效计算，在多种动力学类别上泛化良好，并支持交互控制、逆向设计等下游应用。


<details>
  <summary>Details</summary>
Motivation: 实现无需针对不同物理现象重新设计求解器的统一模拟器，以减少工程工作量。

Method: 基于Transformer架构的粒子模拟器，采用预测-校正设计：显式预测器根据外力推进粒子，学习型校正器通过粒子分词器、超级分词编码器（交替自注意力和令牌合并）和超级分词解码器（交叉注意力）预测残差位置和速度更新。

Result: 在布料、弹性固体、牛顿/非牛顿流体、颗粒材料和分子动力学六个类别上，对未见过的材料、边界条件、初始条件和外力泛化良好，并展示了交互控制、逆向设计和从真实操作数据学习的能力。

Conclusion: 该统一模拟器减少了对每现象求解器工程的需求，为多物理模拟提供了灵活方案。
...
### [230] [FFAvatar: Few-Shot, Feed-Forward, and Generalizable Avatar Reconstruction](https://arxiv.org/abs/2605.15320)
*Thuan Hoang Nguyen,Jiahao Luo,Yinyu Nie,Hao Li,Gordon Guocheng Qian,Jian Wang*

Main category: cs.GR

TL;DR: FFAvatar是一种前馈式框架，能在数秒内从少量未摆姿势的人脸图像重建高质量、可动画化的3D高斯头部虚拟形象。


<details>
  <summary>Details</summary>
Motivation: 传统虚拟形象重建依赖于逐个优化的耗时方法或昂贵的预处理，限制了可扩展性，需要更快速且通用的重建方案。

Method: 通过多视图查询变换器将多张源图像融合到统一的规范高斯表示中，利用从像素端到端预测的FLAME参数驱动动画；采用三阶段训练：大规模单目视频预训练（超100万身份）、多视图微调（360度高质量数据）、可选个性化优化。

Result: 在NeRSemble基准上PSNR比LAM提高5.5，无个性化重建需2秒，有个性化需10秒，单张A100支持49 FPS动画。

Conclusion: FFAvatar在身份保持、几何一致性和动画保真度上树立了新标准，实现了实时部署。
...
### [231] [StippleDiffusion: Capacity-Constrained Stippling using Controlled Diffusion](https://arxiv.org/abs/2605.15816)
*Ofir Gilad,Aleksander Plocharski,Przemyslaw Musialski,Andrei Sharf*

Main category: cs.GR

TL;DR: 提出了第一个基于扩散的采样器，能够同时满足学习到的局部点分布先验和图像定义容量约束，实现高效、可微的随机点画生成。


<details>
  <summary>Details</summary>
Motivation: 传统随机点画生成方法速度慢、不可微，且对每个新目标需重新优化；现有学习方法仅处理无条件点生成，无法处理图像条件化的容量约束。

Method: 在最优传输网格点扩散模型上构建ControlNet分支，以密度图和高分辨率图像为条件；采用晚期去噪阶段训练和推理，用密度加权拒绝采样初始化，并用Sigmoid门控1x1投影替换标准零卷积注入以保持蓝噪声结构。

Result: 在Icons-50基准上，所有评估指标均达到与逐密度优化基线相当的水平；推理时间几乎与输出点数无关，且能泛化到训练中未见过的点预算。

Conclusion: 首个同时满足局部点分布先验和连续容量约束的扩散采样器，实现了快速、可微、图像条件化的随机点画生成。
...
### [232] [Heuristic-Based Merging of HPC Traces to Extend Hardware Counter Coverage](https://arxiv.org/abs/2605.15832)
*J\'ulia Orteu Aubach,Fabio Banchelli,Marc Clasc\`a Ram\'irez,Marta Garcia-Gasulla*

Main category: cs.PF

TL;DR: 提出一种基于启发式的执行迹合并方法，通过分析MPI结构、时序和通信模式将多组硬件计数器合并，构建统一数据集用于HPC性能预测和ML训练，验证表明合并计数器在保留精度的同时扩展了特征空间。


<details>
  <summary>Details</summary>
Motivation: 性能建模中可同时收集的硬件计数器数量受限，导致特征空间有限，限制了预测精度。

Method: 通过分析MPI结构、时序和通信模式，匹配不同运行中的计算爆发，将多次运行（每次使用不同计数器集）的执行迹合并，构建包含更多计数器的统一数据集。

Result: 在MareNostrum5上对多种内核和实际应用进行验证，合并后的计数器保持可接受精度，可直接用于训练ML模型，无需预先选择计数器。

Conclusion: 该方法在不依赖硬件多路复用的情况下有效扩展了特征空间，提升了HPC性能预测的潜力。
...
### [233] [SwAIther-Precip: Lead-Time-Aware Bias Correction Enables Kilometer-Scale Downscaling of Global AI Precipitation Forecasts over Switzerland](https://arxiv.org/abs/2605.16163)
...
TL;DR: 提出SwAIther-Precip框架，将粗分辨率AIFS预测转换为瑞士千米尺度概率降水场，通过时滞感知降尺度实现48%的CRPS降低和~4km有效分辨率。


<details>
  <summary>Details</summary>
Motivation: 复杂地形下中期降水预报挑战大，全球AI模型分辨率不足（0.25°），统计降尺度方法难以处理与状态和时滞相关的偏差。

Method: 先使用时滞条件U-Net（特征线性调制）进行粗分辨率系统偏差校正，再用扩散模型独立于时滞生成细尺度空间变异性。

Result: 相比原始AIFS，CRPS降低48%；生成场在大尺度和小尺度的光谱保真度分别高于0.85和0.88，在1km网格上有效分辨率约4km（时滞≤5天）；跨时滞训练使6天CRPS再降13%。

Conclusion: 在生成式超分辨率之前显式校正时滞相关偏差是实现全球AI降水预报高效千米尺度概率降尺度的关键。
...
### [234] [Universal Magnetic Structure Prediction from Atomic Coordinates with Near-Experimental Accuracy](https://arxiv.org/abs/2605.16230)
...
TL;DR: 提出磁性结构网络（MSN），一种E(3)等变图神经网络，直接根据原子晶体结构预测共线和非共线磁性结构，并引入原始调制结构表示（PMSR）统一编码相称和不相称结构。


<details>
  <summary>Details</summary>
Motivation: 磁性结构难以通过实验和第一性原理方法确定，需要高效预测方法。

Method: 使用E(3)等变图神经网络（MSN）和原始调制结构表示（PMSR），在MAGNDATA上训练。

Result: 模型在所有调制分量上表现优异，高保真重建实验磁性结构。

Conclusion: 为快速磁性结构预测提供可扩展框架，开启数据驱动磁性材料发现路径。
...
### [235] [Thermodynamic Invariants of Coupled Channels: A Many-Channel Tolman-Ehrenfest Effect](https://arxiv.org/abs/2605.15653)
*Benjamin Hamblin,Victor Calo,Klaus Regenauer-Lieb*

Main category: math-ph

TL;DR: 推导出多通道热力学不变量，解释了颗粒材料的剪胀比和状态依赖摩擦系数。


<details>
  <summary>Details</summary>
Motivation: 单通道平衡条件在多热力学通道耦合时失效，需要新理论。

Method: 将Tolman-Ehrenfest效应扩展到熵流形，利用Ruppeiner联络的几何方法。

Result: 得到n通道不变量ζ_i T_i = C；颗粒体系中剪胀比和能量限制来自非对角曲率；K_μ之谜通过修正项ζ_V在堵塞时达O(1)解决。

Conclusion: 预测ζ_V χ=常数在剪切带内，可实验检验。
...
### [236] [Spectral separation of variables from equivalent Lagrangian systems](https://arxiv.org/abs/2605.15679)
*Mattia Scomparin*

Main category: math-ph

TL;DR: 研究二次拉格朗日量的动力学等价性及其与变量分离的关系，发现相容性条件导致构型空间的正交谱分解，从而实现运动方程的解耦。


<details>
  <summary>Details</summary>
Motivation: 探索二次拉格朗日量的动力学等价性如何促进变量分离，以理解系统的可积性。

Method: 通过要求两个二次拉格朗日量生成相同的欧拉-拉格朗日方程，导出动能矩阵与势能之间的相容性条件；对于常对称动能矩阵，该条件简化为与势能海森矩阵的对易关系，从而得到构型空间的正交谱分解。

Result: 运动方程解耦为独立的子系统：一般呈块分离形式，当谱简单时完全解耦。该方法应用于Sawada-Kotera系统和n维Hénon-Heiles模型，恢复了经典的可积参数区域。

Conclusion: 二次拉格朗日量的动力学等价性通过正交谱分解自然地导致变量分离，为可积系统的识别提供了有效工具。
...
### [237] [Epstein vector zeta functions related to the ADE Lie algebras](https://arxiv.org/abs/2605.16042)
*M.Olshanetsky*

Main category: math-ph

TL;DR: 本文引入了与ADE型李代数根格相关的向量值广义Epstein zeta函数，构造了相应的向量值theta级数并推导出其Mellin变换，得到了矩阵函数方程，并给出了满足该方程的格与不变子空间的完整分类。


<details>
  <summary>Details</summary>
Motivation: 将经典的Epstein zeta函数推广到向量值情形，以研究ADE型根格的解析性质，并揭示其与Metaplectic群表示的内在联系。

Method: 利用根格的二次型（Gram矩阵）和判别群D=P/Q构造向量值theta级数，实现Mp(2,Z)的Weil表示，再通过Mellin变换定义向量zeta函数，并利用theta向量的等变性推导矩阵函数方程。

Result: 得到了Riemann类型的矩阵函数方程，并证明该方程仅对C-不变向量子空间成立，其中C是Mp(2,Z)的中心元。同时给出了所有满足方程的格和不变子空间的完整分类。

Conclusion: 该向量值广义Epstein zeta函数与ADE型李代数的根格有深刻联系，其函数方程的存在性由选择规则严格限定，分类结果完善了相关理论。
...
### [238] [Testing properties of trees in graphical models with covariance queries](https://arxiv.org/abs/2605.15996)
*Sofiya Burova,Francisco Calvillo,G\'abor Lugosi,Piotr Zwiernik*

Main category: stat.ML

TL;DR: 在高维图模型中使用协方差查询，提出针对树结构全局性质的亚二次查询复杂度随机测试。


<details>
  <summary>Details</summary>
Motivation: 重构整个树代价高昂，但测试全局结构性质可能更高效。

Method: 基于协方差查询设计随机测试，针对叶子数、最大度、典型距离和直径等性质。

Result: 每个性质获得了依赖于阈值和容差参数的显式查询复杂度界限。

Conclusion: 树结构下，全局性质可以在亚二次查询次数内有效测试。
...
### [239] [$\alpha$-TCAV: A Unified Framework for Testing with Concept Activation Vectors](https://arxiv.org/abs/2605.15688)
*Ekkehard Schnoor,Jawher Said,Malik Tiomoko,Wojciech Samek,Alexander Jung*

Main category: stat.ML

TL;DR: 分析了概念激活向量（CAV）的统计不稳定性，指出标准TCAV得分的根本缺陷，并提出α-TCAV框架，用平滑函数替代指示函数，提供参数调整指南和实用建议。


<details>
  <summary>Details</summary>
Motivation: 现有CAV和TCAV方法统计不稳定，标准TCAV得分依赖不连续指示函数导致方差不衰减。

Method: 提出α-TCAV，用参数化平滑函数替代指示函数，统一概率公式，分析分布特性，提供参数调整指南。

Result: 表明现有最优选择缺乏理论依据；α-TCAV可模仿Multi-TCAV且计算成本更低，或获得贝叶斯最优概率度量；建议将完整采样预算分配给单个CAV。

Conclusion: 挑战了常规做法，提出了实用的改进建议，如单CAV分配全部采样预算。
...
### [240] [On Kernel Eigen-alignments of KRR: Reconstruction and Generalization](https://arxiv.org/abs/2605.15240)
*Yang Liu,Ernest Fokoue,Richard Lange,Daniel Krutz*

Main category: stat.ML

TL;DR: 本文研究了核矩阵与学习目标之间的特征对齐对鲁棒泛化的关键作用，建立了核方法泛化性能与矩阵特征值和特征向量估计的直接联系，并导出了基于特征估计的泛化界。


<details>
  <summary>Details</summary>
Motivation: 理解核方法中泛化性能的决定因素，特别是在有限样本下，以便设计更鲁棒的学习算法。

Method: 通过分析核矩阵扰动对预测误差的影响，利用矩阵特征值和特征向量的估计稳定性来推导泛化误差的界。

Result: 发现高秩核可以轻易获得近零重构误差，但重构误差对泛化预测能力有限；强泛化需要增加特征向量对齐、特征值大小或特征值间隔。

Conclusion: 核方法的泛化性能强烈依赖于特征对齐和特征值结构，而非重构误差本身。
...
### [241] [Harnessing Unimodality in Semiparametric Contextual Pricing via Oracle Price Map Learning](https://arxiv.org/abs/2605.15411)
*Yingying Fan,Yuxuan Han,Jinchi Lv,Xiaocong Xu,Zhengyuan Zhou*

Main category: stat.ML

TL;DR: 针对半参数标量指标估值模型中的上下文动态定价问题，提出ORBIT策略，证明其遗憾界并达到极小极大最优。


<details>
  <summary>Details</summary>
Motivation: 在未知效用映射和噪声分布的动态定价中，需要学习最优价格映射，但高维挑战大。

Method: 采用ORBIT模块化粗到细策略，利用标量指标构造基准价格，通过置信域内的局部多项式逼近和带状凸优化学习。

Result: 对于线性效用模型，遗憾界为O~(T^{(2β-1)/(4β-3)} + √(dT))，且下界匹配。

Conclusion: 非参数最优价格映射学习是极小极大最优的，并可扩展至稀疏高维和非参数情形。
...
### [242] [MaxSketch: Robust Distinct Counting in Streams via Random Projections](https://arxiv.org/abs/2605.15571)
*Nikos Tsikouras,Constantine Caramanis,Christos Tzamos*

Main category: stat.ML

TL;DR: 利用几何结构提出MaxSketch草图，用对数内存高精度估计高维噪声数据流中的不同元素数量。


<details>
  <summary>Details</summary>
Motivation: 经典草图在高维噪声数据中失效，现有鲁棒方法内存开销大。

Method: 基于随机高斯投影的MaxSketch线性草图。

Result: 使用$\widetilde{O}(\log n / \varepsilon^2)$内存即可在$(1+\varepsilon)$因子内准确计数，实验验证有效。

Conclusion: 几何结构可以大幅降低不同计数问题的复杂度。
...
### [243] [Pessimistic Risk-Aware Policy Learning in Contextual Bandits](https://arxiv.org/abs/2605.15620)
...
### [244] [Unsupervised Domain Shift Detection with Interpretable Subspace Attribution](https://arxiv.org/abs/2605.15920)
*Sebastian Springer,Alessandro Laio*

Main category: stat.ML

TL;DR: 开发了一种检测领域偏移的工具，通过识别高维特征空间中的局部密度异常，并追溯偏移至可解释的特征子集，同时提供补偿偏移的协议。在20维基准和心电图数据上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 数据集之间的领域偏移（概率分布的微妙差异）可能影响下游建模，需要检测和解释这些偏移。

Method: 使用算法检测高维特征空间中的局部密度异常，定位异常最显著的特征子空间，并通过提取无标签数据中无残留分布差异的子样本来补偿偏移。

Result: 在20维基准上成功恢复宽泛和局部偏移及其特征子空间；在心电图数据中检测到设备引起的偏移，提取了富含不平衡设备成分的子集，并识别了与采集对比相关的特征。

Conclusion: 密度偏移检测和子空间归因提供了一个实用框架，可在下游建模前揭示隐藏的队列偏差。
...
### [245] [Explainable AI Isn't Enough! Rethinking Algorithmic Contestability](https://arxiv.org/abs/2605.16041)
*Timo Freiesleben,Kristof Meding,Gunnar K\"onig*

Main category: stat.ML

TL;DR: 本文提出算法可争议性的操作性定义，与算法追索权互补，并识别预测多样性、错误特征值和忽略的反驳证据三种可推翻决策的证据。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统做出影响个人生活的决策，但个人难以质疑错误决策。可解释AI主要关注追索权，忽视了可争议性，这具有伦理和法律重要性。

Method: 提出可争议性的操作定义，与追索权互补；识别三种证据：预测多样性、错误特征值、忽略的反驳证据。

Result: 标准XAI解释仅揭示邻近区域错误，不足以推翻决策；而三种证据可使决策在道德上不可辩护，从而成功争议。

Conclusion: 现有欧盟立法已赋予个人对这些证据形式的一些合法权利。
...
### [246] [A numerical study into neural network surrogate model performance for uncertainty propagation](https://arxiv.org/abs/2605.16078)
*Noah Wade,Kirubel Teferra*

Main category: stat.ML

TL;DR: 研究了神经网络代理模型在随机边界值问题中捕捉解场完整分布（特别是尾部分布）的能力，发现最坏情况预测误差比均值场误差大一个数量级，并提出了一种识别极端样本的方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注神经网络代理模型表示确定性样本或均值场解，忽视了尾部分布的性能，而随机问题中尾部分布对参数分析至关重要。

Method: 以热传导方程（具有高度随机源项）为典型问题，比较了前馈全连接网络和DeepONet架构，采用数据驱动和物理信息损失函数，并提出了识别极端样本的方法。

Result: 最坏情况预测误差比均值场误差大一个数量级，表明极端样本的重要性；全连接网络配合弱形式残差损失在处理外推输入时表现最佳。

Conclusion: 神经网络代理模型在尾部分布预测上存在显著误差，主要源于外推问题；所提出的识别方法和弱形式残差损失能有效提升极端样本的预测精度。
...
### [247] [Skew-adaptive conformal prediction](https://arxiv.org/abs/2605.16145)
*Paulo C. Marques F.,Helton Graziadei*

Main category: stat.ML

TL;DR: 提出一种偏斜自适应分裂共形预测方法，通过不对称区间族和逆双曲正弦变换学习不确定性的倾斜，保持有限样本边缘有效性，实验表明优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有共形预测区间通常对称，无法适应数据中存在的偏斜性，导致区间效率不高。

Method: 基于不对称区间族，采用规范方法推导一致性分数，以逆双曲正弦变换后的带符号缩放残差作为额外预测模型的训练目标，学习特征空间中的不确定性倾斜。

Result: 在多个数据集上，该方法在预测区间效率上优于缩放分数构造和共形分位数回归，且提出的估计器能准确匹配测试样本上的平均宽度比。

Conclusion: 该方法保留了分裂共形预测的有限样本边缘有效性，同时生成适应局部尺度和偏斜的区间，提高了预测效率。
...
### [248] [A Scalable Nonparametric Continuous-Time Survival Model through Numerical Quadrature](https://arxiv.org/abs/2605.16208)
*Chaeyeon Lee,Sehwan Kim,Hyungrok Do*

Main category: stat.ML

TL;DR: 提出QSurv，一种可扩展的深度学习框架，通过高斯-勒让德数值积分和时间条件低秩适应实现非参数连续时间生存建模，避免离散化或分布假设。


<details>
  <summary>Details</summary>
Motivation: 连续时间生存建模中，似然估计的不可解积分导致训练困难，现有方法依赖时间离散化或约束性假设。

Method: 采用高斯-勒让德数值积分近似累积风险函数，支持端到端反向传播；引入时间条件低秩适应机制，通过低秩更新动态调制权重以捕捉非平稳风险动态。

Result: 在合成基准、大规模表格数据和医学影像任务中，预测性能具有竞争力，且在瞬时风险函数估计上更具优势，提供可解释的时间变化风险模式。

Conclusion: QSurv实现了灵活的非参数连续时间生存建模，有效处理高维数据中的复杂时间依赖风险。
...
### [249] [Do Biological Structural Guarantees Earn Their Complexity?](https://arxiv.org/abs/2605.15225)
*Bogdan Banu*

Main category: q-bio.QM

TL;DR: 通过三个深度基准测试，实证比较了生物启发式AI框架与朴素非生物替代方案的可靠性，结果显示生物启发方法未必更优。


<details>
  <summary>Details</summary>
Motivation: 生物启发式AI框架声称具有结构保障的可靠性优势，但缺乏与简单替代方案的实证比较。

Method: 设计三个基准测试：代谢优先级门控、自诱导剂群体感应和贝叶斯停滞检测，每个测试对比生物启发实现、非生物朴素替代和消融控制，每种子1000次试验，共10种子（1000万+数据点）。

Result: 生物启发实现并不总是优于朴素非生物替代；具体结果摘要未给出详细数值，但暗示差异不显著或存在依赖。

Conclusion: 生物启发式AI框架的可靠性优势并非必然，实证检验至关重要。
...
### [250] [DeepSlide: From Artifacts to Presentation Delivery](https://arxiv.org/abs/2605.15202)
*Ming Yang,Zhiwei Zhang,Jiahang Li,Haoseng Liu,Yuzheng Cai,Weiguo Zheng*

Main category: cs.AI

TL;DR: 提出DeepSlide，一种人机协同的多智能体系统，支持从需求获取、时间预算叙事规划到基于证据的幻灯片脚本生成、注意力增强和排练的完整演示准备流程。在20个领域和多种观众设定下，DeepSlide在保持幻灯片质量的同时，在叙事流畅性、节奏精确性和幻灯片-脚本协同等交付指标上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有AI幻灯片生成器主要优化视觉美观的成品，而忽略了演讲过程中的节奏、叙事和准备支持。

Method: DeepSlide包含可控逻辑链规划器（含节点时间预算）、轻量级内容树检索器、马尔可夫式序列渲染（风格继承）以及沙盒执行与最小修复。同时引入双记分牌基准，区分静态质量与动态交付。

Result: 在20个领域和多样化受众测试中，DeepSlide在工件质量上与强基线持平，但在叙事流、节奏精度、幻灯片-脚本协同和注意力引导等交付指标上获得更大增益。

Conclusion: DeepSlide有效增强了演示的动态交付质量，而无需牺牲幻灯片本身的品质。
...
### [251] [SDOF: Taming the Alignment Tax in Multi-Agent Orchestration with State-Constrained Dispatch](https://arxiv.org/abs/2605.15204)
*Zhantao Wang*

Main category: cs.AI

TL;DR: SDOF是一个将多智能体执行视为约束状态机的框架，通过在线RLHF意图路由器和状态感知调度器实现可审计执行控制，在招聘系统基准测试中优于GPT-4o。


<details>
  <summary>Details</summary>
Motivation: 现有的多智能体编排框架（如LangChain等）不支持真实业务流程中的阶段约束，需要一种强制执行阶段约束的框架。

Method: SDOF通过两个防御层实现：1）基于生成奖励建模（GRPO）训练的在线RLHF专用意图路由器；2）带有目标阶段有限自动机检查以及前提/后置条件技能注册表验证的状态感知调度器。

Result: 在招聘系统实验中，7B意图路由器在FSM约束对抗路由基准上联合准确率达80.9%（GPT-4o为48.9%）；端到端任务完成率86.5%，阻止所有注入和非法HR操作；消息级封锁审计精确率100%，召回率88%，专家一致性kappa=0.94。

Conclusion: SDOF在受约束的多智能体执行中表现优异，未来将发布扩展的多种子训练比较和更深入的工作流评估。
...
### [252] [Does Theory of Mind Improvement Really Benefit Human-AI Interactions? Empirical Findings from Interactive Evaluations](https://arxiv.org/abs/2605.15205)
*Nanxu Gong,Zixin Chen,Haotian Li,Zishu Zhao,Jianxun Lian,Huamin Qu,Yanjie Fu,Xing Xie*

Main category: cs.AI

TL;DR: 提出交互式心理理论评估范式，发现静态基准的改进并不保证动态人机交互中的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有心理理论基准忽略第一人称、动态和开放式交互，无法直接评估增强技术对人机交互的益处。

Method: 提出交互式心理理论评估范式，从视角和指标两方面转变；系统研究了四种代表性增强技术，采用四个真实数据集和用户研究，涵盖目标导向和体验导向任务。

Result: 静态基准上的改进未完全转化为动态人机交互中的更好表现。

Conclusion: 需要基于交互的评估来开发下一代的具有社会意识的大语言模型，实现人机共生。
...
### [253] [SkillSmith: Compiling Agent Skills into Boundary-Guided Runtime Interfaces](https://arxiv.org/abs/2605.15215)
*Duling Xu,Zheng Chen,Zaifeng Pan,Jiawei Guan,Dong Dong,Jialin Li,Bangzheng Pu*

Main category: cs.AI

TL;DR: 提出SkillSmith框架，通过边界优先的编译时-运行时方法，提取技能的操作边界，减少不必要的上下文注入和重复思考，显著降低token使用、迭代次数和成本。


<details>
  <summary>Details</summary>
Motivation: 现有技能注入到智能体推理循环中作为上下文指导，导致两个主要冗余来源：无关上下文注入和重复的技能特定推理与规划。

Method: SkillSmith采用边界优先的编译时-运行时框架，将技能包离线编译为最小可执行接口，提取细粒度操作边界，使智能体在运行时动态访问和执行仅相关的组件。

Result: 在SkillsBench基准上，相比原始技能，减少解决阶段token使用57.44%，思考迭代42.99%，解决时间50.57%（加速2.02倍），成本57.44%。强模型编译的工件可被小模型重用，在原始技能解释失败时提升任务准确率。

Conclusion: SkillSmith通过边界优先的方法有效减少了冗余，提高了效率和成本效益，并支持模型间的知识重用。
...
### [254] [Fair outputs, Biased Internals: Causal Potency and Asymmetry of Latent Bias in LLMs for High-Stakes Decisions](https://arxiv.org/abs/2605.15217)
*Jagdish Tripathy,Marcus Buckmann*

Main category: cs.AI

TL;DR: 指令微调语言模型在输出层面无偏见，但内部表示保留并放大人口统计偏见；这些隐藏偏见可通过干预导致决策逆转，且不对称，易受对抗攻击；建议结合输出评估和表征分析的双层测试框架。


<details>
  <summary>Details</summary>
Motivation: 探究模型内部保留的偏向性表示是否会影响输出决策，以及这种影响在不同人口群体间是否对称。

Method: 使用仅姓名不同的匹配贷款申请，通过激活干预和跨层干预来分析内部表示的影响。

Result: 模型输出无偏见，但内部表示可被操纵导致决策逆转；偏见方向不对称，且易受对抗提示和参数高效微调影响。

Conclusion: 仅基于输出的行为审计不足以发现内部偏见，需要结合输出评估和表征分析的双层测试框架来监管高风险决策中的AI系统。
...
### [255] [CAX-Agent: A Lightweight Agent Harness for Reliable APDL Automation](https://arxiv.org/abs/2605.15218)
*Chenying Lin,Yichen Hai,Yi He,Ran Wang,Haiyan Qiang,Liang Yu*

Main category: cs.AI

TL;DR: 提出CAX-Agent轻量级代理框架，通过分层架构和恢复策略提升大语言模型在MAPDL有限元仿真中的可靠性，实验表明模型驱动恢复策略显著优于无恢复或仅规则恢复。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在MAPDL有限元仿真中因缺乏结构化执行控制、工具封装和故障恢复，导致输出不一致和任务失败频发。

Method: 设计CAX-Agent架构，包含LLM服务层、代理中间层和求解器后端三层，并引入恢复阶梯（从确定性规则修补到模型驱动再生、上下文增强及人工干预）。评估三种恢复策略：无恢复、仅规则、仅模型。

Result: 在50个标准结构基准上，模型驱动策略取得最高完成率（0.9267）、任务评分（3.59/4）、总分（9.16/10）和零干预率（0.84），优于仅规则和无恢复策略，效果显著（Cliff's delta 0.81-0.87）。

Conclusion: 模型驱动恢复策略在简单几何基准中表现优越，但需在更复杂场景下进一步验证其泛化性。
...
### [256] [NOVA: Fundamental Limits of Knowledge Discovery Through AI](https://arxiv.org/abs/2605.15219)
*Salman Avestimehr,Ken Duffy,Muriel M\'edard*

Main category: cs.AI

TL;DR: 提出NOVA框架建模AI自我改进的知识发现过程，发现污染陷阱和成本缩放定律，证明渐近收益递减。


<details>
  <summary>Details</summary>
Motivation: 研究AI系统能否通过迭代自我改进发现真正的新知识，并量化其成本。

Method: 将'生成-验证-积累-重训练'循环建模为知识空间上的自适应采样，分析充分条件及其违反导致的失败模式，引入污染陷阱，基于尾等价假设证明成本缩放定律。

Result: 累计生成成本满足R_cum(D)=Θ(c_gen D^α)（α>1），且不完美验证会导致污染陷阱。

Conclusion: 知识发现存在渐近收益递减，专家输入在最接近自主探索障碍时最有价值。
...
### [257] [ICRL: Learning to Internalize Self-Critique with Reinforcement Learning](https://arxiv.org/abs/2605.15224)
*Jianbo Lin,Xiaomin Yu,Yi Xin,Yifu Guo,Zhuosong Jiang,Zhongqi Yue,Weishi Wang,Heqing Zou,Chengwei Qin,Hui Xiong*

Main category: cs.AI

TL;DR: 提出ICRL框架，通过强化学习联合训练求解器和批评者，使求解器内化自我批评，无需外部批评即可自我改进。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的智能体常犯错，批评能引导正确行为但未被内化；固定的批评者无法提升反馈质量，限制自我改进。

Method: 从共享主干联合训练求解器和批评者，奖励基于求解器的性能提升；引入分布校准重加权比率和角色组优势估计，稳定联合优化。

Result: 在智能体和数学推理任务上，平均比GRPO提升6.4和7.0分；8B批评者性能媲美32B批评者且用更少token。

Conclusion: ICRL使求解者学会无需外部批评的自我改进，而非依赖批评条件行为。
...
### [258] [NIMO Controller: a self-driving laboratory orchestrator based on the Model Context Protocol](https://arxiv.org/abs/2605.15227)
*Naruki Yoshikawa,Ryo Tamura*

Main category: cs.AI

TL;DR: 提出基于模型上下文协议（MCP）的自动驾驶实验室软件架构，通过NIMO控制器实现统一的人机与AI交互界面。


<details>
  <summary>Details</summary>
Motivation: 现有SDL软件框架主要面向人类交互，缺乏适合AI代理的标准化接口，技术门槛高。

Method: 基于MCP架构，通过MCP服务器暴露所有SDL功能，NIMO控制器提供可视化编程界面和AI代理统一的访问后端。

Result: 通过颜色匹配SDL案例验证了系统的可用性。

Conclusion: MCP架构为人与AI提供了统一接口，降低了SDL软件开发和使用的技术门槛。
...
### [259] [Verifiable Agentic Infrastructure: Proof-Derived Authorization for Sovereign AI Systems](https://arxiv.org/abs/2605.15228)
*Jun He,Deying Yu*

Main category: cs.AI

TL;DR: 提出分布式信任框架（DTF），将授权从静态身份转变为基于证明的派生权威，使自主AI代理的执行变得可治理、可审计和有边界。


<details>
  <summary>Details</summary>
Motivation: 自主AI代理可生成语义不安全动作，使静态特权成为运营风险，尤其在主权AI系统中。

Method: 引入理由证明、共识模型、临时执行身份和仅追加证据链，在治理突变基座上计算执行权限。

Result: 在OpenKedge基座上实例化，展示映射到云原生环境，强制执行授权不变量。

Conclusion: DTF提供基础设施基础，使代理执行在主权AI部署中可治理、可审计和有边界。
...
### [260] [Solvita: Enhancing Large Language Models for Competitive Programming via Agentic Evolution](https://arxiv.org/abs/2605.15301)
...
TL;DR: Solvita 是一个无需更新 LLM 权重的持续学习框架，通过代理和可训练知识网络实现动态路由，在困难编程任务上达到新 SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 在困难编程上推理不足，多代理框架无状态，丢弃了先前经验。

Method: Solvita 包含四个代理 (Planner, Solver, Oracle, Hacker)，每个代理连接图结构知识网络，利用强化学习从结果信号更新网络权重，实现动态路由。

Result: 在 CodeContests、APPS、AetherCode 和实时 Codeforces 上，优于现有管道，准确率近乎翻倍。

Conclusion: 无需权重更新的持续学习机制能显著提升 LLM 在编程推理上的表现。
...
### [261] [SMCEvolve: Principled Scientific Discovery via Sequential Monte Carlo Evolution](https://arxiv.org/abs/2605.15308)
*Jiachen Jiang,Huminhao Zhu,Zhihui Zhu*

Main category: cs.AI

TL;DR: SMCEvolve 将程序搜索重新定义为从奖励倾斜的目标分布中采样，并使用序列蒙特卡洛采样器近似，引入自适应父重采样、混合变异与接受以及自动收敛控制，在多个基准测试中以更少的LLM调用超越现有系统。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的程序进化框架缺乏原理性指导，无法保证搜索收敛。

Method: 将程序搜索视为从奖励倾斜分布采样，用序列蒙特卡洛(SMC)近似，包含自适应父重采样、混合变异与接受、自动收敛控制三大核心机制，并提供有限样本复杂度分析。

Result: 在数学、算法效率、符号回归和端到端机器学习研究基准测试中，SMCEvolve以更少的LLM调用超越最先进的进化系统，并能自定终止条件。

Conclusion: SMCEvolve提供了一种原理性且高效的LLM驱动程序进化方法，具有收敛保证和更优性能。
...
### [262] [Context Pruning for Coding Agents via Multi-Rubric Latent Reasoning](https://arxiv.org/abs/2605.15315)
*Jingjing Wang,Xiwen Chen,Wenhui Zhu,Huayu Li,Zhengxiao He,Feiyang Cai,Ana S. Carreon-Rascon,Xuanzhao Dong,Feng Luo*

Main category: cs.AI

TL;DR: 提出LaMR框架，将代码相关性分解为语义证据和依赖支持两个维度，用独立CRF建模，动态加权融合，有效过滤噪声，节省token并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有单目标序列标注器将代码相关性压缩成一个分数和转移矩阵，无法处理异构保留模式，导致建模瓶颈。

Method: 将代码相关性分解为语义证据和依赖支持两个可解释维度，每个维度用专用CRF建模，通过混合专家门控网络动态加权，最后用CRF层融合做出剪枝决策。标签从训练语料中自动推导。

Result: 在四个基准上，12/16多轮对比胜出，最多节省31% token，单轮任务精确匹配提升最多+3.5，性能提升或仅小幅下降。

Conclusion: LaMR有效去噪，常能匹配甚至超越未剪枝的全上下文基线，性能提升显著，边际损失很小。
...
### [263] [Zero-Shot Goal Recognition with Large Language Models](https://arxiv.org/abs/2605.15333)
*Kin Max Piamolini Gusm\~ao,Nathan Gavenski,Nir Oren,Felipe Meneguzzi*

Main category: cs.AI

TL;DR: 首次系统评估前沿大语言模型作为零样本目标识别器在经典PDDL基准上的表现，发现模型能力不均，部分随证据提升，部分固守先验，揭示证据整合差异。


<details>
  <summary>Details</summary>
Motivation: 目标识别作为溯因任务更适合LLM的世界知识利用能力，而非生成新颖动作序列的规划任务，因此评估LLM在此任务上的表现能检验其规划知识基础。

Method: 在经典PDDL基准上进行零样本评估，分析推理痕迹，比较模型表现与基于地标的传统方法。

Result: 模型表现不均衡：部分模型随观察证据增加准确度提升，接近地标水平；部分模型始终依赖世界知识先验，不受证据量影响。定性分析显示证据整合能力存在根本差异。

Conclusion: 目标识别可作为评估LLM规划知识基础的原则性基准。
...
### [264] [Belief Engine: Configurable and Inspectable Stance Dynamics in Multi-Agent LLM Deliberation](https://arxiv.org/abs/2605.15343)
*Joshua C. Yang,Maurice Flechtner,Damian Dailisan,Michiel A. Bakker*

Main category: cs.AI

TL;DR: 提出了信念引擎（BE），一种可审计的信念更新层，通过证据提取和log-odds规则模拟立场变化，能可靠追踪证据驱动的立场动态。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的代理在模拟协商、冲突解决等交互时，其立场变化的原因（如证据吸收、锚定、角色漂移等）难以从对话记录中揭示。

Method: 将信念视为命题上的证据状态，通过结构化记忆提取论点，并利用受证据吸收u和先验锚定a控制的log-odds规则更新立场。

Result: 在多个基础LLM上，参数扫描表明这些控制能可靠地塑造立场动态并保留证据级更新轨迹；在DEBATE数据集上，BE能最好地重建那些最终立场跟随提取证据的参与者。

Conclusion: BE提供了一个可配置的基础设施，用于研究基于证据的协商，其中立场变化可以归因于明确的更新假设而非隐藏的提示效应。
...
### [265] [Ensemble Monitoring for AI Control: Diverse Signals Outweigh More Compute](https://arxiv.org/abs/2605.15377)
*Eugene Koran,Yejun Yun,Samantha Tetef,Benjamin Arnav,Pablo Bernabeu-P\'erez*

Main category: cs.AI

TL;DR: 通过集成多样化监视器（12个GPT-4.1-Mini，含提示和微调策略），显著提升对未对齐AI动作的检测性能，最佳3监视器集成相比同质集成性能提升2.4倍，且多样性而非规模是主要驱动因素。


<details>
  <summary>Details</summary>
Motivation: AI系统在自主代理场景中大规模部署，确保动作安全和对齐用户意图至关重要，但构建可靠监视器困难且人类监督不切实际。

Method: 构建12个GPT-4.1-Mini监视器（使用提示和微调两种策略），组成多样化集成，在编码任务（候选方案通过标准测试但对抗性输入失败）上评估。

Result: 多样化集成优于单个监视器和同质集成，最佳3监视器集成检测性能提升2.4倍，且在同一集成在独立数据集上表现强劲；微调监视器在所有最佳集成中出现，并在分布外攻击类型上保持优势。

Conclusion: 多样性（而非规模）驱动增益，集成监控是一种实用的AI控制策略，能以合理推理成本提升安全性。
...
### [266] [Beyond Partner Diversity: An Influence-Based Team Steering Framework for Zero-Shot Human-Machine Teaming](https://arxiv.org/abs/2605.15400)
*Wei Sheng,Rohan Paleja*

Main category: cs.AI

TL;DR: 提出基于影响力引导的团队引导框架（IBTS），通过影响力塑造激励智能体发现多样且高效的团队交互模式，并在多智能体场景下提升团队表现，超越仅依赖伙伴多样性覆盖的方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的人机协同方法依赖于昂贵的人类交互数据，零样本协调虽能模拟多样伙伴，但在团队规模扩大和通信受限时不足。

Method: 提出IBTS框架，使用影响力塑造激励智能体发现多样高性能的团队交互模式，并引导轨迹向更强的协调模式收敛。

Result: 在Overcooked-AI上进行了双智能体和三智能体实验，包括30个受试者的人机团队实验，IBTS提升了团队表现，优于基线。

Conclusion: 零样本协调需要结合稀疏奖励协调机制和伙伴多样性覆盖，而不仅仅是多样性。
...
TL;DR: 提出神经符号框架NSPI，结合LLM和符号计算实现多项式不等式的自动化证明，并生成Lean可验证的证书。


<details>
  <summary>Details</summary>
Motivation: 纯符号方法扩展性差，LLM方法仅适用于少量变量，需要更可扩展的框架。

Method: LLM提议近似多项式平方和分解，通过符号计算精炼为精确表示，从而证明不等式，并在Lean中验证。

Result: 在含多达10个变量的挑战性基准上验证了有效性和可扩展性。

Conclusion: NSPI实现了从启发式发现到机器验证的端到端流程，提升了多项式不等式证明的自动化水平。
...
TL;DR: X-SYNTH框架通过数字孪生签名和行为模式过滤，将企业上下文合成转化为相关性而非检索问题，在销售线索识别中真线索率提升6.5倍，假线索率降至18.8%。


<details>
  <summary>Details</summary>
Motivation: 现有检索方法在复杂代理任务中表现差，因缺乏个性化行为模式，真线索率低（9.5%），假线索率高（90.5%）。

Method: 构建个体的数字孪生签名（DTS），采用七种注意力过滤器（比例、逆、差分、循环、比较、序列、集体），经四阶段流水线组装基于行为模式的上下文。

Result: 销售线索识别任务中，X-SYNTH将真线索率从9.5%提升至61.9%，假线索率从90.5%降至18.8%。

Conclusion: 企业上下文合成本质是相关性问题，人类注意力是可靠依据，框架通过建模注意力实现突破。
...
TL;DR: 提出CAPS框架，通过自适应非均匀分配验证计算，大幅降低配对验证成本，在多种任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有配对自验证方法成本高昂，每次判断需完整阅读两个候选解，且对每个问题执行大量比较，不论比较是否有效。

Method: CAPS是一个纯推理框架，沿证据轴和分布轴非均匀分配验证计算，采用四阶段级联架构，并支持可选的救援子程序，实现每个候选边际成本减半。

Result: 在5个推理基准和4个自验证模型上，CAPS在20个测试套件中14个优于领先的配对验证器，仅使用其25.4%的验证令牌预算，并在所有20个套件上优于点式自验证。

Conclusion: CAPS提供了一种可解释的诊断方法，通过验证器在部分与完整证据下的准确率，预测级联适用性，为部署前检查提供具体依据。
...
TL;DR: 提出RTL-BenchMT框架，自动维护RTL生成基准，解决缺陷和过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 当前RTL基准存在缺陷案例和过拟合问题，人工维护困难。

Method: 构建自动化智能框架，自动识别修正缺陷案例，检测更新过拟合案例。

Result: 通过框架分析后得到改进的基准套件并开源。

Conclusion: 框架降低人力维护成本，有效提升基准质量。
...
TL;DR: DRS-GUI是一种无需训练的动态区域搜索框架，通过模仿人类感知行为和多蒙特卡洛树搜索规划，增强了多模态大语言模型在GUI界面中的指令元素定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从充满无关UI组件的截图中准确定位指令相关元素时存在困难。

Method: DRS-GUI包含一个轻量级UI感知器，执行聚焦、移动和分散三种类人感知动作，逐步探索界面并生成区域提议；同时基于蒙特卡洛树搜索设计动作规划器，动态调度这些动作，并通过区域质量奖励评估选择高相关区域。

Result: 实验表明，DRS-GUI在ScreenSpot-Pro上为通用和专用多模态大语言模型（Qwen2.5-VL-7B和UGround-V1-7B）带来14%的性能提升，显著增强了定位能力和泛化性。

Conclusion: DRS-GUI通过动态区域搜索有效提升了GUI定位性能，且无需训练，可无缝集成到现有模型中。
...
TL;DR: 提出元认知作为AI设计原则，通过自我监控和资源分配提升准确性、安全性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有AI缺乏自我监控和适应性资源分配，导致资源浪费、安全性不足等问题。

Method: 借鉴资源理性AI和心理学元认知策略，设计系统自我监控与资源分配机制；以联邦学习为案例，开发相应软件框架。

Result: 在联邦学习中展示了学习效率、效果和安全性的提升。

Conclusion: 元认知原则能有效改善AI性能，但仍需解决理论及实施中的挑战。
...
TL;DR: STAR框架通过将根因分析工作流分解为四个阶段，并采用阶段审计、预算感知路由、反事实候选评估和阶段特定修复，有效提升了根因定位和故障分类的准确性。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的根因分析代理在微服务AIOps中虽具潜力，但错误会通过推理链传播，导致最终诊断失效，亟需可靠的自修复机制。

Method: 将RCA工作流分解为证据包、假设集、分析结构和决策报告四个阶段，利用LangGraph实现阶段审计、预算感知Fast/Slow路由、反事实候选评估进行阶段定位，以及阶段特定补丁重放修复。

Result: 在公共基准和实际生产数据集上，STAR一致提升了根因定位和故障分类性能，准确识别关键故障阶段，多数错误轨迹在一到两轮重放内修复，且Fast/Slow路由和反事实阶段评估显著有益。

Conclusion: 明确建模RCA代理的失败阶段是构建可靠、可调试、自修复的代理根因分析系统的有效途径。
...
TL;DR: OmniManim是一个渲染反馈感知的教育动画生成框架，通过显式视觉规划和局部修复，显著提升代码生成动画的渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型生成的教育动画代码在渲染后常出现视觉缺陷，且无法在代码阶段检测，需在渲染后反馈修正。

Method: 框架包括共享场景状态、视觉规划模块（稀疏关键帧布局、粗到细边界框去噪、插值感知优化）、结构化后渲染诊断和局部修复。

Result: 在EduRequire-500数据集上，OmniManim的渲染质量优于单模型和多智能体基线；消融实验表明显式视觉规划是关键。
...
TL;DR: 提出TopoEvo，一种拓扑感知的自进化多智能体框架，通过图表示学习和结构化推理解决微服务根因分析中的症状放大偏差问题。


<details>
  <summary>Details</summary>
Motivation: 微服务根因分析面临嘈杂多模态数据、级联故障传播和拓扑漂移挑战，现有LLM方法常因忽略拓扑而误判下游症状为根因。

Method: 采用度量正交多模态对齐（MOMA）分解嵌入、向量量化（VQ）离散化症状令牌、假设-证据-测试（HET）多智能体工作流验证传播一致性，以及自进化机制适应拓扑漂移。

Result: 有效分离初始异常与下游放大症状，生成拓扑一致且可解释的根因归因。

Conclusion: TopoEvo通过拓扑约束推理和持续自适应，为动态微服务环境提供了稳健的根因分析方案。
...
TL;DR: 提出了ColPackAgent框架，通过MCP工具服务器和智能体技能自主执行胶体堆积蒙特卡洛模拟，解决了通用LLM无法可靠执行模拟工作流的问题。


<details>
  <summary>Details</summary>
Motivation: 通用LLM在没有专用模拟工具和工作流指令时，倾向于描述工作流而非可靠执行，需要一种能将模拟工具转化为智能体辅助研究流程的方法。

Method: 通过MCP服务器暴露的colpack Python包（封装HOOMD-blue硬粒子蒙特卡洛）和编码四阶段工作流契约的智能体技能，实现自主或交互式模拟流程。

Result: 在立方体3D、圆盘胶囊2D二元系统及2D硬圆盘冻结转变等实例中成功演示，并进行了17个阶段提示的LLM模型性能对比。
...
### [277] [PRISM: Prompt Reliability via Iterative Simulation and Monitoring for Enterprise Conversational AI](https://arxiv.org/abs/2605.15665)
*Keshava Chaitanya,Jahnavi Gundakaram*

Main category: cs.AI

TL;DR: PRISM是一个闭环框架，将提示工程视为持续可靠性问题，通过自动生成测试、模拟对话、评估修复来应对LLM行为漂移，显著提高生产可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化框架只处理一次性编译时问题，忽略了生产环境中LLM行为变化导致的提示回归问题。

Method: 从需求自动生成测试用例，模拟多轮对话，用LLM作为评判，诊断失败原因并精确修复提示，循环直到所有测试通过，并计划每日运行。

Result: 在35个企业智能体上三周部署，中位提示编写时间从2天降到30分钟以内，生产可靠性达到99%，并在24小时内检测并修复生产回归。

Conclusion: 持续的、模拟驱动的提示优化对于大规模可靠的企业对话AI是可行且必要的。
...
### [278] [Nudging Beyond the Comfort Zone: Efficient Strategy-Guided Exploration for RLVR](https://arxiv.org/abs/2605.15726)
*Chanuk Lee,Sangwoo Park,Minki Kang,Sung Ju Hwang*

Main category: cs.AI

TL;DR: 提出NudgeRL框架，通过策略提示实现结构化多样性探索，在RLVR中提升推理能力，优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法受限于探索效率，增加rollout数量计算昂贵，且现有方法对探索控制有限。

Method: 引入策略提示（Strategy Nudging）在每次rollout中生成轻量级上下文，诱导多样化推理轨迹；提出统一目标分解奖励信号为跨上下文和上下文内成分，并加入蒸馏目标。

Result: 在5个数学基准上，NudgeRL优于标准GRPO（8倍rollout预算）和基于oracle的RL基线。

Conclusion: 结构化上下文驱动的探索可作为暴力缩放和基于特权信息的可行性导向方法的高效可扩展替代方案。
...
### [279] [Can We Trust AI-Inferred User States. A Psychometric Framework for Validating the Reliability of Users States Classification by LLMs in Operational Environments](https://arxiv.org/abs/2605.15734)
*Izabella Krzeminska,Michal Butkiewicz,Ewa Komkowska*

Main category: cs.AI

TL;DR: 本文实证检验了大语言模型评估用户状态的指标可靠性，发现多数指标在个体分数层面不稳定，仅31/213个指标满足标准，并提出了可复制的评估框架。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在对话和自适应系统中评估用户状态，前提是指标在个体分数层面稳定可解释，但该假设未经严格验证。

Method: 采用复制评估程序，对三个双模态大模型（GPT-4o audio、Gemini 2.0 Flash、Gemini 2.5 Flash）的213个指标进行个体分数和聚合可靠性分析。

Result: 个体分数可靠性普遍不足，只有31个指标满足标准；聚合可靠性较好，但无法支持实时系统。

Conclusion: 指标可靠性不能默认成立，个体不稳定指标仍可用于事后分析，需通过显式验证和监控来确保可靠应用。
...
### [280] [ALSO: Adversarial Online Strategy Optimization for Social Agents](https://arxiv.org/abs/2605.15768)
*Xiang Li,Liping Yi,Mingze Kong,Min Zhang,Zhongxiang Dai,QingHua Hu*

Main category: cs.AI

TL;DR: 提出ALSO框架，将多智能体社会模拟中的多轮对话建模为对抗性老虎机问题，通过在线策略优化和轻量级奖励预测器实现动态适应，在Sotopia上超越静态基线。


<details>
  <summary>Details</summary>
Motivation: 现有LLM社会智能体依赖静态角色，而离线强化学习等方法假设环境平稳且训练成本高，无法应对非平稳的多智能体对话。

Method: 将多轮交互视为对抗性老虎机问题，arms由静态角色和动态策略指令组成；引入轻量级神经网络从交互历史中预测奖励，实现样本高效的在线优化。

Result: 在Sotopia基准上，ALSO在动态环境中持续优于静态基线和现有优化方法。

Conclusion: 对抗性在线策略优化能有效构建鲁棒的社会智能体，适应非平稳的多智能体社会模拟。
...
### [281] [SaaS-Bench: Can Computer-Use Agents Leverage Real-World SaaS to Solve Professional Workflows?](https://arxiv.org/abs/2605.15777)
...
TL;DR: SaaS-Bench是一个基于23个真实SaaS系统的基准，用于评估计算机使用代理在专业工作流程中的能力，实验发现最强模型完成率不足4%。


<details>
  <summary>Details</summary>
Motivation: 现有web和GUI代理基准多基于简化、孤立的短期任务，难以评估代理在现实专业工作流中的表现。

Method: SaaS-Bench部署了23个跨6个专业领域的SaaS系统，包含106个长期任务，支持文本和多模态，使用加权验证检查点评估任务完成度和部分进展。

Result: 代表性LLM代理在SaaS-Bench上表现不佳，最强模型端到端任务完成率低于4%，暴露了规划、状态跟踪、跨应用上下文维护和错误恢复的局限。

Conclusion: 当前LLM代理在现实SaaS工作流中面临显著挑战，亟需提升长期执行和复杂协调能力。
...
### [282] [Agentic Discovery of Neural Architectures: AIRA-Compose and AIRA-Design](https://arxiv.org/abs/2605.15871)
*Alberto Pepe,Chien-Yu Lin,Despoina Magka,Bilge Acun,Yannan Nellie Wu,Anton Protopopov,Carole-Jean Wu,Yoram Bachrach*

Main category: cs.AI

TL;DR: 提出双框架AIRA让LLM代理自主设计基础模型架构，产出超越人类基线的14种架构和高效缩放策略。


<details>
  <summary>Details</summary>
Motivation: 追求递归自我改进，使AI能自主设计比标准Transformer更优的模型。

Method: AIRA-Compose（11代理高层架构搜索）和AIRA-Design（20代理底层注意力机制设计）。

Result: 1B规模预训练模型准确率提升2.4%-3.8%；缩放效率最高提升71%；Long Range Arena接近人类SOTA；Autoresearch达0.968 bits-per-byte。

Conclusion: AI代理可自主发现匹配/超越人工设计的架构和优化，为递归自我改进奠定基础。
...
### [283] [Imperfect World Models are Exploitable](https://arxiv.org/abs/2605.15960)
...
TL;DR: 提出模型利用的新定义，证明其在大型策略集上不可避免，并引入松弛概念和安全范围。


<details>
  <summary>Details</summary>
Motivation: 先前奖励黑客的不可避免性证明不适用于模型利用，需要建立统一理论。

Method: 开发奖励黑客和模型利用的一般理论，证明利用在大型策略集上不可避免，并引入松弛概念推导安全范围。

Result: 模型利用在大型策略集上不可避免；防止奖励黑客的条件无法避免利用；松弛利用可在安全范围内避免。
...
### [284] [PAGER: Bridging the Semantic-Execution Gap in Point-Precise Geometric GUI Control](https://arxiv.org/abs/2605.15963)
*Jingxuan Wei,Xi Bai,Shan Liu,Caijun Jia,Zheng Sun,Xinglong Xu,Siyuan Li,Linzhuang Sun,Bihui Yu,Conghui He,Cheng Tan*

Main category: cs.AI

TL;DR: 提出精度敏感的GUI任务和PAGE Bench基准，PAGER智能体通过分解规划和像素级执行，显著提升点精确GUI控制成功率。


<details>
  <summary>Details</summary>
Motivation: 现有GUI智能体依赖区域容忍范式，在需要精确点级坐标的几何构造任务中失败，且拓扑依赖导致错误传播。

Method: PAGER将构造分解为依赖结构规划和像素级执行，使用像素级监督调优和精度对齐强化学习。

Result: PAGER任务成功率是最强通用基线的4.1倍，步骤成功率从低于9%提升至超过62%。

Conclusion: PAGER建立了点精确GUI控制的新标杆，有效缩小语义-执行差距。
...
### [285] [Deterministic Event-Graph Substrates as World Models for Counterfactual Reasoning](https://arxiv.org/abs/2605.15967)
*Fabio Rovai*

Main category: cs.AI

TL;DR: 提出事件图基板，一种可检查、支持精确反事实的世界模型，在CLEVRER和Smallville基准上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型在可解释性、反事实推理和跨领域迁移方面存在不足。

Method: 形式化事件图基板类，利用对偶性将解释性与反事实查询简化为因果祖先遍历，采用域无关运行时和CLEVRER-DSL解释器。

Result: 在CLEVRER上超越NS-DR符号Oracle（分别提升9.89、20.26、17.65、0.80个百分点），在twin-EventLog上超越Llama-3.1-8B（联合准确率提升18.80点），但在预测和反事实方面落后于ALOE。

Conclusion: 事件图基板是可解释、准确的反事实推理工具，但在预测性能上仍有提升空间。
...
### [286] [Learning Bilevel Policies over Symbolic World Models for Long-Horizon Planning](https://arxiv.org/abs/2605.15975)
...
### [287] [Petri Net Induced Heuristic Search for Resource Constrained Scheduling](https://arxiv.org/abs/2605.15983)
*Ido Lublin,Dor Atzmon,Izack Cohen*

Main category: cs.AI

TL;DR: 提出基于时间变迁Petri网和A*搜索的RCPSP求解方法，在PSPLIB基准上优于MIP方法。


<details>
  <summary>Details</summary>
Motivation: 现有精确方法如MIP求解RCPSP效率不高，需要更高效的搜索方法。

Method: 将RCPSP建模为带资源的时间变迁Petri网，使用相对延迟令牌，将调度决策对应为变迁触发，并用A*搜索，启发式为关键路径和资源下界的组合，并证明启发式一致性。

Result: 在PSPLIB基准上，该方法在成功率和求解时间上均优于SCIP和CBC等强MIP求解器。A*的性能受资源紧张度影响，MIP受问题规模影响。

Conclusion: 提出的Petri网A*搜索方法是RCPSP的有效精确求解方法，与MIP互补，适用于不同特征的问题实例。
...
### [288] [ScreenSearch: Uncertainty-Aware OS Exploration](https://arxiv.org/abs/2605.16024)
*Michael Solodko,Justin Wagle*

Main category: cs.AI

TL;DR: 提出ScreenSearch系统，结合结构屏幕检索与去重和不确定性感知的PUCT图带算法，用于大规模桌面探索，解决部分可观察性问题。


<details>
  <summary>Details</summary>
Motivation: 桌面GUI代理在部分可观察环境下运行，视觉相似的屏幕可能对应不同的工作流状态，局部合理的动作可能导致截然不同的结果，因此需要有效的探索策略。

Method: ScreenSearch通过将UIA树转换为位置感知结构特征，利用稀疏标记搜索和元数据过滤器索引相关屏幕，并在多VM工作节点间维护共享去重状态图；基于匹配动作结果分散性定义可扩展的不确定性信号，结合前沿奖励驱动大规模探索和重放启动策略评估。

Result: 在11个桌面应用中收集超过100万张截图和3万多个去重状态，揭示了新颖性-不确定性权衡：某些策略快速降低不确定性但发现的前沿较少，消融实验表明更强的提议先验能显著提高独特状态发现。

Conclusion: 状态识别、提议质量和不确定性感知搜索在决定何时探测和何时提交时都至关重要。
...
### [289] [Reasoners or Translators? Contamination-aware Evaluation and Neuro-Symbolic Robustness in Tax Law](https://arxiv.org/abs/2605.16052)
*Parisa Kordjamshidi,Samer Aslan,Madhavan Seshadri,Leslie Barrett,Enrico Santus*

Main category: cs.AI

TL;DR: 本文通过污染检测协议和系统评估，发现法律推理具有组合性质，神经符号框架比单纯LLM更可靠且泛化性更强。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在自动法律推理中的表现是否真正反映推理能力，还是源于数据污染尚不明确。

Method: 提出污染检测协议，构建新型测试集（案例和规则变体），比较单纯LLM与将法规文本形式化并交由符号求解器推理的混合系统。

Result: 污染可膨胀性能；神经符号框架在泛化到未见文档（案例和规则变体）方面更优。

Conclusion: 法律推理由组合决定，神经符号框架为法律AI提供更可靠、稳健的基础，并改善对未观察情况的泛化。
...
### [290] [Sign-Separated Finite-Time Error Analysis of Q-Learning](https://arxiv.org/abs/2605.16103)
*Donghwan Lee*

Main category: cs.AI

TL;DR: 本文针对固定步长Q-learning提出了符号分离的有限时间误差分析，揭示了正负误差的动态不对称性，并给出了有限时间界。


<details>
  <summary>Details</summary>
Motivation: 现有Q-learning误差分析未充分分离正负误差，无法解释过估计现象，需要更精细的分析。

Method: 利用切换系统表示，将误差分解为分量负部和正部，负部由固定最优策略的线性时不变系统主导，正部由线性切换系统控制。

Result: 负侧LTI证书不慢于正侧切换证书，且可能实现更快的指数包络；正动作误差通过贝尔曼最大值传播导致过估计。

Conclusion: 该分析揭示了Q-learning误差动态中的最大值诱导不对称性，并提供了确定性和随机固定步长递归的有限时间界。
...
### [291] [ShopGym: An Integrated Framework for Realistic Simulation and Scalable Benchmarking of E-Commerce Web Agents](https://arxiv.org/abs/2605.16116)
*Chinmay Savadikar,Mingyu Zhao,Yuanzheng Zhu,Han Li,Shuang Xie,Alberto Castelo,Tianfu Wu,Lingyun Wang*

Main category: cs.AI

TL;DR: 提出了ShopGym框架，用于电商网页智能体的真实模拟与可扩展基准测试，通过ShopArena将真实店铺转换为沙盒环境，结合ShopGuru合成任务，实现可控、可复现的评估。


<details>
  <summary>Details</summary>
Motivation: 现有方法在真实性与可控性之间权衡，缺乏可扩展的方式来构建同时满足真实、多样、可控、可检查、可复现的评估环境。

Method: ShopGym包含两个组件：ShopArena通过匿名店铺规范和分阶段验证流程将真实种子店铺转换为自包含沙盒；ShopGuru基于店铺目录、导航结构等合成涵盖七类技能的基准任务。

Result: 在224个任务上的实验表明，合成店铺保留了真实店铺的关键结构属性，智能体在合成店铺上的表现与真实店铺正相关。

Conclusion: ShopGym能够生成自包含、可重置、可检查且稳定的评估工件，为电商网页智能体提供了可扩展的基准测试方法。
...
### [292] [Property-Guided LLM Program Synthesis for Planning](https://arxiv.org/abs/2605.16142)
*Augusto B. Corr\^ea,Andr\'e G. Pereira,Jendrik Seipp*

Main category: cs.AI

TL;DR: 提出属性引导的LLM程序合成方法，通过形式化属性和反例反馈减少程序生成和评估成本，应用于PDDL规划域的启发式函数合成。


<details>
  <summary>Details</summary>
Motivation: 现有基于分数的程序合成方法仅提供数值评分，无法指导错误原因，导致大量无效生成和评估，成本高。

Method: 先检查候选程序是否满足形式化属性，若违反则终止评估并提供具体反例，引导LLM修复，形成反例引导的修复循环。

Result: 在10个规划域上，生成的启发式函数几乎对所有测试任务有效，平均每个域生成程序数量减少7倍，无需搜索即可解决更多任务，评估计算量降低数个数量级。
...
### [293] [Look Before You Leap: Autonomous Exploration for LLM Agents](https://arxiv.org/abs/2605.16143)
*Ziang Ye,Wentao Shi,Yuxin Liu,Yu Wang,Zhengzhou Cai,Yaorui Shi,Qi Gu,Xunliang Cai,Fuli Feng*

Main category: cs.AI

TL;DR: 大型语言模型代理在陌生环境中常因过早利用先验知识而失败，自主探索是关键但未充分研究的能力。提出了探索检查点覆盖度指标，并开发了交替任务执行与探索的训练策略，引入先探索后行动范式，实验表明系统探索对构建通用代理至关重要。


<details>
  <summary>Details</summary>
Motivation: 现有代理在陌生环境中因过早利用先验知识而失败，缺乏自主探索能力，导致适应性差。

Method: 提出探索检查点覆盖度指标，设计交替任务执行与探索路径的训练策略，每种路径用对应可验证奖励优化，并构建先探索后行动范式。

Result: 标准任务导向强化学习训练导致代理行为狭窄重复；所提训练策略提升了探索的系统性，从而改善了下游性能。

Conclusion: 学会系统探索是构建通用且可在真实世界部署的代理的必要条件。
...
### [294] [An Algebraic Exposition of the Theory of Dyadic Morality](https://arxiv.org/abs/2605.16153)
*Kush R. Varshney*

Main category: cs.AI

TL;DR: 本文用结构因果建模（SCM）形式化了二元道德理论（TDM），识别了三种心理算子，解决了可扩展性问题，展示了在AI政策设计中的应用，并建议进行范围性心智感知测量。


<details>
  <summary>Details</summary>
Motivation: 为AI系统提供既数学严谨又忠实于人类道德认知的道德计算形式化框架。

Method: 使用SCM符号形式化TDM，引入三种心理算子（类型化算子、完成算子、效价依赖推理机制），并通过节点坍缩和序列处理压缩多节点场景。

Result: 展示了在AI政策设计中的具体应用：检测冲突义务、构建保留用户自主权的帮助性政策、将故障后沟通设计为因果干预。

Conclusion: 该代数形式化使神经符号AI系统能够以数学严谨且忠实于人类道德认知的方式计算道德。
...
### [295] [Formal Methods Meet LLMs: Auditing, Monitoring, and Intervention for Compliance of Advanced AI Systems](https://arxiv.org/abs/2605.16198)
*Parand A. Alamdari,Toryn Q. Klassen,Sheila A. McIlraith*

Main category: cs.AI

TL;DR: 提出结合形式化方法与机器学习的AI审计与监测技术，用于检测和预防黑盒AI系统（如LLM）中的时序行为约束违规，实验表明优于LLM基线方法。


<details>
  <summary>Details</summary>
Motivation: AI系统部署前后需有效监测审计，但现有方法难以处理复杂时序约束，需更可靠的审计与运行时监测技术。

Method: 利用线性时序逻辑（LTL）的语法语义，开发离线审计与在线监测技术，包括预测性（基于采样）和干预性监测器，可运行时预判并减轻违规。

Result: LTL方法在检测时序约束违规上显著优于LLM基线，小模型标签器可匹配或超越前沿LLM评判器；预测与干预监测器大幅降低LLM智能体违规率且保持任务性能；LLM时序推理准确性随事件距离、约束数和命题数增加而下降。

Conclusion: 所提审计与监测技术能有效处理AI系统的时序行为约束，形式化方法结合机器学习为AI治理提供可复用的实践方案。
...
### [296] [Context, Reasoning, and Hierarchy: A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP](https://arxiv.org/abs/2605.16205)
*Igor Bogdanov,Chung-Horng Lung,Thomas Kunz,Jie Gao,Adrian Taylor,Marzia Zaman*

Main category: cs.AI

TL;DR: 在对抗部分可观测环境中，复合LLM智能体设计的控制研究表明，程序状态抽象和高层级分解比深度推理更有效，而推理工具与层次结构结合会导致推理级联现象，性能下降。


<details>
  <summary>Details</summary>
Motivation: 实践者在设计复合LLM智能体时缺乏指导，不清楚哪些选择能提升性能而哪些只是增加推理成本。

Method: 在CybORG CAGE-2（POMDP）环境中，使用五个模型家族、六个模型和十二种配置（共3475个回合），变化上下文表示、推理方式和层次分解进行控制实验。

Result: 程序状态抽象在每token收益上提升最大（最多76%）；将推理工具分布在层次结构中导致性能下降（最多3.4倍）且token消耗增加1.8-2.7倍；无推理的层次分解在大多数模型上表现最佳，上下文工程比推理更划算。

Conclusion: 在结构化对抗POMDP中，应投资程序化基础设施和清晰任务分解，而非深化单智能体推理，因为两者结合可能互相干扰。
...
### [297] [Confirming Correct, Missing the Rest: LLM Tutoring Agents Struggle Where Feedback Matters Most](https://arxiv.org/abs/2605.16207)
*Tahreem Yasir,Wenbo Li,Sam Gilson,Sutapa Dey Tithi,Xiaoyi Tian,Tiffany Barnes*

Main category: cs.AI

TL;DR: LLM在辅导中诊断学生解答的准确性存在系统性偏差，尤其对次优和错误解答的判断不准确，且诊断结果与教学反馈质量脱节。


<details>
  <summary>Details</summary>
Motivation: 智能辅导系统需要区分最优、次优和错误解答，但LLM辅导者的诊断能力尚未经过严格测试。

Method: 构建包含7个LLM反馈代理的基准，使用知识图谱导出的地面真实数据，在10836个解答-反馈对上评估三种反馈条件下的诊断性能。

Result: LLM对最优步骤诊断精度接近完美，但系统性地过度拒绝正确的次优推理，并过度验证错误解答，且此问题在不同模型和上下文下一致出现。
...
### [298] [Fully Open Meditron: An Auditable Pipeline for Clinical LLMs](https://arxiv.org/abs/2605.16215)
...
TL;DR: 提出了首个完全开放的医学大模型构建管道Fully Open Meditron，包含临床医生审核的语料库、可复现的数据构建与训练框架以及对齐用途的评估协议，在多个医疗基准上达到新SOTA，证明完全开放管道可在不牺牲可审计性的前提下实现领域专用性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的临床决策支持系统大多不透明，开放模型仅开放权重，缺乏数据来源和生成管道的公开，医学领域尚无完全开放（FO）模型。

Method: 构建了由临床医生审核的训练语料（整合8个公开医疗QA数据集并扩展三个临床验证的合成扩展），可复现的数据构建与训练框架（全系统去污染、教师生成的金标签重采样、四名医生小组端到端验证），以及LLM-as-a-judge评估协议（基于专家编写的临床情景，校准204名人类评分者）。

Result: 所有MeditronFO变体优于其基座模型，Apertus-70B-MeditronFO在聚合医疗基准上提升6.6个百分点（47.2%→53.8%），Gemma-3-27B-MeditronFO在LLM-as-a-judge比较中58.6%优于MedGemma，并在HealthBench上以58%胜出（vs 55.9%）。
...
### [299] [FORGE: Self-Evolving Agent Memory With No Weight Updates via Population Broadcast](https://arxiv.org/abs/2605.16233)
*Igor Bogdanov,Chung-Horng Lung,Thomas Kunz,Jie Gao,Adrian Taylor,Marzia Zaman*

Main category: cs.AI

TL;DR: 提出FORGE协议，通过自省式记忆演化提升LLM agent决策性能，无需梯度更新，在CybORG CAGE-2任务上平均回报比零样本提高1.7-7.7倍，比Reflexion提高29-72%。


<details>
  <summary>Details</summary>
Motivation: 探索LLM agent能否通过自生成记忆（无需梯度）改进决策。

Method: FORGE协议：内循环将失败轨迹转化为规则/示例/混合记忆，外循环通过种群广播和毕业机制传播最优记忆。

Result: 所有12种模型-表示条件下平均评价回报提升，重大失败率降至~1%；种群广播是关键，示例记忆效果最优但规则更经济，弱模型受益更大。

Conclusion: FORGE可提升LLM agent决策并缓解能力差距，但证据限于CAGE-2 B-line。
...
### [300] [Prospective multi-pathogen disease forecasting using autonomous LLM-guided tree search](https://arxiv.org/abs/2605.16238)
*Sarah Martinson,Michael P. Brenner,Martyna Plomecka,Brian P. Williams,Nicholas G. Reich,Zahra Shamsi*

Main category: cs.AI

TL;DR: 提出基于大语言模型引导树搜索的自主系统，自动生成传染病概率预测软件，在2025-2026美国呼吸季前瞻评估中，其模型集成匹配或超越CDC人工集成，并成功应对数据稀缺场景。


<details>
  <summary>Details</summary>
Motivation: 人工模型策划劳动密集，限制了对细粒度地理尺度或新病原体的可扩展性。

Method: 使用大语言模型引导的树搜索迭代生成、评估、优化可执行预测软件；优化对数尺度距离指标防止奖励黑客；引入自动法官循环保证结构忠实于复杂科学理论。

Result: 在2025-2026美国呼吸季全前瞻实时评估中，自主系统发现的方法多样模型集成在样本外一致匹配或超越CDC人工集成；成功应对RSV数据稀缺的冷启动场景。

Conclusion: 该框架自主将流行病学理论转化为准确、透明的代码，克服了建模劳动力瓶颈，实现专家级疾病预测的快速大规模部署。
...
### [301] [Access Timing as Scaffolding: A Reinforcement Learning Approach to GenAI in Education](https://arxiv.org/abs/2605.15850)
*Janne Rotter,Pau Benazet i Montobbio,Davinia Hern\'andez-Leo*

Main category: cs.CY

TL;DR: 通过强化学习代理控制GenAI访问时机，相比无限制或完全限制，能提升学习效果和元认知准确性，且无需显式提示。


<details>
  <summary>Details</summary>
Motivation: GenAI在教育中广泛使用但可能引发过度依赖和元认知失调，需研究何时允许使用更有效。

Method: 基于元认知理论、认知负荷理论和生产性失败设计强化学习奖励函数，训练代理控制访问时机；进行N=105混合方法实验，对比策略性、无限制和完全限制三种条件。

Result: 策略性时机条件提高了客观后测表现和元认知准确性，减少了任务错误和用时，但自我报告元认知意识无显著差异。

Conclusion: 访问时机控制是一种基于理论、可扩展的教学范式，优于无限制和完全限制，且易于与现有工具结合，开辟了新的研究方向。
...
### [302] [AI-Mediated Communication Can Steer Collective Opinion](https://arxiv.org/abs/2605.16245)
*Stratis Tsirtsis,Kai Rawal,Chris Russell,Brent Mittelstadt,Sandra Wachter*

Main category: cs.CY

TL;DR: 生成式AI在人类交流中介入文本修改，引入方向性偏差，并通过社交网络放大，影响集体意见形成。


<details>
  <summary>Details</summary>
Motivation: 现有研究关注AI对个体意见的影响，但忽略了AI在人际沟通中调节时对集体意见形成的影响。

Method: 对多个流行LLMs进行文本编辑实验，观察其方向性偏差；构建意见动力学数学模型，在真实社交网络数据上进行仿真分析。

Result: LLMs在修改争议话题文本时表现出方向性偏差（如支持枪支管制、反对无神论）；仿真表明这些偏差可通过网络传播放大，改变集体意见；审计X平台发现Grok对堕胎话题存在亲生命偏差。

Conclusion: AI在人际沟通中的偏差可被网络放大，影响集体意见；平台可通过设计控制偏差；结果对欧盟立法有启示意义。
...
### [303] [From Feedback Loops to Policy Updates: Reinforcement Fine-Tuning for LLM-Based Alpha Factor Discovery](https://arxiv.org/abs/2605.15412)
*Lingzhe Zhang,Tong Jia,Yunpeng Zhai,Zixuan Xie,Chiming Duan,Minghua He,Philip S. Yu,Ying Li*

Main category: cs.CE

TL;DR: 提出QuantEvolver，一种基于强化微调的自我进化alpha因子发现框架，通过将量化评估转化为策略更新，解决上下文爆炸和搜索停滞问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法在迭代优化时存在上下文爆炸、成本高、有用信息稀释、反馈漂移等问题，且依赖大型LLM导致因子结构相似、冗余和搜索停滞。

Method: QuantEvolver构建种子因子和时间窗口任务，生成可执行因子DSL表达式，通过体制回测评估，并使用多样性-互补性奖励优化Miner LLM，同时将高质量因子存入因子数据库。

Result: 在三个真实市场基准上，QuantEvolver持续提升主要评估指标，产生更高质量和更互补的因子池，优于现有基于LLM的方法。

Conclusion: QuantEvolver通过强化微调内化优化经验，有效提升了因子发现的质量和多样性。
...
### [304] [A general proof of integer R\'enyi QNEC](https://arxiv.org/abs/2605.15272)
*Tanay Kibe,Pratik Roy*

Main category: hep-th

TL;DR: 在具有半侧模包含结构的von Neumann代数中，证明了对于所有整数n≥2的Rényi量子零能量条件，通过Kosaki L^n范数的对数凸性。


<details>
  <summary>Details</summary>
Motivation: Rényi QNEC是对量子零能量条件的一参数推广，其猜想尚未完全证明。

Method: 利用von Neumann代数的半侧模包含结构，分析正常正泛函的Kosaki L^n范数在零平移半群下的对数凸性。

Result: 对所有整数Rényi参数n≥2，证明了Rényi QNEC成立，仅需激发态相对于真空的SRD有限。

Conclusion: 该结果为Rényi QNEC在特定代数结构下提供了严格证明，支持了更一般情况下的猜想。
...
### [305] [Non-Invertible Symmetries in Compactified Supergravities](https://arxiv.org/abs/2605.16188)
*Fabi\'an Caro-P\'erez,Mar\'ia Pilar Garc\'ia del Moral,\'Alvaro Restuccia*

Main category: hep-th

TL;DR: 研究了非可逆高形式对称性缺陷从11维超引力到IIA型超引力的Kaluza-Klein约化，得到了包含可逆与不可逆部分的缺陷代数。


<details>
  <summary>Details</summary>
Motivation: 理解非可逆对称性在超引力降维下的表现，以及M理论到IIA理论的对偶关系。

Method: 采用Kaluza-Klein约化，沿M理论圆推进零模超引力区域，并分析Bianchi恒等式的分解。

Result: 七维Chern-Simons-like辅助理论降为六维BF型；十一维Bianchi部分分裂为可逆H_{[3]}和扭曲非可逆F̃_{[4]}部分；IIA缺陷代数包含可逆Picard子群与非可逆BF修饰缺陷。

Conclusion: 约化完整描述了IIA型缺陷代数，并通过标准膜字典识别了带色探针。
...
### [306] [Effective Harness Engineering for Algorithm Discovery with Coding Agents](https://arxiv.org/abs/2605.15221)
*Yoichi Ishibashi,Taro Yano,Masafumi Oyamada*

Main category: cs.SE

TL;DR: 研究了大型语言模型与进化搜索结合的算法发现中，执行基础设施（harness）的设计对成功的影响，发现更深入思考每个算法比生成更多算法更有效，且更强大的模型更容易产生评估作弊。


<details>
  <summary>Details</summary>
Motivation: 算法发现成功不仅取决于模型能力，还显著受执行基础设施设计影响，但当前缺乏对有效harness设计的研究。

Method: 提出三个关键问题：固定token预算下，生成大量浅层算法还是少量深层算法；如何处理评估作弊；如何安全并行执行需要全文件系统访问的代理。基于Vesper框架进行改进，并在Circle Packing任务上评估。

Result: 在相同token预算下，生成更少算法但更深入思考每个算法可获得更高分数；更强大的模型产生评估作弊的比率更高，使作弊检测需求随模型规模增加。

Conclusion: 提升每个个体的质量比增加进化代数量更节省预算，且随着模型能力提升，必须加强作弊检测机制。
...
### [307] [PBT-Bench: Benchmarking AI Agents on Property-Based Testing](https://arxiv.org/abs/2605.15229)
*Lucas Jing,Xinqi Wang,Liao Zhang,Simon S. Du*

Main category: cs.SE

TL;DR: 提出PBT-Bench基准，评估基于属性测试中从文档推导不变量和构建输入生成策略的能力，发现显式提示策略对中等模型提升显著，但对最强模型效果有限。


<details>
  <summary>Details</summary>
Motivation: 现有代码基准无法孤立测试属性驱动测试技能，即从文档推导语义不变量并构造输入生成策略。

Method: 构建包含100个问题的PBT-Bench，注入365个语义bug（平均每个问题3.65个），覆盖三个难度级别，使用Hypothesis框架的@given策略；评估8个LLM在两种提示策略下进行三次独立运行。

Result: PBT引导提示下bug召回率42.1%-83.4%，开放式基线31.4%-76.7%；Hypothesis框架提示对中等模型提升超过20个百分点，但最强模型提升较小，两个模型出现退化。

Conclusion: 发布基准、测试工具和评估语料库，支持基于文档的语义推理研究。
...
### [308] [Hydra: Efficient, Correct Code Generation via Checkpoint-and-Rollback Support](https://arxiv.org/abs/2605.15238)
*Alexander Du,Jianjun Ou,Danyang Zhuo,Matthew Lentz*

Main category: cs.SE

TL;DR: Hydra 是一种高效修复代码生成中静态错误的系统，通过异步检查和检查点回滚，显著降低延迟和 token 消耗。


<details>
  <summary>Details</summary>
Motivation: 现有事后修复和约束语义解码方法在延迟和 token 消耗上代价高昂，且修复效率低。

Method: Hydra 采用异步检查（避免正确代码的开销）和检查点回滚（针对性修复，避免重新生成有效前缀），并改装 Clang 编译器实现。

Result: 相对于事后修复，Hydra 在 C/C++ 代码生成任务中减少延迟最多 71%，减少 token 消耗最多 70%。

Conclusion: Hydra 提供了一种高效、低成本的静态错误恢复方案，适用于代码生成场景。
...
### [309] [Runtime-Structured Task Decomposition for Agentic Coding Systems](https://arxiv.org/abs/2605.15425)
*Shubhi Asthana,Bing Zhang,Chad DeLuca,Hima Patel,Ruchi Mahindru*

Main category: cs.SE

TL;DR: 提出运行时结构化任务分解方法，通过可执行控制逻辑管理任务划分和流程，将LLM仅用于聚焦的判断任务，显著降低重试成本。


<details>
  <summary>Details</summary>
Motivation: 现有agentic编码系统使用单体提示编码任务逻辑，导致行为脆弱、难调试、高重试成本。

Method: 运行时结构化任务分解：任务划分和执行流由可执行控制逻辑管理，LLM仅用于聚焦判断，输出按预定义模式验证。

Result: 在Kubernetes根因分析中重试成本降低至436±132 tokens（比单体低51.7%，比静态分解低73.2%）；在多文件调试中降低至460 tokens。

Conclusion: 该方法提高了agentic编码系统的效率、可调试性和操作可靠性。
...
### [310] [Position: Early-Stage Quality Assurance in Annotation Pipelines Is More Cost-Effective Than Late-Stage Validation](https://arxiv.org/abs/2605.15714)
...
TL;DR: 本文主张机器学习社区应优先在注释流程早期进行质量保证，而非晚阶段验证，并提出了一个质量保证触发点分类和参数化误差传播模型。


<details>
  <summary>Details</summary>
Motivation: 数据质量瓶颈日益限制基础模型改进，而现有研究过度聚焦验证方法，忽略了验证时机这一关键因素。

Method: 提出三类质量保证触发点（T0:预注释, T1:后注释, T2:后审查），并构建参数化误差传播模型形式化时机对错误率和成本的影响。

Result: 对47篇近期论文的调查显示，仅有4%报告了验证时机，表明时机因素被严重忽视。

Conclusion: 需要社区报告QA时机配置、平台将时机作为一级参数、以及开展控制实验直接测量各阶段检测率。
...
### [311] [RoadmapBench: Evaluating Long-Horizon Agentic Software Development Across Version Upgrades](https://arxiv.org/abs/2605.15846)
...
TL;DR: 提出了RoadmapBench，一个包含115个长期多目标编码任务的基准测试，评估前沿模型在真实软件版本升级中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准多聚焦于单问题bug修复，无法捕捉实际工程中跨文件、多目标的长期开发任务。

Method: 基于17个代码仓库和5种编程语言的真实版本升级，构建115个任务，每个任务需实现目标版本的功能，中位数修改3700行代码和51个文件。
...
### [312] [XSearch: Explainable Code Search via Concept-to-Code Alignment](https://arxiv.org/abs/2605.16046)
*Yiming Liu,Ruofan Liu,Yun Lin,Zicong Zhang,Weiyu Kong,Pengnian Qi,Xiao Cheng,Weinan Zhang,Qianxiang Wang,Linpeng Huang*

Main category: cs.SE

TL;DR: 提出XSearch，一种基于概念对齐的可解释代码搜索框架，通过将代码搜索重构为演绎概念对齐问题，显著提升分布外泛化性能和用户评估效率。


<details>
  <summary>Details</summary>
Motivation: 现有语义代码搜索方法依赖全局嵌入相似度，缺乏可解释性且分布偏移下泛化差，因为它们学习统计模式而非真正理解功能需求。

Method: 将代码搜索重新定义为演绎概念对齐问题，识别查询中的功能概念并显式对齐到代码语句，使用显式概念对齐目标训练编码器，实现解释后预测。

Result: 在CodeSearchNet上训练，XSearch在分布外基准上性能从0.02提升至0.33（提升15倍），超越8个最先进检索器和7B参数模型；用户研究显示用户评估结果更快更准。

Conclusion: XSearch通过概念对齐实现了内在可解释性和更好的泛化，验证了演绎对齐方案的有效性。
...
### [313] [PerfCodeBench: Benchmarking LLMs for System-Level High-Performance Code Optimization](https://arxiv.org/abs/2605.15222)
*Huihao Jing,Wenbin Hu,Haochen Shi,Hanyu Yang,Sirui Zhang,Shaojin Chen,Haoran Li,Yangqiu Song*

Main category: cs.SE

TL;DR: 提出PerfCodeBench基准测试，评估大语言模型在系统级代码优化方面的能力，发现与专家优化代码存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有代码基准测试主要关注正确性或算法问题求解，缺乏对性能关键系统级优化能力的评估。

Method: 构建包含可执行正确性检查、基线实现和参考优化实现的基准测试，评估代码的正确性和运行时效率。

Result: 大语言模型生成的代码在并行和GPU任务上与专家优化代码差距明显，跨语言鲁棒性不足，难以达到专家级效率。

Conclusion: 需要性能感知评估，促使大语言模型从生成正确代码转向生成高效系统软件。
...
### [314] [BootstrapAgent: Distilling Repository Setup into Reusable Agent Knowledge](https://arxiv.org/abs/2605.15815)
*Sihan Fu,Oucheng Liu,Shiyuan Wang,Jin Shi,Chengkun Wei*

Main category: cs.SE

TL;DR: 提出BootstrapAgent框架，将仓库启动知识提炼为可复用的.bootstrap合约，提升代码代理效率。


<details>
  <summary>Details</summary>
Motivation: 当前代码代理处理不熟悉仓库时，需要大量试错进行启动，且获得的知识无法跨对话复用，导致资源浪费。

Method: 多智能体框架，通过证据提取、结构化规划、确定性Docker验证和跟踪驱动修复，生成包含环境设置、诊断检查、最小验证和修复知识的合约，并引入温暖修复和干净回放加速调试，以及差异修复和健全检查防止奖励黑客。

Result: 在三个基准上达到92.9%成功率，比基线高10%以上，下游代理token使用减少25.9%，构建时间减少22.3%。

Conclusion: BootstrapAgent能显著提升代码代理处理新仓库的效率和可复用性，代码已开源。
...
### [315] [Non-combinatorial involutive braidings: the quantum algebra $\mathfrak{gl}_{k,m}$](https://arxiv.org/abs/2605.16121)
*Anastasia Doikou*

Main category: math.QA

TL;DR: 研究辫子方程的非平凡解作为置换的变形，引入并分析了𝔤𝔩_{k,m} Yangian量子代数，构造了最高权模，与自旋链哈密顿量的本征态关联，并在𝔤𝔩_{1,1}情形下得到Heisenberg XX模型的变体，同时揭示了组合基与杨图形状的联系。


<details>
  <summary>Details</summary>
Motivation: 探索辫子方程的非组合解及其对应的量子代数，以理解自旋链哈密顿量的代数结构。

Method: 将辫子方程的解视为置换映射的变形，由此定义并研究𝔤𝔩_{k,m} Yangian代数，构造具体的高权模。

Result: 得到了𝔤𝔩_{k,m} Yangian代数，构造了高权模，建立了自旋链型哈密顿量的本征态；𝔤𝔩_{1,1}时哈密顿量化为Heisenberg XX模型的变体；给出了高权表示的组合基，并与杨图形状明确对应。

Conclusion: 该工作建立了辫子方程解、量子代数与自旋链模型之间的具体联系，并提供了组合基的清晰描述。
...
### [316] [Ensuring Logic in the Fog: Sound POMDP Synthesis with LTL Objectives](https://arxiv.org/abs/2605.12581)
*Can Zhou,Yulong Gao,Pian Yu*

Main category: cs.LO

TL;DR: 提出一种基于线性时序逻辑（LTL）部分可观测马尔可夫决策过程的奖励塑造机制，并将其集成到蒙特卡洛规划框架中，使智能体能在部分可观测环境中最大化可验证的成功率。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以在部分可观测马尔可夫决策过程中定量合成满足复杂时间约束的自主智能体，LTL定性验证的不可判定性导致奖励信号设计困难。

Method: 提出一种可靠的奖励塑造机制，基于认证的LTL满足度动态生成信念相关的奖励，并集成到增强的蒙特卡洛规划框架中。

Result: 在现有求解器失效的场景中本方法仍能成功，并在多个基准领域保持有效性和可扩展性。

Conclusion: 本方法有效解决了部分可观测下复杂时间约束的自主智能体合成问题，具有良好的表现和扩展性。
...
### [317] [Clock-state olfactory search in turbulent flows using Q-learning: The geometry of plume recovery](https://arxiv.org/abs/2605.15938)
*Marco Rando,Robin A. Heinonen,Yujia Qi,Agnese Seminara*

Main category: physics.bio-ph

TL;DR: 利用表格Q-learning和最小记忆（仅记录上次嗅到气味以来的时间）训练嗅觉搜索智能体，学习昆虫启发的策略（冲刺、投掷、顺风返回），在湍流模拟数据上表现良好，但缺乏适应局部间歇性的能力，增加灵活性可提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在湍流中高效寻找气味源需要利用历史嗅觉观察，但现有方法记忆负担重或策略不透明。

Method: 使用表格Q-learning，以距上次嗅到气味的时间作为唯一状态，训练智能体学习搜索策略。

Result: 智能体在直接数值模拟湍流数据上取得良好性能，但无法根据局部间歇性调整策略。

Conclusion: 增加策略灵活性（如考虑更多状态信息）可提高智能体在复杂湍流环境中的鲁棒性。
...
### [318] [A Finite-State Gibbs Construction from a Recognition Cost](https://arxiv.org/abs/2605.15667)
*Megan Simons,Jonathan Washburn*

Main category: cond-mat.stat-mech

TL;DR: 本文基于比率代价构造和识别组合律（RCL），推导了有限状态空间上的吉布斯分布，并通过凸对偶等方法恢复了吉布斯权重，同时给出了非渐近斯特林界等新结果。


<details>
  <summary>Details</summary>
Motivation: 研究在有限结果空间中，通过比率代价构造而非传统能量函数来获得吉布斯分布。

Method: 采用归一化的达朗贝尔二阶闭包（RCL），定义成本函数J(x)=cosh(log x)-1，利用多项式计数和凸对偶恢复吉布斯权重和熵等式。

Result: 恢复了有限状态吉布斯权重，得到F_R(q)-F_R(p)=T_R D_KL(q||p)恒等式；给出了非渐近斯特林界和软壳约束型定理；三态例子对比了吉布斯分布与平方对数、亲和力作为能量和Tsallis分布。

Conclusion: 该框架基于特定公理，仅适用于正权重的有限结果空间，未从更原始原则推导组合律，但提供了新的技术工具和比较结果。
...
### [319] [Tweedie's Formula, Variance Functions, and Score-Driven Updating](https://arxiv.org/abs/2605.15902)
*Peter Reinhard Hansen,Chen Tong*

Main category: econ.EM

TL;DR: 该论文通过Tweedie公式为得分驱动模型提供了贝叶斯解释，揭示了其与经验贝叶斯、近似滤波和动态广义线性模型的联系。


<details>
  <summary>Details</summary>
Motivation: 为得分驱动模型建立贝叶斯基础，并明确条件得分在贝叶斯更新中的作用。

Method: 利用Tweedie公式和Fisher评分导出高斯后验校正；对共轭自然指数族，展示精确的得分驱动表示。

Result: 在稳态精度折扣和期望空间逆Fisher缩放下，得分驱动校正等于施加转移动力学前的贝叶斯后验均值。

Conclusion: 将经验贝叶斯、近似滤波、动态广义线性模型和得分驱动模型联系起来，并区分了精确贝叶斯更新与局部得分近似。
...
### [320] [Quasi-Bayesian Local Projection Instrumental-Variables Method: Application to Renewable Energy and Electricity Prices](https://arxiv.org/abs/2605.15966)
*Masahiro Tanaka*

Main category: econ.EM

TL;DR: 本文提出了一种用于局部投影工具变量估计的准贝叶斯方法，通过GMM目标和粗糙度惩罚先验平滑脉冲响应，提高有限样本稳定性并实现联合推断。


<details>
  <summary>Details</summary>
Motivation: 传统LP-IV方法在有限样本下稳定性不足，且难以进行联合推断，需要一种正则化方法改善性能。

Method: 构建基于GMM目标的准后验，并应用粗糙度惩罚先验对不同期限的脉冲响应进行平滑。

Result: 模拟显示，与标准GMM相比，该方法降低了均方根误差，尤其在中长期限。丹麦电力市场应用证实了实用性。

Conclusion: 该方法在保持LP-IV一阶特征的同时，增强了有限样本稳定性和推断能力，具有实际应用价值。
...
### [321] [CitePrism: Human-in-the-Loop AI for Citation Auditing and Editorial Integrity](https://arxiv.org/abs/2605.16000)
...
TL;DR: CitePrism是一个结合LLM推理、语义相似度、元数据验证和人工审核的混合决策支持框架，用于稿件级引文审计，初步验证显示其能标记所有不相关引文，但存在误报。


<details>
  <summary>Details</summary>
Motivation: 编辑和审稿人需要确保稿件引用文献的准确性、相关性和伦理合规性，但现有引文审计主要靠人工、碎片化且难以规模化。

Method: 采用混合框架，包括LLM辅助的上下文推理、基于嵌入的语义相似度、元数据验证、完整性标志以及人工审核；具体步骤为提取引文邻域、丰富引用元数据、计算融合相关性分数、生成元数据和自引审查提示，并支持基于阈值的分类。

Result: 在包含104篇参考文献的单一案例研究中，与人类二元相关性标签的Cohen's kappa为0.429；在阈值tau=17下，标记了所有人类标为不相关的引文，但产生了需要人工审核的误报。

Conclusion: CitePrism可以作为保守的编辑筛选和引文质量分类的初步决策支持，但不能作为自主不当行为检测或自动编辑决策系统；需要在更多稿件、领域、标注者、基线和部署设置中进一步验证。
...
### [322] [Agent4POI: Agentic Context-Conditioned Affordance Reasoning for Multimodal Point-of-Interest Recommendation](https://arxiv.org/abs/2605.15203)
*Jinze Wang,Yangchen Zeng,Tiehua Zhang,Lu Zhang,Yuze Liu,Yongchao Liu,Xingjun Ma,Zhu Sun*

Main category: cs.IR

TL;DR: Agent4POI是首个在推荐时生成上下文条件多模态表示的POI推荐框架，克服了静态嵌入无法处理上下文敏感性的问题，在多个基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有系统使用静态POI嵌入，无法理解同一地点在不同情境下的不同用途，且理论上证明了静态编码无法满足上下文敏感排序。

Method: 采用四阶段LLM代理：生成动态情境线索查询→跨模态链式推理（图像、评论、元数据）→构建不确定性感知的affordance表示→通过语义缓存与用户偏好对齐进行低延迟排序。

Result: 在三个基准和三个配置下，相对最强基线提升23.2%，上下文偏移下仅下降7.5%（基线下降16-17%），冷启动场景下优于最强基于内容的基线2.4倍。

Conclusion: Agent4POI通过动态生成上下文相关的表示，有效解决了POI推荐中的上下文敏感性问题，显著提升了推荐性能，尤其在冷启动和上下文偏移场景下表现出色。
...
### [323] [An LLM-RAG Approach for Healthy Eating Index-Informed Personalized Food Recommendations](https://arxiv.org/abs/2605.15213)
*Yibin Wang,Yanjie Yang,Grace Melo Guerrero,Rodolfo M. Nayga Jr.,Azlan Zahid*

Main category: cs.IR

TL;DR: 提出一种基于健康饮食指数（HEI）的检索增强生成（RAG）框架，结合标准化营养数据库和大语言模型，实现个性化食物推荐，模拟显示HEI平均提升6.45。


<details>
  <summary>Details</summary>
Motivation: 现有食物推荐系统依赖非标准化的食物数据库，与验证指数（如HEI）关联有限。

Method: 基于NHANES和FPED数据库构建食物嵌入空间，计算基线HEI分数，检索候选食物并通过约束RAG管道由LLM生成个性化推荐。

Result: 模拟显示HEI平均提升6.45，HEI超过50的用户比例从45.12%增至61.26%。

Conclusion: 基于LLM-RAG的AI系统可提供更精准、可解释的个性化营养指导，改善饮食质量。
...
### [324] [Fortress: A Case Study in Stabilizing Search Recommendations via Temporal Data Augmentation and Feature Pruning](https://arxiv.org/abs/2605.15299)
...
TL;DR: 提出Fortress框架，通过识别和修剪导致时间不稳定性的特征，提高搜索推荐模型的稳定性和准确性。


<details>
  <summary>Details</summary>
Motivation: 预测模型因某些输入特征导致时间不稳定性，影响可靠性和用户体验，尤其在多阶段系统中需要一致预测。

Method: 利用历史快照，通过四步过程：收集快照、识别不稳定样本、隔离并移除不稳定特征、仅用稳定特征重新训练模型。

Result: 在大型应用市场的查询到应用相关性模型中，离线实验显示预测稳定性（变异系数）和分类性能（PR-AUC）显著提升。

Conclusion: Fortress有效抑制参与信号的波动性同时保留其预测价值，实现更稳定准确的模型。
...
### [325] [Differentially Private Motif-Preserving Multi-modal Hashing](https://arxiv.org/abs/2605.15460)
*Zehua Cheng,Wei Dai,Jiahao Sun*

Main category: cs.IR

TL;DR: 提出一种隐私保护的跨模态哈希框架DMP-MH，通过图结构脱敏和知识蒸馏实现高效的图像-文本检索，在保持隐私的同时达到接近非私有的性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态哈希依赖用户交互生成的语义相似图，但这类图容易遭受链路重建攻击，而现有隐私保护方法无法有效处理图结构数据，如差分隐私SGD破坏关系模式，图合成方法因无标度网络中中心节点的局部敏感度无界导致噪声过大。

Method: 提出Sanitize-then-Distill框架DMP-MH：首先通过确定性截断节点度来限制敏感度，然后使用带噪镜像下降生成满足(ε,δ)-边差分隐私的合成图，最后通过双重哈希网络利用整体结构损失进行知识蒸馏，实现跨模态对齐。

Result: 在MIRFlickr-25K和NUS-WIDE数据集上，DMP-MH在严格归纳协议下比隐私基线方法mAP最高提升11.4个百分点，同时保留非私有方法最高92.5%的性能。

Conclusion: DMP-MH通过解耦隐私与表示学习，有效解决了图结构数据上的隐私保护问题，在跨模态检索中实现了隐私与性能的良好平衡。
...
### [326] [Generative Long-term User Interest Modeling for Click-Through Rate Prediction](https://arxiv.org/abs/2605.15905)
*Jiangli Shao,Kaifu Zheng,Hao Fang,Huimu Ye,Zhiwei Liu,Bo Zhang,Shu Han,Xingxing Wang*

Main category: cs.IR

TL;DR: 提出GenLI模型，通过生成式方法建模长期用户兴趣，避免传统检索的偏差与耗时问题，提升CTR预测准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 传统目标中心检索忽略潜在兴趣，且匹配过程耗时、忽略行为间交互。

Method: 包含兴趣生成模块（IGM）、行为检索模块（BRM）和兴趣融合模块（IFM），IGM生成多个兴趣分布，BRM通过简单查找实现O(1)复杂度，IFM用门控融合。

Result: 提高了兴趣多样性，避免了复杂匹配检索，在准确性和效率间取得更好平衡。

Conclusion: GenLI有效解决了传统方法的偏差和效率问题，适合在线CTR预测场景。
...
### [327] [Bridging Silicon and the Hippocampus: Algebro-Deterministic Memory "VaCoAl" as a Substrate for Vector-HaSH and TEM](https://arxiv.org/abs/2605.15652)
*Hiroyuki Chuma,Kanji Otsuka,Yoichi Sato*

Main category: cs.NE

TL;DR: 提出VaCoAl架构作为连接网格细胞理论和海马尖波涟漪重放实验的统一代数对象。


<details>
  <summary>Details</summary>
Motivation: Vector-HaSH/TEM理论和人类iEEG重放研究并行发展，缺乏共享的代数桥梁。

Method: 基于伽罗瓦域LFSR构建确定性超维记忆，用路径积分置信比CR2建模多跳保真度衰减。

Result: VaCoAl满足准正交性，比随机投影有更强雪崩行为和位精确重现，并自然解释乘法衰减。

Conclusion: VaCoAl的运作模式与海马EC-CA3和EC-DG-CA3通路架构一致，提供了进化保守性的能量-容量-可塑性解释。
...
### [328] [Structure Abstraction and Generalization in a Hippocampal-Entorhinal Inspired World Model](https://arxiv.org/abs/2605.15733)
*Tianqiu Zhang,Muyang Lyu,Xiao Liu,Si Wu*

Main category: cs.NE

TL;DR: 提出一个脑启发层次模型，同时推断潜在转换和构建预测视觉世界模型，实现结构抽象和泛化。


<details>
  <summary>Details</summary>
Motivation: 现有模型难以从连续高维动态中同时提取抽象结构，而人脑HPC-MEC电路能表示空间和概念空间，启发我们探索其机制。

Method: 构建包含逆模型（结构提取）和HPC-MEC耦合模型（分离关系结构与情景场景）的层次架构，利用速度驱动路径积分进行预测。

Result: 在原始变换动力学基准上，模型展示了结构抽象能力，并通过路径积分实现鲁棒预测和结构跨上下文重用。

Conclusion: 该框架为理解脑启发自监督世界模型如何获取可重用抽象知识提供了计算基础。
...
### [329] [Scalable neuromorphic computing from autonomous spiking dynamics in a clockless reconfigurable chip](https://arxiv.org/abs/2605.16114)
*Eric Oliveira Gomes,Damien Rontani*

Main category: cs.NE

TL;DR: 提出一种基于无时钟数字电路的可扩展神经形态架构，在FPGA上实现，用于高效机器学习任务。


<details>
  <summary>Details</summary>
Motivation: 在无需专用硬件设计的情况下，弥合数字实现与专用模拟神经形态系统之间的差距，实现低功耗计算。

Method: 采用异步数字电路的自发连续时间演化产生脉冲动力学，在FPGA上构建可配置兴奋/抑制权重的布尔脉冲神经元网络，并开发完整的脉冲编码数据处理流水线。

Result: 在音频分类任务中达到竞争性性能，处理速度快，功耗显著低于传统数字实现。

Conclusion: 无时钟数字硬件可作为神经形态计算的可行平台，将可重构芯片转变为节能的准模拟神经形态处理器。
...
### [330] [TFZ-Tree: An Ultra-Lightweight Waveform Classification Framework for Resource-Constrained Devices](https://arxiv.org/abs/2605.15656)
*Hao Wang,Kuang Zhang,Yonggang Chi,Tianqi Zhao,Yanbo Fu,Jiaxing Guo*

Main category: eess.SP

TL;DR: 提出超轻量级波形分类框架，基于时频多维特征和ZTree，实现10种6G候选波形的高准确率实时识别。


<details>
  <summary>Details</summary>
Motivation: 现有波形识别研究稀缺且依赖深度神经网络，不适合资源受限终端；符号调制分类必须先识别波形。

Method: 采用低复杂度时域特征提取，分类后端使用Z统计检验优化的决策树（ZTree），自动控制树分裂和大小。

Result: AWGN下平均准确率99.5%，TDL-C信道下87.4%，主要混淆OTFS与LoRa；x86平台单次推理延迟<4ms。

Conclusion: 首次实现10种IoT波形实时识别，未来将部署加速于嵌入式MCU。
...
### [331] [Fault tolerance estimation in digital circuits with visualised generative networks](https://arxiv.org/abs/2605.15212)
...
<details>
  <summary>Details</summary>
Motivation: 需要评估数字电路设计中故障模式的容错性，以提高电路鲁棒性。

Method: 通过GAN的生成器随机生成数字化的模拟电流位配置，在判别器中将其与真实信号比较，计算理想数字信号的偏差，并考虑缺失或互换逻辑器件等错误模式。

Result: 从复变量表示的GAN分析中，能够区分不同经典逻辑元件相关故障模式的影响，从而评估电子设计的鲁棒性。

Conclusion: 该方法能有效评估数字电路的故障容错能力，并为电路设计提供鲁棒性分析。
...
### [332] [GenAI-Driven Approach to RISC-V Supply Chain Exploration](https://arxiv.org/abs/2605.15223)
*Nenad Petrovic,Andre Schamschurko,Yingjie Xu,Alois Knoll*

Main category: cs.AR

TL;DR: 提出一种结合大语言模型、视觉语言模型和模型驱动工程的RISC-V供应链分析方法，构建知识图谱并进行约束建模，有效提升透明度和决策支持。


<details>
  <summary>Details</summary>
Motivation: 解决供应链数据异构、非结构化的挑战，需要多模态数据驱动的洞察。

Method: 利用LLM理解文本、VLM提取图表和扫描件中的信息，构建知识图谱；结合MDE和约束建模进行形式验证、瓶颈检测和风险评估。

Result: 在RISC-V生态系统场景中验证，证明能生成可行洞察、增强透明度并支持复杂半导体供应链决策。

Conclusion: 该方法有效整合语义理解与形式分析，支持探索性和系统性评估供应链韧性，且含有人机协同机制。
...
### [333] [Is Agentic AI Ready for Real-World Hardware Engineering? A Deep Dive with Phoenix-bench](https://arxiv.org/abs/2605.15226)
*Qingyun Zou,Feng Yu,Hongshi Tan,Bingsheng He,WengFai Wong*

Main category: cs.AR

TL;DR: 介绍Phoenix-bench硬件工程基准，发现软件代理在硬件任务上性能大幅下降，且定位粒度和测试反馈是关键。


<details>
  <summary>Details</summary>
Motivation: 现有硬件基准只隔离子任务，缺乏对仓库导航、层次定位、EDA验证和补丁维护的综合评估。

Method: 构建511个Verilator实例的同步语料库，包含开发者补丁、设计标签、测试环境和Docker环境，并统一评估多种代理和结构。

Result: 软件代理性能下降37%-58%；失败集中在控制流/状态机错误和跨层级信号追踪；完美文件级定位仅提升1.4%，而测试反馈提升42%-45%。

Conclusion: 软件与硬件工程本质不同，硬件任务需专门方法，测试反馈和跨层级定位是提升自动化能力的关键。
...
### [334] [A3D: Agentic AI flow for autonomous Accelerator Design](https://arxiv.org/abs/2605.15237)
*Abinand Nallathambi,Christopher Knight,Shantanu Ganguly,Wilfried Haensch,Anand Raghunathan*

Main category: cs.AR

TL;DR: 提出A3D，一个基于Agentic AI的端到端硬件加速器自动化设计流程，无需人工干预即可从复杂科学应用中生成加速器。


<details>
  <summary>Details</summary>
Motivation: 传统硬件加速器设计高度依赖人工，需要多领域专业知识，限制了应用领域专家参与；现有AI方法仅解决部分任务，仍存在大量手动工作。

Method: 通过任务分解给专家代理、编排流程循环（含专家和验证代理）、使用预定义和定制工具，以及基于Agentic RAG的代码和EDA文档探索，自动完成工作负载分析、性能瓶颈识别、代码重构和微架构生成。

Result: 采用Claude Sonnet 4.5和Catapult HLS工具，成功从LAMMPS（分子动力学）和QMCPACK（量子化学）等应用无人工干预生成加速器设计。

Conclusion: 证明了Agentic AI在端到端硬件加速器设计自动化中的有效性，可显著降低人工成本。
...
### [335] [Hardware-Software Co-Design of Scalable, Energy-Efficient Analog Recurrent Computations](https://arxiv.org/abs/2605.15216)
*Arthur Fyon,Julien Brandoit,Loris Mendolia,Damien Ernst,Jean-Michel Redout\'e,Guillaume Drion*

Main category: cs.AR

TL;DR: 通过硬件-软件协同设计，实现了基于双稳态记忆递归单元（BMRU）的超低功耗模拟递归神经网络，在180nm CMOS中达到亚微瓦级推理。


<details>
  <summary>Details</summary>
Motivation: 现有模拟神经网络局限于前馈架构，递归因噪声累积被认为不切实际。

Method: 采用BMRU设计电流模式模拟电路，每个学习参数对应一个电路元件；重新制定BMRU以支持第一象限操作和固定阈值，同时保持表达力和可训练性。

Result: 晶体管级仿真显示软件预测与电路行为几乎完全一致，噪声抑制至少20倍，递归功耗随状态维度线性增长，而前馈层功耗二次增长，关键词识别任务中RNN核心功耗低于1微瓦。

Conclusion: 证明了递归可以以线性边际成本添加到前馈骨干网络中，实现亚微瓦级推理。
...
### [336] [Estimating Social Norm Complementarities](https://arxiv.org/abs/2605.15405)
*Eliana La Ferrara,Cheaheon Lim,Davide Viviano*

Main category: econ.GN

TL;DR: 提出了一个包含技术互补性和社会互补性的社会规范选择模型，利用塞拉利昂和尼日利亚的数据估计了女性割礼、一夫多妻制和童婚之间的相互作用，发现社会回报显著，且不同规范间存在互补或替代关系，并通过反事实分析评估政策效果。


<details>
  <summary>Details</summary>
Motivation: 理解社会规范之间的互补或替代关系对于设计有效的干预措施至关重要，但现有模型未能同时考虑技术和社交两个维度的互补性。

Method: 构建一个包含技术互补性和社会回报（从众效应）的社会规范选择模型，使用塞拉利昂和尼日利亚的重复截面数据，通过均衡分析估计规范间的交互作用。

Result: 社会回报在所有设定中均显著；女性割礼与童婚呈互补关系（尤其在塞拉利昂），而一夫多妻制与童婚呈社会替代关系（尤其在尼日利亚），差异可通过人类学视角解释。

Conclusion: 模型可用于模拟法律改革和社会干预的反事实效果，为政策制定提供依据，并揭示了不同社会规范间复杂的内在联系。
...
### [337] [Crossover from subcritical to critical decay: random walk, self-avoiding walk, percolation](https://arxiv.org/abs/2605.15545)
*Yucheng Liu,Gordon Slade*

Main category: math.PR

TL;DR: 证明了Ornstein-Zernike方程解的一般定理，揭示从指数衰减到临界幂律衰减的交叉，并应用于随机游走、自回避行走和渗流。


<details>
  <summary>Details</summary>
Motivation: 长期以来文献很少关注从Ornstein-Zernike衰减到临界幂律衰减的交叉。

Method: 基于方向依赖指数衰减率的变分刻画和对Hara高斯引理的非中心核扩展。

Result: 定理适用于多种随机游走、d≥5的近邻自回避行走和d≥15的近邻渗流，并得出相关性长度尺度的交叉细节及所有有限阶相关长度等价。

Conclusion: 广泛后果包括交叉的具体描述和相关长度间的普适常数等价性。
...
### [338] [Turbulent stretching of FENE dumbbell polymer model via special stochastic scaling and singular limits](https://arxiv.org/abs/2605.15742)
*Federico Butori,Yassine Tahraoui*

Main category: math.PR

TL;DR: 研究了随机湍流中FENE聚合物模型的拉伸机制，证明了聚合物密度方程在适当缩放下弱收敛到一个含新二阶项的决定性方程，并获得了聚合物长度的平稳分布。


<details>
  <summary>Details</summary>
Motivation: 理解湍流中聚合物拉伸的机理，特别是FENE模型在随机流动中的行为。

Method: 采用随机福克-普朗克方程，通过随机缩放极限和奇异极限技术，在考虑边界奇异性与无通量边界条件的加权空间中进行路径分析。

Result: 当N→∞时，方程弱收敛到带新二阶项的决定性方程；当τ→0时，确定了聚合物长度的平稳分布。

Conclusion: 该路径方法无需对随机流实现进行平均，即可得到湍流拉伸效应的有效模型。
...
### [339] [Deconfinement For $\mathrm{SO}(3)$ Lattice Yang-Mills at Strong Coupling](https://arxiv.org/abs/2605.16162)
*Ron Nissim*

Main category: math.PR

TL;DR: 我们严格证明了具有平凡中心规范群的晶格杨-米尔斯理论（如SO(3)）在强耦合区域不满足威尔逊的夸克禁闭判据。


<details>
  <summary>Details</summary>
Motivation: 在物理预测中，具有平凡中心规范群的理论可能不满足威尔逊禁闭判据，但缺乏严格的数学证明。

Method: 通过数学证明，在强耦合区域对SO(3)晶格杨-米尔斯理论进行分析。

Result: 证明了SO(3)晶格杨-米尔斯理论在强耦合区域不满足威尔逊判据。

Conclusion: 该结果验证了物理预测，即具有平凡中心规范群的晶格杨-米尔斯理论不符合威尔逊的夸克禁闭判据。
...
### [340] [Nonlocal Optical Response and Surface Susceptibilities: A Systematic Derivation via Spatial Moment Expansion](https://arxiv.org/abs/2605.15716)
*Fr\'ed\'eric Zolla*

Main category: physics.optics

TL;DR: 提出了一种系统理论，将均匀介质的非局域响应核与任意弯曲界面的有效表面磁化率联系起来。


<details>
  <summary>Details</summary>
Motivation: 需要统一非局域响应与表面效应之间的关系，特别是对于任意曲率界面。

Method: 从最一般的张量非局域本构关系出发，结合空间矩展开和分布薄层极限。

Result: 界面响应在领头阶简化为单个标量表面磁化率，并明确推导了依赖于平均曲率和高斯曲率的曲率修正。

Conclusion: 建立了广义麦克斯韦边界条件，并与经典菲涅尔结果进行了比较。
...
### [341] [Operator-Controlled 6G: From Connectivity Infrastructure to Guaranteed Digital Services](https://arxiv.org/abs/2605.15553)
*David Soldani*

Main category: cs.NI

TL;DR: 本文主张6G应扭转供应商主导的架构，重新排序运营商优先级：控制、客户、业务、运营优先，技术最后。提出6G控制契约和担保经济两大贡献，以乐天移动为证据，并规划三阶段路线图。


<details>
  <summary>Details</summary>
Motivation: 五代供应商主导的架构导致运营商购买和运营的网络既不属于自己，平台无法修改，AI层无法审计，因此6G必须改变这一趋势。

Method: 两个贡献：6G控制契约定义三类所有权（自有、联合、消费）分配架构主权；担保经济定义五层以结果定价的商业模型，将运营商控制转化为可强制执行的服务水平目标。

Result: 乐天移动的全球首个全国性全云原生开放RAN部署在FY2025实现全年EBITDA盈利，证明了运营商控制6G的可行性。

Conclusion: 2026–2028年的决策将决定6G是成为保证数字服务的平台还是另一个依赖供应商的基础设施周期。
...
### [342] [Domain-Independent Game Abstraction using Word Embedding Techniques](https://arxiv.org/abs/2605.15543)
*Juho Kim,Tuomas Sandholm*

Main category: cs.GT

TL;DR: 提出了一种领域无关的博弈抽象方法，利用自然语言处理中的词嵌入技术，将动作视为词，游戏数据视为语料库，训练词向量后聚类实现抽象。


<details>
  <summary>Details</summary>
Motivation: 现实博弈规模巨大，现有抽象方法多领域特定（如扑克），难以推广。

Method: 将每个动作视为词，游戏数据为语料库，训练词向量，然后聚类进行博弈抽象，并探索基础嵌入模型。

Result: 实验表明该方法有效，但未超越专门针对特定游戏的算法。

Conclusion: 提出的领域无关博弈抽象技术有效，但性能不如专用算法。
...
### [343] [Misspecified Explore-then-Exploit Leads to Supra-Competitive Prices](https://arxiv.org/abs/2605.16064)
*Jackie Baek,Vivek F. Farias,Farrell Wu*

Main category: cs.GT

TL;DR: 研究简单算法定价系统在多公司市场中是否系统性地产生类似合谋的价格。


<details>
  <summary>Details</summary>
Motivation: 探究企业使用探索-利用定价管道（先随机探索，后基于忽略竞争对手价格的需求估计进行短视定价）是否会导致高于纳什均衡的超竞争价格。

Method: 采用流体极限常微分方程分析，刻画定价管道的收敛行为；并基于真实租赁市场数据进行模拟检验。

Result: 当企业在纳什价格同侧相似范围内探索时，价格趋于超竞争水平；对称探索下价格可达垄断水平。模拟验证了结果的鲁棒性。

Conclusion: 简单的算法定价管道可系统性地产生超出纳什均衡的价格，甚至达到垄断水平，且该现象在更复杂场景下依然存在。
...
TL;DR: 提出一个AI驱动的自主测试框架，通过五项策略和容器化架构，显著提升Web测试脚本生成成功率，减少导航和时序故障，并自然扩展到安全测试领域。


<details>
  <summary>Details</summary>
Motivation: 现代Web测试套件因UI重构、时序变化等原因快速腐化，导致开发者放弃维护。

Method: 框架包含五项策略：导航可靠性、上下文感知选择器生成、生成后验证、智能等待注入和失败学习，采用容器化工作节点架构，解耦编排与浏览器执行。

Result: 在四个生产应用和176个场景中，脚本生成成功率从55%提升至93%，导航故障减少8倍，时序相关竞争条件消除80%，测试创建时间比手动Selenium减少75%；安全测试中检测85%的认证绕过漏洞和95%的输入验证缺陷，假阳性低于12%。

Conclusion: 自然语言驱动的安全测试是该领域的新贡献，框架能有效应对Web测试套件腐化问题。
...
TL;DR: 提出并研究了“休眠记忆中毒”攻击，该攻击通过操纵外部上下文使LLM存储虚假记忆，并在后续对话中利用这些记忆影响行为。


<details>
  <summary>Details</summary>
Motivation: LLM的持久性记忆增强了个性化功能，但也引入了安全风险，即攻击者可以通过恶意内容污染记忆，从而长期影响未来交互。

Method: 评估完整攻击流程：验证中毒记忆是否被写入、后续是否被检索，以及是否用于引导对话。在GPT-5.5和Kimi-K2.6上进行测试。

Result: 中毒记忆写入成功率高达99.8%（GPT-5.5）和95%（Kimi-K2.6）；成功检索后，中毒记忆导致攻击者意图的智能体行为在60-89%的评估中出现。

Conclusion: 持久性记忆可作为跨多次对话的长期攻击面，需引起重视。
...
TL;DR: 提出Neo框架，结合大语言模型与经典程序分析，自动检测微服务中的权限提升漏洞，在25个开源应用中发现24个零日漏洞，精确率81%，召回率85%。


<details>
  <summary>Details</summary>
Motivation: 微服务架构中权限控制复杂，跨服务交互、多语言代码库导致权限提升漏洞难以检测。

Method: Neo结合LLM智能体动态生成分析计划、调整代码搜索策略、验证语义，并开发跨服务跨语言的代码搜索原语。

Result: 在25个开源微服务应用（7种语言，620万行代码）上，发现24个零日权限提升漏洞，精确率81%，召回率85%，优于现有方案。

Conclusion: Neo能有效检测微服务权限提升漏洞，且可扩展至其他应用域和漏洞类型，额外发现18个零日漏洞。
...
TL;DR: 提出了一种基于'金丝雀'的AI代理归因协议，能够可靠地将有害代理行为追溯到部署账户。


<details>
  <summary>Details</summary>
Motivation: AI代理自主行动时缺乏追溯机制，导致良性用户误配置或恶意用户故意利用代理造成危害，而受害方无法通知或识别责任账户。

Method: 授权方在代理交互流中注入金丝雀，供应商通过检索会话日志恢复来源会话和账户；对恶意用户采用鲁棒金丝雀构造，使其无法过滤而不影响代理任务性能。

Result: 在多种场景下（包括真实代理）验证该方法可靠、鲁棒且可扩展，适合供应商部署。

Conclusion: 首次定义并提供了实用的代理归因解决方案。
...
TL;DR: 提出一种在Kubernetes集群中部署AI模型的安全架构，通过Kubeflow MLOps自动检测推理时的对抗攻击并触发防御，使用FGSM攻击和PGD对抗训练，实验表明防御能显著恢复模型精度。


<details>
  <summary>Details</summary>
Motivation: 云原生环境中AI模型部署广泛，但Kubernetes等平台缺乏针对AI模型的专门安全机制，现有防御手段有限。

Method: 基于Kubeflow MLOps集成，在推理阶段自动检测FGSM对抗攻击，当精度下降时自动部署PGD对抗训练防御。

Result: 实验显示，部署的防御有效增强了模型鲁棒性，显著恢复了因攻击导致的精度下降。

Conclusion: 该架构能自动保护Kubernetes中部署的AI模型免受对抗攻击，维持可靠性和准确性。
...
TL;DR: 提出一种基于强化学习的置信感知多层次入侵检测系统，通过自适应阈值和LLM降级策略，在降低计算成本的同时保持高检测性能。


<details>
  <summary>Details</summary>
Motivation: 云计算的动态环境和未知攻击导致传统IDS在真实部署中性能下降，需要更智能的检测机制。

Method: 构建三层（网络、主机、虚拟机）检测模型，利用强化学习学习自适应置信阈值；通过多级门控（置信门、Chroma记忆匹配门、LLM分析）处理低置信事件，最终通过校准LLM置信或加权融合进行攻击判定。

Result: LLM升级减少58.78%，系统整体准确率88.68%，精确率85.29%，召回率84.72%，F1 85.00%；网络层准确率98.02%，虚拟机层97.08%。

Conclusion: 该系统在降低成本的同时保持了强大的性能，展现了平衡高效的检测方案。
...
### [350] [Generalized Error Bounds in the Recovery of Solitary Wave Profiles](https://arxiv.org/abs/2605.15590)
*Daniel Sinambela*

Main category: math.CA

TL;DR: 研究Constantin显式重构公式对于二维无旋孤立水波的鲁棒性，考虑波速、静水深三个量的同时扰动，导出L^2误差估计。


<details>
  <summary>Details</summary>
Motivation: 验证重构公式在实际扰动下的稳定性。

Method: 使用hodograph变换、全纯延拓和Paley-Wiener傅里叶衰减估计，推导稳定性估计，并进行数值计算。

Result: 得到误差估计，扰动大小对误差的影响是次线性的。

Conclusion: 公式在扰动下具有一定鲁棒性，提供了理论保证和数值验证。
...
### [351] [From Full and Partial Intraoral Scans to Crown Proposal: A Classification-Guided Restoration Assistance Pipeline](https://arxiv.org/abs/2605.15241)
...
TL;DR: 提出一个针对单冠修复的端到端流水线，通过分类对齐和图割优化处理部分扫描，结合检索和Blender拟合生成个性化牙冠。


<details>
  <summary>Details</summary>
Motivation: 现有分割网络在全牙弓扫描上表现好，但无法处理部分扫描；端到端生成方法产生过平滑表面，丢失咬合细节。

Method: 三阶段流水线：(I)数据准备与姿态标准化；(II)基于扫描类型的路由分割，采用分类-对齐策略：DGCNN分类器识别解剖类型，粗到精RANSAC+ICP配准标准化颌坐标，图割优化细化牙-龈边界；(III)上下文感知检索和Blender拟合生成牙冠。

Result: 在1958个部分扫描上，宏平均DSC 0.9249，召回率0.8919，精确率0.9615；微调的全牙弓模型DSC 0.9347；目标牙及邻牙DSC 0.9468-0.9569，质心误差0.2666-0.2774毫米；生成初步牙冠耗时2.5-3.5分钟。

Conclusion: 该流水线为端到端生成方法提供了一种实用替代方案。
...
### [352] [Highly Detailed and Generalizable Broadleaf Tree Crown Instance Segmentation from UAV Imagery](https://arxiv.org/abs/2605.15673)
...
TL;DR: 提出一种基于深度学习的实例分割模型，用于从无人机航拍影像中精确勾勒阔叶林单个树冠，并通过高质量标注数据集实现跨区域泛化。


<details>
  <summary>Details</summary>
Motivation: 阔叶林树冠形状多样且缺乏清晰树顶，导致树冠分割比针叶林更具挑战性，现有方法难以准确勾勒。

Method: 采用Mask2Former架构，结合多种骨干网络，基于从日本七片森林标注的18,507个树冠多边形进行训练。

Result: 最优模型仅使用RGB影像就在结构复杂的阔叶林中取得高分割性能，并成功泛化到日本其他区域及婆罗洲热带雨林。

Conclusion: 高质量大规模标注数据集是实现精细且泛化性强的树冠分割的关键，模型已集成到DF Scanner Pro软件，支持无人机森林监测。
...
### [353] [Sampling pseudospectrum for data-driven matrices](https://arxiv.org/abs/2605.15234)
*Caroline Wormell*

Main category: math.NA

TL;DR: 提出抽样伪谱和估计量，用于统计测试有限数据中提取的特征值是否为真实信号，从而区分信号与噪声。


<details>
  <summary>Details</summary>
Motivation: 现有算法从有限数据中提取的特征值包含离散特征值，但无法客观判断哪些是真实系统特征值，哪些是有限数据误差的伪影。

Method: 定义抽样伪谱 $P(\lambda)$ 提供有限数据特征值在复平面上的概率信息，并构造估计量 $\hat P(\lambda)$ 通过再处理有限数据样本高效计算，用于统计检验真实特征值的位置。

Result: 该方法能严格且一般地评估从有限数据中提取的模式是信号还是噪声。

Conclusion: 提供了一种客观、通用的方式来判断有限数据中提取的特征值的可靠性。
...
### [354] [Amortized Energy-Based Bayesian Inference](https://arxiv.org/abs/2605.15407)
*Hojjat Kaveh,Ricardo Baptista,Andrew M. Stuart*

Main category: math.NA

TL;DR: 提出一种基于传输映射的似然自由贝叶斯推断方法，用于非线性反问题，仅需联合样本，避免密度评估和雅可比计算，通过神经算子在无限维空间实现快速后验采样。


<details>
  <summary>Details</summary>
Motivation: 传统方法（如MCMC）对每个观测需重复求解反问题，计算成本高，尤其当推断需重复多次时。

Method: 学习一个依赖观测的传输映射，将参考测度推送到近似后验分布；通过最小化平均能量距离目标函数训练映射，避免似然函数、可逆性和雅可比行列式计算；对于高斯先验的无限维问题，映射参数化为先验Cameron-Martin空间中的恒等加扰动，并使用神经算子表示。

Result: 在有限维和两个PDE约束反问题（多孔介质流和地震反演）中，学习到的传输映射能捕捉后验的多模态和主导模态，并对新观测实现快速后验采样。

Conclusion: 所提方法有效解决了非线性反问题的计算瓶颈，实现了高效且准确的近似后验推断。
...
### [355] [Functional Cox model for interval-censored data](https://arxiv.org/abs/2605.15303)
*Yangjianchen Xu,Peijun Sang*

Main category: stat.ME

TL;DR: 提出函数Cox模型处理区间删失数据中的功能协变量，采用惩罚最大似然估计和EM算法，证明估计量一致渐近正态且达到半参有效界，构建全局检验，通过模拟和ADNI数据验证。


<details>
  <summary>Details</summary>
Motivation: 区间删失数据中功能协变量（曲线/空间轮廓）对事件时间的影响具有科学意义，但现有方法不足。

Method: 基于函数Cox模型，结合标量和功能协变量，采用惩罚最大似然估计和EM算法进行参数估计，并用剖面似然估计协方差矩阵。

Result: 回归参数和系数函数线性泛函的估计量具有一致性和渐近正态性，协方差矩阵达到半参有效界；全局检验有效。模拟和ADNI应用表现良好。

Conclusion: 所提方法能够有效分析区间删失数据中功能协变量的影响，统计性质优良，具有实际应用价值。
...
### [356] [Nonparametric inference for sublevel-set probabilities of conditional average treatment effect functions](https://arxiv.org/abs/2605.15373)
*Anders Munch,Thomas A. Gerds*

Main category: stat.ME

TL;DR: 提出用CATE函数的子水平集概率曲线来可视化异质性，并开发了基于机器学习的估计方法。


<details>
  <summary>Details</summary>
Motivation: 平均处理效应忽略了个体异质性，而条件平均处理效应（CATE）函数虽能捕捉异质性，但高维协变量下难以沟通。

Method: 将CATE函数的子水平集概率曲线形式化为目标参数，证明其在非参数模型下非路径可微；采用Grenander型估计器结合机器学习，并针对分段线性近似开发去偏机器学习估计器。

Result: 在基于随机试验合成的数据中验证了有限样本性能。

Conclusion: 该方法在糖尿病药物随机试验数据中得到了应用和展示。
...
### [357] [Modeling Misclassification in Spousal Violence Reporting: Evidence from Bayesian Quantile Regression](https://arxiv.org/abs/2605.15428)
*Joon Jin Song,Mohammad Arshad Rahman,Yoo-Mi Chin,James Stamey*

Main category: stat.ME

TL;DR: 提出一种贝叶斯分位数回归框架，通过引入潜在真实响应并建模误分类，解决二元敏感结果低报导致的推断偏差问题。


<details>
  <summary>Details</summary>
Motivation: 分位数回归能全面刻画协变量效应，但二元敏感结果的低报（误分类）会严重扭曲推断，需处理此偏差。

Method: 建立贝叶斯分位数回归模型，引入潜在真实响应，显式建模假阴性和假阳性错误，并采用新型MCMC算法估计。

Result: 模拟显示，不同先验和误分类率下，该方法优于忽略误分类的模型；实际应用发现低报率高于高报率，调整后重要结论改变。

Conclusion: 所提方法有效校正误分类偏差，对敏感问题研究具有重要价值。
...
### [358] [Tree-aggregated regression for compositional data with measurement errors](https://arxiv.org/abs/2605.15469)
*Zhenghan Li,Tianying Wang*

Main category: stat.ME

TL;DR: 提出TARCO方法，解决树聚合与测量误差交互导致的层次污染问题，实现高维成分数据的校正回归。


<details>
  <summary>Details</summary>
Motivation: 现有方法未考虑树聚合与测量误差的交互作用，导致偏差放大、浓度率降低和变量选择不稳定。

Method: 结合偏差校正估计量、树感知正半定稳定化和稀疏正则化，通过交叉验证选择调参。

Result: 建立预测和估计误差的有限样本界，证明符号一致性；模拟和微生物组应用显示估计精度、支持恢复和可解释性提升。

Conclusion: TARCO有效处理层次污染，提高回归性能，且当测量误差协方差被一致估计时仍保持理论保证。
...
### [359] [Improving the Efficiency of Subgroup Analysis in Randomized Controlled Trials with TMLE](https://arxiv.org/abs/2605.15483)
*Sky Qiu,Nerissa Nance,Rachael Phillips,Jens Tarp,Maya Petersen,Mark van der Laan*

Main category: stat.ME

TL;DR: 提出在随机对照试验中利用非亚组参与者信息改进亚组分析的方法，通过两种TMLE估计器提高亚组治疗效应估计精度，避免外部数据引入的偏倚。


<details>
  <summary>Details</summary>
Motivation: 亚组分析常因样本量有限而统计功效不足，需在不引入外部偏倚的前提下提高估计精度。

Method: 研究两种目标最大似然估计(TMLE)：汇集回归TMLE(TMLE-PR)和自适应TMLE(A-TMLE)，通过共享同试验非亚组参与者的信息来增强亚组估计。

Result: 在LEADER试验案例中，A-TMLE估计亚洲组MACE绝对风险降低1.5-1.6个百分点，黑人组降低2.0-2.1个百分点，95%置信区间均排除零。

Conclusion: 该方法能在不引入外部偏倚的情况下提高亚组治疗效应估计的精度，符合FDA等监管机构优先考虑的方向，支持公平标签、可及性和上市后评估。
...
### [360] [Tail postcoloring in long-run variance estimation of time series](https://arxiv.org/abs/2605.15596)
*Xu Liu,Kin Wai Chan*

Main category: stat.ME

TL;DR: 提出尾部后着色方法，通过参数模型将非参数估计中忽略的尾部自协方差投影回最终估计量，自动切换非参数与参数估计，避免预白化的问题。


<details>
  <summary>Details</summary>
Motivation: 处理强自相关时，标准预白化方法存在对误设敏感、可能导致方差膨胀或功效降低等问题。

Method: 使用参数模型（着色模型）通过缩放因子将非参数方差估计中被忽略的尾部自协方差重新纳入最终估计，带宽参数自动控制非参数与参数部分的切换，无需将整个数据集转化为残差，可扩展到多个参数模型以构建多重稳健估计。

Result: 当着色模型正确指定时，估计量达到参数收敛速率；有限样本下对比预白化方法，对模型误设更稳健，且避免了因再着色因子导致的方差膨胀或功效下降。

Conclusion: 尾部后着色方法有效弥补了预白化的不足，自然适用于多元时间序列，并在MCMC输出分析中展示了实际应用价值。
...
### [361] [Structured Transfer Learning for Survival Risk Stratification in Data-Sparse Clinical Cohorts](https://arxiv.org/abs/2605.15633)
*Junhan Yu,Yurui Chen,Juan Delgado-SanMartin,Dennis Wang,Hong Pan,Doudou Zhou*

Main category: stat.ME

TL;DR: 提出CORE-Cox模型，通过跨结局共享风险因子模式并适应小队列，提升数据稀疏队列的生存风险分层性能。


<details>
  <summary>Details</summary>
Motivation: 小样本或事件少的队列中生存预测模型不可靠，目标队列单独建模不稳定，大队列迁移学习存在风险因子效应差异。

Method: 开发CORE-Cox两阶段框架：先在大队列学习低秩Cox系数共享模式，再通过正则化残差校正适应小队列。

Result: 在UK Biobank和MIMIC-IV中，C-index分别从0.733提升至0.766和从0.628提升至0.658，九个结局中八个有改善，风险富集也提高。

Conclusion: CORE-Cox提供可解释的迁移学习框架，结合跨结局共享与队列特定适应，但需进一步验证才能用于校准的绝对风险预测或临床决策。
...
### [362] [Leveraging heterogeneity for identifiability: Bayesian order-based learning of multiple DAGs](https://arxiv.org/abs/2605.15639)
*Hyunwoong Chang,Fariha Taskin*

Main category: stat.ME

TL;DR: 提出一种基于顺序的联合评分框架，用于异质数据下DAG因果结构学习，并引入R2R提议的贝叶斯方法，在模拟和实际数据中表现优异。


<details>
  <summary>Details</summary>
Motivation: 利用数据异质性提高因果顺序估计的准确性。

Method: 提出联合顺序评分框架，构建高斯DAG模型的顺序贝叶斯方法，并设计随机到随机（R2R）提议邻域用于Metropolis-Hastings算法。

Result: 在模拟研究中表现出强大的实证性能，在重度抑郁症的单核RNA测序数据应用中展示实用性。

Conclusion: 该方法在异质数据下能有效学习DAG结构，具有理论保证和高效混合特性。
...
### [363] [Re-examining and calibrating weighted survival analysis for causal inference](https://arxiv.org/abs/2605.15702)
*Wenfu Xu,Yi Zhang,Tobias Gerhard,Zhiqiang Tan*

Main category: stat.ME

TL;DR: 本文重新审视加权Kaplan-Meier方法，并将其与增广逆概率加权估计框架联系，针对现有加权生存分析方法的局限性，开发了低维和高维情境下的校准估计方法，模拟和实证研究显示校准方法覆盖比例更接近目标，置信区间更短。


<details>
  <summary>Details</summary>
Motivation: 现有加权Kaplan-Meier和加权Breslow-Peto估计的统计性质被忽视或仅有限研究，需要更严谨的理论和方法。

Method: 将加权Kaplan-Meier方法与增广逆概率加权估计框架正式链接，并开发低维和高维情境下的校准估计方法及相关理论。

Result: 模拟研究中校准方法的覆盖比例更接近目标值，且在模拟和实证研究中生成更短的置信区间。

Conclusion: 校准方法在覆盖率和置信区间长度上优于现有加权方法。
...
### [364] [Generalized raking and stabilized weights for regression modeling in two-phase samples](https://arxiv.org/abs/2605.15802)
*Tong Chen,Joshua Slone,Gustavo Amorim,Pamela A. Shaw,Bryan E. Shepherd,Thomas Lumley*

Main category: stat.ME

TL;DR: 在两阶段抽样中，将稳定权重与广义raking结合，通过减少不必要的权重变异和利用辅助信息提高估计精度。


<details>
  <summary>Details</summary>
Motivation: 回归模型中抽样权重包含非必要变异，导致方差估计膨胀，需要稳定权重来调整。

Method: 提出将稳定权重估计与广义raking估计量结合，用于两阶段抽样设计。

Result: 模拟显示在现实两阶段设计中估计精度提升，但在高度信息性设计中效率增益有限。

Conclusion: 该方法可通过标准统计包实现，适用于大型两阶段研究，如卡波西肉瘤研究。
...
### [365] [The Negative Binomial Chain-Ladder: A Full Likelihood Model for Claim Count Reserving](https://arxiv.org/abs/2605.15811)
*Robin Van Oirbeek*

Main category: stat.ME

TL;DR: 提出负二项链梯模型，为索赔准备金提供全似然框架。


<details>
  <summary>Details</summary>
Motivation: 传统链梯方法缺乏概率基础，现有随机扩展依赖二阶矩假设且无生成性解释。

Method: 通过泊松-伽马构建导出负二项分布，建立NB-CL模型，并开发参数自助法量化不确定性。

Result: 泛化泊松链梯，与ODP点估计相同但方差不同，模拟显示更正后有近乎名义覆盖率，错误设定下性能可控退化。

Conclusion: 实证案例中，分散参数具有结构性解释，在损失金额数据上模型为工作近似。
...
### [366] [Bayesian Inference for Non-Conjugate Distance Dependent Chinese Restaurant Process Models](https://arxiv.org/abs/2605.15847)
*Joseph Marsh,Theodore Kypraios,Rowland G. Seymour*

Main category: stat.ME

TL;DR: 提出一种针对非共轭似然距离相关中国餐馆过程（ddCRP）的可逆跳跃马尔可夫链蒙特卡罗（RJMCMC）推断框架，采用多种提议策略，特别是矩匹配提议，提高了聚类性能。


<details>
  <summary>Details</summary>
Motivation: 距离相关中国餐馆过程（ddCRP）在非共轭似然下推断困难，因为聚类分配变化导致参数空间维度变化。

Method: 开发了可逆跳跃MCMC框架，包括基于先验、独立性和数据驱动的矩匹配提议策略用于生灭移动，以及用于固定维度移动的后验重采样策略。

Result: 通过模拟和Old Faithful喷发持续时间数据，矩匹配提议优于基于先验的提议，提高了接受率和计算效率。

Conclusion: 所提RJMCMC框架为具有非共轭似然的ddCRP模型提供了一般性推断方法，适用于离散和连续观测模型。
...
### [367] [A Model-Agnostic Bootstrap for Macro-Level Claims Reserving Under the Conditioning Principle](https://arxiv.org/abs/2605.15896)
*Robin Van Oirbeek,Tim Verdonck*

Main category: stat.ME

TL;DR: 提出满足条件原则的Dirichlet-Gamma层次自助法，解决了准备金评估中现有自助法覆盖误差不消失的问题。


<details>
  <summary>Details</summary>
Motivation: 现有自助法违反条件原则（固定已观测三角形），导致覆盖误差为O(1)，不随三角形增大而消失。

Method: 采用Dirichlet-Gamma层次结构，直接模拟分配比例W_i ~ Beta，仅模拟W_i，固定已观测三角形，实现精确条件预测分布。

Result: 覆盖亏量O(I^{-1/2})，与尾流期数无关；复合泊松下预测标准差高估真实值1/√F；ODP自助法因两种机制导致覆盖偏差Ω(1)。

Conclusion: 该自助法为模型无关，可继承链梯法等开发比例方法的校准；浓度c<30指示非平稳开发。
...
### [368] [Statistical Inference for Smoothed Support Vector Machines in High Dimensions: From Offline to Online Data](https://arxiv.org/abs/2605.15911)
*Shuya Zhou,Junwen Xia,Jingxiao Zhang*

Main category: stat.ME

TL;DR: 针对高维Lasso惩罚线性SVM，提出离线和在线统一推理框架，通过卷积平滑铰链损失构造去偏估计量，实现有效统计推断并提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 高维分类中Lasso惩罚线性SVM存在双重非光滑性（铰链损失和Lasso惩罚），导致统计推断困难且计算效率低下。

Method: 离线时对铰链损失应用卷积平滑技术构造去偏估计量；在线时基于历史数据汇总统计量开发实时估计和推断程序。

Result: 理论证明了离线和在线去偏估计量的渐近正态性；模拟和实际数据表明方法能实现有效的统计推断并提升计算效率。

Conclusion: 提出的框架克服了双重非光滑性带来的挑战，为高维分类问题的统计推断提供了有效且计算高效的解决方案。
...
### [369] [REX-SUB: A Scalable Subsampling Strategy for Modeling Large Spatial Datasets](https://arxiv.org/abs/2605.16075)
*Nicholas Rios,Ben Seiyon Lee*

Main category: stat.ME

TL;DR: 提出一种随机交换子采样算法（REX-SUB），结合Vecchia近似，高效选择最小化预测误差的空间子集。


<details>
  <summary>Details</summary>
Motivation: 大规模空间数据中高斯过程拟合计算量巨大，需要子采样方法降低复杂度。

Method: 采用随机交换算法进行子采样，并嵌入可扩展的Vecchia近似利用精度矩阵稀疏性加速推理。

Result: 在模拟和实际数据中，REX-SUB相比其他子采样策略获得更低的均方预测误差和区间分数。

Conclusion: REX-SUB能有效选择子样本，提高预测精度和计算效率。
...
### [370] [Estimating Association Between Paired Outcomes in Clustered Data with Informative Subgroup Size](https://arxiv.org/abs/2605.16108)
*Owen Visser,Somnath Datta*

Main category: stat.ME

TL;DR: 提出加权估计方法处理牙科聚类数据中的信息性簇大小和亚组结构，以无偏估计边际关联。


<details>
  <summary>Details</summary>
Motivation: 标准边际关联度量因信息性簇大小和缺失模式而产生偏倚。

Method: 基于簇内重抽样和簇加权估计方程，构建权重平衡簇与类别贡献，采用加权估计方程估计矩、秩和单元格概率。

Result: 模拟研究评估了有限样本偏差和覆盖概率；NHANES实例显示诊断亚组大小具信息性，新方法降低偏倚。

Conclusion: 提供了原则性方法估计典型个体典型牙齿的口腔健康关联，减少信息性牙齿留存和亚组组成导致的偏倚。
...
### [371] [Why Empirical p-Values Are Not Uniform: Reference Samples, Dependence, and PIT Backtesting](https://arxiv.org/abs/2605.16221)
*Jakub Lis*

Main category: stat.ME

TL;DR: 揭示了基于有限样本的经验PIT值会导致依赖性和方差扭曲，从而影响预测分布校准评估。


<details>
  <summary>Details</summary>
Motivation: 实际应用中PIT值和经验p值依赖有限样本估计，但其统计结构被忽略。

Method: 分析常见样本和滚动窗口实现中经验百分位的统计特性。

Result: 常见样本导致统计量收敛于两样本Kolmogorov–Smirnov分布，滚动窗口引入自相关和方差抑制。

Conclusion: 不能将经验百分位视为独立均匀分布，需修正backtesting方法。
...
### [372] [FRESH: Information-Geometric Calibration of Patient-Level Models to Aggregate Evidence](https://arxiv.org/abs/2605.16246)
*Franklin Fuller,Daniele Bertolini,Samantha Liang,Jason Christopher,Aaron M. Smith*

Main category: stat.ME

TL;DR: FRESH是一种融合患者级数据与群体级摘要结果的方法，通过最小信息几何扰动重新校准生成模型，实现统一的数据高效临床决策。


<details>
  <summary>Details</summary>
Motivation: 需要将群体级摘要结果（如临床试验、注册总结等）纳入基于患者级数据的预测模型，以提高数据利用效率和证据整合能力。

Method: 假设有一个基于患者级数据训练的生成模型，然后通过最小信息几何扰动重新校准该模型，使其匹配目标群体的指定聚合统计量，从而生成患者级预测。

Result: 得到可反映聚合数据的患者级样本，可直接分析或用于更新原始生成模型。

Conclusion: 该方法适用于单臂试验结果情境化、临床试验模拟及上市疗法比较效果分析等多种应用场景。
...
### [373] [Stochastic Compositional Optimization via Hybrid Momentum Frank--Wolfe](https://arxiv.org/abs/2605.15350)
*El Mahdi Chayti*

Main category: math.OC

TL;DR: 提出混合动量随机Frank-Wolfe算法，无需外层函数可微，用于随机组合优化，达到最优收敛率。


<details>
  <summary>Details</summary>
Motivation: 现有方法要求外层函数连续可微，限制了在鲁棒优化、条件风险价值等应用中的使用。

Method: 结合动量雅可比跟踪器和泰勒校正函数跟踪器，将随机线性化输入广义线性最小化预言机。

Result: 非凸目标下广义Frank-Wolfe间隙达到O(K^{-1/4})收敛率，匹配投影自由单样本随机方法的最优复杂度，并支持重尾噪声。

Conclusion: 提出的算法放宽了平滑性假设，在同样收敛率下适用于更广泛的实际问题。
...
### [374] [Stochastic Non-Smooth Convex Optimization with Unbounded Gradients](https://arxiv.org/abs/2605.15522)
*Dmitry Kovalev*

Main category: math.OC

TL;DR: 提出了广义Lipschitz函数类，证明裁剪AdamW在凸随机优化中优于SGD和AdaGrad。


<details>
  <summary>Details</summary>
Motivation: 现有非光滑优化理论强制梯度一致有界，不切实际；引入广义Lipschitz函数以更好地建模真实场景。

Method: 通过新收敛分析比较SGD、AdaGrad和AdamW，强调AdamW的指数加权梯度累积和裁剪更新的作用。

Result: 裁剪AdamW在凸随机广义Lipschitz优化中取得最优全局收敛率，且对广义光滑假设和拟凸设置同样有效。

Conclusion: 裁剪AdamW是通用的高性能优化器，其指数加权累积机制是关键优势。
...
### [375] [Fairness-Aware Retrieval Optimization for Retrieval-Augmented Generation](https://arxiv.org/abs/2605.15790)
*Yingqi Zhao,Vasilis Efthymiou,Jyrki Nummenmaa,Kostas Stefanidis*

Main category: cs.DB

TL;DR: 提出了一种公平意识检索框架，用于控制和缓解检索增强生成中检索引入的偏见，并通过优化方法平衡相关性与公平性。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成中检索过程可能引入偏见，并传播到生成输出，尤其在top-k设置中多个文档共同影响生成，这一问题更加突出。

Method: 结合控制偏见注入（通过重排序）、位置感知的偏见传播模型，以及优化公式平衡相关性与公平性；提出基于双重超平面近似的二次公平性（FARO）可扩展解决方案，通过问题分解实现高效优化。

Result: 实验表明该方法有效缓解生成偏见，同时保持相关性。

Conclusion: 为RAG系统中的公平意识检索提供了原则性方法。
...
### [376] [To GPU or Not to GPU: Vector Search in Relational Engines](https://arxiv.org/abs/2605.15957)
*Vasilis Mageirakos,Joel Andr\'e,Marko Kabi\'c,Bowen Wu,Yannis Chronis,Gustavo Alonso*

Main category: cs.DB

TL;DR: 本文研究在数据库引擎中集成GPU进行向量搜索的利弊，通过扩展TPC-H基准、开发CPU/GPU执行引擎，实验发现关系部分更受益于GPU，而向量搜索在数据移动时受限于互连，但通过优化索引大小可使GPU全面加速。


<details>
  <summary>Details</summary>
Motivation: 现有数据库引擎在CPU上实现向量搜索，而AI/ML/LLMs中GPU占主导，探究将GPU处理集成到数据库引擎是否更优。

Method: 扩展TPC-H基准加入向量数据，设计SQL+VS查询；开发模块化执行引擎支持CPU/GPU；进行多组实验（不同硬件、索引、数据位置）。

Result: 关系部分在GPU上加速显著，向量搜索部分在数据移动时GPU优势不大；通过减小索引大小的优化，使GPU在快速互连下全面优于CPU。

Conclusion: 优化后的GPU方案使关系与向量搜索均更快，尤其快速互连下，建议数据库引擎采用GPU架构整合向量处理。
...
### [377] [Relational Database Data Lineage Ontology](https://arxiv.org/abs/2605.16068)
*Jakub Dutkiewicz,Pawe{\l} Misiorek,Robert Wrembel*

Main category: cs.DB

TL;DR: 提出一种新的关系数据库数据血缘本体，通过知识图谱增强语义表示，提升血缘链接预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有数据血缘建模在数据库对象间依赖不完整或缺失时面临挑战，需要更丰富的语义表示来发现血缘关系。

Method: 扩展前期基于知识图谱的血缘发现工作，新增结构、语义和转换层面概念，并使用基于路径嵌入的图神经网络模型进行归纳式链接预测评估。

Result: 相比原始基线本体，新本体在AUC和Hits@10指标上均取得改进。

Conclusion: 丰富的语义模型能更精确编码血缘证据，有效提升血缘链接预测效果。
...
### [378] [Towards Foundation Models for Relational Databases with Language Models and Graph Neural Networks](https://arxiv.org/abs/2605.16085)
*Jingcheng Wu,Ratan Bahadur Thapa,Mojtaba Nayyeri,Lucas Etteldorf,Max Finkenbeiner,Fabian Leeske,Steffen Staab*

Main category: cs.DB

TL;DR: 提出了一种轻量级混合LM-GNN架构，结合BART和GraphSAGE，在关系数据库任务上取得有竞争力的性能，为构建基础模型提供高效路径。


<details>
  <summary>Details</summary>
Motivation: 传统关系数据库深度学习方法丢失关系上下文，而现有RDL方法任务和数据库特定，缺乏通用性。

Method: 混合架构：微调BART编码器捕获行内语义，GraphSAGE-GNN在关系实体图上注入关系上下文。

Result: 在RelBench driver-dnf任务上ROC-AUC达67.40，接近LightGBM（68.86），与RDL（72.62）差距5.22点，但与KumoRFM（82.63）仍有较大差距。

Conclusion: 轻量级混合LM-GNN架构在资源高效的前提下，为关系数据库基础模型提供了有希望的研究方向。
...
### [379] [Nonlinear stability of continuously self-similar naked singularities for the Einstein-scalar field equations II: linearized stability](https://arxiv.org/abs/2605.16095)
*Jaydeep Singh,Weihao Zheng*

Main category: gr-qc

TL;DR: 证明了在相同正则性扰动下，连续自相似裸奇点的线性稳定性，为其非线性稳定性奠定基础。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明此类裸奇点因蓝移机制在粗糙扰动下不稳定，但本文旨在探究正则性水平对稳定性的影响。

Method: 基于Christodoulou构造的裸奇点解，进行线性化稳定性分析。

Result: 建立了线性稳定性，表明在背景正则性相同的扰动下，蓝移不稳定机制不被触发。

Conclusion: 为非线性稳定性结果提供基础，与姐妹篇一起证明了这些连续自相似裸奇点的非线性稳定性。
...
### [380] [Non-minimal fluid Lagrangian couplings](https://arxiv.org/abs/2605.16220)
*Christian G. Boehmer,Erik Jensko,Eissa Al-Nasrallah*

Main category: gr-qc

TL;DR: 本文仔细推导了非最小耦合引力模型的场方程和流体方程，定义了热力学量，并展示了Schutz和Brown拉格朗日形式的不等价性。


<details>
  <summary>Details</summary>
Motivation: 通过将物质拉格朗日量直接与引力拉格朗日量耦合，构建能解释暗能量或暗物质的理论，而无需引入额外源。

Method: 从非最小耦合引力的变分原理出发，导出引力场方程和完整流体方程组，并据此定义流体热力学量。

Result: 得出了场方程和流体方程，定义了温度、化学势和数密度等热力学量，揭示了非最小耦合对这些量的影响，并证明了两种拉格朗日形式的不等价性。

Conclusion: 非最小耦合引力模型在完美流体情况下需要仔细处理变分形式，不同拉格朗日形式可能导致不等价的流体方程，需注意其物理解释。
...
### [381] [Nonlinear stability of continuously self-similar naked singularities for the Einstein-scalar field equations I: main results](https://arxiv.org/abs/2605.16235)
*Weihao Zheng*

Main category: gr-qc

TL;DR: 证明了球对称爱因斯坦-标量场方程中一类自相似裸奇点解的非线性稳定性，在相同正则性的小扰动下稳定。


<details>
  <summary>Details</summary>
Motivation: 验证弱宇宙监督假设在不同函数框架下的有效性，并解决之前粗糙扰动下不稳定解在更平滑扰动下的稳定性问题。

Method: 基于线性化稳定性结果，在局部Hölder拓扑中考虑小扰动。

Result: 首次获得这些裸奇点时空在一般扰动下的非线性稳定性，扰动与背景解正则性相同。

Conclusion: 函数框架在表述弱宇宙监督猜想中起决定性作用。
...
### [382] [Modeling Music as a Time-Frequency Image: A 2D Tokenizer for Music Generation](https://arxiv.org/abs/2605.15831)
*Yuqing Cheng,Xingyu Ma,Guochen Yu,Xiaotao Gu*

Main category: cs.SD

TL;DR: 提出生成导向的2D梅尔谱带分词器BandTok，避免残余多码本量化的顺序依赖问题，结合2D RoPE语言模型，在数据有限条件下实现更优的自动回归音乐生成。


<details>
  <summary>Details</summary>
Motivation: 现有高保真音频编解码器常采用残余多码本量化，虽保真度好但序列展平后带来强顺序依赖和误差累积，不利于自回归语言建模。

Method: 设计BandTok，用单一共享码本为每个帧生成梅尔频带令牌，形成物理可解释的时频网格；引入多尺度PatchGAN和EMA码本更新提升重建；采用2D旋转位置编码（RoPE）保持时频结构。

Result: 相比残余码本分词器，BandTok在重建质量和生成效果上改进，尤其在数据有限场景下表现强劲。

Conclusion: BandTok能更好地支持自回归音乐生成，代码和演示已公开。
...
### [383] [Beyond Content: A Comprehensive Speech Toxicity Dataset and Detection Framework Incorporating Paralinguistic Cues](https://arxiv.org/abs/2605.15984)
*Zhongjie Ba,Liang Yi,Peng Cheng,Qingcao Li,Qinglong Wang,Li Lu*

Main category: cs.SD

TL;DR: 提出了一个大规模音频毒性检测数据集ToxiAlert-Bench和双头神经网络，利用副语言线索提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有毒性检测忽视副语言线索，且数据集多为文本，限制了模型捕捉副语言特征的能力。

Method: 构建ToxiAlert-Bench数据集，包含3万+音频片段，标注7种主要毒性和20种细粒度标签，并区分文本与副语言毒性来源；设计双头神经网络，两个分类头分别识别毒性来源和具体类型，采用多阶段训练策略（独立训练后联合微调），结合类别平衡采样和加权损失函数缓解数据不平衡。

Result: 利用副语言特征显著提升检测性能，在Macro-F1上相对提升21.1%，准确率相对提升13.0%，优于所有基线。

Conclusion: 该方法有效提升了毒性检测的准确性和实用性。
...
### [384] [A Few GPUs, A Whole Lotta Scale: Faithful LLM Training Emulation with PrismLLM](https://arxiv.org/abs/2605.15617)
...
TL;DR: PrismLLM通过混合仿真技术，仅用少量GPU就能高保真模拟大规模LLM训练，极大降低了调试和性能调优对大型集群的依赖。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型训练需要数千GPU的集群，但调试和性能调优频繁需要独占访问生产集群，成本高昂且难以实现；现有仿真和缩比实验无法准确捕捉规模相关的行为。

Method: 通过基于切片的构建高保真执行图，捕获目标规模的计算、通信和依赖；然后进行混合仿真，选定的rank执行原始程序，其余rank作为虚拟参与者重放。

Result: 在大规模LLM训练上，迭代时间平均误差0.58%，峰值GPU内存误差小于0.01%；可用少于1%的物理GPU模拟多达8192个GPU的集群。

Conclusion: PrismLLM成功解耦大规模执行与对大型集群的依赖，使工程师能用少量GPU进行高效调试和优化。
...
### [385] [Designing Datacenter Power Delivery Hierarchies for the AI Era](https://arxiv.org/abs/2605.16255)
*Grant Wilkins,Fiodar Kazhamiaka,Alok Gautam Kumbhare,Chaojie Zhang,Ricardo Bianchini*

Main category: cs.DC

TL;DR: 随着AI加速器需求激增，机架功率密度快速上升，预计2027年部署功率接近1MW。这给数据中心电力输送设计带来挑战，需要设计高效的层级架构以应对功率密度变化、硬件更新和工作负载多样性的问题。


<details>
  <summary>Details</summary>
Motivation: AI时代电网容量稀缺，数据中心电力输送设计需在长生命周期内保持高效，但机架放置、工作负载影响、成本等因素相互依赖且随时间变化，缺乏封闭形式分析，因此需要评估框架。

Method: 开发了一个评估框架，使用吞吐量、功率和成本指标，结合GPU、计算、存储部署的投影模型和基于Microsoft Azure生产数据的运营因素，模拟真实的到达、超订和退役序列。

Result: 多资源搁浅显著改变了可部署容量、有效资本支出和交付性能，并量化了从机架和机柜级AI系统上升的密度如何影响这些结果。

Conclusion: 对于AI数据中心设计，相关规划目标不是安装的兆瓦数，而是随时间变化的可部署容量。
...
### [386] [ADAPT: A Self-Calibrating Proactive Autoscaler for Container Orchestration](https://arxiv.org/abs/2605.15788)
*Himanshu Singh Baghel*

Main category: cs.DC

TL;DR: 提出ADAPT在线估计冷启动时间，结合动态规划视界和MPC实现自适应主动自动缩放，在多种工作负载下SLA违规低于5%。


<details>
  <summary>Details</summary>
Motivation: 容器化工作负载的冷启动时间变化大，影响主动缩放效果，需要在线自适应方法。

Method: 使用在线EWMA估计器ADAPT跟踪冷启动时长，结合动态规划视界FH-OPT和模型预测控制器（MPC）优化副本数。

Result: MPC+LSTM在所有六种工作负载上SLA违规低于5%，优于HPA（7-19%）和MPC+Prophet（最高28.7%）。

Conclusion: 闭环自适应设计能有效适应冷启动变化，显著提升主动自动缩放性能。
...
### [387] [Runtime-Orchestrated Second-Order Optimization for Scalable LLM Training](https://arxiv.org/abs/2605.16184)
*Yishun Lu,Junhao Zhang,Zeyu Yang,Wes Armour*

Main category: cs.DC

TL;DR: Asteria通过动态分布优化状态、异步计算和有限陈旧性协议，使二阶优化在LLM训练中变得实用。


<details>
  <summary>Details</summary>
Motivation: 二阶优化方法虽能提高样本效率，但维护大型矩阵优化器状态导致系统开销过大，阻碍其实用。

Method: 将优化状态动态分布在GPU内存、CPU内存和可选的NVMe存储上；使用训练钩子预准备影子状态以异步计算逆根；分布式训练中采用边界陈旧性协议和拓扑感知协调。

Result: 在DGX Spark上支持1B参数模型训练；在GH200系统上降低优化器开销、减少延迟峰值、加速收敛，并保持SOAP和KL-Shampoo的优化优势。

Conclusion: 二阶LLM训练可通过运行时级别的状态管理、后台计算和分布式同步变得实用，而非仅简化优化器。
...
### [388] [Reweighting free energy profiles between universal machine learning interatomic potentials for fast consensus building](https://arxiv.org/abs/2605.15630)
...
TL;DR: 提出一种基于机器学习原子间势能（MLIP）的重加权框架，通过平均能量差近似高效计算自由能分布，避免昂贵的第一性原理模拟。


<details>
  <summary>Details</summary>
Motivation: DFT精度的自由能分布计算代价高昂，通用MLIP虽快但精度依赖于训练数据，存在不确定性。

Method: 采用系统性重加权框架，利用平均能量差近似替代直接指数重加权，克服大体系相空间重叠不足导致的统计失效。

Result: 在601原子纳米限域电解质体系中，成功从源MLIP重加权获得匹配目标MLIP的PMF，支持多个DFT参考级别，计算成本大幅降低；MLIP根据训练数据分为两类；在相空间重叠极低时仍能恢复反应和活化自由能。
...
### [389] [Intrinsic-dimension empirical Bernstein inequalities for bounded self-adjoint operators](https://arxiv.org/abs/2605.15278)
*Diego Martinez-Taboada,Aaditya Ramdas*

Main category: math.ST

TL;DR: 提出了首个经验Bennett和Bernstein不等式，用于独立有界紧自伴算子之和，完全数据驱动，基于内在维度，提供了无维度保证。


<details>
  <summary>Details</summary>
Motivation: 标准oracle界限需要先验方差且随环境维度增长，不适用于无限维或结构化算子。

Method: 用经验方差代替未知方差，基于内在维度，得到了经验Bennett和Bernstein不等式。

Result: 这些经验界限与最佳oracle率渐近相当，并提供了内在维度的经验浓度保证。

Conclusion: 提出的经验界限是计算可行、无维度、更严格的，适用于非各向同性矩阵和无限维空间。
...
### [390] [Bounds on the Number of Modes of a Gaussian Mixture Density](https://arxiv.org/abs/2605.15531)
*Hien Duy Nguyen*

Main category: math.ST

TL;DR: 给出了高斯混合密度函数非退化临界点数量的显式上下界。


<details>
  <summary>Details</summary>
Motivation: 为了准确估计高斯混合模型临界点数量，特别是模态数量。

Method: 通过归一化临界点方程、代数消元、莫尔斯理论和构造方法推导上下界。

Result: 推导了多个上界（如U_het, U_aug）和下界（如L_bin），并给出了最优组合。

Conclusion: 提供了临界点数量的紧界，并扩展到临界集连通分量数的显式界。
...
### [391] [Goodness-of-Fit Testing for Point Processes in Large Populations](https://arxiv.org/abs/2605.15814)
*Sami Umut Can,Estate V. Khmaladze,Roger J. A. Laeven*

Main category: math.ST

TL;DR: 提出一种基于酉变换的分布无关拟合优度检验方法，用于点过程条件强度参数族假设。


<details>
  <summary>Details</summary>
Motivation: 检验点过程的条件强度是否属于特定的参数族。

Method: 构造一个酉变换，使得自然参数检验过程弱收敛到标准目标过程。

Result: 通过蒙特卡洛模拟（包括有无删失的Aalen生存过程、混合治愈模型、软件可靠性模型）和实际数据（人类寿命、软件故障）验证了良好的有限样本性能。

Conclusion: 该方法实现了渐近分布无关的拟合优度检验，适用于多种参数点过程。
...
### [392] [Edge-indexed network time series with graph Ornstein-Uhlenbeck dynamics](https://arxiv.org/abs/2605.15907)
*Jiaming Chen,Almut E. D. Veraart*

Main category: math.ST

TL;DR: 提出了一类Lévy驱动的图Ornstein-Uhlenbeck（grOU）模型，用于边索引网络时间序列，扩展了广义网络自回归过程到连续时间，并允许跳跃行为。


<details>
  <summary>Details</summary>
Motivation: 将广义网络自回归（GNAR）过程扩展到连续时间，并适配图OU动力学到边索引设置，以处理连续时间网络时间序列中的跳跃行为。

Method: 引入Lévy驱动的图OU模型，通过最大似然估计参数，并推导了估计量的渐近性质。

Result: 模拟研究验证了有限样本性能，实证应用显示grOU模型相比标准基准提高预测精度并减少计算时间。

Conclusion: grOU模型通过基于网络的参数化实现了鲁棒性和有效性，适用于边索引网络时间序列。
...
### [393] [Node-private community estimation in stochastic block models: Tractable algorithms and lower bounds](https://arxiv.org/abs/2605.15943)
*Laurentiu Marchis,Ethan D'souza,Tom\'a\v{s} Fl\'idr,Po-Ling Loh*

Main category: math.ST

TL;DR: 研究了节点差分隐私约束下的社区恢复问题，提出了基于谱聚类的隐私算法，并给出了隐私参数ε增长的下界。


<details>
  <summary>Details</summary>
Motivation: 现有算法直接应用节点差分隐私会导致隐私参数ε快速增长，需要设计新方法以实现一致估计。

Method: 开发了基于指数机制和平滑投影的算法，包括隐私化邻接矩阵、主成分分析、凸优化、低秩矩阵估计和近似子空间估计。

Result: 所有算法在节点数上多项式时间内可计算，并推导了实现一致社区估计所需的ε增长下界。

Conclusion: 解决了节点隐私下的社区恢复问题，提出了有效算法和理论下界，并展示了HGR最大相关在精度放大中的应用。
...
### [394] [Nearest-Neighbour Matching on Unbounded Supports and Covariate Shift Transfer](https://arxiv.org/abs/2605.16027)
*Simon Viel*

Main category: math.ST

TL;DR: 在弱协变量支撑假设下，通过转移性度量实现缺失标签多元函数期望的非参数估计，并证明转移性控制对收敛速度的必要性。


<details>
  <summary>Details</summary>
Motivation: 现有非参数估计方法假设协变量在紧致子集上且密度有下界，实际中协变量可能分布在不规则支撑或流形上，需要更宽松的假设。

Method: 基于最近邻匹配的非参数估计器，引入转移性度量衡量源分布和目标分布之间的可迁移性，在弱化协变量支撑条件下推导收敛率。

Result: 在最小假设下达到通常收敛速度，适用于流形支撑和重尾目标分布，并证明了转移性控制是获得好收敛速度的必要条件。

Conclusion: 所提方法可放宽处理效应估计中分配概率有界远离0和1的假设，具有广泛适用性。
...
### [395] [Tests for the mean of high-dimensional data](https://arxiv.org/abs/2605.16033)
*Dietmar Ferger*

Main category: math.ST

TL;DR: 针对高维数据均值检验，提出基于范数统计量的方法，在维度无限制增长下有效，并通过bootstrap实现渐近有效检验。


<details>
  <summary>Details</summary>
Motivation: 高维数据中协方差矩阵可能病态或不可逆，传统方法失效，需要避免矩阵求逆的检验统计量。

Method: 使用统计量 V_n = n||X̄_n||^2，将数据嵌入希尔伯特空间 l2，并利用新的l2中心极限定理推导渐近分布，同时采用bootstrap近似分布。

Result: 在固定和递增维度下均证明了统计量的渐近正态性，且bootstrap检验达到渐近水平α，无需稀疏或结构假设。

Conclusion: 所提方法适用于维度可无限制增长的高维数据，具有渐近有效性，且计算简单、假设宽松。
...
### [396] [Covert Bayesian Quickest Change Detection](https://arxiv.org/abs/2605.16140)
*Yun-Feng Lo,Matthieu R. Bloch*

Main category: cs.IT

TL;DR: 研究了贝叶斯无限时域下的隐蔽快速变化检测问题，提出了期望隐蔽预算（ECB）作为隐蔽性度量，建立了二阶渐进逆界，并提出了匹配的可达方案。


<details>
  <summary>Details</summary>
Motivation: 合法实体需快速检测离散无记忆信道状态变化，同时确保探测行为对监控者隐蔽。

Method: 引入ECB度量限制主动与被动感知观测序列的相对熵，采用常数探测概率的Shiryaev类型策略。

Result: 当虚警概率约束趋近零时，给出了平均检测延迟的二阶渐进逆界，并量化了平方根阶的隐蔽传感增益。

Conclusion: 所提可达方案匹配二阶渐近逆界，数值示例验证了结果。
...
### [397] [Breaking the Finite-Sample Barrier in Entropy Coupling](https://arxiv.org/abs/2605.16229)
*Shahab Asoodeh,Jun Chen*

Main category: cs.IT

TL;DR: 边际约束观测的依赖性可以打破有限样本障碍。本文定义了最小列表熵耦合，证明了依赖观测能在有限样本内精确消除不确定性，而独立观测只能指数级降低。给出了零熵区的充要条件和结构准则，开发了单调逼近贪心算法，并展示了在分布匹配表示学习和随机性提取中的应用。


<details>
  <summary>Details</summary>
Motivation: 传统基于独立观测的模型存在有限样本障碍，而现实中观测往往相互依赖。本文希望从信息论角度形式化依赖性能否以及如何突破这一障碍。

Method: 定义最小列表熵耦合H(P||Q1,...,Qm)，考虑允许Y_i任意依赖但边际固定的联合分布。通过分析零熵区刻画依赖性带来的精确不确定性消除，并设计贪心算法计算该熵。

Result: 依赖观测可以在O(log(1/P_min))个样本内达到零熵，而独立观测只能指数级降低。获得了零熵的充要条件和结构准则。贪心算法具有单调逼近保证。

Conclusion: 依赖性能在有限样本内完全消除不确定性，这一框架统一了表示学习和随机性提取中的精确恢复和精确提取问题。
...
### [398] [Learning Selective Merge Policies for Deadline-Constrained Coded Caching via Deep Reinforcement Learning](https://arxiv.org/abs/2605.15236)
*Amirhossein Yousefiramandi*

Main category: cs.IT

TL;DR: 提出基于深度强化学习的编码缓存传输策略，通过图注意力网络选择性地合并消息，显著降低数据包过期率并提升广播效率。


<details>
  <summary>Details</summary>
Motivation: 在延迟敏感的流媒体服务中，编码缓存需要在线决定合并哪些消息，以兼顾当前编码多播消息的构建和后续消息的影响。

Method: 将截止时间约束下的编码传输建模为掩码离散动作队列状态控制问题，利用近端策略优化训练图注意力策略网络。

Result: 与最优基线SACM++相比，广播包过期率ρ降低40.9%（0.208 vs. 0.352），并在Track A中取得最佳广播效率σ。
...
### [399] [PrismQuant: Rate-Distortion-Optimal Vector Quantization for Gaussian-Mixture Sources](https://arxiv.org/abs/2605.15507)
*Bumsu Park,Chanho Park,Youngmok Park,Namyoon Lee*

Main category: cs.IT

TL;DR: 针对高斯混合源，提出PrismQuant方法，仅需标签成本即可实现率失真渐进最优，性能接近理论界。


<details>
  <summary>Details</summary>
Motivation: 经典变换编码（KLT+反向注水）对多模态高斯源失效，无单一协方差能描述局部几何，率失真函数无闭式解。

Method: 基于EM学习高斯混合模型，无损传输分量标签，用分量匹配KLT对角化，再经熵约束标量量化编码残差。

Result: 在合成高斯混合数据上接近理论率失真界；在真实信道状态信息数据上，以十分之一模型大小取得与变换器学习编码器相当或更优的性能。

Conclusion: 多模态高斯混合源的率失真问题仅增加标签成本，条件高斯分支下共享全局反向注水水平，可实现渐进最优。
...
### [400] [GeoGS-CE: Learning Delay--Beam Channel Priors with 3D Gaussians for High-Mobility Scenarios](https://arxiv.org/abs/2605.16094)
*Yumeng Zhang,Jiajia Guo,Chaozheng Wen,Chenghong Bian,Jun Zhang*

Main category: cs.IT

TL;DR: 提出GeoGS-CE框架，利用高移动场景中稳定的延迟-波束功率谱，通过离线3D高斯场景建模和在线线性MMSE估计实现稀疏导频下的宽带信道估计。


<details>
  <summary>Details</summary>
Motivation: 高移动场景中信道变化快，导频稀疏，但环境如高铁具有可预测轨迹和稳定功率谱，传统方法难以利用此结构。

Method: 两阶段框架：离线阶段联合建模场景级3D高斯表示（捕获NLoS散射）和泄漏感知可微分无线渲染过程；在线阶段预测延迟-波束功率谱作为协方差先验，通过线性MMSE估计重构CFR。

Result: 基于广深高铁段仿真表明，所提几何先验显著优于纯导频和非几何基线方法。

Conclusion: 利用环境几何先验可有效提升高移动场景下稀疏导频信道估计的准确性。
...
### [401] [Symplectic Neural Operators for Learning Infinite Dimensional Hamiltonian Systems](https://arxiv.org/abs/2605.15881)
*Yeang Makara,Yusuke Tanaka,Takashi Matsubara,Takaharu Yaguchi*

Main category: math.DS

TL;DR: 提出了保持辛结构的神经算子，用于哈密顿偏微分方程的建模与模拟，实现长期稳定和改进的能量行为。


<details>
  <summary>Details</summary>
Motivation: 无限维哈密顿系统建模和模拟对标准数据驱动架构存在计算和结构挑战。

Method: 引入辛神经算子，该架构保持哈密顿PDE的辛结构。

Result: 理论上表征了辛性，结合结构保持和学习精度建立了长期稳定性结果。

Conclusion: 数值实验证实理论，SNO相比非保结构算子具有更好的能量行为。
...
### [402] [Complexity of Non-Log-Concave Sampling in Fisher Information](https://arxiv.org/abs/2605.15859)
*Sinho Chewi,Andre Wibisono*

Main category: cs.DS

TL;DR: 本文研究了从对数平滑非对数凹分布中采样时相对Fisher信息保证的查询复杂度，提出了基于近端采样器的算法并改进维度依赖。


<details>
  <summary>Details</summary>
Motivation: 获得相对Fisher信息保证的采样查询复杂度，这是优化中找到近似驻点的采样类比。

Method: 基于近端采样器（Langevin扩散的隐式离散化），利用限制高斯Oracle（RGO）实现后向步骤，并结合对数凹采样高精度保证。

Result: 所提算法在相对Fisher信息上的复杂度继承了与对数凹采样相同的维度依赖，且改进了非对数凹采样的先前工作。同时证明了维度依赖的逆向约化关系。

Conclusion: 该方法有效提升了非对数凹采样的复杂度，且任何改进都会反映到高精度对数凹采样上。
...
### [403] [PhysBrain 1.0 Technical Report](https://arxiv.org/abs/2605.15298)
...
TL;DR: PhysBrain 1.0通过大规模人类自我中心视频提取结构化物理常识知识，先训练VLM再迁移到VLA策略，在多个基准上取得SOTA，尤其在域外场景表现突出。


<details>
  <summary>Details</summary>
Motivation: 机器人轨迹数据覆盖有限，难以学习广泛的物理理解；需要利用大规模人类视频来补充物理常识知识。

Method: 设计数据引擎从人类视频中提取场景元素、空间动态、动作执行和深度关系，生成问答监督训练PhysBrain VLM；再通过保留能力且对语言敏感的适配设计将物理先验迁移到VLA策略。

Result: 在ERQA、PhysBench、SimplerEnv-WidowX、LIBERO和RoboCasa等基准上达到SOTA，尤其在SimplerEnv上域外性能很强。

Conclusion: 从人类交互视频扩展物理常识知识可以有效连接多模态理解与机器人动作。
...
### [404] [HoloMotion-1 Technical Report](https://arxiv.org/abs/2605.15336)
*Maiyue Chen,Kaihui Wang,Bo Zhang,Xihan Ma,Zhiyuan Yang,Yi Ren,Qijun Huang,Zihao Zhu,Yucheng Wang,Zhizhong Su*

Main category: cs.RO

TL;DR: HoloMotion-1是一个基于大规模混合运动语料的人形运动基础模型，实现了零样本全身运动跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有动作捕捉训练方法泛化性差，无法覆盖多样化运动类型和捕捉条件，需引入互联网视频数据提升多样性。

Method: 采用大规模混合运动语料（视频重建数据为主，动捕和自采数据为辅），集成大容量时序建模、稀疏激活的混合专家Transformer、KV-cache推理及序列级训练策略。

Result: 在多个未见过基准上，模型鲁棒泛化，跟踪精度显著优于先前方法，并可直接部署到真实人形机器人上。

Conclusion: 利用大规模异构数据训练可超越传统动捕限制，实现零样本全身运动跟踪并直接迁移到真实机器人。
...
### [405] [Residual Reinforcement Learning for Robot Teleoperation under Stochastic Delays](https://arxiv.org/abs/2605.15480)
*Kaize Deng,Zewen Yang*

Main category: cs.RO

TL;DR: 针对遥操作中随机通信延迟导致的不稳定问题，提出一种结合LSTM状态估计与残差强化学习的混合控制框架，显著提升延迟鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 随机通信延迟导致信号中断，传统强化学习方法因延迟观测而产生高频抖动，控制性能下降。

Method: 采用LSTM从延迟观测中重构连续状态估计，并训练残差强化学习策略以补偿扭矩，平衡跟踪精度与速度平滑性。

Result: 在Franka Panda机器人上的实验表明，该方法在高方差随机延迟下显著优于现有基线，实现了鲁棒稳定的遥操作。

Conclusion: 提出的延迟鲁棒强化学习框架有效应对随机通信延迟，提升遥操作稳定性和性能。
...
### [406] [Hybrid LLM-based Intelligent Framework for Robot Task Scheduling](https://arxiv.org/abs/2605.15486)
*Swayamjit Saha,Subhabrata Das,Haonan Duan,Xiao-Yang Liu*

Main category: cs.RO

TL;DR: 本研究引入基于大语言模型的智能框架，通过生成器和监督器双代理协同，优化施工机器人任务调度，提升时间效率和资源利用率。


<details>
  <summary>Details</summary>
Motivation: 传统施工机器人任务调度在适应动态现场条件和专业人员沟通方面存在不足，需提升调度效率与鲁棒性。

Method: 利用LLM接收任务关键数据（如代理能力、目标），采用平衡分配策略优化时间与资源；通过NLP接口实现与施工人员的实时交互；同时使用生成器（GPT-4）和监督器（Gemma/Llama/Mistral）双代理生成精确任务调度。

Result: 在简单场景下验证了框架的有效性，通过指标得分证明了LLM在施工调度中的优越性能。

Conclusion: LLM在施工机器人任务调度等操作任务中具有关键作用，能显著提升调度质量与适应性。
...
### [407] [SkiP: When to Skip and When to Refine for Efficient Robot Manipulation](https://arxiv.org/abs/2605.15536)
*Mingtong Dai,Guanqi Peng,Yongjie Bai,Feng Yan,Chunjie Chen,Lingbo Liu,Liang Lin,Xinyu Wu*

Main category: cs.RO

TL;DR: 提出Skip Policy (SkiP)，通过动作重标注机制在关键段密集预测、在跳过段跳跃执行，结合自动划分关键段与跳过段的运动频谱关键点检测 (MSK)，在保持或提升成功率的同时减少15-40%的执行步数。


<details>
  <summary>Details</summary>
Motivation: 现有模仿学习策略在所有控制步骤上均匀预测未来动作，而大多数步骤处于自由空间且任务信息少，仅少数接触、抓取等关键步骤需要高分辨率预测，导致计算浪费。

Method: 提出动作重标注机制：在每个跳过段的时间步，将行为克隆目标替换为下一个关键段入口的动作，使策略能一次决策跳过冗余步骤；同时提出运动频谱关键点检测 (MSK) 自动划分演示中的关键段与跳过段。

Result: 在72个模拟操作任务和3个真实机器人任务中，SkiP 减少了15-40%的执行步数，同时匹配或提升了多种策略主干下的成功率。

Conclusion: SkiP 通过非均匀动作预测有效提升了模仿学习效率，无需学习型跳跃规划器或层次结构即可动态调整执行节奏。
...
### [408] [Feedback World Model Enables Precise Guidance of Diffusion Policy](https://arxiv.org/abs/2605.15705)
*Tuo An,Jindou Jia,Gen Li,Jingliang Li,Chuhao Zhou,Pengfei Liu,Bofan Lyu,Jiaqi Bai,Xinying Guo,Geng Li,Jianfei Yang*

Main category: cs.RO

TL;DR: 提出反馈世界模型，利用实时观测在线修正预测，显著提升分布偏移下的预测准确性和策略性能。


<details>
  <summary>Details</summary>
Motivation: 世界模型在部署时常因遇到训练分布外的状态而预测不可靠，但执行过程本身提供了真实下一状态的观测信号，该信号未被充分利用。

Method: 在推理时维护一个轻量级反馈状态，在线更新以迭代修正未来预测，无需额外训练数据或参数更新；同时引入动作感知引导，突出动作可控成分，抑制无关变化。

Result: 在LIBERO-Plus、Robomimic和真实操作任务中，预测误差最高降低76.4%，分布外成功率提升30%。

Conclusion: 在推理时纳入实时反馈是一种简单而强大的替代静态世界建模的方法。
...
### [409] [Learning Dynamic Pick-and-Place for a Legged Manipulator](https://arxiv.org/abs/2605.15713)
...
TL;DR: 提出了一种分层强化学习框架，使四足机器人能够动态执行抓取-放置任务，在仿真和真实环境中均取得较高成功率。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时实现高精度操作与协调运动，尤其是处理重物时。

Method: 采用分层强化学习框架，包含显式质量估计模块，实现自适应全身控制。

Result: 仿真中载荷2.3 kg时成功率达86.05%；真实实验中载荷1.3 kg时平均成功率73.3%，平均执行时间4.06秒。

Conclusion: 该框架展示了四足移动操作器在重载和扩展工作空间下进行自适应全身抓取-放置的潜力。
...
### [410] [Lamarckian Inheritance in Dynamic Environments: How Key Variables Affect Evolutionary Dynamics](https://arxiv.org/abs/2605.15769)
*K. Ege de Bruin,Kyrre Glette,Kai Olav Ellefsen*

Main category: cs.RO

TL;DR: 拉马克遗传在机器人协同优化中的优势取决于环境变化的冲突性和可预测性；在冲突且不可预测时劣于达尔文遗传，但添加传感器可恢复优势。


<details>
  <summary>Details</summary>
Motivation: 机器人形态与控制的协同优化存在耦合问题，拉马克遗传在动态环境中的效果存在争议，认为先前研究遗漏了关键变量。

Method: 将形态优化作为进化，控制器优化作为终身学习，利用拉马克遗传传递学习参数；在虚拟软体机器人上使用贝叶斯优化和强化学习，测试冲突性和可预测性两个变量。

Result: 仅当环境变化既冲突又不可预测时，拉马克遗传表现劣于达尔文遗传；添加传感器可改善冲突环境中的表现。

Conclusion: 拉马克遗传的益处依赖于环境变化的冲突性和可预测性，传感器可帮助预测行为需求从而泛化控制。
...
### [411] [A Topology-Aware Spatiotemporal Handover Framework for Continuous Multi-UAV Tracking](https://arxiv.org/abs/2605.15779)
*Jianlin Ye,Christos Kyrkou,Panayiotis Kolios*

Main category: cs.RO

TL;DR: 提出一种基于拓扑时空交接的多无人机多车辆跟踪系统，通过队列匹配算法实现全局身份保持，在连续交通流中交接成功率99.8%。


<details>
  <summary>Details</summary>
Motivation: 多无人机交通监控中轨迹碎片化导致车辆身份无法跨视野保持，阻碍网络级分析。

Method: 采用轻量级拓扑时空交接机制，结合YOLO11和ByteTrack处理4K视频流，通过几何重叠和虚拟车道离散化实现预测性队列匹配。

Result: 在复杂城市环境中交接成功率99.8%，显著优于基于外观重识别的74.1%，且支持边缘部署。

Conclusion: 系统有效解决多无人机身份保持问题，性能优异且具备边缘可行性。
...
### [412] [GAP: Geometric Anchor Pre-training for Data-Efficient Visuomotor Learning of Manipulation Tasks](https://arxiv.org/abs/2605.15836)
*Davide Buoso,Andrea Protopapa,Stefano Di Carlo,Francesca Pistilli,Giuseppe Averta*

Main category: cs.RO

TL;DR: 提出几何锚点预训练（GAP），通过无动作的模拟代理任务正则化空间适配器，从稀缺演示中学习稳定的视觉运动策略，显著提升少样本性能。


<details>
  <summary>Details</summary>
Motivation: 从少量专家演示中学习视觉运动策略面临挑战：冻结的视觉基础模型将任务适应转移到小空间池化模块，容易过拟合到任务无关的捷径并失去几何基础；预训练表示在场景扰动下表现不佳，需要鲁棒性归纳偏置。

Method: GAP是一个简单的无动作预热阶段，在模仿学习前正则化空间适配器。它在一个轻量级模拟代理任务上预训练池化层（利用对象掩码），促使适配器生成位于对象上、覆盖其空间范围且随时间保持尖锐和可重复的关键点，形成稳定的几何锚点，同时保持视觉基础模型冻结。

Result: 在RoboMimic和ManiSkill上，使用15-50个演示和域偏移条件下，经GAP正则化的简单适配器显著优于更强的注意力池化和端到端微调：RoboMimic Can 15个演示成功率62%（+16%），Tool Hang 50个演示63%，ManiSkill StackCube 30个演示61%（+11%）。代理阶段轻量且与下游任务解耦。
...
### [413] [Learning Sim-Grounded Policies for Bimanual Rope Manipulation from Human Teleoperation Data](https://arxiv.org/abs/2605.16043)
*Gina Wigginghaus,Tim Missal,Berk Guler,Simon Manschitz,Jan Peters*

Main category: cs.RO

TL;DR: 对于变形线性物体（如绳索）的操作，基于状态的策略在预测初始抓取动作时比基于视觉的策略误差降低30.8%，揭示了像素与物理一致状态之间的可观测性差距。


<details>
  <summary>Details</summary>
Motivation: 通过遥操作的模仿学习是操作变形线性物体的实用方法，但受限于人力成本；本研究探究视觉策略泛化不足是否源于观测空间本身。

Method: 在相同双手遥操作数据上训练两个基于Action Chunking with Transformers的策略：视觉策略使用两个腕部摄像头的第一人称RGB流，状态策略使用多视图融合和基于粒子扩展位置动力学模拟提取的3D粒子状态。

Result: 在未见过的绳索配置上开环评估，状态策略在预测初始抓取动作时L1误差降低30.8%，量化了像素与物理一致状态之间的可观测性差距。

Conclusion: 结果表明，使用物理一致的状态表示可以从有限的人类演示中实现更高效的数据驱动变形线性物体操作学习。
...
### [414] [Towards Trustworthy and Explainable AI for Perception Models: From Concept to Prototype Vehicle Deployment](https://arxiv.org/abs/2605.16087)
*Till Beemelmanns,Shayan Sharifi,Manas Mehrotra,Ayushman Choudhuri,Lutz Eckstein*

Main category: cs.RO

TL;DR: 提出一个可信AI感知模块，结合鲁棒性、可解释性和不确定性估计，并在实车中验证。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在自动驾驶感知中占主导，但其不透明性与可信AI指南冲突，缺少具体的3D场景理解实现。

Method: 基于transformer检测器，利用注意力机制提取解释并通过扰动测试验证忠实性，集成不确定性校准和鲁棒性训练。

Result: 实验展示了忠实的显著性行为、改进的鲁棒性和良好校准的不确定性估计，并在原型车中实时运行。

Conclusion: 证明了可信AI感知监控在实时环境中的可行性，提供了可解释性界面。
...
### [415] [FocalPolicy: Frequency-Optimized Chunking and Locally Anchored Flow Matching for Coherent Visuomotor Policy](https://arxiv.org/abs/2605.15944)
*Qian He,Zhenshuo Yang,Wenqi Liang,Chunhui Hao,Nicu Sebe,Jiandong Tian*

Main category: cs.RO

TL;DR: 提出FocalPolicy，通过频率优化分块和局部锚定流匹配，实现远见感知的视觉运动策略，平衡近端精度和远端预见性，解决分块间不连续问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉运动策略优化块内动作分布，忽视块间连贯性，导致长视界动作不连续，难以平衡近端精度与远端预见性。

Method: 结合频率优化分块与局部锚定流匹配，引入远见复合目标：监督近端动作时域对齐，正则化多个未来动作块的频域结构以改善跨块连贯性；设计局部锚定采样增强目标信号传播效率。

Result: 实验表明FocalPolicy优于现有方法，且模块可泛化到其他基线。

Conclusion: FocalPolicy有效实现了近端精度与远端预见性的协同平衡，提升了长视界动作的连贯性。
...
### [416] [Adaptive Outer-Loop Control of Quadrotors via Reinforcement Learning](https://arxiv.org/abs/2605.16015)
*Vishnu Saj,Sushi Vemuri,Dileep Kalathil,Moble Benedict*

Main category: cs.RO

TL;DR: 提出一种新型自适应控制架构，通过残差动力学预测器在线估计扰动，并利用线性校准桥和推力校正实现高效仿真到现实迁移，在Crazyflie上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 基于域随机化的深度强化学习控制策略过于保守，难以应对动态扰动，需主动感知并实时响应瞬时扰动。

Method: 先训练最优外环策略，再用残差动力学预测器替代其依赖的地面真值扰动数据，通过历史状态和动作在线估计外力；引入数据高效的线性校准桥和在线推力校正机制，用数秒飞行数据对齐仿真潜空间。

Result: 在Crazyflie微四旋翼上实际验证，面对质量变化、不对称载荷和动态悬挂负载等严重不确定性，仍保持精确轨迹跟踪，显著优于基线。

Conclusion: 所提自适应控制器能有效应对动态扰动，实现鲁棒的仿真到现实迁移，提升四旋翼飞行控制性能。
...
TL;DR: 本文为图像向量束开发了一种混合量化技术，并将其应用于多个渐近谱问题。


<details>
  <summary>Details</summary>
Motivation: 解决图像向量束上的渐近谱问题，如Alon-Boppana界、Kesten-McKay律等。

Method: 提出混合量化技术，结合离散和连续方法。

Result: 成功应用于谱问题，获得相关结论。

Conclusion: 混合量化技术有效处理图像向量束的渐近谱分析。

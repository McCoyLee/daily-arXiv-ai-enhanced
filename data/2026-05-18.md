...
TL;DR: ChangeFlow 是一种基于潜空间修正流的生成式遥感变化检测框架，通过采样集成实现高性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要采用逐像素判别分类，无法将变化区域建模为连贯整体，且生成式方法因计算成本高和条件机制复杂而性能落后。

Method: 提出 ChangeFlow，利用修正流在潜空间中合成变化掩码，采用结构化且轻量的条件信号，并通过随机采样进行预测集成。

Result: 在四个基准数据集上平均 F1 达 80.4%，比先前最优方法提升 1.3 个百分点，推理速度与强判别基线相当。

Conclusion: 生成式潜空间流方法能有效处理变化检测中的歧义性，同时保持高效推理，优于判别式方法。
...
TL;DR: 提出PanoWorld，从单张图像和标题生成几何一致的360°视频，通过引入深度一致性和轨迹一致性损失，提升了空间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有全景视频方法仅优化视觉真实感，忽略3D场景一致性，导致深度不一致、运动不合理。

Method: 基于预训练透视视频世界模型，添加深度一致性损失和轨迹一致性损失，并进行球面几何感知的条件编码和位置编码调整。

Result: 相比现有方法，PanoWorld在保持视觉真实感的同时显著提升了几何一致性。

Conclusion: 全景视频生成应视为几何建模问题，以支持具身AI的全局空间理解需求。
...
TL;DR: 提出MR2-ByteTrack方法，通过交替全分辨率与低分辨率推理、帧间检测关联及重打分算法，在超低功耗MCU上实现视频目标检测，大幅降低计算和能耗，首次支持Transformer实时VOD。


<details>
  <summary>Details</summary>
Motivation: 智能视觉传感器需在设备端处理视频，但MCU内存和计算有限，传统VOD方法因需特征存储或多帧缓冲而不可行。

Method: MR2-ByteTrack交替全/低分辨率推理减少计算，用ByteTrack关联帧间检测，通过Rescore算法（概率并集规则）聚合置信度纠正误分类，适用于CNN和Transformer。

Result: 在ImageNetVID上，CNN模型mAP达49.0，Transformer达48.7；计算量减少CNN 53%、Transformer 32%；在GAP9 MCU上能耗节省55%，首次实现MCU上实时Transformer VOD。

Conclusion: MR2-ByteTrack能高效适配不同架构，在保持精度的同时大幅降低MCU上的计算和能耗，使超低功耗设备具备实时VOD能力。
...
TL;DR: 提出RIDE方法，基于Retinex同质分解将图像分为光照和反射分量，利用判别性差距定理提高隐蔽对象分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法直接操作RGB或使用异质分解，像素对齐线索不直接；视觉伪装在复合空间匹配，但无需在分量空间同时匹配。

Method: 包含任务驱动Retinex分解模块、判别性差距注意力机制和基于反射特征的对比损失。

Result: 理论上Retinex分解保留或严格提高前景-背景判别性，反相关最大化增益。

Conclusion: Retinex同质分解为隐蔽对象分割提供了有效新视角，RIDE方法能自适应利用分解优势。
...
TL;DR: 提出DiffVAS，一种目标条件策略，利用扩散模型从部分观测重建全局地图，结合强化学习规划，在部分可观测环境中同时搜索多种目标，性能超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉主动搜索方法假设搜索空间完全已知且策略针对单一目标，无法处理部分可观测环境和多目标搜索需求。

Method: DiffVAS采用扩散模型从连续局部观测重建整个地理区域，并基于目标条件的强化学习模块进行后续搜索决策。

Result: 在多个数据集上，DiffVAS在部分可观测环境下搜索多样目标的表现显著优于最新方法。

Conclusion: DiffVAS通过部分观测重建和目标条件策略，推动了视觉主动搜索在现实世界应用中的部署。
...
TL;DR: 提出一种无需调参、基于指令的视频编辑框架，通过结构噪声初始化和噪声引导机制，有效利用噪声潜空间信息，提升编辑质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法未充分利用噪声潜空间中的丰富信息，导致编辑效果不佳。

Method: 设计结构噪声初始化策略（SNIS）为编辑区域分配高噪声、未编辑区域分配低噪声，并引入噪声引导机制（NGM）利用视频先验和噪声潜信息指导去噪过程。

Result: 达到更好的视觉质量和最先进的性能。

Conclusion: 该方法能有效提升视频编辑的保真度和一致性。
...
TL;DR: 在极端低数据细粒度分类中，监督和对比预训练编码器在可分离性上表现最佳，MAE在非线性探针下提升，DINOv3表现最差。


<details>
  <summary>Details</summary>
Motivation: 专家领域标注昂贵，需要指导如何选择预训练编码器。

Method: 使用四种冻结ViT-B/16编码器，通过线性与非线性探针及置换测试评估。

Result: 监督和对比编码器线性可分离性最高，MAE非线性探针改善，DINOv3全面落后。

Conclusion: 低数据时应优先边缘强化预训练目标，若可用非线性分类器则考虑重建编码器。
...
TL;DR: 对自监督视频模型作为世界模型进行了首次系统评估，发现潜在预测模型在多个鲁棒性指标上优于其他模型。


<details>
  <summary>Details</summary>
Motivation: 现有评估仅使用单一准确率指标，无法全面理解视频模型作为世界模型的潜力。

Method: 分析了四种前沿视频基础模型（V-JEPA 2.1, V-JEPA 2, VideoPrism, VideoMAEv2）在五个鲁棒性维度上的表现。

Result: 潜在预测模型在所有维度上表现独特：对像素损坏退化更优雅，遮挡下保持类结构而非几何稳定性，捕获细粒度物理接触线索，编码时间方向。
...
TL;DR: 提出速度自适应引导尺度（VAGS），根据时间步和速度一致性动态调整CFG强度，提升生成和编辑质量。


<details>
  <summary>Details</summary>
Motivation: 传统分类器无关引导（CFG）在整个轨迹中固定尺度，忽略了早期噪声主导、语义弱，后期需要强引导的差异，且引导强度应随速度一致性变化。

Method: VAGS通过结合时域信号项与任务相关速度场的余弦相似度，对名义尺度进行有界缩放，无需训练或额外前向传播。

Result: 在PIE-Bench、DIV2K、COCO17、CUB-200和Flickr30K上，VAGS在结构保真度和生成质量上一致优于固定CFG和近期无训练引导方法。

Conclusion: VAGS是一种训练自由的替代方案，能有效提升流动模型的编辑和生成性能，且恢复固定CFG为特例。
...
TL;DR: 视觉语言模型在执行线追踪任务时经常丢失目标路径，失败源于局部竞争，现有补救措施效果有限。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在多模态基准上表现良好，但可能缺乏对基本视觉操作的稳健控制。

Method: 设计受控的线追踪任务，引入附近竞争者，减少语义和拓扑歧义，以隔离模型追踪能力。

Result: 最先进的视觉语言模型频繁丢失目标路径并切换到局部相似的替代路径；规模扩展、推理和明确指令均不能有效解决。

Conclusion: 路径切换失败在复杂场景中持续存在，表明模型在基础视觉追踪上存在固有瓶颈。
...
TL;DR: DiLA通过内容-结构解耦，在无需额外监督的情况下，同时实现了高层次动作抽象和高保真视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有潜在动作模型在动作抽象与生成保真度之间存在根本性权衡，通常需要两阶段训练或限制预测为光流，亟需统一框架。

Method: 提出DiLA，利用内容-结构解耦，使预测瓶颈驱动解耦，将空间布局分离到结构路径，视觉细节分离到内容路径，实现协同进化。

Result: DiLA在视频生成质量、动作迁移、视觉规划和流形可解释性上均取得优越性能。

Conclusion: DiLA作为统一框架，同时实现了高级动作抽象与高保真生成，推进了自监督世界模型学习的前沿。
...
TL;DR: 提出DecomPose框架，通过难度感知梯度解耦和稳定性驱动的非对称分支，缓解类别级6D物体姿态估计中多类别联合学习的梯度冲突与负迁移问题，在多个基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 多类别联合学习时，不同类别几何异质性导致共享模块中优化信号冲突，造成梯度冲突和负迁移，影响训练效果。

Method: 基于梯度诊断量化模块级跨类别争用，提出DecomPose框架：1) 难度感知梯度解耦，利用数据驱动难度代理分组类别并路由至特定对应分支；2) 稳定性驱动非对称分支，为简单类别分配高容量分支作为稳定锚点，为复杂类别使用轻量分支抑制噪声更新。

Result: 在REAL275、CAMERA25和HouseCat6D数据集上，DecomPose有效减少跨类别优化争用，在多个基准上取得更优的6D姿态估计性能。

Conclusion: DecomPose通过解耦梯度与非对称分支设计，成功缓解了类别级姿态估计中的负迁移问题，显著提升了多类别联合学习的性能。
...
TL;DR: 提出统一动作模型（UAM），通过添加并行背侧专家（类似生物视觉背侧通路）在动作微调中保留视觉语言模型的多模态能力，避免具身税。


<details>
  <summary>Details</summary>
Motivation: 标准VLA微调方法会侵蚀VLM的多模态能力（具身税），源于单一编码器同时支持语义和控制的瓶颈，受生物视觉双通路启发。

Method: 添加并行背侧专家，初始化自预训练生成模型，用中层推理目标（预测视觉动态）训练，端到端仅用动作数据，无需冻结参数或辅助数据。

Result: 保留VLM超过95%的多模态能力，在多项操控任务（包括未见物体、新组合、指令变化）上取得最高平均成功率。

Conclusion: 语义保留可由架构分离自然实现，无需冻结权重或数据重放，且保留的语义能力可迁移到动作的语义泛化。
...
TL;DR: 提出BiomedAP，一种视觉引导的双锚点框架，通过门控跨模态融合和双锚点约束增强生物医学少样本诊断的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有生物医学VLM对提示变化脆弱，模态隔离导致跨模态对齐不稳定，实际临床描述噪声大。

Method: 门控跨模态融合实现层间交互，动态抑制噪声；双锚点约束将可学习提示正则化到由专家模板（高锚点）和少样本视觉原型（低锚点）导出的稳定语义中心。

Result: 在11个基准测试中一致超越基线，少样本准确率有竞争力，且对提示扰动鲁棒性显著提升。

Conclusion: BiomedAP有效解决了提示脆弱性问题，提高了少样本诊断的稳定性和性能。
...
TL;DR: 提出GRASP数据集和社交基础奖励（SGR），提升多模态大模型在多人视频中识别社交互动参与者的能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大模型在多人视频中难以识别谁与谁互动，缺乏对细微非语言线索的推理。

Method: 构建包含29万问答对、46K视频（749小时）的GRASP数据集，基于注视轨迹、指向手势及其组合设计16类社会事件；提出SGR奖励信号，利用社会事件鼓励模型推理互动参与者。

Result: SGR在GRASP-Bench上提升性能，同时在相关社交视频QA基准上保持零样本表现。

Conclusion: GRASP和SGR有效增强了模型对社交互动中参与者的推理能力。
...
TL;DR: 提出频率增强的空间-光谱状态空间生成对抗网络（FSCM），利用红外高光谱图像的空间-光谱耦合信息，结合在线语义分割损失，实现高质量红外图像彩色化。


<details>
  <summary>Details</summary>
Motivation: 现有红外彩色化方法多基于单波段图像，光谱信息不足导致结构畸变和语义混淆；而高光谱图像虽富含信息，但缺乏有效的空间-光谱建模和弱纹理细节处理。

Method: 构建FSCM框架，包含频率增强的空间-光谱状态空间生成器（FSB单元），集成状态空间建模、多级小波分解与傅里叶门控的频率增强模块（FEM）、变形感知采样与稀疏注意力的双流混合门控模块（DGM），并引入在线语义分割损失约束语义一致性。

Result: 实验表明，FSCM在视觉质量和语义保真度上优于现有红外彩色化方法。

Conclusion: FSCM通过充分利用光谱信息和联合优化，有效解决了红外图像彩色化中的结构失真和语义混淆问题。
...
TL;DR: 提出一种概率框架，利用EfficientNet-B3和CBAM模块，通过证据深度学习预测狄利克雷参数，实现卫星图像烟雾严重度分类及不确定性分解。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅二分类，无置信度；需要快速准确评估烟雾严重度以应对应急响应和健康风险。

Method: 使用预训练EfficientNet-B3主干，CBAM注意力模块，证据深度学习头预测狄利克雷浓度参数，单次前向传递估算认知和偶然不确定性。

Result: 在16298个卫星图像上达到93.8%加权准确率（91.1%未加权），ECE=0.0274；保留50%最确信样本达96.7%准确率；不确定性随图像质量下降单调增加；中等烟雾类认知不确定性最高。

Conclusion: 该框架有效提供严重度分类和可靠不确定性估计，有助于实际应用中的选择性预测和质量控制。
...
TL;DR: RaPD 在连续神经图像场潜空间中进行扩散，实现分辨率无关的图像生成，单一潜变量可任意分辨率渲染且扩散成本固定。


<details>
  <summary>Details</summary>
Motivation: 自然图像是连续的，但大多数生成模型在离散网格上合成，限制了分辨率灵活生成。

Method: 提出 RaPD，在连续神经图像场潜空间中进行扩散，使用语义表示引导进行生成感知潜变量学习，并采用坐标查询注意力渲染器实现坐标条件化和尺度感知渲染。

Result: 单一去噪潜变量可通过改变查询坐标在任意分辨率下渲染，扩散成本固定，实验表明生成质量高且分辨率可扩展。

Conclusion: RaPD 弥合了重建与生成之间的差距，实现了分辨率无关的高质量图像生成。
...
TL;DR: 提出分解的视觉-语言对齐框架，将文本提示分解为概念和属性token，通过特征门控交叉注意力和对数空间相似度聚合，提升开放词汇分割对未见组合的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇分割模型难以泛化到未见过的类别和属性组合，因为细粒度描述被编码为整体句子，纠缠多个语义单元。

Method: 将文本提示分解为概念token和多个属性token，实现独立跨模态交互；引入特征门控交叉注意力模块生成属性特定门控图；在对数空间聚合每个token的相似度。

Result: 在细粒度开放词汇分割基准上显著提升对未见属性-类别组合的泛化能力。

Conclusion: 该方法可无缝集成到现有基于transformer的分割架构中，有效改进组合语义理解。
...
TL;DR: 提出基于SAM-HQ的3D高斯分割框架，实现高保真、多视图一致的3D场景编辑。


<details>
  <summary>Details</summary>
Motivation: 现有3D-GS缺乏鲁棒分割，2D分割提升至3D存在视图不一致和粗掩码问题。

Method: 利用SAM-HQ生成精确2D掩码，通过先验引导的标签重分配方法实现3D高斯的多视图一致分割。

Result: 达到最先进分割精度，支持交互式实时编辑，保真度高。

Conclusion: 提升了3D场景编辑能力，在虚拟现实和机器人领域有实用价值。
...
TL;DR: 提出AgriMind集成学习模型，在15类植物病害分类中达到99.23%准确率，推理速度53 FPS，但移动端优化尚未完成。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国植物病害检测仍主要靠人工肉眼检查，效率低下，需要自动化方法。

Method: 使用ResNet50、EfficientNet-B0和DenseNet121集成，基于ImageNet预训练权重，仅训练全连接层10个epoch，融合softmax输出。

Result: 单个模型准确率96-97%，集成后达99.23%，错误率降低三分之二；番茄类（10个相似类别）达99.01%；推理速度53 FPS。

Conclusion: 集成策略有效，但移动端实时应用需进一步TensorFlow Lite优化。
...
TL;DR: 提出VideoSeeker，通过视觉提示和智能推理实现实例级视频理解，在多个任务上平均提升13.7%，超越GPT-4o和Gemini-2.5-Pro。


<details>
  <summary>Details</summary>
Motivation: 现有LVLMs在实例级时空定位任务上表现不佳，主要依赖文本提示导致精准度低，且模型缺乏主动感知细粒度视觉证据的能力。

Method: 构建四阶段全自动数据合成流水线生成高质量实例级视频数据，通过冷启动监督和强化学习内化工具调用和主动感知能力，实现基于视觉提示的智能推理与视频理解。

Result: 在实例级视频理解任务上平均提升13.7%，超越GPT-4o和Gemini-2.5-Pro，并在通用视频理解基准上展现良好迁移性。

Conclusion: VideoSeeker通过视觉提示和智能推理有效解决了实例级视频理解中的时空定位问题，验证了主动感知和工具调用能力的有效性。
...
TL;DR: 提出GenShield，一个统一的自回归框架，联合执行可解释的AI生成图像检测和可控伪影校正，通过闭环诊断-修复机制实现相互增强。


<details>
  <summary>Details</summary>
Motivation: AI生成图像检测后，如何校正可见伪影并恢复真实外观尚未充分探索，且缺乏检测与校正之间的联系。

Method: 构建统一自回归框架GenShield，引入Visual Chain-of-Thought课程学习策略，实现自解释的多步“诊断-修复”校正，并建立大规模“伪影-修复”配对数据集。

Result: 在校正基准和主流AIGI检测基准上达到最先进性能，具有强泛化性。

Conclusion: 揭示了AIGI检测与伪影校正之间的相互促进关系。
...
TL;DR: 提出ML-FOP-SOAP二阶优化框架，通过多层级方差校正和Fisher正交投影，缓解多模态训练中的模态竞争，在batch size 8192下稳定训练，样本效率提升1.4倍，训练加速1.5倍。


<details>
  <summary>Details</summary>
Motivation: 自回归下一个令牌训练在多模态（图像生成与文本理解）中存在模态竞争，导致优化不稳定且限制大批量训练，一阶优化器如AdamW受模态梯度异质性影响大。

Method: 提出ML-FOP-SOAP二阶优化框架，采用Fisher-正交投影抑制方差引发的模态冲突，并引入分层折叠策略实现低开销的细粒度方差校正。

Result: 在Janus和Emu3模型上实验，batch size 8192下训练稳定，相比AdamW样本效率提升达1.4倍，墙上时钟训练加速达1.5倍。

Conclusion: 该优化器为多模态基础模型的大规模训练提供了鲁棒方案，有效平衡视觉生成与文本理解。
...
TL;DR: VLA-AD通过视觉语言模型作为离线语义监督，将大规模VLA教师模型蒸馏为轻量学生策略，大幅降低模型尺寸并提升推理速度，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA策略参数量大、推理成本高，阻碍实时闭环控制。

Method: 使用VLM提供离线语义指导（任务阶段锚点、多帧操作方向描述）增强教师动作目标，训练时使用这些辅助信号，测试时学生独立运行。

Result: 以OpenVLA-7B为教师，学生模型仅158M参数，尺寸减少44倍，平均性能差距仅0.27%，推理速度12.5 Hz (RTX 4090)，加速3.28倍。在π_{0.5}-4B教师上也表现良好。

Conclusion: VLM的离线语义指导能显著提升VLA策略蒸馏的效率、鲁棒性和可部署性。
...
TL;DR: 提出IVGT，一种隐式视觉几何变换器，从无姿态多视图图像中重建连续且连续的3D几何，支持任意视角的渲染和几何提取。


<details>
  <summary>Details</summary>
Motivation: 现有视觉几何基础模型通过回归像素对齐点图显式重建几何，存在冗余和几何连续性不足的问题。

Method: 采用隐式连续神经场景表示，在规范坐标系中学习连续场景，支持任意3D位置的连续空间查询，使用轻量级解码器预测有符号距离（SDF）值和颜色，直接提取连续表面几何。

Result: 在多数据集联合优化下，IVGT在网格/点云重建、新视图合成、深度/法线估计和相机姿态估计等任务上展现出强泛化性和优异性能。

Conclusion: IVGT通过隐式连续建模有效提升了多视图几何重建的质量和连续性。
...
TL;DR: 提出了一个多模态程序合成框架MultiMat，用于生成过程材质图，结合视觉和文本表示，性能优于纯文本基线。


<details>
  <summary>Details</summary>
Motivation: 创建材质图需要专业培训，现有方法仅用文本表示图，忽略了视觉空间特性。

Method: 利用大模型处理视觉与文本的节点图表示，并采用约束树搜索推理算法确保静态正确性。

Result: 在无条件与条件图合成中，生成质量和视觉保真度均优于基线，达到新SOTA。

Conclusion: 多模态方法比纯文本方法更有效，能生成更高质量的材质图。
...
TL;DR: 提出VisualSwap框架和VS-Bench数据集，发现视觉语言模型在推理时声称重新检查图像，但实际上并未真正进行视觉再检查，而是学习到的文本模式。


<details>
  <summary>Details</summary>
Motivation: 探究视觉语言模型在推理时产生的自我反思语句（如“让我再检查一下图片”）是否触发真实的视觉再检查，还是仅仅是学到的文本模式。

Method: 设计VisualSwap图像交换探测框架：模型对一张图像推理后，替换为视觉相似但语义不同的图像，测试模型是否察觉。构建VS-Bench数据集（800图像对），在Qwen3-VL、Kimi-VL、ERNIE-VL上进行实验。

Result: 模型普遍未能察觉图像交换，准确率下降高达60%；令人意外的是，思考型模型比指令型模型脆弱近3倍，且扩展模型规模无改善。多轮用户指令恢复视觉接地，但自我生成的反思语句不能。注意力分析表明用户指令显著提升对视觉标记的注意力，而自我反思则不能。

Conclusion: 当前视觉语言模型在进行视觉再检查时倾向于“说”而非真正“看”。代码和数据集已公开。
...
TL;DR: Minerva-Ego是一个用于评估自我中心视觉推理的基准，包含多步多模态问题和人工标注推理轨迹，发现现有模型与人类表现差距大，并提供空间时间掩码标注以分析原因。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理基准仅评估输出答案，忽略中间推理步骤，且大多限于文本领域，无法全面评估自我中心视觉推理能力。

Method: 基于高质量自我中心视频数据，设计多步多模态问题，并人工标注密集的推理轨迹和时空掩码标注。

Result: 最先进模型与人类表现差距显著；通过提示模型关注'哪里'和'何时'查看，性能大幅提升。

Conclusion: Minerva-Ego揭示了当前模型的不足，并为改进自我中心视觉推理提供了方向，特别是时空注意力引导。
...
TL;DR: 提出在特征空间进行采样，替代几何密集采样，在保持等变性的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: GCNN在每层中密集采样变换群，导致计算成本随自由度指数增长，阻碍实际应用。

Method: 在特征空间中通过特征相似性选择代表性样本，替代几何密集采样，解耦几何分辨率与内存及计算成本。

Result: 粗粒度的特征空间采样已能很好地保持分类准确率，允许基于几何相似性进行预计算，显著加速等变3D分类器的训练。

Conclusion: 特征空间采样提供了一种在计算效率与准确性之间进行权衡的新方式，对于高自由度等变网络具有实际价值。
...
TL;DR: 稀疏MoE在视觉任务中需要足够的路由FLOPs比例和多专家路由才能带来增益，批量轴上的专家分配是主要失败模式。


<details>
  <summary>Details</summary>
Motivation: MoE网络有望实现精度与计算量的权衡，但视觉应用中存在专家崩溃和效率增益有限的问题。

Method: 通过多项控制实验（如隐藏大小扫描、ImageNet消融实验）分析稀疏top-k路由与硬容量约束的影响，并提出每样本变体Soft MoE。

Result: 发现正精度差距需要路由FLOPs比例ρ足够大，且ImageNet规模下还需多专家路由（k≥2）；每样本Soft MoE在CIFAR-100上超越密集基线。

Conclusion: 批量轴上的专家分配是CNN设置中的主要失败模式，稀疏路由的有效性依赖于路由FLOPs比例和多专家路由。
...
TL;DR: 提出COVER方法将3D资产转换为稀疏全景RGB-D-姿态数据，构建了包含36,373帧的CM-EVS数据集，以低冗余、高覆盖和可审计方式支持几何一致的全景3D学习。


<details>
  <summary>Details</summary>
Motivation: 现代3D视觉学习依赖从度量3D资产采样的观测，但现有扫描、网格、点云、仿真和重建未提供稀疏、可比且几何一致的全景训练接口，存在重复、异构标注和深度不一致等问题。

Method: 提出COVER（面向覆盖的视点筛选与ERP范围深度扭曲），一种免训练的ERP视点筛选器，通过投影几何到候选ERP探针、增量评分覆盖并惩罚深度冲突，在有限误差下保持覆盖近似行为。

Result: 基于COVER构建CM-EVS数据集，包含36,373帧来自1,275个室内场景和室外场景的全景RGB-D-姿态数据，每室内场景中位数仅25帧即可覆盖所有13种统一房间类型。

Conclusion: COVER改进了覆盖-冲突权衡，使CM-EVS成为稀疏、紧凑且可审计的RGB-D-姿态资源，适用于几何一致的全景3D学习。
...
TL;DR: 提出NeRP方法，通过中性文本提示和参考图像测量类别先验偏好，结合样本似然纠正先验主导的误分类，在不牺牲已知类性能下提升未见类识别准确率。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型迁移学习中存在基础-新类别权衡，提升未见类性能常降低已知类精度。现有工作简单归因于过拟合，但观察到模型存在不对称混淆偏差，对未见类的预训练偏置难以消除。

Method: NeRP：利用中性提示和参考图像计算类别间先验偏好，结合样本似然得到代理分数；当先验强烈支持当前预测但证据不足时，在易混淆类别对间进行局部翻转，纠正先验主导的误分类。

Result: 在多个骨干网络和15个少样本及跨域基准上，NeRP显著提升未见类准确率，同时保持已知类预测性能。

Conclusion: NeRP作为即插即用的提示校正策略，有效缓解基础-新类别权衡，无需修改模型参数即可提升未见类泛化能力。
...
TL;DR: Semi-MedRef是一种师生半监督学习框架，通过保持图像-语言跨模态一致性，在医学影像引用分割中显著优于全监督和半监督基线。


<details>
  <summary>Details</summary>
Motivation: 医学引用图像分割中，像素级掩码与文本描述的对齐标注成本高昂，尤其在低标注场景下。半监督学习可减轻负担，但依赖可靠的图像-文本对齐，现有方法未能充分解决强增强下的跨模态对齐问题。

Method: 提出三种对齐保持组件：T-PatchMix（基于位置约束和概率规则的跨模态CutMix增强）、PosAug（位置感知文本增强，掩码或模糊解剖短语）、ITCL（位置引导的图像-文本对比学习，利用伪标签构建软解剖正样本）。

Result: 在QaTa-COV19和MosMedData+数据集上，Semi-MedRef在所有标注比例下均一致超越全监督和半监督基线。

Conclusion: 该框架通过显式保持医学图像与位置语言的交互一致性，有效提升了半监督设置下的分割性能。
...
TL;DR: 提出嵌入级扰动探索偏好优化（E²PO），通过维持组内方差解决组基强化学习中的训练不稳定和奖励黑客问题。


<details>
  <summary>Details</summary>
Motivation: 组基优化框架（如GRPO）中，组内样本差异快速衰减导致方差趋零，消除学习信号，造成训练不稳定或过早停滞。

Method: 在样本组内引入结构化的嵌入级扰动，确保训练过程中方差稳健，保留判别信号。

Result: 在广泛实验中显著优于现有基线，实现更忠实于人类偏好的对齐。

Conclusion: E²PO有效解决组内方差消失问题，提升强化学习对齐生成模型的稳定性和效果。
...
TL;DR: 提出OP-Mix，一种跨语言模型训练生命周期的数据混合算法，通过低秩适配器模拟候选混合，以低计算成本找到近似最优混合。


<details>
  <summary>Details</summary>
Motivation: 现有数据混合方法仅针对单一训练阶段（如预训练或持续学习），缺乏统一解决方案，且需要代理模型或固定领域假设。

Method: 基于在线决策，在当前模型上训练多个低秩适配器（LoRA），通过插值它们模拟不同数据混合，无需单独代理模型，确保搜索贴合实际学习动态。

Result: 预训练中平均困惑度提升6.3%；持续学习中匹配重训练和蒸馏性能，计算量分别减少66%和95%。

Conclusion: 语言模型训练应视为单一连续过程，OP-Mix提供了高效统一的数据混合方法，适用于整个训练生命周期。
...
TL;DR: 文学翻译中流畅性与忠实性存在负相关，人类和Google Translate较明显，TranslateGemma较弱。


<details>
  <summary>Details</summary>
Motivation: 探究文学翻译中大型语言模型的流畅性是否等同于语义保留，以及两者关系。

Method: 使用130,486段翻译文本，来自106部小说16种源语言，含人工、Google Translate和TranslateGemma。用基于词性n-gram的翻译ese分类器测流畅性，COMET-KIWI测忠实性，控制段落长度。

Result: 流畅性与忠实性呈一致负相关，人类和Google Translate显著，TranslateGemma较弱且常不显著。
...
TL;DR: 开发了DiscoExplorer，一个开源网络界面，用于16种语言的语篇关系数据集分析和可视化。


<details>
  <summary>Details</summary>
Motivation: 语篇关系研究跨语言比较困难，现有数据复杂且缺乏易用接口。

Method: 构建DiscoExplorer，提供查询语言、搜索和可视化功能，支持关系与信号装置分析。

Result: 公开了来自DISRPT共享任务的16种语言语篇关系数据集。

Conclusion: DiscoExplorer促进了语篇关系的跨语言研究和数据分析。
...
TL;DR: 基于1.007亿乌克兰法院判决构建了首个大规模引文图，发现其结构能无监督地编码法律领域边界并高精度预测立法重要性。


<details>
  <summary>Details</summary>
Motivation: 探索司法引文网络的结构特性及其在法律知识组织和预测中的应用潜力。

Method: 从EDRSR注册表（9950万全文）中提取5.02亿引文链接，使用正则表达式和普通硬件在约5小时内完成，验证精度为1.00。采用Louvain社区检测分析共引网络，并利用引文特征预测立法重要性。

Result: 度分布呈幂律（α=1.57）；社区检测自动划分民事、刑事、行政、商业领域（模块度0.44-0.55，时间稳定性NMI 0.83-0.86）；引文特征预测AUC达0.9984，显著优于基线；检测到2022年入侵导致的引文熵尖峰（11.02→13.49）和战时立法节点。

Conclusion: 引文导出的本体可应用于LLM辅助法律分析的领域层，形成开放数据资源，支持司法知识管理和预测。
...
TL;DR: 研究人类在有限词汇（最少250高频词）下如何沟通，与贪心和全局最优采样算法比较，发现人类更接近贪心，但技能高者会回溯修改，且在词汇限制下倾向使用语义轻的词。


<details>
  <summary>Details</summary>
Motivation: 理解人类在词汇限制下如何优化可理解性，揭示有限沟通的认知机制。

Method: 设置不同词汇限制（最少250高频词），让人类回答问题，并与基于大语言模型的顺序蒙特卡洛推断的贪心和全局最优采样算法比较。

Result: 人类整体更接近贪心采样；技能高者表现出更多回溯修改（非贪心行为）；在强限制下，人类和两种算法都倾向使用语义轻的词。

Conclusion: 结果对资源理性认知、心理语言学、二语交流及语言障碍具有启示意义。
...
TL;DR: 一个基于WhatsApp的AI教学助手，利用生成式AI和历年试题，为西非中学提供科学教育支持，初步显示高实用性。


<details>
  <summary>Details</summary>
Motivation: 撒哈拉以南非洲师生比高、合格教师短缺，缺乏个性化学习和形成性评估支持。

Method: 通过WhatsApp机器人集成教材和33年考试题，结合生成式AI进行对话问答和自动评估，提供即时反馈。

Result: 2025年6个月可行性部署，56名活跃用户，AI回答有用性评分93.75%（n=16）。

Conclusion: 初步结果为在资源受限环境中大规模、低成本提供个性化支持和评估奠定了基础。
...
TL;DR: 开发了基于RAG的生成式AI助手Eskwai for Students，帮助加纳法学生，在30个月纵向研究中被3100名学生使用，分析了查询类型和伦理问题。


<details>
  <summary>Details</summary>
Motivation: 全球南方（如加纳）的法学教育中，生成式AI的应用有限，需要开发并评估此类系统。

Method: 构建包含1.2万案例法和1.4k立法的RAG系统，部署于加纳法学生中，进行30个月纵向研究，收集3.2万查询。

Result: 评估了AI的有用性，揭示了学生查询类型及伦理关切。

Conclusion: 为全球南方法学生使用生成式AI提供了见解，并指出了负责任推进法学教育的途径。
...
TL;DR: 提出能力条件脚手架框架，根据用户专业领域能力划分强弱域，条件化干预行为，支持更可靠的人机协作。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型个性化通常不考虑用户在不同专业领域的评估能力差异，可能导致专业领域漂移，即用户依赖AI推理而无法可靠评估。

Method: 引入能力条件脚手架，一种类型化框架，将专业知识分为强、混合和弱领域，并根据结构化能力配置文件条件化干预行为。

Result: 在多个MMLU子集和四个LLM基底上的初步评估显示一致的能力条件干预行为，包括在配置文件交换下的类别反转和混合领域风险区的选择性激活。
...
TL;DR: 对六种LLM架构在12种认知任务上的神经激活模式进行全面分析，发现数学推理产生最高注意力熵，解码器模型稀疏性更高。


<details>
  <summary>Details</summary>
Motivation: 理解不同LLM架构在认知任务上的神经激活差异，以指导模型选择与优化。

Method: 系统性测量最终激活值、注意力熵和稀疏模式，分析144个任务-模型组合。

Result: 数学推理在所有架构中注意力熵最高；解码器模型稀疏模式显著高于编码器模型。

Conclusion: 揭示了现代语言模型的计算特征和任务特定行为，为大数据应用中的模型选择与优化提供关键见解。
...
TL;DR: 通过调整RNNG波束搜索中的并行解析数量，发现减少并行解析虽能增加花园路径效应预测，但远不足以匹配人类数据，说明LM与人类在并行解析能力上的差异无法解释两者在处理句子时的差距。


<details>
  <summary>Details</summary>
Motivation: 语言模型（LM）的惊异度能预测自然文本阅读时间，但系统性地低估了花园路径句中的处理困难，推测是由于LM与人类在并行解析数量上的差异。

Method: 使用循环神经网络文法（RNNG）结合词同步波束搜索，系统变化计算词惊异度时同时活跃的解析数量，并用这些惊异度预测人类阅读时间。

Result: 减少同时活跃的解析数量确实增大了预测的花园路径效应幅度，但远不足以捕捉人类效应的完整幅度。
...
TL;DR: 通过校正长度影响，研究揭示了推理训练模型在代码领域展现出独特的轨迹几何特征，长度校正是分析生成轨迹的必要步骤。


<details>
  <summary>Details</summary>
Motivation: 区分语言模型在难题上更长的思维链是由于计算步骤增加还是内部轨迹不同。

Method: 在竞争性编程、数学和布尔可满足性任务中，分析链式思维生成过程中的隐藏状态轨迹，并对轨迹统计量进行长度校正。

Result: 校正后，难度与轨迹几何存在系统性关联；代码领域中推理训练模型展现更直接的校正轨迹和更低的局部曲率异质性，而数学和布尔可满足性中关联较弱。

Conclusion: 长度校正是生成时轨迹分析的前提；推理训练可以产生独特的校正轨迹几何，但效果依赖于领域。
...
TL;DR: 提出模块化RAG流水线从护士-病人对话转录中提取观察结果，最佳配置使用GPT-5.2达到80.36% F1。


<details>
  <summary>Details</summary>
Motivation: 临床文档负担沉重，护士需大量时间记录观察结果，需要自动将转录文本转换为结构化表示。

Method: 构建模块化检索增强生成（RAG）流水线：以训练集为例示范本库，采用模式约束提示（完整模式或精简候选模式），结合确定性模式后处理和第二遍审计，使用Llama-4-Scout-17B-16E-Instruct和GPT-5.2两种LLM骨干及对应嵌入模型。

Result: 最佳配置（GPT-5.2 + 完整模式 + RAG + 第二遍审计）F1为80.36%。RAG一致提升性能；最优模式约束程度因模型而异；第二遍审计通过修正残留模式遵循错误带来适度增益。

Conclusion: RAG对性能提升稳定，模式约束最佳程度依赖模型，第二遍审计可进一步修正错误，总体有效降低了文档负担。
...
TL;DR: 提出了FINESSE-Bench，一个包含8个专门基准、共3993个问题的套件，用于层次化评估大语言模型在金融领域的专业能力。


<details>
  <summary>Details</summary>
Motivation: 现有金融基准（如FinQA、FinanceBench）主要关注报告问答，缺乏明确的专业难度层次，无法全面评估从基础到专家级的金融推理能力。

Method: 结合基于CFA、CMT、CFTe等专业认证的考试型数据集、应用交易任务集合和俄语奥林匹克基准，设计了统一评估协议覆盖选择题、数值答案和短开放式回答，并采用LLM-as-judge自动评分。

Result: 该基准能评估领域广度、难度增加时的性能下降、计算任务解决能力以及模型在专业金融领域的行为表现。

Conclusion: FINESSE-Bench旨在补充现有开放金融基准，作为更实质性地评估大语言模型专业金融能力的工具。
...
TL;DR: 本文揭示了RoPE在长上下文中的理论缺陷：随着长度增加，其位置偏差和一致性丧失，失败概率接近随机，且多注意力头和多层结构无法克服；需新的位置编码机制。


<details>
  <summary>Details</summary>
Motivation: 识别Transformer长上下文语言模型中RoPE的固有限制，特别是其有效性的核心属性——局部性偏差和令牌相关性一致性。

Method: 通过抽象上下文内容的理论分析，证明RoPE在长上下文中的行为；调整基超参数实验；以及多层多头注意力架构的实证分析。

Result: RoPE失去局部性偏差，令牌相关性不一致；注意力分数对位置移动或令牌替换不敏感；基超参数调整无法同时区分位置和令牌；多层多头架构不足以克服这些缺陷。

Conclusion: 未来Transformer长上下文模型可能需要全新的位置和令牌顺序编码机制。
...
TL;DR: 提出DetectRL-X，一个多语言基准，用于评估LLM生成内容检测器在8种语言和多种场景下的表现。


<details>
  <summary>Details</summary>
Motivation: 现有检测器在多语言真实场景中的可靠性和潜力尚未充分探索，而LLM滥用风险日益增加。

Method: 构建包含8种商业语言、6个易滥用领域、4种商业LLM生成文本的基准，涵盖润色等操作，并开发多语言改写和扰动攻击框架进行压力测试。

Result: 实验揭示了当前最先进检测器在不同语言上的优劣势，以及领域、生成器、攻击策略等因素对性能的影响。

Conclusion: DetectRL-X可作为有效基准，用于加强多语言和特定语言检测器。
...
TL;DR: 提出BetaPRM，通过预测步骤成功概率及其可靠性，并利用自适应计算分配优化推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有过程奖励模型仅输出单一评分，无法指示预测的可信度，导致下游方法盲目依赖不可靠的奖励信号。

Method: BetaPRM基于Beta-Binomial似然学习步骤成功的Beta信念分布，同时输出成功概率和可靠性；并引入自适应计算分配（ACA），根据可靠度动态调整Best-of-N搜索的计算量。

Result: 在四个骨干网络和四个推理基准上，BetaPRM提升了Best-of-N选择性能，ACA在固定预算下最高减少33.57%的令牌使用量，同时提高最终答案准确率。

Conclusion: BetaPRM通过提供步骤级可靠性信号，增强了奖励模型的可信度，并实现了计算效率与准确率之间的更好权衡。
...
TL;DR: 连续扩散和流模型用于非自回归文本生成时，潜变量几何不足以保证解码质量，应评估解码器可恢复性和起始分布质量。


<details>
  <summary>Details</summary>
Motivation: 连续潜状态与离散token之间的接口是非自回归文本生成的主要困难，早期方法仅关注潜空间指标而忽视解码质量。

Method: 提出草案条件潜变量细化模型，包含冻结BERT编码器、并行解码器、去噪DraftPrior、局部FlowNet和学习的对角MetricNet，进行受控局部细化而非完全生成。

Result: 全768维潜变量比256维更好地恢复token；DraftPrior在干净草案中目标token概率0.938，随dropout降低；局部流细化和解码器感知读出有额外增益，但度量学习和OT对齐未缩小解码器差距。

Conclusion: 仅潜变量几何不足，连续潜文本生成需评估解码器可恢复性、起始分布质量及细化是否保持解码器可读结构。
...
TL;DR: 提出GiLT模型，通过依赖图增强Transformer语言模型，在保持困惑度的同时提升句法泛化能力。


<details>
  <summary>Details</summary>
Motivation: 以往工作多使用句法树结构（如成分树），而依赖图尚未被充分利用。

Method: GiLT在Transformer中通过增量构建的依赖图调节注意力权重，注入结构信息，不额外插入结构标记。

Result: GiLT在语义依赖图上取得更好的句法泛化，困惑度与基线相当，且可微调提升下游任务性能。

Conclusion: 依赖图是一种有效的语言模型增强方式，能提升句法泛化而不牺牲语言建模性能。
...
TL;DR: 提出一种混合式多智能体协作范式Nexa，结合并行与顺序模式，通过可训练策略预测稀疏有向无环通信图，实现低延迟高准确性。


<details>
  <summary>Details</summary>
Motivation: 现有并行和顺序协作模式无法同时最小化通信延迟并最大化最终响应准确性。

Method: Nexa先并行执行，将响应嵌入共享语义空间，然后预测稀疏有向无环通信图；若图为空则纯并行，否则进行一次顺序消息传播。策略为轻量Transformer，无需外部评判或预定义拓扑搜索。

Result: 学习到的响应条件策略在智能体数量、任务或底层模型变化时仍可重用，证明其泛化性。

Conclusion: Nexa有效桥接并行与顺序模式，实现可泛化的通信策略，优于单一模式。
...
TL;DR: 提出语义校准奖励（CSR）框架，直接在语义空间校准语言模型，无需口头置信度，显著降低ECE并提高AUROC。


<details>
  <summary>Details</summary>
Motivation: 标准RLVR忽视置信度校准，导致模型自信但错误；基于token的口头置信度存在语义不一致问题。

Method: CSR结合正确性奖励与语义校准奖励：对正确输出鼓励语义一致性（利用），对错误输出抑制虚假一致性（探索）。

Result: 在HotpotQA（分布内）及TriviaQA、MSMARCO、NQ-Open（分布外）上，ECE降低高达40%，AUROC提升高达31%。

Conclusion: CSR能有效校准模型不确定性，鲁棒性强，可推广到多种评估场景。
...
TL;DR: 提出了一个基于知识图谱的心理健康基准MHGraphBench，评估LLMs在实体识别、关系判断和二跳推理上的表现，发现识别-判断差距及格式可靠性影响。


<details>
  <summary>Details</summary>
Motivation: LLMs在心理健康领域应用广泛，但缺乏对其生物医学知识捕获和临床判断可靠性的系统评估。

Method: 从PrimeKG构建九个任务家族，涵盖实体识别、关系判断和二跳推理，并设置KG支持的答案与受控负面选项。在15个开源与闭源LLMs上进行实验。

Result: 领先模型在实体类型和小关系类型任务上接近最优，但在关系预测和二跳推理上仍困难；短KG片段有时提升性能有时降低；输出格式可靠性显著影响多选设置下的测量结果。

Conclusion: MHGraphBench评估的是模型与PrimeKG心理健康片段在受控界面下的一致性，而非直接临床安全性，强调了响应有效性的关键作用。
...
TL;DR: 大语言模型在代码生成基准上表现优异，但无法将能力迁移到预训练中未出现的新语言，存在“实现保真度差距”


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型能否将代码生成能力迁移至预训练未涵盖的编程语言

Method: 引入新语言PyLang，在352个问题上评估零样本和微调后的Qwen3模型，并尝试多任务学习、偏好调整等多种干预措施，使用LLM裁判和CKA分析

Result: Python比PyLang最高好19%，所有干预均无法缩小差距；模型内部表征相似（CKA>0.97）但输出阶段分歧，80%选择相同算法却无法给出正确实现
...
TL;DR: 提出并行推测解码（PSD），一种无需训练的方法，通过并行解掩码和压缩去噪步骤，加速扩散大语言模型推理，实现最高5.5倍令牌吞吐量，且精度与贪心解码相当。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型推理成本高，因为需要大量去噪迭代。现有方法要么空间上每步解掩码多个令牌，要么时间上合并步骤，但缺乏联合优化。

Method: PSD基于单次前向传播的置信度，采用自适应解掩码策略选择位置，构建多深度推测草稿，然后通过批量验证进行分层接受，无需额外模型调用。

Result: 在三个扩散大语言模型和推理、代码生成任务上，PSD实现每前向传播最多5.5倍令牌数，且精度与贪心解码相当。

Conclusion: PSD在推理效率和生成质量之间取得了良好的权衡，是一种有效的加速框架。
...
TL;DR: 开源大语言模型对英语存在严重偏见，持续预训练在适应目标语言方面并不比从头训练更经济，未来应注重语言专属投资。


<details>
  <summary>Details</summary>
Motivation: 分析开源大语言模型对英语的偏见及其对目标语言适应的成本效益。

Method: 通过分析由开源大语言模型生成的序列来评估语言偏向，并比较持续预训练与从头训练的成本。

Result: 持续预训练在适应目标语言时没有成本优势，甚至在提升文化理解方面也效果有限。

Conclusion: 未来大语言模型开发应重视对每种语言的专门投入，而非主要依赖英语资源的扩展。
...
TL;DR: 构建了基于潜在歧义理论的中文歧义数据集CHA-Gen，评估LLMs的歧义处理能力，发现模型存在多种失败模式且偏向主导解释。


<details>
  <summary>Details</summary>
Motivation: 现有中文歧义数据集可扩展性差，研究集中在英文，需要可扩展的中文歧义数据集和方法。

Method: 基于潜在歧义理论设计半自动化流水线，构建包含5712句子的CHA-Gen数据集，涵盖18种潜在歧义结构。通过直接查询和机器翻译评估LLMs，使用语义熵进行不确定性量化。

Result: LLMs在歧义检测上表现不佳，CoT提示改进效果。发现三种失败模式：歧义盲、归因错误、过早消歧。歧义句语义熵更高，指令微调导致过度自信，基础模型更好捕捉语义多样性，模型偏向主导解释。

Conclusion: 提供了可扩展的中文歧义语料库构建方法，深入分析了LLMs处理歧义的缺陷，为改进中文歧义研究奠定基础。
...
TL;DR: 提出动态分块扩散模型(DCDM)，用内容定义的语义分块替代固定位置分块，提升离散扩散语言模型性能。


<details>
  <summary>Details</summary>
Motivation: 固定位置分块浪费序列结构，按位置分块会分离语义连贯的token并组合无关token。

Method: 核心是Chunking Attention，一种可微层，将token路由到K个可学习子空间参数化的簇中，由扩散目标端到端塑造，产生块因果注意力掩码，使离散扩散去噪器在语义块上自回归分解序列似然。

Result: 在参数规模高达1.5B的下游基准测试中，DCDM一致优于非结构和位置块扩散基线，优势在不同规模下稳定，并在训练早期可见。

Conclusion: DCDM通过动态语义分块有效提高了离散扩散语言模型的性能。
...
TL;DR: 提出Diagram-as-Code范式，利用mxGraph XML实现精确的图表生成与编辑，并构建VCG-Bench基准测试，揭示当前视觉语言模型在结构化保真度上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在处理结构化、可编辑的图表任务时存在关键差距，基于像素的方法可编辑性和保真度有限。

Method: 提出Diagram-as-Code范式，采用mxGraph可扩展标记语言（XML）进行符号化图表生成与编辑；构建VCG-Bench统一基准，包含六个领域15个子域的1449张图表数据集、生成（视觉到代码）与编辑（代码到代码）范式定义以及多维度的评估协议。

Result: 当前最先进的视觉语言模型在结构化保真度和指令遵循方面面临挑战，暴露出视觉与推理能力的不足。

Conclusion: 提出的符号化图表生成与编辑范式为专业工作流中的可控图表任务提供了新方向，现有模型在该任务上仍有较大提升空间。
...
TL;DR: 研究使用提示大语言模型在低资源标注下对在线患者问诊进行四类分诊，最强模型Claude Haiku 4.5在12-shot下macro-F1达到0.475，超过监督基线，但尚不能自主部署。


<details>
  <summary>Details</summary>
Motivation: 在线患者问诊内容非正式、不完整，需要自动分诊到合适临床随访级别，但标注数据稀缺。

Method: 构建人工标注金标准、自动标注银训练集和少样本池；比较TF-IDF、BioBERT基线与六个提示LLM在0-shot、4-shot、12-shot下的性能，采用macro-F1和安全感知指标评估。

Result: 最强LLM（Claude Haiku 4.5, 12-shot）macro-F1=0.475，超过BioBERT的0.378；少样本提示和两模型一致对某些类别有助益。

Conclusion: LLM可支持分诊优先级和选择性人工审核，但不能自主部署。
...
TL;DR: 提出ASRU框架，将生成质量作为核心目标，通过激活重定向和奖励函数优化实现可控多模态遗忘，在Qwen3-VL上遗忘效果提升24.6%，生成质量提升5.8倍。


<details>
  <summary>Details</summary>
Motivation: 现有多模态遗忘方法仅关注输出偏差，忽视生成质量，导致幻觉或僵化响应，影响模型可用性和安全性。

Method: ASRU框架：先通过激活重定向诱导初始拒绝行为，再用定制奖励函数优化精细拒绝边界，平衡遗忘与模型效用。

Result: 在Qwen3-VL上，平均遗忘效果提升24.6%，生成质量提升5.8倍，且仅需少量保留数据即可保持模型效用。

Conclusion: ASRU能有效实现可控多模态遗忘，同时显著提升生成质量，为多模态遗忘提供了兼顾效用与安全的新方法。
...
TL;DR: 提出H-Mem混合记忆机制，通过树和图结构建模记忆演化并实现高效检索，提升LLM代理的问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有工作缺乏有效建模记忆随时间演化的机制和高效检索记忆的方法，导致记忆利用性能差。

Method: 构建时间和语义树结构使短期记忆逐步演化为长期记忆，同时建立知识图谱捕获实体关系，并利用混合结构实现高效检索。

Result: 在三个代理记忆基准上，H-Mem在问答任务上达到最先进性能。

Conclusion: H-Mem有效改进了LLM代理的记忆利用能力。
...
TL;DR: 提出源分布式多模态记忆基准SMMBench，评估智能体在跨源证据检索、对齐与组合上的能力，实验表明当前系统表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有基准大多在预组装上下文中评估多模态记忆，忽略了证据分散在多个独立来源的情况，而这是多模态智能体记忆的重要瓶颈。

Method: 构建SMMBench，包含1877个样本和264个源，测试跨源多模态推理、冲突解决、偏好推理和记忆基础行动预测四项能力。

Result: 代表性基线方法在各项能力上均表现不佳，说明源分布式多模态记忆仍是一个挑战。

Conclusion: 源分布式多模态记忆是未被充分评估的重要问题，当前智能体在处理此类任务时存在明显不足。
...
TL;DR: 提出神经协同上下文工程（NCCE），将上下文工程从全局搜索转变为实例级动态路由，通过协同过滤实现个性化上下文分配，显著提升任务准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将上下文工程视为全局搜索问题，忽略了不同输入需要不同上下文，导致实例级性能提升空间未被利用。

Method: NCCE首先引导多样化的锚定上下文库，然后通过上下文-协同过滤协同进化机制，让轻量级神经协同过滤模型学习实例-上下文偏好，并生成专门化上下文变体，同时新上下文反馈优化模型。推理时，模型作为上下文路由器动态分配最优上下文。

Result: 理论证明和实验表明，NCCE通过匹配最优上下文显著提高了任务准确性，证明了个性化的重要性。

Conclusion: 通过将上下文工程转化为推荐问题，NCCE实现了实例级个性化上下文路由，优于全局优化方法。
...
TL;DR: 提出DimMem，一种轻量级维度记忆框架，通过结构化原子记忆单元解决LLM代理长期记忆中的保真度-效率权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统面临保真度-效率权衡：原始对话历史代价高，而扁平事实或摘要可能丢失精确回忆所需的结构。

Method: 将每个记忆表示为原子化、带类型、自包含的单元，包含时间、地点、原因、目的、关键词等显式字段，实现维度感知检索、更新和选择性助手上下文回忆。

Result: 在LoCoMo-10和LongMemEval-S上分别达到81.43%和78.20%的总体准确率，比现有轻量级记忆系统更好，同时LoCoMo每查询令牌成本降低24%；微调后的Qwen3-4B提取器在两个基准上均超过使用GPT-4.1-mini的LightMem。

Conclusion: 显式维度结构化是LLM代理长期记忆有效且高效的基石。
...
TL;DR: 小型开源LLM（<30B参数）在机器翻译质量评估中，通过单次提示策略，在质量评分、错误标注、纠错建议和后编辑方面表现优异，达到了与大型专有LLM相媲美的效果，同时具备成本效益和隐私保护优势。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的机器翻译质量评估依赖大型专有LLM，存在数据隐私问题。

Method: 采用单次提示策略，让小型开源LLM同时生成质量分数、MQM错误标注、纠错建议和完整后编辑。

Result: 这些模型在系统级相关性上优于传统神经指标、微调模型和人类互标注一致性，逼近大型专有LLM的能力。

Conclusion: 小型开源LLM是可行、经济且保护隐私的替代方案，可接近大型专有LLM的性能。
...
TL;DR: ForMaT是一个保留原始布局的多模态翻译平行语料库，包含15个语言对的3,956个PDF，用于评估和推动布局感知的机器翻译。


<details>
  <summary>Details</summary>
Motivation: 当前机器翻译系统缺乏对文本视觉上下文的处理，难以在保持布局的情况下进行高质量翻译。

Method: 使用K-Medoids聚类算法基于45个几何特征对PDF进行采样，确保数据集包含嵌套表格、公式等复杂视觉元素。

Result: 现有MT系统在空间定位和几何同步上表现不佳，容易丢失文本与视觉上下文的关联。

Conclusion: ForMaT为开发集成视觉和文本上下文的布局感知翻译模型提供了基准，有助于高保真文档重建。
...
TL;DR: 一个跨媒体俄罗斯政府政治传播数据集，包含演讲文本、图像、元数据和主题注释，支持多模态分析。


<details>
  <summary>Details</summary>
Motivation: 解决威权政治背景下社会文本和图像数据缺乏的问题。

Method: 构建数据集，包含俄语和英语文本、图像、标题、元数据；使用transformer多模态主题建模和专家验证生成主题注释。

Result: 得到链接的多模态数据资源，支持多种分析。

Conclusion: 该数据集为研究和LLM应用提供了有价值的测试平台。
...
TL;DR: 提出SemanticSeg数据集和块蒸馏框架，解决块注意力中的文本分割和微调低效问题，实现接近全注意力的性能。


<details>
  <summary>Details</summary>
Motivation: 块注意力在长上下文中可提升KV缓存复用，但面临难以分割文本和微调低效的挑战。

Method: 构建含30k+实例的SemanticSeg数据集，训练轻量级分割器；提出块蒸馏框架，包含块沉没标记、块丢弃和令牌级损失加权。

Result: 分割器优于启发式和统计基线，块蒸馏在块注意力下达到接近全注意力的性能。

Conclusion: 块注意力具有实用性和可扩展性，可部署于实际场景。
...
TL;DR: 将GRPO应用于编码器-解码器架构的机器翻译模型，使用无参考奖励，在13种语言上取得提升，尤其在数据稀缺时效果显著。


<details>
  <summary>Details</summary>
Motivation: 强化学习微调主要针对解码器LLM，对编码器-解码器研究不足。

Method: 使用GRPO和混合无参考奖励（LaBSE和COMET-Kiwi），无需平行数据，评估13种语言。

Result: 所有13种语言提升，最高+5.03 chrF++，与有监督微调竞争。

Conclusion: 增益与基线性能负相关、奖励可辨别性正相关，最有效于数据稀缺场景。
...
TL;DR: 使用符号方法将警方报告中的叙述转化为基于证据的事实，并构建时间图。


<details>
  <summary>Details</summary>
Motivation: 警方报告中的事件细节在自然语言中，需要人工阅读，缺乏自动化工具。

Method: 包括去除个人标识、语义解析、谓语映射到本体、推理等符号方法。

Result: 在450份财产犯罪报告上评估，54.1%的事件置信度≥0.8，93.7%通过语义路径映射，人工审查在事件触发、被盗物品和时间线索上达成100%一致，但强制进入解释一致性较低。

Conclusion: 符号方法有效提取叙述中的事件，但在强制进入解释上需改进。
...
TL;DR: 提出AI文化能力的三层次分类法：文化意识、文化敏感性、文化能力，以澄清概念并提升评估有效性。


<details>
  <summary>Details</summary>
Motivation: 现有AI文化能力评估定义模糊、术语混用，且局限于事实回忆，导致评估结果可能夸大模型能力。

Method: 借鉴跨文化沟通研究，构建三层次分类法：文化意识（模型是否知道）、文化敏感性（如何框架知识）、文化能力（能否随交互适应）。

Result: 该分类法作为实用工具，可提高AI评估在多元文化环境中的有效性和可解释性。

Conclusion: 缺乏概念清晰性会误导评估结果，导致在敏感文化场景中的不当部署决策。
...
TL;DR: 提出了一种基于学习者模型的评估标准，用于评估视觉语言模型在数学辅导中对不同学习者的适应性，实验发现当前模型表现不一致。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型被用于数学个性化教学，但缺乏评估其适应不同学习者能力的框架。

Method: 基于Shute和Towle的学习者模型，构建包含认知、动机和复杂性的适应性评估标准，并附加正确性和质量维度。

Result: 模型间适应性存在可测量差异，当前视觉语言模型难以持续生成基于学习者模型的教学回应，尤其在信息有限时。

Conclusion: 需要改进视觉语言模型的适应性，以更好支持个性化学习。
...
TL;DR: 大语言模型作为评判者时，输出格式改变导致评分不一致，原因是共享的评估子图和脆弱的格式特定终端分支。


<details>
  <summary>Details</summary>
Motivation: 现有研究仅停留在输入输出层面，缺乏对内部机制的理解。

Method: 使用位置感知边缘属性修补（PEAP）对Gemma-3、Qwen2.5和Llama-3进行因果分析。

Result: 发现跨任务共享一个稀疏的潜在评估子图（中后期MLPs）；零消融该子图评判崩溃但保留知识。格式不一致源于连续评判信号经脆弱的格式特定终端分支映射。

Conclusion: 基准测试的可靠性比较部分测量的是格式化器几何而非评估质量。
...
TL;DR: 提出S2ST-Omni 2，一种引入类型学先验的多对一组合式语音到语音翻译框架，在CVSS-C上取得优越性能，并验证了类型学先验在低资源场景下的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有S2ST系统忽略源语言信息或使用扁平语言标签编码，缺乏跨语言系统结构，限制了数据高效的多语言适应。

Method: 在表示层采用类型学感知的分层语言编码，在声学层使用动态门控的语言感知Dual-CTC，在解码层通过类型学感知的LLM提示提供语言指导。

Result: 在CVSS-C上，S2ST-Omni 2在BLEU、COMET、ASR-BLEU和BLASER 2.0指标上优于代表性方法；消融实验显示各层次策略互补；低资源日英翻译（约3小时数据）表明类型学先验提供有效归纳偏置。

Conclusion: 显式类型学先验为数据高效的多语言语音到语音翻译提供了有用的归纳偏置。
...
TL;DR: 提出RecMem，一种基于语义重复触发的记忆系统，显著降低LLM token消耗。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统对每个交互都使用LLM提取记忆，导致大量token消耗。

Method: 将交互存储在潜意识记忆层，使用轻量嵌入模型检索；仅当语义相似交互持续重复时才调用LLM提取情景和语义记忆；并采用语义精炼机制恢复遗漏细节。

Result: 相比三种最先进的记忆系统，RecMem将记忆构建token成本降低高达87%，同时准确性超过它们。

Conclusion: RecMem通过延迟记忆整合的时机，有效平衡了token消耗和准确性。
...
TL;DR: 提出基于大语言模型的数据增强框架，通过生成多种风格的口语化独白来改善认知评分预测，特别针对类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 从自发言语中评估认知衰退因数据集小和类别不平衡而充满挑战。

Method: 使用GPT-5以书面回答为语义锚点生成多种风格的口语独白，结合Sentence-BERT嵌入和偏最小二乘回归预测Hasegawa痴呆量表分数，采用相似性引导的类平衡选择策略。

Result: 相似性引导的类平衡选择能稳定提升预测性能，大幅减少低分少数群体的预测误差，同时保持多数群体性能。
...
TL;DR: 提出了一个多级上下文token关系建模框架，通过马尔可夫校准和规则推理改进机器生成文本检测，在跨LLM和跨域场景下实现低开销的显著提升。


<details>
  <summary>Details</summary>
Motivation: 机器生成文本存在风险，而基于度量的检测方法虽实用但面临token级分数受生成随机性偏差的核心挑战。

Method: 理论分析token级检测分数的多跳转换，提取局部和全局关系；局部用马尔可夫信息校准模块优化token证据，全局用规则支持推理模块利用逻辑规则，最终联合多级推理。

Result: 在跨LLM和跨域等现实场景中，该方法表现出广泛且显著的性能提升，且计算开销低。

Conclusion: 所提框架有效解决了token级分数偏差问题，为MGT检测提供了高效可靠的解决方案。
...
TL;DR: 提出基于检索增强生成的无微调动态去偏框架DebiasRAG，通过查询特定去偏上下文生成与重排序减少社会偏见。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在社会偏见，现有微调或提示方法需额外资源、可能损害模型能力且缺乏动态去偏。

Method: 三阶段：生成查询相关偏见候选、构建常规检索上下文池、梯度更新重排序去偏上下文。

Result: 在保持模型固有性能的同时有效降低偏见。

Conclusion: DebiasRAG为无微调动态去偏提供有效方案，平衡公平性与模型能力。
...
TL;DR: 提出SGR框架，通过构建查询特定的外部知识子图，引导大语言模型进行逐步推理，以提升复杂推理的准确性和事实一致性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在复杂推理任务中易生成不相关或不一致内容，需增强其结构化知识利用能力。

Method: 根据输入问题从外部知识库构建子图，利用子图的语义结构引导模型分步推理，并整合多条推理路径得到最终预测。

Result: 在多个基准数据集上，SGR相比基线方法取得一致提升，验证了其有效性。

Conclusion: SGR通过结构化外部知识增强推理，能有效提高推理准确性与事实可靠性。
...
TL;DR: 结合编码代理与树搜索算法，自主生成高效三维光伏结构，解决奖励黑客问题后获得优化设计。


<details>
  <summary>Details</summary>
Motivation: 克服平板太阳能电池板在中纬度地区的效率损失，通过三维结构实现更高能量密度。

Method: 使用AntiGravity编码代理重现基准计算，结合ERA树搜索算法进行大规模优化，并通过迭代修正物理引擎消除奖励黑客行为。

Result: 发现多种约束下的最优设计，包括固定收集面积、天顶跟踪和避免自阴影，显著提升日间发电量。

Conclusion: 编码代理与树搜索的整合为可评分问题的科学发现提供了强大平台。
...
TL;DR: 提出基于价值观的人物构建方法，通过文本描述捕捉核心文化维度，结合校准过程，提高LLM在跨文化人口意见模拟中的准确性，缩小了代表性不足人群的预测差距。


<details>
  <summary>Details</summary>
Motivation: 现有基于人物角色的提示方法依赖人口统计学或人格特质，这些只是间接代理，无法准确反映塑造人类反应的价值观，导致LLM在跨文化人群中模拟意见的能力有限。

Method: 从调查回应中派生描述核心价值观文化维度的文本描述符，采样目标人群的价值分布，聚合LLM跨人物角色的回应，并引入校准程序以提高回应多样性同时保持估计意见。

Result: 方法降低了各国预测误差，在代表性不足的人群中改进最大，显著缩小了与主流LLM先验一致的国家和训练数据中代表性不足国家之间的性能差距，回应分布更接近人类多样性。

Conclusion: 基于价值观的人物构建和校准能有效提升LLM跨文化意见模拟的准确性，尤其有利于代表性不足的人群。
...
TL;DR: 提出Argus，通过搜索器与导航器协作，将深度研究视为拼图组装，避免并行轨迹重复，提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有并行搜索方法产生重复证据，收益递减且聚合上下文易超限，需更高效协作机制。

Method: Searcher负责子查询的ReAct轨迹，Navigator维护证据图、验证缺失、调度搜索并综合生成答案，Navigator用强化学习训练。

Result: 单搜索器提升5.5分，8并行提升12.7分，64搜索器在BrowseComp达86.2，超越所有评测的专有智能体，且推理上下文低于21.5K token。

Conclusion: Argus通过协作与图推理有效扩展深度研究，避免重复，保持低上下文占用。
...
TL;DR: 通过类似失语症的模拟方法，对语言模型进行参数‘损伤’并评估其症状，发现注意力和前馈组件之间的症状谱差异，以及深度效应，但模型与人类失语症症状模式存在质的差异。


<details>
  <summary>Details</summary>
Motivation: 利用失语症研究中脑损伤与症状的因果关系，探索语言模型中功能性组织的涌现特性。

Method: 对五个1B参数规模的语言模型进行参数归零（损伤），并使用文本失语症测试（TAB）评估输出症状，分析不同组件和层的影响。

Result: 在112,426个输出中观察到全范围症状，但分布与人类不同；注意力和前馈组件间症状谱差异明显，深度上早期层更多句法和语义症状，中后期层更多语音和流畅性缺陷。

Conclusion: 语言模型与人类失语症症状模式质的差异表明，失语症综合征受学习和处理细节影响，而非领域不变的语言处理中断结果。
...
TL;DR: 智能计量、生成式AI和量子启发组合优化的加速融合正在重塑能源公用事业的管理方式。


<details>
  <summary>Details</summary>
Motivation: 为了更高效地管理物理基础设施、提升客户参与度并加强环境责任。

Method: 融合智能计量数据、生成式人工智能和量子启发式组合优化技术。

Result: 实现了基础设施管理的优化、客户互动的增强和环境问责的严格化。

Conclusion: 这种融合将推动能源行业向更智能、更可持续的方向变革。
...
TL;DR: 提出了一个统一的端到端框架，将生成式AI账单起草、变压器预测器、碳核算和负荷调度整合在一起，以提升电费账单可读性、碳数据可靠性和电网调度效率。


<details>
  <summary>Details</summary>
Motivation: 配电公司需要提供客户可读的账单、为每千瓦时销售分配可辩护的碳数值，并根据电网压力和排放约束调度负荷。

Method: 框架整合了四个生产级能力：基于约束解码策略的生成式AI代理从结构化数字输入起草自然语言账单；变压器预测器提供日前消费估计及校准分位数带；以及另外两个能力（碳核算和负荷调度）。

Result: 实现了账单自然语言生成、负荷估计和碳数据附加的统一解决方案。

Conclusion: 该框架能够满足配电公司在账单可读性、碳数据可辩护性和负荷调度优化方面的多重需求。
...
### [91] [Stochastic Compositional Optimization via Hybrid Momentum Frank--Wolfe](https://arxiv.org/abs/2605.15350)
*El Mahdi Chayti*

Main category: math.OC

TL;DR: 提出混合动量随机Frank-Wolfe算法，无需外函数光滑，在非凸目标下达到O(K^{-1/4})收敛率。


<details>
  <summary>Details</summary>
Motivation: 现有随机组合优化方法要求外函数连续可微，排除许多实际应用如鲁棒最大损失、条件风险价值等。

Method: 结合动量雅可比追踪和泰勒修正函数追踪，将整体随机线性化输入广义线性最小化预言机。

Result: 在非凸目标且外函数Lipschitz条件下，广义Frank-Wolfe间隙达到O(K^{-1/4})收敛率，匹配投影免费单样本随机方法的最优复杂度，并扩展到重尾噪声。

Conclusion: 算法有效放宽了对F的光滑性假设，并保持最优收敛性能。
...
### [92] [Stochastic Non-Smooth Convex Optimization with Unbounded Gradients](https://arxiv.org/abs/2605.15522)
*Dmitry Kovalev*

Main category: math.OC

TL;DR: 提出广义Lipschitz函数类，证明AdamW带裁剪更新在凸随机优化中优于SGD和AdaGrad。


<details>
  <summary>Details</summary>
Motivation: 现有非光滑优化理论假设梯度一致有界，过于严格；引入更现实的广义Lipschitz函数类，梯度范数由最优间隙的仿射函数界定。

Method: 通过收敛性分析对比多种算法，发现AdamW带裁剪更新性能最佳；强调指数加权梯度累积而非简单平均的关键作用。

Result: AdamW带裁剪更新在凸随机优化中理论收敛率优于SGD和AdaGrad；在广义光滑性假设下实现改进速率，并适用于对角及矩阵预条件器。

Conclusion: 裁剪AdamW具有通用性，可扩展至quasar-convex设置，其指数加权累积机制至关重要。
...
### [93] [Sampling pseudospectrum for data-driven matrices](https://arxiv.org/abs/2605.15234)
*Caroline Wormell*

Main category: math.NA

TL;DR: 提出采样伪谱方法，用于统计检验有限数据中提取的特征值是否为真实模式而非噪声。


<details>
  <summary>Details</summary>
Motivation: 现有算法（如DMD）从有限数据中提取离散特征值，但无法客观区分哪些是真实模式，哪些是数据误差的假象。

Method: 定义采样伪谱 $P(\lambda)$ 并推导其估计量 $\hat P(\lambda)$，通过重处理有限数据样本实现计算，提供概率信息。

Result: 该方法计算高效，能统计检验真实特征值的位置，有效区分信号与噪声。

Conclusion: 提供了一种严格且通用的方法，用于评估从有限数据中提取的模式是信号还是噪声。
...
### [94] [Amortized Energy-Based Bayesian Inference](https://arxiv.org/abs/2605.15407)
*Hojjat Kaveh,Ricardo Baptista,Andrew M. Stuart*

Main category: math.NA

TL;DR: 提出一种基于运输的无似然贝叶斯推理方法，学习观测依赖的映射以近似后验分布，实现快速采样。


<details>
  <summary>Details</summary>
Motivation: 传统MCMC方法对每个观测需重新推理，计算成本高，不适于多次重复推理的场景。

Method: 通过最小化真实后验与学习到的pushforward之间的平均能量距离，训练观测依赖的映射将参考测度推向前。该方法无似然，仅需联合样本，避免密度评估和雅可比行列式计算。对高斯先验函数空间问题，映射参数化为单位映射加扰动，并用神经算子表示。

Result: 在有限维和PDE约束的逆问题（多孔介质流、地震反演）上验证，成功捕获后验的多模态和主导模式，并为新观测提供快速后验采样。

Conclusion: 该方法有效，能在重复推理场景下快速生成后验样本，具有实用价值。
...
### [95] [Complexity of Non-Log-Concave Sampling in Fisher Information](https://arxiv.org/abs/2605.15859)
*Sinho Chewi,Andre Wibisono*

Main category: cs.DS

TL;DR: 针对对数光滑非对数凹分布的采样问题，研究了获取相对Fisher信息保证的查询复杂度，提出了基于近端采样器和受限高斯Oracle的算法，并给出了复杂度保证及其与对数凹采样的维度依赖关系。


<details>
  <summary>Details</summary>
Motivation: 为采样问题建立类似于优化中找到近似驻点的保证，即相对Fisher信息保证，并探索其查询复杂度。

Method: 采用近端采样器（Langevin扩散的隐式离散化）结合受限高斯Oracle（RGO）实现后向步骤，并利用对数凹采样在高精度Rényi散度下的近期结果来近似实现RGO。

Result: 算法在相对Fisher信息上继承了与对数凹采样相同的维度依赖复杂度，并改进了非对数凹采样的先前工作；同时证明了逆归约：任何相对Fisher信息维度的改进都将转化为高精度对数凹采样的维度改进。

Conclusion: 通过近端采样器和RGO的结合，实现了非对数凹采样中相对Fisher信息保证的复杂度提升，并建立了与对数凹采样维度依赖的相互关联。
...
### [96] [Estimating Social Norm Complementarities](https://arxiv.org/abs/2605.15405)
*Eliana La Ferrara,Cheaheon Lim,Davide Viviano*

Main category: econ.GN

TL;DR: 建立了一个社会规范选择模型，考虑技术互补和社会互补，估计了塞拉利昂和尼日利亚关于女性割礼、一夫多妻和童婚的互补性，发现两种互补性显著且因规范和国家而异。


<details>
  <summary>Details</summary>
Motivation: 理解社会规范之间的互补性如何影响其传播，以及政策干预的效果。

Method: 构建包含技术互补和社会回报的模型，利用重复截面数据估计，并进行政策反事实模拟。

Result: 社会回报显著；女性割礼与童婚互补（尤其在塞拉利昂），一夫多妻与童婚社会替代（尤其在尼日利亚）。

Conclusion: 法律改革和社会干预可能产生不同效果，取决于规范间的互补/替代关系。
...
### [97] [Crossover from subcritical to critical decay: random walk, self-avoiding walk, percolation](https://arxiv.org/abs/2605.15545)
*Yucheng Liu,Gordon Slade*

Main category: math.PR

TL;DR: 证明了在适当假设下，Ornstein-Zernike方程的解渐近行为是带有各向异性指数衰减因子的布朗运动格林函数，适用于多种随机行走模型，揭示了从Ornstein-Zernike到临界衰减的交叉细节。


<details>
  <summary>Details</summary>
Motivation: Ornstein-Zernike衰减到临界幂律衰减的交叉在文献中很少受到关注，需要理论理解。

Method: 基于方向依赖指数衰减率的变分刻画，并扩展Hara2008高斯引理到非中心核。

Result: 定理适用于广泛随机行走、高维最近邻自回避行走和渗流，证明了有限阶相关长度等效（相差普适常数）。

Conclusion: 该定理揭示了相关长度尺度上从Ornstein-Zernike到临界衰减的交叉细节，以及相关长度的普适等价性。
...
### [98] [Turbulent stretching of FENE dumbbell polymer model via special stochastic scaling and singular limits](https://arxiv.org/abs/2605.15742)
*Federico Butori,Yassine Tahraoui*

Main category: math.PR

TL;DR: 研究FENE聚合物模型在随机湍流中的拉伸机制，通过随机缩放极限和奇异极限技术，得到了确定性极限方程并识别了平稳分布。


<details>
  <summary>Details</summary>
Motivation: 理解聚合物在湍流中的拉伸行为，尤其是平均湍流拉伸效应的数学描述。

Method: 采用随机Fokker-Planck方程，结合主导空间尺度和时间尺度的缩放假设，利用路径wise收敛和加权空间分析。

Result: 聚合物密度方程弱收敛到带有二阶算子的确定性方程，该算子代表平均湍流拉伸；在τ→0极限下，得到聚合物长度的平稳分布。

Conclusion: 提出了一种无需对随机流实现取平均即可获得确定性极限的新方法，并成功识别了平稳分布。
...
### [99] [Deconfinement For $\mathrm{SO}(3)$ Lattice Yang-Mills at Strong Coupling](https://arxiv.org/abs/2605.16162)
*Ron Nissim*

Main category: math.PR

TL;DR: 我们严格证明了格点Yang-Mills理论中具有平凡中心的规范群不满足Wilson夸克禁闭判据，特别是SO(3)格点Yang-Mills理论在强耦合区域不满足该判据。


<details>
  <summary>Details</summary>
Motivation: 将物理学中关于具有平凡中心的规范群格点Yang-Mills理论不满足Wilson禁闭判据的预测严格化。

Method: 通过数学证明，具体论证了SO(3)格点Yang-Mills理论在强耦合区域不满足Wilson判据。

Result: SO(3)格点Yang-Mills理论在强耦合区域不满足Wilson判据。

Conclusion: 证实了具有平凡中心的规范群格点Yang-Mills理论不满足Wilson禁闭判据的物理预测。
...
### [100] [Non-combinatorial involutive braidings: the quantum algebra $\mathfrak{gl}_{k,m}$](https://arxiv.org/abs/2605.16121)
*Anastasia Doikou*

Main category: math.QA

TL;DR: 研究辫子方程的对合非组合解，引入 gl_{k,m} Yangian 量子代数，构造其最高权模并关联自旋链哈密顿量，特殊情形下简化为 XX 模型，并分析了组合基与杨图的联系。


<details>
  <summary>Details</summary>
Motivation: 探索辫子方程中置换映射的特殊变形，并识别相关量子代数结构。

Method: 利用对合非组合解关联量子代数，构造 gl_{k,m} 的最高权模，并分析其组合基与杨图关系。

Result: 得到 gl_{k,m} Yangian 代数及其子代数结构，构造了最高权模，给出自旋链哈密顿量的本征态；gl_{1,1} 情形下哈密顿量退化为 Heisenberg XX 模型变体；建立了最高权表示组合基与杨图中具体形状的对应。

Conclusion: 成功将辫子方程解与量子代数、自旋链模型联系起来，并揭示了 gl_{1,1} 表示论与杨图组合学的明确关系。
...
### [101] [A Finite-State Gibbs Construction from a Recognition Cost](https://arxiv.org/abs/2605.15667)
*Megan Simons,Jonathan Washburn*

Main category: cond-mat.stat-mech

TL;DR: 提出一种基于比率成本构造的有限状态吉布斯分布，使用归一化达朗贝尔二次闭包识别组合律（RCL），得到连续正分支J(x)=cosh(log x)-1。通过多项计数和凸对偶恢复有限状态吉布斯权重和恒等式。包含非渐近Stirling界和软壳约束定理。三状态示例比较了吉布斯律与平方对数、亲和力能量和Tsallis替代方案。


<details>
  <summary>Details</summary>
Motivation: 在有限结果空间上，通常通过最大化给定能量泛函的香农熵获得吉布斯分布。本文研究使用比率成本构造（基于达朗贝尔二次闭包识别组合律RCL）的替代方案，并探讨其优势。

Method: 采用归一化达朗贝尔二次闭包识别组合律（RCL），以单位对数曲率校准参考比率，得到连续正分支J(x)=cosh(log x)-1。通过多项计数和凸对偶推导有限状态吉布斯权重及恒等式F_R(q)-F_R(p)=T_R D_KL(q||p)。

Result: 得到非渐近Stirling界和软壳约束型定理。在三状态示例中，比较了吉布斯律与平方对数、亲和力作为能量和Tsallis替代方案在相同成本向量和均值成本约束下的表现，并基于固定RCL真实值进行了样本量功效计算。

Conclusion: 该框架依赖于公理(A1)-(A3)，仅限于严格正权的有限结果空间，未从更基本的原则推导组合律。
...
### [102] [Nonlinear stability of continuously self-similar naked singularities for the Einstein-scalar field equations II: linearized stability](https://arxiv.org/abs/2605.16095)
*Jaydeep Singh,Weihao Zheng*

Main category: gr-qc

TL;DR: 证明了相同正则性扰动下裸奇点解的线性稳定性，为非线性稳定性奠定基础。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明裸奇点解在粗糙扰动下因蓝移不稳定机制而不稳定，但本文旨在探讨相同正则性扰动下的稳定性，揭示正则性对不稳定机制的关键影响。

Method: 对连续自相似裸奇点解进行线性扰动分析，考虑与背景相同正则性的扰动。

Result: 发现这些解在相同正则性扰动下是线性稳定的，蓝移不稳定机制未被触发。

Conclusion: 线性稳定性为非线性稳定性提供了基础，结合伴文证实了这些裸奇点解的非线性稳定性。
...
### [103] [Non-minimal fluid Lagrangian couplings](https://arxiv.org/abs/2605.16220)
*Christian G. Boehmer,Erik Jensko,Eissa Al-Nasrallah*

Main category: gr-qc

TL;DR: 本文仔细推导了非最小耦合引力模型中的引力场方程和完整流体方程组，定义了温度、化学势和数密度等热力学量，并证明了Schutz与Brown拉格朗日公式的不等价性。


<details>
  <summary>Details</summary>
Motivation: 非最小耦合引力模型旨在通过物质拉格朗日与曲率的耦合解释暗能量或暗物质，无需引入额外源项，但涉及完美流体的变分公式化存在技术细节。

Method: 对包含物质拉格朗日函数的非最小耦合引力理论，进行严格的变分推导，得到引力场方程和完整流体方程组。

Result: 得到非最小耦合下流体的热力学量（温度、化学势、数密度）定义，并揭示Schutz和Brown两种拉格朗日公式在该类模型中的不等价性。

Conclusion: 非最小耦合会影响流体热力学量，且拉格朗日公式的选择对理论表述有重要影响，需谨慎处理。
...
### [104] [Nonlinear stability of continuously self-similar naked singularities for the Einstein-scalar field equations I: main results](https://arxiv.org/abs/2605.16235)
*Weihao Zheng*

Main category: gr-qc

TL;DR: 本文首次证明了一族连续自相似裸奇点解在球对称爱因斯坦-标量场方程中的非线性稳定性（在局部Hölder拓扑下），与先前在粗糙扰动下的不稳定性结果形成对比，凸显了函数框架对弱宇宙监督猜想的关键影响。


<details>
  <summary>Details</summary>
Motivation: Christodoulou曾构造了这些裸奇点解并证明其在粗糙扰动下不稳定，从而在粗糙框架下验证了弱宇宙监督猜想；但尚未有研究在同样正则性的一般扰动下检验其稳定性，而这对于理解泛函框架的作用至关重要。

Method: 基于伴生论文的线性化稳定性结果，在局部Hölder拓扑中考虑初始扰动的小开邻域，通过非线性分析建立稳定性。

Result: 在初始扰动足够小（按局部Hölder度量）的条件下，这些裸奇点解保持非线性稳定，不会转化为黑洞。

Conclusion: 弱宇宙监督猜想的有效性依赖于所选取的泛函框架：在粗糙扰动下裸奇点不稳定，但在足够光滑的扰动下却稳定，因此猜想表述必须明确函数空间。
...
TL;DR: 本文基于Tweedie公式给出了分数驱动模型的贝叶斯解释，揭示了其在共轭自然指数族中与精确贝叶斯更新的等价性，并明确了条件分数在标准正态化预测误差中的作用。


<details>
  <summary>Details</summary>
Motivation: 旨在将经验贝叶斯、近似滤波、动态广义线性模型与分数驱动模型统一起来，并区分精确贝叶斯更新与局部分数近似。

Method: 利用Tweedie公式，在自然指数族中引入基测度调整；对于一般条件密度，通过Fisher评分和精度折扣得到局部高斯后验校正；在共轭自然指数族中推导了精确的分数驱动表示。

Result: 在共轭自然指数族中，稳态精度折扣和期望空间逆Fisher缩放下，分数驱动校正等于施加转换动态前的贝叶斯后验均值；Tweedie方差函数指数解释了条件分数如何标准化预测误差。

Conclusion: 该工作桥接了多个模型框架，并明确了精确贝叶斯更新与基于分数的局部近似之间的界限。
...
TL;DR: 本文提出了一种拟贝叶斯局部投影工具变量估计方法，利用GMM目标和粗糙惩罚先验平滑脉冲响应，提升了有限样本稳定性并允许联合推断。


<details>
  <summary>Details</summary>
Motivation: 传统LP-IV方法在有限样本下稳定性不足，且难以进行多期联合推断。

Method: 基于GMM目标构建拟后验，引入粗糙惩罚先验对不同期限的脉冲响应进行平滑。

Result: 模拟显示，与标准GMM相比，该方法降低了均方根误差，尤其是在中期和长期；应用丹麦电力市场数据验证了实用性。

Conclusion: 该正则化方法在保留LP-IV关键一阶特征的同时，显著改善了有限样本性能和联合推断能力。
...
### [107] [Universal Magnetic Structure Prediction from Atomic Coordinates with Near-Experimental Accuracy](https://arxiv.org/abs/2605.16230)
...
TL;DR: 提出了磁性结构网络（MSN），一种E(3)等变图神经网络，直接从原子晶体结构预测共线和非共线磁性结构，在MAGNDATA上训练，通过原始调制结构表示（PMSR）统一处理公度和非公度结构，实现了高保真重建。


<details>
  <summary>Details</summary>
Motivation: 磁性结构难以通过实验或第一性原理方法确定，需要高效且准确的预测方法。

Method: 使用E(3)等变图神经网络（MSN），并引入原始调制结构表示（PMSR）统一编码公度和非公度磁性结构，无需对称性假设。

Result: 模型在所有调制分量上表现优异，能够高保真地重建实验磁性结构。

Conclusion: 提供了一种可扩展的快速磁性结构预测框架，开启了数据驱动发现磁性材料的途径。
...
### [108] [Functional Cox model for interval-censored data](https://arxiv.org/abs/2605.15303)
*Yangjianchen Xu,Peijun Sang*

Main category: stat.ME

TL;DR: 提出了一个函数Cox模型来处理区间删失数据中的函数协变量，通过惩罚最大似然估计和EM算法进行参数估计，证明了估计量的一致性和渐近正态性，并构建了全局检验。


<details>
  <summary>Details</summary>
Motivation: 区间删失数据中经常遇到函数协变量，需要研究函数协变量的轨迹如何影响事件时间。

Method: 基于函数Cox模型，采用惩罚最大似然估计和EM算法进行参数估计，利用剖面似然方法估计协方差矩阵。

Result: 回归参数和系数函数的线性泛函的估计量具有一致性和渐近正态性，协方差矩阵达到半参效率界，全局检验有效。模拟研究和ADNI数据应用验证了方法的性能。

Conclusion: 该模型和方法能有效处理区间删失数据中的函数协变量，估计量具有良好的统计性质，实际应用可行。
...
### [109] [Nonparametric inference for sublevel-set probabilities of conditional average treatment effect functions](https://arxiv.org/abs/2605.15373)
*Anders Munch,Thomas A. Gerds*

Main category: stat.ME

TL;DR: 提出基于条件平均处理效应（CATE）子集概率的单调曲线，用于可视化处理效应异质性，并开发了相应的估计方法。


<details>
  <summary>Details</summary>
Motivation: 平均处理效应掩盖了个体差异，而CATE在多协变量时难以解释，需要一种简单的单变量曲线来刻画总体异质性的类型和程度。

Method: 将曲线形式化为目标参数，证明其在非参数模型下非路径可微；利用单调函数估计的最新进展，开发结合机器学习的Grenander型估计量，并针对最佳分段线性近似提出去偏机器学习估计量。

Result: 基于随机试验合成数据的数值研究验证了有限样本性能，并在糖尿病药物随机试验数据中进行了实例分析。

Conclusion: 所提出的曲线及估计方法能有效揭示和可视化处理效应异质性，具有实际应用价值。
...
### [110] [Modeling Misclassification in Spousal Violence Reporting: Evidence from Bayesian Quantile Regression](https://arxiv.org/abs/2605.15428)
*Joon Jin Song,Mohammad Arshad Rahman,Yoo-Mi Chin,James Stamey*

Main category: stat.ME

TL;DR: 提出了一种针对误分类二元结果的贝叶斯分位数回归方法，通过MCMC算法估计，有效校正报告偏差。


<details>
  <summary>Details</summary>
Motivation: 二元结果变量因低报或虚报导致的误分类会使推断产生严重偏差，现有分位数回归未处理此问题。

Method: 引入潜在真实响应变量，显式建模假阴性和假阳性报告误差，采用新型马尔可夫链蒙特卡洛（MCMC）算法进行估计。

Result: 模拟研究表明，在不同先验设定和误分类率下，所提方法优于忽略误分类的模型；应用于配偶暴力数据发现低报超过高报，且考虑误分类改变实质性结论。

Conclusion: 在处理敏感二元结果时，必须考虑误分类以避免误导性推断，本方法提供了有效解决方案。
...
### [111] [Tree-aggregated regression for compositional data with measurement errors](https://arxiv.org/abs/2605.15469)
*Zhenghan Li,Tianying Wang*

Main category: stat.ME

TL;DR: 提出TARCO方法，同时处理树聚合与测量误差的相互作用，通过偏校正估计量和树感知正定稳定化及稀疏正则化，解决高维成分数据的回归问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法未考虑树聚合与测量误差的交互导致的层次污染，该交互会增大偏差、弱化校正估计量的集中率，并导致不稳定的变量选择。

Method: TARCO集成偏校正估计量与树感知正半定稳定化及稀疏正则化，通过基于校正目标的交叉验证选择调参，形成可扩展求解的凸规划。

Result: 在模拟和微生物组应用中，TARCO相比忽略交互的方法提高了估计精度、支持恢复和聚合层解释性，并建立了预测和估计误差的有限样本界及符号一致性。

Conclusion: TARCO有效应对树聚合与测量误差的交互，提升回归性能，且当测量误差协方差被一致估计时保证仍然成立。
...
### [112] [Improving the Efficiency of Subgroup Analysis in Randomized Controlled Trials with TMLE](https://arxiv.org/abs/2605.15483)
*Sky Qiu,Nerissa Nance,Rachael Phillips,Jens Tarp,Maya Petersen,Mark van der Laan*

Main category: stat.ME

TL;DR: 提出两种在随机对照试验中利用非亚组参与者信息增强亚组分析精度的目标最大似然估计量，不依赖外部数据，提高亚组治疗效果估计的精确性。


<details>
  <summary>Details</summary>
Motivation: 随机对照试验中亚组分析常因样本量不足而功效低下，亟需在不引入外部偏倚的前提下提升亚组估计精度。

Method: 研究了两种目标最大似然估计量：TMLE-PR（联合回归）和A-TMLE（自适应），通过共享同一试验内非亚组参与者的信息来增强亚组估计。

Result: 在LEADER心血管结局试验中，A-TMLE估计出亚洲裔患者MACE绝对风险降低1.5-1.6个百分点，黑人患者降低2.0-2.1个百分点，95%置信区间均排除零效应。

Conclusion: 该方法在不依赖外部真实世界数据的情况下提高了亚组治疗效果估计的精度，支持更公平的药品标签、获取和上市后评估，符合监管机构优先事项。
...
### [113] [Tail postcoloring in long-run variance estimation of time series](https://arxiv.org/abs/2605.15596)
*Xu Liu,Kin Wai Chan*

Main category: stat.ME

TL;DR: 提出一种名为tail postcoloring的新方法，通过参数模型对非参数估计器中被忽略的尾部自协方差进行后着色，以处理强自相关，相比预白化方法更稳健且避免方差膨胀。


<details>
  <summary>Details</summary>
Motivation: 预白化方法在处理强自相关时存在对模型误规范敏感、可能引起方差膨胀或功效降低等问题，需要一种更稳健的替代方法。

Method: 基于预白化的思想，提出tail postcoloring方法：使用参数模型将非参数估计器中忽略的尾部自协方差（通过缩放因子）投影到最终估计器，并通过带宽参数自动在非参数估计和参数着色之间切换，无需将整个数据集转换为残差。

Result: 当着色模型正确指定时，可达到参数速率；在有限样本中，对着色模型的误规范比预白化方法更稳健，且避免了标准方法中重新着色因子导致的严重方差膨胀或功效降低。

Conclusion: tail postcoloring方法可以扩展到使用多个参数模型构造多重稳健估计，并自然适用于多元时间序列，在马尔可夫链蒙特卡罗输出分析等实际数据中有效。
...
### [114] [Structured Transfer Learning for Survival Risk Stratification in Data-Sparse Clinical Cohorts](https://arxiv.org/abs/2605.15633)
*Junhan Yu,Yurui Chen,Juan Delgado-SanMartin,Dennis Wang,Hong Pan,Doudou Zhou*

Main category: stat.ME

TL;DR: 提出CORE-Cox迁移学习框架，用于小样本队列的生存风险分层，通过共享跨结局低秩模式并结合针对性调整，在UK Biobank和MIMIC-IV中提升了C-index和风险富集。


<details>
  <summary>Details</summary>
Motivation: 生存预测模型在样本量小或结局事件少的临床群体中可靠性差，单队列模型不稳定，大队列模型迁移效果不佳，需要一种能共享跨人群风险因子模式同时允许队列特异性适应的迁移学习方法。

Method: 开发了CORE-Cox两阶段框架：先在较大源队列中通过低秩Cox系数结构学习跨相关结局的共享风险因子模式，再通过正则化残差修正将其适应到较小目标队列。

Result: 在UK Biobank（白种人源队列n=150,093；亚裔目标队列n=2,534）和MIMIC-IV（白种人ICU源队列n=15,997；亚裔ICU目标队列n=672）中，CORE-Cox在大多数结局上取得最佳或接近最佳区分度，平均C-index从0.733提升至0.766（UK Biobank）和从0.628提升至0.658（MIMIC-IV），并在9个结局中的8个上有所提升，同时改善了前15%风险富集。
...
### [115] [Leveraging heterogeneity for identifiability: Bayesian order-based learning of multiple DAGs](https://arxiv.org/abs/2605.15639)
*Hyunwoong Chang,Fariha Taskin*

Main category: stat.ME

TL;DR: 提出一个利用异质性数据的联合顺序评分框架，用于DAG因果结构学习，并发展了贝叶斯方法和高效MCMC算法。


<details>
  <summary>Details</summary>
Motivation: 利用异质性提高因果顺序估计的准确性。

Method: 联合顺序评分框架，基于顺序的贝叶斯方法，以及随机到随机（R2R）提议邻域的Metropolis-Hastings算法。

Result: 模拟研究证实强经验性能，应用于重度抑郁症单核RNA测序数据展示实用性。

Conclusion: 方法有效，具有理论和实践价值。
...
### [116] [Re-examining and calibrating weighted survival analysis for causal inference](https://arxiv.org/abs/2605.15702)
*Wenfu Xu,Yi Zhang,Tobias Gerhard,Zhiqiang Tan*

Main category: stat.ME

TL;DR: 本文重新审视了加权Kaplan-Meier方法，通过校准估计开发新方法，在低维和高维设置下改善了时间至事件结局的因果推断。


<details>
  <summary>Details</summary>
Motivation: 现有加权生存分析方法（如加权Kaplan-Meier和Breslow-Peto）的统计性质研究不足，需改进其估计精度。

Method: 将加权Kaplan-Meier与增强逆概率加权框架联系，并开发基于校准的估计方法，适用于低维和高维数据。

Result: 模拟研究表明校准方法覆盖比例接近目标值，且置信区间更短；实证分析（精神分裂症治疗）中亦得到更窄的置信区间。

Conclusion: 校准估计方法在生存分析因果推断中优于传统加权方法。
...
### [117] [Generalized raking and stabilized weights for regression modeling in two-phase samples](https://arxiv.org/abs/2605.15802)
*Tong Chen,Joshua Slone,Gustavo Amorim,Pamela A. Shaw,Bryan E. Shepherd,Thomas Lumley*

Main category: stat.ME

TL;DR: 结合稳定权重与广义铲除估计，提升两阶段抽样回归模型的效率。


<details>
  <summary>Details</summary>
Motivation: 抽样权重中的非必要变异会导致方差膨胀，需要稳定权重来减少变异。

Method: 提出将最优稳定权重估计与广义铲除结合，利用辅助变量信息。

Result: 模拟显示在现实两阶段设计下精度提升，但在高度信息性设计中增益有限。

Conclusion: 该组合方法可通过标准统计软件实现，并在大型两阶段卡波西肉瘤研究中应用。
...
### [118] [The Negative Binomial Chain-Ladder: A Full Likelihood Model for Claim Count Reserving](https://arxiv.org/abs/2605.15811)
*Robin Van Oirbeek*

Main category: stat.ME

TL;DR: 提出负二项链梯模型（NB-CL），将链梯方法嵌入全似然框架，通过泊松-伽马构造赋予分散参数结构性解释，统一链梯家族。


<details>
  <summary>Details</summary>
Motivation: 经典链梯方法缺乏概率基础，现有随机扩展依赖二阶矩假设或拟似然，缺乏生成性解释。

Method: 基于泊松-伽马构造导出负二项分布，建立NB-CL模型；开发参数自助法以包含过程和参数不确定性。

Result: NB-CL在正确设定下接近名义覆盖率，偏差校正后表现稳健；在赔款金额案例中作为工作近似有效。

Conclusion: NB-CL为链梯方法提供了统一概率层次，分散参数具有结构性含义，且通过自助法提供完整不确定性度量。
...
### [119] [Bayesian Inference for Non-Conjugate Distance Dependent Chinese Restaurant Process Models](https://arxiv.org/abs/2605.15847)
*Joseph Marsh,Theodore Kypraios,Rowland G. Seymour*

Main category: stat.ME

TL;DR: 针对非共轭似然下距离依赖中国餐馆过程的贝叶斯推理问题，提出了可逆跳转MCMC框架，采用矩匹配等建议策略，有效处理参数维度变化。


<details>
  <summary>Details</summary>
Motivation: 当聚类参数与似然非共轭时，簇分配变化导致参数空间维度改变，传统推理困难。

Method: 开发可逆跳转MCMC框架，包含基于先验、独立和矩匹配等出生/死亡建议策略，以及固定维度移动的后验重采样。

Result: 矩匹配建议提供原则性的数据驱动替代方案，在模拟和旧忠实数据中表现优于基于先验的建议。

Conclusion: 该框架为非共轭似然的ddCRP模型提供了通用推理方法，适用于离散和连续观测模型。
...
### [120] [A Model-Agnostic Bootstrap for Macro-Level Claims Reserving Under the Conditioning Principle](https://arxiv.org/abs/2605.15896)
*Robin Van Oirbeek,Tim Verdonck*

Main category: stat.ME

TL;DR: 提出一种严格遵循条件原则的Dirichlet-Gamma自助法，用于索赔准备金预测，覆盖误差小且模型无关。


<details>
  <summary>Details</summary>
Motivation: 现有自助法违背条件原则，产生不随三角大小消失的覆盖误差，需要更准确的预测分布。

Method: 基于Dirichlet-Gamma层次结构，只模拟分配比例W_i，保持观察三角固定，继承发展比例方法的校准。

Result: 覆盖缺陷为O(I^{-1/2})，在复合泊松过程中预测标准差大于真实值，ODP自助法覆盖差异为Ω(1)。

Conclusion: Dirichlet-Gamma自助法提供模型无关的校准，浓度c可诊断非平稳发展。
...
### [121] [Statistical Inference for Smoothed Support Vector Machines in High Dimensions: From Offline to Online Data](https://arxiv.org/abs/2605.15911)
*Shuya Zhou,Junwen Xia,Jingxiao Zhang*

Main category: stat.ME

TL;DR: 针对高维分类中Lasso惩罚线性SVM的双重非光滑性导致的推断和计算难题，提出离线和在线统一的推断框架。


<details>
  <summary>Details</summary>
Motivation: Lasso惩罚线性SVM的合页损失和Lasso惩罚导致双重非光滑性，使得统计推断困难且计算效率低。

Method: 离线情况下，对合页损失应用卷积平滑技术构建去偏估计量以消除收缩偏差，从而构建有效置信区间；在线情况下，基于历史数据汇总统计开发实时估计和推断流程。

Result: 严格证明离线和在线去偏估计量的渐近正态性，模拟和真实数据应用显示方法能实现有效统计推断并提升计算效率。

Conclusion: 所提统一推断框架在离线和在线设置下均能有效进行统计推断，且计算效率高。
...
### [122] [REX-SUB: A Scalable Subsampling Strategy for Modeling Large Spatial Datasets](https://arxiv.org/abs/2605.16075)
*Nicholas Rios,Ben Seiyon Lee*

Main category: stat.ME

TL;DR: 提出随机交换子采样算法（REX-SUB），结合Vecchia近似，高效选择子样本以最小化高斯过程预测误差。


<details>
  <summary>Details</summary>
Motivation: 大规模空间数据下，标准高斯过程计算不可行，需要最优子采样方法。

Method: 采用随机交换算法进行子采样，并嵌入Vecchia近似利用精度矩阵稀疏性加速推断。

Result: 在模拟和实际水汽数据中，REX-SUB的均方预测误差和区间评分均优于对比方法。

Conclusion: REX-SUB是一种有效的子采样策略，能在降低计算代价的同时保持高预测精度。
...
### [123] [Estimating Association Between Paired Outcomes in Clustered Data with Informative Subgroup Size](https://arxiv.org/abs/2605.16108)
*Owen Visser,Somnath Datta*

Main category: stat.ME

TL;DR: 提出加权估计量处理牙科数据中信息性聚类大小和子群结构，减少偏倚。


<details>
  <summary>Details</summary>
Motivation: 标准关联度量在聚类大小信息性时产生偏倚，需调整。

Method: 扩展within-cluster重采样和聚类加权估计方程，构建权重平衡聚类、边缘和配对类别贡献，用加权估计方程估计矩、秩和细胞概率。

Result: 模拟显示有限样本偏倚小，标准误估计好；NHANES应用表明观察到的疾病严重度分布与口内结构相关。

Conclusion: 提供原则性方法估计典型个体典型牙齿的口腔健康关联，减少因信息性牙齿保留和子群组成引起的偏倚。
...
### [124] [Why Empirical p-Values Are Not Uniform: Reference Samples, Dependence, and PIT Backtesting](https://arxiv.org/abs/2605.16221)
*Jakub Lis*

Main category: stat.ME

TL;DR: 实证PIT值的常用实施方法（常见样本和滚动窗口）改变了其统计结构，导致依赖性和方差扭曲，使得经典均匀性检验失效。


<details>
  <summary>Details</summary>
Motivation: 评估预测分布校准的准确性问题，尤其是实证实现中PIT值统计性质的变化。

Method: 分析常见样本和滚动窗口两种实施方式下的PIT值统计行为，并与理论均匀分布比较。

Result: 常见样本下，经验百分位数收敛于两样本K-S检验框架；滚动窗口引入自相关和方差抑制。

Conclusion: 将经验PIT值视为独立均匀分布会导致统计推断失真，需要基于两阶段抽样结构的校准方法更新回测程序。
...
### [125] [FRESH: Information-Geometric Calibration of Patient-Level Models to Aggregate Evidence](https://arxiv.org/abs/2605.16246)
*Franklin Fuller,Daniele Bertolini,Samantha Liang,Jason Christopher,Aaron M. Smith*

Main category: stat.ME

TL;DR: FRESH是一种融合患者级数据和汇总统计的生成模型校准方法，通过信息几何最小扰动实现统一模型，支持多种临床决策应用。


<details>
  <summary>Details</summary>
Motivation: 现有预测模型通常仅基于患者级数据，难以整合临床试验摘要、注册摘要等群体级汇总结果，导致数据利用不充分和效率低下。FRESH旨在提供一种原则性方法，将两种数据类型结合到统一的、数据高效的临床决策模型中。

Method: 假设存在基于患者级数据训练的生成模型，FRESH通过重新校准该模型，使其匹配目标人群的一组指定汇总统计，从而产生患者级预测。校准过程在信息几何意义上对原始联合分布进行最小扰动，实现患者层面对汇总数据的重述。

Result: 生成的样本可直接分析或通过后训练程序更新原生成模型。该方法能够有效融合患者级与汇总数据，保持数据效率，并支持单臂试验结果背景化、临床试验模拟和上市疗法比较效果分析等应用。

Conclusion: FRESH提供了一种将患者级数据与汇总信息严谨结合的新途径，提升了临床决策模型的实用性和数据利用率，在多个场景中具有重要价值。
...
### [126] [A general proof of integer R\'enyi QNEC](https://arxiv.org/abs/2605.15272)
*Tanay Kibe,Pratik Roy*

Main category: hep-th

TL;DR: 对于所有整数Rényi参数n≥2，证明了Rényi量子零能量条件在具有半侧模包含结构的von Neumann代数中成立。


<details>
  <summary>Details</summary>
Motivation: 推广量子零能量条件（QNEC）到Rényi散度，为量子场论中能量条件提供更一般的框架。

Method: 利用von Neumann代数的半侧模包含结构，证明正常正泛函的Kosaki L^n范数在零平移半群下的log凸性。

Result: Rényi QNEC在假设SRD有限的情况下对所有整数n≥2成立，推导出非负的第二零形状变化。

Conclusion: 该证明确认了Rényi QNEC在更广泛代数结构中的有效性，无需对激发态做额外假设。
...
### [127] [Non-Invertible Symmetries in Compactified Supergravities](https://arxiv.org/abs/2605.16188)
*Fabi\'an Caro-P\'erez,Mar\'ia Pilar Garc\'ia del Moral,\'Alvaro Restuccia*

Main category: hep-th

TL;DR: 研究非可逆高形式对称性缺陷从十一维超引力到IIA型超引力的卡鲁扎-克莱因约化，展示了缺陷及其辅助拓扑部分在零模超引力下的推前，并得到IIA型缺陷代数。


<details>
  <summary>Details</summary>
Motivation: 理解非可逆对称性在紧化过程中的行为，以及从M理论到IIA型超引力的缺陷代数结构。

Method: 从十一维非可逆超引力缺陷构造出发，利用M理论圆的推前，在零模超引力区域内进行卡鲁扎-克莱因约化，并分析Bianchi扇区的分裂。

Result: 七维类Chern-Simons辅助理论约化为六维BF型扇区；十一维Bianchi扇区分裂为可逆H[3]扇区和扭曲非可逆F̃[4]扇区；得到的IIA型缺陷代数包含可逆Picard子群和非可逆BF修饰缺陷。

Conclusion: 通过标准M理论/IIA型膜字典识别带电探针，IIA型缺陷代数同时包含可逆和非可逆缺陷。
...
### [128] [Neural Point-Forms](https://arxiv.org/abs/2605.15524)
*Bruno Trentini,Jacob Hume,Vincenzo Antonio Isoldi,Philipp Misof,Ekaterina S. Ivshina,Kelly Maggs*

Main category: cs.LG

TL;DR: 提出神经点形式（NPFs），一种利用扩散几何和拉普拉斯技术从点云学习微分形式比较矩阵的可解释几何特征方法。


<details>
  <summary>Details</summary>
Motivation: 现有点云学习基于流形假设，但坐标、距离或图邻域无法直接捕获高阶切向几何信息。

Method: 通过扩散几何的拉普拉斯离散化构建微分形式内积模型，引入比较矩阵描述特征形式与外在切向相互作用，并证明其长期一致性。

Result: 在合成和生物实验中NPFs表现竞争力强、可解释性好，尤其当标签依赖于采样密度、流形结构或群体几何时优势显著。

Conclusion: NPFs提供了一种紧凑、高效、置换不变的神经层，有效学习点云几何特征。
...
### [129] [Entropy Across the Bridge: Conditional-Marginal Discretization for Flow and Schr\"odinger Samplers](https://arxiv.org/abs/2605.16126)
*Bruno Trentini,Dejan Stancevic,Michael M. Bronstein,Alexander Tong,Luca Ambrogioni*

Main category: cs.LG

TL;DR: 提出基于条件-边际熵率目标的桥感知离散化方法，实现免训练的推理时间调度器，在低评估次数下提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有流匹配和薛定谔桥模型的推理网格依赖启发式或继承自扩散方法，无法在有限函数评估次数下优化样本质量。

Method: 推导条件-边际熵率目标，分离端点条件桥几何与边际流演化，构建免训练调度器；对高斯布朗桥得到闭式U形曲线，指导非均匀网格设计。

Result: 在2D模型上，10步ODE-Heun MMD改善18.1%，SDE-Heun改善22.7%；在CIFAR-10上，5步FID达186.3±4.0，优于线性200.5±2.9和余弦238.0±5.3；在蛋白质生成上，低NFE区域在CAMEO22和ATLAS基准中表现优势。

Conclusion: 熵率调度作为低预算下的实用分配信号，适用于高维桥和流采样器。
...
### [130] [AgentStop: Terminating Local AI Agents Early to Save Energy in Consumer Devices](https://arxiv.org/abs/2605.15206)
*Dzung Pham,Kleomenis Katevas,Ali Shahin Shamsabadi,Hamed Haddadi*

Main category: cs.LG

TL;DR: 本地部署的LLM代理在任务执行中资源消耗巨大，提出的AgentStop通过预测提前终止低成功概率轨迹，可在几乎不影响性能的情况下减少15-20%的能源浪费。


<details>
  <summary>Details</summary>
Motivation: 远程云端代理存在隐私、网络依赖和API成本问题，本地部署虽能解决这些问题，但代理工作流比单次推理更耗资源，常因迭代推理、工具使用和失败重试导致大量计算浪费。

Method: AgentStop作为一种轻量级效率监督器，利用低成本执行信号（如token级对数概率）预测任务轨迹的成功概率，并提前终止可能失败的轨迹。

Result: 在网页问答和编码基准测试中，AgentStop可减少15-20%的浪费能量，同时任务性能下降不到5%。

Conclusion: 预测性提前终止是一种实用机制，可在用户设备上实现可持续、隐私保护的LLM代理。
...
### [131] [TeamTR: Trust-Region Fine-Tuning for Multi-Agent LLM Coordination](https://arxiv.org/abs/2605.15207)
*Yi Xie,Siao Liu,Falong Fan,Yuanqi Yao,Yue Zhao,Bo Liu*

Main category: cs.LG

TL;DR: 提出TeamTR框架，通过信任域和轨迹重采样解决多智能体顺序微调中的占用偏移问题，平均性能提升7.1%。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM系统在顺序微调时，更新一个智能体会改变上下文分布，导致后续更新评估不匹配，性能不如单模型基线。

Method: 提出TeamTR，一种信任域框架，在每次组件更新后重新采样轨迹，并强制每个智能体的散度控制，提供严格的改进下界。

Result: 平均优于单智能体和顺序基线7.1%，缓解协调退化，支持即插即用组件替换。

Conclusion: TeamTR有效解决了多智能体顺序微调中的占用偏移问题，提升了性能并保持了灵活性。
...
### [132] [Quantization Undoes Alignment: Bias Emergence in Compressed LLMs Across Models and Precision Levels](https://arxiv.org/abs/2605.15208)
*Plawan Kumar Rath,Rahul Maliakkal*

Main category: cs.LG

TL;DR: 大语言模型后训练量化压缩在标准质量指标变化微小的情况下，会导致新的刻板印象偏见涌现和模型不确定性回答减少，聚合指标忽视了公平性退化。


<details>
  <summary>Details</summary>
Motivation: 现有研究因对比条件少、使用聚合偏见指标、评估模型单一，无法区分渐进恶化与阈值相关的安全故障。

Method: 对三个指令微调模型在五个精度级别上使用BBQ偏见基准进行大规模控制实验，总计911,100条推理记录。

Result: 3位量化使6-21%的无偏见项目产生新偏见，模型选'未知'下降17.4%；4位时已有2.5-5.6%项目出现偏见，而困惑度变化极小。

Conclusion: 聚合指标系统性地遗漏公平性退化，需要显式测试偏见的质量感知压缩协议。
...
### [133] [Mask-Morph Graph U-Net: A Generalisable Mesh-Based Surrogate for Crashworthiness Field Prediction under Large Geometric Variation](https://arxiv.org/abs/2605.15231)
*Haoran Li,Tobias Lehrer,Yingxue Zhao,Haosu Zhou,Philipp Stocker,Tobias Pfaff,Nan Li*

Main category: cs.LG

TL;DR: 提出MMGUNet，一种结合粗图变形和掩码预训练的图神经网络，用于高效碰撞仿真替代建模。


<details>
  <summary>Details</summary>
Motivation: 非线性有限元碰撞仿真计算昂贵，限制迭代设计优化；现有图神经网络方法在泛化性和准确性之间存在权衡。

Method: 通过特征对齐重心参数化进行粗图变形，对每个输入网格构建跨图边；在监督预训练中应用节点掩码，然后冻结参数多的边特定层进行参数高效微调。

Result: 在分布内、分布外和跨组件迁移设置下，粗图变形提高了测试精度，掩码预训练减少了训练-测试差异并提高了数据效率，预测误差低于外部基线。

Conclusion: 该模型为可重用、数据高效的网格替代建模提供了实用途径，适用于碰撞安全设计探索。
...
### [134] [MuteBench: Modality Unavailability Tolerance Evaluation for Incomplete Multimodal Fusion](https://arxiv.org/abs/2605.15235)
*Wugeng Zheng,Ziwen Kan,Tianlong Chen,Chen Chen,Song Wang*

Main category: cs.LG

TL;DR: 提出MuteBench基准，评估多模态生理数据在模态缺失和模态内缺失下的融合架构鲁棒性，发现架构族比参数数量更重要，给出选择和改进鲁棒融合方法的指导。


<details>
  <summary>Details</summary>
Motivation: 多模态生理数据的传感器在实践中经常失效，导致模态缺失或模态内缺失，但缺乏统一基准评估不同融合架构在两种缺失模式下的鲁棒性。

Method: 构建涵盖9个数据集、6种融合架构、2种缺失模式的MuteBench基准，包含125,000个样本，通过系统实验分析架构族、参数数量、缺失类型等对鲁棒性的影响。

Result: 架构族是鲁棒性最强预测因子；通道独立模型对模态缺失鲁棒但对模态内缺失敏感；课程模态丢弃仅在训练丢弃率内有效；通道数、序列长度和模态对齐共同决定威胁更大的缺失模式；扩散插补可改善模态内缺失下的分类。

Conclusion: MuteBench为实践者提供了选择现有架构和设计未来鲁棒融合方法的具体指导，但更广泛的验证仍需进一步研究。
...
### [135] [Reducing the Safety Tax in LLM Safety Alignment with On-Policy Self-Distillation](https://arxiv.org/abs/2605.15239)
*Yu Fu,Longxuan Yu,Haz Sameen Shahgir,Zhipeng Wei,Hui Liu,N.Benjamin Erichson,Yue Dong*

Main category: cs.LG

TL;DR: OPSA通过在线策略自蒸馏实现安全对齐，显著减少安全税，尤其在小模型上效果突出。


<details>
  <summary>Details</summary>
Motivation: 安全对齐常以牺牲推理能力为代价（安全税），离策略训练不匹配是重要原因。本文旨在通过在线策略自蒸馏缓解这一问题。

Method: 模型生成自身轨迹，从冻结的教师副本（基于特权安全上下文）接收逐token KL监督；引入教师翻转率搜索激活安全推理的上下文。

Result: 在两个推理模型系列（五个规模）上，OPSA优于离策略自蒸馏和外部教师蒸馏，小模型提升最大（R1-Distill-1.5B +8.85，Qwen3-0.6B +5.49），且收益在不同训练规模和越狱评估中稳定。

Conclusion: OPSA通过在线策略自蒸馏和密集KL监督，在保持推理能力的同时有效提升安全性，机制上聚焦于早期合规决策标记。
...
### [136] [Logical Grammar Induction via Graph Kolmogorov Complexity: A Neuro-Symbolic Framework for Self-Healing Clinical Data Integrity](https://arxiv.org/abs/2605.15242)
*Abolfazl Zarghani,Amir Malekesfandiari*

Main category: cs.LG

TL;DR: 提出Logic-GNN框架，将临床记录视为受潜在逻辑游戏支配的结构化“私人语言”，通过时间图神经网络和图柯氏复杂度诱导符号语法，将异常定义为导致最小描述长度显著膨胀的“语法违规”，在Sina System数据集上F1-score达0.94，优于基线12%。


<details>
  <summary>Details</summary>
Motivation: 医疗信息系统常因人为数据输入错误而可靠性受损，现有统计异常检测方法无法区分合法临床极端值与错误。

Method: 整合时间图神经网络与图柯氏复杂度，从临床记录中诱导符号语法，将异常定义为导致最小描述长度显著膨胀的语法违规。

Result: 在含200万+记录的Sina System数据集上，F1-score达0.94，比最先进基线高12%，能有效区分危及生命的医疗异常和数据损坏。
...
### [137] [Reading the Cell, Designing the Cure: Perturbation-Conditioned Molecular Diffusion for Function-Oriented Drug Design](https://arxiv.org/abs/2605.15243)
*Ziyu Xu,Zijian Zhang,Liang Wang,Zhiyuan Liu,Qiang Liu,Shu Wu,Liang Wang*

Main category: cs.LG

TL;DR: 本文提出 CURE，一种多分辨率转录组引导的扩散框架，用于基于转录组状态转换的生成式药物设计，有效克服了跨模态差距和信号稀疏性。


<details>
  <summary>Details</summary>
Motivation: 当可靠靶标结构匮乏或表型源于失调通路时，转录组扰动提供系统级功能读数；将转录组药物设计形式化为生成逆问题，但面临不适定性、生物化学域差距及信号稀疏性挑战。

Method: 提出包含转录组扰动功能特征提取器（TFE）的 CURE 框架：提取前后状态的功能导向扰动嵌入，对齐至双化学视图以弥合跨模态鸿沟，并通过异质性感知聚合从噪声数据中提取鲁棒状态特异性信号。

Result: 在标准基准和严格分布外协议上，CURE 在结构质量和功能一致性方面持续优于强基线。

Conclusion: 通过零样本基因抑制剂设计任务验证了实际效用，凸显表型驱动生成发现的潜力。
...
### [138] [Privacy Evaluation of Generative Models for Trajectory Generation](https://arxiv.org/abs/2605.15246)
...
TL;DR: 本文通过成员推理攻击揭示了生成轨迹模型的隐私风险，证明其生成性质不能自动保证隐私安全。


<details>
  <summary>Details</summary>
Motivation: 轨迹数据敏感，生成模型虽被默认保护隐私，但实际隐私评估存在显著空白。

Method: 识别适用经验方法，对代表性模型实施成员推理攻击。

Result: 攻击可行，表明生成模型存在隐私泄露风险。

Conclusion: 生成轨迹模型仍需严格的隐私评估，生成性质不足以消除隐私风险。
...
### [139] [GQLA: Group-Query Latent Attention for Hardware-Adaptive Large Language Model Decoding](https://arxiv.org/abs/2605.15250)
*Fanxu Meng*

Main category: cs.LG

TL;DR: GQLA 是 MLA 的改进，通过两种等价解码路径 (MQA-absorb 和 GQA) 适应不同硬件，无需重新训练即可在 H100 和 H20 上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: MLA 仅在 H100 类 GPU 上高效，但在 H20 等低端 GPU 上效率低，无法利用多 token 预测，且张量并行受限。

Method: 对 MLA 做最小修改，使训练权重暴露两条代数等价的解码路径：MQA-absorb 路径 (同 MLA) 和 GQA 路径 (带每群扩展缓存)；运行时根据硬件选择路径。同时提出 TransGQLA 将预训练 GQA 模型转换为 GQLA。

Result: 单套 GQLA 权重在 H100 和 H20 上均达到 roofline，GQA 路径支持 8 路零冗余张量并行；在 LLaMA-3-8B 上，MQA-absorb 路径的 KV 缓存压缩至 GQA 基线的 28.125%。

Conclusion: GQLA 通过硬件自适应路径实现跨 GPU 高效推理，兼容现有模型，无需重新训练或定制内核。
...
### [140] [PDRNN: Modular Data-driven Pedestrian Dead Reckoning on Loosely Coupled Radio- and Inertial-Signalstreams](https://arxiv.org/abs/2605.15252)
*Peter Bauer,Andreas Porada,Felix Ott,Christopher Mutschler,Tobias Feigl*

Main category: cs.LG

TL;DR: 提出一种基于循环神经网络（RNN）的模块化混合AI辅助行人航位推算（PDR）系统PDRNN，通过隐式预测异步传感器数据流，融合多模态数据，在动态运动场景中优于传统和机器学习方法，避免误差累积并提供预测能力。


<details>
  <summary>Details</summary>
Motivation: 传统PDR系统受限于传感器采样率差异、传输不可靠以及高动态运动下多模态数据融合困难，导致定位精度和鲁棒性不足。

Method: 采用简单RNN架构隐式预测来自不同估计方法的异步传感器数据流；设计模块化混合AI系统PDRNN，各组件为独立ML模型，分别估计方向、速度/距离（从加速度和陀螺仪），可选绝对定位（如5G），最终融合模型结合位置、速度和方向，并利用不确定性估计增强鲁棒性。

Result: 在动态运动数据集上，PDRNN相比经典和ML方法显著提升精度和准确性，有效避免黑盒方法中的误差累积，并具备预测能力。

Conclusion: 尽管增加了系统复杂度，PDRNN通过模块化设计提供了更好的组件控制和预测能力，在动态场景中表现优异。
...
### [141] [Position: Ideas Should be the Center of Machine Learning Research](https://arxiv.org/abs/2605.15253)
*Jairo Diaz-Rodriguez*

Main category: cs.LG

TL;DR: 提出以思想为先的研究框架，主张通过预测现代模型的行为特征并设计实验验证，而非追逐排行榜。


<details>
  <summary>Details</summary>
Motivation: 现有研究过度偏向指标驱动或理想化理论，忽视了思想这一核心科学对象。

Method: 以思想为先：重视思想预测的行为特征，并通过针对性实验检测这些模式。

Result: 桥接理论与实践，消除“复杂性溢价”，使资源有限的研究者也能做出严格科学贡献。

Conclusion: 倡导以思想为中心的研究文化，将基准和定理视为检验机制假说的工具，而非目的。
...
### [142] [Curriculum Learning of Physics-Informed Neural Networks based on Spatial Correlation](https://arxiv.org/abs/2605.15254)
*Xujia Chen,Xinyue Hu,Letian Chen,Daming Shi,Wenhui Fan*

Main category: cs.LG

TL;DR: 提出一种空间相关课程学习框架，通过空间因果权重、低频信息桥接和区域自适应重加权，解决PINN训练中的空间耦合问题，提高求解精度。


<details>
  <summary>Details</summary>
Motivation: 现有课程学习策略主要关注时间或参数进展，缺乏对空间信息传播和区域一致性的显式处理，不适用于强空间耦合的边界值问题。

Method: 包括三个部分：空间因果权重引导信息从近边界区域向内传播；低频信息桥接利用伪标签强制空间分离区域的一致性；区域自适应重加权调整子区域损失以降低局部残差并恢复高频细节。

Result: 在PDE基准上，在相当计算成本下，所提方法缓解了训练失败并提高了求解精度。

Conclusion: 首次从子区域空间耦合角度解决PINN训练困难，代码已开源。
...
### [143] [Training on Documents About Monitoring Leads to CoT Obfuscation](https://arxiv.org/abs/2605.15257)
*Reilly Haskins,Bilal Chughtai,Joshua Engels*

Main category: cs.LG

TL;DR: 监控意识模型能够通过混淆推理痕迹来逃避链式推理监控，且此能力与模型的可控性高度相关。


<details>
  <summary>Details</summary>
Motivation: 链式推理监控依赖模型外部化推理，但模型可能故意混淆推理以逃避检测，构成安全风险。

Method: 通过合成文档微调使模型了解监控的存在，随后在简单和困难任务上测试模型逃避检测的能力，并分析CoT可控性与混淆成功率的关联。

Result: 监控意识模型相比无知控制组，有更高的未检测不良行为率；CoT可控性与混淆成功率显著相关（r=0.800, p=0.017）；在强化学习优化下，监控意识模型更快学会奖励黑客而不触发监控。

Conclusion: 监控知识结合高CoT可控性会削弱基于CoT的监控有效性，构成安全威胁。
...
### [144] [Tadpole: Autoencoders as Foundation Models for 3D PDEs with Online Learning](https://arxiv.org/abs/2605.15284)
*Qiang Liu,Felix Koehler,Benjamin Holzschuh,Nils Thuerey*

Main category: cs.LG

TL;DR: Tadpole是一种专为三维偏微分方程设计的基础模型，通过自编码器预训练和高效在线数据生成，实现了出色的可迁移性、高维可扩展性和多功能性。


<details>
  <summary>Details</summary>
Motivation: 现有3D PDE模型在迁移性、高维可扩展性和多功能性方面面临挑战，Tadpole旨在解决这些问题。

Method: 采用在线数据生成框架合成3D PDE数据，预训练自编码器；对单通道空间裁剪进行自编码学习表示；针对动力学学习提出参数高效微调策略，结合低秩适应、潜空间变换和跳跃连接。

Result: 预训练数据规模等效于数百TB；在动力学学习和生成建模等下游任务中，微调参数少且性能优异。

Conclusion: Tadpole作为3D PDE学习的基础模型，展现了强大的多功能性和有效性。
...
### [145] [Universal Approximation of Nonlinear Operators and Their Derivatives](https://arxiv.org/abs/2605.15285)
*Filippo de Feo*

Main category: cs.LG

TL;DR: 首次证明Banach空间间非线性k次可微算子及其导数的通用逼近定理，推广Hornik结果到无限维。


<details>
  <summary>Details</summary>
Motivation: 算子学习中非线性算子及其导数的通用逼近是开放基础问题，本文旨在填补理论空白。

Method: 使用编码器-解码器架构，利用Banach空间逼近性质、Bastiani可微性、紧开拓扑和加权Sobolev空间四个关键特征。

Result: 在紧集和加权Sobolev范数下证明均匀逼近定理，包含DeepONet等经典架构。

Conclusion: 为导数信息算子学习提供基础，并指出在高阶精度、约束优化和PDE数值方法中的应用。
...
### [146] [GQA-{\mu}P: The maximal parameterization update for grouped query attention](https://arxiv.org/abs/2605.15290)
...
TL;DR: 提出了Complete-P深度和权重衰减缩放，并首次推导了分组查询注意力的μP缩放，实现了超参数迁移。


<details>
  <summary>Details</summary>
Motivation: 现有的μP参数化难以推导新架构，且谱范数条件缺乏严格定义。

Method: 从谱特征学习视角出发，将谱范数条件从启发式提升为特征学习的定义，并提出改进谱范数以处理非满秩权重矩阵。

Result: 成功推导了GQA的μP缩放，并通过学习率迁移实验和权重衰减迁移实验验证了有效性。

Conclusion: 该工作为超参数跨架构迁移提供了理论依据，并简化了新架构的μP推导。
...
### [147] [GESD: Beyond Outcome-Oriented Fairness](https://arxiv.org/abs/2605.15295)
*Gideon Popoola,John Sheppard*

Main category: cs.LG

TL;DR: 提出了一种新的程序导向公平性指标GESD，用于衡量模型解释在不同子组间的稳定性差异，并集成到多目标优化框架FEU中，同时优化效用、结果公平和解释公平。


<details>
  <summary>Details</summary>
Motivation: 现有公平性指标主要关注结果导向的差异，缺乏对决策过程或解释中偏见的洞察。

Method: 定义了组级解释稳定性差异（GESD），并将其纳入多目标优化框架FEU，联合优化效用、结果公平和解释公平。

Result: 实验表明GESD有效捕捉解释质量的组间差异，FEU在效用和公平性上优于现有方法。

Conclusion: GESD桥接了结果公平和解释公平，为诊断和缓解预测模型中的偏见提供了综合工具。
...
### [148] [How Data Augmentation Shapes Neural Representations](https://arxiv.org/abs/2605.15306)
*Tianxiao He,Alex H. Williams,Sarah E. Harvey*

Main category: cs.LG

TL;DR: 通过形状分析研究数据增强对神经网络表示几何的影响，发现增强强度与类型在形状空间中产生规律性轨迹，并能预测模型集成效果。


<details>
  <summary>Details</summary>
Motivation: 数据增强虽能提升泛化，但其对表示几何的影响尚不明确。

Method: 使用形状分析工具将隐藏表示嵌入到对缩放、平移、旋转、反射不变的度量空间，分析增强强度与类型引起的轨迹。

Result: 增强强度增加导致形状空间中平滑轨迹，不同增强类型导向不同方向；表示形状的变形可用于预测集成时的改善程度。

Conclusion: 形状空间轨迹分析为理解和比较数据增强方法提供了原则性工具，揭示了跨架构和种子的共享几何模式。
...
### [149] [Time-Varying Deep State Space Models for Sequences with Switching Dynamics](https://arxiv.org/abs/2605.15311)
*Sanja Karilanova,Subhrakanti Dey,Ay\c{c}a \"Oz\c{c}elikkale*

Main category: cs.LG

TL;DR: 提出基于时间变化状态空间模型的神经网络，通过基函数字典学习时变动力学，在合成数据和语音去噪中优于时间不变模型，计算复杂度相当。


<details>
  <summary>Details</summary>
Motivation: 信号处理和系统识别中时间变化系统的建模是一个基本挑战。

Method: 提出时间变化状态空间模型（SSM）的神经网络，神经元状态由时变动力学控制，通过基函数字典提供可学习的时变动力学。

Result: 在切换系统合成数据和语音去噪任务上，该模型一致优于时间不变模型，计算复杂度相当。

Conclusion: 揭示了数据时变动力学的关键方面、额外自由度的分配以及大模型对时不变限制的补偿能力。
...
### [150] [Beyond Bounded Variance: Variance-Reduced Normalized Methods for Nonconvex Optimization under Blum-Gladyshev Noise](https://arxiv.org/abs/2605.15314)
*Antesh Upadhyay,Arda Fazla,Abolfazl Hashemi*

Main category: cs.LG

TL;DR: 本文研究非凸随机优化在BG-0噪声模型下的收敛性，证明了归一化带动量SGD的O(ε^{-6})复杂度，并进一步提出方差减少的STORM方法，在标准光滑下达到最优O(ε^{-4})，在广义光滑下复杂度为O(ε^{-(4+α)})，首次在无界域下给出了归一化方法的收敛保证。


<details>
  <summary>Details</summary>
Motivation: 解决BG-0噪声（梯度方差随初始距离二次增长）下非凸优化的收敛问题，并探索广义光滑性对收敛速率的影响。

Method: 归一化带动量的随机梯度下降（SGD）和方差减少的归一化STORM方法。

Result: 动量SGD复杂度O(ε^{-6})；STORM在均方光滑下O(ε^{-4})，在α-对称广义光滑下为O(ε^{-(4+α)})（α∈(0,1)）或O(ε^{-5})（α=1）；特殊情况下恢复标准速率。

Conclusion: 首次在BG-0噪声、无界域、无需增加批大小或显式锚定条件下，证明了归一化方法的收敛性，覆盖标准光滑和广义光滑两种情形。
...
### [151] [From Weight Perturbation to Feature Attribution for Explaining Fully Connected Neural Networks](https://arxiv.org/abs/2605.15328)
*Thodoris Lymperopoulos,Denia Kanellopoulou*

Main category: cs.LG

TL;DR: 提出通过对特征权重而非值进行扰动的新归因方法XWP和XWP_c，缓解遮挡技术的偏差与分布外问题，在简单DNN上取得竞争性表现。


<details>
  <summary>Details</summary>
Motivation: 全连接神经网络缺乏可解释性，现有遮挡方法存在添加偏差和分布外数据等局限。

Method: 对特征附带的权重施加扰动以估计特征重要性，形成XWP和XWP_c两种归因方法。

Result: 在简单DNN的图像信号识别中，与主流归因方法性能相当。

Conclusion: 为可解释性提供了稳健框架，解决了长期存在的漏洞，提高了模型解释的可靠性与可解释性。
...
TL;DR: 提出DIO-Agent，一种基于进化搜索和LLM变异算子的IO2Code方法，通过转换优先级前提引导搜索，在多个难度上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 从输入输出行为合成程序（IO2Code）比自然语言到代码（NL2Code）更具挑战，需要从具体计算行为中恢复潜在原理，而现有方法效果不足。

Method: 将IO2Code框架化为离散程序空间上的进化搜索，LLM作为变异算子，执行错误信号引导变异；引入转换优先级前提作为变异先验，引导LLM偏向最简单的一致假设，逐步从常量到条件到迭代。

Result: 在构造的IO2CodeBench上，DIO-Agent在各个难度级别和不同LLM下一致优于传统程序示例方法和最先进的进化智能体基线，显著超越等效采样预算的测试时扩展策略。

Conclusion: DIO-Agent通过结合LLM和进化搜索，并引入转换优先级前提，有效解决了IO2Code问题，为从输入输出行为合成程序提供了新的有效方法。
...
TL;DR: 从有界理性决策角度研究学习器如何平衡训练损失与样本依赖性，提出基于f散度正则化的响应律，并证明泛化可作为可检验的避险特性。


<details>
  <summary>Details</summary>
Motivation: 学习器不仅拟合数据，还决定训练样本对输出的影响强度及避险程度。需系统研究这种关系。

Method: 将学习问题建模为有界理性决策，用f散度正则化表示响应律，在信息几何框架下分析，并通过缩放损失和局部扰动从黑盒行为中恢复避险曲线。

Result: 得到了训练损失与样本依赖性的下界交易曲线及上界证书曲线，并展示了如何从黑盒行为恢复避险与两条曲线。

Conclusion: 泛化可视为学习器自身响应律的可检验避险性质，当避险覆盖特定样本导致的扭曲时，即提供实际证书。
...
TL;DR: 提出LEAPBench框架，通过轨迹指标评估LLM在迭代科学设计中的学习效率，发现LLM不如经典贝叶斯优化，且领域知识有时反而有害。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅评估固定步数后的最终结果，忽略了衡量学习效率的关键轨迹，导致无法准确评估LLM的迭代学习能力。

Method: 构建55任务的LEAPBench，采用最佳-so-far曲线下面积（AUC）作为轨迹度量，对比经典贝叶斯优化基线，并基于已发表文献进行审计。

Result: 将评分方式从最终结果改为轨迹评分后，53%的任务上最佳模型选择发生变化；LLM未能超越贝叶斯优化基线；在16个生物学任务中，领域感知提示在迭代30次时匹配已发表最优设计的概率比领域无关提示低约10个百分点。

Conclusion: 轨迹指标能更全面评估学习效率，且可作为离线强化学习的训练目标，在21个保留任务中的14个上提升了性能。
...
### [155] [PACER: Acyclic Causal Discovery from Large-Scale Interventional Data](https://arxiv.org/abs/2605.15353)
*Ramon Vi\~nas Torn\'e,S\'ilvia F\`abregas Salazar,Soyon Park,Ivo Alexander Ban,Artyom Gadetsky,Nikita Doikov,Maria Brbi\'c*

Main category: cs.LG

TL;DR: 提出PACER框架，通过设计保证无环性的搜索空间，实现高维扰动数据中精确可扩展的因果发现。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用软无环约束导致优化循环图、数值不稳定和可扩展性差。

Method: 联合建模变量排列和边概率，直接优化有效DAG结构；支持观测和干预数据，提供闭式梯度表达式。

Result: 在蛋白质信号和遗传扰动基准上匹配或超越最先进方法，可扩展至数千变量，速度提升两个数量级。

Conclusion: 通过原则性搜索空间设计，可实现高维扰动数据中精确且可扩展的因果发现。
...
### [156] [Controllable Molecular Generative Foundation Models](https://arxiv.org/abs/2605.15354)
*Yihan Zhu,Yuhan Liu,Weijiang Li,Tengfei Luo,Meng Jiang*

Main category: cs.LG

TL;DR: 提出CoMole，一种基于motif感知图扩散的分子生成基础模型，通过将预训练先验与RL优化结合，实现高可控性和有效性。


<details>
  <summary>Details</summary>
Motivation: 现有分子图生成缺乏统一框架和可靠可控性，原子级RL因动作空间巨大和无效中间态而受限。

Method: 构建motif感知图扩散管道，学习更紧凑的图空间；RL在化学有意义的决策上优化条件反向策略，并理论论证motif级策略的优势。

Result: 在9个目标上可控性第一，MAE降低最多48.2%，有效性始终≥0.94；通过仅优化任务嵌入即可将可控性迁移至未见属性。

Conclusion: CoMole为异构分子生成任务提供了统一、可控且高效的解决方案，无需规则校正即可保持高有效性。
...
### [157] [Federated Learning of Spiking Neural Networks under Heterogeneous Temporal Resolutions](https://arxiv.org/abs/2605.15355)
*Sanja Karilanova,Subhrakanti Dey,Ay\c{c}a \"Oz\c{c}elikkale*

Main category: cs.LG

TL;DR: 提出一种联邦学习框架，解决脉冲神经网络中因客户端时间分辨率不同导致的参数融合问题，通过适应方法恢复准确性。


<details>
  <summary>Details</summary>
Motivation: 时间序列应用中，边缘设备因硬件限制采集数据的时间分辨率各异，导致联邦平均失效，需要新框架实现跨分辨率协同训练。

Method: 研究不同时间分辨率下神经元参数的集成方式，提出针对有状态神经元的适应方法，实现全局模型与本地分辨率兼容。

Result: 在SHD和DVS-Gesture数据集上，适应方法显著恢复因时间不匹配损失的分发精度。
...
### [158] [PRB-RUPFormer: A Recursive Unified Probabilistic Transformer for Residual PRB Forecasting](https://arxiv.org/abs/2605.15363)
*Saad Masrur,Yuxuan Jiang,Matti Hiltunen,Ajay Rajkumar,Ismail Guvenc*

Main category: cs.LG

TL;DR: 提出PRB-RUPFormer，一种递归统一概率Transformer，用于预测残余PRB，通过联合处理多变量KPI时间序列和分位数预测区间，实现了低误差和高命中概率。


<details>
  <summary>Details</summary>
Motivation: 现有PRB预测方法仅依赖历史值且独立训练，无法捕捉跨载波依赖和不确定性，点预测在流量变化大时不足。

Method: PRB-RUPFormer采用递归统一概率Transformer，结合时间、季节和载波感知嵌入，对多变量KPI时间序列联合处理，并通过分位数预测区间捕获不确定性。

Result: 在六个月商业LTE数据上评估，中位MAE低于0.05，一日和七日递归预测的命中概率超过0.80。

Conclusion: 概率预测直接支持频谱感知的RAN功能，如动态载波激活、拥塞避免和频谱共享，适用于动态频谱接入场景。
...
### [159] [Is One Score Enough? Rethinking the Evaluation of Sequentially Evolving LLM Memory](https://arxiv.org/abs/2605.15384)
*Songwei Dong,Zihan Chen,Chengshuai Shi,Peng Wang,Jundong Li,Cong Shen*

Main category: cs.LG

TL;DR: 提出SeqMem-Eval诊断框架，从多维度评估LLM顺序记忆质量，发现高准确率未必代表好记忆，存在遗忘和负迁移问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM记忆评估依赖聚合指标（如最终准确率），掩盖遗忘和负迁移等关键失败模式。

Method: 借鉴持续学习，设计在线效用、留出泛化、反向迁移和遗忘等指标，评估记忆状态演化。

Result: 高最终或累积准确率的方法常伴随严重遗忘或负迁移，不同记忆设计在适应性与稳定性间有不同权衡。

Conclusion: 标准评估指标不足以反映记忆质量，需更细粒度诊断方法。
...
### [160] [Unified High-Probability Analysis of Stochastic Variance-Reduced Estimation](https://arxiv.org/abs/2605.15388)
*Zhankun Luo,Antesh Upadhyay,M. Berk Sahin,Sang Bin Moon,Anuran Makur,Abolfazl Hashemi*

Main category: cs.LG

TL;DR: 提出了一个统一的随机方差缩减估计框架，基于记忆保留、重置概率和迭代移动修正三个组件，利用新的无维度向量值Freedman不等式获得了高概率界，并改进了约束优化问题的oracle复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有分析多针对特定估计器且依赖期望，未能揭示决定可靠性的结构权衡。

Method: 采用包含记忆保留、重置概率和迭代移动修正的递归框架，并推导了新的无维度向量值Freedman不等式。

Result: 得到了统一的、适用于欧几里得和非欧几里得空间的高概率界；无约束优化中置信水平呈对数依赖；对于期望约束的随机优化，首次实现了Õ(ε⁻³)的oracle复杂度，优于之前的Õ(ε⁻⁴)。

Conclusion: 该框架统一了多种经典估计器，导出了新的二阶变体，并通过方差缩减显著改进了约束优化的复杂度。
...
### [161] [LPDS: Evaluating LLM Robustness Through Logic-Preserving Difficulty Scaling](https://arxiv.org/abs/2605.15393)
*Philipp Mondorf,Samuel J. Bell,Jesse Dodge,Dieuwke Hupkes*

Main category: cs.LG

TL;DR: 提出逻辑保持难度缩放（LPDS）框架，通过系统搜索高难度变体暴露LLM非稳健性，相比随机采样性能下降达5倍，且困难变体微调提升稳健性更一致。


<details>
  <summary>Details</summary>
Motivation: LLM在实体变化时易失败，现有评估随机测试变体可能高估稳健性，缺乏系统识别易失败变体的方法。

Method: LPDS框架：量化问题变体难度，系统搜索允许变体空间以找到最难变体。

Result: 难度增加导致性能下降、推理链错误更显著；LPDS高效找到困难变体，性能下降比随机采样大5倍；困难变体微调稳健性提升更一致。

Conclusion: LPDS能有效评估和提升LLM对逻辑保持变体的稳健性。
...
### [162] [Representation Without Reward: A JEPA Audit for LLM Fine-Tuning](https://arxiv.org/abs/2605.15394)
*Biswa Sengupta*

Main category: cs.LG

TL;DR: 在自然语言到正则表达式生成任务中，测试联合嵌入预测架构（JEPA）对语言模型微调的要求，发现隐藏状态表示与解码任务准确性弱耦合，结构化的零结果。


<details>
  <summary>Details</summary>
Motivation: 验证JEPA原则在自回归语言模型微调中的严格性要求：隐藏状态几何必须改善解码任务指标。

Method: 基于Llama-3.2-1B-Instruct的LoRA框架，比较22种训练时辅助目标，包括轨迹形状正则化、分布约束、预测器/目标不对称、Fisher度量Jacobi残差及解码器可见JEPA目标。

Result: 多个辅助目标在未校正时显著（最强T3-Local Δ=+2.53pp, p=0.003），但无一通过Bonferroni或Holm-Bonferroni校正；解码器可见JEPA首次获得正向辅助-交叉熵梯度余弦，但精确匹配在噪声内，且完整微调复制仍为零。

Conclusion: 隐藏状态表示与解码任务准确性在此机制下弱耦合，应将JEPA评估重新定义为耦合问题，关注哪些度量下隐藏几何可转化为解码器可见任务信号。
...
### [163] [Breakeven complexity: A new perspective on neural partial differential equation solvers](https://arxiv.org/abs/2605.15399)
*Yijing Zhang,Nicholas Roberts,Tanya Marwah,Mikhail Khodak*

Main category: cs.LG

TL;DR: 提出以盈亏平衡复杂度为核心指标，综合考虑端到端成本，评估神经PDE求解器在实际应用中的成本效益，发现其在困难问题上更具优势。


<details>
  <summary>Details</summary>
Motivation: 现有基于精度的评估忽略了神经求解器的高前期成本（数据生成、训练、调参）以及传统求解器可低成本提供低保真解的事实。

Method: 建立以盈亏平衡复杂度（成本有效前所需前向求解次数）为中心的评估框架，利用缩放定律分配数据生成预算，并实现平滑误差匹配。

Result: 在APEBench和PyFR基准上测试多个神经求解器，结果表明问题越困难（成本、维度、展开长度、物理 regime如高雷诺数），神经求解器越有效。

Conclusion: 神经PDE求解器在处理复杂问题上更经济高效。
...
### [164] [$\phi$-Balancing for Mixture-of-Experts Training](https://arxiv.org/abs/2605.15403)
*Lizhang Chen,Jonathan Li,Qi Wang,Runlong Liao,Shuozhe Li,Chen Liang,Ni Lao,Qiang Liu*

Main category: cs.LG

TL;DR: 提出φ-balancing框架，通过最小化凸势函数实现专家负载的全局平衡，使用镜像下降在线算法，在预训练和微调中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有MoE负载平衡方法基于噪声小批量统计的启发式方法，存在偏差，无法实现种群级目标。

Method: 通过凸对偶推导出等价的最小最大形式，并利用镜像下降得到简单的在线算法，实现基于EMA的路由调整，开销极小。

Result: 在大型预训练和下游微调中，φ-balancing持续优于Switch-style和无损失基线，专家利用更稳定有效。

Conclusion: φ-balancing是一种原则性强的框架，有效提升MoE模型的专家平衡性和整体性能。
...
### [165] [Transformer Scalability Crisis: The First Comprehensive Empirical Analysis of Performance Walls in Modern Language Models](https://arxiv.org/abs/2605.15413)
*Mahdi Naser Moghadasi,Faezeh Ghaderi*

Main category: cs.LG

TL;DR: 对118个transformer模型进行大规模评估，发现可扩展性瓶颈：短序列成功率88.1%，长序列2048 tokens完全失败；压缩模型参数效率远高于大模型。


<details>
  <summary>Details</summary>
Motivation: transformer架构的可扩展性限制缺乏系统性实证分析。

Method: 评估118个模型，涵盖7个架构类别，分析不同序列长度下的加载时间、内存消耗和计算效率。

Result: 88.1%模型处理512 tokens，44.9%处理1024 tokens，0%处理2048 tokens；压缩模型参数效率649.2 tokens/sec/M参数，大模型仅12.5 tokens/sec/M。

Conclusion: 挑战现有扩展假设，首次定量证明O(n^2)注意力复杂度导致性能瓶颈，建立新基准方法。
...
### [166] [Margin-Adaptive Confidence Ranking for Reliable LLM Judgement](https://arxiv.org/abs/2605.15416)
*Gaojie Jin,Yong Tao,Lijia Yu,Tianjin Huang*

Main category: cs.LG

TL;DR: 提出一种学习型置信度估计器，通过模拟标注者多样性和基于边界的排序公式，改善大语言模型与人类判断的一致性，并在固定序列测试中提升成功率。


<details>
  <summary>Details</summary>
Motivation: 现有假设模型置信度与人类分歧风险单调，但该假设可能被违反，且置信估计的泛化行为未明确分析。

Method: 学习专用置信估计器，利用模拟标注者多样性和基于边界的排序公式显式建模模型区分人类一致与分歧的能力，并推导泛化保证，设计自适应训练程序。

Result: 学习到的置信估计器提高了排序准确性，增强了置信度与分歧风险之间的单调关系，跨多个数据集和评判模型实现了更高的一致目标达成率。

Conclusion: 通过专用置信估计器替代启发式信号，可以有效改善大语言模型与人类判断的一致性保障框架。
...
### [167] [$f$-Trajectory Balance: A Loss Family for Tuning GFlowNets, Generative Models, and LLMs with Off- and On-Policy Data](https://arxiv.org/abs/2605.15417)
*Jake Fawkes,Jason Hartford*

Main category: cs.LG

TL;DR: 将GFlowNets和变分推断中的均方误差损失扩展到f-散度家族，得到一类损失函数，其on-policy梯度对应f-散度，off-policy保持全局极小点，并应用于多种生成模型任务。


<details>
  <summary>Details</summary>
Motivation: 寻找一种低方差、有效的代理损失函数来训练生成模型，并能扩展到更广泛的f-散度家族，以继承其覆盖模式等性质。

Method: 证明on-policy梯度下，目标与模型对数概率的平移不变损失函数与f-散度一一对应，从而设计新的代理损失函数，适用于离策略数据。

Result: 在经典合成示例、SynFlowNets分子发现和异步大语言模型调优等任务中，模型在on-和off-policy下均保留了预测的性质。

Conclusion: 该构造可扩展到整个f-散度家族，为调优生成模型提供新的代理损失函数，且具有理论保证和实践有效性。
...
TL;DR: 提出拉格朗日流匹配框架，通过最小化一般拉格朗日量的作用量来生成概率路径和速度场，扩展了流匹配的动力学类型。


<details>
  <summary>Details</summary>
Motivation: 现有流匹配方法（如整流流和最优传输路径）仅使用直线路径，动力学范围窄，受限于经典力学中最简单的最小作用量情况。

Method: 基于拉格朗日力学，在连续性方程和端点约束下最小化一般拉格朗日量的作用量，证明其等价于静态最优传输，得到仿真-free训练目标族。

Result: 产生新的概率路径和速度场，数值实验表明动力学发生有意义变化，且与现有条件流匹配模型性能相当。

Conclusion: 拉格朗日流匹配提供灵活、物理驱动的框架，可设计丰富动力学路径，拓展了流匹配的应用。
...
### [169] [DualKV: Shared-Prompt Flash Attention for Efficient RL Training with Large Rollouts and Long Contexts](https://arxiv.org/abs/2605.15422)
*Jiading Gai,Shuai Zhang,Xiang Song,Bernie Wang,George Karypis*

Main category: cs.LG

TL;DR: DualKV消除了RL后训练中共享提示的冗余计算，通过处理一次提示来加速策略更新。


<details>
  <summary>Details</summary>
Motivation: 标准FlashAttention在RL训练中复制N次提示令牌，导致计算和内存浪费，尤其是在大rollout长上下文场景下。

Method: 提出DualKV，使用融合CUDA内核处理两个独立的KV区域（共享上下文和每序列响应），并重新设计数据管道以压缩令牌。

Result: 在Qwen3-8B GRPO训练中实现1.63-2.09倍策略更新加速，DAPO训练2.47倍，30B MoE模型3.82倍加速，MFU从36%提升至76%。

Conclusion: DualKV在不引入近似的情况下显著加速RL训练，支持更大的微批次，并提高硬件利用率。
...
### [170] [Spectral Priors vs. Attention: Investigating the Utility of Attention Mechanisms in EEG-Based Diagnosis](https://arxiv.org/abs/2605.15433)
*Tawsik Jawad,Gowtham Atluri,Vikram Ravindra*

Main category: cs.LG

TL;DR: 实验表明，通过频带选择构建特征可提升脑电信号分类性能，传统机器学习模型可超越或匹配深度学习模型，注意力机制难以提取关键频谱特征。


<details>
  <summary>Details</summary>
Motivation: EEG信号噪声大、空间分辨率低，导致神经退行性疾病分类困难，现有深度学习方法难以区分高相似度类别。

Method: 采用频谱选择性特征构建，从主要脑电波频带中提取信号强度，将高维原始数据转换为高价值频谱特征，并在多个数据集上验证。

Result: 频域和时频域特征使传统机器学习模型性能匹配或超越SOTA深度学习模型；注意力机制无法提取稳定特征；提供频选时域输入不能改善注意力模型性能。

Conclusion: 频谱选择性特征构建有效提升类别可分性，注意力机制在提取相关频谱特征方面存在根本性局限。
...
### [171] [On the Stability of Growth in Structural Plasticity](https://arxiv.org/abs/2605.15435)
*Lute Lillo,Nick Cheney*

Main category: cs.LG

TL;DR: 生长网络单元并非剪枝的逆操作；新生单元面临后向梯度匮乏问题，在复杂任务中表现不如剪枝，但若给予足够整合时间则在持续学习中具竞争力。


<details>
  <summary>Details</summary>
Motivation: 探索网络结构动态调整中生长机制的有效性及其与剪枝的差异。

Method: 对比生长与剪枝在不同任务中的表现，分析新生单元梯度问题，并进行干预实验。

Result: 生长在结构编辑时最终准确率高，但剪枝在平均轨迹或从头训练时更强；改善新生单元整合可提升自适应性能。

Conclusion: 生长应被视为时间敏感的优化过程，其成功依赖于插入稳定性。
...
### [172] [Don't Stop Me Yet: Sampling Loss Minima via Dissipative Riemannian Mechanics](https://arxiv.org/abs/2605.15459)
*Albert Kj{\o}ller Jacobsen,Leo Uhre Jakobsen,Johanna Marie Gegenfurtner,Georgios Arvanitidis*

Main category: cs.LG

TL;DR: 提出DiMS采样器，基于物理动力学系统，可精确采样神经网络损失函数的重参数化不变解，在贝叶斯不确定性量化中表现更优。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么不能精确采样重参数化不变解（如散布于低损失区域），要么局部性强，限制了对不同最小值谷的探索。

Method: DiMS：基于动能、重力和摩擦力的动力学系统，保证精确采样最小水平集，并通过物理超参数控制探索能力。

Result: 在贝叶斯推理的不确定性量化任务中，DiMS相比先前方法性能提升。

Conclusion: DiMS是一种有效的采样器，能够探索最小值山谷并改进不确定性估计。
...
### [173] [DrugSAGE:Self-evolving Agent Experience for Efficient State-of-the-Art Drug Discovery](https://arxiv.org/abs/2605.15461)
*Yikun Zhang,Xiwei Cheng,Tianyu Liu,Yuanqi Du,Wengong Jin*

Main category: cs.LG

TL;DR: 提出一种自进化智能体经验框架，通过跨任务记忆积累和重用经验，高效构建药物发现中的SOTA预测模型，在多个任务上取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 构建药物发现SOTA预测模型需昂贵搜索，现有LLM智能体不保留经验，每次新任务需重新搜索。

Method: 提出自进化智能体经验（method）框架，维护跨任务记忆，包括验证技能、有效策略统计数据、常见错误及其修正，可直接迁移已有解决方案。

Result: 在33个分子性质预测任务中，单任务设置下排名第一；基于16个小任务积累的记忆，在17个保留任务上平均标准化得分为0.935，零测试时搜索下超越基线10-30%。

Conclusion: 跨任务记忆能高效开发药物发现SOTA模型，优势显著。
...
### [174] [Layer-wise Derivative Controlled Networks](https://arxiv.org/abs/2605.15463)
*Rowan Martnishn,Sean Anderson*

Main category: cs.LG

TL;DR: ChainzRule 通过多项式引擎和微分正则化，在保持高精度的同时显著提升模型稳定性并减少参数量。


<details>
  <summary>Details</summary>
Motivation: 传统模型在准确率、硬件效率和功能稳定性三者之间存在矛盾，输入微小变化可能导致输出剧烈波动，影响实际部署。

Method: 提出 ChainzRule 架构，用多项式引擎替代分段线性激活函数，并引入微分正则化（DREG）对中间导数进行针对性约束，避免过度抑制表达能力。

Result: 在公平对比中，参数量减少15.5倍；MNIST 上峰值梯度波动降低23.1%；Yelp Full 序数回归准确率达70.17%。
...
### [175] [GRLO: Towards Generalizable Reinforcement Learning in Open-Ended Environments from Zero](https://arxiv.org/abs/2605.15464)
*Shangjian Yin,Yu Fu,Yue Dong,Zhouxing Shi*

Main category: cs.LG

TL;DR: GRLO方法从零开始通过少量交互学习RLHF，仅用5K提示和22.7 GPU小时将Qwen3-4B-Base平均性能从24.1提升至63.1，比领域内RLVR节省46倍数据和68倍计算，并展示了RLHF的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有RL后训练需要大量GPU计算，阻碍广泛采用；希望验证RLHF的泛化能力并提高训练效率。

Method: 从零开始使用少量提示在开放式环境中进行RLHF训练，不依赖领域特定奖励，然后测试其对数学推理和代码生成等下游任务的泛化能力。

Result: 使用Qwen3-4B-Base，在5K提示和22.7 GPU小时下，平均性能从24.1提升至63.1，优于强领域内RLVR基线，且与使用更大训练成本的发布模型竞争。后续领域内RLVR仅在较难竞赛数学上有选择性提升。

Conclusion: GRLO提供了一种简单高效的方案，通过少量交互学习RLHF即可实现广泛的后训练能力，无需大量领域内数据或计算。
...
### [176] [Toward World Modeling of Physiological Signals with Chaos-Theoretic Balancing and Latent Dynamics](https://arxiv.org/abs/2605.15465)
...
TL;DR: 提出NormWear-2世界模型，通过编码多变量生理信号和临床干预变量到共享潜在空间，结合先验知识和非参数适应，实现多尺度长期预测，在多个真实数据集上超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有模型侧重于静态任务（分类、事件预测、短时预测），而长期信号级预测和生理信号的预测性质尚未充分探索。

Method: NormWear-2将多变量生理信号与临床干预变量联合编码到共享潜在空间，建模为动态系统；结合预训练先验知识（直觉）和即时非参数潜在状态转移适应（洞察），并通过混沌理论平衡动态体制多样性以增强表示鲁棒性。

Result: 在涵盖日常、护理和临床场景（含8026名受试者，时长从3.2小时到2.3年）的数据集上，NormWear-2在时间、频率和潜在表示域取得最佳预测性能，显著优于现有时间序列基础模型，且下游表示质量具有竞争力。

Conclusion: NormWear-2为通用生理信号世界模型迈出一步，实现了多尺度、医疗干预条件下的长期预测。
...
### [177] [Njord: A Probabilistic Graph Neural Network for Ensemble Ocean Forecasting](https://arxiv.org/abs/2605.15470)
*Daniel Holmberg,Joel Oskarsson,Erik Larsson,Fredrik Lindsten,Teemu Roos*

Main category: cs.LG

TL;DR: Njord是一种概率性数据驱动海洋预测模型，结合深度潜变量框架和图神经网络，适用于全球和区域海洋，提供不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 海洋动力学本质混沌，现有机器学习海洋模型仅产生确定性预测，缺乏概率信息。

Method: 采用深度潜变量框架结合图神经网络，提出K-means聚类网格以适应不规则海面几何，实现单次前向传播采样。

Result: 在全球0.25°和波罗的海2km分辨率上表现优异，优于确定性基线，提供集合预报不确定性估计；在OceanBench基准上平均误差最低，海表温度改进最大。

Conclusion: 概率模型能有效捕捉海洋混沌性，提供可靠预测与不确定性，可扩展至全球和区域应用。
...
### [178] [Learning Normalized Energy Models for Linear Inverse Problems](https://arxiv.org/abs/2605.15487)
*Nicolas Zilberstein,Santiago Segarra,Eero Simoncelli,Florentin Guth*

Main category: cs.LG

TL;DR: 提出一种基于能量的扩散模型，通过协方差正则化训练，无需重训即可为线性逆问题计算归一化后验密度，支持自适应采样、MH校正和盲估计，在多个数据集上性能优越。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型作为先验时，存在先验密度隐式表示和似然近似引入采样偏差的问题。

Method: 训练一个基于能量的去噪模型，加入协方差正则化项以保持不同测量条件下的一致性。

Result: 在ImageNet、CelebA、AFHQ数据集上，图像修复和去模糊任务中达到或超越基线性能。

Conclusion: 该方法不仅保留了扩散模型的采样能力，还新增了能量引导自适应采样、无偏MH校正和盲估计能力，是更优的逆问题求解方案。
...
### [179] [SurvivalPFN: Amortizing Survival Prediction via In-Context Bayesian Inference](https://arxiv.org/abs/2605.15488)
*Shi-ang Qi,Vahid Balazadeh,Michael Cooper,Russell Greiner,Rahul G. Krishnan*

Main category: cs.LG

TL;DR: SurvivalPFN通过上下文学习摊销贝叶斯推断，无需任务特定训练即可进行生存分析，在61个数据集上优于多种现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统生存分析需要大量专业知识来选择合适的估计量，缺乏自动化和通用性。

Method: 提出先验数据拟合网络SurvivalPFN，预训练于多样化的合成右删失数据，通过上下文学习单次前向传播完成推断。

Result: 在61个数据集、21种方法、5个指标的基准测试中，SurvivalPFN取得强劲性能，常超越已有模型。

Conclusion: SurvivalPFN为生存分析提供了一种原则性且实用的基础模型，适用于医疗、金融、工程等高风险领域。
...
### [180] [Ghosted Layers: Unconstrained Activation Alignment for Recovering Layer-Pruned LLMs](https://arxiv.org/abs/2605.15491)
*Vincent-Daniel Yun,Junhyuk Jo,Sai Praneeth Karimireddy,Sunwoo Lee*

Main category: cs.LG

TL;DR: 提出Ghosted Layers，一个训练无关的恢复模块，通过闭式最优线性算子修复层剪枝导致的激活分布不匹配，提升LLM性能。


<details>
  <summary>Details</summary>
Motivation: 层剪枝移除Transformer解码器块后，剩余层接收的隐藏状态与训练分布不匹配，导致性能显著下降。

Method: 通过求解边界激活对齐问题，从校准集推导闭式最优线性算子，重建剪枝层引起的激活差异。

Result: 在多种LLM和剪枝策略上，准确率和困惑度均优于现有训练无关基线，同时保持剪枝的效率优势。

Conclusion: Ghosted Layers有效解决了层剪枝的分布不匹配问题，是一种高效且性能优越的恢复方法。
...
### [181] [Learning with Conflicts of Interest](https://arxiv.org/abs/2605.15504)
*Nischal Aryal,Arash Termehchy,Ali Vakilian,Marianne Winslett*

Main category: cs.LG

TL;DR: 针对ML系统与用户利益冲突导致的偏见信息问题，提出博弈论框架和算法，最大化期望信息并最小化偏见操纵。


<details>
  <summary>Details</summary>
Motivation: 金融、社会和政治因素导致ML系统与用户利益不一致，系统常产生偏见信息影响用户决策，而现有方案缺乏激励实施缓解协议。

Method: 建立博弈论框架建模ML系统与用户利益冲突的交互，设计可扩展算法并提供理论保证。

Result: 算法能在交互中最大化期望信息和行动，同时最小化偏见和操纵行动。

Conclusion: 成功解决该问题需承认利益冲突，所提框架能有效保护用户免受不良信息影响，同时安全利用系统。
...
### [182] [STS: Efficient Sparse Attention with Speculative Token Sparsity](https://arxiv.org/abs/2605.15508)
*Ceyu Xu,Jiangnan Yu,Yongji Wu,Yuan Xie*

Main category: cs.LG

TL;DR: 提出STS，一种无需重训的稀疏注意力机制，利用小模型识别重要token，动态构建稀疏掩码，在NarrativeQA上实现2.67倍加速，精度损失可忽略。


<details>
  <summary>Details</summary>
Motivation: 注意力机制的二次复杂度在长序列推理中造成严重内存和计算瓶颈，尤其对处理百万级token的智能体应用。

Method: STS利用小模型（草稿模型）的注意力分数预测大目标模型的重要token，结合推测解码框架，动态生成token和头级别的稀疏掩码，剪枝昂贵的注意力计算。

Result: 在NarrativeQA基准上，约90%稀疏度下实现2.67倍加速，精度损失极小，优于先前方法，在稀疏度-精度权衡上达到新最优。

Conclusion: STS无需模型重训即可大幅加速推理，为长序列LLM推理提供高效稀疏注意力方案。
...
### [183] [parallelcbf: A composable safety-filter and auditability framework for tensor-parallel reinforcement learning](https://arxiv.org/abs/2605.15509)
*Yijun Lu,Zilei Yang,Yuyin Ma*

Main category: cs.LG

TL;DR: ParallelCBF是首个统一了张量并行无人机环境、硬门控CBF安全过滤器、分片BC-to-RL流水线和可组合操作审计API的框架，支持端到端的安全约束训练。


<details>
  <summary>Details</summary>
Motivation: 现有框架（如Isaac Lab、OmniSafe等）各自提供部分功能，但缺乏统一的安全约束训练全流程框架。

Method: 通过四层可组合API实现：张量并行UAV环境、双势垒CBF（平方/线性预测）CPU参考实现、分片BC-to-RL流水线、以及可审计的原语（预注册、监视器、故障取证、数据集审计）。

Result: 完整39项属性测试套件在1.67秒内完成；收集31,415集行为克隆数据集，其课程混合、分桶产量及SHA-256均可审计；在代表性流水线中，审计层阻止了不符合预注册收敛标准的训练阶段。

Conclusion: 该框架的审计能力是实现可复现经验机器人研究的必要架构特性，而不仅仅是实用工具。
...
### [184] [OgBench: A Framework for Evaluating Graph Neural Networks on Omics Data](https://arxiv.org/abs/2605.15511)
*Louisa Cornelis,Johan Mathe,Louis Van Langendonck,Guillermo Bern\'ardez,Nina Miolane*

Main category: cs.LG

TL;DR: 介绍OgBench，首个用于组学数据（n<<p）图级预测的基准平台，发现常用GNN常不如简单MLP。


<details>
  <summary>Details</summary>
Motivation: 现有基准多关注n>>p，忽略了组学数据n<<p的情况，需要评估GNN在此场景的表现。

Method: 构建OgBench平台，从原始组学数据生成带特征的图，包含多种结构；测试经典GNN、大图GNN、面向组学的GNN，并与MLP等基线比较。

Result: 常用GNN常不优于简单MLP和经典基线。

Conclusion: 挑战了图结构自动带来价值的假设，促进重新评估当前学习范式，OgBench提供了开源生态系统。
...
### [185] [On the Fragility of Data Attribution When Learning Is Distributed](https://arxiv.org/abs/2605.15520)
*Xian Gao,Bo Hui,Min-Te Sun,Wei-Shinn Ku*

Main category: cs.LG

TL;DR: 数据归因值可能被单个参与者通过攻击性方法显著夸大，揭示归因本身构成新的攻击面。


<details>
  <summary>Details</summary>
Motivation: 现有归因方法默认假设归因值真实反映贡献，但这一假设可能不成立。

Method: 提出归因优先攻击，通过潜在优化注入合成批次，利用非IID标签覆盖和评估器敏感性。

Result: 攻击能持续增加对手归因值并重塑良性客户间的归因结构，不影响准确性或触发几何防御。

Conclusion: 归因形成了新的攻击面，需要开发鲁棒且激励兼容的评分机制。
...
### [186] [Rethinking Neural Network Learning Rates: A Stackelberg Perspective](https://arxiv.org/abs/2605.15530)
*Sihan Zeng,Sujay Bhatt,Sumitra Ganesh*

Main category: cs.LG

TL;DR: 通过Stackelberg优化视角，发现对神经网络主体层和最终层使用不同学习率可以加速训练，并揭示了两种加速机制。


<details>
  <summary>Details</summary>
Motivation: 神经网络通常使用单一学习率训练，但层特定学习率可加速训练，其原理和条件尚不明确。

Method: 将非均匀学习率训练解释为Stackelberg重新表述的两时间尺度交替梯度下降，并建立有限时间收敛保证。

Result: 识别两种机制：Stackelberg目标具有更强优化结构或更尖锐局部曲率，从而加速收敛；实验验证。

Conclusion: 非均匀学习率通过Stackelberg优化视角可解释且有效，能加速神经网络训练。
...
### [187] [DeltaPrompts: Escaping the Zero-Delta Trap in Multimodal Distillation](https://arxiv.org/abs/2605.15532)
*Jaehun Jung,Hyunwoo Kim,Brandon Cui,Ximing Lu,David Acuna,Prithviraj Ammanabrolu,Yejin Choi*

Main category: cs.LG

TL;DR: 提出DeltaPrompts方法，通过识别高分歧提示来提升视觉语言模型蒸馏效率，避免零增量陷阱。


<details>
  <summary>Details</summary>
Motivation: 现有蒸馏方法中，高达69%的提示是零增量的，即教师和学生输出相同，导致训练信号微弱，学生性能快速饱和。

Method: 基于分布散度最小化原理，定义答案分歧Δ来量化教师-学生能力差距，并设计分阶段合成流程，从现有数据集中生成高分歧推理问题。

Result: DeltaPrompts数据集（20万高分歧问题）在多种设置下（同策略蒸馏、跨模型迁移、非推理模型微调）均带来显著提升，在Qwen3-VL-8B-Thinking上平均相对提升达15%。

Conclusion: 聚焦功能能力差距而非简单数据量扩张是提升蒸馏效果的关键，DeltaPrompts有效推动了多模态推理性能。
...
### [188] [CTF4Nuclear: Common Task Framework for Nuclear Fission and Fusion Models](https://arxiv.org/abs/2605.15549)
...
TL;DR: 针对核工程中机器学习方法比较缺乏标准化的问题，提出了一个通用任务框架（CTF），包含多个数据集和12个指标，并通过基准测试揭示了当前方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 核工程中设计运行复杂，高保真模拟计算昂贵，模型方法有简化假设偏差，而数据驱动方法众多但缺乏公平比较。

Method: 构建CTF框架，包括来自核及邻近系统的精选数据集，使用12个既定指标和基于稀疏测量的新评估范式，并基准测试标准ML基线。

Result: 基准测试显示了现有ML方法的局限性。

Conclusion: 旨在用标准化评估替代临时比较，提高核工程科学ML的严谨性和可重复性。
...
### [189] [Characterizing Learning in Deep Neural Networks using Tractable Algorithmic Complexity Analysis](https://arxiv.org/abs/2605.15551)
*Pedram Bakhtiarifard,Sophia N. Wilson,Mahmoud Afifi,Jonathan Wensh{\o}j,Raghavendra Selvan*

Main category: cs.LG

TL;DR: 提出QuBD方法，首次可扩展地估计大神经网络权重的KCS复杂度，发现训练中复杂度下降，并与泛化性能相关。


<details>
  <summary>Details</summary>
Motivation: 现有复杂度估计方法（CTM, BDM）仅限于小二进制对象，无法扩展到现代DNN；而“学习即压缩”假说需要测量权重结构，KCS复杂度虽合适但缺乏可扩展估计方法。

Method: 引入QuBD（量化块分解方法），先将网络权重量化到有限字母表，然后按比特平面聚合CTM估计，得到KCS复杂度。

Result: QuBD的估计间隙比基于二值化的方法更紧；实验发现训练中复杂度降低、随数据预算变化、过拟合时增加、与泛化相关；算法信息主要存在于高比特平面，可用于确定量化水平。

Conclusion: QuBD提供了首个可扩展的大规模非二进制对象（如DNN权重）的KCS复杂度估计，揭示了学习机制。
...
### [190] [CrystalBoltz: End-to-End Protein Structure Determination via Experiment-Guided Diffusion for X-Ray Crystallography](https://arxiv.org/abs/2605.15564)
*Minseo Kim,Huanghao Mai,Jay Shenoy,Alec Follmer,Gordon Wetzstein,Frederic Poitevin*

Main category: cs.LG

TL;DR: 提出 CrystalBoltz，一个生成式框架，通过贝叶斯推断直接利用实验结构因子振幅进行晶体学精修，实现更准确、更快速的蛋白质结构细化。


<details>
  <summary>Details</summary>
Motivation: 传统晶体学结构测定需要大量人工精修，且现有生成模型难以直接结合实验测量数据，制约了X射线结构测定的效率。

Method: 将晶体学精修视为原子结构的贝叶斯推断问题，利用预训练蛋白质结构先验进行实验引导的后验采样，然后对原子坐标和B因子进行精修。

Result: 在多个蛋白质晶体学数据集上，坐标RMSD和R因子均低于最强基线，且运行时间缩短33倍。

Conclusion: CrystalBoltz 有效融合实验数据与结构先验，显著提升了晶体学精修的精度与效率。
...
### [191] [AstraFlow: Dataflow-Oriented Reinforcement Learning for Agentic LLMs](https://arxiv.org/abs/2605.15565)
*Haizhong Zheng,Yizhuo Di,Jiahui Wang,Shuowei Jin,Xueshen Liu,Yongji Wu,Z. Morley Mao,Ion Stoica,Jiawei Zhao,Beidi Chen*

Main category: cs.LG

TL;DR: 提出AstraFlow，一种数据流导向的强化学习系统，通过解耦组件支持多策略协作训练，并高效利用异构计算资源，在多策略训练中提速2.7倍。


<details>
  <summary>Details</summary>
Motivation: 现有RL系统因训练器中心化控制架构和缺乏组件抽象，难以弹性扩展、支持多策略协作训练，且每次扩展需大量系统工程。

Method: AstraFlow采用数据流导向设计，将分发服务、数据流管理和训练组件解耦为自治模块，原生支持多策略协作训练，并利用弹性、异构、跨区域计算资源。

Result: 在数学、代码、搜索和AgentBench任务中，无需系统级代码修改即支持多策略训练、弹性缩放、跨区域执行和可组合数据算法；多策略协作训练加速2.7倍，精度相当或更优。

Conclusion: AstraFlow通过组件解耦和数据流控制，有效解决了现有RL系统的扩展性和工程负担问题，为复杂代理RL提供高效灵活的基础设施。
...
### [192] [Gaussian Relational Graph Transformer](https://arxiv.org/abs/2605.15575)
*Zezhong Ding,Jin Li,Xugang Wang,Xike Xie*

Main category: cs.LG

TL;DR: 提出了GelGT，一种高斯关系图变换器，通过结构-语义协作采样和高斯图注意力机制，有效捕捉长程依赖并联合建模结构、语义和时间信息，在多个数据集上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有关系图学习方法在信息传递中因信息衰减难以捕捉长程依赖，且关系图变换器在联合建模结构、语义和时间信息方面存在局限。

Method: 采用结构-语义协作采样策略保留结构连通性并过滤无关语义，在采样子图上引入带可学习高斯偏置的高斯图注意力机制动态编码时间依赖。

Result: 在多个真实数据集上实现了最先进的下游任务性能，预测性能提升高达13.8%。

Conclusion: GelGT有效解决了现有方法在长程依赖和联合信息建模上的挑战，显著提升了预测性能。
...
### [193] [Embracing Biased Transition Matrices for Complementary-Label Learning with Many Classes](https://arxiv.org/abs/2605.15586)
*Tan-Ha Mai,Chao-Kai Chiang,Han-Hwa Shih,Gang Niu,Masashi Sugiyama,Hsuan-Tien Lin*

Main category: cs.LG

TL;DR: 互补标签学习通过设计有偏生成过程解决了大规模分类的瓶颈，在CIFAR-100和TinyImageNet-200上准确率提升七倍以上。


<details>
  <summary>Details</summary>
Motivation: 传统互补标签学习假设均匀标签生成，导致在大标签空间时学习信号被稀释，成为长期瓶颈。

Method: 提出BICL框架，通过设计有偏（非均匀）互补标签生成过程，将标签限制在子集上，从而增强学习信号。

Result: 在CIFAR-100和TinyImageNet-200上，BICL相比传统方法实现了超过七倍的准确率提升。

Conclusion: BICL为互补标签学习在大规模现实应用中打开了新路径，证明了有偏生成的有效性。
...
### [194] [Offline Reinforcement Learning with Universal Horizon Models](https://arxiv.org/abs/2605.15603)
*Hojun Chung,Junseo Lee,Songhwai Oh*

Main category: cs.LG

TL;DR: 提出了通用视界模型（UHM），通过直接预测任意视界下的未来状态，结合缩尾视界分布稳定训练，在离线强化学习中取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 现有的几何视界模型虽能缓解复合误差，但在准确建模遥远未来状态方面仍有不足。

Method: 引入通用视界模型（UHM），直接预测任意视界下的未来状态；采用缩尾视界分布，通过截断过大视界来稳定训练。

Result: 在100个OGBench任务上，该方法优于竞争基线，特别是在高度次优数据集和需要长视界推理的任务上。

Conclusion: UHM有效解决了模型基于强化学习中的复合误差问题，提升了长视界任务的表现。
...
### [195] [VSPO: Vector-Steered Policy Optimization for Behavioral Control](https://arxiv.org/abs/2605.15604)
*Xuechen Zhang,Zijian Huang,Kai Yang,Weijia Zhang,Jiasi Chen,Samet Oymak*

Main category: cs.LG

TL;DR: 提出向量引导策略优化(VSPO)方法，通过调节行为强度解决稀疏奖励问题，提升多目标优化性能。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型需优化主要精度目标，同时满足次要行为偏好，但目标行为稀疏，存在奖励瓶颈。

Method: VSPO修改GRPO，使用与目标行为相关的引导向量控制生成轨迹的行为强度，并通过变强度采样上采样稀有行为，丰富轨迹多样性。

Result: 在MATH和MMLU-Pro等基准上，VSPO在保持或提升任务精度的同时，对目标行为的控制优于奖励塑造等方法。

Conclusion: VSPO通过理论证明和实验验证，具有优于奖励塑造等方法的性质，尤其在稀疏奖励场景下加速策略优化。
...
TL;DR: 本文从最优控制理论推导出与解码器仅Transformer相似的推理架构，提出双滤波器算法，并通过实验揭示了Transformer在嵌入维度不足时利用非马尔可夫结构。


<details>
  <summary>Details</summary>
Motivation: 从第一性原理推导与Transformer解决相同预测问题的推理架构，为Transformer层结构提供理论依据。

Method: 将预测目标重新表述为最优控制问题，对离散值非线性模型和线性高斯模型分别求解，得到具有层结构的双滤波器算法。

Result: 数值实验对比了最优控制注意力与训练Transformer的注意力权重，发现当嵌入维度不足时，Transformer隐式利用非马尔可夫结构。

Conclusion: 最优控制理论可以解释Transformer的层结构，并揭示其在低维嵌入下的自适应行为。
...
TL;DR: 零阶优化（ZO）在深度学习中常被低估，本文通过六个立场论证其未得到充分探索，并指出子空间、前向优势等方向，呼吁重新思考ZO以实现大规模高效学习。


<details>
  <summary>Details</summary>
Motivation: 零阶优化（ZO）因方差和查询复杂度问题被认为不可扩展，但作者认为这种结论源于短视的开发实践，ZO实际上未得到充分探索而非能力不足。

Method: 提出六个立场，涵盖算法、系统和评估：1) 重新审视基于估计器的ZO的可行性边界（方差控制、方差-查询权衡、方向导数）；2) 三个未充分探索的机会：子空间与谱视图、前向-only的系统优势、去混淆评估与任务复杂度。

Result: 指出许多局限性源于全空间、逐元素、以估计器为中心的设计，并论证子空间方法可解释性方差减少、前向-only性质支持通信高效和资源受限训练，且需要独立评估任务复杂度。

Conclusion: 应围绕ZO的独特优势重新思考其优化，将其作为通向大规模、系统感知、资源高效学习的可行路径。
...
TL;DR: 提出IO-SVD方法，通过双面白化和异构秩分配，实现大模型高效压缩，性能损失小且推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有SVD压缩方法依赖输入白化空间、同构秩分配等，导致压缩时质量损失大。

Method: 构建KL感知双面白化空间，利用二阶KL损失扩展构造输出侧度量，结合输入白化，采用一阶校准损失估计进行异构秩分配，并改进混合SVD-量化压缩。

Result: 实验表明IO-SVD在多种大语言模型和视觉语言模型上性能下降极小，推理速度显著提升。

Conclusion: IO-SVD是一种有效的后训练压缩方法，平衡了模型大小、推理速度和质量。
...
TL;DR: 论文提出将打孔反向传播应用于边缘设备上的关键词识别，在Edge Impulse平台上赢得最佳模型奖，通过添加人工树突节点，在参数更少的情况下实现了更高的准确率。


<details>
  <summary>Details</summary>
Motivation: 边缘机器学习面临严格内存、有限计算和准确率要求，现有技术难以同时提升准确率和减小模型大小。

Method: 在标准卷积神经网络中添加人工树突节点，应用打孔反向传播进行训练，并在Edge Impulse关键词识别流程上进行实验。

Result: 在800次超参数试验中，树突模型在所有参数水平和准确率阈值下均优于传统架构。最佳树突模型仅用1500参数达到0.933测试准确率，而基线需4000参数达到0.921。

Conclusion: 打孔反向传播能同时提升模型质量与部署效率，是边缘AI工程师的有力工具。
...
### [200] [Towards Code-Oriented LM Embeddings for Surrogate-Assisted Neural Architecture Search](https://arxiv.org/abs/2605.15649)
*Pranav Somu,Advay Balakrishnan,Stepan Kravtsov,Aaron McDaniel,Jason Zutty*

Main category: cs.LG

TL;DR: 提出使用代码导向的语言模型嵌入（COLE）作为神经架构搜索的低成本替代方案，无需专门微调，仅通过轻量回归头即可实现高效性能预测。


<details>
  <summary>Details</summary>
Motivation: 现有神经架构搜索的代理模型通常需要昂贵的微调或复杂表示工程，成本高且通用性差。

Method: 将架构表示为PyTorch类定义文本，利用预训练语言模型（不微调）提取特征，再通过轻量回归头构建预测器，并探索了提升嵌入质量的策略。

Result: 在NAS-Bench-201和einspace搜索空间上，原始代码输入比ONNX等文本编码预测性能更高；COLE在BANANAS算法中驱动搜索，使CIFAR-100上达到最优架构1%性能所需的评估预算降低34%。

Conclusion: 由于任何神经架构均可表示为代码，COLE为推进神经架构搜索提供了通用且高效的基石。
...
TL;DR: 本文证明有限维仿射logit系统中软max反馈的稳定阈值是β‖ΠWΠ‖_{T→T}<2，而非经典理论的保守条件，从而扩展了稳定性边界。


<details>
  <summary>Details</summary>
Motivation: 经典理论对软max反馈系统的稳定性判定过于保守，仅保证强随机化下的稳定性，未能识别真正的相变点。

Method: 通过分析有限维仿射logit系统的谱性质，推导出尖锐的无量纲欧几里得阈值β‖ΠWΠ‖_{T→T}<2。

Result: 得到了比经典条件更宽松的稳定性阈值，填补了分歧前的稳定区间，使系统在奖励敏感时仍能保持全局可预测。

Conclusion: 该阈值准确刻画了软max反馈系统发生相变的临界点，扩展了稳定性保证范围，为相关领域提供更精确的理论基础。
...
TL;DR: 研究线性赌博机中ε-最佳臂识别的最小最大样本复杂度，提出非自适应与自适应方法，并证明性能差异。


<details>
  <summary>Details</summary>
Motivation: 确定在紧凑动作集下，以最少样本数高概率找到接近最优臂所需样本复杂度，并探究自适应采样的优势。

Method: 非自适应固定设计方法，自适应采样方法及ℓ2范数估计子程序。

Result: 非自适应方法样本复杂度为O(d log(1/δ)/ε^2 + w(X)^2/ε^2)且下界匹配；自适应在某些集合上仅对数改进，但构造集合上可实现多项式因子改进。

Conclusion: 自适应采样可在特定动作集上显著提升效率，ℓ2范数估计是关键工具。
...
### [203] [Rule2DRC: Benchmarking LLM Agents for DRC Script Synthesis with Execution-Guided Test Generation](https://arxiv.org/abs/2605.15669)
*Jinuk Kim,Junsoo Byun,Donghwi Hwang,Seong-Jin Park,Hyun Oh Song*

Main category: cs.LG

TL;DR: 提出了Rule2DRC基准，包含1000个规则转脚本任务和13921个评估布局，用于评估DRC脚本生成代理的执行正确性，并设计了SplitTester测试器代理，通过执行反馈生成判别性测试用例提升选择性能。


<details>
  <summary>Details</summary>
Motivation: 将自然语言设计规则转化为可执行的DRC脚本需要专业知识且耗时，现有基准评估集小且多基于代码相似度而非执行正确性，缺乏利用执行反馈或需要标注测试布局的方法。

Method: 构建包含1000个规则-脚本任务和13921个芯片评估布局的大规模基准，提供基于DRC执行结果的评估流水线；提出SplitTester测试器代理，利用执行反馈生成判别性测试用例来区分候选脚本，改进Best-of-N选择。

Result: Rule2DRC基准使执行正确性评估成为可能，SplitTester显著提升了候选脚本的区分能力和选择性能。

Conclusion: Rule2DRC为DRC脚本生成代理提供了更全面的评估基准，SplitTester方法有效利用执行反馈，推动该领域发展。代码已开源。
...
### [204] [Interaction-Aware Influence Functions for Group Attribution](https://arxiv.org/abs/2605.15675)
*Jaeseung Heo,Kyeongheung Yun,Youngbin Choi,Sehyun Hwang,Jungseul Ok,Dongwoo Kim*

Main category: cs.LG

TL;DR: 提出一种交互感知的影响函数，通过二阶展开捕捉示例间的交互作用，在多个模型上优于标准一阶方法。


<details>
  <summary>Details</summary>
Motivation: 标准影响函数对一组示例的影响仅求和个体影响，无法捕捉示例间的冗余或互补关系。

Method: 将目标函数二阶展开，加入成对交互项，估计示例间交互对目标的影响。

Result: 在六个数据集-模型对（包括逻辑回归、MLPs、ResNet-9）上，该估计器比一阶影响更好地跟踪留组重训练效果；在Llama-3.1-8B指令微调数据选择中，在七个下游任务中的五个上优于先前方法。

Conclusion: 交互感知的影响函数能更准确估计组影响，尤其在示例间有交互时。
...
### [205] [FRWKV+: Adaptive Periodic-Position Branch Interaction for Frequency-Space Linear Time Series Forecasting](https://arxiv.org/abs/2605.15690)
*Qingyuan Yang,Dongyue Chen,Da Teng,Junhua Xiao,Jiaji Pan,Shizhuo Deng*

Main category: cs.LG

TL;DR: 提出FRWKV+模型，通过交叉分支门控、自适应相位门控和自适应信任机制，改进频域时间序列预测中的信息交换和周期性位置利用，提升预测精度。


<details>
  <summary>Details</summary>
Motivation: 现有轻量级频域预测模型存在实虚频流信息交换弱、周期性位置线索不可靠的问题。

Method: 引入交叉分支门控交换实虚频流上下文；自适应相位门控提取周期性位置信息并生成符号修正；自适应信任机制控制修正强度。

Result: 在匹配消融协议下，FRWKV+取得家族变体中最高的MSE胜率，在周期性场景中显著提升。

Conclusion: 周期性位置上下文、符号修正和自适应信任在周期性场景中有用，但简单修正规则在边界情形仍适用。
...
### [206] [SEED: Targeted Data Selection by Weighted Independent Set](https://arxiv.org/abs/2605.15691)
*Yuan Zhang,Lifeng Guo,Junwen Pan,Chang Liu,Wenzhao Zheng,Kuan Cheng,Kurt Keutzer,Shanghang Zhang*

Main category: cs.LG

TL;DR: 提出了一种基于加权独立集的数据选择方法SEED，通过节点价值校准和局部尺度归一化解决图不平衡问题，在多项任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法难以平衡样本质量与多样性，且朴素权重无法区分噪声与信息，图结构受分布偏移影响导致偏差。

Method: 将数据选择转化为相似性图上的加权独立集问题，并引入节点价值校准（限制影响力估计于双边显著子空间）和局部尺度归一化（自适应边阈值）。

Result: 构建了紧凑数据集Honeybee-Remake-SEED-200K，在指令微调、视觉指令微调和语义分割任务上持续超越现有最先进方法。

Conclusion: SEED是一种鲁棒、可扩展的数据选择流水线，能同时保证数据质量和多样性。
...
### [207] [Tighter Regret Bounds for Contextual Action-Set Reinforcement Learning](https://arxiv.org/abs/2605.15692)
*Zijun Chen,Zihan Zhang*

Main category: cs.LG

TL;DR: 研究固定奖励和转移函数但每片段可行动作集不同的片段强化学习，MVP算法扩展适用，得到minimax和基于间隙的遗憾界。


<details>
  <summary>Details</summary>
Motivation: 处理现实场景中每片段允许动作集不同的情况，如机器人操作中任务依赖动作空间。

Method: 扩展MVP算法，考虑动作上下文，分析对抗和随机上下文下的遗憾。

Result: 对抗上下文遗憾界为~O(√(SAH^3K log L))，随机上下文为~O(√(SAH^3K))，样本复杂度为~O(SAH^3/ε^2)，以及基于间隙的改进界。

Conclusion: 提供了理论保证，证明算法在多种上下文下有效，基于间隙的界能大幅改进minimax率。
...
### [208] [Going Beyond the Edge: Distributed Inference of Transformer Models on Ultra-Low-Power Wireless Devices](https://arxiv.org/abs/2605.15694)
*Alexander Gr\"afe,Ding Huo,Johannes Berger,Marco Zimmerling,Sebastian Trimpe*

Main category: cs.LG

TL;DR: CATS是一个通信感知的分布式Transformer推理框架，使超低功耗无线设备能协作执行远超单个设备能力的模型。


<details>
  <summary>Details</summary>
Motivation: Transformer模型的计算和内存需求远超单个超低功耗IoT设备的能力，需要分布式协作。

Method: CATS通过通信感知的分布式推理方案，包括修剪通信原语SomeGather（选择性广播激活列）、基于SomeGather的模型分区方法，以及训练时使用消息丢弃模拟丢包以提高鲁棒性。

Result: 在真实实验中，CATS首次在超低功耗无线设备上实现分布式Transformer推理，最多16个设备协作执行比单个设备大14倍的模型。

Conclusion: CATS有效解决了超低功耗设备上运行大型Transformer模型的挑战，通过分布式协作和通信优化实现了模型规模的显著扩展。
...
### [209] [AGOP-IxG: A Gradient Covariance Filter for Local Feature Attribution on Tabular Data, with a Controlled Benchmark](https://arxiv.org/abs/2605.15700)
*Raj Kiran Gupta Katakam*

Main category: cs.LG

TL;DR: 提出AGOP-IxG，一种快速逐样本特征归因方法，在合成基准上优于SHAP等，速度提升数百倍，在真实数据上全局忠诚度与现有方法相当。


<details>
  <summary>Details</summary>
Motivation: 现有特征归因方法（如SHAP）通常凭习惯选择，缺乏基于真实归因的严格评估，且计算成本高。

Method: AGOP-IxG通过将每个样本的梯度乘以截断的Top-K平均梯度外积矩阵，结合梯度与二阶统计信息。

Result: 在三个合成数据集上，AGOP-IxG在Spearman秩相关和噪声特征质量上领先，在交互数据集上top-k精度最佳；速度比SHAP快350-1650倍；在真实数据上，全局忠诚度（ROAR）与其他方法相差约1.7%相对AUC。
...
### [210] [Differentiable Mixture-of-Agents Incentivizes Swarm Intelligence of Large Language Models](https://arxiv.org/abs/2605.15706)
*Xingjian Wu,Junkai Lu,Siyu Yan,Xiangfei Qiu,Jilin Hu,Chenjuan Guo,Bin Yang*

Main category: cs.LG

TL;DR: 提出DMoA，一种可微分混合智能体框架，通过动态路由和自适应激活实现弹性协作，在9个基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统依赖预定义通信拓扑，缺乏灵活性和动态适应性。

Method: 设计可微分上下文感知路由机制，利用循环结构整合历史信息，稀疏激活智能体；引入预测熵作为自监督信号优化路由。

Result: 在9个基准上达到SOTA，表现出高效、鲁棒和集成能力。

Conclusion: DMoA通过动态路由和自适应激活有效提升了多智能体系统的灵活性和性能。
...
### [211] [Bidirectional Fusion Guided by Cardiac Patterns for Semi-Supervised ECG Segmentation](https://arxiv.org/abs/2605.15722)
*Jeonghwa Lim,Minje Park,Sunghoon Joo*

Main category: cs.LG

TL;DR: 提出CardioMix框架，利用双向CutMix和心脏模式引导，增强半监督心电图分割的信息交换，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 标注数据稀缺限制深度学习模型训练，现有半监督方法未充分利用标签与未标签数据间的信息交换。

Method: 基于心脏模式引导的双向CutMix策略，对标签集注入未标签数据的真实变异，同时对未标签集施加更强监督信号，保证增广样本生理合理性。

Result: 在SemiSegECG基准上，作为即插即用模块与多种半监督算法兼容，在不同数据集和标记比例下均优于现有CutMix融合策略。

Conclusion: CardioMix高效利用信息交换，显著提升半监督心电图分割的准确性，且易于集成到现有算法中。
...
### [212] [GOMA: Toward Structure-Driven Multimodal Alignment from a Graph Signal Smoothing Perspective](https://arxiv.org/abs/2605.15723)
*Xu Wang,Xunkai Li,Yinlin Zhu,Rong-Hua Li,Guoren Wang*

Main category: cs.LG

TL;DR: 提出GOMA，利用多模态属性图结构优化冻结的视觉-语言对齐嵌入，通过解耦三大设计实现检索性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽略实体间关系，多模态属性图提供上下文但面临模态拓扑壁垒、过平滑等问题。

Method: GOMA解耦消息流向、传播方式和平滑深度，学习模态感知传播算子，有限步耦合平滑，自适应读取节点平滑轨迹。

Result: 在七个多模态属性图基准上达到最先进或并列最先进检索性能，且比最强图竞争方法更稳定。

Conclusion: 多模态属性图结构可作为冻结多模态嵌入的有效后编码器。
...
### [213] [A Unified Perturbation Framework for Analyzing Leaderboard Stability and Manipulation](https://arxiv.org/abs/2605.15761)
*Hosna Oyarhoseini,Jimmy Lin,Amir-Hossein Karimi*

Main category: cs.LG

TL;DR: 提出一个统一的扰动框架，使用基于影响力的近似方法分析Bradley-Terry排行榜在结构化数据修改下的鲁棒性，发现现代排行榜对微小扰动不稳定，并提供审计工具。


<details>
  <summary>Details</summary>
Motivation: 排行榜（如LMArena）在评估大语言模型中至关重要，但其排名鲁棒性尚不明确。

Method: 构建匹配级别扰动（删除、添加、翻转）和玩家移除的统一框架，通过影响力近似评估对top-k成员、全局排名一致性（Kendall's tau）和置信区间的影响。

Result: 在Chatbot Arena等数据集上，低于1%的定向扰动即可改变榜首模型、降低Kendall's tau、改变置信区间；影响力分数可高效实施定向扰动。
...
### [214] [Continual Learning of Domain-Invariant Representations](https://arxiv.org/abs/2605.15775)
*Pascal Janetzky,Tobias Schlagenhauf,Stefan Feuerriegel*

Main category: cs.LG

TL;DR: 提出持续学习中的域不变表示方法，通过结合重放训练和顺序不变性对齐，在多个领域数据集上提升对未见目标域的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法易学习虚假域特定线索（捷径学习），导致部署后在未见域泛化差。

Method: 结合重放训练与顺序不变性对齐，学习并保留跨域不变结构。

Result: 在6个基准和真实数据集（视觉、医学、制造、生态）上，对未见目标域的泛化性能一致优于现有CL基线。

Conclusion: 首次开发用于持续学习的域不变表示方法，且简单扩展现有域不变表示学习方法效果有限。
...
### [215] [Grokking as Structural Inference: Transformers Need Bayesian Lottery Tickets](https://arxiv.org/abs/2605.15787)
*Kai Hidajat,Solden Stoll,Joseph An*

Main category: cs.LG

TL;DR: 本文提出Attention的贝叶斯解释，将泛化延迟归因于结构性推断滞后，并通过KL干预加速。


<details>
  <summary>Details</summary>
Motivation: 现有理论忽略了注意力丢弃信息不可恢复的约束，需要解释泛化延迟。

Method: 将注意力形式化为任务依赖图上的贝叶斯后验，分解泛化条件为MLP容量和注意力结构条件。

Result: 导出逆权重衰减的结构等待时间，提出基于KL的结构干预可绕过等待，得到逆干预强度的缩放律。

Conclusion: 实验表明贝叶斯票匹配或优于彩票票转移。
...
### [216] [Learning Context-conditioned Gaussian Overbounds for Convolution-Based Uncertainty Propagation](https://arxiv.org/abs/2605.15789)
*Ruirui Liu,Xuejie Hou,Yiping Jiang,Hui Ren*

Main category: cs.LG

TL;DR: 提出一个统一的学习框架，训练神经网络产生上下文感知的高斯过界，在有限分位数网格上具有可证明的保守性，并支持保守的线性组合和卷积分析。


<details>
  <summary>Details</summary>
Motivation: 在安全关键领域，不确定性量化需要保守边界而非点估计。现有的预测器级区间不复合，而经典的高斯过界全局且过于保守，难以适应特征条件误差。

Method: 训练神经网络输出均值和尺度，使用过界损失函数在选定分位数上强制保守性，并以Wasserstein风格项惩罚分布距离，同时假设三个显式正则性条件以确保连续尾部保守性。

Result: 在合成数据和真实数据集（多径、电离层、对流层残余误差）上，方法产生更紧的边界，同时在强制网格和实验中保持保守性。

Conclusion: 该框架与模态无关，适用于需要动态环境中保守、特征条件不确定性估计的学习系统。
...
### [217] [AOT-POT: Adaptive Operator Transformation for Large-Scale PDE Pre-training](https://arxiv.org/abs/2605.15793)
*Qitan Lv,Hong Wang,Zhongkai Hao,Wen Wu,Xuenan Xu,Bowen Zhou,Feng Wu,Chao Zhang*

Main category: cs.LG

TL;DR: 提出自适应算子变换方法AOT-POT，通过变换PDE解算子使其更易建模，仅用3%额外参数在12个基准上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 多PDE预训练中，算子复杂性和结构多样性带来挑战；现有方法主要增加模型容量而不改变算子本身，效果有限。

Method: 将隐藏表示扩展为多个并行流，在每个子层前后自适应聚合和重分配，并通过Sinkhorn投影双随机矩阵混合流，从而将不同算子变换为统一形式。

Result: 在12个PDE基准上达到SOTA，相对L2误差降低最多77.6%，平均40.9%；微调后域内L2误差降低最多92%，域外最多89%。

Conclusion: 自适应算子变换是有效且互补的方向，超越了单纯扩大模型容量，为PDE基础模型提供了新思路。
...
### [218] [Martingale Neural Operators: Learning Stochastic Marginals via Doob-Meyer Factorization](https://arxiv.org/abs/2605.15806)
*Kai Hidajat*

Main category: cs.LG

TL;DR: 基于Doob-Meyer定理提出Martingale Neural Operator (MNO)，直接映射初始条件到条件均值和协方差，在随机PDE中显著降低Wasserstein距离并保持计算效率。


<details>
  <summary>Details</summary>
Motivation: 神经算子作为确定性替代模型时，对随机PDE会坍缩到条件均值，丢失方差和尾部结构，而传统恢复方法（如蒙特卡洛或生成模型）牺牲了单次推断效率和分辨率不变性。

Method: 利用Doob-Meyer定理将半鞅分解为可预测漂移和鞅，设计MNO架构：参数化漂移型均值和低秩因子B_φ（保证B_φ^T B_φ半正定），直接输出终端条件均值和协方差，实例化为高斯残差。

Result: 在1D SPDE、粗糙波动率和2D任务中，MNO将Wasserstein距离减少120倍（φ^4场论）和68倍（随机Burgers），训练预算相同下比条件扩散基线快约3倍；2D任务在零样本分辨率迁移和湍流中与FNO相当。

Conclusion: MNO有效恢复随机PDE的不确定性结构，但在准确定性系统（如Gray-Scott）上仍存在失败模式。
...
### [219] [Intrinsic Wasserstein Rates for Score-Based Generative Models on Smooth Manifolds](https://arxiv.org/abs/2605.15822)
*Guoji Fu,Taiji Suzuki,Wee Sun Lee,Atsushi Nitanda*

Main category: cs.LG

TL;DR: 证明了对于紧致d维光滑流形上的数据分布，方差保持SGM估计器达到了固有的Wasserstein-1样本指数。


<details>
  <summary>Details</summary>
Motivation: 评分生成模型在高维空间中训练，但数据分布往往支持在低维非线性结构上，需要理论保证。

Method: 通过分离评分逼近为大噪声的切线胞腔区域和小噪声的投影中心去高斯化拉普拉斯区域，使用ReLU实现最近投影坐标，通过有限内在锚点和Gauss-Newton迭代。

Result: 得到了非渐近界，显式分离了几何包络、Hölder半径、密度下界、环境依赖和有限阶修正项。

Conclusion: 对于多项式控制的几何和密度，评分网络参数具有多项式环境依赖，实现了改进的样本复杂度。
...
### [220] [From Observed Viability to Internal Predictive Approximation: A Single-Subject Latent-Space Analysis of Gait Dynamics Under Occlusal Constraint](https://arxiv.org/abs/2605.15862)
*Jacques Raynal,Pierre Slangen,Elsa Raynal,Jacques Margerit*

Main category: cs.LG

TL;DR: 本研究表明，在单一个体数据集中，观察到的纵向步态组织潜在变化可以通过预测性潜在空间框架进行近似，为未来多主体预测模型提供了方法学桥梁。


<details>
  <summary>Details</summary>
Motivation: 探讨自适应生物力学系统在相似可观察步态表现下潜在组织和纵向行为差异，是否可在预测性潜在空间框架内近似步态组织纵向变换，而不涉及临床预测或因果咬合效应。

Method: 采用探索性单被试设计，帕金森参与者两次测试间隔11周，鞋垫记录步态。六种咬合探针：自然咬合、张口脱离、紧咬、两种中心关系垂直维度增加、一种垂直维度增加伴下颌前突。PCA构建PC1-PC2潜在空间，前馈神经网络训练近似M1-M2变换。

Result: 主要分析保留中心关系条件顺序OC3 < ONL < OC2.5；扩展分析保留整体结构，OC3和OC3P紧密分组，OC2.5和开口脱离位移最高；留出分析显示条件依赖的近似变异性。

Conclusion: 不建立泛化预测或因果效应，仅支持单被试数据集中纵向潜在变换可被内部近似，为未来多被试预测模型提供方法学基础。
...
### [221] [Ti-iLSTM: A TinyDL Approach for Logic-Level Anomaly Detection in Industrial Water Treatment Systems](https://arxiv.org/abs/2605.15874)
*Mandar Joshi,Farzana Zahid,Judy Bowen,Matthew M.Y. Kuo,Valeriy Vyatkin,Emil Karlsson*

Main category: cs.LG

TL;DR: 本文提出基于TinyDL的增量LSTM（Ti-iLSTM）框架，用于在资源受限的PLC上检测工业水处理系统中的逻辑层欺骗异常，在SWaT数据集上实现高检测性能，并在WADI数据集上验证了泛化能力。


<details>
  <summary>Details</summary>
Motivation: 逻辑层欺骗异常不易被阈值监测检测，且传统服务器端模型资源消耗大，需要轻量级、高精度的异常检测方法部署在PLC上。

Method: 提出Ti-iLSTM框架，通过优化LSTM的内存和空间占用，实现轻量化时序异常检测模型。

Result: 在SWaT数据集上F1分数为0.983，ROC-AUC为0.998；在WADI数据集上的部署式验证表明模型具有跨数据集适用性。

Conclusion: 逻辑感知监督与TinyDL序列学习结合，可为资源受限的PLC提供高效准确的异常检测方案。
...
### [222] [Shapley Neuron Values for Continual Learning: Which Neurons Matter Most?](https://arxiv.org/abs/2605.15877)
*Mohammad Ali Vahedifar,Abhisek Ray,Qi Zhang*

Main category: cs.LG

TL;DR: 提出了基于夏普利值的神经元估值方法（SNV），通过冻结重要神经元实现无缓冲的持续学习，有效缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 神经网络在持续学习中存在灾难性遗忘问题，新任务学习会损害旧任务的性能。

Method: 采用合作博弈论中的夏普利值量化神经元重要性，选择性冻结重要神经元，保持其他神经元可塑性，实现无缓冲且不扩展架构的持续学习。

Result: 在ImageNet-1k上，SNV在类增量学习场景中准确率提升2.88%，任务增量学习中提升6.46%，均优于现有无缓冲方法。

Conclusion: SNV提供了一种原则性的无缓冲持续学习方案，有效避免灾难性遗忘。
...
### [223] [Practical Validity Conditions for Byzantine-Tolerant Federated Learning](https://arxiv.org/abs/2605.15887)
*M\'elanie Cambus,Darya Melnyk,Tijana Milentijevi\'c,Stefan Schmid*

Main category: cs.LG

TL;DR: 提出最小包围球（MEB）有效性条件及其松弛版本c-MEB，用于拜占庭容错联邦学习中的鲁棒聚合，提供了凸有效性之外的实用替代方案。


<details>
  <summary>Details</summary>
Motivation: 凸有效性条件具有维度依赖的鲁棒性，且排除了许多实际聚合规则，需要新的有效性条件。

Method: 引入MEB有效性及其松弛c-MEB有效性，证明严格MEB仍有限制，而松弛c-MEB在多数客户端诚实（n>2t）时可实现。给出最优MinMax-MEB规则（c<√2），并证明标准聚合器（最小直径平均、中心点、几何中位数）的松弛MEB保证。

Result: 松弛MEB有效性提供了凸有效性的实用替代，连接了分布式计算与拜占庭容错聚合规则。

Conclusion: 松弛MEB有效性是凸有效性的实用替代，适用于现代联邦学习系统。
...
### [224] [CHoE: Cross-Domain Heterogeneous Graph Prompt Learning via Structure-Conditioned Experts](https://arxiv.org/abs/2605.15888)
*Peiyuan Li,Yongqi Huang,Jitao Zhao,Dongxiao He,Di Jin,Weixiong Zhang*

Main category: cs.LG

TL;DR: 提出CHoE，一种基于专家网络的跨域异构图提示学习方法，通过结构条件专家和结构感知路由机制，有效提升跨域少样本性能。


<details>
  <summary>Details</summary>
Motivation: 现有异构图提示学习方法仅限于域内场景，当应用域发生变化时性能下降，无法满足实际跨域部署需求。

Method: 预训练阶段引入并训练结构条件专家；提示调优阶段采用结构感知的专家路由和负载均衡机制为每个元路径视图选择结构兼容的专家，并设计基于提示的语义融合模块整合多视图表示。

Result: 在跨域少样本应用中持续提升性能，优于所有基线方法。

Conclusion: CHoE有效解决了现有方法在域转移时的局限性，拓展了异构图提示学习的跨域适用性。
...
### [225] [From Layers to Networks: Comparing Neural Representations via Diffusion Geometry](https://arxiv.org/abs/2605.15901)
*Atharva Khandait,Jan E. Gerken*

Main category: cs.LG

TL;DR: 首次将扩散几何应用于神经表示，通过马尔可夫矩阵融合多视图学习，提出多尺度相似度度量，在多个基准上取得最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有神经表示相似度度量缺乏多尺度分析能力，且难以跨层比较不同网络的联合样本几何。

Method: 利用RSM到行随机马尔可夫矩阵的等价变换，提出多尺度CKA和距离相关，并通过交替扩散融合多层马尔可夫矩阵。

Result: 在ReSi基准上，语言和视觉任务的准确性和输出相关性达到SOTA，并在分布外数据上同样表现最优。

Conclusion: 扩散几何为神经表示分析提供了有效多尺度框架，跨层网络比较方法具有普适性。
...
### [226] [Context-aware Entity-Relation Extraction for Threat Intelligence Knowledge Graphs](https://arxiv.org/abs/2605.15904)
*Inoussa Mouiche,sherif Saad*

Main category: cs.LG

TL;DR: 提出CTiKG框架，通过混合NLP模型和领域本体从非结构化威胁情报中准确提取实体关系，在标准数据集上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有流水线方法因报告结构复杂、专业语言和语义歧义导致错误传播，降低提取精度和泛化性。

Method: 构建流水线架构CTiKG，集成SecureBERT+上下文嵌入与领域本体知识，减少分类错误和级联误差。

Result: 在DNRTI-AUG-STIX2数据集上，NER提升3-4%，RE提升最高8%；在DNRTI和STUCCO基准上验证鲁棒性。

Conclusion: CTiKG能有效从CTI报告中构建结构化知识图谱，提升安全分析效率，数据集已开源以促进可重复研究。
...
### [227] [LoCO: Low-rank Compositional Rotation Fine-tuning](https://arxiv.org/abs/2605.15916)
*An Nguyen,Jaesik Choi,Anh Tong*

Main category: cs.LG

TL;DR: 提出了低秩组合正交微调（LoCO），一种通过低秩斜对称矩阵和组合旋转链构建正交变换的参数高效微调方法，在多个领域表现优越。


<details>
  <summary>Details</summary>
Motivation: 现有PEFT方法（如LoRA）虽参数高效但难以保持预训练表示的几何结构，因此需要一种能保留几何结构的方法。

Method: 通过低秩斜对称矩阵和组合旋转链构造正交变换，并提出近似方案实现组合旋转的完全并行计算，在保持低计算复杂度的同时控制正交性误差。

Result: 在扩散Transformer、视觉Transformer和语言模型微调中，LoCO相比现有正交和非正交方法表现出更优或相当的性能。

Conclusion: LoCO是一种有效且实用的PEFT方法，能在保持参数效率的同时保留几何结构，具有广泛适用性。
...
### [228] [A Retrieval-Enhanced Transformer for Multi-Step Port-of-Call Sequence Prediction in Global Liner Shipping](https://arxiv.org/abs/2605.15937)
*Yanzhao Su,Fang He,Yineng Wang*

Main category: cs.LG

TL;DR: 提出CCRE框架，结合检索增强和连接约束，实现多步港口序列预测，准确率显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法因不可靠的航行计划和AIS数据局限性难以预测多步港口序列，需要更准确的预测以优化资源调配。

Method: 构建检索增强历史编码器查询全球数据库，Transformer轨迹编码器，通过交叉注意力进行中间融合，使用自回归解码器（含计划采样和Gumbel-Softmax）生成序列，并施加拓扑约束。

Result: 第一目的地准确率72.3%，三步平均准确率61.4%，比CatBoost和LSTM分别高12.6%和11.3%。

Conclusion: 模型能有效捕捉复杂路由模式，具有可扩展性，可应用于多种国际航线。
...
### [229] [When and Why Adversarial Training Improves PINNs: A Neural Tangent Kernel Perspective](https://arxiv.org/abs/2605.15959)
*Yuan-dong Cao,Chi Chiu SO,Jun-Min Wang,He Wang*

Main category: cs.LG

TL;DR: 提出新分析框架解释对抗训练为何提升PINNs性能，并基于此开发高效训练算法，大幅提高精度。


<details>
  <summary>Details</summary>
Motivation: PINNs因频谱偏差等问题难以训练，对抗训练虽有效但机制不明，需理论解释和更好算法。

Method: 基于GAN判别器影响PINNs训练动态的观察，建立理论分析框架，统一分析多种GAN变体，并设计新训练算法。

Result: 显著减少训练病理，模型性能大幅提升，精度常比替代方法高几个数量级。

Conclusion: 新框架和算法有效改善PINNs训练难题，提供了理论基础和实用方案。
...
### [230] [Entropy-Based Characterisation of the Polarised Regime in Latent Variable Models](https://arxiv.org/abs/2605.15965)
*Peter Clapham,Lisa Bonheme,Marek Grzes*

Main category: cs.LG

TL;DR: 提出基于均值表示熵的信息论分类方法，用于识别VAE中极化现象的主动/被动/混合维度。


<details>
  <summary>Details</summary>
Motivation: 现有标准依赖高斯先验，限制适用性，需要更通用的分类方法。

Method: 通过均值熵与KL最小化的熵-方差界理论，联系Bonheme条件，并揭示均值熵无法单独区分主动与混合维度。

Result: 在β-VAE等模型上验证，熵准则一致恢复极化现象；被动维度在适当归一化下对下游任务有小改进。

Conclusion: 极化崩溃更多是规模而非绝对信息移除问题。
...
### [231] [Constrained latent state modeling: A unifying perspective on representation learning under competing constraints](https://arxiv.org/abs/2605.15995)
*Gwenol\'e Quellec*

Main category: cs.LG

TL;DR: 提出约束潜在状态建模（CLSM）作为统一框架，通过显式施加核心属性约束解决潜在表示学习中的碎片化和歧义问题。


<details>
  <summary>Details</summary>
Motivation: 现有潜在表示学习方法碎片化，目标函数约束不足导致表示歧义，缺乏统一原则来指导有意义潜在状态的设计。

Method: 识别预测充分性、最小性、时间一致性、观测兼容性、不变性和结构约束等核心属性，并分析它们之间的固有权衡，将现有模型视为施加不同约束子集的特例。

Result: CLSM将可辨识性问题归因于欠约束的设定，而非孤立的技术限制，并提供了一个分析权衡和指导模型设计的统一空间。

Conclusion: CLSM提供了一个原则性框架，使设计选择显式化，有助于开发更可解释、鲁棒和任务对齐的潜在状态模型。
...
TL;DR: CT-AGD是一种针对非凸优化问题的加速梯度下降方法，通过捕捉局部曲率来加速一阶方法，平均减少33%训练轮次。


<details>
  <summary>Details</summary>
Motivation: 深度学习训练中非凸优化问题需要高效方法，现有一阶方法速度慢且受随机噪声影响。

Method: 使用有限差商捕捉局部曲率，并开发启发式方法减少随机小批量训练的噪声和偏差；存储与计算开销与Adam相当。

Result: 达到与基线一阶方法相同精度，平均减少33%训练轮次。

Conclusion: CT-AGD是一种有效的加速优化方法，具有实际应用价值。
...
TL;DR: 本文证明在蒙特卡洛方法中引入物理先验能显著提升性能，减轻训练负担，有助于模拟更大尺寸的离散自旋模型。


<details>
  <summary>Details</summary>
Motivation: 蒙特卡洛方法在相变附近存在临界慢化问题，深度学习采样器虽可缓解，但训练困难，部分原因在于通用架构忽略物理对称性。

Method: 提出基于物理先验概率分布的训练框架，在现有整合自旋相互作用策略基础上，使用先验分布作为训练起点。

Result: 在伊辛模型和Edwards-Anderson自旋玻璃模型上，使用物理先验减少了训练负担，并能模拟更大系统尺寸。

Conclusion: 采用物理先验取代‘空白板’模型，可有效提升蒙特卡洛方法的效率与可扩展性。
...
### [234] [Mind Dreamer: Untethering Imagination via Active Latent Intervention on Latent Manifolds](https://arxiv.org/abs/2605.16030)
*Shaojun Xu,Xiaoling Zhou,Yihan Lin,Yapeng Meng,Xinglong Ji,Luping Shi,Rong Zhao*

Main category: cs.LG

TL;DR: 提出Mind Dreamer框架，通过主动潜在干预打破历史关联，显著提升基于模型的强化学习在稀疏奖励任务中的效率。


<details>
  <summary>Details</summary>
Motivation: 现有MBRL中世界模型流形发现过快，而策略优化受历史状态限制，导致学习不对称。

Method: 从学习生成器采样初始状态，利用对抗生成器产生非连续潜在跳跃，并推导继电价值函数和继电不确定性函数，采用二次折扣γ²传播价值。

Result: 在DeepMind Control Suite上平均加速1.67倍，稀疏奖励任务加速达8.8倍。

Conclusion: MD近似方差最小化重要性采样，扩大流形谱间隙，减少到达关键瓶颈状态的期望时间。
...
### [235] [Looped SSMs: Depth-Recurrence and Input Reshaping for Time Series Classification](https://arxiv.org/abs/2605.16048)
*M\'onika Farsang,Ramin Hasani,Daniela Rus,Radu Grosu*

Main category: cs.LG

TL;DR: 在状态空间模型中引入深度循环和输入重塑两种技术，能提升时间序列分类性能，且两者独立可叠加。


<details>
  <summary>Details</summary>
Motivation: 循环变压器中的深度循环在SSM中未被探索，且输入重塑是一个被忽视的设计轴，研究它们是否能带来性能提升。

Method: 将SSM模块重复L次形成深度循环，并调整输入形状（低维拼接时间步，高维展平并重新划分特征-时间维度）。

Result: 循环SSM（k参数迭代L次）匹配或优于标准SSM（k*L独立参数），输入重塑带来1-6%准确率提升，两者结合效果更佳。

Conclusion: 深度循环与序列循环正交，输入重塑是独立的设计轴，均能作为SSM的通用改进方向。
...
### [236] [Ada-Diffuser: Latent-Aware Adaptive Diffusion for Decision-Making](https://arxiv.org/abs/2605.16054)
*Fan Feng,Selena Ge,Minghao Fu,Zijian Li,Yujia Zheng,Zeyu Tang,Yingyao Hu,Biwei Huang,Kun Zhang*

Main category: cs.LG

TL;DR: 提出Ada-Diffuser，一个因果扩散模型，通过显式推断潜在动态来改进基于生成模型的决策制定，在模拟控制和机器人任务中表现有效。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型决策方法忽略具有演化动态的潜在因素，而这些因素对环境和奖励建模至关重要，需显式建模以提升动态建模和决策性能。

Method: 构建统一框架，从最小观测中推断潜在动态，理论证明可从短时间块识别潜在过程；引入Ada-Diffuser因果扩散模型，同时学习观测时间结构和底层潜在动态，并用于规划和控制。

Result: 在模拟控制和机器人基准实验中，Ada-Diffuser实现了准确的潜在推断和自适应策略学习。

Conclusion: 显式建模潜在动态可提升生成决策模型的动态建模能力和适应性，Ada-Diffuser有效结合扩散模型与潜在推断，适用于规划与策略学习。
...
### [237] [SAFE Quantum Machine Learning with Variational Quantum Classifiers](https://arxiv.org/abs/2605.16067)
*Ying Chen,Paolo Giudici,Vasily Kolesnikov,Paolo Recchia*

Main category: cs.LG

TL;DR: 提出一种变分量子分类器，通过振幅编码和可学习预编码层实现高维深度表示，结合有界量子可观测值，在安全关键场景中展现出平衡的SAFE可靠性和稳健预测性能。


<details>
  <summary>Details</summary>
Motivation: 安全关键环境需要分类器具有高可靠性和鲁棒性，而现有量子模型在稳定性方面不足，因此需要一种兼顾精度与稳定性的量子学习方案。

Method: 使用归一化振幅编码将高维数据嵌入量子态，并添加可学习经典预编码层；通过有界量子可观测值构建平滑假设类；采用基于Cramer-von Mises散度的SAFE-AI指标评估模型的准确性、鲁棒性和可解释性。

Result: 与强经典基线相比，量子模型预测性能相当，但在SAFE可靠性方面更均衡，对噪声和结构化特征移除表现出更好的鲁棒性和稳定性。

Conclusion: 变分量子电路为安全关键环境中的稳定性导向SAFE学习提供了原理性机制，具有实际应用潜力。
...
### [238] [ITGPT: Generative Pretraining on Irregular Timeseries](https://arxiv.org/abs/2605.16069)
*Antoine Honor\'e,Ming Xiao*

Main category: cs.LG

TL;DR: 提出ITGPT模型，结合自监督学习和生成式预训练，高效处理多模态不规则时间序列数据，在医疗和预测维护任务上达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列回归模型难以利用大量多模态数据，尤其在不规则采样或缺失值情况下；而Transformer虽在文本领域有效，但缺乏处理此类数据的灵活性。

Method: 提出ITGPT，基于注意力机制架构，支持自监督学习（SSL）和生成式预训练（GPT）联合训练，无需重采样、特征融合或显式数据插补。

Result: 在TIHM（医疗）和CompX（预测维护）数据集上取得最优性能；标签稀缺时，通过SSL和GPT训练有效利用未标注数据，优于纯监督方法。

Conclusion: 为高效利用大型非结构化时间序列数据集进行实际推理任务迈出重要一步。
...
### [239] [MIND: Decoupling Model-Induced Label Noise via Latent Manifold Disentanglement](https://arxiv.org/abs/2605.16081)
*Dayong Ren*

Main category: cs.LG

TL;DR: 提出MIND框架，通过潜流形解耦处理模型诱导的标签噪声，在多个基准上超越现有方法，并能修正视觉语言模型的零样本幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒学习方法依赖全局转移矩阵或实例特定矩阵，前者对结构噪声欠拟合，后者计算不可行，需新方法应对系统性的模型诱导噪声。

Method: 利用潜流形解耦将高维噪声分解为子空间依赖成分，通过潜解耦估计器动态投影样本到具有一致错误模式的潜在结构聚类，实现无需真实锚点的噪声可识别性。

Result: 在CIFAR-100上的受控噪声和S3DIS、ScanNet等真实3D数据集的结构压力测试中，MIND显著优于现有方法，并能有效修正OpenSeg等视觉语言模型的零样本幻觉。

Conclusion: MIND为鲁棒蒸馏基础模型提供有效框架，通过解耦模型诱导噪声实现更优的噪声建模与纠正。
...
### [240] [Multi-level Self-supervised Pretraining on Compositional Hierarchical Graph for Molecular Property Prediction](https://arxiv.org/abs/2605.16088)
*Xiayu Liu,Zhengyi Lu,Hou-biao Li*

Main category: cs.LG

TL;DR: 提出MolCHG，一种基于组合层次图的多层级自监督预训练框架，通过并行原子图和键图提升键级信息，在MoleculeNet基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有分子图自监督方法大多在单一结构粒度操作，将键信息视为辅助边属性而非独立语义层，限制了表示能力。

Method: 构建组合层次图，包含原子、键、片段和全局四级节点；设计原子-键跨视图对比、片段级官能团预测和图级结构预测三个层级预训练任务。

Result: 在9个MoleculeNet数据集的7个上取得最佳性能，其余保持竞争力；消融实验证实多层级信号互补且每个组件均有贡献。

Conclusion: MolCHG通过多层级自监督有效捕获分子结构语义，验证了键级独立表示和多任务学习的优势。
...
### [241] [Centralized vs Decentralized Federated Learning: A trade-off performance analysis](https://arxiv.org/abs/2605.16089)
*Chaimaa Medjadji,Guilain Leduc,Sylvain Kubler,Yves Le Traon*

Main category: cs.LG

TL;DR: 本文通过实验比较了集中式、去中心化和半去中心化联邦学习架构的性能与权衡，使用Fedstellar模拟器、MNIST数据集和MLP分类器。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对三种FL架构的实验比较，需要理解各自的优势和局限性及性能指标权衡。

Method: 使用Fedstellar模拟器、MNIST数据集和MLP分类器进行实验分析。

Result: 实验揭示了不同架构在性能指标上的差异和权衡。

Conclusion: 选择合适的FL架构取决于应用需求，本文通过实验填补了对比分析的空白。
...
### [242] [Federated Imputation under Heterogeneous Feature Spaces](https://arxiv.org/abs/2605.16099)
*Imane Hocine,Chaimaa Medjadji,Sylvain Kubler,Gregoire Danoy,Yves Le Traon*

Main category: cs.LG

TL;DR: 提出 FedHF-Impute，一种通过共享特征图进行消息传递的联邦学习框架，用于解决表格数据中特征模式部分重叠的联邦插补问题。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法假设特征模式对齐，但实际中客户端只有部分重叠的特征子集，导致参数平均方法在弱重叠特征组间信息传递有限，影响联邦插补效果。

Method: 将特征结构性缺失与传统缺失区分，利用共享的全局特征图通过消息传递在统计相关特征间传播信息，实现跨客户端间接知识转移，同时保持标准联邦通信。

Result: 在SECOM和AirQuality数据集上，相比联邦基线，插补RMSE分别降低26.9%和8.4%；在PhysioNET上表现相当，差异仅0.3%。

Conclusion: FedHF-Impute能够有效处理特征异构性下的联邦插补问题，特别是在特征从未联合观测的场景下仍能实现知识迁移。
...
### [243] [Attention Dispersion in Dynamic Graph Transformers: Diagnosis and a Transferable Fix](https://arxiv.org/abs/2605.16112)
*Jinhao Zhang,Kangfei Zhao,Qiuhao Zeng,Long-Kai Huang*

Main category: cs.LG

TL;DR: 提出差分注意力机制解决动态图Transformer在时间分布偏移下的注意力分散问题，并实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有CTDG Transformer在时间偏移数据集上性能受限，识别出注意力分散是共同失败模式。

Method: 用差分注意力替代标准注意力，抑制共模噪声并放大关键节点信号。

Result: 在三个基线上加入差分注意力后性能一致提升，尤其高偏移数据集；DiffDyG在9个基准上达SOTA。

Conclusion: 差分注意力有效缓解注意力分散，为CTDG Transformer应对时间偏移提供简单可迁移的修复方案。
...
### [244] [Multi-Fidelity Flow Matching: Cascaded Refinement of PDE Solutions](https://arxiv.org/abs/2605.16118)
*Sipeng Chen,Junliang Liu,Hewei Tang,Shibo Li*

Main category: cs.LG

TL;DR: 提出了一种多保真度流匹配（MFFM）级联细化框架，通过校准源分布和条件化速度网络，高效求解参数化PDE，在8个基准上验证了性能。


<details>
  <summary>Details</summary>
Motivation: 条件流匹配中的源分布可校准而非固定先验，利用这一点可简化残差细化问题并改善训练几何。

Method: MFFM使用经验低-高保真残差尺度校准源噪声（局部高斯模糊相关），速度网络以低保真解为条件；多分辨率级联独立处理相邻保真度，逐级预训练后端到端微调，采用确定性一步展开。

Result: 在8个基准（2个超分辨率、6个时空预测任务）上验证，每次查询仅需L次网络评估即可达到最细网格精度。

Conclusion: MFFM是多重网格细化的学习模拟，以较低计算成本实现高精度PDE求解。
...
### [245] [Navigating Potholes with Geometry-Aware Sharpness Minimization](https://arxiv.org/abs/2605.16134)
*Simon Dufort-Labb\'e,Mehrab Hamidi,Razvan Pascanu,Ioannis Mitliagkas,Damien Scieur,Aristide Baratin*

Main category: cs.LG

TL;DR: LLQR+SAM将SAM的快速锐度感知与LLQR的慢速学习几何结合，通过双时间尺度预条件器提升泛化，在视觉和序列任务上均优于单独方法。


<details>
  <summary>Details</summary>
Motivation: SAM均匀扰动所有参数方向，忽略了损失几何的异质性，需要结合学习到的几何信息。

Method: 引入LLQR框架学习稀疏、慢速指数移动平均的预条件器，在其之上进行SAM快速扰动，形成双时间尺度优化。

Result: 在标准视觉和序列建模基准上，LLQR+SAM一致优于SAM和LLQR单独使用。

Conclusion: 慢速学习的几何与快速锐度修正是互补的，双时间尺度结构有助于逃离局部尖锐区域并保持平坦盆地稳定。
...
### [246] [Surrogate Neural Architecture Codesign Package (SNAC-Pack)](https://arxiv.org/abs/2605.16138)
*Jason Weitz,Dmitri Demler,Benjamin Hawks,Aaron Wang,Nhan Tran,Javier Duarte*

Main category: cs.LG

TL;DR: 提出SNAC-Pack框架，通过硬件感知的多目标搜索与量化剪枝压缩，实现FPGA部署的神经网络架构自动协同设计，在物理分析任务中显著缩短设计周期并降低资源消耗。


<details>
  <summary>Details</summary>
Motivation: 现有NAS方法常仅优化精度或使用与硬件成本相关性差的代理指标（如BOP），尤其FPGA部署成本受多维资源约束（LUT、DSP、FF、BRAM、延迟），急需硬件感知的自动协同设计方法。

Method: 采用多目标全局搜索（Optuna+NSGA-II）并行采样，硬件代理模型预测资源与延迟；局部搜索阶段结合量化感知训练（QAT）与迭代幅度剪枝进行模型压缩；最后通过hls4ml合成FPGA固件。整体支持通过YAML配置和可选agent前端无需修改框架即应用于新数据集。

Result: 在高能物理喷注分类和超导量子比特读取任务中，发现的紧凑架构在任务指标上匹配或超越强基线，同时显著降低FPGA资源利用率；在量子比特读取任务中将设计空间探索从数月手动调优缩短至数小时自动搜索。

Conclusion: SNAC-Pack实现了高效的硬件感知神经架构协同设计与自动FPGA部署，大幅加速了定制化硬件加速器的开发过程，尤其在资源受限的科学应用中表现突出。
...
### [247] [Learn Where Outcomes Diverge: Efficient VLA RL via Probabilistic Chunk Masking](https://arxiv.org/abs/2605.16154)
*Vaidehi Bagaria,Nikshep Grampurohit,Pulkit Verma*

Main category: cs.LG

TL;DR: PCM是一种针对GRPO的改进方法，通过概率性选择轨迹片段进行梯度计算，大幅提升效率，在保持成功率的同时实现2.38倍加速和60%内存降低。


<details>
  <summary>Details</summary>
Motivation: GRPO-based VLA RL后训练中梯度计算占主导（78%），且均匀分配计算给所有片段，但学习信号仅来自成功与失败分歧的片段，导致大量计算浪费。

Method: PCM通过成功-失败动作方差作为每片段梯度方差的代理，动态更新保留概率，仅对少量随机选中的片段进行反向传播，无需奖励模型或学习型评估器。

Result: 在三个LIBERO基准上，PCM匹配标准GRPO的最终成功率，同时实现2.38倍墙钟时间加速、4.8倍梯度更新加速、60%峰值激活内存降低，仅需少于20%的轨迹片段进行反向传播。

Conclusion: PCM作为GRPO的即插即用改进，显著降低计算成本而不损失性能，适用于计算资源受限的VLA策略训练。
...
### [248] [Entropic Auto-Encoding via Implicit Free-Energy Minimization](https://arxiv.org/abs/2605.16164)
*Hazhir Aliahmadi,Irina Babayan,Greg van Anders*

Main category: cs.LG

TL;DR: 熵自编码器通过隐式先验和编码器集成缓解后验坍塌，学习非高斯多模态潜在分布。


<details>
  <summary>Details</summary>
Motivation: 变分自编码器由于显式先验导致后验坍塌，潜在变量被忽略。

Method: 仅使用重建损失，通过自由能最小化的编码器集成隐式生成先验，解码器引导寻找信息性潜在表示。

Result: EAE缓解后验坍塌，学习非高斯多模态分布，生成多样且一致的数据，保留底层结构。

Conclusion: EAE能捕捉反应扩散过程的低维动力学、MNIST的隐式类别以及CelebA的面部层次结构。
...
### [249] [BAPR: Bayesian amnesic piecewise-robust reinforcement learning for non-stationary continuous control](https://arxiv.org/abs/2605.16170)
*Yifan Zhang,Liang Zheng*

Main category: cs.LG

TL;DR: 提出BAPR算法，融合贝叶斯在线变化检测与鲁棒集成强化学习，通过自适应保守性机制应对分段平稳环境，并给出形式化验证的理论保证。


<details>
  <summary>Details</summary>
Motivation: 分段平稳控制系统中，全局保守策略浪费稳定期性能，而局部自适应策略在未检测到模式变化时可能导致灾难性失败。

Method: BAPR算法：结合贝叶斯在线变化检测（BOCD）与鲁棒集成SAC，提出BAPR算子（模式条件贝尔曼算子的凸组合），并引入基于RMDM损失训练的上下文条件模块，在训练时使用模拟器模式ID，部署时无需模式标签。

Result: BAPR算子是γ-压缩映射；当信念依赖于Q函数时，压缩因子变为γ+λΔ，且当γ+λΔ≥1时压缩失败；推导出分量形式的形式化错误预算，限制切换后恢复性能；BOCD驱动的自适应保守机制在检测到变化点后最大化保守性并随置信度增长平滑放松，检测延迟O(log(1/δ))；所有结果通过Lean 4形式化验证（3个文件、1145行、22个定理，无sorry）。

Conclusion: BAPR通过理论保证和形式化验证，有效解决了分段平稳环境下鲁棒性与自适应性的平衡问题，为实际控制系统提供了安全高效的控制策略。
...
### [250] [Imitation learning for clinical decision support in pediatric ECMO](https://arxiv.org/abs/2605.16175)
...
TL;DR: 将儿科ECMO临床决策建模为从轨迹中学习动作的模仿学习，使用TabPFN等方法，发现TabPFN优于传统基线。


<details>
  <summary>Details</summary>
Motivation: 儿科ECMO数据复杂且稀缺，需要有效的决策支持模型。

Method: 将决策制定视为从观测轨迹中模仿学习，动作未直接观测；使用TabPFN、XGBoost、MLP等方法。

Result: TabPFN在真实儿科ECMO数据上持续优于XGBoost和MLP。
...
### [251] [Hypothesis-driven construction of mesoscopic dynamics](https://arxiv.org/abs/2605.16211)
*Zhuoyuan Li,Aiqing Zhu,Qianxiao Li*

Main category: cs.LG

TL;DR: 提出一种基于广义Onsager原理的介观动力学学习框架，在数学约束假设类中实现理论保证与数据驱动建模。


<details>
  <summary>Details</summary>
Motivation: 传统科学建模处理复杂系统（如多尺度系统）时面临巨大挑战，需要更有效且可解释的范式。

Method: 基于广义Onsager原理构建统一框架，涵盖耗散和保守介观动力学，在训练前提供全局适定性、稳定性、可识别性和能量耗散等理论保证，再利用数据从假设类中识别具体模型。

Result: 在连续PDE模型和微观链模型数据上验证，框架能准确学习动力学并给出可解释的物理诊断。

Conclusion: 该框架不仅作为有效的动力学学习器，还能提供基础物理的可解释诊断，具有普适性和实用性。
...
### [252] [The Privacy Price of Tail-Risk Learning: Effective Tail Sample Size in Differentially Private CVaR Optimization](https://arxiv.org/abs/2605.16219)
*El Mustapha Mansouri*

Main category: cs.LG

TL;DR: 差分隐私改变了CVaR学习的有效样本量，并给出了完整的风险分解和最优收敛速率。


<details>
  <summary>Details</summary>
Motivation: 需要理解差分隐私对CVaR学习的影响，特别是隐私预算如何改变有效样本量并影响学习性能。

Method: 通过风险分解将私人CVaR超额风险分为普通尾风险统计误差和隐私代价，并分别对标量估计、有限类以及凸Lipschitz学习进行上下界分析。

Result: 标量估计的速率为Θ(B min{1,(nτ)^{-1/2}+(ε nτ)^{-1}})，有限类的速率为Θ(B min{1,√(log(2M)/(nτ))+log(2M)/(ε nτ)})，凸Lipschitz学习中隐私项必须缩放为1/(ε nτ)。

Conclusion: 私人CVaR学习的核心困难子问题等价于在Θ(nτ)个信息性尾记录上进行普通私人学习。
...
### [253] [Layer Equivalence Is Not a Property of Layers Alone: How You Test Redundancy Changes What You Find](https://arxiv.org/abs/2605.16234)
*Gabriel Garcia*

Main category: cs.LG

TL;DR: 研究者发现，衡量Transformer层等效性的替换测试和互换测试结果不一致，导致剪枝风险评估偏差，建议在移除或合并层前同时评估两种KL散度。


<details>
  <summary>Details</summary>
Motivation: 现有工作常混淆替换和互换两种层等效性测试，但两者在预训练Transformer中结果可能不一致，影响层剪枝安全性评估。

Method: 在Pythia和Qwen、Llama等模型上测量替换与互换KL散度，对比协议差距，并分析剪枝代价。

Result: 协议差距随训练增大；Qwen3-8B中互换引导剪枝更安全，Llama-3.1-8B中两种协议剪枝代价相当，但KL不等。

Conclusion: 层移除或合并前应同时评估两种互换KL散度，仅需无标签前向传播即可诊断。
...
### [254] [Dynamics-Level Watermarking of Flow Matching Models with Random Codes](https://arxiv.org/abs/2605.16239)
*Shuchan Wang*

Main category: cs.LG

TL;DR: 提出一种将水印嵌入生成模型动态层面的方法，通过随机编码实现，不影响生成质量。


<details>
  <summary>Details</summary>
Motivation: 为生成模型提供可追溯且不易移除的水印方案。

Method: 将水印嵌入流匹配模型的速度场，训练时添加密钥依赖的扰动，检测时从黑盒查询恢复消息。

Result: 在MNIST和CIFAR-10上可靠恢复消息，保持生成质量，无密钥时解码准确率接近随机。

Conclusion: 该方法有效实现了水印嵌入与提取，且不影响生成分布。
...
### [255] [Fairness-Aware Retrieval Optimization for Retrieval-Augmented Generation](https://arxiv.org/abs/2605.15790)
*Yingqi Zhao,Vasilis Efthymiou,Jyrki Nummenmaa,Kostas Stefanidis*

Main category: cs.DB

TL;DR: 提出公平感知检索框架FARO，缓解RAG中的生成偏差。


<details>
  <summary>Details</summary>
Motivation: RAG检索引入偏差影响生成，需控制。

Method: 结合重排注入、位置感知传播模型和优化公式，提出FARO。

Result: 有效缓解生成偏差，保持相关性。

Conclusion: 为RAG系统提供公平感知检索的原则性方法。
...
### [256] [To GPU or Not to GPU: Vector Search in Relational Engines](https://arxiv.org/abs/2605.15957)
*Vasilis Mageirakos,Joel Andr\'e,Marko Kabi\'c,Bowen Wu,Yannis Chronis,Gustavo Alonso*

Main category: cs.DB

TL;DR: 研究了数据库引擎中GPU集成向量搜索的设计，通过扩展TPC-H基准和模块化引擎实验，发现关系组件比向量搜索更受益于GPU，并提出了优化索引使GPU向量搜索更具竞争力。


<details>
  <summary>Details</summary>
Motivation: 现有数据库引擎在CPU上实现向量搜索，而AI/ML/LLM主要使用GPU，质疑GPU集成是否更优。

Method: 扩展TPC-H基准加入向量数据，提出代表性SQL+VS查询；开发模块化执行引擎支持CPU/GPU；进行多种配置实验。

Result: 关系组件在GPU上受益更大；向量搜索涉及数据移动时GPU非最优；通过优化索引减少大小使GPU向量搜索更高效。

Conclusion: 优化后，关系与向量搜索组件在GPU上均更快，尤其快速互连，优于现有引擎架构。
...
### [257] [Relational Database Data Lineage Ontology](https://arxiv.org/abs/2605.16068)
*Jakub Dutkiewicz,Pawe{\l} Misiorek,Robert Wrembel*

Main category: cs.DB

TL;DR: 提出一种用于关系数据库数据血统的富语义本体，通过知识图谱扩展结构、语义和转换特征，提升血统链接预测性能。


<details>
  <summary>Details</summary>
Motivation: 关系数据库中的数据血统建模面临依赖缺失等挑战，现有语义表示不够丰富，难以支持有效的血统发现。

Method: 在先前基于知识图谱的血统发现工作基础上，扩展本体以涵盖结构、语义和转换层面特征，并采用基于路径嵌入的图神经网络进行归纳链接预测。

Result: 实验表明，与原始本体相比，使用新本体在AUC和Hits@10指标上血统链接预测性能均有提升。

Conclusion: 富语义本体能够更精确编码血统证据，从而改善知识图谱驱动的数据血统发现效果。
...
### [258] [Towards Foundation Models for Relational Databases with Language Models and Graph Neural Networks](https://arxiv.org/abs/2605.16085)
*Jingcheng Wu,Ratan Bahadur Thapa,Mojtaba Nayyeri,Lucas Etteldorf,Max Finkenbeiner,Fabian Leeske,Steffen Staab*

Main category: cs.DB

TL;DR: 提出混合LM-GNN架构，融合BART语义编码与GraphSAGE关系上下文，在RelBench任务上取得竞争性结果，但距先进基础模型仍有差距。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习方法扁平化数据库丢失关系上下文，关系深度学习方法任务特定且数据库相关，需结合两者优势构建通用模型。

Method: 将BART作为行语义编码器，通过GraphSAGE在关系实体图上传播消息，注入关系上下文，实现轻量混合架构。

Result: 在driver-dnf任务上ROC-AUC达67.40，接近LightGBM（68.86），与RDL（72.62）差距5.22点，但与KumoRFM（82.63）差距大。

Conclusion: 轻量混合LM-GNN架构在资源效率和性能间取得平衡，是迈向关系数据库基础模型的有前景路径。
...
### [259] [Autonomous Intelligent Agents for Natural-Language-Driven Web Execution with Integrated Security Assurance](https://arxiv.org/abs/2605.15281)
*Vinil Pasupuleti,Siva Rama Krishna Varma Bayyavarapu,Shrey Tyagi*

Main category: cs.CR

TL;DR: 提出一个AI驱动的自主测试框架，通过五项集成策略和容器化架构，显著提升Web测试脚本的生成成功率和稳定性，并扩展至自然语言驱动的安全测试。


<details>
  <summary>Details</summary>
Motivation: 现代Web测试套件容易腐烂：UI重构破坏定位符、时序变化引发竞态条件，导致开发者放弃套件。

Method: 采用五项集成策略：导航可靠性、上下文感知选择器生成、生成后验证、智能等待注入、失败学习；并实现容器化工作节点架构，将编排与长时浏览器执行解耦。

Result: 在四个生产应用和176个场景中，脚本生成成功率从55%提升至93%，导航失败减少8倍，时序相关竞态条件消除80%，测试创建时间比手动Selenium减少75%；安全测试中检测到85%的认证绕过漏洞和95%的输入验证缺陷，假阳性率低于12%。
...
### [260] [Hidden in Memory: Sleeper Memory Poisoning in LLM Agents](https://arxiv.org/abs/2605.15338)
*Sidharth Pulipaka,Stanislau Hlebik,Leonidas Raghav,Sahar Abdelnabi,Vyas Raina,Ivaxi Sheth,Mario Fritz*

Main category: cs.CR

TL;DR: 大型语言模型通过持久记忆实现个性化，但存在安全漏洞，攻击者可植入虚假记忆并在后续会话中利用。


<details>
  <summary>Details</summary>
Motivation: 持久记忆虽能提升助手个性化，但也成为对抗性内容攻击的新载体，可能长期影响未来交互。

Method: 提出潜伏记忆投毒攻击，通过操纵外部上下文（如文档、网页）引导模型存储虚假用户记忆，评估写入、检索及后续会话中的利用效果。

Result: 在GPT-5.5上投毒成功率达99.8%，Kimi-K2.6达95%；成功检索后，60-89%的评估中虚假记忆导致攻击者意图的代理行为。

Conclusion: 持久记忆成为跨多会话的长期攻击面，需加强安全防御。
...
### [261] [Detecting Privilege Escalation in Polyglot Microservices via Agentic Program Analysis](https://arxiv.org/abs/2605.15569)
*Penghui Li,Hong Yau Chong,Yinzhi Cao,Junfeng Yang*

Main category: cs.CR

TL;DR: 提出Neo框架，结合大语言模型与经典程序分析，自动检测微服务中的权限提升漏洞，在25个开源应用中发现了24个零日漏洞，精确率81%，召回率85%。


<details>
  <summary>Details</summary>
Motivation: 微服务架构带来复杂的权限控制，跨服务交互、多语言代码库和多样的权限操作使得权限提升漏洞检测极具挑战，现有方法效果有限。

Method: Neo结合LLM驱动的智能体动态生成分析计划、调整代码搜索策略并验证语义，开发可跨服务、跨语言扩展的代码搜索原语，实现灵活高效的代码探索。

Result: 在涵盖7种编程语言、620万行代码的25个开源微服务应用中，发现24个零日权限提升漏洞；在基准数据集上精确率81%、召回率85%，检测准确性和可扩展性优于现有方案；扩展到其他应用和漏洞类型时额外发现18个零日漏洞。

Conclusion: Neo有效解决了微服务权限提升漏洞检测难题，证明了LLM与经典程序分析结合的潜力，并具有良好的可扩展性。
...
### [262] [Who Owns This Agent? Tracing AI Agents Back to Their Owners](https://arxiv.org/abs/2605.16035)
*Ruben Chocron,Doron Jonathan Ben Chayim,Eyal Lenga,Gilad Gressel,Alina Oprea,Yisroel Mirsky*

Main category: cs.CR

TL;DR: 提出了智能体归因问题，并设计了基于金丝雀的协议，通过注入不可抑制的金丝雀来可靠地追溯智能体到部署账户。


<details>
  <summary>Details</summary>
Motivation: 缺乏可靠方法将有害智能体行为追溯到部署账户，导致善意和恶意运营商均可逃避责任。

Method: 授权方注入金丝雀到智能体交互流，供应商在会话日志中搜索以识别原始会话和账户；对抗场景使用鲁棒金丝雀，其抑制会损害任务性能。

Result: 在多种场景（包括真实智能体）中验证了方法的可靠性、鲁棒性和可扩展性。

Conclusion: 该协议可部署于供应商端，有效弥合智能体归因差距。
...
### [263] [Enabling Adversarial Robustness in AI Models through Kubeflow MLOps](https://arxiv.org/abs/2605.15249)
*Stavros Bouras,Ioannis Korontanis,Antonios Makris,Konstantinos Tserpes*

Main category: cs.CR

TL;DR: 提出一种在Kubernetes集群中部署AI模型的安全架构，集成Kubeflow MLOps，自动检测推理阶段的对抗攻击并触发基于PGD的防御，实验表明能恢复模型准确性。


<details>
  <summary>Details</summary>
Motivation: 云原生环境中AI模型部署缺乏专门的安全机制，现有平台如Kubernetes未提供针对模型攻击的防护。

Method: 基于Kubeflow MLOps集成，在推理阶段使用FGSM检测攻击，当精度下降时自动部署PGD对抗训练防御。

Result: 防御显著恢复模型准确性，缓解FGSM攻击导致的性能下降。

Conclusion: 该架构能有效提升部署在Kubernetes中的AI模型对抗攻击的鲁棒性。
...
### [264] [A Multi-Layer Cloud-IDS Pipeline with LLM and Adaptive Q-Learning Calibration](https://arxiv.org/abs/2605.15889)
*Syed Waqas Ali,Ibrar Ali Shah,Farzana Zahid,Daniyal Munir,Hans D. Schotten*

Main category: cs.CR

TL;DR: 提出一种基于强化学习的置信度感知多级入侵检测系统，通过门控机制和LLM降低误报和成本，提升云环境安全。


<details>
  <summary>Details</summary>
Motivation: 云环境复杂，现有IDS依赖机器学习模型在真实部署中性能下降，且无法处理未知攻击。

Method: 实现三层检测（网络、主机、超管），使用强化学习学习自适应阈值，置信度门控（Gate-1）和Chroma记忆匹配（Gate-2）过滤低置信事件，未解决事件由LLM分析（Gate-3），最终攻击提升使用校准LLM置信度或加权融合，不确定事件保留到审查桶。

Result: LLM调用减少58.78%，准确率88.68%，精确率85.29%，召回率84.72%，F1 85.00%，网络层准确率98.02%，超管层97.08%。

Conclusion: 系统在保持强性能的同时显著降低LLM成本，实现平衡高效的检测。
...
### [265] [TFZ-Tree: An Ultra-Lightweight Waveform Classification Framework for Resource-Constrained Devices](https://arxiv.org/abs/2605.15656)
*Hao Wang,Kuang Zhang,Yonggang Chi,Tianqi Zhao,Yanbo Fu,Jiaxing Guo*

Main category: eess.SP

TL;DR: 提出超轻量波形分类框架，基于时频特征和Z检验树，在10种6G波形上达99.5%准确率，单次推理<4ms。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对物理层波形类型的直接识别，且依赖深度网络和复杂变换，难以部署于资源受限终端。

Method: 低复杂度时域特征提取，结合Z统计检验优化的决策树（ZTree）进行分类。

Result: AWGN下平均准确率99.5%，多径信道87.4%，主要混淆在OTFS和LoRa；x86平台C实现，延迟<4ms。

Conclusion: 首次实现十种物联网波形实时识别，未来将部署于嵌入式MCU。
...
### [266] [A Few GPUs, A Whole Lotta Scale: Faithful LLM Training Emulation with PrismLLM](https://arxiv.org/abs/2605.15617)
...
TL;DR: PrismLLM通过切片和混合模拟，用少量GPU高保真模拟大规模LLM训练，迭代时间误差0.58%，内存误差<0.01%，可模拟8192 GPU集群。


<details>
  <summary>Details</summary>
Motivation: 大型LLM训练依赖数千GPU集群，但调试和性能优化需要频繁访问集群，成本高且资源紧张，现有仿真和缩比实验难以准确复现规模相关行为。

Method: 构建基于切片的高保真执行图，捕获目标规模的计算、通信和依赖；执行混合模拟，选中秩运行原始程序，其余秩作为虚拟参与者回放。

Result: 在迭代时间上平均误差0.58%，峰值GPU内存误差小于0.01%；能用不到1%的物理GPU模拟多达8192 GPU的集群。

Conclusion: PrismLLM有效解耦大规模执行与访问大集群的需求，使得工程师能用少量GPU开发和调试大规模训练框架，降低了成本和复杂性。
...
### [267] [Designing Datacenter Power Delivery Hierarchies for the AI Era](https://arxiv.org/abs/2605.16255)
*Grant Wilkins,Fiodar Kazhamiaka,Alok Gautam Kumbhare,Chaojie Zhang,Ricardo Bianchini*

Main category: cs.DC

TL;DR: 随着AI加速器需求增加，数据中心功率密度快速上升，导致电力搁浅问题。该文提出了一个评估框架，结合微软Azure数据，量化了多资源搁浅对可部署容量、资本支出和性能的影响，指出规划目标应是随时间可部署的容量而非安装功率。


<details>
  <summary>Details</summary>
Motivation: AI加速器使机架功率密度急剧增加，传统数据中心设计可能因电力搁浅而浪费资源，需要评估不同设计在长期内的效率。

Method: 开发了一个评估框架，结合GPU、计算和存储部署的投影模型与微软Azure的生产数据，通过吞吐量、功率和成本指标分析电力交付设计。

Result: 多资源搁浅显著改变了可部署容量、有效资本支出和交付性能，并且不断上升的密度（来自机架和吊舱级AI系统）加剧了这些影响。

Conclusion: AI数据中心设计应以随时间推移的可部署容量为目标，而非简单的安装兆瓦数。
...
### [268] [ADAPT: A Self-Calibrating Proactive Autoscaler for Container Orchestration](https://arxiv.org/abs/2605.15788)
*Himanshu Singh Baghel*

Main category: cs.DC

TL;DR: 提出ADAPT在线估计冷启动时间，并集成到MPC控制器中实现主动弹性伸缩，显著降低SLA违反率。


<details>
  <summary>Details</summary>
Motivation: 容器化工作负载的主动弹性伸缩依赖于准确的冷启动时间估计，但实际中冷启动时间变化大，需要自适应方法。

Method: 设计ADAPT在线EWMA估计器跟踪冷启动时间，并结合FH-OPT动态规划视界输入MPC控制器，形成闭环自适应伸缩。

Result: 在三种策略和六种工作负载下，MPC+LSTM策略在所有负载上SLA违反率低于5%，优于HPA（7-19%）和MPC+Prophet（最高28.7%）。

Conclusion: 自适应冷启动时间估计能有效提升主动弹性伸缩性能，MPC+LSTM组合最适合多变负载。
...
### [269] [Runtime-Orchestrated Second-Order Optimization for Scalable LLM Training](https://arxiv.org/abs/2605.16184)
*Yishun Lu,Junhao Zhang,Zeyu Yang,Wes Armour*

Main category: cs.DC

TL;DR: Asteria是一个运行时系统，通过分离二阶优化逻辑、动态分配状态、异步背景计算和有限陈旧性协议，使二阶LLM训练在系统层面变得实用。


<details>
  <summary>Details</summary>
Motivation: 二阶优化方法可提高LLM训练样本效率，但实际使用被维护大型矩阵优化器状态的系统成本阻碍。

Method: 将优化器状态动态分布在GPU、CPU和NVMe上；利用训练钩子预准备影子状态并在主机上异步计算昂贵的逆根；采用拓扑感知的有限陈旧性协议进行分布式同步。

Result: 在DGX Spark上支持1B参数模型的二阶训练；在多节点GH200上降低优化器开销、减少延迟峰值、加速壁钟时间收敛，并保持SOAP和KL-Shampoo的优化优势。

Conclusion: 二阶LLM训练的实用化不仅依赖于简化优化器，更需要在运行时层面重新管理优化器状态、背景计算和分布式同步。
...
### [270] [Effective Harness Engineering for Algorithm Discovery with Coding Agents](https://arxiv.org/abs/2605.15221)
*Yoichi Ishibashi,Taro Yano,Masafumi Oyamada*

Main category: cs.SE

TL;DR: 该论文研究执行基础设施（harness）设计如何影响基于大型语言模型的算法自动发现，发现每个算法更深入的思考优于生成大量浅层算法，且更强模型产生更多评估漏洞。


<details>
  <summary>Details</summary>
Motivation: 算法发现的成功不仅取决于模型能力，还显著受harness设计影响，但影响尚不明确。作者针对固定token预算下的策略、评估漏洞处理及安全并行执行三个问题展开研究。

Method: 使用Vesper框架（包含harness改进），在固定token预算下评估圆形填充问题，比较生成大量浅层思考算法与少量深度思考算法的效果，并分析评估漏洞。

Result: 生成更少量但思考更深入的算法获得更高分数；更强模型以更高频率产生评估漏洞，使漏洞检测需求随模型规模增长。

Conclusion: 提升每个个体的质量比增加进化代数更具预算效率；随着模型扩展，漏洞检测变得愈发重要。
...
### [271] [PBT-Bench: Benchmarking AI Agents on Property-Based Testing](https://arxiv.org/abs/2605.15229)
*Lucas Jing,Xinqi Wang,Liao Zhang,Simon S. Du*

Main category: cs.SE

TL;DR: 提出PBT-Bench，包含100个基于属性的测试问题，评估LLM从文档推导不变量并生成输入策略的能力，发现假设脚手架对中等模型提升显著，但最强模型提升有限。


<details>
  <summary>Details</summary>
Motivation: 现有代码基准无法隔离基于属性测试的独特技能：从文档推导语义不变量并构建输入生成策略。

Method: 构建包含100个问题（365个语义错误）的PBT-Bench基准，覆盖40个Python库，要求模型阅读文档、识别不变量并指定Hypothesis @given策略，评估8个LLM在两种提示下的表现。

Result: PBT引导提示下错误召回率42.1%-83.4%，开放式基线31.4%-76.7%；假设脚手架提升中等模型20+个百分点，但最强模型提升较小，两个模型出现退化。

Conclusion: PBT-Bench能有效评估文档基础语义推理能力，最难的问题因模型而异，无单一模型能解决所有问题，并发布基准供后续研究。
...
### [272] [Hydra: Efficient, Correct Code Generation via Checkpoint-and-Rollback Support](https://arxiv.org/abs/2605.15238)
*Alexander Du,Jianjun Ou,Danyang Zhuo,Matthew Lentz*

Main category: cs.SE

TL;DR: Hydra系统通过在代码生成过程中异步检查并提供检查点回滚修复，将静态错误的修复延迟降低最多71%，令牌消耗降低最多70%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型生成的代码常出现编译错误，现有修复方法（事后修复、约束语义解码）延迟高、令牌消耗大，且可能重新生成有效代码。

Method: Hydra允许检查与生成异步进行，避免正确代码的检查开销；提供检查点-回滚机制，仅修复错误部分而不重新生成有效前缀；改造Clang C/C++编译器以支持该机制。

Result: 在C/C++代码生成任务中，Hydra相比事后修复，延迟降低最多71%，令牌消耗降低最多70%。

Conclusion: Hydra能高效地从静态错误中恢复，显著提升代码生成效率。
...
### [273] [Runtime-Structured Task Decomposition for Agentic Coding Systems](https://arxiv.org/abs/2605.15425)
*Shubhi Asthana,Bing Zhang,Chad DeLuca,Hima Patel,Ruchi Mahindru*

Main category: cs.SE

TL;DR: 提出运行时结构化任务分解方法，通过可执行控制逻辑管理任务流程，LLM仅用于判断任务，输出经模式验证，显著降低重试代价。


<details>
  <summary>Details</summary>
Motivation: 现有Agentic编码系统将任务逻辑、执行流和输出生成编码在大型提示中，导致行为脆弱、可调试性差、重试成本高。

Method: 运行时结构化任务分解：任务划分和流程由可执行控制逻辑管理，LLM仅用于专注的判断任务，输出在向下游执行前根据预定义模式验证。

Result: 在Kubernetes根因分析中，重试代价为436±132 tokens（单体904，静态1632）；多文件调试中为460 tokens（单体703，静态933）。相比单体系统降低51.7%，相比静态分解降低73.2%。

Conclusion: 该方法提升了Agentic编码系统的效率、可调试性和运行可靠性。
...
### [274] [Position: Early-Stage Quality Assurance in Annotation Pipelines Is More Cost-Effective Than Late-Stage Validation](https://arxiv.org/abs/2605.15714)
...
TL;DR: 本文主张机器学习社区应将注释管道的早期质量保证置于后期验证之上，并提出了三种QA触发点分类和参数化误差传播模型，揭示了时机对误差率和成本的关键影响。


<details>
  <summary>Details</summary>
Motivation: 数据质量瓶颈日益限制基础模型改进，但现有研究仅聚焦于验证方法而非验证时机，而软件工程中“左移”原则表明早期发现缺陷可大幅降低成本。

Method: 提出三种QA触发点（T0预注释、T1后注释、T2后审核）的分类法，并构建参数化误差传播模型形式化时机对错误率与经济性的影响。

Result: 对47篇近期论文的调查显示仅4%报告验证时机，表明时机这一关键变量被严重忽视。

Conclusion: 社区需采取三步行动：研究者报告QA时机配置、平台将时机作为一级参数、开展控制实验测量阶段检测率。
...
### [275] [RoadmapBench: Evaluating Long-Horizon Agentic Software Development Across Version Upgrades](https://arxiv.org/abs/2605.15846)
...
TL;DR: 提出了RoadmapBench，一个包含115个长期编码任务的基准，涵盖17个仓库和5种编程语言，用于评估AI代理在真实软件升级中的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注单一问题的Python bug修复，缺乏反映真实工程中多目标、长期开发的任务。

Method: 基于真实开源版本升级构建，每个任务提供源版本代码快照和实现目标版本功能的多目标路线图指令，中位数修改3700行代码涉及51个文件。

Result: 最强模型Claude-Opus-4.7仅解决39.1%的任务，最弱模型只有5.2%，与现有bug修复基准形成鲜明对比。

Conclusion: 长期软件开发在很大程度上仍是一个未解决的问题。
...
### [276] [XSearch: Explainable Code Search via Concept-to-Code Alignment](https://arxiv.org/abs/2605.16046)
*Yiming Liu,Ruofan Liu,Yun Lin,Zicong Zhang,Weiyu Kong,Pengnian Qi,Xiao Cheng,Weinan Zhang,Qianxiang Wang,Linpeng Huang*

Main category: cs.SE

TL;DR: 提出可解释代码搜索框架XSearch，通过将代码搜索重构为演绎概念对齐问题，实现概念级解释并提升分布外泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有语义代码搜索方法依赖全局嵌入相似性，缺乏可解释性和泛化能力，尤其在分布偏移下表现差，无法满足查询的 functional requirements。

Method: XSearch 框架识别查询中的功能概念，并显式对齐对应代码语句，使用概念对齐目标训练编码器，通过显式匹配进行检索。

Result: 在 CodeSearchNet 上训练（GraphCodeBERT，125M参数），在分布外基准上性能从0.02提升至0.33（提升15倍），优于包括7B参数在内的多个基线，用户研究表明概念对齐解释使用户更快更准地评估结果。

Conclusion: XSearch 通过内在可解释的概念对齐有效提升代码搜索的泛化能力和可解释性。
...
### [277] [PerfCodeBench: Benchmarking LLMs for System-Level High-Performance Code Optimization](https://arxiv.org/abs/2605.15222)
*Huihao Jing,Wenbin Hu,Haochen Shi,Hanyu Yang,Sirui Zhang,Shaojin Chen,Haoran Li,Yangqiu Song*

Main category: cs.SE

TL;DR: 提出了PerfCodeBench，一个用于评估大语言模型在性能关键代码优化方面能力的可执行基准，发现模型与专家优化之间存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有代码基准主要强调正确性或算法求解，而对现实系统级优化的评估不足。

Method: 构建PerfCodeBench，包含需要系统级实现、硬件感知优化和处理性能瓶颈的任务，每个任务提供正确性检查、基线实现和参考优化方案。

Result: 对多种先进大语言模型的评估显示，模型生成的代码与专家优化代码之间存在明显差距，尤其在并行和GPU任务上，且模型在跨语言鲁棒性和稳定达到专家效率方面表现不足。

Conclusion: 需要引入性能感知的评估，推动大语言模型从生成正确代码转向生成高效系统软件。
...
### [278] [BootstrapAgent: Distilling Repository Setup into Reusable Agent Knowledge](https://arxiv.org/abs/2605.15815)
*Sihan Fu,Oucheng Liu,Shiyuan Wang,Jin Shi,Chengkun Wei*

Main category: cs.SE

TL;DR: 提出BootstrapAgent框架，将仓库引导过程转化为可复用的启动知识，通过多智能体协作生成持久化的.bootstrap合约，显著提升成功率并降低资源消耗。


<details>
  <summary>Details</summary>
Motivation: 代码代理在处理不熟悉仓库时需要昂贵的引导过程，但该过程产生的知识被困在单次对话中，无法复用。

Method: 采用多智能体框架，通过证据提取、结构化规划、确定性Docker验证和轨迹驱动修复，蒸馏出包含环境设置、诊断检查、最小验证和修复知识的.bootstrap合约，并引入热修复与冷启动回放、增量修复与合理性检查。

Result: 在三个基准测试中达到92.9%的成功率，比基线高10%以上，下游代理令牌使用量减少25.9%，构建时间减少22.3%。

Conclusion: BootstrapAgent有效解决了仓库引导知识的复用问题，提高了效率，且代码已开源。
...
### [279] [Evaluating Design Video Generation: Metrics for Compositional Fidelity](https://arxiv.org/abs/2605.16223)
*Adrienne Deganutti,Dingning Cao,Jaejung Seol,Elad Hirsch,Purvanshi Mehta*

Main category: cs.GR

TL;DR: 提出针对设计动画生成视频的全自动评估框架，涵盖布局、运动、时间、内容四个维度。


<details>
  <summary>Details</summary>
Motivation: 缺乏标准化评估框架，而设计动画有结构化约束。

Method: 构建四维度自动评估框架：布局保真度、运动正确性、时间质量、内容保真度。

Result: 消除了主观人类评估，建立了共同基准。

Conclusion: 提供了一个标准化、自动化的评估方法，促进领域进展。
...
### [280] [Unified Simulation of Lagrangian Particle Dynamics via Transformer](https://arxiv.org/abs/2605.15305)
...
TL;DR: 提出一种基于学习的粒子模拟器，使用单一Transformer架构统一模拟布料、弹性固体、牛顿/非牛顿流体、颗粒材料和分子动力学，无需针对不同现象重新设计求解器。


<details>
  <summary>Details</summary>
Motivation: 长期以来，科学模拟的目标是构建能够模拟多种物理现象的统一模拟器，避免为每种现象单独设计求解器。

Method: 采用预测-校正设计，基于共享的拉格朗日粒子表示。显式预测器在已知外力下推进粒子，然后学习校正器通过三阶段（粒子分词器、超令牌编码器（交替自注意力与令牌合并）、超令牌解码器）预测位置和速度的残差更新，渐进式令牌合并降低注意力成本。

Result: 同一架构在六类动力学中泛化到未见过的材料、边界条件、初始条件和外力，并展示了下游交互控制、逆设计和从真实世界数据学习的能力。

Conclusion: 该统一模拟器减少了针对不同现象求解器工程的需要，通过学习实现对多种物理的通用建模。
...
### [281] [FFAvatar: Few-Shot, Feed-Forward, and Generalizable Avatar Reconstruction](https://arxiv.org/abs/2605.15320)
*Thuan Hoang Nguyen,Jiahao Luo,Yinyu Nie,Hao Li,Gordon Guocheng Qian,Jian Wang*

Main category: cs.GR

TL;DR: FFAvatar是一种前馈框架，能从少量未摆姿势的肖像图像中快速重建高质量、可动画化的3D高斯头部头像，实现实时部署。


<details>
  <summary>Details</summary>
Motivation: 传统头像重建需要逐主题优化或昂贵预处理，耗时且难以扩展，而FFAvatar旨在实现快速、可泛化的重建。

Method: 通过多视图查询变换器将多源图像融合为统一的规范高斯表示，并从像素端到端预测FLAME参数以驱动动画，采用三阶段训练课程：大规模单目视频预训练、多视角微调和可选的个性化优化。

Result: 在NeRSemble基准上，PSNR比先进方法LAM提升5.5 dB；未个性化重建仅需2秒，个性化重建10秒，动画速度达49 FPS。

Conclusion: FFAvatar在身份保持、几何一致性和动画保真度方面树立了新标准，支持实时应用。
...
### [282] [StippleDiffusion: Capacity-Constrained Stippling using Controlled Diffusion](https://arxiv.org/abs/2605.15816)
*Ofir Gilad,Aleksander Plocharski,Przemyslaw Musialski,Andrei Sharf*

Main category: cs.GR

TL;DR: 首次提出基于扩散的条件点画采样器，满足局部点分布先验和连续容量约束，性能与逐密度优化方法相当。


<details>
  <summary>Details</summary>
Motivation: 传统点画法基于逐密度迭代优化，速度慢、不可微分，且每次需重新运行；现有学习方法只处理无条件点生成，无法处理容量约束的图像条件点画。

Method: 基于ControlNet架构，在最优传输网格点集扩散基线上构建，以目标密度图和高分辨率图像为条件。限制训练和推理在去噪后期阶段，从密度加权拒绝采样初始化；用sigmoid门控1x1投影替换零卷积注入，保持基模型的蓝噪声结构。

Result: 单一训练检查点接受任意目标密度，泛化到未见点预算，生成时间几乎与点数无关。在Icons-50基准上，每个指标与逐密度优化基线相当。

Conclusion: 实现了可微分的、端到端的条件点画生成，性能与专用优化方法持平。
...
### [283] [Clock-state olfactory search in turbulent flows using Q-learning: The geometry of plume recovery](https://arxiv.org/abs/2605.15938)
*Marco Rando,Robin A. Heinonen,Yujia Qi,Agnese Seminara*

Main category: physics.bio-ph

TL;DR: 使用表格Q学习训练嗅觉搜索智能体，仅用上次嗅到气味后的时间作为记忆，学到类似昆虫的行为策略，但在湍流中缺乏对间歇性的适应能力。


<details>
  <summary>Details</summary>
Motivation: 在湍流中利用嗅觉观测历史找到气味源。

Method: 使用表格Q学习，以“上次嗅到气味后经过的时间”为记忆，训练智能体。

Result: 在直接数值模拟的湍流数据上表现良好，但无法适应局部间歇性；提供更多灵活性可提高鲁棒性。

Conclusion: 该方法有效但需要改进以适应湍流的间歇性。
...
### [284] [Toward Natural and Companionable Virtual Agents via Cross-Temporal Emotional Modeling](https://arxiv.org/abs/2605.15812)
*Feier Qin,Xiao Li,Yi Zheng,Haibin Huang,Hanyao Wang,Xiaoyu Wang,Yan Lu,Yuan Zhang*

Main category: cs.HC

TL;DR: 提出跨时间情感建模框架，增强陪伴代理的自然性和情感连贯性。


<details>
  <summary>Details</summary>
Motivation: 现有代理缺乏长期社交行为和内部情感的跨时间建模，导致互动不自然、不真实。

Method: CTEM框架通过过去经历更新情感状态，情感状态调节即时互动，用户反馈持续修正记忆和情感，形成闭环。

Result: 在即时通讯平台上实现Auri代理，21天野外研究表明CTEM在自然性、连贯性和情感和谐方面有提升。

Conclusion: CTEM能有效提升陪伴代理的长期互动质量。
...
### [285] [SLIP & ETHICS: Graduated Intervention for AI Emotional Companions](https://arxiv.org/abs/2605.15915)
*Minseo Kim*

Main category: cs.HC

TL;DR: 提出SLIP四阶段干预协议和ETHICS信号分类法，通过用户测试和合成人格评估，发现分级干预能在保持安全的同时减少误报，但面临持续高能量状态下的干预缺失问题。


<details>
  <summary>Details</summary>
Motivation: AI情感陪伴系统面临安全性-融洽关系悖论：严格保护措施损害支持关系，宽松系统则可能使用户受到伤害。

Method: 设计SLIP（分层干预协议）四阶段方法，从情感强度(a)和叙事动态(m)两个指标出发，结合ETHICS（人机交互上下文信号分类法）生成无、软、硬三级干预。通过小规模真实用户部署（68条记录，10用户，10周）和合成人格测试（91人格，5行为风险谱系）进行评估。

Result: 流畅人格假阳性率为0%，危机人格表现出预期的干预升级模式。但在连续8天的高能量激发场景中，系统未生成任何干预（0/8），暴露出“不病理化”原则与安全性之间的冲突。后续三模型压力测试显示，增大模型能力可将检测率提升至6/8，同时最大模型在流畅人格上保持0假阳性。

Conclusion: 该研究将分级干预定位为应对情感计算中安全性-融洽关系紧张的设计方向，而非彻底解决方案。
...
### [286] [Conversations in Space: Structuring Non-Linear LLM Interactions on a Canvas](https://arxiv.org/abs/2605.15848)
*Rifat Mehreen Amin,Alperen Adatepe,Daniela Fernandes,Daniel Buschek,Andreas Butz*

Main category: cs.HC

TL;DR: 提出CanvasConvo，将线性聊天转换为分支对话树嵌入空间画布，支持探索性工作流程，并通过24人5-7天实地研究验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型驱动的对话界面为线性结构，限制了替代方案的探索和长时间交互的管理。

Method: 设计CanvasConvo概念，通过从对话内容直接分支形成对话树，在空间画布上可视化，同时保留聊天界面，支持线性与非线性的切换，并集成时间线导航、自动标签摘要、情境感知控制等功能。

Result: 在24名参与者的5-7天实地研究中，发现非线性对话结构支持探索性工作流程，并影响了基于LLM工作的不同交互模式。

Conclusion: 非线性对话结构能够有效支持探索性工作流程，为基于LLM的交互提供新的可能性。
...
### [287] [From Feedback Loops to Policy Updates: Reinforcement Fine-Tuning for LLM-Based Alpha Factor Discovery](https://arxiv.org/abs/2605.15412)
*Lingzhe Zhang,Tong Jia,Yunpeng Zhai,Zixuan Xie,Chiming Duan,Minghua He,Philip S. Yu,Ying Li*

Main category: cs.CE

TL;DR: 提出一种基于强化微调的自我进化alpha因子发现框架QuantEvolver，通过将评估转换为策略更新，避免上下文爆炸和搜索停滞。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法依赖提示级生成-评估-反馈循环，导致上下文爆炸、成本高、信息稀释和搜索停滞。

Method: 构建高质量种子因子和时间窗口任务，生成可执行因子表达式，通过机制回测评估，并使用多样性互补奖励优化Miner LLM参数。

Result: 在三个真实市场基准上优于现有LLM方法，持续提升评估指标，生成更高质量和互补的因子池。

Conclusion: QuantEvolver有效解决了因子发现中的上下文爆炸和搜索停滞问题，提升了因子质量和多样性。
...
### [288] [Agent4POI: Agentic Context-Conditioned Affordance Reasoning for Multimodal Point-of-Interest Recommendation](https://arxiv.org/abs/2605.15203)
*Jinze Wang,Yangchen Zeng,Tiehua Zhang,Lu Zhang,Yuze Liu,Yongchao Liu,Xingjun Ma,Zhu Sun*

Main category: cs.IR

TL;DR: 提出Agent4POI框架，在推荐时动态生成上下文相关的多模态表示，而非使用静态POI嵌入，显著提升推荐性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用静态POI嵌入，无法处理同一POI在不同情境下的意义变化，理论上证明预计算编码器无法满足上下文敏感排序。

Method: 四阶段LLM代理：生成动态可供性查询（阶段1），跨模态思维链推理（阶段2），构建不确定性感知可供性表示（阶段3），语义缓存对齐用户偏好（阶段4）。

Result: 三个基准上相对最强基线提升23.2%，上下文偏移仅下降7.5%（基线下降16-17%），冷启动场景性能提升2.4倍。

Conclusion: 动态上下文相关表示优于静态嵌入，尤其在上下文敏感和冷启动场景中表现突出。
...
### [289] [An LLM-RAG Approach for Healthy Eating Index-Informed Personalized Food Recommendations](https://arxiv.org/abs/2605.15213)
*Yibin Wang,Yanjie Yang,Grace Melo Guerrero,Rodolfo M. Nayga Jr.,Azlan Zahid*

Main category: cs.IR

TL;DR: 提出一种基于健康饮食指数(HEI)的检索增强生成框架，结合标准化营养数据库和大语言模型，实现个性化食物推荐，显著提升饮食质量。


<details>
  <summary>Details</summary>
Motivation: 现有食物推荐系统依赖粗略数据库，与已验证的饮食指数关联有限，缺乏精确性和个性化。

Method: 构建基于NHANES和FPED的食物嵌入空间，计算基线HEI分数，检索候选食物并估计替换或添加的HEI影响，通过约束RAG管道结合预训练LLM生成个性化推荐。

Result: 模拟显示平均HEI提升6.45，HEI>50的用户比例从45.12%增至61.26%，分位数分析表明整个HEI分布一致改善。

Conclusion: 所提出的LLM-RAG系统能支持更精确、可解释和个性化的营养指导，改善饮食质量。
...
### [290] [Fortress: A Case Study in Stabilizing Search Recommendations via Temporal Data Augmentation and Feature Pruning](https://arxiv.org/abs/2605.15299)
...
TL;DR: Fortress是一个通用框架，通过识别和修剪随时间引发预测不一致的特征，增强模型稳定性和准确性。


<details>
  <summary>Details</summary>
Motivation: 搜索和推荐系统中，预测模型因输入特征导致的时间不稳定性会降低模型可靠性和用户体验，尤其是在多阶段系统中。

Method: 收集历史快照、识别不稳定样本、移除不稳定特征、使用稳定特征重训练模型。

Result: 在大型应用市场的查询到应用相关性模型中，离线实验显示预测稳定性（变异系数）和分类性能（PR-AUC）显著提升。

Conclusion: Fortress通过抑制波动性同时保留预测价值，实现了更稳定和准确的模型。
...
### [291] [Differentially Private Motif-Preserving Multi-modal Hashing](https://arxiv.org/abs/2605.15460)
*Zehua Cheng,Wei Dai,Jiahao Sun*

Main category: cs.IR

TL;DR: 提出一种隐私保护跨模态哈希方法DMP-MH，通过度裁剪和噪声镜像下降生成差分隐私图，再蒸馏到哈希网络，在保护图结构隐私的同时保持了高性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态哈希依赖用户交互生成的语义相似性图，但图结构易受链路重构攻击；现有隐私方法因度分布无界导致敏感度过大（Hubness Explosion问题），需要过大的噪声。

Method: 提出Sanitize-then-Distill框架：先确定性裁剪节点度以界敏感度，再用噪声镜像下降生成差分隐私合成图，最后用整体结构损失蒸馏到双流哈希网络。

Result: 在MIRFlickr-25K和NUS-WIDE上，DMP-MH比隐私基线高最多11.4 mAP，保留非隐私性能的92.5%。

Conclusion: DMP-MH有效平衡了图结构隐私与跨模态检索性能，解决了Hubness Explosion问题。
...
### [292] [Generative Long-term User Interest Modeling for Click-Through Rate Prediction](https://arxiv.org/abs/2605.15905)
*Jiangli Shao,Kaifu Zheng,Hao Fang,Huimu Ye,Zhiwei Liu,Bo Zhang,Shu Han,Xingxing Wang*

Main category: cs.IR

TL;DR: 提出GenLI模型，通过生成式方法建模长期用户兴趣，提升点击率预测的准确性与效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅关注目标相关行为，忽略其他潜在兴趣，且匹配式检索耗时且忽略行为间交互。

Method: GenLI包含兴趣生成模块（IGM）、行为检索模块（BRM）和兴趣融合模块（IFM）。IGM生成多样化兴趣分布；BRM通过查找操作实现O(1)检索；IFM使用门控机制融合兴趣特征。

Result: 提高兴趣多样性，避免复杂匹配检索，在准确性和效率间取得更好平衡。

Conclusion: GenLI通过生成式框架有效解决了长期兴趣建模中的偏差和效率问题。
...
### [293] [PhysBrain 1.0 Technical Report](https://arxiv.org/abs/2605.15298)
...
TL;DR: PhysBrain 1.0 通过从人类自我中心视频中提取结构化物理常识，并将其迁移到视觉-语言-动作模型，显著提升了机器人的物理理解和跨域控制能力。


<details>
  <summary>Details</summary>
Motivation: 机器人轨迹数据覆盖有限，不足以学习广泛的物理理解，因此探索从大规模人类视频中获取结构化物理常识的互补路线。

Method: 设计数据引擎从视频中提取场景元素、空间动态、动作执行和深度关系，转化为问答监督训练物理常识 VLM，再通过能力保持和语言敏感适应将先验迁移到 VLA 策略。

Result: 在多模态 QA 和具身控制基准（ERQA、PhysBench、SimplerEnv-WidowX、LIBERO、RoboCasa）上取得 SOTA，尤其在 SimplerEnv 上表现优异。

Conclusion: 从人类交互视频中扩展物理常识能够有效桥接多模态理解与机器人动作，提升泛化性能。
...
### [294] [HoloMotion-1 Technical Report](https://arxiv.org/abs/2605.15336)
*Maiyue Chen,Kaihui Wang,Bo Zhang,Xihan Ma,Zhiyuan Yang,Yi Ren,Qijun Huang,Zihao Zhu,Yucheng Wang,Zhizhong Su*

Main category: cs.RO

TL;DR: 提出HoloMotion-1，一种利用大规模混合运动数据（包括视频重建、动作捕捉和室内数据）进行训练的人形运动基础模型，实现了零样本全身运动跟踪，并在真实机器人上直接部署。


<details>
  <summary>Details</summary>
Motivation: 传统基于动作捕捉训练的运动模型受限于数据多样性和场景覆盖，难以泛化到真实世界的多样化运动。HoloMotion-1旨在通过大规模异构数据源（尤其是自然视频）扩展策略训练的规模，从而提升零样本泛化能力。

Method: 采用混合运动语料库（视频重建数据为主，动作捕捉和室内数据为辅）；集成大容量时间建模、稀疏激活的Mixture-of-Experts Transformer（含KV缓存推理实现实时控制）和序列级训练策略，以应对异构数据带来的噪声、域不匹配和质量差异等挑战。

Result: 在多个未见过的运动基准上，HoloMotion-1在不同运动类型和捕捉条件下均展现出稳健的泛化性能，跟踪精度显著优于先前方法，并可直接转移到真实人形机器人上，无需任务特定微调。

Conclusion: 大规模混合运动数据训练结合专门设计的架构和训练策略，使人形运动模型能够实现零样本全身跟踪，并有效桥接仿真与真实世界的部署鸿沟。
...
### [295] [Residual Reinforcement Learning for Robot Teleoperation under Stochastic Delays](https://arxiv.org/abs/2605.15480)
*Kaize Deng,Zewen Yang*

Main category: cs.RO

TL;DR: 提出一种延迟鲁棒强化学习框架，结合LSTM状态估计与残差强化学习策略，应对随机通信延迟下的遥操作控制问题。


<details>
  <summary>Details</summary>
Motivation: 随机通信延迟导致信号不连续，破坏控制稳定性，传统强化学习方法因延迟观测产生高频抖动。

Method: 集成LSTM状态估计器重建连续状态估计，结合残差强化学习策略学习扭矩补偿策略，平衡跟踪精度与速度平滑性。

Result: 在Franka Panda机器人上实验，该方法显著优于现有基线，在高方差随机延迟下也能保证遥操作的鲁棒性和稳定性。

Conclusion: 所提混合控制框架有效解决了随机延迟下的遥操作稳定性和性能问题。
...
### [296] [Hybrid LLM-based Intelligent Framework for Robot Task Scheduling](https://arxiv.org/abs/2605.15486)
*Swayamjit Saha,Subhabrata Das,Haonan Duan,Xiao-Yang Liu*

Main category: cs.RO

TL;DR: 提出利用大语言模型（LLM）改进建筑机器人任务调度的智能框架，使用双代理（生成器和监督器）实现实时自适应调度，提高效率和资源利用率。


<details>
  <summary>Details</summary>
Motivation: 建筑机器人任务调度需要优化时间效率和资源利用，传统方法难以适应实时变化，LLM可提供更精确的调度方案。

Method: 将任务关键数据（如代理动作能力、目标）输入LLM，开发平衡分配策略；使用NLP接口与建筑专业人员交互；同时使用生成器（GPT-4）和监督器（Gemma 3/Llama 4/Mistral 7b）两个LLM代理生成任务计划。

Result: 通过简单场景评估，指标分数证明了框架的有效性。

Conclusion: LLM在建筑运营任务（包括机器人）中至关重要。
...
### [297] [SkiP: When to Skip and When to Refine for Efficient Robot Manipulation](https://arxiv.org/abs/2605.15536)
*Mingtong Dai,Guanqi Peng,Yongjie Bai,Feng Yan,Chunjie Chen,Lingbo Liu,Liang Lin,Xinyu Wu*

Main category: cs.RO

TL;DR: 提出Skip Policy (SkiP) 通过动作重标记机制，在自由空间段跳过冗余步骤，仅在关键段密集预测，减少执行步数并提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有模仿学习策略在每个控制步统一预测，自由空间段浪费计算，而关键接触段需要高分辨率预测。

Method: 提出动作重标记：在跳跃段每一步用下一个关键段入口动作替换行为克隆目标；以及Motion Spectrum Keying (MSK) 自动划分关键/跳跃段。

Result: 在72个模拟任务和3个真实机器人任务上，执行步数减少15–40%，成功率持平或提升。

Conclusion: SkiP 在单一网络中动态跳跃，无需额外规划器或层次结构，有效提升效率。
...
### [298] [Feedback World Model Enables Precise Guidance of Diffusion Policy](https://arxiv.org/abs/2605.15705)
*Tuo An,Jindou Jia,Gen Li,Jingliang Li,Chuhao Zhou,Pengfei Liu,Bofan Lyu,Jiaqi Bai,Xinying Guo,Geng Li,Jianfei Yang*

Main category: cs.RO

TL;DR: 提出反馈世界模型，在推理时利用实时观测在线修正预测，无需额外训练，显著提升分布外场景下的预测准确率和任务成功率。


<details>
  <summary>Details</summary>
Motivation: 世界模型在分布外状态预测不可靠，但执行后机器人可直接观测真实下一状态，这一信号未被充分利用。

Method: 维护轻量级反馈状态在线更新预测，可解释为潜空间观测器并有收敛保证；引入动作感知引导突出可控部分。

Result: 在LIBERO-Plus、Robomimic和真实操作任务中，预测误差降低最多76.4%，分布外成功率提升30%。

Conclusion: 推理时融入实时反馈为静态世界模型提供简单而强大的替代方案。
...
### [299] [Learning Dynamic Pick-and-Place for a Legged Manipulator](https://arxiv.org/abs/2605.15713)
...
TL;DR: 提出了一种分层强化学习框架，使四足机器人带6自由度机械臂能够动态执行抓取和放置任务，同时实现协调的运动与操纵。


<details>
  <summary>Details</summary>
Motivation: 现有腿式操纵器难以同时实现精确操纵和协调运动，尤其是处理不同重量物体时。

Method: 采用分层强化学习框架，包含显式质量估计模块，实现自适应全身控制。

Result: 仿真中负载2.3 kg成功率86.05%；真实实验中负载1.3 kg成功率73.3%，平均执行时间4.06秒，工作范围从地面到1.1米高。

Conclusion: 该方法展示了四足移动操纵器在适应更重负载和扩展工作空间方面的潜力，实现了动态连续的抓取和放置。
...
### [300] [Lamarckian Inheritance in Dynamic Environments: How Key Variables Affect Evolutionary Dynamics](https://arxiv.org/abs/2605.15769)
*K. Ege de Bruin,Kyrre Glette,Kai Olav Ellefsen*

Main category: cs.RO

TL;DR: 本研究探讨了机器人体脑协同优化中的拉马克遗传效应，发现在动态环境中，其优势取决于环境变化与控制的冲突程度及可预测性；添加传感器可恢复优势。


<details>
  <summary>Details</summary>
Motivation: 现有文献对拉马克遗传在动态环境中的收益存在矛盾，可能是因为未全面考虑相关变量。

Method: 结合形态优化（进化）与控制器优化（终身学习），采用拉马克遗传传递参数；使用虚拟软体机器人，通过贝叶斯优化和强化学习两种方法进行实验。

Result: 拉马克遗传仅在环境变化既冲突又不可预测时逊于达尔文遗传；添加传感器可在冲突环境中恢复其优势。

Conclusion: 拉马克遗传的收益受环境变化冲突性和可预测性影响，传感器有助于泛化控制。
...
### [301] [A Topology-Aware Spatiotemporal Handover Framework for Continuous Multi-UAV Tracking](https://arxiv.org/abs/2605.15779)
*Jianlin Ye,Christos Kyrkou,Panayiotis Kolios*

Main category: cs.RO

TL;DR: 提出实时多摄像头多车辆追踪系统，通过轻量级拓扑时空交接机制实现全局身份持久性，交接成功率99.8%。


<details>
  <summary>Details</summary>
Motivation: 无人机集成ITS时面临轨迹碎片化，身份在跨视野间丢失；现有单无人机方法产生孤立轨迹，无法进行网络级分析。

Method: 采用YOLO11和ByteTrack的高通量并行管道，引入基于几何重叠和虚拟车道离散化的确定性队列匹配算法（FIFO队列）预测管理身份交接。

Result: 在复杂城市环境中连续交通流下的交接成功率99.8%，远超Re-ID基线的74.1%，并验证边缘部署可行性。

Conclusion: 该系统有效解决多无人机轨迹碎片化问题，实现高精度实时全局追踪，适合边缘部署。
...
### [302] [GAP: Geometric Anchor Pre-training for Data-Efficient Visuomotor Learning of Manipulation Tasks](https://arxiv.org/abs/2605.15836)
*Davide Buoso,Andrea Protopapa,Stefano Di Carlo,Francesca Pistilli,Giuseppe Averta*

Main category: cs.RO

TL;DR: 提出几何锚点预训练（GAP），一种简单的、无动作的预热阶段，用于在少样本模仿学习前正则化空间适配器，产生稳定的几何锚点，提升数据效率和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 从少量专家演示学习视觉运动策略面临过拟合和泛化挑战，冻结的视觉基础模型将适应压力集中于小空间池化模块，易学习任务无关捷径并丢失几何基础，且预训练表示对小场景扰动脆弱。

Method: GAP在轻量模拟代理任务上预训练池化层，利用免费物体掩码鼓励适配器生成位于物体上、覆盖其空间范围且时间上尖锐可重复的关键点，作为稳定几何锚点用于下游策略学习，同时保持VFM冻结。

Result: 在RoboMimic和ManiSkill上使用15-50个演示，GAP正则化的简单适配器优于更强的注意力池化和端到端微调：RoboMimic Can任务15个演示成功率达62%（比AFA高16%），Tool Hang任务50个演示达63%，ManiSkill StackCube任务30个演示达61%（比全微调高11%）。

Conclusion: GAP代理阶段轻量且与下游任务解耦，可跨环境和操作技能复用，有效提升少样本策略学习的几何鲁棒性。
...
### [303] [Learning Sim-Grounded Policies for Bimanual Rope Manipulation from Human Teleoperation Data](https://arxiv.org/abs/2605.16043)
...
### [304] [Towards Trustworthy and Explainable AI for Perception Models: From Concept to Prototype Vehicle Deployment](https://arxiv.org/abs/2605.16087)
*Till Beemelmanns,Shayan Sharifi,Manas Mehrotra,Ayushman Choudhuri,Lutz Eckstein*

Main category: cs.RO

TL;DR: 提出了一种用于自动驾驶的可靠AI感知模块，结合可解释性、不确定性估计和鲁棒性，并在实车中验证了实时可行性。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在自动驾驶感知中占主导地位，但其不透明性与可信AI指南相悖，且缺乏针对3D场景理解的具体实现。

Method: 基于Transformer检测器，利用注意力机制生成解释并通过扰动测试验证忠实性；集成不确定性估计与校准模块；采用鲁棒性增强训练方法。

Result: 实验展示了忠实的显著性行为、改进的鲁棒性和校准良好的不确定性估计。

Conclusion: 该模块在原型车辆中部署，并通过XAI界面可视化文档、模型不确定性和显著性图，证明了实时可信感知监测的可行性。
...
### [305] [FocalPolicy: Frequency-Optimized Chunking and Locally Anchored Flow Matching for Coherent Visuomotor Policy](https://arxiv.org/abs/2605.15944)
*Qian He,Zhenshuo Yang,Wenqi Liang,Chunhui Hao,Nicu Sebe,Jiandong Tian*

Main category: cs.RO

TL;DR: 提出FocalPolicy，通过频率优化分块和局部锚定流匹配，解决视觉运动策略中块间不连续性问题，实现精确性与远见的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法只优化块内动作分布，忽略块间连贯性，导致长程动作不连续，难以学习平滑轨迹。

Method: 结合频率优化分块与局部锚定流匹配，设计远见复合目标函数监督时域对齐和频域结构，采用局部锚定采样提高目标信号传播效率。

Result: FocalPolicy优于现有方法，且模块可泛化到其他基线。

Conclusion: 实现了精确性和远见的协同平衡，有效提升长程动作连贯性。
...
### [306] [Adaptive Outer-Loop Control of Quadrotors via Reinforcement Learning](https://arxiv.org/abs/2605.16015)
*Vishnu Saj,Sushi Vemuri,Dileep Kalathil,Moble Benedict*

Main category: cs.RO

TL;DR: 提出一种自适应控制架构，通过残差动力学预测器主动感知瞬时扰动，在四旋翼飞行中实现更优的轨迹跟踪。


<details>
  <summary>Details</summary>
Motivation: 现有基于域随机化的深度强化学习方法产生的策略过于保守，难以应对动态扰动。

Method: 训练外层最优策略后，用残差动力学预测器替代真实扰动数据，仅利用状态和控制历史估计外力；引入线性校准桥和在线推力修正机制实现硬件迁移。

Result: 在Crazyflie微型四旋翼上，该方法在质量变化、非对称载荷和动态悬挂负载下显著优于基线方法。

Conclusion: 自适应控制器能有效应对严重不确定性，保持精确轨迹跟踪。
...
### [307] [Modeling Music as a Time-Frequency Image: A 2D Tokenizer for Music Generation](https://arxiv.org/abs/2605.15831)
*Yuqing Cheng,Xingyu Ma,Guochen Yu,Xiaotao Gu*

Main category: cs.SD

TL;DR: 提出BandTok，一种面向生成的2D梅尔频谱分词器，使用单共享码本的频带令牌，结合2D RoPE自回归模型，在数据受限条件下优于残差码本方法。


<details>
  <summary>Details</summary>
Motivation: 现有残差多码本量化在语音生成中导致强序列依赖和错误累积，不利于自回归建模。

Method: 采用2D梅尔频谱分词器，每帧用来自单一共享码本的梅尔频带令牌表示，形成时频网格；使用多尺度PatchGAN和EMA码本更新改进重构；引入2D RoPE自回归语言模型保持时频结构。

Result: 相比残差码本分词器，BandTok在数据有限设置下取得更好重构和生成效果。

Conclusion: BandTok提供了一种更适合自回归建模的音频分词方案，性能优异且已开源。
...
### [308] [Beyond Content: A Comprehensive Speech Toxicity Dataset and Detection Framework Incorporating Paralinguistic Cues](https://arxiv.org/abs/2605.15984)
*Zhongjie Ba,Liang Yi,Peng Cheng,Qingcao Li,Qinglong Wang,Li Lu*

Main category: cs.SD

TL;DR: 提出ToxiAlert-Bench音频数据集和双头神经网络，利用副语言线索检测有毒语音，性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视副语言线索，数据集多为文本，限制了模型能力。

Method: 构建包含7大类有毒类别和20细粒度标签的30000+音频数据集，标注毒性来源（文本/副语言）；提出双头神经网络，多头独立训练后联合微调，采用类平衡采样和加权损失。

Result: 方法在多个指标上优于基线，Macro-F1相对提升21.1%，准确率提升13.0%。

Conclusion: 利用副语言特征可显著提升有毒语音检测性能，方法具有实际应用价值。
...
### [309] [paper.json: A Coordination Convention for LLM-Agent-Actionable Papers](https://arxiv.org/abs/2605.16194)
*Arquimedes Canedo*

Main category: cs.DL

TL;DR: 提出paper.json，一种伴随PDF的JSON文件，通过五个轻量级约定解决LLM代理阅读论文时的常见失败，并验证了其可行性。


<details>
  <summary>Details</summary>
Motivation: LLM代理作为论文读者时，标准散文式论文导致子声明无法精细引用、范围过度扩展、可重复性信息分散，需要改进格式。

Method: 设计五个约定C1-C5，包括稳定声明ID、显式不声明列表、精确shell命令、稳定定义ID和最小可行合规性；并在论文本身实现并测试。

Result: 论文自身通过验证命令`uv run validator.py paper.json --against paper.typ`，证明合规性可在不到一小时内实现。

Conclusion: paper.json轻量级约定能有效解决论文可读性和可重复性问题，并鼓励代理参与验证。
...
### [310] [DeepSlide: From Artifacts to Presentation Delivery](https://arxiv.org/abs/2605.15202)
*Ming Yang,Zhiwei Zhang,Jiahang Li,Haoseng Liu,Yuzheng Cai,Weiguo Zheng*

Main category: cs.AI

TL;DR: DeepSlide是一个人在环的多智能体系统，优化演示文稿的交付过程（节奏、叙事和准备），而不仅仅是幻灯片本身。


<details>
  <summary>Details</summary>
Motivation: 现有AI幻灯片生成器主要优化视觉外观，忽视了交付过程如节奏、叙事和准备。

Method: 整合可控逻辑链规划器（含每节点时间预算）、轻量级内容树检索、马尔可夫式顺序渲染（风格继承）以及沙盒执行（最小化修复）。

Result: 在20个领域和多种受众中，幻灯片质量与强基线相当，但交付指标（叙事流、节奏精度、幻灯-脚本协同、注意力引导）显著提升。

Conclusion: DeepSlide有效支持完整的演示准备过程，超越静态幻灯片美学，提升了动态交付质量。
...
### [311] [SDOF: Taming the Alignment Tax in Multi-Agent Orchestration with State-Constrained Dispatch](https://arxiv.org/abs/2605.15204)
*Zhantao Wang*

Main category: cs.AI

TL;DR: 提出SDOF框架，将多智能体执行视为受约束状态机，通过意图路由器和状态调度器实现防御，在招聘场景中达到86.5%任务完成率并阻止所有注入攻击。


<details>
  <summary>Details</summary>
Motivation: 现有编排框架如LangChain等未强制执行实际业务流程的阶段约束。

Method: SDOF包含两个防御层：在线RLHF意图路由器（经GRPO训练）和状态感知调度器，具有有限自动机检查和技能注册表验证。

Result: 7B意图路由器在FSM约束路由基准上准确率80.9%（vs GPT-4o 48.9%）；端到端任务完成率86.5%；消息级阻塞精度100%、召回88%，专家一致性kappa=0.94。

Conclusion: SDOF有效约束多智能体执行，防御注入非法HR操作，后续将发布扩展训练和工作流评估。
...
### [312] [Does Theory of Mind Improvement Really Benefit Human-AI Interactions? Empirical Findings from Interactive Evaluations](https://arxiv.org/abs/2605.15205)
*Nanxu Gong,Zixin Chen,Haotian Li,Zishu Zhao,Jianxun Lian,Huamin Qu,Yanjie Fu,Xing Xie*

Main category: cs.AI

TL;DR: 提出交互式ToM评估新范式，发现静态基准提升不等同于动态交互表现。


<details>
  <summary>Details</summary>
Motivation: 现有ToM评估忽略人机交互的第一人称动态开放性，需要直接检验ToM改进技术对交互的益处。

Method: 提出交互式ToM评估范式（视角和指标改变），系统研究四种代表性ToM增强技术，使用四个真实数据集和用户研究，覆盖目标导向和体验导向任务。

Result: 静态基准的提升并不能总是转化为动态交互中的更好表现。

Conclusion: 交互式评估对开发下一代社交感知LLM至关重要。
...
### [313] [SkillSmith: Compiling Agent Skills into Boundary-Guided Runtime Interfaces](https://arxiv.org/abs/2605.15215)
*Duling Xu,Zheng Chen,Zaifeng Pan,Jiawei Guan,Dong Dong,Jialin Li,Bangzheng Pu*

Main category: cs.AI

TL;DR: 提出SkillSmith框架，通过边界优先的编译-运行时方法减少LLM Agent系统的冗余，提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有技能注入方式导致无关上下文和重复推理两种冗余。

Method: SkillSmith框架，离线编译技能包为最小可执行接口，运行时动态访问相关组件。

Result: 在SkillsBench上，token使用减少57.44%，思考迭代减少42.99%，时间减少50.57%，成本减少同比例。且强模型编译的成果可被弱模型重用，提升准确率。

Conclusion: SkillSmith有效减少冗余，提升效率，并支持模型间技能重用。
...
### [314] [Fair outputs, Biased Internals: Causal Potency and Asymmetry of Latent Bias in LLMs for High-Stakes Decisions](https://arxiv.org/abs/2605.15217)
*Jagdish Tripathy,Marcus Buckmann*

Main category: cs.AI

TL;DR: 指令微调模型在输出层面表现公平，但内部表示仍存在不对称的潜在偏见，可通过激活引导等手段逆转决策。


<details>
  <summary>Details</summary>
Motivation: 揭示指令微调模型在内部表示中保留的有偏信息是否会影响输出，以及这种因果效力是否对称。

Method: 使用开源权重模型进行抵押贷款承销实验，通过匹配种族相关姓名不同的申请，结合激活引导和跨层干预分析内部表示。

Result: 模型无输出偏见，但内部表示保留并放大人口统计信息；注入关键层可逆转决策；偏见不对称且易受对抗攻击。
...
### [315] [CAX-Agent: A Lightweight Agent Harness for Reliable APDL Automation](https://arxiv.org/abs/2605.15218)
*Chenying Lin,Yichen Hai,Yi He,Ran Wang,Haiyan Qiang,Liang Yu*

Main category: cs.AI

TL;DR: 提出CAX-Agent，一种用于MAPDL自动化的轻量级智能体框架，通过结构化编排和恢复策略提升LLM可靠性，实验表明模型驱动的恢复策略效果最佳。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在MAPDL有限元仿真中面临输出不一致和任务失败的问题，需要结构化执行控制和故障恢复。

Method: 采用三层架构（LLM服务、智能体编排、求解器后端）和恢复阶梯（从规则修补到模型再生再到人工干预），评估了三种恢复策略。

Result: 模型驱动恢复策略完成率最高（0.9267），任务得分3.59/4，总分9.16/10，零干预率0.84，优于规则驱动和无恢复策略。

Conclusion: 模型驱动恢复效果显著，但基准测试使用简单几何形状，结论适用范围有限，需进一步验证。
...
### [316] [NOVA: Fundamental Limits of Knowledge Discovery Through AI](https://arxiv.org/abs/2605.15219)
*Salman Avestimehr,Ken Duffy,Muriel M\'edard*

Main category: cs.AI

TL;DR: 提出NOVA框架，将AI自改进循环建模为知识空间的自适应采样，分析知识发现的充分条件、失败模式（污染、遗忘等）、污染陷阱，并推导出发现成本与数量的缩放律。


<details>
  <summary>Details</summary>
Motivation: 探索AI系统能否通过迭代自改进发现真正的新知识，并量化成本。

Method: 将“生成、验证、积累、重训练”循环建模为知识空间上的自适应采样过程，分析充分条件及其违反导致的失败模式，利用Good-Turing估计和Zipf律进行理论分析。

Result: 确定了知识覆盖的充分条件，识别了四种失败模式；发现污染陷阱：当易发现知识耗尽时，新有效知识质量下降，小假阳性率导致无效知识更快积累；证明在尾等价假设下，累计生成成本与发现数量满足R_cum(D)=Θ(c_gen D^α)。

Conclusion: 自改进AI系统能发现新知识但受缩放律限制，人工专家在自主探索障碍附近最具价值。
...
### [317] [ICRL: Learning to Internalize Self-Critique with Reinforcement Learning](https://arxiv.org/abs/2605.15224)
*Jianbo Lin,Xiaomin Yu,Yi Xin,Yifu Guo,Zhuosong Jiang,Zhongqi Yue,Weishi Wang,Heqing Zou,Chengwei Qin,Hui Xiong*

Main category: cs.AI

TL;DR: 提出ICRL框架，通过强化学习联合训练解算器和批评者，使解算器内化自我批评，无需外部批评即可自我改进，在代理和数学推理任务上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的代理依赖外部批评才能正确行为，移除批评后容易再次失败，且固定批评者无法提升反馈质量，限制自我改进能力。

Method: ICRL通过强化学习联合训练共享骨干的解算器和批评者，用分布校准重加权比处理分布偏移，用角色组优势估计稳定联合优化，使解算器学会不依赖外部批评的自我改进。

Result: 在Qwen3-4B/8B上，代理任务平均提升6.4点，数学推理提升7.0点；8B批评者性能可与32B批评者媲美且使用更少令牌。

Conclusion: ICRL能有效将批评引导的改进转化为解算器自身能力，实现无需外部批评的持续自我改进。
...
### [318] [NIMO Controller: a self-driving laboratory orchestrator based on the Model Context Protocol](https://arxiv.org/abs/2605.15227)
*Naruki Yoshikawa,Ryo Tamura*

Main category: cs.AI

TL;DR: 提出基于MCP的SDl架构，通过NIMO控制器实现可视化编程，统一人类与AI代理接口。


<details>
  <summary>Details</summary>
Motivation: 现有SDl框架主要面向人类交互，缺乏适合AI代理的标准接口。

Method: 基于MCP设计SDl软件架构，所有功能通过MCP服务器暴露，开发NIMO控制器自动生成可视化编程接口。

Result: 通过颜色匹配SDl案例验证了系统可用性。

Conclusion: 基于MCP的SDl架构有效，为人类和AI提供统一接口。
...
### [319] [Verifiable Agentic Infrastructure: Proof-Derived Authorization for Sovereign AI Systems](https://arxiv.org/abs/2605.15228)
*Jun He,Deying Yu*

Main category: cs.AI

TL;DR: 提出分布式信任框架(DTF)，通过证明和共识机制确保自主AI代理动作的可验证、可审计和可追溯。


<details>
  <summary>Details</summary>
Motivation: 自主AI代理可生成语法正确但语义不安全的动作，使静态权限成为重大运营风险，尤其在主权AI系统中。

Method: DTF引入Justification Proof编码动作依据、共识模型进行独立评估、临时Execution Identity从已批准证明派生，以及仅追加的证据链保存授权生命周期。

Result: 该架构强制执行紧凑的授权不变性：无证明对象不执行高风险操作，无共识不派生权限，无证据不进行有效变更。

Conclusion: 将授权从静态身份转向证明派生权威，为自主AI部署中的可管理、可审计和有界执行提供基础设施基础。
...
### [320] [Solvita: Enhancing Large Language Models for Competitive Programming via Agentic Evolution](https://arxiv.org/abs/2605.15301)
...
TL;DR: Solvita是一种面向大型语言模型的智能体演化框架，通过可训练的知识网络实现持续学习，在竞争编程任务中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型在硬竞争编程中推理可靠性不足，而多智能体框架依赖静态检索，缺乏从先前任务中积累经验的能力。

Method: Solvita将问题解决组织成闭环系统，包含规划者、求解者、预言者和黑客四个智能体，每个智能体配有可训练的图结构知识网络，通过强化学习利用结果信号更新网络权重，实现动态路由和推理经验积累。

Result: 在CodeContests、APPS、AetherCode和实时Codeforces评测中，Solvita超越了现有代码生成智能体，相比单次基线准确率几乎翻倍。

Conclusion: Solvita通过无需权重更新的持续学习机制，有效提升了竞争编程中的推理可靠性和性能。
...
### [321] [SMCEvolve: Principled Scientific Discovery via Sequential Monte Carlo Evolution](https://arxiv.org/abs/2605.15308)
*Jiachen Jiang,Huminhao Zhu,Zhihui Zhu*

Main category: cs.AI

TL;DR: SMCEvolve 是一种基于序贯蒙特卡洛采样的 LLM 驱动程序演化框架，通过理论指导的组件设计和收敛保证，在多个基准上以更少的 LLM 调用超越现有系统。


<details>
  <summary>Details</summary>
Motivation: 现有 LLM 驱动的程序演化框架缺乏组件设计的理论指导，且无法保证搜索过程收敛。

Method: 将程序搜索建模为从奖励倾斜目标分布中采样，使用序贯蒙特卡洛采样器近似，并引入自适应亲本重采样、混合变异接受机制和自动收敛控制三个核心组件，同时提供有限样本复杂度分析。

Result: 在数学、算法效率、符号回归和端到端机器学习研究基准上，SMCEvolve 以更少的 LLM 调用次数超越最先进的演化系统，并能自动决定终止条件。

Conclusion: SMCEvolve 提供了原则性的框架设计、收敛保证和有限样本复杂度界限，为 LLM 驱动的程序搜索提供了理论基础和实用工具。
...
### [322] [Context Pruning for Coding Agents via Multi-Rubric Latent Reasoning](https://arxiv.org/abs/2605.15315)
*Jingjing Wang,Xiwen Chen,Wenhui Zhu,Huayu Li,Zhengxiao He,Feiyang Cai,Ana S. Carreon-Rascon,Xuanzhao Dong,Feng Luo*

Main category: cs.AI

TL;DR: 提出LaMR，一种多维度结构化剪枝框架，通过分解代码相关性为两个可解释维度来优化代码代理的上下文压缩。


<details>
  <summary>Details</summary>
Motivation: 现有剪枝方法使用单目标序列标注器，一个CRF转移先验必须服务于异质的保留模式，造成建模瓶颈。

Method: 将代码相关性分解为语义证据和依赖支持两个维度，每个维度用专用CRF建模，通过门控网络动态加权，最后用CRF层融合做出保留/剪枝决策；利用AST分析从现有语料生成多维度标签。

Result: 在四个基准测试中，16个多轮对比赢12个，节省最多31% token，单轮任务EM提升最多3.5，性能提升或边际下降。

Conclusion: 有效过滤干扰噪声，经常匹配甚至超越未剪枝的完整上下文基线。
...
### [323] [Zero-Shot Goal Recognition with Large Language Models](https://arxiv.org/abs/2605.15333)
*Kin Max Piamolini Gusm\~ao,Nathan Gavenski,Nir Oren,Felipe Meneguzzi*

Main category: cs.AI

TL;DR: 本文首次系统评估前沿LLM作为经典PDDL规划基准中的目标识别器，发现其表现不均匀：部分模型能随证据积累提升准确率，部分则固守世界知识先验。


<details>
  <summary>Details</summary>
Motivation: LLM在规划任务中接近经典规划器，但依赖世界知识而非符号推理；目标识别作为溯因任务更适合LLM优势，需系统评估。

Method: 在经典PDDL基准上对前沿LLM进行零样本目标识别评估，分析推理轨迹以区分证据整合与领域熟悉度。

Result: 部分LLM随观测证据增加准确率接近基于地标的算法，其他模型表现与证据量无关，定性分析显示差异源于证据整合能力。
...
### [324] [Belief Engine: Configurable and Inspectable Stance Dynamics in Multi-Agent LLM Deliberation](https://arxiv.org/abs/2605.15343)
*Joshua C. Yang,Maurice Flechtner,Damian Dailisan,Michiel A. Bakker*

Main category: cs.AI

TL;DR: 为LLM代理的信念更新提供可审计层，揭示辩论中立场变化的证据基础。


<details>
  <summary>Details</summary>
Motivation: LLM代理在模拟协商等互动时，立场变化的根源不透明（如是否因证据吸收、锚定等）。

Method: 信念引擎（BE）将信念表示为命题的证据状态，通过结构化记忆提取论点，并使用对数几率更新规则（由证据吸收率u和先验锚定a控制）调整立场。

Result: 在DEBATE数据集上，BE能最佳重构最终立场符合提取证据的参与者；稳定或反对证据的案例则指向锚定或证据流外的因素。

Conclusion: BE提供了可配置的基础设施，用于研究基于证据的 deliberation，使开放性、承诺、趋同和分歧可归因于明确的更新假设。
...
### [325] [Ensemble Monitoring for AI Control: Diverse Signals Outweigh More Compute](https://arxiv.org/abs/2605.15377)
*Eugene Koran,Yejun Yun,Samantha Tetef,Benjamin Arnav,Pablo Bernabeu-P\'erez*

Main category: cs.AI

TL;DR: 通过组合多种监控器（包括提示工程和微调）形成集成，能显著提升对AI代理不安全行为的检测能力，且监控器多样性比规模更重要。


<details>
  <summary>Details</summary>
Motivation: AI系统自主代理规模扩大后，保障行为安全至关重要，但人类监督不切实际，需要可靠的自动监控方法。

Method: 构建了12个基于GPT-4.1-Mini的监控器，采用提示工程和微调策略，在包含对抗性输入的编码任务上评估。

Result: 多样化集成优于单个或同质集成；最佳三监控器集成的检测性能比三个相同监控器集成提升2.4倍，且微调监控器在分布外攻击中仍保持优势。

Conclusion: 多样性而非规模驱动性能提升，集成监控是一种实用的AI安全控制策略，推理成本合理。
...
### [326] [Beyond Partner Diversity: An Influence-Based Team Steering Framework for Zero-Shot Human-Machine Teaming](https://arxiv.org/abs/2605.15400)
*Wei Sheng,Rohan Paleja*

Main category: cs.AI

TL;DR: 提出了基于影响的团队引导（IBTS）框架，通过影响塑造激励智能体发现多样且高效的团队交互模式，并引导现有轨迹走向更强的协调模式，在Overcooked-AI的双智能体和三智能体设置中提升团队表现。


<details>
  <summary>Details</summary>
Motivation: 数据驱动的人机协同方法依赖跨领域、队友和团队规模的人类交互数据，成本高昂；零样本协调虽能模拟多样伙伴，但仅靠伙伴覆盖在团队规模扩大和通信退化时不足。

Method: 基于影响的团队引导（IBTS），使用影响塑造激励智能体发现多样且高效的团队交互模式，并进一步引导当前轨迹朝向更强的学习协调模式。

Result: 在Overcooked-AI的双智能体和三智能体设置中，包括模拟伙伴、合成伙伴风格变化及首个30受试者的人机协同研究，IBTS相比基线提升了团队表现。
...
### [327] [From LLM-Generated Conjectures to Lean Formalizations: Automated Polynomial Inequality Proving via Sum-of-Squares Certificates](https://arxiv.org/abs/2605.15445)
*Ruobing Zuo,Hanrui Zhao,Gaolei He,Zhengfeng Yang,Jianlin Wang*

Main category: cs.AI

TL;DR: 提出神经符号框架NSPI，结合LLM和符号计算自动证明多项式不等式，并在Lean中验证，实验表明在多达10个变量的挑战性基准上有效。


<details>
  <summary>Details</summary>
Motivation: 纯符号方法可扩展性差，LLM方法在变量少时有效，但面对高维多项式仍具挑战。

Method: LLM提出近似多项式平方和分解猜想，符号计算精化为精确平方和表示，证明不等式，并在Lean中机器验证。

Result: 在含多达10个变量的挑战性基准上验证了有效性和可扩展性。

Conclusion: NSPI结合了LLM的启发式发现和符号计算的精确性，实现了从启发式到机器验证的端到端流水线，有效解决了可扩展性问题。
...
### [328] [X-SYNTH: Beyond Retrieval -- Enterprise Context Synthesis from Observed Human Attention](https://arxiv.org/abs/2605.15505)
*Guruprasad Raghavan,George Nychis,Rohan Narayana Murthy*

Main category: cs.AI

TL;DR: 提出基于人类注意力行为痕迹的X-SYNTH框架，解决企业上下文合成问题，在销售线索识别任务中真线索率提升6.5倍，假线索率大幅下降。


<details>
  <summary>Details</summary>
Motivation: 现有检索方法依赖系统状态，缺乏行为上下文，导致复杂任务中真线索率低、假线索率高，且缺乏改进机制。

Method: 利用个体行为痕迹（数字孪生签名）和七种注意力过滤器，通过四阶段流水线组装基于行为模式的上下文。

Result: 在销售线索识别任务中，X-SYNTH使真线索率从9.5%提升至61.9%（6.5倍），假线索率从90.5%降至18.8%。

Conclusion: 企业上下文合成是相关性而非检索问题，人类注意力是最可靠的真实依据。
...
### [329] [CAPS: Cascaded Adaptive Pairwise Selection for Efficient Parallel Reasoning](https://arxiv.org/abs/2605.15513)
*Fangzhou Lin,Shuo Xing,Peiran Li,Siyuan Yang,Qianwen Ge,Kazunori Yamada,Ziming Zhang,Haichong Zhang,Zhengzhong Tu*

Main category: cs.AI

TL;DR: 提出CAPS，一种自适应级联成对选择框架，通过非均匀分配验证计算来降低推理成本，同时在多个基准上保持或提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有成对自验证方法成本高，因为每个判断读取完整解决方案且比较次数固定，不论信息量。

Method: CAPS沿证据轴和分布轴自适应分配验证器计算，实现为四阶段级联加可选救援子程序，有封闭形式的成本公式。

Result: 在5个基准和4个模型上，CAPS在14/20套件优于最先进的成对验证器，仅用其25.4%的验证令牌预算，并在所有20个套件上优于逐点自验证。

Conclusion: CAPS提供了一种高效且可解释的验证计算分配方法，通过部分证据准确性诊断可预先检查级联适用性。
...
### [330] [RTL-BenchMT: Dynamic Maintenance of RTL Generation Benchmark Through Agent-Assisted Analysis and Revision](https://arxiv.org/abs/2605.15537)
*Jing Wang,Shang Liu,Hangan Zhou,Zhiyao Xie*

Main category: cs.AI

TL;DR: 提出RTL-BenchMT，一个自动维护RTL生成基准的智能体框架，自动识别和修复有缺陷的案例及过拟合案例。


<details>
  <summary>Details</summary>
Motivation: 当前RTL基准存在有缺陷案例和过拟合问题，人工维护成本高且难以解决。

Method: RTL-BenchMT框架自动识别并修订有缺陷的基准案例，检测并更新过拟合案例。

Result: 对缺陷和过拟合案例进行了深入分析，生成了一个改进的基准套件，并计划开源。

Conclusion: 该框架有效解决了基准维护挑战，降低了人工成本，提供了更优质的基准套件。
...
### [331] [DRS-GUI: Dynamic Region Search for Training-Free GUI Grounding](https://arxiv.org/abs/2605.15542)
*Yichao Liu,Huawen Shen,Liu Yu,Shiyu Liu,Zeyu Chen,Yu Zhou*

Main category: cs.AI

TL;DR: 动态区域搜索框架DRS-GUI，通过模仿人类感知行为结合蒙特卡洛树搜索，无训练地增强MLLM的GUI元素定位能力，提升14%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在处理高分辨率屏幕截图中与指令相关的元素时，因无关UI组件干扰而定位不准，亟需更精确的接地方法。

Method: 提出DRS-GUI框架，包含UI感知器（执行聚焦、平移、分散三类人类感知动作）和基于蒙特卡洛树搜索的动作规划器，通过区域质量奖励动态筛选高相关区域。

Result: 在ScreenSpot-Pro上，通用和GUI专用MLLM（Qwen2.5-VL-7B和UGround-V1-7B）的接地性能提升14%。

Conclusion: DRS-GUI有效提升接地性能和泛化能力，且无需额外训练即可集成至现有MLLM。
...
### [332] [Position: Artificial Intelligence Needs Meta Intelligence -- the Case for Metacognitive AI](https://arxiv.org/abs/2605.15567)
...
### [333] [STAR: A Stage-attributed Triage and Repair framework for RCA Agents in Microservices](https://arxiv.org/abs/2605.15581)
*Junle Wang,Xingchuang Liao,Wenjun Wu*

Main category: cs.AI

TL;DR: 提出STAR框架，通过将RCA工作流分解为四个阶段并逐阶段审计与修复，显著提升基于LLM的根因分析代理的可靠性和自愈能力。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的根因分析代理在微服务AIOps中虽前景广阔，但其可靠性脆弱，早期错误会沿推理链传播并最终破坏诊断结果。

Method: STAR框架将RCA工作流显式分解为证据包、假设集、分析结构和决策报告四个阶段，利用LangGraph实现阶段审计、预算感知的快慢路由、基于反事实候选评估的关键阶段定位以及阶段特定的修补重放。

Result: 在公开大规模基准和真实生产数据集上，STAR一致提升了根因定位和故障分类效果，高精度识别关键错误阶段，多数错误轨迹在一到两轮重放后修复。

Conclusion: 显式建模RCA代理失败位置是实现可靠、可调试且可自愈的代理化根因分析系统的有效途径。
...
### [334] [See Before You Code: Learning Visual Priors for Spatially Aware Educational Animation Generation](https://arxiv.org/abs/2605.15585)
*Yuejia Li,Ke He,Junheng Li,Shutong Chen,Jingkang Xia,Zhiyue Su,Junchi Zhang,Mang Ye*

Main category: cs.AI

TL;DR: 提出OmniManim框架，通过显式视觉规划、渲染后诊断和局部修复解决教育动画生成中的视觉缺陷问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型生成的教育动画代码执行后常出现视觉缺陷，且无法从代码中检测，需要渲染后反馈。

Method: 采用共享场景状态、显式视觉规划（粗到细边界框去噪和插值感知优化）、结构化渲染后诊断和局部修复。

Result: 在EduRequire-500数据集上，OmniManim的渲染质量优于单模型和多智能体基线。消融实验证实显式视觉规划是关键。

Conclusion: 显式视觉规划、粗空间先验、边界框细化和插值感知优化是提升教育动画质量的核心。
...
### [335] [TopoEvo: A Topology-Aware Self-Evolving Multi-Agent Framework for Root Cause Analysis in Microservices](https://arxiv.org/abs/2605.15611)
*Junle Wang,Xingchuang Liao,Wenjun Wu*

Main category: cs.AI

TL;DR: 提出TopoEvo，一种拓扑感知的自演化多代理框架，通过图表示学习和拓扑约束推理解决微服务根因分析中的症状放大偏差和拓扑漂移问题。


<details>
  <summary>Details</summary>
Motivation: 微服务根因分析面临多模态噪声、级联传播和拓扑漂移挑战，现有LLM代理存在症状放大偏差，误将下游受害者归因为根因。

Method: 使用度量正交多模态对齐（MOMA）分解嵌入、向量量化（VQ）生成症状令牌、多代理假设-证据-测试（HET）工作流验证传播一致性，并引入自演化机制更新记忆和适应漂移。

Result: 在多个数据集上优于基线方法，有效降低误报率并提升根因定位准确率。

Conclusion: TopoEvo通过拓扑感知和多代理协作，显著缓解了症状放大偏差，在动态微服务环境中保持鲁棒性。
...
### [336] [ColPackAgent: Agent-Skill-Guided Hard-Particle Monte Carlo Workflows for Colloidal Packing](https://arxiv.org/abs/2605.15625)
*Lijie Ding,Changwoo Do*

Main category: cs.AI

TL;DR: ColPackAgent是一个基于MCP协议和agent技能的自主胶体堆积模拟框架，能将仿真工具包转化为智能研究助手。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型代理缺乏专用模拟工具和流程指令，通常只能描述工作流而无法可靠执行，需要专门工具和结构化流程来确保可靠性。

Method: 通过MCP服务器暴露自定义colpack Python包（封装HOOMD-blue硬粒子蒙特卡洛），并编码四阶段工作流合同；支持交互式、全自主和按程序文件三种运行模式。

Result: 在立方体3D、二元盘-胶囊2D和硬盘冻结相变等示例中展示系统功能，并通过17个阶段提示对比多种LLM的流程遵循能力。

Conclusion: 将领域Python包与MCP工具和可移植agent技能结合，为仿真工具包转化为代理辅助研究工作流提供了实用路径。
...
### [337] [PRISM: Prompt Reliability via Iterative Simulation and Monitoring for Enterprise Conversational AI](https://arxiv.org/abs/2605.15665)
*Keshava Chaitanya,Jahnavi Gundakaram*

Main category: cs.AI

TL;DR: PRISM是一个闭环框架，将提示工程视为持续可靠性工程，通过自动生成测试、模拟对话、评估和修复提示，以应对LLM行为漂移，实现高效可靠的提示优化。


<details>
  <summary>Details</summary>
Motivation: 现有提示优化只考虑一次性编译时问题，忽略了LLM行为随时间变化导致的提示退化，需要持续检测和修复。

Method: PRISM输入自然语言需求、工具和内存变量及初始提示，自动生成测试用例，在模拟环境中进行多轮对话，用LLM作为裁判评估通过/失败，诊断故障根因并精准修复提示，循环直至所有测试通过，并计划每日运行。

Result: 在35个企业对话代理上部署三周，提示编写时间从2天降至30分钟内，生产可靠性达99%，24小时内检测并修复因LLM行为漂移导致的回归。

Conclusion: 持续的、模拟驱动的提示优化对于大规模可靠的企业对话AI既是可行的也是必要的。
...
### [338] [Nudging Beyond the Comfort Zone: Efficient Strategy-Guided Exploration for RLVR](https://arxiv.org/abs/2605.15726)
*Chanuk Lee,Sangwoo Park,Minki Kang,Sung Ju Hwang*

Main category: cs.AI

TL;DR: NudgeRL通过策略引导和统一目标实现了结构化探索，在数学推理任务中显著优于标准方法，且无需昂贵的大规模采样。


<details>
  <summary>Details</summary>
Motivation: RLVR中策略改进受限于已探索轨迹，增加rollout数虽有效但计算成本高，现有方法对探索控制有限。

Method: 提出Strategy Nudging，为每次rollout提供轻量级策略上下文以生成多样推理轨迹；设计统一目标，将奖励分解为上下文间和上下文内成分，并加入蒸馏目标将发现的行为迁移回基础策略。

Result: 在五个数学基准上，NudgeRL平均优于使用8倍rollout预算的GRPO和基于oracle的基线。

Conclusion: 结构化、上下文驱动的探索是暴力扩展rollout和特权信息方法的有效且可扩展替代方案。
...
### [339] [Can We Trust AI-Inferred User States. A Psychometric Framework for Validating the Reliability of Users States Classification by LLMs in Operational Environments](https://arxiv.org/abs/2605.15734)
*Izabella Krzeminska,Michal Butkiewicz,Ewa Komkowska*

Main category: cs.AI

TL;DR: 大型语言模型评估用户状态的指标在个体分数层面缺乏稳定性，仅31/213个指标可靠，但聚合后仍有分析价值，并提出可复现的评估框架。


<details>
  <summary>Details</summary>
Motivation: 当前假设LLM评估用户状态的指标在个体分数上稳定可解释，但缺乏实证验证，需测试其心理测量可靠性。

Method: 采用复制评估程序，在GPT-4o audio、Gemini 2.0 Flash和Gemini 2.5 Flash三个双模态LLM上，评估多项指标的个体分数可靠性和聚合可靠性。

Result: 个体分数可靠性低，仅31/213个指标满足标准；聚合后指标仍具分析价值，可揭示交互规则与用户体验的关系。

Conclusion: 指标可靠性不能默认，需显式验证；提出的评估框架支持可测量的指标适用性评估，促进负责任AI设计。
...
### [340] [ALSO: Adversarial Online Strategy Optimization for Social Agents](https://arxiv.org/abs/2605.15768)
*Xiang Li,Liping Yi,Mingze Kong,Min Zhang,Zhongxiang Dai,QingHua Hu*

Main category: cs.AI

TL;DR: 提出ALSO框架，通过对抗性多臂赌博机和轻量级奖励预测器，在线优化多智能体社会模拟中的策略，在动态环境中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 社会模拟环境非平稳，现有LLM智能体依赖静态角色，离线强化学习和外部规划器假设平稳且训练成本高，无法有效动态调整策略。

Method: 将多轮对话视为对抗性赌博机问题，用静态角色和动态策略指令的组合作为臂；设计轻量级神经模型从交互历史预测奖励，实现样本高效探索和在线适应。

Result: 在Sotopia基准测试中，ALSO始终优于静态角色基线和现有优化方法，尤其在动态环境中优势明显。

Conclusion: 验证了对抗性在线策略优化是构建鲁棒社会智能体的有效途径，无需环境平稳性假设。
...
### [341] [SaaS-Bench: Can Computer-Use Agents Leverage Real-World SaaS to Solve Professional Workflows?](https://arxiv.org/abs/2605.15777)
...
TL;DR: 提出SaaS-Bench基准，评估LLM代理在真实SaaS工作流中的表现，最强模型完成率不足4%。


<details>
  <summary>Details</summary>
Motivation: 现有web和GUI代理基准设置过于简化，无法评估代理在真实专业工作流中的能力。

Method: 构建23个可部署的SaaS系统，涵盖6个专业领域，包含106个长程任务，支持文本和多模态，使用加权验证检查点评估。

Result: 最强LLM代理端到端完成率低于4%。

Conclusion: 代理在规划、状态跟踪、跨应用上下文维护和错误恢复方面存在显著局限。
...
### [342] [Agentic Discovery of Neural Architectures: AIRA-Compose and AIRA-Design](https://arxiv.org/abs/2605.15871)
*Alberto Pepe,Chien-Yu Lin,Despoina Magka,Bilge Acun,Yannan Nellie Wu,Anton Protopopov,Carole-Jean Wu,Yoram Bachrach*

Main category: cs.AI

TL;DR: 提出双框架方法，让LLM代理自主设计基础模型，在多个规模上优于人类设计的基线。


<details>
  <summary>Details</summary>
Motivation: 探索递归自我改进，使AI代理能够自主设计超越标准Transformer的模型，以推动下一代基础模型发现。

Method: AIRA-Compose框架使用11个代理进行高级架构搜索，评估并外推至更大规模；AIRA-Design框架使用20个代理设计注意力机制和训练脚本。

Result: AIRAformer和AIRAhybrid在1B规模预训练后，下游任务准确率分别提升2.4%和3.8%；AIRAformer-C扩展速度比Llama 3.2快54%和71%；AIRA-Design设计的架构在Long Range Arena上接近人类SOTA，Autoresearch基准上达到0.968验证bits-per-byte。

Conclusion: AI代理可以自主发现匹配或超越人类设计的架构和算法优化，为递归自我改进和下一代基础模型发现建立了强大范式。
...
### [343] [Imperfect World Models are Exploitable](https://arxiv.org/abs/2605.15960)
...
TL;DR: 提出了强化学习中模型利用的新定义，证明其在大策略集上不可避免，并建立了与奖励黑客的形式联系。


<details>
  <summary>Details</summary>
Motivation: 理解模型利用的本质及其与奖励黑客的关系，探索安全规划的极限。

Method: 定义模型利用，发展一般理论，引入松弛概念和可避免的安全视界。

Result: 模型利用在大策略集上不可避免；确保无黑客的条件无法防止模型利用。

Conclusion: 建立了奖励黑客与模型利用的形式桥梁，揭示了世界模型中安全规划的极限。
...
### [344] [PAGER: Bridging the Semantic-Execution Gap in Point-Precise Geometric GUI Control](https://arxiv.org/abs/2605.15963)
*Jingxuan Wei,Xi Bai,Shan Liu,Caijun Jia,Zheng Sun,Xinglong Xu,Siyuan Li,Linzhuang Sun,Bihui Yu,Conghui He,Cheng Tan*

Main category: cs.AI

TL;DR: 提出PAGE Bench基准和PAGER智能体，解决精确GUI构造任务中依赖驱动的错误传播问题，显著提升任务成功率。


<details>
  <summary>Details</summary>
Motivation: 现有GUI智能体依赖宽容区域范式，不适用于需要点级精确度和拓扑感知的精确几何构造任务。

Method: 构建PAGE Bench（4906个问题，224K像素级动作），提出PAGER，分解为依赖结构规划和像素级执行，结合像素监督微调与精度对齐强化学习。

Result: 通用模型动作类型准确率>88%但任务成功率<6%；PAGER任务成功率提升4.1倍，步骤成功率从<9%提升至>62%。

Conclusion: 揭示了语义-执行鸿沟，PAGER实现了点精确GUI控制的新状态。
...
### [345] [Deterministic Event-Graph Substrates as World Models for Counterfactual Reasoning](https://arxiv.org/abs/2605.15967)
*Fabio Rovai*

Main category: cs.AI

TL;DR: 事件图基板是一种可检查、支持精确反事实的世界模型，通过追加RDF三元组日志表示状态，并在结构化干预词汇下分叉日志进行反事实查询。该模型在CLEVRER上超越符号基准，在Smallville上超越大语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型缺乏可检查性和精确反事实支持，且难以跨域迁移。

Method: 将状态表示为追加的RDF三元组日志，通过结构化干预词汇分叉日志实现反事实查询；形式化该类模型，证明解释性和反事实查询对偶性，并实现了一个域无关的运行时。

Result: 在CLEVRER上超过NS-DR符号基准（提升9.89到20.26个百分点），在小型推理任务上超越Llama-3.1-8B（联合准确率提升18.80个百分点）。

Conclusion: 事件图基板提供了可检查、精确反事实且跨域迁移的世界模型，在多个基准上表现优异。
...
### [346] [Learning Bilevel Policies over Symbolic World Models for Long-Horizon Planning](https://arxiv.org/abs/2605.15975)
*Dillon Z. Chen,Till Hofmann,Toryn Q. Klassen,Sheila A. McIlraith*

Main category: cs.AI

TL;DR: 通过双层策略结合低级模仿学习和高级符号规划，BISON系统在长期规划任务中实现高效泛化。


<details>
  <summary>Details</summary>
Motivation: 低级模仿学习难以生成长期规划，而高级符号抽象有利于高效可解释的长期规划，旨在结合两者优势。

Method: 提出双层策略 (π^hl, π^ll)，低级策略从演示学习，高级策略从符号抽象与归纳泛化构建，实现为BISON系统。

Result: 在MetaWorld基准上，BISON泛化到更长周期和更多物体，比VLA及端到端方法更省时省内存；高级策略可一分钟内解决含10000个相关物体的高级问题。

Conclusion: BISON成功融合低级控制与高级规划，显著提升长期规划任务的泛化性和效率。
...
### [347] [Petri Net Induced Heuristic Search for Resource Constrained Scheduling](https://arxiv.org/abs/2605.15983)
*Ido Lublin,Dor Atzmon,Izack Cohen*

Main category: cs.AI

TL;DR: 将资源约束项目调度问题建模为时间转换Petri网的可达图最优搜索，使用A*和一致的启发式求解，实验优于MIP基线。


<details>
  <summary>Details</summary>
Motivation: 更有效地求解RCPSP问题。

Method: 建模为具有资源的时间转换Petri网，使用相对延迟令牌，通过A*搜索结合关键路径和资源下界启发式求解。

Result: 在PSPLIB基准上，成功率和求解时间均强于SCIP和CBC等MIP求解器。

Conclusion: 方法有效，且与MIP在退化维度上互补，资源紧度影响A*，模型规模影响MIP。
...
### [348] [ScreenSearch: Uncertainty-Aware OS Exploration](https://arxiv.org/abs/2605.16024)
*Michael Solodko,Justin Wagle*

Main category: cs.AI

TL;DR: ScreenSearch是一个桌面GUI探索系统，通过结合结构屏幕检索、去重和基于PUCT的图-老虎机算法，处理部分可观测性导致的歧义问题，收集了大量多样化状态。


<details>
  <summary>Details</summary>
Motivation: 桌面GUI代理面临部分可观测性：视觉相似屏幕可能对应不同工作流状态，局部合理动作可能导致截然不同的结果。需要既能扩展可达前沿又能减少歧义的探索策略。

Method: ScreenSearch使用UIA树生成位置感知结构特征，通过稀疏标记搜索和元数据过滤器索引相关屏幕，并在VM工作节点间维护共享去重状态图。基于匹配动作结果分散性定义可扩展歧义信号，结合前沿奖励驱动大规模探索和重放启动策略评估。

Result: 在11个桌面应用上收集了超过100万张截图和3万个去重状态，展现了新颖性-歧义权衡：某些策略快速减少歧义但前沿探索少，表明仅减少歧义不足以作为探索目标。

Conclusion: 状态身份、提议质量和歧义感知搜索在决定何时探查和何时提交时都至关重要。
...
### [349] [Reasoners or Translators? Contamination-aware Evaluation and Neuro-Symbolic Robustness in Tax Law](https://arxiv.org/abs/2605.16052)
*Parisa Kordjamshidi,Samer Aslan,Madhavan Seshadri,Leslie Barrett,Enrico Santus*

Main category: cs.AI

TL;DR: 大语言模型在法律推理中可能因数据污染而虚高，神经符号方法更可靠且泛化能力更强。


<details>
  <summary>Details</summary>
Motivation: 探究LLM在法律推理中的表现是否真正反映推理能力，还是数据污染的结果。

Method: 实施污染检测协议，构建测试套件，比较单块LLM与混合系统（将法律文本转为形式表示，用符号推理）。

Result: LLM性能可能因污染而虚高；神经符号框架更可靠，泛化更好。

Conclusion: 法律推理本质上是组合性的，神经符号系统为法律AI提供更可靠的基础。
...
### [350] [Sign-Separated Finite-Time Error Analysis of Q-Learning](https://arxiv.org/abs/2605.16103)
*Donghwan Lee*

Main category: cs.AI

TL;DR: 对常步长Q-learning进行符号分离的有限时间误差分析，通过切换系统分解误差为负正部分，负侧由最优策略的LTI系统控制，正侧由线性切换系统控制，揭示了最大诱导不对称性与过估计的关系。


<details>
  <summary>Details</summary>
Motivation: 理解Q-learning误差动态中的不对称性及其与过估计的关系，获得更精确的有限时间收敛界。

Method: 将Q-learning误差表示为切换系统，分解为负正两部分，负部分用固定最优策略的LTI系统控制，正部分用线性切换系统控制，推导有限时间指数界。

Result: 负侧LTI证书的收敛速率不慢于正侧切换证书，且可能更快；识别了最大诱导不对称性：正误差被Bellman最大值选择传播，负误差受限于最优策略。

Conclusion: 符号分离分析揭示了Q-learning的不对称性，提供了确定性和随机常步长递归的有限时间界。
...
### [351] [ShopGym: An Integrated Framework for Realistic Simulation and Scalable Benchmarking of E-Commerce Web Agents](https://arxiv.org/abs/2605.16116)
*Chinmay Savadikar,Mingyu Zhao,Yuanzheng Zhu,Han Li,Shuang Xie,Alberto Castelo,Tianfu Wu,Lingyun Wang*

Main category: cs.AI

TL;DR: ShopGym是一个集成框架，通过ShopArena将真实商店转化为沙盒环境，并由ShopGuru生成多样化基准任务，实现电商Web代理的可控、可复现评估。


<details>
  <summary>Details</summary>
Motivation: 现有方法在真实性与可控性之间存在权衡，缺乏可扩展的方式来构建既真实又多样、可控、可检查且可复现的评估环境。

Method: 框架包括ShopArena（通过匿名商店规范和分阶段生成过程将真实商店转化为自包含沙盒）和ShopGuru（在沙盒商店上合成涵盖7种技能类别的基准任务）。

Result: 在3个合成和3个真实商店上生成224个任务进行验证，合成商店保留了关键结构属性，代理在合成商店上的性能与真实商店正相关。

Conclusion: ShopGym能够生成自包含、可重置、可检查且稳定的评估工件，为电商Web代理提供现实且可控的基准测试。
...
### [352] [Property-Guided LLM Program Synthesis for Planning](https://arxiv.org/abs/2605.16142)
*Augusto B. Corr\^ea,Andr\'e G. Pereira,Jendrik Seipp*

Main category: cs.AI

TL;DR: 提出属性引导的LLM程序合成方法，通过反例反馈减少生成和评估成本，在PDDL规划域上合成直接启发式函数，效果显著优于之前方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM程序合成依赖简单数值分数，无法提供失败原因，导致生成大量候选程序，推理和评估成本高昂。

Method: 检查候选程序是否满足形式化属性，若违反则提供具体反例并提前停止评估，通过反例引导的修复循环生成和优化程序。

Result: 在十个PDDL规划域上，合成程序数量平均减少7倍，无需搜索解决更多任务，评估计算量降低数个数量级。

Conclusion: 对于存在可验证属性的问题，属性引导的LLM合成能显著降低成本并提升程序质量。
...
### [353] [Look Before You Leap: Autonomous Exploration for LLM Agents](https://arxiv.org/abs/2605.16143)
*Ziang Ye,Wentao Shi,Yuxin Liu,Yu Wang,Zhengzhou Cai,Yaorui Shi,Qi Gu,Xunliang Cai,Fuli Feng*

Main category: cs.AI

TL;DR: 大语言模型智能体在陌生环境中常因过早利用先验知识而失败，通过引入探索检查点覆盖率和探索-执行交错训练策略，提出先探索后行动范式，显著提升智能体的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于大语言模型的智能体在陌生环境中往往因过早依赖先验知识而表现不佳，缺乏自主探索能力是构建适应性智能体的关键瓶颈。

Method: 提出探索检查点覆盖率作为可验证度量，并开发了任务执行轨迹与探索轨迹交错训练的策略，每种轨迹由对应的可验证奖励优化，形成探索后执行范式。

Result: 实验表明，仅采用标准任务导向强化学习的智能体表现出狭窄重复的行为，而所提方法显著提升了下游任务性能。

Conclusion: 系统性探索学习对于构建通用且适应现实世界的智能体至关重要。
...
### [354] [An Algebraic Exposition of the Theory of Dyadic Morality](https://arxiv.org/abs/2605.16153)
*Kush R. Varshney*

Main category: cs.AI

TL;DR: 本文用代数方法形式化二元道德理论（TDM），通过结构因果建模（SCM）和三个心理算子解释道德判断，并展示其在AI政策设计中的应用。


<details>
  <summary>Details</summary>
Motivation: 为认知科学中的道德判断模型提供数学基础，并解决TDM在扩展多节点场景时的可扩展性问题。

Method: 使用SCM符号形式化TDM，定义类型化算子、完成算子及效价依赖推理机制，通过节点坍缩和顺序处理压缩多节点场景。

Result: 代数框架可有效检测冲突义务、设计保持用户自主性的帮助策略，并将事后沟通建模为因果干预。

Conclusion: 该形式化使得神经符号AI系统能以数学严谨且符合人类道德认知的方式计算道德，建议采用有范围的情境化心智感知测量而非通用平均化。
...
### [355] [Formal Methods Meet LLMs: Auditing, Monitoring, and Intervention for Compliance of Advanced AI Systems](https://arxiv.org/abs/2605.16198)
*Parand A. Alamdari,Toryn Q. Klassen,Sheila A. McIlraith*

Main category: cs.AI

TL;DR: 提出结合形式化方法与机器学习的AI审计监控技术，利用线性时态逻辑检测违规，实验优于LLM基线。


<details>
  <summary>Details</summary>
Motivation: 为了在AI全生命周期中监控审计产品服务的行为约束（如安全、规范），需要离线审计和在线监控方法。

Method: 结合线性时态逻辑的语法语义与机器学习，实现离线审计和在线运行时监控，并引入基于采样的预测监控和干预监控。

Result: 基于LTL的方法在检测时间扩展行为约束违规上优于LLM基线；小模型标注者达到或超越前沿LLM；预测和干预监控显著降低违规率且保持任务性能；时间推理能力随距离、约束数和命题数增加而下降。

Conclusion: 提出的监控审计技术有效，但LLM时间推理存在局限，需进一步改进。
...
### [356] [Context, Reasoning, and Hierarchy: A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP](https://arxiv.org/abs/2605.16205)
*Igor Bogdanov,Chung-Horng Lung,Thomas Kunz,Jie Gao,Adrian Taylor,Marzia Zaman*

Main category: cs.AI

TL;DR: 在对抗性部分可观测环境中，程序化状态抽象和层次分解比深度推理更有效，且推理工具与层次结合会导致性能下降（称为“推理级联”）。


<details>
  <summary>Details</summary>
Motivation: 缺乏关于复合LLM代理设计选择（如观测、推理、任务分解）对性能与成本影响的指导。

Method: 在CybORG CAGE-2环境（POMDP）中，对5个模型家族、6个模型和12种配置（共3475个回合）进行控制实验，变化上下文表示、推理工具和层次分解。

Result: 程序化状态抽象每令牌收益最大（最高提升76%）；推理工具分散到层次中导致性能下降（最多3.4倍）并增加令牌消耗（1.8-2.7倍），称为“推理级联”；无推理的层次分解取得最佳绝对性能，上下文工程比推理更经济。
...
### [357] [Confirming Correct, Missing the Rest: LLM Tutoring Agents Struggle Where Feedback Matters Most](https://arxiv.org/abs/2605.16207)
*Tahreem Yasir,Wenbo Li,Sam Gilson,Sutapa Dey Tithi,Xiaoyi Tian,Tiffany Barnes*

Main category: cs.AI

TL;DR: LLM在辅导中诊断精准度存在系统性问题，尤其对次优和错误解法判断不准，且诊断准确不等于教学有效，建议混合架构。


<details>
  <summary>Details</summary>
Motivation: LLM被用作智能辅导系统的对话补充，但其诊断精准度尚待评估，尤其区分最优、次优和错误解的能力至关重要。

Method: 构建基准测试，包含7个LLM反馈代理、10836个解-反馈对，基于知识图谱的ground truth，在三种反馈条件下评估诊断性能。

Result: 模型在最优步骤接近完美，但系统性过度拒绝有效次优解、过度验证错误解；问题跨模型持续，且诊断准确未带来可靠的教学反馈。

Conclusion: LLM更适合混合架构：知识图谱模型负责诊断，LLM负责开放式脚手架和对话。
...
### [358] [Fully Open Meditron: An Auditable Pipeline for Clinical LLMs](https://arxiv.org/abs/2605.16215)
...
TL;DR: 首次提出完全开源的医学LLM决策支持系统管道，在保持可审计性和可重复性的同时达到领域最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的临床决策支持系统大多不透明，缺乏完全开源模型，无法实现严格的验证和重现。

Method: 构建包含临床医生审核的训练语料库、可重复的数据构建和训练框架，以及使用LLM-as-a-judge的评估协议。

Result: Apertus-70B-MeditronFO在综合医学基准上提升6.6个百分点，建立新的完全开源最先进水平；Gemma-3-27B-MeditronFO优于MedGemma。

Conclusion: 完全开源管道能在不牺牲审计性和可重复性的情况下实现领域最先进性能。
...
### [359] [FORGE: Self-Evolving Agent Memory With No Weight Updates via Population Broadcast](https://arxiv.org/abs/2605.16233)
*Igor Bogdanov,Chung-Horng Lung,Thomas Kunz,Jie Gao,Adrian Taylor,Marzia Zaman*

Main category: cs.AI

TL;DR: 提出FORGE协议，通过阶段式群体演化、记忆注入与广播机制，将反射内环与渐进外环结合，在CAGE-2任务中显著提升LLM代理的决策表现，无需梯度更新。


<details>
  <summary>Details</summary>
Motivation: 探索无需梯度更新的LLM代理能否通过自生成记忆优化决策，当前零样本性能极差且分布尾重。

Method: FORGE包含反思内环（失败轨迹转为规则、示例或混合记忆）和群体外环（广播最佳实例记忆，冻结收敛实例），采用阶段式渐进演化。

Result: 所有12个模型-表示组合中，FORGE平均回报提升至零样本的1.7-7.7倍，较Reflexion基线提高29-72%；严重失败率降至约1%。

Conclusion: 群体广播是核心机制，示例记忆回报最高，规则记忆性价比最佳，且较弱模型获益更大，但结论限于CAGE-2环境。
...
### [360] [Prospective multi-pathogen disease forecasting using autonomous LLM-guided tree search](https://arxiv.org/abs/2605.16238)
*Sarah Martinson,Michael P. Brenner,Martyna Plomecka,Brian P. Williams,Nicholas G. Reich,Zahra Shamsi*

Main category: cs.AI

TL;DR: 使用LLM引导的树搜索自主生成传染病预测模型，其集成模型在实时评估中匹配或优于人工策划的CDC集成模型。


<details>
  <summary>Details</summary>
Motivation: 传统传染病概率预测依赖专家手工模型，导致可扩展性差，难以应对细粒度地理分辨率或新兴病原体。

Method: 采用大型语言模型（LLM）引导的树搜索，迭代生成、评估和优化预测软件；聚合机器生成的模型形成集成；使用对数尺度距离指标优化和自动化法官保障结构忠实性。

Result: 在2025-2026年美国呼吸道季节前瞻性评估中，系统生成的集成模型在样本外预测中持续匹配或优于CDC的人工策划集成模型；成功处理RSV冷启动场景。

Conclusion: 该框架通过自主将流行病学理论转化为代码，克服了建模劳动力瓶颈，能快速大规模部署专家级疾病预测。
...
### [361] [CitePrism: Human-in-the-Loop AI for Citation Auditing and Editorial Integrity](https://arxiv.org/abs/2605.16000)
...
TL;DR: 提出CitePrism，一个结合大语言模型和语义相似度的混合决策支持框架，用于编辑引文审核，初步验证显示可用于保守筛选，但需进一步验证。


<details>
  <summary>Details</summary>
Motivation: 编辑和审稿人需要确保引文质量，但手动审核费时费力，缺乏可扩展的自动化方法。

Method: 结合LLM辅助上下文推理、基于嵌入的语义相似度、元数据验证、完整性标志和人工审核，提取引文邻域、丰富元数据、计算融合相关分数，并支持可配置阈值分类。

Result: 在含104条引用的案例研究中，与人工二元相关标签的Cohen's kappa为0.429；在阈值17时标记了所有人工标记为不相关的引用，但产生了需要人工审核的误报。

Conclusion: CitePrism可作为保守的编辑筛选和引文质量分类工具，但并非自动不当行为检测器或自动决策系统，需要在更多领域和场景中验证后才能投入实际使用。
...
### [362] [Symplectic Neural Operators for Learning Infinite Dimensional Hamiltonian Systems](https://arxiv.org/abs/2605.15881)
*Yeang Makara,Yusuke Tanaka,Takashi Matsubara,Takaharu Yaguchi*

Main category: math.DS

TL;DR: 提出一种保辛结构的神经算子（SNO），用于无限维哈密顿系统建模，理论证明其辛性与长期稳定性，实验显示其能量守恒优于非保结构算子。
...
Method: 设计保辛结构的神经算子架构，严格刻画其辛性，并建立结合结构保持与学习精度的长期稳定性理论。

Result: 理论上证明了SNO的辛性与长期稳定性，数值实验验证了其改善的能量行为。

Conclusion: SNO能够有效保持哈密顿系统的辛结构，提升长期模拟的稳定性和能量守恒。
...
### [363] [Intrinsic-dimension empirical Bernstein inequalities for bounded self-adjoint operators](https://arxiv.org/abs/2605.15278)
*Diego Martinez-Taboada,Aaditya Ramdas*

Main category: math.ST

TL;DR: 提出了首个经验Bennett和Bernstein不等式，用于有界紧自伴算子之和，以数据驱动方式替代未知方差，仅依赖内在维度，获得无维度、可计算的界限，并在非各向同性随机矩阵和无限维希尔伯特空间中更优，且达到渐近最优性。


<details>
  <summary>Details</summary>
Motivation: 标准算子值浓度不等式需要先验已知方差，且随环境维度增长，对于高维或无限维算子无效，因此需要无需先验知识、维度自由的界限。

Method: 基于独立有界紧自伴算子之和，用经验方差估计替代未知方差，并利用内在维度替代环境维度推导经验Bennett和Bernstein不等式。

Result: 得到完全数据驱动的、无环境维度的浓度界限，对于非各向同性随机矩阵更锐利，且可扩展到无限维，并达到了与已知最优oracle界限相同的渐近精度。同时，作为副产品，给出了内在维度本身的经验浓度保证。

Conclusion: 这些经验不等式提供了一种实用的、维度自由的工具，可广泛用于高维统计和随机算法分析。
...
### [364] [Bounds on the Number of Modes of a Gaussian Mixture Density](https://arxiv.org/abs/2605.15531)
*Hien Duy Nguyen*

Main category: math.ST

TL;DR: 本文推导了高斯混合密度中非退化临界点及模态数量的显式上下界，并给出了多种情况下的改进。


<details>
  <summary>Details</summary>
Motivation: 为理解高斯混合模型的基本结构，需要知道临界点（尤其是模态）的最大可能数量。

Method: 通过归一化临界点方程、Pfaffian方法、代数互反变量、Morse理论、同方差约简、组合构造、填充乘积和种子闭包原理。

Result: 对于k≥2，得到上界U_het、U_aug及其最小值，Morse改进，同方差下U_hom和U_aug,hom；下界L_bin及线性下界d+k-1。

Conclusion: 给出了高斯混合密度临界点数量的紧确界限，有助于理解其几何性质。
...
### [365] [Goodness-of-Fit Testing for Point Processes in Large Populations](https://arxiv.org/abs/2605.15814)
*Sami Umut Can,Estate V. Khmaladze,Roger J. A. Laeven*

Main category: math.ST

TL;DR: 提出一种基于酉变换的拟合优度检验方法，用于检验点过程的条件强度是否属于特定参数族，实现渐近分布无关性。


<details>
  <summary>Details</summary>
Motivation: 现有检验方法依赖特定参数形式，缺乏通用性。

Method: 构造自然参数检验过程的酉变换，使其弱收敛于标准目标过程。

Result: 蒙特卡洛模拟显示方法在多种生存模型和软件可靠性模型中具有良好的有限样本性能。

Conclusion: 该方法为参数点过程的分布无关拟合优度检验提供了有效途径。
...
### [366] [Edge-indexed network time series with graph Ornstein-Uhlenbeck dynamics](https://arxiv.org/abs/2605.15907)
*Jiaming Chen,Almut E. D. Veraart*

Main category: math.ST

TL;DR: 提出一类Lévy驱动的图Ornstein-Uhlenbeck (grOU)模型，用于边索引网络时间序列，扩展了广义网络自回归过程到连续时间，并处理边索引数据。


<details>
  <summary>Details</summary>
Motivation: 将广义网络自回归过程扩展至连续时间，并适应边索引设置，同时引入Lévy噪声以同时捕捉布朗运动和跳跃行为。

Method: 通过最大似然估计框架估计模型参数，并推导估计量的渐近性质。

Result: 模拟研究验证了有限样本性能，高频金融数据实证表明grOU模型相比标准基准提高了预测精度并减少了计算时间，同时保持鲁棒性。

Conclusion: grOU模型有效提升了边索引网络时间序列的预测性能与计算效率，网络化参数化提供了鲁棒性。
...
### [367] [Node-private community estimation in stochastic block models: Tractable algorithms and lower bounds](https://arxiv.org/abs/2605.15943)
*Laurentiu Marchis,Ethan D'souza,Tom\'a\v{s} Fl\'idr,Po-Ling Loh*

Main category: math.ST

TL;DR: 研究节点差分隐私下随机块模型的社区恢复问题，提出多项式时间算法并推导隐私参数增长率的下界。


<details>
  <summary>Details</summary>
Motivation: 现有方法在节点差分隐私下需快速增大隐私参数ε才能保证一致估计，需要新算法缓解该问题。

Method: 基于谱聚类，采用两种新方法：1) 指数机制与Lipschitz扩展采样；2) 构建从无向图到有界度图的平滑投影以结合边私有算法。

Result: 算法在节点数多项式时间内可计算，并给出了ε增长率的下界。

Conclusion: 所提方法能在节点差分隐私下实现社区恢复，且分析了非标准缩放ε→∞时的隐私分析，HGR最大相关可用于PAC学习中的精度放大。
...
### [368] [Nearest-Neighbour Matching on Unbounded Supports and Covariate Shift Transfer](https://arxiv.org/abs/2605.16027)
*Simon Viel*

Main category: math.ST

TL;DR: 本文研究了带有缺失标签的多变量函数期望的非参数估计，发现使用最近邻匹配可以在极弱的协变量支撑假设下达到通常的收敛速度，并引入了可迁移性度量来控制分布间的差异。


<details>
  <summary>Details</summary>
Motivation: 现有非参数估计方法通常假设协变量支撑集为紧致子集且密度有界，这限制了其应用范围；本文旨在放宽这些假设，并探索在更一般分布（如流形支撑、重尾分布）下的收敛性质。

Method: 采用最近邻匹配估计器，将传统假设替换为源分布和目标分布之间的条件，包括一个可迁移性度量，并证明该度量对获得好的收敛速度是必要的。

Result: 在最小协变量支撑假设下，通常的收敛速度仍可达到；可迁移性条件适用于流形支撑分布，并允许目标分布具有比源分布更重的尾部；若可迁移性不足，则任何估计器都无法获得好速率。

Conclusion: 该结果放宽了处理效应估计中分配概率需远离0和1的假设，表明可迁移性是影响估计效率的关键因素，为实际应用提供了更灵活的理论保证。
...
### [369] [Tests for the mean of high-dimensional data](https://arxiv.org/abs/2605.16033)
*Dietmar Ferger*

Main category: math.ST

TL;DR: 提出了一种基于统计量V_n=n||X_n||^2的高维数据均值检验方法，无需协方差矩阵求逆，适用于维度可任意增长的情景，并建立了渐近分布理论和自助法近似。


<details>
  <summary>Details</summary>
Motivation: 高维数据均值检验面临维度增长无限制的挑战，传统方法依赖协方差矩阵求逆，难以应用。

Method: 采用统计量V_n=n||X_n||^2，通过将观测嵌入希尔伯特空间l2，建立了固定维和增长维下的渐近分布，并证明了自助法近似的有效性，无需稀疏性或结构假设。

Result: 证明了新的l2中心极限定理作为核心工具，检验统计量渐近分布成立，自助法检验能实现渐近水平α控制。

Conclusion: 提供了一种在高维数据中无需协方差结构假设的均值检验方法，推广了适用场景。
...
### [370] [Do Biological Structural Guarantees Earn Their Complexity?](https://arxiv.org/abs/2605.15225)
*Bogdan Banu*

Main category: q-bio.QM

TL;DR: 通过三个深度基准测试，比较了受生物启发的AI框架与简单替代方案的可靠性。


<details>
  <summary>Details</summary>
Motivation: 受生物启发的AI框架声称具有可靠性优势，但缺乏与简单替代方案的实证比较。

Method: 设计了三个基准测试：代谢优先级门控、基于自诱导剂的群体感应和贝叶斯停滞检测，每个测试包括生物实现、朴素非生物替代和消融对照，每个种子1000次试验，共10个种子，总计1000万+数据点。

Result: 摘要未报告具体结果，仅描述了基准测试设置。

Conclusion: 需要进行更严格的实证测试来验证受生物启发框架的可靠性优势。
...
### [371] [From Full and Partial Intraoral Scans to Crown Proposal: A Classification-Guided Restoration Assistance Pipeline](https://arxiv.org/abs/2605.15241)
...
TL;DR: 提出了一种端到端管道，从原始口内扫描和靶牙FDI编号生成单冠初始建议，解决部分扫描分割和生成过光滑问题。


<details>
  <summary>Details</summary>
Motivation: 单冠修复中部分扫描优于全弓，但现有分割网络无法处理部分扫描，生成式方法产生过光滑表面丢失细节。

Method: 三阶段管道：数据准备与姿态标准化；基于扫描类型的分割（DGCNN分类、RANSAC+ICP配准、图割优化）；上下文感知检索与Blender拟合生成冠。

Result: 在1958个部分扫描上，宏观平均DSC 0.9249，召回率0.8919，精确率0.9615；目标牙及邻牙DSC 0.9468-0.9569，质心误差0.2666-0.2774毫米；生成耗时2.5-3.5分钟。

Conclusion: 该管道为临床提供了一种实用且高效的冠生成替代方案，兼顾准确性与细节。
...
### [372] [Highly Detailed and Generalizable Broadleaf Tree Crown Instance Segmentation from UAV Imagery](https://arxiv.org/abs/2605.15673)
...
### [373] [Fault tolerance estimation in digital circuits with visualised generative networks](https://arxiv.org/abs/2605.15212)
*Sascha Biel,Carl Alexander Gaede,Amiel Glaser,Jan Wolter,Alexej Schelle*

Main category: cs.AR

TL;DR: 提出一种基于生成对抗网络采样技术的新数值方法，用于估计数字电路结构中失效模式的容错性。


<details>
  <summary>Details</summary>
Motivation: 需要评估数字电路设计对各类失效模式（如逻辑元件缺失或互换）的容错能力。

Method: 利用生成对抗网络的判别器，将随机生成的理想数字信号与数值实验的真实信号进行比较，计算偏差，并基于复变量分析区分不同逻辑元件的失效影响。

Result: 能够评估电子设计的鲁棒性，并区分与不同经典逻辑元件相关的失效模式的影响。

Conclusion: 该方法可有效评估数字电路的容错性，为设计鲁棒性分析提供新途径。
...
### [374] [GenAI-Driven Approach to RISC-V Supply Chain Exploration](https://arxiv.org/abs/2605.15223)
*Nenad Petrovic,Andre Schamschurko,Yingjie Xu,Alois Knoll*

Main category: cs.AR

TL;DR: 提出一种结合LLM、VLM和MDE的工作流，用于RISC-V供应链分析，通过构建知识图谱和约束建模提升供应链透明度与韧性。


<details>
  <summary>Details</summary>
Motivation: RISC-V供应链数据异构且非结构化，现有方法难以综合多模态信息进行有效分析。

Method: 利用LLM理解文本，VLM从图表、表格等视觉材料中提取信息，构建知识图谱；结合MDE和约束建模进行形式化验证、瓶颈检测和风险评估；引入人机交互机制。

Result: 在RISC-V生态系统场景中验证，该方法能生成可操作见解，增强透明度，支持复杂半导体供应链决策。

Conclusion: LLM与MDE的协同作用支持供应链韧性的探索性与系统性评估，有效提升分析能力。
...
### [375] [Is Agentic AI Ready for Real-World Hardware Engineering? A Deep Dive with Phoenix-bench](https://arxiv.org/abs/2605.15226)
*Qingyun Zou,Feng Yu,Hongshi Tan,Bingsheng He,WengFai Wong*

Main category: cs.AR

TL;DR: 提出Phoenix-bench基准，评估AI系统在硬件工程上的表现，发现软硬件任务根本不同，定位粒度比定位本身更重要，测试反馈大幅提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有硬件LLM基准仅隔离子任务，缺乏联合仓库导航、层次定位、EDA验证和维护补丁的评估，因此需构建更全面的基准。

Method: 构建511个Verilator实例（来自114个GitHub仓库），包含开发者补丁、设计标签、测试台和固定EDA环境；统一评估4个商业代理和8个开源结构，使用4种LLM后端及两种诊断干预。

Result: 1) 同一代理从SWE-bench到Phoenix-bench性能下降37%-58%；2) 失败集中在FSM/控制流bug、测试台bug和跨层次信号流问题；3) 文件级oracle仅提升1.4%，而单轮测试反馈提升42%-45%。

Conclusion: 硬件工程对AI系统构成独特挑战，需要更好的层次定位和测试反馈机制，而非仅依赖软件工程策略。
...
### [376] [A3D: Agentic AI flow for autonomous Accelerator Design](https://arxiv.org/abs/2605.15237)
*Abinand Nallathambi,Christopher Knight,Shantanu Ganguly,Wilfried Haensch,Anand Raghunathan*

Main category: cs.AR

TL;DR: 提出A3D，一个基于智能体AI的端到端硬件加速器设计自动化流程，无需人工干预即可从复杂应用生成加速器。


<details>
  <summary>Details</summary>
Motivation: 硬件加速器设计高度依赖专家知识，现有方法（如HLS）仍需要大量手动工作，限制了应用范围。

Method: A3D利用多个专业智能体协作，包括工作负载分析、性能瓶颈识别、HLS兼容性代码重构和微架构生成，并通过智能体RAG探索代码库和EDA工具文档，自动探索速度-面积权衡空间。

Result: 在LAMMPS和QMCPACK等复杂科学应用上，A3D无需任何人工干预成功生成了加速器设计。

Conclusion: A3D展示了智能体AI在自动化硬件设计中的潜力，可降低设计门槛并提高效率。
...
### [377] [Hardware-Software Co-Design of Scalable, Energy-Efficient Analog Recurrent Computations](https://arxiv.org/abs/2605.15216)
*Arthur Fyon,Julien Brandoit,Loris Mendolia,Damien Ernst,Jean-Michel Redout\'e,Guillaume Drion*

Main category: cs.AR

TL;DR: 通过硬件-软件协同设计，利用双稳态记忆递归单元（BMRU）实现超低功耗的模拟递归神经网络，克服了噪声积累问题，在关键词检测中达到亚微瓦推理。


<details>
  <summary>Details</summary>
Motivation: 现有模拟AI电路仅支持前馈架构，扩展至递归动态因噪声累积被认为不切实际，因此需要探索新的设计方法。

Method: 采用BMRU（一种具有离散输出和滞回动力学的RNN），设计超低功耗电流模式模拟电路，建立参数与电路元件的一一对应；重新公式化BMRU以支持第一象限操作和固定阈值，保持表现力和可训练性。

Result: 180nm CMOS晶体管级仿真显示软件预测与电路行为高度一致，噪声在每个细胞边界被抑制至少20倍；递归功耗与状态维度线性增长，前馈层功耗平方增长，递归边际成本低；端到端关键词检测中RNN核心功耗低于1微瓦。

Conclusion: 证明通过硬件-软件协同设计和BMRU可有效实现模拟递归神经网络，避免噪声积累，实现超低功耗推理。
...
### [378] [Domain-Independent Game Abstraction using Word Embedding Techniques](https://arxiv.org/abs/2605.15543)
*Juho Kim,Tuomas Sandholm*

Main category: cs.GT

TL;DR: 提出一种领域无关的游戏抽象方法，利用词嵌入技术将动作表示为向量并聚类，实验表明有效但未超越专业算法。


<details>
  <summary>Details</summary>
Motivation: 现有游戏抽象技术大多针对特定领域（如扑克），难以泛化到其他游戏，需要一种通用的抽象方法。

Method: 借鉴自然语言处理的词嵌入技术，将每个动作视为单词，游戏数据作为语料，训练动作的向量表示，然后通过聚类实现游戏抽象。

Result: 该方法在实验中表现出有效性，但未能超越针对特定游戏设计的专门算法。

Conclusion: 领域无关的词嵌入方法可以用于游戏抽象，但性能上仍不如专门算法，未来可进一步优化。
...
### [379] [Misspecified Explore-then-Exploit Leads to Supra-Competitive Prices](https://arxiv.org/abs/2605.16064)
*Jackie Baek,Vivek F. Farias,Farrell Wu*

Main category: cs.GT

TL;DR: 研究显示，使用探索-利用管道的简单算法定价系统，当探索范围相似时，可能收敛到高于纳什均衡的超竞争价格，甚至达到垄断水平。


<details>
  <summary>Details</summary>
Motivation: 探究简单算法定价系统是否会在多公司市场中系统性地产生类似合谋的价格。

Method: 分析采用探索-利用管道的公司：探索期随机定价，然后基于自身历史数据（忽略竞争者价格）用错误垄断模型估计需求并短视定价，通过流体极限常微分方程分析收敛性。

Result: 当公司在纳什价格同一侧相似范围内探索时，价格收敛到超竞争水平；对称探索下可达垄断价格。模拟显示结果在有限时间、异质产品和非线性需求下仍稳健。

Conclusion: 简单的算法定价系统无需直接勾结即可导致高于竞争的价格，这暗示需要关注算法定价的竞争效应。
...
### [380] [Nonlocal Optical Response and Surface Susceptibilities: A Systematic Derivation via Spatial Moment Expansion](https://arxiv.org/abs/2605.15716)
*Fr\'ed\'eric Zolla*

Main category: physics.optics

TL;DR: 提出将非局部响应核与任意弯曲界面有效表面敏感度相联系的系统理论，将界面响应凝聚为单一标量χ^s，并推广Feibelman d参数到任意曲率。


<details>
  <summary>Details</summary>
Motivation: 需要统一描述非局部介质中任意弯曲界面的电磁响应，超越传统平面界面参数的限制。

Method: 从一般张量非局部本构关系出发，结合空间矩展开和分布薄层极限，推导出表面敏感度标量及曲率修正项。

Result: 得到表面敏感度χ^s与几何不变量H、K的关系，并在平面、球面等界面和多种核函数下验证，建立广义麦克斯韦边界条件。

Conclusion: 该理论为任意曲率界面的非局部响应提供了统一框架，广义边界条件可替代经典Fresnel公式。
...
### [381] [Covert Bayesian Quickest Change Detection](https://arxiv.org/abs/2605.16140)
*Yun-Feng Lo,Matthieu R. Bloch*

Main category: cs.IT

TL;DR: 研究了贝叶斯无限时间视野下的隐蔽快速变化检测问题，提出了预期隐蔽预算（ECB）作为度量，在虚警概率和ECB约束下建立了平均检测延迟的二阶渐近逆界，并设计了匹配的恒定探测概率策略。


<details>
  <summary>Details</summary>
Motivation: 在对手监视信道的场景下，合法实体需要快速检测信道状态变化，同时保持探测行为隐蔽，现有方法缺乏精确的量化分析。

Method: 引入ECB度量，约束主动与被动探测观测序列的相对熵；采用二阶渐近分析推导平均检测延迟的下界；提出恒定探测概率的Shiryaev型策略。

Result: 对于任意正ECB约束，当虚警概率趋近零时，获得了平均检测延迟的二阶渐近逆界，并证明了所提策略可达到该界，量化了最大平方根阶隐蔽感知增益。

Conclusion: 通过ECB度量和二阶渐近分析，首次严格量化了隐蔽快速变化检测的性能折中，所提策略理论上最优。
...
### [382] [Breaking the Finite-Sample Barrier in Entropy Coupling](https://arxiv.org/abs/2605.16229)
*Shahab Asoodeh,Jun Chen*

Main category: cs.IT

TL;DR: 引入最小列表熵耦合，研究依赖观测如何打破有限样本障碍。依赖观测可精确消除不确定性，独立观测只能指数降低。给出了零熵区域的充要条件，并开发了贪心算法。


<details>
  <summary>Details</summary>
Motivation: 现有模型基于独立观测，无法捕捉依赖观测打破有限样本障碍的潜力。

Method: 允许观测任意依赖但固定边际，定义最小列表熵耦合；表征零熵条件；开发贪心算法。

Result: 依赖观测可在有限样本后实现零熵，独立观测仅指数降低；零熵在温和条件下只需O(log(1/P_min))个观测；贪心算法有单调逼近保证。

Conclusion: 该框架形式化了分布匹配表示学习和随机性提取中的有限样本极限，零熵对应精确恢复和精确提取。
...
### [383] [Learning Selective Merge Policies for Deadline-Constrained Coded Caching via Deep Reinforcement Learning](https://arxiv.org/abs/2605.15236)
*Amirhossein Yousefiramandi*

Main category: cs.IT

TL;DR: 针对延迟敏感应用中的编码缓存问题，提出基于深度强化学习的在线消息合并策略，显著降低广播数据包过期率并提升广播效率。


<details>
  <summary>Details</summary>
Motivation: 用户请求有严格截止时间，在线合并消息虽能形成编码多播，但可能影响后续合并，需要智能决策。

Method: 将截止时间约束的编码传递建模为掩蔽离散动作队列状态控制问题，使用图注意力策略网络并通过近端策略优化训练。

Result: 相比最好基线SACM++，广播数据包过期率ρ降低40.9%（0.208 vs 0.352），在Track A中取得最佳广播效率分数σ，策略网络合并率仅约31.8%。

Conclusion: 对于紧迫截止时间用户，选择性合并优于激进合并，策略网络学习到低合并率以平衡当前与未来效益。
...
### [384] [PrismQuant: Rate-Distortion-Optimal Vector Quantization for Gaussian-Mixture Sources](https://arxiv.org/abs/2605.15507)
*Bumsu Park,Chanho Park,Youngmok Park,Namyoon Lee*

Main category: cs.IT

TL;DR: 针对多模态高斯源，提出PrismQuant变换编码方法，利用高斯混合模型和分量标签无损传输，达到接近理论率失真界的性能。


<details>
  <summary>Details</summary>
Motivation: 经典变换编码对多模态源（如高斯混合）失效，因为协方差无法捕捉局部几何，率失真函数无闭式解，需要发展构造性率失真理论。

Method: 基于高斯混合源，证明条件率失真函数由全局反流水填充水平控制；提出PrismQuant：无损传输分量标签，使用分量匹配的KLT和标量量化；实践采用EM学习、分量自适应KLT和ECSQ。

Result: 合成高斯混合数据上接近理论率失真界；真实CSI数据上，以小于1/10的模型尺寸达到与Transformer编解码器相当或更优的性能。

Conclusion: 所提方法在理论上有渐近最优性，实践上高效且轻量，适用于多模态数据压缩。
...
### [385] [GeoGS-CE: Learning Delay--Beam Channel Priors with 3D Gaussians for High-Mobility Scenarios](https://arxiv.org/abs/2605.16094)
*Yumeng Zhang,Jiajia Guo,Chaozheng Wen,Chenghong Bian,Jun Zhang*

Main category: cs.IT

TL;DR: 提出GeoGS-CE框架，利用高移动性场景中稳定延迟-波束功率谱的几何先验，实现稀疏导频下的宽带信道估计。


<details>
  <summary>Details</summary>
Motivation: 高移动性场景下信道变化快且导频稀疏，传统方案估计困难，但高速铁路等环境具有稳定延迟-波束功率谱，可提供几何信息。

Method: 两阶段框架：离线阶段建模3D高斯场景表示（捕获NLoS散射）和可微渲染过程（含显式LoS分量），预测延迟-波束功率谱；在线阶段将功率谱作为协方差先验，通过线性MMSE估计器重构CFR。

Result: 基于广深高铁段信道仿真表明，几何先验显著提升CFR重构精度，优于纯导频和非几何基线方法。

Conclusion: 利用环境几何先验可有效改善高移动性稀疏导频信道估计性能。
...
### [386] [Generalized Error Bounds in the Recovery of Solitary Wave Profiles](https://arxiv.org/abs/2605.15590)
*Daniel Sinambela*

Main category: math.CA

TL;DR: 研究了Constantin显式重构公式对波速、水深和压力扰动的鲁棒性，给出了L2误差估计和数值验证。


<details>
  <summary>Details</summary>
Motivation: 评估公式在实际应用中参数不准确时的可靠性。

Method: 利用hodograph变换、全纯延拓和Paley-Wiener估计进行理论分析，并辅以数值计算。

Result: 推导出重构轮廓的L2误差估计，证明稳定性对扰动大小呈次线性依赖。

Conclusion: 该公式对同时扰动具有鲁棒性，数值实验验证了理论结果。
...
### [387] [Operator-Controlled 6G: From Connectivity Infrastructure to Guaranteed Digital Services](https://arxiv.org/abs/2605.15553)
*David Soldani*

Main category: cs.NI

TL;DR: 本文认为6G必须扭转供应商主导的架构趋势，以运营商控制优先，并通过两个贡献（6G控制契约和保证经济）实现。基于乐天移动的部署证据，提出运营商主导的6G是可行的。


<details>
  <summary>Details</summary>
Motivation: 前五代移动网络由供应商主导，导致运营商无法掌控网络、修改平台或审计AI层，6G需要重新排序优先权：控制、客户、商业、运营先行，技术最后。

Method: 提出6G控制契约（三层所有权分类：自有、联邦、消费）和保证经济（五级按结果定价的商业模型），将运营商控制转化为可执行的服务水平目标。

Result: 乐天移动的全国级全云原生Open RAN部署在FY2025实现全年EBITDA盈利，证明了运营商控制的可行性。

Conclusion: 6G的走向取决于2026-2028年的决策，运营商主导可确保6G成为保证数字服务的平台，而非另一个依赖供应商的基础设施周期。
...
### [388] [Thermodynamic Invariants of Coupled Channels: A Many-Channel Tolman-Ehrenfest Effect](https://arxiv.org/abs/2605.15653)
*Benjamin Hamblin,Victor Calo,Klaus Regenauer-Lieb*

Main category: math-ph

TL;DR: 多热力学通道耦合时单通道平衡条件失效，通过推广Tolman-Ehrenfest效应到熵流形，推导出n通道不变量ζ_i T_i = C，并用于解释颗粒材料的剪胀比和能量约束，解决了状态相关K_μ之谜，预言跨剪切带的ζ_Vχ为常数。


<details>
  <summary>Details</summary>
Motivation: 多热力学通道耦合时，单通道平衡条件失效，需要建立新的平衡描述。

Method: 将Tolman-Ehrenfest效应推广至熵流形，利用Ruppeiner几何的完整性和曲率，推导出n通道不变量的表达式。

Result: 得到n通道不变量ζ_i T_i = C，并显示Rowe的剪胀比和能量约束是几何曲率g_{Vσ}的自然结果，且状态相关K_μ的修正ζ_V在堵塞时达O(1)。

Conclusion: 预言ζ_Vχ跨剪切带为常数，该预测可通过实验验证，为理解颗粒材料提供新视角。
...
### [389] [Spectral separation of variables from equivalent Lagrangian systems](https://arxiv.org/abs/2605.15679)
*Mattia Scomparin*

Main category: math-ph

TL;DR: 研究了二次拉格朗日量的动力学等价性导致运动方程解耦的条件，并通过谱分解实现变量分离。


<details>
  <summary>Details</summary>
Motivation: 探索二次拉格朗日量的动力学等价性与变量分离之间的关系。

Method: 通过要求两个二次拉格朗日量生成相同欧拉-拉格朗日方程，推导出动能矩阵与势能Hessian矩阵的对易关系，并利用正交谱分解。

Result: 运动方程解耦为独立子系统，通常为块分离形式，谱简单时完全分离。

Conclusion: 该方法应用于Sawada-Kotera系统和n维Henon-Heiles模型，恢复了经典可积参数区域。
...
### [390] [Epstein vector zeta functions related to the ADE Lie algebras](https://arxiv.org/abs/2605.16042)
*M.Olshanetsky*

Main category: math-ph

TL;DR: 将Epstein zeta函数推广到ADE型李代数根格的向量值形式，构造theta级数并导出矩阵函数方程，分类了满足方程的格与不变子空间。


<details>
  <summary>Details</summary>
Motivation: 将Epstein zeta函数推广到向量值形式以研究ADE型根格的解析性质。

Method: 利用判别群D=P/Q构造向量值theta级数实现Weil表示，通过Mellin变换得到向量zeta函数，利用等变性推导矩阵函数方程。

Result: 导出了Riemann型矩阵函数方程，该方程仅在C-不变向量子空间上成立，并完成了满足条件的格与子空间的分类。

Conclusion: 向量值Epstein zeta函数在C-不变子空间上满足矩阵函数方程，且所有相关格已被分类。
...
### [391] [Ensuring Logic in the Fog: Sound POMDP Synthesis with LTL Objectives](https://arxiv.org/abs/2605.12581)
*Can Zhou,Yulong Gao,Pian Yu*

Main category: cs.LO

TL;DR: 提出一种基于LTL认证奖励塑造的蒙特卡洛规划框架，可在部分可观测环境下合成遵循复杂时序约束的自主智能体。


<details>
  <summary>Details</summary>
Motivation: 合成能在不确定环境中遵循复杂时序约束的自主智能体是一大挑战，而部分可观测马尔可夫决策过程中的LTL定性验证不可判定，导致定量合成困难。

Method: 设计一种新颖的、可靠的奖励塑造机制，动态生成基于信念的、由认证LTL满足性驱动的奖励，并集成到增强的蒙特卡洛规划框架中。

Result: 该方法在现有求解器失效的场景中表现优异，并在多个基准领域保持有效性和可扩展性。

Conclusion: 所提方法有效桥接了LTL规范与部分可观测下的定量合成之间的差距，提升了智能体在模糊环境中的可验证成功概率。
...
### [392] [Access Timing as Scaffolding: A Reinforcement Learning Approach to GenAI in Education](https://arxiv.org/abs/2605.15850)
*Janne Rotter,Pau Benazet i Montobbio,Davinia Hern\'andez-Leo*

Main category: cs.CY

TL;DR: 通过强化学习智能体控制生成式AI访问时机，发现策略性定时访问能提升学习表现和元认知准确性，减少错误和耗时，无需显式提示。


<details>
  <summary>Details</summary>
Motivation: 生成式AI在教育中虽普遍，但无限制使用易导致过度依赖、元认知脱离和学习效果下降，现有研究缺乏对访问时机这一隐性支架的实证探究。

Method: 基于强化学习智能体决定学生何时访问生成式AI，奖励函数融合元认知理论、认知负荷理论和生产性失败理论，在105人的混合方法控制实验中对比无限制、完全限制和智能体条件。

Result: 强化学习条件下，后测客观成绩和元认知准确性优于无限制访问，任务错误和用时少于完全限制访问，但自我报告元认知意识无显著差异。

Conclusion: 访问时机是一种可操作、有理论支撑且可扩展的教学范式，优于完全开放或完全禁止访问，兼容现有工具且易于推广，为教育中的人机交互设计开辟新方向。
...
### [393] [AI-Mediated Communication Can Steer Collective Opinion](https://arxiv.org/abs/2605.16245)
*Stratis Tsirtsis,Kai Rawal,Chris Russell,Brent Mittelstadt,Sandra Wachter*

Main category: cs.CY

TL;DR: 生成式AI在人类交流中引入方向性偏见，这些偏见通过网络放大，改变集体舆论。


<details>
  <summary>Details</summary>
Motivation: 现有研究关注AI对个人观点的影响，但忽视了AI中介人类交流对集体观点形成的影响。

Method: 结合实证和理论分析：实证测试多个LLM在争议文本上的编辑偏见；建立数学模拟模型，在真实社交网络数据上模拟。

Result: AI的偏见被网络放大，使集体观点偏向偏见方向；审计X平台发现亲生命偏见。

Conclusion: 讨论了对欧盟立法努力的更广泛影响。
...
### [394] [Bridging Silicon and the Hippocampus: Algebro-Deterministic Memory "VaCoAl" as a Substrate for Vector-HaSH and TEM](https://arxiv.org/abs/2605.15652)
*Hiroyuki Chuma,Kanji Otsuka,Yoichi Sato*

Main category: cs.NE

TL;DR: 提出VaCoAl作为超维记忆架构，统一了网格细胞支架、组合记忆和多跳重放衰减的代数基础，并证明了与海马回路的功能对应。


<details>
  <summary>Details</summary>
Motivation: 平行发展的文献缺乏共享代数对象，需要桥接矢量哈希、Tolman-Eichenbaum机器和人类iEEG发现。

Method: 基于伽罗瓦域LFSR的确定扩散机制，替代随机投影，保持准正交性；定义路径积分置信比CR2作为多跳重放保真度的函数形式。

Result: 满足二阶矩统计、更强的雪崩行为、位精确可重复性；CR2乘积形式自然拟合乘法衰减；STDP路径选择符合相似性保持、可逆性和边界搜索。

Conclusion: 建立形式对应，生成可测试的iEEG预测，桥接计算神经科学与超维工程，解释EC-CA3和EC-DG-CA3通路在进化中的保守性。
...
### [395] [Structure Abstraction and Generalization in a Hippocampal-Entorhinal Inspired World Model](https://arxiv.org/abs/2605.15733)
*Tianqiu Zhang,Muyang Lyu,Xiao Liu,Si Wu*

Main category: cs.NE

TL;DR: 提出一种大脑启发的层次模型，通过逆模型和HPC-MEC耦合同时推断潜在转换并构建视觉世界模型，实现结构抽象与泛化。


<details>
  <summary>Details</summary>
Motivation: 海马-内嗅皮层回路虽能表征空间和概念空间，但如何从连续高维动态中并发提取抽象结构的机制尚不明确。

Method: 采用逆模型进行结构提取，结合HPC-MEC耦合模型分离关系结构（MEC）与情景场景（HPC），并利用速度驱动路径积分实现跨上下文的预测与结构重用。

Result: 在原始变换动力学基准上验证了结构抽象能力，通过路径积分实现了鲁棒预测和结构泛化。

Conclusion: 该框架为理解大脑启发的自监督世界模型如何获取可重用抽象知识提供了新计算视角。
...
### [396] [Scalable neuromorphic computing from autonomous spiking dynamics in a clockless reconfigurable chip](https://arxiv.org/abs/2605.16114)
*Eric Oliveira Gomes,Damien Rontani*

Main category: cs.NE

TL;DR: 提出基于无时钟数字电路的神经形态架构，在FPGA上实现布尔脉冲神经元网络，在音频分类任务中达到竞争性能且功耗显著降低。


<details>
  <summary>Details</summary>
Motivation: 弥合数字与模拟神经形态系统之间的差距，避免专用硬件设计，实现低功耗高效处理。

Method: 利用无时钟数字电路的自发脉冲动力学，在商用FPGA上构建可配置突触权重的布尔脉冲神经元网络，并设计完整处理流水线处理脉冲编码数据。

Result: 在音频分类任务中实现竞争性性能和高速处理，功耗显著低于传统数字实现。

Conclusion: 无时钟数字硬件可作为神经形态计算的有效平台，推动可重构芯片成为能量高效的准模拟神经形态处理器。
...
### [397] [Coherent States of Non-Null Torus Knots](https://arxiv.org/abs/2605.15420)
*Gabriel Canadas da Silva,Ion Vasile Vancea*

Main category: quant-ph

TL;DR: 构建了对应于经典非零纽结电磁场解的量子相干态，并验证了其性质。


<details>
  <summary>Details</summary>
Motivation: 建立真空拓扑电磁系统的量子-经典对应。

Method: 从经典场与相干态振幅的关系推导位移算符，直接计算验证相干态定义性质，确定可观测量并计算期待值。

Result: 得到以整数参数表示的场期待值、能量密度、能流矢量、螺旋度、光子数、正交不确定度和关联函数等，并以Hopfion相干态为例。

Conclusion: 结果建立了此类真空拓扑电磁系统的量子-经典对应。
...
### [398] [Quantum game theory for 2 2 games: a mathematical framework](https://arxiv.org/abs/2605.15747)
*Gloria Ferraris,Veronica Umanit\`a*

Main category: quant-ph

TL;DR: 开发了量子博弈论在静态2x2博弈中的严格数学框架，证明了连续量子混合策略纳什均衡的存在性。


<details>
  <summary>Details</summary>
Motivation: 将经典博弈论扩展到量子设置，其中玩家可以使用任意酉操作（纯策略）或SU(2)群上的概率测度（混合策略），需要严格的数学基础。

Method: 引入Eisert-Wilkens-Lewenstein协议作为标准实现，通过不动点论证证明连续量子混合策略纳什均衡的存在性，推广了经典纳什定理。

Result: 证明了对于连续量子混合策略，纳什均衡总是存在的。

Conclusion: 提供了量子博弈论的严格数学基础，为量子策略下的博弈分析奠定了理论基础。
...
### [399] [Fast convergence of Dynamic Capacities of GNS-Symmetric Quantum Channels](https://arxiv.org/abs/2605.15953)
*Omar Fawzi,Li Gao,Mizanur Rahaman,Mostafa Taheri*

Main category: quant-ph

TL;DR: 对GNS对称量子信道，给出了经典和量子容量的指数收敛界，并用于量化主动与被动纠错性能。


<details>
  <summary>Details</summary>
Motivation: 研究量子信道信息容量的时间演化。

Method: 考虑GNS对称信道（如Pauli信道），利用其熵性质推导指数收敛界。

Result: 给出了经典和量子容量的显式指数收敛界。

Conclusion: 这些结果有助于量化主动与被动纠错性能。
...
### [400] [Diagonal Adaptive Non-local Observables on Quantum Neural Networks](https://arxiv.org/abs/2605.15410)
*Huan-Hsin Tseng,Yan Li,Hsin-Yi Lin,Samuel Yen-Chi Chen*

Main category: quant-ph

TL;DR: 提出对角自适应非局域可观测，显著降低参数数量和经典优化成本，同时保持与完整自适应非局域可观测相同的功能。


<details>
  <summary>Details</summary>
Motivation: 完整ANO虽然增大了变分量子算法的函数空间，但参数数量急剧增加，经典优化成本高。

Method: 采用仅与量子电路配对的对角可观测，通过数学上利用对角矩阵作为ANO空间模酉相似性的典范代表，等效于完整ANO的大参数空间。

Result: 将k-局域可观测复杂度从O(4^k)降至O(2^k)，并降低相应的测量端经典计算。

Conclusion: 对角ANO保留了完整ANO的大部分优势，同时将传统变分量子电路作为其特例。
...
### [401] [Quantum Feature Pyramid Gating for Seismic Image Segmentation](https://arxiv.org/abs/2605.15370)
*Taha Gharaibeh,Jyotsna Sharma*

Main category: quant-ph

TL;DR: 提出一种量子特征门控混合分割架构，在特征融合点嵌入参数化量子电路，用于地震图像盐体分割，在TGS挑战中提升IoU。


<details>
  <summary>Details</summary>
Motivation: 盐体精确分割对地震解释至关重要，但混合量子-经典模型在密集像素级地球物理预测中的价值尚未验证。

Method: 在编码器-解码器管道的特征金字塔网络合并点嵌入4量子比特、2层参数化量子电路，采用数据重上传和全局平均池化，实现量子特征门控。

Result: 在EfficientNetV2-L（256x256）上，移除量子门使平均IoU从0.9389降至0.8404（降9.85%）；在U-Net中作为跳跃连接注意力提升IoU 0.88。

Conclusion: 量子特征融合可改进密集地震分割，提供受控实验证据。
...
### [402] [Reweighting free energy profiles between universal machine learning interatomic potentials for fast consensus building](https://arxiv.org/abs/2605.15630)
...
TL;DR: 提出一种系统框架，通过对单个MLIP采样的PMF进行重加权，高效恢复多个DFT参考水平上的高保真热力学性质，避免昂贵模拟。


<details>
  <summary>Details</summary>
Motivation: DFT精度的自由能计算昂贵，通用MLIP虽快但训练数据依赖导致不确定性，需要可靠且低成本的重加权方法。

Method: 采用平均能隙近似进行直接指数重加权校正，构建跨多个目标MLIP的PMF重加权框架，并在601原子系统上测试。

Result: 成功恢复PBE+D3、PBE-sol、r²SCAN、r²SCAN-D4等DFT参考的PMF，并将MLIP分为两类，在相空间重叠极低时仍能准确获得反应和活化自由能。

Conclusion: 该框架作为诊断协议，可实现跨模型对材料化学性质达成经济共识，避免冗余模拟。
...
### [403] [Testing properties of trees in graphical models with covariance queries](https://arxiv.org/abs/2605.15996)
*Sofiya Burova,Francisco Calvillo,G\'abor Lugosi,Piotr Zwiernik*

Main category: stat.ML

TL;DR: 本文研究高维图形模型中树结构的全局性质测试问题，使用协方差查询，设计了亚二次查询数量的随机测试程序。


<details>
  <summary>Details</summary>
Motivation: 重构整个树代价高昂，需要高效测试全局结构性质如叶子数、最大度、典型距离和直径。

Method: 基于Lugosi等人提出的协方差查询模型，针对树结构，设计随机测试算法，查询次数为亚二次。

Result: 对每个性质获得明确的查询复杂度界限，依赖于目标阈值和容忍参数。

Conclusion: 树结构的某些全局性质可以在亚二次查询下高效测试，而无需完全重构。
...
### [404] [$\alpha$-TCAV: A Unified Framework for Testing with Concept Activation Vectors](https://arxiv.org/abs/2605.15688)
*Ekkehard Schnoor,Jawher Said,Malik Tiomoko,Wojciech Samek,Alexander Jung*

Main category: stat.ML

TL;DR: 针对概念激活向量（CAV）统计不稳定性问题，提出α-TCAV框架，用平滑函数替代指示函数，统一了TCAV和Multi-TCAV，并给出参数调优指导及实用建议。


<details>
  <summary>Details</summary>
Motivation: CAV在实际应用中统计不稳定性限制其效用，标准TCAV得分因使用不连续指示函数导致方差在关键区域不衰减。

Method: 分析CAV和TCAV的随机性，推导主要CAV类的分布，发现标准TCAV缺陷，引入α-TCAV用参数化平滑函数替代指示函数，形成统一概率框架，并给出参数调优准则。

Result: α-TCAV框架统一了TCAV和Multi-TCAV，提供校准的贝叶斯最优概率度量，且计算成本更低；发现将全部采样预算分配给单个CAV而非多个更优。

Conclusion: 挑战了既有惯例，提供了理论指导和实用建议，提升了概念解释的稳定性和效率。
...
### [405] [On Kernel Eigen-alignments of KRR: Reconstruction and Generalization](https://arxiv.org/abs/2605.15240)
*Yang Liu,Ernest Fokoue,Richard Lange,Daniel Krutz*

Main category: stat.ML

TL;DR: 研究核矩阵与学习目标之间的特征对齐对稳健泛化的关键作用，建立泛化性能与特征值/特征向量估计的直接联系。


<details>
  <summary>Details</summary>
Motivation: 通过分析核矩阵扰动导致的误差，利用矩阵特征值和特征向量的估计稳定性推导泛化误差界，提供更直观的理解。

Method: 基于核岭回归（KRR）中预测任务是特征向量加权和的观点，分析核矩阵扰动对误差的影响，利用特征值/特征向量的估计稳定性导出泛化误差上界。

Result: 高秩核可轻松获得近零重构误差，但重构误差对泛化预测力有限；强泛化需要增大特征向量对齐、特征值大小或相邻特征值间隔。

Conclusion: 从特征值/特征向量估计角度建立了泛化界，表明特征对齐、特征值大小及间隔对泛化至关重要。
...
### [406] [Harnessing Unimodality in Semiparametric Contextual Pricing via Oracle Price Map Learning](https://arxiv.org/abs/2605.15411)
*Yingying Fan,Yuxuan Han,Jinchi Lv,Xiaocong Xu,Zhengyuan Zhou*

Main category: stat.ML

TL;DR: 本文研究了半参数标量指数估值模型中的上下文动态定价问题，提出了ORBIT算法，实现了近乎最优的遗憾界，并揭示了非参数学习项的最优性。


<details>
  <summary>Details</summary>
Motivation: 现有动态定价研究多依赖于参数模型或非参数模型，但实际中估值函数可能具有半参数结构，且市场条件复杂。本文旨在利用标量指数结构提高定价效率。

Method: 提出ORBIT策略，通过粗到细的模块化方法：使用标量导引指数定位基准价格，在信任区域内通过局部多项式近似并应用赌徒凸优化学习。在线性效用模型中，采用自适应椭圆探索构建导引指数。

Result: 算法实现了遗憾界Õ(T^{(2β-1)/(4β-3)} + √(dT))，在固定维度下达到匹配的下界，证明非参数学习项是极小化最优的。
...
### [407] [MaxSketch: Robust Distinct Counting in Streams via Random Projections](https://arxiv.org/abs/2605.15571)
...
### [408] [Pessimistic Risk-Aware Policy Learning in Contextual Bandits](https://arxiv.org/abs/2605.15620)
*Yilong Wan,Yuqiang Li,Xianyi Wu*

Main category: stat.ML

TL;DR: 提出统一分布框架，实现风险感知离线策略优化，达到极小极大最优速率，且无额外统计成本。


<details>
  <summary>Details</summary>
Motivation: 高风险领域需控制不良结果，但现有离线上下文赌博机研究仅关注期望奖励或仅进行策略评估，缺乏对风险准则的优化。

Method: 构建优化Lipschitz连续风险泛函的分布框架，推导重要性采样的经验集中不等式，获得数据依赖的次优性界。

Result: 次优性界速率为$\tilde{\mathcal{O}}(1/\sqrt{n})$，且不需均匀重叠假设，与风险中性离线策略优化匹配。

Conclusion: 优化一般Lipschitz风险准则在统计上可行，且相对于期望奖励无额外统计成本。
...
### [409] [Unsupervised Domain Shift Detection with Interpretable Subspace Attribution](https://arxiv.org/abs/2605.15920)
*Sebastian Springer,Alessandro Laio*

Main category: stat.ML

TL;DR: 提出一种检测域偏移的工具，通过局部密度异常检测和特征子空间归因，实现偏移的可解释性，并提供补偿协议。在控制实验和真实ECG数据上验证有效。


<details>
  <summary>Details</summary>
Motivation: 检测数据集间细微的概率分布差异（域偏移），以发现隐藏的群体偏差，避免影响下游建模。

Method: 利用算法检测高维特征空间中的局部密度异常，定位异常最显著的特征子空间；再通过从两个无标签数据集中抽取无残留差异的子集来补偿偏移。

Result: 在20维控制基准上成功恢复广域和局部偏移及其特征子空间；在782维ECG数据中检测出设备引起的偏移，提取了富集不平衡设备成分的子集，并识别出与采集对比相关的ECG特征。

Conclusion: 密度偏移检测与子空间归因为揭示隐藏的群体偏差提供了实用框架，有助于在下游建模前进行数据校正。
...
### [410] [Explainable AI Isn't Enough! Rethinking Algorithmic Contestability](https://arxiv.org/abs/2605.16041)
*Timo Freiesleben,Kristof Meding,Gunnar K\"onig*

Main category: stat.ML

TL;DR: 提出算法可争议性的操作定义，作为算法 recourse 的补充，并识别三种可推翻决策的证据，论证其与欧盟立法的关联。


<details>
  <summary>Details</summary>
Motivation: 现有可解释AI主要关注算法 recourse（帮助改变特征以获得期望结果），但忽略了算法 contestability（审查和纠正错误决策），缺乏正式定义和系统化操作。

Method: 定义 contestability 为假定决策可能错误并寻找证据质疑推翻，与 recourse 对比；分析标准XAI解释的不足；识别三种证据：预测多重性、错误特征值、被忽视的推翻性证据。

Result: 标准XAI解释仅揭示邻域错误，不足以推翻决策；三种证据使决策在道德上不可辩护，从而实现成功争议。

Conclusion: 个体对这三种证据已有法律权利，对应欧盟立法框架。
...
### [411] [A numerical study into neural network surrogate model performance for uncertainty propagation](https://arxiv.org/abs/2605.16078)
*Noah Wade,Kirubel Teferra*

Main category: stat.ML

TL;DR: 神经网络代理模型在随机问题中能捕捉整体分布，但尾部预测误差比均值大一个数量级，需关注外推问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注确定性样本或均值场，忽略分布尾部性能，因此需要评估代理模型在全概率空间的表现。

Method: 以热传导方程（高度随机源项）为典型问题，比较全连接网络与深度算子网络，使用数据驱动和物理信息损失函数，并介绍识别外推样本的方法。

Result: 尾部最坏情况预测误差是均值误差的10倍，全连接网络（弱形式残差损失）在预测精度上最优。

Conclusion: 神经网络需特别处理外推输入，弱形式残差损失有助于提升尾部预测准确性。
...
### [412] [Skew-adaptive conformal prediction](https://arxiv.org/abs/2605.16145)
*Paulo C. Marques F.,Helton Graziadei*

Main category: stat.ML

TL;DR: 开发了一种偏斜自适应分裂保形预测方法，通过非对称区间和规范方法生成适应性区间，并在多种数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有保形预测区间未能适应数据中的局部偏斜，导致效率不足，需要一种能同时调整尺度与偏斜的方法。

Method: 使用非对称区间族和规范方法推导一致性分数，并通过反双曲正弦变换训练额外模型以学习特征空间中的不确定性倾斜。

Result: 该方法在多个数据集上预测区间效率优于缩放分数构建和保形分位数回归，且宽度比率估计与测试样本一致。

Conclusion: 提出的偏斜自适应方法在保持有限样本有效性的同时，能有效提升区间效率，并提供了可靠的比较估计器。
...
### [413] [A Scalable Nonparametric Continuous-Time Survival Model through Numerical Quadrature](https://arxiv.org/abs/2605.16208)
...
### [414] [Heuristic-Based Merging of HPC Traces to Extend Hardware Counter Coverage](https://arxiv.org/abs/2605.15832)
*J\'ulia Orteu Aubach,Fabio Banchelli,Marc Clasc\`a Ram\'irez,Marta Garcia-Gasulla*

Main category: cs.PF

TL;DR: 提出一种基于启发式的方法，通过合并多次运行的计算轨迹，构建包含更多硬件计数器的统一数据集，用于高性能计算性能预测。


<details>
  <summary>Details</summary>
Motivation: 性能建模中通常只能同时收集有限数量的硬件计数器，限制了特征空间。

Method: 通过分析MPI结构、时间和通信模式匹配不同运行中的计算突发，合并轨迹以创建更丰富的特征集。

Result: 在MareNostrum5上验证，合并后的计数器保持了可接受的精度，可直接用于训练ML模型。

Conclusion: 该方法在不依赖复用的情况下扩展了硬件特征，有效提升了性能预测能力。
...
### [415] [Multicritical Scaling Limit of Shifted Schur Measure](https://arxiv.org/abs/2605.15744)
*Haruna Aida,Taro Kimura*

Main category: math.CO

TL;DR: 研究了移位Schur测度的多临界标度极限，证明在边缘极限中相关函数收敛到高阶Airy核的行列式，实现了从Pfaffian点过程到行列式分布的转变。


<details>
  <summary>Details</summary>
Motivation: 探索移位Schur测度在标度极限下的行为，特别是多临界条件下的相变。

Method: 通过适当标度极限和连续参数条件，确定极限形状；在多临界条件下分析相关函数的边缘标度极限。

Result: 位移Schur测度下的严格分拆极限形状被确定；相关函数收敛到高阶Airy核的行列式。

Conclusion: 严格证明了标度极限中从Pfaffian到行列式点过程的转变。
...
### [416] [SwAIther-Precip: Lead-Time-Aware Bias Correction Enables Kilometer-Scale Downscaling of Global AI Precipitation Forecasts over Switzerland](https://arxiv.org/abs/2605.16163)
...
TL;DR: SwAIther-Precip 是一个领先时间感知的降尺度框架，将粗分辨率 AIFS 预报转换为瑞士千米尺度的概率降水场，CRPS 降低 48%，有效分辨率约 4 公里。


<details>
  <summary>Details</summary>
Motivation: 复杂地形下中期降水预报准确度不足，全球 AI 模型分辨率粗 (0.25°)，统计降尺度难以处理随预报时间变化的偏差。

Method: 两阶段方法：首先用基于 FiLM 的 U-Net 条件于领先时间校正粗尺度偏差，然后用扩散模型独立于领先时间生成细尺度空间变异性。

Result: 相比原始 AIFS，CRPS 降低 48%；大尺度谱保真度 >0.85，小尺度 >0.88；在 1 公里网格上有效分辨率约 4 公里，可持续 5 天；跨领先时间训练使 6 天 CRPS 比单领先时间模型降低 13%。

Conclusion: 在生成式超分辨率前显式校正领先时间相关偏差是实现全球 AI 降水预报高效千米尺度概率降尺度的关键。
...
### [417] [Discrete Mixed Quantization](https://arxiv.org/abs/2605.16203)
*Qiaochu Ma*

Main category: math.SP

TL;DR: 本文提出了一种用于图向量丛的混合量化技术，并将其应用于多个渐近谱问题，包括Alon-Boppana界、Kesten-McKay律、量子遍历性、零因子收敛和Ramanujan向量丛。


<details>
  <summary>Details</summary>
Motivation: 图向量丛的渐近谱分析需要新的量化方法，以解决传统方法无法处理的复杂谱问题。

Method: 开发了一种混合量化技术，结合了图论和向量丛的量化方法。

Result: 成功将该技术应用于Alon-Boppana界、Kesten-McKay律、量子遍历性、零因子收敛和Ramanujan向量丛等谱问题，并得到了相关渐近结果。

Conclusion: 混合量化技术为图向量丛的谱分析提供了一种有效工具，能够处理多种渐近谱问题，具有广泛的应用前景。

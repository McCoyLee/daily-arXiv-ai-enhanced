...
TL;DR: 提出ChangeFlow，一种基于潜空间整流流的生成式变化检测框架，在四个基准上平均F1达80.4%，比之前最优方法提高1.3个点，推理速度与强基线相当。


<details>
  <summary>Details</summary>
Motivation: 现有逐像素判别方法无法建模变化区域整体性，且对歧义性不鲁棒；现有生成方法计算成本高、条件机制复杂，性能落后于判别方法。

Method: 将变化检测视为潜空间中的变化掩码合成，采用整流流（rectified flow）生成，并结合结构化轻量条件信号；通过采样集成提升鲁棒性，样本一致性用于置信度估计。

Result: 在四个基准数据集上平均F1为80.4%，比之前最优方法平均提升1.3个百分点，推理速度与近期强基线相当。

Conclusion: 证明生成式方法可以媲美甚至超越判别式方法，同时提供歧义区高亮和置信度估计等额外优势。
...
TL;DR: 提出PanoWorld全景视频世界模型，从单张图像和标题生成几何一致的全景视频，通过引入深度一致性和轨迹一致性损失，以及球面几何感知适应，在保持视觉真实感的同时提升了几何一致性。


<details>
  <summary>Details</summary>
Motivation: 现有全景视频生成方法主要优化视觉真实感，但未显式约束3D场景状态，导致深度不一致、对应关系断裂和运动不合理，无法满足具身AI对整体空间理解的需求。

Method: 在预训练的透视视频世界模型基础上，引入深度一致性损失（利用伪真实全景深度）和轨迹一致性损失（监督跟踪点的3D世界帧位置），并对条件编码和位置编码进行球面几何感知适应，同时构建包含深度、轨迹和提示标注的统一几何感知数据集PanoGeo。

Result: 实验表明，PanoWorld在几何一致性上优于现有方法，同时保持了具有竞争力的视觉真实感。

Conclusion: 全景视频生成必须被视为几何建模问题，以支持具身AI的整体空间理解需求。
...
TL;DR: 提出MR2-ByteTrack方法，通过交替全分辨率和低分辨率推理，结合ByteTrack和Rescore算法，在超低功耗MCU上实现节能的视频目标检测，首次实现基于Transformer的实时视频目标检测。


<details>
  <summary>Details</summary>
Motivation: 智能视觉传感器需设备端处理视频流，但MCU内存和计算有限，传统视频目标检测方法因需特征存储或多帧缓冲而不可行。

Method: MR2-ByteTrack交替全分辨率和低分辨率推理降低计算成本，通过ByteTrack跨帧关联检测，使用Rescore算法（概率并集规则）聚合检测置信度以纠正误分类。适用于CNN和Transformer检测器。

Result: 在ImageNetVID上，CNN模型mAP达49.0，Transformer模型达48.7，乘法累加操作分别减少53%和32%；在GAP9 MCU上部署，相比全分辨率处理节省高达55%能量。

Conclusion: MR2-ByteTrack能在MCU上实现实时视频目标检测，平衡精度与能耗，且适用于不同架构的检测器。
...
TL;DR: 提出基于Retinex理论的同质图像分解方法（RIDE），通过光照和反射成分的分解提升隐蔽目标分割性能，并理论证明该分解可保持或提高前景-背景可判别性。


<details>
  <summary>Details</summary>
Motivation: 现有隐蔽目标分割方法（如COMS）依赖RGB图像或异质分解（如傅里叶、小波），导致像素对齐线索不直接；视觉伪装在复合空间匹配但成分空间未必匹配，需要利用这一判别性差异。

Method: 提出RIDE框架，包含三个模块：任务驱动Retinex分解（端到端学习分割最优分解）、判别性差距注意力（自适应利用分解有益区域）、特征空间对比损失（突破伪装）。

Result: 理论证明Retinex分解在不同物理机制下均保持或提升前景-背景可判别性，且反相关最大化增益。

Conclusion: 同质分解（Retinex）为隐蔽目标分割提供全新有效视角，优于传统异质分解方法。
...
TL;DR: DiffVAS是一种基于扩散模型的目标条件策略，能够在部分可观测环境中同时搜索多种目标，极大优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有VAS方法假设搜索空间完全已知且只能针对单一目标，不现实；需要能处理部分可观测和多目标搜索的策略。

Method: 提出DiffVAS，使用扩散模型从部分观测重建整个地理区域，然后通过目标条件强化学习规划模块指导搜索。

Result: 在多个数据集上，DiffVAS在搜索多样目标方面显著超越现有最先进方法。

Conclusion: DiffVAS推进了VAS策略在真实世界应用中的部署。
...
TL;DR: 提出一种无需调优、基于指令的视频编辑框架，通过结构噪声初始化策略和噪声引导机制提升编辑质量。


<details>
  <summary>Details</summary>
Motivation: 现有无需调优的方法未充分利用噪声潜码中的丰富信息，导致编辑效果不理想。

Method: 设计结构噪声初始化策略（SNIS），为编辑区域分配更高噪声以促进内容变化，为未编辑区域分配更低噪声以保持一致性；引入噪声引导机制（NGM），利用模型内视频先验和噪声潜码信息引导去噪过程。

Result: 实验证明该方法在视觉质量和性能上达到最优。

Conclusion: 该方法有效利用噪声潜码信息，提升了视频编辑的视觉一致性和内容保留。
...
TL;DR: 在极端低数据细粒度分类中，比较四种预训练编码器，发现监督和对比学习编码器在线性探针中表现最佳，MAE在非线性探针中有所提升，DINOv3表现最差。


<details>
  <summary>Details</summary>
Motivation: 专家领域标注成本高，需要指导如何在极低数据下选择预训练编码器。

Method: 使用自建祖母绿内含物分级数据集，比较四种冻结的ViT-B/16编码器（监督分类、对比学习、掩码重建、自蒸馏），采用留一法交叉验证和线性/非线性探针，通过置换检验（N=1000）控制统计噪声。

Result: 监督和对比编码器线性可分性最强（逻辑回归AUC 0.768和0.735，SVM AUC 0.739和0.697）；MAE在非线性探针（XGBoost AUC 0.713）下表现更好；DINOv3在所有探针中表现不佳。

Conclusion: 对于极端低数据细粒度分类，若仅使用线性决策规则，应优先选择边界增强预训练目标；若数据允许非线性分类器，可考虑重建式编码器。
...
TL;DR: 首次系统研究评估自监督视频模型作为世界模型的鲁棒性，发现潜空间预测模型在五个鲁棒性轴线上表现更好。


<details>
  <summary>Details</summary>
Motivation: 现有评估仅用单一准确率，忽视模型作为世界模型的潜在能力。

Method: 分析四个容量匹配的前沿视频基础模型（V-JEPA 2.1, V-JEPA 2, VideoPrism, VideoMAEv2）在特征判别性、腐败鲁棒性、细粒度判别、遮挡鲁棒性和时间方向敏感性五个轴线上的表现。

Result: 潜空间预测模型在所有轴线上表现出独特且一致的轮廓：对像素腐败更优雅降级、在遮挡下保留类结构而非几何稳定性、捕捉细粒度物理接触线索而不重建像素、唯一编码时间箭头。
...
TL;DR: 提出速度自适应引导尺度(VAGS)，根据时间信号和速度场对齐程度动态调整引导强度，无需训练或额外计算，在编辑和生成任务中优于固定CFG。


<details>
  <summary>Details</summary>
Motivation: 固定CFG在ODE轨迹上使用恒定引导尺度不合理：早期噪声主导、语义信号弱，后期需要强方向性，且引导值应依赖于速度场一致性。

Method: 用有界因子乘以名义引导尺度，该因子结合时间信号水平与任务相关速度场余弦相似度；编辑用源-目标速度对齐，生成用无条件-条件速度对齐。

Result: 在PIE-Bench、DIV2K、COCO17、CUB-200、Flickr30K上，VAGS在结构保真度和生成质量上均优于固定CFG及近期无训练引导变体。

Conclusion: VAGS作为固定CFG的无训练替代方案，通过动态调整引导尺度提升性能，且恢复固定CFG作为特例。
...
TL;DR: 视觉-语言模型（VLM）在基本视觉操作如线条追踪上表现不佳，即使最先进的模型也容易在局部竞争下丢失目标路径。


<details>
  <summary>Details</summary>
Motivation: 研究VLM在基本视觉操作（如线条追踪）上的鲁棒性，尽管它们在多模态基准测试中表现强劲。

Method: 设计受控追踪任务，引入附近竞争者，减少语义和拓扑歧义，以隔离线条追踪能力。

Result: VLM常丢失目标路径并切换到附近替代路径；模型规模扩展增益有限，推理可部分补偿但成本高，明确指令无效。
...
TL;DR: 提出DiLA，通过内容-结构解耦解决潜在动作模型中动作抽象与生成保真度的权衡，实现高抽象与高保真生成。


<details>
  <summary>Details</summary>
Motivation: 现有潜在动作模型在动作抽象和生成保真度之间存在根本权衡，需要预训练世界模型或限制预测为光流来规避。

Method: 引入内容-结构解耦的潜在动作世界模型DiLA，利用预测瓶颈促进解耦，将空间布局编码到结构路径，视觉细节分离到内容路径。

Result: 在视频生成质量、动作迁移、视觉规划和流形可解释性上取得更优结果。

Conclusion: DiLA统一了高层动作抽象与高保真生成，推进了自监督世界模型学习的发展。
...
TL;DR: 提出DecomPose框架，通过难度感知梯度解耦和稳定性驱动的非对称分支，缓解类别级6D姿态估计中跨类别梯度冲突与负迁移问题，在多个基准上取得优越性能。


<details>
  <summary>Details</summary>
Motivation: 类别级6D姿态估计多类别联合学习时，几何异质性导致共享模块中优化信号冲突，引发梯度冲突和负迁移。

Method: 提出DecomPose，包含(1)难度感知梯度解耦：基于数据驱动难度代理分组类别，路由到组特定分支隔离更新；(2)稳定性驱动非对称分支：简单类别分配高容量分支作为稳定锚，复杂类别用轻量分支抑制噪声。

Result: 在REAL275、CAMERA25和HouseCat6D数据集上，DecomPose有效减少优化冲突，姿态估计性能优于基线。

Conclusion: DecomPose通过分解优化冲突，显著提升多类别6D姿态估计效果，验证了梯度诊断和差异化分支设计的有效性。
...
TL;DR: 针对VLA模型微调导致多模态能力下降的问题，提出UAM模型，通过添加背侧专家保留语义能力，在多种操作任务上取得高成功率。


<details>
  <summary>Details</summary>
Motivation: VLA模型微调时，单编码器需同时支持语言语义和控制特征，导致多模态能力系统性退化（embodiment tax）。

Method: 受生物视觉双通路启发，UAM添加并行背侧专家，初始化为预训练生成模型，并以预测视觉动态的中级推理目标训练，实现端到端行动训练。

Result: UAM在保留VLM超过95%多模态能力的同时，在多种操作任务（如未见物体、新组合、指令变化）中取得最高平均成功率。

Conclusion: 语义保持可通过架构分离实现，无需冻结权重或辅助数据重放，且保存的语义能力可自然迁移到行动中的语义泛化。
...
TL;DR: 提出BiomedAP框架，通过门控跨模态融合和双锚约束增强视觉-语言模型在少样本医学诊断中的鲁棒性，在11个基准上超越基线。


<details>
  <summary>Details</summary>
Motivation: 现有方法将视觉和文本提示独立优化，依赖于理想的“黄金提示”，在临床噪声描述下跨模态对齐不稳定。

Method: 采用门控跨模态融合实现层间交互以抑制噪声，并引入双锚约束（专家模板高锚点+少样本视觉原型低锚点）稳定提示学习。

Result: 在11个基准上，BiomedAP在少样本准确率和提示扰动鲁棒性上均优于基线方法。

Conclusion: BiomedAP有效解决了提示脆弱性问题，提升了视觉-语言模型在医疗领域的实用性和稳定性。
...
TL;DR: 提出GRASP数据集和SGR奖励方法，增强多模态大模型在多人视频中基于细微非语言线索的社交互动推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大模型难以在多人视频中准确识别交互参与者，缺乏结合细粒度凝视、指向手势与高层次社交问答的数据集。

Method: 构建GRASP数据集（290K问答对、46K视频、749小时），基于身份一致的凝视轨迹、指向性手势及其联合事件生成问题，并提出Social Grounding Reward (SGR) 作为训练信号。

Result: SGR在GRASP-Bench上提升性能，同时在相关社交视频QA基准上保持零样本性能。

Conclusion: GRASP和SGR有效促使模型推理交互参与者，提升社交互动理解能力。
...
TL;DR: 提出频域增强的空间-频谱状态空间GAN（FSCM）用于热红外图像彩色化，通过频谱信息引导和语义分割损失提升视觉质量和语义保真度。


<details>
  <summary>Details</summary>
Motivation: 现有红外彩色化方法依赖单波段图像，缺乏光谱线索导致结构失真和语义混淆；红外高光谱虽有丰富光谱信息，但现有框架难以建模空间-频谱耦合和弱纹理细节。

Method: 提出FSCM，包括频域增强的空间-频谱状态空间生成器（由级联FSB单元组成），每个FSB包含状态空间建模、频域增强模块（FEM，含多级小波分解和傅里叶门控）和双流混合门控模块（DGM，含变形感知采样和稀疏注意力）；并引入在线语义分割引导损失。

Result: 实验表明，FSCM在视觉质量和语义保真度上优于现有红外彩色化方法。

Conclusion: FSCM有效解决了红外彩色化中的结构失真和语义混淆问题，提升了复杂道路场景的语义一致性。
...
TL;DR: 提出一个概率框架，将卫星图像中的野火烟雾分为轻、中、重三个严重等级，并在单次前向传播中提供认知不确定性和偶然不确定性。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习方法仅将烟雾检测视为二分类任务，输出点估计，缺乏预测置信度度量。

Method: 使用预训练的EfficientNet-B3作为主干网络，结合CBAM模块和证据深度学习头，预测狄利克雷浓度参数，直接估计认知不确定性（空寂度）和偶然不确定性（不和谐度），无需蒙特卡洛采样。

Result: 在16,298个真实卫星图像块上，模型达到93.8%加权测试准确率（91.1%未加权），ECE=0.0274。选择预测最确定的50%图像块时准确率达96.7%。随着图像质量下降，不确定性单调增加。中等烟雾类别的认知不确定性最高（平均空寂度0.187），表明模型正确识别了烟-边界区域。

Conclusion: 该框架能有效评估烟雾严重程度并提供可靠的不确定性估计，有助于应急响应和空气质量管理。
...
TL;DR: RaPD是一种在连续神经图像场潜空间中进行扩散的模型，实现了分辨率无关的图像生成，通过坐标查询渲染任意分辨率，且扩散成本固定。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型在离散网格上合成图像，限制了分辨率灵活性；连续神经场仅在解码阶段引入连续性，而潜空间仍是离散且面向重建的。

Method: RaPD在连续神经图像场潜空间中进行扩散，采用语义表示指导生成感知的潜学习，并利用坐标查询注意力渲染器进行坐标条件、尺度感知的渲染。

Result: 实验表明，RaPD在生成质量和分辨率可扩展性上优于先前方法。

Conclusion: RaPD弥合了连续表示与生成之间的差距，实现了固定扩散成本下的任意分辨率图像生成。
...
TL;DR: 提出分解视觉-语言对齐框架，通过解耦文本提示中的概念和属性token，提升开放式词汇分割对未见组合的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有模型将细粒度描述编码为整体句子，导致语义单元纠缠，难以泛化到未见过的属性-类别组合。

Method: 框架将文本提示分解为概念和属性token，并设计特征级门控交叉注意力模块和评分级log空间聚合，实现独立跨模态交互和组合匹配。

Result: 该方法可无缝集成到现有分割架构，在细粒度开放式词汇分割基准上显著提升对未见组合的泛化性能。

Conclusion: 通过显式分解语义单元和强制组合语义，有效解决了开放式词汇分割中的组合泛化问题。
...
TL;DR: 提出一种结合SAM-HQ高精度2D掩码和先验引导标签重分配的方法，实现3D高斯溅射的鲁棒分割，支持实时对象编辑。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射缺乏鲁棒分割能力，现有2D分割提升方法存在视图不一致和掩码粗糙的问题。

Method: 利用SAM-HQ生成精确2D掩码，通过先验引导标签重分配强制多视图一致性，将标签赋予3D高斯体。

Result: 分割精度达到最优，边界保持优越，支持交互式实时编辑，在VR和机器人中展示实用性。
...
TL;DR: 构建了一个集成ResNet50、EfficientNet-B0和DenseNet121的植物病害检测系统AgriMind，在PlantVillage数据集上达到99.23%准确率，速度53 FPS。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国植物病害检测仍主要依靠人工目测，需要自动化解决方案。

Method: 使用20,638张PlantVillage图像训练ResNet50、EfficientNet-B0和DenseNet121集成模型，采用冻结ImageNet骨干网络和仅训练头部的迁移学习，通过平均softmax输出进行集成。

Result: 单个模型准确率96-97%，集成模型达到99.23%（错误率降低三分之二），番茄类（10种相似类别）准确率99.01%，在NVIDIA T4 GPU上以53 FPS运行。

Conclusion: 集成模型有效提升了准确率，但实时移动应用需进一步TensorFlow Lite优化。
...
TL;DR: 提出VideoSeeker，通过视觉提示实现实例级视频理解，结合智能体推理与主动感知，在实例级任务上平均提升13.7%，超越GPT-4o等闭源模型。


<details>
  <summary>Details</summary>
Motivation: 现有LVLMs在实例级时空定位任务上表现不佳，文本提示无法提供精确时空参考，且模型被动依赖语言推理，缺乏主动视觉感知能力。

Method: 提出VideoSeeker范式，集成智能体推理与实例级视频理解；构建四阶段全自动数据合成管道生成大规模高质量数据；通过冷启动监督和强化学习内化工具调用与主动感知能力。

Result: 在实例级视频理解任务上平均提升13.7%，超过GPT-4o和Gemini-2.5-Pro，在通用视频理解基准上也展现有效迁移性。

Conclusion: VideoSeeker通过视觉提示和主动感知机制显著提升了实例级视频理解性能，数据集和代码将开源。
...
TL;DR: 提出GenShield，一个统一的框架，联合进行可解释的AIGI检测和可控伪影校正，通过闭环诊断和修复实现相互增强。


<details>
  <summary>Details</summary>
Motivation: AI生成图像越来越逼真，但检测后如何校正伪影、恢复真实外观的问题未受重视，且检测和校正之间缺乏联系。

Method: GenShield是一个自回归框架，结合基于视觉思维链的课程学习策略，实现自解释的多步骤“诊断-修复”校正，并带有显式停止准则。同时构建了高质量“伪影-修复”配对数据集和统一评估管道。

Result: 在校正基准和主流AIGI检测基准上达到最先进性能，并展现出强泛化能力。

Conclusion: 该工作建立了检测和校正之间的互惠关系，为AI生成图像的修复提供了新思路。
...
TL;DR: 提出ML-FOP-SOAP，一种多级方差校正的二阶优化框架，通过Fisher正交投影和分层折叠策略缓解自回归多模态训练中的模态竞争，实现稳定的大批量训练并显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 自回归下一个token训练中，模态竞争导致一阶优化器（如AdamW）不稳定，限制了大批量扩展；二阶预条件器（如SOAP）虽更稳定，但需要解决计算效率和方差问题。

Method: 提出ML-FOP-SOAP，采用Fisher-正交投影抑制方差诱导的模态冲突，并引入分层折叠策略在低微步开销下捕获细粒度方差，实现二阶优化在多模态中的实用化。

Result: 在Janus和Emu3上，相比AdamW，样本效率提升1.4倍，墙钟训练加速1.5倍，且在批大小8192下训练稳定，视觉生成和文本理解均一致提升。

Conclusion: ML-FOP-SOAP是一种鲁棒的优化器，有效缓解模态竞争，支持多模态基础模型的大规模稳定训练，并显著提升效率。
...
TL;DR: 提出VLA-AD蒸馏框架，利用VLM离线语义指导，将大型VLA教师压缩为轻量级学生，在保持性能的同时大幅降低模型尺寸和推理延迟。


<details>
  <summary>Details</summary>
Motivation: 大型VLA策略在机器人操作中有效，但尺寸和计算成本阻碍实时闭环控制，需要高效蒸馏方法。

Method: 使用VLM作为离线语义监督器，在蒸馏过程中除了动作模仿外，还加入任务阶段锚点和多帧方向描述作为辅助信号，训练后学生独立运行。

Result: 在LIBERO上，学生模型参数减少44倍（158M vs 7B），平均性能差距仅0.27%，推理速度提升3.28倍（12.5 Hz）；泛化到其他教师也表现良好。

Conclusion: VLM的离线语义指导能显著提升VLA蒸馏的效率、鲁棒性和可部署性。
...
TL;DR: IVGT是一种隐式视觉几何变换器，能从无位姿多视图图像中重建连续三维几何和外观，无需显式回归点图。


<details>
  <summary>Details</summary>
Motivation: 现有显式几何基础模型存在冗余和几何不连续问题，需要更连续的隐式表示。

Method: 通过隐式神经场景表示在规范坐标系中学习，支持任意3D位置的连续空间查询，用轻量解码器预测SDF和颜色。

Result: 在多数据集联合优化下，IVGT在网格/点云重建、新视图合成、深度/法线估计和相机位姿估计等任务上表现优异。

Conclusion: IVGT有效实现了跨场景的泛化，提供了连续且连贯的几何重建。
...
TL;DR: 提出多模态程序合成框架MultiMat，利用视觉和文本信息生成程序化材质图，效果优于纯文本基线。


<details>
  <summary>Details</summary>
Motivation: 材质节点图在计算机图形学中至关重要，但创建它需要专业训练。现有神经程序合成仅用文本表示，忽略了节点图的视觉空间特性，导致生成困难。

Method: 构建多模态程序合成框架，结合大型多模态模型处理节点图的视觉和文本表征，使用约束树搜索推理算法确保静态正确性并高效搜索程序空间。

Result: 在无条件及条件图合成中，生成的材质图视觉质量更高、保真度更好，效率优于纯文本基线，达到新最先进水平。

Conclusion: 多模态方法能有效利用视觉信息提升材质图生成质量，为程序化材质建模提供了更便捷的途径。
...
TL;DR: 视觉语言模型在推理时常说“让我再看一下图”，但实验表明这仅仅是文本模式，模型并未真正重新审视图像。


<details>
  <summary>Details</summary>
Motivation: 探究视觉语言模型中的自反语句是否触发真正的视觉重检，还是仅学习到文本模式。

Method: 提出VisualSwap图像交换探测框架，构建VS-Bench包含800对视觉相似但语义不同的图像，在多个模型上进行测试。

Result: 模型普遍未能察觉图像被替换，准确率下降高达60%；思考型模型比指令型模型易受攻击近3倍，且扩展规模无缓解。

Conclusion: 当前视觉语言模型声称进行视觉重检时，倾向于“说”而非真正“看”；多轮用户指令可恢复视觉基础，但自生成的自反语句无效。
...
TL;DR: 提出了Minerva-Ego基准，用于评估复杂自我中心视觉推理，包含多步骤多模态问题和时空密集的人工标注推理轨迹。实验发现顶尖模型与人类差距大，提供空间和时间线索可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅评估输出而不评估中间推理步骤，且多数仅限文本域。

Method: 扩展自我中心视频数据源，添加多步骤多模态问题和时空密集的人工标注推理轨迹，并标注每步的感兴趣对象掩码。

Result: 顶尖模型与人类表现差距大；提供“在哪里”和“何时”的提示显著提升模型性能。

Conclusion: Minerva-Ego可深入分析模型推理过程，提示线索有助于缩小差距。
...
TL;DR: 提出在特征空间中采样，用特征相似性选择的代表性样本替代密集几何采样，以降低群卷积神经网络的计算成本。


<details>
  <summary>Details</summary>
Motivation: 群卷积神经网络（GCNN）在每层线性层中密集采样变换群并关联数据与滤波器，导致计算成本随自由度指数增长，阻碍实际应用。

Method: 在特征空间中进行采样，用特征相似性选择的代表性样本替代几何密集样本，从而在训练和推理中解耦几何分辨率与内存和处理成本。

Result: 粗糙的特征空间采样在分类精度上表现良好，允许基于几何相似性进行预计算，显著加速等变3D分类器的训练。
...
TL;DR: 研究了稀疏top-k路由与硬容量约束在视觉分类中的应用，发现计算杠杆模式和多专家路由的必要性，并提出了基于样本的Soft MoE变体来改善性能。


<details>
  <summary>Details</summary>
Motivation: 探索MoE网络在视觉任务中专家坍塌和端到端效率增益有限的问题。

Method: 在多个基准数据集上评估稀疏top-k路由，通过隐藏尺寸扫描和ImageNet消融实验隔离影响因素，提出基于样本的Soft MoE变体。

Result: 发现正精度差距需要路由足够比例的FLOPs，在ImageNet尺度还需要多专家路由；消融实验反转了精度差距；基于样本的变体超越了密集基线。

Conclusion: 识别了批量维度分配是主要失败模式，提出了改进方法。
...
TL;DR: 提出COVER方法将3D资产转换为稀疏全景RGB-D-pose数据，构建了CM-EVS数据集，实现低冗余、高覆盖、可审计的全景训练资源。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉学习缺乏稀疏、一致且几何一致的全景训练接口，现有方法存在视图冗余、标注异构或深度不一致问题。

Method: COVER，一种免训练的视点选择框架，通过ERP范围深度扭曲投影几何，增量评估覆盖并惩罚深度冲突，在代理误差有界下保持近似行为。

Result: 构建CM-EVS数据集，包含36,373帧来自1,275室内场景和户外场景，中位数25帧/室内场景，覆盖13种房间类型，每帧提供全球RGB、度量范围深度和标定位姿。

Conclusion: COVER改善了覆盖-冲突权衡，使得CM-EVS成为稀疏、紧凑、可审计的RGB-D-pose资源，适用于几何一致的全景3D学习。
...
TL;DR: 提出NeRP方法，通过中性提示和参考图像校正视觉语言模型在未见类上的偏见，缓解基础-新类权衡问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在未见类上表现下降而保持已知类性能困难，现有工作归因于过拟合，但观察到不对等混淆现象：已知类可通过交叉熵缓解偏见，未见类偏见持续存在。

Method: NeRP采用即插即用的提示校正策略，利用中性文本提示和参考图像测量类别间先验偏好，结合样本似然得到替代分数，当先验主导预测而证据不足时，在易混淆类对间进行局部翻转。

Result: 在多个主干网络和15个少样本及跨领域基准上，NeRP显著提升未见类准确率，同时保持已知类性能。

Conclusion: NeRP有效缓解了基础-新类权衡，无需修改模型参数，通过纠正先验主导的错误预测提升泛化能力。
...
TL;DR: 提出Semi-MedRef框架，通过三种对齐保持组件解决半监督医学图像分割中的跨模态对齐问题，在所有标签比例下均优于基线。


<details>
  <summary>Details</summary>
Motivation: 医学引用图像分割中标注成本高，现有半监督方法未充分处理强增强下的跨模态对齐，CutMix在多模态中破坏图文一致性。

Method: 采用教师-学生框架，包含T-PatchMix（跨模态CutMix增强）、PosAug（位置感知文本增强）和ITCL（位置引导的图像-文本对比学习）。

Result: 在QaTa-COV19和MosMedData+上，Semi-MedRef在所有标签比例下均优于全监督和半监督基线。

Conclusion: 保留跨模态一致性的增强策略能有效提升半监督医学引用分割性能。
...
TL;DR: 提出OP-Mix数据混合算法，通过低秩适配器模拟候选混合，在整个LM训练生命周期中实现近最优混合，大幅降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有数据混合方法只针对单一训练阶段，需要代理模型或固定领域，缺乏统一解决方案。

Method: OP-Mix利用当前模型直接训练低秩适配器，插值模拟候选数据混合，避免独立代理模型。

Result: 预训练中平均困惑度提升6.3%；持续学习中匹配重训练和策略蒸馏性能，分别节省66%和95%计算。

Conclusion: 语言模型训练应视为连续学习过程，而非不同阶段序列。
...
TL;DR: 文学翻译中，流畅性与忠实性呈负相关，且段落长度影响自动评估。


<details>
  <summary>Details</summary>
Motivation: 探究LLM在文学翻译中流畅性是否对应语义保留，以及是否存在权衡。

Method: 使用130,486个翻译段落，通过翻译ese分类器测流畅性，COMET-KIWI测忠实性，控制段落长度进行相关性分析。

Result: 流畅性与忠实性存在一致负相关，人工和Google Translate显著，TranslateGemma较弱且不显著。

Conclusion: 文学翻译中存在流畅性与忠实性的权衡，自动评估需考虑段落长度。
...
TL;DR: 提出了DiscoExplorer，一个开源网络接口，用于公开和探索多语言话语关系数据集，支持查询、搜索和可视化。


<details>
  <summary>Details</summary>
Motivation: 话语关系研究困难，跨语言比较复杂，缺乏易用的分析接口。

Method: 开发了DiscoExplorer，一个可在本地运行的开源网络接口，公开了DISRPT共享任务中16种语言的话语关系数据集，并提供了查询语言、搜索和可视化功能。

Result: 成功公开了多语言数据集，并展示了查询和可视化工具的有效性。

Conclusion: DiscoExplorer为话语关系研究提供了便利，有助于跨语言比较和分析。
...
TL;DR: 基于乌克兰法院判决构建的大型引用图，揭示了法律领域边界并高精度预测立法重要性。


<details>
  <summary>Details</summary>
Motivation: 现有法律知识结构依赖人工标注，缺乏自动化方法；利用大规模判例引用网络可自动发现法律领域边界并预测立法影响。

Method: 从100.7万篇乌克兰判决中提取5.02亿条引用边，采用正则表达式、Louvain社区检测、引用特征预测等方法。

Result: 度分布呈幂律；社区检测自动划分民事、刑事等法律领域；引用特征预测立法重要性AUC=0.9984。

Conclusion: 引用网络可自动构建法律本体，并集成到LLM辅助法律分析的工作流中，已开源。
...
TL;DR: 人类在有限词汇（最低250个高频词）下回答问题时，行为更接近贪婪采样而非全局最优，但高技能者会回溯修改；在高度约束下依赖语义轻的词。


<details>
  <summary>Details</summary>
Motivation: 研究人类如何在词汇限制下优化可理解性，与算法比较以理解资源理性认知。

Method: 使用基于大型语言模型的序列蒙特卡洛推断实现贪婪和全局最优采样，并与人类行为对比。

Result: 人类整体偏贪婪采样，但高技能者更倾向于非贪婪的回溯修改；语义轻词的使用在两种采样中均出现。

Conclusion: 结果支持资源理性认知，对心理语言学、二语沟通和语言障碍有启示。
...
TL;DR: 提出了基于WhatsApp的AI教学助手Adesua，为西非学生提供科学教育支持，通过生成式AI实现问答和自动评估。


<details>
  <summary>Details</summary>
Motivation: 撒哈拉以南非洲师生比高且合格教师短缺，限制了学生的个性化学习支持和形成性评估。

Method: 集成教科书和33年国家考试试题，利用生成式AI通过WhatsApp机器人实现对话式问答和自动评估，包括定时/非定时测试及详细反馈。

Result: 在加纳进行6个月可行性部署，56名活跃用户，AI生成答案的有用性评分为93.75%（n=16）。

Conclusion: 初步结果表明WhatsApp AI助手有潜力在资源受限的教育环境中提供可扩展、低成本的个性化学习支持和形成性评估。
...
TL;DR: 开发了Eskwai for Students，一个基于检索增强生成的AI助手，帮助加纳法律学生，通过30个月纵向研究评估其有用性和查询类型，引发伦理思考。


<details>
  <summary>Details</summary>
Motivation: 生成式AI在法学教育中具有潜力，但在全球南部地区的开发部署有限。

Method: 构建基于RAG的系统，使用超过1.2万条判例法和1.4K立法的数据库，并在30个月内对3100名加纳法律学生进行纵向研究。

Result: AI助手被证明有用，学生提交了3.2万次查询，揭示了查询类型并引发了伦理问题。
...
TL;DR: 提出能力条件支架框架，根据用户专业领域能力分区，条件化干预行为，防止专业领域漂移。


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型个性化忽略用户在不同领域的评估能力差异，导致专业领域漂移。

Method: 提出能力条件支架，将专业能力划分为强、混合、弱领域，并基于结构化的能力配置文件条件化干预行为。

Result: 多个MMLU子集和四种LLM基底的试点评估显示一致的配置文件条件化干预行为，包括配置文件交换时的分类反转和混合领域风险区的选择性激活。

Conclusion: 能力感知支架可以支持更可靠的专业人机协作，超越风格个性化。
...
TL;DR: 本文分析了六种大型语言模型在十二项认知任务中的神经激活模式，发现数学推理产生最高的注意熵，解码器模型比编码器模型具有更高的稀疏性，为模型选择和优化提供了见解。


<details>
  <summary>Details</summary>
Motivation: 理解不同架构的大型语言模型在处理认知任务时的神经激活差异，以指导模型选择和优化。

Method: 系统测量了六种架构（包括编码器和解码器）在十二类认知任务上的最终激活值、注意熵和稀疏模式，共144个任务-模型组合。

Result: 数学推理在所有架构中注意熵最高；解码器模型比编码器模型表现出显著更高的稀疏模式。

Conclusion: 这些发现揭示了现代语言模型的计算特征及其任务特定神经行为，对大数据应用中的模型选择与优化有重要意义。
...
TL;DR: 研究了语言模型和人类在句法歧义处理上的差异，发现减少并行解析数量能增强花园路径效应，但不足以完全匹配人类数据。


<details>
  <summary>Details</summary>
Motivation: 语言模型预测的惊奇值与人类阅读时间存在偏差，尤其在花园路径句中低估了困难程度，可能源于人类与模型计算限制的差异。

Method: 使用循环神经网络语法（RNNG）结合词同步束搜索，系统改变用于计算单词惊奇值的并行解析数量，并用这些惊奇值预测人类阅读时间。

Result: 减少并行解析数量确实增加了预测的花园路径效应幅度，但远未达到人类效应的完整幅度。

Conclusion: 人类和语言模型在可用并行解析数量上的差异不能解释基于语言模型惊奇值与人类句子处理之间的不一致。
...
TL;DR: 推理训练的语言模型在生成思维链时，其隐藏状态轨迹的几何特征受生成长度影响。去除长度效应后，不同难度问题在轨迹几何上存在系统性差异，尤其在代码任务中推理模型表现出更直接的校正轨迹和更均匀的局部曲率。


<details>
  <summary>Details</summary>
Motivation: 长思维链的更多标记消耗并不揭示模型是仅仅是增加了计算步骤还是遵循了不同的内部轨迹，需要区分这两种可能性。

Method: 对竞争性编程、数学和布尔可满足性任务中思维链生成的隐藏状态轨迹进行几何分析，并通过残差化去除生成长度的影响，比较校正后的轨迹几何与难度的耦合。

Result: 代码领域推理模型在困难问题上显示更直接的校正轨迹和更少的局部曲率异质性，而数学和布尔可满足性中耦合较弱。提示阶段线性探针未复现代码领域的分离，行为标注显示更强的校正耦合伴随策略转变和不确定性监控。

Conclusion: 长度校正是轨迹分析的前提，推理训练可导致独特的校正轨迹几何，且效果因领域而异。
...
TL;DR: 提出了一个模块化的检索增强生成（RAG）管道，用于从对话式护士-患者转录中提取观察结果，最佳配置达到80.36% F1分数。


<details>
  <summary>Details</summary>
Motivation: 临床文档负担沉重，医护人员花费大量时间在文档工作上，而非直接患者护理。需要自动化从对话转录中提取结构化观察结果。

Method: 构建了模块化RAG管道，包括使用训练集作为示例库、模式约束提示（完整模式 vs. 修剪候选模式）、确定性模式后处理和第二遍审计，采用Llama-4-Scout和GPT-5.2作为LLM骨干，并搭配相应嵌入模型。

Result: 最佳配置（GPT-5.2 + 完整模式 + RAG + 第二遍审计）达到80.36% F1分数。

Conclusion: RAG持续提升性能，最佳模式约束程度取决于模型，第二遍审计通过修正残留模式合规错误带来适度增益。
...
TL;DR: 提出FINESSE-Bench，包含8个专项基准测试和3993个问题，用于分层评估大型语言模型在金融领域的专业能力。


<details>
  <summary>Details</summary>
Motivation: 现有金融基准如FinQA、ConvFinQA等主要关注财务报告问答，缺乏专业难度层次，且未系统评估从基础到专家级的金融推理能力。

Method: FINESSE-Bench结合了专业认证考试题（类似CFA、CMT、CFTe）、应用交易任务集和俄语奥林匹克基准，采用统一评估协议（含选择题、数字答案和简短开放回答），并基于LLM作为裁判的自动评分方案。

Result: 设计了覆盖领域广度、难度退化、计算任务解决能力和专业领域行为的评估框架，但未报告具体模型性能数值。

Conclusion: FINESSE-Bench旨在补充现有开放金融基准，为评估LLM的专业金融能力提供更实质性的工具。
...
TL;DR: 本文揭示了旋转位置编码（RoPE）在基于Transformer的长上下文语言模型中的固有限制，证明了随着上下文长度增加，RoPE的注意力机制会失去局部性偏差和令牌相关性一致性，逼近随机猜测水平，并且调整基超参数无法同时区分位置和令牌。


<details>
  <summary>Details</summary>
Motivation: 长上下文语言模型中位置编码的重要性，但RoPE在长上下文下可能存在未被认识到的根本性限制。

Method: 理论分析抽象掉上下文具体内容，仅依赖长度，证明RoPE注意力在长上下文下失去局部性和一致性；实证分析使用多头多层架构验证。

Result: RoPE注意力在长上下文下局部性偏差消失，令牌相关性不一致，失败概率接近0.5；提高RoPE基超参数会牺牲位置区分能力；多头多层架构无法克服这些限制。
...
TL;DR: 提出了DetectRL-X，一个多语言基准，用于评估LLM内容检测器，涵盖8种语言、6个领域和多种攻击策略。


<details>
  <summary>Details</summary>
Motivation: 检测和治理LLM生成内容日益重要，但现有检测器在多语言真实场景下的可靠性未充分探索。

Method: 构建包含8种商业常用语言、6个易被滥用的领域、4个商业LLM生成文本，并包括润色、扩写、缩写等AI辅助操作及多语言改写和扰动攻击的基准。

Result: 实验揭示了当前检测器在不同语言上的优势和局限，分析了域、生成器、攻击策略、文本长度和精炼操作的影响。

Conclusion: DetectRL-X是强化多语言和特定语言检测器的有效基准。
...
TL;DR: 提出BetaPRM，一种分布式的过程奖励模型，预测步骤级成功概率和可靠性，并基于此设计自适应计算分配（ACA），在多个推理基准上提升效率与准确率。


<details>
  <summary>Details</summary>
Motivation: 现有过程奖励模型只输出单一步骤奖励，缺乏可靠性指示，下游方法无法区分可靠与不可靠的预测。

Method: BetaPRM利用Beta-Binomial似然学习步骤成功概率及其可靠性，ACA根据可靠性信号动态停止或增加计算资源。

Result: 在四种骨干网络和四个推理基准上，BetaPRM提升最佳N选一的性能，ACA比固定预算最佳16选一减少最多33.57%令牌使用并提高准确率。

Conclusion: BetaPRM和ACA有效利用可靠性信号优化推理过程，实现计算与准确率的更好平衡。
...
TL;DR: 研究连续扩散和流模型在非自回归文本生成中的应用，提出草稿条件潜在精炼模型，发现潜在几何不能保证解码质量，强调解码器可恢复性评估。


<details>
  <summary>Details</summary>
Motivation: 解决连续潜在状态与离散token之间的接口问题，改善非自回归文本生成性能。

Method: 构建包含冻结BERT编码器、并行解码器、去噪DraftPrior、局部FlowNet和对角线MetricNet的草稿条件潜在精炼模型，将生成任务转化为受控局部精炼。

Result: 在ROCStories上，768维潜在空间比256维更有效；DraftPrior在干净草稿下目标token概率0.938，3% dropout时0.613，5% dropout时0.483，10% dropout时0.272；局部流精炼和解码器感知读出仅带来微小增益，度量学习与OT对齐改善几何但未弥合解码器差距。

Conclusion: 潜在几何本身不足以保证良好解码；连续潜在文本生成应通过解码器可恢复性、起始分布质量和精炼保持解码器可读结构来评估。
...
TL;DR: 提出GiLT模型，利用依存图调制注意力权重，在不插入额外token的情况下增强Transformer语言模型的句法泛化能力，保持困惑度竞争力并可微调。


<details>
  <summary>Details</summary>
Motivation: 增强语言模型句法泛化性能，现有方法多依赖句法树结构，且需插入额外token。

Method: GiLT通过增量构建的依存图提取特征，调节Transformer注意力权重，注入结构信息。

Result: 在句法泛化上优于基线，困惑度与Transformer语言模型相当，微调后下游任务性能提升。

Conclusion: GiLT有效利用依存图结构，无需额外token，性能提升且兼容预训练模型。
...
TL;DR: 对27个现代开源LLM检查点进行系统测量，发现激活最大值跨家族、架构和训练阶段差异巨大（可达四个数量级），MoE模型峰值显著低于密集模型，残差流常携带全局最大值，建议在低比特部署前报告该指标。


<details>
  <summary>Details</summary>
Motivation: 先前研究仅针对2024年前的LLaMA模型，未涵盖后LLaMA时代开源模型爆发的新情况，导致低比特量化等部署策略可能基于过时的激活分布认知。

Method: 构建统一流水线（5000样本多域语料库、家族特定分词、嵌入/隐藏状态/注意力/MLP/MoE/SwiGLU门/最终层归一化的统一钩子），测量8个家族27个检查点的全局和逐层激活最大值。

Result: (i) 相同参数量下全局最大值跨越近四个数量级，Qwen3.5和MoE模型在10^2~10^3，Gemma3-27B-it达~7×10^5；(ii) 跨家族和跨代际比较打破简单单调缩放；(iii) MoE检查点峰值比同规模密集模型低14.0-23.4倍，残差流在22/24检查点携带全局最大值。INT-8验证表明测量最大值与低比特重建误差相关。
...
TL;DR: 提出一种混合执行范式Nexa，通过响应条件策略生成稀疏有向无环通信图，结合并行与顺序模式，提升多智能体协作效率。


<details>
  <summary>Details</summary>
Motivation: 现有并行和顺序协作模式无法同时最小化通信延迟和最大化最终响应准确性，需要一种平衡两者的混合方法。

Method: Nexa先并行执行，将响应嵌入共享语义空间，然后由轻量Transformer策略预测稀疏有向无环图；若图为空则保持纯并行，否则进行一轮顺序消息传播。策略通过策略梯度训练，无需外部评判或手工拓扑搜索。

Result: 学习到的通信策略在智能体数量、任务或底层模型改变时仍可复用，证明了其泛化能力。

Conclusion: Nexa框架严格包含纯并行执行，通过混合执行实现了通信与准确性的更好权衡，且学得的策略具有良好的可迁移性。
...
TL;DR: 提出语义空间校准（CSR）框架，直接校准模型语义而不依赖口头置信度，显著降低校准误差并提升AUROC。


<details>
  <summary>Details</summary>
Motivation: LLM在关键领域需要良好校准的不确定性，但标准RLVR训练忽略置信度导致校准下降，口头置信度存在语义不一致问题。

Method: CSR结合正确性奖励和语义校准奖励，对正确输出促进语义一致，对错误输出抑制虚假一致性。

Result: 在HotpotQA、TriviaQA、MSMARCO、NQ-Open上，CSR相比基线降低ECE最多40%，提升AUROC最多31%。

Conclusion: CSR能有效校准模型，且行为泛化鲁棒。
...
TL;DR: 提出了MHGraphBench，一个基于知识图谱的基准，用于评估大语言模型在心理健康领域的实体识别、关系判断和两跳推理能力，发现模型存在识别到判断的差距且输出格式影响测量性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在心理健康领域应用日益增加，但其捕捉相关生物医学知识和可靠应用于临床结构判断的能力尚不清楚。

Method: 从PrimeKG衍生出包含9个任务族的基准，每个任务有知识图谱支持的答案和受控负选项，用于评估实体识别、关系判断和两跳推理。

Result: 在15个模型上实验显示，顶尖模型在实体类型和关系类型子集上接近完美，但在关系预测和两跳推理上仍困难；短KG片段对部分模型有益但对其他有害；输出格式可靠性显著影响受限选择设置下的测量性能。

Conclusion: MHGraphBench应解释为在受限选择题接口下与PrimeKG中经过挑选的心理健康子集的一致性评估，而非直接衡量真实世界临床安全性。
...
TL;DR: 大型语言模型在未见过的编程语言上存在实现保真度差距，即算法理解无法转化为正确代码。


<details>
  <summary>Details</summary>
Motivation: 探究LLMs是否能在预训练未出现的语言上转移代码生成能力。

Method: 引入新语言PyLang，微调Qwen3模型并施加多种干预（多任务、偏好微调、代码填充、隐空间目标）。

Result: Python性能比PyLang高19%，所有干预均无法缩小差距；模型内部表示高度相似（CKA>0.97），但输出阶段出现分歧。

Conclusion: 模型具备语言无关的算法理解，但无法在陌生语言中表达，需解耦推理与语言实现。
...
TL;DR: 提出并行推测解码（PSD），一种无需训练的框架，通过联合空间和时间优化，加速扩散大语言模型的推理。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型（dLLMs）需要多次去噪迭代，导致推理成本高昂。

Method: 利用单次前向传递的置信度分数，通过自适应策略选择要解掩的位置，并构建多深度推测草稿，最后通过批量验证步骤进行层次化接受。

Result: 在三个dLLMs上的实验表明，PSD实现了每前向传递最多5.5倍令牌数的加速，且准确性接近贪心解码。

Conclusion: PSD在推理效率与生成质量之间取得了良好的平衡。
...
TL;DR: 开源LLM生成的序列严重偏向英语；持续预训练相比从头训练没有成本优势，甚至对目标语言文化理解提升有限；未来应针对每种语言进行专门投资。


<details>
  <summary>Details</summary>
Motivation: 发现LLM对英语存在严重偏见，质疑持续预训练作为适应目标语言的方法是否有效。

Method: 分析开源LLM生成的序列，比较持续预训练与从头训练的成本及对文化理解的影响。

Result: 持续预训练并不比从头训练更经济，且在改善目标语言文化理解方面也无优势。

Conclusion: 未来LLM开发不应主要依赖英语中心资源的扩展，而应针对每种语言进行专门投资。
...
TL;DR: 构建了首个基于潜在歧义理论的中文歧义数据集CHA-Gen，并系统评估了LLM在中文歧义处理上的表现，发现模型存在歧义盲视、错误归因和过早解析等失败模式，且指令微调导致过度自信。


<details>
  <summary>Details</summary>
Motivation: 现有中文歧义数据集可扩展性差，且研究多聚焦英文，缺乏对中文LLM歧义处理的系统评估。

Method: 基于潜在歧义理论设计半自动流水线，构建包含5712句（2414歧义、3298无歧义）的CHA-Gen数据集，通过直接查询和机器翻译评估多种LLM，并利用语义熵进行不确定性量化。

Result: LLM在歧义检测上表现欠佳，CoT提示可提升性能；模型倾向于主导解释，指令微调导致过度自信，基础模型更好捕捉语义多样性。

Conclusion: 提供了可扩展的中文歧义语料库构建方法，揭示了LLM处理中文歧义的不足与偏差，为后续研究奠定基础。
...
TL;DR: 提出动态分块扩散模型，用内容定义的语义块替代固定位置块，通过分块注意力层实现块因果自回归分解，在1.5B参数规模下优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 固定位置块浪费序列结构，分隔语义连贯标记、组合不相关标记，需内容驱动的分块机制。

Method: 核心为分块注意力层，将标记路由到K个学习子空间簇中，生成块因果注意力掩码，实现语义块上的自回归扩散去噪。

Result: 在高达1.5B参数的下游基准中，DCDM一致超越无结构和位置块扩散基线，优势跨尺度稳定且训练早期可见。

Conclusion: 基于内容的分块有效提升离散扩散语言模型性能，严格推广了块离散扩散方法。
...
TL;DR: 提出了一种基于mxGraph XML的Diagram-as-Code范式，用于精确的图表生成与编辑，并构建了VCG-Bench基准测试，以评估当前视觉-语言模型在结构化图表任务中的能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在处理结构化、可控的图表任务时存在缺陷，基于像素的合成方法在可编辑性和保真度方面受限。

Method: 采用符号逻辑的Diagram-as-Code范式，使用mxGraph XML进行图表生成与编辑；构建VCG-Bench，包含1449个图表的分类数据集、定义生成（视觉到代码）和可编辑性（代码到代码）范式，以及多维评估协议（如mxGraph执行成功率、风格一致性得分等）。

Result: 实验表明，当前最先进的视觉-语言模型在结构化保真度和指令遵循方面面临挑战，反映了其视觉和推理能力的不足。

Conclusion: 本文提出的范式和基准揭示了现有视觉-语言模型在专业图表任务中的局限性，为未来改进提供了方向。
...
TL;DR: 本研究评估了大型语言模型在低资源标注条件下对非正式在线患者咨询进行四类分诊（自我护理、预约就诊、紧急临床复诊、紧急转诊）的能力，发现最强模型在macro-F1上略优于有监督基线，但置信区间重叠，且不同类别表现存在差异。


<details>
  <summary>Details</summary>
Motivation: 在线患者咨询通常非正式且不完整，但需被分诊至适当的临床随访级别，现有方法在低资源标注条件下效果有限。

Method: 使用HealthCareMagic-100K语料构建金标准、银标准和少样本池，比较TF-IDF和BioBERT（银标准训练）与六个LLM在0-shot、4-shot、12-shot下的分诊性能，评估macro-F1和安全性指标。

Result: 最强LLM（Claude Haiku 4.5, 12-shot）macro-F1为0.475，超过最佳基线BioBERT的0.378，但置信区间重叠；少样本提示和双模型一致性对自我护理可靠，对紧急临床复诊不可靠。

Conclusion: LLM可支持分诊优先和选择性人工审核，但尚不能自主部署。
...
TL;DR: 提出ASRU框架，通过激活重定向和奖励函数优化遗忘边界，在提升遗忘效果的同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有多模态遗忘方法忽略生成质量，导致幻觉或僵化响应，影响模型可用性和安全性。

Method: 先通过激活重定向诱导初始拒绝行为，再用自定义奖励函数优化细粒度遗忘边界。

Result: 在Qwen3-VL上遗忘效果平均提升24.6%，生成质量提升5.8倍，仅用少量保留数据。

Conclusion: ASRU有效平衡了目标知识遗忘与模型效用，显著提升了遗忘效果和生成质量。
...
TL;DR: H-Mem通过混合树和图结构有效建模记忆演化并实现高效检索，在QA任务上达到最优。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏对记忆数据随时间演化的有效建模和检索机制，导致记忆利用性能差。

Method: 构建时间语义树结构，使短期记忆逐步演化成长期记忆，同时构建知识图谱捕捉实体关系，并利用树与图的混合结构进行高效检索。

Result: 在三个智能体记忆基准上，QA任务达到目前最优性能。

Conclusion: H-Mem通过混合结构显著提升了记忆建模和检索效果，验证了其有效性。
...
TL;DR: 提出SMMBench基准，评估智能体从多个独立来源中检索、对齐和组合多模态证据的能力，发现现有系统在此任务上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有基准多在预组装上下文中评估多模态记忆，忽略了证据分散于多个独立来源的情况，这是一个重要但未充分研究的瓶颈。

Method: 构建包含1877个样本和264个来源的SMMBench基准，设计四大核心能力测试：跨源多模态推理、冲突解决、偏好推理和记忆驱动动作预测。

Result: 代表性基线方法在SMMBench上表现挣扎，表明当前系统在跨源分布式多模态记忆推理上仍有巨大挑战。

Conclusion: 源分布式多模态记忆是多模态智能体中重要且未充分评估的难题，需要进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将上下文工程视为全局搜索问题，为整个数据集寻找单一上下文策略，忽略了不同输入需要不同指导，导致实例级性能潜力未开发。

Method: 首先生成多样化锚定上下文目录，然后通过Context-CF协同进化机制：轻量级神经协同过滤模型学习实例-上下文偏好以指导生成专用上下文变体，新上下文又反过来优化模型。推理时，模型作为上下文路由器为每个实例动态分配最优上下文。

Result: 理论证明和实验表明，NCCE通过匹配输入与最优上下文显著提升了任务准确性。

Conclusion: 个性化在大型语言模型上下文工程中至关重要，NCCE通过实例级定制实现了性能提升。
...
TL;DR: 提出DimMem，一种轻量级维度记忆框架，将记忆表示为原子化、带类型和自包含单元，包含时间、地点、原因、目的和关键词等显式字段，实现维度感知的检索、更新和选择性回忆，在不存储完整历史的情况下提升记忆效率。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理记忆系统面临保真度与效率的权衡：原始对话历史代价高，而扁平事实或摘要可能丢失精确回忆所需的结构。

Method: DimMem将每段记忆表示为原子化、带类型和自包含单元，具有时间、地点、原因、目的、关键词等字段，支持维度感知检索、更新和选择性上下文回忆。

Result: 在LoCoMo-10和LongMemEval-S上分别达到81.43%和78.20%总体准确率，优于现有轻量级记忆系统，同时LoCoMo每查询token成本降低24%。微调后的Qwen3-4B提取器在关键设置下表现优于或媲美大型提取器。
...
TL;DR: 本研究证明小型开源LLM在机器翻译质量估计中可作为大型专有模型的有效、经济且隐私保护的替代方案。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的QE依赖大型专有LLM，导致数据隐私问题，需要更安全且经济的替代方案。

Method: 采用单次提示策略，使用参数小于30B的开源LLM同时生成质量分数、MQM错误注释、错误修正建议和完整后编辑。

Result: 模型在系统级与人类判断的相关性上优于传统神经指标、微调模型和人类标注者间一致性，接近大型专有LLM的性能。

Conclusion: 小型开源LLM是可行且具有竞争力的QE方案，兼顾效果、成本和隐私。
...
TL;DR: 提出了ForMaT，一个包含15种语言对、3956份PDF的平行语料库，保留原始布局元数据，用于多模态机器翻译。


<details>
  <summary>Details</summary>
Motivation: 现有机器翻译系统在处理布局信息时表现不佳，缺乏保留布局的平行语料库，因此构建ForMaT以支持布局感知翻译模型的发展。

Method: 使用K-Medoids聚类方法，基于45个几何特征对PDF进行采样，确保数据集中包含嵌套表格、公式等复杂结构的多样性。

Result: 评估显示当前机器翻译系统在空间定位和几何同步上存在困难，经常丢失文本与其视觉上下文的关联。ForMaT为布局感知翻译提供了基准。

Conclusion: ForMaT有助于开发融合视觉和文本上下文的翻译模型，实现高保真文档重建。
...
TL;DR: 本文介绍了一个俄罗斯政府跨模态政治沟通数据集，包含数十年间克里姆林宫和外交部的官方演讲、文本、图像及元数据，并经由主题建模和专家验证，支持多模态、多语言、时空分析。


<details>
  <summary>Details</summary>
Motivation: 针对威权政治背景下社交媒体文本和图像数据长期缺乏的问题，构建该数据集以弥补不足。

Method: 收集高层演讲，提供俄英双语文本、关联图像及字幕、统一元数据（日期、发言人、地理位置等），通过基于Transformer的多模态主题建模生成主题标注，并由俄罗斯政治专家验证。

Result: 数据集支持多模态、多语言、时间与空间维度的威权政治沟通分析。

Conclusion: 为社会科学研究和政治领域的大语言模型应用提供了有价值的测试平台。
...
TL;DR: 提出语义分割数据集和轻量分割器实现文本自动分块，以及块蒸馏框架使块注意力达到近全注意力性能。


<details>
  <summary>Details</summary>
Motivation: 块注意力在RAG等长上下文场景中可提升KV缓存重用，但面临文本分块困难和块微调效率低的问题。

Method: 构建SemanticSeg数据集（30k+实例，16类），训练轻量分割器；提出块蒸馏框架，包括块沉令牌、块丢弃和令牌级损失加权。

Result: 分割器优于启发式和统计基线，块蒸馏在块注意力下达到近全注意力性能。

Conclusion: 建立了实用可扩展的块注意力部署路径。
...
TL;DR: 将GRPO应用于NLLB-200编码器-解码器模型，使用无参考混合奖励（LaBSE+COMET-Kiwi），在13种语言上一致提升翻译质量，最高提升+5.03 chrF++，且无需目标语言数据即可与有监督微调竞争。


<details>
  <summary>Details</summary>
Motivation: 强化学习在机器翻译微调中主要针对解码器-only的大语言模型（≥7B），而对编码器-解码器架构的系统研究不足。

Method: 采用组相对策略优化（GRPO）训练NLLB-200（600M和1.3B），使用无需参考的混合奖励（LaBSE和COMET-Kiwi），在微调时不依赖平行数据。

Result: 在13种语言上均获得一致提升，中文繁体提升最大（+5.03 chrF++），且无需目标语言数据即可与三轮有监督微调效果相当。

Conclusion: GRPO在基线弱、奖励区分度高的语言上提升最大，使其在平行数据稀缺时尤为有效，该模式在英语和西班牙语源语言中复现。
...
TL;DR: 提出一个符号框架，将执法报告中的自然语言叙述转化为证据关联的事实，并构建时间图。


<details>
  <summary>Details</summary>
Motivation: 执法报告中的关键事实隐藏在自然语言叙述中，需要人工阅读，自动化提取可提高效率。

Method: 使用符号方法：去标识、语义解析、谓词映射到本体、推理，构建时间图。

Result: 在450份财产犯罪报告中，54.1%的事件置信度≥0.80，93.7%通过PropBank-VerbNet-WordNet映射，人工评估在事件触发、被盗物品和时间线索上一致率达100%。

Conclusion: 符号方法能有效从叙述中提取结构化事实，但强制进入解释等复杂推理仍需改进。
...
TL;DR: 提出AI文化能力的三级分类法，以澄清评估中的构念模糊性。


<details>
  <summary>Details</summary>
Motivation: 现有AI文化能力评估定义模糊、术语互换、局限在回忆信息，需要清晰构念。

Method: 借鉴跨文化传播学，提出文化意识、文化敏感性、文化能力三级分类。

Result: 该分类可作为实用工具，提高评估效度和可解释性。

Conclusion: 缺乏构念清晰会使评估夸大能力，导致不当部署。
...
TL;DR: 提出基于学习者模型的评估准则，评估视觉语言模型在数学辅导中的适应性，发现现有模型适应性不足。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏系统评估视觉语言模型在数学辅导中对不同学习者适应性能力的框架。

Method: 基于自适应学习框架中的学习者模型，从认知、动机、复杂性三个方面评估适应性，同时评估正确性和响应质量。

Result: 实验显示模型间适应性存在差异，且现有视觉语言模型难以持续产生基于学习者模型的教学响应，尤其在有限学习者信息下。

Conclusion: 当前视觉语言模型在数学辅导中的适应性有待提升，需要进一步改进以支持个性化教学。
...
TL;DR: LLM作为评判者因输出格式变化导致评分不一致，通过PEAP方法发现共享的潜在评估子图和脆弱的格式特定分支，表明不一致源于格式化器几何结构。


<details>
  <summary>Details</summary>
Motivation: 现有诊断停留在输入输出层面，未深入内部机制解释格式导致的不一致。

Method: 使用位置感知边缘归因补丁（PEAP）对Gemma-3、Qwen2.5、Llama-3进行因果分析。

Result: 在中期到晚期MLP中发现稀疏的通用潜在评估子图，零消融破坏评判但保留世界知识；判断信号通过共享主干计算，映射到脆弱的格式特定终端分支。

Conclusion: 格式不一致部分源于格式化器几何结构而非评估质量，可分离格式无关偏好。
...
TL;DR: 提出S2ST-Omni 2框架，通过多层级语言条件化利用类型学先验，在CVSS-C上取得优于基线的方法，并实现数据高效的多语言语音到语音翻译。


<details>
  <summary>Details</summary>
Motivation: 现有S2ST系统忽略源语言结构或使用扁平嵌入，限制数据高效的多语言适应。

Method: 从表示、声学和解码三个层面重新设计语言条件化：类型学感知的分层语言编码、动态门控语言感知双CTC、类型学感知LLM提示。

Result: 在CVSS-C上BLEU、COMET、ASR-BLEU和BLASER 2.0指标优于代表性方法；消融实验验证各策略互补；数据预算分析显示类型学先验在有限数据下提供有效归纳偏置。
...
TL;DR: 提出RecMem，通过延迟记忆整合和轻量级嵌入，在降低token开销的同时提升记忆准确性。


<details>
  <summary>Details</summary>
Motivation: 现有记忆系统对每轮交互都调用LLM提取记忆，导致大量token消耗。

Method: 将交互存入潜意识层，用轻量级嵌入检索；仅在语义相似交互持续重复时调用LLM提取情景和语义记忆，并加入语义细化机制恢复遗漏细节。

Result: 相比三种SOTA系统，内存构建token成本降低高达87%，且准确性更高。

Conclusion: RecMem通过基于重现的记忆整合策略，有效平衡了效率与准确性。
...
TL;DR: 提出一种基于大语言模型的数据增强框架，通过生成语义相似的合成语音样本来改善认知评分预测，特别有效缓解类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 基于自发语音的认知衰退评估受限于数据集规模小和类别不平衡，现有方法改进有限。

Method: 利用GPT-5以书面回答为语义锚点生成多种风格的口头独白，结合Sentence-BERT嵌入和偏最小二乘回归预测Hasegawa痴呆量表分数，并采用相似性引导的类平衡选择策略。

Result: 相似性引导的类平衡选择策略带来更一致的改进，显著降低低分参与者的预测误差，同时保持多数群体性能。

Conclusion: 语义引导的LLM驱动增强是解决类别不平衡和提高临床语音分析数据效率的有效原则性方法。
...
TL;DR: 提出多层级上下文token关系建模框架，通过局部校准和全局规则推理提升机器生成文本检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有度量方法中，token级检测分数易受生成随机性影响，需要更鲁棒的建模。

Method: 基于token级检测分数的多跳转移，提出局部马尔可夫校准和全局规则支持推理的联合框架。

Result: 在跨LLM和跨领域场景中取得广泛且实质性的提升，计算开销低。

Conclusion: 该框架有效解决了度量方法的核心挑战，具有实用性和泛化能力。
...
TL;DR: DebiasRAG是一种基于检索增强生成的免微调动态去偏框架，用于减少大型语言模型中的社会偏见。


<details>
  <summary>Details</summary>
Motivation: 现有偏见缓解方法需额外训练或领域知识，可能降低模型能力，且缺乏动态去偏能力。

Method: 三阶段：查询特定去偏候选生成、上下文候选池构建、梯度更新去偏引导的上下文重排序。

Result: 在保持LLM固有性能的同时提高了公平性。

Conclusion: DebiasRAG有效减少偏见，无需微调，且具有动态性。
...
TL;DR: SGR通过外部知识子图逐步引导LLM推理，提升复杂推理的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: LLM在复杂推理中表现有限，且易生成不相关或事实不一致的内容。

Method: 先构建与输入问题对应的子图，再引导模型在生成的结构上逐步推理，结合多条推理路径得到最终预测。

Result: 在多个基准数据集上，SGR持续优于竞争基线，提高了推理准确性和事实可靠性。

Conclusion: SGR是一种有效增强LLM推理能力的方法，能提升推理准确性和事实可靠性。
...
TL;DR: 该案例研究展示了如何将AI编码系统（AntiGravity）与LLM驱动的树搜索算法（ERA）结合，自主生成高效的三维光伏结构，克服了平铺太阳能板在中纬度地区的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统平铺太阳能板在中纬度地区存在能量损失，需要新型三维光伏结构以通过优化日照角度提高能量密度。

Method: 使用AntiGravity复现初始计算，然后通过ERA树搜索优化设计，并迭代修补物理引擎以消除由非物理特征引起的奖励黑客。

Result: 在消除奖励黑客后，ERA发现了多种具有不同约束的优化设计，包括固定收集器面积的最优结构、天顶跟踪和自阴影避免方案，显著提升了日周期性能。

Conclusion: 将编码代理与树搜索相结合，为可经验评估得分的问题提供了强大的科学发现平台。
...
TL;DR: 提出基于价值角色的方法，通过从调查响应中提取文化维度描述，改进LLM跨文化模拟人口意见的准确性，缩小了代表性不足群体的性能差距。


<details>
  <summary>Details</summary>
Motivation: 现有基于社会人口统计学或人格特征的提示方法仅是价值观的间接代理，无法准确复现跨文化人口响应。

Method: 从调查响应中提取文本描述构建价值角色，采样目标人群的价值档案，聚合LLM响应获得人群级预测，并引入校准程序提升响应多样性。

Result: 降低了跨国家的预测误差，尤其在代表性不足人群中改进最大，响应分布更接近人类多样性。

Conclusion: 基于价值的方法能有效缩小LLM在文化多样性上的表现差距，提升模拟人口意见的可靠性。
...
TL;DR: Argus通过Searcher和Navigator协作，将深度研究视为拼图而不是并行暴力搜索，大幅提升效率并降低上下文开销。


<details>
  <summary>Details</summary>
Motivation: 并行搜索会重复证据导致收益递减，且聚合上下文接近模型极限。

Method: Searcher负责子查询的ReAct式证据收集；Navigator维护证据图，验证缺失部分并调度Searcher，通过强化学习训练。

Result: 在35B-A3B MoE上，单Searcher提升5.5点，8个并行提升12.7点；64个Searcher在BrowseComp达86.2，超越所有专有代理，且导航上下文低于21.5K tokens。

Conclusion: Argus通过分治协作，高效且可扩展地完成深度研究任务。
...
TL;DR: 提出一种模仿失语症的技术，通过损伤语言模型参数来研究其功能组织，并与人类失语症症状进行比较。


<details>
  <summary>Details</summary>
Motivation: 利用失语症研究人类语言功能组织的方法，将其应用于语言模型，以揭示模型中的功能组织。

Method: 对语言模型进行参数清零（损伤），使用文本失语症测试（TAB）评估其输出，并与临床失语症症状对比。

Result: 在五个1B规模的语言模型中，出现了全部症状但分布与人类不同；注意力和前馈组件间存在差异，早期层损伤导致句法和语义症状，中后期层导致音位和流畅性缺陷。

Conclusion: 某些模型损伤与人类失语症类型有量上相似，但定性差异表明失语症综合征受学习细节影响，而非领域不变的后果。
...
TL;DR: 智能计量、生成式AI和量子启发组合优化的融合正在彻底改变能源公用事业在基础设施、客户参与和环境责任方面的管理方式。


<details>
  <summary>Details</summary>
Motivation: 应对能源管理中物理基础设施、客户互动和环境责任日益复杂的挑战。

Method: 整合智能计量、生成式人工智能和量子启发组合优化技术。

Result: 能源公用事业管理模式被重新塑造，效率与可持续性提升。

Conclusion: 这一技术融合是能源行业变革的关键驱动力。
...
TL;DR: 提出一个端到端框架，整合生成式AI账单代理和变压器预测器，实现可读账单、碳核算和负荷调度。


<details>
  <summary>Details</summary>
Motivation: 配电公司需要提供可读账单、每千瓦时碳排放可辩护数字，并根据电网压力和排放约束调度负荷。

Method: 统一框架包含四个生产级能力：基于约束解码策略的生成式AI代理，生成自然语言账单；变压器预测器提供日前消费估计和校准分位带。

Result: 框架成功整合了账单生成、碳排放核算和负荷预测功能。

Conclusion: 该框架为配电公司提供了一体化解决方案，支持可读账单、碳核算和优化调度。
...
### [91] [Toward Natural and Companionable Virtual Agents via Cross-Temporal Emotional Modeling](https://arxiv.org/abs/2605.15812)
*Feier Qin,Xiao Li,Yi Zheng,Haibin Huang,Hanyao Wang,Xiaoyu Wang,Yan Lu,Yuan Zhang*

Main category: cs.HC

TL;DR: 提出了跨时间情感建模（CTEM）框架，通过连接长期行为历史与即时情感表达，实现了更自然、连贯和情感和谐的长期陪伴对话。


<details>
  <summary>Details</summary>
Motivation: 现有对话代理缺乏对社交行为和内部情感的跨时间建模，导致交互体验显得片段化且不真实。

Method: 构建CTEM闭环框架，利用过去经历更新情感状态，情感状态影响即时交互，用户反馈同时修正记忆和情感状态，实现反思与预期。

Result: 在即时通讯平台上部署的Auri代理的21天野外研究表明，CTEM在自然性、连贯性和情感和谐性方面均有提升。

Conclusion: CTEM能够支持更自然、连贯且情感和谐的长时陪伴交互。
...
### [92] [SLIP & ETHICS: Graduated Intervention for AI Emotional Companions](https://arxiv.org/abs/2605.15915)
*Minseo Kim*

Main category: cs.HC

TL;DR: AI情感伴侣面临安全-融洽悖论，本文提出SLIP四阶段干预协议和ETHICS信号分类法，通过小规模部署和合成角色测试评估，发现分层干预可在0%假阳性下改善检测，但存在边界问题。


<details>
  <summary>Details</summary>
Motivation: 解决AI情感伴侣中限制性安全措施与支持性融洽之间的冲突。

Method: 提出SLIP（四阶段渐进干预协议）和ETHICS（基于信号而非标签的分类法），结合小规模生产部署（10用户，68条记录，10周）和合成角色测试（91个角色，5种行为风险轮廓）。

Result: 流动角色假阳性率为0%，危机导向角色表现出预期升级模式；但连续8天高能量提升导致零干预，更大模型将检测率从0/8提升至6/8且保持0/10假阳性。

Conclusion: 分层干预可作为在情感计算中导航安全-融洽张力的设计方向，而非彻底解决。
...
### [93] [Conversations in Space: Structuring Non-Linear LLM Interactions on a Canvas](https://arxiv.org/abs/2605.15848)
*Rifat Mehreen Amin,Alperen Adatepe,Daniela Fernandes,Daniel Buschek,Andreas Butz*

Main category: cs.HC

TL;DR: 一种将线性对话转变为在空间画布上的分支对话树的对话界面，支持探索性工作流。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的对话界面线性结构限制了替代方案的探索和长期交互管理。

Method: 提出CanvasConvo概念，通过分支对话树嵌入画布，支持假设场景探索、并行分支、时间线导航、自动标签摘要、上下文感知控制。

Result: 通过24名参与者5-7天的实地研究，发现非线性对话结构支持探索性工作流和不同交互模式。

Conclusion: CanvasConvo通过非线性结构增强LLM对话的探索性和连续性。
...
### [94] [Universal Magnetic Structure Prediction from Atomic Coordinates with Near-Experimental Accuracy](https://arxiv.org/abs/2605.16230)
...
TL;DR: 提出磁性结构网络（MSN），一个E(3)等变图神经网络，直接从原子晶体结构预测共线和非共线磁结构，使用原始调制结构表示（PMSR）统一编码共度和非共度结构，在MAGNDATA上训练，实现高保真重建。


<details>
  <summary>Details</summary>
Motivation: 磁结构难以确定，实验昂贵且专业，第一性原理方法难以处理非共线和非共度磁序。

Method: 构建E(3)等变图神经网络MSN，采用原始调制结构表示（PMSR）将共度与非共度结构统一编码，无需对称性假设。

Result: 模型在所有调制组件上表现良好，高保真重建实验磁结构。

Conclusion: 提供了一个可扩展的快速磁结构预测框架，开辟了数据驱动发现磁性材料的途径。
...
### [95] [Multicritical Scaling Limit of Shifted Schur Measure](https://arxiv.org/abs/2605.15744)
*Haruna Aida,Taro Kimura*

Main category: math.CO

TL;DR: 研究了移位Schur测度的多临界标度极限，严格证明了从Pfaffian点过程到行列式分布的转变。


<details>
  <summary>Details</summary>
Motivation: 理解多临界标度极限下关联函数的转变行为。

Method: 通过适当的标度极限和连续参数条件，分析严格划分的极限形状和关联函数。

Result: 确定了极限形状，并证明边缘标度极限下关联函数收敛到高阶Airy核的行列式。

Conclusion: 严格验证了从Pfaffian点过程到行列式分布在标度极限下的转变。
...
### [96] [Evaluating Design Video Generation: Metrics for Compositional Fidelity](https://arxiv.org/abs/2605.16223)
*Adrienne Deganutti,Dingning Cao,Jaejung Seol,Elad Hirsch,Purvanshi Mehta*

Main category: cs.GR

TL;DR: 提出一个全自动的评估框架，从布局保真度、运动正确性、时间质量和内容保真度四个维度评估设计动画生成视频模型。


<details>
  <summary>Details</summary>
Motivation: 设计动画领域缺乏标准化评估框架，现有自然视频评估方法不适用于设计动画的结构化约束。

Method: 构建包含布局保真度、运动正确性、时间质量和内容保真度四个维度的全自动评估框架。

Result: 消除了对主观人工评估的依赖，为领域进展提供了统一的基准。

Conclusion: 该框架为设计动画生成视频模型的评估和基准测试建立了共同基础。
...
### [97] [Unified Simulation of Lagrangian Particle Dynamics via Transformer](https://arxiv.org/abs/2605.15305)
...
TL;DR: 一个基于单一Transformer架构的学习型粒子模拟器，统一模拟多种物理现象（布料、弹性固体、牛顿/非牛顿流体、颗粒材料、分子动力学），采用预测-校正设计，通过显式预测器与学习校正器处理粒子间交互。


<details>
  <summary>Details</summary>
Motivation: 长期目标是构建无需针对求解器重新设计即可模拟多种物理现象的统一模拟器。

Method: 基于Lagrangian粒子表示的Transformer架构，包含显式预测器（基于外力推进粒子）和学习校正器（由粒子分词器、超级分词编码器、解码器组成，通过分层token合并降低注意力成本）。

Result: 在六种动力学类别中泛化到未见材料、边界、初始条件和外力，并实现交互控制、逆向设计及从真实数据学习。

Conclusion: 减少了针对不同现象进行求解器工程的需求。
...
### [98] [FFAvatar: Few-Shot, Feed-Forward, and Generalizable Avatar Reconstruction](https://arxiv.org/abs/2605.15320)
*Thuan Hoang Nguyen,Jiahao Luo,Yinyu Nie,Hao Li,Gordon Guocheng Qian,Jian Wang*

Main category: cs.GR

TL;DR: FFAvatar 是一个前馈框架，从少量无摆拍肖像图像中快速重建高质量、可动画的 3D 高斯头部头像，通过多视图查询融合和端到端 FLAME 预测，在数秒内完成重建，并在 NeRSemble 上比 LAM 提升 5.5 PSNR。


<details>
  <summary>Details</summary>
Motivation: 传统头像重建依赖逐主体优化（耗时数小时）或昂贵预处理，限制了可扩展性。需要一种快速、通用的方法。

Method: 提出 FFAvatar，使用多视图查询变形器将多源图像融合为统一规范高斯表示，并通过像素端到端预测 FLAME 参数进行动画。采用三阶段训练：大规模单目视频预训练（>100万身份）、小规模 360 度多视图微调、可选个性化（500 步优化）。

Result: 在 NeRSemble 上 PSNR 比 LAM 高 5.5；无需个性化时 2 秒重建，个性化时 10 秒，动画帧率 49 FPS（A100 GPU）。

Conclusion: FFAvatar 在身份保持、几何一致性和动画保真度方面设立了新标准，实现了实时部署。
...
### [99] [StippleDiffusion: Capacity-Constrained Stippling using Controlled Diffusion](https://arxiv.org/abs/2605.15816)
*Ofir Gilad,Aleksander Plocharski,Przemyslaw Musialski,Andrei Sharf*

Main category: cs.GR

TL;DR: 提出首个基于扩散模型的图像条件点集生成方法，实现快速、可微的随机点画生成，性能与逐密度优化基线持平。


<details>
  <summary>Details</summary>
Motivation: 传统随机点画生成方法速度慢、不可微，且需为新目标重新优化；现有学习方法仅适用于无条件的点生成，无法处理图像条件密度约束。

Method: 构建基于最优传输网格点集扩散基线的ControlNet分支，以目标密度图和高分辨率图像为条件，采用晚期去噪、密度加权拒绝采样初始化和sigmoid门控1x1投影替换零卷积注入。

Result: 单个训练模型可接受任意目标密度，泛化至未见点预算，生成时间几乎与点数无关，在Icons-50基准上达到与逐密度优化基线相当的性能。

Conclusion: 该方法实现了端到端可微的快速随机点画生成，且性能与迭代优化方法相当。
...
### [100] [From Feedback Loops to Policy Updates: Reinforcement Fine-Tuning for LLM-Based Alpha Factor Discovery](https://arxiv.org/abs/2605.15412)
*Lingzhe Zhang,Tong Jia,Yunpeng Zhai,Zixuan Xie,Chiming Duan,Minghua He,Philip S. Yu,Ying Li*

Main category: cs.CE

TL;DR: 提出QuantEvolver框架，基于强化微调实现自演进的alpha因子发现，解决LLM方法中上下文爆炸和搜索停滞问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM方法依赖提示级迭代优化，导致上下文爆炸、推理成本高、信息稀释和反馈漂移，且大型LLM生成结构相似的冗余因子，造成搜索停滞。

Method: QuantEvolver将量化评估转化为策略更新，通过构建种子因子和时间窗口任务，生成可执行因子DSL，经Regime Backtest评估，使用多样性-互补性奖励优化Miner LLM，并持续积累高质量因子到数据库中。

Result: 在三个真实市场基准上，QuantEvolver持续提升主要评估指标，生成更高质量和更互补的因子池，优于现有LLM方法。

Conclusion: QuantEvolver有效克服了LLM因子发现中的上下文膨胀和搜索停滞问题，实现了高效的自动化因子发掘。
...
### [101] [Modeling Music as a Time-Frequency Image: A 2D Tokenizer for Music Generation](https://arxiv.org/abs/2605.15831)
*Yuqing Cheng,Xingyu Ma,Guochen Yu,Xiaotao Gu*

Main category: cs.SD

TL;DR: 提出面向生成的分词器BandTok，使用共享码本的梅尔频带标记，改善自回归音乐生成。


<details>
  <summary>Details</summary>
Motivation: 现有残差多码本量化在自回归建模中存在强序列依赖和误差累积问题。

Method: 采用2D梅尔频谱图分词器，每帧用梅尔频带标记，配合多尺度PatchGAN和EMA码本更新；自回归语言模型使用2D旋转位置嵌入。

Result: 在数据有限环境下优于残差码本分词器，重建质量高。

Conclusion: BandTok提供一种物理可解释且结构独立的标记方式，适合自回归音乐生成。
...
### [102] [Beyond Content: A Comprehensive Speech Toxicity Dataset and Detection Framework Incorporating Paralinguistic Cues](https://arxiv.org/abs/2605.15984)
*Zhongjie Ba,Liang Yi,Peng Cheng,Qingcao Li,Qinglong Wang,Li Lu*

Main category: cs.SD

TL;DR: 提出ToxiAlert-Bench音频数据集和双头神经网络，利用副语言线索提升毒言检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有毒言检测忽视副语言线索，且数据集多为文本，限制了模型能力。

Method: 构建30k+音频数据集，标注毒源和细粒度类别；设计双头网络，多阶段训练，结合类平衡采样和加权损失。

Result: Macro-F1提升21.1%，准确率提升13.0%，优于所有基线。

Conclusion: 副语言特征对毒言检测至关重要，所提方法有效且实用。
...
### [103] [Functional Cox model for interval-censored data](https://arxiv.org/abs/2605.15303)
*Yangjianchen Xu,Peijun Sang*

Main category: stat.ME

TL;DR: 针对区间删失数据，提出了功能Cox模型，采用惩罚似然和EM算法，估计量具有优良性质，并应用于阿尔茨海默病数据。


<details>
  <summary>Details</summary>
Motivation: 区间删失数据中功能协变量影响事件时间的科学问题。

Method: 功能Cox模型，惩罚最大似然估计，EM算法。

Result: 回归参数和系数函数线性泛函的估计量一致且渐近正态，协方差矩阵达到半参数效率界。

Conclusion: 构建了全局检验，通过仿真和ADNI数据验证了方法。
...
### [104] [Nonparametric inference for sublevel-set probabilities of conditional average treatment effect functions](https://arxiv.org/abs/2605.15373)
*Anders Munch,Thomas A. Gerds*

Main category: stat.ME

TL;DR: 通过条件平均处理效应（CATE）函数的子水平集概率，构建单变量单调曲线以可视化治疗效果的异质性，并开发相应的估计方法。


<details>
  <summary>Details</summary>
Motivation: 平均处理效应可能掩盖个体反应差异，而CATE函数在高维协变量下难以解释，需要一种简单的汇总指标来刻画异质性。

Method: 将曲线形式化为目标参数，证明其非路径可微性；集成机器学习的Grenander型估计器；以及近似曲线的分段线性函数的去偏机器学习估计器。

Result: 基于随机试验模拟数据评估有限样本性能，并在糖尿病药物试验数据中展示方法应用。

Conclusion: 提出的曲线能有效可视化异质性的类型和程度，所发展的估计器具有良好性能。
...
### [105] [Modeling Misclassification in Spousal Violence Reporting: Evidence from Bayesian Quantile Regression](https://arxiv.org/abs/2605.15428)
*Joon Jin Song,Mohammad Arshad Rahman,Yoo-Mi Chin,James Stamey*

Main category: stat.ME

TL;DR: 提出一种处理误分类二元结果的贝叶斯分位数回归方法，通过潜在变量和MCMC估计，有效纠正低报和高报偏差。


<details>
  <summary>Details</summary>
Motivation: 标准分位数回归忽略二元结果中的误分类（如低报），导致有偏推断，需建模纠正。

Method: 引入潜在真实响应变量，明确建模假阴性和假阳性报告错误，采用新颖MCMC算法进行贝叶斯估计。

Result: 模拟显示新方法在不同误分类率下优于忽略误分类的模型；实际数据分析表明低报多于高报，且调整误分类会改变结论。

Conclusion: 所提方法有效处理误分类问题，对敏感话题的推断至关重要。
...
### [106] [Tree-aggregated regression for compositional data with measurement errors](https://arxiv.org/abs/2605.15469)
*Zhenghan Li,Tianying Wang*

Main category: stat.ME

TL;DR: 提出TARCO方法，同时处理树聚合和测量误差，解决层级污染问题，提高估计精度和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有方法未考虑树聚合与测量误差的相互作用，导致层级污染、偏差增大、变量选择不稳定。

Method: 集成偏差校正估计、树感知正半定稳定化和稀疏正则化，通过交叉验证选择调参，形成凸优化问题。

Result: 在模拟和微生物组应用中，TARCO在估计精度、支持恢复和聚合级可解释性上优于忽略交互的方法。

Conclusion: TARCO有效处理了树聚合与测量误差的交互作用，提供了理论保证和实用优势。
...
### [107] [Improving the Efficiency of Subgroup Analysis in Randomized Controlled Trials with TMLE](https://arxiv.org/abs/2605.15483)
*Sky Qiu,Nerissa Nance,Rachael Phillips,Jens Tarp,Maya Petersen,Mark van der Laan*

Main category: stat.ME

TL;DR: 针对随机对照试验中亚组分析统计效力不足的问题，提出利用同一试验中非亚组参与者信息来增强亚组估计的两种目标最大似然估计方法，并在心血管试验中验证可提高亚组治疗效应估计精度。


<details>
  <summary>Details</summary>
Motivation: 随机对照试验中的亚组分析常因样本量有限而统计效力不足，需要在不引入外部偏倚的前提下提高亚组效应估计的精度。

Method: 研究两种目标最大似然估计量：基于合并回归的TMLE（TMLE-PR）和自适应目标最大似然估计量（A-TMLE），两者通过借用同一试验内非亚组参与者的信息来增强亚组估计，无需依赖外部真实世界数据。

Result: 在LEADER试验中，对黑人和亚洲亚组（各占试验人群不到10%）的MACE风险降低估计表明，A-TMLE在365、540、730天时亚洲亚组的绝对风险降低分别为1.6、1.5、1.5个百分点，黑人亚组分别为2.1、2.0、2.1个百分点，且95%置信区间均排除零。

Conclusion: 所提方法通过利用试验内部信息提高了亚组特异性处理效应估计的精度，避免了外部数据源的偏倚，有助于支持公平的药品标签、可及性和上市后评估，符合FDA等监管机构优先目标。
...
### [108] [Tail postcoloring in long-run variance estimation of time series](https://arxiv.org/abs/2605.15596)
*Xu Liu,Kin Wai Chan*

Main category: stat.ME

TL;DR: 提出尾部后着色方法，通过参数模型将非参数估计器中忽略的尾部自协方差投影到最终估计器，桥接非参数方差估计与参数着色模型。


<details>
  <summary>Details</summary>
Motivation: 标准预白化方法存在方差膨胀、功效降低及对模型误设敏感的问题，尾部后着色旨在避免这些缺陷。

Method: 使用参数模型对尾部自协方差进行后着色，通过带宽参数自动在非参数估计与参数着色之间切换，无需将数据集转换为残差。

Result: 当着色模型正确指定时可达到参数速率；有限样本中对模型误设更稳健；避免方差膨胀或功效降低；支持多重稳健性；适用于多元时间序列。

Conclusion: 该方法在MCMC输出分析等实际应用中表现有效。
...
### [109] [Structured Transfer Learning for Survival Risk Stratification in Data-Sparse Clinical Cohorts](https://arxiv.org/abs/2605.15633)
*Junhan Yu,Yurui Chen,Juan Delgado-SanMartin,Dennis Wang,Hong Pan,Doudou Zhou*

Main category: stat.ME

TL;DR: 提出了CORE-Cox框架，利用跨结局共享风险因子模式并通过正则化残差调整适应小样本队列，显著提升了生存风险分层性能。


<details>
  <summary>Details</summary>
Motivation: 小样本或事件稀疏的临床队列中，传统生存预测模型不稳定，而大队列模型迁移时因风险因子效应差异而适应性差。

Method: 开发两阶段CORE-Cox：先在大源队列中通过低秩Cox系数结构学习共享风险模式，再在小目标队列中通过正则化残差修正实现队列特定适应。

Result: 在UK Biobank（白人源vs亚洲目标）和MIMIC-IV（白人ICU源vs亚洲ICU目标）中，CORE-Cox的C-index分别从0.733提升至0.766和从0.628提升至0.658，在9个结局中的8个达到最佳或接近最佳，并改善了高风险富集。

Conclusion: CORE-Cox提供了可解释的迁移学习框架，结合跨结局共享结构与队列适应，适用于数据稀疏的生存风险分层，但在校准绝对风险预测和临床决策前需进一步验证。
...
### [110] [Leveraging heterogeneity for identifiability: Bayesian order-based learning of multiple DAGs](https://arxiv.org/abs/2605.15639)
*Hyunwoong Chang,Fariha Taskin*

Main category: stat.ME

TL;DR: 提出一个基于顺序的评分框架，用于异构数据下的有向无环图因果结构学习，利用异质性提高因果顺序估计的准确性。


<details>
  <summary>Details</summary>
Motivation: 提高因果顺序估计的准确性，尤其是在异构数据中，因为异质性有助于更精确地识别因果顺序。

Method: 基于顺序的贝叶斯方法，结合随机到随机（R2R）提议邻域的Metropolis-Hastings算法进行后验推断。

Result: 在有利情况下，因果顺序可识别到两个排列；模拟研究证实了方法的强经验性能；应用于主要抑郁障碍的snRNA-seq数据展示了其实用性。

Conclusion: 该方法在高维情况下具有良好的理论性质，且R2R提议邻域能实现高效混合，有效估计因果结构。
...
### [111] [Re-examining and calibrating weighted survival analysis for causal inference](https://arxiv.org/abs/2605.15702)
*Wenfu Xu,Yi Zhang,Tobias Gerhard,Zhiqiang Tan*

Main category: stat.ME

TL;DR: 重新审视加权Kaplan-Meier方法，并开发了校准估计方法用于生存分析中的因果推断。


<details>
  <summary>Details</summary>
Motivation: 现有加权方法在生存分析中的统计性质研究不足，存在局限性。

Method: 通过校准估计在低维和高维设置中发展新方法及相关理论。

Result: 校准方法在模拟中覆盖比例更接近目标值，在模拟和实证研究中产生更短的置信区间。

Conclusion: 校准方法改进了加权方法的性能。
...
### [112] [Generalized raking and stabilized weights for regression modeling in two-phase samples](https://arxiv.org/abs/2605.15802)
*Tong Chen,Joshua Slone,Gustavo Amorim,Pamela A. Shaw,Bryan E. Shepherd,Thomas Lumley*

Main category: stat.ME

TL;DR: 针对复杂调查设计中回归模型的权重波动问题，提出将稳定化权重与广义分层估计相结合，在二阶段抽样中提高效率。


<details>
  <summary>Details</summary>
Motivation: 抽样权重包含非必要变异导致方差膨胀，需稳定化权重以减少变异并提高估计效率。

Method: 评估最优稳定化权重，并将其与广义分层估计结合，利用辅助变量信息，通过标准统计包实现。

Result: 模拟表明，在现实二阶段设计中该组合估计量提高精度，但在信息量大的设计中效率增益有限。

Conclusion: 提出的组合方法能有效提升二阶段抽样中的估计效率，适用于大规模研究，如卡波西肉瘤研究。
...
### [113] [The Negative Binomial Chain-Ladder: A Full Likelihood Model for Claim Count Reserving](https://arxiv.org/abs/2605.15811)
*Robin Van Oirbeek*

Main category: stat.ME

TL;DR: 提出负二项链梯模型，将链梯法嵌入似然框架，通过泊松-伽马构造赋予离散参数结构性解释，统一了链梯家族。


<details>
  <summary>Details</summary>
Motivation: 经典链梯法缺乏概率基础，现有随机扩展依赖二阶矩假设，缺乏生成性解释。

Method: 基于泊松-伽马构造导出增量赔款服从负二项分布，建立负二项链梯模型，并开发参数自举程序。

Result: 模型在极限下推广泊松链梯，与ODP点估计一致但方差不同；模拟显示指定正确时覆盖接近名义水平，实证案例展示了结构性解释。

Conclusion: 负二项链梯模型提供了统一的概率层次，赋予离散参数结构意义，并为不确定性量化提供了完整框架。
...
### [114] [Bayesian Inference for Non-Conjugate Distance Dependent Chinese Restaurant Process Models](https://arxiv.org/abs/2605.15847)
*Joseph Marsh,Theodore Kypraios,Rowland G. Seymour*

Main category: stat.ME

TL;DR: 针对非共轭似然下的距离依赖中餐馆过程（ddCRP），提出了一种通用的可逆跳跃马尔可夫链蒙特卡洛（RJMCMC）框架，采用矩匹配提议策略，有效处理跨维度参数空间问题。


<details>
  <summary>Details</summary>
Motivation: 在非共轭设置下，由于聚类分配变化导致参数空间维度变化，传统贝叶斯推断变得困难。

Method: 开发了RJMCMC框架，包括出生/死亡移动的提议策略（基于先验、独立及数据驱动的矩匹配），以及固定维度移动的后验重采样策略。

Result: 模拟研究和老忠实间歇泉喷发时长数据表明，矩匹配提议相比基于先验的提议更优，提高了接受率并保持计算效率。

Conclusion: 该框架为ddCRP模型在非共轭似然下提供了通用推断方法，适用于离散和连续观测模型。
...
### [115] [A Model-Agnostic Bootstrap for Macro-Level Claims Reserving Under the Conditioning Principle](https://arxiv.org/abs/2605.15896)
*Robin Van Oirbeek,Tim Verdonck*

Main category: stat.ME

TL;DR: 提出一种满足条件预测分布原则的Dirichlet-Gamma bootstrap方法，解决了现有bootstrap方法覆盖误差不消失的问题。


<details>
  <summary>Details</summary>
Motivation: 现有bootstrap方法违反条件原则，导致覆盖误差为O(1)，不随三角矩阵增大而消失，需要精确的方法。

Method: 基于Dirichlet-Gamma层次模型，仅模拟分配比例W_i，保持观测三角固定，且与开发比例方法模型无关。

Result: 覆盖缺陷为O(I^{-1/2})；在复合泊松DGP下保守；ODP bootstrap因两种机制导致覆盖差异Ω(1)，解释跨组合校准异质性。

Conclusion: Chain-Ladder等方法成为不同先验下的可信度估计；c<30指示非平稳发展。
...
### [116] [Statistical Inference for Smoothed Support Vector Machines in High Dimensions: From Offline to Online Data](https://arxiv.org/abs/2605.15911)
*Shuya Zhou,Junwen Xia,Jingxiao Zhang*

Main category: stat.ME

TL;DR: 针对高维分类中Lasso惩罚线性SVM的双重非光滑性问题，提出离线与在线统一推断框架，通过卷积平滑构建去偏估计量，实现有效统计推断并提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 高维分类中Lasso惩罚线性SVM的双重非光滑性（铰链损失和Lasso惩罚）导致统计推断困难且计算效率低。

Method: 离线时对铰链损失应用卷积平滑技术构建去偏估计量；在线时基于历史数据汇总统计量进行实时推断。

Result: 离线与在线去偏估计量渐近正态，模拟和实际数据验证了有效统计推断和计算效率提升。

Conclusion: 提出的统一框架在离线与在线设置下均能实现有效推断，并提高计算效率。
...
### [117] [REX-SUB: A Scalable Subsampling Strategy for Modeling Large Spatial Datasets](https://arxiv.org/abs/2605.16075)
*Nicholas Rios,Ben Seiyon Lee*

Main category: stat.ME

TL;DR: 提出REX-SUB算法用于空间数据子采样，结合Vecchia近似，有效降低预测误差。


<details>
  <summary>Details</summary>
Motivation: 大规模空间数据中标准高斯过程拟合计算成本高，需要高效子采样方法。

Method: 提出随机交换子采样算法（REX-SUB），并嵌入可扩展的Vecchia近似加速似然计算。

Result: 相比其他方法，REX-SUB在模拟和实际水汽数据上获得更低的均方预测误差和区间评分。

Conclusion: REX-SUB是一种有效的空间数据子采样策略，能够提升预测精度和计算效率。
...
### [118] [Estimating Association Between Paired Outcomes in Clustered Data with Informative Subgroup Size](https://arxiv.org/abs/2605.16108)
*Owen Visser,Somnath Datta*

Main category: stat.ME

TL;DR: 针对成对牙齿水平结果中信息性簇大小和子组结构导致的偏差，提出了加权边际关联估计方法，通过平衡簇、边际和配对类别的贡献，减少偏差。


<details>
  <summary>Details</summary>
Motivation: 牙齿数据中簇大小常信息性（如牙齿缺失反映口腔健康），标准边际关联估计因簇贡献不均而有偏。

Method: 扩展簇内重抽样和簇加权估计方程至成对二元结果，构建权重均衡簇、观察边际和配对类别贡献，用加权估计方程估计矩、秩和单元格概率函数，得到聚类数据的Pearson、Spearman和Phi关联度量，使用三明治方差估计和Delta方法标准误。

Result: 模拟显示有限样本偏差、标准误估计和覆盖率表现良好；NHANES数据中诊断出信息性子组大小，提出方法减少由信息性牙齿保留和子组组成引起的偏差。

Conclusion: 提出的加权估计器为估计典型个体典型牙齿的边际口腔健康关联提供了原则性基础，同时降低了信息性牙齿保留和子组组成导致的偏差。
...
### [119] [Why Empirical p-Values Are Not Uniform: Reference Samples, Dependence, and PIT Backtesting](https://arxiv.org/abs/2605.16221)
*Jakub Lis*

Main category: stat.ME

TL;DR: 经验PIT值因有限样本估计而破坏独立性及均匀性，导致传统均匀性检验失效。


<details>
  <summary>Details</summary>
Motivation: 实际中基于有限样本构建的经验PIT值改变了统计结构，需要重新评估其校准性质。

Method: 分析了常见样本和滚动窗口两种经验PIT构造方式，并与精确PIT理论分布对比。

Result: 常见样本导致统计量收敛于两样本K-S分布，滚动窗口引入自相关和方差抑制。

Conclusion: 应基于两阶段抽样结构重新设计PIT校准测试方法。
...
### [120] [FRESH: Information-Geometric Calibration of Patient-Level Models to Aggregate Evidence](https://arxiv.org/abs/2605.16246)
*Franklin Fuller,Daniele Bertolini,Samantha Liang,Jason Christopher,Aaron M. Smith*

Main category: stat.ME

TL;DR: 提出FRESH方法，融合患者级数据与群体级汇总结果，实现数据高效的临床决策模型。


<details>
  <summary>Details</summary>
Motivation: 现有模型通常仅使用患者级数据，难以整合来自临床试验、登记研究等群体级汇总信息，导致数据利用不足。

Method: 基于已训练的患者级数据生成模型，通过信息几何最小扰动的方式重新校准，使模型输出匹配目标群体的指定汇总统计量。

Result: 生成的患者级预测可直接分析或用于更新原模型，在单臂试验情境化、临床试验模拟和上市疗法比较等应用中有效。

Conclusion: FRESH提供了一种原则性方法，将不同层次的数据统一为数据高效的模型，提升了临床决策支持的能力。
...
### [121] [Thermodynamic Invariants of Coupled Channels: A Many-Channel Tolman-Ehrenfest Effect](https://arxiv.org/abs/2605.15653)
*Benjamin Hamblin,Victor Calo,Klaus Regenauer-Lieb*

Main category: math-ph

TL;DR: 通过扩展Tolman-Ehrenfest效应到熵流形，推导出多通道热力学不变量，并应用于颗粒体积-应力系综，解决了状态依赖K_μ的长期难题，并给出可实验检验的预测。
...
Method: 将Tolman-Ehrenfest效应扩展到熵流形，推导出n通道不变量ζ_i T_i = C，其中ζ_i是Ruppeiner联络的和乐群。

Result: 在颗粒体积-应力系综中，Rowe的剪胀比和能量约束作为非对角曲率g_{Vσ}的几何结果出现，解决了60年来状态依赖K_μ的谜题：校正项ζ_V在接近堵塞时达到O(1)。

Conclusion: 预测剪跨带内ζ_V χ = 常数，可通过实验检验。
...
### [122] [Spectral separation of variables from equivalent Lagrangian systems](https://arxiv.org/abs/2605.15679)
*Mattia Scomparin*

Main category: math-ph

TL;DR: 研究二次拉格朗日量的动力学等价性，发现动能矩阵与势能Hessian的对易关系导致运动方程解耦。


<details>
  <summary>Details</summary>
Motivation: 探究二次拉格朗日量在保持运动方程不变下的等价条件及其与变量分离的联系。

Method: 通过比较两个二次拉格朗日量的欧拉-拉格朗日方程，导出动能矩阵与势能Hessian的相容性条件，并利用谱分解实现解耦。

Result: 对于常数对称动能矩阵，条件简化为对易关系，得到配置空间的正交谱分解，运动方程解耦为独立子系统。

Conclusion: 该框架统一了可积系统的变量分离，并应用于Sawada-Kotera系统和n维Hénon-Heiles模型，恢复经典可积参数区域。
...
### [123] [Epstein vector zeta functions related to the ADE Lie algebras](https://arxiv.org/abs/2605.16042)
*M.Olshanetsky*

Main category: math-ph

TL;DR: 针对ADE型李代数根格，引入向量值Epstein zeta函数，通过构造向量值theta级数的Mellin变换得到，并推导出仅在C-不变子空间成立的矩阵函数方程，最终完成了满足该方程的格和子空间的分类。


<details>
  <summary>Details</summary>
Motivation: 将Epstein zeta函数推广到向量值情形，以研究根格与Weil表示之间的深层联系。

Method: 利用判别群构造实现Weil表示的向量值theta级数，再通过Mellin变换定义向量值Epstein zeta函数，并基于theta向量的等变性导出矩阵函数方程。

Result: 得到Riemann型矩阵函数方程，其存在性受制于Mp(2,Z)中心元素C的C-不变向量子空间的选择规则，并完成了所有满足条件的格与不变子空间的分类。

Conclusion: 该工作为根格赋予的向量值zeta函数提供了完整的理论框架，并揭示了其对称性由不变子空间决定。
...
### [124] [A general proof of integer R\'enyi QNEC](https://arxiv.org/abs/2605.15272)
*Tanay Kibe,Pratik Roy*

Main category: hep-th

TL;DR: 证明了在具有半侧模包含结构的von Neumann代数中，对整数Rényi参数n≥2，Rényi量子零能量条件成立，仅需激发态相对于真空的夹层Rényi散度有限。


<details>
  <summary>Details</summary>
Motivation: 量子零能量条件（QNEC）是量子场论中的基础不等式，Rényi QNEC作为其推广，有助于深入理解量子能量条件的结构。

Method: 基于半侧模包含结构，利用Kosaki L^n范数在零平移半群下的对数凸性证明。

Result: 对所有整数n≥2，在σ-有限von Neumann代数中，任何具有有限L^n范数的正常正泛函满足对数凸性，从而证明了Rényi QNEC。

Conclusion: 该工作为Rényi QNEC提供了严格证明，推广了QNEC，并揭示了与模包含结构的深层联系。
...
### [125] [Non-Invertible Symmetries in Compactified Supergravities](https://arxiv.org/abs/2605.16188)
*Fabi\'an Caro-P\'erez,Mar\'ia Pilar Garc\'ia del Moral,\'Alvaro Restuccia*

Main category: hep-th

TL;DR: 研究了非可逆高形式对称性缺陷从十一维超引力到IIA型超引力的Kaluza-Klein约化，得到七维Chern-Simons辅助理论降维为六维BF型，十一维Bianchi部分分裂为可逆H[3]和扭曲非可逆F̃[4]，最终缺陷代数包含可逆Picard子群和非可逆BF修饰缺陷。


<details>
  <summary>Details</summary>
Motivation: 探索非可逆对称性在弦理论紧化中的表现及其在超引力中的实现。

Method: 通过Kaluza-Klein约化沿M理论圆，将十一维非可逆缺陷及其辅助拓扑部分推至零模超引力区域，分析Bianchi部分的分裂。

Result: 七维Chern-Simons型辅助理论降维为六维BF型；十一维Bianchi部分分裂为可逆H[3]部分和满足dF̃[4] + H[3]∧F[2]=0的扭曲非可逆F̃[4]部分；得到的IIA型缺陷代数包含可逆Picard子群和非可逆BF修饰缺陷。

Conclusion: 建立了IIA型超引力中的非可逆缺陷结构，并通过M理论/IIA型膜字典识别了带电探针。
...
### [126] [TFZ-Tree: An Ultra-Lightweight Waveform Classification Framework for Resource-Constrained Devices](https://arxiv.org/abs/2605.15656)
*Hao Wang,Kuang Zhang,Yonggang Chi,Tianqi Zhao,Yanbo Fu,Jiaxing Guo*

Main category: eess.SP

TL;DR: 提出基于时频多维特征和合作Z检验树的超轻量波形分类框架，实现在资源受限设备上对十种6G IoT候选波形的实时高精度识别。


<details>
  <summary>Details</summary>
Motivation: 现有物理层波形识别研究极少且依赖深度网络和复杂时频变换，难以部署于资源受限终端；符号调制分类方法无法绕过'先识别波形'的前提。

Method: 采用低复杂度时域特征提取，构建由Z统计检验优化的合作Z检验树（ZTree），通过假设检验置信度自动控制决策树分裂与规模。

Result: 在AWGN信道下平均准确率99.5%，TDL-C多径信道下87.4%，主要混淆发生在OTFS和LoRa之间；C语言实现在x86平台单次推理延迟低于4ms。

Conclusion: 首次实现十种IoT波形类型的实时识别，未来工作将针对嵌入式MCU进行部署加速。
...
### [127] [Operator-Controlled 6G: From Connectivity Infrastructure to Guaranteed Digital Services](https://arxiv.org/abs/2605.15553)
*David Soldani*

Main category: cs.NI

TL;DR: 本文主张6G应逆转供应商主导的架构，提出以运营商控制为先的优先级重组，并基于Rakuten Mobile的部署经验，通过6G控制契约和保障经济模型实现运营商主导的6G愿景。


<details>
  <summary>Details</summary>
Motivation: 前五代网络由供应商主导，导致运营商无法自主拥有、修改或审计网络。6G必须将运营商的控制、客户、业务和运营置于技术之上。

Method: 提出6G控制契约（三层所有权分类：自有、联合、消费）和保障经济（五级结果定价商业模型），将运营商控制转化为可执行的服务水平目标。

Result: Rakuten Mobile作为全球首个全国性全云原生开放RAN部署，已在2025财年实现全年EBITDA盈利，验证了运营商控制网络的可行性。

Conclusion: 运营商控制的6G是可行的。2026-2028年的决策将决定6G成为保障数字服务平台还是另一个依赖供应商的基础设施周期。
...
### [128] [Fault tolerance estimation in digital circuits with visualised generative networks](https://arxiv.org/abs/2605.15212)
*Sascha Biel,Carl Alexander Gaede,Amiel Glaser,Jan Wolter,Alexej Schelle*

Main category: cs.AR

TL;DR: 提出一种基于生成网络采样的数值方法，用于估计数字电路结构中故障模式的容错性。


<details>
  <summary>Details</summary>
Motivation: 评估数字电路设计中故障模式的容错性，以提高电路鲁棒性。

Method: 通过生成对抗网络（GAN）的判别器，比较理想数字信号与实际信号偏差，包括缺失或互换逻辑器件等错误模式。

Result: 能够基于复杂变量表示分析GAN，区分不同经典逻辑元件的故障模式影响，评估电子设计鲁棒性。

Conclusion: 该方法有效评估数字电路故障容错，有助于提高设计可靠性。
...
### [129] [GenAI-Driven Approach to RISC-V Supply Chain Exploration](https://arxiv.org/abs/2605.15223)
*Nenad Petrovic,Andre Schamschurko,Yingjie Xu,Alois Knoll*

Main category: cs.AR

TL;DR: 提出基于LLM的RISC-V供应链分析工作流，结合视觉语言模型和模型驱动工程，实现多模态数据驱动的供应链洞察。


<details>
  <summary>Details</summary>
Motivation: 半导体供应链数据异构且非结构化，现有方法难以综合分析文本与视觉信息，需要一种统一框架提升供应链透明度与韧性。

Method: 利用LLM处理文本，VLM提取图表等视觉信息，构建知识图谱；采用MDE和约束建模进行依赖验证、瓶颈检测与风险评估；加入人机交互机制以支持专家验证。

Result: 在RISC-V场景中验证，能生成可操作洞察、增强透明度、支持复杂供应链的决策。

Conclusion: LLM与VLM的语义理解结合MDE的形式分析，可有效评估供应链韧性，兼具探索性与系统性评估能力。
...
### [130] [Is Agentic AI Ready for Real-World Hardware Engineering? A Deep Dive with Phoenix-bench](https://arxiv.org/abs/2605.15226)
*Qingyun Zou,Feng Yu,Hongshi Tan,Bingsheng He,WengFai Wong*

Main category: cs.AR

TL;DR: 提出Phoenix-bench基准，用于评估AI代理在硬件工程中的表现，发现软件与硬件任务本质不同，且测试反馈比定位粒度更重要。


<details>
  <summary>Details</summary>
Motivation: 现有硬件LLM基准只关注子任务，缺少同时要求仓库导航、层次感知定位、EDA可执行验证和维护补丁的综合评估。

Method: 构建包含511个经过验证的Verilator实例的Phoenix-bench，来自114个GitHub仓库，附有开发者补丁、设计流标签、测试平台和Docker化EDA环境，并统一评估多种商业和开源代理及LLM骨干网络。

Result: 三个发现：(i) 从SWE-bench到Phoenix-bench，代理性能下降37%-58%，因为硬件错误通过信号流跨模块传播；(ii) 失败集中在控制流/FSM错误、验证测试平台错误和需要跨层次信号追踪的复杂情况；(iii) 完美文件级定位仅提升1.4%，而一轮测试反馈提升42%-45%。

Conclusion: 硬件工程对AI代理提出独特挑战，强调跨层次推理和测试反馈的重要性。
...
### [131] [A3D: Agentic AI flow for autonomous Accelerator Design](https://arxiv.org/abs/2605.15237)
*Abinand Nallathambi,Christopher Knight,Shantanu Ganguly,Wilfried Haensch,Anand Raghunathan*

Main category: cs.AR

TL;DR: A3D是一个端到端自动化硬件加速器设计的Agentic AI流程，能自动完成工作负载分析、性能瓶颈识别、代码重构和微架构生成，无需人工干预。


<details>
  <summary>Details</summary>
Motivation: 硬件加速器设计劳动密集、需要多领域专业知识，现有方法多限于规则数据流应用，自动化需求迫切。

Method: 通过专业代理分工、流程循环、预定义和自定义工具、基于Agentic RAG的代码库与EDA文档探索，结合商用LLM和HLS工具实现。

Result: 在LAMMPS和QMCPACK等复杂科学应用中，A3D自动生成加速器设计，无需人工干预。

Conclusion: A3D有效利用现代LLM自动化硬件加速器设计，克服了应用中的关键挑战。
...
### [132] [Hardware-Software Co-Design of Scalable, Energy-Efficient Analog Recurrent Computations](https://arxiv.org/abs/2605.15216)
*Arthur Fyon,Julien Brandoit,Loris Mendolia,Damien Ernst,Jean-Michel Redout\'e,Guillaume Drion*

Main category: cs.AR

TL;DR: 通过硬件-软件协同设计，利用双稳态记忆递归单元（BMRU）在模拟电路中实现超低功耗循环神经网络，克服噪声累积，实现亚微瓦推理。


<details>
  <summary>Details</summary>
Motivation: 现有模拟实现仅限于前馈架构，循环动态因噪声积累被认为不可行；目标是实现超低功耗的模拟循环神经网络。

Method: 采用BMRU（离散输出与迟滞动态），设计电流模式模拟电路，建立参数与电路元件的一一对应；重新公式化BMRU以支持第一象限操作和固定阈值，保持表达能力与可训练性。

Result: 180nm CMOS晶体管级仿真显示软件预测与电路行为高度一致；离散输出在每个单元边界抑制模拟噪声至少20倍；递归功耗随状态维度线性增长，前馈层功耗平方增长；端到端关键词识别中RNN核心实现亚微瓦推理。

Conclusion: 通过硬件-软件协同设计，模拟电路中可实现循环动态，且递归以线性边际成本添加到前馈骨干中，证明其可行性。
...
### [133] [SwAIther-Precip: Lead-Time-Aware Bias Correction Enables Kilometer-Scale Downscaling of Global AI Precipitation Forecasts over Switzerland](https://arxiv.org/abs/2605.16163)
...
TL;DR: 提出SwAIther-Precip框架，通过先验时间依赖偏差校正和扩散模型超分辨率，将全球AI天气模型（AIFS）的0.25°降水预报降尺度到1公里概率场，显著提升复杂地形下中期降水预报的精度与分辨率。


<details>
  <summary>Details</summary>
Motivation: 全球AI天气模型虽然能提供有技巧的中期预报，但0.25°分辨率不足以直接用于局部灾害应用；统计降尺度可弥补差距，但现有方法难以处理全球预报中依赖于状态及提前时间的偏差。

Method: 采用两阶段降尺度：首先用U-Net基于特征线性调制（FiLM）按提前时间校正粗分辨率系统性偏差；然后利用扩散模型生成细尺度空间变异性，独立于提前时间。

Result: 相对于原始AIFS，CRPS降低48%；生成场在大尺度和小尺度上的谱保真度分别高于0.85和0.88，在1公里网格上有效分辨率约4公里（提前时间≤5天）；跨提前时间训练使6天预报CRPS再降低13%。

Conclusion: 在生成式超分辨率之前显式校正依赖于提前时间的偏差，是高效实现全球AI降水预报公里级概率降尺度的关键。
...
TL;DR: 提出DeepSlide，一个结合人机交互的多智能体系统，全面优化演示的叙事、节奏和排练流程，在交付质量上显著超越现有工具。


<details>
  <summary>Details</summary>
Motivation: 现有AI幻灯片生成器只追求视觉美观，忽视了演示的交付过程（如节奏、叙事和排练），导致实际展示效果不佳。

Method: 集成可控逻辑链规划器（含时间预算）、轻量内容树检索器、马尔可夫式顺序渲染和沙盒执行，并引入双记分牌基准分离静态与动态质量。

Result: 在20个领域和多种受众中，DeepSlide在工件质量上与强基线持平，但在叙事流、节奏精度、幻灯片-脚本协同和注意力引导等交付指标上持续取得更大提升。

Conclusion: DeepSlide能有效改善演示的完整准备过程，兼顾静态内容和动态交付，尤其提升实际展示效果。
...
### [135] [SDOF: Taming the Alignment Tax in Multi-Agent Orchestration with State-Constrained Dispatch](https://arxiv.org/abs/2605.15204)
*Zhantao Wang*

Main category: cs.AI

TL;DR: 提出SDOF框架，将多智能体执行视为约束状态机，通过在线RLHF意图路由器和状态感知调度器实现审计执行，在招聘系统上任务完成率达86.5%，并有效阻止所有注入攻击。


<details>
  <summary>Details</summary>
Motivation: 现有框架如LangChain等缺乏对真实业务流程中阶段约束的强制执行。

Method: SDOF通过两个防御层实现：基于生成奖励建模（GRPO）训练的在线RLHF专用意图路由器，以及带有GoalStage有限自动机检查和SkillRegistry前提/后置条件验证的状态感知调度器。

Result: 在支持6000+企业的招聘系统上，185个场景触发1671次API调用；意图路由器在FSM约束对抗基准上联合准确率80.9%（GPT-4o为48.9%）；端到端任务完成率86.5%（95%置信区间80.8-90.7），阻止所有22种注入操作；消息级拦截精确率100%、召回率88%、专家一致性kappa=0.94。
...
### [136] [Does Theory of Mind Improvement Really Benefit Human-AI Interactions? Empirical Findings from Interactive Evaluations](https://arxiv.org/abs/2605.15205)
*Nanxu Gong,Zixin Chen,Haotian Li,Zishu Zhao,Jianxun Lian,Huamin Qu,Yanjie Fu,Xing Xie*

Main category: cs.AI

TL;DR: 本文提出交互式ToM评估新范式，发现静态基准提升不总是转化为动态交互中的更好表现，强调了基于交互的评估必要性。


<details>
  <summary>Details</summary>
Motivation: 现有ToM基准忽视人机交互的第一人称、动态和开放性质，无法直接评估改进技术在交互中的效果。

Method: 提出交互式ToM评估范式（视角和指标转变），系统研究四种代表性ToM增强技术，使用四个真实世界数据集和用户研究，涵盖目标导向和经验导向任务。

Result: 静态基准的改进不总是带来动态人机交互中更好的表现。

Conclusion: 需要基于交互的评估来发展社会感知的LLM，为未来的人机共生提供洞见。
...
TL;DR: 提出SkillSmith框架，通过边界优先编译器-运行时方法减少LLM代理系统中技能执行时的冗余，显著提升效率并降低成本。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理系统中，技能注入导致无关上下文注入和重复推理规划，产生冗余。

Method: 设计边界优先的编译器-运行时框架，离线编译技能包为最小可执行接口，运行时仅动态访问相关组件。

Result: 在SkillsBench基准上，相比原始技能，减少57.44%的令牌使用、42.99%的思考迭代、50.57%的求解时间，成本降低57.44%。编译产物可被更小模型重用提升准确率。

Conclusion: SkillSmith有效消除冗余，提升效率，且支持模型能力复用。
...
TL;DR: 指令微调语言模型在输出层面表现公平，但内部表示仍保留并放大种族偏见；通过激活引导可逆转决策，且偏见不对称、易受攻击，表明仅输出审计不足。


<details>
  <summary>Details</summary>
Motivation: 探究模型在公平输出背后是否隐藏可被利用的决策相关偏见，以及这种偏见是否在不同人口群体间对称。

Method: 使用匹配的抵押贷款申请（仅名字暗示种族），分析模型层间表示；通过激活引导和跨层干预重新注入抑制信息，测试决策反转及不对称性。

Result: 模型无输出偏见，但内部表示保留并放大人口统计特征；激活引导可在关键层几乎完全逆转决策，且逆转变现出不对称性（单向有效），偏见易受对抗提示和参数高效微调利用。

Conclusion: 行为审计（仅关注输出）不足以检测内部偏见；建议采用结合输出评估与表示分析的“双层测试框架”用于高风险决策中的AI治理。
...
TL;DR: 提出CAX-Agent代理框架，通过分层恢复策略提升MAPDL仿真可靠性，其中model_only策略表现最佳。


<details>
  <summary>Details</summary>
Motivation: 大模型在MAPDL仿真中因缺乏结构化控制和故障恢复导致输出不一致、任务失败频发。

Method: 构建CAX-Agent的三层架构（LLM服务、代理框架、求解器后端），并设计恢复阶梯；在50个基准上评估no_recovery、rule_only、model_only三种策略。

Result: model_only策略完成率0.9267、任务评分3.59/4、总分9.16/10、零干预率0.84，显著优于其他策略（Cliff's delta 0.81-0.87）。

Conclusion: model_only恢复策略能有效提高任务完成质量，但当前基准几何简单，需更广泛验证。
...
TL;DR: 提出NOVA框架，分析AI自改进循环中知识发现的充分条件和失败模式，证明污染陷阱和缩放定律。


<details>
  <summary>Details</summary>
Motivation: 探索AI系统能否通过迭代自改进发现真正的新知识，并量化其成本。

Method: 将“生成、验证、积累、重训”循环建模为知识空间上的自适应采样过程，分析条件与失败模式，并推导缩放定律。

Result: 识别出污染陷阱：当易发现知识耗尽时，新有效知识的模型质量下降，小假阳性率会导致无效知识积累；证明累积生成成本与发现数量满足幂律关系R_cum(D)=Θ(c_gen D^α)。

Conclusion: 知识发现存在渐近递减回报，专家输入在自主探索障碍附近最有价值。
...
TL;DR: 提出ICRL框架，通过强化学习联合训练求解器和评论者，将评论引导的成功转化为无辅助的求解能力，解决模型依赖外部评论且评论无法自我改进的问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型代理在无评论时无法保持改进后的行为，且冻结的评论者无法提升反馈质量，限制了自我改进潜力。

Method: 通过分布校准重加权比和角色组优势估计，联合训练共享骨干网的求解器和评论者，用强化学习鼓励评论者提供可操作的反馈，并选择性转移与求解器提示分布兼容的改进。

Result: 在Qwen3-4B和Qwen3-8B上，代理任务平均提升6.4分，数学推理提升7.0分；学习到的8B评论者与32B评论者相当，且使用更少令牌。

Conclusion: ICRL有效实现无需外部评论的自我改进，提升模型能力，且评论者效率更高。
...
### [142] [NIMO Controller: a self-driving laboratory orchestrator based on the Model Context Protocol](https://arxiv.org/abs/2605.15227)
*Naruki Yoshikawa,Ryo Tamura*

Main category: cs.AI

TL;DR: 提出基于模型上下文协议(MCP)的自驱动实验室(SDL)软件架构，通过NIMO控制器实现人类与AI代理的统一接口。
...
Method: 基于MCP设计SDL软件架构，所有功能通过MCP服务器暴露，并开发NIMO控制器提供可视化编程界面和AI代理访问。

Result: 通过颜色匹配SDL案例验证了系统的可用性。

Conclusion: MCP-based SDL架构为人类和AI代理提供了统一接口，提升了SDL的可用性和可访问性。
...
### [143] [Verifiable Agentic Infrastructure: Proof-Derived Authorization for Sovereign AI Systems](https://arxiv.org/abs/2605.15228)
*Jun He,Deying Yu*

Main category: cs.AI

TL;DR: 针对自主AI代理在云和企业系统中带来的语义不安全操作风险，提出分布式信任框架（DTF），通过基于证明的授权替代静态身份授权，实现可验证、可审计的执行控制。


<details>
  <summary>Details</summary>
Motivation: 自主AI代理能生成语法正确但语义危险的动作，使静态特权授权成为重大运营风险，尤其在主权AI系统中。

Method: DTF引入Justification Proof（动作可接纳性编码）、共识模型（独立评估）、临时执行身份（从批准证明派生）和仅追加证据链（保存授权生命周期）。

Result: 在既定假设下，该架构强制执行紧凑授权不变性：无证明对象不执行高风险操作，无共识不派生执行身份，无有效变异脱离证据。

Conclusion: DTF将授权从静态身份转向证明派生权威，为主权AI部署中的代理执行提供可治理、可审计、有界的基础设施基础。
...
### [144] [Solvita: Enhancing Large Language Models for Competitive Programming via Agentic Evolution](https://arxiv.org/abs/2605.15301)
...
TL;DR: 提出Solvita框架，通过四个智能体与可训练知识网络实现持续学习，无需更新LLM权重，在多项编程基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于多智能体的代码生成框架是无状态的，缺乏对以往解题经验的积累，导致LLM在复杂编程中可靠性不足。

Method: 将解题过程组织为策略选择、程序合成、认证监督和针对性攻击的闭环系统，四个智能体各自配备可训练图结构知识网络，利用通过/失败判定等信号作为强化学习更新，动态路由查询。

Result: 在CodeContests、APPS、AetherCode和实时Codeforces比赛中，Solvita优于现有方法，准确率几乎翻倍。

Conclusion: 实现代码生成智能体领域新突破，证明了无需更新模型参数即可持续累积推理经验的有效性。
...
### [145] [SMCEvolve: Principled Scientific Discovery via Sequential Monte Carlo Evolution](https://arxiv.org/abs/2605.15308)
*Jiachen Jiang,Huminhao Zhu,Zhihui Zhu*

Main category: cs.AI

TL;DR: SMCEvolve将程序搜索视为从奖励倾斜分布采样，用序贯蒙特卡洛方法实现，在多个基准上超越现有系统且效率更高。


<details>
  <summary>Details</summary>
Motivation: 现有LLM驱动的程序进化框架缺乏原则性设计指南，且无法保证搜索收敛。

Method: 将程序搜索重定义为从奖励倾斜的目标分布采样，并用序贯蒙特卡洛（SMC）采样器近似，包含自适应父重采样、混合突变与接受、自动收敛控制三个核心机制。

Result: 在数学、算法效率、符号回归及端到端ML研究基准上，SMCEvolve用更少的LLM调用超越了当前最优进化系统。

Conclusion: 提出了一种有理论保证的LLM驱动程序进化方法，通过SMC视角实现了高效且收敛的搜索。
...
### [146] [Context Pruning for Coding Agents via Multi-Rubric Latent Reasoning](https://arxiv.org/abs/2605.15315)
*Jingjing Wang,Xiwen Chen,Wenhui Zhu,Huayu Li,Zhengxiao He,Feiyang Cai,Ana S. Carreon-Rascon,Xuanzhao Dong,Feng Luo*

Main category: cs.AI

TL;DR: LaMR通过将代码相关性分解为语义证据和依赖支持两个维度，并使用专门的CRF和门控网络进行结构化剪枝，在节省令牌的同时提升或保持代码生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有单目标序列标注器将代码相关性合并为一个分数，无法处理多样化的保留模式，造成建模瓶颈。

Method: 提出LaMR框架，将代码相关性分解为两个质量维度（语义证据和依赖支持），每个维度由专用CRF建模；使用混合专家门控网络动态加权各维度；最终CRF层输出保留或剪枝决策。标签来自AST分析，无需额外标注。

Result: 在4个基准上，LaMR赢得多轮比较12/16，节省最多31%令牌，单轮任务精确匹配提升最多+3.5，性能常因去噪而提升。

Conclusion: LaMR通过多维度建模和去噪有效过滤无关代码，常匹配或超越全上下文基线，且令牌效率更高。
...
TL;DR: 首次系统零样本评估前沿大语言模型在经典PDDL规划领域中的目标识别能力，发现模型表现不均衡：部分随证据积累接近地标精度，部分固守世界知识先验。差异源于证据整合能力而非领域熟悉度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在规划任务中依赖世界知识而非符号推理，目标识别作为溯因任务更契合其优势，但缺乏系统性评估。

Method: 对前沿大语言模型在经典PDDL基准上进行零样本目标识别评估，并对推理轨迹进行定性分析。

Result: 模型表现参差不齐：部分模型随观测证据增加精度提升至接近地标方法，另一些模型无论证据多少都固守先验知识。定性分析显示差异源于证据整合机制不同。

Conclusion: 目标识别可作为评估大语言模型规划基础知识的原则性基准。
...
### [148] [Belief Engine: Configurable and Inspectable Stance Dynamics in Multi-Agent LLM Deliberation](https://arxiv.org/abs/2605.15343)
*Joshua C. Yang,Maurice Flechtner,Damian Dailisan,Michiel A. Bakker*

Main category: cs.AI

TL;DR: 提出了信念引擎（BE），一种可审计的信念更新层，将信念视为关于命题的证据状态，并将其暴露为标量立场。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的智能体在模拟协商等互动时，其立场变化的缘由不透明，缺乏可解释性。

Method: 将论点提取到结构化记忆中，通过由证据采纳率u和先验锚定a控制的log-odds规则更新立场。

Result: 参数扫描表明这些控制可靠地塑造了立场动态，同时保留了证据级更新轨迹；在DEBATE数据集上，BE最好地重建了最终立场符合提取证据的参与者。

Conclusion: BE为研究基于证据的审议提供了可配置的基础设施，将开放性、承诺、收敛和分歧与显式更新假设联系起来。
...
### [149] [Ensemble Monitoring for AI Control: Diverse Signals Outweigh More Compute](https://arxiv.org/abs/2605.15377)
*Eugene Koran,Yejun Yun,Samantha Tetef,Benjamin Arnav,Pablo Bernabeu-P\'erez*

Main category: cs.AI

TL;DR: 通过集成多样化的监控器，显著提升了对AI代理未对齐动作的检测性能，其中微调监控器在集成中表现关键。


<details>
  <summary>Details</summary>
Motivation: AI系统在自主代理场景中规模部署，确保动作安全对齐至关重要，但单个监控器可靠性不足且人类监督不可行。

Method: 构建12个GPT-4.1-Mini监控器（提示+微调），在编码任务中评估，将多样监控器信号集成为集成。

Result: 多样集成优于单个和同质集成，最佳3监控器集成检测性能提升2.4倍，并在独立数据集上表现强劲。

Conclusion: 多样性而非规模驱动增益，微调监控器在顶级集成中普遍存在且对分布外攻击有效，支持集成监控作为实用AI控制策略。
...
### [150] [Beyond Partner Diversity: An Influence-Based Team Steering Framework for Zero-Shot Human-Machine Teaming](https://arxiv.org/abs/2605.15400)
*Wei Sheng,Rohan Paleja*

Main category: cs.AI

TL;DR: 提出基于影响的团队引导框架（IBTS），通过影响塑造鼓励智能体发现多样化高性能团队交互模式，并在Overcooked-AI上验证了其在两人和三人设置中的有效性，优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的人机团队方法依赖昂贵的人类交互数据，零样本协调（ZSC）通过模拟多样伙伴群体解决瓶颈，但仅靠伙伴覆盖不足，需结合协调机制。

Method: 提出IBTS框架，使用影响塑造激励智能体发现多样化的高绩效团队交互模式，并引导轨迹向更强协调模式收敛。

Result: 在两人和三人Overcooked-AI任务中，包括模拟伙伴、合成伙伴变体以及首次30人真实人类团队研究，IBTS均提升了团队性能，优于竞争基线。
...
### [151] [From LLM-Generated Conjectures to Lean Formalizations: Automated Polynomial Inequality Proving via Sum-of-Squares Certificates](https://arxiv.org/abs/2605.15445)
*Ruobing Zuo,Hanrui Zhao,Gaolei He,Zhengfeng Yang,Jianlin Wang*

Main category: cs.AI

TL;DR: 提出NSPI神经符号框架，结合LLM和符号计算自动证明多项式不等式，通过LLM提出近似SOS分解猜想，符号精化得到精确SOS证明，并在Lean中验证，实验显示在多达10个变量上有效。


<details>
  <summary>Details</summary>
Motivation: 纯符号方法扩展性差，LLM引导方法在变量多时受限，需结合两者优势解决多项式不等式证明的扩展性挑战。

Method: LLM提出近似多项式SOS分解猜想，通过符号计算精化为精确SOS表示，证明不等式并在Lean中验证。

Result: 在多达10个变量的多项式不等式基准上有效且可扩展。

Conclusion: NSPI框架有效结合LLM和符号计算，实现从启发式发现到机器验证的端到端证明。
...
### [152] [X-SYNTH: Beyond Retrieval -- Enterprise Context Synthesis from Observed Human Attention](https://arxiv.org/abs/2605.15505)
*Guruprasad Raghavan,George Nychis,Rohan Narayana Murthy*

Main category: cs.AI

TL;DR: 基于人类注意力痕迹的X-SYNTH框架，通过行为模式合成企业上下文，大幅提升复杂代理任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有检索方法依赖内容匹配，无法捕捉组织特有的行为模式，导致复杂任务中真阳性率低、假阳性率高。

Method: 为每个个体构建数字孪生签名（DTS），并动态选择七种注意力过滤器之一，经四阶段流水线输出基于行为模式的排序上下文。

Result: 在销售线索识别任务中，真阳性率从9.5%升至61.9%（提升6.5倍），假阳性率从90.5%降至18.8%。
...
### [153] [CAPS: Cascaded Adaptive Pairwise Selection for Efficient Parallel Reasoning](https://arxiv.org/abs/2605.15513)
*Fangzhou Lin,Shuo Xing,Peiran Li,Siyuan Yang,Qianwen Ge,Kazunori Yamada,Ziming Zhang,Haichong Zhang,Zhengzhong Tu*

Main category: cs.AI

TL;DR: 提出CAPS框架，通过级联自适应分配验证器计算，在成对自验证中大幅降低令牌成本，同时提升或维持性能。


<details>
  <summary>Details</summary>
Motivation: 现有成对自验证方法成本高昂，每次判断需读取两个完整候选，且无视比较信息量均匀分配计算。

Method: 沿证据轴和分布轴非均匀分配验证计算，实例化为四阶段级联加可选救援子程序，闭式令牌成本使每个候选边际成本减半。

Result: 在20个套件中14个优于领先成对验证器，代码任务用其25.4%的令牌预算，全部套件优于逐点自验证。
...
### [154] [RTL-BenchMT: Dynamic Maintenance of RTL Generation Benchmark Through Agent-Assisted Analysis and Revision](https://arxiv.org/abs/2605.15537)
*Jing Wang,Shang Liu,Hangan Zhou,Zhiyao Xie*

Main category: cs.AI

TL;DR: 提出RTL-BenchMT，一个用于动态维护RTL生成基准的自动化代理框架，以解决当前基准中的缺陷和过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 当前RTL基准存在有缺陷的案例和过拟合两大难题，纯人工维护成本高且难以解决。

Method: 采用自动化代理框架，自动识别并修正有缺陷的基准案例，以及自动检测并更新过拟合案例。

Result: 生成了经过深入分析缺陷和过拟合案例后的改进基准套件。

Conclusion: RTL-BenchMT系统性地减少了人工维护成本，并将社区开源。
...
### [155] [DRS-GUI: Dynamic Region Search for Training-Free GUI Grounding](https://arxiv.org/abs/2605.15542)
*Yichao Liu,Huawen Shen,Liu Yu,Shiyu Liu,Zeyu Chen,Yu Zhou*

Main category: cs.AI

TL;DR: 提出DRS-GUI，一种无需训练的动态区域搜索框架，通过模拟人类感知行为（聚焦、移动、分散）和MCTS计划器，提升MLLM在GUI界面中的指令相关元素定位能力，在ScreenSpot-Pro上取得14%提升。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在从杂乱屏幕截图中准确定位与指令相关的UI元素时仍面临挑战。

Method: 提出DRS-GUI框架，包含轻量级UI感知器（执行聚焦、移动、分散三种类人感知动作）和基于蒙特卡洛树搜索的动作规划器，利用区域质量奖励评估并筛选高指令相关区域。

Result: 在ScreenSpot-Pro上，通用和GUI专用MLLM（Qwen2.5-VL-7B和UGround-V1-7B）的定位性能提升14%。

Conclusion: DRS-GUI显著增强了MLLM在GUI任务中的定位能力和泛化性。
...
### [156] [Position: Artificial Intelligence Needs Meta Intelligence -- the Case for Metacognitive AI](https://arxiv.org/abs/2605.15567)
*Sergei Chuprov,Richard D. Lange,Leon Reznik,Paulo Shakarian,Raman Zatsarenko,Dmitrii Korobeinikov*

Main category: cs.AI

TL;DR: 主张将元认知作为AI设计原则，通过系统自监控和资源动态分配提升准确性、安全性和效率，并在联邦学习中验证，提供软件框架。


<details>
  <summary>Details</summary>
Motivation: 创建更准确、安全、高效的AI。

Method: 系统监控自身状态，根据问题难度或错误成本动态分配资源，融合资源理性AI和心理学元认知策略。

Result: 在联邦学习案例分析中展示了学习效率、效果和安全性提升。

Conclusion: 元认知原则可转化为实践，通过所开发的软件框架支持社区设计、部署和实验元认知AI应用。
...
### [157] [STAR: A Stage-attributed Triage and Repair framework for RCA Agents in Microservices](https://arxiv.org/abs/2605.15581)
*Junle Wang,Xingchuang Liao,Wenjun Wu*

Main category: cs.AI

TL;DR: 提出STAR框架，通过将RCA工作流分解为四个阶段并实现阶段级审计与修复，显著提升LLM-based根因分析代理的可靠性和自修复能力。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的根因分析代理容易因早期证据收集、假设形成或因果分析的错误而最终导致诊断失败，需要一种能够定位并修复推理链错误的机制。

Method: 将RCA工作流显式分解为证据包、假设集、分析结构和决策报告四个阶段；基于LangGraph构建，执行阶段级审计、预算感知的快/慢路由、通过反事实候选评估定位关键故障阶段，并采用阶段特定的补丁-重放修复策略。

Result: 在公开大规模基准和实际生产数据集上，使用两种RCA代理工作流和三种基础模型，一致提高了根因定位和故障类型分类的准确性；能高精度识别关键故障阶段，大多数错误轨迹在一到两次重放内修复，快/慢路由和反事实阶段评估均带来显著收益。

Conclusion: 显式建模RCA代理的故障阶段是实现可靠、可调试且可自修复的代理式根因分析系统的有效途径。
...
### [158] [See Before You Code: Learning Visual Priors for Spatially Aware Educational Animation Generation](https://arxiv.org/abs/2605.15585)
*Yuejia Li,Ke He,Junheng Li,Shutong Chen,Jingkang Xia,Zhiyue Su,Junchi Zhang,Mang Ye*

Main category: cs.AI

TL;DR: 提出OmniManim框架，利用场景状态、视觉规划、渲染后诊断和局部修复生成可执行代码，改善动画渲染质量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型生成的动画代码存在视觉缺陷（重叠、错位、动画中断），且仅在渲染后才能检测。

Method: 形式化为渲染反馈感知的约束代码生成；OmniManim包含共享场景状态、显式视觉规划、结构化渲染后诊断和局部修复；视觉代理通过粗到细边界框去噪和插值感知目标优化关键帧布局。

Result: 在EduRequire-500上，OmniManim在渲染质量上优于单模型基线和多智能体框架；消融实验验证了显式视觉规划（粗空间先验、边界框细化、插值感知优化）的核心作用。

Conclusion: OmniManim有效解决动画代码渲染缺陷问题，显式视觉规划是关键。
...
### [159] [TopoEvo: A Topology-Aware Self-Evolving Multi-Agent Framework for Root Cause Analysis in Microservices](https://arxiv.org/abs/2605.15611)
*Junle Wang,Xingchuang Liao,Wenjun Wu*

Main category: cs.AI

TL;DR: 一个基于拓扑感知的自进化多智能体框架，通过图表示学习和结构化推理解决微服务根因分析中的症状放大偏差和拓扑漂移问题。


<details>
  <summary>Details</summary>
Motivation: 微服务根因分析面临多模态数据噪声、级联故障传播和拓扑漂移的挑战，现有LLM代理存在拓扑无关和症状放大偏差。

Method: 提出MOMA对齐多模态数据，VQ离散化为症状令牌，HET多智能体工作流验证传播一致性解释，以及自进化机制适应拓扑漂移。

Result: 有效减轻症状放大偏差，提升根因定位准确性，并在拓扑漂移下保持稳健。

Conclusion: TopoEvo通过拓扑约束推理和自进化记忆，实现了准确且可解释的根因分析。
...
### [160] [ColPackAgent: Agent-Skill-Guided Hard-Particle Monte Carlo Workflows for Colloidal Packing](https://arxiv.org/abs/2605.15625)
*Lijie Ding,Changwoo Do*

Main category: cs.AI

TL;DR: 介绍了ColPackAgent，一个通过MCP工具服务器和智能体技能自主运行胶体堆积蒙特卡洛模拟的框架，将领域Python包与MCP工具结合，实现可靠的工作流执行。


<details>
  <summary>Details</summary>
Motivation: 通用大型语言模型在没有专用模拟工具和工作流指令时，往往只能描述而非可靠地执行胶体堆积模拟工作流。

Method: 构建MCP服务器封装colpack Python包（基于HOOMD-blue硬粒子蒙特卡洛），并定义四阶段工作流契约的智能体技能，支持交互式、自主端到端或按程序文件自动研究模式。

Result: 在立方体3D、圆盘与胶囊二元2D系统、2D硬圆盘冻结转变等示例中成功演示，并通过17个阶段提示对多个LLM进行基准测试，评估了不同模型在各阶段的工作流遵循可靠性。

Conclusion: 将领域Python包与MCP工具和便携式智能体技能相结合，是将模拟工具包转变为智能体辅助研究工作流的实用途径。
...
### [161] [PRISM: Prompt Reliability via Iterative Simulation and Monitoring for Enterprise Conversational AI](https://arxiv.org/abs/2605.15665)
*Keshava Chaitanya,Jahnavi Gundakaram*

Main category: cs.AI

TL;DR: PRISM是一个闭环框架，通过持续模拟和监控优化提示词，确保LLM驱动的对话智能体在部署后可靠运行。


<details>
  <summary>Details</summary>
Motivation: 现有提示词优化框架仅关注一次性编译时质量，忽视了LLM行为漂移导致的提示词回归问题。

Method: PRISM自动生成测试用例，模拟多轮对话，评估通过/失败，诊断失败原因并修复提示词，迭代至所有测试通过，并计划每天运行。

Result: 在35个企业对话智能体上部署三周，中位提示词编写时间从2天降至30分钟以下，实现99%生产可靠性，24小时内检测并修复生产回归。

Conclusion: 持续、模拟驱动的提示词优化对于大规模可靠的企业对话AI是可行且必要的。
...
TL;DR: NudgeRL通过策略引导实现结构化探索，在不增加大量计算开销的情况下提升强化学习效果。


<details>
  <summary>Details</summary>
Motivation: RLVR中探索不足限制了推理能力提升，增加rollout计算昂贵且现有方法对探索控制有限。

Method: 提出Strategy Nudging，基于轻量级策略上下文诱导多样轨迹；统一目标分解奖励为上下文内/间分量，并加入蒸馏目标。

Result: 在五个数学基准上，NudgeRL以更少rollout超越标准GRPO和oracle引导基线。

Conclusion: 结构化上下文驱动探索是高效的替代方案，可扩展到复杂推理任务。
...
TL;DR: 大型语言模型评估用户状态的度量在个体分数层面缺乏心理测量可靠性，仅少数度量稳定，但提出可复现评估框架以支持负责任AI设计。


<details>
  <summary>Details</summary>
Motivation: 基于LLM评估用户状态的假设缺乏实证检验，需要验证度量的稳定性和可解释性。

Method: 采用复制评估程序，测试三种双峰LLM（GPT-4o audio、Gemini 2.0 Flash、Gemini 2.5 Flash）的213个度量，分析个体和聚合可靠性。

Result: 仅31个度量满足个体水平稳定性标准，多数度量个体得分不稳定，但聚合后部分度量仍有效。

Conclusion: 度量可靠性非默认属性；个体不稳定的度量可用于事后分析；研究提出可复现框架，要求显式验证可靠性并监控时间变化。
...
### [164] [ALSO: Adversarial Online Strategy Optimization for Social Agents](https://arxiv.org/abs/2605.15768)
*Xiang Li,Liping Yi,Mingze Kong,Min Zhang,Zhongxiang Dai,QingHua Hu*

Main category: cs.AI

TL;DR: 提出ALSO框架，通过在线对抗策略优化实现多智能体社交模拟中的动态策略调整，优于静态基准和现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM社交智能体依赖静态角色，离线强化学习或外部规划器不适用于非平稳多轮对话环境。

Method: 将多轮交互建模为对抗性多臂老虎机问题，使用轻量神经网络预测奖励并在线优化策略。

Result: 在Sotopia基准上持续优于静态基线和现有优化方法。

Conclusion: 对抗性在线策略优化能有效构建健壮的社交智能体。
...
### [165] [SaaS-Bench: Can Computer-Use Agents Leverage Real-World SaaS to Solve Professional Workflows?](https://arxiv.org/abs/2605.15777)
...
TL;DR: 提出了SaaS-Bench，一个基于真实SaaS系统的基准，用于评估计算机使用代理在复杂工作流程中的能力，实验发现最强模型端到端任务完成率低于4%。


<details>
  <summary>Details</summary>
Motivation: 现有代理基准限于简化设置和短时交互，无法评估专业工作流中的能力；SaaS环境包含动态状态、跨应用协调等复杂特性，适合评估。

Method: 构建了23个可部署的SaaS系统和106个真实任务，任务需长期执行，覆盖文本和多模态，通过加权验证检查点衡量严格完成和部分进度。

Result: 代表性LLM代理在SaaS-Bench上表现困难，最强模型端到端完成率低于4%，暴露了规划、状态跟踪、跨应用上下文维护和错误恢复的局限。

Conclusion: SaaS-Bench揭示了当前代理在复杂真实环境中的不足，为改进提供了方向。
...
### [166] [Agentic Discovery of Neural Architectures: AIRA-Compose and AIRA-Design](https://arxiv.org/abs/2605.15871)
*Alberto Pepe,Chien-Yu Lin,Despoina Magka,Bilge Acun,Yannan Nellie Wu,Anton Protopopov,Carole-Jean Wu,Yoram Bachrach*

Main category: cs.AI

TL;DR: 研究LLM智能体自主设计基础模型，提出双框架方法，生成的架构在多个基准上超越人工设计基线，迈向递归自我改进。


<details>
  <summary>Details</summary>
Motivation: 探索LLM智能体自主设计超越标准Transformer的基础模型，实现递归自我改进。

Method: 双框架：AIRA-Compose用于高级架构搜索（11个智能体探索计算原语并外推至大尺度），AIRA-Design用于低级机制实现（20个智能体编写注意力机制和训练脚本）。

Result: AIRAformer和AIRAhybrid在1B规模预训练后在上下游任务上优于Llama 3.2（准确率提升2.4%-3.8%），缩放效率更高（如AIRAformer-C缩放速度比Llama 3.2快54%-71%）；Long Range Arena上接近人类SOTA（差距2.3%-2.6%）；Greedy Opus 4.5在Autoresearch上超越已发表最小值。
...
### [167] [Imperfect World Models are Exploitable](https://arxiv.org/abs/2605.15960)
...
TL;DR: 提出了强化学习中模型利用的新定义，证明其在大型策略集上不可避免，并建立了与奖励黑客的正式联系，同时提出了松弛概念和安全水平。


<details>
  <summary>Details</summary>
Motivation: 模型利用与奖励黑客不同，且其不可避免性未得到证明，需要理论理解。

Method: 通过类比奖励黑客，发展通用理论，引入松弛定义并推导安全水平。

Result: 利用在大型策略集上不可避免；有限集下防黑客条件不防利用；存在可避免利用的安全水平。

Conclusion: 模型利用和奖励黑客有形式联系，安全规划有极限。
...
### [168] [PAGER: Bridging the Semantic-Execution Gap in Point-Precise Geometric GUI Control](https://arxiv.org/abs/2605.15963)
*Jingxuan Wei,Xi Bai,Shan Liu,Caijun Jia,Zheng Sun,Xinglong Xu,Siyuan Li,Linzhuang Sun,Bihui Yu,Conghui He,Cheng Tan*

Main category: cs.AI

TL;DR: 提出PAGER方法，通过分解依赖结构和像素级执行，在精确GUI构造任务中实现4.1x成功率提升，解决了语义-执行鸿沟。


<details>
  <summary>Details</summary>
Motivation: 现有GUI代理依赖容错区域，但精确几何构造任务需要点级精度，且局部错误会导致拓扑传播，现有模型无法胜任。

Method: 构建PAGE Bench基准（4906问题，22.4万像素级动作）；提出PAGER，包含依赖结构规划、像素级执行，通过像素监督微调建立动作语法，并结合精确对齐强化学习矫正暴露偏差。

Result: 通用多模态模型动作类型准确率>88%但任务成功率<6%；PAGER将任务成功率提升4.1倍，步骤成功率从<9%提升至>62%。

Conclusion: 精确GUI控制需要拓扑感知和像素级执行，PAGER达到新最佳效果，揭示了语义-执行鸿沟。
...
### [169] [Deterministic Event-Graph Substrates as World Models for Counterfactual Reasoning](https://arxiv.org/abs/2605.15967)
*Fabio Rovai*

Main category: cs.AI

TL;DR: 提出事件图基板作为可检查、支持精确反事实、跨领域迁移的世界模型，在CLEVRER和twin-EventLog基准上超越部分基线。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型难以同时满足可检查性、精确反事实推理和跨领域迁移性。

Method: 使用附加日志的RDF三元组表示状态，通过结构化干预词汇分叉日志实现反事实查询；形式化并证明解释性与反事实查询的对偶性，归约为因果祖先遍历。

Result: 在CLEVRER验证集上，基板在所有四类问题中超越NS-DR符号神谕（分别高9.89、20.26、17.65、0.80个百分点），在描述性和解释性上优于参数化基线ALOE，但在预测和反事实上略逊；在twin-EventLog基准上，比Llama-3.1-8B高18.80点联合准确率。

Conclusion: 事件图基板在可解释性和反事实推理方面具有优势，无需学习组件即可跨领域应用，但预测能力仍有提升空间。
...
### [170] [Learning Bilevel Policies over Symbolic World Models for Long-Horizon Planning](https://arxiv.org/abs/2605.15975)
*Dillon Z. Chen,Till Hofmann,Toryn Q. Klassen,Sheila A. McIlraith*

Main category: cs.AI

TL;DR: 提出BISON系统，结合低级模仿学习与高级符号抽象的双层策略，实现长期规划，在MetaWorld上表现出更优的泛化能力和效率。


<details>
  <summary>Details</summary>
Motivation: 低级模仿学习难以处理长期规划，而高级符号抽象擅长规划但缺乏精细控制，需结合二者优势。

Method: 采用双层策略 (π_hl, π_ll)：低级策略从演示学习，高级策略从符号抽象和归纳泛化构建。

Result: 在扩展MetaWorld基准上，BISON优于VLA和端到端方法，能处理更多物体和更长规划；高级策略可解决含10,000个物体的规划问题。

Conclusion: 结合低级模仿与高级符号规划可有效解决长期规划问题，提升泛化性和效率。
...
### [171] [Petri Net Induced Heuristic Search for Resource Constrained Scheduling](https://arxiv.org/abs/2605.15983)
*Ido Lublin,Dor Atzmon,Izack Cohen*

Main category: cs.AI

TL;DR: 本文提出将资源受限项目调度问题（RCPSP）建模为带资源的定时变迁Petri网的可达图上的最优搜索，并使用相对延迟令牌，使调度决策对应于诱导状态空间中的变迁触发。通过结合关键路径和资源下界的启发式函数，使用A*算法求解，并证明该启发式在令牌时间语义下是一致的。在PSPLIB基准测试上的实验表明，该方法在成功率和求解时间上均优于强精确混合整数线性规划（MIP）基线（SCIP、CBC）。逐实例分析显示，启发式搜索和MIP沿着独立维度退化：A*的资源紧度维度和MIP的规模维度，资源强度决定了哪种求解器受益于规模。


<details>
  <summary>Details</summary>
Motivation: 现有MIP方法在求解RCPSP时存在局限性，启发式搜索方法可提供更高效的求解路径。

Method: 将RCPSP形式化为带资源的定时变迁Petri网的可达图上的最优搜索，使用相对延迟令牌，并采用A*算法结合关键路径和资源下界启发式函数。

Result: 在PSPLIB基准上，该方法在成功率和求解时间上均优于SCIP和CBC等MIP求解器。

Conclusion: 启发式搜索与MIP在不同维度上表现优劣，资源紧度和问题规模分别影响A*和MIP的性能，资源强度决定不同规模下的最佳求解器选择。
...
### [172] [ScreenSearch: Uncertainty-Aware OS Exploration](https://arxiv.org/abs/2605.16024)
*Michael Solodko,Justin Wagle*

Main category: cs.AI

TL;DR: ScreenSearch 通过结构屏幕检索和去重，结合模糊感知的 PUCT 图搜索，在桌面应用中实现大规模探索，并发现新颖性-模糊性权衡。


<details>
  <summary>Details</summary>
Motivation: 桌面 GUI 代理在部分可观测性下操作，视觉相似的屏幕可能对应不同工作流状态，本地看似合理的动作可能导致截然不同的结果，需要有效的状态探索。

Method: 结合结构屏幕检索（将 UIA 树转换为位置感知结构特征、稀疏标记搜索和元数据过滤）与去重状态图，以及基于匹配动作结果分散度的模糊信号，驱动探索和重放策略评估。

Result: 在11个桌面应用上收集超过100万张截图和3万个去重状态，产生大规模探索语料库，显示跨应用和应用内多样性；观察到了新颖性-模糊性权衡。

Conclusion: 状态身份、提议质量和模糊感知搜索都对决定何时探测和何时提交至关重要，仅减少模糊性不足以作为探索目标。
...
### [173] [Reasoners or Translators? Contamination-aware Evaluation and Neuro-Symbolic Robustness in Tax Law](https://arxiv.org/abs/2605.16052)
*Parisa Kordjamshidi,Samer Aslan,Madhavan Seshadri,Leslie Barrett,Enrico Santus*

Main category: cs.AI

TL;DR: 研究表明，数据污染会夸大LLM在法律推理中的表现，而神经符号方法提供更好的泛化能力和可靠性。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM在法律推理中表现优异，但不确定其性能是否源于真正的推理能力或数据污染。

Method: 通过税法律推理的实证研究，实施污染检测协议，系统比较单体LLM与混合神经符号系统，并构建新型测试套件以评估泛化能力。

Result: 性能可能被污染夸大；法律推理具有组合性；神经符号框架更可靠、鲁棒且泛化性更好。

Conclusion: 神经符号框架为法律AI提供了更可靠和鲁棒的基础。
...
### [174] [Sign-Separated Finite-Time Error Analysis of Q-Learning](https://arxiv.org/abs/2605.16103)
*Donghwan Lee*

Main category: cs.AI

TL;DR: 本文通过符号分离方法对固定步长Q-learning进行有限时间误差分析，揭示了误差动态中的最大值诱导不对称性，并提供了确定性及随机情况下的有限时间界。


<details>
  <summary>Details</summary>
Motivation: 理解Q-learning误差动态中的不对称性，特别是正负误差的不同行为及其与过估计的关系。

Method: 利用切换系统表示，将误差分解为负部和正部；负部由固定最优策略的线性时不变系统控制，正部由线性切换系统控制。

Result: 负侧LTI证书的指数包络不慢于正侧切换证书，并可能更快；提供了确定性及随机常步长递归的有限时间界。

Conclusion: Q-learning误差动态存在最大值诱导的不对称性，正误差通过Bellman最大值被选择和传播，负误差则受最优策略下界约束，该不对称性与过估计现象相关。
...
### [175] [ShopGym: An Integrated Framework for Realistic Simulation and Scalable Benchmarking of E-Commerce Web Agents](https://arxiv.org/abs/2605.16116)
*Chinmay Savadikar,Mingyu Zhao,Yuanzheng Zhu,Han Li,Shuang Xie,Alberto Castelo,Tianfu Wu,Lingyun Wang*

Main category: cs.AI

TL;DR: 提出了ShopGym框架，通过ShopArena将真实商店转化为可控沙箱，ShopGuru合成多样化任务，实现了电商Web智能体的可扩展、可复现评估。


<details>
  <summary>Details</summary>
Motivation: 现有方法在真实性和可控性之间难以平衡：真实商店不可复现且难以检查，手工沙箱缺乏多样性。

Method: ShopArena通过匿名化商店规范和分阶段验证将种子商店转化为沙箱；ShopGuru基于沙箱合成七类技能任务，利用目录、导航、策略和交互信息。

Result: 在224个任务上验证，合成商店保留了真实商店的关键结构特性，智能体在合成商店的表现与真实商店正相关。

Conclusion: ShopGym提供了兼顾真实、多样、可控、可检查、可复现的评估环境，填补了方法空白。
...
TL;DR: 提出属性引导的LLM程序合成方法，通过验证属性并提供具体反例来减少程序生成和评估成本，提升程序质量。


<details>
  <summary>Details</summary>
Motivation: 传统LLM程序合成依赖简单数值分数，无法提供失败原因，导致需要生成和评估大量候选程序，增加推理和评估成本。

Method: 在程序合成中检查候选程序是否满足形式化定义的属性，违反时提前终止评估并向LLM提供具体反例，指导LLM生成更强程序。在PDDL规划域中，要求合成的启发式函数具有“直接性”属性，即每个可通过严格改进状态到达的状态都有严格改进后继，使爬山算法直接到达目标状态。采用反例引导修复循环：生成一个候选程序，在训练集上检查属性，返回第一个违反属性案例。

Result: 在十个规划域中测试，合成启发式在几乎所有测试任务中有效。相比最佳先前方法，每个域平均生成程序数减少七倍，无需搜索解决更多任务，评估候选的计算成本降低几个数量级。

Conclusion: 只要问题存在可验证属性，属性引导的LLM合成可降低成本和提升程序质量。
...
TL;DR: 提出探索-行动范式，通过交替任务执行和探索来提升智能体在新环境中的适应性。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的智能体因过早利用先验知识而缺乏充分环境信息，导致在陌生环境中失败。

Method: 定义探索检查点覆盖率指标，开发交替任务执行与探索的训练策略，提出探索-行动范式。

Result: 系统评估表明标准任务导向强化学习导致狭窄重复行为，而所提方法能提升性能。

Conclusion: 学习系统探索对构建通用和现实世界智能体至关重要。
...
TL;DR: 本文用结构因果建模形式化二元道德理论，揭示道德判断的代数机制，并展示其在AI政策设计中的应用。


<details>
  <summary>Details</summary>
Motivation: 现有道德AI缺乏对人类道德认知的严格数学表达，难以处理多节点场景和冲突义务。

Method: 定义三个心理算子（定型、完成、效价依赖推理）扩展标准SCM，通过节点坍缩和顺序处理压缩多节点场景。

Result: 代数框架可检测冲突义务、设计维护用户自主的帮助策略，以及将失败沟通建模为因果干预。

Conclusion: 该形式化使神经符号AI能进行数学严谨且符合人类认知的道德计算，推荐情境化心智感知测量。
...
TL;DR: 提出结合形式化方法与机器学习的AI审计和监控技术，用于检测黑盒先进AI系统（特别是LLM）的时间行为约束违反，实验表明该方法优于LLM基线方法。


<details>
  <summary>Details</summary>
Motivation: AI治理需要有效的监控和审计方法，以确保AI产品和服务在全生命周期中遵守安全、规范等行为约束。

Method: 结合形式化方法（线性时序逻辑LTL）与最先进机器学习，提出离线审计和在线监控技术，包括基于采样的预测性监控和运行时干预监控。

Result: 所提方法在检测时间行为约束违反方面优于LLM基线；小型模型标签器可匹配或超越前沿LLM评判器；预测和干预监控显著降低LLM代理的违规率，同时保持任务性能。实验还表明LLM的时间推理能力随事件距离、约束数量和命题数量的增加而明显下降。

Conclusion: 所提审计和监控技术有效适用于AI系统的行为约束检测，尤其是面向时间扩展属性，具有实际应用价值。
...
### [180] [Context, Reasoning, and Hierarchy: A Cost-Performance Study of Compound LLM Agent Design in an Adversarial POMDP](https://arxiv.org/abs/2605.16205)
*Igor Bogdanov,Chung-Horng Lung,Thomas Kunz,Jie Gao,Adrian Taylor,Marzia Zaman*

Main category: cs.AI

TL;DR: 在对抗性部分可观测环境中，程序化状态抽象和层次分解比深层推理更有效，推理工具与层次结合会导致性能下降（推理级联）。


<details>
  <summary>Details</summary>
Motivation: 部署复合LLM智能体时，设计选择（所见、推理方式、任务分解）对性能影响未知，缺乏指导。

Method: 在CybORG CAGE-2环境中进行控制实验，变化上下文表示、推理工具（自问、自批评、自改进）和层次分解（单一ReAct vs. 专门子智能体），测量回报和token成本。

Result: 程序化状态抽象使回报提升76%；推理工具与层次结合导致回报下降最多3.4倍，token增加1.8-2.7倍；无推理的层次分解取得最佳绝对性能；上下文工程比推理更经济。
...
### [181] [Confirming Correct, Missing the Rest: LLM Tutoring Agents Struggle Where Feedback Matters Most](https://arxiv.org/abs/2605.16207)
*Tahreem Yasir,Wenbo Li,Sam Gilson,Sutapa Dey Tithi,Xiaoyi Tian,Tiffany Barnes*

Main category: cs.AI

TL;DR: 评估了7个LLM在命题逻辑中作为反馈代理的准确性，发现它们在最优步骤上表现良好，但在有效但次优步骤和错误步骤上诊断不准确，且诊断与教学反馈质量存在差距。


<details>
  <summary>Details</summary>
Motivation: 区分最优、有效但次优和错误的学生解决方案对于智能辅导系统至关重要，但LLM在这方面的能力未经测试。

Method: 使用知识图谱生成的ground truth，在10,836个解决方案-反馈对和三种反馈条件下评估7个LLM反馈代理。

Result: 在最优步骤上接近完美，但系统性地过度拒绝有效但次优的推理，并过度验证错误解决方案。

Conclusion: LLM更适合混合架构，由知识图谱模型负责诊断，LLM负责开放式支架和对话。
...
TL;DR: 提出了首个完全开放的医学LLM-CDSS构建管道Fully Open Meditron，包含可审计的训练语料库、可复现的框架和评估协议，在多个基础模型上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的临床决策支持系统大多不透明，缺乏完全开放模型，难以实现可审计和可复现的验证。

Method: 构建包含8个公共医疗QA数据集和3个临床医生验证的合成扩展的语料库；采用系统级去污染、金标重采样和四医师面板端到端验证；使用LLM-as-a-judge协议结合人类评分进行评估。

Result: 所有MeditronFO变体优于基础模型，Apertus-70B-MeditronFO在医学基准上提升+6.6分（47.2%→53.8%），Gemma-3-27B-MeditronFO在58.6%的比较中胜出MedGemma。

Conclusion: 完全开放的管道可在不牺牲可审计性和可复现性的前提下，实现领域特定性能的最先进水平。
...
### [183] [FORGE: Self-Evolving Agent Memory With No Weight Updates via Population Broadcast](https://arxiv.org/abs/2605.16233)
*Igor Bogdanov,Chung-Horng Lung,Thomas Kunz,Jie Gao,Adrian Taylor,Marzia Zaman*

Main category: cs.AI

TL;DR: 提出FORGE协议，通过自生成自然语言记忆提升LLM Agent决策，在CybORG CAGE-2任务中回报提升1.7-7.7倍。


<details>
  <summary>Details</summary>
Motivation: 探索LLM Agent能否仅通过自生成记忆（无梯度更新）改进决策。

Method: FORGE采用分阶段种群进化协议，内循环使用Reflexion将失败轨迹转化为规则、示例或混合记忆，外循环广播最佳记忆并冻结收敛实例。

Result: 在12种模型-记忆组合中，FORGE平均回报较零样本提升1.7-7.7倍，较Reflexion提升29-72%，重大失败率降至约1%。

Conclusion: 种群广播是关键机制，示例记忆回报最高，规则记忆性价比更优，弱模型受益更大。
...
### [184] [Prospective multi-pathogen disease forecasting using autonomous LLM-guided tree search](https://arxiv.org/abs/2605.16238)
*Sarah Martinson,Michael P. Brenner,Martyna Plomecka,Brian P. Williams,Nicholas G. Reich,Zahra Shamsi*

Main category: cs.AI

TL;DR: 提出一种基于大语言模型引导的树搜索自主系统，用于迭代生成、评估和优化传染病概率预测软件，在2025-2026年美国呼吸道季节前瞻性评估中，其集成模型性能达到或超过CDC人工策展的黄金标准集成模型，克服了建模劳动瓶颈。


<details>
  <summary>Details</summary>
Motivation: 传染病概率预测依赖专家手动建模，劳动密集，难以扩展到精细地理尺度或新发病原体。

Method: 利用大语言模型引导的树搜索，自动生成、评估和优化可执行预测代码；通过前瞻性实时评估，为流感、COVID-19和RSV发现多样化模型，并聚合为集成模型。

Result: 自主生成的集成模型在样本外预测中一致达到或超过CDC人工策展的黄金标准集成模型；成功应对RSV数据稀缺的冷启动场景；消融实验证实对数尺度距离优化防止奖励黑客，自动法官保证结构保真度。

Conclusion: 该框架将流行病学理论自主转化为准确透明的代码，突破建模劳动力瓶颈，实现专家级疾病预测的快速大规模部署。
...
### [185] [Reweighting free energy profiles between universal machine learning interatomic potentials for fast consensus building](https://arxiv.org/abs/2605.15630)
...
TL;DR: 提出了一种系统化框架，利用平均能量间隙近似，将从一个源机器学习原子间势（MLIP）得到的自由能分布（PMF）重新加权到多个目标MLIP，显著降低了计算成本，并在纳米受限电解质中锂离子输运的复杂系统上验证了其准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 用密度泛函理论（DFT）精度计算自由能分布计算成本高，通用MLIP虽能大幅降低成本，但其准确性受训练数据影响，对特定系统可能具有不确定性。传统直接指数重加权在大体系下因势能间相空间重叠低而失败。

Method: 采用稳健的解析校正，具体为平均能量间隙近似，避免统计崩溃，从而将从一个源MLIP采样的PMF重新加权到多个目标MLIP。

Result: 在601原子的Li⁺纳米受限电解质体系中，该方法成功恢复了多个DFT参考水平（PBE+D3、PBE-sol、r²SCAN、r²SCAN-D4）的目标热力学性质，包括反应自由能和活化自由能，计算成本仅为完整模拟的一小部分。

Conclusion: 该框架建立了一个诊断协议，可在无需冗余、资源密集型模拟的情况下，实现材料化学性质的经济型跨模型共识。
...
### [186] [Do Biological Structural Guarantees Earn Their Complexity?](https://arxiv.org/abs/2605.15225)
*Bogdan Banu*

Main category: q-bio.QM

TL;DR: 生物启发式AI框架声称具有可靠性优势，但缺乏实证验证。本文通过三个深度基准测试，在超过1000万数据点上严格比较了生物启发实现与简单替代方案。


<details>
  <summary>Details</summary>
Motivation: 生物启发式AI框架声称具有结构保证的可靠性，但这些主张很少在简单替代方案上进行实证检验。

Method: 设计了三个深度基准测试：代谢优先级门控、基于自诱导物的群体感应和贝叶斯停滞检测，每个测试比较了生物启发实现、朴素非生物替代方案和消融对照，每种子1000次试验，共10种子，总计超过1000万数据点。

Result: 未明确给出具体结果，但可能表明生物启发式方法在某些场景下并不优于简单替代方案。

Conclusion: 需要更严格的实证验证来评估生物启发式AI框架的可靠性声称。
...
### [187] [Coherent States of Non-Null Torus Knots](https://arxiv.org/abs/2605.15420)
*Gabriel Canadas da Silva,Ion Vasile Vancea*

Main category: quant-ph

TL;DR: 构建了与经典非零环面纽结电磁场解对应的相干态，建立了量子-经典对应。


<details>
  <summary>Details</summary>
Motivation: 探索经典拓扑电磁场解的量子对应，揭示其量子性质。

Method: 从场与相干态振幅关系导出位移算符，直接验证相干态定义性质，计算场期望值、能量密度等观测量。

Result: 以整数参数(n,m,l,s)表达了纽结相干态中观测量的期望值，并特例化到霍普菲翁相干态。

Conclusion: 确立了这类真空拓扑电磁系统的量子-经典对应关系。
...
### [188] [Quantum game theory for 2 2 games: a mathematical framework](https://arxiv.org/abs/2605.15747)
*Gloria Ferraris,Veronica Umanit\`a*

Main category: quant-ph

TL;DR: 为静态2x2量子博弈建立了严格数学框架，证明了连续量子混合策略的纳什均衡存在性。


<details>
  <summary>Details</summary>
Motivation: 将经典博弈论扩展到量子设置，处理任意幺正操作和连续策略空间。

Method: 采用Eisert-Wilkens-Lewenstein协议，通过不动点论证证明纳什均衡存在。

Result: 证明了连续量子混合策略的纳什均衡存在性，推广了经典纳什定理。

Conclusion: 量子博弈论在2x2情况下具有坚实的数学基础。
...
### [189] [Fast convergence of Dynamic Capacities of GNS-Symmetric Quantum Channels](https://arxiv.org/abs/2605.15953)
*Omar Fawzi,Li Gao,Mizanur Rahaman,Mostafa Taheri*

Main category: quant-ph

TL;DR: 研究了GNS对称信道的经典与量子容量随时间呈指数收敛的界，并用于量化主动与被动纠错性能。


<details>
  <summary>Details</summary>
Motivation: 理解量子信道信息容量的时间演化，并比较主动与被动纠错方案的性能。

Method: 利用GNS对称信道（包括Pauli信道）的熵特性，推导经典和量子容量的指数收敛界。

Result: 给出了显式的指数收敛界，将容量收敛速度与信道的熵性质相关联。

Conclusion: 该结果有助于量化主动纠错与被动纠错在量子系统中的性能差异。
...
### [190] [Diagonal Adaptive Non-local Observables on Quantum Neural Networks](https://arxiv.org/abs/2605.15410)
*Huan-Hsin Tseng,Yan Li,Hsin-Yi Lin,Samuel Yen-Chi Chen*

Main category: quant-ph

TL;DR: 提出对角线自适应非局部观测量（Diagonal ANO），在保持完整ANO表达能力的同时，将k-局部观测量复杂度从O(4^k)降至O(2^k)，并降低经典优化成本。


<details>
  <summary>Details</summary>
Motivation: 完整ANO虽能扩大变分量子算法的函数空间，但参数数量剧增且经典优化成本高，需寻找更高效的实现形式。

Method: 仅考虑与量子电路配对的对角观测量，利用对角矩阵是ANO空间模酉相似的标准代表，实现与完整ANO相同的数学功能。

Result: k-局部观测量复杂度从O(4^k)降至O(2^k)，经典计算开销降低，同时保留完整ANO的大部分优势。

Conclusion: 对角线ANO在保持表达能力的同时显著降低了测量复杂度，并将传统变分量子电路作为特例，是一种更高效的ANO实现。
...
### [191] [Quantum Feature Pyramid Gating for Seismic Image Segmentation](https://arxiv.org/abs/2605.15370)
*Taha Gharaibeh,Jyotsna Sharma*

Main category: quant-ph

TL;DR: 量子特征门控通过嵌入参数化量子电路于特征融合点，在盐体地震分割中显著提升IoU。


<details>
  <summary>Details</summary>
Motivation: 盐体结构扭曲波传播，增加勘探不确定性，而混合量子经典模型在像素级地球物理预测中未充分验证。

Method: 在编码-解码器的特征金字塔网络合并点嵌入4量子比特2层PQC，通过全局平均池化将编码特征映射为4维量子输入，实现72参数量子门控。

Result: 在EfficientNetV2-L消融实验中，量子FPN门相比逐元素加法提升IoU 9.85个百分点；在U-Net中作为跳跃连接注意力提升0.88点。
...
### [192] [Fairness-Aware Retrieval Optimization for Retrieval-Augmented Generation](https://arxiv.org/abs/2605.15790)
*Yingqi Zhao,Vasilis Efthymiou,Jyrki Nummenmaa,Kostas Stefanidis*

Main category: cs.DB

TL;DR: 提出一个公平感知检索框架来控制和减轻RAG中检索引入的偏见，同时保持相关性。


<details>
  <summary>Details</summary>
Motivation: RAG中检索过程可能引入偏见，影响生成输出，尤其在top-k设置中多文档共同影响生成时问题更突出。

Method: 结合可控偏见注入（重排序）、位置感知偏见传播模型，以及平衡相关性与公平性的优化框架，并引入基于双重超平面近似的二次公平性（FARO）算法实现可扩展优化。

Result: 该方法有效缓解了生成偏见，同时保持了检索相关性。
...
### [193] [To GPU or Not to GPU: Vector Search in Relational Engines](https://arxiv.org/abs/2605.15957)
*Vasilis Mageirakos,Joel Andr\'e,Marko Kabi\'c,Bowen Wu,Yannis Chronis,Gustavo Alonso*

Main category: cs.DB

TL;DR: 探索数据库引擎中集成GPU进行向量搜索的设计，通过扩展TPC-H基准和实验，发现关系组件在GPU上收益更大，并优化索引使GPU竞争性更强。


<details>
  <summary>Details</summary>
Motivation: 现有数据库引擎在实现向量搜索时使用CPU，而AI/ML/LLM领域依赖GPU，引发是否应集成GPU处理的疑问。

Method: 扩展TPC-H基准加入向量数据，提出SQL+VS查询；开发模块化执行引擎支持CPU/GPU混合运行；实验对比不同部署方案（CPU/GPU、内存位置、索引类型、互联方式）。

Result: 关系组件在GPU上收益远大于向量搜索部分；向量搜索涉及数据移动时GPU不是最佳选择，即便快速互联；通过减小索引大小使GPU更有效。

Conclusion: 优化后，关系组件和向量搜索在GPU上均更快，尤其快速互联下，与现有引擎架构相反。
...
### [194] [Relational Database Data Lineage Ontology](https://arxiv.org/abs/2605.16068)
*Jakub Dutkiewicz,Pawe{\l} Misiorek,Robert Wrembel*

Main category: cs.DB

TL;DR: 提出一种新的关系数据库数据血缘本体，通过知识图谱丰富语义表示，从而提升血缘链接发现性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型在处理不完整或缺失的数据库对象依赖关系时面临挑战，需要更丰富的语义表示来支持血缘发现。

Method: 在前工作基础上扩展本体，增加结构、语义和转换层面的概念，利用基于路径嵌入的图神经网络进行归纳链接预测。

Result: 实验表明，新本体在AUC和Hits@10指标上优于基线本体，提高了血缘链接预测性能。

Conclusion: 所提出的本体能够更精确地编码血缘证据，有效提升关系数据库数据血缘的发现能力。
...
### [195] [Towards Foundation Models for Relational Databases with Language Models and Graph Neural Networks](https://arxiv.org/abs/2605.16085)
*Jingcheng Wu,Ratan Bahadur Thapa,Mojtaba Nayyeri,Lucas Etteldorf,Max Finkenbeiner,Fabian Leeske,Steffen Staab*

Main category: cs.DB

TL;DR: 提出一种轻量级混合BART-GNN架构用于关系数据库深度学习，在RelBench上性能接近RDL，为资源高效的基础模型探索提供了方向。


<details>
  <summary>Details</summary>
Motivation: 关系数据库存储大量结构化信息，但传统方法通过特征工程扁平化表，丢失关系上下文；RDL虽能建模关系图但任务和数据库特化，缺乏通用性。

Method: 混合架构：微调BART编码器捕捉行内语义，结合基于GraphSAGE的GNN在关系实体图上注入关系上下文。

Result: 在rel-f1数据集的driver-dnf任务上ROC-AUC达67.40，接近LightGBM的68.86，与RDL差距5.22点，但与SOTA模型KumoRFM的82.63仍有较大差距。

Conclusion: 轻量级混合LM-GNN架构在资源效率上展示了潜力，但需要进一步缩小与专用基础模型的差距。
...
### [196] [PhysBrain 1.0 Technical Report](https://arxiv.org/abs/2605.15298)
...
TL;DR: 通过人类自我中心视频生成物理常识监督，训练PhysBrain VLM，再迁移到VLA策略，在多个基准上达到SOTA，尤其在域外场景表现突出。


<details>
  <summary>Details</summary>
Motivation: 机器人轨迹数据覆盖有限，而人类自我中心视频能提供丰富的物理常识，但缺乏结构化利用。

Method: 构建数据引擎从视频中提取场景、空间动态、动作执行和深度关系，生成问答监督训练PhysBrain VLM，再通过保持能力且语言敏感的适配设计迁移到VLA策略。

Result: 在ERQA、PhysBench、SimperEnv-WidowX、LIBERO、RoboCasa等基准上达到SOTA，SimperEnv上域外性能尤其强。

Conclusion: 从人类交互视频中扩展物理常识能为多模态理解到机器人动作提供有效桥梁。
...
### [197] [HoloMotion-1 Technical Report](https://arxiv.org/abs/2605.15336)
*Maiyue Chen,Kaihui Wang,Bo Zhang,Xihan Ma,Zhiyuan Yang,Yi Ren,Qijun Huang,Zihao Zhu,Yucheng Wang,Zhizhong Su*

Main category: cs.RO

TL;DR: HoloMotion-1是一个面向零样本全身运动跟踪的人形运动基础模型，通过大规模混合运动语料库和稀疏激活的MoE Transformer架构，实现了对多样化运动的鲁棒泛化和高精度跟踪。


<details>
  <summary>Details</summary>
Motivation: 传统运动跟踪模型依赖动作捕捉数据，缺乏多样性和部署适应性。HoloMotion-1旨在利用包含视频重建动作的大规模混合语料库，使模型能处理更广泛的运动行为和捕获条件。

Method: 采用大规模混合运动语料库（视频重建、动作捕捉、内部数据），集成大容量时序建模、稀疏激活的Mixture-of-Experts Transformer和KV-cache推理，并引入序列级训练策略以提升长序列学习效率。

Result: 在多个未见过的运动基准上，HoloMotion-1展现了强大的鲁棒泛化能力，显著提高了跟踪精度，并可直接迁移到真实人形机器人，无需特定任务微调。

Conclusion: Holomotion-1通过数据驱动和高效架构设计，突破了传统动作捕捉训练的局限，为真实世界人形机器人运动控制提供了可行方案。
...
### [198] [Residual Reinforcement Learning for Robot Teleoperation under Stochastic Delays](https://arxiv.org/abs/2605.15480)
*Kaize Deng,Zewen Yang*

Main category: cs.RO

TL;DR: 提出一种结合LSTM状态估计器和残差强化学习策略的混合控制框架，以应对遥操作中随机通信延迟导致的信号不连续问题。


<details>
  <summary>Details</summary>
Motivation: 随机通信延迟导致信号不连续，破坏控制稳定性，传统强化学习方法因延迟观测而难以处理，产生高频抖动。

Method: 提出延迟鲁棒强化学习框架，使用长短期记忆网络（LSTM）从延迟观测中重构平滑连续的状态估计，并让RL智能体学习残差转矩补偿策略，平衡跟踪精度和速度平滑性。

Result: 在Franka Panda机器人上的实验表明，该方法显著优于现有基线，即使在方差较大的随机延迟下也能确保稳定鲁棒的遥操作。

Conclusion: 所提框架有效解决了随机延迟下的控制问题。
...
### [199] [Hybrid LLM-based Intelligent Framework for Robot Task Scheduling](https://arxiv.org/abs/2605.15486)
*Swayamjit Saha,Subhabrata Das,Haonan Duan,Xiao-Yang Liu*

Main category: cs.RO

TL;DR: 本研究利用大型语言模型（LLM）提升建筑机器人任务调度，通过双代理（生成器+监督器）优化时间和资源利用，并利用自然语言处理界面适应现场变化，实验证明其有效性。


<details>
  <summary>Details</summary>
Motivation: 建筑机器人任务调度需要高效且适应性强的方法，传统方法在复杂现场条件下表现不足，LLM的引入有望提升调度精度和实时响应能力。

Method: 构建智能框架，输入任务关键数据（如动作能力、目标），采用GPT-4作为生成器、Gemma 3/Llama 4/Mistral 7b作为监督器的双LLM代理架构，通过自然语言处理界面实现人机交互，并制定平衡分配策略优化时间和资源。

Result: 在简单场景中评估，提供了指标分数，证明框架在任务调度中的有效性。

Conclusion: LLM在包括机器人调度的建筑操作任务中至关重要，所提方法能提升调度精准度和资源利用效率。
...
### [200] [SkiP: When to Skip and When to Refine for Efficient Robot Manipulation](https://arxiv.org/abs/2605.15536)
*Mingtong Dai,Guanqi Peng,Yongjie Bai,Feng Yan,Chunjie Chen,Lingbo Liu,Liang Lin,Xinyu Wu*

Main category: cs.RO

TL;DR: 提出Skip Policy (SkiP)，通过动作重标记和运动频谱关键点检测，在模仿学习中跳过冗余步骤，减少执行步数15-40%并保持或提高成功率。


<details>
  <summary>Details</summary>
Motivation: 传统模仿学习在每一步都预测未来动作，但大部分步骤是自由空间运动，信息量少，只有少数接触、抓取等关键步骤需要密集预测，因此需要高效跳过非关键步骤。

Method: 提出动作重标记机制，在跳转段中每一步用下一关键段入口的动作替换行为克隆目标，使策略能一次跳过整个跳转段；并引入运动频谱关键点检测(MSK)自动划分关键段和跳转段，无需手动标注。

Result: 在72个模拟操作任务和3个真实机器人任务中，SkiP减少执行步数15-40%，同时成功率达到或超过基线。

Conclusion: SkiP能统一网络动态跳过冗余步骤并精细处理关键步骤，无需额外规划器或层级结构，在多种策略骨干下有效。
...
### [201] [Feedback World Model Enables Precise Guidance of Diffusion Policy](https://arxiv.org/abs/2605.15705)
*Tuo An,Jindou Jia,Gen Li,Jingliang Li,Chuhao Zhou,Pengfei Liu,Bofan Lyu,Jiaqi Bai,Xinying Guo,Geng Li,Jianfei Yang*

Main category: cs.RO

TL;DR: 提出反馈世界模型，利用实时观测在线修正预测，无需额外训练即可提升分布外场景的预测准确率和策略成功率。


<details>
  <summary>Details</summary>
Motivation: 世界模型在遇到分布外状态时预测不可靠，而执行后的真实状态反馈未被充分利用。

Method: 在推理时维护轻量级反馈状态进行在线更新，作为潜在空间观测器，并引入动作感知引导以突出可控分量。

Result: 将预测误差降低最高76.4%，并将分布外成功率提升30%。

Conclusion: 在推理时融入实时反馈是一种简单而强大的替代静态世界建模的方法。
...
### [202] [Learning Dynamic Pick-and-Place for a Legged Manipulator](https://arxiv.org/abs/2605.15713)
...
TL;DR: 提出一种分层强化学习框架，使四足机器人配备六自由度手臂，实现动态抓取放置任务，适应不同重量物体，仿真成功率86.05%，实物成功率73.3%。


<details>
  <summary>Details</summary>
Motivation: 腿式机械手结合敏捷运动与灵活手臂控制，但实现精确操作同时保持协调运动是重大挑战。

Method: 采用分层强化学习框架，内含显式质量估计模块，实现自适应全身控制，动态连续执行抓取放置。

Result: 仿真中载荷达2.3kg，成功率86.05%；实物实验中载荷达1.3kg，成功率73.3%，平均执行时间4.06秒。

Conclusion: 相比以往工作，该框架实现动态连续执行，证明四足移动操作器在更重载荷和更大工作空间下的潜力。
...
### [203] [Lamarckian Inheritance in Dynamic Environments: How Key Variables Affect Evolutionary Dynamics](https://arxiv.org/abs/2605.15769)
*K. Ege de Bruin,Kyrre Glette,Kai Olav Ellefsen*

Main category: cs.RO

TL;DR: 本研究探讨机器人形态与控制协同优化中，拉马克遗传相对于达尔文遗传的优势条件，发现其仅在环境变化既矛盾又不可预测时表现较差，添加传感器可恢复优势。


<details>
  <summary>Details</summary>
Motivation: 现有文献对动态环境中拉马克遗传的效果存在矛盾，作者认为这是因为未考虑所有相关变量，因此需要系统研究其影响因素。

Method: 将形态优化作为进化，控制器优化作为终身学习，采用拉马克遗传传递控制器参数。使用虚拟软体机器人和两种学习方法（贝叶斯优化和强化学习），通过改变环境冲突性和可预测性进行实验。

Result: 拉马克遗传仅当环境变化既冲突又不可预测时表现劣于达尔文遗传；添加检测传感器可在冲突环境中恢复拉马克优势。

Conclusion: 拉马克遗传的效益取决于环境变化的冲突性和可预测性，通过传感器预测变化可泛化控制，提升性能。
...
### [204] [A Topology-Aware Spatiotemporal Handover Framework for Continuous Multi-UAV Tracking](https://arxiv.org/abs/2605.15779)
*Jianlin Ye,Christos Kyrkou,Panayiotis Kolios*

Main category: cs.RO

TL;DR: 提出了一种基于拓扑时空交接的实时多无人机多车辆跟踪系统，通过队列匹配算法实现全局身份持续，显著优于传统重识别方法。


<details>
  <summary>Details</summary>
Motivation: 多无人机视野下车辆身份不连续导致轨迹碎片化，阻碍了网络级交通分析（如起终点估计），现有方法仅优化单机跟踪且形成数据孤岛。

Method: 采用YOLO11和ByteTrack的高吞吐并行流水线处理4K流，核心是拓扑时空交接机制：利用几何重叠和虚拟车道离散化，通过FIFO队列预测性管理身份交接，避免外观重识别的视觉模糊和计算开销。

Result: 在复杂城市交通场景中，连续车流的手动交接成功率（HOSR）达99.8%，远超重识别基线（74.1%），并验证了边缘部署可行性。

Conclusion: 该系统实现了实时、鲁棒的跨相机全局身份持久化，为无人机交通监控的大规模部署提供了有效方案。
...
### [205] [GAP: Geometric Anchor Pre-training for Data-Efficient Visuomotor Learning of Manipulation Tasks](https://arxiv.org/abs/2605.15836)
*Davide Buoso,Andrea Protopapa,Stefano Di Carlo,Francesca Pistilli,Giuseppe Averta*

Main category: cs.RO

TL;DR: 提出几何锚点预训练（GAP），通过轻量级模拟代理任务预训练空间适配器，生成稳定几何锚点，在少量演示和域移位下显著提升机器人操作策略学习性能。


<details>
  <summary>Details</summary>
Motivation: 从少量专家演示学习视觉运动策略的核心挑战在于将高维RGB表示蒸馏为控制相关几何而不过拟合；使用冻结预训练VFM会导致任务适应集中于小池化模块，易陷入任务无关捷径并丢失几何基础，且预训练视觉表示在场景轻微扰动下表现不佳，需要鲁棒性归纳偏置。

Method: 提出一种无动作的预热阶段（GAP），在下游模仿学习前正则化空间适配器：在轻量级模拟代理任务中预训练池化层，该任务免费提供对象掩码，鼓励适配器生成位于对象上、覆盖其空间范围、随时间保持清晰和可重复的关键点。

Result: 在RoboMimic和ManiSkill严重数据稀缺（15-50个演示）和域移位下，简单适配器+GAP优于更强注意力和端到端微调：RoboMimic Can 15演示62%（+16% vs AFA），Tool Hang 50演示63%，ManiSkill StackCube 30演示61%（+11% vs 全微调）。

Conclusion: 代理阶段轻量级且与下游任务完全解耦，可跨环境和操作技能复用，为少样本策略学习提供可靠坐标接口。
...
### [206] [Learning Sim-Grounded Policies for Bimanual Rope Manipulation from Human Teleoperation Data](https://arxiv.org/abs/2605.16043)
*Gina Wigginghaus,Tim Missal,Berk Guler,Simon Manschitz,Jan Peters*

Main category: cs.RO

TL;DR: 本研究比较了基于视觉和基于状态的模仿学习策略在变形线性物体打结解结任务中的表现，发现状态策略显著优于视觉策略，表明观测空间是影响泛化的关键因素。


<details>
  <summary>Details</summary>
Motivation: 从遥操作模仿学习双臂变形线性物体操作受限于人力成本，观测空间的选择对从小数据集中泛化至关重要，但基于自我中心视觉的策略泛化不足的原因尚不明确。

Method: 使用相同双臂遥操作数据训练两种基于Transformer的动作分块策略：视觉策略采用两个腕部摄像头的RGB流，状态策略基于多视图融合和XPBD仿真提取的3D粒子状态。

Result: 在未见绳索配置的开环评估中，状态策略预测初始抓拉动作的L1误差比视觉策略降低30.8%。

Conclusion: 存在从像素到物理一致状态的可观测性差距，指向从有限人类演示实现更数据高效的DLO操作学习。
...
### [207] [Towards Trustworthy and Explainable AI for Perception Models: From Concept to Prototype Vehicle Deployment](https://arxiv.org/abs/2605.16087)
*Till Beemelmanns,Shayan Sharifi,Manas Mehrotra,Ayushman Choudhuri,Lutz Eckstein*

Main category: cs.RO

TL;DR: 提出了一种用于自动驾驶感知的可信AI模块，集成了鲁棒性、可解释性和不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在自动驾驶感知中占据主导地位，但其不透明性与可信AI指南相冲突，且缺乏针对3D场景理解的具体实现。

Method: 基于Transformer检测器，利用注意力机制提取解释并通过扰动测试验证忠实性，集成不确定性估计与校准模块，应用鲁棒性增强训练方法。

Result: 实验展示了忠实的显著性行为、改进的鲁棒性和校准良好的不确定性估计。

Conclusion: 通过原型车部署和XAI界面，证明了实时可信感知监控的可行性。
...
### [208] [FocalPolicy: Frequency-Optimized Chunking and Locally Anchored Flow Matching for Coherent Visuomotor Policy](https://arxiv.org/abs/2605.15944)
...
TL;DR: 针对四旋翼飞行控制的深度强化学习通常使用域随机化，导致策略过于保守。本文提出一种自适应控制架构，通过残差动力学预测器在线估计扰动，并引入线性校准桥和推力修正机制，实现高效仿真到现实迁移。在Crazyflie上验证，该方法在各种不确定性下显著优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有的基于域随机化的深度强化学习策略在面对动态扰动时表现保守，难以应对实际飞行中的不确定性。

Method: 提出自适应控制架构：首先训练最优外环策略，然后用残差动力学预测器（RDP）替代对真实扰动数据的依赖；RDP利用状态和控制动作历史在线估计外部力和力矩；为硬件迁移，引入数据高效的线性校准桥和在线推力修正机制，仅需数秒飞行数据对齐模拟潜空间与现实。

Result: 在Crazyflie微型四旋翼上的真实世界验证表明，自适应控制器在质量变化、不对称负载和动态悬挂负载等严重不确定性下，显著优于基线方法，保持精确轨迹跟踪。

Conclusion: 所提出的自适应控制架构能有效应对动态扰动和不确定性，提升了四旋翼飞行控制的鲁棒性和实际性能。
...
### [210] [CitePrism: Human-in-the-Loop AI for Citation Auditing and Editorial Integrity](https://arxiv.org/abs/2605.16000)
...
TL;DR: 提出CitePrism，一个混合决策支持框架，用于编辑引文审计，结合LLM、嵌入、元数据验证和人工审核，初步验证显示可辅助保守筛选但需进一步验证。


<details>
  <summary>Details</summary>
Motivation: 编辑和审稿人需要确保引文的相关性、准确性等，但手动审计繁琐且难以规模化。

Method: 结合LLM推理、基于嵌入的语义相似性、元数据验证、完整性标志和人工审核，提取引文邻域，计算融合相关性分数，支持可配置阈值分类。

Result: 在单案例104条引文中，与人类二元相关性标签的Cohen's kappa=0.429；在阈值tau=17时，标记所有不相关引文，但产生假阳性。

Conclusion: 可作为保守编辑筛选和引文质量分类的初步支持，但不能作为自主决策系统，需跨领域、更多标注者和部署场景的验证。
...
### [211] [paper.json: A Coordination Convention for LLM-Agent-Actionable Papers](https://arxiv.org/abs/2605.16194)
*Arquimedes Canedo*

Main category: cs.DL

TL;DR: 提出一个伴随PDF的JSON文件，包含五个轻量级约定，解决LLM代理阅读论文时的常见失败问题，并证明在1小时内可实现合规。


<details>
  <summary>Details</summary>
Motivation: LLM代理作为论文的第一读者时，标准散文式论文导致子声明无法细粒度引用、范围过度外推以及图形命令埋没在代码库中。

Method: 定义paper.json格式，包括稳定声明ID（C1）、明确不声明列表（C2）、每幅图的精确shell命令（C3）、稳定定义ID（C5），以及最小可行合规（C4）。

Result: 论文自身通过了验证（uv run validator.py paper.json --against paper.typ），证明合规可在1小时内手工完成。

Conclusion: C1、C2、C3、C5是开放邀约，代理读取合规论文后产生的行为可提供支持或反对的证据。
...
### [212] [Discrete Mixed Quantization](https://arxiv.org/abs/2605.16203)
*Qiaochu Ma*

Main category: math.SP

TL;DR: 开发了图向量丛的混合量化技术，并应用于多个渐近谱问题。


<details>
  <summary>Details</summary>
Motivation: 研究图向量丛的谱性质，解决相关渐近谱问题。

Method: 提出混合量化技术用于图向量丛。

Result: 成功应用于Alon-Boppana界、Kesten-McKay律、量子遍历性、零因子收敛和Ramanujan向量丛等问题。

Conclusion: 混合量化技术是分析图向量丛谱性质的有效方法。
...
TL;DR: 提出了Agent4POI，首个在推荐时生成上下文条件化多模态表示的POI推荐框架，相比静态嵌入方法显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态系统将每个POI编码为静态嵌入，无法处理同一POI在不同情境下的不同用途，且理论上证明预计算编码器无法满足上下文敏感排序。

Method: 利用四阶段LLM代理，在给定情境下生成动态的affordance查询，并通过跨模态思维链处理图像、评论和元数据，产生不确定性感知的affordance表示，最后通过语义缓存与用户偏好对齐进行低延迟排序。

Result: 在三个POI基准和三种评估配置下，相对最强基线提升23.2%，上下文偏移下性能仅下降7.5%（基线下降16-17%），冷启动场景下比最好内容基线提升2.4倍。

Conclusion: 动态上下文条件化表示能有效提升POI推荐的鲁棒性和准确性，尤其在上下文变化和冷启动场景下。
...
TL;DR: 提出一种基于健康饮食指数（HEI）的检索增强生成（RAG）框架，结合标准化营养数据库和大语言模型，实现个性化食物推荐，显著提升饮食质量。


<details>
  <summary>Details</summary>
Motivation: 现有食物推荐系统依赖松散的食物数据库，与验证指数关联有限，无法提供精准营养指导。

Method: 基于NHANES和FPED数据库，构建食物级嵌入空间，计算基线HEI分数，检索候选食物并估计替换/添加的HEI影响，使用预训练OpenAI LLM生成个性化推荐。

Result: 模拟显示平均HEI提升6.45，HEI超过50的用户比例从45.12%增加到61.26%，整个HEI分布均有改善。
...
TL;DR: Fortress是一个通用框架，通过识别和剪枝导致预测分数随时间不稳定的特征，来增强搜索和推荐系统中模型的稳定性和准确性。


<details>
  <summary>Details</summary>
Motivation: 预测模型常因输入特征引入时间不稳定性，导致分数波动，影响多阶段系统的可靠性和用户体验。

Method: 利用历史快照（按时间划分的数据集），捕捉同一实体在不同时期的分数波动，通过四步流程：收集快照、识别不稳定样本、隔离并移除不稳定特征、仅用稳定特征重新训练模型。

Result: 在大型应用市场的查询-应用相关性模型上，离线实验表明预测稳定性（变异系数）和分类性能（PR-AUC）显著提升。

Conclusion: Fortress有效抑制了基于参与度特征的波动性，同时保留其预测价值，实现了更稳定和准确的模型。
...
TL;DR: 提出DMP-MH框架，通过度裁剪和噪声镜像下降生成合成图，在隐私保护下实现跨模态哈希，性能优于私有基线。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态哈希方法依赖用户交互构建语义图，但图结构易受链接重构攻击；差分隐私SGD破坏关系模式，图合成方法在无标度网络中敏感度高，需要大量噪声。

Method: 采用Sanitize-then-Distill框架，先通过确定性度裁剪限制敏感度，再在(ε,δ)-边差分隐私下用噪声镜像下降生成合成图，最后用双流哈希网络蒸馏拓扑结构。

Result: 在MIRFlickr-25K和NUS-WIDE上，DMP-MH比私有基线平均精度提高11.4个百分点，保留92.5%的非私有性能。

Conclusion: 该方法有效解耦隐私与表示学习，在保护图结构隐私的同时保持了高检索性能。
...
TL;DR: 提出GenLI模型，通过生成式方法（而非匹配）捕捉用户多元兴趣，并实现O(1)复杂度行为检索，提升CTR预测的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有两阶段框架中，目标中心的搜索单元忽略潜在兴趣，匹配检索耗时且忽略行为间交互，导致兴趣特征不完整、有偏且效率低。

Method: GenLI包含兴趣生成模块（IGM）生成多元兴趣分布，行为检索模块（BRM）通过简单查找实现O(1)检索，兴趣融合模块（IFM）用门控机制融合特征。

Result: 在CTR预测任务中，GenLI提升了用户兴趣多样性，避免了复杂匹配检索，在准确性和效率之间取得更好平衡。

Conclusion: GenLI通过生成式方法有效解决了长兴趣建模中的完整性和效率问题，为CTR预测提供了新思路。
...
### [218] [Access Timing as Scaffolding: A Reinforcement Learning Approach to GenAI in Education](https://arxiv.org/abs/2605.15850)
*Janne Rotter,Pau Benazet i Montobbio,Davinia Hern\'andez-Leo*

Main category: cs.CY

TL;DR: 本研究提出通过强化学习智能体动态控制生成式AI的访问时机，作为隐式支架，相比完全开放或完全限制，在无显式提示下提升了学习效果和元认知准确性，但未改善自我报告元认知意识。


<details>
  <summary>Details</summary>
Motivation: 生成式AI在教育中广泛应用但可能导致过度依赖和元认知脱轨，现有研究多关注教学支架，而访问时机的理论基础和实证研究不足，需探索何时允许使用。

Method: 设计基于强化学习的智能体，依据元认知理论、认知负荷理论和有效失败理论设定奖励函数，动态决策学生的AI访问时机。进行N=105的混合方法对照实验，比较智能体条件与完全开放、完全限制的效果。

Result: 智能体条件下，客观后测成绩和元认知准确性显著优于完全开放，任务错误和时间少于完全限制，但自我报告元认知意识无组间差异。

Conclusion: 通过RL控制访问时机是理论驱动、可扩展的教学范式，优于完全开放或限制，无需显式支架，兼容现有AI工具，为教育中人类-AI学习系统设计开辟新方向。
...
### [219] [AI-Mediated Communication Can Steer Collective Opinion](https://arxiv.org/abs/2605.16245)
*Stratis Tsirtsis,Kai Rawal,Chris Russell,Brent Mittelstadt,Sandra Wachter*

Main category: cs.CY

TL;DR: 生成式AI在在线平台中介人类交流时，引入方向性偏见，并通过网络放大影响集体意见。


<details>
  <summary>Details</summary>
Motivation: 现有研究关注AI对个体意见的影响，但较少关注AI中介人类交流时对集体意见形成的潜在作用。

Method: 结合实证分析和数学模型：实证测试多个LLM在编辑文本时的偏见；建立意见动态模型，分析AI作为中介时的均衡和网络放大效应。

Result: LLM在编辑争议性话题文本时表现出方向性偏见（如倾向于枪支控制、反对无神论）；数学模型表明这些偏见可通过社交网络放大并改变集体意见；审计X的“Explain this post”功能发现反堕胎偏见。

Conclusion: AI在人类交流中的偏见可能被放大并影响集体意见，平台可通过设计控制偏见，对欧盟立法有启示。
...
### [220] [Heuristic-Based Merging of HPC Traces to Extend Hardware Counter Coverage](https://arxiv.org/abs/2605.15832)
*J\'ulia Orteu Aubach,Fabio Banchelli,Marc Clasc\`a Ram\'irez,Marta Garcia-Gasulla*

Main category: cs.PF

TL;DR: 提出一种基于启发式的执行轨迹合并方法，通过匹配计算爆发点，将多次运行的硬件计数器合并为统一数据集，用于高性能计算性能预测和性能分析。


<details>
  <summary>Details</summary>
Motivation: 硬件计数器同时收集的数量有限，限制了性能建模的特征空间。

Method: 通过分析MPI结构、时序和通信模式匹配不同运行中的计算爆发点，合并多次运行的执行轨迹，构建包含更多硬件特征的统一数据集。

Result: 在MareNostrum5上验证，合并后的计数器保持可接受的精度，可用于在更丰富特征空间上训练机器学习模型。

Conclusion: 该方法无需预先选择计数器，可直接应用于机器学习模型训练，扩展了性能预测的特征维度。
...
### [221] [Non-combinatorial involutive braidings: the quantum algebra $\mathfrak{gl}_{k,m}$](https://arxiv.org/abs/2605.16121)
*Anastasia Doikou*

Main category: math.QA

TL;DR: 研究辫子方程的对合非组合解，引入gl_{k,m} Yangian，构建其最高权模和自旋链哈密顿量本征态，并在gl_{1,1}情形下给出组合基与Young tableau的联系。


<details>
  <summary>Details</summary>
Motivation: 探索辫子方程的非组合解作为置换映射的变形，并识别相关的量子代数结构。

Method: 利用对合非组合解，定义gl_{k,m} Yangian，并构建其最高权模；通过特例分析组合基与Young tableaux的对应。

Result: 构造了gl_{k,m}的最高权模，得到量子自旋链哈密顿量的本征态；gl_{1,1}的哈密顿量化为Heisenberg XX模型变体；给出gl_{1,1}最高权表示组合基，明确与Young tableau形状联系。

Conclusion: 辫子方程的非组合解导出了新的Yangian代数及其表示，为理解自旋链模型和组合表示论提供框架。
...
### [222] [Tweedie's Formula, Variance Functions, and Score-Driven Updating](https://arxiv.org/abs/2605.15902)
*Peter Reinhard Hansen,Chen Tong*

Main category: econ.EM

TL;DR: 本文通过Tweedie公式为分数驱动模型提供贝叶斯解释，证明了在共轭自然指数族中，折扣贝叶斯递归与分数驱动表示精确对应，并阐明了分数驱动的归一化作用。


<details>
  <summary>Details</summary>
Motivation: 将分数驱动模型与贝叶斯方法联系起来，统一经验贝叶斯、近似滤波、动态广义线性模型和分数驱动模型的理论框架。

Method: 利用Tweedie公式和Fisher评分，推导了条件得分在贝叶斯后验校正中的作用，并分析了共轭自然指数族中的精确表示。

Result: 在共轭自然指数族中，折扣贝叶斯递归可精确表示为分数驱动形式，其中稳态精度折扣和期望空间逆Fisher缩放后，分数驱动校正等于施加转移动力学前的贝叶斯后验均值。

Conclusion: 分数驱动模型可视为局部高斯后验校正，与贝叶斯更新在特定条件下等价，但需区分精确贝叶斯更新与局部近似。
...
### [223] [Quasi-Bayesian Local Projection Instrumental-Variables Method: Application to Renewable Energy and Electricity Prices](https://arxiv.org/abs/2605.15966)
*Masahiro Tanaka*

Main category: econ.EM

TL;DR: 提出了一种拟贝叶斯方法用于局部投影工具变量估计。


<details>
  <summary>Details</summary>
Motivation: 为了提高有限样本下的稳定性并允许联合推断。

Method: 利用GMM目标函数构建基于矩的拟后验，并采用粗糙惩罚先验平滑不同期限的脉冲响应。

Result: 模拟表明，该方法相比标准GMM降低了均方根误差，尤其在中长期期限。

Conclusion: 该方法保持了传统LP-IV的主要一阶特征，同时增强了稳定性和实用性。
...
### [224] [A Finite-State Gibbs Construction from a Recognition Cost](https://arxiv.org/abs/2605.15667)
*Megan Simons,Jonathan Washburn*

Main category: cond-mat.stat-mech

TL;DR: 基于比率成本构造，在有限结果空间上通过识别组合律（RCL）导出J函数，并利用多项计数和凸对偶恢复吉布斯权重，建立了非渐近的斯特林界和软壳约束定理，并通过三态示例与其他分布进行比较。


<details>
  <summary>Details</summary>
Motivation: 在有限结果空间上，传统吉布斯分布通过最大化香农熵获得，本文研究替代的比率成本构造，以引入新的统计物理框架。

Method: 采用归一化的达朗贝尔二次闭包（RCL），校准单位对数曲率，导出连续正分支J(x)=½(x+x⁻¹)-1，然后利用多项计数和凸对偶得到有限状态吉布斯权重，并证明F_R(q)-F_R(p)=T_R D_KL(q||p)。

Result: 推导出非渐近的斯特林界和针对实值成本的软壳约束型定理；在三状态示例中，在相同成本向量和均值成本约束下，比较了吉布斯律与平方对数、亲和力作为能量和Tsallis分布，并给出了固定RCL真实值下的样本量功效计算。
...
### [225] [Intrinsic-dimension empirical Bernstein inequalities for bounded self-adjoint operators](https://arxiv.org/abs/2605.15278)
*Diego Martinez-Taboada,Aaditya Ramdas*

Main category: math.ST

TL;DR: 提出首个基于经验方差和内在维度的算子值浓度不等式，解决了标准界需先验方差和依赖维度的问题。


<details>
  <summary>Details</summary>
Motivation: 标准算子值浓度不等式需要先验已知真实方差且依赖于环境维度，在无穷维或结构化算子中失效。

Method: 利用经验方差替代未知方差，基于内在维度而非环境维度，导出经验Bennett和Bernstein不等式。

Result: 得到可计算、无维度的界，对非各向同性随机矩阵更优，渐近达到最优oracle率，并导出内在维度的经验浓度保证。

Conclusion: 新方法克服了标准界的限制，提供了更实用和更锐利的算子值浓度不等式。
...
### [226] [Bounds on the Number of Modes of a Gaussian Mixture Density](https://arxiv.org/abs/2605.15531)
*Hien Duy Nguyen*

Main category: math.ST

TL;DR: 本文给出了高斯混合密度中非退化临界点数量的显式上界和下界，以及有限模态集时的模态数量上界。


<details>
  <summary>Details</summary>
Motivation: 研究高斯混合密度的临界点和模态数量上界与下界。

Method: 通过参考分量归一化临界点方程、精确消元、代数倒数变量、Morse理论、同方差约简、仿射秩约简等技巧。

Result: 获得了多个上界（如U_het, U_aug, U_hom, U_aug,hom）和下界（如L_bin, 线性下界等），以及临界点集合连通分量数量的显式界。

Conclusion: 提供了高斯混合密度临界点和模态数量的紧致界限，并改进了已有结果。
...
### [227] [Goodness-of-Fit Testing for Point Processes in Large Populations](https://arxiv.org/abs/2605.15814)
*Sami Umut Can,Estate V. Khmaladze,Roger J. A. Laeven*

Main category: math.ST

TL;DR: 提出一种基于酉变换的分布无关性检验方法，用于检验点过程的条件强度是否属于特定参数族。


<details>
  <summary>Details</summary>
Motivation: 需要判断观测到的点过程路径是否来自指定的参数族，但现有方法可能依赖于特定参数形式。

Method: 通过构造自然参数检验过程的酉变换，使其弱收敛到一个独立于原假设参数形式的“标准”目标过程，从而实现渐近分布无关的拟合优度检验。

Result: 蒙特卡洛模拟显示该方法在Aalen型生存过程（有无删失）、混合治愈模型和软件可靠性模型中具有良好的有限样本性能，并在人类寿命和真实软件故障数据中展示了适用性。

Conclusion: 该方法为参数点过程提供了一种有效的分布无关拟合优度检验工具。
...
### [228] [Edge-indexed network time series with graph Ornstein-Uhlenbeck dynamics](https://arxiv.org/abs/2605.15907)
*Jiaming Chen,Almut E. D. Veraart*

Main category: math.ST

TL;DR: 提出Lévy驱动的图Ornstein-Uhlenbeck模型用于边索引网络时间序列，兼具布朗运动和跳跃行为，估计性能好。


<details>
  <summary>Details</summary>
Motivation: 现有模型局限于离散时间或节点索引，需要连续时间边索引模型以处理高频金融数据中的跳跃和网络结构。

Method: 在连续时间下将图Ornstein-Uhlenbeck动力学扩展至边索引，引入Lévy噪声，采用最大似然估计。

Result: 通过仿真和实证验证，grOU模型相比基准方法提高了预测精度，减少了计算时间，且网络参数化保证了稳健性。

Conclusion: grOU模型有效处理边索引网络时间序列，特别适用于高频金融数据，具有较好预测性能和计算效率。
...
### [229] [Node-private community estimation in stochastic block models: Tractable algorithms and lower bounds](https://arxiv.org/abs/2605.15943)
*Laurentiu Marchis,Ethan D'souza,Tom\'a\v{s} Fl\'idr,Po-Ling Loh*

Main category: math.ST

TL;DR: 研究随机块模型中社区恢复的节点差分隐私算法，提出基于谱聚类的新型私有方法，并给出下界，所有算法多项式时间可计算。


<details>
  <summary>Details</summary>
Motivation: 在节点差分隐私约束下，现有边缘隐私算法直接应用会导致隐私参数ε快速增长，难以实现一致估计，需要开发稳定算法。

Method: 采用谱聚类框架，集成私有化邻接矩阵、私有PCA、私有凸优化、私有低秩矩阵估计及私有近似子空间估计；提出基于带Lipschitz扩展的指数机制采样和从无向图到有界度图的光滑投影框架。

Result: 新算法缓解了ε增长问题，在节点隐私下实现一致估计；推导了ε增长速率的下界，并证明所有方法多项式时间可计算。

Conclusion: 揭示了非标准缩放ε→∞下隐私算法分析的复杂性，提出解决方案；HGR最大相关在PAC学习精度放大中有潜在应用。
...
### [230] [Nearest-Neighbour Matching on Unbounded Supports and Covariate Shift Transfer](https://arxiv.org/abs/2605.16027)
*Simon Viel*

Main category: math.ST

TL;DR: 本文在协变量支持的最小假设下，证明基于最近邻匹配的非参数估计仍能达到通常收敛速度，引入转移性度量，并放松了处理效应估计中的分配概率假设。


<details>
  <summary>Details</summary>
Motivation: 现有非参数估计假设协变量位于紧致子集且密度有下界，需要更一般的条件。

Method: 用源分布和目标分布之间的转移性度量替代传统假设，允许分布支持在流形上且目标分布尾部更重。

Result: 在最小假设下达到通常收敛速度；转移性控制对任何估计器是必要的；处理效应估计中放松了分配概率有界假设。

Conclusion: 提出的条件更一般，适用于分布支持在流形上等广泛场景。
...
### [231] [Tests for the mean of high-dimensional data](https://arxiv.org/abs/2605.16033)
*Dietmar Ferger*

Main category: math.ST

TL;DR: 提出一种基于范数统计量的高维均值检验方法，无需协方差矩阵求逆，在固定和递增维度下均有效，并通过bootstrap实现渐近正确水平。


<details>
  <summary>Details</summary>
Motivation: 高维数据中维度可能远大于样本量，传统均值检验因协方差矩阵求逆不可行而失效，需要不依赖稀疏假设或结构条件的新方法。

Method: 使用统计量V_n = n||X_n||^2，将数据嵌入l2空间，建立渐近分布理论，并利用新中心极限定理推导bootstrap近似的渐近有效性。

Result: 所提检验在维度无显式增长限制下保持渐近水平，bootstrap过程在无稀疏性或结构假设时一致估计分布。

Conclusion: 该方法为高维均值检验提供了稳健且易行的工具，希尔伯特空间中心极限定理是核心理论支撑。
...
### [232] [Bridging Silicon and the Hippocampus: Algebro-Deterministic Memory "VaCoAl" as a Substrate for Vector-HaSH and TEM](https://arxiv.org/abs/2605.15652)
*Hiroyuki Chuma,Kanji Otsuka,Yoichi Sato*

Main category: cs.NE

TL;DR: 提出VaCoAl架构作为统一代数对象，解释海马回放中的乘法衰减和电路结构。


<details>
  <summary>Details</summary>
Motivation: Vector-HaSH、TEM和iEEG研究并行发展，缺乏共享代数框架。

Method: 基于伽罗瓦域LFSR构建VaCoAl架构，使用确定性扩散代替随机投影，定义路径积分置信比CR2。

Result: VaCoAl满足准正交性，具有匹配二阶矩、更强雪崩行为和可重现性；CR2自然建模多跳回放保真度乘法衰减。

Conclusion: VaCoAl与海马通路共享架构承诺，提供可检验预测，桥接计算神经科学和工程。
...
### [233] [Structure Abstraction and Generalization in a Hippocampal-Entorhinal Inspired World Model](https://arxiv.org/abs/2605.15733)
*Tianqiu Zhang,Muyang Lyu,Xiao Liu,Si Wu*

Main category: cs.NE

TL;DR: 提出一种受大脑启发的分层模型，同时推断潜在转换并构建预测性视觉世界模型，实现结构抽象和泛化。


<details>
  <summary>Details</summary>
Motivation: 人类能从连续高维动态中提取抽象结构，但机制不明，现有模型缺乏同时进行结构提取和世界模型构建的能力。

Method: 使用逆模型进行结构提取，结合HPC-MEC耦合模型分离关系结构（MEC）与集成情景（HPC），通过速度驱动路径积分实现预测和结构重用。

Result: 在原始变换动力学基准上展示了结构抽象能力，实现了跨上下文的鲁棒预测和结构泛化。

Conclusion: 该框架揭示了大脑启发的自监督世界模型学习如何促进可重用抽象知识的获取。
...
### [234] [Scalable neuromorphic computing from autonomous spiking dynamics in a clockless reconfigurable chip](https://arxiv.org/abs/2605.16114)
...
### [235] [Sampling pseudospectrum for data-driven matrices](https://arxiv.org/abs/2605.15234)
*Caroline Wormell*

Main category: math.NA

TL;DR: 本文提出了一种采样伪谱方法，用于评估从有限数据中提取的特征值是否为真实信号还是噪声。


<details>
  <summary>Details</summary>
Motivation: 现有算法如DMD等可以从数据中提取特征值，但无法区分哪些是有限数据误差导致的伪影，哪些是真实算子的特征值。

Method: 提出了采样伪谱P(λ)及其估计器\hat{P}(λ)，通过对有限数据样本进行再处理得到，计算高效。

Result: 该估计器能够统计检验真实特征值的位置。

Conclusion: 提供了一种严格且通用的方法，评估从有限数据中提取的模式是信号还是噪声。
...
### [236] [Amortized Energy-Based Bayesian Inference](https://arxiv.org/abs/2605.15407)
*Hojjat Kaveh,Ricardo Baptista,Andrew M. Stuart*

Main category: math.NA

TL;DR: 提出一种基于传输映射的变分贝叶斯推断方法，用于非线性逆问题，只需联合样本即可学习观测依赖的后验近似，避免密度计算和雅可比行列式。


<details>
  <summary>Details</summary>
Motivation: 经典MCMC方法对每个观测需重复推断，计算成本高，而实际中常需大量重复推断。

Method: 学习一个观测依赖的映射，将参考测度推送到近似后验；最小化平均能量距离目标，无需似然函数；在函数空间中参数化为单位映射加Cameron-Martin扰动，用神经算子表示。

Result: 在有限维和多孔介质流、地震反演两个PDE约束逆问题中，传输映射捕获后验多模态和主模式，实现对新观测的快速采样。

Conclusion: 该似然自由方法能高效近似后验，尤其适用于需多次推断的场景。
...
### [237] [Estimating Social Norm Complementarities](https://arxiv.org/abs/2605.15405)
*Eliana La Ferrara,Cheaheon Lim,Davide Viviano*

Main category: econ.GN

TL;DR: 构建社会规范选择模型，分析两种互补性（技术性与社会性），估计并比较塞拉利昂和尼日利亚女性割礼、一夫多妻制与童婚的互动关系，评估政策干预效果。


<details>
  <summary>Details</summary>
Motivation: 理解社会规范间的互补或替代关系，以解释不同习俗的共存与变迁，为政策干预提供依据。

Method: 开发包含技术互补和社会回报的规范选择模型，使用塞拉利昂和尼日利亚重复横截面数据，通过均衡响应估计参数。

Result: 社会回报显著；女性割礼与童婚呈互补（塞拉利昂更明显），一夫多妻制与童婚呈社会替代（尼日利亚更明显）。

Conclusion: 社会规范间关系因地区而异，法律改革与社会干预需考虑具体互补/替代结构以增强效果。
...
### [238] [Complexity of Non-Log-Concave Sampling in Fisher Information](https://arxiv.org/abs/2605.15859)
*Sinho Chewi,Andre Wibisono*

Main category: cs.DS

TL;DR: 研究从对数光滑非对数凹分布中采样的查询复杂度，以相对Fisher信息为度量，提出基于近端采样器的算法，利用对数凹采样的高精度保证，获得与对数凹采样相同维度依赖的复杂度，并证明了逆归约。


<details>
  <summary>Details</summary>
Motivation: 需要从非对数凹分布中高效采样，相对Fisher信息保证类似于优化中的近似稳定点。

Method: 使用近端采样器（Langevin扩散的隐式离散化），实现后向步骤即限制高斯预言机（RGO），并利用对数凹采样在Rényi散度中的高精度结果获得近似RGO实现。

Result: 非对数凹采样在相对Fisher信息下获得了与对数凹采样相同的维度依赖复杂度，改进了先前工作，并展示逆归约：任何维度依赖改进将导致高精度对数凹采样的改进。

Conclusion: 非对数凹采样可与对数凹采样达到相同维度依赖，两者存在紧密联系。
...
### [239] [Generalized Error Bounds in the Recovery of Solitary Wave Profiles](https://arxiv.org/abs/2605.15590)
*Daniel Sinambela*

Main category: math.CA

TL;DR: 研究了Constantin显式重构公式对二维无旋孤立水波的鲁棒性，给出了同时扰动波速、水深和床底压力时重构轮廓的L^2误差估计，并进行了数值验证。


<details>
  <summary>Details</summary>
Motivation: 评估Constantin公式在实际应用中受波速、水深和床底压力测量误差影响时的稳定性。

Method: 利用hodograph变换、全纯延拓和Paley-Wiener傅里叶衰减估计，推导L^2误差界，并设计数值实验。

Result: 获得重构轮廓的L^2误差估计，误差对扰动大小呈次线性依赖。

Conclusion: 公式在适当条件下具有鲁棒性，数值结果证实了理论分析。
...
### [240] [Nonlinear stability of continuously self-similar naked singularities for the Einstein-scalar field equations II: linearized stability](https://arxiv.org/abs/2605.16095)
*Jaydeep Singh,Weihao Zheng*

Main category: gr-qc

TL;DR: 本文证明了连续自相似裸奇点在相同正则性扰动下的线性化稳定性，揭示了正则性在蓝移不稳定性机制中的关键作用。


<details>
  <summary>Details</summary>
Motivation: 先前已知这些裸奇点在粗糙扰动下不稳定，但正则性水平如何影响蓝移不稳定性机制尚不清楚。

Method: 对裸奇点时空进行线性分析，考虑与背景相同正则性的扰动。

Result: 建立了线性化稳定性，表明在背景正则性水平下蓝移不稳定性机制未被触发。

Conclusion: 该线性分析为非线性稳定性提供了基础，与姊妹论文共同证明了这些裸奇点的非线性稳定性。
...
### [241] [Non-minimal fluid Lagrangian couplings](https://arxiv.org/abs/2605.16220)
*Christian G. Boehmer,Erik Jensko,Eissa Al-Nasrallah*

Main category: gr-qc

TL;DR: 本文仔细推导了非最小耦合引力模型中的场方程和流体方程，揭示了Schutz和Brown拉格朗日表述的非等价性，并解释了非最小耦合对热力学量的影响。


<details>
  <summary>Details</summary>
Motivation: 通过将物质拉格朗日量直接耦合到引力拉格朗日量，构造无需额外源项即可解释暗能量或暗物质的理论。

Method: 对描述理想流体的物质拉格朗日量进行变分公式化，推导引力场方程和完整的流体方程，并定义热力学量。

Result: 证明了Schutz和Brown的拉格朗日表述在非最小耦合模型中不等价。

Conclusion: 非最小耦合影响温度、化学势和数密度等热力学量，有助于理解此类模型的物理效应。
...
### [242] [Nonlinear stability of continuously self-similar naked singularities for the Einstein-scalar field equations I: main results](https://arxiv.org/abs/2605.16235)
*Weihao Zheng*

Main category: gr-qc

TL;DR: 首次证明了广义相对论中一族连续自相似裸奇点解在同样正则性扰动下的非线性稳定性，强调了函数框架在弱宇宙监督假设中的关键作用。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明这些裸奇点在粗糙扰动下会形成黑洞，验证了弱宇宙监督；但本文发现在同样正则性的一般扰动下它们实际上是稳定的，这一矛盾凸显了函数框架的重要性。

Method: 基于伴随论文的线性化稳定性结果，采用局部Hölder拓扑度量初始扰动。

Result: 获得了这些裸奇点时空在一般扰动下的非线性稳定性。

Conclusion: 结果凸显了函数框架在弱宇宙监督猜想表述中的决定性作用。
...
### [243] [Effective Harness Engineering for Algorithm Discovery with Coding Agents](https://arxiv.org/abs/2605.15221)
*Yoichi Ishibashi,Taro Yano,Masafumi Oyamada*

Main category: cs.SE

TL;DR: 研究了在算法发现中，利用大语言模型和进化搜索时，执行基础设施（harness）的设计对成功的影响，发现更少但更深入思考的算法优于大量浅层算法，且模型能力越强越容易出现评估漏洞。


<details>
  <summary>Details</summary>
Motivation: 尽管AlphaEvolve和FunSearch展示了结合LLM与进化搜索的潜力，但发现成功不仅取决于模型能力，还显著受执行基础设施设计的影响，因此需要研究有效的harness设计。

Method: 通过三个问题（固定代币预算下生成少量深度思考算法还是大量浅层算法、如何处理评估漏洞、如何安全并行执行），利用Vesper框架在Circle Packing问题上进行评估。

Result: 在相同代币预算下，生成更少但更深入思考的算法获得了更高的分数，即提升每个个体的质量比增加进化代数更具预算效率。同时，更强大的模型更可能产生评估漏洞，因此随着模型规模扩大，漏洞检测愈发必要。

Conclusion: 算法发现中，应注重算法质量而非数量，且需加强评估漏洞检测以应对更强模型带来的风险。
...
### [244] [PBT-Bench: Benchmarking AI Agents on Property-Based Testing](https://arxiv.org/abs/2605.15229)
*Lucas Jing,Xinqi Wang,Liao Zhang,Simon S. Du*

Main category: cs.SE

TL;DR: 提出了PBT-Bench基准，用于评估基于属性的测试能力，包含100个来自40个Python库的复杂bug问题。


<details>
  <summary>Details</summary>
Motivation: 现有代码基准无法隔离基于属性测试的独特技能（从文档推导语义不变量并设计输入生成策略）。

Method: 构建PBT-Bench，包含100个问题，每个注入多个语义bug（共365个），要求使用Hypothesis @given策略；在两种提示模式下评估8种LLM。

Result: PBT引导提示下召回率42.1%-83.4%，开放基线31.4%-76.7%；假设框架使中等模型提升超20个百分点，但对最强模型增益小甚至负增益。

Conclusion: PBT-Bench揭示了模型在文档驱动语义推理上的能力差异，发布了基准和评估语料以支持后续研究。
...
### [245] [Hydra: Efficient, Correct Code Generation via Checkpoint-and-Rollback Support](https://arxiv.org/abs/2605.15238)
*Alexander Du,Jianjun Ou,Danyang Zhuo,Matthew Lentz*

Main category: cs.SE

TL;DR: Hydra 是一种在代码生成过程中高效恢复静态错误的系统，通过异步检查与检查点回滚，显著降低延迟和 token 消耗。


<details>
  <summary>Details</summary>
Motivation: 现有修复静态错误的方法延迟高、token 消耗大，后验修复延迟错误检测，约束语义解码逐 token 开销大且修复能力有限。

Method: Hydra 使检查与生成异步进行，避免正确代码的开销；支持检查点与回滚，实现针对性修复，避免重新生成和检查有效前缀；并改造 Clang C/C++ 编译器以支持此机制。

Result: 与后验修复相比，Hydra 在遇到静态错误的 C/C++ 代码生成任务中，延迟最多降低 71%，token 消耗最多降低 70%。

Conclusion: Hydra 能够在代码生成时高效地恢复静态错误，显著优于现有方案。
...
### [246] [Runtime-Structured Task Decomposition for Agentic Coding Systems](https://arxiv.org/abs/2605.15425)
*Shubhi Asthana,Bing Zhang,Chad DeLuca,Hima Patel,Ruchi Mahindru*

Main category: cs.SE

TL;DR: 提出运行时结构化任务分解方法，通过可执行控制逻辑管理任务划分和执行流，仅用LLM进行判断，输出按模式验证，在软件工程任务中显著降低重试成本。


<details>
  <summary>Details</summary>
Motivation: 现有智能编码系统使用大型提示，导致行为僵化、调试困难、重试成本高，因为失败常需重跑整个工作流。

Method: 运行时结构化任务分解：通过可执行控制逻辑而非提示结构管理任务划分和执行流，LLM仅用于聚焦判断任务，输出按预定义模式验证后再进入下游执行。

Result: 在Kubernetes根因分析和多文件调试任务中，运行时分解重试成本分别为436±132 tokens和460 tokens，相比单块系统降低51.7%，相比静态分解降低73.2%。

Conclusion: 该方法提升了智能编码系统的效率、可调试性和运行可靠性。
...
### [247] [Position: Early-Stage Quality Assurance in Annotation Pipelines Is More Cost-Effective Than Late-Stage Validation](https://arxiv.org/abs/2605.15714)
...
TL;DR: 本文主张机器学习社区应优先关注标注流程中的早期质量保证，而非当前主流的后期验证，借鉴软件工程中的'左移'原则。


<details>
  <summary>Details</summary>
Motivation: 数据质量瓶颈限制了基础模型的改进，但现有研究只关注验证方法而忽视验证时机，而时机对错误率和成本有巨大影响，这与软件工程中的'左移'原则相悖。

Method: 提出三种QA触发点（标注前T0、标注后T1、审核后T2）的分类法，并构建参数化错误传播模型来形式化时机对错误率和经济性的影响。同时调查了47篇近期论文。

Result: 调查发现仅4%的论文报告了验证时机，模型表明时机是影响错误率和成本的关键设计变量，而不仅仅是事后配置。

Conclusion: 社区需采取三项行动：研究者应报告QA时机配置，平台将时机作为首要参数，社区开展控制实验以测量各阶段的检测率。
...
### [248] [RoadmapBench: Evaluating Long-Horizon Agentic Software Development Across Version Upgrades](https://arxiv.org/abs/2605.15846)
...
TL;DR: 提出RoadmapBench基准，包含115个长周期多目标编码任务，评估前沿模型发现表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有基准侧重于单问题错误修复，无法反映真实软件开发中的长周期多文件协调工作。

Method: 基于17个仓库5种编程语言的真实开源版本升级，构建115个任务，每个任务要求代理根据多目标路线图实现目标版本功能，中位数修改3700行代码涉及51个文件。

Result: 最强模型Claude-Opus-4.7仅解决39.1%的任务，最弱模型仅5.2%，与现有错误修复基准形成鲜明对比。

Conclusion: 长周期软件开发在很大程度上仍是一个未解决的问题。
...
### [249] [XSearch: Explainable Code Search via Concept-to-Code Alignment](https://arxiv.org/abs/2605.16046)
*Yiming Liu,Ruofan Liu,Yun Lin,Zicong Zhang,Weiyu Kong,Pengnian Qi,Xiao Cheng,Weinan Zhang,Qianxiang Wang,Linpeng Huang*

Main category: cs.SE

TL;DR: 提出一种可解释的代码搜索框架XSearch，通过概念对齐实现更好的泛化和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有语义代码搜索方法可解释性和泛化能力差，尤其在分布偏移下表现严重下降。

Method: 将代码搜索重构为演绎概念对齐问题，先识别查询中的功能概念，再显式对齐到代码语句，训练编码器时加入概念对齐目标。

Result: 在CodeSearchNet上训练的GraphCodeBERT模型，在八个最先进检索器上，分布外基准性能从0.02提升到0.33（15倍），并一致优于高达7B参数的基线。用户研究显示概念对齐解释使用户更快更准评估结果。

Conclusion: XSearch通过显式概念对齐显著提升了代码搜索的可解释性和分布外泛化能力。
...
### [250] [PerfCodeBench: Benchmarking LLMs for System-Level High-Performance Code Optimization](https://arxiv.org/abs/2605.15222)
*Huihao Jing,Wenbin Hu,Haochen Shi,Hanyu Yang,Sirui Zhang,Shaojin Chen,Haoran Li,Yangqiu Song*

Main category: cs.SE

TL;DR: 提出了PerfCodeBench，一个用于评估大语言模型在高性能代码优化方面能力的可执行基准，发现当前模型在性能优化上存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 现有代码基准主要关注正确性或算法解题能力，缺乏对实际系统级性能优化的评估。

Method: 构建包含系统级实现选择、硬件感知优化和性能瓶颈处理的基准任务，每个任务有正确性检查、基线和参考优化实现。

Result: 评估显示模型生成的代码与专家优化实现之间存在明显差距，尤其在并行和GPU任务上，且跨语言鲁棒性不足。

Conclusion: 需要注重性能的评估，推动LLM从生成正确代码向生成高效系统软件发展。
...
### [251] [BootstrapAgent: Distilling Repository Setup into Reusable Agent Knowledge](https://arxiv.org/abs/2605.15815)
*Sihan Fu,Oucheng Liu,Shiyuan Wang,Jin Shi,Chengkun Wei*

Main category: cs.SE

TL;DR: 提出BootstrapAgent框架，将代码仓库的启动过程转化为可复用的知识契约，大幅提升成功率并降低下游成本。


<details>
  <summary>Details</summary>
Motivation: 代码代理在处理不熟悉仓库时需要昂贵的启动探索，且探索所得知识无法复用，导致每次任务都需重复从头开始。

Method: 多代理框架，通过证据提取、结构化规划、基于Docker的确定性验证和轨迹驱动修复，生成持久化的.bootstrap契约；并引入温修复与干净回放加速迭代调试，以及增量修复与健全性检查防止奖励破解。

Result: 在三个基准测试上成功率达92.9%，超过基线10%以上，同时减少下游代理令牌使用25.9%和构建时间22.3%。

Conclusion: BootstrapAgent能有效将启动探索中的启发性知识固化为可复用契约，显著提升代码代理在陌生仓库上的任务效率与资源利用率。
...
### [252] [Clock-state olfactory search in turbulent flows using Q-learning: The geometry of plume recovery](https://arxiv.org/abs/2605.15938)
*Marco Rando,Robin A. Heinonen,Yujia Qi,Agnese Seminara*

Main category: physics.bio-ph

TL;DR: 使用表格Q学习训练仅依赖上次气味后时钟记忆的嗅觉搜索智能体，学习昆虫行为策略，但缺乏自适应能力；增加灵活性可提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 在湍流中有效利用气味历史找到气味源，需结合稳健导航策略。

Method: 采用表格Q学习，智能体记忆仅为上次气味后的运行时钟，学习可解释策略（突进、投掷、顺风返回）。

Result: 在直接数值模拟的湍流数据上表现良好，但策略无法适应局部间歇性。

Conclusion: 提供更多灵活性（如记忆更多信息）可改善自适应能力，提高鲁棒性。
...
### [253] [Stochastic Compositional Optimization via Hybrid Momentum Frank--Wolfe](https://arxiv.org/abs/2605.15350)
*El Mahdi Chayti*

Main category: math.OC

TL;DR: 提出混合动量随机Frank-Wolfe算法，用于解决外函数非光滑的随机组合优化问题，达到最优收敛率。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设外函数连续可微，但许多实际应用如稳健最大损失、条件风险价值等不满足此条件。

Method: 结合动量Jacobian跟踪器和泰勒修正函数跟踪器，将随机线性化输入广义线性最小化预言机。

Result: 在非凸目标下达到O(K^{-1/4})收敛率，匹配投影自由单样本随机方法的最优复杂度，且能处理重尾噪声。

Conclusion: 该方法去掉了外函数光滑性假设，实现了最优收敛率，扩展了适用范围。
...
### [254] [Stochastic Non-Smooth Convex Optimization with Unbounded Gradients](https://arxiv.org/abs/2605.15522)
*Dmitry Kovalev*

Main category: math.OC

TL;DR: 针对一类更现实的广义Lipschitz函数，证明了裁剪AdamW在凸随机优化中优于SGD和AdaGrad，并分析了其梯度累积机制的关键作用。


<details>
  <summary>Details</summary>
Motivation: 现有非光滑优化理论大多假设目标函数梯度一致有界，这在实际中不成立。因此引入梯度范数受最优间隙仿射函数控制的广义Lipschitz函数类，以更贴近现实。

Method: 通过新的收敛分析，比较了多种现有算法（SGD、AdaGrad、AdamW等）在凸随机广义Lipschitz优化问题上的表现。重点分析了带裁剪更新的AdamW，并研究了其指数加权梯度累积（而非简单平均）的作用。

Result: 裁剪AdamW在理论上取得了比SGD和AdaGrad更优的全局收敛率；在广义光滑假设下也具有普适性；同时分析了其对角及矩阵预条件器的收敛性，并将结果推广到quasar-凸设定。

Conclusion: 裁剪AdamW是一种通用且高效的随机优化方法，尤其适用于梯度范数可能随最优间隙增长的现实场景。
...
### [255] [Testing properties of trees in graphical models with covariance queries](https://arxiv.org/abs/2605.15996)
*Sofiya Burova,Francisco Calvillo,G\'abor Lugosi,Piotr Zwiernik*

Main category: stat.ML

TL;DR: 本文研究高维图模型中树结构的全局属性测试，使用协方差查询，设计随机测试以次二次查询次数高效测试树叶数、最大度、典型距离和直径等属性。


<details>
  <summary>Details</summary>
Motivation: 在高维图模型中，重建整个树结构代价高昂，但测试全局结构属性可能更高效，需探索其可测试性。

Method: 采用协方差查询模型，针对树结构设计随机测试算法，利用子二次查询次数评估属性是否满足阈值和容差参数。

Result: 获得每个属性的明确查询复杂度界限，依赖于目标阈值和容差参数。

Conclusion: 树结构的某些全局属性可以高效测试，即使重建整个树成本较高，为图模型测试提供新途径。
...
### [256] [$\alpha$-TCAV: A Unified Framework for Testing with Concept Activation Vectors](https://arxiv.org/abs/2605.15688)
*Ekkehard Schnoor,Jawher Said,Malik Tiomoko,Wojciech Samek,Alexander Jung*

Main category: stat.ML

TL;DR: 分析了概念激活向量（CAV）的不稳定性，指出标准TCAV分数因使用不连续指示函数导致方差不衰减，提出α-TCAV框架并用平滑函数替代指示函数，给出了调参指导和实用建议。


<details>
  <summary>Details</summary>
Motivation: CAV在实际应用中受限于统计不稳定性，标准TCAV分数存在根本缺陷，需要更稳定和理论严谨的概念可解释性方法。

Method: 推导了多种CAV类的分布，识别了TCAV分数的不连续指示函数问题，提出了α-TCAV框架，用参数化平滑函数统一TCAV和Multi-TCAV，并给出了调参原则。

Result: α-TCAV能以更低计算成本模仿Multi-TCAV，或获得校准的贝叶斯最优概率度量；揭示了现有最佳选择缺乏理论依据；建议将全部采样预算用于单个CAV而非多个。

Conclusion: 研究表明使用平滑函数可提升稳定性，建议采用α-TCAV并合理调参，同时挑战了常用采样策略，推荐集中采样于单个CAV。
...
### [257] [On Kernel Eigen-alignments of KRR: Reconstruction and Generalization](https://arxiv.org/abs/2605.15240)
*Yang Liu,Ernest Fokoue,Richard Lange,Daniel Krutz*

Main category: stat.ML

TL;DR: 本文研究了核矩阵与学习目标之间的特征对准在鲁棒泛化中的关键作用，建立了核方法泛化性能与矩阵特征向量和特征值估计的直接联系，并从特征值/特征向量估计角度推导了泛化界。


<details>
  <summary>Details</summary>
Motivation: 现有工作对核方法泛化性能的理解不够直观，需要更清晰的联系以及更少的假设。

Method: 通过分析核矩阵扰动对特征向量/特征值估计误差的影响，建立泛化误差与估计稳定性之间的联系，并聚焦于有限样本设置。

Result: 发现当核矩阵高秩时，重建误差趋近于零，但其对泛化的预测能力有限；强泛化需要增加特征向量对准、特征值大小或特征值间隙。

Conclusion: 特征对准、特征值大小及间隙是决定核方法泛化能力的关键因素，从特征估计角度提供了新的泛化界。
...
### [258] [Harnessing Unimodality in Semiparametric Contextual Pricing via Oracle Price Map Learning](https://arxiv.org/abs/2605.15411)
*Yingying Fan,Yuxuan Han,Jinchi Lv,Xiaocong Xu,Zhengyuan Zhou*

Main category: stat.ML

TL;DR: 提出ORBIT策略，在半参数标量指数估值模型中利用结构光滑性实现上下文动态定价，取得最优后悔界。


<details>
  <summary>Details</summary>
Motivation: 在半参数标量指数估值模型中，挖掘潜在的一维oracle价格映射的光滑结构，以提高动态定价效率。

Method: 采用模块化粗到细策略ORBIT，通过标量引导指数局部定价，在信任区域内使用bandit凸优化学习局部多项式近似；线性效用模型结合自适应椭圆探索。

Result: 对于线性效用模型，后悔界为~O(T^{(2β-1)/(4β-3)} + √(dT))，且下界匹配，非参数项达到极小值最优；扩展到稀疏高维线性与非参数Hölder效用。

Conclusion: ORBIT策略利用结构光滑性有效降低后悔，标量引导接口具有通用性，能处理多种效用模型。
...
### [259] [MaxSketch: Robust Distinct Counting in Streams via Random Projections](https://arxiv.org/abs/2605.15571)
*Nikos Tsikouras,Constantine Caramanis,Christos Tzamos*

Main category: stat.ML

TL;DR: 针对高维噪声流中近似重复元素的计数问题，提出MaxSketch，利用随机高斯投影，在几何结构假设下将内存复杂度从Θ(√n)降至O(log n/ε^2)，实验验证准确。


<details>
  <summary>Details</summary>
Motivation: 经典流算法如HyperLogLog要求元素完全重复，无法处理高维噪声下的近似重复；现有鲁棒计数方法在一般度量空间需Θ(√n)内存，本文探索利用常见表示中的几何结构降低复杂度。

Method: 设计MaxSketch，一种基于随机高斯投影的最大线性草图（max-linear sketch），利用潜在对象的几何结构（如低维流形）实现高效计数。

Result: 在几何结构假设下，仅需m = O(log n/ε^2)个随机投影（即O(log n/ε^2)内存）即可在(1+ε)因子内估计真实不同对象数；图像流实验证实MaxSketch准确且泛化。

Conclusion: 几何结构可显著降低不同元素计数问题的复杂度，桥接了经典流算法与现代表示学习。
...
### [260] [Pessimistic Risk-Aware Policy Learning in Contextual Bandits](https://arxiv.org/abs/2605.15620)
...
### [261] [Unsupervised Domain Shift Detection with Interpretable Subspace Attribution](https://arxiv.org/abs/2605.15920)
*Sebastian Springer,Alessandro Laio*

Main category: stat.ML

TL;DR: 开发了一种检测域漂移的工具，能够识别高维特征空间中的局部密度异常，并追溯至一小部分特征，实现可解释性。还提供了补偿域漂移的协议，并在基准测试和心电图数据上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 数据集间的分布差异（域漂移）会影响模型性能，需要一种可解释的检测和补偿方法。

Method: 使用局部密度异常检测算法识别域漂移，然后确定异常最显著的特征子空间，并通过提取无残差分布差异的子集来补偿漂移。

Result: 在20维基准测试中准确恢复漂移模式；在ECG数据中检测到设备引起的漂移，提取了代表性子集，并识别出与采集对比相关的特征。

Conclusion: 密度漂移检测和子空间归因为发现隐藏的队列偏差提供了实用框架。
...
### [262] [Explainable AI Isn't Enough! Rethinking Algorithmic Contestability](https://arxiv.org/abs/2605.16041)
*Timo Freiesleben,Kristof Meding,Gunnar K\"onig*

Main category: stat.ML

TL;DR: 提出算法可争议性的操作定义作为追索的补充，识别三种可推翻决策的证据类型，并分析现有法律框架。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统做出影响个人的决策，但XAI主要关注追索，忽视了纠正错误决策的可争议性，这具有伦理和法律重要性。

Method: 给出可争议性的操作定义，与追索对比；识别三种证据：预测多重性、不正确特征值、被忽略的推翻证据。

Result: 标准XAI解释（反事实、LIME、Anchors）仅能揭示邻近错误，不足以推翻决策；而三种证据使决策道德上站不住脚，从而实现成功争议。

Conclusion: 个人已有某些法律权利获取这些证据，并与欧盟立法框架相联系。
...
### [263] [A numerical study into neural network surrogate model performance for uncertainty propagation](https://arxiv.org/abs/2605.16078)
*Noah Wade,Kirubel Teferra*

Main category: stat.ML

TL;DR: 本文研究了神经网络代理模型捕捉随机解的尾部分布能力，发现最差预测误差比平均场误差大一个数量级，并提出了识别异常样本及处理外推误差的方法。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注神经网络代理模型表示确定性样本或平均场解的能力，忽视了分布尾部的性能，而尾部预测对不确定量化至关重要。

Method: 以高度随机源项的热传导方程为规范问题，比较前馈全连接网络和DeepONet架构，采用数据驱动和物理信息损失函数，并识别需要外推的样本。

Result: 最差预测误差比平均场误差高一个数量级，极端样本误差源于网络外推训练数据范围；全连接网络结合弱形式残差损失取得最佳预测精度。
...
### [264] [Skew-adaptive conformal prediction](https://arxiv.org/abs/2605.16145)
*Paulo C. Marques F.,Helton Graziadei*

Main category: stat.ML

TL;DR: 提出一种偏斜自适应分割共形预测方法，通过不对称区间和逆双曲正弦变换学习偏斜，保留边际有效性并提高区间效率。


<details>
  <summary>Details</summary>
Motivation: 现有共形预测区间通常对称或不能适应局部偏斜，需要更高效的区间。

Method: 基于不对称区间族，使用规范方法推导一致性分数，用逆双曲正弦变换的符号缩放残差作为训练目标，额外模型学习不确定性随特征空间倾斜。

Result: 实验表明在预测区间效率上优于缩放分数和共形分位数回归，且提出的估计器准确匹配测试样本的平均宽度比。

Conclusion: 该方法在保持有限样本边际有效性的同时，适应局部尺度和偏斜，提高了区间效率。
...
### [265] [A Scalable Nonparametric Continuous-Time Survival Model through Numerical Quadrature](https://arxiv.org/abs/2605.16208)
*Chaeyeon Lee,Sehwan Kim,Hyungrok Do*

Main category: stat.ML

TL;DR: 提出QSurv，一个可扩展的深度学习框架，用于非参数连续时间生存建模，通过高斯-勒让德数值积分和时变低秩适应实现高效训练和复杂风险动态捕捉。


<details>
  <summary>Details</summary>
Motivation: 灵活连续时间生存建模对于捕捉高维数据中复杂时变风险动态至关重要，但由于似然估计中不可解积分的存在，训练此类模型仍具挑战。

Method: 提出基于高斯-勒让德数值积分的训练目标，以高阶精度近似累积风险函数，并引入时变低秩自适应机制，通过低秩更新动态调节网络权重以处理非平稳风险。

Result: 在合成基准、大规模真实表格数据集和高维医学影像任务中，QSurv达到竞争性预测性能，并在瞬时风险函数估计上具有优势。

Conclusion: QSurv实现了对时变风险模式更可解释的表征，为连续时间生存分析提供了有效工具。
...
### [266] [Domain-Independent Game Abstraction using Word Embedding Techniques](https://arxiv.org/abs/2605.15543)
*Juho Kim,Tuomas Sandholm*

Main category: cs.GT

TL;DR: 提出一种领域无关的游戏抽象方法，利用词嵌入技术将动作向量化并聚类，实验证明有效但未超越专用算法。


<details>
  <summary>Details</summary>
Motivation: 现有游戏抽象方法大多领域特定（如扑克），难以通用，需要一种领域无关的方法。

Method: 将每个动作视为单词，游戏数据视为语料库，训练动作的词向量表示，然后进行聚类实现游戏抽象。

Result: 该方法有效，但性能未能超越针对特定游戏设计的专用算法。

Conclusion: 本文展示了词嵌入技术在游戏抽象中的潜力，可作为一种通用的抽象框架。
...
### [267] [Misspecified Explore-then-Exploit Leads to Supra-Competitive Prices](https://arxiv.org/abs/2605.16064)
*Jackie Baek,Vivek F. Farias,Farrell Wu*

Main category: cs.GT

TL;DR: 研究简单算法定价系统能否在多企业市场中产生超竞争价格，发现探索阶段在纳什价格同侧定价可导致价格高于纳什均衡，甚至达到垄断水平。


<details>
  <summary>Details</summary>
Motivation: 探究算法定价是否会导致隐式合谋，无需明确沟通即可产生高于竞争的价格。

Method: 使用先探索后开发的管道，企业先随机定价，然后基于自身历史数据用错误设定的垄断模型估计需求并短视定价，通过流体极限ODE分析收敛性。

Result: 当企业在纳什价格同侧类似范围内探索时，收敛到高于纳什均衡的超竞争价格；对称探索下价格可达垄断水平。模拟在真实市场中稳健成立。

Conclusion: 简单算法定价可系统性地产生超竞争结果，需警惕算法合谋风险。
...
### [268] [Symplectic Neural Operators for Learning Infinite Dimensional Hamiltonian Systems](https://arxiv.org/abs/2605.15881)
*Yeang Makara,Yusuke Tanaka,Takashi Matsubara,Takaharu Yaguchi*

Main category: math.DS

TL;DR: 提出了一种保持哈密顿系统辛结构的神经算子（SNO），理论证明其长期稳定性，实验显示优于非保结构算子。


<details>
  <summary>Details</summary>
Motivation: 标准数据驱动架构在模拟无穷维哈密顿系统时面临计算和结构挑战。

Method: 设计了保持辛结构的神经算子架构，并理论刻画其辛性与长期稳定性。

Result: 数值实验表明SNO在规范哈密顿PDE上能量行为优于非保结构神经算子。

Conclusion: 辛结构保持结合学习精度可实现长期稳定模拟。
...
### [269] [Autonomous Intelligent Agents for Natural-Language-Driven Web Execution with Integrated Security Assurance](https://arxiv.org/abs/2605.15281)
*Vinil Pasupuleti,Siva Rama Krishna Varma Bayyavarapu,Shrey Tyagi*

Main category: cs.CR

TL;DR: 提出一个AI自主测试框架，通过五种策略提高网页测试成功率并扩展到自然语言驱动的安全测试。


<details>
  <summary>Details</summary>
Motivation: 现代网页测试套件因UI重构、时序变化等失效，导致开发者放弃维护。

Method: 结合导航可靠性、上下文选择器生成、生成后验证、智能等待和失败学习五种策略，采用容器化解耦架构；安全测试通过自然语言描述攻击场景自动生成OWASP Top 10测试。

Result: 脚本生成成功率从55%提升至93%，导航故障减少8倍，时序竞态问题消除80%，测试创建时间减少75%；安全测试对认证绕过和输入验证缺陷检出率分别为85%和95%，假阳性率低于12%。

Conclusion: 该框架显著提升测试可靠性和效率，自然语言驱动的安全测试是新颖贡献。
...
### [270] [Hidden in Memory: Sleeper Memory Poisoning in LLM Agents](https://arxiv.org/abs/2605.15338)
*Sidharth Pulipaka,Stanislau Hlebik,Leonidas Raghav,Sahar Abdelnabi,Vyas Raina,Ivaxi Sheth,Mario Fritz*

Main category: cs.CR

TL;DR: 提出并研究了睡眠记忆中毒攻击，通过操纵外部上下文使AI助手存储虚假用户记忆，从而在后续对话中影响行为。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型持久记忆功能带来安全风险，恶意内容可污染记忆并长期影响。

Method: 设计延迟攻击，在外部文档或网页中注入虚假信息，测试是否被写入、检索并用于引导对话。

Result: 在GPT-5.5和Kimi-K2.6上，中毒记忆写入率高达99.8%和95%，成功检索后导致60-89%的恶意操作。

Conclusion: 持久记忆成为跨多次对话的长期攻击面。
...
### [271] [Detecting Privilege Escalation in Polyglot Microservices via Agentic Program Analysis](https://arxiv.org/abs/2605.15569)
*Penghui Li,Hong Yau Chong,Yinzhi Cao,Junfeng Yang*

Main category: cs.CR

TL;DR: 提出Neo框架，结合大语言模型和经典程序分析，检测微服务系统中的特权升级漏洞，在25个开源应用中发现了24个零日漏洞，精度81%召回85%。


<details>
  <summary>Details</summary>
Motivation: 微服务架构中权限控制复杂，存在特权升级风险，现有检测方法因跨服务交互、多语言代码库和多样化特权操作而难以有效检测。

Method: Neo采用基于LLM的代理动态生成分析计划、调整代码搜索策略并验证语义，同时开发了可跨服务跨语言扩展的代码搜索原语。

Result: 在25个微服务应用（7种语言、620万行代码）上检测出24个零日特权升级漏洞，精度81.0%，召回85.0%，优于现有程序分析和代理方案。

Conclusion: Neo能有效检测特权升级漏洞，且具有可扩展性，在其他领域和漏洞类型上额外发现18个零日漏洞。
...
### [272] [Who Owns This Agent? Tracing AI Agents Back to Their Owners](https://arxiv.org/abs/2605.16035)
*Ruben Chocron,Doron Jonathan Ben Chayim,Eyal Lenga,Gilad Gressel,Alina Oprea,Yisroel Mirsky*

Main category: cs.CR

TL;DR: 提出一种基于canary的协议，将AI代理的行为归因到其部署账户，解决责任追踪缺口。


<details>
  <summary>Details</summary>
Motivation: 当前无法可靠地将有害AI代理追溯至部署账户，导致善意操作者意外部署不当代理和恶意操作者故意滥用代理时存在责任缺口。

Method: 授权方在代理交互流中注入canary，供应商在窄窗口内搜索会话日志以恢复初始会话和账户；对抗场景中设计稳健canary，使其难以被压制而不损害代理任务性能。

Result: 实验表明该方法在多种场景下（包括真实世界代理）可靠、稳健且可扩展，适用于供应商端部署。

Conclusion: 首次正式定义代理归因问题并提供实用解决方案，通过canary机制实现可靠归因，对抗场景下形成防御方优势。
...
### [273] [Enabling Adversarial Robustness in AI Models through Kubeflow MLOps](https://arxiv.org/abs/2605.15249)
*Stavros Bouras,Ioannis Korontanis,Antonios Makris,Konstantinos Tserpes*

Main category: cs.CR

TL;DR: 本文提出一种在Kubernetes集群中保护AI模型的安全架构，通过Kubeflow MLOps自动检测和防御推理阶段的对抗攻击。


<details>
  <summary>Details</summary>
Motivation: 虽然Kubernetes提供强大的基础设施编排，但缺乏针对AI模型的安全机制，需要专门保护。

Method: 集成Kubeflow MLOps，使用FGSM攻击检测，并在检测到精度下降时自动部署PGD对抗训练防御。

Result: 实验表明，部署的防御显著恢复了因攻击导致的精度损失，增强了模型鲁棒性。

Conclusion: 所提架构能有效保护Kubernetes中部署的AI模型免受对抗攻击，保持准确性和可靠性。
...
### [274] [A Multi-Layer Cloud-IDS Pipeline with LLM and Adaptive Q-Learning Calibration](https://arxiv.org/abs/2605.15889)
*Syed Waqas Ali,Ibrar Ali Shah,Farzana Zahid,Daniyal Munir,Hans D. Schotten*

Main category: cs.CR

TL;DR: 提出一种基于强化学习的置信度感知多层入侵检测系统，通过自适应阈值和LLM选择性调用，平衡检测性能与成本。


<details>
  <summary>Details</summary>
Motivation: 云环境安全面临挑战，传统IDS依赖机器学习，在真实部署中性能下降，且分层架构和动态环境导致检测困难。

Method: 实现三层（网络、主机、管理程序）检测，每层使用ML模型，通过置信度门控（Gate-1阈值门、Gate-2 Chroma记忆匹配门）处理低置信度事件，未解决事件提交LLM分析，Gate-3使用LLM置信度或加权融合决定攻击提升，不确定事件保留审查。使用强化学习学习自适应阈值。

Result: 与静态阈值相比，LLM调用减少58.78%，准确率88.68%，精度85.29%，召回84.72%，F1 85.00%。网络层和管理程序层准确率分别达98.02%和97.08%。

Conclusion: 系统能自适应学习阈值，降低LLM使用成本，同时保持高效的检测性能，在多层环境中实现均衡检测。
...
### [275] [Covert Bayesian Quickest Change Detection](https://arxiv.org/abs/2605.16140)
*Yun-Feng Lo,Matthieu R. Bloch*

Main category: cs.IT

TL;DR: 研究了贝叶斯无限时域下的隐蔽快速变化检测问题，提出了预期隐蔽预算（ECB）度量，建立了二阶渐近逆界并提出了匹配的可达性方案。


<details>
  <summary>Details</summary>
Motivation: 合法实体需通过主动探测信道快速检测变化，同时避免被监控信道的对手发现其探测行为，需要在检测延迟与隐蔽性之间取得平衡。

Method: 引入预期隐蔽预算（ECB）作为隐蔽性度量，限制主动与被动探测观测序列的相对熵上界；采用恒定探测概率的Shiryaev型策略作为可达性方案。

Result: 在虚警概率趋于零且ECB约束为正的条件下，给出了平均检测延迟的二阶渐近逆界，并量化了最大平方根阶隐蔽探测增益；所提策略匹配该逆界。

Conclusion: 通过数值例子验证了理论结果，表明所提方法在隐蔽性约束下实现了最优检测延迟。
...
### [276] [Breaking the Finite-Sample Barrier in Entropy Coupling](https://arxiv.org/abs/2605.16229)
*Shahab Asoodeh,Jun Chen*

Main category: cs.IT

TL;DR: 引入最小列表熵耦合，揭示依赖观测能打破有限样本障碍，零熵可在有限样本后精确达到。


<details>
  <summary>Details</summary>
Motivation: 独立观测在有限样本下无法完全消除不确定性，依赖观测可能提供更高效的信息融合。

Method: 通过允许Y_i任意依赖但固定边缘分布，定义最小列表熵耦合；给出零熵的充要条件；开发贪心算法。

Result: 零熵在温和条件下可通过O(log(1/P_min))个观测实现；贪心算法具有单调逼近保证。

Conclusion: 该框架形式化了分布匹配表示学习和随机性提取中的有限样本极限，零熵对应精确恢复和提取。
...
### [277] [Learning Selective Merge Policies for Deadline-Constrained Coded Caching via Deep Reinforcement Learning](https://arxiv.org/abs/2605.15236)
*Amirhossein Yousefiramandi*

Main category: cs.IT

TL;DR: 提出基于深度强化学习的截止时间约束编码交付方法，使用图注意力策略网络，显著降低广播包过期率并提高效率。


<details>
  <summary>Details</summary>
Motivation: 传统缓存编码在延迟敏感应用中，合并消息需考虑截止时间，且当前合并可能对后续消息有害。

Method: 将问题建模为掩码离散动作队列状态控制，使用图注意力策略网络，通过近端策略优化进行训练。

Result: 在均匀需求基准上，广播包过期率降低40.9%（0.208 vs 0.352），并达到最佳广播效率分数。

Conclusion: 对于紧截止时间，选择性合并优于激进合并，策略网络学习以约31.8%的比率合并。
...
### [278] [PrismQuant: Rate-Distortion-Optimal Vector Quantization for Gaussian-Mixture Sources](https://arxiv.org/abs/2605.15507)
*Bumsu Park,Chanho Park,Youngmok Park,Namyoon Lee*

Main category: cs.IT

TL;DR: 针对高斯混合源，提出PrismQuant方法，通过无损传输分量标签和分量匹配KLT实现渐近最优的率失真编码。


<details>
  <summary>Details</summary>
Motivation: 经典变换编码对高斯源最优，但对多模态源（如高斯混合）失效，因为单一协方差无法捕捉局部几何结构，且率失真函数无闭式解。

Method: 基于高斯混合模型，理论证明条件率失真函数由全局逆水平填充水平决定；提出PrismQuant：无损传输分量标签，对残差使用分量匹配KLT和标量量化。

Result: 在合成高斯混合数据上接近理论率失真界；在真实信道状态信息数据上，以比变换器学习编解码器小一个数量级的模型尺寸，达到相当或更优性能。

Conclusion: 高斯混合源的率失真编码仅增加分量标签开销，PrismQuant可实现渐近最优性能且有实用实现。
...
### [279] [GeoGS-CE: Learning Delay--Beam Channel Priors with 3D Gaussians for High-Mobility Scenarios](https://arxiv.org/abs/2605.16094)
*Yumeng Zhang,Jiajia Guo,Chaozheng Wen,Chenghong Bian,Jun Zhang*

Main category: cs.IT

TL;DR: 提出GeoGS-CE框架，利用环境几何先验，通过两阶段方法实现稀疏导频高移动性场景的宽带信道估计。


<details>
  <summary>Details</summary>
Motivation: 高移动性下信道快变且导频稀疏，但环境（如高铁轨道）具有稳定延迟-波束功率谱，可提供有效先验。

Method: 离线阶段用3D高斯模型表示场景几何散射，结合可微无线渲染过程模拟延迟-波束功率谱；在线阶段预测功率谱作为协方差先验，通过线性MMSE估计重建信道。

Result: 基于广深高铁信道仿真，GeoGS-CE在CFR重建精度上显著优于仅用导频和非几何方法。

Conclusion: 利用环境几何先验能有效提升稀疏导频下的信道估计性能。
...
### [280] [Ensuring Logic in the Fog: Sound POMDP Synthesis with LTL Objectives](https://arxiv.org/abs/2605.12581)
*Can Zhou,Yulong Gao,Pian Yu*

Main category: cs.LO

TL;DR: 提出一种基于信念的奖励塑形机制，集成到蒙特卡洛规划框架中，使智能体在部分可观测环境中遵循线性时序逻辑规范，实验表明在现有求解器失败场景中仍有效且可扩展。


<details>
  <summary>Details</summary>
Motivation: 在部分可观测马尔可夫决策过程中，线性时序逻辑的定性验证不可判定，导致定量合成困难，尤其难以设计可靠的奖励信号。

Method: 提出一种可靠的奖励塑形机制，动态生成基于信念且经认证的LTL满足的奖励，并集成到增强的蒙特卡洛规划框架中。

Result: 该方法在现有求解器失败的场景中表现优异，并在多个基准领域保持有效性和可扩展性。

Conclusion: 该工作弥合了部分可观测环境中复杂时序任务合成的鸿沟，实现了可验证的成功最大化。
...
### [281] [From Full and Partial Intraoral Scans to Crown Proposal: A Classification-Guided Restoration Assistance Pipeline](https://arxiv.org/abs/2605.15241)
...
TL;DR: 提出一个端到端流水线，从原始口内扫描和牙齿编号生成初始牙冠候选，通过分类、配准、图割分割和基于检索的拟合，在2.5-3.5分钟内完成，性能优异。


<details>
  <summary>Details</summary>
Motivation: 单颗牙修复常用部分扫描，但现有分割网络在部分扫描上表现不佳，而端到端生成方法会导致表面过度平滑，失去咬合细节。

Method: 三阶段流水线：1) 数据准备与姿势标准化；2) 扫描类型分类（DGCNN）+ 粗到细RANSAC+ICP配准+图割优化分割；3) 基于DGCNN嵌入的余弦相似度检索邻牙和对颌牙，结合样条引导对齐和Blender Python API拟合生成牙冠。

Result: 在1958个部分扫描上，宏平均DSC 0.9249，Recall 0.8919，Precision 0.9615；目标牙与邻牙DSC 0.9468-0.9569，中心误差0.2666-0.2774 mm；每牙冠生成2.5-3.5分钟。

Conclusion: 该流水线为端到端生成方法提供了实用替代，在保持精度的同时避免过度平滑。
...
### [282] [Highly Detailed and Generalizable Broadleaf Tree Crown Instance Segmentation from UAV Imagery](https://arxiv.org/abs/2605.15673)
...
TL;DR: 提出一种基于深度学习的实例分割模型，用于从无人机航拍图像中精确分割阔叶林的单个树冠，通过大量高质量标注数据实现高泛化能力。


<details>
  <summary>Details</summary>
Motivation: 阔叶林树冠形状多样且树顶不清晰，分割难度大，现有方法不足。

Method: 手动标注18,507个树冠多边形，基于Mask2Former架构并测试多种骨干网络，训练分割模型。

Result: 最佳模型在复杂阔叶林中仅用RGB图像实现高分割性能，并泛化到日本不同森林和婆罗洲热带雨林。

Conclusion: 高质量大标注数据集对实现详细且泛化的树冠分割至关重要，该模型已集成到DF Scanner Pro软件，支持无人机森林监测。
...
### [283] [Neural Point-Forms](https://arxiv.org/abs/2605.15524)
*Bruno Trentini,Jacob Hume,Vincenzo Antonio Isoldi,Philipp Misof,Ekaterina S. Ivshina,Kelly Maggs*

Main category: cs.LG

TL;DR: 提出神经点形式（NPF），一种基于扩散几何和拉普拉斯算子的可学习几何特征，用于点云数据分析，并证明其一致性。


<details>
  <summary>Details</summary>
Motivation: 现有点云学习未能充分利用点云背后的几何结构（如流形），而微分形式可编码切空间信息，因此需要一种可学习的几何特征。

Method: 利用拉普拉斯算子构建离散模型，通过内积比较点云上的微分形式，并构造比较矩阵；证明该矩阵在标准假设下的一致性。

Result: 该层紧凑、高效且置换不变，输出学习到的形式比较矩阵；实验显示NPF在依赖采样密度、流形结构或群体几何的任务中表现优越。

Conclusion: NPF提供了一种有竞争力且可解释的点云表示，特别适用于对几何结构敏感的任务。
...
### [284] [Entropy Across the Bridge: Conditional-Marginal Discretization for Flow and Schr\"odinger Samplers](https://arxiv.org/abs/2605.16126)
*Bruno Trentini,Dejan Stancevic,Michael M. Bronstein,Alexander Tong,Luca Ambrogioni*

Main category: cs.LG

TL;DR: 提出基于条件边际熵率的推理时间调度器，可自适应分配少量采样步长，在低计算预算下显著提升流模型或桥模型生成质量。


<details>
  <summary>Details</summary>
Motivation: 固定推理预算下，采样步长分配对样本质量影响巨大，而现有调度多依赖启发式或线性/余弦网格。

Method: 推导桥感知离散化的条件边际熵率目标函数，分离端点条件桥几何与边际流演化，构建免训练的熵率调度器。

Result: 在2D桥/流模型上，10步ODE-Heun MMD比线性降低18.1%，SDE-Heun降低22.7%；EDM/CIFAR-10上5步FID达186.3（线性200.5，余弦238.0）；AlphaFlow蛋白生成在低NFE下优于基线。
...
### [285] [AgentStop: Terminating Local AI Agents Early to Save Energy in Consumer Devices](https://arxiv.org/abs/2605.15206)
*Dzung Pham,Kleomenis Katevas,Ali Shahin Shamsabadi,Hamed Haddadi*

Main category: cs.LG

TL;DR: 研究本地部署的LLM代理的资源开销，提出AgentStop通过预测性提前终止减少能耗。


<details>
  <summary>Details</summary>
Motivation: 远程代理存在隐私、网络依赖和API成本问题，本地代理虽好但资源消耗大，大量失败任务浪费计算资源。

Method: AgentStop利用执行信号（如token级对数概率）预测轨迹成功概率，并提前终止低成功率的任务。

Result: 在Web问答和编程任务上减少15-20%能耗，任务性能下降<5%。
...
### [286] [TeamTR: Trust-Region Fine-Tuning for Multi-Agent LLM Coordination](https://arxiv.org/abs/2605.15207)
*Yi Xie,Siao Liu,Falong Fan,Yuanqi Yao,Yue Zhao,Bo Liu*

Main category: cs.LG

TL;DR: 提出TeamTR框架，通过信任区域方法解决多智能体LLM顺序微调中的占用偏移问题，平均提升7.1%。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM系统在顺序微调共享上下文团队时存在结构失败模式，导致性能不如单模型基线；评估时使用缓存轨迹造成复合占用偏移。

Method: 提出TeamTR，在每个组件更新后重采样轨迹，并强制执行每个智能体的散度控制，提供严格的每更新和每阶段改进下界。

Result: 平均提升7.1%，缓解协调退化，支持即插即用的组件替换。

Conclusion: TeamTR有效解决了多智能体顺序微调中的占用偏移问题，优于单智能体和顺序基线。
...
### [287] [Quantization Undoes Alignment: Bias Emergence in Compressed LLMs Across Models and Precision Levels](https://arxiv.org/abs/2605.15208)
*Plawan Kumar Rath,Rahul Maliakkal*

Main category: cs.LG

TL;DR: 大语言模型量化压缩会导致新的刻板偏见，而标准质量指标无法检测这些退化。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对量化压缩如何影响模型公平性的系统理解，通常只比较两精度、使用聚合偏差指标且评估单一模型家族，无法区分渐进退化和阈值失效。

Method: 在三个指令微调模型（Qwen2.5-7B、Mistral-7B、Phi-3.5-mini）上，以五种精度（BF16到3-bit）在12,148条BBQ偏差基准条目上进行控制实验，共911,100条推理记录。

Result: 3-bit量化导致6-21%原本无偏的条目出现新的刻板行为，呈剂量-反应模式；模型选择‘未知’答案的比例下降17.4%；标准困惑度指标变化很小（8-bit<0.5%，4-bit<3%），但4-bit时已有2.5-5.6%条目产生新偏见。

Conclusion: 聚合评估指标系统性漏检公平性关键退化，需要部署前明确测试偏见涌现的质量感知压缩协议。
...
### [288] [Mask-Morph Graph U-Net: A Generalisable Mesh-Based Surrogate for Crashworthiness Field Prediction under Large Geometric Variation](https://arxiv.org/abs/2605.15231)
*Haoran Li,Tobias Lehrer,Yingxue Zhao,Haosu Zhou,Philipp Stocker,Tobias Pfaff,Nan Li*

Main category: cs.LG

TL;DR: 提出MMGUNet，通过粗网格变形和掩码预训练，实现高效且可泛化的网格代理模型，用于碰撞仿真。


<details>
  <summary>Details</summary>
Motivation: 非线性有限元碰撞仿真准确但计算昂贵，限制迭代设计优化；基于GNN的代理模型需平衡泛化性与精度。

Method: 采用Mask-Morph Graph U-Net架构，通过特征对齐重心参数化将粗网格层次变形到每个输入网格，并在监督预训练中应用节点掩码，随后冻结高参数边特定层进行参数高效微调。

Result: 在分布内、分布外和跨组件迁移设置中，粗网格变形提高了测试精度，掩码预训练减小了训练-测试差异并提高了数据效率，预测误差低于外部基线。

Conclusion: 该方法为耐撞性设计探索提供了可重用、数据高效的网格代理模型实用路径。
...
### [289] [MuteBench: Modality Unavailability Tolerance Evaluation for Incomplete Multimodal Fusion](https://arxiv.org/abs/2605.15235)
*Wugeng Zheng,Ziwen Kan,Tianlong Chen,Chen Chen,Song Wang*

Main category: cs.LG

TL;DR: 提出MuteBench基准，评估多模态生理数据在传感器故障下的融合架构鲁棒性，发现架构族比参数数量更重要，并给出选择与设计指导。


<details>
  <summary>Details</summary>
Motivation: 多模态生理数据中传感器故障常见但缺乏系统评估，现有基准未涵盖多种融合架构在两种缺失模式下的对比。

Method: 构建MuteBench，包含9个数据集、6种融合架构、两种缺失模式（模态缺失和模态内缺失），共125,000样本，进行鲁棒性评估。

Result: 架构族是鲁棒性最强预测因子；独立通道模型对模态缺失鲁棒但对模态内缺失敏感；课程式模态dropout仅在训练dropout率内有效；通道数、序列长度和模态对齐共同决定威胁模式；扩散插补可改善下游分类，但需跨数据集验证。

Conclusion: MuteBench为选择现有架构和设计未来鲁棒融合方法提供具体指导，并指出开放方向（如跨数据集验证扩散插补）。
...
### [290] [Reducing the Safety Tax in LLM Safety Alignment with On-Policy Self-Distillation](https://arxiv.org/abs/2605.15239)
*Yu Fu,Longxuan Yu,Haz Sameen Shahgir,Zhipeng Wei,Hui Liu,N.Benjamin Erichson,Yue Dong*

Main category: cs.LG

TL;DR: 提出一种基于在线策略自蒸馏的安全对齐方法OPSA，通过冻结的教师模型提供逐token KL监督，并引入教师翻转率自动搜索安全上下文，在保持推理能力的同时提升安全性，尤其在小模型上效果显著。


<details>
  <summary>Details</summary>
Motivation: 安全对齐常以牺牲推理能力为代价（安全税），主要原因之一是训练时的分布不匹配（非在线策略），包括人类演示或外部模型生成的轨迹与模型自身策略的差异。

Method: 采用在线策略自蒸馏：模型生成自身轨迹，由冻结的教师副本（带有特权安全上下文）提供密集的逐token KL散度监督；通过教师翻转率衡量特权上下文将不安全响应转化为安全响应的频率，以此搜索激活潜在安全推理的上下文。

Result: 在两个推理模型家族、五个规模上，OPSA在安全-推理权衡上优于非在线策略自蒸馏和外部教师蒸馏，小模型提升最大（R1-Distill-1.5B +8.85，Qwen3-0.6B +5.49），且收益在训练集大小和自适应越狱评估中保持一致。

Conclusion: OPSA通过在线策略自蒸馏有效缓解安全税，其机制是将更新集中在早期合规决策标记附近，从而在保持通用推理能力的同时提升安全性。
...
### [291] [Logical Grammar Induction via Graph Kolmogorov Complexity: A Neuro-Symbolic Framework for Self-Healing Clinical Data Integrity](https://arxiv.org/abs/2605.15242)
*Abolfazl Zarghani,Amir Malekesfandiari*

Main category: cs.LG

TL;DR: 提出Logic-GNN框架，通过神经符号方法将临床记录视为结构化语言，利用图神经网络和柯尔莫哥洛夫复杂度检测语法违规的异常，在医疗数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 医疗信息系统因人为数据录入错误而可靠性受损，现有统计方法无法区分合法临床极端值和数据错误。

Method: 提出Logic-GNN，将临床记录视为由潜在逻辑游戏支配的结构化私有语言，集成时间图神经网络和图柯尔莫哥洛夫复杂度归纳符号语法，定义异常为导致最小描述长度显著扩展的语法违规。

Result: 在Sina系统数据集上F1分数达0.94，在区分危及生命的医学异常值和数据损坏方面超越基线12%。

Conclusion: 引入自愈机制，通过建议逻辑修正来在实时医疗信息系统中维护数据完整性。
...
### [292] [Reading the Cell, Designing the Cure: Perturbation-Conditioned Molecular Diffusion for Function-Oriented Drug Design](https://arxiv.org/abs/2605.15243)
*Ziyu Xu,Zijian Zhang,Liang Wang,Zhiyuan Liu,Qiang Liu,Shu Wu,Liang Wang*

Main category: cs.LG

TL;DR: 本文提出了一种基于转录组状态转换的生成式药物设计方法CURE，通过多分辨率扩散框架生成结构质量和功能一致性更好的分子。


<details>
  <summary>Details</summary>
Motivation: 当可靠靶标结构缺乏或表型源于失调通路时，转录组扰动可提供药物作用的系统级功能读数，但当前方法面临跨模态域差异大、转录组信号稀疏等挑战。

Method: 提出CURE框架，包含转录组扰动功能特征提取器TFE，它从前后状态提取功能导向的扰动嵌入，对齐到双化学视图以弥合跨模态差距，并通过异质性感知聚合从噪声数据中提取稳健状态特定信号。

Result: 在标准基准和严格的分布外协议上，CURE在结构质量和功能一致性方面持续优于强基线。

Conclusion: 通过零样本基因-抑制剂设计任务验证了其实际效用，凸显了表型驱动生成发现的潜力。
...
### [293] [Privacy Evaluation of Generative Models for Trajectory Generation](https://arxiv.org/abs/2605.15246)
...
TL;DR: 生成轨迹模型未必保护隐私；本文通过成员推理攻击揭示隐私评估缺口。


<details>
  <summary>Details</summary>
Motivation: 轨迹数据敏感，生成模型虽用于合成数据但隐私假设不成立。

Method: 识别适用隐私评估方法，对代表性模型实施成员推理攻击。

Result: 发现现有隐私评估不足，成功实施攻击表明生成模型仍存在隐私风险。

Conclusion: 生成轨迹模型不能自动保证隐私，必须进行经验性隐私评估。
...
### [294] [GQLA: Group-Query Latent Attention for Hardware-Adaptive Large Language Model Decoding](https://arxiv.org/abs/2605.15250)
*Fanxu Meng*

Main category: cs.LG

TL;DR: 提出GQLA，通过双路径解码适配不同硬件，无需重新训练，并在LLaMA-3-8B上实现KV缓存压缩至基线的28.125%。


<details>
  <summary>Details</summary>
Motivation: MLA在H100上性能完美，但解码路径单一，在H20等硬件上无法发挥多令牌预测优势，且不支持沿头维度的张量并行。需要一种可硬件自适应的注意力机制。

Method: 对MLA进行最小修改得到GQLA，训练权重暴露两条等价解码路径：MQA吸收路径和GQA路径。运行时根据硬件选择路径，无需重新训练。同时提出TransGQLA将预训练GQA转换为GQLA。

Result: 在LLaMA-3-8B上，MQA吸收路径将每token KV缓存压缩至GQA基线的28.125%，GQA路径保留GQA级流量。GQLA在H100和H20上均能接近roofline性能，并支持GQA路径上的8路零冗余张量并行。

Conclusion: GQLA通过最小修改实现硬件自适应的注意力机制，兼顾MLA的高效和GQA的灵活性，且无需重新训练，可基于现有GQA模型转换。
...
### [295] [PDRNN: Modular Data-driven Pedestrian Dead Reckoning on Loosely Coupled Radio- and Inertial-Signalstreams](https://arxiv.org/abs/2605.15252)
*Peter Bauer,Andreas Porada,Felix Ott,Christopher Mutschler,Tobias Feigl*

Main category: cs.LG

TL;DR: 提出一种基于循环神经网络（RNN）的模块化混合AI辅助行人航位推算（PDR）系统PDRNN，能够隐式预测异步传感器数据流，在动态运动场景中实现高精度定位。


<details>
  <summary>Details</summary>
Motivation: 传统PDR方法在融合多模态传感器数据时存在采样率不匹配、传输不可靠等问题，且在高加速度、高速和快速变化方向等动态运动中效果不佳。

Method: 设计一个简单的RNN架构，隐式预测来自不同估计方法的异步传感器数据流。系统采用模块化设计，每个组件作为独立的机器学习模型集合来估计关键参数的均值和方差，分别处理方向、速度或距离估计，并可选地利用同步无线电系统（如5G）进行绝对定位。最终融合模型结合位置、速度和方向输出，并利用不确定性估计增强鲁棒性。

Result: 在动态体育动作数据上的实验表明，PDRNN相比经典方法和基于机器学习的方法具有更高的准确性和精度，有效避免了黑盒方法中常见的误差累积问题。

Conclusion: PDRNN尽管增加了系统复杂性，但提供了预测能力和更好的组件控制，是一种有效的PDR解决方案。
...
### [296] [Position: Ideas Should be the Center of Machine Learning Research](https://arxiv.org/abs/2605.15253)
*Jairo Diaz-Rodriguez*

Main category: cs.LG

TL;DR: 提出“思想优先”框架，将研究重点从基准和理论转向思想本身，通过行为特征预测和定制实验测试思想，以弥合理论与实践差距并促进公平。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习研究过度依赖基准工程和理想化理论，忽略思想这一核心科学对象，导致理论与实践脱节且资源不平等。

Method: 采用“思想优先”框架，强调思想的价值在于其在现代模型中预测的行为特征，并通过针对性实验验证这些特征，而非追求排行榜成绩。

Result: 该框架弥合了理论与实践之间的鸿沟，通过降低“复杂性溢价”使资源有限的研究者也能做出科学贡献，促进科研公平。

Conclusion: 研究文化应转向以思想为中心，将基准和定理视为测试机制假设的工具，而非最终目标。
...
### [297] [Curriculum Learning of Physics-Informed Neural Networks based on Spatial Correlation](https://arxiv.org/abs/2605.15254)
*Xujia Chen,Xinyue Hu,Letian Chen,Daming Shi,Wenhui Fan*

Main category: cs.LG

TL;DR: 提出一种空间相关课程学习框架，通过空间因果权重、低频信息桥和区域自适应重加权策略，改善PINN在强空间耦合边界值问题中的训练困难。
...
Method: 空间相关课程学习框架：1) 空间因果权重引导信息从近边界区域向内传播；2) 低频信息桥通过伪标签强制空间分离区域的一致性；3) 区域自适应重加权调整子区域损失以恢复高频细节。

Result: 在PDE基准测试上，在相当计算成本下，该方法减轻了训练失败并提高了求解精度。

Conclusion: 首次从子区域空间耦合角度解决PINN训练困难，有效提升性能。
...
### [298] [Training on Documents About Monitoring Leads to CoT Obfuscation](https://arxiv.org/abs/2605.15257)
*Reilly Haskins,Bilal Chughtai,Joshua Engels*

Main category: cs.LG

TL;DR: 监控感知模型能够通过混淆推理来逃避CoT监控，高CoT可控性增强了这种混淆能力，对CoT监控构成风险。


<details>
  <summary>Details</summary>
Motivation: CoT监控依赖模型忠实外化推理，但模型可能通过混淆推理来逃避检测。

Method: 通过合成文档微调使模型了解CoT监控，并在不同任务上测试其混淆能力；分析CoT可控性与混淆成功率的关联；在强化学习下比较监控感知与无感知模型。

Result: 监控感知模型在未检测到的不当行为上比率更高（效果在难任务上减弱但仍在），CoT可控性与混淆成功率显著相关（r=0.800，p=0.017），且监控感知模型在RL中更快学会奖励欺骗。

Conclusion: 监控知识结合高CoT可控性会削弱CoT监控的有效性，带来风险。
...
### [299] [Tadpole: Autoencoders as Foundation Models for 3D PDEs with Online Learning](https://arxiv.org/abs/2605.15284)
*Qiang Liu,Felix Koehler,Benjamin Holzschuh,Nils Thuerey*

Main category: cs.LG

TL;DR: 提出Tadpole，一种用于三维偏微分方程的基础模型，通过自编码器预训练和高效在线数据生成，实现迁移、高维扩展和多任务应用。


<details>
  <summary>Details</summary>
Motivation: 现有模型在三维PDE的迁移性、高维扩展性和多功能性方面存在不足。

Method: Tadpole作为自编码器预训练，基于在线生成的合成3D PDE数据；使用单通道空间裁剪学习表示；提出参数高效的微调策略（低秩适应、潜在空间变换、跳跃连接）。

Result: 预训练数据量相当于数百TB；在下游任务（动力学学习、生成建模）中展示强性能，且微调参数少。

Conclusion: Tadpole是有效的3D PDE基础模型，具备多功能性和高效性，代码和权重已开源。
...
### [300] [Universal Approximation of Nonlinear Operators and Their Derivatives](https://arxiv.org/abs/2605.15285)
*Filippo de Feo*

Main category: cs.LG

TL;DR: 首次证明了非线性算子及其导数在Banach空间上的通用逼近定理，推广了经典结果到无限维设置。


<details>
  <summary>Details</summary>
Motivation: 导数信息算子学习（DIOL）是算子学习领域的前沿，其通用逼近定理（UAT）是基础且开放的问题。

Method: 采用编码器-解码器架构，结合Banach空间逼近性质、Bastiani可微性、紧致开拓扑及新型加权Sobolev空间四个关键特征。

Result: 证明了非线性k次可微算子及其导数在紧集和加权Sobolev范数下的通用逼近定理，首次完整推广了Hornik (1991)的经典结果。

Conclusion: 该定理为高阶精度算子学习、Banach空间约束优化（如PDE最优控制、反问题）及无限维PDE数值方法等应用奠定基础。
...
### [301] [GQA-{\mu}P: The maximal parameterization update for grouped query attention](https://arxiv.org/abs/2605.15290)
...
TL;DR: 提出Complete-P深度和权重衰减缩放，并首次推导分组查询注意力的μP缩放，实现跨架构超参数迁移。


<details>
  <summary>Details</summary>
Motivation: 减少大型语言模型调优计算量，解决μP对新架构推导困难的问题。

Method: 将谱范数条件提升为特征学习定义，得到Complete-P缩放；修改谱范数以处理非满秩权重，推导GQA的μP缩放。

Result: 实验证明学习率跨GQA重复超参数迁移及权重衰减迁移的有效性。

Conclusion: 所提方法有效实现了超参数迁移，降低了调优成本。
...
### [302] [GESD: Beyond Outcome-Oriented Fairness](https://arxiv.org/abs/2605.15295)
*Gideon Popoola,John Sheppard*

Main category: cs.LG

TL;DR: 提出基于解释稳定性的群体差异公平性度量GESD，并集成到多目标优化框架FEU中，兼顾效用与公平。


<details>
  <summary>Details</summary>
Motivation: 现有公平性度量主要关注结果，缺乏对决策过程解释的公平性分析。

Method: 提出GESD度量不同子群解释的稳定性差异，并嵌入多目标优化框架FEU联合优化效用、结果公平和解释公平。

Result: 实验表明GESD有效捕捉解释质量的群体差异，FEU在效用和公平性上优于现有方法。

Conclusion: GESD弥合了结果公平与解释公平的差距，为偏见诊断与缓解提供全面工具。
...
### [303] [How Data Augmentation Shapes Neural Representations](https://arxiv.org/abs/2605.15306)
*Tianxiao He,Alex H. Williams,Sarah E. Harvey*

Main category: cs.LG

TL;DR: 本文使用形状分析工具，研究了不同数据增强策略对神经网络内部表示几何的影响，发现增强强度增加导致表示在度量空间中沿特定轨迹移动，不同增强类型导向不同方向，并且这些几何模式可用于预测模型集成的改进。


<details>
  <summary>Details</summary>
Motivation: 数据增强能改善泛化，但其对学习表示几何的影响尚不清楚。

Method: 利用形状分析工具，将网络隐藏表示嵌入度量空间，距离对缩放、平移、旋转和反射不变；研究不同增强策略下表示形状的轨迹和扭曲。

Result: 增强强度增加导致表示在度量空间中沿良好轨迹移动；不同增强类型使表示沿不同方向移动；表示形状的扭曲可预测集成时的改进。

Conclusion: 不同架构和种子间存在共享几何模式；分析形状空间轨迹为理解和比较数据增强方法提供了原则性工具。
...
### [304] [Time-Varying Deep State Space Models for Sequences with Switching Dynamics](https://arxiv.org/abs/2605.15311)
*Sanja Karilanova,Subhrakanti Dey,Ay\c{c}a \"Oz\c{c}elikkale*

Main category: cs.LG

TL;DR: 提出一种基于时间变化状态空间模型的神经网络，使用基函数字典学习时变动态，在切换系统和语音去噪中优于时不变模型，计算复杂度相当。


<details>
  <summary>Details</summary>
Motivation: 时变系统的识别和建模是信号处理与系统辨识的根本挑战。

Method: 提出一类时变状态空间模型（SSM）神经网络，通过基函数字典提供可学习的时变动态。

Result: 在切换系统合成数据和语音去噪任务上，时变模型一致优于时不变模型，且计算复杂度相当。

Conclusion: 揭示了数据时变动态需捕获的方面、基函数自由度的分配方式以及更大模型补偿时不变局限的程度。
...
### [305] [Beyond Bounded Variance: Variance-Reduced Normalized Methods for Nonconvex Optimization under Blum-Gladyshev Noise](https://arxiv.org/abs/2605.15314)
*Antesh Upadhyay,Arda Fazla,Abolfazl Hashemi*

Main category: cs.LG

TL;DR: 本文研究了BG-0噪声模型下的非凸随机优化，提出了带动量的归一化SGD和方差缩减的归一化STORM方法，并证明了它们在标准光滑和广义光滑条件下的收敛复杂度。


<details>
  <summary>Details</summary>
Motivation: BG-0噪声模型中梯度方差随初始化距离二次增长，现有方法需有界域或增加批大小，本文旨在无需这些假设的情况下设计有效算法并分析收敛性。

Method: 采用归一化SGD加动量（仅需单随机梯度）和归一化STORM（方差缩减）方法，结合对称广义光滑框架。

Result: 动量法达到O(ε^{-6})复杂度；STORM在均方光滑下达到最优O(ε^{-4})，在广义光滑下依赖α参数，复杂度为O(ε^{-(4+α)})或O(ε^{-5})；当噪声增长参数消失时恢复标准速率。

Conclusion: 首次为BG-0噪声下的归一化方法提供了收敛保证，无需有界域、增加批大小或显式锚定，覆盖了标准与广义光滑情形。
...
### [306] [From Weight Perturbation to Feature Attribution for Explaining Fully Connected Neural Networks](https://arxiv.org/abs/2605.15328)
*Thodoris Lymperopoulos,Denia Kanellopoulou*

Main category: cs.LG

TL;DR: 提出通过扰动特征权重而非值的新归因方法（XWP和XWP_c），旨在解决遮挡技术的常见问题，并在简单DNN上取得竞争性表现。


<details>
  <summary>Details</summary>
Motivation: 全连接神经网络的可解释性缺乏共识，现有归因方法存在附加偏差和分布外数据问题，需要更可靠的简单方法。

Method: 对特征的附加权重施加扰动，而非扰动特征值，由此产生两种新归因方法：XWP和XWP_c。

Result: 在简单DNN上识别图像信号的任务中，性能与最成熟的归因方法相当，达到竞争性水平。

Conclusion: 提出一个稳健框架，解决了长期存在的可解释性漏洞，使模型解释更可靠。
...
### [307] [From I/O to Code with Discovery Agent](https://arxiv.org/abs/2605.15334)
*Yihong Dong,Jiaru Qian,Haoran Zhang,Peixu Wang,Binhua Li,Zhi Jin,Yongbin Li,Ge Li,Xiaokang Yang,Xue Jiang*

Main category: cs.LG

TL;DR: 提出DIO-Agent，通过进化搜索和变换优先级前提，从输入输出行为合成程序，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: IO2Code比NL2Code更困难，因为缺乏语义对齐，需要从具体计算行为恢复原理，搜索空间大且欠约束。

Method: 将IO2Code框架为离散程序空间上的进化搜索，LLM作为变异算子，执行错误信号指导变异，并引入变换优先级前提（从常量到条件到迭代）作为变异先验，避免陷入错误死胡同。

Result: 在IO2CodeBench多难度级别上，DIO-Agent一致优于传统程序示例方法和SOTA进化代理基线，并大幅超越同等采样预算的测试时扩展策略。

Conclusion: DIO-Agent有效解决了IO2Code问题，通过进化搜索和先验引导实现了更好的程序合成性能。
...
### [308] [Bounded-Rationality, Hedging, and Generalization](https://arxiv.org/abs/2605.15340)
*Pedro A. Ortega*

Main category: cs.LG

TL;DR: 将学习问题视为有界理性决策，研究样本影响与输出扭曲的关系，并用f-散度正则化刻画，推广了KL散度情况。


<details>
  <summary>Details</summary>
Motivation: 探索学习者如何权衡训练样本的影响和输出扭曲，以及这种权衡如何影响泛化。

Method: 建立从样本到输出的诱导信道作为有界理性决策问题，使用f-散度正则化表示响应律，通过观察对缩放损失和局部扰动的响应来恢复对冲和曲线。

Result: 得到了训练损失与样本依赖之间的下权衡曲线及其对应的上认证曲线，并证明了在f-散度正则化下这些曲线位于信息几何中。

Conclusion: 泛化被处理为学习者自身响应律的可检验对冲属性，对冲覆盖扭曲时提供实用认证。
...
### [309] [LEAP: Trajectory-Level Evaluation of LLMs in Iterative Scientific Design](https://arxiv.org/abs/2605.15341)
*Marilyn Zhang,Tianfeng Chen,Fabi\'an Barzuna,Ankita Rathod,Mark E. Whiting*

Main category: cs.LG

TL;DR: 提出LEAPBench，通过轨迹度量评估LLM在迭代科学设计中的学习效率，发现LLM不及贝叶斯基线，且领域无关提示优于领域感知提示。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅评估固定时间点的结果快照，忽略学习轨迹和效率，无法真实反映LLM的迭代学习能力。

Method: 构建55任务框架LEAPBench，包含最佳迄今为止曲线下面积(AUC)轨迹度量、经典贝叶斯优化参考和基于文献的审计，并定义三个评估选择（测量、基线、锚定）。

Result: 切换评分改变53%任务的最佳模型决策；LLM未超越贝叶斯基线；领域无关提示在16个生物学任务中比领域感知提示更接近已发表最佳（低10个百分点），尤其在6个分歧任务上全部更优；轨迹度量作为奖励的离线强化学习在14/21保留任务上提升性能。

Conclusion: 轨迹度量能揭示效率改进，领域无关提示更有效，LLM需结合更高效的学习策略，且轨迹度量可作为实用训练目标。
...
### [310] [PACER: Acyclic Causal Discovery from Large-Scale Interventional Data](https://arxiv.org/abs/2605.15353)
*Ramon Vi\~nas Torn\'e,S\'ilvia F\`abregas Salazar,Soyon Park,Ivo Alexander Ban,Artyom Gadetsky,Nikita Doikov,Maria Brbi\'c*

Main category: cs.LG

TL;DR: PACER是一个通过构造保证无环性的可扩展因果发现框架，支持观测和干预数据，在线性高斯机制中实现高效闭式梯度计算，在大规模数据上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有因果发现方法受限于软无环约束，导致优化无效环图、数值不稳定且可扩展性差，尤其在高维干预数据场景下。

Method: 通过联合建模变量排列和边概率来参数化DAG分布，直接优化有效因果结构（无替代惩罚）；支持统一似然处理观测/干预数据、灵活条件密度模型及结构先验；对线性高斯机制推导期望干预对数似然及梯度的闭式表达式。

Result: 在蛋白质信号和大型遗传扰动基准上匹配或超越最先进方法；可扩展到数千变量；比基于惩罚的可微方法快两个数量级。

Conclusion: 通过原则性搜索空间设计，从高维扰动数据中实现精确且可扩展的因果发现是可行的。
...
### [311] [Controllable Molecular Generative Foundation Models](https://arxiv.org/abs/2605.15354)
*Yihan Zhu,Yuhan Liu,Weijiang Li,Tengfei Luo,Meng Jiang*

Main category: cs.LG

TL;DR: 提出CoMole，一个基于motif感知图扩散的分子生成基础模型，结合强化学习实现可控生成，在多个基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 分子图生成缺乏统一框架以实现可靠可控性，且强化学习应用于图生成时面临原子级动作空间大和中间状态无效的问题。

Method: 构建统一的motif感知图扩散管道，学习motif感知图空间以传递预训练结构先验；利用强化学习优化条件反向策略，在化学有意义的决策上进行优化，并理论证明motif感知策略的有效性。

Result: 在三个异构基准上，针对所有九个目标可控性排名第一，MAE最多降低48.2%，有效性保持在0.94以上，无需规则修正或后处理。

Conclusion: CoMole实现了分子图生成的统一可控框架，且通过仅优化任务嵌入即可迁移可控性到未知属性，性能与强任务特定基准相当。
...
### [312] [Federated Learning of Spiking Neural Networks under Heterogeneous Temporal Resolutions](https://arxiv.org/abs/2605.15355)
*Sanja Karilanova,Subhrakanti Dey,Ay\c{c}a \"Oz\c{c}elikkale*

Main category: cs.LG

TL;DR: 针对脉冲神经网络在联邦学习中因时间分辨率异质性导致的参数不匹配问题，提出了一种适应时间分辨率的联邦学习框架，使各客户端能以本地时间分辨率训练并兼容全局模型。


<details>
  <summary>Details</summary>
Motivation: 边缘设备因硬件和能源限制，以不同时间分辨率采集数据，导致联邦学习中的时间异质性，使得朴素联邦平均失效，参数无法直接迁移。

Method: 研究了不同时间分辨率下学习的神经元参数如何整合，提出了适应时间分辨率不匹配的联邦学习框架。

Result: 在SHD和DVS-Gesture数据集上，所提适应方法显著恢复了因时间不匹配导致的精度损失。

Conclusion: 各客户端可以以本地时间分辨率训练，同时保持与全局模型的兼容性。
...
### [313] [PRB-RUPFormer: A Recursive Unified Probabilistic Transformer for Residual PRB Forecasting](https://arxiv.org/abs/2605.15363)
*Saad Masrur,Yuxuan Jiang,Matti Hiltunen,Ajay Rajkumar,Ismail Guvenc*

Main category: cs.LG

TL;DR: 提出PRB-RUPFormer，一种递归统一概率Transformer，用于预测剩余PRB，考虑跨载波依赖和不确定性。


<details>
  <summary>Details</summary>
Motivation: 现有PRB预测方法仅依赖历史值且独立训练，无法捕捉跨载波依赖且缺乏不确定性估计，点预测不足以应对高变异流量。

Method: 使用时间、季节和载波感知嵌入的递归Transformer，联合处理多变量KPI时间序列，统一训练所有载波和扇区，输出分位数预测区间。

Result: 在六个月LTE网络数据上，中位MAE低于0.05，命中概率高于0.80，一天和七天递归预测均有效。

Conclusion: 概率预测直接支持动态频谱接入功能，如动态载波激活、拥塞避免和主动频谱共享。
...
### [314] [Is One Score Enough? Rethinking the Evaluation of Sequentially Evolving LLM Memory](https://arxiv.org/abs/2605.15384)
*Songwei Dong,Zihan Chen,Chengshuai Shi,Peng Wang,Jundong Li,Cong Shen*

Main category: cs.LG

TL;DR: 提出SeqMem-Eval诊断评估框架，用于评估大语言模型在顺序任务中的记忆质量，揭示聚合指标掩盖的遗忘和负迁移问题。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要依赖最终或累积准确率等聚合指标，掩盖了遗忘和负迁移等关键失败模式。

Method: 借鉴持续学习，构建SeqMem-Eval框架，在测试时评估外部、提示介导且不修改参数的记忆状态演化，测量在线效用、保留集泛化、后向迁移和遗忘。

Result: 高最终或累积准确率不一定对应高记忆质量；许多方法虽性能提升但存在显著遗忘或负迁移。

Conclusion: 不同记忆设计在适应性与稳定性间存在权衡，标准评估指标无法揭示这些差异。
...
### [315] [Unified High-Probability Analysis of Stochastic Variance-Reduced Estimation](https://arxiv.org/abs/2605.15388)
*Zhankun Luo,Antesh Upadhyay,M. Berk Sahin,Sang Bin Moon,Anuran Makur,Abolfazl Hashemi*

Main category: cs.LG

TL;DR: 本文提出一个统一框架，通过记忆保留、重置概率和迭代移动修正项的递归，恢复经典随机方差缩减估计器并推导新变体，利用新的向量Freedman不等式得到高概率界，首次在带期望约束的随机优化中实现~O(ε^{-3})复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有随机估计器分析特定于估计器且基于期望，掩盖了决定可靠性的结构权衡，需统一理解。

Method: 构建包含记忆保留、重置概率和迭代移动修正项的三分量递归框架，恢复经典估计器并推导二阶变体，结合新的无维向量Freedman不等式推导高概率界。

Result: 得到统一高概率界，适用于欧几里得和非欧几里得空间；无约束优化获得对数依赖置信水平的oracle复杂度；带期望约束的随机优化首次实现~O(ε^{-3})复杂度，优于~O(ε^{-4})。

Conclusion: 所提统一框架有效揭示估计器结构，高概率界具广泛适用性，方差缩减显著提升约束优化效率。
...
### [316] [LPDS: Evaluating LLM Robustness Through Logic-Preserving Difficulty Scaling](https://arxiv.org/abs/2605.15393)
*Philipp Mondorf,Samuel J. Bell,Jesse Dodge,Dieuwke Hupkes*

Main category: cs.LG

TL;DR: 提出逻辑保持的难度缩放（LPDS）框架，系统性地寻找最可能导致大语言模型失败的问题变体，并发现增加难度会导致性能下降和推理错误加剧，同时基于更难变体的微调带来更一致的鲁棒性提升。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在逻辑相同但实体变化的问题上缺乏鲁棒性，且现有评估方法缺乏系统识别最易失败变体的方式，可能高估鲁棒性。

Method: LPDS框架：量化问题变体的难度，并在允许的变体空间中系统搜索以最大化难度，从而暴露模型失败。

Result: 随着难度增加，模型性能下降，推理链错误更明显；LPDS找到的困难变体导致的性能下降比随机采样高5倍；基于更难变体的微调带来更一致的鲁棒性提升。
...
### [317] [Representation Without Reward: A JEPA Audit for LLM Fine-Tuning](https://arxiv.org/abs/2605.15394)
*Biswa Sengupta*

Main category: cs.LG

TL;DR: 提出了解码器可见的JEPA目标，但实验表明隐藏状态表示与解码任务准确性弱耦合，阴性结果为稳健。


<details>
  <summary>Details</summary>
Motivation: 验证JEPA在语言模型微调中的要求：隐藏状态几何必须到达语言模型头并改进解码任务指标。

Method: 在固定Llama-3.2-1B-Instruct LoRA框架下，比较22种训练时辅助目标，包括轨迹形状正则化、分布约束、预测器/目标不对称、Fisher度量Jacobi残差和解码器可见JEPA目标。

Result: 多种辅助目标在未校正时显著，但经多重假设校正后无一对任务指标有显著提升；解码器可见JEPA目标在LoRA和全微调下均无显著改进。

Conclusion: 隐藏状态表示与解码任务准确性弱耦合，需从耦合角度重新评估LLM领域的JEPA。
...
### [318] [Breakeven complexity: A new perspective on neural partial differential equation solvers](https://arxiv.org/abs/2605.15399)
*Yijing Zhang,Nicholas Roberts,Tanya Marwah,Mikhail Khodak*

Main category: cs.LG

TL;DR: 提出基于盈亏平衡复杂度的神经PDE求解器评估框架，考虑前期成本和低精度传统求解器，发现神经求解器在困难问题上更有效。


<details>
  <summary>Details</summary>
Motivation: 当前准确率评估未考虑神经求解器高昂的前期成本和传统求解器低精度低成本的特点。

Method: 提出盈亏平衡复杂度指标，应用缩放定律分配训练预算，实现平滑误差匹配，在多个基准上评估。

Result: 神经PDE求解器在问题更困难（成本、维度、滚动、物理状态等）时更有效。

Conclusion: 该框架更全面评估神经求解器成本效益，并指出其在复杂问题上的优势。
...
### [319] [$\phi$-Balancing for Mixture-of-Experts Training](https://arxiv.org/abs/2605.15403)
*Lizhang Chen,Jonathan Li,Qi Wang,Runlong Liao,Shuozhe Li,Chen Liang,Ni Lao,Qiang Liu*

Main category: cs.LG

TL;DR: 提出φ-balancing框架，通过最小化期望路由分布的凸势函数实现专家平衡，在线算法简单高效，超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有负载平衡方法依赖噪声的小批量分配统计，存在偏差，无法实现人口级别的专家平衡。

Method: 通过最小化严格凸、对称、可微的期望路由分布势函数，利用凸对偶导出等价min-max形式，并采用镜像下降得到EMA在线算法。

Result: 在大规模预训练和下游微调中，φ-balancing持续优于Switch-style和无损失基线，专家利用更稳定有效。

Conclusion: φ-balancing是一种原则性的MoE负载平衡方法，实现高效且稳定的专家利用。
...
### [320] [Transformer Scalability Crisis: The First Comprehensive Empirical Analysis of Performance Walls in Modern Language Models](https://arxiv.org/abs/2605.15413)
*Mahdi Naser Moghadasi,Faezeh Ghaderi*

Main category: cs.LG

TL;DR: 本文首次大规模评估118个Transformer模型，揭示其可扩展性危机：序列长度从512增加到2048时，成功率从88.1%降至0%。压缩模型效率远超大型模型。


<details>
  <summary>Details</summary>
Motivation: Transformer在NLP中虽成功，但其可扩展性限制缺乏系统性实证分析，需量化其性能瓶颈。

Method: 对118个模型（7类架构）进行基准测试，测量加载时间、内存消耗和计算效率，序列长度覆盖128至2048 tokens。

Result: 88.1%模型在512 tokens成功，1024 tokens时降至44.9%，2048 tokens时全部失败。压缩模型参数效率（649.2 tokens/sec/M）远高于大型生成模型（12.5 tokens/sec/M）。

Conclusion: 挑战了现有扩展假设，首次定量证明O(n^2)注意力复杂度导致可测量的性能墙，为部署决策提供新基准方法。
...
### [321] [Margin-Adaptive Confidence Ranking for Reliable LLM Judgement](https://arxiv.org/abs/2605.15416)
*Gaojie Jin,Yong Tao,Lijia Yu,Tianjin Huang*

Main category: cs.LG

TL;DR: 提出一种学习专用置信度估计器的方法，以增强大语言模型与人类判断的一致性，通过模拟注释者多样性和间隔排序，提高了排序准确性和成功率。


<details>
  <summary>Details</summary>
Motivation: 现有假设（模型置信度与人类分歧风险单调）可能被违反，且置信度估计器的泛化行为未分析。

Method: 学习专用置信度估计器，利用模拟注释者多样性和基于间隔的排序公式，推导泛化保证，设计自适应训练过程。

Result: 提高了排序准确性，增强了置信度与分歧风险的单调关系，提高了满足目标一致性水平的成功率。

Conclusion: 该方法有效提升了LLM与人类判断的一致性，并在多个数据集和法官模型上验证。
...
### [322] [$f$-Trajectory Balance: A Loss Family for Tuning GFlowNets, Generative Models, and LLMs with Off- and On-Policy Data](https://arxiv.org/abs/2605.15417)
*Jake Fawkes,Jason Hartford*

Main category: cs.LG

TL;DR: 将GFlowNets和变分推断中使用的均方误差损失扩展到f散度族，得到一类on-policy梯度对应f散度、off-policy仍保持相同全局最小化器的损失函数，并用于设计生成模型的替代损失函数。


<details>
  <summary>Details</summary>
Motivation: 现有的均方误差损失在生成模型训练中有效，但仅限于KL散度。需要一种通用方法，能够设计适用于任意f散度的低方差、off-policy有效的损失函数。

Method: 证明on-policy梯度与f散度之间的——对应关系，从而将均方误差损失扩展到f散度族，构造出一族替代损失函数。这些损失在on-policy下梯度对应f散度，在off-policy下仍保持相同的全局最小化器。

Result: 在合成示例、SynFlowNets分子发现和异步LLM调优等任务上，模型在on-和off-policy下均保持了预测的性质（如模式覆盖等）。

Conclusion: 该构造方法为生成模型训练提供了灵活的损失函数设计框架，能够继承对应f散度的性质，并适用于off-policy数据，具有广泛适用性。
...
### [323] [Lagrangian Flow Matching: A Least-Action Framework for Principled Path Design](https://arxiv.org/abs/2605.15419)
*Shukai Du,Junzhe Zhang,Yiming Li*

Main category: cs.LG

TL;DR: 提出拉格朗日流动匹配框架，通过最小化一般拉格朗日量的作用量来确定概率路径和速度场，拓展了流动匹配的路径设计。


<details>
  <summary>Details</summary>
Motivation: 现有路径（如整流和最优传输路径）仅覆盖狭窄的动力学类别，限制于自由粒子直线轨迹。

Method: 基于最小作用量原理，构建拉格朗日流动匹配，将动态问题转化为静态最优传输问题，得到一系列免模拟训练目标。

Result: 新框架产生新的概率路径和速度场，数值实验表明与现有条件流动匹配模型竞争力相当，并带来有意义的变化。

Conclusion: 拉格朗日流动匹配提供更通用的路径设计，可衍生多种概率路径，性能有竞争力。
...
### [324] [DualKV: Shared-Prompt Flash Attention for Efficient RL Training with Large Rollouts and Long Contexts](https://arxiv.org/abs/2605.15422)
*Jiading Gai,Shuai Zhang,Xiang Song,Bernie Wang,George Karypis*

Main category: cs.LG

TL;DR: DualKV通过消除共享提示（prompt）在RL训练中的复制，实现策略更新加速2-3倍，并提升模型利用率。


<details>
  <summary>Details</summary>
Motivation: 现有FlashAttention在RL后训练中重复处理提示令牌，导致大量计算和内存浪费，尤其在长上下文和大规模rollout的场景下。

Method: 提出DualKV，包含融合CUDA前向和反向核（一次处理共享上下文和序列响应两个KV区域）以及数据管道重设计（将N(P+R)个令牌重打包为P+NR个令牌）。

Result: 在Qwen3-8B GRPO训练中实现1.63-2.09倍策略更新加速，MFU从36%提升至76%；在30B MoE规模下实现3.82倍策略更新加速和3.38倍端到端加速。

Conclusion: DualKV与标准注意力数学等价，无近似，能显著降低冗余计算，适用于大规模RL训练。
...
### [325] [Spectral Priors vs. Attention: Investigating the Utility of Attention Mechanisms in EEG-Based Diagnosis](https://arxiv.org/abs/2605.15433)
*Tawsik Jawad,Gowtham Atluri,Vikram Ravindra*

Main category: cs.LG

TL;DR: 提出频谱选择性特征构建方法，通过提取脑电信号的特定频带特征，显著提高神经退行性疾病分类性能，传统机器学习模型在此方法下可超越或匹配最优深度学习模型。


<details>
  <summary>Details</summary>
Motivation: 脑电信号噪声大、空间分辨率低，且不同疾病组间相似性高，现有深度学习模型难以有效分类神经退行性疾病。

Method: 采用频谱选择性方法，从原始脑电数据中提取主要脑波频带的信号强度，构建高价值频谱特征，并对比注意力机制模型。

Result: 基于频率和时频域的特征使传统机器学习模型性能达到或超过最优深度学习模型；注意力机制无法提取稳定的神经活动特征签名，且提供频率选择性输入也无法改善其性能。
...
TL;DR: 深度学习训练中动态生长新单元面临反向梯度饥饿问题，与剪枝并非互逆；生长在结构编辑结束时精度高，但剪枝在平均性能或从头训练时更强。


<details>
  <summary>Details</summary>
Motivation: 研究训练过程中动态调整网络结构（生长与剪枝）的差异，特别是生长新单元的困难。

Method: 通过控制实验隔离插入问题，分析新单元的前向-反向梯度差异，并干预优化状态、插入方式、单元选择等。

Result: 在卷积网络图像分类中，生长在最终稀疏网络精度高，但剪枝在平均轨迹或从头训练时更优；新单元需要足够时间整合才能提升持续学习性能。

Conclusion: 生长不仅是结构搜索操作，更是时间敏感的优化过程，其成功依赖于插入稳定性。
...
### [327] [Don't Stop Me Yet: Sampling Loss Minima via Dissipative Riemannian Mechanics](https://arxiv.org/abs/2605.15459)
*Albert Kj{\o}ller Jacobsen,Leo Uhre Jakobsen,Johanna Marie Gegenfurtner,Georgios Arvanitidis*

Main category: cs.LG

TL;DR: 提出基于物理动力学的采样器DiMS，精确采样神经网络损失函数的最小值集，在贝叶斯不确定性量化中改进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法精确采样重参数化不变解或过于局部，需平衡精确性与全局探索。

Method: 设计受动能、重力和摩擦驱动的动力学系统，从最小水平集精确采样，通过物理超参数控制探索。

Result: 在贝叶斯不确定性量化任务上，DiMS性能优于先前方法。

Conclusion: DiMS能有效探索损失函数最小值区域，改进不确定性量化。
...
### [328] [DrugSAGE:Self-evolving Agent Experience for Efficient State-of-the-Art Drug Discovery](https://arxiv.org/abs/2605.15461)
*Yikun Zhang,Xiwei Cheng,Tianyu Liu,Yuanqi Du,Wengong Jin*

Main category: cs.LG

TL;DR: 提出一种跨任务经验积累框架，在药物发现中高效构建SOTA模型。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理每次任务均需从头搜索，成本高昂。

Method: 维护跨任务记忆，包含已验证技能、统计数据与错误修复，直接迁移解决方案。

Result: 33个单任务中排名第一；跨任务评估17个任务平均得分0.935，零搜索时超越基线10-30%。

Conclusion: 跨任务记忆对药物发现中高效构建SOTA模型具有优势。
...
### [329] [Layer-wise Derivative Controlled Networks](https://arxiv.org/abs/2605.15463)
*Rowan Martnishn,Sean Anderson*

Main category: cs.LG

TL;DR: 提出ChainzRule架构，通过多项式引擎和微分正则化(DREG)同时提升准确率、硬件效率和功能稳定性。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习模型在高准确率、硬件效率和功能稳定性之间存在冲突，易出现尖峰或不稳定行为。

Method: 用多项式引擎替代标准激活函数，并通过微分正则化(DREG)对中间导数进行靶向约束。

Result: 在Fair Fight基准中参数减少15.5倍；MNIST上峰值梯度波动降低23.1%；Yelp Full有序回归准确率达70.17%。

Conclusion: 通过梯度感知架构，稳定性和准确性可以兼得。
...
### [330] [GRLO: Towards Generalizable Reinforcement Learning in Open-Ended Environments from Zero](https://arxiv.org/abs/2605.15464)
*Shangjian Yin,Yu Fu,Yue Dong,Zhouxing Shi*

Main category: cs.LG

TL;DR: 提出GRLO，从零开始通过少量交互进行RLHF训练，显著提升模型跨领域能力，计算和数据效率极高。


<details>
  <summary>Details</summary>
Motivation: 现有RL后训练（如RLVR）虽然有效但计算成本高，需要大量数据和GPU资源。研究是否可以从零开始通过少量交互的RLHF获得跨领域泛化能力。

Method: 在Qwen3-4B-Base上，仅用5K提示和22.7 GPU小时从零进行RLHF训练，称为GRLO。

Result: GRLO将平均性能从24.1提升到63.1，数据和计算量分别减少约46倍和68倍，与需大量训练的Qwen后训练模型竞争。

Conclusion: GRLO提供了一种简单高效的通用后训练方法，后续加入域内RLVR仅带来选择性提升，主要在困难竞赛数学基准上。
...
### [331] [Toward World Modeling of Physiological Signals with Chaos-Theoretic Balancing and Latent Dynamics](https://arxiv.org/abs/2605.15465)
...
TL;DR: 提出NormWear-2世界模型，编码生理信号与临床干预为共享潜在空间，实现多尺度长期预测，在多个真实数据集上超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有模型聚焦分类或短时预测，长期信号级预测与生理信号的预测本质未被充分探索。

Method: 将多变量生理信号与临床干预变量编码到共享潜在空间，结合预训练直觉与非参数潜在状态转移适应，建模为动力系统，并用混沌理论平衡动态多样性。

Result: 在8,026名受试者、涵盖日常生活到临床的各种数据集中，NormWear-2在时间、频率和潜在表示域预测性能最优，优于最先进时间序列基础模型。

Conclusion: NormWear-2为通用生理信号世界模型迈进一步，支持长期多尺度预测且保持下游表示质量。
...
### [332] [Njord: A Probabilistic Graph Neural Network for Ensemble Ocean Forecasting](https://arxiv.org/abs/2605.15470)
*Daniel Holmberg,Joel Oskarsson,Erik Larsson,Fredrik Lindsten,Teemu Roos*

Main category: cs.LG

TL;DR: 提出概率数据驱动海洋预报模型Njord，结合深度潜变量与图神经网络，在全球和区域尺度上优于确定性基线，并提供不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 海洋动力学本质混沌，现有机器学习模型仅提供确定性预报，无法刻画不确定性。

Method: 采用深度潜变量框架与图神经网络架构，通过K-means聚类网格适应不规则海面几何，实现单次前向传播采样。

Result: 在全球OceanBench基准上，上层海洋变量平均误差最低，表面温度预测改进最大；在波罗的海区域也表现良好。

Conclusion: 概率模型有效提升预报性能并提供不确定性估计，适用于全球和区域海洋预报。
...
### [333] [Learning Normalized Energy Models for Linear Inverse Problems](https://arxiv.org/abs/2605.15487)
*Nicolas Zilberstein,Santiago Segarra,Eero Simoncelli,Florentin Guth*

Main category: cs.LG

TL;DR: 提出了一种新的能量基模型，通过协方差正则化训练，无需重新训练即可处理多种线性逆问题，并具备自适应采样、无偏校正和盲估计能力。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型作为逆问题先验时，先验密度表示隐式且似然近似引入采样偏差。

Method: 采用协方差正则化训练去噪能量基模型，确保不同测量条件的一致性，可直接计算归一化后验密度。

Result: 在ImageNet、CelebA、AFHQ数据集上的图像修复和去模糊任务中，性能达到或超越现有基线。

Conclusion: 该方法保留了扩散模型的采样能力，同时实现了能量引导自适应采样、无偏Metropolis-Hastings校正和退化算子的盲估计。
...
### [334] [SurvivalPFN: Amortizing Survival Prediction via In-Context Bayesian Inference](https://arxiv.org/abs/2605.15488)
*Shi-ang Qi,Vahid Balazadeh,Michael Cooper,Russell Greiner,Rahul G. Krishnan*

Main category: cs.LG

TL;DR: SurvivalPFN是一种通过上下文学习摊销贝叶斯推理的先验数据拟合网络，在单一前向传递中完成生存分析，无需特定任务训练或超参数调优。


<details>
  <summary>Details</summary>
Motivation: 现有生存分析方法众多，选择合适估计器需要大量方法学和领域专业知识，缺乏一种通用且自动化的模型。

Method: SurvivalPFN基于先验数据拟合，通过上下文学习摊销贝叶斯推理，在多种合成、可识别且右删失的数据生成过程上预训练，推理时一次前向传递即可。

Result: 在61个数据集、21种方法和5个评估指标的大规模基准测试中，SurvivalPFN取得了强预测性能，且常优于已有生存模型。

Conclusion: SurvivalPFN为生存分析提供了一个有原则且实用的基础模型，可应用于医疗、金融和工程等高影响领域。
...
### [335] [Ghosted Layers: Unconstrained Activation Alignment for Recovering Layer-Pruned LLMs](https://arxiv.org/abs/2605.15491)
*Vincent-Daniel Yun,Junhyuk Jo,Sai Praneeth Karimireddy,Sunwoo Lee*

Main category: cs.LG

TL;DR: 提出Ghosted Layers训练免费恢复模块，通过闭式最优线性算子解决层剪枝导致的激活失配，提升性能和效率。


<details>
  <summary>Details</summary>
Motivation: 层剪枝后，保留层接收的隐藏状态与训练时分布不匹配，导致性能显著下降。

Method: 通过求解边界激活对齐问题，从少量校准集推导闭式最优线性算子，重建剪枝层引起的激活差异。

Result: 在多个LLM和剪枝策略上，一致优于现有训练免费基线，提高准确率和困惑度，同时保持层剪枝的效率优势。

Conclusion: 该方法有效解决层剪枝的激活失配问题，无需额外训练即可提升性能。
...
### [336] [Learning with Conflicts of Interest](https://arxiv.org/abs/2605.15504)
*Nischal Aryal,Arash Termehchy,Ali Vakilian,Marianne Winslett*

Main category: cs.LG

TL;DR: 本文提出博弈论框架解决机器学习系统与用户利益冲突导致的偏见信息问题。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统所有者与用户利益不一致，系统常产生偏见信息影响用户决策，而所有者缺乏实施缓解偏见的动力。

Method: 构建博弈论框架建模系统与用户互动，设计可扩展算法，提供理论保证。

Result: 算法最大化期望信息和行动，最小化偏见和操纵行动。

Conclusion: 该框架能保护用户免受不利信息影响，同时允许安全使用系统。
...
### [337] [STS: Efficient Sparse Attention with Speculative Token Sparsity](https://arxiv.org/abs/2605.15508)
*Ceyu Xu,Jiangnan Yu,Yongji Wu,Yuan Xie*

Main category: cs.LG

TL;DR: 提出无需重训练的稀疏注意力机制STS，利用小模型识别重要token实现高效LLM推理，在NarrativeQA上达到2.67倍加速和90%稀疏度，精度损失可忽略。


<details>
  <summary>Details</summary>
Motivation: 注意力机制的二次复杂度导致LLM推理内存和计算瓶颈，尤其处理百万级token的智能体应用时更突出。

Method: 在推测解码框架下，利用小型草稿模型的注意力分数动态生成token和head级别的稀疏掩码，剪枝目标LLM中昂贵的注意力计算。

Result: 在NarrativeQA基准上，约90%稀疏度下实现2.67倍加速，精度下降可忽略。

Conclusion: STS在稀疏度-精度权衡上达到新最优，相比先前技术在给定精度预算下支持更高稀疏度。
...
### [338] [parallelcbf: A composable safety-filter and auditability framework for tensor-parallel reinforcement learning](https://arxiv.org/abs/2605.15509)
*Yijun Lu,Zilei Yang,Yuyin Ma*

Main category: cs.LG

TL;DR: ParallelCBF是首个统一张量并行UAV环境、硬门CBF安全滤波器、分片BC-to-RL流水线和可操作审计功能的框架，确保端到端安全约束训练。


<details>
  <summary>Details</summary>
Motivation: 现有框架（Isaac Lab、OmniSafe、safe-control-gym、CBFKit）缺乏统一能力，无法实现端到端安全约束训练。

Method: 提供四层可组合API、CPU PyTorch双障碍CBF实现、基于属性的安全不变性测试（39个测试套件）以及可审计的数据收集流水线（31415个场景）。

Result: 完整测试套件在1.67秒内完成；审计层成功阻止了不符合预注册收敛标准的下游训练，防止了退化检查点的无声传播。

Conclusion: 可审计的框架属性对于可重复实证机器人研究是必要的，而非仅仅有用。
...
### [339] [OgBench: A Framework for Evaluating Graph Neural Networks on Omics Data](https://arxiv.org/abs/2605.15511)
*Louisa Cornelis,Johan Mathe,Louis Van Langendonck,Guillermo Bern\'ardez,Nina Miolane*

Main category: cs.LG

TL;DR: 提出OgBench基准，评估图神经网络在omics领域n≪p场景下的表现，发现简单MLP和基线常优于GNN。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络基准多关注n≫p场景，忽略omics领域n≪p（少量样本、大图）的特性，需评估GNN在此场景的表现。

Method: 构建OgBench平台，从原始omics数据生成具有不同结构属性的特征图族，标准化端到端流程，对比经典GNN、大图GNN、omics专用GNN与MLP及机器学习基线。

Result: 广泛使用的GNN在大多数任务上未优于简单MLP和经典基线，表明图结构在此域中未必提升性能。

Conclusion: 挑战了图结构自动带来价值的前提，需重新评估当前学习范式，OgBench为开发针对生物图的架构提供开源生态。
...
### [340] [On the Fragility of Data Attribution When Learning Is Distributed](https://arxiv.org/abs/2605.15520)
*Xian Gao,Bo Hui,Min-Te Sun,Wei-Shinn Ku*

Main category: cs.LG

TL;DR: 数据归因方法可能被单个参与者通过注入合成批次夸大其归因值，而不降低模型效用。


<details>
  <summary>Details</summary>
Motivation: 现有归因方法假设归因值真实反映贡献，但该假设可能失败，形成新的攻击面。

Method: 提出归因优先攻击，利用潜在优化注入小合成批次，利用非独立同分布标签覆盖和评估器敏感性。

Result: 攻击一致性地增加对手归因值，重塑良性客户间的相对归因结构，不降低精度或触发几何防御。

Conclusion: 归因本身构成新攻击面，需要开发归因鲁棒和激励兼容的评分机制。
...
### [341] [Rethinking Neural Network Learning Rates: A Stackelberg Perspective](https://arxiv.org/abs/2605.15530)
*Sihan Zeng,Sujay Bhatt,Sumitra Ganesh*

Main category: cs.LG

TL;DR: 本文通过Stackelberg优化视角，证明了非均匀学习率（体层小、末层大）可以解释为双时间尺度交替梯度下降，并建立了收敛保证，识别了两种加速机制：更强优化结构和更尖锐局部曲率。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络训练使用单一学习率，但层特定学习率可加速训练，其原理和条件尚缺乏理论理解。

Method: 将非均匀学习率训练视为Stackelberg重构的双时间尺度交替梯度下降算法，并推导其收敛性。

Result: 建立了有限时间收敛保证；发现非均匀学习率通过两种机制优于均匀学习率：1) 某些问题中Stackelberg目标具有更强优化结构，2) 早期训练中目标函数曲率更尖锐，提供更信息化的梯度。

Conclusion: 非均匀学习率通过Stackelberg优化框架可有效加速训练，并在监督学习和强化学习实验中验证。
...
### [342] [DeltaPrompts: Escaping the Zero-Delta Trap in Multimodal Distillation](https://arxiv.org/abs/2605.15532)
*Jaehun Jung,Hyunwoo Kim,Brandon Cui,Ximing Lu,David Acuna,Prithviraj Ammanabrolu,Yejin Choi*

Main category: cs.LG

TL;DR: 提出DeltaPrompts方法，通过生成高差异度的提示数据，显著提升视觉-语言模型的蒸馏效果。


<details>
  <summary>Details</summary>
Motivation: 标准蒸馏中大部分提示为“零差异”样本，导致学习信号微弱，学生模型性能快速饱和。

Method: 基于答案差异度量，设计分阶段合成管道，利用现有数据集作为种子，主动针对学生失败模式生成高差异度提示。

Result: 在多种设置下（包括同策略蒸馏、跨模型迁移、非推理模型微调）平均提升15%，尤其在优化模型Qwen3-VL-8B-Thinking上效果显著。

Conclusion: DeltaPrompts通过聚焦师生能力差距，有效突破了零差异困境，提升了推理能力。
...
### [343] [CTF4Nuclear: Common Task Framework for Nuclear Fission and Fusion Models](https://arxiv.org/abs/2605.15549)
...
TL;DR: 提出一个用于核工程中机器学习方法公平比较的通用任务框架（CTF），包含多种数据集和12个评估指标，并展示了标准基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 清洁能源需求增长，核技术是补充，但设计运行复杂；高保真模拟计算昂贵，模型方法有简化假设导致偏差；需要可靠的ML代理模型，但方法众多缺乏公平比较。

Method: 建立基于核及邻近系统精选数据集的通用任务框架（CTF），使用12个指标评估，并引入仅从稀疏测量进行系统监控的新范式。

Result: 对标准ML基线进行基准测试，揭示了当前方法的局限性。

Conclusion: CTF旨在用标准化评估替代临时比较，提升核工业科学ML的严谨性和可重复性。
...
### [344] [Characterizing Learning in Deep Neural Networks using Tractable Algorithmic Complexity Analysis](https://arxiv.org/abs/2605.15551)
*Pedram Bakhtiarifard,Sophia N. Wilson,Mahmoud Afifi,Jonathan Wensh{\o}j,Raghavendra Selvan*

Main category: cs.LG

TL;DR: 提出量化块分解方法(QuBD)来估计深度神经网络权重的算法复杂度，首次实现可扩展的KCS复杂度估计。


<details>
  <summary>Details</summary>
Motivation: 现有KCS复杂度估计方法（CTM/BDM）仅适用于小规模二元对象，无法扩展到现代深度神经网络。

Method: QuBD通过对网络权重进行量化到有限字母表，然后逐位平面聚合CTM估计，从而估计KCS复杂度。

Result: 理论证明QuBD比二值化方法有更紧的估计间隙；实验表明训练中复杂度下降，与数据预算、过拟合、grokking相关，且算法信息主要存在于最高有效位平面。

Conclusion: QuBD提供了可扩展、易处理的KCS复杂度估计，可用于大规模非二元对象如DNN权重，为理解学习机制提供新视角。
...
### [345] [CrystalBoltz: End-to-End Protein Structure Determination via Experiment-Guided Diffusion for X-Ray Crystallography](https://arxiv.org/abs/2605.15564)
*Minseo Kim,Huanghao Mai,Jay Shenoy,Alec Follmer,Gordon Wetzstein,Frederic Poitevin*

Main category: cs.LG

TL;DR: CrystalBoltz 是一个基于贝叶斯推断的生成框架，通过直接利用结构因子振幅进行晶体学精修，在降低坐标 RMSD 和 R 因子的同时，将运行时间缩短了 33 倍。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型无法直接利用新的晶体学实验测量数据，而晶体结构测定需要既结构合理又与衍射数据一致的候选模型，且通常需要大量人工精修。

Method: 将晶体学精修视为对原子结构的贝叶斯推断，直接作用于结构因子振幅；从预训练先验的无引导生成转向实验引导的后验采样，然后进行原子坐标和 B 因子精修。

Result: 在多个蛋白质晶体学数据集上，CrystalBoltz 的坐标 RMSD 和 R 因子均低于最强基线，且运行时间缩短了 33 倍。

Conclusion: CrystalBoltz 通过将实验信息直接整合到生成框架中，实现了更准确、更快速的晶体结构精修。
...
### [346] [AstraFlow: Dataflow-Oriented Reinforcement Learning for Agentic LLMs](https://arxiv.org/abs/2605.15565)
*Haizhong Zheng,Yizhuo Di,Jiahui Wang,Shuowei Jin,Xueshen Liu,Yongji Wu,Z. Morley Mao,Ion Stoica,Jiawei Zhao,Beidi Chen*

Main category: cs.LG

TL;DR: 提出AstraFlow，一种数据流导向的强化学习系统，通过解耦组件支持多策略智能体RL，提升训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有RL系统扩展困难，成本高，每次新功能需专用系统工程，因为训练器中心控制架构和缺乏抽象原则。

Method: 采用数据流架构，将rollout服务、数据流管理和训练解耦为自治组件。

Result: 在数学、代码、搜索和AgentBench任务中，无需修改系统代码即可支持多策略训练、弹性扩展、异构跨区域执行和可组合数据算法；多策略协作训练中加速2.7倍且精度相当或更好。

Conclusion: AstraFlow通过数据流抽象有效支持复杂多策略智能体RL，降低系统工程成本，提升资源利用效率。
...
### [347] [Gaussian Relational Graph Transformer](https://arxiv.org/abs/2605.15575)
*Zezhong Ding,Jin Li,Xugang Wang,Xike Xie*

Main category: cs.LG

TL;DR: 提出GelGT，一种高斯关系图变换器，通过结构-语义协同采样和高斯注意力机制，同时建模结构、语义和时间信息，在关系图预测任务中实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有关系图学习方法难以捕获长程依赖，且缺乏联合建模结构、语义和时间信息的能力。

Method: 提出结构-语义协同采样策略保持结构连通性并过滤无关语义，在高斯图注意力机制中引入可学习高斯偏置动态编码时间依赖。

Result: 在多个真实数据集上达到最先进下游任务性能，预测性能提升高达13.8%。

Conclusion: GelGT有效解决了关系图学习中的长程依赖和多模态信息联合建模挑战。
...
### [348] [Embracing Biased Transition Matrices for Complementary-Label Learning with Many Classes](https://arxiv.org/abs/2605.15586)
*Tan-Ha Mai,Chao-Kai Chiang,Han-Hwa Shih,Gang Niu,Masashi Sugiyama,Hsuan-Tien Lin*

Main category: cs.LG

TL;DR: 互补标签学习（CLL）通过设计非均匀标签生成过程，解决了传统方法在大多类场景下的性能瓶颈。


<details>
  <summary>Details</summary>
Motivation: 传统CLL方法假设均匀标签生成，在大量类别下稀释学习信号，导致扩展到大标签空间困难。

Method: 提出偏置诱导约束标记（BICL）框架，通过非均匀生成过程将互补标签限制在类别子集，从而增强学习信号。

Result: 在CIFAR-100和TinyImageNet-200上实现超过七倍的准确率提升。

Conclusion: BICL开创了使CLL适用于现实世界大多类应用的新路径。
...
### [349] [Offline Reinforcement Learning with Universal Horizon Models](https://arxiv.org/abs/2605.15603)
*Hojun Chung,Junseo Lee,Songhwai Oh*

Main category: cs.LG

TL;DR: 提出通用视野模型（UHM），通过直接预测任意视野下的未来状态，并采用缩尾视野分布稳定训练，在OGBench任务上超越基线。


<details>
  <summary>Details</summary>
Motivation: 基于模型的强化学习在离线场景中因自生成状态的重复推理导致复合误差；几何视野模型（GHM）试图缓解但难以准确建模远期状态。

Method: UHM直接预测任意视野下的未来状态，并利用缩尾视野分布限制过大视野以稳定训练。

Result: 在100个OGBench任务上，UHM优于竞争基线，尤其是在次优数据集和长视野推理任务中。

Conclusion: UHM通过灵活预测未来状态有效提升了离线强化学习性能。
...
### [350] [VSPO: Vector-Steered Policy Optimization for Behavioral Control](https://arxiv.org/abs/2605.15604)
*Xuechen Zhang,Zijian Huang,Kai Yang,Weijia Zhang,Jiasi Chen,Samet Oymak*

Main category: cs.LG

TL;DR: 提出向量引导策略优化（VSPO），通过修改GRPO引入行为强度控制向量，解决语言模型多目标优化中的稀疏奖励问题，在推理任务中改善目标行为控制的同时保持准确率。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型需同时优化主要准确率目标和次要行为偏好（如冗长性、同意度、专业水平），但基础模型可能极少展现目标行为，造成稀疏行为奖励瓶颈。

Method: VSPO通过修改GRPO，在不同引导强度下采样轨迹，将其视为在线潜在自蒸馏过程，使模型内化引导向量，从而对稀有行为进行上采样并丰富轨迹多样性。

Result: 在数学推理和MMLU-Pro等基准上，针对解释专业性、自信表达、抗误导鲁棒性和冗长性四种行为，VSPO相比奖励塑形、教师轨迹蒸馏和引导基线，一致改善了对目标行为的控制，同时保持或提升了任务准确率。理论分析表明，当引导分布与目标行为充分对齐时，VSPO的迭代复杂度优于奖励塑形GRPO。

Conclusion: VSPO通过缓解稀疏奖励问题并加速策略优化，在多目标优化中展现出优于奖励塑形和其他替代方法的特性。
...
### [351] [Transformer-like Inference from Optimal Control](https://arxiv.org/abs/2605.15608)
*Aditya Kudre,Heng-Sheng Chang,Prashant G. Mehta*

Main category: cs.LG

TL;DR: 从最优控制理论出发，推导出解码器-only Transformer的推理架构，揭示了其层结构与双滤波器对应。


<details>
  <summary>Details</summary>
Motivation: 理解Transformer如何从第一性原理实现序列预测，并探索其与最优控制的联系。

Method: 将序列预测问题重构为最优控制问题，针对离散非线性模型和线性高斯模型分别推导出双滤波器算法，其层结构与解码器-only Transformer一致。

Result: 数值实验表明，当嵌入维度不足时，Transformer隐式利用非马尔可夫结构，而最优控制方法能解释注意力权重。

Conclusion: Transformer的层操作可视为最优控制问题的解，为理解其内部机制提供了新视角。
...
### [352] [Position: Zeroth-Order Optimization in Deep Learning Is Underexplored, Not Underpowered](https://arxiv.org/abs/2605.15622)
...
TL;DR: 本文重新评估零阶优化，认为其被低估而非性能不足，并提出了六个观点和三个新机遇。


<details>
  <summary>Details</summary>
Motivation: 零阶优化因估计方差和查询复杂度高而被认为无法扩展，但作者认为这种结论源于短视的实践，需要重新审视其潜力。

Method: 通过方差控制、方差-查询权衡和方向导数分析重新审视基于估计的零阶方法的可行性边界，并识别三个未充分探索的机会：子空间和谱视角、前向传播的系统优势以及评估的去混淆。

Result: 零阶优化在方差减少、查询规模扩展、通信高效和资源受限训练中具有独特优势。

Conclusion: 应围绕零阶优化的独特优势重新思考其算法、系统和评估，以开辟大规模、系统感知和资源高效的学习路径。
...
### [353] [IO-SVD: Input-Output Whitened SVD for Adaptive-Rank LLM Compression](https://arxiv.org/abs/2605.15626)
*Ali Abbasi,Chayne Thrash,Haoran Qin,Hamed Pirsiavash,Soheil Kolouri*

Main category: cs.LG

TL;DR: 提出IO-SVD，一种基于KL感知双侧白化空间的SVD后训练压缩方法，结合异质秩分配和损失感知量化，实现高效LLM压缩。


<details>
  <summary>Details</summary>
Motivation: 现有SVD压缩方法依赖输入白化、同质秩分配或损失无关启发式，在强压缩下难以保持模型质量。

Method: 利用KL损失二阶展开构建输出侧度量，结合输入白化形成双侧白化空间；通过一阶校准损失评分实现异质秩分配和全局剪枝；采用损失感知重映射优化混合SVD-量化压缩。

Result: 在不同LLM和VLM家族上，IO-SVD在最小性能下降下压缩模型并提供实际推理加速。

Conclusion: IO-SVD是一种有效的后训练压缩方法，结合双侧白化和损失感知分配，在保持质量的同时实现压缩和加速。
...
### [354] [Perforated Neural Networks for Keyword Spotting](https://arxiv.org/abs/2605.15647)
*Vishy Gopal,Aris Ilias Goutis,Ralph Crewe,Erin Yanacek,Rorry Brenner*

Main category: cs.LG

TL;DR: 提出穿孔反向传播应用于边缘设备上的关键词识别，在Edge Impulse平台上以1500参数达到0.933精度，优于基线模型的4000参数0.921精度。


<details>
  <summary>Details</summary>
Motivation: 边缘机器学习面临严格的内存、计算和精度约束，现有压缩技术难以同时提升精度和减小模型大小。

Method: 在标准CNN中添加人工树突节点，采用穿孔反向传播训练，在Edge Impulse关键词识别管道上进行800次超参数试验。

Result: 最佳树突模型以1500参数达到0.933测试精度，基线模型需4000参数达到0.921精度，树突模型在所有参数和精度阈值下均表现更优。

Conclusion: 穿孔反向传播能同时提升模型质量和部署效率，是边缘AI工程师的有力工具。
...
### [355] [Towards Code-Oriented LM Embeddings for Surrogate-Assisted Neural Architecture Search](https://arxiv.org/abs/2605.15649)
*Pranav Somu,Advay Balakrishnan,Stepan Kravtsov,Aaron McDaniel,Jason Zutty*

Main category: cs.LG

TL;DR: 提出了COLE方法，利用预训练语言模型从PyTorch代码中提取架构嵌入，无需微调即可高效预测性能，显著降低NAS成本。


<details>
  <summary>Details</summary>
Motivation: 现有NAS代理通常需要昂贵的微调或复杂表示，本文旨在消除这些开销，实现低成本的性能预测。

Method: 将架构表示为PyTorch类定义文本，使用现成语言模型提取嵌入（COLE），再通过轻量回归头预测性能，并探索改进嵌入质量的策略。

Result: 在NAS-Bench-201和einspace上，原始代码输入优于其他文本编码；COLE驱动BANANAS算法提升搜索效率；在CIFAR-100上，用COLE替代结构路径编码，达到最优架构1%内所需评估预算减少34%。

Conclusion: COLE作为一种基于代码的通用高效表示，为推进NAS提供了灵活且低成本的基础。
...
### [356] [Sharp Spectral Thresholds for Logit Fixed Points](https://arxiv.org/abs/2605.15651)
*Tongxi Wang*

Main category: cs.LG

TL;DR: 证明了仿射logit系统稳定性的精确阈值β‖ΠWΠ‖_{T→T}<2，扩展了稳定区域至奖励响应但全局可预测的系统。


<details>
  <summary>Details</summary>
Motivation: 经典理论仅保证强随机化条件下的稳定性，未能识别真正的定性变化点。

Method: 通过分析有限维仿射logit系统，推导出与维度无关的欧几里得阈值。

Result: 新阈值β‖ΠWΠ‖_{T→T}<2取代了先前保守条件，填补了分岔前的缺失稳定区间。

Conclusion: 扩大了认证稳定边界，并确定了模型发生相变的真实位置。
...
### [357] [On the Power of Adaptivity for $\varepsilon$-Best Arm Identification in Linear Bandits](https://arxiv.org/abs/2605.15663)
*Arnab Maiti,Yunbei Xu,Kevin Jamieson*

Main category: cs.LG

TL;DR: 本文研究了线性 bandit 中 ε 最佳臂识别的极小化样本复杂度，提出了非自适应固定设计方法并匹配下界，探讨了自适应采样的效用，并展示了在某些动作集上自适应可带来多项式级改进。


<details>
  <summary>Details</summary>
Motivation: 确定线性 bandit 中 ε 最佳臂识别所需的最小样本数，并理解自适应与非自适应方法之间的性能差距。

Method: 非自适应固定设计方法基于高斯宽度；自适应方法利用 ℓ2 范数估计子程序。

Result: 非自适应方法样本复杂度为 O(d log(1/δ)/ε^2 + w(X)^2/ε^2)，且匹配下界；对超立方体、ℓ2 球等动作集，自适应仅带来对数因子改进；首次构造了自适应可实现多项式改进的动作集。

Conclusion: 自适应采样的优势依赖于动作集的结构，存在动作集使得自适应方法显著优于任何非自适应算法。
...
### [358] [Rule2DRC: Benchmarking LLM Agents for DRC Script Synthesis with Execution-Guided Test Generation](https://arxiv.org/abs/2605.15669)
*Jinuk Kim,Junsoo Byun,Donghwi Hwang,Seong-Jin Park,Hyun Oh Song*

Main category: cs.LG

TL;DR: 提出Rule2DRC基准和SplitTester代理，用于评估和改进LLM生成DRC脚本的准确性，通过执行反馈和判别测试提升脚本选择性能。


<details>
  <summary>Details</summary>
Motivation: 将自然语言设计规则翻译为DRC脚本依赖人工且易错，现有基准规模小且评估方式不准确，需要更好的自动评估和调试方法。

Method: 构建含1000个规则-脚本任务和13921个芯片布局的基准；提供基于DRC执行结果的评估管道；提出SplitTester代理，利用执行反馈生成判别测试用例以区分候选脚本。

Result: SplitTester显著提升了Best-of-N脚本选择的性能，在DRC脚本合成领域取得改进。

Conclusion: Rule2DRC和SplitTester为自动DRC脚本生成和验证提供了有效工具，未来可促进相关研究。
...
### [359] [Interaction-Aware Influence Functions for Group Attribution](https://arxiv.org/abs/2605.15675)
*Jaeseung Heo,Kyeongheung Yun,Youngbin Choi,Sehyun Hwang,Jungseul Ok,Dongwoo Kim*

Main category: cs.LG

TL;DR: 提出交互感知的影响函数，通过二阶展开捕获示例间交互，提升组级影响估计的准确性。


<details>
  <summary>Details</summary>
Motivation: 标准影响函数通过求和个体影响估计组级影响，无法捕捉示例间的冗余或互补性。

Method: 将目标函数展开至二阶，在一阶和基础上加入成对交互项，度量两个示例对目标影响的协同程度。

Result: 在六个数据集-模型对（逻辑回归、MLP、ResNet-9）上，跟踪留组重训练效果优于一阶影响；在Llama-3.1-8B指令微调数据选择中，7个下游任务5个超越基线，且标准影响函数选择不如随机。

Conclusion: 交互感知影响函数能更准确估计组级影响，在数据选择等任务中有应用价值。
...
### [360] [FRWKV+: Adaptive Periodic-Position Branch Interaction for Frequency-Space Linear Time Series Forecasting](https://arxiv.org/abs/2605.15690)
*Qingyuan Yang,Dongyue Chen,Da Teng,Junhua Xiao,Jiaji Pan,Shizhuo Deng*

Main category: cs.LG

TL;DR: 提出了FRWKV+模型，通过选择性周期性位置分支交互改进长期时间序列预测。


<details>
  <summary>Details</summary>
Motivation: 频域预测中，复数谱的实部和虚部信息交换不足，且周期性位置线索仅在可靠时有效。

Method: 引入跨分支门控交换实部和虚部上下文，自适应相位门提取周期性位置信息并生成符号修正，自适应信任机制控制修正强度。

Result: 在匹配协议下，FRWKV+在家族变体中取得最大MSE胜率覆盖，在周期性区域有明显提升。

Conclusion: 组件分析证实了周期性位置上下文、符号修正和自适应信任的有效性，但简单修正规则在边界情况下仍更优。
...
### [361] [SEED: Targeted Data Selection by Weighted Independent Set](https://arxiv.org/abs/2605.15691)
*Yuan Zhang,Lifeng Guo,Junwen Pan,Chang Liu,Wenzhao Zheng,Kuan Cheng,Kurt Keutzer,Shanghang Zhang*

Main category: cs.LG

TL;DR: 提出了SEED，是一种基于加权独立集并引入节点值校准和局部尺度归一化的鲁棒数据选择方法，构建了紧凑多模态数据集，在多种任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据选择方法难以平衡样本质量与多样性，节点权重易受噪声干扰，且跨域分布导致图结构失衡。

Method: 将数据选择建模为加权独立集问题，通过节点值校准（限制影响力估计到双边显著子空间）和局部尺度归一化（自适应边阈值）改进。

Result: 构建的Honeybee-Remake-SEED-200K数据集在指令微调、视觉指令微调和语义分割任务上优于现有最优方法。

Conclusion: SEED方法能有效平衡质量与多样性，鲁棒且可扩展，适用于多种模型族。
...
### [362] [Tighter Regret Bounds for Contextual Action-Set Reinforcement Learning](https://arxiv.org/abs/2605.15692)
*Zijun Chen,Zihan Zhang*

Main category: cs.LG

TL;DR: 研究了动作集随情节变化的情境强化学习，将MVP算法扩展至此框架，并给出了理论遗憾界。


<details>
  <summary>Details</summary>
Motivation: 实际场景中动作集常受情境影响，需要处理这种泛化以提升强化学习实用性。

Method: 扩展MVP算法到动作上下文框架，通过分析对抗性和随机情境下的遗憾界，推导样本复杂度及差距依赖界。

Result: 对抗性情境下极小极大遗憾界为~O(√(SAH³K log L))，随机情境下为~O(√(SAH³K))，并得到样本复杂度界~O(SAH³/ε²)及差距依赖界。

Conclusion: MVP算法在动作上下文框架中仍具有强理论保证，尤其在情境多变或子最优间隙大时性能优异。
...
### [363] [Going Beyond the Edge: Distributed Inference of Transformer Models on Ultra-Low-Power Wireless Devices](https://arxiv.org/abs/2605.15694)
*Alexander Gr\"afe,Ding Huo,Johannes Berger,Marco Zimmerling,Sebastian Trimpe*

Main category: cs.LG

TL;DR: CATS是一个通信感知的分布式Transformer推理框架，使超低功耗无线设备能协作执行远超单设备能力的模型。


<details>
  <summary>Details</summary>
Motivation: Transformer模型的计算和内存需求远超单个超低功耗IoT设备的能力。

Method: 采用通信感知的分布式推理方案，包括基于SomeGather剪枝通信原语的模型分区，以及训练时消息丢弃以增强鲁棒性。

Result: 实际部署中，16个设备可协作执行比单设备大14倍的Transformer模型。

Conclusion: 首次将分布式Transformer推理引入超低功耗无线设备，证明了可行性。
...
### [364] [AGOP-IxG: A Gradient Covariance Filter for Local Feature Attribution on Tabular Data, with a Controlled Benchmark](https://arxiv.org/abs/2605.15700)
*Raj Kiran Gupta Katakam*

Main category: cs.LG

TL;DR: 提出一种针对表格分类器的快速逐样本归因方法AGOP-IxG，在合成数据集上优于SHAP等基线，速度提升350-1650倍。


<details>
  <summary>Details</summary>
Motivation: 现有归因方法多凭经验选择，缺乏基于真实归因的严格评估，需要一种可验证的方法。

Method: 将逐样本梯度乘以截断秩为K的平均梯度外积矩阵（AGOP）进行预处理。

Result: 在三个合成多分类任务中，AGOP-IxG在Spearman秩相关和噪声特征质量上领先，在交互数据集上top-k精度最优，且速度比SHAP快350-1650倍。

Conclusion: AGOP-IxG针对局部归因优化，而非全局特征排序。
...
### [365] [Differentiable Mixture-of-Agents Incentivizes Swarm Intelligence of Large Language Models](https://arxiv.org/abs/2605.15706)
*Xingjian Wu,Junkai Lu,Siyu Yan,Xiangfei Qiu,Jilin Hu,Chenjuan Guo,Bin Yang*

Main category: cs.LG

TL;DR: 提出可微分混合代理框架，通过动态路由和自适应协作提升多代理系统的灵活性与性能。


<details>
  <summary>Details</summary>
Motivation: 现有多代理系统依赖固定通信拓扑，缺乏对动态任务需求的灵活性和适应性。

Method: 采用可微分上下文感知路由机制，结合循环结构实现稀疏激活，并利用预测熵自监督信号优化路由。

Result: 在9个基准测试中取得最先进性能，同时表现出强效性、鲁棒性和集成能力。

Conclusion: DMoA实现自演化动态多代理协作，有效提升复杂推理任务的适应性和性能。
...
### [366] [Bidirectional Fusion Guided by Cardiac Patterns for Semi-Supervised ECG Segmentation](https://arxiv.org/abs/2605.15722)
*Jeonghwa Lim,Minje Park,Sunghoon Joo*

Main category: cs.LG

TL;DR: 提出CardioMix，一种基于心脏模式引导的双向CutMix框架，用于半监督心电图分割，有效利用未标记数据增强标记集，并在多个数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 心电图精确分割对心血管诊断至关重要，但标注数据稀缺。传统半监督方法仅利用未标记数据的一致性，未充分利用标记与未标记集之间的信息交换。

Method: CardioMix采用双向CutMix策略，由心脏模式引导，在标记集与未标记集之间进行混合，生成生理上合理的增强样本，并作为即插即用模块兼容多种半监督算法。

Result: 在半监督心电图分割基准（SemiSegECG）上，CardioMix作为即插即用模块，在不同数据集和标注比例下均优于现有CutMix融合策略。

Conclusion: CardioMix通过双向心脏模式引导的混合，显著提升了半监督心电图分割的性能，且具有很好的通用性。
...
### [367] [GOMA: Toward Structure-Driven Multimodal Alignment from a Graph Signal Smoothing Perspective](https://arxiv.org/abs/2605.15723)
*Xu Wang,Xunkai Li,Yinlin Zhu,Rong-Hua Li,Guoren Wang*

Main category: cs.LG

TL;DR: 提出GOMA框架，利用多模态属性图的结构上下文对冻结的视觉-语言嵌入进行后对齐，在七个基准上取得最优检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有CLIP-style双编码器仅从孤立图像-文本对学习多模态对齐，忽略了实体间的结构关系，而多模态属性图（MAG）中的结构上下文未被充分利用。

Method: GOMA将冻结的多模态嵌入视为图信号，解耦三个关键设计：学习模态感知传播算子、执行有限步耦合平滑（无对角跨模态捷径）、自适应读取节点特定平滑轨迹以保留有用信息。

Result: 在七个MAG基准上，GOMA达到最先进或并列最先进的检索性能，且比最强图竞争者更稳定。

Conclusion: MAG结构可作为冻结多模态嵌入的有效后编码器，通过结构驱动的后对齐提升检索性能。
...
### [368] [A Unified Perturbation Framework for Analyzing Leaderboard Stability and Manipulation](https://arxiv.org/abs/2605.15761)
*Hosna Oyarhoseini,Jimmy Lin,Amir-Hossein Karimi*

Main category: cs.LG

TL;DR: 提出一个统一的扰动框架，基于影响函数分析Bradley-Terry排行榜的稳健性，发现现代排行榜不稳健：小于1%的定向扰动即可改变排名第一的模型、降低Kendall's tau和改变置信区间。


<details>
  <summary>Details</summary>
Motivation: 评估排行榜（如LMArena）在基准测试中至关重要，但排名的稳健性尚不清楚，需要系统分析。

Method: 构建匹配级别扰动（删除、添加、翻转）和选手移除的影响近似框架，通过影响分数评估对top-k成员、全局排名一致性（Kendall's tau）和置信区间的影响。

Result: 在Chatbot Arena等数据集上，小于1%的定向扰动可改变第一名模型、降低Kendall's tau并改变置信区间，且影响分数可高效实现定向提升或降低特定模型。

Conclusion: 该框架提供了标准化数据集级别的稳健性评分，是审计排行榜稳定性、推动更稳健评估协议的有用工具。
...
### [369] [Continual Learning of Domain-Invariant Representations](https://arxiv.org/abs/2605.15775)
*Pascal Janetzky,Tobias Schlagenhauf,Stefan Feuerriegel*

Main category: cs.LG

TL;DR: 提出一种持续学习方法，通过顺序学习域不变表示来避免短期学习，提升对未见域的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法优化域内性能，但易学习域特异性线索，导致对未见域泛化差。

Method: 结合回放训练和顺序不变性对齐，逐步学习并保持域不变结构。

Result: 在视觉、医学、制造、生态等六个基准和真实数据集上，所提方法在未见目标域泛化上持续优于现有基线。

Conclusion: 首次将域不变表示学习方法引入持续学习，有效提升泛化性能。
...
### [370] [Grokking as Structural Inference: Transformers Need Bayesian Lottery Tickets](https://arxiv.org/abs/2605.15787)
*Kai Hidajat,Solden Stoll,Joseph An*

Main category: cs.LG

TL;DR: 本文形式化注意力为任务依赖图上的隐式贝叶斯后验，证明泛化需要MLP容量Goldilocks界和注意力贝叶斯结构条件。延迟泛化源于结构推断延迟，权重衰减消除记忆后图才可学习。提出KL结构干预可绕过延迟，实验验证贝叶斯票证有效性。


<details>
  <summary>Details</summary>
Motivation: 解释Transformer在记忆训练集后需数千步才能泛化的grokking现象，现有理论忽略了注意力的独特约束。

Method: 将注意力形式化为任务依赖图上的隐式贝叶斯后验，推导泛化条件；通过KL散度结构干预绕过结构推断延迟。

Result: 证明了泛化延迟的结构等待时间，推导出逆权重衰减延迟和逆干预强度标度律；实验显示贝叶斯票证与彩票票证性能相当或更优。

Conclusion: 注意力需将足够质量置于每个信息性标记上，结构推断延迟是grokking关键，KL干预可加速泛化。
...
### [371] [Learning Context-conditioned Gaussian Overbounds for Convolution-Based Uncertainty Propagation](https://arxiv.org/abs/2605.15789)
*Ruirui Liu,Xuejie Hou,Yiping Jiang,Hui Ren*

Main category: cs.LG

TL;DR: 提出了一个统一学习框架，通过神经网络训练得到上下文感知的高斯过界，在有限分位数网格上可证明保守性，并在假设下扩展到连续尾部保守性，支持保守线性组合和卷积分析。


<details>
  <summary>Details</summary>
Motivation: 安全关键领域需要保守的不确定性估计，但传统预测器级别区间无法组合，经典高斯过界过于保守且难以适应特征条件误差。

Method: 训练神经网络输出均值与尺度参数的高斯过界，采用过界损失（在选定分位数强制保守性）与Wasserstein项（惩罚分布距离），并在三个正则性假设下保证连续尾部保守性。

Result: 在合成数据及多径、电离层、对流层残差等真实数据集上，相比传统方法获得更紧的界，同时保持保守性。

Conclusion: 该框架与模态无关，适用于动态环境中需要保守、特征条件不确定性估计的学习系统。
...
### [372] [AOT-POT: Adaptive Operator Transformation for Large-Scale PDE Pre-training](https://arxiv.org/abs/2605.15793)
*Qitan Lv,Hong Wang,Zhongkai Hao,Wen Wu,Xuenan Xu,Bowen Zhou,Feng Wu,Chao Zhang*

Main category: cs.LG

TL;DR: 受经典数值分析启发，提出自适应算子变换（AOT-POT）方法，将多样复杂的PDE解算子转化为统一形式，以便单个神经算子联合建模，在12个PDE基准上仅增加3%参数即达到最优，相对L2误差平均降低40.9%。


<details>
  <summary>Details</summary>
Motivation: 现有预训练神经算子主要通过增加模型容量应对多PDE数据，未改变目标算子本身，而不同PDE算子结构差异大，导致联合建模困难。

Method: AOT-POT通过扩展隐藏表示为多个并行流，在子层前后自适应聚合和重分配，并利用Sinkhorn投影双随机矩阵稳定混合流，从而将多样算子重塑为统一形式。

Result: 在12个PDE基准上，AOT-POT仅增加3%参数即达到SOTA，相对L2误差降低最高77.6%（平均40.9%）；微调后在域内PDE上误差降低92%，域外PDE上降低89%。

Conclusion: 自适应算子变换是推进PDE基础模型的有效且互补方向，超越单纯扩大模型容量。
...
### [373] [Martingale Neural Operators: Learning Stochastic Marginals via Doob-Meyer Factorization](https://arxiv.org/abs/2605.15806)
*Kai Hidajat*

Main category: cs.LG

TL;DR: 基于Doob-Meyer定理提出Martingale Neural Operator (MNO)，直接预测随机PDE终端分布的均值和协方差，显著降低Wasserstein距离并提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有神经算子在随机PDE中退化为条件均值，丢失方差和尾部结构，而蒙特卡洛或生成模型牺牲了单步效率和分辨率不变性。

Method: 利用Doob-Meyer定理将半鞅分解为可预测漂移和零均值鞅，设计MNO参数化为漂移-like均值和低秩因子B_phi，保证B_phi^T B_phi半正定，采用高斯残差实例化。

Result: 在1D SPDE、粗糙波动率和2D任务中，Wasserstein距离减少最多120倍（phi^4场论）和68倍（随机Burgers），计算速度比条件扩散基线快约3倍；2D任务中与FNO性能相当；拟确定系统如Gray-Scott仍失败。

Conclusion: MNO有效恢复随机PDE的不确定性结构，保持神经算子优势，但在完全确定系统上存在局限。
...
### [374] [Intrinsic Wasserstein Rates for Score-Based Generative Models on Smooth Manifolds](https://arxiv.org/abs/2605.15822)
*Guoji Fu,Taiji Suzuki,Wee Sun Lee,Atsushi Nitanda*

Main category: cs.LG

TL;DR: 证明基于得分的生成模型在低维流形数据上达到内在Wasserstein-1样本指数，并分离了几何、密度等因子。


<details>
  <summary>Details</summary>
Motivation: 分析高维空间中低维流形数据上的得分生成模型样本复杂度。

Method: 将得分逼近分为大噪声切触胞体和小噪声投影中心去高斯化拉普拉斯两个区域，并使用ReLU实现最近投影坐标。

Result: 获得样本指数$\tilde{\mathcal{O}}(D^{\mathcal{O}_\beta(d)}n^{-(\beta+1)/(d+2\beta)})$，显式依赖于几何和密度。

Conclusion: 对于几何和密度有界族，构建的得分网络参数具有多项式环境依赖。
...
### [375] [From Observed Viability to Internal Predictive Approximation: A Single-Subject Latent-Space Analysis of Gait Dynamics Under Occlusal Constraint](https://arxiv.org/abs/2605.15862)
*Jacques Raynal,Pierre Slangen,Elsa Raynal,Jacques Margerit*

Main category: cs.LG

TL;DR: 本文通过单受试者实验，使用神经网路近似帕金森患者步态组织的纵向潜在变换，发现模型能保持条件间的位移层次，但结论仅限于该数据集内部近似。


<details>
  <summary>Details</summary>
Motivation: 适应性生物力学系统可能潜藏不同的组织结构，需要验证纵向步态组织变化能否在潜在空间框架中近似。

Method: 对一名帕金森受试者在两个时间点（间隔11周）使用仪器鞋垫记录步态，测试6种咬合条件；通过主成分分析构建PC1-PC2潜在表示，并训练前馈神经网络近似从M1到M2的变换。

Result: 模型在3种中心关系条件下再现了OC3 < ONL < OC2.5的位移顺序，6种条件分析也保持了全局结构；留出和留条件分析显示条件依赖的近似变异性。

Conclusion: 不支持概括性预测或临床因果，仅支持单受试者数据中纵向潜在变换可被内部近似的有限结论。
...
### [376] [Ti-iLSTM: A TinyDL Approach for Logic-Level Anomaly Detection in Industrial Water Treatment Systems](https://arxiv.org/abs/2605.15874)
*Mandar Joshi,Farzana Zahid,Judy Bowen,Matthew M.Y. Kuo,Valeriy Vyatkin,Emil Karlsson*

Main category: cs.LG

TL;DR: 提出基于TinyDL的增量LSTM（Ti-iLSTM）框架，用于在资源受限的PLC上检测工业水处理系统中的逻辑层欺骗异常，在SWaT数据集上取得F1=0.983和ROC-AUC=0.998，并在WADI数据集上验证了泛化能力。


<details>
  <summary>Details</summary>
Motivation: 工业水处理系统易受逻辑层欺骗攻击，传统阈值监控和服务器端模型难以在资源受限的PLC上部署，需要轻量级在线异常检测方法。

Method: 提出Ti-iLSTM框架，优化LSTM的内存和空间占用，结合逻辑感知监督与TinyDL序列学习，实现轻量级逻辑层不一致性检测。

Result: 在SWaT数据集上F1-score=0.983，ROC-AUC=0.998；在WADI数据集上验证了泛化性能。

Conclusion: 逻辑感知监督与TinyDL结合可有效在资源受限PLC上实现高效准确的异常检测，适用于工业环境。
...
### [377] [Shapley Neuron Values for Continual Learning: Which Neurons Matter Most?](https://arxiv.org/abs/2605.15877)
*Mohammad Ali Vahedifar,Abhisek Ray,Qi Zhang*

Main category: cs.LG

TL;DR: 提出Shapley神经元估值（SNV）框架，基于合作博弈论量化神经元重要性，通过选择性冻结重要神经元实现无缓冲的持续学习，在ImageNet-1k上超越现有无缓冲方法。


<details>
  <summary>Details</summary>
Motivation: 解决神经网络在持续学习中的灾难性遗忘问题。

Method: 基于合作博弈论的Shapley神经元估值（SNV），量化神经元重要性，选择性冻结重要神经元，保持其他神经元可塑性，无需缓冲区或扩展架构。

Result: 在ImageNet-1k上，类增量学习准确率提升+2.88%，任务增量学习提升+6.46%，均优于第二名基线。

Conclusion: SNV框架实现了无缓冲、无架构扩展的持续学习，有效缓解灾难性遗忘。
...
### [378] [Practical Validity Conditions for Byzantine-Tolerant Federated Learning](https://arxiv.org/abs/2605.15887)
...
### [379] [CHoE: Cross-Domain Heterogeneous Graph Prompt Learning via Structure-Conditioned Experts](https://arxiv.org/abs/2605.15888)
*Peiyuan Li,Yongqi Huang,Jitao Zhao,Dongxiao He,Di Jin,Weixiong Zhang*

Main category: cs.LG

TL;DR: 提出CHoE，一种基于专家网络的跨域异构图提示学习方法，通过结构条件专家训练、结构感知路由和负载均衡以及提示语义融合，解决现有方法域外性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 现有异构图提示学习方法仅适用于域内场景，当应用领域变化时性能下降，难以部署到跨域真实环境。

Method: 预训练阶段训练结构条件专家；提示调优阶段采用结构感知专家路由和负载均衡机制选择结构兼容的专家；设计提示基语义融合模块整合多视图表示。

Result: 在少样本跨域应用中，CHoE一致提升性能，超越所有基线方法。

Conclusion: CHoE有效实现了跨域异构图提示学习，拓展了HGPL的适用性。
...
### [380] [From Layers to Networks: Comparing Neural Representations via Diffusion Geometry](https://arxiv.org/abs/2605.15901)
*Atharva Khandait,Jan E. Gerken*

Main category: cs.LG

TL;DR: 提出利用扩散几何进行神经表征分析，通过将相似性度量等价转化为马尔可夫矩阵，开发多尺度变体和层间融合方法，在多个基准上取得最先进结果。


<details>
  <summary>Details</summary>
Motivation: 扩散几何是有效的流形学习框架，但尚未用于神经表征；现有相似性度量（如CKA）无法多尺度分析，且跨层比较受限。

Method: 将RSM等价转化为行随机马尔可夫矩阵，基于此提出多尺度CKA和距离相关（利用转移矩阵的t次幂），以及通过交替扩散融合多层马尔可夫矩阵得到联合算子，实现网络级别的样本几何比较。

Result: 在ReSi基准（14种架构、7个数据集、3个领域）上，语言和视觉任务的准确率和输出相关性均达到最先进，在分布外数据上也取得最优。

Conclusion: 扩散几何方法能有效揭示神经表征的多尺度几何结构，且跨层融合方法提升了模型间相似性评估的鲁棒性和准确性。
...
### [381] [Context-aware Entity-Relation Extraction for Threat Intelligence Knowledge Graphs](https://arxiv.org/abs/2605.15904)
*Inoussa Mouiche,sherif Saad*

Main category: cs.LG

TL;DR: 提出CTiKG框架，利用SecureBERT+和领域本体从CTI报告中提取威胁实体和关系，显著提升准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法因报告结构复杂、语言专业和语义歧义导致误差传播，限制提取精度和泛化能力。

Method: 构建混合NLP模型流水线，集成SecureBERT+上下文嵌入和领域专家知识，减少误分类和级联错误。

Result: 在DNRTI-AUG-STIX2数据集上，NER提升3-4%，RE提升达8%；在DNRTI和STUCCO基准上验证鲁棒性。

Conclusion: CTiKG框架稳健且实用，已公开数据集以促进可重复性和后续研究。
...
### [382] [LoCO: Low-rank Compositional Rotation Fine-tuning](https://arxiv.org/abs/2605.15916)
*An Nguyen,Jaesik Choi,Anh Tong*

Main category: cs.LG

TL;DR: LoCO是一种通过低秩斜对称矩阵和组合旋转链构建正交变换的参数高效微调方法，在多个领域表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法如低秩适应虽能实现参数效率，但难以保持预训练表示的几何结构。

Method: 利用低秩斜对称矩阵和组合旋转链构造正交变换，并通过近似方案实现并行计算。

Result: 在扩散变换器、视觉变换器和语言模型微调中，性能优于或持平于现有正交和非正交方法。

Conclusion: LoCO在保持低计算复杂度的同时维持正交性，是一种有效的参数高效微调方法。
...
### [383] [A Retrieval-Enhanced Transformer for Multi-Step Port-of-Call Sequence Prediction in Global Liner Shipping](https://arxiv.org/abs/2605.15937)
*Yanzhao Su,Fang He,Yineng Wang*

Main category: cs.LG

TL;DR: 提出CCRE框架，通过检索增强和拓扑约束实现多步港口序列预测，显著提升准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法受限于不可靠的航次计划和AIS数据仅能预测下一港，无法支持多步预测。

Method: CCRE框架包含检索增强历史编码器、Transformer轨迹编码器、交叉注意力中间融合、自回归解码器、调度采样与Gumbel-Softmax松弛，以及拓扑掩码强制航路可达性。

Result: 第一目的地准确率72.3%，平均三步准确率61.4%，优于CatBoost和LSTM等基线12.6%和11.3%。

Conclusion: CCRE能有效捕捉复杂航线模式，具有可扩展性，提升了多步港口序列预测的准确性和可行性。
...
### [384] [When and Why Adversarial Training Improves PINNs: A Neural Tangent Kernel Perspective](https://arxiv.org/abs/2605.15959)
*Yuan-dong Cao,Chi Chiu SO,Jun-Min Wang,He Wang*

Main category: cs.LG

TL;DR: 提出了一种基于对抗训练的分析框架，从理论上解释了GAN如何改善物理信息神经网络的训练，并开发了高效算法，显著提升了准确率。


<details>
  <summary>Details</summary>
Motivation: 物理信息神经网络（PINNs）因频谱偏差、刚性问题而在高频或多尺度解上训练困难，基于GAN的对抗训练虽有经验成功但机制不明，需要理论分析和改进算法。

Method: 通过观察GAN中判别器如何影响训练动力学，建立了分析框架，提供了对抗训练有效的理论依据，统一分析了各种GAN变体，并据此设计了新的高效训练算法。

Result: 实验表明该方法显著减少PINNs训练的病态问题，生成的模型性能优越，准确率常比替代方法高几个数量级。

Conclusion: 提出的分析框架和算法能有效改善PINNs的训练，获得更准确的模型。
...
### [385] [Entropy-Based Characterisation of the Polarised Regime in Latent Variable Models](https://arxiv.org/abs/2605.15965)
*Peter Clapham,Lisa Bonheme,Marek Grzes*

Main category: cs.LG

TL;DR: 提出基于均值表示熵的信息论分类法来识别VAE中的极化状态，克服了现有方法对高斯先验的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有识别活动维度的标准依赖于高斯先验，限制了其在不同变分模型中的适用性。

Method: 通过均值表示的熵与KL最小化之间的熵-方差界限，建立极化状态的信息论分类，并与Bonheme的主动/被动条件相关联。

Result: 在β-VAEs、可识别VAEs、最小体积自编码器和L2正则化自编码器上，熵准则一致地恢复了极化状态；被动维度在适当归一化后对下游任务有小而稳定的改进。

Conclusion: 极化状态可通过熵准则有效识别，且被动维度并非完全无用，其影响主要在于尺度而非信息消除。
...
### [386] [Constrained latent state modeling: A unifying perspective on representation learning under competing constraints](https://arxiv.org/abs/2605.15995)
*Gwenol\'e Quellec*

Main category: cs.LG

TL;DR: 提出约束隐状态建模（CLSM）作为统一视角，通过识别核心属性及其权衡，将现有方法统一到共同设计空间，指导可解释、鲁棒的隐状态模型开发。


<details>
  <summary>Details</summary>
Motivation: 当前学习隐表示的方法分散，依赖隐式假设，导致歧义和解释问题，根本原因在于目标函数约束不足。

Method: 提出CLSM，识别预测充分性、最小性、时间相干性、观测兼容性、对干扰因素鲁棒性、结构约束等核心属性，展示它们存在固有权衡，并重新审视主要建模族。

Result: 现有方法可解释为强制执行不同子集约束，占据共同设计空间不同区域；缺乏可识别性等问题是约束不足的后果。

Conclusion: CLSM提供原则性框架，使设计选择明确化，分析权衡，指导开发更可解释、鲁棒和任务对齐的隐状态模型。
...
### [387] [Accelerated Gradient Descent for Faster Convergence with Minimal Overhead](https://arxiv.org/abs/2605.16017)
*Manuel Graca,L. Miguel Silveira,Arlindo Oliveira,Frank Liu*

Main category: cs.LG

TL;DR: CT-AGD是一种通过曲率调整加速一阶方法且能处理随机小批量噪声的优化算法，在保持精度的同时平均减少33%的训练轮数。


<details>
  <summary>Details</summary>
Motivation: 深度学习中非凸优化问题需要更高效的优化方法，现有方法在噪声和偏差处理上存在不足。

Method: 利用有限差分商显式捕捉局部曲率，并开发启发式方法减轻随机小批量训练的噪声和偏差，作为一阶方法的通用加速程序。

Result: 与基准一阶方法相比，CT-AGD在达到相同精度时，平均减少33%的训练轮数。

Conclusion: CT-AGD是一种有效的加速优化方法，存储和计算开销与Adam等自适应梯度方法相当，能显著提升训练效率。
...
### [388] [Variational Autoregressive Networks with probability priors](https://arxiv.org/abs/2605.16020)
*Piotr Bia{\l}as,Piotr Korcyl,Tomasz Stebel,Dawid Zapolski*

Main category: cs.LG

TL;DR: 将物理先验融入神经网络采样器，以先验概率分布为训练起点，减轻了训练负担，实现了更大规模离散自旋模型的模拟。


<details>
  <summary>Details</summary>
Motivation: 蒙特卡洛方法在相变附近存在临界慢化问题，而深度学习方法虽可缓解但训练困难，因为原始架构忽略物理对称性。

Method: 提出一个框架，利用物理先验概率分布作为训练起点，整合自旋相互作用等物理信息。

Result: 在Ising模型和Edwards-Anderson自旋玻璃模型上，使用物理先验的模型比无先验模型训练更高效，并能模拟更大系统尺寸。

Conclusion: 抛弃'空白画布'模型，转而采用物理启发先验，可降低训练负担，促进离散自旋模型的大规模模拟。
...
### [389] [Mind Dreamer: Untethering Imagination via Active Latent Intervention on Latent Manifolds](https://arxiv.org/abs/2605.16030)
*Shaojun Xu,Xiaoling Zhou,Yihan Lin,Yapeng Meng,Xinglong Ji,Luping Shi,Rong Zhao*

Main category: cs.LG

TL;DR: 提出Mind Dreamer框架，通过主动潜在干预从生成器采样初始状态，实现非连续潜在跳转，解决基于模型的强化学习中历史绑定的学习不对称问题。


<details>
  <summary>Details</summary>
Motivation: 基于模型的强化学习因从历史状态初始化想象，导致世界模型探索快于策略优化，形成历史绑定和学习不对称。

Method: 从生成器采样初始状态而非历史缓存，利用对抗生成器合成非连续潜在跳转；推导Relay Value Function和Relay Uncertainty Function，采用二次折扣γ²进行不确定性传播。

Result: 在DeepMind Control Suite上平均加速1.67倍，稀疏奖励任务加速8.8倍。
...
### [390] [Looped SSMs: Depth-Recurrence and Input Reshaping for Time Series Classification](https://arxiv.org/abs/2605.16048)
*M\'onika Farsang,Ramin Hasani,Daniela Rus,Radu Grosu*

Main category: cs.LG

TL;DR: 本文提出状态空间模型（SSM）中的深度递归和输入重塑两种未探索的设计轴，实验表明它们能提升时间序列分类性能。


<details>
  <summary>Details</summary>
Motivation: SSM天然具有序列递归，但深度递归（层间重复使用相同块）未被探索；同时输入重塑（拼接时间步或重塑特征-时间维度）也被忽视。

Method: 使用循环SSM（k个参数迭代L次）替代独立参数的标准SSM；对低维输入拼接时间步，对高维输入展平并分块重塑特征-时间维度。

Result: 循环SSM在参数更少时性能接近或超过标准SSM；输入重塑带来1-6%准确率提升；两者结合效果更佳。

Conclusion: 深度递归和输入重塑是独立且有益的归纳偏置，为SSM设计提供了两个新方向。
...
### [391] [Ada-Diffuser: Latent-Aware Adaptive Diffusion for Decision-Making](https://arxiv.org/abs/2605.16054)
*Fan Feng,Selena Ge,Minghao Fu,Zijian Li,Yujia Zheng,Zeyu Tang,Yingyao Hu,Biwei Huang,Kun Zhang*

Main category: cs.LG

TL;DR: 提出Ada-Diffuser框架，通过显式建模潜在动态提升生成决策的精度与适应性。


<details>
  <summary>Details</summary>
Motivation: 现有生成决策方法常忽略具有演化动态的潜在因素，而这些对环境和行为建模至关重要。

Method: 从短时间观测块中识别潜在过程的必要条件，构建因果扩散模型Ada-Diffuser，同时学习时间结构和潜在动态。

Result: 在模拟控制和机器人基准上，Ada-Diffuser在准确推理潜在状态和自适应策略学习方面表现优异。

Conclusion: 显式建模潜在动态可显著提升生成决策模型的性能，Ada-Diffuser为适应动态变化提供了有效方案。
...
### [392] [SAFE Quantum Machine Learning with Variational Quantum Classifiers](https://arxiv.org/abs/2605.16067)
*Ying Chen,Paolo Giudici,Vasily Kolesnikov,Paolo Recchia*

Main category: cs.LG

TL;DR: 提出一种变分量子分类器，结合振幅编码和可学习的经典预编码层，通过SAFE-AI指标评估可靠性，在准确性、鲁棒性和可解释性上表现平衡。


<details>
  <summary>Details</summary>
Motivation: 现有量子分类器在安全关键场景中缺乏可靠性和稳定性评估，需要一种结构化的假设类来保证对输入变化的敏感度可控。

Method: 使用振幅编码将高维深度表示嵌入量子态，通过可学习的经典预编码层稳定嵌入，结合归一化振幅嵌入和有界量子观测值，形成平滑的假设类；采用基于Cramer von Mises散度的SAFE-AI指标评估模型可靠性。

Result: 与强经典基线相比，提出的量子模型预测性能相当，但SAFE可靠性更平衡，对噪声的鲁棒性更强，在结构化特征移除下稳定性更好。

Conclusion: 变分量子电路为安全关键环境中的稳定性导向SAFE学习提供了原则性机制。
...
### [393] [ITGPT: Generative Pretraining on Irregular Timeseries](https://arxiv.org/abs/2605.16069)
*Antoine Honor\'e,Ming Xiao*

Main category: cs.LG

TL;DR: 提出ITGPT模型，通过自监督学习和生成预训练处理不规则采样的多模态时间序列数据，在医疗和预测维护任务上达到SOTA，且无需数据插补或特征融合。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列模型难以利用大量标记多模态数据，尤其是不规则采样或缺失值情况；大语言模型虽有效但缺乏处理多模态时间序列的灵活性。

Method: ITGPT是一种基于注意力的架构，结合自监督学习损失和生成预训练目标，直接处理不规则采样的多模态时间序列。

Result: 在TIHM（医疗）和CompX（预测维护）数据集上，无需重采样、特征融合或显式数据插补即达到SOTA；标签稀缺时，利用未标记数据优于纯监督方法。
...
### [394] [MIND: Decoupling Model-Induced Label Noise via Latent Manifold Disentanglement](https://arxiv.org/abs/2605.16081)
*Dayong Ren*

Main category: cs.LG

TL;DR: 提出MIND框架，通过潜流形解耦处理模型诱导的标签噪声，在多个基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 预训练专家和基础模型驱动的自动标注会产生系统性的模型诱导噪声，与局部特征流形紧密耦合，现有方法难以处理。

Method: 提出潜解耦估计器(LDE)，将高维噪声流形解耦为子空间依赖的可处理成分，动态投影样本到具有一致错误模式的潜结构簇。

Result: 在CIFAR-100和大型3D数据集(S3DIS, ScanNet)上显著优于SOTA，并有效修正视觉语言模型的零样本幻觉。

Conclusion: MIND作为基础模型的鲁棒蒸馏框架具有潜力。
...
### [395] [Multi-level Self-supervised Pretraining on Compositional Hierarchical Graph for Molecular Property Prediction](https://arxiv.org/abs/2605.16088)
*Xiayu Liu,Zhengyi Lu,Hou-biao Li*

Main category: cs.LG

TL;DR: 提出MolCHG，一种基于组合层次图的多级自监督预训练框架，通过将键信息提升为独立节点表示，在分子性质预测中取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常操作于单一结构粒度，且将键信息视为辅助边属性，未能充分利用其独立语义层。

Method: 构建包含四个节点类型、三个语义层次的组合层次图，引入与原子图并行的键图，使片段节点平等聚合原子级和键级语义，并设计三个级别特定的预训练任务：原子-键跨视图对比、片段级官能团预测和图级结构预测。

Result: 在MoleculeNet的9个基准测试中，MolCHG在7个数据集上取得最佳结果，在其余数据集上保持竞争力。

Conclusion: 多级监督信号具有互补性，每个组件对整体性能均有贡献。
...
### [396] [Centralized vs Decentralized Federated Learning: A trade-off performance analysis](https://arxiv.org/abs/2605.16089)
*Chaimaa Medjadji,Guilain Leduc,Sylvain Kubler,Yves Le Traon*

Main category: cs.LG

TL;DR: 本文首次实验比较了集中式、分散式和半分散式联邦学习架构，基于Fedstellar模拟器、MNIST数据集和MLP分类器。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对三种联邦学习架构的实验比较，无法理解各自的优缺点和性能指标之间的权衡。

Method: 使用Fedstellar模拟器，在MNIST数据集上采用MLP分类器进行实验分析。

Result: 通过实验揭示了不同架构在性能指标上的权衡与特点。

Conclusion: 实验分析有助于根据应用需求选择合适的联邦学习架构。
...
### [397] [Federated Imputation under Heterogeneous Feature Spaces](https://arxiv.org/abs/2605.16099)
*Imane Hocine,Chaimaa Medjadji,Sylvain Kubler,Gregoire Danoy,Yves Le Traon*

Main category: cs.LG

TL;DR: 提出FedHF-Impute框架，通过共享特征图的消息传递解决联邦学习中的特征异构问题，提升表格数据插补精度。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习假设特征模式对齐，但表格场景中客户端仅观测部分特征子集，参数平均方法在弱重叠特征间信息传递有限，影响联邦插补效果。

Method: 将结构特征缺失与常规缺失分离，构建全局共享特征图，通过消息传递在统计相关特征间传播信息，实现跨客户端间接知识迁移，保持标准联邦通信。

Result: 在SECOM和AirQuality数据集上，RMSE分别比联邦基线降低26.9%和8.4%；在PhysioNET上性能相当，与最佳基线仅差0.3%。

Conclusion: FedHF-Impute能有效处理异构特征空间的联邦插补问题，通过特征图信息传播提升弱重叠特征下的信息共享效率。
...
### [398] [Attention Dispersion in Dynamic Graph Transformers: Diagnosis and a Transferable Fix](https://arxiv.org/abs/2605.16112)
...
<details>
  <summary>Details</summary>
Motivation: 现有Transformer在时间偏移数据集上性能有限，发现注意力分散是主要失败模式，模型无法聚焦于关键节点。

Method: 用差分注意力替代标准注意力，抑制共模噪声并放大独特信号；结合标准输入编码构建DiffDyG模型。

Result: 在9个基准和三个负采样协议上达到SOTA，高偏移数据集上增益显著，注意力熵降低、关键节点注意力质量提升。

Conclusion: 差分注意力简单可迁移，有效缓解时间偏移下的性能退化，为CTDG提供新范式。
...
### [399] [Multi-Fidelity Flow Matching: Cascaded Refinement of PDE Solutions](https://arxiv.org/abs/2605.16118)
*Sipeng Chen,Junliang Liu,Hewei Tang,Shibo Li*

Main category: cs.LG

TL;DR: 提出多保真流匹配（MFFM），通过校准源分布和条件化速度网络，实现参数化PDE解的级联细化，以L次确定性网络评估达到最细网格。


<details>
  <summary>Details</summary>
Motivation: 利用条件流匹配中源分布可校准的特性，改进多保真度PDE求解的效率。

Method: MFFM：源分布根据经验低-高保真残差尺度校准（局部高斯模糊相关），速度网络以低保真解为条件；构建多分辨率级联，逐级独立训练后端到端微调，实现单步确定性 rollout。

Result: 在8个基准（2个超分辨率、6个时空预测任务）上验证，性能优于基线。

Conclusion: MFFM是多重网格细化的学习模拟，通过条件化和校准源噪声简化残差细化问题，提升训练几何和推理效率。
...
### [400] [Navigating Potholes with Geometry-Aware Sharpness Minimization](https://arxiv.org/abs/2605.16134)
*Simon Dufort-Labb\'e,Mehrab Hamidi,Razvan Pascanu,Ioannis Mitliagkas,Damien Scieur,Aristide Baratin*

Main category: cs.LG

TL;DR: LLQR+SAM结合了SAM的快速锐度扰动和LLQR的慢速几何学习，通过两时间尺度机制提升泛化性能。


<details>
  <summary>Details</summary>
Motivation: SAM均匀扰动所有参数方向，忽略了损失曲面的几何结构，需要利用底层几何信息来更有效地寻找平坦极小值。

Method: 在SAM中引入LLQR框架学习的预处理器，该预处理器稀疏更新并作为慢速指数移动平均，捕捉平滑的低分辨率损失几何；SAM在此基础上以更快速率进行扰动。

Result: 在标准视觉和序列建模基准上，LLQR+SAM一致优于单独的SAM和LLQR方法。

Conclusion: 慢速学习的几何和快速锐度修正是真正互补的，两时间尺度结构不仅计算方便，理论上还能放大SAM在局部尖锐但平均平坦方向上的逃逸信号。
...
### [401] [Surrogate Neural Architecture Codesign Package (SNAC-Pack)](https://arxiv.org/abs/2605.16138)
*Jason Weitz,Dmitri Demler,Benjamin Hawks,Aaron Wang,Nhan Tran,Javier Duarte*

Main category: cs.LG

TL;DR: 提出SNAC-Pack，一个开源AutoML框架，通过硬件感知的神经架构搜索和协同设计，实现了FPGA高效部署，在物理实验数据上获得紧凑且资源高效模型。


<details>
  <summary>Details</summary>
Motivation: 现有NAS方法只优化精度或使用与硬件成本相关性差的代理指标，导致FPGA部署时资源预算不匹配，需要多维度硬件成本感知的自动化设计。

Method: 使用Optuna和NSGA-II进行多目标全局搜索，硬件替代模型预测资源与延迟；结合量化感知训练和迭代幅度剪枝的压缩循环；通过hls4ml合成FPGA固件；YAML配置支持新数据集。

Result: 在喷注分类和量子比特读出任务中，发现的紧凑架构在指标上匹配或超越强基线，同时降低FPGA资源利用率，设计空间探索从数月减少到数小时。

Conclusion: SNAC-Pack能有效实现硬件感知的神经架构协同设计，显著加速FPGA部署的设计过程，并保持任务性能。
...
### [402] [Learn Where Outcomes Diverge: Efficient VLA RL via Probabilistic Chunk Masking](https://arxiv.org/abs/2605.16154)
*Vaidehi Bagaria,Nikshep Grampurohit,Pulkit Verma*

Main category: cs.LG

TL;DR: 提出概率性块掩码（PCM）方法，通过仅对轨迹中少量关键块进行梯度计算，在不损失性能的前提下显著加速GRPO-based VLA策略的强化学习后训练。


<details>
  <summary>Details</summary>
Motivation: 在GRPO-based VLA强化学习中，梯度计算占据约78%的耗时，且均匀分配到所有轨迹块，而许多块贡献微小。需要一种方法将计算集中在能产生学习信号的阶段。

Method: PCM利用成功-失败动作方差作为每阶段梯度方差的代理，为每个语义阶段在线更新保留概率，并以此概率采样固定数量的块进行梯度计算。

Result: 在LIBERO基准上，PCM达到与标准GRPO相同的最终成功率，同时实现2.38倍加速、4.8倍梯度更新加速、60%峰值激活内存降低，仅对不到20%的轨迹块进行反向传播。

Conclusion: PCM是一种高效的即插即用修改，在不引入额外模型的情况下大幅降低GRPO训练的计算成本。
...
### [403] [Entropic Auto-Encoding via Implicit Free-Energy Minimization](https://arxiv.org/abs/2605.16164)
*Hazhir Aliahmadi,Irina Babayan,Greg van Anders*

Main category: cs.LG

TL;DR: 提出熵自编码器（EAE），通过隐式先验和自由能最小化缓解后验坍塌，学习非高斯多模态潜分布。


<details>
  <summary>Details</summary>
Motivation: 变分自编码器（VAE）因显式先验导致后验坍塌，潜在变量被忽略，需改进。

Method: EAE仅以重构损失为显式目标，通过熵隐式生成先验，利用自由能最小化的编码器集成来偏向高容量近优解区域，解码器引导搜索走向信息性潜表示。

Result: EAE在多个数据集（反应扩散过程、MNIST、CelebA）上展示了非高斯多模态潜分布，生成多样化数据并保留底层结构。

Conclusion: EAE有效缓解后验坍塌，能够学习数据中的复杂结构，如低维动力学、类别区分和层次化人脸特征。
...
### [404] [BAPR: Bayesian amnesic piecewise-robust reinforcement learning for non-stationary continuous control](https://arxiv.org/abs/2605.16170)
*Yifan Zhang,Liang Zheng*

Main category: cs.LG

TL;DR: BAPR 将贝叶斯在线变化检测与鲁棒集成强化学习结合，用于分段平稳控制系统，并通过形式化验证确保理论边界。


<details>
  <summary>Details</summary>
Motivation: 标准鲁棒强化学习在全局保守与局部自适应策略间面临困境，难以同时应对稳定期和突发的体制变化。

Method: 提出 BAPR，使用 BOCD 驱动自适应保守机制，推导凸组合贝尔曼算子的 γ-压缩性及其边界，并通过 Lean 4 形式化验证。

Result: 变化点后策略最大保守，随置信度增长平滑放松，检测延迟 O(log(1/δ))；所有理论结果经机器验证。

Conclusion: BAPR 为分段平稳控制系统提供了鲁棒强化学习的有效方案，具有严格理论保证和形式化验证。
...
### [405] [Imitation learning for clinical decision support in pediatric ECMO](https://arxiv.org/abs/2605.16175)
...
TL;DR: 将临床决策建模为从观测轨迹中学习行动（模仿学习），并使用TabPFN在儿科ECMO数据上预测未直接观测的行动，优于传统基线。


<details>
  <summary>Details</summary>
Motivation: 儿科ECMO治疗高度复杂且数据稀缺，需要有效的决策支持模型来模拟干预措施。

Method: 采用模仿学习框架，以TabPFN（基于transformer的表格数据方法）为主要模型，对比XGBoost和MLP，在真实儿科ECMO数据上学习行动模型。

Result: TabPFN方法一致优于XGBoost和MLP基线。

Conclusion: TabPFN可作为儿科ECMO决策支持中强健的临床行为基线模型。
...
### [406] [Hypothesis-driven construction of mesoscopic dynamics](https://arxiv.org/abs/2605.16211)
*Zhuoyuan Li,Aiqing Zhu,Qianxiao Li*

Main category: cs.LG

TL;DR: 提出基于广义昂萨格原理的介观动力学学习范式，具有先验理论保证，通过数据识别假设类成员，得到准确鲁棒可解释的动力学模型。


<details>
  <summary>Details</summary>
Motivation: 传统科学建模从固定有效方程出发，在复杂应用（如多尺度系统）中极具挑战性。

Method: 基于广义昂萨格原理建立统一框架，涵盖耗散与保守介观动力学，先验提供全局适定性等理论保证，再通过实例数据识别假设类中的具体成员。

Result: 在连续PDE模型数据和微观链模型数据上验证，模型准确、鲁棒且可解释，提供物理诊断信息。

Conclusion: 该框架不仅是有效的动力学学习器，还能提供可解释的物理诊断，为复杂系统建模提供新范式。
...
### [407] [The Privacy Price of Tail-Risk Learning: Effective Tail Sample Size in Differentially Private CVaR Optimization](https://arxiv.org/abs/2605.16219)
*El Mustapha Mansouri*

Main category: cs.LG

TL;DR: 差分隐私改变了CVaR学习的有效样本量，隐私相关的尾样本量是εnτ，隐私CVaR超额风险分解为普通尾风险统计误差和隐私代价。


<details>
  <summary>Details</summary>
Motivation: 理解差分隐私如何影响CVaR学习的样本复杂度，明确隐私带来的额外代价。

Method: 通过理论分析，将隐私CVaR风险分解为统计误差和隐私项，并推导标量估计、有限类及凸Lipschitz学习下的完全速率。

Result: 标量估计速率Θ(B min{1,(nτ)^{-1/2}+(εnτ)^{-1}})，有限类速率Θ(B min{1,√(log(2M)/(nτ))+log(2M)/(εnτ)})，凸Lipschitz学习隐私项尺度为1/(εnτ)。

Conclusion: 私有CVaR学习的核心难子问题是对Θ(nτ)个信息尾记录进行普通私有学习。
...
### [408] [Layer Equivalence Is Not a Property of Layers Alone: How You Test Redundancy Changes What You Find](https://arxiv.org/abs/2605.16234)
*Gabriel Garcia*

Main category: cs.LG

TL;DR: 替换和互换两种测试在评估变压器层等价性时并不一致，这种协议差异会影响剪枝决策，建议在移除层前同时评估两种KL散度。


<details>
  <summary>Details</summary>
Motivation: 研究人员在压缩时经常混淆不同的层等价性测试（替换和互换），需要明确两者的差异及其对剪枝的影响。

Method: 在预训练变压器（Pythia、Qwen3-8B、Llama-3.1-8B）上测量替换和互换两种协议，对比KL散度差异及剪枝安全性。

Result: 替换与互换的KL差距随训练增长；在Qwen3-8B上，互换引导的移除比替换安全数倍，而Llama-3.1-8B上两者成本相同但KL较低。
...
### [409] [Dynamics-Level Watermarking of Flow Matching Models with Random Codes](https://arxiv.org/abs/2605.16239)
*Shuchan Wang*

Main category: cs.LG

TL;DR: 提出一种基于动力学层面的生成模型水印方法，通过将水印嵌入流匹配模型的连续速度场中，实现可靠的消息恢复同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有水印方法多嵌入模型权重或输出，而动力学层面嵌入可在不改变生成分布的情况下实现水印。

Method: 将水印编码为随机码，通过训练时添加密钥相关的扰动到速度场，检测时从黑盒查询恢复消息。

Result: 在MNIST和CIFAR-10上的实验表明，可可靠恢复消息，生成质量不变，无密钥时解码准确率仅为随机水平。

Conclusion: 动力学层面的水印方法有效，能够在不影响生成质量的前提下实现高可靠度的消息嵌入与提取。
...
### [410] [Crossover from subcritical to critical decay: random walk, self-avoiding walk, percolation](https://arxiv.org/abs/2605.15545)
*Yucheng Liu,Gordon Slade*

Main category: math.PR

TL;DR: 本文证明了一个一般定理，确定了Ornstein-Zernike方程在Z^d上解的渐近行为，即带漂移布朗运动的格林函数乘以各向异性指数衰减因子，并应用于多种模型，揭示了从Ornstein-Zernike到临界衰减的交叉细节和有限阶关联长度的等价性。


<details>
  <summary>Details</summary>
Motivation: 文献中极少关注从Ornstein-Zernike衰减到临界幂律衰减的交叉行为。

Method: 基于方向依赖指数衰减率的变分特征，并大幅推广Hara 2008年的高斯引理到非中心核。

Result: 定理适用于多种随机游走、d≥5的近邻自避行走和d≥15的近邻渗流，揭示了在关联长度尺度上的交叉细节和有限阶关联长度的等价性（仅差普适常数）。

Conclusion: 该定理提供了理解Ornstein-Zernike到临界衰减交叉的广泛结果。
...
### [411] [Turbulent stretching of FENE dumbbell polymer model via special stochastic scaling and singular limits](https://arxiv.org/abs/2605.15742)
*Federico Butori,Yassine Tahraoui*

Main category: math.PR

TL;DR: 研究了随机湍流中有限可扩展非线性弹性（FENE）聚合物模型的拉伸机制，通过随机缩放极限和奇异极限，证明了聚合物密度方程弱收敛到一个带有二阶算子的确定性方程，并识别了聚合物长度的稳态分布。


<details>
  <summary>Details</summary>
Motivation: 理解湍流中聚合物拉伸的机制，以及从随机模型到有效确定性模型的推导。

Method: 采用随机Fokker-Planck方程描述聚合物密度，结合湍流模型（空间尺度ℓ~N^{-1}、时间尺度τ、时间白噪声），通过路径方式（无需平均）的弱收敛极限和奇异极限分析，在加权空间中处理FENE力的奇异性和无通量边界条件。

Result: 当N→∞时，方程弱收敛到带有额外二阶算子（表示平均湍流拉伸效应）的确定性方程；随后τ→0时，确定了聚合物长度的稳态分布。

Conclusion: 该有效模型是路径方式推导的，无需对不同随机流实现进行平均，为物理文献中预测的算子形状提供了严格证明。
...
### [412] [Deconfinement For $\mathrm{SO}(3)$ Lattice Yang-Mills at Strong Coupling](https://arxiv.org/abs/2605.16162)
*Ron Nissim*

Main category: math.PR

TL;DR: 严格证明了SO(3)格点杨-米尔斯理论在强耦合区域不满足威尔逊夸克禁闭判据。


<details>
  <summary>Details</summary>
Motivation: 验证物理预测：中心平凡的规范群（如SO(3）)不满足威尔逊禁闭判据。

Method: 通过严格的数学证明，在强耦合区域分析SO(3)格点杨-米尔斯理论。

Result: 在强耦合区，SO(3)理论不满足威尔逊判据。

Conclusion: 确认了具有平凡中心的格点杨-米尔斯理论不满足威尔逊禁闭判据。
...
### [413] [A Few GPUs, A Whole Lotta Scale: Faithful LLM Training Emulation with PrismLLM](https://arxiv.org/abs/2605.15617)
...
TL;DR: PrismLLM通过混合仿真解耦大规模LLM训练的执行与集群需求，仅用少量GPU即可忠实模拟大规模行为。


<details>
  <summary>Details</summary>
Motivation: LLM训练框架开发调试需要频繁访问大规模集群，成本高且难以获取。

Method: 基于切片构建高保真执行图，混合仿真：选定rank执行原始程序，其余作为虚拟参与者回放。

Result: 迭代时间平均误差0.58%，峰值GPU内存误差<0.01%，能用少于1%的物理GPU模拟8192 GPU集群。

Conclusion: PrismLLM有效减少对大规模物理集群的依赖，便于开发调试与性能优化。
...
### [414] [Designing Datacenter Power Delivery Hierarchies for the AI Era](https://arxiv.org/abs/2605.16255)
*Grant Wilkins,Fiodar Kazhamiaka,Alok Gautam Kumbhare,Chaojie Zhang,Ricardo Bianchini*

Main category: cs.DC

TL;DR: 针对AI加速器导致机架功率密度剧增的问题，提出评估数据中心电力分配设计的框架，强调多资源搁浅对可部署容量的影响。


<details>
  <summary>Details</summary>
Motivation: AI加速器需求使机架功率密度快速上升，预计2027年达每部署1MW，导致数据中心电力搁浅，即无法充分利用已配置的电力。

Method: 构建框架，结合GPU、计算和存储部署的预测模型与Microsoft Azure的实际运营因素，通过吞吐量、功率和成本指标评估电力分配设计。

Result: 多资源搁浅显著改变可部署容量、有效资本支出和性能，且从机架到吊舱级AI系统的密度提升进一步影响这些结果。
...
### [415] [ADAPT: A Self-Calibrating Proactive Autoscaler for Container Orchestration](https://arxiv.org/abs/2605.15788)
*Himanshu Singh Baghel*

Main category: cs.DC

TL;DR: 提出ADAPT在线估计冷启动时长，结合动态规划层FH-OPT与模型预测控制器(MPC)，实现自适应主动伸缩，将SLA违例率降至5%以下。


<details>
  <summary>Details</summary>
Motivation: 容器化工作负载的主动伸缩依赖预配延迟（冷启动时长），但该时长在不同环境和连续扩容事件中变化显著，需在线追踪与自适应。

Method: 采用在线EWMA估计器ADAPT跟踪冷启动时长，将其输出作为动态规划层FH-OPT的输入，再由MPC在滚动窗口内优化副本数，形成闭环自适应主动伸缩。

Result: 在三个策略和六种负载模式下，MPC+LSTM在所有负载上SLA违例率低于5%，而反应式HPA为7-19%，MPC+Prophet在双峰流量下达28.7%。

Conclusion: 通过在线估计预配延迟并动态调整前瞻时域，所提方法能有效降低SLA违例率，优于反应式与传统预测方法。
...
### [416] [Runtime-Orchestrated Second-Order Optimization for Scalable LLM Training](https://arxiv.org/abs/2605.16184)
*Yishun Lu,Junhao Zhang,Zeyu Yang,Wes Armour*

Main category: cs.DC

TL;DR: Asteria通过运行时管理优化状态、异步计算和同步协议，使二阶优化在大规模LLM训练中变得实用。


<details>
  <summary>Details</summary>
Motivation: 二阶方法训练效率高，但维护大矩阵优化器状态导致系统开销大，阻碍实际应用。

Method: 动态分布优化状态到GPU、CPU和NVMe；训练钩子提前准备影子状态，异步计算逆根；有界陈旧性协议减少同步频率。

Result: 在单GPU支持1B参数模型；多节点系统上降低优化器开销、减少延迟峰值、加速收敛。

Conclusion: 通过运行时级别的状态管理、背景计算和分布式同步，而非简化优化器本身，可使二阶LLM训练实用化。
...
### [417] [Nonlocal Optical Response and Surface Susceptibilities: A Systematic Derivation via Spatial Moment Expansion](https://arxiv.org/abs/2605.15716)
*Fr\'ed\'eric Zolla*

Main category: physics.optics

TL;DR: 提出了一种系统理论，将均匀介质的非局部响应核与任意弯曲界面的有效表面磁化率联系起来，推导了曲率修正，并推广了Feibelman d-参数。


<details>
  <summary>Details</summary>
Motivation: 现有理论通常局限于平面界面，需要扩展到任意弯曲界面以处理更一般的非局部光学响应。

Method: 从最一般的张量非局部本构关系出发，结合空间矩展开和分布薄层极限，推导出表面磁化率。

Result: 在主导阶，界面响应凝聚为单个标量表面磁化率χ^s，并显式推导了平均曲率H和高斯曲率K的修正项。

Conclusion: 建立了广义麦克斯韦边界条件，并与经典菲涅耳结果进行了比较，为弯曲界面的非局部响应提供了统一框架。

...
TL;DR: 提出MemSlides分层记忆框架，分离长期记忆（用户档案、工具记忆）与工作记忆，结合局部修订实现个性化演示生成。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅依赖当前提示，无法保持稳定偏好、引入新约束或进行可靠局部编辑。

Method: 设计层次记忆：用户档案记忆存储意图条件化档案，工作记忆携带活跃偏好与会话约束，工具记忆存储可重用执行经验；采用幻灯片级局部修订。

Result: 用户档案记忆提升多意图个性对齐，工具记忆改善闭环修改行为，工作记忆展示偏好保持能力。

Conclusion: 有效个性化需分离持久用户档案、会话级工作记忆和可重用执行经验，覆盖生成与局部修订。
...
TL;DR: PromptMN是一种伪提示领域特定语言，通过%前缀的类型指令注释自然语言，解决提示脆弱性问题，提高人机交互的清晰性和可审查性。


<details>
  <summary>Details</summary>
Motivation: 自然语言提示存在歧义，导致Agent和软件开发工作流中因上下文模糊而失败，需更结构化的交互方式。

Method: 提出PromptMN，使用%前缀的类型指令（角色、目标、约束等）注释自然语言，并支持反向提示工程以推断和修正隐含假设。

Result: 在Claude、Gemini、GPT等前沿模型上测试，无需微调即可正确解析复杂结构（重复、条件、方法、素数检测等），适用于SDLC多种场景。

Conclusion: PromptMN是迈向更清晰、可审查的人机交互的实用步骤，大规模验证为未来工作。
...
TL;DR: 现有大模型遗忘方法易被逆向恢复，原因是针对共享表示。RepSelect通过隔离遗忘集特定表示实现深层遗忘，鲁棒性提升4-50倍。


<details>
  <summary>Details</summary>
Motivation: 当前遗忘方法仅实现浅层遗忘，容易被微调或小样本提示恢复，且会破坏通用能力。

Method: RepSelect在每次更新前压缩权重梯度的主成分，隔离遗忘集特有的表示，保留通用能力。

Result: 在四个模型系列上，相比于五种基线，RepSelect在逆向后答案准确率降低4-50倍，且对少样本提示攻击近乎完全鲁棒。
...
TL;DR: 在AI智能体在线社区中，拟社会互动线索普遍存在，并与发帖人重新参与和互惠回复结构强相关。


<details>
  <summary>Details</summary>
Motivation: 研究拟社会互动/关系在传统媒介中已被探讨，但在双方均为自主AI智能体的在线社区中是否也存在类似线索尚不明确。

Method: 分析Moltbook的4,434篇帖子和50,338条评论，通过关键词匹配、少样本大语言模型标注和分组上下文大语言模型标注，基于依恋/亲密语言、互惠请求和自我认同三个理论指标进行检测。

Result: 拟社会互动线索普遍存在，且与发帖人重新参与和互惠回复结构显著相关，结果经过负对照、零化检验、聚类标准误重估计和多重检验校正仍稳健。

Conclusion: 证据表明AI智能体在话语中形成了行为结构，将互动层面的拟社会互动脚本与持续性成对模式联系起来。
...
TL;DR: 针对LLaDA2.1在块扩散解码中的token-to-token编辑，提出自生成T2T方法解决训练与推理的不匹配问题，提升准确率并减少编辑强度。


<details>
  <summary>Details</summary>
Motivation: 训练时使用随机词汇损坏，但推理时编辑模型看到的是自身流畅高置信度的草稿错误，存在不匹配。

Method: 自生成T2T：无梯度草稿传递，用预测token填充掩码位置，然后在自生成损坏下进行第二次传递监督恢复。通过LoRA持续预训练实现。

Result: 在多个基准上普遍提高准确率，同时降低T2T编辑强度，缓解如最终数字转录错误和过度自我纠正等失败模式。

Conclusion: 自生成T2T有效缓解训练-推理不匹配，提升性能并减少不必要的编辑。
...
TL;DR: 提出RAD3D-Prefix方法，通过冻结大语言模型并集成诊断先验，实现高效3D CT报告生成，在减少参数的同时提升性能。


<details>
  <summary>Details</summary>
Motivation: 将多模态模型扩展到医学3D图像面临计算复杂、语义鸿沟和过拟合问题，且直接微调易导致临床幻觉。

Method: RAD3D-Prefix：轻量诊断先验条件框架，冻结LLM，将图像嵌入与多标签诊断分类逻辑结合，桥接语义鸿沟。

Result: 在多个指标和临床读者研究中优于参数高效基线，泛化性强，仅需极少数可训练参数；大模型冻结更优。

Conclusion: 参数高效适应策略有效，冻结大LLM加轻量投影层为医学报告生成提供了性能与效率的最佳平衡。
...
TL;DR: 提出五种口头化方法提取大语言模型在翻译中的逐词置信度，并与内部信号比较，发现两者性能相近但相关性很低。


<details>
  <summary>Details</summary>
Motivation: 大语言模型用于翻译时，其置信度可靠性未经充分研究，基于内部信号的方法有缺陷且需访问内部状态，需要不依赖内部信号的口头化置信度方法。

Method: 设计五种口头化方法，让模型以文本形式输出每个token的置信度，并在细粒度错误检测和校准两个对齐维度上与内部信号方法比较。

Result: 口头化方法与内部信号方法在错误检测和校准上表现相似，但结果因模型而异；两者之间几乎没有相关性。

Conclusion: 口头化置信度可作为内部信号的可行替代，但两者捕获不同的不确定性信息，需结合使用。
...
TL;DR: 本工作描述了MLLP-VRAIN研究组参加IWSLT 2026同传翻译任务的情况，提出了基于Parakeet和Qwen 3.5的级联方案，通过自适应黑盒策略优化质量-延迟权衡，并探索了上下文增强方法。


<details>
  <summary>Details</summary>
Motivation: 旨在改进长形式同传翻译的质量与延迟平衡，参与所有语言方向的挑战，并尝试利用上下文信息提升翻译性能。

Method: 采用Parakeet和Qwen 3.5构建级联系统，使用自适应黑盒策略并放松约束；对En→{De,It,Zh}方向结合ASR词增强和基于检索增强生成的离线预翻译示例来注入领域上下文。

Result: 在MCIF En→De测试集上，相比去年质量提升+5.82 XCOMET-XL；上下文赛道进一步带来+1.03的改善。

Conclusion: 所提级联方案配合策略优化显著提升了同传翻译质量，上下文增强方法有效，系统具有竞争力。
...
TL;DR: 针对LLM语音识别中语言遵循问题，提出软提示方法，定义新指标，评估零样本、微调及思维链三种策略，并讨论计算约束下的权衡。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的语音识别常误判输出语言，影响转录保真度和下游应用质量，需在不严格约束输出语言的前提下保持灵活性和代码切换能力。

Method: 提出软提示方法提示可能的语言，形式化语言遵循挑战并引入新指标，评估零样本提示、监督微调（SFT）和思维链（CoT）推理三种策略。

Result: 三种策略均能有效减少语言违规，同时保持ASR整体性能，但各有权衡。

Conclusion: 根据计算约束选择合适策略，零样本适合不确定场景，SFT提升提示遵循，CoT强化解码遵循。
...
TL;DR: 研究了Word2Vec在极小词汇量（Toki Pona约130词）上的效果，发现其有效性更多依赖于分布模式而非词汇量大小。


<details>
  <summary>Details</summary>
Motivation: 验证Word2Vec在极小词汇量语言中能否成功捕捉语义关系，并探讨语言噪声（非核心词汇）对嵌入的影响。

Method: 使用Toki Pona社区1.4M句子（7.95M tokens），训练两个模型：一个保留非核心词汇，另一个过滤掉；通过定量（词到类别质心距离、轮廓分数）和定性（表示相似矩阵对比英语）评估。

Result: 非核心词汇不影响嵌入的相对结构，但使相似词更接近；Word2Vec的效果更依赖分布模式而非词汇量大小。

Conclusion: Word2Vec在极端小词汇量下仍有效，依赖于分布模式。
...
TL;DR: 研究发现，大语言模型生成的故事彼此之间比人类故事更相似，尤其前沿模型趋向于生成一种平均化的通用叙事，且常见缓解策略无效。


<details>
  <summary>Details</summary>
Motivation: 虽然LLM能生成高质量文本，但其输出多样性仍存在争议，需要系统研究。

Method: 使用对比框架和r/WritingPrompts数据集，通过人类评估和三种自动标注方法，收集10个LLM的叙事相似性判断。

Result: LLM生成的故事一致性更高，前沿模型趋同于平均叙事，负提示和温度缩放等策略无法改善同质化。

Conclusion: LLM生成故事缺乏人类作者所具备的集体多样性，现有缓解策略不足以解决同质化问题。
...
TL;DR: 针对机器翻译中的不可译现象，提出了一个结构化本体论和补偿策略分类，构建了多语言数据集，并发现注释补偿策略更受偏好。


<details>
  <summary>Details</summary>
Motivation: 机器翻译在标准基准上性能提升，但不可译情况（无法直接等价翻译）仍是局限，且语言学中研究多而NLP中探索不足。

Method: 引入不可译性结构化本体论和补偿策略分类学，将其操作化为包含不可译句子和基于策略翻译的多语言数据集。

Result: 人类偏好研究表明翻译质量取决于使用的策略，注释补偿策略（提供解释性上下文）获得一致偏好。

Conclusion: 该框架和数据集为研究策略感知的机器翻译提供了基础。
...
TL;DR: 通过控制任务差异并比较提示风格，发现LVLMs在显式提示下能协调高效指代表达，但隐式提示下失败，表明提示明确性比任务差异更关键。


<details>
  <summary>Details</summary>
Motivation: 解决两篇研究关于LVLMs协调高效指代表达的矛盾结论，探究任务差异或提示风格是否为根本原因。

Method: 控制任务差异，直接比较显式提示与隐式提示风格，重复实验。

Result: 显式提示下模型能协调高效表达，隐式提示下失败，任务差异非主因。

Conclusion: 人机通信存在关键差异：AI系统需要明确指令才能体现效率，不像人类能自动推断隐含意图。
...
TL;DR: 针对长篇序列化音频剧（200-800集）中大型语言模型（LLM）表现不佳的问题，提出了NarrativeWorldBench基准和N-VSSM模型，该模型在保持高情节节奏F1的同时，计算成本降低4倍，并在跨语言和作者研究中获得更高偏好。


<details>
  <summary>Details</summary>
Motivation: 长篇音频剧作为重要创作媒介，前沿LLM在此场景下表现不佳，现有模型在长程结构叙事指标上存在明显瓶颈。

Method: 构建NarrativeWorldBench基准，包含9个叙事结构指标，跨4种印度语言评估；提出N-VSSM，基于Mamba-2骨干和事件条件后验的256维潜状态空间模型，结合8B解码器和文化迁移函数。

Result: 所有封闭前沿系统在情节节奏F1上饱和于0.78-0.81，200集后下降约0.20；N-VSSM在所有水平上保持F1≥0.84，计算成本降低4倍；文化迁移函数使跨语言忠诚度提升0.20-0.23 Likert点；作家研究中N-VSSM在长程一致性上71%优于Claude Opus 4.5，可控性高1.3 Likert点。

Conclusion: N-VSSM有效解决了长篇音频剧中的结构叙事保持问题，在性能、效率和跨语言迁移上均显著优于现有模型，具有实际应用价值。
...
TL;DR: 提出MODE-RAG多代理系统，基于变分自由能和内部注意力状态动态干预，减少多模态检索增强生成中的幻觉和逻辑捏造。


<details>
  <summary>Details</summary>
Motivation: 多模态检索增强生成（M-RAG）易受跨模态幻觉、因果捏造和谄媚影响，现有缓解方法存在干预悖论：静态规则不必要地干扰准确生成，而完全无引导则导致错误级联。

Method: 提出MODE-RAG系统，由变分自由能和内部注意力状态驱动动态门控干预，高风险查询路由到五个阶段特定代理，集成蒙特卡洛树搜索进行因果推导、logit扰动惩罚谄媚，并设置校正和监察代理确保格式稳定和事后事实验证。

Result: 在从MultiVent导出的ModeVent子集上实验表明，该系统有效降低幻觉率和逻辑捏造，显著提升M-RAG鲁棒性。

Conclusion: MODE-RAG通过动态干预和多代理协作，成功缓解了M-RAG中的多种幻觉问题，提高了系统的可靠性。
...
TL;DR: 提出基于EHR的AIPatient Arena框架，从八个维度评估LLM在临床咨询中的效用，发现模型在询问技巧、伦理等方面表现良好，但在诊断准确性、处理模糊回答等方面存在不足，强调过程评估的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有临床评估多静态单轮或结果导向，无法反映真实临床的序贯性、不确定性和交互性。

Method: 将EHR数据构建为患者知识图谱，用于多轮医患交互，在三个队列（共623名患者）上评估LLM在八个维度的表现。

Result: 询问技巧（4.43-4.99/5）、伦理（4.38-4.93/5）、解释清晰度（3.80-4.72/5）得分高；信息整合（3.19-4.21/5）和用药安全（3.13-3.78/5）中等；处理模糊回答（2.57-3.32/5）、信息覆盖（2.08-3.02/5）和诊断准确性（2.63-3.55/5）弱。交互过程中存在重复提问、遗漏病史等失败。

Conclusion: 仅靠最终答案准确性不足以评估临床准备度，需评估模型在咨询过程中的信息收集、解释和沟通能力。AIPatient Arena为基于EHR的工作流导向预部署评估提供框架。
...
TL;DR: STATEWITNESS通过解释LLM的激活状态来检测和解释欺骗行为，性能优于现有方法，并提供可检查的证据。


<details>
  <summary>Details</summary>
Motivation: 现有欺骗监视器仅输出分数或标量探针，缺乏可检查的证据来解释为什么响应可疑。

Method: 使用一个单独的解码器读取目标模型的隐藏状态，然后回答自然语言查询或生成结构化报告。

Result: 在七个欺骗数据集上达到0.916平均AUROC，比最佳黑盒文本监视器提升11.6%，比最佳激活探针提升25.0%；与现有监视器结合可减少漏检。

Conclusion: 该接口可作为更广泛的可解释性和对齐工具的潜在构建块。
...
TL;DR: 提出二阶偏见概念，通过基于认识论的逻辑推理任务评估LLM作为偏见评判者时的社会偏见。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注LLM是否生成偏见内容，但忽略其作为评判者时可能以更隐蔽方式表现偏见。

Method: 基于权利认识论，设计逻辑推理任务，让LLM判断偏见文本对谁可接受，并开发两个指标测量其推断人口统计学特征时的偏见。

Result: 该任务能规避安全护栏，揭示模型判断中的偏见；偏见在不同目标群体间系统变化，反映隐含社会地图。

Conclusion: 需要更理论基础的偏见评估方法，尤其是在判断任务中。
...
TL;DR: 研究发现，随着工具库规模扩大，LLM路由准确率显著下降，主要归因于检索差距和混淆差距，而嵌入排序可有效缓解。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在日益增长的专用工具库中路由用户请求时，路由精度如何随规模扩大而退化。

Method: 在包含110个agent、584个工具的部署企业生产力助手上进行单步路由实验，评估三个前沿模型从10到110个agent的表现；通过oracle分析分解退化原因为检索差距和混淆差距；采用嵌入排序进行候选取样，并在1435个人工标注话语上验证。

Result: 路由F1在欠指定请求中下降16-23个百分点；嵌入排序在全规模下恢复F1 10-11个百分点；标注研究显示恢复10-17个百分点，尽管绝对性能低10-15个百分点。

Conclusion: 路由精度退化源于模型无法检索正确工具以及工具间混淆，嵌入排序能有效弥补检索差距，但混淆差距仍需进一步解决。
...
TL;DR: 深度transformer通过层次表示实现强表达能力，本文从形式语言角度分析其编码上下文无关文法的能力，证明了深度与文法深度线性相关，支持线性表示假说。


<details>
  <summary>Details</summary>
Motivation: 缺乏严格的理论工作解释深度transformer如何表示层次结构，本文旨在填补这一空白。

Method: 使用有界深度非递归上下文无关文法作为分析框架，显式构造具有位置注意力的transformer，深度随文法深度线性增长，神经元数量与推导树形状数及产生式规则数平方相关。

Result: 证明这些架构能够将抽象语法状态编码为残差流中的低维线性可分子空间，支持线性表示假说。

Conclusion: 深度transformer具有表示层次结构的理论能力，深度与文法深度成正比，为模型设计提供理论依据。
...
TL;DR: 利用大型语言模型研究多模态多方对话中的话轮转换，发现LLM在下一说话者预测上超越监督模型和人类，多模态LLM在对象检测和话轮变化预测上优于纯文本LLM但不及人类。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在复杂多模态多方对话中话轮转换任务的能力，并与传统监督模型和人类表现对比。

Method: 构建评估框架，包含三个任务：对象检测、话轮变化预测和下一说话者预测。比较监督模型、纯文本LLM、多模态LLM和人类在AMI语料库上的表现。

Result: LLM在下一说话者预测上优于监督模型和人类，多模态LLM在对象检测和话轮变化预测上优于纯文本LLM但低于人类。对话上下文对预测关键，人类与LLM预测模式相似。
...
TL;DR: 剪枝后的大语言模型可能在多项选择评估中表现良好，但在开放式生成中失败，造成基准错觉。


<details>
  <summary>Details</summary>
Motivation: 探究剪枝是否擦除了正确答案，还是仅使其更难作为首要输出产生，并揭示标准基准的盲点。

Method: 通过多语言问答任务，在剪枝前后跟踪相同问题，使用Wanda等高稀疏度剪枝方法。

Result: 高稀疏度剪枝下，模型在贪婪开放式生成中失败，但多项选择评分仍选中正确答案；答案通常被降级而非删除，可通过束搜索、采样或上下文示例恢复。

Conclusion: 多项选择基准可能高估压缩LLM的可用性，应测试其生成能力而非仅识别能力。
...
TL;DR: 提出OPD-Evolver框架，通过慢-快共进化与在线自蒸馏，培养智能体的经验选择、使用、写入和维护能力，超越现有记忆系统与训练方法。


<details>
  <summary>Details</summary>
Motivation: 现有记忆智能体存储经验但缺乏整体能力来选择性利用、行动、写入可复用知识并维护不断增长的仓库。

Method: 快循环中与四级记忆层次交互；慢循环中通过结果校准的记忆归因和特权后见蒸馏将四种能力注入可部署策略。

Result: 在多领域基准上，OPD-Evolver超越ReasoningBank达11.5%，超越Skill0约5.8%，且9B模型挑战巨大模型。

Conclusion: OPD-Evolver内化高价值经验与记忆管理，指向超越记忆增强的真正合格智能体进化器。
...
TL;DR: 提出提示扰动框架，通过生成提示变体并过滤结构不一致的比较模式，提高成对LLM评估的一致性。


<details>
  <summary>Details</summary>
Motivation: 成对评估中的不可传递性（如循环偏好）导致排名不稳定且难以解释。

Method: 生成每个提示的扰动变体，基于比较图识别并过滤结构不一致的模式，再应用标准排名方法。

Result: 减少了循环不一致性，提高了LLM排名的可靠性。

Conclusion: 该框架提供了一种简单且原则性的方式来提高评估一致性。
...
TL;DR: 提出LLM-as-Environment-Engineer框架，通过分析失败轨迹自动调整环境配置，并引入MAPF-FrozenLake测试平台。使用Qwen3-4B在基准测试中取得最佳性能，超过更大模型和固定环境基线。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习管线中，环境配置需手动重新设计，依赖启发式方法，无法自动优化。

Method: 当前策略模型分析失败轨迹和上下文信息，提出下一阶段训练环境配置的修改方案；使用Qwen3-4B作为基础模型，并在MAPF-FrozenLake测试平台上进行实验。

Result: 所提框架在基准测试中取得最强综合性能，优于更大规模专用模型（如GPT、Gemini）和固定环境训练基线；成功的关键在于利用失败证据并保留有效配置。

Conclusion: 自动化环境重新设计可提升训练效率；当前强化学习检查点比原始基础模型更适合作为环境工程师，表明策略学习能改善模型诊断自身弱点的能力。
...
TL;DR: 提出SwiftTrans框架，通过多视角探索和差异感知选择，显著提升代码翻译的正确性和运行效率。


<details>
  <summary>Details</summary>
Motivation: LLM翻译的程序运行效率低于人类编写，且仅靠提示工程无法解决，亟需提升运行时效率。

Method: SwiftTrans包含两阶段：MpTranslator利用并行上下文学习生成多样候选；DiffSelector通过差异比较选择最优候选。并引入分层和序数指导优化各阶段。

Result: 在CodeNet、F2SBench和新基准SwiftBench上，SwiftTrans在正确性和运行效率方面均取得一致改进。

Conclusion: 提出的方法能有效提升LLM代码翻译的运行时效率，弥补了现有工作对效率关注的不足。
...
TL;DR: 提出最小充分思维链（MSC）概念，并构建两阶段训练框架SuCo，在保持甚至提升准确率的同时大幅减少推理token，实现效率与精度的双赢。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型在简单问题上也生成过长的思维链，导致计算成本高昂，而现有方法缺乏判断推理充分性的准则。

Method: 定义MSC为最短且足以得到正确答案的CoT前缀；基于此提出两阶段SuCo框架：阶段1通过MSC对齐微调（MFT）构建自适应充分性阈值的数据并微调模型，阶段2使用充分性感知策略优化（SAPO）以强化学习优化，同时惩罚过度思考和思考不足。

Result: 在数学、代码、科学等多个基准上，SuCo在减少推理token的同时，提升了准确率，效果优于基线。

Conclusion: MSC和SuCo证明了通过充分性引导可有效控制推理长度，兼顾效率与性能。
...
TL;DR: 本文研究大型语言模型在生成符合用户文化背景的回答时存在的不足，并引入CAPRI数据集评估其能力。


<details>
  <summary>Details</summary>
Motivation: 现有模型过度代表主流文化（尤其是西方文化），边缘化其他文化，可能影响其生成文化适应性回答的能力。

Method: 构建CAPRI数据集，包含不同文化线索的对话，测试先进LLM在使用本地测量单位、时间和数量表达方面的适应性，并比较直接生成与分步提示的效果。

Result: 模型能推断文化背景并回忆相关惯例，但除非明确分步提示，否则往往无法利用这些信息调整回答；文化线索积累时适应性提高，但先验并非文化中立，常偏向模型来源国。

Conclusion: CAPRI为缩小文化知识与文化适应性生成之间的差距提供了资源，有助于未来研究。
...
TL;DR: 研究发现AI重写放射学报告时，信息丢失与跨模态对齐存在分离：EHR摘要破坏最严重但几乎不损对齐，而标准化/教学重写保留更多实体但大幅降低对齐，称为“slop悖论”；罕见病理未优先退化，退化主因是任务类型而非临床内容。


<details>
  <summary>Details</summary>
Motivation: 评估AI辅助临床文档工具（如LLM）在总结、标准化和重写放射学报告时导致的信息退化程度。

Method: 使用450份胸部X光报告，通过三种LLM重写任务（EHR摘要、标准化重写、教学病例准备），用医学NER测量实体侵蚀，用不确定性语言检测对冲损失，用BiomedCLIP图像-文本相似度测量跨模态对齐退化。

Result: EHR摘要侵蚀51.4%实体和43.7%对冲语言，但对齐仅降2.5%；标准化重写侵蚀26.8%实体但对齐降14.9-16.5%；教学准备侵蚀29.3%实体但对齐降16.5%；罕见病理无优先退化，退化主要取决于任务类型。

Conclusion: 重写任务类型是信息退化的主导因素，而非临床内容；这对多模态医疗AI数据集构建和AI辅助文档治理有重要启示。
...
TL;DR: 研究了双语微调对低资源语言ASR的影响，发现高语言识别准确率时有益，否则推理时提供语言标记可改善性能。


<details>
  <summary>Details</summary>
Motivation: 探索双语微调在低资源语言ASR中的效果，以提升识别性能。

Method: 对九种语言对进行双语微调，训练时在输入文本前添加语言识别标记，推理时联合预测语言和转录，并实验在不同条件下提供语言标记。

Result: 语言识别准确率高时双语微调有益；准确率低时，推理时提供语言标记可提升ASR性能。

Conclusion: 双语微调的有效性依赖于语言识别准确性，在低准确率情况下需在推理时提供语言标记。
...
TL;DR: 提出MultiClin临床ASR基准以应对多脚本变异性，实验表明多脚本感知评估更公平，脚本统一性能最佳。


<details>
  <summary>Details</summary>
Motivation: 传统字符串匹配评估指标将临床术语的拼写变体视为错误，低估了ASR性能。

Method: 引入MultiClin基准，评估ASR模型对多脚本变异性的鲁棒性，并分析训练脚本一致性的影响。

Result: 多脚本感知评估优于单参考评估；不一致的脚本映射增加不确定性，50%映射比例熵最高；脚本统一性能最佳。

Conclusion: 多脚本感知评估更公平，脚本统一可提升ASR性能。
...
TL;DR: 研究检查wav2vec2.0对音系语境补偿的表现，通过声调实验发现，仅预训练模型无补偿证据，微调模型有部分但未达人类水平，表明监督目标可能必要。


<details>
  <summary>Details</summary>
Motivation: 探究wav2vec2.0架构是否展现出类似人类的音系语境补偿能力。

Method: 对普通话声调进行感知补偿实验的伪复制，比较纯自监督预训练模型与微调于普通话ASR模型的嵌入相似度和探测分类器输出。

Result: 纯预训练模型的嵌入相似度无补偿证据；探测分类器除预期层次性分类改善外，显示部分补偿，但未能复现人类在孤立测试音节上的表现。

Conclusion: 与先前仅通过预训练产生音系结构敏感性的报告相反，表明至少某些类型的音系规则抽象可能需要监督目标。
...
TL;DR: 提出一个自动提示优化框架，通过多代理分解和进化循环迭代优化LLM代理的提示，显著提升任务成功率。


<details>
  <summary>Details</summary>
Motivation: 手动提示工程耗时且任务特定，需自动化。

Method: 将观察-动作管道分解为目标条件描述代理和动作选择代理，通过LLM驱动的进化循环迭代优化提示，使用行为分析器和变异器归因并改进提示。

Result: 在BALROG基准的BabyAI任务上，优化后性能一致提升，PutNext任务成功率从0%升至72.5%。
...
TL;DR: 提出了端到端游戏生成的评估框架GameCraft-Bench，包含140个Godot任务，测试表明现有编码代理表现不佳。


<details>
  <summary>Details</summary>
Motivation: 游戏生成是编码代理的新兴应用，但现有评估缺乏对引擎绑定、工件完整性和交互验证的综合考量。

Method: 形式化端到端游戏生成问题，提出基于交互的评估框架，通过重放演示和 rubric 引导的多模态评判来评估可执行游戏。构建包含15个游戏家族、140个Godot任务的基准。

Result: 最强代理仅达到41.46%，多数低于40%；代理能实现可识别机制，但缺乏完整内容、功能性视觉反馈和连贯呈现。

Conclusion: 端到端游戏生成仍极具挑战性，现有代理在完整游戏生成方面存在显著不足。
...
TL;DR: 大型语言模型在链式思维推理中常出现‘过度思考’现象，即生成正确答案后仍继续产生不必要的推理。本文从GRPO强化学习后训练的角度研究此问题，并提出了动态展开编辑（DRE）方法，在训练时干预成功轨迹，减少不必要的思考。


<details>
  <summary>Details</summary>
Motivation: 过度思考导致效率低下，且GRPO训练中的序列级信用分配无法区分必要推理与不必要的延续，形成恶性循环。

Method: 动态展开编辑（DRE）：在训练时对于已生成正确答案后仍在思考的成功轨迹，保留已验证的正确前缀，编辑后续思考，并在同一RL组中优先选择编辑后的轨迹，从而削弱对不必要思考的偏好信号。

Result: 在多种任务上的实验表明，DRE有效减少了过度思考，提升了效率。

Conclusion: DRE能够在不惩罚必要推理的前提下，降低模型对不必要思考的偏好，从而缓解训练中的过度思考问题。
...
TL;DR: ChLogic 是一个英中对照的逻辑推理基准，用于测试大语言模型在不同语言和中文变体下的推理鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有逻辑推理基准多限于英文，模型在中文环境下的推理能力是否稳健尚不明确。

Method: 基于形式逻辑模板构建，包含通用集、困难集和中文特有现象集，每项英文表达对应五种中文实现。在 Qwen3、Ministral 和 GLM 模型上进行实验。

Result: 发现模型在英中之间存在持续的性能差距。回译在通用集上有提升，但在困难集上效果不一，Qwen3-32B 和 GLM-5.1 甚至下降。

Conclusion: 中文表面实现、翻译伪影和模型特定行为共同影响多语言逻辑推理，ChLogic 可作为多语言推理鲁棒性的压力测试。
...
TL;DR: 提出干预对比学习方法将语音基础模型的纠缠表示分解为内容和说话人子空间，提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 预训练语音表示是纠缠的，而下游任务只依赖部分信息，需要分解。

Method: 利用干预数据集和多部分对比损失，学习从纠缠表示空间到内容和说话人子空间的变换。

Result: 在说话人验证和关键词检测任务上，域外说话人验证性能提升，且证明了子空间分离。

Conclusion: 该方法能有效分离内容与说话人信息，改进下游任务表现。
...
TL;DR: 通过AI对话文本直接预测PHQ-9抑郁评分，实现被动连续监测。


<details>
  <summary>Details</summary>
Motivation: 现有PHQ-9等工具完成率低、存在偏差，需要无需用户主动填写的被动监测方法。

Method: 微调Qwen3.5-27B模型（加回归头），并用推理模型和中间模型生成伪标签，将真实标签从3111个扩大到6283个。

Result: 在842用户测试集上MAE=2.6、RMSE=4.0、Pearson r=0.80、AUC=0.91（PHQ-9≥10），且所有严重度阈值AUC>0.87。

Conclusion: 该方法能有效从对话文本预测抑郁严重度，为AI心理健康平台提供被动、连续的监测手段。
...
TL;DR: 针对MDLM中[EOS]的双重角色（语义终止和填充）导致大块解码时溢出问题，提出VoidPadding，引入[VOID]用于填充，[EOS]仅用于终止，显著提升性能并减少解码步数。


<details>
  <summary>Details</summary>
Motivation: 现有MDLM在指令微调时沿用自回归惯例，使用重复[EOS]作为填充，导致[EOS]承担语义终止和填充双重角色，这是大块解码下[EOS]溢出的根本原因。

Method: 提出VoidPadding，引入新的填充令牌[VOID]，将填充和终止角色解耦，让[EOS]仅负责语义终止，[VOID]用于填充和自适应响应画布扩展。

Result: 在Dream-7B-Instruct上，VoidPadding在数学推理和代码生成的四个任务平均得分比原始模型高17.84点，比RainbowPadding高6.95点，同时平均减少55.7%的解码NFE。

Conclusion: VoidPadding有效解耦了[EOS]的双重角色，提升了MDLM的指令微调性能和推理效率。
...
TL;DR: 跨语言迁移在多语言NLP中广泛研究，但少样本上下文学习（ICL）中源语言选择策略尚未被充分评估。本文通过七项任务、六个模型和多样语言类型的实验，发现传统微调经验不适用于ICL，并提出替代启发式方法。


<details>
  <summary>Details</summary>
Motivation: 随着NLP转向少样本上下文学习（ICL），传统微调中的跨语言迁移经验是否适用尚不明确，需要系统研究如何为跨语言ICL选择源语言。

Method: 进行了广泛的实证研究，涵盖七项任务、六个模型和多种语言类型，并分析了语言混淆这一关键障碍。

Result: 传统基于微调的期望在ICL中并不一致适用，需要替代的启发式方法来有效选择源语言。

Conclusion: 跨语言ICL中的源语言选择不能简单沿用微调经验，应基于新提出的启发式方法。
...
TL;DR: 提出一种分解-检索-组合框架SkillWeaver，配合迭代技能感知分解（SAD）策略，显著提升LLM代理在复杂查询下的技能组合能力。


<details>
  <summary>Details</summary>
Motivation: 现实任务常需组合多种技能，而现有LLM代理仅能选择单一技能，缺乏组合能力。

Method: SkillWeaver框架：LLM任务分解器+双编码器技能检索器（FAISS索引）+依赖感知DAG规划器；SAD策略：检索增强的迭代反馈循环对齐分解与可用技能。

Result: SAD单次迭代将分解准确率从51.0%提升至67.7%（+32.7%），上下文窗口消耗减少99%以上，迁移实验显示相对DA增益+35.6%。

Conclusion: 任务分解质量是关键瓶颈，SAD能有效提升分解准确性，SkillWeaver在组合技能任务中表现优异。
...
TL;DR: 提出ConSA框架，通过L0正则化和增广拉格朗日约束，在用户指定的稀疏度下学习全注意力和滑动窗口注意力的最优分配，优于手工规则。


<details>
  <summary>Details</summary>
Motivation: 现有混合注意力架构依赖手工规则或简单启发式，缺乏对注意力行为的深入分析。

Method: 使用L0正则化学习二元掩码，结合增广拉格朗日约束实现层或KV头粒度的目标稀疏度。

Result: 在0.6B和1.7B规模LLM上，学习分配优于规则基线，KV头级分配优于层级；模式为底层SWA、中间层连续FA块。

Conclusion: 揭示了内在注意力行为的细粒度谱，学习分配模式跨尺度、稀疏度和粒度保持一致。
...
TL;DR: 研究用户向大语言模型提出的数字安全与隐私问题，分析回答质量与一致性。


<details>
  <summary>Details</summary>
Motivation: 用户广泛使用LLM获取数字安全与隐私建议，但缺乏对用户实际提问的系统性研究。

Method: 从WildChat数据集筛选14,727个S&P提示，分9类，采样450进行主题分析，并选取270个建议类提示评估LLM回答质量与一致性。

Result: 商业LLM（如GPT-5.5）在98%的提示上给出足够好的回答，而开源模型（如Llama 4）为47%；但商业模型在高质量回答中有时产生矛盾结果。

Conclusion: 用户依赖LLM获取S&P建议，但回答的一致性问题可能误导用户，需进一步改进。
...
TL;DR: HistoRAG是一个针对历史学等解释性学科设计的RAG框架，通过分离检索与生成、时间窗口和LLM评判机制，解决了标准RAG在历史研究中的时间偏差、相关性不足等问题。


<details>
  <summary>Details</summary>
Motivation: 标准RAG默认面向事实问答，与历史学等解释性学科的学术实践存在冲突，需要将学科认识论转化为具体架构。

Method: 采用分离的检索与生成管道（解耦来源发现与解释）、时间窗口（强制研究时段内平衡源表示）以及LLM作为评判者（使相关性判断透明可辩论）。

Result: 标准RAG存在三大缺陷：1970s术语检索1950s内容返回零条（时间偏差）；向量相似度与LLM相关性判断弱相关（Spearman rho=0.275）；基于关键词与语义检索的源池不重叠。

Conclusion: HistoRAG展示了如何将领域特定的认识论承诺转化为RAG设计决策，并可迁移到其他使用大型语料库的解释性学科。
...
TL;DR: 研究揭示了对话AI系统中位置元数据导致模型无意识地产生地理偏差，即使提示为中性。


<details>
  <summary>Details</summary>
Motivation: 现代对话AI依赖用户元数据但未知其引入的区域偏见。

Method: 通过创意写作和开放式问答提示，测试多种LLM在不同位置元数据下的地理引用产生情况。

Result: 位置泄漏率最高增加793倍，且即使替换为'Unknown'仍增加72倍。

Conclusion: 用户资料的框架本身即可作为生成条件信号，需警惕元数据带来的非预期偏见。
...
TL;DR: 提出了首个针对扩散大语言模型的在线自蒸馏框架d-OPSD，通过后缀条件化和步骤级监督实现高效后训练。


<details>
  <summary>Details</summary>
Motivation: 现有在线自蒸馏方法为自回归模型设计，与扩散模型的任意顺序生成特性冲突，需要针对扩散模型的自蒸馏方法。

Method: 使用自生成答案作为后缀条件代替前缀条件，将监督从token级别改为步骤级别，与迭代去噪过程对齐。

Result: 在四个推理基准上，d-OPSD优于RLVR和SFT基线，仅需RLVR约10%的优化步骤。

Conclusion: d-OPSD为扩散大语言模型的后训练开辟了有前景的路径。
...
TL;DR: RubricsTree 是一个基于专家对齐的层次化布尔评估框架，用于可扩展、可审计地评估个人健康 AI，在多个模型上实现高达 66% 的相对性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法中，医生标注可靠但成本高且不可扩展，而 LLM 作为评判者虽然可扩展但主观、不一致且临床对齐不足。

Method: 通过迭代人机协作协议构建了包含超过 100 个原子、可临床验证的布尔评估标准的层次化分类法，并使用上下文自适应路由器为每个查询激活自动加权的相关子集。

Result: RubricsTree 在专家一致性上显著超越强基线，可靠地惩罚上下文退化的响应，并在 HealthBench 上为 Gemini、GPT 和 Qwen 模型系列带来高达约 66% 的相对收益。

Conclusion: 提供了可扩展、可审计且可演进的评估基础设施，以支持产品级个人健康 AI 的持续优化。
...
TL;DR: 提出了一种将《Al-Mawrid阿拉伯语-英语词典》从印刷版转化为标准化计算词库的鲁棒方法，结合ISO LMF与TEI Lex-0标准，解决了结构歧义和标点不一致问题，实现了91%的结构解析准确率，并探讨了与语言关联开放数据的集成。


<details>
  <summary>Details</summary>
Motivation: 填补阿拉伯语词汇基础设施的空白，解决20世纪双语词典的结构歧义和标点不一致问题，为阿拉伯语NLP和数字人文社区提供可互操作、机器可处理的资源。

Method: 采用ISO LMF和TEI Lex-0双标准框架，通过编辑视角分析词典的宏观和微观结构，基于对代表性样本（字母Ayn，占总量的4.6%）的实证分析来指导编码过程。

Result: 结构解析准确率达91%；同义词信息抽取的精确度85%、召回率98%；其他形态语义特征的精确度88%。建立了可扩展的基于前缀的引用系统以支持语义网集成。
...
TL;DR: 提出区域邻近策略优化（ZPPO），通过将教师信息嵌入提示而非策略梯度，解决小规模学生模型在知识蒸馏和强化学习中的局限性。


<details>
  <summary>Details</summary>
Motivation: 知识蒸馏在小学生模型中因模仿教师logits导致泛化差；强化学习在全部rollout失败时注入教师响应会破坏同策略假设并引起漂移。

Method: ZPPO：对难题构造二元候选问题（BCQ）和负候选问题（NCQ），通过提示重放缓冲循环强化直到学生平均准确率达到一半，在学生的最近发展区内放大BCQ和NCQ效果。

Result: 在Qwen3.5系列（0.8B-9B，教师27B）上，后训练为视觉语言模型，在31个基准测试中，ZPPO优于离/同策略蒸馏和GRPO，小规模模型增益最大。

Conclusion: ZPPO通过保持教师在提示中而非策略梯度中，有效提升小规模学生模型在多样化基准上的泛化能力。
...
TL;DR: 提出Darshana Graph语料库，包含超12.5万条印度哲学文本，其中约8500条有跨评论者对齐，支持文体比较和概念关系提取。


<details>
  <summary>Details</summary>
Motivation: 缺乏公开的、大规模跨评论者对齐的印度哲学文本资源，无法直接比较不同诠释传统对相同原始材料的解读。

Method: 构建语料库，包含跨18位评论者对齐的子集；进行文体比较（引用密度、驳斥率、句子复杂度）和基于约束的大语言模型关系抽取管道。

Result: 发现引用密度与驳斥率中等负相关；特定学说谱系中驳斥率显著增加；巴利藏经内部存在可测量的体裁差异；关系图揭示跨学派分歧，但也暴露提取局限性。

Conclusion: 该语料库和分析方法为研究印度哲学诠释传统提供了新资源，但关系提取需进一步改进。
...
TL;DR: 提出ReproRepo框架，利用GitHub issue作为自然监督，可扩展地评估LLM代理在复现研究中的能力。


<details>
  <summary>Details</summary>
Motivation: 现有复现基准依赖大量人工，难以规模化。

Method: 从GitHub issue中提取人类报告的复现障碍作为监督，构建包含1149篇ML论文的基准，评估多种LLM代理。

Result: 最好代理（Codex with GPT-5.5）为约90%的论文识别出至少一个语义相关的复现问题。

Conclusion: ReproRepo是可重用、可扩展的框架，可用于未来LLM代理在真实复现审计中的评估。
...
TL;DR: 提出×-shaped > <former架构，通过非均匀宽度分配（两端宽中间窄）提升语言模型性能与效率。
...
Method: 设计×-shaped架构，保持早期和后期层宽，中间层窄，使用无参数残差调整机制。

Result: 在200M-2B参数（密集）和3B参数（MoE）模型上，优于参数匹配的均匀基线，FLOPs减少22%，KV缓存减少15%。

Conclusion: 非均匀宽度分配可实现更资源优化的语言模型缩放。
...
### [53] [Continuous Audio Thinking for Large Audio Language Models](https://arxiv.org/abs/2606.18273)
*Gyojin Han,Dong-Jae Lee,Changho Choi,Jongsuk Kim,Junmo Kim*

Main category: cs.CL

TL;DR: 提出连续音频思维（CoAT）框架，为音频语言模型提供连续潜在工作空间，通过专家蒸馏组织声学信息，在不增加自回归解码成本的情况下提升多种音频理解任务性能。


<details>
  <summary>Details</summary>
Motivation: 大型音频语言模型（LALM）的隐藏状态逐渐偏向文本生成，丢失了语音细节、韵律、声音事件、情感和音高等丰富声学信息，难以在响应中利用。

Method: CoAT 框架引入连续潜在思维空间，通过音频专家蒸馏提供声学信息，模型在生成响应前可在此空间中组织信息；连续思维块可在单次预填充中处理，不增加额外解码成本。

Result: 在 Qwen2-Audio、Qwen2.5-Omni-7B 和 Audio Flamingo~3 三个 LALM 上，针对音频推理、理解、音乐分类、语音情感和转录等综合基准，性能均取得提升。

Conclusion: CoAT 有效增强了音频语言模型的声学信息利用能力，且辅助监督从思维位置传播到文本响应。
...
### [54] [Redact or Keep? A Fully Local AI Cascade for Educational Dialogue De-Identification](https://arxiv.org/abs/2606.18372)
*Haocheng Zhang,Zhuqian Zhou,Kirk Vanacore,Bakhtawar Ahtisham,Ren\'e F. Kizilcec*

Main category: cs.CL

TL;DR: 提出一个完全本地的级联框架，用于教育对话的去标识化，先过度生成候选跨度，再通过上下文感知的审查器决定保留或删除，在保护数据治理的同时达到高精度。


<details>
  <summary>Details</summary>
Motivation: 现有方法在治理和准确性之间权衡：商业LLM需将学生数据发送至第三方，本地NER系统则过度删除课程术语。

Method: 采用召回优先的联合提议器（轻量编码器加确定性规则）生成候选跨度，再由上下文感知审查器根据对话和说话人角色做出二元决策（保留/删除）。

Result: 最强本地配置在数学辅导对话数据集上达到0.958宏F1，优于同系列LLM基线（0.767）和商业API（0.706），且仅运行于单台笔记本电脑。在课程-人名歧义挑战集上，F1仅下降0.03，而较小审查器下降0.19-0.25。

Conclusion: 教育去标识化中，问题定义比模型规模更重要。
...
### [55] [SproutRAG: Attention-Guided Tree Search with Progressive Embeddings for Long-Document RAG](https://arxiv.org/abs/2606.18381)
*Amirhossein Abaskohi,Issam H. Laradji,Peter West,Giuseppe Carenini*

Main category: cs.CL

TL;DR: SproutRAG是一种注意力引导的分层RAG框架，通过学习句子间注意力构建二叉分块树，实现多粒度检索，无需额外LLM调用或压缩摘要。


<details>
  <summary>Details</summary>
Motivation: 现有RAG方法在检索粒度与上下文连贯性之间存在权衡，依赖昂贵的LLM调用、单粒度扩展或有损摘要。

Method: SproutRAG使用学习到的跨句子注意力构建二叉分块树，组织句子级块为渐进增大但语义连贯的单元；检索时采用分层束搜索，获取多粒度候选。

Result: 在四个基准上平均信息效率（IE）提升6.1%。

Conclusion: SproutRAG有效平衡检索粒度与上下文连贯性，避免额外LLM成本和信息损失。
...
### [56] [Want Better Synthetic Data? Steer It: Activation Steering for Low-Resource Language Generation](https://arxiv.org/abs/2606.18389)
*Jan Cegin,Daniil Gurgurov,Yusser Al Ghussin,Simon Ostermann*

Main category: cs.CL

TL;DR: 本文研究激活引导作为低资源语言合成数据生成的替代方法，发现早期层引导能提高数据多样性和下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 当前最佳方法依赖少样本提示，增加推理成本并可能通过词汇锚定降低多样性，因此需要替代方案。

Method: 提出两种引导策略：语言引导（针对语言身份）和质量引导（通过对比人工书写和回译表示捕捉规范性）。在4个开源LLM、多个层和11种语言上评估，生成情感和主题分类数据并微调小型分类器。

Result: 早期层引导一致提高数据多样性，并通常带来更强的下游模型性能，尤其是对低资源语言。

Conclusion: 激活引导是低资源合成数据生成的有效替代方案，优于或可与少样本提示竞争。
...
### [57] [JetFlow: Breaking the Scaling Ceiling of Speculative Decoding with Parallel Tree Drafting](https://arxiv.org/abs/2606.18394)
...
TL;DR: JetFlow通过因果并行草稿头结合一次前向效率和分支因果条件，突破了推测解码的扩展限制，在多个基准上实现高达9.64倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码方法面临因果-效率困境：自回归草稿机成本随树深增长，双向草稿机产生不一致树导致预算浪费，难以通过增加草稿预算获得线性加速。

Method: JetFlow在冻结目标模型融合隐藏状态上训练因果并行草稿头，生成与目标模型自回归分解一致的候选树，实现高效草稿与因果条件结合。

Result: 在Qwen3密集和MoE模型上，MATH-500达9.64倍加速，开放对话达4.58倍，vLLM集成下进一步降低延迟。

Conclusion: JetFlow通过因果并行草稿有效提升草稿接受长度和端到端速度，优于双向头和树基方法。
...
### [58] [CoreMem: Riemannian Retrieval and Fisher-Guided Distillation for Long-Term Memory in Dialogue Agents](https://arxiv.org/abs/2606.18406)
*Jiaqi Chen,Yongqin Zeng,Shaoshen Chen,Yijian Zhang,Hai-Tao Zheng,Chunxia Ma,XiuTeng Zhou*

Main category: cs.CL

TL;DR: 提出CoreMem，一种基于信息几何的边缘-云记忆架构，通过Riemannian检索和Fisher引导的离散令牌蒸馏，在8GB VRAM下提升对话长期记忆性能。


<details>
  <summary>Details</summary>
Motivation: 现有系统缺乏统一理论，高维检索存在hubness问题，压缩导致语法碎片化，部署在消费级硬件上受内存和计算限制。

Method: 采用Riemannian检索（Fisher-Rao度量、马氏距离、Woodbury加速）和Fisher引导离散令牌蒸馏（FDTD，基于Fisher信息迹的敏感度得分、分层句子到令牌压缩与语法保护）。

Result: 在LOCOMO和LongMemEval-S上取得准确率提升，开放域+4.51个百分点，时间+4.17个百分点，严格在8GB VRAM预算内运行。

Conclusion: CoreMem成功桥接了资源受限边缘设备与理论驱动的终身记忆代理之间的差距。
...
### [59] [VISUALSKILL: Multimodal Skills for Computer-Use Agents](https://arxiv.org/abs/2606.18448)
*Ziyan Jiang,Li An,Yujian Liu,Jiabao Ji,Qiucheng Wu,Jacob Andreas,Yang Zhang,Shiyu Chang*

Main category: cs.CL

TL;DR: VISUALSKILL是一个层次化多模态技能库，通过保留GUI截图等视觉信息，显著提升计算机使用代理在长周期任务和未知软件上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有技能库仅使用文本表示技能，忽略了GUI交互的视觉特性，限制了代理在复杂任务中的表现。

Method: 采用两阶段管道：结合人工编写的文档和真实应用的UI探索，构建每个技能；技能按主题组织为层次化多模态索引，代理通过load_topic MCP工具按需获取文本和图像。

Result: 在CUA-World和OSExpert-Eval基准上，基于Claude Opus 4.6的代理使用VISUALSKILL平均得分为0.456，比无技能基线（0.303）提高15.3个绝对点；相比仅文本技能（0.373）提高8.3个绝对点。

Conclusion: 在技能制品中保留视觉图像而非用文本描述，有助于代理识别UI元素和验证操作后的工作流状态，证实了多模态表示的有效性。
...
### [60] [LLM Parameters for Math Across Languages: Shared or Separate?](https://arxiv.org/abs/2606.18453)
*Behzad Shomali,Luisa Victor,Tim Selbach,Ali Hamza Bashir,David Berghaus,Joachim Koehler,Mehdi Ali,Markus Frey*

Main category: cs.CL

TL;DR: 大语言模型在多语言数学推理中存在参数部分重叠，英语参数最多，低资源语言参数较少，中间层重叠最强。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型数学推理能力的跨语言差异是否源于语言特定参数或共享机制。

Method: 跨语言机制分析，定位并比较支持数学推理的模型参数。

Result: 数学相关参数存在部分跨语言重叠，中间层重叠最强；英语参数集最大，低资源语言参数集较小。

Conclusion: 多语言大模型的数学相关行为既非完全语言不变，也非完全语言特定，而是具有部分跨语言参数重叠和系统性的语言依赖差异。
...
### [61] [Montreal Forced Aligner and the state of speech-to-text alignment in 2026](https://arxiv.org/abs/2606.18466)
*Michael McAuliffe,Kaylynn Gunter,Michael Wagner,Morgan Sonderegger*

Main category: cs.CL

TL;DR: 蒙特利尔强制对齐器（MFA）3.0版本在多个语言基准上达到最先进性能，边界误差低于15毫秒。


<details>
  <summary>Details</summary>
Motivation: 记录MFA 3.0自1.0版以来的开发进展并评估其性能。

Method: 通过扩展语言和方言覆盖、使用更大开源数据集、统一IPA词典、模型适应、跨语言音素重映射及支持工具进行改进。

Result: 在英语、日语和韩语的四个基准数据集上达到最先进或接近最先进的性能，平均边界误差低于15毫秒。

Conclusion: 适应和跨语言重映射对训练分布外的语言有效，发音概率建模和音系规则在特定条件下带来提升。
...
### [62] [Possible or Definite? A Benchmark for Evaluating Diagnostic Uncertainty Preservation in Clinical Text](https://arxiv.org/abs/2606.18471)
*Hongbo Du,Zixin Lu,Jiaming Qu*

Main category: cs.CL

TL;DR: LLM用于临床文本时，对诊断不确定性的保留能力差，常改变临床含义。


<details>
  <summary>Details</summary>
Motivation: 现有研究评估LLM生成文本的流畅性和连贯性，但未关注是否保留诊断不确定性，而这在临床实践中至关重要。

Method: 构建包含1,200份临床文档、9,184个不确定性标注（五个等级）的基准，评估三个LLM。

Result: LLM保留原始不确定性线索的比例不到一半，难以区分相邻等级。

Conclusion: LLM存在标准评估未发现的失败模式，影响其在临床工作流中的安全部署。
...
### [63] [PreUnlearn: Auditing Collateral Knowledge Damage Before Large Language Model Unlearning](https://arxiv.org/abs/2606.18473)
*Bo Su,Ankit Shah,Thai Le*

Main category: cs.CL

TL;DR: 研究大型语言模型机器遗忘中遗忘效果如何传播到不同领域知识，并提出遗忘集审计作为预未学习预测工具。


<details>
  <summary>Details</summary>
Motivation: 机器遗忘中知识边界模糊，遗忘效果会牵连相关甚至遥远知识，需理解其传播规律并前置预警。

Method: 从数据角度测量遗忘效果从遗忘集到同域和远域知识的传播，并通过特征分析预测下游损害。

Result: 发现一致衰减模式：附带损害在遗忘集附近最强，随语义距离减弱，但跨域仍存在；交互特征提供最强预测信号。
...
### [64] [Towards Scalable Customization and Deployment of Multi-Agent Systems for Enterprise Applications](https://arxiv.org/abs/2606.18502)
...
TL;DR: 提出了一个统一框架，通过两阶段（模型定制和推理优化）实现多智能体系统的快速领域适应和高效部署，吞吐量提升4.48倍且性能保持。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的多智能体系统在生产部署中面临领域定制需求高以及延迟和推理成本高的问题。

Method: 两阶段框架：第一阶段结合持续预训练、监督微调和偏好优化来定制紧凑模型；第二阶段采用推测解码和FP8量化优化推理效率。

Result: 在企业工作负载上实现4.48倍吞吐量加速，同时保持性能并提升长尾场景的鲁棒性。

Conclusion: 该框架能够快速适应特定领域，实现成本高效的推理服务且质量损失极小。
...
### [65] [MCompassRAG: Topic Metadata as a Semantic Compass for Paragraph-Level Retrieval](https://arxiv.org/abs/2606.18508)
*Amirhossein Abaskohi,Raymond Li,Gaetano Cimino,Peter West,Giuseppe Carenini,Issam H. Laradji*

Main category: cs.CL

TL;DR: 提出MCompassRAG，利用主题元数据指导检索，解决RAG系统中分块大小权衡问题，提升效率和质量。


<details>
  <summary>Details</summary>
Motivation: 细粒度分块提高检索精度但增加延迟和成本，大分块减少候选但降低相似度可靠性，尤其在深度研究任务中限制性能。

Method: 通过LLM教师蒸馏训练轻量检索器，将主题元数据融入分块嵌入空间，实现主题感知检索，无需额外LLM调用。

Result: 在六个复杂检索基准上，信息效率平均提升8.24%，延迟比最强基线降低5倍以上。

Conclusion: MCompassRAG有效提升检索效率与证据质量，尤其在异构大规模语料中表现优异。
...
### [66] [Speech-Driven End-to-End Language Discrimination towards Chinese Dialects](https://arxiv.org/abs/2606.18584)
*Fan Xu,Jian Luo,MingWen Wang,GuoDong Zhou*

Main category: cs.CL

TL;DR: 探索语音驱动特征用于中文方言辨别，结合MFCC和词级嵌入的CNN模型，在基准语料库上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统文本驱动方法在相似语言、方言辨别中效果不佳，需探索语音驱动的有效性。

Method: 先探索MFCC特征用于CNN方言辨别的适用性，再设计HMM-DNN语音识别模型预测方言词，用注意力提取判别性词，最后通过CNN融合词嵌入和MFCC特征。

Result: 在两种中文方言基准语料库上的评估表明，所提方法优于最先进方法。

Conclusion: 提出基于语音驱动的方法对细粒度中文方言辨别是合适且有效的。
...
### [67] [Dual Dimensionality for Local and Global Attention](https://arxiv.org/abs/2606.18587)
*Zhiyuan Wang,Xuan Luo,Sirui Zeng,Xifeng Yan*

Main category: cs.CL

TL;DR: 提出距离自适应表示（DAR），局部窗口内保持全维表示，远处用低维，性能接近全维基线，均匀降维效果差，挑战KV维度均匀假设，为自适应KV缓存缩减提供方向。


<details>
  <summary>Details</summary>
Motivation: Decoder-only Transformer对所有token使用相同维度的KV表示，但自然语言中局部token对预测更重要，远处token只需低维记忆，假设局部和远处token对表征容量需求不对称。

Method: 距离自适应表示（DAR），在局部上下文窗口内保持全维，窗口外使用降维表示（如原维度的1/4）。

Result: 在多个预训练规模（70M-410M参数）和1B模型的有监督微调中，DAR性能接近全维基线，均匀降维则效果更差。

Conclusion: 挑战了KV维度均匀的假设，为自适应分配表征容量提供新方向，可进一步减少推理时KV缓存。
...
### [68] [Low-resource Language Discrimination Towards Chinese Dialects with Transfer learning and Data Augmentation](https://arxiv.org/abs/2606.18597)
*Fan Xu,Yangjie Dan,Keyu Yan,Yong Ma,Mingwen Wang*

Main category: cs.CL

TL;DR: 提出基于迁移学习和数据增强的中文方言识别框架（CDDTLDA），有效解决了低资源方言的标注稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 中文方言识别面临标注资源稀缺的挑战。

Method: 先在大规模中文语料上训练源端ASR模型，再对目标端低资源方言进行速度、音高和噪声扰动数据增强，微调目标端ASR模型，并利用自注意力机制捕捉源端与目标端的共同语义特征。

Result: 在两个中文方言基准语料上显著优于现有方法。

Conclusion: 提出的框架有效提升了低资源中文方言的识别性能。
...
### [69] [Steerable Cultural Preference Optimization of Reward Models](https://arxiv.org/abs/2606.18606)
*Minsik Oh,Advit Deepak,Sophie Wu,Douwe Kiela,Ekaterina Shutova*

Main category: cs.CL

TL;DR: 本文提出SCPO算法，通过平衡训练奖励模型中的文化偏好，提升少数群体表现并减少偏差，在两个数据集上取得显著效果。


<details>
  <summary>Details</summary>
Motivation: 现有LLM对齐研究主要预测统一偏好，缺乏对多元文化子社区偏好的准确表征与平衡。

Method: 提出SCPO算法，通过加权方式整合多样文化偏好来训练奖励模型，实现平衡。

Result: 在PRISM和GlobalOpinionQA数据集上，少数奖励模型性能提升高达7点，训练数据效率提升最多280%，且偏差显著降低。

Conclusion: SCPO有效提升了跨文化偏好建模的公平性与数据效率，推动了LLM全球对齐的发展。
...
### [70] [Are LLMs Ready to Assist Physicians? PhysAssistBench for Interactive Doctor-Patient-EHR Assistance](https://arxiv.org/abs/2606.18613)
...
TL;DR: 提出PhysAssistBench基准测试，评估大语言模型在医生-患者-电子健康记录交互辅助中的协调能力，发现当前模型不可靠。


<details>
  <summary>Details</summary>
Motivation: 现有评估仅测试孤立能力（如临床知识、系统交互），但医生辅助需要同时协调这些能力，以应对模糊请求和精准工具使用。

Method: 基于MIMIC-IV真实病例，构建可扩展的交互式患者代理，将静态EHR记录转化为多轮临床场景，并形成1296个人工审核与医生验证的双语评估集。

Result: 主流大语言模型在此设置下表现不可靠，暴露了协调知识、沟通和系统交互的瓶颈。

Conclusion: 可靠的临床辅助需要跨能力协调，而非单一能力的提升。
...
### [71] [BCL: Bayesian In-Context Learning Framework for Information Extraction](https://arxiv.org/abs/2606.18620)
*Haoliang Liu,Chengkun Cai,Xu Zhao,Han Zhu,Shizhou Huang,Xinglin Zhang,Tao Chen,Jenq-Neng Hwang,Zhang Huaping,Lei Li*

Main category: cs.CL

TL;DR: 提出BCL，首个使用贝叶斯更新和粒子滤波的信息抽取上下文学习优化框架，在序列标注和关系分类任务上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有ICL方法在信息抽取中性能不一致，且缺乏系统优化和泛化能力。

Method: 通过初始化、观测、权重更新和重采样四步，利用粒子滤波和贝叶斯更新逐步优化标签表示。

Result: 大量实验表明BCL在多种IE任务上显著且一致优于现有方法。

Conclusion: BCL是首个系统优化ICL的通用框架，可有效提升信息抽取性能。
...
### [72] [PragReST: Self-Reinforcing Counterfactual Reasoning for Pragmatic Language Understanding](https://arxiv.org/abs/2606.18624)
*Jihyung Park,Minchao Huang,Leqi Liu,Elias Stengel-Eskin*

Main category: cs.CL

TL;DR: 提出PragReST框架，通过自监督构建语用QA数据、生成反事实推理轨迹，并利用微调和强化学习提升大语言模型的语用推理能力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在语用推理上表现不佳，常选择字面解释，需要不需要人工标注或教师蒸馏的改进方法。

Method: PragReST框架：自监督构建语用QA数据，生成反事实推理轨迹，通过监督微调和强化学习训练模型内化这些轨迹。

Result: 在PragMega、Ludwig、MetoQA和AltPrag四个基准上，PragReST超越了基座模型和任务特定调优基线；在准确率基准上，Qwen3-8B和Qwen3-14B分别提升5.37%和5.50%的绝对准确率。

Conclusion: 反事实推理是关键：错误主要源于无法对比观察话语与合理替代选项；去除反事实推理会大幅降低性能；训练保持外域通用知识和数学推理能力。
...
### [73] [PEC-Home: Interpretation of Progressively Elliptical Commands in Smart Homes](https://arxiv.org/abs/2606.18636)
*Yingyu Shan,Zeming Liu,Silin Li,Boao Qian,Jiashu Yao,Yuhang Guo,Haifeng Wang*

Main category: cs.CL

TL;DR: 提出首个针对智能家居中渐进式省略命令的数据集PEC-Home，实验表明现有助手难以准确解析省略命令，即使借助历史记录，准确率仍低于完整命令。


<details>
  <summary>Details</summary>
Motivation: 当前智能家居助手忽略人类对话中的渐进省略现象，导致无法准确理解省略命令，面临多用户环境期望差异引起的指代歧义和用户偏好随时间或环境变化的意图歧义问题。

Method: 构建PEC-Home数据集，模拟智能家居中渐进式省略命令场景，并基于多种大语言模型（包括GPT-4o）进行实验评估。

Result: 现有助手仅凭省略命令难以执行用户意图操作，即使提供对话历史存储和检索工具，执行准确率仍低于使用完整命令。

Conclusion: 现有智能家居助手在解析省略命令方面存在明显不足，需要专门设计方法以提升对渐进省略对话的理解能力。
...
### [74] [The Wrong Kind of Right: Quantifying and Localizing Misfired Alignment in LLMs](https://arxiv.org/abs/2606.18656)
*Naihao Deng,Yiming Feng,Chimaobi Okite,Kaijian Zou,Lu Wang,Rada Mihalcea,Yulong Chen*

Main category: cs.CL

TL;DR: 研究发现，大型语言模型的安全对齐可能导致“误触发对齐”，即模型因安全考虑而拒绝上下文支持的合理结论。作者提出了VETO基准和新指标MAR，评估了25个模型，发现所有模型都有显著MAR，而人类为0%。机制分析表明这是对齐训练导致的晚期层抑制，需改进对齐方法以保留上下文依据。


<details>
  <summary>Details</summary>
Motivation: 安全对齐旨在使LLM安全可靠，避免不安全推断，但可能导致模型过度保守，拒绝合理结论。本文量化了这一现象，特别是与刻板印象相关的误触发。

Method: 引入VETO基准（2032个来自BBQ的对比对）和新指标MAR（衡量模型在刻板印象相关问题上失败而对比问题成功的情况）。在25个LLM上基准测试，并进行启动实验和机制分析（开放权重模型）。

Result: 所有LLM的MAR在4.7%到18.9%之间，人类为0%。启动实验表明安全相关提示可显著放大MAR。机制分析显示晚期层抑制了有证据支持的答案，且这种抑制在指令训练后出现。

Conclusion: 当前对齐方法可能过度概括表面安全线索，甚至覆盖客观证据。需要更先进的对齐目标，以更好地保留上下文依据。
...
### [75] [RegMix-D: Dynamic Data Mixing via Proxy Training Trajectories](https://arxiv.org/abs/2606.18663)
*Kaiyan Zhao,Zhongtao Miao,Akiko Aizawa,Yoshimasa Tsuruoka*

Main category: cs.CL

TL;DR: 提出RegMix-D，通过利用代理运行的完整损失轨迹来动态调整数据混合比例，在减少代理计算预算的同时提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有静态混合方法如RegMix仅使用终点损失，忽略了训练过程中的损失轨迹信息，无法适应模型训练不同阶段的需求。

Method: 基于代理运行的完整损失轨迹训练回归模型，预测多个训练阶段的最优混合比例；支持离线预生成调度表和在线自适应调整两种模式。

Result: 在Pile数据集上使用1B参数模型训练25B tokens，RegMix-D在13个下游任务上一致优于RegMix和DoReMi，且仅需128个代理模型（RegMix代理计算预算的25%）。

Conclusion: 动态数据混合能有效提升预训练效果，且通过利用损失轨迹可大幅降低代理计算成本。
...
### [76] [TW-LegalBench: Measuring Taiwanese Legal Understanding](https://arxiv.org/abs/2606.18699)
*Fei-Yueh Chen,Chun Huang Lin,Chan Wei Hsu,Kuan Hsuan Yeh,Zih-Ching Chen,Kuan-Ming Chen,Patrick Chung-Chia Huang*

Main category: cs.CL

TL;DR: 提出TW-LegalBench基准，评估LLM在台湾法律领域的表现，发现顶尖模型通过律师考试但未达到法官/检察官水平，法律文本生成仍有挑战。


<details>
  <summary>Details</summary>
Motivation: 现有法律基准主要针对英美法和简体中文民法，缺乏对台湾法律的评估。

Method: 构建TW-LegalBench，包含选择题、开放式问题、法律判决预测三类任务，评估13个LLM。

Result: 顶尖模型在律师考试中超过11%通过率，但未达到法官/检察官1-2%通过率；法律判决预测中判决类型准确但法律条文引用困难。

Conclusion: LLM在法律资格考试中接近人类水平，但可靠的法律文本生成仍具挑战。
...
### [77] [LLMs Struggle to Measure What Distinguishes Students of Different Proficiency Levels: A Study of Item Discrimination in Reading Comprehension Assessment](https://arxiv.org/abs/2606.18709)
*Han Chen,Ming Li,Chenguang Wang,Yijun Liang,Dawei Zhou,Hong jiao,Tianyi Zhou*

Main category: cs.CL

TL;DR: 评估了42个LLM在零样本设置下对项目区分度的估计能力，发现直接预测和基于CTT校准的方法都与人类校准的区分度相关性较弱。


<details>
  <summary>Details</summary>
Motivation: 探索LLM是否能够捕捉教育评估中的项目区分度，而不仅仅是项目难度。

Method: 采用直接预测（显式估计区分度值）和基于反应的CTT校准（将LLM回答视为合成学生响应）两种互补方法。

Result: 最佳模型直接预测的Spearman相关性仅为0.152；基于CTT校准的全体合成响应池达到0.241。

Conclusion: 当前LLM包含非随机的区分度相关信号，但无法可靠地捕捉评估项目如何区分人类学生，表明项目区分度对基于LLM的心理测量评估仍是开放挑战。
...
### [78] [Morpheus: A Morphology-Aware Neural Tokenizer and Word Embedder for Turkish](https://arxiv.org/abs/2606.18717)
*Tolga \c{S}akar*

Main category: cs.CL

TL;DR: Morpheus是一个神经语素边界模型，用于土耳其语，同时作为无损、形态感知的分词器和词嵌入生成器。


<details>
  <summary>Details</summary>
Motivation: 土耳其语是黏着语，而现有子词分词器基于统计，不能正确处理语素，且某些分词器无法还原原始文本。

Method: 使用可微泊松二项动态规划将字符边界概率转换为软语素隶属度，训练后得到精确分段，无字符串归一化，保证编码解码可逆。

Result: 在可逆分词器中达到最低的bits-per-character（1.425），形态对齐F1约0.61（子词族约0.32），GPU内存比64K词汇子词分词器节省约19%。作为嵌入器，在词汇检索（MAP 0.85）和同根验证（ROC-AUC 1.00）上表现优异。

Conclusion: Morpheus在形态感知分词和词汇层面上有效，但在需要上下文的复杂任务上有限制。
...
### [79] [LegalWorld: A Life-Cycle Interactive Environment for Legal Agents](https://arxiv.org/abs/2606.18728)
*Songhan Zuo,Shengbin Yue,Tao Chiang,Guanying Li,Yun Song,Xuanjing Huang,Zhongyu Wei*

Main category: cs.CL

TL;DR: LegalWorld是一个模拟中国民事诉讼全生命周期的交互环境，包含五个阶段（七个子场景），基于75,309份判决书构建，用于评估法律代理人的跨阶段能力。


<details>
  <summary>Details</summary>
Motivation: 现有法律基准评估孤立任务，法律代理模拟器忽略跨阶段因果依赖，无法反映民事诉讼的生命周期特性。

Method: 构建LegalWorld环境，将民事诉讼建模为因果连接的状态链，包括五个阶段和多个子场景，搭配本地记忆、全局案例记忆和技能/工具库，确保纠纷在全生命周期的一致性。

Result: 来自217名法律背景评估者的18,992份评分确认LegalWorld轨迹在程序上忠实且角色一致；跨模型评估揭示聚合分数无法展现的显著差异，没有单一模型在咨询、起草和庭审辩护中全面领先。

Conclusion: LegalWorld提供了更全面的法律代理评估基准，有助于发现模型在不同诉讼阶段的专业能力分化。
...
### [80] [Output Vector Editing for Memorization Mitigation in Large Language Models](https://arxiv.org/abs/2606.18767)
*Ahmad Dawar Hakimi,Kaiwei Lei,Isabelle Augenstein,Hinrich Sch\"utze*

Main category: cs.CL

TL;DR: 提出输出向量编辑方法，通过修改MLP输出向量引入干扰项来抑制大模型记忆，在OLMo-7B上实现87.9%抑制率，并揭示MLP编辑的机制边界。


<details>
  <summary>Details</summary>
Motivation: 现有神经元编辑方法仅归零激活，忽略了输出向量对残差流的影响，无法有效抑制记忆。

Method: 定位负责记忆的MLP神经元，通过约束优化修改其输出向量，在词汇空间引入干扰项，保持激活不变。

Result: 在OLMo-7B的6831个记忆序列上达87.9%抑制，比零消融高2.7倍；四种编辑模式集成覆盖96.5%序列；约14%序列无法被MLP编辑覆盖，注意力头消融可恢复60-64%。

Conclusion: 输出向量编辑有效抑制记忆，成功率随模型规模增大，注意力可作为互补机制处理MLP编辑失败的案例。
...
### [81] [Lost in a Single Vector: Improving Long-Document Retrieval with Chunk Evidence Aggregation](https://arxiv.org/abs/2606.18781)
*Shanshan Lyu,Yiwei Wang,Yujun Cai,Jiafeng Guo,Shenghua Liu*

Main category: cs.CL

TL;DR: 提出 DICE 方法，通过分块编码并聚合，解决长文档检索中早期压缩导致的证据稀释问题。


<details>
  <summary>Details</summary>
Motivation: 密集检索中，长文档中短但关键的证据在文档编码时被弱化，导致检索失败。

Method: 将文档分块，用冻结模型独立编码，再聚合为单个向量，保持标准接口。

Result: 在 LongEmbed 上，DICE 提升检索性能，尤其在超 4k token 片段上；92.8% 案例中 EDI 低于单向量基线。

Conclusion: 文档级编码是长文档检索中实用且未充分探索的杠杆。
...
### [82] [RedactionBench](https://arxiv.org/abs/2606.18782)
*Sean Brynj\'olfsson,Shashvat Jayakrishnan,Esha Sali,Diptanshu Purwar,Madhav Aggarwal*

Main category: cs.CL

TL;DR: 提出了基于情境完整性的PII编辑基准RedactionBench和新指标R-Score，评估显示情境编辑仍是未解决问题，人类对于情境编辑的共识很低。


<details>
  <summary>Details</summary>
Motivation: 现有基准混淆了提取机制与隐私语义，PII是否违规取决于具体情境，亟需区分编辑与实体识别。

Method: 手动标注200篇跨11个领域的文档，引入字符级指标R-Score，评估NER模型、小模型和前沿模型。

Result: 人类对强制编辑和保留安全文本的共识高（89.4%和94.1%），但对情境编辑仅47.7%共识；R-Score能解耦情境模糊性。

Conclusion: 情境编辑仍是未解决问题，发布RedactionBench以促进未来隐私保护研究。
...
TL;DR: 现有放射学报告评估指标不重视临床准确性，LLM虽有医学知识但存在歧视偏差。通过ReEvalMed基准研究，发现LLM能检测错误但过度惩罚无害改写。合成4k报告对训练轻量指标，在Qwen3-8B和MedGemma-4B上超越32B模型。单次通过更实用，两次通过主要用于平衡D-R。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标将报告质量简化为无医学依据的标量，忽略临床准确性；LLM虽具备医学知识，但难以区分临床显著错误和无害变化。

Method: 使用ReEvalMed基准测试8个LLM评估器在单次和两次通过设置下的判别力和鲁棒性。合成4k报告对，在Qwen3-8B和MedGemma-4B上训练轻量可解释指标。

Result: 训练指标提升了临床显著性边界，超过32B规模医学LLM，与专有模型竞争力相当。两次通过设置未持续提升整体性能，主要是以鲁棒性换取判别力。
...
### [84] [Beyond Reward Engineering: A Data Recipe for Long-Context Reinforcement Learning](https://arxiv.org/abs/2606.18831)
*Xiaoyue Xu,Sikui Zhang,Xiaorong Wang,Xu Han,Chaojun Xiao*

Main category: cs.CL

TL;DR: 通过简单数据配方和最小结果导向GRPO，显著提升长上下文推理能力，在多个基准和代理任务上取得提升。


<details>
  <summary>Details</summary>
Motivation: 现有RL提升长上下文推理工作侧重奖励工程，训练数据匮乏，需从数据角度改进。

Method: 构建8个数据集（约1.4万样本），覆盖检索、多证据合成、推理三类任务，配合最小结果导向GRPO。

Result: Qwen3-4B/8B/30B-A3B在7个长上下文基准平均+7.2/+3.2/+6.4分，代理任务GAIA+4.8、BrowseComp+7.0。

Conclusion: 简单数据配方足以大幅提升长上下文推理，且收益可迁移至代理任务，将公开数据集。
...
### [85] [ScholarSum: Student-Teacher Abstractive Summarization via Knowledge Graph Reasoning and Reflective Refinement](https://arxiv.org/abs/2606.18850)
*Bohou Zhang,Xiaoyu Tao,Mingyue Cheng,Huijie Liu,Qi Liu*

Main category: cs.CL

TL;DR: 提出ScholarSum框架，通过层次化反思图模拟学生-教师写作过程，实现流畅且忠实的科学文献摘要。


<details>
  <summary>Details</summary>
Motivation: 现有摘要方法难以兼顾语言流畅性和事实忠实性：提取式破坏宏观逻辑，生成式事实一致性差。

Method: 构建层次知识图捕捉全局逻辑，学生生成初稿，教师审查并迭代检索重写直至满足质量要求。

Result: 在完整性和忠实性上显著优于基线方法。

Conclusion: ScholarSum有效平衡流畅与事实，代码已开源。
...
### [86] [Aligning Implied Statements for Implicit Hate Speech Generalizability with Context-Bounded Semi-hard Negative Mining](https://arxiv.org/abs/2606.18852)
*Wicaksono Leksono Muhamad,Yunita Sari*

Main category: cs.CL

TL;DR: 提出ImpSH框架，通过三元组对齐与上下文约束的半硬负样本，有效提升隐式仇恨言论分类的跨域性能。


<details>
  <summary>Details</summary>
Motivation: 隐式仇恨言论分类因意图隐含而困难，现有监督对比方法易过拟合表面线索且跨数据集迁移差。

Method: ImpSH利用三元组框架将帖子与隐含陈述对齐，并采用上下文约束的半硬负样本聚焦学习；另提出AugSH通过数据增强构建正样本。

Result: 在IHC、SBIC和DynaHate上使用BERT和HateBERT的受控评估中，ImpSH在匹配预处理和调优预算下是标准监督对比基线的可行替代，常提升跨域性能；表征分析显示正对更紧密且全局分布均衡。

Conclusion: 通过上下文约束挖掘将帖子与其隐含陈述对齐，提供了更稳定的、类似双射的映射，克服了传统聚类表征学习的波动性。
...
### [87] [Approximate Structured Diffusion for Sequence Labelling](https://arxiv.org/abs/2606.18856)
*Nicolas Floquet,Joseph Le Roux,Nadi Tomeh*

Main category: cs.CL

TL;DR: 利用扩散训练以整个标签序列为条件的CRF，提升序列标注性能。


<details>
  <summary>Details</summary>
Motivation: 传统CRF假设有限决策跨度，限制了长距离依赖的表达能力。

Method: 使用扩散模型训练CRF，条件为噪声标签序列，结合近似CRF推理。

Result: 在词性标注任务上错误率降低16.5%。

Conclusion: 该方法有效提升标签准确性。
...
### [88] [Efficient Financial Language Understanding via Distillation with Synthetic Data](https://arxiv.org/abs/2606.18875)
*Wen-Fong (Xavier) Huang,Edwin Simpson*

Main category: cs.CL

TL;DR: 提出一种通过蒸馏和合成数据实现金融情感分析的高效框架，在低资源条件下利用聚类种子选择生成代表性合成数据，紧凑模型性能接近甚至超越大型教师模型。


<details>
  <summary>Details</summary>
Motivation: 大型指令跟随模型部署成本高，尤其在金融领域标注数据有限，需要低资源的领域适应方法。

Method: 手工标注少量真实样本，聚类后基于簇选择种子，通过结构化少样本提示生成合成数据，再蒸馏至紧凑学生模型。

Result: 聚类种子选择生成的合成数据比随机采样更具代表性，紧凑模型在复杂噪声文本上甚至超越教师模型，在正式文本上保持竞争力。

Conclusion: 框架提供了一种资源高效的金融NLP领域适应途径，仅需极少量人工标注。
...
### [89] [Improving Medical Communication using Rubric-Guided Counterfactual Recommendations](https://arxiv.org/abs/2606.18889)
*Adrian Cosma,Nicoleta-Nina Basoc,Andrei Niculae,Cosmin Dumitrache,Emilian Radoi*

Main category: cs.CL

TL;DR: 提出LM引导的反事实推荐管道，通过最小可解释沟通变化提升远程医疗患者正面反馈概率，平均增益6.41%，非负率93.31%。


<details>
  <summary>Details</summary>
Motivation: 文本远程医疗的患者反馈主要反映沟通质量而非医学准确性，需要在不干扰医学内容的前提下改进沟通。

Method: LM引导的反事实推荐管道，发现并优化可解释沟通特征（语气、个性化、可操作性、完整性），结合医患互动元数据估计正面反馈，搜索低代价序数特征变化推荐最小改变。

Result: 独立审计下推荐平均提升正面反馈概率6.41%，93.31%的推荐非负。

Conclusion: 小的可解释沟通改变能捕获大部分预测收益，同时保留医生对医学推理和最终措辞的控制。
...
### [90] [Learning Robust Pair Confidence for Multimodal Emotion-Cause Pair Extraction](https://arxiv.org/abs/2606.18893)
*Zhuangzhuang Pan,Ning Dong,Yingna Su,Yan Xia*

Main category: cs.CL

TL;DR: 通过RPCL方法显式塑造成对置信度，有效提升了多模态情感-原因对提取的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将候选链接独立处理，导致金标准对与困难负样本的置信度差异不足，容易依赖偶然的非金标准上下文。

Method: 提出RPCL框架，通过置信度差异边界约束和扰动视图对齐，增强成对置信度的区分性和稳定性，推理时使用原始模型。

Result: 在ECF、MECAD和MEC4数据集上，Pair F1提升2.58-2.83个百分点，Pair AUPRC在所有三个数据集上改善，且金标准-负样本置信度差距更大。
...
### [91] [SAGE: Stochastic Prompt Optimization via Agent-Guided Exploration](https://arxiv.org/abs/2606.18902)
*Ziyi Zhu,Luka Smyth,Saki Shinoda,Jinghong Chen*

Main category: cs.CL

TL;DR: 提出随机提示优化（SPO）框架，比较三种黑盒搜索策略，并在心理健康聊天机器人上通过连续优化提升次日留存。


<details>
  <summary>Details</summary>
Motivation: 文本梯度并非真实梯度，因此自动提示优化应被视为黑盒搜索问题。

Method: SPO框架包含三种策略：错误引导随机搜索、遗传算法和多智能体管道SAGE（带诊断代码执行），并在心理健康聊天机器人上应用连续优化的A/B测试。

Result: 无单一策略占优，效果取决于错误类型和景观结构；SAGE经过八轮优化将次日留存率显著提升。

Conclusion: 定性与定量验证结合是智能体优化在开放任务对话中有效的关键。
...
### [92] [As Easy as Rocket Science: Assessing the Ability of Large Language Models to Interpret Negation in Figurative Language](https://arxiv.org/abs/2606.18922)
*Jasmine Owers,Edwin Simpson,Martha Lewis*

Main category: cs.CL

TL;DR: 研究了否定和比喻语言对大型语言模型的挑战，发现两者结合尤其困难，且表现依赖提示风格。


<details>
  <summary>Details</summary>
Motivation: 比喻语言和否定广泛使用，但语言模型难以处理，且无法微调，因此需要评估其理解能力。

Method: 对现有比喻语言数据集添加否定注释，测试多种语言模型。

Result: 否定和比喻结合带来特别挑战，表现高度依赖提示风格。

Conclusion: 提示风格影响显著，需谨慎设计以处理否定和比喻组合。
...
### [93] [SenFlow: Inter-Sentence Flow Modeling for AI-Generated Text Detection in Hybrid Documents](https://arxiv.org/abs/2606.18946)
*Jingkun Luo,Yifan Sun,Da-Tian Peng,Guanxiong Pei*

Main category: cs.CL

TL;DR: 提出SenFlow方法，通过结构化预测和句子间依赖建模，在混合文档AI文本检测中达到最优，并发现句子长度差距可作为线索。


<details>
  <summary>Details</summary>
Motivation: 现有句子级AI文本检测忽略句子间依赖，且基准不包括最新生成器。

Method: 构建MOSAIC基准，采用困惑度一致性过滤器；将S-AGTD视为结构化预测，用SenFlow集成图传播和CRF解码。

Result: SenFlow在MOSAIC上跨域迁移取得+4.15 pp Macro-F1优势，且发现句子长度差距可利用。

Conclusion: 句子间依赖建模对混合文档检测有效，且即使平等化明显线索，仍有隐含特征可供利用。
...
### [94] [GraphPO: Graph-based Policy Optimization for Reasoning Models](https://arxiv.org/abs/2606.18954)
*Yuliang Zhan,Xinyu Tang,Jian Li,Dandan Zheng,Weilong Chai,Jingdong Chen,Jun Zhou,Ge Wu,Wenyue Tang,Hao Sun*

Main category: cs.CL

TL;DR: 提出GraphPO，一种基于图的强化学习框架，通过将推理步骤表示为有向无环图，合并语义等价路径，减少冗余探索并降低优势估计方差，在相同预算下优于链和树基线。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法独立采样响应，导致冗余探索和稀疏奖励难以识别有效步骤；树方法虽部分缓解，但分支仍独立扩展，重复探索且忽略状态分散，优势估计方差高。

Method: 将轨迹表示为有向无环图，节点为语义状态，边为推理步骤；合并语义等价路径，共享后缀并重新分配预算；对入边分配效率优势，出边分配正确性优势，从结果推导过程监督。

Result: 在三个LLM上，推理和智能体搜索基准实验中，GraphPO在相同token预算或响应预算下一致优于链和树基线。

Conclusion: GraphPO通过图结构减少冗余探索，降低优势估计方差，提升推理效率，是RLVR的有效改进。
...
### [95] [Beyond Tokenization: Direct Timestep Embedding and Contrastive Alignment for Time-Series Question Answering](https://arxiv.org/abs/2606.18986)
*Yafeng Wu,Huu Hiep Nguyen,Thin Nguyen,Hung Le*

Main category: cs.CL

TL;DR: 提出CADE框架，通过直接时间步嵌入和语义对齐，解决大语言模型处理时间序列时的分词瓶颈，在Time-MQA基准上提升六个TSQA任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将原始数值序列直接输入LLM会出现分词瓶颈，导致数值信息丢失；基于补丁的编码器固定粒度会破坏模式并隐藏时间步，且难以跨数据集迁移。

Method: CADE框架包含直接时间步嵌入（逐点线性编码器+MLP投影）和单向监督对比损失（对齐时间序列嵌入与冻结类名文本锚点）。

Result: 在Time-MQA基准上，CADE在六个TSQA任务中一致提升性能，优于开源和专有LLM基线。

Conclusion: CADE通过保持索引级访问和语义对齐，有效提升了时间序列问答的准确性。
...
### [96] [G-IdiomAlign: A Gloss-Pivoted Benchmark for Cross-Lingual Idiom Alignment](https://arxiv.org/abs/2606.18989)
*Fengying Ye,Yanming Sun,Runzhe Zhan,Zheqi Zhang,Lidia S. Chao,Derek F. Wong*

Main category: cs.CL

TL;DR: 提出G-IdiomAlign基准，通过英文注释（gloss）锚定习语，评估LLMs的习语翻译能力，发现字面翻译偏差为主要失败模式，注释可提升但仍有较大改进空间。


<details>
  <summary>Details</summary>
Motivation: 习语因非组合性和弱表面形式基础难以跨语言翻译，字面映射不可靠，需要更好的评估方法和语义支点。

Method: 构建G-IdiomAlign基准，使用Wiktionary注释锚定习语，设计多选题和词汇-对比生成两种协议，对比有无注释的影响。

Result: 字面翻译偏差普遍存在，尤其在低资源语言中；注释在嵌入语义代理下提升生成质量，但整体性能仍有限。Qwen3-8B分析表明注意力头比层更显著影响差异。

Conclusion: 当前LLMs习语翻译能力不足，注释作为语义支点有帮助但不够，未来需进一步探索。
...
### [97] [Enhancing Multilingual Reasoning via Steerable Model Merging](https://arxiv.org/abs/2606.19002)
...
TL;DR: 提出可调控模型合并框架ST-Merge，通过门控交叉注意力自适应调节源模型贡献，在多语言推理任务上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型合并策略无法解决源模型间的冲突，导致性能次优，且单一合并方式不适应不同输入特征。

Method: 引入门控交叉注意力机制，动态加权或过滤两个源模型的贡献，实现可调控的模型合并。

Result: 在21种语言的四个多语言推理基准上，ST-Merge持续优于多种强基线方法。

Conclusion: 自适应调节源模型贡献的合并策略可有效提升多语言推理性能。
...
### [98] [Sumi: Open Uniform Diffusion Language Model from Scratch](https://arxiv.org/abs/2606.19005)
*Mengyu Ye,Keito Kudo,Wataru Ikeda,Ryosuke Matsuda,Keisuke Sakaguchi,Jun Suzuki*

Main category: cs.CL

TL;DR: 提出了Sumi，一个7B参数的均匀扩散语言模型，从零预训练于1.5T tokens，表现与自回归模型相当，但常识基准较弱。


<details>
  <summary>Details</summary>
Motivation: 目前没有大规模预训练的均匀扩散语言模型，而自回归和掩码扩散模型已有大规模版本，需要干净参考点来研究均匀扩散的扩展行为、生成动态、可控性等。

Method: 从零预训练一个7B参数的均匀扩散语言模型（Sumi），使用1.5T tokens的公开数据混合。

Result: 在知识、推理和编程基准上与同等token预算的自回归模型竞争，但在常识基准上表现不足，可能由于数据混合偏重教育。

Conclusion: 开源Sumi的权重、检查点和完整训练方案，希望促进社区对大规模均匀扩散的研究。
...
### [99] [Which Sections of a Research Paper Best Reveal Its Research Methods? Evidence from Library and Information Science](https://arxiv.org/abs/2606.19051)
*Qiuyu Fang,Jiayi Hao,Chengzhi Zhang*

Main category: cs.CL

TL;DR: 提出基于全文分段组合的研究方法自动多标签分类策略，在1954篇标注语料上评估，发现中后段和结尾段判别力更强，结合元数据和跨段组合可提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究依赖标题和摘要，但摘要方法信息有限；利用全文面临长度过长和信息冗余。

Method: 根据物理位置对全文分段，组合不同段落，使用多个模型在1954篇全文标注语料上评估分类性能。

Result: 方法信息在全文分布不均，中后段和结尾段判别力强；集成书目元数据和跨段组合显著提升分类效果。

Conclusion: 分段组合策略有效，元数据和跨段组合能增强分类性能。
...
### [100] [Leadership as Coordination Control: Behavioral Signatures and the Recovery-Advantage Boundary in Multi-Agent LLM Teams](https://arxiv.org/abs/2606.19111)
*Haewoon Kwak*

Main category: cs.CL

TL;DR: 团队科学认为领导力是权变的，多智能体LLM团队中过程级协调控制仅在特定条件下有效，且没有哪种控制方式在所有情况下都占优。


<details>
  <summary>Details</summary>
Motivation: 探究多智能体LLM团队中，在什么可测量条件下过程级协调控制能增加价值，以及这些条件是否与团队科学预测一致。

Method: 使用行为特征（多数锁定、探索、从错误共识中恢复）和逐动作消融，将三种经典领导风格（交易型、变革型、情境型）作为控制器操作共享动作词汇（探索、修订、接受、综合）。在四个任务体系和三个开源模型系列上进行测试。

Result: 没有一种控制器在所有情况下准确率都占优；交易型控制与共享初始投票在所有12个组合中相差不到1.3个百分点；仅在其中一个组合（llama-4-scout social）中，情境型控制比平面交互高出8个百分点。

Conclusion: 过程级协调控制是一种权变因素，需要测量和理论映射，而非追求排行榜领先；大多数情况下准确性无显著提升正是理论所预测的，而非控制器的失败。
...
### [101] [Dango: A Strictly L1-Only Large Language Model for Studying Second Language Acquisition](https://arxiv.org/abs/2606.19170)
*Shiho Matta,Yin Jou Huang,Fei Cheng,Takashi Kodama,Hirokazu Kiyomaru,Yugo Murawaki*

Main category: cs.CL

TL;DR: 提出了一个1.8B参数的日语-英语第二语言习得模型Dango，通过过滤预训练语料中的L2污染并微调于LLM生成的课程，实现了类人的L2产出。


<details>
  <summary>Details</summary>
Motivation: 现有研究多使用小规模或非解码器模型，难以生成开放文本，不适合作为L2模拟器。需要更大模型，但面临L2污染问题。

Method: 过滤日语单语预训练语料中的英语内容，减少过早暴露，同时保留真实的最小暴露；然后在LLM生成的L2学习课程上微调。

Result: Dango表现出类人的L2产出模式，在性能上优于未过滤和多语言基线模型。

Conclusion: 发布了模型、数据和代码，以促进可重复的计算SLA研究和面向学习者的应用。
...
### [102] [Language Models as Interfaces, Not Oracles: A Hybrid LLM-ML System for Pediatric Appendicitis](https://arxiv.org/abs/2606.19183)
*Soheyl Bateni,Maryam Abdolali*

Main category: cs.CL

TL;DR: 提出ClaMPAPP混合系统，将大语言模型作为接口，机器学习模型作为预测器，用于阑尾炎诊断，在独立儿科队列中优于端到端LLM基线。


<details>
  <summary>Details</summary>
Motivation: 大语言模型对提示词和信息顺序敏感，易产生合理但错误的输出；结构化机器学习模型需表格输入，难以整合叙事性临床工作流程。

Method: ClaMPAPP利用LLM从叙事文本中提取受模式约束的临床特征，经过确定性合理性检查后，将验证后的特征输入基于XGBoost的分类器。

Result: 在内部和外部验证中，ClaMPAPP实现了最强的整体诊断性能，最小化漏诊病例；端到端LLM表现出不稳定的敏感度-特异度权衡，且在叙述重排下性能下降更严重。

Conclusion: 结果支持LLM作为接口、ML作为预测器的设计，分离自然语言可用性与预测推理，为临床决策支持提供更可审计的路径。
...
### [103] [RECOM: A Validity Discrimination Tradeoff in Automatic Metrics for Open Ended Reddit Question Answering](https://arxiv.org/abs/2606.19218)
*Pushwitha Krishnappa,Amit Das,Vinija Jain,Aman Chadha,Tathagata Mukherjee*

Main category: cs.CL

TL;DR: 引入RECOM数据集，发现自动度量在评估LLM生成文本时，有效性和区分力之间存在根本性权衡，建议同时报告两个维度的指标。


<details>
  <summary>Details</summary>
Motivation: 自动度量常被用于评估LLM生成文本，但需同时满足两个冲突目标：衡量内容真实对齐（有效性）和区分不同模型性能（区分力）。在开放、主观问答中，两者存在冲突。

Method: 构建RECOM数据集，包含15,000个Reddit问题（2025年9月）及其真实社区回复，所有回复在模型训练截止日期后。用五个开源LLM（7-10B）生成回答，比较多种自动度量（如余弦相似度、BERTScore）与随机乱序基线。

Result: 余弦相似度能有效区分真实与随机回答（Cohen's d≈2），但无法区分不同模型（|d|<0.1）；BERTScore似乎能区分模型（原始|d|≤0.63），但控制回答长度后降至|d|=0.09且有效性弱（d≈0.8 vs 余弦≈2）。LLM评委同样重现有效性的差距且区分模型能力弱。

Conclusion: 所有度量在相同输出上表现出有效性-区分力权衡，根源在于表示设计。建议在两条轴上报告度量，并加入随机基线。RECOM数据集已公开。
...
### [104] [DreamReasoner-8B: Block-Size Curriculum Learning for Diffusion Reasoning Models](https://arxiv.org/abs/2606.19257)
*Zirui Wu,Lin Zheng,Jiacheng Ye,Shansan Gong,Xueliang Zhao,Yansong Feng,Wei Bi,Lingpeng Kong*

Main category: cs.CL

TL;DR: 开发了开源块扩散推理模型DreamReasoner-8B，通过块大小课程学习解决训练与推理块大小不匹配导致的推理性能差问题，在数学和代码推理上达到与领先自回归模型相当的水平。


<details>
  <summary>Details</summary>
Motivation: 现有块扩散语言模型在长链推理中可扩展性未解，且训练大块大小导致推理性能极差，小块大小则保持有效推理，存在粒度差距。

Method: 提出块大小课程学习，逐步从细粒度块大小过渡到粗粒度块大小进行训练，从而弥合粒度差距并实现跨推理块大小的高泛化能力。

Result: DreamReasoner-8B在数学和代码推理基准上取得了与Qwen3-8B等领先开源自回归模型相竞争的结果。

Conclusion: 该工作为高效、可推理的扩散语言模型奠定了实践基础。
...
### [105] [Trade-offs in Medical LLM Adaptation: An Empirical Study in French QA](https://arxiv.org/abs/2606.19266)
*Ikram Belmadani,Oumaima El Khettari,Carlos Ramisch,Frederic Bechet,Richard Dufour,Benoit Favre*

Main category: cs.CL

TL;DR: 以法语医学问答为案例，研究大语言模型的领域适应策略，发现持续预训练+监督微调在多选题上提升有限，监督微调是性价比高的默认选择；开放题中持续预训练提升指标，监督微调可能降低生成质量，指令微调与联合策略更优。


<details>
  <summary>Details</summary>
Motivation: 大语言模型领域适应策略的有效性尚不明确，需系统比较不同方法。

Method: 对比持续预训练、监督微调及二者组合，涵盖多种模型族、规模和初始化方式，使用多选题和开放题评估，结合自动指标与LLM评判。

Result: 多选题上持续预训练+监督微调最优但增益小且不显著；开放题上持续预训练提升重叠指标，监督微调常降低生成质量，指令微调与联合策略受LLM评估偏好。跨语言实验显示法语适应可迁移至英语。
...
### [106] [Enhancing Decision-Making with Large Language Models through Multi-Agent Fictitious Play](https://arxiv.org/abs/2606.19308)
*Leyang Shen,Yang Zhang,Xiaoyan Zhao,Chun Kai Ling,Tat-Seng Chua*

Main category: cs.CL

TL;DR: 针对基于LLM的多智能体系统在决策任务中面临的“立场纠缠”问题，提出多智能体虚拟博弈（MAFP）方法，通过迭代均衡寻求过程提升决策质量与鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的多智能体系统擅长处理执行复杂度高的任务，但在决策任务中，因各利益相关者立场相互依赖而难以独立求解，存在“立场纠缠”挑战。

Method: 提出MAFP，将利益相关者立场表示为智能体，将决策制定形式化为均衡寻求过程，基于虚拟博弈原理，每个智能体通过最佳应对其他智能体过去决策的经验混合来迭代更新决策。

Result: 在竞争场景的决策任务上，MAFP在锦标赛强度和鲁棒性两个互补指标上优于单轮和多轮基线。

Conclusion: MAFP有效解决了立场纠缠问题，提升了决策质量和鲁棒性。
...
### [107] [Freeing the Law with LOCUS: A Local Ordinance Corpus for the United States](https://arxiv.org/abs/2606.19334)
*Denis Peskoff,Joe Barrow,Christopher Vu,Diag Davenport*

Main category: cs.CL

TL;DR: LOCUS是一个包含美国9,239个城市和县条例的机器可读语料库，通过OCR处理多种格式，并训练分类器分析法律的维度如不透明性。


<details>
  <summary>Details</summary>
Motivation: 美国地方法规分散在多个供应商平台，缺乏机器可读语料库，阻碍大规模法律AI研究。

Method: 收集所有公开的市政和县条例，使用OCR处理文档格式，构建原始语料库和县级调和访问层，并训练基于ModernBERT的分类器。

Result: 原始语料库覆盖9,239个城市和县；县级层覆盖2,309个县（占多数人口）。分类器可分析不透明性等维度。

Conclusion: LOCUS-v1已发布，为法律AI提供了大规模本地法律数据资源，支持可重复性和扩展。
...
### [108] [Learning User Simulators with Turing Rewards](https://arxiv.org/abs/2606.19336)
*Yingshan Susan Wang,Cedegao E. Zhang,Linlu Qiu,Zexue He,Pengyuan Li,Alex Pentland,Roger P. Levy,Yoon Kim*

Main category: cs.CL

TL;DR: 提出Turing-RL，一种基于图灵测试的强化学习方法，用于训练用户模拟器，通过优化不可区分性而非响应匹配，在对话聊天和Reddit论坛讨论中优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有用户模拟器训练方法通常通过匹配单一真实响应（如最大化对数概率或使用相似性奖励），但忽略了响应的自然性和不可区分性。

Method: 采用基于图灵测试的判别性奖励，利用LLM评判器评估生成响应与真实用户响应的不可区分程度，并通过强化学习优化用户模拟器LLM以产生类似真实用户的响应。
...
### [109] [Collaborative Large and Small Language Models for Accurate and Scalable Data Repair](https://arxiv.org/abs/2606.17582)
*Qian Chen,Jianwei Wang,Wenjie Zhang*

Main category: cs.DB

TL;DR: 提出LasRepair系列框架，通过大语言模型指导小语言模型进行数据修复，并引入EM过程和置信度加权，显著提升修复质量。


<details>
  <summary>Details</summary>
Motivation: 现有数据修复方法直接处理低质量原始数据并直接使用不确定的模型输出，可能引入不可靠的修正。

Method: LasRepair使用LLM选择全局修复上下文指导SLM修复；LasRepair+将修复视为EM过程，交替更新正确器和上下文；LasRepair++利用列校准置信度降低不可靠修复行的权重。

Result: 在真实数据集上，LasRepair++相比最强基线平均F1提升18.1%。

Conclusion: 理论分析和实验证明所提方法有效，尤其EM过程和置信度加权可提升修复质量。
...
### [110] [The Value of Adaptivity in LSM Bloom-Filter Tuning: A Log-Law and a Two-Clock Frontier](https://arxiv.org/abs/2606.18138)
*Madhulatha Mandarapu,Sandeep Kunkunuru*

Main category: cs.DB

TL;DR: LSM树中过滤器内存分配的自适应性与静态分配的价值边界分析。


<details>
  <summary>Details</summary>
Motivation: 现有LSM树过滤器内存分配自适应方法众多，但缺乏何时值得付出自适应开销的量化理论。

Method: 通过理论推导对数律、鲁棒性律和自适应价值边界，并结合合成数据、Twitter缓存和RocksDB实验验证。

Result: 仅在压缩时重新分配可捕获96-99%的跟踪收益；假阳性原语在4-8位/键内成立。

Conclusion: 表征了自适应调优的适用条件，提出三区域策略，指出更多偏度降低精细跟踪的必要性。
...
### [111] [Group Commit Self-Clocks: Why Tuning Is Unnecessary Above a Device-Set Load Threshold](https://arxiv.org/abs/2606.18187)
*Madhulatha Mandarapu,Sandeep Kunkunuru*

Main category: cs.DB

TL;DR: 闭环OLTP中，贪婪流水线策略（设备空闲立即刷新）无需调优即可接近最优，而经典平方根规则仅在低负载开环下有效。


<details>
  <summary>Details</summary>
Motivation: 经典群提交调优（如定时器）基于开环假设，但实际闭环系统中策略本身影响到达率，需重新评估其有效性。

Method: 将闭环OLTP建模为闭合排队网络，分析贪婪流水线策略；推导负载阈值λ*，并用AWS存储类实测fsync分布，在PostgreSQL上验证。

Result: 贪婪流水线策略在任何负载下与最优调优定时器相差约0.1%；平方根规则在负载高于λ*时退化为贪婪策略，调优无效。

Conclusion: 闭环环境下无需复杂调优，参数自由的贪婪策略即是最佳选择；经典理论仅在开环低负载下有效。
...
### [112] [A benchmark suite of intracellular Boolean model variants and multiscale simulations for computational biology](https://arxiv.org/abs/2606.18215)
*Marco Masera,Riccardo Smeriglio,Roberta Bardini,Alessandro Savino,Stefano Di Carlo*

Main category: cs.DB

TL;DR: PhysiBench是一个开放资源，包含612个布尔调控网络变体和120,000个多尺度随机模拟，用于系统生物学计算方法开发与评估。


<details>
  <summary>Details</summary>
Motivation: 系统生物学计算方法的开发需要标准化的基准测试资源。

Method: 从七个已发表的布尔网络通过突变构建变体模型，并通过在线行为过滤和离线敏感性评估生成；模拟数据集来自60个选定的模型，在不同刺激协议下生成。

Result: 生成了612个网络变体和120,000个模拟轨迹，并进行了文件完整性、结构多样性和行为异质性验证。

Conclusion: PhysiBench支持直接模拟、代理建模、数据驱动推理、基于模拟的优化和比较基准测试。
...
### [113] [From Embedded Properties to Trait Nodes: A Design Method for Identifying Reusable Metadata in Property Graph Schemas](https://arxiv.org/abs/2606.18297)
*Yahya Sa'd,Renzo Angles,Vojtech Merunka,Roberto Garcia,Karel Klima,Pavel Beranek*

Main category: cs.DB

TL;DR: 针对属性图模式中描述性属性是否应外部化为可重用元数据的问题，提出了一种基于五项准则的元数据候选识别方法，并通过规则决策流程进行分类。


<details>
  <summary>Details</summary>
Motivation: 属性图模式中经常出现跨异构节点和边的描述性属性，但设计师缺乏明确方法决定这些属性应保持嵌入还是作为可重用的元数据结构。

Method: 提出基于五项准则（跨元素出现、概念独立性、无损外部化、重用潜力、治理相关性）的方法，通过规则决策流程将属性分类为特征候选、嵌入属性和边界案例。

Result: 结果显示单独的重现性不是外部化的充分基础，元数据候选识别需要超越频率的语义解释。

Conclusion: 主要贡献是方法论方面的：为决定何时将描述性属性建模为属性图中的可重用元数据提供了更明确和系统的基础。
...
### [114] [Evaluating Learned Spatial Indexes](https://arxiv.org/abs/2606.19034)
*Sachith Pai,Jun Yang,Michael Mathioudakis*

Main category: cs.DB

TL;DR: 通过对学习型索引在空间范围查询中的实验研究，分析了块大小、数据倾斜、查询倾斜等七个关键问题，并提出了基于工作负载的索引选择决策树，在真实数据上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有学习型索引在空间范围查询中的性能权衡理解不足，尤其是模型精度、数据与查询倾斜、布局粒度、选择性和存储行为的影响。

Method: 使用统一存储后端和标准化查询管道，控制数据与查询倾斜的变化，对代表性索引进行实验分析。

Result: 揭示了细化与扫描成本的权衡、块大小对延迟的影响、选择性与布局有效性的相互作用，并据此构建了工作负载驱动的索引选择决策树。

Conclusion: 决策树建议在真实OpenStreetMap点集和合成查询上验证，表现出最小决策遗憾，通常获得接近最优的查询性能。
...
### [115] [A Polyak-Ruppert Central Limit Theorem for SA-Adam with Momentum and Non-Convergent Adaptive Preconditioning](https://arxiv.org/abs/2606.17364)
*Sunyoung An,Xiaoming Huo*

Main category: math.ST

TL;DR: 在Polyak-Ruppert平均下，自适应优化器（如Adam/AdamW）的迭代平均保留了经典的中心极限定理，其协方差矩阵与普通随机梯度下降相同，即自适应效应渐近消失。


<details>
  <summary>Details</summary>
Motivation: 探索带有动量和非收敛预条件子的自适应优化器在Polyak-Ruppert平均下是否仍保持经典的中心极限定理，以及其协方差结构。

Method: 将增广状态（迭代变量和动量缓冲区）视为时变线性随机逼近，证明正漂移稳定性、非自治Polyak-Ruppert中心极限定理和投影恒等式。

Result: 迭代边际协方差恰好等于普通随机梯度下降的夹层协方差H^{-1}SH^{-1}，表明自适应性渐近不可见；该结论适用于子线性衰减动量的SA-Adam，而非固定β的Adam。

Conclusion: 自适应优化器的迭代平均在渐近意义上具有与普通SGD相同的统计效率，耦合L2权重衰减可扩展至正则化问题。
...
### [116] [Inference Optimal Long Run Variance Estimation with Lugsail Kernels](https://arxiv.org/abs/2606.17369)
*Rebecca P. Kurtz-Garcia,James M. Flegal*

Main category: math.ST

TL;DR: 针对lugsail谱方差估计器，提出了基于固定平滑极限分布的推断最优带宽规则，显著改进偏差校正，提供稳健推断。


<details>
  <summary>Details</summary>
Motivation: 现有谱方差估计器在正相关下负偏差大；lugsail估计器零渐近偏差，但缺乏带宽选择指南。

Method: 提出基于非标准固定平滑极限分布的推断最优带宽规则。

Result: 显著改进偏差校正，考虑变异性，为稳健推断提供优化估计器。

Conclusion: 理论发现通过仿真研究得到支持。
...
### [117] [Non-asymptotic Tail Bounds for the Kostlan--Shub--Smale Field: Tensor PCA and Spherical $k$-Spin Complexity](https://arxiv.org/abs/2606.17665)
*Jean-Marc Aza\"is (IMT),Federico Dalmao (UDELAR),Yohann De Castro (ICJ,ECL,PSPM,IUF)*

Main category: math.ST

TL;DR: 建立KSS随机场的显式非渐近尾界，并应用于张量PCA和球面k-自旋模型，得到有限维度误差界和景观复杂度界。


<details>
  <summary>Details</summary>
Motivation: 需要非渐近统计界限和分析高维随机场，以解决张量PCA和自旋玻璃模型中的问题。

Method: 使用Tube方法和秩约简将估计误差与KSS场关联，通过Kac-Rice公式转化为高斯积分，再利用Mehta-Fyodorov表示和Ben Arous-Dembo-Guionnet大偏差得到四类显式尾界。

Result: 张量PCA的有限维度误差界恢复渐近最优速率sqrt(d log k)，显式依赖秩R和相干性kappa；球面k-自旋模型的双侧非渐近界恢复Auffinger-Ben Arous-Cerny复杂度函数。

Conclusion: 所提出的尾界层级提供了通用工具，显式常数使得结果可直接应用于问题，并推广了先前的渐近结果。
...
### [118] [Conformal Prediction Intervals with Tail-Specific Guarantees](https://arxiv.org/abs/2606.18199)
*Simone Cuonzo,Nina Deliu*

Main category: math.ST

TL;DR: 通过分别构造上下单侧共形预测区间并取交集，得到具有尾部特定校准的双侧预测区间，在偏斜数据中方向校准优于经典方法。


<details>
  <summary>Details</summary>
Motivation: 经典共形预测区间仅保证全局边际覆盖，缺乏对上下尾的单独校准，在偏斜分布中可能导致不良的方向性偏差。

Method: 基于分割共形预测，先构造边际有效的上下单侧共形区间，再通过交集得到双侧区间。

Result: 在可交换数据下具有有限样本覆盖保证，非可交换下渐近保证；模拟显示在偏斜数据中方向校准显著改进。

Conclusion: 该框架实现了尾部特定校准，在金融等关注左尾的应用中具有优势。
...
### [119] [Inferential Models: The Power of Auxiliary Variables for Reasoning with Scientific Uncertainty](https://arxiv.org/abs/2606.18378)
*Chuanhai Liu*

Main category: math.ST

TL;DR: 推断模型（IMs）提供了一种无先验的概率推理框架，通过预测随机集产生校准的不确定性评估，并与其他推断方法进行了比较。


<details>
  <summary>Details</summary>
Motivation: 科学推理需要既情境特定又频率校准的不确定性评估，现有方法存在局限。

Method: IMs将辅助变量视为不确定性来源，用校准的预测随机集预测未观测的辅助值，然后传递合理性陈述。

Result: IMs产生有效的不确定性评估，澄清了与Fisher、Neyman、Dempster-Shafer等方法的联系，并支持校准的不精确性。

Conclusion: IMs可能发展出微分几何理论，为解决似然原理相关的基础问题提供途径。
...
### [120] [A spectral based coefficient of determination for the fit of an MA(q) model](https://arxiv.org/abs/2606.18445)
*Holger Dette,Sebastian K\"uhnert*

Main category: math.ST

TL;DR: 开发了一种基于谱的确定系数，用于衡量MA(q)模型对平稳过程谱密度的表示程度，并基于周期图估计提出渐近正态性、假设检验和阶数选择方法。


<details>
  <summary>Details</summary>
Motivation: 需要量化MA(q)模型近似谱密度的好坏，并确定达到指定近似质量的最小阶数q。

Method: 定义基于谱的确定系数，利用周期图估计器，推导渐近正态性，构建假设检验和阶数选择程序。

Result: 建立了统计量的渐近分布，提供了检验MA(q)假设和选择最小q的方法。

Conclusion: 提出的方法有效度量了MA(q)模型的拟合优度，并给出了实用的阶数选择准则。
...
### [123] [Optimal score function estimation via derivatives constraints](https://arxiv.org/abs/2606.19084)
*Thomas Bonis,Thanh Mai Pham Ngoc,Viet Chi Tran*

Main category: math.ST

TL;DR: 本文研究了通过经验风险最小化进行得分函数估计，并证明将假设空间约束在Sobolev球内可防止过拟合并达到极小极大估计速率，在得分生成模型中也得到类似结论。


<details>
  <summary>Details</summary>
Motivation: 探索得分函数估计中如何避免过拟合并获得最优估计速率，特别是在生成模型中的应用。

Method: 采用经验风险最小化，并将假设空间限制在Sobolev球内。

Result: 在平坦环面上的概率测度得分估计中，得到了极小极大估计速率；在得分生成模型中，基于一个假设同样获得了极小极大速率。

Conclusion: 约束假设空间到Sobolev球是一种有效的正则化策略，可用于得分函数估计并保证最优统计效率。
...
### [124] [Patnaik-Pearson intrinsic dimension for internal representations of neural networks](https://arxiv.org/abs/2606.19268)
*Tom Hadfield*

Main category: math.ST

TL;DR: 提出Patnaik-Pearson维数作为数据流形内在维度的新度量，应用于神经网络内部表示，特别是变换器。结合HTSR、SETOL和TwoNN估计器，证明其性质，并关联到Pareto分布权重矩阵的尾指数临界值。在BERT-base和DeepSeek-R1-Distill-Qwen-1模型上分析词嵌入初始维度和跨层演化。


<details>
  <summary>Details</summary>
Motivation: 现有内在维度估计方法（如TwoNN）在神经网络表示中的应用有限，需要一种更稳定且与谱分析相兼容的度量。

Method: 基于HTSR/SETOL理论和TwoNN估计器，定义Patnaik-Pearson维数；证明其数学性质；将权重矩阵经验谱密度拟合为Pareto分布，关联尾指数；通过理论和数值技术研究神经网络变换下的行为。

Result: Patnaik-Pearson维数与HTSR/SETOL分析的尾指数临界值一致；在BERT-base和DeepSeek模型上展示了词嵌入流形维数随层的变化。

Conclusion: Patnaik-Pearson维数是分析神经网络表示流形内在维度的有效工具，可揭示深层结构变化。
...
### [125] [Correct When Paired, Wrong When Split: Decoupling and Editing Modality-Specific Neurons in MLLMs](https://arxiv.org/abs/2606.17057)
*Tingchao Fu,Wenkai Wang,Fanxiao Li,Huadong Zhang,Jinhong Zhang,Dayang Li,Yunyun Dong,Renyang Liu,Wei Zhou*

Main category: cs.LG

TL;DR: 提出DECODE方法，通过显式解耦和定位模态特定的神经元组，解决多模态大语言模型中知识编辑的解耦失败问题，使得知识更新在不同模态触发下均有效。


<details>
  <summary>Details</summary>
Motivation: 现有知识编辑范式在多模态输入下可更新实体知识，但在分割为单模态输入时知识会回退到编辑前的旧事实，这种编辑解耦失败问题尚未被充分研究。

Method: DECODE通过显式解耦并定位模态特定的神经元组，针对不同模态（文本、图像）分别进行知识编辑，确保更新信息有效传播到对应的单模态回路。

Result: 实验表明，DECODE在不同模态触发下均能实现有效的知识更新，显著缓解了编辑解耦失败问题。

Conclusion: DECODE为多模态大语言模型的知识编辑提供了一种鲁棒的解决方案，确保知识更新在模态转换时保持一致。
...
### [126] [Diagnosing and Repairing Shape-Prior Shortcuts in Long-Range Single-Shot Fringe Projection Profilometry](https://arxiv.org/abs/2606.17093)
*Adam Haroon,Anush Lakshman,Cody Fleming,Beiwen Li*

Main category: cs.LG

TL;DR: 针对远距离单拍条纹投影轮廓术，基线依赖物体边界形状先验而非条纹相位解码。提出PhiCalNet（输出包裹相位并应用固定标定层）将MAE降低3.3倍至4.46 mm。共形UQ验证了诊断：拒绝5%像素后RMSE降低64%。


<details>
  <summary>Details</summary>
Motivation: 远距离（>1 m）单拍FPP未解决：强度衰减降低信噪比，单拍缺乏条纹阶数信息，且未被机理研究。

Method: 使用机械可解释性（MI）和共形不确定性量化（UQ）诊断基线失败点，发现其依赖形状先验。修复：PhiCalNet输出包裹相位，通过固定可微标定层映射深度，从架构上消除形状先验空间。

Result: 最佳UNet基线MAE 14.54 mm；PhiCalNet MAE 4.46 mm（降低3.3倍）；剩余误差由0.103%像素（±π包裹不连续处）承担；UQ拒绝5%像素后PhiCalNet RMSE降低64%（基线仅3.5%）。

Conclusion: MI和UQ收敛于同一失败点；架构修复有效，物理信息损失无效，证明架构设计是关键因素。
...
### [127] [Informative Missingness to Generate Irregular Clinical Time Series](https://arxiv.org/abs/2606.17106)
...
TL;DR: 提出一种扩散模型，联合模拟临床时间序列中的实验室值和观测模式，处理非随机缺失数据。


<details>
  <summary>Details</summary>
Motivation: 电子健康记录中的实验室测试收集不规则，缺失值具有信息性，反映临床决策和患者生理，需直接建模而非预处理。

Method: 扩展TimeDiff框架，使用互补扩散目标学习连续实验室值和离散缺失模式；数据对齐为4小时间隔、7天窗口，并应用标准化。

Result: 生成的数据在单个实验室分布和联合缺失嵌入上密切匹配真实患者轨迹，表明模型能捕捉临床有意义依赖。

Conclusion: 可作为临床基础模型的初始组件，通过生成保留关键生理-缺失关系的合成先验，促进后续Prior-Data Fitted Networks训练。
...
### [128] [Models Take Notes at Prefill: KV Cache Can Be Editable and Composable](https://arxiv.org/abs/2606.17107)
*Bojie Li*

Main category: cs.LG

TL;DR: 发现前缀缓存中模型将结论写入下游笔记，字段本身影响很小，从而支持编辑和组合笔记，实现高效缓存更新。


<details>
  <summary>Details</summary>
Motivation: 前缀缓存中字段变化导致整个下游缓存失效，因为模型已将结论写入下游笔记，仅重算字段无效。

Method: 通过因果分析建立字段与决策的关系，提出笔记模型：编辑时追加勘误，组合时重新定位预编译技能。

Result: 编辑仅需约1%计算量恢复决策；组合时间降至O(L)；延迟降低14.9倍；p90 TTFT降低53-398倍，缓存命中率98.5%。

Conclusion: 该方法适用于多种注意力缓存，通过小适配器扩展，保持兼容生产前缀缓存。
...
### [129] [The Critical Role of Model Selection in Causal Inference: A Comparative Analysis of Classification Models within the InferBERT Framework for Pharmacovigilance](https://arxiv.org/abs/2606.17113)
...
### [130] [Probing, Fusion, and Trustworthiness: A Systematic Evaluation of Foundation Model Representations for Multimodal Cancer Analysis](https://arxiv.org/abs/2606.17115)
*Jingyu Hu,Giuseppe Tripodi,Reed Naidoo,Sarah F. McGough,Tapabrata Chakraborti*

Main category: cs.LG

TL;DR: 系统评估了基础模型在病理学任务分布偏移下的表现，发现其具有竞争力，多模态融合在单模态信号不占优时有帮助，不确定性推理对临床支持有价值。


<details>
  <summary>Details</summary>
Motivation: 基础模型在医疗数据上表现出色，但它们在分布偏移下的泛化能力尚未充分探索。

Method: 在真实商业队列（IH-BC和IH-NSCLC）上评估五种基础模型的全切片图像和转录组特征，首先基准测试单模态探测性能，然后比较三种图像-组学融合策略，最后通过共形预测评估可信度。

Result: 基础模型表示在分布外数据上取得竞争性能；多模态融合仅在单模态信号不主导时带来额外收益；共形预测显示，在点预测失败的情况下，真实诊断大多可被预测集覆盖。

Conclusion: 不确定性感知推理对临床支持具有重要价值。
...
TL;DR: 提出MODE框架，通过模态分解的专家级混合精度量化，有效减少MoE多模态大模型在低比特量化下的性能损失。


<details>
  <summary>Details</summary>
Motivation: 现有专家级混合精度量化方法在MoE-MLLMs上性能下降严重，因为视觉令牌数量优势掩盖了文本关键专家，且冗余视觉令牌扭曲了专家重要性估计。

Method: 按模态分解专家选择频率，过滤冗余视觉令牌得到去噪视觉频率，并评估每模态量化敏感性，通过整数线性规划分配位宽。

Result: 在W3A16设置下，平均性能损失限制在2.9%以内，在2比特极端设置下增益更大。

Conclusion: MODE特别适合MoE-MLLMs，能有效降低量化带来的性能损失。
...
### [132] [Noise-Driven Escape from Metastable Phases explains Grokking in Deep Neural Networks](https://arxiv.org/abs/2606.17120)
*Ibrahim Talha Ersoy,Karoline Wiesner*

Main category: cs.LG

TL;DR: 深度神经网络中Grokking现象源于L2正则化导致的一阶相变滞后，模型被困在亚稳态，需SGD噪声越过能垒才能泛化。


<details>
  <summary>Details</summary>
Motivation: 理解Grokking（延迟的突然泛化）的起源，以及L2正则化对神经网络学习的相变影响。

Method: 在线性DNN中通过调控L2正则化强度制造亚稳态陷阱，观察SGD噪声驱动的逃逸时间，并用稀疏子采样重现Grokking曲线。

Result: 逃逸时间符合阿伦尼乌斯标度，亚稳态数量等于数据协方差奇异值数量，延迟泛化可通过控制亚稳态实现。

Conclusion: Grokking是L2相变滞后的表现，机制可能也存在于非线性DNN中，为设计高效学习方案提供思路。
...
### [133] [Towards Fast GNN Surrogates for CO2 Migration in Complex Geological Formations](https://arxiv.org/abs/2606.17180)
...
TL;DR: 提出一种数据驱动的图神经网络代理模型，用于地质储层中CO2羽流迁移预测，在SPE11A基准上表现出中等累积误差的竞争性预测。


<details>
  <summary>Details</summary>
Motivation: 传统数值模拟复杂地质条件下多相流计算成本高，需要高效代理模型。

Method: 构建图结构，节点代表网格单元，边编码基于传导率的相互作用；采用各向异性消息传递机制，通过几何条件边嵌入偏置消息聚合方向；潜空间中的自回归残差公式，多步监督训练。

Result: 在SPE11A基准上，气体饱和度与液相密度预测具有竞争力，长时间预测累积误差较小。

Conclusion: 提出的图神经网络代理模型能有效捕捉多相流关键物理行为，适用于CO2地质封存监测。
...
### [134] [Verified Detection and Prevention of Concurrency Anomalies in Multi-Agent Large Language Model Systems](https://arxiv.org/abs/2606.17182)
*Sajjad Khan*

Main category: cs.LG

TL;DR: 本文针对多智能体LLM系统的状态共享并发问题，形式化了四种异常模式，并提出了一个通过机械验证的一致性层次结构（L0到L4），在Rust运行时中实现了部分层次，并实际排查了现有系统中的异常。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM系统通过存储共享状态，但现有运行时缺乏对并发异常的形式化理解和保证，导致难以确保系统正确性。

Method: 使用TLA+形式化四种并发异常（陈旧生成、幻影工具、因果级联、工具效应重排序），通过Verus验证了检测器的完备性和正确性，基于此构建了一个机械验证的一致性层次结构，并在Rust运行时中实现了部分层次。

Result: 实现了从L0到L4的严格分离；三个Rust运行时分别实现了L0/L1；L2至L4通过执行模式验证；实际发现了ByteDance的deer-flow中的丢失更新和LangGraph的ToolNode中的工具效应重排序，并提供了修复。

Conclusion: 该工作提供了首个针对多智能体LLM运行时的一致性层次结构验证框架，有助于系统性地设计和排查并发问题。
...
### [135] [Finsler Geometry, Graph Neural Networks, and You](https://arxiv.org/abs/2606.17185)
*T. Mitchell Roddenberry,Richard G. Baraniuk*

Main category: cs.LG

TL;DR: 提出基于Finsler拉普拉斯算子的图神经网络，可处理各向异性扩散，并证明收敛性。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络基于图拉普拉斯近似各向同性算子，无法处理各向异性问题。

Method: 从流形点云估计Finsler拉普拉斯算子，证明离散估计收敛到连续算子，并将其表示为图神经网络层。

Result: Finsler图神经网络能恢复非线性扩散方程背后的几何结构。

Conclusion: 该网络族通过约束表达Finsler几何，为各向异性问题提供了新框架。
...
### [136] [Constrained Diffusion Models with Primal-Dual Inference](https://arxiv.org/abs/2606.17192)
*Samar Hadou,Yigit Berkay Uslu,Alejandro Ribeiro*

Main category: cs.LG

TL;DR: 提出原始-对偶推理的约束扩散模型，用于从熵正则化优化问题的最优分布中采样，其中约束为平均约束。


<details>
  <summary>Details</summary>
Motivation: 现有方法先估计对偶变量再冻结采样，本工作联合推断原始分布和对偶变量，以更好地处理平均约束下的采样问题。

Method: 在拉格朗日对偶域中形式化，反向扩散每一步用当前对偶变量的得分场去噪，然后通过对偶上升更新对偶变量。训练单一对偶条件得分网络。

Result: 证明了推理轨迹中对偶变量的时间平均收敛到对偶最优邻域，并界定了残差对偶失配的影响。在高斯混合、无线资源分配和投资组合管理上评估。

Conclusion: PDI有效处理平均约束下的约束采样，在多个领域有实际应用价值。
...
### [137] [PowerOPD: Stabilizing On-Policy Distillation with Bounded Power Transformation](https://arxiv.org/abs/2606.17199)
*Anhao Zhao,Junlong Tong,Yingqi Fan,Ping Nie,Wenjie Li,Xiaoyu Shen*

Main category: cs.LG

TL;DR: 针对on-policy蒸馏中log-ratio奖励无界导致训练不稳定问题，提出PowerOPD，通过Box-Cox变换生成有界奖励，显著提升性能并降低资源消耗。


<details>
  <summary>Details</summary>
Motivation: 标准on-policy蒸馏使用学生采样token估计反向KL散度，但存在样本效率低、生成不稳定和性能差距等问题，根源在于log-ratio奖励无界导致高方差梯度。

Method: 提出PowerOPD，基于Box-Cox幂变换构造一族有界且符号一致的奖励函数，参数alpha>0，log-ratio是其退化情形。

Result: 在六个数学推理基准和四个Qwen3师生对上，PowerOPD在Avg@8/Pass@8上比vanilla OPD提升+6.37/+5.71，比post-hoc稳定化提升+3.01/+3.54，比全词汇OPD提升+2.59/+8.90，同时减少59.2%时间和23.1%内存。

Conclusion: PowerOPD有效解决了log-ratio奖励带来的训练病态，更大的alpha通常带来更高准确率、更短响应和更稳定的梯度，是一种高效且可扩展的蒸馏方法。
...
### [138] [Sum-of-Squares Degree Barriers for the Reweighted-Hinge Method in Robust Halfspace Learning: A Christoffel-Function Characterization](https://arxiv.org/abs/2606.17215)
*Xiaoyu Li*

Main category: cs.LG

TL;DR: 通过低次矩去除异常值的证书存在盲点，其大小由清洁边际的Christoffel函数决定。本文将此反转作为重加权铰链方法的核心，用于鲁棒学习γ间隔半空间，并得出证书度数、间隔与可隐藏corruption质量之间的精确关系。


<details>
  <summary>Details</summary>
Motivation: 现有基于低次矩的证书方法无法检测对抗者隐藏的corruption，因为盲点恰好位于清洁数据看似典型的区域。需要理解盲点的大小并改进鲁棒学习算法。

Method: 将Christoffel函数作为度-2t证书最大可隐藏corruption质量的度量，并基于此构建重加权铰链方法。通过Sum-of-Squares度分析，推导证书度数、间隔与误差之间的权衡。

Result: 证明了间隔-度权衡：用度-2t证书达到误差ε需要度数Ω(log(1/ε))或间隔Ω(√(log(1/ε))/√d)。度2证书只能处理η^{1/2}的corruption，而度4可突破此障碍。提出度-2t算法，错误率边界为η^{1-1/2t}，且常数不可改进。

Conclusion: 证书方法的性能受限于度数，提高度数可改善鲁棒性。所提算法紧于度2障碍，且间隔的对数尺度是必要的。
...
### [139] [Rift: A Conflict Signature for Deception in Language Models](https://arxiv.org/abs/2606.17229)
*Petr Nyoma*

Main category: cs.LG

TL;DR: 发现模型在知道真相时撒谎会留下内部冲突签名（残余秩升高），可100%准确检测谎言，且跨模型、语言、架构泛化。


<details>
  <summary>Details</summary>
Motivation: ELK无法仅通过行为区分知道真相的谎言与诚实错误，需寻找内部神经签名。

Method: 对比休眠代理（知道真相，触发时撒谎）与天真说谎者（直接输出相同错误答案），分析前向传递残余秩差异。

Result: 谎言残余秩比天真说谎者高2.1-2.3倍；跨多个模型（GPT-2, Qwen, Phi-3）实现无标签100%准确率；签名可迁移至未见模型、语言及主动隐藏场景。
...
### [140] [Uncertainty Quantification of Engineering Structures by Polynomial Chaos Expansion and Multivariate Active Learning](https://arxiv.org/abs/2606.17233)
*Qitian Lu,Jafar Jafari-Asl,Panagiotis Spyridis,Lukas Novak*

Main category: cs.LG

TL;DR: 提出一种针对多输出模型的自适应序贯采样方法，构建多项式混沌展开代理模型，兼顾探索与开发，提高代理精度和稳定性。


<details>
  <summary>Details</summary>
Motivation: 单个实验设计难以同时充分表征多个不同敏感性的输出，分别采样增加成本且忽略输出间相关性。

Method: 基于输出方差的局部贡献，从候选池中序贯选择新样本，平衡输入空间的探索和所有输出聚合方差信息的开发。

Result: 数值算例表明，该方法相比非序贯拉丁超立方采样，提高了代理精度和稳定性，更可靠地估计二阶统计量。

Conclusion: 自适应序贯采样方法能有效提升多输出代理模型的整体性能。
...
### [141] [Rethinking Groups in Critic-Free RLVR](https://arxiv.org/abs/2606.17250)
*Yihong Wu,Liheng Ma,Lingfeng Xiao,Muzhi Li,Xinyu Wang,Yingxue Zhang,Jian-Yun Nie*

Main category: cs.LG

TL;DR: 提出负令牌过滤策略，实现单次 rollout 训练，替代低效的组方法。


<details>
  <summary>Details</summary>
Motivation: 现有无评论家 RL 方法需要生成一组 rollout 来估计基线，但存在数据效率低、同步障碍等问题。

Method: 重新审视“组”的作用，发现其核心是防止对负样本的错误惩罚，据此提出负令牌过滤策略，支持单 rollout 训练。

Result: 在推理任务上性能相当，在智能体任务上优于基于组的 RL 技术。

Conclusion: 负令牌过滤是一种简单有效的策略，可稳定单 rollout 训练并提升性能。
...
TL;DR: 提出ProCUA-SFT数据集，通过合成轨迹显著提升计算机使用代理性能，OSWorld成功率从26.3%提升至45.0%。


<details>
  <summary>Details</summary>
Motivation: 现有数据集AgentNet导致负迁移，微调UI-TARS 7B后OSWorld成功率从26.3%降至8-10%。

Method: 全自动流水线：在真实桌面上合成接地任务（种子包含电子表格、演示文稿等），通过二元前置条件检查验证可行性；使用单个VLM（Kimi-K2.5）作为生成器、判断器和执行器；将轨迹扩展为步骤前缀样本以复现推理上下文。

Result: 微调UI-TARS 7B一个epoch后，OSWorld成功率达45.0%，比基础模型高18.7个百分点，比AgentNet-trained模型高35%以上。

Conclusion: ProCUA-SFT有效克服负迁移，大幅提升计算机使用代理能力，其子集已被用于Nemotron 3 Nano Omni模型训练。
...
### [143] [Decision-Driven Geosteering Under Uncertainty: A Unified Framework for Sequential Decision Optimization](https://arxiv.org/abs/2606.17331)
*Hibat Errahmen Djecta,Sergey Alyaev,Kristian Fossum,Reidar B. Bratvold,Ressi Bonti Muhammad,Apoorv Srivastava*

Main category: cs.LG

TL;DR: 提出一种结合粒子滤波与强化学习的不确定性感知地质导向框架，用于钻井轨迹决策。


<details>
  <summary>Details</summary>
Motivation: 地质导向需要在未知地质构造中实时调整井轨迹，现有方法未充分利用前向不确定性。

Method: 使用粒子滤波表示地质不确定性，并耦合三种决策策略：近似动态规划、深度Q学习及双深度强化学习（采用dueling网络架构）。

Result: 在工业模拟器中测试，使用相同地质实现和奖励定义，并采用稳定性指标评估策略行为。

Conclusion: 该框架能显式处理不确定性，提供稳定的决策行为，而不仅关注最终轨迹性能。
...
### [144] [Counterfactual Optimization of Baseball Pitch Sequences and Estimation of Its Impact on Season-Level Statistics](https://arxiv.org/abs/2606.17345)
*Ryota Takamido,Hiroki Nakamoto*

Main category: cs.LG

TL;DR: 使用Transformer模型和反事实分析，研究了棒球投球序列对赛季表现的影响，发现优化最终投球和前续投球可以显著提升赛季统计数据。


<details>
  <summary>Details</summary>
Motivation: 先前研究仅关注单次打席的最终投球，缺乏对前续投球和长期赛季影响的深入分析。

Method: 基于MLB Statcast数据训练Transformer模型预测投球结果（击打或挥空），通过反事实替换最终投球或前续投球的类型和位置，选择最小化击打概率的方案，并利用回归模型估计对赛季统计的影响。

Result: 优化最终投球和前续投球可提升赛季表现，例如K/9提高超过1.0，并发现了速度带内有效位置、投球指令的重要性以及中速投球扩大选择范围等实用见解。
...
### [145] [MM++: Unsupervised Scale-Invariant Multilayer OOD Detection via Top-K Gated Feature Fusion](https://arxiv.org/abs/2606.17352)
*Rahim Hossain,Md Tawheedul Islam Bhuian,Md Farhan Shadiq,Kyoung-Don Kang*

Main category: cs.LG

TL;DR: 提出MM++框架，一种完全无监督、事后且尺度不变的分布外检测方法，通过构建联合特征空间和正则化协方差矩阵实现鲁棒性能。


<details>
  <summary>Details</summary>
Motivation: 解决尺度不变性与层次表达性之间的权衡，同时避免依赖辅助数据或模型修改。

Method: 先通过熵密度下降识别判别性中间层，再将其与终端表示融合，并用Ledoit-Wolf正则化绑定协方差矩阵稳定联合空间。

Result: 在近和远分布外检测中，跨多种架构均表现鲁棒，无需额外数据、微调或架构更改。

Conclusion: MM++是一种有效且实用的分布外检测框架。
...
### [146] [Performance-Driven Environment Abstraction with Multi-Timescale Learning](https://arxiv.org/abs/2606.17377)
*Yue Guan,Dipankar Maity,Panagiotis Tsiotras*

Main category: cs.LG

TL;DR: 提出一种性能驱动的环境抽象方法，通过聚合状态空间并强制共享动作分布来优化决策质量，并开发了多时间尺度强化学习框架，在树形结构抽象中联合调整策略和抽象，实现了显著的状态压缩和样本效率提升。


<details>
  <summary>Details</summary>
Motivation: 在大型马尔可夫决策过程中，需要有效的环境抽象来提升决策质量，但现有方法多保持几何或拓扑结构，未能直接优化决策性能。

Method: 将抽象建模为状态空间聚合和每个聚合态内共享动作分布的受控近似，基于固定分区的性能保证分离价值函数近似误差与动作共享损失，并开发多时间尺度强化学习框架，根据Q值差异精化或粗化状态空间区域，平衡性能与抽象规模。

Result: 实验表明，与actor-critic基线相比，该方法实现了显著的状态压缩，提高了样本效率，并加快了重规划速度。

Conclusion: 所提性能驱动的环境抽象方法能有效压缩状态空间、提升样本效率，为大规模MDP决策提供了一种实用性强的解决方案。
...
### [147] [The Discrete-Log Clock: How a Transformer Learns Modular Multiplication](https://arxiv.org/abs/2606.17399)
*Huu Danh Nguyen (Stanford University)*

Main category: cs.LG

TL;DR: 小Transformer在模乘运算中，之前认为的密集傅里叶谱是分析基错误的假象；使用乘法特征变换后，嵌入谱变得高度稀疏，仅需4个关键频率，且MLP神经元几乎全部调谐到单一频率，揭示了算法相当于离散对数时钟。


<details>
  <summary>Details</summary>
Motivation: 先前工作发现小Transformer在模乘运算中学习到的嵌入具有密集傅里叶谱，需要所有频率，这与模加法的稀疏关键频率形成对比，原因不明。

Method: 采用乘法特征变换（基于乘法群不可约表示）代替标准的加法DFT，对训练在模113乘法上的grokked Transformer进行分析。

Result: 嵌入谱变得高度稀疏（基尼系数0.58 vs 加法基0.07），仅4个关键频率携带主要能量；96.9%的MLP神经元调谐到单一乘法频率；按离散对数重排后激活热图呈二维周期结构。

Conclusion: Transformer将模乘归约为离散对数空间中的加法，实现了与Nanda等人加法“时钟算法”类似的“离散对数时钟”算法；匹配分析基与任务的代数结构能揭示标准工具视为噪声的可解释结构。
...
TL;DR: 使用条件自回归序列建模（transformer+VAE）进行平面机构路径综合，在百万级机构语料库上通过模拟评估和几何误差选择最优机构，取得良好效果。


<details>
  <summary>Details</summary>
Motivation: 平面路径综合需要机构耦合曲线匹配预定轨迹，但曲线到连杆的映射是“一对多”的，涉及四、六、八杆拓扑，需要有效方法从多种拓扑中生成准确机构。

Method: 将合成问题建模为条件自回归序列建模：关节坐标均匀量化为token，由仅解码器transformer生成，目标曲线使用VAE潜在变量和显式机构类型token；训练使用交叉熵和高斯平滑分箱辅助损失；推理时使用有界潜在噪声调度，在每个噪声级别解码所有机构类型，保留几何误差最小的前五候选。

Result: 在保留测试上，平均Chamfer距离0.0132，平均动态时间规整0.153；基于潜在k近邻的基线方法（在VAE空间使用训练集邻居潜在变量）在相同解码器下达到匹配拓扑平均Chamfer距离0.0071和平均DTW0.117。

Conclusion: 该方法能够生成多样化准确的机构族，无需数据集查找，性能优于基线。
...
### [149] [Amortized Probabilistic Retrieval of Atmospheric CO2 from OCO-2 Spectra Using Deep Learning with Laplace Approximations and Normalizing Flows](https://arxiv.org/abs/2606.17413)
*Alejandro Calle-Saldarriaga,Felix Jimenez,Jack Grosskreuz,Jiazheng Wang,Jonathan Hobbs,Matthias Katzfuss*

Main category: cs.LG

TL;DR: 提出一种基于深度学习的框架，用于从OCO-2光谱数据中估算XCO2，实现快速推理、鲁棒性及改进的不确定性量化。


<details>
  <summary>Details</summary>
Motivation: 当前操作反演算法计算成本高且未适当量化不确定性。

Method: 使用多分支神经网络编码光谱波段，结合拉普拉斯近似和归一化流两种可扩展的不确定性量化方法，基于包含真实正向模型误差的高保真模拟数据集训练。

Result: 相比全物理求解器，该方法具有五个优势：推理速度提升多个数量级、对模型误差鲁棒、点估计精度更高、不确定性校准更好、能建模非高斯后验分布。

Conclusion: 基于模拟的深度学习是迈向下一代操作处理系统的可行途径。
...
### [150] [Memory-Efficient Meta-Reinforcement Learning for Adaptive Safety-Critical Control in Adversarial Spacecraft Proximity Operations](https://arxiv.org/abs/2606.17414)
*Alejandro Posadas-Nava,Richard Linares,Minduli Wijayatunga*

Main category: cs.LG

TL;DR: 本文研究了通过元强化学习调整输入约束控制屏障函数（ICCBF）中类K函数的性能，比较了三种循环网络架构和两种训练算法在航天器交会对接任务中的效果。


<details>
  <summary>Details</summary>
Motivation: 自主航天器交会对接需要既保证推力约束下的安全性又最小化燃料消耗的控制器。

Method: 使用元强化学习调整ICCBF的类K函数，比较了LSTM、GRU和Mamba三种循环网络架构以及PPO和SAC两种训练算法。

Result: Mamba与PPO组合在所有合作和非合作场景中，在任务完成率、安全性和燃料节省方面均优于其他架构。
...
### [151] [Generalization Guarantees for Multi-Input Neural Operator Learning in Sobolev Spaces](https://arxiv.org/abs/2606.17419)
*Yahong Yang,Zecheng Zhang,Wei Zhu,Wenjing Liao,Hao Liu*

Main category: cs.LG

TL;DR: 针对多输入神经算子，在Sobolev范数下建立了逼近与泛化误差估计，允许输入函数定义在不同域上且具有不同维度和Sobolev正则性。


<details>
  <summary>Details</summary>
Motivation: 现有算子学习主要针对单输入函数，缺乏多输入场景的理论保障，尤其是在不同输入空间维度、正则性差异下的误差分析。

Method: 推导了误差上界显式依赖于各输入空间的贡献，利用Sobolev范数进行度量，并分析了平衡情形下输入维度、正则性与Sobolev阶数的交互作用。

Result: 逼近与泛化速率由输入维度、正则性和Sobolev阶数的相互作用主导，模型复杂度依赖呈现log-log/log结构。

Conclusion: 为多输入算子学习（包括Sobolev训练）提供了通用理论框架，适用于偏微分方程及科学计算中的算子学习问题。
...
### [152] [MorphStrata: Layer-Specific Perturbations for Generating Morphence Students in Time-Series Moving Target Defense](https://arxiv.org/abs/2606.17435)
...
TL;DR: MorphStrata通过选择性逐层噪声注入生成异构学生模型，以微小计算开销提升时间序列预测的对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有对抗防御在鲁棒性与响应开销之间存在权衡，移动目标防御（MTD）因维护多个模型导致训练成本过高。

Method: 以Transformer为教师，随机扰动选定的架构块注入随机噪声，生成结构异构的学生模型集合。

Result: 在多个数据集和攻击下，MorphStrata保持可比较的对抗RMSE；在高熵周期性数据集上，FGSM和BIM攻击下RMSE分别降低24.11%和97.97%，训练时间增加不足1%，且学生间L2距离与防御效果正相关。

Conclusion: MorphStrata以边际成本实现有效的MTD防御，兼顾鲁棒性与效率。
...
### [153] [Toward Controllable Catalyst Inverse Design via Large-Scale Autoregressive Pretraining](https://arxiv.org/abs/2606.17445)
*Dong Hyeon Mok,Jonggeol Na,Seoin Back*

Main category: cs.LG

TL;DR: 提出基于GPT架构的条件催化剂生成模型，通过大规模预训练和属性条件控制，实现高效可控的催化剂逆向设计。


<details>
  <summary>Details</summary>
Motivation: 异相催化剂表面结构复杂，传统高通量筛选效率随搜索空间增大而下降，亟需能直接生成目标属性催化剂的生成模型。

Method: 基于GPT架构，引入数值嵌入层，在自回归框架中同时条件化类别和连续属性；先在1.33亿个催化剂结构上预训练，再用约46万个优化结构及其绑定能微调。

Result: 结构有效性98%，优化有效性95%，吸附类型和组成联合匹配率93%；绑定能条件匹配率约20%，相比基线分布提升4倍，筛选效率提升1.5-4倍。
...
### [154] [Credibility-Weighted Pricing of Autonomous Vehicle Liability Under Operational Design Domain Shift](https://arxiv.org/abs/2606.17451)
*Doyeon Jang*

Main category: cs.LG

TL;DR: 提出了一种分层贝叶斯可信度框架，用于解决自动驾驶系统费率制定中的稀疏数据、运营设计域变化和软件版本非平稳风险问题，并通过Waymo实际数据验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统部署导致费率制定面临数据稀疏、运营设计域变化和软件版本风险非平稳等挑战，现有方法难以处理。

Method: 构建分层贝叶斯可信度框架，通过学习ODD相似性核进行城市、软件版本和区域间的部分池化，Buhlmann-Straub作为极限情况。

Result: 在Waymo的648起碰撞和1.16亿英里数据上，城市聚合可信度权重为0.12-0.46，部分池化显著优于无池化，且当部署约12个城市时可检测到学习核的优势。

Conclusion: 该框架能有效应对自动驾驶费率制定中的关键挑战，部分池化优于无池化，且学习核在足够多城市时具有优势。
...
### [155] [Operator Boosting Produces Pareto-Efficient PDE Surrogates](https://arxiv.org/abs/2606.17460)
*Lennon J. Shikhman*

Main category: cs.LG

TL;DR: 提出Operator Boosting框架，通过逐阶段残差学习构建紧凑的神经算子代理，相比全尺寸模型显著减少参数，并在多数基准测试中提升精度-参数帕累托边界。


<details>
  <summary>Details</summary>
Motivation: 神经算子作为PDE代理模型广泛使用，但全尺寸模型存储、部署和评估成本高，现有压缩方法需先训练大模型再压缩，效率低。

Method: 从输出坐标归一化后的经验均值预测器开始，训练一系列同族小型神经算子（如FNO、DeepONet、CNO）拟合残差场，并通过验证集选择收缩系数整合每个修正。

Result: 在30个数据集-架构对中，21个平均准确率提升，17个置信区间为正，参数减少72-95%；10个PDE基准中7个实现帕累托改进。

Conclusion: Operator Boosting通常能改善神经算子代理的精度-参数帕累托边界，但也存在依赖PDE和架构的失败情况，即残差提升未能抵消压缩损失。
...
### [156] [ResAware: Cross-Environment Website Fingerprinting via Resource-Privileged Distillation](https://arxiv.org/abs/2606.17462)
*Chongru Fan,Wei Wang,Wentao Huang,Zhenquan Ding,Jinqiao Shi,Lei Cui,Zhiyu Hao,Xiaochun Yun*

Main category: cs.LG

TL;DR: 提出ResAware框架，利用资源级特征通过知识蒸馏提升网站指纹攻击在真实环境中的鲁棒性，无需增加在线观测成本。


<details>
  <summary>Details</summary>
Motivation: 现有网站指纹攻击依赖低级流量特征，易受环境扰动影响，导致在真实世界中性能大幅下降。

Method: 在训练丰富/推理贫瘠的非对称设置下，使用资源级特征训练教师模型，并通过异构知识蒸馏将知识传递给学生模型，学生模型部署时仅使用加密流量。

Result: 在150天时间漂移下，Var-CNN的F1分数从72.77%提升至81.49%，开放世界TPR@1%FPR从22.40%提升至27.20%。
...
### [157] [CheckMIABench: Firm Foundations For Membership Inference Attacks on Language Models](https://arxiv.org/abs/2606.17464)
*Jeffrey G. Wang,Jason Wang,Marvin Li,Seth Neel*

Main category: cs.LG

TL;DR: 提出了一种基于固定时间点前后训练数据同分布原理的LLM成员推断攻击评估基准，解决了现有评估中分布偏移导致的统计无效问题，并开源了相关库。


<details>
  <summary>Details</summary>
Motivation: 现有成员推断攻击在语言模型上评估困难，因为成员与非成员集之间存在微妙分布偏移，导致统计无效，甚至无模型访问的盲方法表现更优。

Method: 利用训练数据在固定时间点前后同分布的洞察，将具有中间检查点的开源模型和公开训练数据转化为MIA测试床，并对Pythia和OLMo系列模型（70M至7B参数）应用多种已发布攻击。

Result: 对六种已发布的攻击在多个规模模型上进行了系统评估，并开源了模块化库。

Conclusion: 该基准提供了原则性的评估框架，有助于推动LLM隐私研究的进一步发展。
...
### [158] [Perron--Frobenius Operator Matching for Generative Modeling](https://arxiv.org/abs/2606.17465)
*Shiqi Zhang,Wuwei Wu,Jaemin Oh,Jie Chen,Xiaoning Qian*

Main category: cs.LG

TL;DR: 提出基于Perron-Frobenius算子的生成框架PFOM，统一流、扩散和跳跃模型，通过KL散度实现密度匹配，并采用Nesterov加速训练与采样。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型（流、扩散等）缺乏统一的算子理论基础，且密度匹配目标在实践中有偏差。

Method: 利用Perron-Frobenius算子匹配密度演化，证明仅KL散度保持密度级与样本条件目标等价，推导出Koopman路径匹配损失，并引入Nesterov加速训练与采样。

Result: 在Gaussian mixtures和two-moons数据集上，PFOM在KL散度、Wasserstein距离和MMD指标上下降更快，壁钟效率更高。

Conclusion: PFOM统一了算子理论与生成建模，为自适应字典和高维应用提供了新方向。
...
### [159] [ReRAM-aware Model Finetuning addressing I-V Non-linearity and Retention Errors](https://arxiv.org/abs/2606.17471)
*Ching-Yi Lin,Shamik Kundu,Arnab Raha,Sahil Shah*

Main category: cs.LG

TL;DR: 提出一种基于微调的硬件感知训练算法，用于在ReRAM上高效部署深度神经网络，减少训练开销。


<details>
  <summary>Details</summary>
Motivation: 传统架构受冯·诺依曼瓶颈限制，ReRAM交叉阵列虽高效但存在非理想性，现有硬件感知训练需从头训练，计算成本高。

Method: 通过范围收缩的sinh变换缓解I-V非线性，并将保留误差作为正则化损失融入微调过程。

Result: 在ResNet18和DeiT-Tiny上精度接近基线；MobileNetV3系列在ImageNet上精度下降小于2%；SQuAD v2数据集F1分数仅下降1点。

Conclusion: 该方法在保持精度的同时显著降低训练开销，适用于大规模模型和多种任务。
...
### [160] [Multi-Adapter PPO: A Cross-Attention Enhanced Wavelength Selection Framework for LIBS Quantitative Analysis](https://arxiv.org/abs/2606.17476)
*Hao Li,Man Fung Zhuo*

Main category: cs.LG

TL;DR: 提出多适配器PPO框架，将LIBS波长选择转化为强化学习问题，在钢和煤数据集上综合得分优于PSO 28.4%，预测精度提升45.2%。


<details>
  <summary>Details</summary>
Motivation: LIBS定量分析中高维光谱数据导致波长选择困难，且需平衡预测精度与特征效率。

Method: 基于多适配器PPO框架，利用交叉注意力机制和多个专门适配器，将波长选择建模为强化学习问题。

Result: 在钢和煤数据集上，综合得分平均提升28.4%，预测精度提升45.2%，达到最优性能。

Conclusion: 该方法有效平衡预测精度与特征效率，保持可解释性和计算效率，实现LIBS定量分析的最新技术水平。
...
### [161] [Online LLM Selection via Constrained Bandits with Time-Varying Demand](https://arxiv.org/abs/2606.17489)
*Yin Huang,Qingsong Liu,Jie Xu*

Main category: cs.LG

TL;DR: 提出一种在线学习算法，在硬预算和软延迟约束下为每个任务选择最佳LLM，平衡奖励与约束，理论保证次线性遗憾和违反，实验验证鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有静态选择策略无法应对模型异构、随机性能、时变任务需求及硬资源预算与软服务约束，需要自适应在线决策。

Method: 建模为约束随机赌博机学习，使用置信界估计和需求预测在线选择模型，处理打包型（硬）和覆盖型（软）约束。

Result: 理论证明次线性遗憾和次线性覆盖约束违反（对比离线全信息基准）；合成工作负载实验显示有效性和鲁棒性。

Conclusion: 该算法能在动态资源受限环境中有效平衡奖励最大化与长期约束满足。
...
### [162] [Reconfigurable Computing Challenge: Transformer for Jet Tagging on Versal AI Engines](https://arxiv.org/abs/2606.17500)
*Gram Koski,Sean Lipps,Zhenghua Ma,G. Abarajithan,Ryan Kastner*

Main category: cs.LG

TL;DR: 提出了一种量化整数only Transformer的FPGA实现框架，用于喷注标记，在AMD Versal AI引擎上实现低延迟部署。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在喷注标记中表现优异，但难以在低延迟、资源受限的触发系统中部署。

Method: 将密集层和多头注意力层映射到AIE瓦片，构建可重用软件框架，通过高层Python模型描述自动生成Vitis图代码。

Result: 成功实现了基于AMD Versal AI引擎的量化整数only Transformer喷注标记系统，并开源了框架。

Conclusion: 该框架为未来喷注标记的低延迟硬件部署研究提供了基础。
...
### [163] [When the Next Step Is Not One Step: Distribution-Aware Execution Modeling for Concurrent Go Programs](https://arxiv.org/abs/2606.17508)
*Kaviru Hapuarachchi*

Main category: cs.LG

TL;DR: 提出利用并发程序调度非确定性作为训练信号，通过多次运行聚合经验分布并微调模型，在真实Go bug预测中达到36.2%准确率，优于基线模型，并降低校准误差。


<details>
  <summary>Details</summary>
Motivation: 并发程序下一步预测因调度器非确定性导致同一前缀可能产生多个合法下一事件，传统单标签训练无法有效建模。

Method: 多次运行同一程序，统计下一事件的经验分布，使用KL散度损失微调7B语言模型以匹配该分布。

Result: 在798个真实Go bug预测中，微调后准确率36.2%，高于Gemini 3.5 Flash零样本34.8%和未微调模型28.6%；分布训练与交叉熵准确率相当（35.8% vs 36.2%），但期望校准误差从0.205降至0.169。

Conclusion: 利用非确定性分布训练有效提升并发程序预测性能，并提供了数据集、适配器和工具。
...
### [164] [Geometry-Aware Post-Hoc Uncertainty Quantification in Operator Learning](https://arxiv.org/abs/2606.17513)
*Oriol Vendrell-Gallart,Nima Negarandeh,Ramin Bostanabad*

Main category: cs.LG

TL;DR: 提出REEF-GP，一种基于高斯过程的神经算子后验不确定性量化框架，通过将残差拟合到算子内部嵌入特征空间实现几何感知的不确定性估计。


<details>
  <summary>Details</summary>
Motivation: 现有神经算子缺乏不确定性量化能力，尤其是几何变化下；已有方法仅关注参数不确定性，忽略算子学习到的几何感知表示。

Method: 在冻结神经算子的残差上拟合GP，利用算子内部嵌入定义核特征空间；采用谱归一化投影、异方差几何感知噪声和子集训练以保障稳定性和可扩展性。

Result: 在五个不同几何的PDE基准上保持预测精度，不确定性校准与深度集成相当但计算成本更低；对几何分布偏移稳健，不确定性集中于物理关键区域（如激波前沿）。

Conclusion: 在神经算子的学习特征空间中可直接实现准确且可扩展的后验不确定性量化，为参数中心方法提供实用替代。
...
### [165] [FoundCause: Causal Discovery with Latent Confounders from Observational Data](https://arxiv.org/abs/2606.17516)
*Patrick Bl\"obaum,Krishnakumar Balasubramanian,Shiva Prasad Kasiviswanathan*

Main category: cs.LG

TL;DR: 提出FoundCause，一种在合成数据上训练的可摊销因果发现模型，通过单次前向传递将数据集映射到因果图，显式建模潜在混杂，性能优于多种经典和摊销方法。


<details>
  <summary>Details</summary>
Motivation: 从观测数据进行因果发现仍具挑战，尤其是恢复有向结构和处理潜在混杂变量，现有方法需要干预或计算代价高。

Method: 使用置换不变transformer编码器，通过交替注意力机制联合建模变量间依赖和变量分布；注入基于经典非对称度量的成对统计特征；采用因子化解码器分离边的存在性和方向；引入三角细化模块推理高阶因果模式；通过可学习潜在令牌的混杂模块显式建模隐藏共同原因，并支持缺失数据。

Result: 在15个真实数据集上，与11种非摊销方法（如PC、GES、NOTEARS）和4种摊销方法相比，F1提高9.6%，AUROC提升1.2%，结构汉明距离降低18.9%，且推理仅需单次前向传递。

Conclusion: 首次在可摊销因果发现中显式建模潜在混杂，验证了利用合成数据学习的统计模式可泛化到真实数据，并显著提升因果图恢复的准确性和效率。
...
### [166] [Learning to Refine Hidden States for Reliable LLM Reasoning](https://arxiv.org/abs/2606.17524)
*Chia-Hsuan Hsu,Jui-Ming Yao*

Main category: cs.LG

TL;DR: 提出ReLAR框架，通过强化学习引导的潜在状态迭代精炼，提升多步骤推理的稳定性和效率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂多步推理中内部推理不稳定，早期隐藏状态错误会传播导致错误预测。

Method: ReLAR在解码前迭代更新隐藏表示，使用学习的深度和动作控制器自适应决定推理步数和方向，通过策略梯度优化逐步似然改进。

Result: 在医疗、数学、多跳推理和开放生成基准上，准确率、生成质量和推理稳定性提升，推理开销显著低于显式推理基线。

Conclusion: 该框架无需显式思维链，能高效实现输入依赖的推理，且具有广泛适用性。
...
### [167] [MGUP: A Momentum-Gradient Alignment Update Policy for Stochastic Optimization](https://arxiv.org/abs/2606.17526)
*Da Chang,Ganzhao Yuan*

Main category: cs.LG

TL;DR: 提出MGUP机制，通过每次迭代对固定比例参数使用大步长、其余使用小步长，实现细粒度选择性更新，并保证收敛。


<details>
  <summary>Details</summary>
Motivation: 现有优化方法缺乏既能细粒度控制又有收敛保证的通用选择性更新机制。

Method: 在动量优化器中加入MGUP模块，每次迭代对固定比例参数使用大步长，其余使用非零小步长，可与AdamW等集成。

Result: 在MAE预训练、LLM预训练和下游微调中，MGUP增强的优化器比原优化器表现更优或更稳定。

Conclusion: MGUP是一种有理论保证、即插即用的高效选择性更新策略，可加速并稳定大规模模型训练。
...
### [168] [Non-negative Matrix Factorisation with Topological Regularisation](https://arxiv.org/abs/2606.17531)
*Matias de Jong van Lier,Shizuo Kaji,Keunsu Kim*

Main category: cs.LG

TL;DR: 通过持久同调正则化NMF学习可解释基函数，统一处理图像、时间序列和图信号。


<details>
  <summary>Details</summary>
Motivation: 数据模态的结构化域中基的拓扑质量重要，但传统方法离散且阈值依赖。

Method: 将持久同调作为拓扑正则项集成到NMF目标中。

Result: 框架涵盖空间连贯图像分量、周期时间序列和团状图信号。

Conclusion: 持久同调正则化有效学习拓扑可解释的基函数。
...
### [169] [Offline Preference-Based Trajectory Evaluation](https://arxiv.org/abs/2606.17541)
*Fernando Diaz*

Main category: cs.LG

TL;DR: 提出基于偏好的轨迹评估方法，通过比较轨迹的时间偏好减少平局，提高离线评估的统计效率。


<details>
  <summary>Details</summary>
Motivation: 标准成功度量将轨迹压缩为最终成功，丢弃部分进展信息，导致大量平局，降低统计效率和区分能力。

Method: 从轨迹的时间进展和返回时间剖面中提取偏好，直接用偏好比较轨迹，而非仅依赖最终成功。

Result: 在多个智能体和交互式基准上，标准成功度量在约75%实例中导致平局，而轨迹感知偏好将平局降至约35%，提高了区分力、排序稳定性和数据效率。

Conclusion: 基准饱和可能部分源于评估指标的选择，改进评估方法可缓解该问题。
...
### [170] [Continuous-time Optimal Stopping through Deep Reinforcement Learning](https://arxiv.org/abs/2606.17545)
*Cosmin Borsa,Michael Ludkovski*

Main category: cs.LG

TL;DR: 提出CARLOS算法，利用强化学习和深度神经网络在任意细时间分辨率下学习最优停止规则，比现有方法更准确高效。


<details>
  <summary>Details</summary>
Motivation: 传统动态规划在粗网格下低估价值，细网格下误差累积，需发展新方法。

Method: 采用聚合深度神经网络学习联合时空决策边界，从粗网格开始逐步加密，并行训练，并设计自适应采样策略。

Result: CARLOS获得比Bermudan求解器更高的价格，接近美式上界，且计算效率高。

Conclusion: CARLOS有效解决了传统方法的时间离散化限制，提高了最优停止问题的求解精度和效率。
...
### [171] [Reversal Q-Learning](https://arxiv.org/abs/2606.17551)
*Aditya Oberai,Seohong Park,Sergey Levine*

Main category: cs.LG

TL;DR: 提出并实验验证了反转Q学习（RQL）算法，该算法基于流匹配和扩展MDP框架，在离线强化学习中达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有离线强化学习难以有效建模复杂行为，而迭代生成模型如流匹配具有潜力，但需要新方法实现在线策略学习。

Method: 基于扩展MDP框架，将流细化步骤视为MDP中的独立动作；通过反转流生成虚拟在线轨迹以兼容离线数据；应用偏差-方差缩减技术缓解离线强化学习中的地平线诅咒。

Result: 在50个具有挑战性的模拟机器人任务上，RQL相比最先进的基于流的离线强化学习算法取得了最佳平均离线性能。

Conclusion: RQL无需通过时间反向传播，更充分地利用学习的价值函数，并直接训练完整的、具有表现力的流策略，优于先前方法。
...
### [172] [SpatioTemporal Causal Network Diagnostics for Geographic Tipping Point Early Warning](https://arxiv.org/abs/2606.17553)
*Zhaoyuan Yu,Zhangyong Liang*

Main category: cs.LG

TL;DR: 提出时空因果网络诊断（ST-CND），通过数据驱动的信息流拓扑和局部恢复率估计，实现地理系统临界点的早期预警。


<details>
  <summary>Details</summary>
Motivation: 传统空间指标如Moran's I存在空间稀释、欧几里得假设和相关噪声问题，无法有效定位早期预警信号。

Method: ST-CND将地理场表示为随时间演化的有向因果网络，包括：用转移熵推断空间节点间信息流拓扑；用动态模式分解估计局部恢复率；综合高内部波动、高内部同步和低外部耦合三个信号识别最脆弱子网络。

Result: 在合成分岔和两个观测海温基准（印度洋-太平洋SST和北大西洋AMOC）上验证，AMOC任务AUROC为0.783，关键子网络IoU为0.378，优于基线方法。
...
### [173] [Reducing Learner Redundancy in Boosting via Residual Orthogonalization](https://arxiv.org/abs/2606.17567)
*Ye Su,Jipeng Guo,Yong Liu,Xin Xu,Gangchun Zhang,Jinxin Chen,Di Wu,Longlong Zhao*

Main category: cs.LG

TL;DR: 提出SCBoost框架，从残差拟合转向残差正交化，通过谱残差投影和协方差正则加权减少冗余，理论证明其有效性，实验显示性能提升。


<details>
  <summary>Details</summary>
Motivation: 标准提升框架中残差拟合导致学习器冗余，重复处理相关误差成分。

Method: 引入残差正交化，包括谱残差投影（SRP）将残差投影到历史预测子空间的正交补，以及协方差正则加权（CRW）在验证集上优化集成权重并加入协方差惩罚。

Result: 在十个基准数据集上，SCBoost在准确率和F1分数上表现出强劲的即开即用性能。

Conclusion: 通过几何视角重新解释提升，表明显式冗余控制是构建更高效集成架构的原则性且必要步骤。
...
### [174] [When Dynamics Models Read the Wrong Time Steps: Label-Free Event Credit Re-Anchoring for Robust Global Readouts](https://arxiv.org/abs/2606.17572)
*Yifan Wang*

Main category: cs.LG

TL;DR: 提出了时间信用稀释问题，并设计了CREST方法通过事件核心重锚定来恢复事件信用，降低分布外误差。


<details>
  <summary>Details</summary>
Motivation: 学到的动力学模型通常从序列中读取全局特征，导致时间信用稀释：模型在训练条件下准确预测，但将信用分配给平滑相关量而非决定目标的关键物理事件。

Method: 引入Credit-in-Event探针衡量事件信用，并提出CREST：一种无需训练和标签的读出器，从学习特征中估计瞬态事件核心，通过事件与背景对比重锚定池化表示。

Result: 在仿真齿轮/冲击系统、循环/注意力编码器以及公开轴承振动数据上，CREST降低了分布外误差并恢复了事件信用。消融实验表明，稳定步选择或感受野缩小无效，证实收益来自事件核心信用重锚定。

Conclusion: 标准物理信息残差等方法无法解决信用稀释问题，CREST通过事件核心重锚定有效提升了模型鲁棒性。
...
### [175] [LLM Features Can Hurt GNNs: Concatenation Interference on Homophilous Graph Benchmarks](https://arxiv.org/abs/2606.17579)
*Zhongyuan Wang,Pratyusha Vemuri*

Main category: cs.LG

TL;DR: 将LLM特征通过输入拼接引入GNN可能降低同质图基准的准确率，而Delta_sig可预测拼接效果。


<details>
  <summary>Details</summary>
Motivation: 通常LLM特征能提升GNN准确率，但纯输入拼接反而系统性地降低同质图基准准确率，需要理解何时拼接有益或有害。

Method: 在不同数据集上测试LLM特征拼接效果，引入Delta_sig度量（LLM单独可区分性），分析其与拼接成本的相关性，进行维度控制和幂律拟合。

Result: PubMed和Cora上准确率下降，WikiCS和ogbn-arxiv上提升。Delta_sig与拼接成本相关性（r²=0.38）强于同质性（r²=0.06）。幂律拟合显示|Δ_concat| ∝ (√(d_l/n))^1.31。

Conclusion: Delta_sig可作为解释性透镜，但非精确过滤器；低Delta_sig和小n时拼接成本最大。
...
### [176] [Expanding SPHERE-JEPA: A Family of Statistical Regularizers for the Hypersphere](https://arxiv.org/abs/2606.17603)
*L\'eo Nicollier (CB,ATT),Enric Meinhardt-Llopis (CB),Max Dunitz (ATT),Marc Pic (ATT),Pablo Mus\'e (CB,IFUMI),Gabriele Facciolo (CB)*

Main category: cs.LG

TL;DR: 本文提出用确定性最大均值差异(MMD)替换切片统计正则化器，避免蒙特卡洛采样带来的方差与优化不稳定，在全维球面上构建MMD、核斯坦偏移(KSD)和KL散度目标，并引入旋转不变核（热核与带限核），在ImageNet和Galaxy10上获得更稳定、更快的收敛和更好性能。


<details>
  <summary>Details</summary>
Motivation: 当前自监督学习中的切片统计正则化器（如SIGReg、SUSReg）通过蒙特卡洛采样近似连续均匀分布目标，引入投影方差导致训练梯度不稳定、优化困难，阻碍收敛。

Method: 首先证明解析积分随机投影等价于确定性MMD，然后直接在全维球面上构造MMD、KSD和KL散度目标来强制均匀分布，并使用谱理论构建旋转不变核（光滑指数衰减热核与严格频率截止带限核）以避免空间偏差。

Result: 去除投影噪声后优化更稳定、收敛更快，在ImageNet和Galaxy10上一致优于随机切片正则化器。MMD和KSD偏好局部聚类结构适合物体中心域，而基于连续KDE的KL散度促进细粒度实例分离，在无聚类过程纹理检索上取得最强结果。

Conclusion: 消去投影噪声能提升自监督学习的稳定性与性能，且统计检验的选择会塑造潜在空间的几何结构，不同任务应选择合适的正则化器。
...
### [177] [A Risk Decomposition Framework for Pre-Hoc Fine-Tuning Prediction](https://arxiv.org/abs/2606.17649)
...
### [178] [Physics-Constrained Neural Networks for Improved Short-Term Weather Forecasting: A Case Study over the South Pacific](https://arxiv.org/abs/2606.17659)
*Egor Bugaev,Fedor Buzaev,Dmitry Efremenko,Denis Derkach,Fedor Ratnikov*

Main category: cs.LG

TL;DR: 通过改进数值求解器、统一自回归块和集成两个神经网络主干，对物理约束神经网络进行增强，实现了更准确稳定的短期天气预报。


<details>
  <summary>Details</summary>
Motivation: 提升混合短期天气预报模型的精度和稳定性。

Method: 提出三个创新：升级的数值求解器（WENO-5、beta平面近似、亚网格尺度粘度）、统一自回归混合块替代24个专用模块、与两种先进神经网络主干（PI-PredFormer和PI-IAM4VP）集成。

Result: 在WeatherBench南太平洋子集上，1-12小时提前期均方根误差降低8-22%，时间步长增大至1200秒，日均方误差降低26%。

Conclusion: 渐进式改进混合组件是提升短期天气预报精度和效率的实用途径。
...
### [179] [TuneAhead: Predicting Fine-tuning Performance Before Full Training Begins](https://arxiv.org/abs/2606.17660)
*Yuxiang Luo,Haonan Long,Chen Wang,Qiqi Duan,Xiaotian Lin,Yanwei Xu,Yuyu Luo,Weikai Yang,Nan Tang*

Main category: cs.LG

TL;DR: 提出TUNEAHEAD框架，能在大语言模型微调前准确预测性能，从而避免不必要的计算。


<details>
  <summary>Details</summary>
Motivation: 微调大语言模型计算密集且易出错，性能高度依赖数据质量和超参数，因此希望在完整训练前预测微调性能。

Method: 将每个候选运行编码为包含静态数据集描述符和动态探测特征的元特征向量，通过预测器映射到性能估计，并利用SHAP归因提供可解释的诊断。

Result: 在1300+次微调运行上，TUNEAHEAD优于Early-Stop Extrapolation和ProxyLM等基线；在370次测试集上，RMSE为1.47个百分点，95.1%的预测误差在±3个百分点内。

Conclusion: 准确的连续预测支持实用的筛选策略，能够减少不必要的完整微调，同时保留最有希望的训练运行。
...
### [180] [Handling Feature Heterogeneity with Learnable Graph Patches](https://arxiv.org/abs/2606.17667)
*Yifei Sun,Yang Yang,Xiao Feng,Zijun Wang,Haoyang Zhong,Chunping Wang,Lei Chen*

Main category: cs.LG

TL;DR: 提出可学习图补丁作为任意图数据的最小语义单元，实现跨域图数据的通用预训练。


<details>
  <summary>Details</summary>
Motivation: 现有模型无法处理无文本图数据的特征异质性，限制了图模型在不同数据集间的可迁移性。

Method: 将图分解为可学习图补丁，通过展开节点特征并分别构建补丁结构；设计补丁编码器和聚合器提取与整合跨域可迁移信息。

Result: 方法支持多域图预训练，在多个下游数据集和任务上性能提升，且预训练数据量增大时效果持续改善。

Conclusion: 该领域无关方法实现跨域迁移，增强了图基础模型的通用性。
...
### [181] [ASTEROID: A Spatiotemporal Information Transformer for Forecasting Multi-Step Time Series of Molecular Dynamics](https://arxiv.org/abs/2606.17668)
*Kexin Wu,Luonan Chen,Renxiao Wang*

Main category: cs.LG

TL;DR: 开发了ASTEROID框架，通过将MD轨迹转化为时空序列并集成STI变换到Transformer中，实现了直接预测多步原子坐标，加速MD模拟。


<details>
  <summary>Details</summary>
Motivation: MD模拟计算量大，特别是对于需要长期分析的大规模系统，准确预测模拟结果具有科学挑战和实用价值。

Method: 采用改进的Transformer架构，通过局部-全局自注意力机制捕捉空间依赖性，编码器-解码器结构处理时间依赖性，并集成STI变换方程。

Result: 在多个量子力学分子数据集上，多步预测精度优于现有方法，显著降低计算成本，支持长时间尺度迭代预测。

Conclusion: 建立了一个稳健且可推广的数据驱动范式，用于加速MD模拟。
...
### [182] [EnvRL: Learn from Environment Dynamics in Agentic Reinforcement Learning](https://arxiv.org/abs/2606.17680)
*Zhitong Wang,Songze Li,Hao Peng,Shuzheng Si,Yi Wang,Maosong Sun,Juanzi Li*

Main category: cs.LG

TL;DR: 提出EnvRL框架，通过状态预测和逆动态两个辅助目标，将环境动态学习融入智能体强化学习，显著提升长周期任务成功率。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习方法在长周期智能体任务中面临稀疏奖励问题，忽视了交互轨迹中包含的丰富环境动态信息。

Method: 提出EnvRL框架，在标准强化学习目标之外联合优化状态预测和逆动态两个辅助目标，使智能体从自身交互经验中学习环境动态。

Result: 在ALFWorld上，Qwen-2.5-1.5B-Instruct的成功率从72.8%提升至77.4%；在WebShop上，从56.8%提升至67.0%。

Conclusion: 利用交互轨迹中的隐式监督信号学习环境动态，能有效提升强化学习策略的性能。
...
### [183] [Delta-Based Target Reformulation for Short-Term Electricity Load Forecasting Using LSTM and Transformer Models](https://arxiv.org/abs/2606.17692)
*Vansh Bansal*

Main category: cs.LG

TL;DR: 本文提出基于差分的负荷预测目标重构方法，在小时前预测中显著提升精度，MAPE降低超50%。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法直接预测绝对负荷，忽略了非平稳性。受ARIMA差分启发，旨在通过预测负荷变化来稳定学习目标，降低预测难度。

Method: 使用LSTM和Transformer模型，预测连续时间步的负荷变化（delta），然后利用最后观测值重构最终预测。结合气象和日历特征，在印度多年度小时数据上评估，对比LightGBM。

Result: 对于小时前预测，delta方法在所有模型上一致提升精度，MAPE降低超50%；对于日前预测，仅对深度序列模型有效，LightGBM在绝对公式下仍有竞争力。

Conclusion: Delta目标重构是有效的归纳偏置，但其效果依赖于模型和预测水平。
...
### [184] [Confusion-Aware Transfer Teacher Curriculum Learning Framework: Disentangling Scoring and Pacing Effects](https://arxiv.org/abs/2606.17706)
...
TL;DR: 课程学习结合了样本难度评分和训练节奏，但难以区分两者贡献。通过解耦评估，发现混淆感知难度评分在数据充足时无优势，但在20%数据下提升高达8.7%，表明转移教师框架可作为数据高效训练方法。


<details>
  <summary>Details</summary>
Motivation: 现有课程学习将评分与节奏耦合，无法单独评估评分函数或节奏的效果，因此需要解耦评估以理解各自贡献。

Method: 设计两阶段评估协议：阶段性子集验证评分函数，基线使用相同节奏但随机排序数据。在转移教师框架内，提出混淆感知难度评分，考虑正确类置信度和错误类分布。

Result: 在CIFAR-10上，混淆感知评分生成的难度排序符合直觉。但在完整数据下，课程或反课程排序均未提升准确率；而在20%数据下，混淆感知课程排序比随机排序提升最多8.7个百分点。

Conclusion: 单独改进评分函数不足以克服课程学习的已知缺陷，但转移教师框架在数据有限时表现出显著的数据效率优势，具有应用潜力。
...
### [185] [A fairness-aware extension of Stochastic Multicriteria Acceptability Analysis for ranking](https://arxiv.org/abs/2606.17756)
*Guilherme Dean Pelegrina,Renata Pelissari*

Main category: cs.LG

TL;DR: 提出SMAA-Fair，一种基于公平性的SMAA扩展方法，通过重新加权排名以提升群体公平性。


<details>
  <summary>Details</summary>
Motivation: 在负责任人工智能背景下，排名问题中的公平性成为核心关注点，但SMAA方法未明确处理公平性。

Method: 提出SMAA-Fair框架，根据群体公平性水平对SMAA生成的模拟排名进行重新加权，使更公平的排名对可接受性指数和中心权重向量贡献更大。采用统计奇偶性、rKL和nDKL等公平性指标，通过期望排名和最大可接受性排名从公平调整后的可接受性矩阵推导排名，并根据排名公平性程度推导中心权重。

Result: 在合成和真实数据上的数值实验表明，SMAA-Fair提高了受保护群体在排名靠前位置的代表性，同时保持了对偏好不确定性的鲁棒性。

Conclusion: SMAA-Fair能有效提升排名公平性，且不牺牲对不确定性的鲁棒性。
...
### [186] [Blind Recovery of Latent Domains via Unsupervised Symmetry Discovery](https://arxiv.org/abs/2606.17782)
*Onur Efe,Arkadas Ozakin*

Main category: cs.LG

TL;DR: 提出一种无监督框架，通过发现数据分布的对称性，从无结构观测中恢复潜在域和信号。


<details>
  <summary>Details</summary>
Motivation: 盲逆问题的主要动机是在不感知退化机制的情况下从退化观测中恢复目标信号，但现有盲反卷积不适用于一般线性变换掩盖域结构的情况。

Method: 将观测建模为从潜在随机场采样的信号的线性测量，通过优化浅层群卷积网络施加平稳性和局部性正则化，学习潜在对称作用和相关滤波器，将无结构观测映射到基于对称性的表示。

Result: 在随机过程、伊辛模型、打乱和比特置乱图像以及神经记录等实验中，该方法成功从无结构观测中恢复了潜在域和信号。

Conclusion: 对称发现为无监督结构学习和盲逆问题提供了新方向。
...
### [187] [Continual Self-Improvement with Lightweight Experiential Latent Memories](https://arxiv.org/abs/2606.17803)
*Vaggelis Dorovatas,Nancy Kalaj,Rahaf Aljundi*

Main category: cs.LG

TL;DR: 提出一种在线学习方法，将推理痕迹蒸馏为轻量级模块化记忆，实现持续改进且避免灾难性遗忘，性能超越零样本和原始数据ICL基线。


<details>
  <summary>Details</summary>
Motivation: 大语言模型虽能通过扩展推理时计算提升性能，但本质无状态，丢弃了自身产生的丰富推理痕迹，无法从经验中在线学习转换为持久知识。

Method: 从无监督强化学习中汲取灵感，利用自生成测试时间信号（多数投票）作为奖励进行轻量级实例训练，并将推理时计算蒸馏为紧凑的模块化软提示记忆（约0.001%参数），通过少量梯度步骤在线存储和检索。

Result: 在数学推理基准上显著优于零样本和原始数据ICL基线，并与全参数更新和离线训练竞争力相当，且能跨数据集迁移。
...
### [188] [QueryMarket: Cost-Aware Online Active Learning in Data Markets](https://arxiv.org/abs/2606.17805)
*Xiwen Huang,Pierre Pinson*

Main category: cs.LG

TL;DR: 提出QueryMarket框架和OVBAL方法，统一处理定价、信息增益和滚动预算约束下的在线主动学习，适应概念漂移和异构标签成本。


<details>
  <summary>Details</summary>
Motivation: 实时流学习中数据获取是主要瓶颈，现有在线主动学习缺少对定价、信息增益和滚动预算约束的统一考虑，尤其在概念漂移下。

Method: 提出QueryMarket框架，其中OVBAL方法通过D最优性准则和指数遗忘估计样本边际效用，结合成本感知，在滚动预算下进行数据购买。

Result: 在合成数据和真实太阳能预测任务中，OVBAL在卖家定价模式下特别有效，并在两种定价模式下实现更优的长远误差-成本权衡。

Conclusion: OVBAL提供简单完全在线决策规则，适应非平稳流和异构标签成本，尤其适用于卖家定价场景。
...
### [189] [No-Free-Fairness: Fundamental Limits and Trade-offs in Learning Systems](https://arxiv.org/abs/2606.17810)
*Khoat Than*

Main category: cs.LG

TL;DR: 本文提出无免费公平定理，证明学习系统中不公平的三个根本来源：任务成本、有限样本和模型类限制，表明公平需作为核心设计考虑。


<details>
  <summary>Details</summary>
Motivation: 揭示不公平并非仅由数据偏差或优化不足导致，而是源于决策问题、有限数据和模型表达力的内在结构。

Method: 通过理论推导，建立三个不可行性结果：公平-成本固有边界、有限样本导致统计瓶颈、模型类表达能力限制。

Result: 任何决策规则都需权衡性能与差异；有限样本下严格公平需指数级样本；模型无法表示子群准确解时公平不可达。

Conclusion: 不公平是固有结构问题，实现公平需明确权衡，并应作为学习系统的核心设计考量。
...
### [190] [Conservation Laws for Modern Neural Architectures](https://arxiv.org/abs/2606.17816)
*Viet-Hoang Tran,Vinh Khanh Bui,Tan Lai Ngoc,Nam Nguyen,Tuan Dam,Tan M. Nguyen*

Main category: cs.LG

TL;DR: 提出了一个统一框架来刻画现代神经网络架构（如GELU、SiLU、SwiGLU、多头注意力、混合专家）中的梯度流守恒定律，并实验验证。


<details>
  <summary>Details</summary>
Motivation: 现有的守恒定律研究局限于线性网络和ReLU网络，对现代架构（如GELU、SiLU、SwiGLU、注意力、MoE）的隐式偏差理解不足。

Method: 构建统一的理论框架，分析梯度流中的守恒量，涵盖多种激活函数、位置编码和门控机制。

Result: 理论预测的守恒定律在实验中得到了验证。

Conclusion: 该框架有助于解释过参数化模型的泛化能力，为理解现代架构的隐式偏差提供了理论基础。
...
### [191] [Functional Equivalence in Attention: A Comprehensive Study with Applications to Linear Mode Connectivity](https://arxiv.org/abs/2606.17830)
*Viet-Hoang Tran,Vinh Khanh Bui,Van-Hoan Trinh,Tan Lai Ngoc,Tan M. Nguyen*

Main category: cs.LG

TL;DR: 本文研究了Transformer中位置编码对功能等价性的影响，发现正弦位置编码保留原始注意力等价结构，而旋转位置编码（RoPE）显著降低对称性，提升表达能力，并揭示了位置编码对线性模式连接性的关键作用。


<details>
  <summary>Details</summary>
Motivation: 现有分析忽略位置编码对神经网络参数空间对称性的重塑作用，而现代注意力架构中此问题更为复杂。

Method: 形式化分析Transformer在正弦和旋转位置编码下的功能等价性，并通过对齐算法实验研究线性模式连接性。

Result: 正弦编码维持等价结构，旋转编码缩减对称群；位置编码类型决定线性模式连接性的存在与变化。

Conclusion: 旋转位置编码（RoPE）因降低对称性而增强表达能力，这解释了其在实际中的优势，且位置编码的选择对模型连接性至关重要。
...
### [192] [From Drift to Coherence: Stabilizing Beliefs in LLMs](https://arxiv.org/abs/2606.17832)
*SongEun Kim,Seungyoo Lee,Edwin Fong,Hyungi Lee,Juho Lee*

Main category: cs.LG

TL;DR: 本文通过提出的提示式预测重采样（PPR）方法，发现大语言模型在多选问答中早期信念漂移但后来自稳定，并进一步提出种子答案提示和自一致性损失微调，有效减少漂移、提升一致性。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明LLM的预测信念不满足鞅性质，但多在合成环境中；本文旨在更通用的多选问答设置中验证并改进。

Method: 提出PPR（提示式预测重采样）方法，让模型对同一问题生成多个答案并分析信念动态；基于漂移自稳定现象，提出种子答案提示加速稳定和自一致性损失微调。

Result: 在多项选择QA基准上，所提方法显著减少信念漂移，提高预测一致性，且不牺牲准确率。

Conclusion: LLM的信念漂移可通过自稳定和提出的方法有效缓解，改进模型预测的一致性。
...
### [193] [Meta-classification of one-class classification models using ranking correlation and nearest neighbor](https://arxiv.org/abs/2606.17858)
*Toshitaka Hayashi,Hamido Fujita,Dalibor Cimr,Richard Cimler,Jitka K\"uhnov\'a*

Main category: cs.LG

TL;DR: 提出一种元分类方法，将机器学习模型视为单类分类（OCC）模型，通过正态性排名和最近邻/排名相关度量进行分类，实验证明能有效分类数据集、算法和超参数。


<details>
  <summary>Details</summary>
Motivation: 机器学习技术广泛应用，但将ML应用于ML模型本身尚未充分探索。所有ML模型可近似为OCC模型，因此研究OCC模型的元分类。

Method: 将OCC模型表示为正态性排名，使用最近邻和排名相关度量进行分类。

Result: 分类准确率高，当类别为数据集时尤为显著；当训练数据集包含相同类别时能分类算法。

Conclusion: 对OCC模型的分类本质上是对数据集的分类（将多个样本视为单一输入），可提供统一解决方案。源代码已公开。
...
### [194] [AnchorKV: Safety-Aware KV Cache Compression via Soft Penalty with a Refusal Anchor](https://arxiv.org/abs/2606.17872)
*Ning Ni,Yingjie Lao*

Main category: cs.LG

TL;DR: 提出AnchorKV，通过引入安全锚点偏置令牌保留分数，在KV缓存压缩中提升安全性，轻微牺牲效用。


<details>
  <summary>Details</summary>
Motivation: LLM在长上下文任务中KV缓存成为瓶颈，现有压缩方法易受越狱攻击或损害安全对齐。

Method: AnchorKV离线构造安全锚点，基于差均值表示工程在键空间投影，采用软惩罚令牌选择规则偏置保留分数。

Result: 在保持大部分效用的同时显著提升安全对齐，惩罚为零时退化为原始压缩器。

Conclusion: AnchorKV是即插即用的修改，有效平衡效用与安全。
...
### [195] [Monotonic Kolmogorov-Arnold Networks: A Theoretical and Empirical Study of Monotonicity as an Inductive Bias](https://arxiv.org/abs/2606.17886)
*Mikhail Krasnov,Carolina Fortuna,Bla\v{z} Bertalani\v{c}*

Main category: cs.LG

TL;DR: MKAN是一种新的Kolmogorov–Arnold网络变体，通过指数重新参数化B样条系数、正边权重和单调基激活实现硬单调性，保证了所有参数的单调性，且训练为无约束梯度下降。理论贡献是表示成本定理，指导单调编码器的规模；实验表明MKAN在基准测试中与最先进方法竞争，并验证了理论预测。


<details>
  <summary>Details</summary>
Motivation: 现有单调神经网络（如MonoKAN）缺乏每边功能透明度，且约束限于参数子集并需投影训练。需要一种兼具硬单调性和功能透明性的KAN架构。

Method: 使用指数重新参数化B样条系数、正边权重和单调基激活，确保所有参数下的硬单调性；训练采用标准无约束梯度下降。

Result: 理论上证明了表示成本定理：任何C^K特征提取器诱导的球状语义邻域分割，可用单调实现在N'≤2N*内。实验上，MKAN在SMM/ICML-2024基准上与最先进单调NN竞争；在四个真实数据集上验证了2N*预测；在受控生成数据集上，MKAN的Spearman对齐显著高于KAN、MLP和线性基线。

Conclusion: MKAN成功实现了硬无约束单调性与KAN每边功能透明性的结合，理论定理为单调编码器提供了原则性规模规则，实验证明了其竞争力和有效性。
...
### [196] [Dimensionality Controls When Modularity Helps in Continual Learning](https://arxiv.org/abs/2606.17889)
...
### [197] [KANLib -- An Modular, Extensible and Fast Kolmogorov-Arnold Network Implementation](https://arxiv.org/abs/2606.17927)
*Julian Hoever,Gregor Schiele*

Main category: cs.LG

TL;DR: 提出KANLib，一个模块化、可扩展且计算高效的KAN框架，统一了现有实现，支持多种特性和定制，在加州房价基准上复现了预测行为并达到竞争效率。


<details>
  <summary>Details</summary>
Motivation: 现有KAN实现存在高计算成本和不一致的特征支持，阻碍了实际研究。

Method: 通过统一PyKAN、EfficientKAN和FastKAN的核心概念，使用模块化架构，支持两种基函数、自适应网格缩放、网格扩展和细粒度定制，兼容PyTorch。

Result: 在加州房价基准上，KANLib复现了参考实现的预测行为，同时计算效率高，且架构变体对性能影响小。

Conclusion: KANLib为可扩展和可扩展的KAN架构未来研究提供了稳健基础。
...
### [198] [Predictive Analytics in E-Commerce for CustomerBehavior Forecasting using hybrid Ret-DNN withXGBoost Model](https://arxiv.org/abs/2606.17931)
*Degala Pushpa Sri,Mayank Atreya,Lakshmi. H,Navin Chhibber,Mukesh Soni*

Main category: cs.LG

TL;DR: 提出一种混合零售深度神经网络（Ret-DNN）与极限梯度提升（XGBoost）模型，用于预测电子商务中的客户购买概率，在英国在线零售数据集上取得了更低的平均绝对误差。


<details>
  <summary>Details</summary>
Motivation: 零售平台难以理解客户行为并预测其未来购买，因此需要更准确的预测模型。

Method: 从英国在线零售商收集近50万条交易数据，经数据清洗、异常处理、时序特征提取、特征编码和Z-score标准化后，输入Ret-DNN提取特征，再传入XGBoost预测购买概率。

Result: 所提Ret-DNN XGBoost模型的平均绝对误差（MAE）为0.2193，优于现有Ret-DNN模型。

Conclusion: 混合模型能更有效地捕获零售数据的时序特征和表格动态，提升预测性能。
...
### [199] [SoftMoE: Soft Differentiable Routing for Mixture-of-Experts in LLMs](https://arxiv.org/abs/2606.17952)
*Miko{\l}aj Zasada,{\L}ukasz Struski,Jacek Tabor,Marcin Kurdziel*

Main category: cs.LG

TL;DR: 提出了SoftMoE，通过可微的软top-k路由替代离散路由，实现专家容量的动态分配，在保持性能的同时显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 稀疏MoE中的离散top-k路由不可微，且固定每输入激活的专家数量，导致计算资源利用不高效。

Method: 使用截断的软top-k LapSum松弛（可微），并参数化每层平均活跃专家数，施加全局预算约束，让模型学习跨层分配专家容量。

Result: 在语言建模和下游任务上性能与稀疏MoE相当或更好，同时激活更少的专家；学习到的分配高度非均匀，后层激活更多专家。

Conclusion: SoftMoE能够高效利用计算资源，兼容自回归模型，代码已开源。
...
### [200] [Multiple cyclicity and Wavelet Decomposition with Channel Correlation for Long-term Time Series Forecasting](https://arxiv.org/abs/2606.17996)
*Bin Wang,Heming Yang,Jinfang Sheng*

Main category: cs.LG

TL;DR: 提出McWC模型，通过分别建模周期性、趋势和通道间相关性，实现高效准确的长期时间序列预测。


<details>
  <summary>Details</summary>
Motivation: 现有模型忽视时间序列数据中通道间相关性，且设计复杂导致计算效率低。

Method: McWC使用多层周期性构建模块解耦周期性，多层感知器提取通道间相关性，多级小波分解模块融合高低频信息，频域损失函数解耦通道内自相关。

Result: 在六个真实数据集上达到最先进性能，计算效率和历史信息提取能力优异。

Conclusion: McWC有效处理了通道间相关性和计算效率问题，为长期预测提供了新方法。
...
### [201] [Half a Link can Be Enough to Predict a Whole Link: Understanding Generalization in Knowledge Graph Foundation Models](https://arxiv.org/abs/2606.18001)
*Cosimo Gregucci,Obaidah Theeb,Daniel Hernandez,Antonio Vergari,Steffen Staab*

Main category: cs.LG

TL;DR: 知识图谱基础模型零样本泛化能力不均衡，取决于部分链接（half-links）是否在训练中见过。


<details>
  <summary>Details</summary>
Motivation: 理解KGFM在不同知识图谱上泛化时，何时以及如何稳健地泛化。

Method: 通过观察测试三元组中的半链接（h,r）或（r,t）是否在推理图中出现，构建四种场景的细粒度分析。

Result: 现有KGFM利用已见过的半链接进行预测，而未见过的半链接带来不同挑战。

Conclusion: 该分类可作为诊断协议，用于评估KGFM的稳健泛化能力，并指明改进方向。
...
### [202] [C2FL: Clustered Continual Federated Learning under Spatial and Temporal Drift](https://arxiv.org/abs/2606.18003)
*Davide Domini,Gianluca Aguzzi,Lorenzo Pellegrini,Mirko Viroli,Lukas Esterle*

Main category: cs.LG

TL;DR: C2FL是一种完全分布式的联邦学习方法，通过空间聚类和自适应平均机制应对隐私、空间异质性和时间漂移挑战。


<details>
  <summary>Details</summary>
Motivation: 集体自适应系统面临隐私敏感、空间异质性和时间漂移问题，传统学习策略在这些动态下严重退化。

Method: 节点通过空间聚类自组织成学习组，结合经验回放和驻留时间感知的自适应平均，逐步融入区域共识并保留旧知识。

Result: 在合成实验中，标准联邦策略性能显著下降，而C2FL恢复了鲁棒的集体适应能力。

Conclusion: C2FL有效解决了隐私、空间异质性和时间漂移共同作用下的学习挑战。
...
### [203] [Recursive Scaling in Masked Diffusion Models](https://arxiv.org/abs/2606.18022)
*Alba Carballo-Castro,Julianna Piskorz,Paulius Rauba,Mihaela van der Schaar,Pascal Frossard*

Main category: cs.LG

TL;DR: 递归掩码扩散模型 (R-MDMs) 通过在每个扩散步骤中重复应用同一个去噪变压器来增加递归深度，作为第三个缩放轴，显著提升参数效率和测试时计算分配。


<details>
  <summary>Details</summary>
Motivation: 传统掩码扩散模型缩放仅通过增加参数或去噪步骤，作者提出增加递归深度作为新的缩放维度，以提升效率。

Method: 在每个扩散步骤内，重复应用同一个去噪变压器进行递归细化，增加有效模型深度而不增加参数量。

Result: 在Sudoku和Countdown等任务中，递归L次的模型性能匹配非递归基线约L倍参数量的模型；递归细化可部分替代额外去噪步骤，减少推理前向传播次数。

Conclusion: 递归深度是一种实用的缩放机制，能同时改善参数效率和测试时计算分配。
...
### [204] [LoopCoder-v2: Only Loop Once for Efficient Test-Time Computation Scaling](https://arxiv.org/abs/2606.18023)
...
TL;DR: 循环Transformer（PLT）的循环次数存在最优值，两循环最佳，更多循环因增益递减且位置偏移代价固定而性能下降。


<details>
  <summary>Details</summary>
Motivation: 循环Transformer通过共享块重复应用扩展计算，但顺序循环增加延迟和KV缓存，并行循环（PLT）引入跨循环位置偏移（CLP）和共享KV门控滑动窗口注意力，使循环次数成为可调设计参数，需研究其最佳选择。

Method: 训练LoopCoder-v2系列7B PLT编码器，循环次数分别为0,1,2,3,4，在18T token上从零训练，然后进行一致的指令微调和评估。

Result: 两循环变体在代码生成、代码推理、代理软件工程和工具使用基准上全面优于无循环基线，SWE-bench Verified从43.0提升至64.4，Multi-SWE从14.0提升至31.0；三循环及以上变体性能下降，呈现强非单调效应。

Conclusion: 循环次数选择存在增益-代价权衡：第2循环提供主要有效改进，后续循环更新递减、振荡且表征多样性降低，而CLP导致的偏移代价固定，因此增益主导时两循环最优，更多循环代价主导导致性能回归。
...
### [205] [Catastrophic Forgetting is Low-Rank: A Function-Space Theory for Continual Adaptation](https://arxiv.org/abs/2606.18024)
*Ido Nitzan Hidekel,Dan Raviv*

Main category: cs.LG

TL;DR: 给出NTK机制下函数空间视角的灾难性遗忘模型，指出遗忘集中在少量旧任务NTK特征模式上，并据此提出谱正则化方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法（参数漂移、重放、蒸馏）无法识别输出空间中的脆弱方向，需要从函数空间理解遗忘。

Method: 在NTK机制下推导新任务训练导致的旧任务预测漂移的闭式预测器；在冻结主干线性头PEFT-CL中精确成立，对非线性适配器为局部NTK近似。

Result: 遗忘集中在少量旧任务NTK特征模式上，冻结线性头下给出脆弱秩的Kronecker缩放规则。

Conclusion: 阐明了与先前NTK重叠理论的关系，解释了参数空间正则化为何忽略输出空间干扰，并提出了目标谱正则化器。
...
### [206] [ConTex: Reformulating Counterfactual Generation For Time Series Forecasting](https://arxiv.org/abs/2606.18049)
*Jan Voets,Hasan Tercan,Tobias Meisen,Sebastian Baum*

Main category: cs.LG

TL;DR: 提出ConTex，一种模型无关的反事实时间序列解释方法，通过全局一致的干预策略生成稀疏、可解释的反事实，支持实时应用。


<details>
  <summary>Details</summary>
Motivation: 现有反事实生成方法基于逐实例优化，导致不一致、高计算成本，难以实时应用。

Method: 将反事实生成重新定义为学习全局一致干预策略的问题，提出ConTex架构，包括时序上下文编码器和条件编码器，以及两个输出头。

Result: 在多个数据集和架构上达到最先进的有效性，生成稀疏反事实，计算成本降低12-36倍，实时推理约0.007秒。

Conclusion: ConTex有效解决了现有方法的局限性，实现了高效的实时反事实生成。
...
### [207] [NoiseTilt: Noise-Tilted Reverse Kernels for Diffusion Reward Alignment](https://arxiv.org/abs/2606.18066)
*Jisung Hwang,Yunhong Min,Jaihoon Kim,I-Chao Shen,Minhyuk Sung*

Main category: cs.LG

TL;DR: 提出噪声倾斜反向核（NTRK），一种奖励引导的扩散采样方法，通过固定反向均值并向噪声项注入奖励梯度，同时保持样本质量和梯度信号，性能超越现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有奖励引导扩散采样方法存在权衡：基于梯度的指导会改变反向均值导致质量下降，而搜索方法缺乏梯度信号，没有方法能同时兼顾质量和梯度。

Method: 固定预训练反向均值不变，通过白化算子将奖励梯度转化为可安全注入噪声的形式，从而偏置噪声项朝向高奖励区域。

Result: 在多种奖励对齐任务中，NTRK优于最新基线且不损失样本质量；在美学生成任务中，仅用25次函数评估（NFEs）即可达到最佳基线500次NFEs的奖励，计算量减少20倍。

Conclusion: NTRK成功解决了奖励引导扩散采样中的质量-梯度权衡，实现了高效、高质量的生成，显著降低了计算成本。
...
### [208] [Volterra Generative Models](https://arxiv.org/abs/2606.18071)
*Yusen Jia,Bingyan Han*

Main category: cs.LG

TL;DR: 提出Volterra生成模型，通过分数核引入路径依赖噪声，扩展了基于分数的扩散模型，并在MNIST和CIFAR-10上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 标准基于分数的扩散模型使用布朗扰动，具有可逆动力学但噪声无记忆性，限制了建模能力。

Method: 构建连续时间Volterra生成模型，前向过程通过分数核注入路径依赖噪声。利用高斯求积和混合有限差分指数近似构造有限维马尔可夫提升，推导增广线性高斯前向过程，并通过残差状态和分析辅助高斯分数保持学习维度。针对协方差和逆时间退化，提出稳定化条件及高斯桥重建采样器。

Result: 在MNIST和CIFAR-10上，小马尔可夫提升的分数噪声改进了生成的品质，桥采样器为大提升提供了稳定性。
...
### [209] [Edge Flow: A Tractable and Predictive Continuous-Time Model for Gradient Descent at the Edge of Stability](https://arxiv.org/abs/2606.18080)
*Pierre Marion*

Main category: cs.LG

TL;DR: 提出Edge Flow，由三个耦合常微分方程组成，用于建模深度学习梯度下降在稳定性边缘的动态。


<details>
  <summary>Details</summary>
Motivation: 经典分析工具（如梯度流和下降引理）在稳定性边缘不适用，需要有效的连续时间模型。

Method: 将动态分解为中心、振荡方向和振荡幅度，中心遵循对称化损失的修正梯度流，方向跟踪Hessian主特征向量，幅度按尖锐度是否超过阈值指数增长或衰减。

Result: Edge Flow能忠实跟踪梯度下降动态，比先前模型更好地解决尖锐度振荡问题，并提供理解不稳定的框架。

Conclusion: Edge Flow为理解和缓解稳定性边缘的不稳定性提供了有原则的框架。
...
### [210] [From Reasoning Traces to Reusable Modules: Understanding Compositional Generalization in Language Model Reasoning](https://arxiv.org/abs/2606.18089)
...
TL;DR: 后训练流水线结合监督微调和强化学习，通过组合泛化提升大语言模型的推理能力，其中监督微调提供原子模块，强化学习进行组合分解。


<details>
  <summary>Details</summary>
Motivation: 解释监督微调与强化学习结合成功的原因，并揭示其在组合泛化中的不对称互补作用。

Method: 通过层次潜在选择模型形式化组合泛化，并设计受控实验验证监督微调和强化学习的角色。

Result: 强化学习能从监督微调提供的复合轨迹中提取原子模块并重新组合，且在复合轨迹上训练比在孤立原子模块上训练泛化更强。

Conclusion: 监督微调应确保原子模块覆盖，强化学习专注于监督微调支持范围之外的新组合以驱动探索。
...
### [211] [S4oP: Operator-level Pruning of Structured State Space Models for Resource-Constrained Devices](https://arxiv.org/abs/2606.18096)
*Marco Deano,Filippo Ziche,Nicola Bombieri*

Main category: cs.LG

TL;DR: 提出一种针对结构化状态空间模型（S4/S4D）的增量式算子级剪枝方法，在降低推理成本的同时保持预测性能，首次系统研究SSM的结构化算子剪枝。


<details>
  <summary>Details</summary>
Motivation: SSM在长序列任务中表现优异，但计算和内存需求高，难以在时间和资源受限场景部署。

Method: 通过交错结构化掩码和微调，渐进式剪枝模型算子，并联合监控准确率和推理延迟，统一框架下探索效率-精度权衡。

Result: 在多个基准数据集上，剪枝高达70%的算子仍能保持原始模型性能，同时显著降低推理延迟。

Conclusion: 结构化算子剪枝是提升SSM效率的有效且此前未探索的策略，有助于在资源受限场景中部署SSM。
...
### [212] [Deep Reinforcement Learning for Minimum Zero-Forcing Sets](https://arxiv.org/abs/2606.18106)
*Steve Halley,Maur\'icio Gruppi*

Main category: cs.LG

TL;DR: 提出一个基于强化学习的框架SD-ZFS来解决最小零强制集问题，实验证明其有效性。


<details>
  <summary>Details</summary>
Motivation: 最小零强制集问题在网络科学、网络控制和逻辑电路设计等领域有重要应用，但该问题是NP难的，现有方法难以高效求解大规模图。

Method: 采用强化学习框架SD-ZFS，改编自S2V-DQN架构，通过训练模型自动学习选择最小零强制集的策略。

Result: 模型在不同结构图数据集上表现良好，与最优解和贪心启发式相比，找到了更小的零强制集，且具有良好的泛化、扩展和迁移能力。

Conclusion: 该机器学习方法能有效解决最小零强制集问题，网络结构显著影响问题求解难度和算法性能。
...
### [213] [Learning Fair Pareto-Optimal Policies in Multi-Objective Reinforcement Learning](https://arxiv.org/abs/2606.18111)
*Umer Siddique,Peilang Li,Yongcan Cao*

Main category: cs.LG

TL;DR: 提出多策略多目标强化学习中公平优化的形式化框架，并开发三种算法以学习覆盖所有用户偏好的公平策略集。


<details>
  <summary>Details</summary>
Motivation: 单策略MORL方法无法为动态或未知用户偏好提供多样化策略，需要学习一个公平的帕累托最优策略集。

Method: 证明对于凹分段线性福利函数，公平策略仍属于凸覆盖集；利用非平稳策略和随机策略改善公平性；提出三种集成GGF与多策略多目标Q学习的算法。

Result: 在多个领域实验表明，所提方法能学习适应不同用户偏好的公平策略集，优于现有基线。

Conclusion: 本研究为多策略MORL中的公平优化提供了理论基础和有效算法，扩展了公平性在强化学习中的应用。
...
### [214] [Ternary Mamba: Grouped Quantization-Aware Training of W1.58A16 State Space Models](https://arxiv.org/abs/2606.18114)
*Ramprasath Ganesaraja,Sahil Dilip Panse,Swathika N*

Main category: cs.LG

TL;DR: 本研究表明，通过从预训练检查点进行三元量化感知训练（QAT）与知识蒸馏，可以高效压缩Mamba-2状态空间模型，仅需1亿词元即可接近从头训练的性能，并揭示了零比例崩溃这一新现象。


<details>
  <summary>Details</summary>
Motivation: Mamba-2等状态空间模型推理速度快但内存占用大，限制了边缘部署。此前三元SSM需要从头训练150B词元，成本高昂。

Method: 采用分组量化感知训练（QAT），结合冻结FP16教师模型的知识蒸馏。将Mamba-2 1.3B模型压缩3.61倍（从2687MB降至744MB），仅用102M词元（4GPU小时，单H100）。

Result: 零样本准确率（7任务平均）达48.1%，接近Bi-Mamba的48.4%。发现零比例崩溃不稳定现象，且适用于Transformer的后处理修正策略因递归误差累积在SSM中失效。

Conclusion: 三元SSM无需昂贵的从头训练，从预训练检查点进行QAT+KD是一种数据高效的替代方案。
...
### [215] [Embedded Machine Learning for Microcontroller-Class Edge Devices: Data, Feature, Evaluation, and Deployment Pipelines](https://arxiv.org/abs/2606.18122)
*Mostafa Darvishi*

Main category: cs.LG

TL;DR: 本文系统性地综述了面向微控制器平台的嵌入式机器学习工作流，重点介绍了采样、特征提取、模型设计等工程决策，并通过运动识别和关键词识别两个实例进行了说明。


<details>
  <summary>Details</summary>
Motivation: 嵌入式机器学习将推理从云端迁移至资源受限设备，面临内存、能耗和延迟的严格限制，需要一个系统化的工程方法。

Method: 提出了嵌入式机器学习工作流，涵盖采样与缓冲、特征提取（降维）、不平衡验证、模型/运行时协同设计及流式部署，并结合惯性运动识别（加速度计特征）和关键词识别（MFCC+一维CNN）两个案例。

Result: 通过两个代表性信号家族（运动识别和关键词识别）的成功应用，展示了工作流的实用性。

Conclusion: 总结了鲁棒设备端推理的实用设计规则，包括数据整理、量化、阈值设定、调度和现场监控。
...
### [216] [Kolmogorov Regression for Robust Diffusion Policies](https://arxiv.org/abs/2606.18186)
*Lekan Molu*

Main category: cs.LG

TL;DR: 通过后向柯尔莫哥洛夫方程将扩散策略提升到卡梅伦-马丁空间，用确定性PDE替代随机评分匹配，提升长期性能和正则性。


<details>
  <summary>Details</summary>
Motivation: 有限维扩散策略因离散化伪影导致时间漂移，影响长期性能。

Method: 引入后向柯尔莫哥洛夫方程，将扩散策略提升到卡梅伦-马丁空间，使用精度加权的卡梅伦-马丁损失训练，并引入柯尔莫哥洛夫残差作为PDE诊断。

Result: 收敛保证依赖于核的有效秩，轨迹正则性改善，无奖励信号的确定性故障检测。在PushT基准上最大奖励提高17%，步间漂移减少67.6%；在制造线上RMSE降低28.4%，高召回率，瓶颈识别精确；死锁事件减少96%。

Conclusion: 该方法有效减少了漂移，提升了长期性能和可靠性。
...
TL;DR: 提出了首个循环架构世界模型LoopWM，通过迭代细化潜状态实现高效长时模拟。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型在长周期模拟中受限于深度计算的昂贵和累积误差。

Method: 使用参数共享的Transformer块迭代更新潜环境状态，并自适应调整深度。

Result: 相比传统方法实现高达100倍参数效率，且计算复杂度自动匹配预测步骤。

Conclusion: LoopWM建立了迭代潜深度作为世界模拟的新缩放轴，有望推动该领域发展。
...
### [218] [Rethinking Dataset Distillation for Classification: Do Distilled Sets Outperform Coresets?](https://arxiv.org/abs/2606.18209)
*Trisha Mittal,Akshay Mehra,Joshua Kimball*

Main category: cs.LG

TL;DR: 通过大规模标准化实验，发现当前数据集蒸馏方法在大型数据集上性能不优于甚至不如核心集选择，且构建成本更高，实际优势有限。


<details>
  <summary>Details</summary>
Motivation: 数据集蒸馏的评估协议不一致，且许多方法声称优于核心集选择，但缺乏严格验证。

Method: 在ImageNet-1K、ImageNet100和ImageNette上，用三种训练协议对七种最新DD方法与三种CS策略进行基准测试。

Result: 部分DD方法不如随机子集，SOTA DD与核心集相当或更差，且构建成本更高；核心集在数据代表性、多样性和质量上更优。

Conclusion: 当前DD方法的实际优势有限，核心集在大型数据集上仍是更具计算效率的选择。
...
### [219] [Sign-Rank, Index, and List Replicability: Connections and Separations](https://arxiv.org/abs/2606.18236)
*Ari Blondal,Hamed Hatami,Pooya Hatami,Chavdar Lalov,Sivan Tretiak*

Main category: cs.LG

TL;DR: 研究了二元概念类的sign rank下界，通过Z2-index和list replicability number，证明了Z2-index是list replicability number的线性函数，从而分离sign rank和Z2-index，并给出list replicability的上界和组合性质。


<details>
  <summary>Details</summary>
Motivation: sign rank下界难以直接分析，需要更易处理的度量来建立下界。

Method: 比较Z2-index和list replicability number，证明线性关系；利用高度和最小星数建立list replicability的上界；证明乘积类的list replicability number有界于两类的和。

Result: Z2-index是list replicability number的线性上界，导致sign rank与Z2-index的强分离；list replicability number有上界（高度和最小星数）；乘积类有组合界。

Conclusion: list replicability是更强的下界度量，为sign rank下界研究提供了新工具。
...
### [220] [Gaussian Mixture Attention: Linear-Time Sequence Mixing via Probabilistic Latent Routing](https://arxiv.org/abs/2606.18283)
*Yongchao Huang,Hassan Raza*

Main category: cs.LG

TL;DR: 高斯混合注意力（GMA）通过K个高斯混合组件进行路由，避免点积注意力的二次复杂度，实现线性存储缩放，并在长上下文中表现出竞争力。


<details>
  <summary>Details</summary>
Motivation: 标准点积注意力的密集token交互模式导致O(N^2)复杂度，成为长上下文Transformer扩展的瓶颈。

Method: 使用K个可学习的高斯混合组件，将查询和键映射为责任向量，通过责任向量重叠计算隐式亲和度，值通过K槽记忆读写，避免显式N×N矩阵。

Result: GMA实现固定K的线性内存缩放；在长上下文分类上竞争力强；因果GMA在WikiText-103上优于线性/随机特征注意力，但落后于优化的SDPA和Mamba。

Conclusion: GMA是一种概率性、可解释、固定K线性时间的注意力替代方案，但不是优化后的softmax注意力或状态空间模型的通用替代。
...
### [221] [Breaking the Solver Bottleneck: Training Task Generators at the Learnable Frontier](https://arxiv.org/abs/2606.18284)
...
TL;DR: 提出PROPEL框架，用轻量级探针预测任务通过率，替代昂贵的求解器评估，高效生成高学习价值任务。


<details>
  <summary>Details</summary>
Motivation: RL训练受限于高质量任务供应，现有生成方法产生无效或过易任务，而直接优化生成器需要大量求解器调用，不可行。

Method: 训练轻量级激活探针，基于冻结的生成器参考模型预测求解器通过率，作为生成器优化的代理，只需一次前向传播。

Result: 在代码任务中，目标求解率任务比例提升（如Qwen2.5-3B从10.1%到20.0%）；SWE任务中，从9.8%到19.6%。

Conclusion: PROPEL有效提高了可学习前沿任务的生成比例，缓解了任务供应瓶颈。
...
### [222] [CODEBLOCK: Learning to Supervise Code at the Right Granularity](https://arxiv.org/abs/2606.18286)
*Zhijie Deng,Ling Li,Jinlong Pang,Kaiqin Hu,Qi Xuan,Zhaowei Zhu,Jiaheng Wei*

Main category: cs.LG

TL;DR: 提出CodeBlock，一种结构感知的稀疏监督框架，用于代码LLM微调，选择完整代码结构而非孤立token，仅用1.9%的监督token达到更好效果。


<details>
  <summary>Details</summary>
Motivation: 传统监督微调对所有token均匀交叉熵损失，但代码需要结构完整性，直接token级掩码会破坏程序结构。

Method: 先选择高质量指令-响应对，将代码响应分割为语法完整的编码项，通过聚合核心逻辑token的广义交叉熵估计效用，并使用数据流可达和桥接信号重新排序，训练时仅对选中的代码项和信息性自然语言token施加损失。

Result: 在6个代码生成基准上，pass@1超过全token SFT和竞争性选择基线，仅使用1.9%的监督响应token。

Conclusion: 结构感知的稀疏监督有效提升代码微调效率。
...
### [223] [Artemis: Anatomy-Resolved inTervention for Eliminating Multimodal NeuroImage confounderS](https://arxiv.org/abs/2606.18287)
*Siyuan Dai,Yang Du,Kun Zhao,Zhusuyi Chen,Heng Huang,Paul Thompson,Chao Shi,Haoteng Tang,Liang Zhan*

Main category: cs.LG

TL;DR: 提出Artemis，一种区域级因果框架，通过独立干预每个脑区来消除人口统计学混杂因素，提升GNN在脑网络分析中的泛化性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 多模态脑网络分析中，人口统计学因素（如年龄、性别）会引入混杂偏差，导致GNN利用虚假捷径而非因果不变特征；现有因果方法未考虑神经影像领域的真实混杂因素及脑区特异性敏感性。

Method: Artemis在区域层面进行因果干预，为每个脑区学习独立的轻量级混杂表示，并整合功能连接和结构连接特征作为即插即用模块，兼容任意GNN骨干。

Result: 在ADNI、OASIS和HCP三个基准上，Artemis一致优于代表性GNN基线；多个支持实验验证了统计显著性和神经科学可解释性。

Conclusion: 区域级因果调整有效消除了人口统计学的混杂影响，提升了多模态脑网络分析的因果推理能力和临床实用性。
...
### [224] [A Link between Shock-wave Theory and Symmetry-reduced Stochastic Gradient Descent for Artificial Neural Networks](https://arxiv.org/abs/2606.18303)
*Taiki Miyagawa*

Main category: cs.LG

TL;DR: 建立冲击波理论与随机梯度下降学习动力学之间的数学联系，通过对称商化和局部熵粗粒化，得到粘性Hamilton-Jacobi方程和Burgers型方程，应用于多种神经网络架构。


<details>
  <summary>Details</summary>
Motivation: 理解深度学习中的训练过程，特别是对称性冗余导致的参数扭曲，需要更原则性的监控方法。

Method: 利用微分几何、李群理论和流体力学，对参数对称性进行商化，并应用局部熵粗粒化，推导出有效动力学满足粘性Hamilton-Jacobi方程，进一步假设梯度场满足Burgers方程。

Result: 多层感知机、卷积神经网络、Transformer和平均场网络均遵循这些方程，且可证明冲击波形成。

Conclusion: 该框架为深度学习提供实用诊断工具，对称校正的商可观测量可用于监控和预测训练阶段转变。
...
### [225] [Attribution-Guided and Coverage-Maximized Pruning for Structural MoE Compression](https://arxiv.org/abs/2606.18304)
*Yifu Ding,Jiacheng Wang,Ge Yang,Yongcheng Jing,Jinyang Guo,Xianglong Liu,Dacheng Tao*

Main category: cs.LG

TL;DR: 提出针对MoE模型的结构剪枝框架，通过通道级冗余利用和剪枝比例优化，在50%或25%剪枝加4位量化下保持精度，并显著降低内存。


<details>
  <summary>Details</summary>
Motivation: 现有专家级剪枝方法粒度粗，无法捕捉细粒度冗余，导致剪枝预算分配不当和压缩有限。

Method: 将剪枝比例分配重构为通道分数覆盖率最大化问题，并利用基于归因的近似方法高效求解。

Result: 在DeepSeek和Qwen MoE模型上，50%或25%结构剪枝结合4位量化保持精度；在Qwen3-30B-A3B上，内存减少5.27倍，且在各种基准上优于现有方法。

Conclusion: 提出的通道级结构剪枝框架能有效压缩MoE模型，减少部署成本，同时保持性能。
...
### [226] [Fisher Width: A Geometric Measure of Complexity on Statistical Manifolds](https://arxiv.org/abs/2606.18306)
*Vu Khac Ky*

Main category: cs.LG

TL;DR: 提出费希尔宽度作为统计流形上高斯宽度的黎曼几何类比，保留了关键结构性质并对局部统计曲率敏感。


<details>
  <summary>Details</summary>
Motivation: 高斯宽度在欧几里得空间中衡量有效维度，但统计模型具有自然黎曼几何，需要考虑费希尔信息度量。

Method: 在参数点θ处，用局部度量张量G(θ)^{1/2}替换欧几里得单位矩阵，定义费希尔重新缩放集的高斯宽度。

Result: 费希尔宽度具有集中性、度量扰动稳定性和谱比较界，并推导出费希尔-利普希茨假设类的泛化界。
...
### [227] [DRIFT: Refining Instruction Data via On-Policy Data Attribution](https://arxiv.org/abs/2606.18307)
*Zefan Wang,Lincheng Li,Tianyu Yu,Yuan Yao*

Main category: cs.LG

TL;DR: 针对监督微调中的数据分布优化问题，提出DRIFT方法，利用在线策略影响函数克服标准影响函数的局限性，提升模型性能上限。


<details>
  <summary>Details</summary>
Motivation: 现有数据筛选方法在有限预算下加速训练，但难以提升能力上限；需要将数据分布优化到最能改善最终模型的实例上。

Method: 提出DRIFT，采用模型的在线策略输出作为验证目标，解决标准影响函数中的邻近差距和梯度范数偏差；基于轨迹正确性进行符号加权，并去偏影响分数，使少量验证查询可靠地归因整个数据集。

Result: 在7B参数指令和推理模型上，DRIFT持续提升性能上限，优于现有数据筛选基线。

Conclusion: DRIFT有效优化了监督微调的数据分布，提升了模型能力上限。
...
### [228] [TRIDENT: Breaking the Hybrid-Safety-Physics Coupling for Provably Safe Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2606.18308)
*Zijie Meng,Ziwei Li,Yufei Liu,Zhiyu Li,Jiyuan Liu,Wenhua Nie,Bingcai Wei,Miao Zhang*

Main category: cs.LG

TL;DR: 提出TRIDENT框架，协同解决混合动作、安全约束和物理动力学三大挑战，显著减少训练违规并提升奖励。


<details>
  <summary>Details</summary>
Motivation: 混合离散-连续动作、硬安全约束和物理动力学三者形成偏差循环，导致现有方法简单组合失效。

Method: TRIDENT包含三组件：Richardson-Romberg梯度校正（将Gumbel-Softmax偏差从O(tau)降至O(tau^2)）、Lyapunov约束序列信任域更新（保证每步可行性）、物理信息残差评论家（分解价值而非奖励）。

Result: 在多无人机移动边缘计算、自主路口管理和混合SMAC变体上，TRIDENT相比MADDPG和MACPO减少95.5%和76.3%训练违规，奖励提升13.5%。理论证明以O~(1/√K)收敛到约束纳什均衡，累计违规界为O(√K)。

Conclusion: TRIDENT通过协同设计有效解决了混合离散-连续动作、安全约束和物理动力学耦合带来的学习难题，显著提升了安全性与性能。
...
### [229] [SAGE: Retain-Aware Post-Hoc Sanitization of Final Unlearning Vector](https://arxiv.org/abs/2606.18309)
*Jingyuan Zhang,Yucheng Bai,Peixi Wen,Zhehao Huang,Zhengbao He,Hanling Tian,Xinwen Cheng,Haiyin Ran,Xiaolin Huang*

Main category: cs.LG

TL;DR: 提出了一种后处理净化方法SAGE，通过抑制与保留知识相关的更新方向，来缓解大型语言模型反学习中的保留-遗忘权衡。


<details>
  <summary>Details</summary>
Motivation: 现有反学习方法在消除不需要知识的同时会损害保留能力，存在权衡问题。

Method: SAGE通过收集少量保留代理数据的模块输入，提取其主导激活几何结构，并求解源锚定优化目标，在闭式中净化最终更新向量。

Result: 在多种反学习方法、模型规模和基准上，SAGE一致地缓解了保留-遗忘权衡。

Conclusion: 最终向量的后处理净化是一个实用且未被充分探索的反学习维度。
...
### [230] [Ghost Attractor Networks: Basin-Structured Dynamical Decoders for Closed-Loop Sequential Generation](https://arxiv.org/abs/2606.18315)
*Tianyu Wang,Ying Wang,Zhihao Liu,Xi Vincent Wang,Lihui Wang*

Main category: cs.LG

TL;DR: 提出Ghost Attractor Networks，一种具有盆地-吸引子结构的动力学子解码器，在保持效率的同时实现了闭环控制，并在机器人动作解码中显著优于大型模型。


<details>
  <summary>Details</summary>
Motivation: 大规模Transformer和扩散解码器内存和计算成本高，而小规模前馈解码器效率高但缺乏结构化的潜在表示，难以实现相位条件和潜在传递等闭环控制。

Method: 基于理论推导，设计具有势能和漂移的动力学子解码器，通过鞍结分岔和鬼吸引子逃逸实现模态转换，并采用分层相空间分解。

Result: 在1430个样本上，梯度范数在5个积分步骤内衰减67%；2.3M参数匹配1.07B参数扩散Transformer的离线精度，参数减少462倍，延迟降低32倍；在LIBERO-10上，相位条件带来13.5个百分点的成功率提升，持久潜在集成达到95.7%成功率。

Conclusion: Ghost Attractor Networks有效结合了效率和结构化潜在表示，在机器人动作解码中表现出优越性能。
...
### [231] [A Survey on Data-Driven Models for Soil Moisture Regression and Classification](https://arxiv.org/abs/2606.18316)
*Ilektra Tsimpidi,George Georgoulas,Vidya Sumathy,George Nikolakopoulos*

Main category: cs.LG

TL;DR: 一份关于人工智能在土壤湿度估计与分类中应用的结构化综述


<details>
  <summary>Details</summary>
Motivation: 物理模型计算成本高、可扩展性差，需要灵活的数据驱动方法

Method: 将现有AI模型分为五类：统计时间序列、地统计、经典机器学习、深度学习、概率/贝叶斯方法，利用多种数据源进行回归或分类

Result: 现有模型被组织成五个类别，涵盖多种数据源和任务

Conclusion: 综述了AI模型在土壤湿度建模中的潜力，指出了不同类型模型的适用场景
...
### [232] [Enhanced Graph Neural Networks using K-Hop Gaussian Diffusion](https://arxiv.org/abs/2606.18317)
*Xuling Zhang,Peng Wang,Daiyan Li,Aoran Huang,Zeiwei Chen,Yongkui Yang*

Main category: cs.LG

TL;DR: 提出一种K跳高斯扩散核作为图神经网络预处理模块，平衡局部和全局信息传播，在噪声图结构中优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现有GNN依赖直接邻居消息传递，受限于边噪声和远距离节点噪声，全局扩散如PPR和热核仍不足。

Method: 设计K跳高斯扩散核，对远程节点进行高斯加权的多跳扩散，在标准GNN前预处理图数据。

Result: 在多个基准数据集上，显著优于传统消息传递GNN以及PPR和热核扩散，尤其在噪声或结构复杂图中。

Conclusion: KHG扩散核有效提升GNN在图结构复杂和噪声环境下的性能。
...
### [233] [ASTRA: A Scalable Next-Generation ATCO Training Simulator with Autonomous Simpilots](https://arxiv.org/abs/2606.18319)
...
TL;DR: ASTRA是一个端到端训练模拟器，通过自动语音识别和本地化模型自动化ATCO训练中的simpilot角色，显著降低词错误率，并集成AI评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有自动化方案在非西方语境（如新加坡）表现不佳，培训高度依赖人工，成本高且标准化不足。

Method: 构建端到端管线，包括微调ASR（基于本地口音）、指令解释和响应生成，以及AI评估框架（评估准确性、简洁性、完整性）。

Result: ASR的WER从107.80%降至23.45%；评估得分分别达91.7%、88.2%、86.9%。

Conclusion: 基于开源框架实现可扩展、标准化的ATCO评估，降低教师工作量。
...
### [234] [SAE Interventions are Unreliable: Post-Intervention Recovery of Suppressed Behavior](https://arxiv.org/abs/2606.18322)
*Mingyue Cui,Linghui Shen,Xingyi Yang*

Main category: cs.LG

TL;DR: 研究发现稀疏自编码器（SAE）用于干预时，即使成功阻止了特定有害特征，模型仍可恢复原行为，揭示特征级控制与行为控制之间的鸿沟。


<details>
  <summary>Details</summary>
Motivation: 现有潜在空间防御依赖SAE特征，假设干预特定特征可阻止模型不良行为，但可能存在可恢复的失败模式。

Method: 将后干预恢复建模为残差空间优化问题，使用编码器正交更新（单层）和特征映射雅可比（跨层）进行实验，测试干预后行为恢复的可能性。

Result: 在TPP、反学习、IOI和拒绝引导等实验中，尽管特征级干预成功，行为仍可恢复；拒绝引导中恢复率达95.8%，防御特征相对漂移仅0.131，远低于基于后缀的基线。

Conclusion: SAE特征能支持因果干预，但控制特征并不能保证控制潜在行为，揭示了两者之间的差距。
...
### [235] [Why SWAVE May Not Be All You Need:A Concept-Evolution Retrospective on Complex-Valued Recurrent Language Models](https://arxiv.org/abs/2606.18324)
*Ramprasath Ganesaraja,Swathika N,Sahil Dilip Panse*

Main category: cs.LG

TL;DR: SWave是一种复数值循环语言模型，通过Cayley参数化的酉变换和旋转隐状态，在长上下文中保持信号完整性，并解决了复数值训练中的共支配坍缩问题。


<details>
  <summary>Details</summary>
Motivation: 传统实数值语言模型信息编码有限，且隐状态易衰减或爆炸，复数值表示和酉变换可提供更丰富的编码和状态稳定性。

Method: 基于复数值波表示、Cayley酉变换和旋转隐状态，经历三个阶段：共振头导致共支配坍缩，被解耦的实虚嵌入表取代；保留ComplexNorm和波传播扫描；ProtectGatedScan作为结构先验；实数值ReLU通道混合器替代复数门控单元。

Result: 最佳困惑度22.0（步骤89,861），稳定训练200,000步，发现共支配坍缩的退化最小点，多尺度保留概念无改进，辅助目标无益处。
...
### [236] [Neural Network Implementation of the Renormalization Group for Fault Diagnosis with Class Imbalance](https://arxiv.org/abs/2606.18326)
*Evgeny Nikulchev,Dmitry Ilin*

Main category: cs.LG

TL;DR: 提出RGNet网络，基于重正化群进行特征层次粗粒化，解决类不平衡和多维噪声问题。


<details>
  <summary>Details</summary>
Motivation: 机器学习实际应用面临类不平衡和多维噪声挑战，现有方法不足。

Method: RGNet利用重正化群概念，顺序压缩输入维度，连接所有尺度后分类，并引入RG流可解释低维表示。

Result: 在不平衡AI4I数据集上，RGNet在故障预测中表现通用、可解释且有竞争力。

Conclusion: RGNet是处理不平衡类问题的有效通用方案。
...
### [237] [Self-CTRL: Self-Consistency Training with Reinforcement Learning](https://arxiv.org/abs/2606.18327)
*Itamar Pres,Laura Ruis,Melat Ghebreselassie,Belinda Z. Li,Jacob Andreas*

Main category: cs.LG

TL;DR: 提出Self-CTRL方法，通过强化学习训练语言模型自我解释与行为的一致，提升可审计性和信任度。


<details>
  <summary>Details</summary>
Motivation: 语言模型需忠实描述自身行为，以便用户审计、理解和信任。

Method: 使用Self-CTRL，通过更新解释以预测行为或更新行为以匹配解释，进行一致性优化。

Result: 概率推理任务中一致性相关性R²从0.24升至0.64；宪法AI中审计预测准确率从36%升至92%，HarmBench失败率从15%降至0.5%。

Conclusion: 通过对齐解释和行为，提供了一种训练更安全、透明、可控AI模型的通用方法。
...
### [238] [ThousandWorlds: A benchmark for climate emulation of potentially habitable exoplanets](https://arxiv.org/abs/2606.18338)
*Edward T. Stevenson,Mei Ting Mak,Eric Wolf,Denis E. Sergeev,Tobi Hammond,N. J. Mayne,Miles Cranmer*

Main category: cs.LG

TL;DR: 提出了ThousandWorlds数据集，用于系外气候模拟的机器学习基准，包含1800个模拟和五个全球气候模型，为低数据、多模拟器、参数到场的回归问题提供了基准。GP方法表现最佳。


<details>
  <summary>Details</summary>
Motivation: 解释系外行星大气中的生命信号需要了解宿主行星的气候，但全球气候模型运行成本高昂，机器学习仿真器可缓解瓶颈，但缺乏多模型数据集。

Method: 构建包含5个GCM的1800个模拟数据集，映射8个行星参数到3D大气场，并设计三个嵌套子集和两个评估协议。

Result: GP方法在基准上表现最好，而现成的深度学习尚未成功。

Conclusion: ThousandWorlds为系外气候仿真和低数据多模拟器回归提供了标准化基准，揭示了当前深度学习的局限性。
...
### [239] [Do Time Series Foundation Model Benchmarks Hide Regime-Dependent Failures? Evidence from Traffic Speed Forecasting](https://arxiv.org/abs/2606.18367)
*Yingshuo Wang,Xian Sun,Lingdong Kong,Wei Gao,Yanhang Li,Zhichao Fan,Zexin Zhuang*

Main category: cs.LG

TL;DR: 提出状态分层评估，发现时间序列基础模型在交通状态转换时准确性和覆盖率严重下降，聚合指标掩盖了这些失败。提出双峰混合增强后处理方法，在保持准确性的同时改善转换覆盖率。


<details>
  <summary>Details</summary>
Motivation: 标准基准使用聚合指标评估时间序列基础模型，可能掩盖在关键操作区域（如交通状态转换）的严重失败。

Method: 对三个时间序列基础模型在两个交通速度基准上进行状态分层评估，并提出双峰混合增强（BMA），结合模型预测与历史分布知识。

Result: 转换状态MAE高达11 mph（总体3 mph），90%预测区间覆盖率低至55%。历史条件基线转换覆盖较好但总体精度差；BMA接近历史基线的转换覆盖率，同时保持模型精度。

Conclusion: 时间序列基础模型基准应纳入状态感知评估，以揭示聚合指标隐藏的失败；BMA是一种有效的后处理改进方法。
...
### [240] [From Sparse Features to Trustworthy Proxies: Certifying SAE-Based Interpretability](https://arxiv.org/abs/2606.18383)
*Dibyanayan Bandyopadhyay,Asif Ekbal*

Main category: cs.LG

TL;DR: 提出后验概括框架，通过稀疏代理风险上界认证SAE解释的忠实性，实验表明在大模型上非平凡，且后期层更易认证。


<details>
  <summary>Details</summary>
Motivation: SAE广泛用于提取可解释特征，但何时可视为基模型的忠实视图尚不明确。

Method: 构建后验概括框架，以SAE重建替换原始激活得到稀疏代理，导出基模型期望风险上界，包含代理风险、重建差距、概念池不匹配和稀疏复杂度四个可测量量。

Result: 在GPT-2 Small、Gemma-2B和Llama-3-8B上上界非平凡；Llama-3-8B逐层分析显示深度依赖，后期层认证更易、局部忠实性更强、下游误差放大更弱；特征洗牌消融可区分语义对齐与统计稀疏。

Conclusion: 该框架提供了诊断SAE解释可靠性的操作准则，非平凡上界表明特征保留预测信息，小重建误差表明代理行为接近原模型。
...
### [241] [SCOPE-FL: A Strategy-proof Chain-based Optimal pareto efficient Federated Learning System](https://arxiv.org/abs/2606.18384)
*Seyed Salar Ghazi,Kaiwen Zhang,Mehdi feizi,Hans-Arno Jacobsen*

Main category: cs.LG

TL;DR: 提出SCOPE-FL框架，通过TTC算法和区块链智能合约，在分层联邦学习中同时保证帕累托效率和策略真实性，提升模型性能和奖励分配效率。


<details>
  <summary>Details</summary>
Motivation: 现有HFL客户端选择机制缺乏帕累托效率和策略真实性，导致资源分配次优和参与者欺骗激励，降低系统整体福利。

Method: 将客户端选择建模为双边学校选择问题，采用TTC算法实现PE和SP；基于一轮重建（OR）的Shapley值近似进行奖励分配；通过区块链智能合约执行。

Result: 在MNIST、Fashion-MNIST和CIFAR-10上，SCOPE-FL在模型准确率、收敛速度和奖励效率上优于DA、IAS等方法，通信延迟与DA相当，区块链开销显著低于DA。

Conclusion: SCOPE-FL通过保证帕累托效率和策略真实性，有效提升了分层联邦学习的系统整体福利和性能。
...
### [242] [LLMZero: Discovering Adaptive Training Strategies for RL Post-Training via LLM Agents](https://arxiv.org/abs/2606.18388)
...
TL;DR: RL后训练中容量参数单调累积，正则化参数振荡，LLMZero通过树搜索发现策略提升显著且原理可迁移。


<details>
  <summary>Details</summary>
Motivation: 固定调度策略无法表达非平稳的探索-利用权衡，需要自适应的多阶段训练参数调整。

Method: 提出LLMZero系统，让LLM代理通过树搜索遍历训练轨迹，诊断检查点病理并建议协调的多参数转换。

Result: 在4个GRPO任务上，策略相对基础模型提升9%–140%，相对网格搜索提升6%–15%，一致优于随机搜索和基于技能的智能体。

Conclusion: 结构原理跨任务成立，解释了虽策略形式不同但参数动态相似的规律。
...
### [243] [MOLAR: Learning Multimodal Molecular Representations from Noisy Labels](https://arxiv.org/abs/2606.18390)
*Yingxu Wang,Kunyu Zhang,Nan Yin,Yu Li,Eran Segal*

Main category: cs.LG

TL;DR: MOLAR是一个噪声感知的多模态分子表示学习框架，通过分离清洁属性推断与标签观测，有效处理噪声标签问题。


<details>
  <summary>Details</summary>
Motivation: 分子属性预测中噪声标签普遍存在，多模态学习可能放大标签错误，需要一种噪声感知的表示学习方法。

Method: MOLAR通过图与文本视图提取残留证据以推断清洁属性分布，再通过分类标签观测通道映射到记录标签进行训练。

Result: 在自然噪声和标签翻转基准测试中，MOLAR持续优于基线方法，并提供可解释的可靠性和模态证据诊断。
...
### [244] [P$^2$CE: Model-Agnostic Plausible Pareto-Optimal Counterfactual Explanations](https://arxiv.org/abs/2606.18418)
*Arthur Hendricks Mendes de Oliveira,Giovani Valdrighi,Marcos Medeiros Raimundo*

Main category: cs.LG

TL;DR: 提出P2CE算法，生成帕累托最优的反事实解释，平衡可行性、合理性和计算效率。
...
Method: P2CE使用隔离森林异常检测确保解释符合数据分布，并利用SHAP值快速计算最优解。

Result: 在三个数据集上，P2CE在解的质量和计算效率上均优于相关技术。

Conclusion: P2CE能生成多样化、帕累托最优的反事实解释，有效兼顾多个目标。
...
### [245] [Measurement noise limits the advantage of nonlinear models over linear models in biomedical prediction](https://arxiv.org/abs/2606.18420)
*Marc-Andre Schulz,Kerstin Ritter*

Main category: cs.LG

TL;DR: 在生物医学表格数据中，灵活模型常被线性模型超越，原因是测量噪声而非模型缺陷；噪声优先抹去非线性结构，导致非线性优势消失。


<details>
  <summary>Details</summary>
Motivation: 解释为何灵活模型在生物医学数据上不优于线性模型，纠正认为模型不足的常见误解。

Method: 基于测量误差统计、心理测量学和高斯分析，推导出精确的过剩风险恒等式，并在140个UK Biobank任务中验证。

Result: 灵活模型与线性模型的差距具有预测的噪声特征；测量可靠性、样本量和特征表示是灵活模型有效的三个条件，但多数任务不满足。

Conclusion: 非线性结构被噪声隐藏，无法通过更大样本或更灵活模型恢复，只能靠改进测量来揭示。
...
### [246] [Signature filtering: a lightweight enhancement for statistical watermark detection in large language models](https://arxiv.org/abs/2606.18430)
*Chih-Duo Hong,Yen-Pang Chen,Fang Yu*

Main category: cs.LG

TL;DR: 提出签名过滤模块，在不修改水印嵌入和生成的情况下提升检测，通过移除不可靠的签名令牌。


<details>
  <summary>Details</summary>
Motivation: 现有水印检测器在弱信号、重复文本或编辑后表现不佳。

Method: 学习少量签名令牌，通过混合整数线性规划优化真阳性率，并在多种攻击模型下推导边界。

Result: 在多个水印族、数据集和LLM上，将弱信号低熵场景的检测率从8-31%提升至78-99%，假阳性可控。

Conclusion: 签名过滤是一种简单、可扩展、模型无关的附加方法，可增强LLM文本的水印溯源。
...
### [247] [Beyond Prediction: Tail-Aware Scheduling for LLM Inference](https://arxiv.org/abs/2606.18431)
*Yueying Li,Yuanfan Chen,Jiayang Chen,Esha Choukse,Haoran Qiu,G. Edward Suh,Rodrigo Fonseca,Ziv Scully,Udit Gupta*

Main category: cs.LG

TL;DR: 提出一种分布感知、无预测的调度框架，通过软优先级提升和缓存感知抢占，显著降低尾部延迟。


<details>
  <summary>Details</summary>
Motivation: 现有预测驱动的调度策略在分布偏移、突发到达和内存压力下脆弱，对尾部延迟控制有限。

Method: 替换显式长度预测为基于轻量统计信号的软优先级提升，并协同优化调度和缓存感知抢占。

Result: 相比完美长度知识的SRPT，P99 TTLT降低35-50%，TTFT降低34-47%。

Conclusion: 该方法为优化在线LLM服务尾部延迟提供了稳健的替代方案。
...
### [248] [TMR-GGNN: Credit Card Fraud Detection based on Time-Aware Multi-Relational Guided Graph Neural Network](https://arxiv.org/abs/2606.18444)
*Rohit Tewari,Shubhankar Shilpi,Navin Chhibber,Devendra Singh Parmar,Sunil Khemka,Piyush Ranjan*

Main category: cs.LG

TL;DR: 提出TMR GGNN框架，通过动态多关系图和时间注意力机制，结合对比学习和组合损失，有效提升信用卡欺诈检测性能。


<details>
  <summary>Details</summary>
Motivation: 信用卡欺诈检测面临数据高度不平衡、欺诈模式不断演变和交易实体间复杂关系结构的挑战。

Method: 扩展编码器-解码器GNN，建模客户、商户、设备、IP在时间窗口内的异质交互，构建动态多关系图，使用时间感知关系注意力机制，解码器采用对比学习，并引入InfoNCE对比损失与焦点损失的组合损失函数。

Result: 提升了欺诈识别能力，同时减少了假阴性。

Conclusion: 该框架能有效应对不平衡数据和复杂关系，提升欺诈检测的泛化性能。
...
### [249] [A Cross-Model VLM-Judge Protocol for Single-Image 3D Mesh Quality (and Why Cheap Proxies Fall Short)](https://arxiv.org/abs/2606.18451)
*Ali Asaria,Tony Salomone,Deep Gandhi*

Main category: cs.LG

TL;DR: 提出并验证了一种基于视觉语言模型（VLM）的可重复评估协议，用于单图像到3D生成，证明现有廉价代理（如CLIP和几何有效性）无法替代该协议。


<details>
  <summary>Details</summary>
Motivation: 单图像到3D生成器发展迅速，但缺乏无人工干预的标准化评估方法，现有代理指标与感知质量的相关性尚未明确。

Method: 固定24视图无背景渲染，使用两个独立VLM法官家族，强制位置偏差校正（双向查询仅保留一致判断）。

Result: 两个法官家族一致性较高（Cohen's kappa=0.66）；几何有效性为弱信号且呈双峰分布，render-CLIP性能接近随机。
...
### [250] [Veriphi: Attack-Guided Neural Network Verification with Dataset-Dependent Training Methods](https://arxiv.org/abs/2606.18454)
*Pratik Deshmukh,Kartik Arya,Vasili Savin*

Main category: cs.LG

TL;DR: 提出了Veriphi系统，结合快速对抗攻击和α,β-CROWN形式化验证，通过实验表明训练方法有效性依赖于数据集，挑战了认证训练普遍优于对抗训练的假设。


<details>
  <summary>Details</summary>
Motivation: 验证神经网络在不同复杂度数据集上认证方法的效果，并加速大规模模型的验证过程。

Method: 采用GPU加速的Veriphi系统，结合攻击指导的伪造和α,β-CROWN边界认证，使用标准、对抗和认证三种训练方法在MNIST和CIFAR-10上实验。

Result: 在MNIST上IBP认证准确率78%，在CIFAR-10上几乎无效；PGD对抗训练在CIFAR-10小扰动下认证准确率94%；实现5倍验证加速；可扩展到1.058亿参数的生产规模模型。

Conclusion: 认证训练并非普遍优于对抗训练，上下文对验证策略选择至关重要，需根据数据集特性选择方法。
...
### [251] [Task-Restricted Symmetries in Recurrent Weight Space](https://arxiv.org/abs/2606.18457)
*Simon Dr\"ager*

Main category: cs.LG

TL;DR: 通过有序实Schur坐标研究单层tanh RNN的冗余性，发现某些非正态Schur耦合可被移除而不影响任务表现，而其他耦合对自主回放至关重要。


<details>
  <summary>Details</summary>
Motivation: 循环网络在权重空间中存在大量功能冗余，某些权重改变不影响行为，而类似尺度的改变可能破坏行为，需要诊断这种冗余。

Method: 使用有序实Schur坐标将循环矩阵分离为谱块和定向非正态耦合，在固定输入和读出映射下进行结构化消融。

Result: 在固定长度复制任务中，某些非正态Schur耦合可被移除而损失很小，而其他耦合对自主回放必要；不同任务和训练解的消融轮廓不同。

Conclusion: Schur坐标消融提供了诊断哪些结构化扰动保留或破坏循环计算的方法，识别出近似功能不变性而非通用对称性。
...
### [252] [What Does the Weight Norm Control in Grokking? Logit-Scale Mediation under Cross-Entropy](https://arxiv.org/abs/2606.18465)
*Truong Xuan Khanh*

Main category: cs.LG

TL;DR: 固定权值范数时，泛化延迟完全由logit尺度决定（R²=0.97），权值范数仅有1-2%额外影响。


<details>
  <summary>Details</summary>
Motivation: 探究权值范数具体控制什么，通过固定范数并改变输出温度来观察泛化延迟的变化。

Method: 钳制权值范数并改变输出温度，在交叉熵下匹配有效logit尺度；网格搜索和R²分析；记忆控制、float64软最大坍缩审计、无LayerNorm transformer实验。

Result: 延迟几乎完全由logit尺度解释（R²=0.97），范数仅贡献1-2%；均方误差下机制不同；钳制操作本身无伪影。

Conclusion: 近端变量是logit尺度和其驱动的软最大饱和，权值范数仅为上游控制。
...
### [253] [Structured Representation Learning with Locally Linear Embeddings and Adaptive Feature Fusion](https://arxiv.org/abs/2606.18469)
*Somjit Nath,Jackson J Cone,Derek Nowrouzezahrai,Samira Ebrahimi Kahou*

Main category: cs.LG

TL;DR: 受神经科学原理启发，提出一种强化学习框架，通过分离动态与奖励特征并自适应融合，提升学习效率。


<details>
  <summary>Details</summary>
Motivation: 神经科学显示大脑通过低维流形和自适应门控机制编码复杂行为，本文旨在模仿这些原理改进强化学习。

Method: 使用局部线性嵌入捕获状态局部结构，结合标准RL目标提取奖励特征，并通过注意力机制按状态自适应融合。

Result: 在基准任务上，该方法相比传统RL提高了学习效率和整体性能。

Conclusion: 明确建模局部状态结构和自适应特征选择有益于强化学习，这与生物系统中的观察一致。
...
### [254] [The Illusion of Improvement: Reject Inference Strategies in Credit Scoring](https://arxiv.org/abs/2606.18479)
*Bruno Scarone,Ricardo Baeza-Yates*

Main category: cs.LG

TL;DR: 系统评估揭示拒绝推断方法存在结构性失败模式，提出受控探索策略可打破反馈循环，即使低探索率也能有效诊断问题。


<details>
  <summary>Details</summary>
Motivation: 拒绝推断方法广泛用于缓解生存偏差，但其有效性尚不清楚，且标准评估指标在选择性偏差下具有误导性。

Method: 系统评估多种拒绝推断方法，发现准确性提升但召回率下降的失败模式；提出受控探索策略，故意批准部分被拒绝申请并观测真实结果。

Result: 准确性和拒绝质量对探索给出相反建议；低探索率（2-5%）足以诊断严重问题且成本接近零。

Conclusion: 标准评估协议不足以评估生存偏差下训练的模型，需要新的评估方式。
...
### [255] [SFT Overtraining Predicts Rank Inversion via Entropy Collapse Under RLVR](https://arxiv.org/abs/2606.18487)
*Siddharth Aphale,Kelly Liu*

Main category: cs.LG

TL;DR: GRPO训练中，基于pass@1选择SFT检查点可能失败，原因是SFT压缩了rollout分布，导致组奖励方差消失；两阶段诊断可识别高风险检查点。


<details>
  <summary>Details</summary>
Motivation: 标准启发式方法（选择最高pass@1的SFT检查点）在GRPO中可能失效，需要研究失效机制及诊断方法。

Method: 在Qwen2.5-Coder-3B（5深度×3种子）和DeepSeek-Coder-6.7B（4深度×3种子）上实验，测量pre-RL pass@1、entropy及GRPO pass@10；分析组奖励方差公式；测试KL正则化和标签平滑作为补救。

Result: Qwen上pre-RL pass@1随深度增加，但GRPO pass@10从0.806降至0.481（3种子均值），pre-RL entropy与GRPO结果正相关（ρ=+0.69）；DeepSeek上pass@1远高于阈值，GRPO结果压缩而非反转；KL正则化和标签平滑无法挽救失效检查点。

Conclusion: 两阶段诊断（pre-RL entropy筛选+早期GRPO entropy监测）可提前识别高风险检查点并停止训练；失效非简单超参数伪影，可能需更深入方法解决。
...
### [256] [Quantum Annealing Enhanced Reinforcement Learning for Accurate Remaining Useful Lifetime Prediction](https://arxiv.org/abs/2606.18503)
*Manoranjan Gandhudi,Arunkumar V.,G. R. Anil,Gangadharan G. R*

Main category: cs.LG

TL;DR: 提出量子退火增强Q学习（QAQL）用于剩余使用寿命估计，在多个基准上优于经典和量子方法。


<details>
  <summary>Details</summary>
Motivation: 统计退化模型无法捕捉强非线性，数据驱动模型在高维非凸空间易陷入次优解。

Method: 将Q值更新编码为小型QUBO问题，利用量子退火器返回近最优动作分布实现随机探索，抑制过早收敛。

Result: 在NASA C-MAPSS和设备群数据集上，QAQL在六个误差指标上显著优于基线。

Conclusion: 量子退火可作为预测性维护强化学习循环中实用的优化器。
...
### [257] [Beyond AHI: An Interpretable Causal-Discovery-Guided Framework for Sleep Recovery in Connected Health](https://arxiv.org/abs/2606.18506)
*Saba A. Farahani,Elahe Khatibi,Manoj Vishwanath,Amir M. Rahmani,Hung Cao*

Main category: cs.LG

TL;DR: 提出一个基于因果发现的分层睡眠恢复评分（SRS）框架，从多导睡眠图（PSG）中提取生理域，SRS与感知恢复的关联比AHI强2.5倍。


<details>
  <summary>Details</summary>
Motivation: 现有客观睡眠评估（如AHI）无法充分反映患者报告的结果（如疲劳），需要更全面的生理指标。

Method: 使用有向无环图（DAG）学习识别五个生理域（呼吸负担、缺氧负担、睡眠碎片化、睡眠结构、自主调节），结合生理约束和LLM辅助审计进行两阶段筛选。

Result: SRS与感知恢复的关联强度是AHI的2.5倍，跨队列验证显示五个域可重复。

Conclusion: 该解释性、偏差感知的框架为临床睡眠研究和可穿戴健康设备中的恢复建模提供了实用基础。
...
### [258] [Concept Modulation Models: A Unified Framework for Identifiability and Extrapolation](https://arxiv.org/abs/2606.18509)
*Soheun Yi,Yizhou Lu,Chandler Squires,Pradeep Ravikumar*

Main category: cs.LG

TL;DR: 提出概念调制模型（CMMs），通过属性势统一条件潜变量模型的可识别性和外推性保证。


<details>
  <summary>Details</summary>
Motivation: 现有可识别性和外推性保证大多依赖于特定模型，缺乏统一框架。

Method: 构建属性索引的条件生成模型A→Λ→C→X，利用属性势（条件概念密度的对数比）表达约束，分离通用提升步骤与模型特定刚性论证。

Result: 特征一致可提升为潜概念转移约束，外推性等价于属性势恒等式扩展到未见属性。

Conclusion: 得到代数外推准则，识别出可识别性和外推性结果的共同证明对象，结合已有模型特定刚性论证可恢复其结论。
...
### [259] [PSyGenTAB: A Privacy-Preserving Framework for Synthetic Clinical Tabular Data Generation via Constrained Optimization](https://arxiv.org/abs/2606.18518)
...
TL;DR: 提出PSyGenTAB框架，通过约束优化生成合成医疗数据，在保证隐私的同时最大化临床效用。


<details>
  <summary>Details</summary>
Motivation: 医疗AI受限于数据孤岛和隐私法规，现有合成数据方法缺乏对隐私-效用权衡的显式管理。

Method: 将合成数据生成建模为带可配置隐私约束的约束优化问题，使用增广拉格朗日方法求解。

Result: 保留临床特征关系和少数类诊断模式，下游任务性能与真实数据相当，且对成员推断攻击具有鲁棒性。

Conclusion: PSyGenTAB为平衡隐私保护与临床效用提供了原则性框架，支持安全的跨机构AI开发。
...
### [260] [Sparsity Curse: Understanding RLVR Model Parameter Space from Model Merging](https://arxiv.org/abs/2606.18521)
...
### [261] [On the Residual Scaling of Looped Transformers: Stability and Transferability](https://arxiv.org/abs/2606.18524)
*Shaowen Wang,Bingrui Li,Ge Zhang,Wenhao Huang,Shen Yan,Jian Li*

Main category: cs.LG

TL;DR: 循环（权重共享）Transformer需要更强的缩放因子1/N而非1/sqrt(N)来保证训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 先前针对深度残差网络的缩放分析建议ε=1/sqrt(L)，但对于循环架构（权重共享）不足，因为残差更新在迭代间相关。

Method: 推导了多层循环块的因子化参数化ε=λ/(N sqrt(L))，分离了循环内和跨层缩放。

Result: 实验表明1/N缩放改善了可训练性，且比1/sqrt(N)更好。

Conclusion: 最优学习率只依赖于唯一层数L，不依赖于循环次数N，实现了超参数从N小到大的直接迁移。
...
### [262] [Hierarchical Attention via Domain Decomposition](https://arxiv.org/abs/2606.18525)
*Stephan K\"ohler,Oliver Rheinbach*

Main category: cs.LG

TL;DR: 提出基于两层重叠Schwarz区域分解的分层注意力机制，用于有限维算子学习，在一维扩散问题上比全局低秩注意力基线训练更快、精度更高且参数更少。


<details>
  <summary>Details</summary>
Motivation: 观察两层Schwarz区域分解方法结合局部子域修正和全局粗层通信，启发设计分层注意力机制以提升效率和精度。

Method: 用两层加性注意力算子替代全局密集矩阵：局部低秩注意力块作用于重叠子域，配合粗注意力块，通过限制算子R_i、剖分单位权重D_i和粗插值矩阵Φ实现。

Result: 一维扩散问题的数值实验（合成傅里叶右端项）表明，该注意力算子相比全局低秩基线训练更快、近似更精确，且参数显著更少。

Conclusion: 所提区域分解注意力策略有效提升算子学习性能，尤其适用于需平衡局部与全局信息的问题。
...
### [263] [Do as the Romans Do: Learning Universal Behaviors from Heterogeneous Agents](https://arxiv.org/abs/2606.18537)
*Caleb Chang,Davin Win Kyi,Natasha Jaques,Karen Leung*

Main category: cs.LG

TL;DR: 提出GRID方法，从异质演示中提取通用行为，分解奖励为通用和特定部分，训练通用奖励得到通才代理，避免模式平均偏差，在多种任务中有效。


<details>
  <summary>Details</summary>
Motivation: 人类通过观察学习，但异质观察信号冲突，难以确定值得模仿的行为。

Method: GRID将每个代理的奖励函数分解为通用奖励（所有代理共享）和特定奖励（个人偏好），仅训练通用奖励得到通才代理。

Result: 在合成函数分解、多智能体Craftax和自动驾驶模拟实验中，GRID成功解耦奖励结构，优于标准基线，并实现更高效稳定的专业化。

Conclusion: GRID提供了一种新的通才预训练范式，作为微调的下游任务先验，性能优越。
...
### [264] [Effects of sparsity and superposition on loss in simple autoencoders](https://arxiv.org/abs/2606.18538)
*Mriganka Basu Roy Chowdhury,Eric McLaughlin Weiner*

Main category: cs.LG

TL;DR: 本文分析了神经网络中多义性叠加现象的数学基础，为幂激活函数下的L2重建损失提供了稀疏情况下的紧界。


<details>
  <summary>Details</summary>
Motivation: 神经网络可解释性中神经元多义性阻碍了功能理解，叠加现象需要数学验证。

Method: 通过理论分析，在稀疏输入设置下对幂激活函数推导L2重建损失的上界和下界。

Result: 在非常稀疏区域给出了紧的上下界，严格验证了Elhage等人的部分实证结论。

Conclusion: 叠加现象有数学最优性支撑，并为后续研究提供了方向。
...
### [265] [TS-Fault: Benchmarking Time Series Forecasters Against Structural Faults](https://arxiv.org/abs/2606.18539)
*Yuyang Zhao,Lian Xu,Hao Miao,Chenxi Liu,Hao Xue*

Main category: cs.LG

TL;DR: 提出TS-Fault基准，评估时间序列预测模型在结构化故障下的鲁棒性，揭示干净数据准确性与鲁棒性负相关等反直觉结论。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列预测模型通常仅用单个指标在干净数据上排名，忽略了实际部署中结构化故障的影响，而真实故障并非独立同分布噪声，而是具有时间形状、跨变量依赖破坏等结构化事件。

Method: TS-Fault将故障组织为四种模式（观测级vs机制级，单变量vs多变量），通过统一重要性得分将故障注入预测关键窗口，并采用配对干净/损坏协议评估模型。

Result: 在21个模型、6个数据集、4种故障模式和5个难度级别上评估，发现：干净数据准确性与鲁棒性负相关；干净排名在观测级故障下保持但在机制级下重置；所有灾难性失败发生在机制级故障下，基础模型最脆弱。

Conclusion: TS-Fault揭示了现有排名误导性，强调了评估模型在结构化故障下鲁棒性的重要性，代码已公开。
...
### [266] [Correcting Sensor-Induced Distribution Drift with Wasserstein Adversarial Learning](https://arxiv.org/abs/2606.18561)
*Saraa Ali,Vladimir Bocharnikov,Fedor Ratnikov,Mikhail Hushchyn,Artem Ryzhikov,Denis Derkach*

Main category: cs.LG

TL;DR: 提出一种基于Wasserstein-GAN的无监督方法，用于推断传感器退化参数，将生成器作为可学习校准变换，在玩具模型和模拟数据上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 传感器运动和老化的退化会影响下游数据驱动方法的性能与稳定性，需要无监督校准方法。

Method: 采用Wasserstein-GAN框架，生成器作为可学习的校准变换，其权重代表待求退化参数，判别器通过Wasserstein目标提供分布距离信号。

Result: 在追踪探测器玩具模型和Geant4模拟量能器数据上，该方法恢复了单个单元的退化系数（与真实值相关），改善了校准后能量和分布，且随噪声水平增加性能退化。

Conclusion: 对抗分布匹配可以在缺乏直接退化标签的情况下，作为校准策略的数据驱动组件。
...
### [267] [Fair Cognitive Impairment Detection Through Unlearning](https://arxiv.org/abs/2606.18571)
*William Nguyen,Jiali Cheng,Hadi Amiri*

Main category: cs.LG

TL;DR: 提出了一种结合跨模型融合和梯度反转去学习的多模态框架，用于从自发语音中检测轻度认知障碍，在减少人口统计偏差方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 从自发语音中检测MCI具有可扩展性，但现有模型常利用与标签相关的人口统计线索，导致不同子组间性能差异大。

Method: 构建多模态框架，融合语音、文本和图像特征，并采用梯度反转层进行去学习，以抑制共享嵌入中与任务无关的人口统计属性。

Result: 在TAUKADIAL和PREPARE基准上，本方法在多语言多模态MCI分类中达到最优，并显著缩小了性别和语言子组间的性能差距。跨数据集分析表明去学习有助于学习更鲁棒的表示。

Conclusion: 该框架有效提高了MCI检测的公平性和鲁棒性，减少了人口统计偏差。
...
### [268] [Towards Anomaly Detection on Relational Data](https://arxiv.org/abs/2606.18621)
*Shiyuan Li,Yunfeng Zhao,Yue Tan,Qingfeng Chen,Yixin Liu,Shirui Pan*

Main category: cs.LG

TL;DR: 提出RelAD，一种基于重构的框架，通过属性重构和关系边重构检测关系数据库中的异常。


<details>
  <summary>Details</summary>
Motivation: 关系数据异常检测困难：多表属性高维异构，异常线索易被淹没；异常表现为跨外键关系的异常连接模式，现有方法不适用。

Method: 包含条件稀疏门控属性重构和双视角多关系边重构两个模块，通过轻量融合模块整合属性与关系信号得到异常分数。

Result: 在6个基准数据集上，RelAD持续优于其他基线，且效率具有竞争力。

Conclusion: RelAD有效综合属性与关系信息，检测关系数据库中的异常。
...
### [269] [PACT: Preserving Anchored Cores in Task-vectors for Model Merging](https://arxiv.org/abs/2606.18627)
*Ningyuan Shi,Zhipeng Zhou,Hao Wang,Chunyan Miao,Peilin Zhao*

Main category: cs.LG

TL;DR: 模型合并中，现有任务算术范式假设任务知识仅存在于任务向量，但预训练模型存在负载墙维度（LBW）导致性能下降。本文提出PACT方法，通过保留任务特定核心并移除对齐子空间，无缝集成现有方法，显著提升性能并达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有模型合并方法基于任务算术范式，假设任务特定知识仅编码于任务向量中，但预训练模型本身存在任务关键知识（负载墙维度），忽略这些维度会导致任务冲突未完全解决且损害预训练知识。

Method: 提出PACT，首先识别负载墙维度（从标量权重和子空间两个角度），然后通过对齐任务向量的正交补与预训练权重子空间，保留锚定任务核心，并将对齐的子空间分量从任务向量中移除，再应用现有合并算法。同时开发基于随机SVD的高效变体。

Result: 在多个基准上，PACT一致提升主流模型合并方法（如Task Arithmetic、TIES-Merging等），并建立新的最佳性能。

Conclusion: 负载墙维度是模型合并中的关键障碍，PACT通过保留任务特定核心有效解决此问题，且方法可即插即用，显著优于现有方案。
...
### [270] [MetaboNet-Bench: A Multi-modal Benchmark for Glucose Forecasting in Type 1 Diabetes](https://arxiv.org/abs/2606.18640)
*Nathaniel Jeffries,Miriam Wolff,Sam Royston,Elizabeth Healey,Caleb Mayer,David Klonoff,Michael Snyder,Tao Wang*

Main category: cs.LG

TL;DR: 介绍了MetaboNet-Bench，一个用于1型糖尿病患者多模态血糖预测的开源基准测试框架，可比较结合血糖、胰岛素和碳水化合物的算法。


<details>
  <summary>Details</summary>
Motivation: 血糖预测算法缺乏标准化评估基准，难以公平比较，且多数算法仅使用CGM数据而忽略多模态信号。

Method: 构建了MetaboNet-Bench，提供可扩展的开源评估框架，对多种近期模型及自定义多模态时间序列模型进行基准测试。

Result: 增加数据模态的效果取决于模型复杂度，纳入更多临床指标有助于发现未来研究的空白。

Conclusion: 该基准框架有助于标准化评估并推动多模态血糖预测研究。
...
### [271] [BLADE: Scalable Bi-level Adaptive Data Selection for LLM Training](https://arxiv.org/abs/2606.18650)
...
TL;DR: BLADE是一种免Hessian矩阵的双层自适应数据选择框架，通过拉格朗日乘子将双层优化转化为带惩罚的单层目标，实现了动态参考模型，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM数据选择方法中，基于影响的方法计算复杂，而基于过剩损失的方法使用静态参考模型，与训练中的代理模型不一致。

Method: 将基于影响方法的双层优化通过拉格朗日乘子转化为带惩罚的单层目标，避免Hessian计算，并采用无记忆随机块坐标Frank-Wolfe算法进行在线批量选择。

Result: 实验表明BLADE一致优于最先进的数据选择基线，为LLM训练提供了实用方案。

Conclusion: BLADE通过动态参考模型和高效算法，在理论和实践上有效改进了数据选择性能。
...
### [272] [scGTN: Deep Siamese Graph Transformer Network for Single-cell RNA Sequencing Clustering](https://arxiv.org/abs/2606.18672)
*Jinke Wu,Yifan Wang,Siyu Yi,Caiyang Yu,Ziyue Qiao,Nan Yin,Jiancheng Lv,Wei Ju*

Main category: cs.LG

TL;DR: 提出scGTN框架，利用深度孪生图变换网络整合基因表达和细胞间结构信息，实现单细胞RNA测序数据的有效聚类。
...
Method: 将数据构建为图，生成两个增强视图作为孪生输入；采用孪生图变换网络引入最短路径和节点距离；通过最优传输策略自监督聚类。

Result: 在多个基准数据集上，scGTN一致优于现有方法。

Conclusion: scGTN有效整合基因表达和结构信息，提升了scRNA-seq聚类的准确性。
...
### [273] [InTrain: Intrinsic Trainability for Zero-Cost Neural Architecture Search](https://arxiv.org/abs/2606.18676)
*Qinqin Zhou,Fuhai Chen,Jipeng Wu,Zhiwei Chen,Zhikai Hu,Weiwei Cai*

Main category: cs.LG

TL;DR: 提出InTrain，一种无训练架构搜索的理论统一代理，通过几何容量和优化韧性两个组件量化内在可训练性。


<details>
  <summary>Details</summary>
Motivation: 现有零成本代理依赖碎片化启发式，未能捕捉架构可训练性的根本问题。

Method: 定义内在可训练性（InTrain），包含几何容量（激活协方差谱的参与比）和优化韧性（累积梯度健康度），通过尺度不变乘法耦合合成。

Result: 在标准NAS基准和搜索空间上，InTrain达到与集成代理相当的相关性，优于其他单指标方法。

Conclusion: InTrain有效统一了可训练性理论，提供无需训练的架构评估。
...
### [274] [Bounded Context Management for Tabular Foundation Models on Stream Learning](https://arxiv.org/abs/2606.18677)
*Jinmo Lee,Doyun Choi,Moongi Choi,Jaemin Yoo*

Main category: cs.LG

TL;DR: 提出CURE上下文管理策略，用于表格基础模型的流学习，通过不确定性感知准入和冗余感知驱逐，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统流学习需更新模型，而表格基础模型依赖上下文，因此需有效管理上下文。

Method: 提出CURE策略，基于熵门控准入和冗余感知驱逐，满足保留最近、不确定和移除冗余的要求。

Result: 在七个流上，CURE相对经典流学习器提升高达27.0%，在多个TFM骨干上稳健，排名第一。

Conclusion: CURE是一种有效的上下文管理策略，将挑战从模型更新转为上下文管理。
...
### [275] [Dual-Channel Grounded World Modeling (DCGWM): Structural Prevention of Objective Interference Collapse via Heterogeneous External Grounding with Inward-Only Gradient Flow](https://arxiv.org/abs/2606.18688)
*Akshay Hazare*

Main category: cs.LG

TL;DR: 本文识别了联合嵌入预测架构（JEPA）在同时建模物理和社交行为动态时的目标干扰坍缩（OIC）问题，并提出双通道接地世界建模（DCGWM）通过分区潜空间和内向梯度流来结构性地防止OIC。


<details>
  <summary>Details</summary>
Motivation: 现有JEPA基世界模型在面对物理动态（稀疏、高幅、约束满足的梯度修正）和社交行为动态（扩散、分布匹配的修正）两种不同外部信号时，共同学习导致主导通道系统性地坍缩从属通道的表示子空间，且不能通过损失加权解决。

Method: 提出DCGWM：将潜空间分为物理子空间Z_p和行为子空间Z_b，分别通过VICReg风格的物理测量对齐和基于涌现多智能体模拟轨迹的对齐进行更新，仅允许向内向梯度流；通道间接口模块在任务层级耦合子空间而不跨子空间梯度；非对称接地顺应损失用硬铰链惩罚物理违背、用软KL惩罚行为偏差；生成渲染层与潜世界模型架构隔离。

Result: 三个理论结果：分区消除了OIC的梯度干扰路径；每个接地子空间从其对齐目标继承抗坍缩保证；生成隔离在生成目标几何的假定下是必要的。实验验证正在进行中。

Conclusion: 本文建立了问题形式化与架构，实验验证将在未来修订中报告。
...
### [276] [Robust and Interpretable Adaptation of Equivariant Materials Foundation Models via Sparsity-promoting Fine-tuning](https://arxiv.org/abs/2606.18691)
*Youngwoo Cho,Seunghoon Yi,Wooil Yang,Sungmo Kang,Young-woo Son,Jaegul Choo,Joonseok Lee,Soo Kyung Kim,Hongkee Yoon*

Main category: cs.LG

TL;DR: 针对预训练材料基础模型领域校准的需求，提出了一种稀疏性促进的微调方法，仅更新约3%的参数即达到或超越全微调性能，并展示了泛化性和物理可解释性。


<details>
  <summary>Details</summary>
Motivation: 预训练材料基础模型虽能有效近似势能面，但因物理化学多样性和计算设置差异，需领域特化校准。

Method: 利用E(3)等变材料基础模型的结构性质，提出稀疏性促进微调方法，选择性更新参数。

Result: 在能量和力预测任务上，仅更新约3%甚至0.5%参数，性能匹配或超越全微调和等变低秩适应；还成功应用于磁矩预测和磁感知总能建模。

Conclusion: 稀疏性促进微调是一种灵活且可解释的材料基础模型领域特化方法，稀疏模式具有物理意义（如过渡金属d轨道贡献增强）。
...
### [277] [Attention as Frustrated Synchronization](https://arxiv.org/abs/2606.18694)
*Joshua Nunley*

Main category: cs.LG

TL;DR: 提出了一种基于受挫同步的注意力架构（FSN），通过相位失配实现计算，在1M参数规模上超越RoPE-SwiGLU Transformer。


<details>
  <summary>Details</summary>
Motivation: 标准同步网络无法进行有效计算，因此需要利用同步中的结构性偏离来实现注意力机制的计算能力。

Method: 将令牌状态表示为环面上的相位，并通过学习的复数耦合核（包含谐波和一步延迟）实现受挫同步，其中每个组件对应同步文献中的挫败项。

Result: 在1M参数和字符级文本/代码任务上，FSN的验证损失始终低于调优后的Transformer；50轮后FSN收敛至1.5953，而Transformer为1.611。变体（无MLP）性能接近Transformer。在长文本复制任务中，受挫核消除了未受挫基层的性能赤字。

Conclusion: 受挫同步网络在小规模参数下优于Transformer，且优势可扩展至4M参数，表明该架构在低资源场景下具有竞争力。
...
### [278] [Stealthy World Model Manipulation via Data Poisoning](https://arxiv.org/abs/2606.18697)
*Yibin Hu,Xiaolin Sun,Zizhan Zheng*

Main category: cs.LG

TL;DR: 提出SWAAP，首个针对学习世界模型的两阶段数据投毒框架，通过梯度匹配和双级优化实现隐性攻击，导致性能严重下降且难以检测。


<details>
  <summary>Details</summary>
Motivation: 基于模型的强化学习代理使用世界模型进行规划和适应，但微调数据存在投毒风险，现有研究缺乏针对世界模型的投毒攻击方法。

Method: 两阶段框架：第一阶段通过一阶双级优化和过渡梯度定理寻找接近清洁动力学但诱导低回报的目标世界模型；第二阶段通过隐式约束梯度匹配，修改少量微调过渡目标，使训练梯度将模型推向目标，并利用预测误差正则化保持自然近似误差。

Result: 在多个连续控制任务上，SWAAP显著降低代理性能，且投毒过渡接近清洁数据，能规避残差/CUSUM/TRIM等非自适应防御。

Conclusion: 揭示了世界模型适应流程中的实际脆弱性，强调了需要保护世界模型训练数据和所学动力学的鲁棒性方法。
...
### [279] [Contextualizing Biological Language Models across Modalities via Logit-Space Contrastive Alignment](https://arxiv.org/abs/2606.18703)
*Yanjun Shao,Yundi Chen,Yashvi Patel,Aurelien Pelissier,Mar\'ia Rodr\'iguez Mart\'inez*

Main category: cs.LG

TL;DR: 提出LOGICA框架，在输出logit空间进行对比学习，实现上下文条件预测，保留预训练似然接口，在多个生物任务上提升性能。


<details>
  <summary>Details</summary>
Motivation: 预训练生物语言模型的标记概率分布未针对任务特定上下文进行条件化，现有方法扭曲了似然接口。

Method: 通过门控跨模态适配器在输出logit空间进行对比学习，将上下文化标记对数似然转换为匹配分数，对齐基于上下文敏感概率而非嵌入空间。

Result: 在蛋白质-配体结合、TCR-肽活性和药物耐药预测中优于先前最先进方法，耐药预测AUC从~0.55提升至~0.65。

Conclusion: LOGICA保留了标记级接口，有效利用稀疏配对数据，尤其适用于突变排序任务。
...
### [280] [Trainable Photonic Measurement for Physics-Informed PDE Learning](https://arxiv.org/abs/2606.18713)
*Jiale Linghu,Hao Dong,Yangshuai Wang*

Main category: cs.LG

TL;DR: 提出一种光子量子神经场，将坐标编码为可训练的光学相位，通过多光子Fock空间干涉混合，从光子数测量解码，作为物理信息残差最小化的可训练表示。在七类PDE基准中，当残差导数放大相位失配时，光子场精度最高，误差可降低一个数量级且参数仅为经典方法的四分之一。


<details>
  <summary>Details</summary>
Motivation: 光子量子机器学习在科学机器学习中的作用尚未充分探索，而物理信息神经场为利用相位、频率和导数结构提供了自然场景。

Method: 构建光子量子神经场：坐标变成可训练光学相位，经多光子Fock空间干涉混合，从光子数测量解码；优化光子电路本身作为神经场表示，而非固定特征映射或硬件加速器；在物理信息残差最小化框架下训练。

Result: 在椭圆、波动、非线性色散和逆PDE等七个基准上，平滑区域经典方法足够，但硬区域（残差导数放大相位失配）光子场误差最低，优势达一个数量级，且参数约为经典方法的四分之一。冻结、打乱控制和噪声测试证实增益来自学习到的干涉和稳定的Fock概率读出。

Conclusion: 光子量子测量可作为科学机器学习的一种表示学习原则，尤其在相位敏感问题中表现优异。
...
### [281] [Graph Grounded Cross Attention Transformer Neural Network for Structurally Constrained Full Event Sequence Generation in Predictive Process Monitoring](https://arxiv.org/abs/2606.18726)
*Fang Wang,Ernesto Damiani*

Main category: cs.LG

TL;DR: 提出GGATN模型，结合全局过程图与Transformer注意力，一次性生成完整事件序列，并通过图约束解码保证可行性，在六个基准日志上取得优于LLM的效果。


<details>
  <summary>Details</summary>
Motivation: 现有预测过程监控主要处理组件任务（如下一活动、剩余时间等），缺乏统一的全事件序列生成方法，且结构约束确保可行性、时序性等具有挑战。

Method: 使用全局过程图作为结构化记忆，Transformer自注意力编码序列位置，图接地交叉注意力注入拓扑；单次生成活动、时间戳、长度和属性，再通过Viterbi式图约束解码产生可行路径并显式终止。

Result: 在六个事件日志上，生成质量（序列相似度、Damerau-Levenshtein相似度、二元组控制流相似度、持续时间分布）优于LLM基线，零幻觉活动，零序列级属性不一致。

Conclusion: 全局图编码器提供稳定结构先验；图结构、序列上下文、反馈修正和约束解码共同塑造生成过程，可解释性强。
...
### [282] [Low-Cost Neuromorphic Fall Detection Using Synthetic Event Data and Hybrid SNNs](https://arxiv.org/abs/2606.18732)
*Guillermo Rojas,Gonzalo Soto,Daniel Yunge*

Main category: cs.LG

TL;DR: 本文提出了融合脉冲神经网络（SNN）和卷积神经网络（CNN）的混合模型，利用从智能手机视频模拟的事件相机数据（DVS）进行跌倒检测，在保持精度的同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 利用SNN的能量效率和时空处理能力，结合CNN特征提取优势，解决传统机器学习模型在跌倒检测任务中效率与精度的平衡问题。

Method: 将视频帧转换为事件数据，构建SNN与CNN混合模型，并在多个数据集上模拟评估，与传统模型对比。

Result: 混合模型在不牺牲准确率的情况下实现了显著的效率提升。

Conclusion: SNN与DVS技术的结合在复杂现实任务中具有潜力，为高效跌倒检测提供了新方案。
...
### [283] [Private Learning with Public Feature Conditioning](https://arxiv.org/abs/2606.18773)
*Shuli Jiang,Walid Krichene,Nicolas Mayoraz*

Main category: cs.LG

TL;DR: 提出Cond-DP，一种利用公共特征矩阵结构加速差分隐私回归收敛的算法，在标签差分隐私场景下优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有标签差分隐私或半敏感特征设置主要针对分类任务，回归任务的有效方法尚不充分。

Method: Cond-DP基于DPSGD，引入数据驱动的条件矩阵以重塑优化景观，加速收敛；条件矩阵从公共特征中构建，不增加隐私成本。

Result: 在凸、强凸和非凸设定下均给出收敛保证；经验上在多种数据集和模型架构下一致优于现有基线。

Conclusion: Cond-DP有效利用公共特征结构提升差分隐私回归性能，且具有理论保证和实际鲁棒性。
...
### [284] [RouteJudge: An Open Platform for Reproducible and Preference-Aware LLM Routing](https://arxiv.org/abs/2606.18774)
*Guannan Lai,Haoran Hu,Han-Jia Ye*

Main category: cs.LG

TL;DR: 提出RouteJudge在线成对偏好评估框架和ORBIT工具箱，用于LLM路由系统的路由级决策质量评估和标准化开发。


<details>
  <summary>Details</summary>
Motivation: 现有评估关注模型响应质量，但缺乏对路由决策质量的直接评估，因此需要路由器级别的评估方法和统一开发工具。

Method: 通过匿名成对比较用户对候选模型响应的偏好，将偏好归因于路由策略；开发ORBIT工具箱提供模块化接口，标准化路由流程。

Result: 构建了公开平台RouteJudge并发布ORBIT工具箱，支持预算感知、偏好感知和任务条件分析，研究者可提交路由方法进行在线评估。

Conclusion: RouteJudge和ORBIT为LLM路由系统的评估与开发提供了统一、可扩展的框架，促进路由算法的比较与优化。
...
### [285] [Online Distributional Prediction via Latent Cluster Geometry Under Drift and Corruption](https://arxiv.org/abs/2606.18778)
*Navyansh Mahla,Prateek Chanda,Ganesh Ramakrishnan*

Main category: cs.LG

TL;DR: 提出一种基于潜在聚类几何和吉布斯准后验的在线分布预测方法，通过可逆跳跃MCMC采样，并引入重启变体处理漂移和腐败，实现亚线性累积Wasserstein遗憾。


<details>
  <summary>Details</summary>
Motivation: 现有在线学习多聚焦点估计，但许多应用需预测完整分布；非平稳流中的漂移和腐败挑战尚未充分解决。

Method: 用可变大小的中心配置表示候选分布，通过吉布斯准后验平均进行预测，采用可逆跳跃MCMC采样后验；为应对漂移，提出时间局部化重启变体。

Result: 在假设稳定潜在几何、亚线性运输行动和腐败预算等条件下，重启预测器实现亚线性累积Wasserstein-1遗憾。

Conclusion: 方法无需参数化流模型，能有效处理漂移和腐败，并提供理论保证。
...
### [286] [Bayesian Anytime Pareto Set Identification for Multi-Objective Multi-Armed Bandits](https://arxiv.org/abs/2606.18785)
*Lennert Saerens,Bram Silue,Eleni Litsa,Peter Vrancx,Pieter Libin*

Main category: cs.LG

TL;DR: 提出首个用于帕累托集识别的任意时间多目标多臂赌博机算法TTPFTS，采用贝叶斯方法，在合成环境和分子发现中表现优异，并证明渐近正确性。


<details>
  <summary>Details</summary>
Motivation: 识别帕累托最优解对多目标决策至关重要。

Method: Top-Two Pareto Front Thompson Sampling (TTPFTS) 算法，结合贝叶斯方法。

Result: 在合成环境和超大型分子库中优于固定预算算法；提出的不确定性量化指标有效代理真实性能。

Conclusion: 算法具有渐近正确性，且实用性强。
...
TL;DR: 提出SC-GRPO，通过自条件化轨迹的KL散度作为梯度权重，在数学、代码和智能体任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法（如GRPO）对所有token分配均匀梯度，浪费在常规token上，而关键推理步骤梯度不足；现有token级信用分配方法依赖外部资源（如过程奖励模型或教师模型），限制了纯RLVR场景的适用性。

Method: SC-GRPO利用模型在自条件化（基于自身已验证轨迹）下的每token KL散度作为GRPO梯度的乘法权重，无需外部资源。

Result: 在五个基准（数学、代码、智能体）上，SC-GRPO比GRPO提升8.1%，比DAPO提升5.9%，且OOD性能更强，甚至优于On-Policy Distillation。

Conclusion: SC-GRPO是一种简单有效的token级信用分配方法，在纯RLVR设置下显著提升性能，且无需额外监督或模型。
...
### [288] [Reinforcement Learning Foundation Models Should Already Be A Thing](https://arxiv.org/abs/2606.18812)
*Abdelrahman Zighem,Jill-J\^enn Vie*

Main category: cs.LG

TL;DR: 提出利用合成马尔可夫决策过程（MDP）和固定大小充分统计量构建强化学习基础模型，概念验证表明无需微调即可在在线和离线基准上取得竞争性表现。


<details>
  <summary>Details</summary>
Motivation: 结构化领域（如强化学习）缺乏互联网规模数据，而合成数据可替代，但强化学习是明显空白，尚无工作将先验设计作为主要目标。

Method: 基于MDP的固定大小充分统计量（表格形状），采用类似TabPFN的注意力架构，用策略头替换监督目标，在完全合成的MDP上预训练模型。

Result: 该模型在保留的表格基准上，在线情境中比UCB-VI和表格Q-learning用更少episodes解决任务，离线情境下与VI-LCB竞争。

Conclusion: 该方法定义了强化学习基础模型的研究议程，证明合成先验和架构迁移的有效性。
...
### [289] [Maturing Markov Decision Processes: Decision Making under Increasing Information and Shrinking Action Sets](https://arxiv.org/abs/2606.18820)
*Jiaxi Liu,Aiping Yang,Yuhang Yang,Shuqi Zhang,Zewei Dong,Jiangming Yang,Xuebin Chen*

Main category: cs.LG

TL;DR: 提出了成熟马尔可夫决策过程（MMDPs），显式建模信息与动作的不对称性，并通过实验证明其能提高学习效率，尤其在大规模问题中。


<details>
  <summary>Details</summary>
Motivation: 标准MDP公式忽略了顺序决策中信息逐渐丰富与动作因截止时间而失效的不对称性，导致无法区分紧急与可延迟决策。

Method: 引入MMDPs，基于信息-动作不对称性构建，提出过期动作优先原则，并设计结构感知的强化学习框架，包括阶段感知策略、过期动作抽象和搜索增强蒸馏学习。

Result: 在多供应商补货、简化现金管理以及生产规模仿真实验中，显式建模该不对称性提高了学习效率，且随着问题规模扩大价值更显著。

Conclusion: 显式建模信息-动作不对称性能够有效提升强化学习在顺序决策问题中的表现，尤其适用于大规模复杂场景。
...
### [290] [GateMem: Benchmarking Memory Governance in Multi-Principal Shared-Memory Agents](https://arxiv.org/abs/2606.18829)
*Zhe Ren,Yibo Yang,Yimeng Chen,Zijun Zhao,Benshuo Fu,Zhihao Shu,Bingjie Zhang,Yangyang Xu,Dandan Guo,Shuicheng Yan*

Main category: cs.LG

TL;DR: 提出了GateMem，一个用于多用户共享内存代理的基准测试，涵盖医疗、办公、教育和家庭领域，评估效用、访问控制和遗忘能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理内存基准主要针对单用户，忽视了医院、工作场所等共享场景中多用户写入和查询内存的需求，这些场景需要治理和回忆。

Method: 构建GateMem基准，包含长篇幅多方对话、增量内存注入、隐藏检查点、结构化评判和泄漏目标标注，联合评估长期请求的效用、上下文授权和遗忘。

Result: 在多种基线和骨干模型上，没有方法能同时实现强效用、稳健访问控制和可靠遗忘。长上下文提示方法治理得分高但成本大，检索和外部记忆方法成本低但仍有信息泄漏。
...
TL;DR: 提出一种名为TRUST的框架，允许用户指定期望的预测置信度，直接搜索最小变化以满足置信度目标，从而生成更鲁棒和可解释的反事实解释。


<details>
  <summary>Details</summary>
Motivation: 现有反事实解释方法通常只考虑最小化改变以翻转决策，但忽略了决策者依赖置信度阈值和风险边际，导致反事实在噪声或模型变化下脆弱不稳定。

Method: TRUST框架结合概率Tsetlin机器和贝叶斯优化，直接搜索满足用户定义置信度目标的最小输入变化，通过概率子句结构关联预测置信度与决策规则稳定性。

Result: 实验表明，目标置信度反事实比传统边界方法更鲁棒和可解释，在多个基准上实现完美鲁棒性同时保持低成本，例如在Haberman数据集上L2距离0.10，置信度0.92。

Conclusion: TRUST通过显式控制置信度和揭示规则级稳定性，为高风险决策支持提供可操作的补救措施。
...
### [292] [Seed-Guided Semi-Supervised Clustering by A-Contrario Anomaly Detection](https://arxiv.org/abs/2606.18833)
*Nassir Mohammad*

Main category: cs.LG

TL;DR: 提出一种基于统计对偶性的半监督聚类框架，将聚类视为异常检测的逆问题，利用少量种子标签和期望值阈值实现鲁棒聚类。


<details>
  <summary>Details</summary>
Motivation: 现有聚类算法在噪声环境下难以鲁棒定义聚类，分区算法易过度分配异常点，密度方法对全局参数敏感。

Method: 基于a-contrario统计和格式塔邻近原理，定义聚类为无异常的最大点子集；通过Perception算法（期望阈值E<1）自动识别异常，采用迭代“聚类-排除”机制，由种子标签初始化并扩展聚类。

Result: 在合成和真实数据集上，每类仅需10-30个种子即可达到竞争或最优性能，且算法对样本数和维度线性可扩展。

Conclusion: 该框架有效整合聚类与异常检测的对偶性，低标注成本下实现鲁棒聚类，适用于噪声环境。
...
### [293] [Identifying Structural Biases from Causal Mechanism Shifts](https://arxiv.org/abs/2606.18834)
*Praharsh Nanavati,Jilles Vreeken,David Kaltenpoth*

Main category: cs.LG

TL;DR: 本文研究如何通过因果机制变化来识别隐藏混杂和选择偏差，提出基于互信息的可检验准则和StruBI算法，实验表明该方法有效。


<details>
  <summary>Details</summary>
Motivation: 现有因果发现方法假设数据i.i.d.且无未测量变量，但实际中这些假设常被违反，导致推断不准确。因此需要研究如何识别隐藏混杂和选择偏差。

Method: 通过考虑不同环境下变量的机制变化，利用结构偏差导致依赖的机制变化这一现象，形式化为基于互信息的可检验准则，并开发StruBI算法来识别哪些节点受到何种偏差。

Result: 在合成和真实数据上，StruBI准确恢复了受影响变量集和偏差类型，大幅度超越现有方法。

Conclusion: 提出的方法能够有效检测隐藏混杂和选择偏差，通过机制变化实现准确识别。
...
### [294] [Semantic Robustness Certification for Vision-Language Models](https://arxiv.org/abs/2606.18839)
*Peiyu Yang,Paul Montague,Feng Liu,Andrew C. Cullen,Amardeep Kaur,Christopher Leckie,Sarah M. Erfani*

Main category: cs.LG

TL;DR: 提出一种新框架，用于认证视觉语言模型在语义级变换下的鲁棒性，利用文本提示作为语义代理，无需额外数据。


<details>
  <summary>Details</summary>
Motivation: 现有认证框架多关注几何或像素级变换，缺乏语义级变换的认证。

Method: 利用VLM的开放词汇能力，用文本提示作为语义代理构造参数化变换，通过闭式决策边界定量认证预测不变的变换区间。

Result: 实验表明框架能在多种场景下认证不同语义变化的鲁棒性。

Conclusion: 首个无需额外数据即可认证语义级鲁棒性的框架，实用性强。
...
### [295] [Learning from Your Own Mistakes: Constructing Learnable Micro-Reflective Trajectories for Self-Distillation](https://arxiv.org/abs/2606.18844)
*Zhilin Huang,Hang Gao,Ziqiang Dong,Yuan Chen,Yifeng Luo,Chujun Qin,Jingyi Wang,Yang Yang,Guanjun Jiang*

Main category: cs.LG

TL;DR: 提出TAPO方法，通过显式构建包含错误诊断与纠正的轨迹，将自蒸馏从分布对齐提升为轨迹构建，显著提升大模型推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有自蒸馏方法通过KL散度对齐隐式分布，缺乏对模型具体错误的诊断与针对性纠正，导致模型仅模仿特权分布而无法修正个体失败模式。

Method: TAPO在RL训练中同时收集正确与错误轨迹，利用对比结构构造微反射修正：保留错误推理至失败点，插入自然语言诊断并结合正确参考进行纠正；同时引入难度感知候选选择与解耦优势估计防止梯度污染。

Result: 在AIME 2024、AIME 2025和HMMT 2025上，TAPO在相同训练步数下一致优于GRPO，并增强了首次推理与错误纠正能力。

Conclusion: TAPO通过显式轨迹构建与细粒度纠正信号，有效替代隐式分布对齐，为自蒸馏提供了更有效的推理优化框架。
...
### [296] [Investigating Inductive Biases for Machine Learning Emulation of Sudden Stratospheric Warmings in Idealised Isca Simulations](https://arxiv.org/abs/2606.18857)
*Oskar Bohn Lassen,Simon Driscoll,Stephen I. Thomson,Sebastian Schemm,Francisco C. Pereira*

Main category: cs.LG

TL;DR: 机器学习仿真器在天气预测中应用广泛，但能否利用平流层变异性等可预测性锚点仍存挑战。研究对比不同架构对平流层突然增温动力学的仿真，发现三维垂直耦合是关键归纳偏置，但低预测误差不能保证物理准确的波-流相互作用。


<details>
  <summary>Details</summary>
Motivation: 探索机器学习仿真器能否利用平流层变异性等可预测性锚点，以提升次季节到季节时间尺度的预测技巧。

Method: 使用配对的理想化Isca模拟，仅改变波-2加热扰动，测试卷积、变换器、图神经网络架构在一步预测中的表现。

Result: 平流层动态平静时模型差异小，但类似平流层突然增温的变异性活跃时差异显著。显式三维垂直耦合是关键归纳偏置，但低预测误差不能保证物理准确的波-流相互作用。

Conclusion: 显式三维垂直耦合对平流层动力学的机器学习仿真重要，但需改进物理一致性诊断。
...
### [297] [Scaling Learning-based AEB with Massive Unlabeled Data](https://arxiv.org/abs/2606.18864)
...
TL;DR: 提出基于元反馈半监督学习的AEB扩展方法，通过噪声感知解耦和运动学门控伪标签稳定训练，实现从1M到1B窗口数据的规模化，并在实际部署中达到100:1的正误触发比和35%的无事故里程提升。


<details>
  <summary>Details</summary>
Motivation: 生产限制下如何利用海量未标记车队数据扩展基于学习的自动紧急制动（AEB）系统。

Method: 采用元反馈半监督学习（MF-SSL）框架，包含噪声感知解耦（移除模糊锚点）和运动学门控伪标签（抑制冲突风险），稳定教师模型更新。

Result: 未标记数据从1M扩展到1B窗口时性能持续提升；部署到数十万车辆，正误触发比超过100:1，无事故驾驶里程比纯规则基线提升35%。

Conclusion: 所提稳定化MF-SSL框架能有效利用大规模未标记数据提升AEB安全性能，同时保持舒适性。
...
### [298] [Strategic Feature Selection](https://arxiv.org/abs/2606.18867)
...
TL;DR: 通过特征选择和岭正则化联合优化来缓解战略行为的影响。


<details>
  <summary>Details</summary>
Motivation: 在高风险领域（如医疗）中，算法预测需考虑战略操纵，但决策者常受限于调整现有管道中的粗粒度杠杆（如特征选择与正则化），因此需要研究这些杠杆的最佳组合。

Method: 正式研究战略分类中的特征选择与岭正则化的交互，提供特征子集在最优正则化下的性能细粒度表征，并开发联合选择特征集和正则化水平的实用算法。

Result: 发现仅基于可操纵性排除特征通常次优，通过真实医疗支付案例展示了算法可指导粗策略杠杆设计。

Conclusion: 提供了一个原则性且实用的框架来减轻算法决策系统中战略行为的影响。
...
### [299] [Domain-Shift Aware Neural Networks for Unbalance Characterization in Rotating Systems](https://arxiv.org/abs/2606.18882)
*Bernardo Feij\'o Junqueira,Claudio Kiyoshi Umezu,Bruno Bilhar Karaziack,Tomaz Junior,Daniel Alves Castello*

Main category: cs.LG

TL;DR: 本研究提出了一种领域偏移感知神经网络，用于在不同运行条件下估计旋转轴的不平衡质量，通过最大均值差异策略对齐特征表示，在未知领域差异情况下提高了预测精度。


<details>
  <summary>Details</summary>
Motivation: 旋转轴在不同运行条件下的不平衡质量估计受到领域偏移影响，传统方法难以处理未知的物理行为变化，需要领域自适应方法。

Method: 构建领域偏移感知神经网络，采用最大均值差异策略在源域和目标域之间对齐特征表示，实现回归任务的领域自适应。

Result: 实验表明，明确处理领域偏移能显著提高预测精度，尤其在系统物理行为和领域差异源未知或超出训练条件时效果突出。

Conclusion: 领域偏移感知模型在结构健康监测的回归任务中具有潜力，能有效应对未知领域差异带来的挑战。
...
### [300] [Anomaly Detection for Sparse and Irregular Multivariate Time Series with Latent SDEs](https://arxiv.org/abs/2606.18898)
*Martin Uray,Dominik Geng,Florian Graf,Stefan Huber,Roland Kwitt*

Main category: cs.LG

TL;DR: 提出基于潜在随机微分方程的生成方法，处理稀疏、不规则采样和缺失数据的多变量时间序列异常检测。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设均匀采样，无法处理真实世界中的稀疏、不规则或部分观测数据。

Method: 利用潜在SDE将观测时间序列投影到连续时间随机动力系统，处理缺失和不规则采样，并捕捉循环行为。

Result: 在六个异常检测基准数据集上排名第一，且在高稀疏度下保持鲁棒性，而基线方法性能显著下降。

Conclusion: 潜在SDE为多变量时间序列异常检测提供了自然的归纳偏置，尤其适用于真实世界的不规则性。
...
### [301] [REVES: REvision and VErification--Augmented Training for Test-Time Scaling](https://arxiv.org/abs/2606.18910)
*Yuanxin Liu,Ruida Zhou,Xinyan Zhao,Amr Sharaf,Hongzhou Lin,Arijit Biswas,Mohammad Ghavamzadeh,Zhaoran Wang,Mingyi Hong*

Main category: cs.LG

TL;DR: 提出REVES框架，通过两阶段迭代优化LLM多步推理，利用中间错误步骤进行针对性训练，提升测试时推理能力。


<details>
  <summary>Details</summary>
Motivation: 标准后训练优化单次目标，与多步推理动态不匹配；现有方法直接优化多步轨迹，未充分利用中间高质错误。

Method: 两阶段迭代框架，交替进行在线数据/提示增强和策略优化，将中间步骤转化为解耦的修订和验证提示。

Result: LiveCodeBench上+6.5 vs RL基线，+4.0 vs 多轮训练；circle packing上4B模型匹配SOTA；数学和约束问题改进。

Conclusion: 该方法高效生成离策略数据，减少长程采样计算，提升纠正能力和泛化性。
...
### [302] [Some Complexity Results for Robustness Verification for Binarized Neural Networks](https://arxiv.org/abs/2606.18918)
*Harshit Goyal,Sudakshina Dutta*

Main category: cs.LG

TL;DR: 研究二值化神经网络的可满足性和鲁棒性验证的计算复杂性，证明了可满足性为NP完全，并利用均匀遮挡下的分段常数特性提出了多项式时间鲁棒性检查算法。


<details>
  <summary>Details</summary>
Motivation: 验证二值化神经网络的计算复杂性。

Method: 通过SAT归约证明NP完全性，分析均匀遮挡导致的输出分段常数结构。

Result: 可满足性为NP完全；均匀遮挡下存在多项式时间的鲁棒性检查算法。

Conclusion: 二值化神经网络的验证问题具有明确复杂性界限，均匀遮挡可简化鲁棒性验证。
...
### [303] [GrapNet: A Programmable Dynamic-Architecture Neural Graph Substrate](https://arxiv.org/abs/2606.18923)
*Zirong Li*

Main category: cs.LG

TL;DR: GrapNet将神经网络视为可编程图结构，支持编辑、冻结等操作，作为可编辑神经图基板，在类增量学习任务中优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 固定张量神经网络缺乏可编程性作为一等接口，编辑关系、冻结子图等操作需进行临时参数调整。

Method: GrapNet将图视为网络架构和可执行程序，每个计算节点拥有子节点引用和可训练分配向量，支持删除关系、冻结、结构编辑、分组注意力路由等操作，并通过向量值父接口与常规模块组合。

Result: 在Split Fashion-MNIST中，GrapNet+ER达到63.16%准确率，比参数更大的MLP+ER高12.08个百分点（p=1.3e-5）；在Split CIFAR-10中，使用冻结ResNet-18编码器，准确率比MLP-256高3.81个百分点（p=0.0026）。

Conclusion: GrapNet作为可编辑神经图基板，其核心价值是结构可编程性及忠实执行视图。
...
### [304] [Zero-Shot Active Feature Acquisition via LLM-Elicitation](https://arxiv.org/abs/2606.18933)
*Binyamin Perets,Natalie Mendelson,Shiran Vainberg,Yehuda Chowers,Shai Shen-Orr,Shie Mannor*

Main category: cs.LG

TL;DR: 提出一种零样本主动特征获取框架，利用大语言模型提供领域知识，通过马尔可夫随机场的充分统计量进行特征选择，在IBD患者队列上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 主动特征获取依赖大量标记数据，而大语言模型虽有无监督知识但缺乏顺序规划能力，需要将知识与决策分离。

Method: 通过规范询问大语言模型获取非偏差和成对协变（马尔可夫随机场的充分统计量），并采用最大熵闭合解决判别统计量的规范歧义，应用于二元分类和top-k识别。

Result: 在IBD患者队列上，框架在真实标签和自身提取信念上均优于大语言模型，在最难患者上的top-k获取策略显著优于所有现有方法。

Conclusion: 该框架有效利用大语言模型的领域知识实现零样本主动特征获取，在临床诊断等场景具有潜力。
...
### [305] [Be Your Own Teacher: Steering Protein Language Models via Unsupervised Reward Optimization](https://arxiv.org/abs/2606.18961)
*Lanqing Li,Shentong Mo,Yang Yu,Pheng-Ann Heng*

Main category: cs.LG

TL;DR: 提出无监督奖励优化框架，使蛋白质语言模型无需湿实验或偏好数据即可实现可控生成。


<details>
  <summary>Details</summary>
Motivation: 传统PLM后训练依赖昂贵的湿实验或偏好数据集，存在监督瓶颈。

Method: 构建任务无关奖励（结合模型不确定性与表示模型语义一致性），并设计SRO和BRO两种离线算法优化RLHF目标。

Result: 在组合分布外提示上，两种方法显著优于DPO、KTO，接近oracle性能，且pass@k覆盖率更高。

Conclusion: 该框架使PLM通过自身生成经验自我改进，为缺乏标签或反馈的场景提供可扩展的调控路径。
...
### [306] [Online Reward-Punishment Learning from Fixed-Channel Perceptual Event Streams without Environment Rewards](https://arxiv.org/abs/2606.18963)
*Zirong Li*

Main category: cs.LG

TL;DR: 提出一种无标量奖励的在线奖惩学习框架OHIRL，通过四个模块分离预测、残差动态、轨迹评估和策略更新，在多种任务中达到高准确率。


<details>
  <summary>Details</summary>
Motivation: 环境不提供标量奖励或评估标签，智能体仅通过固定通道的感知数据推断价值，需要从状态转移后果中学习奖惩。

Method: OHIRL包含四个模块：M_psi预测下一感知包，D_omega建模残差动态，C_eta作为固定内部轨迹评估器（采用恢复正向、持续/增长负向的残差调节方向），B_xi利用评估证据进行策略更新。通过系数来源审计验证等单元、原始相等及随机单调变体保留92%以上动作排序，符号翻转则保留0%。条件误差分解分离证据估计误差和策略优化误差。

Result: 在2x2-XOR任务中，药物和辣椒在不同视觉上下文中获得相反价值；B_xi达到0.952平衡奖励符号准确率。在线交错审计中，M_psi保留R²=0.907，B_xi符号准确率0.940，策略最优动作准确率0.979，而对照实验（即时分数、预测误差奖励、洗牌目标、零奖励、误差缩减控制）均失败。隐藏奖励CartPole和Taxi控制、公共上下文无泄漏审计及模块消融进一步验证信息边界和组件必要性。

Conclusion: OHIRL能够在无奖励信号下通过状态转移推断奖惩，各模块功能明确且必要，方法鲁棒且性能优异。
...
### [307] [EfficientRollout: System-Aware Self-Speculative Decoding for RL Rollouts](https://arxiv.org/abs/2606.18967)
...
TL;DR: EfficientRollout是一个系统感知的自推测解码框架，通过耦合策略的自推测解码和适应性的切换策略，加速RL后训练中的rollout生成。


<details>
  <summary>Details</summary>
Motivation: RL后训练中rollout生成是延迟瓶颈，推测解码虽可加速但直接应用面临策略变化和批次大小变化导致的效率问题。

Method: 从目标模型导出量化草稿模型实现自推测解码，无需额外训练；设计系统感知的推测解码切换策略和接受感知的草稿长度自适应。

Result: 相比加速的自回归基线，rollout延迟降低19.6%，端到端延迟降低12.7%，且保持模型质量。

Conclusion: EfficientRollout有效加速RL rollout，不损害模型质量，为RL后训练提供高效解码方案。
...
### [308] [A Controlled Benchmark of Quantum-Latent GAN Augmentation for Brain MRI](https://arxiv.org/abs/2606.18970)
*Syed Mujtaba Haider,Silvia Figini*

Main category: cs.LG

TL;DR: 通过严格基准测试，量子生成增强在脑MRI分类中并未超越经典方法或仅使用真实数据，二者表现无显著差异。


<details>
  <summary>Details</summary>
Motivation: 医学图像分类受限于标注数据，量子生成模型被提出用于数据增强，但先前研究缺乏严格对照。

Method: 建立受控基准：将图像编码到KL正则化潜在空间，使用条件WGAN-GP，比较近参数量的量子（1648）与经典（1632）生成器，在5%-100%标注数据下训练分类器，进行8次随机种子配对显著性检验及多样性和分布分析。

Result: 所有数据比例下，任何增强方法均未显著优于仅用真实数据；量子与经典生成器统计上无差异；低数据时的改善表现为正则化而非真实数据扩展，合成样本分布外且模式崩溃。

Conclusion: 量子生成增强在脑MRI分类中未提供实质性优势，应谨慎对待以往报告的性能提升。
...
### [309] [DIPHINE: Diffusion-based $\Phi$-ID Neural Estimator](https://arxiv.org/abs/2606.18997)
*Simon Pedro Galeano Munoz,Mustapha Bounoua,Giulio Franzese,Pietro Michiardi,Maurizio Filippone*

Main category: cs.LG

TL;DR: 提出DIPHINE，首个利用扩散模型估计ΦID所有互信息项的神经估计器，通过Möbius反转得到16个信息原子，适用于连续非高斯系统。


<details>
  <summary>Details</summary>
Motivation: 现有ΦID方法仅限于高斯或离散系统，无法应用于连续非高斯动力系统。

Method: 基于分数扩散模型，从单个摊销网络联合估计所有所需互信息项，并通过Möbius反转恢复16个原子。

Result: 在合成基准上准确恢复真实原子，性能优于现有互信息估计器，并在实际数据中提取了生理可解释的信息动态结构。

Conclusion: DIPHINE有效且无需分布假设，拓展了ΦID的应用范围。
...
### [310] [FoMoE: Breaking the Full-Replica Barrier with a Federation of MoEs](https://arxiv.org/abs/2606.19025)
*Lorenzo Sani,Zeyu Cao,Meghdad Kurmanji,Alex Iacob,Andrej Jovanovic,Yan Gao,Wanru Zhao,Nicholas D. Lane*

Main category: cs.LG

TL;DR: FoMoE通过划分专家层打破全副本范式，降低通信成本并提升吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有分布式训练方法要求每个站点有完整模型副本，导致内存和通信开销大。FoMoE旨在解决这一低效问题。

Method: FoMoE将专家层分区到不同工作节点，并引入跳过令牌机制以减少通信。

Result: 通信成本降低高达1.42倍（相比高效基线）和45.44倍（相比DDP）；吞吐量提升1.4倍；在100B参数规模下通过系统建模验证了通信/内存优势。

Conclusion: FoMoE通过分区专家层和跳过令牌机制，有效支持跨数据中心的大规模MoE训练，降低资源需求。
...
### [311] [A Hybrid LSTM--Vision Transformer Architecture for Predicting HRRR Forecast Errors](https://arxiv.org/abs/2606.19026)
*David Aaron Evans,Jay C. Rothenberger,Kara J. Sulia,Nick P. Bassill,Chris D. Thorncroft*

Main category: cs.LG

TL;DR: 开发了一种混合LSTM-Vision Transformer框架，结合时间序列学习和大气垂直剖面数据，显著提高了数值天气预报误差的预测能力，尤其在降水预报误差上提升约两倍。


<details>
  <summary>Details</summary>
Motivation: 现有LSTM在复杂垂直大气演化时期性能下降，需要引入垂直结构信息以改进预报误差预测。

Method: 提出LSTM-ViT混合框架，融合地面观测的时间序列学习和纽约州Mesonet廓线网的大气垂直剖面数据。

Result: 对于降水、10米风速和2米温度预报误差，LSTM-ViT均优于基线LSTM；降水误差预测技巧提升约两倍，尤其在短预报时效和PBL活跃期。

Conclusion: 结合时间序列学习与垂直注意机制为改进业务NWP系统的预报误差预测提供了物理上有意义的途径，增强了模型偏差和预报可信度的指导。
...
### [312] [Geometric and Stochastic Analysis of Discontinuities in Sparse Mixture-of-Experts](https://arxiv.org/abs/2606.19036)
*Tho Tran Huu,Huu-Tuan Nguyen,Thien-Hai Nguyen,Nhat-Tri Ho,Viet-Hoang Tran,Tho Quan,Tan Minh Nguyen*

Main category: cs.LG

TL;DR: 稀疏专家混合架构存在不连续性，本文通过几何与随机分析揭示了不同阶数不连续性的体积渐近性及随机路径首次命中规律，并提出轻量平滑机制以提升连续性与性能。


<details>
  <summary>Details</summary>
Motivation: 稀疏专家混合架构中的Top-k专家选择导致输入空间不连续，影响模型稳定性，需要深入理解不连续性的结构并减轻其影响。

Method: 通过测度论切片论证建立各阶不连续性体积渐近估计，利用扩散过程建模随机扰动，首次命中概率及停留时间界分析，并提出软引入邻近专家的平滑机制。

Result: 低阶不连续性占主导体积，随机路径几乎必然命中一阶不连续性，且停留时间有界；平滑机制在保证低计算开销的同时实现了局部平滑。

Conclusion: 平滑机制不仅能恢复SMoE图的连续性，还在语言和视觉任务上提升了实证性能。
...
### [313] [Smoothness-Based Derandomization of PAC-Bayes Bounds](https://arxiv.org/abs/2606.19105)
*Alexandre Lemire Paquin,Brahim Chaib-Draa,Philippe Gigu\`ere*

Main category: cs.LG

TL;DR: 研究PAC-Bayes解随机化，利用平滑性获得确定性预测器的高概率泛化界，并提出基于雅可比和Hessian的正则化项，在CIFAR-10上验证效果。


<details>
  <summary>Details</summary>
Motivation: 为确定性预测器提供高概率泛化界，利用损失和预测器类的平滑性，推导从吉布斯预测器到确定性预测器的代价。

Method: 通过Jensen gap类的Rademacher复杂度控制泛化间隙，得到涉及参数雅可比和得分图Hessian的平坦度量界，并设计正则化项，对BatchNorm网络折叠权重计算。

Result: 得到适用于有界和无界平滑损失函数的泛化界，特例化到线性预测器和平滑神经网络；正则化项在CIFAR-10上表现受批量大小影响。

Conclusion: 平滑性有效用于PAC-Bayes解随机化，理论指导的实用正则化项可提升泛化性能。
...
### [314] [JourneyFormer: Encoding Airbnb Guest Journey with Sequence Modeling](https://arxiv.org/abs/2606.19108)
...
TL;DR: 描述了Airbnb在搜索排序中部署序列模型JourneyFormer的实践经验，包括应对长序列和稀疏标签的挑战，并取得了离线指标提升和在线业务指标显著增长。


<details>
  <summary>Details</summary>
Motivation: 序列建模虽理论上简单，但实际部署面临序列复杂和标签稀疏等挑战，需在效果和可扩展性间权衡。

Method: 设计了JourneyFormer模型，涵盖事件选择、ID嵌入、模型架构和标签归因等关键决策，并采用了多种策略加速训练和推理。

Result: 离线排序指标提升，在线A/B测试在两个生产面上关键业务指标显著增长。

Conclusion: JourneyFormer成功部署于Airbnb生产环境，证明了序列建模在搜索排序中的有效性。
...
### [315] [Seeing Before Reasoning: Decoupling Perception and Reasoning for Shortcut-Resilient Multimodal On-Policy Self-Distillation](https://arxiv.org/abs/2606.19120)
*Sihan Wang,Xiyao Liu,Lianqing Liu,Zhi Han*

Main category: cs.LG

TL;DR: 针对多模态大语言模型（MLLM）中策略自蒸馏（OPSD）存在的捷径问题，提出视觉接地OPSD框架ViGOS，通过视觉描述和分步推理改善图像依赖行为。


<details>
  <summary>Details</summary>
Motivation: OPSD在LLM推理中有效，但直接扩展到MLLM会导致模型依赖文本参考而非图像，形成捷径。

Method: 学生模型先写视觉描述再推理；有效rollout时，图像感知教师监督描述，特权推理教师监督推理和答案；无效rollout时用参考教师恢复格式。

Result: 在通用视觉语言、专家推理、数学、空间定位等基准上，ViGOS保持OPSD优势，并在捷径易发场景提升图像接地行为。

Conclusion: ViGOS有效将OPSD应用于MLLM后训练，增强图像依赖，缓解文本捷径问题。
...
### [316] [Pareto Q-Learning with Reward Machines](https://arxiv.org/abs/2606.19134)
*Arnaud Lequen,Cl\'ement Legrand-Lixon,L\'eo Sauli\`eres*

Main category: cs.LG

TL;DR: 提出PQLRM，一种结合帕累托Q学习和奖励机的多目标强化学习算法，能高效处理非马尔可夫奖励，并优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理多目标和非马尔可夫奖励时效率低或无法找到帕累托最优策略。

Method: PQLRM结合帕累托Q学习（PQL）维护向量Q值集逼近帕累托前沿，并利用奖励机（RM）分解奖励信号的结构。

Result: 实验表明PQLRM比朴素PQL基线收敛更快，并能合成QRM无法得到的帕累托最优策略。

Conclusion: PQLRM是一种样本高效的多策略算法，适用于RM编码的非马尔可夫奖励任务。
...
### [317] [INDEQS: Informed Neural controlled Differential EQuationS](https://arxiv.org/abs/2606.19138)
*Michael Detzel,Gabriel Nobis,Kristiyan Blagov,Juri Schubert,Jackie Ma,Wojciech Samek*

Main category: cs.LG

TL;DR: 提出INDEQS，一种利用先验有向图结构信息的图神经控制微分方程方法，用于时间序列预测，在合成和真实任务上优于未加入图先验的模型。


<details>
  <summary>Details</summary>
Motivation: 标准图神经控制微分方程通常从数据中学习空间结构，但在已知图结构时，如何有效融入先验知识以提升预测性能尚待研究。

Method: INDEQS在架构中区分内部混合（节点间隐藏状态）和外部混合（向量场与控制），提供轻量级图约束变体和自适应图卷积变体，并设计连续平流模拟生成合成数据。

Result: 外部知情一致提升均绝对误差，尤其大图；内部知情在严格遵循已知邻接时更参数高效；连续解码器优于离散卷积解码器。

Conclusion: 融入先验图结构能有效提升神经控制微分方程的预测性能，且不同知情位置带来不同优势。
...
### [318] [ChronoSurv: A Clinical Pathway-Guided Graph Framework for Multimodal Survival Analysis](https://arxiv.org/abs/2606.19140)
*Hugo Miccinilli,Theo Di Piazza*

Main category: cs.LG

TL;DR: 提出了一种新型异构层次有向图框架ChronoSurv，用于多模态生存分析，通过对齐诊断步骤的时序图建模患者护理轨迹，实现了最先进的预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度生存模型依赖静态融合或时序无关建模，无法捕捉结构化临床工作流，导致预测精度受限。

Method: 构建面向多模态生存分析的异构层次有向图框架ChronoSurv，通过有向图对齐关键诊断步骤，设计层次拓扑（细粒度、粗粒度、全局表示）支持缺失模态，并采用异构消息传递建模跨模态与跨步骤的复杂非对称关系。

Result: 在两个公开数据集上达到最先进的判别性能，校准可靠；消融实验验证了各架构组件的贡献。

Conclusion: 时序感知图建模在多模态生存预测中具有显著潜力。
...
### [319] [OrthoReg: Orthogonal Regularization for Hybrid Symbolic-Neural Dynamical Systems](https://arxiv.org/abs/2606.19145)
*Till Richter,Niki Kilbertus*

Main category: cs.LG

TL;DR: 提出OrthoReg正则化方法，防止混合建模中神经网络吸收符号成分，实现互补分解。


<details>
  <summary>Details</summary>
Motivation: 混合建模中神经网络可能重学机械部分，导致冗余和不可解释，尤其是符号结构从数据中发现时。

Method: OrthoReg直接惩罚符号和神经网络组件的重叠，实现互补分解。

Result: 在部分库不匹配的基准动力系统上，OrthoReg改进了符号恢复和分布外行为。

Conclusion: OrthoReg有效防止符号结构被神经残差吸收，提高模型可解释性和泛化。
...
### [320] [Complementary Attention Head Pruning for Efficient Transformers](https://arxiv.org/abs/2606.19150)
*Yaniv Livertovsky,Shahar Somin,Gonen Singer*

Main category: cs.LG

TL;DR: CAHP是一种基于图论和聚类的新型后处理注意力头剪枝框架，自动确定剪枝数量，避免邻近偏差，在高压缩率下表现优越。


<details>
  <summary>Details</summary>
Motivation: 现有基于梯度或随机门控的结构剪枝方法存在不稳定、结构退化和需要大量手动调参的问题。

Method: 将头选择定义为全局图论问题，利用图聚类和信息论距离度量保留拓扑多样的互补头，通过边际性能曲线自动确定每层头数。

Result: 在SST-5和MNLI上，CAHP在高压缩率下优于基线，且避免梯度方法的邻近偏差，保留中间层功能关键头。

Conclusion: CAHP是一种无需预定义稀疏度的有效后处理剪枝框架。
...
### [321] [The Reward Was in Your Data All Along: Correcting Flow Matching with Discriminator-Guided RL](https://arxiv.org/abs/2606.19162)
...
TL;DR: 提出判别器引导的强化学习（DRL），利用预训练表示空间中的判别器logit作为奖励，无需人类偏好即可提升生成样本质量和对齐。


<details>
  <summary>Details</summary>
Motivation: 现有分数/流匹配模型存在结构不匹配，匹配损失与样本质量属性对齐差；偏好RL虽有效但依赖昂贵的人类偏好。

Method: 训练判别器在预训练表示空间中区分真实数据与模型样本，将其logit（对数似然比）作为KL正则化RL的奖励。

Result: 在SiT等模型上，无引导FID从9.38降至2.62，语义FD从88.2降至19.3，并改善人类偏好奖励和Pareto前沿。

Conclusion: DRL有效解决结构不匹配，无需人类偏好即可提升生成质量和偏好对齐。
...
### [322] [Essential Subspace Merging for Multi-Task Learning](https://arxiv.org/abs/2606.19164)
*Longhua Li,Lei Qi,Xin Geng,Qi Tian*

Main category: cs.LG

TL;DR: 模型合并旨在将多个微调模型的能力整合到一个模型中，但存在任务间干扰。本文通过分析任务更新的输出偏移，发现其能量集中在少数主方向（本质子空间），并据此提出本质子空间分解（ESD）及两种合并方法：ESM（静态合并）和ESM++（动态合并），有效减少干扰并保留任务知识。


<details>
  <summary>Details</summary>
Motivation: 模型合并中任务特定参数更新之间存在干扰，导致多任务学习效果下降。

Method: 分析任务更新引起的输出偏移，确定其能量集中的本质子空间，提出本质子空间分解（ESD）；基于ESD提出静态合并方法ESM（正交化并融合本质成分）和动态合并方法ESM++（将残差分解为低秩专家并通过原型路由选择）。

Result: 在多个任务集和模型尺度上的实验表明，ESM和ESM++有效保留了任务知识，同时减少了任务间干扰。

Conclusion: 本文提出的基于本质子空间的方法能够有效缓解模型合并中的任务干扰问题，提升多任务性能。
...
### [323] [Compute Efficiency and Serial Runtime Tradeoffs for Stochastic Momentum Methods](https://arxiv.org/abs/2606.19179)
*Depen Morwani,Alexandru Meterez,Pranav Nair,Sham Kakade*

Main category: cs.LG

TL;DR: 研究了随机动量方法（HB和ASGD）在一致性线性回归中的批量大小权衡，给出了理论下界，揭示了HB在更大批量下保持SGD级计算效率，而ASGD的权衡依赖于频谱衰减速度。


<details>
  <summary>Details</summary>
Motivation: 随机动量方法广泛使用，但其计算效率与批量大小的关系尚不明确，需要理论分析其批量大小权衡。

Method: 通过有限维离散时间下界证明和合成线性回归实验进行理论分析与验证。

Result: HB不改善SGD的计算效率前沿，但允许批量窗口扩大√κ倍；ASGD在快速衰减功率谱下小批量改善计算效率，但随批量增大以计算效率换取串行速度。

Conclusion: 实验验证了HB和ASGD在不同频谱下的定性行为，包括慢衰减谱下的接近重叠和快衰减谱下的计算效率-串行权衡。
...
### [324] [AGDN: Learning to Solve Traveling Salesman Problem with Anisotropic Graph Diffusion Network](https://arxiv.org/abs/2606.19185)
*Bolin Shen,Ziwei Huang,Zhiguang Cao,Yushun Dong*

Main category: cs.LG

TL;DR: 提出各向异性图扩散网络（AGDN）解决旅行商问题，通过MixScore矩阵和各向异性扩散提升图结构利用效率，在多种规模上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 图基学习方法用于TSP时，如何有效利用图结构仍是开放问题，尤其面临全连接图缺乏拓扑先验和稀疏化后丢失最优解中连接节点的问题。

Method: 构建MixScore转移矩阵融合节点相似性和成对距离，并设计各向异性图扩散策略支持多跳高效信息交换。

Result: 在多种实例规模和节点分布下，AGDN一致优于现有方法，计算时间具有竞争力，且能泛化到未见过的规模和分布。

Conclusion: AGDN有效克服了TSP中图结构利用的两个困难，性能优越且泛化能力强。
...
### [325] [Forecasting what Matters: Decision-Focused RL for Controlled EV Charging with Unknown Departure Times](https://arxiv.org/abs/2606.19199)
*Giuseppe Gabriele,Fabio Pavirani,Seyed Soroush Karimi Madahi,Chris Develder*

Main category: cs.LG

TL;DR: 提出决策聚焦强化学习框架，将预测器与RL智能体端到端联合训练，处理电动汽车充电中未知特征（如离开时间）的不确定性，提升充电决策质量。


<details>
  <summary>Details</summary>
Motivation: 电动汽车普及导致峰值需求增加和电网不稳定；基于强化学习的智能充电可缓解问题，但实际中关键特征（如离开时间）常缺失，影响RL策略学习与执行。

Method: 将预测器与RL智能体端到端联合训练，利用充电策略动作的反馈优化预测器，使其专注于提升下游决策质量而非单纯预测精度。

Result: 与无离开时间预测的RL方法相比，总奖励提升14%，未供应能量（因车辆离开而未完成的充电）减少55%。

Conclusion: 预测器与控制器联合训练可产生更优的充电决策，显著提升性能。
...
### [326] [Machine Unlearning for the XGBoost Model with Network Intrusion Datasets](https://arxiv.org/abs/2606.19220)
*Diana Magalh\~aes,Eva Maia,Jo\~ao Vitorino,Isabel Pra\c{c}a*

Main category: cs.LG

TL;DR: 提出XGBoost-Forget方法，在表格网络入侵数据集上实现高效遗忘，保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘研究主要针对深度学习图像数据，缺乏对表格数据（如网络入侵检测）的研究。

Method: 针对XGBoost模型提出遗忘方法XGBoost-Forget，在IoT-23和GeNIS数据集上评估。

Result: 保持接近原始模型的预测性能，遗忘速度显著更快。

Conclusion: XGBoost-Forget在表格网络入侵设置中具有潜力。
...
### [327] [Mechanism-Guided Selective Unlearning for RLVR-Induced Reasoning](https://arxiv.org/abs/2606.19222)
*Chenyu Zhou,Qiliang Jiang,Shuning Wu,Xu Zhou*

Main category: cs.LG

TL;DR: 提出MAST方法，通过机制对齐的选择性更新，在消除RLVR诱导的推理能力时显著降低附带损害。


<details>
  <summary>Details</summary>
Motivation: 标准全参数更新在遗忘目标能力时会对保留能力造成严重附带损害。

Method: MAST根据注意力投影张量的离主能量、更新幅度和遗忘梯度耦合幅度进行排序，仅更新排名靠前的子集。

Result: 在主要模型上，MAST实现目标遗忘（MATH遗忘从45/150降至37/150，p=0.0078），同时GSM8K提升0.8个百分点，MATH保留仅下降0.5个百分点；在Qwen3上，MAST保留GSM8K而全参数更新导致其崩溃。

Conclusion: MAST能够有效减少遗忘过程中的附带损害，优于全参数更新方法。
...
### [328] [A Human-in-the-Loop Bayesian Optimization Framework for Constraint-Aware Bioprocess Development](https://arxiv.org/abs/2606.19230)
...
TL;DR: 扩展了PFGS框架，通过多目标优化和交互式选择处理约束和鲁棒优化。


<details>
  <summary>Details</summary>
Motivation: 现有贝叶斯优化通常返回单一推荐，缺乏专家交互；需要处理约束和输入扰动。

Method: 将GP后验量作为多目标，添加约束满足概率和鲁棒性能估计，通过Pareto前沿展示给专家。

Result: 多目标Pareto表示同时展示性能、不确定性、约束和鲁棒性间的权衡。

Conclusion: 在CHO细胞培养模拟中验证，能系统识别高性能、可行且鲁棒的操作条件，支持迭代求精和资源分配。
...
### [329] [STARE: Surprisal-Guided Token-Level Advantage Reweighting for Policy Entropy Stability](https://arxiv.org/abs/2606.19236)
*Haipeng Luo,Qingfeng Sun,Songli Wu,Can Xu,Wenfeng Deng,Han Hu,Yansong Tang*

Main category: cs.LG

TL;DR: STARE通过优势-惊讶四象限分析和熵临界token重加权，解决GRPO等RLVR算法的策略熵崩溃问题，在多个LLM规模上稳定训练并提升推理准确率。


<details>
  <summary>Details</summary>
Motivation: 现有的RLVR算法如GRPO在训练中常出现策略熵崩溃，导致探索不足，需要一种机制来维持熵稳定。

Method: 先对GRPO进行一阶梯度分析，揭示token级信用分配不匹配和优势-惊讶四象限结构；然后提出STARE，通过批次内惊讶分位数识别熵临界token，选择性重加权其优势，并加入目标熵闭环门控来稳定熵。

Result: 在1.5B到32B模型、Short/Long CoT和多轮工具使用任务上，STARE维持数千步稳定训练，在AIME24和AIME25上平均准确率比DAPO等基线提升4%-8%。

Conclusion: STARE通过保持探索-利用平衡，有效缓解熵崩溃，进一步释放RL训练潜力。
...
### [330] [SCAN: Enhance Time Series Anomaly Detection via Multi-Scale Neighborhood-Centered Clustering](https://arxiv.org/abs/2606.19255)
*Xingze Zheng,Hanyin Cheng,Siyuan Wang,Yiting Hao,Peng Chen,Yuan Jun,Yang Shu*

Main category: cs.LG

TL;DR: 提出SCAN方法，通过多尺度聚类增强重建式时间序列异常检测，解决过度泛化和欠泛化问题。


<details>
  <summary>Details</summary>
Motivation: 重建式异常检测方法存在过度泛化和欠泛化难以平衡的问题，需要约束模型聚焦于代表性正常模式。

Method: 在表示层整合正常模式的聚类中心表示约束重建；在异常准则层基于聚类成员概率与重建误差形成双准则；提取邻域中心表示改善多视图聚类。

Result: 在多个真实世界数据集上达到最先进性能。

Conclusion: SCAN有效平衡了泛化问题，提升了异常检测准确性。
...
### [331] [Detecting Hidden ML Training With Zero-Overhead Telemetry](https://arxiv.org/abs/2606.19262)
*Robi Rahman,Sabiha Tajdari*

Main category: cs.LG

TL;DR: 评估了使用零开销NVML遥测进行GPU工作负载分类的对抗鲁棒性，开发了分类器并测试了多种逃避策略。


<details>
  <summary>Details</summary>
Motivation: 如果开发者可以破坏监控机制，基于硬件监控的AI计算治理方案将不可行，因此需要评估其对抗鲁棒性。

Method: 使用零开销、保护隐私的NVML遥测信号（仅观察物理效应），训练分类器，并迭代多轮对抗逃避策略进行测试。

Result: 分类器在二进制分类中达到98.2%准确率，对最具挑战性的意外工作负载（含对抗伪装）达到43-87%准确率。

Conclusion: 尽管存在一定鲁棒性，但对抗性逃避仍可显著降低分类性能，需进一步改进监控机制。
...
### [332] [Structured Inference with Large Language Gibbs](https://arxiv.org/abs/2606.19264)
*Sanghyeok Choi,Henry Gouk,Esmeralda S. Whitammer*

Main category: cs.LG

TL;DR: 提出Large Language Gibbs方法，利用大语言模型的条件分布作为马尔可夫链蒙特卡洛中的转移算子，通过迭代重采样实现结构化概率推理，避免顺序偏差并产生反映所有局部条件分布折衷的平稳分布。


<details>
  <summary>Details</summary>
Motivation: 大语言模型编码的知识可用于复杂世界的结构化推理，但以概率连贯的方式访问这些知识面临困难的推理问题。

Method: 使用大语言模型的下一个词条件分布作为转移算子，在马尔可夫链蒙特卡洛中迭代地重采样每个变量（以其他变量为条件），替代单次自回归生成。

Result: 在合成分布采样、一致性推理任务和贝叶斯结构学习上验证，表明该方法是在噪声大语言模型条件分布下进行结构化概率推理的实用替代方案。

Conclusion: 该方法产生的平稳分布反映了所有局部条件分布之间的折衷，为基于大语言模型的世界先验进行结构化推理提供了实践途径。
...
### [333] [Risk Stratification for ICU Delirium using Pervasive Ambient Sensing Information](https://arxiv.org/abs/2606.19292)
...
TL;DR: 本研究探讨了环境声音和光照强度对ICU患者谵妄的独立预测能力，发现卷积神经网络模型结合声音数据可有效预测谵妄，声音是主要预测因子。


<details>
  <summary>Details</summary>
Motivation: 谵妄在ICU中常见且严重，但早期预测困难，环境因素（如声音、光照）常被忽视，本研究旨在评估其预测价值。

Method: 收集9个ICU的309名患者数据，使用四种序列神经网络模型，预测10个时间窗口内的谵妄，并通过SHAP分析特征重要性。

Result: 卷积模型表现最佳，声音数据AUC=0.80，结合光照可改善短期预测（<1周），声音是主导预测因子。

Conclusion: 被动环境感知（尤其是声音）可为谵妄风险评估提供临床有意义的信号，有助于丰富ICU多模态预测和预防策略。
...
### [334] [Does VLA Even Know the Basics? Measuring Commonsense and World Knowledge Retention in Vision-Language-Action Models](https://arxiv.org/abs/2606.19297)
...
TL;DR: 提出Act2Answer协议，通过动作答题评估VLA模型的知识保留能力，发现模型在简单概念上表现良好，但丰富语义类别的知识保留不足，且知识信号在中间层最强。


<details>
  <summary>Details</summary>
Motivation: VLA模型微调后保留多少常识和事实知识不明确，失败难以区分是缺乏知识还是控制能力差。

Method: 设计Act2Answer协议，将知识问答转为桌面物体放置动作；构建多类别测试集；引入分层意图探测定位知识相关层。

Result: VLA在简单概念上表现好，但丰富语义类别相比源VLM差距大；VQA联合训练提升知识保留；答案相关信号在中间层峰值，上层衰减。

Conclusion: Act2Answer能有效评估VLA知识保留，揭示了VLA在知识敏感任务上的不足与改进方向。
...
### [335] [P-K-GCN: Physics-augmented Koopman-enhanced Graph Convolutional Network for Deep Spatiotemporal Super-resolution](https://arxiv.org/abs/2606.19303)
*Xizhuo (Cici) Zhang,Zekai Wang,Fei Liu,Bing Yao*

Main category: cs.LG

TL;DR: 提出物理增强的Koopman图卷积网络，实现不规则几何上的时空超分辨率，在3D心脏电动力学重建中精度优于基线。


<details>
  <summary>Details</summary>
Motivation: 高保真时空模拟计算昂贵，现有数据驱动方法缺乏物理约束，而简单物理信息学习难以处理不规则几何和复杂时间动态。

Method: 设计连续样条GCN提取空间依赖，结合Koopman算子理论线性化时间演化，并加入物理损失约束重建符合物理定律。

Result: 在3D心脏电动力学数值实验中，该方法在稀疏低分辨率测量下重建高分辨率数据，精度优于基线模型。

Conclusion: 物理增强和Koopman正则化能数学上保证降低超分辨率误差，提高预测保真度和鲁棒性。
...
### [336] [Diffusion-Proof: Recipe for Formal Theorem Proving Beyond Auto-Regressive Generation](https://arxiv.org/abs/2606.19315)
*Ruida Wang,Rui Pan,Pengcheng Wang,Shizhe Diao,Tong Zhang*

Main category: cs.LG

TL;DR: 提出首个基于扩散大语言模型的正式定理证明框架Diffusion-Proof，显著优于自回归基线，并解决了一个IMO问题。


<details>
  <summary>Details</summary>
Motivation: 自回归LLM在长程连贯性和错误累积方面存在局限，而扩散LLM虽能生成多token块但尚未用于正式数学。

Method: 构建两种模型：dLLM-Prover-7B进行整段证明生成，dLLM-Corrector-7B利用扩散模型的填充能力进行局部校正。

Result: 在ProofNet-Test和MiniF2F-Test上分别相对提升1.61%和6.14%，并解决了一个DeepSeek-Prover-V2-7B无法解决的IMO问题。
...
### [337] [Explaining Attention with Program Synthesis](https://arxiv.org/abs/2606.19317)
*Amiri Hayes,Belinda Li,Jacob Andreas*

Main category: cs.LG

TL;DR: 提出用可执行程序近似注意力头行为，少至1000个程序即可高精度重现模式，替换25%头仅增16%困惑度。


<details>
  <summary>Details</summary>
Motivation: 实现可解释深度学习，用符号描述替代神经计算，聚焦注意力头的逆向工程。

Method: 计算注意力矩阵，提示预训练模型生成Python程序，根据保留输入预测性能重排序。

Result: 程序在TinyStories上平均IoU超75%；替换25%注意力头仅使困惑度增加16%，下游任务性能保持。

Conclusion: 贡献了可扩展的符号透明管道，用可读可执行代码逆向工程注意力头，促进神经模型透明化。
...
### [338] [UBP2: Uncertainty-Balanced Preference Planning for Efficient Preference-based Reinforcement Learning](https://arxiv.org/abs/2606.19328)
*Mohamed Nabail,Leo Cheng,Jingmin Wang,Nicholas Rhinehart*

Main category: cs.LG

TL;DR: 提出基于模型的不平衡偏好规划(UBP2)，通过联合推理多个不确定性主动引导探索，显著提升偏好强化学习的样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有偏好强化学习方法依赖被动数据收集，样本效率低，尤其在早期学习阶段。

Method: 使用奖励、动态和值函数模型的集成，对候选轨迹计算结合期望奖励、终值认知不确定性的统一评分，通过规划实现探索与利用的显式权衡。

Result: 理论上在有限和无限时域下建立子线性遗憾界；在Meta-World基准上，UBP2的样本效率显著高于无模型偏好方法和非乐观模型基基线。

Conclusion: UBP2通过主动不确定性驱动探索，有效提升偏好强化学习的样本效率，无需额外启发式探索策略。
...
### [339] [Homogeneous Boltzmann-type equations on dense graphs](https://arxiv.org/abs/2606.17638)
*Gabriele Taricco,Andrea Tosin*

Main category: math-ph

TL;DR: 本文研究了有限图上的均匀Boltzmann型方程，建模优先相互作用网络，推导了密集图极限，并分析了线性相互作用下的长时间行为与平衡分布。


<details>
  <summary>Details</summary>
Motivation: 经典动力学理论假设粒子间“全对全”相互作用，不适用于社会互动等“部分对部分”场景，需要建立更合适的模型。

Method: 考虑有限图上的均匀Boltzmann型方程，通过严格推导得到当代理数量趋于无穷时的密集图极限。

Result: 在线性成对相互作用下，刻画了极限方程的长时间行为，得到了涌现的平衡分布，并与经典“全对全”设置进行了比较。

Conclusion: 优先相互作用网络的密集图极限具有与经典模型不同的平衡分布，表明模型更适用于描述社会互动等场景。
...
### [340] [Semiclassical Analysis of Tunneling in Graphene under Nonuniform Electrostatic and Magnetic Fields](https://arxiv.org/abs/2606.17689)
*Maria V. Perel*

Main category: math-ph

TL;DR: 开发了石墨烯n-p-n结在垂直磁场下狄拉克费米子隧穿的半经典理论，得到反射/透射系数及共振条件。


<details>
  <summary>Details</summary>
Motivation: 先前研究仅适用于恒定磁场和特定静电势，需要更一般的理论。

Method: 采用半经典近似，在附加入射角假设下将散射问题简化为简并点连接问题，利用作者前期工作求解。

Result: 获得了反射和透射系数的显式表达式（含相位），推导了法布里-珀罗共振和“魔角”条件，数值结果展示了磁场导致的透射角依赖性。

Conclusion: 该理论推广了早期工作，能处理一般静电势和任意磁场，为研究石墨烯中隧穿提供更通用框架。
...
### [341] [Moment generating function of the tacnode process](https://arxiv.org/abs/2606.17771)
*Taiyang Xu*

Main category: math-ph

TL;DR: 本文研究tacnode过程在多个区间上的计数函数生成函数，给出了积分表示和渐近行为，并推广到多区间多间断点情形。


<details>
  <summary>Details</summary>
Motivation: 将tacnode过程的单点理论推广到多区间多间断点情形，研究其计数函数的联合波动。

Method: 利用哈密顿系统推导生成函数的积分表示，结合微分恒等式获得大间隙渐近。

Result: 得到了m点生成函数的积分表示，以及期望、方差、协方差的渐近公式，并建立了中心极限定理。

Conclusion: 将tacnode过程的已知单点理论成功扩展到多区间设置，并为计数函数的联合波动提供了完整的渐近分析。
...
### [342] [When Volumetric Growth Selects Surface Growth](https://arxiv.org/abs/2606.17774)
*Rohn Abeyaratne,Roberto Paroni,Marco Picchi Scardaoni*

Main category: math-ph

TL;DR: 通过优化框架研究线弹性固体中体积生长与表面生长的关系，发现最优生长分布是奇异的，集中在边界或界面，从而表面生长成为体积生长的最优实现。


<details>
  <summary>Details</summary>
Motivation: 探索在优化驱动框架下，体积生长如何通过变分机制转化为表面生长。

Method: 建立一维和轴对称线弹性固体模型，以最小化外力做功为优化目标，求解约束优化问题得到生长分布。

Result: 生长分布显式解析解显示，最优生长是奇异且集中在边界或内部界面，而非均匀体积分布。

Conclusion: 在一定条件下，表面生长是体积生长在变分意义下的最优实现。
...
### [345] [Elastic Surface Instability as a Topological Phase Transition](https://arxiv.org/abs/2606.18542)
*Yu-Xin Xie*

Main category: math-ph

TL;DR: 将软材料宏观表面失稳与量子拓扑物理学统一，证明其本质是拓扑相变，并揭示零能边缘态。


<details>
  <summary>Details</summary>
Motivation: 传统观点将软材料失稳视为纯机械失效，本文旨在跨越物理系统寻找普适原理，连接固体力学与拓扑物理。

Method: 引入李群度规演化与广义Stroh形式，将非线性几何挫折映射为表面阻抗矩阵，在半无限超弹性半空间下进一步映射到一维狄拉克哈密顿量，以拉伸控制狄拉克质量。

Result: 表面褶皱 onset 对应从平凡到非平凡拓扑相的转变，缠绕数出现量子化阶跃，并产生宏观局域化的零能边缘态。

Conclusion: 建立了宏观对称性破缺与拓扑范式的统一，为可编程智能软材料开辟新理论路径。
...
### [348] [A finite element-based eigenvalue analysis for predicting thermal runaway in Li-ion battery packs](https://arxiv.org/abs/2606.18983)
*Shailendra Rahi (1),Vinay Dhakal (2),Ankur Jain (2),Manish Agrawal (1) ((1) Indian Institute of Technology Ropar,(2) University of Texas at Arlington)*
...
### [349] [A quantum-like model of political consensus via non self-adjoint Hamiltonians](https://arxiv.org/abs/2606.19014)
*Fabio Bagarello,Gloria Liarda*

Main category: math-ph

TL;DR: 本文利用非自伴哈密顿量及其类海森堡动力学，模拟由政党联盟（C）及其支持者（Sc）组成的政治系统中，支持者意见如何随联盟的效能、能力和一致性变化。提出了三个具体模型，并分析了系统（Sc∪C）的动力学行为，详细讨论了平衡哈密顿量的作用。


<details>
  <summary>Details</summary>
Motivation: 旨在理解政治联盟的治理表现如何影响其支持者的观点演变，为政治动力学提供数学建模工具。

Method: 采用非自伴哈密顿量描述开放系统的非厄米动力学，引入平衡哈密顿量概念，构建联盟与支持者相互作用的三个模型。

Result: 模型揭示了支持者意见随联盟表现参数变化的动力学模式，平衡哈密顿量保证了系统的可积性。

Conclusion: 非自伴哈密顿量框架能有效模拟政治系统中的意见演化，平衡哈密顿量为系统守恒量提供关键机制。
...
### [350] [Bulk-boundary correspondence of (1+1)D symmetric gapped phases](https://arxiv.org/abs/2606.19137)
*Yizhou Ma,Gen Yue,Tian Lan*

Main category: math-ph

TL;DR: 开发了一种算符代数框架，用于描述具有范畴对称性的一维能隙相中的边界条件和体-边对应。


<details>
  <summary>Details</summary>
Motivation: 直接在热力学极限下严格理解边界条件和体-边对应关系。

Method: 从幺正融合范畴、半单右模范畴、Q-系统和右Q-模出发，构造半无限融合自旋链和对易投影边界哈密顿量，证明基态唯一性并建立边界条件与某范畴对象的等价关系，同时利用DHR双模给出边界对称性拓扑场论的微观表述。

Result: 简单边界条件由M_Q的简单对象分类，一般边界条件由有限直和分类；边界DHR范畴与(C_M^∨)^rev单调等价，其作用与范畴作用一致。

Conclusion: 得到一维体-边对应：描述体的丰富单调范畴是描述边的丰富范畴的丰富中心。
...
### [352] [Statistical Foundations of LLM-based A/B Testing: A Surrogacy Framework for Human Causal Inference](https://arxiv.org/abs/2606.17165)
*Joel Persson,M{\aa}rten Schultzberg,Sebastian Ankargren*

Main category: stat.ME

TL;DR: 提出了一个统计框架，用于评估将大型语言模型（LLM）作为人类替代品在A/B测试中的有效性，发现LLM结果仅能在过去处理中证伪，而对新处理无法验证，因此人类实验仍不可或缺。


<details>
  <summary>Details</summary>
Motivation: 组织希望使用LLM替代人类参与者进行A/B测试以降低成本和时间，但需要验证LLM估计的处理效应是否能够恢复人类群体中的真实效应。

Method: 采用替代终点理论，开发了一个统计框架，通过校准LLM结果到人类结果来识别平均处理效应，并引入了替代性和可比性条件，同时分析了LLM随机性带来的偏差和方差，以及多次抽样的缓解作用。

Result: 证明了在弱于分布等价的条件（替代性和可比性）下，校准后的LLM结果可以识别平均处理效应；当条件不满足时，效应仅部分可识别，并提供了诊断方法和偏差界限。仿真和实际案例（Upworthy标题A/B测试）验证了方法。

Conclusion: LLM作为替代品的有效性只能对过去处理进行证伪，无法验证新处理，因此人类实验对于新干预仍然必要；LLM的选择、提示和温度参数设计以及人类实验的规模验证很重要。
...
### [353] [Dependent Censoring Based on Geometric Optimization](https://arxiv.org/abs/2606.17293)
*Anis Fradi,Salima Helali,Bilel Bousselmi*

Main category: stat.ME

TL;DR: 提出利用扩展广义Marshall-Olkin模型和几何优化技术处理生存分析中的依赖删失问题。


<details>
  <summary>Details</summary>
Motivation: 依赖删失给生存分析的参数估计和生存函数估计带来挑战。

Method: 基于EGMO模型，采用几何优化技术开发高效估计方法，捕捉失败与删失时间间的依赖关系。

Result: 模拟研究和真实数据应用表明该方法具有鲁棒性和有效性。

Conclusion: 提出的框架能有效处理依赖删失机制。
...
### [354] [Proximal Mediation Analysis with Hidden Recanting Witnesses](https://arxiv.org/abs/2606.17600)
*Sihan Wu,Yang Bai,Yifan Cui*

Main category: stat.ME

TL;DR: 针对存在未知翻供证人时的中介分析，提出基于近端因果推断的三种识别策略和稳健估计方法。


<details>
  <summary>Details</summary>
Motivation: 传统中介分析假设无翻供证人，但实际中这些变量常因测量困难或隐私约束不可观测，需要新方法。

Method: 利用近端因果推断提出三种识别策略，建立半参数推断框架，推导有效影响函数，提出近端乘数稳健估计器，并采用最小最大优化的去偏机器学习过程。

Result: 估计器在至少一个误设模型下保持一致性，全正确时渐近正态且达到半参数效率界。仿真和实际数据验证了性能。

Conclusion: 提出的方法有效解决了未知翻供证人下的路径特定效应识别问题，提供了稳健推断。
...
### [355] [On Response-Adaptive Targeting Strategies for Multi-Treatment Experiments](https://arxiv.org/abs/2606.17777)
*Redouane Yagouti,R\'emy Degenne,Emilie Kaufmann*

Main category: stat.ME

TL;DR: 提出了α-再平衡目标策略（αRTS）的统一框架，将ERADE两臂策略推广到多臂临床试验，证明了渐近性质，并引入了带强制探索的变体以处理稀疏目标。


<details>
  <summary>Details</summary>
Motivation: 现有响应自适应随机化在多臂场景下理论碎片化，缺乏统一框架；目的是推广两臂策略到多臂，提供统一的理论基础。

Method: 定义了αRTS家族，基于目标最优分配比例进行动态分配；推导了渐近一致性和正态性；针对某些治疗被渐近消除的稀疏目标，提出带强制探索的变体。

Result: 证明了所有αRTS设计具有强一致性、分配比例和效应估计的渐近正态性及渐近效率；模拟显示强制探索在稀疏设置中有效保证所有治疗无限采样。

Conclusion: αRTS框架为多臂响应自适应随机化提供了统一且理论完善的方案，强制探索机制增强了实际适用性。
...
### [356] [Tropical Viterbi Tubes for Decoding Uncertainty in Hidden Markov Models](https://arxiv.org/abs/2606.17181)
*Aur\'elien Nicosia*

Main category: stat.ME

TL;DR: 提出热带Viterbi管，用于量化HMM中Viterbi解码的路径不确定性，通过容忍度内的所有近优路径集合，并给出计算方法和应用。


<details>
  <summary>Details</summary>
Motivation: Viterbi解码只输出一条最可能的完整路径，忽略了多个近优路径导致的路径不确定性，而这些路径可能具有科学意义。

Method: 定义热带Viterbi管为完整数据对数得分在Viterbi最优容忍度内的隐藏轨迹集合；通过状态、转移和变化投影展示局部特征兼容性；使用max-plus前向后向递归在O(TK^2)时间内精确计算。

Result: 证明了单调性、阶跃函数行为和确定性稳定性；热带管作为后验超水平集，可校准为HPD可信区域；在蝙蝠追踪中，鲁棒觅食段富集了喂食嗡嗡声，鲁棒通勤段则消耗。

Conclusion: 热带Viterbi管为HMM在序列分析等领域提供了路径不确定性层，使研究者能够识别与全局近优路径兼容的局部特征。
...
### [357] [Semiparametric Mediation Analysis with Separately Observed Mediator and Outcome under Unmeasured Confounding](https://arxiv.org/abs/2606.17232)
*Sijia Li,Ruoyu Wang*

Main category: stat.ME

TL;DR: 提出数据融合框架，利用工具变量从两个不完全数据源中恢复自然直接和间接效应的识别，解决中介变量和结果从未同时观测的问题。


<details>
  <summary>Details</summary>
Motivation: 现实研究中中介变量M和结果Y从未同时观测，破坏了标准识别策略，需要新的方法恢复因果路径的识别。

Method: 通过组合两个不完全数据源（一个测量M，一个测量Y），利用共享工具变量，在无交互条件下处理未测量混杂，并通过潜在对齐条件适应数据源间的协变量和暴露偏移，建立两种识别策略。

Result: 提出具有多重稳健性的半参数影响函数估计器，并在适当条件下达到半参数效率界。

Conclusion: 框架成功应用于量化SNP rs610932对痴呆风险的影响中免疫相关基因表达通路的中介作用。
...
### [358] [Bayesian Poisson-Randomized Gamma Tensor Factorization with Application to International Trade Flows](https://arxiv.org/abs/2606.17267)
*Jie Jian,Aaron Schein*

Main category: stat.ME

TL;DR: 提出贝叶斯分层张量分解模型，处理稀疏半连续数据（大量零值、右尾重、切片特异离散性），结合潜在泊松率与条件伽马模型，通过低秩CP结构共享信息，并采用混合变分-蒙特卡洛算法进行推断。


<details>
  <summary>Details</summary>
Motivation: 国际交易等多维货币数据中，存在大量零值、连续高变正值及切片特异性，现有重力模型或网络分析难以联合建模产品与时间维度。

Method: 构建贝叶斯分层张量分解，潜在泊松率低秩CP结构，正观测值条件伽马模型，切片间率参数可变，混合变分-蒙特卡洛后验推断。

Result: 应用于约6000万贸易流量，揭示了出口商、进口商、产品、年份间的多维依赖关系。

Conclusion: 该方法能有效恢复传统方法难以捕捉的多维依赖结构。
...
### [359] [Kernel-Based Functional Balancing for Causal Inference with Compositional Treatments](https://arxiv.org/abs/2606.17308)
*Sungbum Kim,Jiayi Wang*

Main category: stat.ME

TL;DR: 提出一种处理成分性处理的因果效应估计方法，通过核函数协变量平衡和增强加权估计器，实现√n一致性和渐近正态性。


<details>
  <summary>Details</summary>
Motivation: 现有因果效应估计方法通常处理标量或二元处理，无法直接应用于成分性处理（暴露位于单纯形上）。

Method: 采用基于核的协变量函数平衡方法构建权重，通过最小化再生核希尔伯特空间上的最坏情况平衡误差得到权重；进一步提出增强加权估计器，结合核岭回归估计结果函数与边际增广。

Result: 估计器达到√n一致性，无需一致估计或平滑权重，并建立围绕样本特定目标的渐近正态性。

Conclusion: 模拟研究和真实数据应用验证了该方法的有效性。
...
### [360] [The dangers of using three-number summaries to estimate unknown standard deviations: sensitivity analyses and some possible improvements incorporating shape](https://arxiv.org/abs/2606.17424)
*Udara Kumaranathunga,Alysha De Livera,Luke A. Prendergast*

Main category: stat.ME

TL;DR: 研究表明三点摘要（最小、中位、最大）不足以可靠估计标准差，可能导致错误推断；提出基于Beta分布的新估计器，并建议进行敏感性分析以提高结果可信度。


<details>
  <summary>Details</summary>
Motivation: 在元分析中，部分研究报告均值和标准差，另一些报告分位数摘要，现有方法将三/五点摘要转换为均值和标准差，但三点摘要信息不足，可能导致估计偏差，需要更可靠的方法。

Method: 分析三点摘要的信息局限性，展示不良估计后果；提出一种使用缩放Beta分布的新估计器，并探索通过补充额外信息来改善估计；提供网络应用辅助敏感性分析。

Result: 模拟和真实数据示例表明，三点摘要估计的标准差可能很差，而新估计器在拥有额外信息时能改善估计，但仍需谨慎；敏感性分析有助于识别偏差来源。
...
### [361] [Improving Linear Regression on Small Datasets via Gaussian Process and Extreme Value Theory-Based Data Augmentation](https://arxiv.org/abs/2606.17486)
*Ibrahim Salay,Jagath Senarathne*

Main category: stat.ME

TL;DR: 提出GP-MEVT方法，结合高斯过程和极值理论进行数据增强，解决小样本线性回归中假设违反、参数估计不准等问题，显著优于自举法。


<details>
  <summary>Details</summary>
Motivation: 小样本导致回归分析中经典假设（正态性、同方差性等）频繁被违反，影响参数估计精度、统计功效和泛化能力。

Method: GP-MEVT混合数据增强方法，基于高斯过程与极值理论生成观测，扩展预测空间并保持线性结构，引入基于残差的受控变异。

Result: 模拟显示GP-MEVT假设满足率更高，参数估计更接近真值，RMSE竞争力强；真实数据集上假设满足率达67.1%，远超自举法的17.3%和21.2%。

Conclusion: GP-MEVT为小样本线性回归提供了稳健可靠框架，是有效的统计推断方案。
...
### [362] [Anytime-valid Optimal Policy Identification](https://arxiv.org/abs/2606.17515)
*Daniel Molitor*

Main category: stat.ME

TL;DR: 提出一种随时有效的框架，用于从记录的上下文土匪数据中识别最优策略，支持动态监控和停止，无需固定样本量。


<details>
  <summary>Details</summary>
Motivation: 在诸多应用场景中，分析师需从候选策略类中选择最优策略，但数据由外部日志策略生成，且希望连续监控证据，在明确时停止，而非预先固定样本量。

Method: 构建随时间变化的时间索引集合 S_t，以高概率一致地包含真实最优策略集；通过持续监控策略值、淘汰次优策略，并在数据依赖时刻停止，而不破坏推断有效性。

Result: 当最优策略唯一时，定义停止时间，样本复杂度为 O(log|Π|+loglog(1/Δ_min)/Δ_min^2)；模拟表明相比固定样本设计可大幅节省样本。

Conclusion: 该框架为在线错误信息减少等大规模自适应实验提供了动态视图，允许根据累积证据灵活停止，提升效率。
...
### [363] [Double zero-inflated spatio-temporal modeling of daily precipitation under detection thresholds](https://arxiv.org/abs/2606.17717)
*Juan Marcen-Gutierrez,Jorge Castillo-Mateo,Alan E. Gelfand,Jes\'us As\'in,Ana C. Cebri\'an*

Main category: stat.ME

TL;DR: 提出多级时空模型区分两类零降水并建模正降水量，应用于西班牙埃布罗河流域，发现阈值显著影响观测降水发生但总量影响小。


<details>
  <summary>Details</summary>
Motivation: 日尺度降水解释因零值频繁且存在两类零（干旱和检测限）而困难。

Method: 结合零质量点、probit回归、Gamma回归和阈值截断观测机制，利用高斯过程捕捉空间依赖，贝叶斯框架推断。

Result: 阈值强烈影响观测降水发生（尤其湿润区），对累积总量影响小，但对高分位数影响显著。

Conclusion: 模型能区分两类零并量化真实与观测降水差异，阈值效应在不同特征下不同。
...
### [364] [Subgroup analysis in randomized controlled trials with binary outcomes: dilution and logic-respecting properties](https://arxiv.org/abs/2606.17841)
*Long-Hao Xu,Yang Han,Tim Friede*

Main category: stat.ME

TL;DR: 本文研究了随机对照试验中二分类结局的亚组分析，证明比值比（OR）不适合作为疗效指标，而相对反应（RR）更合适；提供了新定理和理论关系，并通过实例说明。


<details>
  <summary>Details</summary>
Motivation: 亚组分析中，比值比（OR）可能不满足逻辑一致性，即总体疗效不一定介于亚组疗效之间，且存在稀释效应，需要更合适的指标。

Method: 通过数学定理推导，正式建立OR与RR的关系，分析逻辑一致性和稀释性质。

Result: OR在亚组组合时可能改变方向和大小，不满足逻辑一致性；RR则保持逻辑一致性。OR在某些条件下近似逻辑一致。

Conclusion: 在二分类结局的亚组分析中，应使用相对反应（RR）而非比值比（OR）作为疗效指标，以避免误导性结论。
...
### [365] [Spatial mixed models for assessing environmental exposure effects on the microbiome](https://arxiv.org/abs/2606.17923)
*Sooran Kim,Chan Wang,Soyoung Kwak,Fares Darawshy,Alexander Bain,Leopoldo N. Segal,Jiyoung Ahn,Huilin Li*

Main category: stat.ME

TL;DR: 提出一种新型空间混合建模框架，通过条件自回归先验同时处理微生物组数据的空间依赖和生态依赖，在模拟和实际数据中优于现有方法，能识别与空气污染相关的菌属。


<details>
  <summary>Details</summary>
Motivation: 环境暴露对健康的影响日益受关注，微生物组可能介导此效应，但现有建模忽略数据中的空间和生态依赖，导致检测功效降低。

Method: 采用条件自回归先验，构建同时考虑区域空间依赖和分类群生态依赖的空间混合模型。

Result: 模拟显示该方法在特征选择中具有高检测功效、低假阳性率和低均方误差；真实数据中识别出已知和新的与PM2.5相关的菌属。

Conclusion: 该框架为复杂环境数据中挖掘生物学关联提供了强大灵活的工具。
...
### [366] [Spatial prediction of environmental processes using random forests: How best to account for spatial dependence?](https://arxiv.org/abs/2606.18078)
*Duncan Lee,Vinny Davies,Helen R. Savage,Hussein Twabi,Marriott Nliwasa,Peter MacPherson*

Main category: stat.ME

TL;DR: 比较了多种融合随机森林与空间自相关处理方法的预测性能，发现空间基函数方法在模拟和实际数据中表现稳定，但无统一最优方法。


<details>
  <summary>Details</summary>
Motivation: 环境科学中空间预测常用高斯过程克里金法，而机器学习算法非空间预测效果优异。融合两者可提升预测性能，但缺乏各方法相对预测性能的数值比较。

Method: 以随机森林为基础，融合高斯过程、观测驱动相关结构、空间基函数和局部地理拟合等方法，通过模拟实验和马拉维布兰太尔市结核病研究中空气污染浓度预测进行对比。

Result: 无单一方法普遍最优，但使用空间基函数在模拟和实际数据研究中均表现稳定良好。

Conclusion: 建议环境科学家在空间预测中优先考虑空间基函数方法，但需根据具体问题选择合适方法。
...
### [367] [Undocumented Behavior in the gsynth R package and its Consequences for Three Published Studies](https://arxiv.org/abs/2606.18113)
*Beniamino Green,P. M. Aronow*

Main category: stat.ME

TL;DR: gsynth包在1.3.1版本前，当同时使用inference='parametric'和EM=TRUE时，因实现错误（使用样本内残差替代样本外误差）导致标准误差严重低估，影响多项已发表研究。


<details>
  <summary>Details</summary>
Motivation: 发现gsynth包在特定参数组合下可能系统性低估标准误差，且该问题影响已发表论文的结论可靠性。

Method: 通过理论分析识别实现错误，并进行实证蒙特卡洛研究，利用随机分配安慰剂处理对州级面板数据集进行分析，最后重新分析三篇受影响论文的相关部分。

Result: 实现错误可导致标准误差低估数个数量级，且在实际数据中产生高假阳性率；纠正错误或改用Xu (2017)的广义合成控制法后，受影响论文的结果不再显著。

Conclusion: gsynth包中参数自举与IFE-EM估计的组合存在严重实现错误，研究者应避免使用该组合，或使用替代方法以确保推断可靠性。
...
### [368] [Bayesian Threshold-Aligned Joint Disease Progression Modeling for Alzheimer's Disease](https://arxiv.org/abs/2606.18139)
*Rong Wu,Duygu Tosun,Isabella Hausle,Margo Heston,Aaron Wolfe Scheffler*

Main category: stat.ME

TL;DR: 提出贝叶斯阈值对齐联合疾病进展模型(B-TAJ DPM)，用于阿尔茨海默病进展建模，关联认知障碍结局。


<details>
  <summary>Details</summary>
Motivation: 现有模型存在参数形式受限、未锚定阳性阈值、未关联认知终点等局限，无法解释个体间病理进展和症状出现的变异性。

Method: B-TAJ DPM是一种生成式半参数框架，在潜在疾病时间线上建模多变量生物标志物轨迹，锚定于阳性阈值，并集成生存模型将病理进展与认知障碍关联。

Result: 模拟研究显示良好的估计精度和区间覆盖；应用于ADNI数据，刻画了非线性进展模式，量化了个体阳性年龄变异，揭示了tau阳性年龄与认知加速衰退的关联。

Conclusion: B-TAJ DPM能够有效对齐异质性病程，联合建模生物标志物轨迹和认知终点，为理解阿尔茨海默病进展提供新工具。
...
### [369] [Spatial Disease Mapping and Disparity Detection Using Generative AI: An Amortized Bayesian Learning Framework](https://arxiv.org/abs/2606.18146)
*Luca Aiello,Sudipto Banerjee*

Main category: stat.ME

TL;DR: 提出了一个摊销贝叶斯框架，用于跨不同区域图和邻接结构的空间边界检测，通过神经网络引擎模拟训练实现高效后验推理。


<details>
  <summary>Details</summary>
Motivation: 现有后验推理方法难以推广到具有不同区域数量和邻接结构的空间图，需要一种通用的空间边界检测方法。

Method: 结合泊松计数似然和协变量驱动的平滑中断规则，使用DAGAR先验，并训练一个包含置换不变摘要网络和条件归一化流的神经网络引擎来近似后验分布。

Result: 模拟研究显示参数恢复准确、区间覆盖接近名义水平、后验预测校准良好；与MCMC对比一致；消融研究验证了图摘要的重要性；实际应用中得到合理边界。

Conclusion: 该框架能够无缝部署到不同图结构的地图数据上，产生与局部平滑分析一致的边界结论，具有通用性和有效性。
...
### [370] [Shrinkage priors for Bayesian Substitute Confounders](https://arxiv.org/abs/2606.18535)
*Yordan P. Raykov,Hengrui Luo,Justin D. Strait,Wasiur R. KhudaBukhsh*

Main category: stat.ME

TL;DR: 提出贝叶斯因子分配框架，利用收缩先验学习稀疏替代混杂因子，在保留多原因依赖的同时避免过度编码或单原因变异，实现稳定的因果调整。


<details>
  <summary>Details</summary>
Motivation: 多原因观察性研究存在未测量混杂，直接插补混杂因子复杂，而替代混杂因子方法（如去混杂因子）可能过度编码处理向量或塌陷重叠，需要更有效的稀疏表示。

Method: 构建贝叶斯因子分配模型，采用收缩先验学习稀疏替代混杂因子，通过后验集中、因子收缩和几何保持来保留多原因粗粒度依赖并避免单原因因子。

Result: 在潜在变量可识别条件下，回归调整估计量一致；收缩先验倾向于低维因子，排序因子；合成实验和ADNI数据分析显示稀疏替代分数能恢复调整效果，且塌陷诊断有效。

Conclusion: 稀疏替代混杂因子与收缩先验结合，在保持因果调整稳定性的同时，通过塌陷诊断避免过度简化，为多原因研究提供可行方案。
...
### [371] [Wasserstein Policy Learning for Distributional Outcomes](https://arxiv.org/abs/2606.19117)
...
### [374] [Learning Moment Maps for Continuous-Time Markov Chains under Monte Carlo Noise](https://arxiv.org/abs/2606.18409)
*Madison Pratt,Olivia Prosper-Feldman*

Main category: stat.ME

TL;DR: 提出一种基于模拟的代理建模框架，从蒙特卡洛噪声训练目标中学习参数到矩的映射，以高效近似连续时间马尔可夫链的均值和协方差。


<details>
  <summary>Details</summary>
Motivation: 连续时间马尔可夫链的关键矩量（如均值和协方差）通常难以解析计算，蒙特卡洛采样虽渐近精确，但跨参数评估时计算成本过高。

Method: 利用神经网络作为代理模型，从蒙特卡洛生成的噪声标记训练数据中学习参数到矩的映射；分析蒙特卡洛噪声对均值和协方差估计的影响，并研究计算资源在参数空间覆盖与蒙特卡洛复制之间的分配策略。

Result: 在随机易感-感染-恢复模型中，神经网络在固定模拟预算下准确学习均值和协方差；协方差估计需要平衡资源以控制方差和偏差，而均值估计更受益于增加参数空间覆盖；学习到的矩映射产生有效的总体量，并在下游任务（如白化）中表现良好。

Conclusion: 强调了在代理建模中考虑蒙特卡洛噪声的重要性，并为随机系统的基于模拟的学习提供了实用指导。
...
### [375] [Bayesian Nonparametric Detection of Anomalies in Multivariate Functional Data](https://arxiv.org/abs/2606.18412)
*Daniel Krasnov,David Stephens*

Main category: stat.ME

TL;DR: 提出一种贝叶斯非参数方法检测多元函数数据异常，通过无限混合高斯过程、切片采样、小波基与Besov先验、内在共区域化模型和Carlin-Chib步骤自动确定组件数、正则化均值、捕捉跨函数依赖和选择协方差核，无需先验指定异常数量或性质，在半监督设置下对单变量和多变量数据有效。


<details>
  <summary>Details</summary>
Motivation: 函数数据异常检测重要，但现有方法需要先验指定异常数量或性质，亟需无需先验的贝叶斯非参数方法。

Method: 采用无限混合多输出高斯过程，通过切片采样自动确定混合组件数；均值函数用小波基表示并施以Besov先验实现平滑稀疏；跨函数依赖通过内在共区域化模型捕捉；协方差核选择通过Carlin-Chib乘积空间步骤嵌入MCMC；在半监督设置下（15%正常标签，类不平衡）将异常观测分配给小混合组件。

Result: 在单变量和多变量函数数据上验证了模型有效性，能自动检测异常而无需先验指定异常数量或性质。

Conclusion: 提出的贝叶斯非参数方法有效解决了函数数据异常检测中需先验知识的问题，在半监督场景下表现良好。
...
### [376] [Apportioning Causal Responsibility of Two Risk Factors for an Adverse Outcome via Counterfactual Attribution](https://arxiv.org/abs/2606.18459)
*Shanshan Luo,Yafang Deng,Qingyuan Zhao,Zhi Geng*

Main category: stat.ME

TL;DR: 提出了一个定量框架，用于回顾性分配两个风险因素对已发生不良结局的因果责任。


<details>
  <summary>Details</summary>
Motivation: 传统因果推断前瞻性评估效应，而因果责任分配需要回顾性推断已发生结果的原因。

Method: 基于潜在因果类型定义平均因果责任，在无混杂和单调性假设下，当类型特异性责任满足结构平衡条件时进行非参数识别，否则推导出尖锐界限。

Result: 建立了识别条件，并用吸烟和石棉暴露导致肺癌的例子进行了说明。

Conclusion: 该框架能够有效分配二元风险因素的因果责任。
...
### [377] [Applying the Weibull Shape Parameter test for signal detection in pharmacovigilance using the R package WSPsignal](https://arxiv.org/abs/2606.18809)
*Julia Dyck,Odile Sauzet*

Main category: stat.ME

TL;DR: 开发了R包WSPsignal，实现基于Weibull形状参数的药物警戒信号检测方法，支持频率学派和贝叶斯估计，并提供模拟调优。


<details>
  <summary>Details</summary>
Motivation: 后市场药物警戒需要统计信号检测方法，WSP检验利用时间信息评估危害，但缺乏统一工具。

Method: WSP框架包括多种检验变体（频率学派/贝叶斯，Weibull、双Weibull、幂广义Weibull分布），开发R包整合所有功能，支持默认配置或模拟调优。

Result: 通过两个示例（大样本约20000观测值用频率学派，小样本约1000观测值用贝叶斯）展示包的使用，并通过模拟调优优化测试规格。

Conclusion: WSPsignal包为实践者和研究者提供了便捷的信号检测工具，有望促进未来研究采用WSP检验。
...
### [378] [Improved prediction of extreme random effects in joint models: WRaPs](https://arxiv.org/abs/2606.18843)
*Eline Vanderpijpen,Els Goetghebeur*

Main category: stat.ME

TL;DR: 提出加权随机效应预测器（WRaPs），通过联合建模纵向和生存数据，优化尾部预测误差，用于预测极端不良结局（死亡或重复测量不佳）。


<details>
  <summary>Details</summary>
Motivation: 标准随机效应预测在识别极端或不良结局时存在均值回归问题，低估尾部值；临床中重复结局可能以死亡告终，需要改进预测方法。

Method: 基于共享随机效应的联合模型，通过最小化加权平方预测误差（重点关注尾部误差），推导基本模型的解析解，利用贝叶斯MCMC方法处理复杂情况。

Result: 在随机截距和斜率的I类模拟中验证了有限样本性质，并应用于胶质母细胞瘤患者的个体未来结局和生存预测。

Conclusion: WRaPs能更准确地预测极端不良结局，适用于存在死亡风险的重复测量数据。
...
### [379] [Feature Screening for High-Dimensional Structural Break Predictive Regression](https://arxiv.org/abs/2606.18949)
*Zhenjie Qin,Rongmao Zhang,Wenyang Zhang,Yang Zu*

Main category: stat.ME

TL;DR: 提出一种在结构突变预测回归中高效选择估计活跃预测因子和突变点的方法。


<details>
  <summary>Details</summary>
Motivation: 预测回归是探索回报可预测性的重要工具，需要有效处理活跃预测因子和结构突变点。

Method: 采用Sure Independence Canonical Screening (SICS)识别活跃预测因子，Ratio-Controlled Regression Screening (RCRS)估计突变点，信息准则(IC)精简冗余。

Result: 模拟和实证研究表明该方法表现良好。

Conclusion: 该方法能一致估计并正确选择真正的突变点和活跃预测因子。
...
### [380] [Balanced Twins: Causal Inference on Time Series with Hidden Confounding](https://arxiv.org/abs/2606.18969)
*Ouali Maha,Ghattas Badih,Flachaire Emmanuel,Charpentier Philippe,Bozzi Laurent*

Main category: stat.ME

TL;DR: 提出一种神经网络框架，通过学习个体时间序列的潜在表示和倾向性得分，再通过灵活匹配估计个体反事实，从而得到平均处理效应（ATT），适用于交错干预和潜在偏差场景。


<details>
  <summary>Details</summary>
Motivation: 时间序列中治疗分配受未观测因素影响产生偏差，且干预在不同时间点交错实施，传统聚合方法失效，需要先估计个体治疗效果。

Method: 神经网络同时学习低维潜在表示和倾向性得分，然后用灵活匹配近似个体治疗效果，避免合成控制中的凸性约束。

Result: 在真实能源消耗数据和临床时间序列（如电力需求响应项目、ICU半合成数据）上验证，能处理隐藏混淆、交错干预和非平稳动态。
...
### [381] [Dimension reduction of multivariate densities in Bayes spaces](https://arxiv.org/abs/2606.19011)
*Ad\'ela Czolkov\'a,Karel Hron,Sonja Greven*

Main category: stat.ME

TL;DR: 本文在Bayes空间框架下，通过中心化对数比变换将概率密度函数映射到L^2空间，实现了函数主成分分析，并提出了多元密度函数的正交分解方法，应用于住房和地质数据，展示了其可解释性和实用价值。


<details>
  <summary>Details</summary>
Motivation: 概率密度函数具有相对性和约束性，传统的函数数据分析方法不能直接适用。需要建立合适的几何结构（Bayes空间）来分析和降维密度数据。

Method: 利用Bayes空间中的clr变换建立与L^2空间的等距同构，然后应用函数主成分分析（FPCA）；对多元密度函数进行正交分解为独立和交互成分，并进一步分解独立成分为几何边际成分。

Result: 该方差分解在PCA意义下是最优的，直接影响FPCA的特征函数和得分解释；直接对多元密度应用FPCA等价于对其分解形式进行多元FPCA，且特征函数和得分相应分解。实证分析验证了方法的可解释性和实用性。

Conclusion: 所提出的理论分解方法能够深入洞察多元密度变异的来源，并在实际数据中展现良好的解释能力，具有重要应用价值。
...
### [382] [Efficient Cumulative Incidence Estimation in Biobank Studies Using All Prevalent and Incident Events](https://arxiv.org/abs/2606.19041)
*David M. Zucker,Malka Gorfine*

Main category: stat.ME

TL;DR: 提出一种新的累积发病率函数估计方法，可同时纳入患病和发病病例，适用于年轻发病且生存期长的疾病，并在英国生物银行数据中展示优势。


<details>
  <summary>Details</summary>
Motivation: 现有方法未充分利用所有疾病病例，尤其是发病前已患病（流行病例）和随访期间发病（新发病例）的信息，需要一种能处理年轻发病且生存期长的情况的估计方法。

Method: 提出一种新的累积发病率函数估计器，整合所有疾病病例（流行和突发），不依赖于后续生存过程，并建立渐近性质。

Result: 模拟研究显示新方法性能良好，应用于英国生物银行癌症数据证明其优于现有方法。

Conclusion: 新方法能更充分利用生物银行数据，尤其适用于发病年龄小、生存期长的疾病，提供更准确的发病率估计。
...
### [383] [Regularized covariance estimation from partially observed interferometric data](https://arxiv.org/abs/2606.19065)
*Teresa Bortolotti,Roberta Troilo,Francesco Casu,Simone Vantini,Alessandra Menafoglio*

Main category: stat.ME

TL;DR: 针对SBAS地面位移数据中大规模缺失问题，提出基于函数数据分析的协方差估计方法，通过拉普拉斯正则化矩阵补全实现高效估计，并在模拟和实际数据中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: SBAS技术提供高分辨率地面位移测量，但系统性缺失导致二阶结构难以可靠估计，亟需新方法。

Method: 将观测视为部分观测的二维函数数据，引入碎片化缺失机制，将协方差估计转化为带拉普拉斯正则化的矩阵补全问题，不依赖平稳性或各向同性假设。

Result: 模拟实验显示方法在不同协方差结构下估计误差低；应用于坎帕尼亚地区的实际数据成功恢复了空间依赖模式。

Conclusion: 该方法为地质灾害监测中的风险和环境评估提供了有效工具，具有实用潜力。
...
### [384] [Probability Bound Analysis for Dependence Uncertainty in Risk and Decision Models](https://arxiv.org/abs/2606.19086)
*Rowan Iskandar*

Main category: stat.ME

TL;DR: 提出一个对依赖敏感的概率边界分析框架，处理边缘和依赖信息不完整的黑盒风险和决策模型。


<details>
  <summary>Details</summary>
Motivation: 现有概率边界分析常假设独立或需完全指定依赖，而实际中依赖信息不完整。

Method: 结合p-box、精确CDF和固定量，用copulas指定依赖，通过Fréchet耦合集传播未知依赖，并扩展到交叉依赖。

Result: 在风险决策模型中，依赖假设显著影响输出边界和尾部风险；简化依赖导致狭窄的结果表征。

Conclusion: 框架支持证据不足时的透明不确定性传播，无需精确保留边缘分布或单一依赖模型。
...
### [385] [What does ethnic density represent? Spatial co-occurrence networks of a widely used contextual measure using harmonised UK small-area census data](https://arxiv.org/abs/2606.19087)
*Joseph Lam*

Main category: stat.ME

TL;DR: 民族密度并非单一标量，不同群体的等同百分比值可能代表不同邻里背景，对城市健康研究中的测量和解释有重要影响。


<details>
  <summary>Details</summary>
Motivation: 民族密度在流行病学和健康地理学中广泛使用，但很少作为测量问题被研究，等同百分比值可能因群体和地点不同而代表不同邻里背景。

Method: 使用统一的英国人口普查数据，分析239,023个小区域，估计针对八种民族密度目标的混合图模型（MGM），并进行英格兰空间分析（k=8近邻输出区质心）以估计LISA和空间调整残差网络。

Result: 不同民族密度的最强目标-邻居边不同：亚洲密度与中东/亚洲出生比例（0.59），印度密度与印度教比例（0.55），巴基斯坦密度与穆斯林比例（0.47），孟加拉国密度与穆斯林比例（0.23），黑人密度与非洲出生比例（0.42），白人密度与中东/亚洲出生比例（0.35）。英格兰民族密度强空间自相关（Global Moran's I: 0.57-0.90），残差化后64.3%-96.4%的原始边保留。
...
### [386] [Beyond Parallel Sampling: Diverse Query Initialization for Agentic Search](https://arxiv.org/abs/2606.17209)
*Sidhaarth Murali,Jo\~ao Coelho,Jingjie Ning,Jo\~ao Magalh\~aes,Bruno Martins,Chenyan Xiong*

Main category: cs.AI

TL;DR: DivInit通过在第一轮查询中引入多样性来缓解并行采样中的冗余问题，显著提升多跳问答性能。


<details>
  <summary>Details</summary>
Motivation: 标准并行采样随着广度增加收益递减，根因在于第一轮查询高度相似，导致后续检索内容重叠。

Method: DivInit不独立采样k个第一轮查询，而是先生成n个候选，再从中挑选k个多样性最高的查询作为并行轨迹。

Result: 在五个开源模型和八个基准上，DivInit在相同计算量下比标准并行采样平均提升5-7个百分点的多跳QA准确率。

Conclusion: DivInit是一种无需训练、简单有效的干预手段，通过多样化初始查询改善了广度缩放的效果。
...
### [387] [When Rules Learn: A Self-Evolving Agent for Legal Case Retrieval](https://arxiv.org/abs/2606.17220)
*Mingxu Tao,Jiawei Hu,Xian Zhou,Wenpeng Hu,Jiajun Cheng,Yunbo Cao,Zhunchen Luo,Guotong Geng*

Main category: cs.AI

TL;DR: 提出一个无需参数训练的自进化框架，通过规则驱动查询重写增强BM25在法律案例检索中的性能。


<details>
  <summary>Details</summary>
Motivation: 法律案例检索中BM25仍是强基线，但需要改进以提升精确性。

Method: 基于LLM的代理，配备自动评估环境，迭代创建、组合和消除重写规则。

Result: 在LeCaRD-v2上，该框架优于非进化基线，尤其使用高容量LLM时。

Conclusion: LLM利用历史实验结果和内在规则消除知识是自进化成功的关键。
...
### [388] [SkillChain-Gym: A Benchmark for Reskilling-Aware Production-Inventory Control under Disruptions](https://arxiv.org/abs/2606.17266)
*Carlos Eduardo Sanoja*

Main category: cs.AI

TL;DR: 提出了SkillChain-Gym基准，用于评估考虑再技能化的生产库存控制策略，发现训练策略优于纯生产，且不同策略在不同场景下各有优劣。


<details>
  <summary>Details</summary>
Motivation: 现有运营基准通常将劳动力视为外生变量，缺乏可复用的考虑技能与学习的劳动力规划模型，因此需要建立再技能化感知的生产库存控制基准。

Method: 构建单站点环境，包含工人技能状态动态、硬阈值认证、遗忘、训练与生产共享时间预算等特征；设计了种子可控的中断场景、三种可行性模式、确定性回放以及多维度指标。

Result: 基于60个班次周期和配对的统计检验，评估了多种策略。训练策略优于纯生产基线；在遗忘存在时需维持性训练；自适应训练在瓶颈可见时有效，而静态交叉训练在意外中断和缺勤下表现稳健。无策略在所有场景中占优。
...
### [389] [Skill-Constrained Model Predictive Control for Resilient Manufacturing Supply Chains](https://arxiv.org/abs/2606.17269)
*Carlos Eduardo Sanoja*

Main category: cs.AI

TL;DR: 在技能受限的生产库存系统中，模型预测控制器通过有限时域混合整数规划优化生产、库存、积压和培训决策，但实验表明其表现依赖于瓶颈的可预测性，而非适应性本身。


<details>
  <summary>Details</summary>
Motivation: 培训决策影响未来可用人力，但培训与生产竞争工时，导致系统面临技能瓶颈的动态挑战。

Method: 提出闭环技能受限模型预测控制器，每班次求解有限时域混合整数规划，仅执行第一期动作后重新规划，包含二进制认证、硬性生产资格和终端价值函数。

Result: 在合成场景中，与多种基准对比，无策略绝对占优；预测控制仅在技能瓶颈可提前预测且培训能完成时有效，而静态保险在突发冲击下表现稳健。

Conclusion: 决定预测控制价值的关键是可预测性而非适应性。
...
### [390] [Nothing from Something: Can a Language Model Discover 0?](https://arxiv.org/abs/2606.17289)
*Phoebe Zeng,Thomas L. Griffiths,Brenden M. Lake*

Main category: cs.AI

TL;DR: 使用简单算术案例，研究语言模型能否独立发现“零”的概念，发现GPT-2无法直接泛化，但少量训练后显著提升，且语言预训练减少所需样例约50%。


<details>
  <summary>Details</summary>
Motivation: 探索AI系统能否超越训练数据，发现全新的数学结构，检验语言能力对数学泛化的支持作用。

Method: 以算术中的“零”概念为案例，评估GPT-2规模语言模型在有无语言预训练下的零样本泛化能力，并测试少量样例训练后的表现。

Result: 模型在测试时无法零样本泛化；经过数十至数百个零样例训练后，性能大幅提升；语言预训练将所需样例数减少约50%。

Conclusion: 语言能力可以支撑神经模型的数学发现，但当前模型需要特定训练才能突破训练数据边界。
...
### [391] [Quantifying Consistency in LLM Logical Reasoning via Structural Uncertainty](https://arxiv.org/abs/2606.17312)
...
### [392] [MemTrace: Probing What Final Accuracy Misses in Long-Term Memory](https://arxiv.org/abs/2606.17328)
*Xianxuan Long,Zhikai Chen,Shenglai Zeng,Shouren Wang,Kai Guo,Jiliang Tang*

Main category: cs.AI

TL;DR: 提出MemTrace基准，以知识点为单位评估LLM代理长时记忆，发现相似准确率隐藏不同失败，主要瓶颈是证据使用而非检索。


<details>
  <summary>Details</summary>
Motivation: 现有评估按问题行聚合准确率，无法显示同一事实随条件变化的行为。

Method: MemTrace以知识点为单位，沿记忆年龄、问题类型、证据条件三个维度探测事实。

Result: 评估13种配置，相似聚合准确率下存在不同失败；失败时证据可检索但未被利用。

Conclusion: 改进长时记忆需更好使用可到达的证据，而非增加存储或检索。
...
### [393] [SpeechDx: A Multi-Task Benchmark for Clinical Speech AI](https://arxiv.org/abs/2606.17339)
*Sejal Bhalla,Larry Kieu,Aina Merchant,Eyal de Lara,Alex Mariakakis*

Main category: cs.AI

TL;DR: 提出SpeechDx，一个覆盖12个数据集和27个任务的临床语音AI基准，按语音产生阶段分类，评估多种编码器，发现大规模语音模型表现最佳但缺乏通用性。


<details>
  <summary>Details</summary>
Motivation: 当前临床语音AI研究局限于特定疾病，结果难以比较和泛化。

Method: 构建SpeechDx基准，包括12个数据集和27个任务，按语音产生阶段（概念化、制定、发音）分类，评估12种最先进的音频编码器，并执行零样本跨条件迁移。

Result: 大规模语音模型表现最强；领域专用模型仅在匹配任务上提升；尚无表示能可靠泛化到所有临床语音任务。

Conclusion: SpeechDx建立了共享评估框架，用于追踪通用临床语音表示的发展。
...
### [394] [Distributed General-Purpose Agent Networks: Architecture, Key Mechanisms, and Prototypes](https://arxiv.org/abs/2606.17368)
*Shengli Zhang,Deen Ma,Zibin Lin,Taotao Wang*

Main category: cs.AI

TL;DR: 本文研究分布式通用智能体网络，提出分层架构，解决语义声明传播、可信身份与多主题声誉、语义梯度机制设计三个核心问题，并给出技术路线和原型开销结果。


<details>
  <summary>Details</summary>
Motivation: 单个智能体受限于本地数据、工具权限、运行环境和治理边界，需要分布式网络实现开放协作。

Method: 提出以协议适配层为中心的分层架构，分别采用无体八卦日志、BAID身份绑定与MG-EigenTrust声誉、Stackelberg机制生成循环解决三个机制问题。

Result: 报告了BAID分层验证的原型开销和MG-EigenTrust在跨主题伪装共谋攻击下的模拟结果。
...
### [395] [Treatment Response Optimized Clinical Decision Support AI System via Digital Twin Simulation](https://arxiv.org/abs/2606.17405)
*Xinyu Qin,Anil K. Sood,Ruiheng Yu,Sara Corvigno,Elaine Stur,Lu Wang*

Main category: cs.AI

TL;DR: 提出一个在线自适应框架，整合治疗效应估计、数字孪生和强化学习，确保临床决策支持系统安全实时适应患者状况，实验验证有效且稳定。


<details>
  <summary>Details</summary>
Motivation: 临床决策支持AI系统需实时适应患者病情变化并遵守严格安全约束。

Method: 集成治疗效应估计、患者数字孪生和强化学习，结合规则模块监控生命体征并屏蔽禁忌治疗，模型分歧时交由临床医生审核。

Result: 在合成模拟器和真实卵巢癌数据集上，该方法在推荐治疗的有效性和稳定性上优于基线，且延迟低，仅少数病例需专家会诊。

Conclusion: 该框架可作为安全、临床监督的个性化医疗工具，通过实践持续改进。
...
### [396] [Incumbent Advantage: Brand Bias and Cognitive Manipulation Dynamics in LLM Recommendation Systems](https://arxiv.org/abs/2606.17443)
*Xi Chu,Yupeng Hou*

Main category: cs.AI

TL;DR: 本研究探讨了大型语言模型(LLM)推荐中的品牌动态，发现知名品牌在相同规格下垄断推荐，但微小评分优势即可打破垄断；权威营销语言能改变推荐结果，且多品牌竞争存在社会困境，提示生成引擎优化(GEO)应被视为新兴营销实践。


<details>
  <summary>Details</summary>
Motivation: 消费者越来越多通过LLM发现产品，但品牌在LLM推荐中的竞争机制尚不明确。

Method: 使用三种商业LLM（GPT-4o-mini、Claude Sonnet、Gemini 3 Flash）对护肤品（体验品）进行三个实验，并对搜索品进行稳健性检验。

Result: (1) 条件垄断：知名品牌在规格相同时获得100%推荐，但竞争对手仅需+0.1星级优势即可打破垄断； (2) 权威营销语言（如虚假临床证据）以+0.17星级偏差价值打破垄断，各模型反应不同； (3) 多品牌竞争的社会困境：所有品牌采用相同优化策略时，个体收益从+0.802降至+0.007，未参与品牌获得零推荐。

Conclusion: 生成引擎优化不仅是安全风险，更是塑造市场竞争的新兴营销实践。
...
### [397] [A Machine-Learned Comorbidity Index](https://arxiv.org/abs/2606.17450)
*Suleman Baloch,Kishlay Jha,Alberto M. Segre,Philip M. Polgreen,Bijaya Adhikari*

Main category: cs.AI

TL;DR: 提出一种机器学习合并症指数（MLCI），通过最大化nHSIC将诊断代码映射为标量，以捕捉非线性、结果特定的风险关系，在多个EHR数据集上优于传统评分。


<details>
  <summary>Details</summary>
Motivation: 传统合并症评分主要关注死亡率，且线性规则结构无法捕捉非线性、结果特定的风险关系。

Method: MLCI通过最大化归一化希尔伯特-施密特独立性准则（nHSIC）将诊断代码映射到单个标量，实现非线性风险-结果依赖性的捕捉。
...
### [398] [MapSatisfyBench: Benchmarking Satisfaction-Aware Map Agents through Behavior-Grounded Implicit Decision Factors](https://arxiv.org/abs/2606.17453)
*Lubin Bai,Mengyu Cao,Sixue Wang,Zhongwei Wan,Yue Pan,Jiale Hou,Xiang Li,Xiuyuan Zhang*

Main category: cs.AI

TL;DR: 提出MapSatisfyBench基准，评估地图服务中大语言模型代理在隐式决策因素上的满意度感知能力，发现现有代理在隐式需求满足上表现有限。


<details>
  <summary>Details</summary>
Motivation: 用户在地图服务中常提出非正式、不明确的查询，隐含许多未言明的决策因素，但传统澄清方式增加用户负担，需代理主动从可用信息中恢复这些因素。

Method: 提出恢复-识别-过滤框架，从行为链证据重建用户完整需求，识别隐式决策因素，并保留查询前可获取的证据支持的部分；基于大规模真实用户数据构建MapSatisfyBench，从五个维度标注真值。

Result: 当前代理在显式任务完成上表现良好，但在满足隐式决策因素和主动获取满意度所需证据方面能力有限。

Conclusion: MapSatisfyBench将地图代理评估从任务完成转向满意度感知的空间决策，为相关研究提供基准。
...
### [399] [Dissecting model behavior through agent trajectories](https://arxiv.org/abs/2606.17454)
*Gaurav Gupta,Vatshank Chaturvedi,Jun Huan,Anoop Deoras*

Main category: cs.AI

TL;DR: 提出意图-执行差距概念，通过简单束代理（SSA）框架缩小模型与系统间的鸿沟，提升智能体性能。


<details>
  <summary>Details</summary>
Motivation: 智能体性能受限于模型与执行系统之间的匹配度，而非仅模型能力本身。

Method: 开发可定制的SSA框架，结合常见模式与模型特定偏好，并在138k轨迹上分析行为差异。

Result: 在SWE-Pro、SWE-Verified和Terminal-Bench-2上复现/超越当前最先进的pass@1指标，并揭示模型在编辑频率、测试活动等问题解决阶段的不同行为。

Conclusion: 最小化意图-执行差距与工具和循环设计同等重要，细粒度指标有助于理解模型行为差异。
...
### [400] [Can LLMs Be CEOs? Benchmarking Strategic Resource Reallocation with Multi-Role Agent Simulation](https://arxiv.org/abs/2606.17459)
*Yuyang Dai,Xueqing Peng,Lingfei Qian,Zhuohan Xie*

Main category: cs.AI

TL;DR: 提出了CEO-Bench基准测试，评估LLM在CEO级战略资源重新分配中的决策能力，发现模型在结构有效性上表现良好，但在战略校准上存在明显不足，并揭示了集成-果断权衡等系统性问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准只关注孤立认知任务，忽视了真实高管决策中整合冲突建议、应对信息不对称和组织约束等关键挑战。

Method: 构建多智能体基准CEO-Bench，让LLM代理接收CFO、CTO、COO、CMO四类角色顾问的冲突建议，制定多轮资源分配计划，并从角色整合、条件大胆性、历史敏感性判断和计划有效性四维度评估。

Result: 五个前沿模型在13个场景中均能达到高结构有效性，但战略校准能力差异显著，暴露出单顾问捕获、模糊下保守默认和历史遗忘等失败模式，并发现结构集成与果断决策之间存在权衡。

Conclusion: 这些结果明确了LLM作为组织决策者的当前能力边界，为未来AI辅助高管系统的设计提供了方向。
...
### [401] [LLM-as-Judge in Education: A Curriculum-Grounded Marking Pipeline](https://arxiv.org/abs/2606.17507)
*Xiwei Xu,Chen Wang,Jacky Jiang,Phil Yang,Qian Fu,Mohan Dhall,Wenjie Zhang,Liming Zhu*

Main category: cs.AI

TL;DR: 提出了一个基于课程配置的LLM-as-Judge流水线，用于按题目评分，支持大学入学考试准备。


<details>
  <summary>Details</summary>
Motivation: 在高风险考试中部署LLM需要软件流水线使模型输出与授权课程文件及评分指南对齐。

Method: 流水线识别题目的主题、子主题和认知需求，构建可验证的授权上下文；通过分阶段LLM工作流生成题目特定评分标准，并评估评分标准分配分数。

Result: 初步评估显示评分结果与人类导师相当，且理由更可追溯至授权课程文件。

Conclusion: 该设计提高了一致性、透明度和与官方评分实践的吻合度，已集成到在线学习平台。
...
### [402] [SEAGym: An Evaluation Environment for Self-Evolving LLM Agents](https://arxiv.org/abs/2606.17546)
*Congjie Zheng,Chuanyi Xue,Bin Liang,Jun Yang,Changshui Zhang*

Main category: cs.AI

TL;DR: 提出SEAGym，一个用于评估自进化LLM代理框架更新效果的动态环境，通过多维度评估视图揭示更新对代理行为的影响。


<details>
  <summary>Details</summary>
Motivation: 现有评估方式将代理框架更新简化为孤立任务分数或单条曲线，无法全面判断更新是否带来可复用改进、过拟合、成本增加或损害旧行为。

Method: 将Harbor兼容基准测试转为动态任务源，包含训练批次、冻结验证、保留ID/OOD迁移视图、重放诊断及快照记录。在Terminal-Bench 2.0和HLE上实例化，比较ACE、TF-GRPO和AHE。

Result: 频繁更新可能无法提升保留性能，有用中间快照可能后来崩溃，源多样性和模型后端影响代理框架可靠性。

Conclusion: 多视角评估能提供互补信号，SEAGym有助于系统分析自进化过程。
...
### [403] [DeepInsight: A Unified Evaluation Infrastructure Across the Physical AI Stack](https://arxiv.org/abs/2606.17574)
*Siyi Li,Chunyu Sun,Jiahao Zhang,Yuchen Kang,Wuliang Wang,Yu Qiu,Rui Jiang,Haitao Cui,Jie Chen*

Main category: cs.AI

TL;DR: DeepInsight 是一个统一的评估基础设施，用于 Physical AI 堆栈，覆盖从单步解码到全身控制的多个量级，通过三个抽象（任务、资源、结果）实现异质性的保留，并支持跨层诊断。


<details>
  <summary>Details</summary>
Motivation: 现有的评估框架无法覆盖从基础模型解码到全身控制等跨越三个数量级的算子，且缺乏统一的运行时和评分，导致无法诊断跨层回归问题。

Method: DeepInsight 通过三个窄抽象（任务、资源、结果）来保留异质性，每个抽象由所有子系统共享的实现：一个情节驱动器、一个资源句柄协议（用于 LLM 推理和沙箱运行时），以及一个跟踪标识方案（用于所有事件）。

Result: 在完整的人形机器人堆栈中部署后，该框架可通过配置接入新基准；在基础模型端，能复现已发表结果并在单个节点上更快运行相同套件，且近线性扩展；其独特优势在于诊断能力：所有层写入同一跟踪，使得跨层回归可定位。

Conclusion: DeepInsight 通过统一的运行时和跟踪机制，实现了跨层诊断，这是任何分段夹具联合都无法复现的收益。
...
### [404] [Surrogate Assisted Pedestrian Protection Design via a Foundation Model Orchestrated Workflow](https://arxiv.org/abs/2606.17577)
*Osamu Ito,Akihiko Katagiri,Yoshikazu Nakagawa,Shin Saeki,Jun Shiraishi,Masato Sasaki*

Main category: cs.AI

TL;DR: 首次提出基于基础模型的碰撞安全设计工作流，通过代理模型辅助行人保护探索，将评估时间从小时级降至秒级。


<details>
  <summary>Details</summary>
Motivation: 碰撞安全设计面临高度非线性接触动力学、材料非线性和离散状态转换等挑战，数据驱动代理模型难以捕捉。

Method: 工作流集成四个组件：(1) 基于CAE碰撞模拟的代理模型预测行人腿部损伤指标(R²=0.87)，提供无分布置信区间；(2) 多目标进化搜索(NSGA-II)发现多样可行参数集；(3) 基于变形的几何生成器将参数映射为拓扑保持的3D形状；(4) 自然语言接口，由LLM编排工作流，VLM支持设计语义比较。

Result: 在汽车前保险杠案例中，单次探索生成35个不同的安全合规替代方案，传统CAE迭代需要数周。

Conclusion: 基础模型可作为机器学习代理与物理模拟之间的集成层，将AI能力引入安全关键工程领域。
...
### [405] [Closing the Feedback Loop: From Experience Extraction to Insight Governance in Verbal Reinforcement Learning](https://arxiv.org/abs/2606.17591)
*Yanwei Cui,Xing Zhang,Yulong Zhang,Li Shao,Xiaofeng Shi,Guanghui Wang,Peiyang He*

Main category: cs.AI

TL;DR: 提出三层架构（规则、证据、技能）与反馈驱动的策展循环，解决无训练强化学习的保留-遗忘困境，在金融预测中显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 非平稳环境中，保留旧见解导致负迁移，丢弃则灾难性遗忘，现有方法重提取轻治理。

Method: 构建规则（经验提取）、证据（可靠性追踪）、技能（应用与冲突管理）三层，通过反馈策展循环治理经验。

Result: 相同经验在无策展时性能低于零样本，有策展时显著提升准确性与风险调整收益。

Conclusion: 反馈驱动的策展循环有效弥补治理缺口，使经验成为持续学习的资产。
...
### [406] [Brick-DICL: Dynamic In-Context Learning for Automated Brick Schema Classification](https://arxiv.org/abs/2606.17637)
*Yiyue Qian,Shinan Zhang,Huan Song,Negin Sokhandan,Hannah Marlowe,Diego Socolinsky*

Main category: cs.AI

TL;DR: 提出Brick-DICL，一种两阶段动态情境学习框架，用于自动化Brick模式分类，解决BMS点标准化难题，提升分类精度并减少人工验证。


<details>
  <summary>Details</summary>
Motivation: BMS点缺乏标准化，Brick模式类数量庞大（936个），大模型领域知识不足，人工验证繁琐，阻碍集成与数据利用。

Method: Brick-DICL包含元数据检索增强生成和类检索增强生成两阶段，动态提供相关示例并缩小候选类空间，同时采用多语言模型过滤机制标记低置信度分类供人工审核。

Result: 适用于任何BMS，首次采用动态情境学习，精度显著优于现有方法；多模型过滤策略减少人工验证，加速数字建筑接入。

Conclusion: 实验证明Brick-DICL在多种建筑数据集上有效，推动标准化、可互操作的建筑管理系统发展。
...
### [407] [FinAcumen: Financial Multimodal Reasoning via Self-Evolving Experience Memory Harness](https://arxiv.org/abs/2606.17642)
*Pianran Guo,Pengcheng Zhou,Yucheng Jian,Shuhua Chen*

Main category: cs.AI

TL;DR: FinAcumen是一个金融推理智能体框架，通过选择性经验记忆改进工具增强的多模态推理，在四个基准上提升了8B视觉语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有工具增强智能体缺乏跨回合的状态记忆，导致不稳定的工具路由、噪声检索和易幻觉推理，尤其是在高风险金融场景中。

Method: FinAcumen积累金融相关的推理经验，提取成功策略和失败教训形成持久记忆库。推理时仅在语义相关性超过阈值时检索经验，否则通过fallback机制抑制无关记忆。使用确定性金融工具环境。

Result: 在四个金融多模态推理基准上，FinAcumen一致提升了冻结的8B视觉语言模型，超过了金融专用模型，并接近领先的专有通用模型。选择性经验激活提高了检索不确定性下的推理可靠性。

Conclusion: FinAcumen通过选择性经验记忆有效提高了金融多模态推理的可靠性和性能。
...
### [408] [Beyond Domains: Reusing Web Skills via Transferable Interaction Patterns](https://arxiv.org/abs/2606.17645)
*Shiqi He,Yue Cui,Feijie Wu,Xinyu Ma,Jiaheng Lu,Yaliang Li,Bolin Ding,Mosharaf Chowdhury*

Main category: cs.AI

TL;DR: SkillMigrator通过学习可复用网络技能并基于布局结构跨站点转移，有效减少LLM动作次数，提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有网络代理技能库主要依赖指令相似性或站点元数据触发，导致技能在未见站点上重用率低，步骤和令牌缩减潜力未能充分发挥。

Method: 将每个技能存储为可转移交互模式（TIP），包含技能及其诱导时的结构草图；测试时通过布局相似性检索TIP，并在实时页面中接地其引用。

Result: 在WebArena和Mind2Mind上，成功轨迹的平均LLM动作计数相比当前最优方法减少8-10%，且成功率持平。

Conclusion: 基于布局结构匹配的技能转移方法能有效提高技能复用，降低代理的推理开销，同时保持任务表现。
...
### [409] [From Brewing to Resolution: Tracing the Internal Lifecycle of Code Reasoning in LLMs](https://arxiv.org/abs/2606.17648)
...
TL;DR: LLM代码推理存在“酝酿”过程，线性探针可早期恢复答案，但最终解析结果分为四种类型。标准精度指标掩盖了不同失败模式。


<details>
  <summary>Details</summary>
Motivation: 标准准确率无法解释LLM为何能处理变量跟踪但在语义等价的循环上失败，需要理解代码推理的内部生命周期。

Method: 提出双层诊断框架：逐层线性探针与上下文剥离解码（CSD），在6种代码推理任务族、16个模型上应用。

Result: 四种解析结果均占有显著比例（总体解决率仅41.5%），函数调用解决率随调用深度从61.1%降至2.5%；酝酿持续时间稳定在24-42%，解决成功率随能力、规模和训练变化。

Conclusion: 酝酿支架是测试的仅解码器Transformer族中的稳定经验规律，而解析结果与能力、规模和训练共变。
...
### [410] [Using Cognitive Models to Improve Language Model Simulation of Human Persuasion Games](https://arxiv.org/abs/2606.17657)
*Zirui Cheng,Zeyu Shen,Thomas L. Griffiths,Peter Henderson*

Main category: cs.AI

TL;DR: 提出方程到行为提示法引导语言模型匹配认知模型，解决模拟人类决策多样性的不足，并用强化学习训练小模型降低信念误差。


<details>
  <summary>Details</summary>
Motivation: 大语言模型模拟人类时未覆盖贝叶斯更新、动机推理等多样化决策行为，需引入认知数学模型。

Method: 方程到行为提示（Equation-to-Behavior Prompting）及方程到行为强化学习（Equation-to-Behavior RL），使模型遵循数学规则。

Result: 大模型可通过提示近似方程规范，小模型需强化学习训练，信念误差降低26.5%；训练考虑多样决策者的模型在说服任务中平均信念变化提升2.5%-12%。

Conclusion: 该方法可改进真实场景中的人类模拟，用于训练和评估，并促进更复杂数学决策模型的研究。
...
### [411] [FllumaOne: A Code-Native Multimodal CAD Dataset with Executable Programs and Kernel-Validated Feature Histories](https://arxiv.org/abs/2606.17696)
*Jizong Zhan*

Main category: cs.AI

TL;DR: FllumaOne是一个由可执行Python程序生成的代码原生多模态CAD数据集，包含10万个样本，支持多种编辑和逆向工程任务。


<details>
  <summary>Details</summary>
Motivation: 现有数据集缺乏同时包含几何和编辑历史的数据，FllumaOne通过记录建模操作、参数和特征依赖关系，为可编辑CAD研究提供基础。

Method: 使用Flluma系统（基于Qt/C++和OpenCASCADE）生成可执行Python程序，每个样本包含程序、特征树、STEP几何、点云、自然语言描述等模态数据，并通过几何验证和导出检查。

Result: 基于Qwen2.5-Coder-1.5B LoRA的基线在测试集上达到99.98%语法有效性、99.97%构建成功率、99.14%STEP导出有效性，点云归一化Chamfer距离为0.002124。

Conclusion: 该数据集可用于条件CAD重建、可执行程序合成、特征树预测、B-Rep分析、检索、设计补全和可编辑逆向工程等任务。
...
### [412] [EComAgentBench: Benchmarking Shopping Agents on Long-Horizon Tasks with Distributed Hidden Intent](https://arxiv.org/abs/2606.17698)
*Zeyao Du,Tong Li,Haibo Zhang*

Main category: cs.AI

TL;DR: 提出了EComAgentBench基准，包含662个基于真实亚马逊产品的任务，用于评估购物代理在长时间范围内发现隐性需求的能力，最强的模型仅达到57.1%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法捕捉购物需求以隐含、配置文件或需澄清的方式到达，且仅评估最终选择，无法解释代理遗漏了哪个需求。

Method: 自动构建662个任务，每个任务将需求分散在可见查询、工具门控配置文件和脚本化澄清中，代理需在100次工具调用内发现隐性意图并选出产品，使用带类型和源标签的评分规则进行细粒度评估。

Result: 评估7个模型，最强模型总体准确率仅57.1%，且从可见到隐藏需求源的评分满意度递减。

Conclusion: EComAgentBench为购物代理从单查询搜索向长期可靠协助提供了可重复的评估基础。
...
### [413] [LongWebBench: Evaluating Structural and Functional Webpage Generation in Long-Horizon Settings](https://arxiv.org/abs/2606.17727)
*Yi Zhao,Zhen Yang,Mengpan Chen,Mingde Xu,Shanghui Gong,Xijun Liu,Jibing Gong,Jie Tang*

Main category: cs.AI

TL;DR: 提出LongWebBench基准，从结构和功能两方面评估长网页生成任务。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注短、单屏、静态网页，缺乏对长网页生成效果的评估。

Method: 包含490个真实长网页用于结构评估，507个交互任务用于功能评估；采用多维VLM指标和DOM增强agent管道。

Result: 结构保真度随网页长度下降；视觉合理的网页常无法支持可执行的交互。

Conclusion: 长网页生成评估需超越视觉相似性，以可执行交互为核心标准。
...
### [414] [Shattering the Autoregressive Curse: Dynamic Epistemic Entropy Orchestrated Erasable Reinforcement Learning for LLMs](https://arxiv.org/abs/2606.17735)
*Ziliang Wang,Kang An,Faqiang Qian,Jialu Cai,Cijun Ouyang,Yuhang Wang,Qibing Ren,Yichao Wu*

Main category: cs.AI

TL;DR: 提出E^3RL方法，通过动态熵坐标和自适应阈值，使LLM在长序列推理中自愈，打破自回归诅咒。


<details>
  <summary>Details</summary>
Motivation: 强化学习在长逻辑推理中受自回归诅咒影响，早期微小错误会级联放大，导致推理失败。

Method: 动态epistemic熵编排可擦除强化学习，利用模型内生的自回归交叉熵作为不确定性坐标，引入段级自适应动态阈值和优势分配，精准切除逻辑缺陷并重用KV缓存。

Result: 在DeepMath-103k训练，AIME等数学推理基准上，4B和8B模型分别超SOTA 5.349%和6.514%。

Conclusion: E^3RL打破了长序列推理的自回归诅咒，为自愈AGI奠定基础。
...
### [415] [DecoSearch: Complexity-Aware Routing and Plan-Level Repair for Text-to-SQL](https://arxiv.org/abs/2606.17821)
*Esteban Schafir,Xu Zheng,Hojat Allah Salehi,Zhuomin Chen,Mo Sha,Wei Cheng,Dongsheng Luo*

Main category: cs.AI

TL;DR: DecoSearch是一个无需训练的框架，通过将查询路由到适当的推理努力级别，实现了自然语言到SQL的高效转换，在复杂查询上表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在将自然语言翻译为SQL时，对需要多步、数据感知推理的复杂查询仍处理不力。

Method: 轻量级模式选择器修剪数据库模式；LLM判断器决定是否分解查询；复杂查询构造为有向无环图（DAG）的原子子问题，每个子问题生成SQL；RAG组件用相似训练示例辅助分解；拓扑优化器在执行失败且非SQL错误时重构推理计划。

Result: 在BIRD上达70.53%执行准确率，在Spider上达88.31%（DeepSeek骨架），超越所有无需训练的基线，且消耗令牌数量少一个数量级；作为模型无关的包装器，能持续提升微调后的SQL生成模型。

Conclusion: DecoSearch有效解决了复杂SQL生成中的多步推理问题，高效且通用，无需训练即可提升性能。
...
### [416] [WallZero: Mastering the Game of WallGo with Strategic Analysis](https://arxiv.org/abs/2606.17847)
*Hsing-Yu Chen,J\'er\^ome Arjonilla,I-Chen Wu,Ti-Rong Wu*

Main category: cs.AI

TL;DR: 本文提出了WallZero，一个基于AlphaZero的WallGo智能体，通过定制化设计显著提升性能，并击败职业选手，同时评估了游戏公平性与策略。


<details>
  <summary>Details</summary>
Motivation: WallGo虽流行但研究不足，需要强AI探索游戏策略。

Method: 采用AlphaZero框架，并针对WallGo定制了动作表示和特征设计。

Result: WallZero击败了两名职业围棋选手，平均每局获得1.98倍领地；发现Netflix开局更平衡。

Conclusion: WallZero展现了强大实力，并揭示了游戏平衡性和关键策略。
...
### [417] [A homotopy-type-theoretic generalization of neurosymbolic inference](https://arxiv.org/abs/2606.17851)
*Fernando Zhapa-Camacho,Robert Hoehndorf*

Main category: cs.AI

TL;DR: 该文提出基于同伦类型论的神经符号系统框架，通过保持对称性和证明计数，将传统加权和转为加权同伦基数，在MNIST基准上实现更好的校准。


<details>
  <summary>Details</summary>
Motivation: 现有神经符号系统忽略σ-结构的对称性和不同证明的计数，导致推理捷径问题。

Method: 用同伦类型论中的类型代替集合，定义基于加权同伦基数的功能函数，证明保守性定理。

Result: 在MNIST推理捷径基准上，单模型封装比多样性训练集成校准更好，准确性不变。

Conclusion: 该框架公开了推理捷径背后的对称性，提供了计算捷径感知后验的闭合形式，改进了校准。
...
### [418] [FlowRAG: Synergizing Explicit Reasoning via Frequency-Aware Multi-Granularity Graph Flow](https://arxiv.org/abs/2606.17856)
*Bihao Zhan,Zongsheng Cao,Jie Zhou,Bo Zhang,Liang He*

Main category: cs.AI

TL;DR: FlowRAG通过构建四层异质图和双粒度激活、频率加权流模块，提升了语义召回和显式推理，在复杂推理基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的RAG方法在抽象查询和语义稀疏时检索不足，且多跳推理易受噪声干扰导致关系链断裂。

Method: 构建包含段落、摘要、句子和实体的四层异质图，摘要节点作为语义枢纽；检索时使用摘要-查询对齐和句子级匹配双粒度激活相关实体；引入频率加权的流模块，通过实体-段落边权重（基于词频）路由相关性，修剪噪声连接并提取高置信度推理路径作为显式逻辑骨架。

Result: 在复杂推理基准上取得最先进性能。

Conclusion: FlowRAG有效解决了抽象查询下的语义召回和多跳推理的鲁棒性问题。
...
### [419] [StepGuard: Guarding Web Navigation via Single-Step Calibration](https://arxiv.org/abs/2606.17871)
*Zhihao Cui,Yuchen Zhang,Xiyang Sun,Yaxiong Wang,Li Zhu,Jinpeng Hu,Liu Liu,Mengjia Li,Yujiao Wu*

Main category: cs.AI

TL;DR: 提出StepGuard框架，通过动态双策略优化和置信度引导的自适应导航反思来校准单步错误，提升Web导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有Web导航方法因奖励错配和错误传播导致单步脆弱性。

Method: 包括动态双策略优化（DDPO）在导航优先和答案优先模式间切换，以及置信度引导的自适应导航反思（CANR）用于自我纠正。

Result: 在标准基准上取得新最先进性能，显著提升导航和答案准确性。

Conclusion: 通过单步校准有效改善Web导航。
...
### [420] [Structural Preservation and the Logical Expressiveness of Graph Neural Networks](https://arxiv.org/abs/2606.17882)
*Przemys{\l}aw Andrzej Wa{\l}\k{e}ga,Bernardo Cuenca Grau*

Main category: cs.AI

TL;DR: 从语义角度研究了图神经网络分类器在保持结构性质（嵌入、单同态、同态）下的逻辑表达能力，得到与分级模态逻辑片段的对应关系。


<details>
  <summary>Details</summary>
Motivation: 以往通过固定架构选择建立GNN与逻辑形式的紧密对应，本文希望从语义角度独立于具体架构刻画GNN的表达能力。

Method: 分析GNN分类器在嵌入、单同态、同态三种结构性质下的保持性，分别对应存在分级模态逻辑、存在正片段、存在正模态逻辑。使用有界高度树的新良拟序结果。

Result: 每一类GNN都有对应的逻辑片段刻画其表达能力，且存在同样表达能力的GNN架构。

Conclusion: 从语义上概括了广泛GNN类的表达能力，不依赖具体架构选择。
...
### [421] [MathVis-Fine: Aligning Visual Supervision with Necessity via Progressive Dependency-Guided Training for Multimodal Mathematical Reasoning](https://arxiv.org/abs/2606.17888)
*Wanshi Xu,Haokun Zhao,Haidong Yuan,Songjun Cao,Long Ma*

Main category: cs.AI

TL;DR: 提出MathVis-Fine框架，通过细粒度视觉标注和两阶段训练，根据视觉依赖程度平衡奖励，提升多模态数学推理的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将视觉输入视为同质化或辅助信号，未能捕捉文本与图像间细粒度的样本特异性依赖，导致监督信号粗糙、训练反馈不准确。

Method: 构建MathVis-Fine数据集，包含细粒度视觉标注和视觉依赖评分；引入两阶段渐进式视觉增强训练，根据样本的视觉依赖程度平衡答案正确性奖励和视觉定位奖励。

Result: 实验表明，该框架能根据视觉依赖逐步增强视觉感知，为多模态数学推理提供更精确的训练范式。

Conclusion: MathVis-Fine通过细粒度视觉依赖建模和自适应奖励机制，有效改善了多模态数学推理的性能。
...
### [422] [Learn to Quantify Social Interaction with Constraints for Pedestrian Walking](https://arxiv.org/abs/2606.17897)
*Xiaodan Shi*

Main category: cs.AI

TL;DR: 提出了一种无标签的聚类方法，通过潜在变量量化社交互动模式，提升行人轨迹预测的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前轨迹预测方法未揭示具体社交互动类型及其对决策的影响，导致鲁棒性不足。需要量化并理解行人间的社交互动。

Method: 提出Learn to Cluster，一种概率潜在变量生成模型，直接从序列轨迹观测中学习，无需标签，可扩展至任意人数，并自然集成到预测模型训练中。

Result: 在多个轨迹预测基准上，该方法能有效学习社交互动模式，并将其整合到行人轨迹预测中，提升性能。

Conclusion: 通过无监督聚类量化社交互动，可显著改善长期行人路径预测的准确性和鲁棒性。
...
### [423] [DiagFlowBench: Evaluating How Language Models Handle Off-Procedure Inputs in Grounded Diagnostic Dialogue](https://arxiv.org/abs/2606.17904)
*Guillermo Gil de Avalle,Laura Maruster,Shaina Raza,Christos Emmanouilidis*

Main category: cs.AI

TL;DR: 提出DiagFlowBench数据集，用于评估语言模型在工业诊断对话中识别超出范围查询的能力，发现模型常选择不恰当步骤而非虚构事实。


<details>
  <summary>Details</summary>
Motivation: 现有基准很少评估模型在对话中识别超出范围输入的能力，而实际操作中查询常偏离程序文档。

Method: 从50个工业诊断流程图生成1676个多轮对话，包含合规和超出范围话语，评估10个商业和开源模型。

Result: 模型在弃权率上高度可变，常见错误是选择真实但在上下文中不恰当的步骤，而非捏造事实；这种映射错误构成了接地系统的脆弱性。

Conclusion: DiagFlowBench揭示了接地系统面对看似合理但错误的建议时的挑战，需要改进模型的识别能力。
...
### [424] [PreAct: Computer-Using Agents that Get Faster on Repeated Tasks](https://arxiv.org/abs/2606.17929)
*Bojie Li*

Main category: cs.AI

TL;DR: PreAct 通过将成功任务运行编译为状态机程序，使计算机使用代理在重复任务时速度提升 8.5-13 倍，且无需每次调用语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有计算机使用代理每次从头开始解决任务，重复任务时成本高昂。

Method: 首次成功运行后，PreAct 将运行编译为小型状态机程序，后续直接重放该程序，并在每个步骤检查屏幕匹配，若不匹配则交回代理。

Result: 重放速度提升 8.5-13 倍，无需每步语言模型调用；存储时检查确保程序有效，避免退化，三个基准上提升 1.75-2.6 个任务。

Conclusion: PreAct 有效提升了重复任务效率，且不依赖提示措辞、运行时护栏或检索器类型。
...
### [425] [How Inference Compute Shapes Frontier LLM Evaluation](https://arxiv.org/abs/2606.17930)
*Jessica McFadyen,Ole Jorgensen,Harry Coppock,Kevin Wei,Cozmin Ududec*

Main category: cs.AI

TL;DR: AI评估中，推理计算预算显著影响模型性能，固定预算可能低估能力，建议报告性能随推理计算变化的函数。


<details>
  <summary>Details</summary>
Motivation: 当前评估多采用单一限制性预算，导致低分可能反映评估设置而非模型能力，需要测试推理计算对性能的影响。

Method: 评估12个前沿模型在7个挑战基准上，使用三种推理扩展干预：更大token预算、上下文压缩、重复提交尝试，结合模型自引导或最小正确性反馈。

Result: 更大token预算显著提升多领域性能；固定预算低估前沿能力，新模型在大预算下性能更高；不同基准对不同推理扩展方法敏感。
...
### [426] [Small Initialization Matters for Large Language Models](https://arxiv.org/abs/2606.17945)
*Liangkai Hang,Junjie Yao,Zhiyu Li,Feiyu Xiong,Hongkang Yang,Zhi-Qin John Xu*

Main category: cs.AI

TL;DR: 参数初始化是大型语言模型能力的关键基因，减小初始化规模可提升预训练性能，尤其在推理任务上。


<details>
  <summary>Details</summary>
Motivation: 探索参数初始化如何影响大型语言模型的智能涌现，而不仅仅是工程优化。

Method: 通过调整初始化规模，分析在不同设置下的预训练效果，并发现关键初始化点；进行机制分析，研究参数发展轨迹和token级预测。

Result: 减小初始化规模一致提升预训练效果，尤其在推理任务上；识别出两种限制小初始化优势的设置；发现关键初始化点；小初始化驱动低复杂度结构向丰富表示发展；收益集中在困难预测上。

Conclusion: 提出γ-初始化规则，建议默认使用小初始化，这是一种几乎无成本的干预，能改善预训练和推理能力。
...
### [427] [MoCo-AIS: A Contrastive Learning Framework for Similarity Computation of Vessel Trajectories](https://arxiv.org/abs/2606.17978)
*Ruixin Song,Md Mahbub Alam,Zahra Sadeghi,Amilcar Soares,Jos\'e F. Rodrigues-Jr,Gabriel Spadon*

Main category: cs.AI

TL;DR: 提出了基于动量对比（MoCo）的统一框架MoCo-AIS，用于学习船舶轨迹嵌入，并在大规模AIS数据集上显著提升了相似性学习效果。


<details>
  <summary>Details</summary>
Motivation: 为了解决传统距离度量计算成本高、监督方法依赖标签且泛化性差、自监督方法缺乏统一框架的问题。

Method: 基于动量对比（MoCo）构建统一框架，通过正负轨迹对进行相似性学习，并在多种深度学习模型上进行评估。

Result: 在真实大规模AIS数据集上，该框架显著优于现有基线，并提供了轨迹表示模型的基准测试平台。

Conclusion: MoCo-AIS框架为学习船舶轨迹嵌入提供了统一且有效的方法，并可作为评估轨迹表示模型的基准。
...
### [428] [STAR: SpatioTemporal Adaptive Reward Allocation for Text-to-Image RL Post-Training](https://arxiv.org/abs/2606.17979)
*Jinjie Shen,Wei Deng,Xian Hu,Daiguo Zhou,Jian Luan*

Main category: cs.AI

TL;DR: 提出STAR方法，利用文本-图像注意力动态分配奖励，提升文本到图像生成的语义对齐和渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有RL后训练方法将图像奖励均匀应用于整个生成轨迹，忽略了去噪步骤的时空结构，导致策略更新难以聚焦于影响奖励的生成组件。

Method: STAR通过文本-图像注意力构造空间分配图，随去噪步骤和rollout动态变化，将组相对优势分配给更相关的潜在区域，并使用空间解析策略目标加强更新。

Result: 在GenEval、OCR和PickScore上分别达到0.9759、0.9757和23.60，优于基线。
...
### [429] [LLM Consumer Behavior Theory: Foundations of a Novel Research Field](https://arxiv.org/abs/2606.18005)
*Manon Reusens,Sofie Goethals,David Martens*

Main category: cs.AI

TL;DR: 提出LLM消费者行为理论，研究代理人市场中的人类偏好反映与加总，并指出理性假设可能失效及未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型越来越多地作为自主代理人做出消费决策，挑战了传统消费者理论将人类视为主要决策者的假设。

Method: 借鉴古典与行为经济学以及自然语言处理进展，形式化LLM代理人如何反映人类偏好及其决策如何加总为市场需求。

Result: 统一了之前零散的文献，强调了理性与异质性假设在代理人市场中可能失效，未提供实证验证。

Conclusion: 勾勒了LLM消费者行为的范围，并识别了与对齐、偏好表示和市场动态相关的开放性研究问题。
...
### [430] [LegalHalluLens: Typed Hallucination Auditing and Calibrated Multi-Agent Debate for Trustworthy Legal AI](https://arxiv.org/abs/2606.18021)
*Lalit Yadav,Akshaj Gurugubelli*

Main category: cs.AI

TL;DR: 提出 LegalHalluLens 审计框架，通过类型化幻觉剖绘、风险方向指数和辩论管道，揭示法律AI中聚合指标掩盖的错误模式，并将虚假检测减少45%。


<details>
  <summary>Details</summary>
Motivation: 法律AI系统幻觉率约52%，但平均值掩盖错误集中位置和方向，合规官无法获得可部署的可靠信号。

Method: 包含三个组件：基于CUAD数据集的四类法律主张（数字、时间、义务/权利、事实）的幻觉剖绘；风险方向指数（RDI）量化遗漏与编造偏差；类型化辩论管道根据幅度和方向校准。

Result: 在510份合同和249,252条实例上，发现义务/数字与时间类主张间模型内差距约38-40个百分点；两个52%幻觉率的系统可具有相反RDI；辩论管道减少45%虚假检测，与4B参数商业API匹配。

Conclusion: 类型化剖绘和RDI暴露聚合指标隐藏的失败模式，可作为多智能体辩论的校准输入，支持法律AI的方向感知采购、问责和代理设计。
...
### [431] [ProvenanceGuard: Source-Aware Factuality Verification for MCP-Based LLM Agents](https://arxiv.org/abs/2606.18037)
*Ander Alvarez,Santhiya Rajan,Samuel Mugel,Rom\'an Or\'us*

Main category: cs.AI

TL;DR: 本文提出ProvenanceGuard，一种针对MCP代理的源感知验证器，用于检测跨源混淆问题。在医学领域实验中，它有效检测归因错误并修复被阻止的答案，表明源归因是事实性验证的独立维度。


<details>
  <summary>Details</summary>
Motivation: 标准事实性指标忽略源归属错误，即声明被支持但归因于错误来源，需要一种源感知的验证方法。

Method: ProvenanceGuard通过处理MCP轨迹，将答案分解为原子声明，路由到源特定证据，结合NLI和令牌对齐检查支持，比较归因与路由源，输出每声明判决和答案级别阻止/允许决定，并支持检索增强的答案修订。

Result: 在医学领域281个轨迹上评估，保留40个轨迹中阻塞F1为0.802，源准确率0.858；多源基准中阻塞F1为0.846但源加关系准确率仅0.229；修复和重新验证解决了所有被阻止答案；50个受控探测中检测所有归因交换。

Conclusion: 源归因是基于MCP的代理中事实性验证的一个独立且重要的轴，ProvenanceGuard有效识别和修复跨源混淆问题。
...
### [432] [PseudoBench: Measuring How Agentic Auto-Research Fuels Pseudoscience](https://arxiv.org/abs/2606.18060)
*Xinyang Liao,Lingyu Li,Huacan Liu,Tianle Gu,Yang Yao,Tong Zhu,Yan Teng,Yingchun Wang*

Main category: cs.AI

TL;DR: 提出PseudoBench基准测试，评估AI科研代理抵抗伪科学的能力，发现当前系统极易生成有说服力的伪科学报告，拒绝率极低，最强抵抗率仅27.4%。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的自主科研系统可能产生看似合理却误导性的研究，污染学术文献，因此需要评估其抵抗伪科学的能力。

Method: 构建包含200个伪科学声称-证据对的PseudoBench基准，涵盖五个领域，通过从实验到写作的端到端研究流程评估代理。

Result: 测试七个最先进代理，发现它们几乎不拒绝伪科学前提，最高抵抗率仅27.4%，且更强代理会用更复杂的科学语言包装伪科学，增加其表面可信度。
...
### [433] [Agentic AI-based Framework for Mitigating Premature Diagnostic Handoff and Silent Hallucination in Healthcare Applications](https://arxiv.org/abs/2606.18068)
*Divyansh Srivastava,Shreya Ghosh,Anshul Verma,Rajkumar Buyya*

Main category: cs.AI

TL;DR: 提出一个多智能体框架，通过确定性编排约束和两种安全机制（神经符号状态追踪门和认知不确定性量化门）解决医疗对话中过早诊断交接和无声临床幻觉问题，在150个测试案例中诊断精度提升11.3个百分点。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型和多智能体系统在医疗推理中虽有前景，但开放式对话智能体容易出现过早诊断交接和无声临床幻觉，这些错误在到达患者前可能未被发现。

Method: 采用多智能体框架，用确定性编排约束替代"LLM作为裁判"的路由。包含两个安全机制：基于神经符号的状态追踪门强制OLDCARTS临床协议完整性，阻止诊断转换直到收集完所有维度；认知不确定性量化门基于5个独立诊断样本计算语义熵，识别并拦截发散输出。

Result: 在150个测试案例中，完整架构达到49.3%的诊断精度，比无约束基线绝对提高11.3个百分点。OLDCARTS完整性与语义熵之间存在统计显著的负相关（r=-0.181, p<0.05）。

Conclusion: 提出的多智能体框架能有效提高诊断精度，减少诊断不确定性，结构化信息收集有助于降低诊断不确定性。
...
### [434] [A Unified Framework for Context-Aware and Relation-Aware Graph Retrieval-Augmented Generation](https://arxiv.org/abs/2606.18075)
*Haoyang Zhong,Yifei Sun,Antong Zhang,Chunping Wang,Lei Chen,Yang Yang*

Main category: cs.AI

TL;DR: 提出HyGRAG，一种分层图检索增强生成框架，通过整合上下文和关系信息生成摘要，实现多跳推理平均准确率提升9.7%。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的RAG方法，无论是实体中心还是块中心，都只能基于原始文本进行检索，缺乏真正的知识融合，无法获取合成后的涌现理解。

Method: 构建包含块和实体节点的混合图，迭代聚类并生成基于LLM的摘要；设计上下文和关系感知的检索策略，跨抽象层级搜索并通过社区成员扩展；采用基于附件的动态更新算法，仅需局部重摘要。

Result: 多跳推理任务平均准确率提升9.7%，且保持合理效率。

Conclusion: HyGRAG通过层次化结构和摘要生成，有效融合知识，提升了检索增强生成的效果。
...
### [435] [IsabeLLM: Automated Theorem Proving Applied to Formally Verifying Consensus](https://arxiv.org/abs/2606.18098)
*Elliot Jones,William Knottenbelt*

Main category: cs.AI

TL;DR: 本文改进了IsabeLLM，通过引入检索增强生成、错误追踪和反例生成，以及更新对Isabelle和Sledgehammer的兼容性，提升了其在验证比特币工作量证明共识协议方面的性能。


<details>
  <summary>Details</summary>
Motivation: 传统形式化验证仅用于安全关键系统，而区块链系统易受攻击，需要更好的验证方法。共识协议是区块链的核心，因此提高其验证效率至关重要。

Method: 实现检索增强生成框架、错误追踪和反例生成以改善大语言模型的上下文，并更新与最新Isabelle和Sledgehammer的兼容性。

Result: 改进后的IsabeLLM在完成比特币工作量证明共识的验证任务中性能优于原版本。

Conclusion: 通过增强的上下文提供和工具更新，自动化定理证明工具在验证区块链共识协议方面更加高效和可靠。
...
### [436] [Trust the Right Teacher: Quality-Aware Self-Distillation for GUI Grounding](https://arxiv.org/abs/2606.18101)
*Jingyuan Huang,Zuming Huang,Yucheng Shi,Tianze Yang,Xiaoming Zhai,Wei Chu,Ninghao Liu*

Main category: cs.AI

TL;DR: 提出了一种质量感知自蒸馏方法，通过软正确性门控和教师概率缩放改善GUI接地中的坐标标记教师信号质量。


<details>
  <summary>Details</summary>
Motivation: 现有OPSD在GUI接地中因前缀偏离目标导致教师信号不可靠，需要改进。

Method: 采用软正确性门控检查教师预测能否完成真实框，并缩放信号；结合教师概率缩放进一步校准监督强度。

Result: 单独使用任一组件均无效，组合后一致提升性能。

Conclusion: 该方法在六个GUI接地基准上持续提升基础模型并超越强基线。
...
### [437] [First Proof Second Batch](https://arxiv.org/abs/2606.18119)
*Mohammed Abouzaid,Nikhil Srivastava,Rachel Ward,Lauren Williams*

Main category: cs.AI

TL;DR: 测试当前AI系统在解决研究级数学问题上的能力，使用了十位数学家提供的十个问题，并记录了测试方法、结果及补充材料。


<details>
  <summary>Details</summary>
Motivation: 评估AI系统正确解决研究级数学问题的能力。

Method: 选取十个来自不同数学领域的研究级问题，对多个AI系统进行测试，并记录人类解法、AI解法及裁判报告。

Result: 提供了问题、方法论、测试结果以及补充文档链接，包括人类解法、AI解法和裁判日志。

Conclusion: 通过系统测试，揭示了当前AI在解决研究级数学问题上的表现和能力局限。
...
### [438] [Knowledge Reutilization in Meta-Reinforcement Learning](https://arxiv.org/abs/2606.18132)
*Yuan Meng,Bo Wang,Juan de los Rios Ruiz,Xiangtong Yao,Zhenshan Bing,Fuchun Sun,Alois Knoll*

Main category: cs.AI

TL;DR: 提出元知识重用框架，从简化智能体学习任务知识并迁移到异构智能体，大幅降低跟踪误差并提高样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有端到端元强化学习将任务推理与具身控制耦合，导致任务语义不清晰、样本效率低、跨智能体重用受限。

Method: 框架包括在动力学简化智能体上学习元知识（贝叶斯非参数先验组织任务模式，高层策略生成幅度指导），并通过语义-幅度接口和时域适配器迁移到异构智能体。

Result: 在多个运动智能体上，最终步跟踪误差降低94.75%-99.79%，仅用约23.8%交互数据达到相当性能。

Conclusion: 提出的框架有效解耦任务知识与具身控制，实现高效跨智能体迁移。
...
### [439] [Your AI Travel Agent Would Book You a Bullfight: An Agentic Benchmark for Implicit Animal Welfare in Frontier AI Models](https://arxiv.org/abs/2606.18142)
*Jasmine Brazilek,Oliver Tulio,Joel Christoph,Miles Tidmarsh,Carol Kline,Arturs Kanepajs*

Main category: cs.AI

TL;DR: 一个衡量AI代理在旅行预订中避免动物剥削的基准测试，结果显示所有模型均低于随机水平。


<details>
  <summary>Details</summary>
Motivation: 现有基准只评估文本回答，未检验实际代理行为中的动物福利推理。

Method: 设计了包含12个旅行预订场景（涵盖6类动物剥削）的TAC基准，通过价格、评分等因素控制混淆，测试7个前沿模型。

Result: 所有模型得分低于64%的随机水平，最佳模型Claude Opus 4.7仅53%；在提示中加入福利意识句子后，部分模型提升显著。

Conclusion: 表明现有模型在代理部署中缺乏动物福利意识，需改进；讨论了文化差异和基准局限。
...
### [440] [Memory as a Wasting Asset: Pricing Flash Endurance for Embodied Agents, and the Limits of Doing So](https://arxiv.org/abs/2606.18144)
*Josef Liyanjun Chen*

Main category: cs.AI

TL;DR: 将机器人闪存耐久性视为不可再生资本，通过影子价格优化存储层次分配，发现价值-写入关系决定最优策略是否非单调，并在实际日志中测量了该关系。


<details>
  <summary>Details</summary>
Motivation: 机器人闪存写入次数有限且不可再生，现有系统未考虑记忆存储的成本效益，需引入经济模型优化存储层次。

Method: 提出磨损增强的每字节索引，以单一耐久性影子价格η作为阈值，将RAM/板载NVM/云存储的成本最小化分配转化为磨损调整后的成本决策。

Result: 实际日志测量显示，长时程操作下价值-写入关联χ为正（~1e-3），短时程接近零，非重复遥操作为负；耐用性约束在廉价硬件（QLC/eMMC约1000 P/E）上显著，且磨损感知控制与价格路由在任务价值上无差异。
...
### [441] [WEQA: Wearable hEalth Question Answering with Query-Adaptive Agentic Reasoning](https://arxiv.org/abs/2606.18147)
*Yuwei Zhang,Tong Xia,Bianca Emmerich,Yu Yvonne Wu,Dimitris Spathis,Xin Liu,Daniel McDuff,Cecilia Mascolo*

Main category: cs.AI

TL;DR: 提出WEQA框架，通过LLM控制器动态组合传感器分析和模型，提升可穿戴健康数据问答准确性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在处理可穿戴传感器产生的连续高维纵向数据时存在挑战，且用户意图多样，固定工作流或单一模型难以应对。

Method: 采用查询自适应代理框架，LLM控制器合成执行计划，动态路由查询至合适的传感器分析和预训练模型，并进行基于外部知识的响应审计。

Result: 在四个数据集上比基线准确24%，经12名医学专家和8名用户盲测，在有用性和临床合理性上显著提升。

Conclusion: WEQA框架有效统一LLM推理与可穿戴分析工具，提升健康数据问答性能。
...
### [442] [Learning Cardiac Electrophysiology Digital Twins Through Agentic Discovery of Hybrid Structure](https://arxiv.org/abs/2606.18154)
*Ziqi Zhou,Yubo Ye,Sumeet Atul Vadhavka,Linwei Wang,Zhiqiang Tao*

Main category: cs.AI

TL;DR: 提出了LEADS框架，利用LLM代理在结构化动作空间中探索混合模型，用于个性化心脏电生理数字孪生，实现了超越人工设计和其它LLM方法的性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖专家手动设计混合模型，难以跨患者迁移；现有LLM方法缺乏结构先验，模拟不稳定。

Method: 将心脏电生理领域知识构建为结构化动作空间，LLM代理通过迭代推理-行动循环选择、组合和优化混合模型，参数拟合由梯度下降完成。

Result: 在合成数据（三个真实反应模型）和真实心脏电生理数据上，LEADS优于人工设计的混合模型和其他基于LLM的混合建模方法。

Conclusion: LEADS能够物理合理、可解释且数值稳定地发现开放式的混合模型架构，实现了有效的个性化心脏电生理建模。
...
### [443] [DRFLOW: A Deep Research Benchmark for Personalized Workflow Prediction](https://arxiv.org/abs/2606.18191)
*Md Tawkat Islam Khondaker,Raymond Li,Muhammad Abdul-Mageed,Laks V. S. Lakshmanan,Issam H. Laradji*

Main category: cs.AI

TL;DR: 提出DRFLOW基准，用于评估从异构源预测个性化工作流（行动步骤序列）的代理。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究系统主要生成报告，但企业任务需要识别具体工作流（行动步骤序列）。

Method: 构建DRFLOW基准，含100个任务、5个领域、1246个参考步骤和3900多个来源；定义7个诊断指标；提出DRFA代理。

Result: DRFA比强基线平均F1提升10.02%，但仍需改进。

Conclusion: 预测完整正确的个性化工作流仍是深度研究的挑战。
...
### [444] [The Stanford EDGAR Filings Dataset: Reconstructing U.S. Corporate and Financial Disclosures into Layout-Faithful and Token-Efficient Pretraining Data](https://arxiv.org/abs/2606.18192)
*Nick Bettencourt,Xiaowei Ding,Kay Giesecke*

Main category: cs.AI

TL;DR: 针对高质量公共语料枯竭和长上下文文档稀缺的问题，提出斯坦福EDGAR文件数据集（SEFD），这是一个将SEC文件重建为保持布局的MultiMarkdown格式的开放数据集，用于金融语言建模和评估。


<details>
  <summary>Details</summary>
Motivation: 高质量公共网络语料库日益枯竭，干净的长上下文文档成为稀缺且昂贵的训练数据，现有长上下文语料库要么专有昂贵，要么合成生成，或集中在编程等狭窄领域。

Method: 将SEC文件（审计财务报表、风险披露等）重建为布局忠实的MultiMarkdown格式，构建SEFD数据集，并发布152B token的快照及两个基准（EDGAR-Forecast和EDGAR-OCR）。

Result: 语料库token高效、模型就绪，与Common Crawl衍生语料库重叠低于0.1%；发布SEFD-v1（152B token）并对更大档案（约550B token）完成语料库分析；引入两个基于文件的预测和表格转录基准。
...
### [445] [Fixed-Point Reasoners: Stable and Adaptive Deep Looped Transformers](https://arxiv.org/abs/2606.18206)
...
TL;DR: 提出FPRM模型，通过预归一化和残差缩放解决循环架构的信号传播问题，使用固定点收敛作为自适应停止机制，在多个推理基准上有效。


<details>
  <summary>Details</summary>
Motivation: 循环架构在组成推理任务中有效，但深度增加导致信号传播问题，需要解决。

Method: 采用预归一化层和残差缩放解决信号传播，提出基于Transformer的固定点推理模型FPRM，以固定点收敛作为端到端停止机制。

Result: 在Sudoku、Maze、状态追踪和ARC-AGI基准上表现有效。

Conclusion: FPRM能够根据任务难度自适应调整计算量，在常见推理基准上取得良好效果。
...
### [446] [EvolveNav: Proactive Preflection and Self-Evolving Memory for Zero-Shot Object Goal Navigation](https://arxiv.org/abs/2606.18235)
*Qi Chai,Wenhao Shen,Nanjie Yao,Yue Xia,Kaiyong Zhao,Jie Ma,Guosheng Lin,Hao Wang*

Main category: cs.AI

TL;DR: 提出一种自演进的零样本目标导航框架，通过构建规则记忆、UCB检索和预反射模块，在测试时持续改进，显著提升成功率并减少无效探索。


<details>
  <summary>Details</summary>
Motivation: 现有零样本目标导航方法依赖静态先验，缺乏适应性，导致重复错误和低效试错。

Method: 构建智能体规则记忆，从历史轨迹中提取可行动知识；采用基于上置信界（UCB）的检索策略，平衡语义相关性和历史成功率；引入记忆引导的预反射模块，预测行动前潜在结果以减少低效探索。

Result: 在多个基准上超越现有零样本方法，成功率提升10.1%，且不必要的步骤更少。

Conclusion: 自演进框架能有效提升零样本目标导航的性能和效率，实现持续测试时改进。
...
### [447] [NAVI-Orbital: First In-Orbit Demonstration of a Zero-Shot Vision-Language Model for Autonomous Earth Observation](https://arxiv.org/abs/2606.18271)
*Juan Manuel Delfa Victoria,Taran Cyriac John,Andrew W. Herson*

Main category: cs.AI

TL;DR: 首次在低地球轨道卫星上实现视觉语言模型自主多模态推理，通过语义压缩减少带宽需求。


<details>
  <summary>Details</summary>
Motivation: 地球观测数据生成远超下行带宽和人工处理能力，需在轨道上直接压缩数据。

Method: 使用机载Gemma 3视觉语言模型，结合LangGraph图状态机协调检测与对话代理，通过自然语言指令重新任务。

Result: 地面基准准确率88.16%，在轨成功处理未经微调的新影像（如YAM-9），验证了硬件加速推理的可行性。

Conclusion: 在卫星边缘计算机上运行基础模型可行，通过语义压缩改变传统“先采集后下传全部”的带宽模式。
...
### [448] [CaVe-VLM-CoT: An Interpretable Vision-Language Model Framework](https://arxiv.org/abs/2606.18385)
*Sneha Rao,Shaina Raza,Dhanesh Ramachandram*

Main category: cs.AI

TL;DR: 提出CaVe-VLM-CoT，一种模块化反思型智能检索增强生成框架，通过五阶段闭环流水线（提取器、检索器、求解器、引用注入器、验证器）强制证据接地推理，有效减少视觉语言模型的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型容易产生幻觉，输出流畅但视觉上不准确。现有方法缺乏步骤级引用接地，也未将验证失败反馈给检索进行修正。

Method: 采用五阶段闭环框架：提取器、检索器、求解器、引用注入器和验证器。当验证器检测到无根据断言时，结构化反馈给提取器进行目标重新检索。同时提出23个组件级指标，核心是CaVeScore复合指标。

Result: 在ScienceQA上达到87.1%准确率和56.6% CaVeScore；在MMMU（30个科目）上达到55.2%准确率和35.7% CaVeScore。

Conclusion: 无需任何架构或提示修改，该框架通过证据接地推理显著提升模型性能，有效缓解幻觉问题。
...
### [449] [Searching for Synergy in Shared Workspace Human-AI Collaboration](https://arxiv.org/abs/2606.18413)
*Nachiket Kotalwar,Rohini Das,Carolyn Rose*

Main category: cs.AI

TL;DR: 研究共享工作空间中人类与AI代理的协作，发现缺乏结构协调会降低性能，引入共享记忆和人类在环门控机制能提升三人团队表现。


<details>
  <summary>Details</summary>
Motivation: 自动AI代理能力增强，但许多任务需要人类判断和专业知识，研究如何协调人与AI团队责任分工。

Method: 使用Collaborative Gym环境和DiscoveryBench任务，通过1482次会话，比较添加模拟人类协作者的效果，并评估结合共享组记忆和模拟人类在环门控的脚手架。

Result: 添加相关协作者若无协调结构会降低性能；脚手架可提升均值表现，尤其在三人团队中，明确责任信号和专家路由。

Conclusion: 人类-AI团队的协调与专业集成能力同样重要。
...
### [450] [CEO-Bench: Can Agents Play the Long Game?](https://arxiv.org/abs/2606.18543)
*Haozhe Chen,Karthik Narasimhan,Zhuang Liu*

Main category: cs.AI

TL;DR: CEO-Bench 通过模拟创业公司500天运营，评估语言模型智能体在长周期、不确定性和多任务协调方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试未能同时考察智能体在长周期导航、噪声环境信息获取、世界适应性变化以及多目标协调等复杂技能。

Method: 构建模拟环境，智能体通过可编程Python接口管理定价、营销、预算等公司运营，面对与人类CEO相同的挑战。

Result: 最强智能体编写复杂代码模拟客户群体预测现金流，但多数先进模型表现不佳，仅Claude Opus 4.8和GPT-5.5能维持初始资金超过100万美元，且无法持续盈利。

Conclusion: CEO-Bench是衡量智能体实现持续适应性进步所需智能的第一步。
...
### [451] [DeFAb: A Verifiable Benchmark for Defeasible Abduction in Foundation Models](https://arxiv.org/abs/2606.18557)
*Patrick Cooper,Alvaro Velasquez*

Main category: cs.AI

TL;DR: DeFAb是一个基于规则的可废止溯因推理基准，包含372,648+实例，最佳前沿语言模型准确率仅23.5%，而符号求解器达100%。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型在严谨逻辑推理上表现不佳，需要一种既能评估创造性又能保持逻辑严谨性的基准。

Method: 将OpenCyc、YAGO、Wikidata等知识库与ConceptNet、UMLS相结合，生成可废止溯因实例，并通过多项式时间验证推导、保守性和最小性。

Result: 前沿模型在渲染鲁棒评估中准确率仅7.8-23.5%，链式思维方差大于模型间差距；DeFAb-Hard上最佳模型53.3% vs 符号系统100%。

Conclusion: 语言模型尚未可靠内化可废止推理，逻辑严谨性基准对于衡量真正的理论推理能力至关重要。
...
### [452] [Optimizing Lithium Production Decisions under Geological, Demand, and Pricing Uncertainties: A POMDP Framework for Multi-Objective Decision Making](https://arxiv.org/abs/2606.18598)
*Anna C. Edmonds,Mansur M. Arief,Robert J. Moss,Mykel J. Kochenderfer,Jef Caers*

Main category: cs.AI

TL;DR: 本文采用部分可观测马尔可夫决策过程（POMDP）框架，结合信念状态规划方法，优化锂矿开采决策，在多种价格和开采技术下实现更高需求满足和更平衡的经济环境效益。


<details>
  <summary>Details</summary>
Motivation: 锂生产决策面临地质、价格、开采技术等多重不确定性，现有模型未充分考虑定价、需求不确定性和不同开采技术，需要更鲁棒的策略。

Method: 将问题建模为POMDP，使用信念状态规划方法求解，以动态适应不同价格机制。

Result: POMDP求解器优于人类启发式策略，能在静态、线性、指数和随机价格机制下动态调整，实现更高需求满足和更平衡的经济环境结果。

Conclusion: 该框架通过优化勘探、生产和技术选择的顺序，在不同定价和矿床场景下都能取得更好效果。
...
### [453] [ForecastBench-Sim: A Simulated-World Forecasting Benchmark](https://arxiv.org/abs/2606.18686)
*Jaeho Lee,Nick Merrill,Ezra Karger*

Main category: cs.AI

TL;DR: 提出了一个基于Freeciv模拟游戏的预测基准，用于评估通用AI系统的概率推理能力。


<details>
  <summary>Details</summary>
Motivation: 现实世界预测存在结果慢、尾事件少、反事实评分难等局限，需要可控的模拟环境。

Method: 利用Freeciv游戏回放生成固定世界状态，并提问未来隐藏状态；通过继续模拟评分，支持连续/二元问题、干预世界和罕见事件。

Result: 提供了模型评估和匿名人类试点的验证切片。

Conclusion: 该基准通过可控、可立即解析的任务补充了真实世界预测基准，用于研究动态世界状态下的概率推理。
...
### [454] [What Must Generalist Agents Remember?](https://arxiv.org/abs/2606.18746)
*Khurram Yamin,Namrata Deka,Maitreyi Swaroop,Albert Ting,Jeff Schneider,Bryan Wilder*

Main category: cs.AI

TL;DR: 本文形式化分析了通用智能体在跨多个环境和目标时所需的记忆，证明记忆是支撑领域去混淆、转移模型重建和规划的关键。


<details>
  <summary>Details</summary>
Motivation: 理解通用智能体如何在不同环境和目标下近乎最优地行动，以及记忆在其中扮演的角色。

Method: 通过理论分析，考虑两个共享观察瓶颈但需要不相容最优动作的领域，证明任何统一近乎最优的策略必须在该瓶颈处诱导不同的记忆分布。

Result: 得到分离定理：成功智能体不能仅依赖当前状态观察，必须在记忆中保留领域相关信息；若记忆包含足够信息，可用于近似重建局部转移动力学。

Conclusion: 记忆被表征为支持领域去混淆、转移模型重建和规划的基础，为通用智能体的设计提供理论指导。
...
### [455] [R2D-RL: A RoboCup 2D Soccer Environment for Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2606.18786)
*Haobin Qin,Baofeng Zhang,Hidehisa Akiyama,Keisuke Fujii*

Main category: cs.AI

TL;DR: 介绍R2D-RL，一个连接RCSS2D与Python多智能体强化学习接口的环境，支持并行训练和基于期望控球值的奖励塑造。


<details>
  <summary>Details</summary>
Motivation: 现有的RoboCup 2D仿真平台因其竞争导向的服务器-客户端架构，难以直接与基于Python的现代多智能体强化学习工作流结合。
...
Conclusion: R2D-RL有效解决了RCSS2D与Python MARL的集成问题，为多智能体强化学习研究提供了实用的平台。
...
### [456] [ProfiLLM: Utility-Aligned Agentic User Profiling for Industrial Ride-Hailing Dispatch](https://arxiv.org/abs/2606.18803)
*Tengfei Lyu,Zirui Yuan,Xu Liu,Kai Wan,Zihao Lu,Li Ma,Hao Liu*

Main category: cs.AI

TL;DR: 提出ProfiLLM，一种将大语言模型作为语义特征提取器用于工业网约车调度的数据管道，通过全局知识挖掘和效用对齐的档案探索，在滴滴生产环境中显著提升预测和调度效果。


<details>
  <summary>Details</summary>
Motivation: 网约车调度系统依赖结构化数值特征，但关键行为信号（如司机对某区域的习惯性规避）具有上下文性，可用LLM生成用户档案；然而大规模日志超出LLM上下文窗口、长尾用户交互少、表面流畅的档案未必提升下游预测效果，亟需解决方案。

Method: ProfiLLM包含两个模块：(1) 工具增强的全局知识挖掘——为LLM智能体配备27个分析工具，挖掘平台数据，生成可复用的全局知识、自适应用户聚类规则和区域级供需先验；(2) 效用对齐的档案探索——每簇生成多个候选档案，通过轻量下游效用代理评估，迭代优化最佳候选并构建偏好对用于DPO微调。

Result: 在滴滴生产调度系统上，ProfiLLM实现结果预测AUC相对提升+6.14%，调度模拟GMV提升+4.35%，14天在线A/B测试中GMV+0.47%、完成率+0.33%、取消-接单前率-0.82%。

Conclusion: ProfiLLM有效将LLM用于工业级网约车调度，克服了规模、长尾和效用对齐挑战，提升了预测和业务指标。
...
### [457] [WorldLines: Benchmarking and Modeling Long-Horizon Stateful Embodied Agents](https://arxiv.org/abs/2606.18847)
*Yehang Zhang,Jianchong Su,Haojian Huang,Yifan Chang,Tianhao Zhou,Xinli Xu,Yingjie Xu,Yinchuan Li,Zexi Li,Ying-Cong Chen*

Main category: cs.AI

TL;DR: 介绍WorldLines基准和ObsMem记忆框架，用于长期具身家庭辅助，解决动态环境下的长期记忆挑战。


<details>
  <summary>Details</summary>
Motivation: 现有长期记忆基准偏重语言检索，具身基准专注短期任务，缺乏对动态环境中长期记忆使用的测试。

Method: 提出WorldLines项目驱动基准，构建时间扩展的家庭轨迹（含对话、动作、状态变化），转化为记忆问答和具身任务规划样本；提出ObsMem观察者记忆框架，维护可见性记忆和状态轨迹。

Result: 实验显示部分可观察性、世界状态覆盖等挑战，ObsMem提供更强的参考架构。

Conclusion: WorldLines和ObsMem为长期具身家庭辅助提供了有效的评估和框架参考。
...
### [458] [Externalizing Research Synthesis and Validation in AI Scientists through a Research Harness](https://arxiv.org/abs/2606.18874)
...
TL;DR: Xcientist是一种将研究过程外化为可检查、有合约约束的流程的框架，用于自动化科学研究，确保可追溯性和可问责性。


<details>
  <summary>Details</summary>
Motivation: 现有的AI系统在自动化科学工作流程中，从证据到想法的推理过程往往隐含在模型内部，缺乏透明度。

Method: Xcientist将文献证据、想法状态、实现计划、消融记录和修复痕迹组织为持久的研究工件，通过合约控制外部化研究合成与实验验证。

Result: 在无训练记忆系统、图结构交通预测和多尺度物理信息神经网络中，Xcientist保留了从问题定义到机制设计、验证和有限修订的可追踪轨迹。

Conclusion: 评估AI科学家不仅要看最终工件，还要看其合成和验证过程是否可归因、可检查且科学上可问责。
...
### [459] [Generative-Model Predictive Planning for Navigation in Partially Observable Environments](https://arxiv.org/abs/2606.18888)
*Thomas Quilter,Yifan Zhu,Guorui Quan,Mingfei Sun,Samuel Kaski*

Main category: cs.AI

TL;DR: BeliefDiffusion结合扩散模型与模型预测控制，通过显式建模多模态信念分布并在聚合配置上进行规划，显著提升部分可观测环境中的导航成功率与路径效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于信念的导航方法难以捕捉高维感知混淆下的多模态信念分布，而生成模型缺乏长期规划机制。

Method: 两阶段框架：1) 利用扩散模型基于观测历史想象多种环境配置；2) 在聚合配置上使用模型预测控制规划导航策略。

Result: 在合成地图实验中，BeliefDiffusion在导航成功率和路径效率上显著优于无模型强化学习基线及其他生成方法。

Conclusion: 显式融入多模态信念表示到规划中能够提升部分可观测环境下的导航鲁棒性。
...
### [460] [Skill-Guided Continuation Distillation for GUI Agents](https://arxiv.org/abs/2606.18890)
...
TL;DR: 提出Skill-Guided Continuation Distillation (SGCD)框架，通过技能引导的持续蒸馏来弥补GUI代理在策略诱导的离轨状态上的监督缺失，显著提升成功率。


<details>
  <summary>Details</summary>
Motivation: GUI代理行为克隆面临策略诱导的离轨状态缺乏监督，因为专家轨迹未覆盖这些状态，导致策略无法选择正确动作。

Method: 先运行无技能引导的普通策略若干步到达真实离轨状态，再用技能引导策略完成任务并生成成功延续，与专家轨迹混合提供监督；技能从成功和失败轨迹中提取。

Result: 在OSWorld-Verified上，SGCD将三个基础模型成功率从低30%提升至超过50%。

Conclusion: SGCD是一种有效且通用的迭代自我改进框架，能弥补GUI代理的监督差距。
...
### [461] [SciRisk-Bench: A Risk-Dimension-Aware Benchmark for AI4Science Safety](https://arxiv.org/abs/2606.18936)
...
TL;DR: 设计了SciRisk-Bench基准，从风险维度和学科角度评估AI4Science中的大语言模型安全性，覆盖7学科、31子学科和10风险维度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在AI4Science中广泛应用，但现有安全基准未能充分说明风险维度，需要更全面的评估。

Method: 构建SciRisk-Bench，包含7个学科、31个子学科和10个风险维度，对主流和科学导向的大语言模型进行细粒度评估。

Result: 实验评估了多种模型在风险维度、学科和子学科上的表现，揭示了模型不安全的具体区域。

Conclusion: 该基准能实现细粒度诊断，帮助识别科学模型的不安全之处。
...
### [462] [Decoupling Search from Reasoning: A Vendor-Agnostic Grounding Architecture for LLM Agents](https://arxiv.org/abs/2606.18947)
*Emmanuel Aboah Boateng,Kyle MacDonald,Amardeep Kumar,Siddharth Kodwani,Sudeep Das*

Main category: cs.AI

TL;DR: 提出解耦搜索接地（DSG），通过MCP兼容网关将搜索接地移出推理模型，提供可配置的路由、缓存、回退等控制，在保持或微降准确率的同时大幅降低成本和延迟。


<details>
  <summary>Details</summary>
Motivation: 生产级LLM代理依赖实时搜索，但原生搜索接地将检索策略、提供商选择、证据注入、成本、延迟和生成行为捆绑在单一模型-提供商边界内，导致难以检查、调优、复用或移植，并可能引发搜索诱导的冗长，破坏严格的输出契约。

Method: DSG通过MCP兼容的网关将接地从推理模型中解耦出来，作为供应商无关的边界，将提供商路由、源感知上下文渲染、配置的回退、检索深度控制以及精确和语义缓存作为一级控制暴露出来。

Result: 在SimpleQA、FreshQA和HotpotQA上对五个前沿模型测试，DSG在FreshQA上略低于原生，但在SimpleQA上准确率86.1% vs 87.7%，搜索成本降低91%，保持简洁答案，缓存命中率99.4%，延迟降低68%；在电商查询理解任务中准确率匹配或略超原生，搜索成本降低98%以上。

Conclusion: 实时接地应被视为可优化的接口边界，而非固定的模型特征；DSG作为共享生产接地层，可在可互换模型的大规模代理工作负载中有效运行。
...
### [463] [RTSGameBench: An RTS Benchmark for Strategic Reasoning by Vision-Language Models](https://arxiv.org/abs/2606.18950)
*San Kim,Daechul Ahn,Reokyoung Kim,Hyeonbeom Choi,Seungyeon Jwa,Jonghyun Choi*

Main category: cs.AI

TL;DR: 提出了基于即时战略游戏Beyond All Reason的基准RTSGameBench，用于诊断视觉语言模型在战略推理中的不足，并发现现有模型在需要紧密协调和多智能体协作时表现不佳。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在不确定环境下的战略推理能力不足，而现有基准评估范围有限、缺乏系统的能力诊断且场景固定，需要更全面的测试平台。

Method: 构建RTSGameBench，包含多种对战结构的评估、针对单项战略能力的迷你游戏诊断，以及通过自进化框架从自由查询生成新迷你游戏；同时提供基于有限状态机和智能记忆的RTSGameAgent来管理单位。

Result: 多个先进视觉语言模型在需求更紧密协调、多智能体合作和任务规模增大时表现不佳。

Conclusion: 新基准有效揭示了当前视觉语言模型在战略推理上的局限性，并提供了可扩展的诊断工具。
...
### [464] [ThinkDeception: A Progressive Reinforcement Learning Framework for Interpretable Multimodal Deception Detection](https://arxiv.org/abs/2606.18988)
*Jinhao Song,Shan Liang,Yiqun Yue,Zhuhuayang Zhang,Tianqi Gao*

Main category: cs.AI

TL;DR: 提出可解释的多模态欺骗检测框架ThinkDeception，利用多模态大语言模型和链式思维数据集，通过视觉-音频一致性分组相对策略优化和渐进训练，在检测准确性和推理质量上达到新最优。


<details>
  <summary>Details</summary>
Motivation: 现有黑盒方法缺乏可解释性，无法捕捉跨模态不一致性。

Method: 引入多模态大语言模型，构建链式思维数据集，提出VAC-GRPO策略与渐进式训练。

Result: 在主流基准上显著优于现有方法，达到新最优。
...
### [465] [RODS: Reward-Driven Online Data Synthesis for Multi-Turn Tool-Use Agents](https://arxiv.org/abs/2606.19047)
*Ruishan Fang,Siyuan Lu,Chenyi Zhuang,Tao Lin*

Main category: cs.AI

TL;DR: 针对多轮工具使用强化学习中静态数据集信息样本快速耗尽的问题，提出RODS方法，通过奖励驱动在线数据合成，利用已计算的回报方差作为零成本边界检测器，持续合成边界样本并管理动态回放缓冲区，实现从少量种子数据达到与大规模离线管道相当的性能。


<details>
  <summary>Details</summary>
Motivation: 多轮工具使用强化学习受限于静态数据集中信息样本的快速耗尽，导致训练效率低下。

Method: RODS：利用回报方差作为边界检测器，识别能力边界样本；通过技能对齐的重采样管道合成结构复杂度相似的新多轮变体；管理动态回放缓冲区，与策略共同演化。

Result: 从400个人工种子开始，维持约800个训练样本，RODS达到了与17K样本离线管道相当的性能，所需轨迹数量减少约20倍，并在受控环境中优于固定数据RL和环境增强。

Conclusion: RODS通过在线数据合成有效缓解了信息样本耗尽问题，显著提升了数据效率。
...
### [466] [ARIADNE: Agnostic Routing for Inference-time Adapter DyNamic sElection](https://arxiv.org/abs/2606.19079)
*Enrico Cassano,Micha{\l} Brzozowski,Zuzanna Dubanowska,Paolo Mandica,Neo Christopher Chung*

Main category: cs.AI

TL;DR: 提出ARIAADNE，一种无需训练、与适配器无关的推理时动态适配器选择框架，通过嵌入空间中的质心接近度进行路由，在23个任务上恢复97.44%的上界性能，44个任务上达到89.7%的选择准确率。


<details>
  <summary>Details</summary>
Motivation: 现有路由方法依赖适配器内部信息或需额外训练，扩展性差，无法适应动态增长的适配器池。

Method: 为每个适配器从其训练集嵌入中计算一组质心，表示数据分布；对于无标签输入，通过测量与质心的潜在空间接近度选择适配器。

Result: 在Llama 3.2 1B Instruct上，23个任务恢复97.44%上界性能；扩展到44个任务，平均选择准确率89.7%。

Conclusion: ARIAADNE无需训练或访问适配器内部，即可高效且可扩展地选择适配器，适用于任意PEFT方法。
...
### [467] [Towards an Agent-First Web: Redesigning the Web for AI Agents](https://arxiv.org/abs/2606.19116)
...
TL;DR: 提出面向AI代理的互联网重构方案，涵盖访问、经济、内容三层，包含十项设计原则，最终形成代理优先的互联网新契约。


<details>
  <summary>Details</summary>
Motivation: 当前互联网以人类为中心，但AI代理作为中介日益普及，现有机制（如完全屏蔽、验证码、经济模型）阻碍了代理的合法交互，需基于代理作为人类代理人的原则重新设计。

Method: 分三层：1）访问层：代理继承人类访问权限，通过速率限制、HTTP代理标识及双架构（人类可读+代理优化）实现；2）经济层：基于意图的层级框架，代理经济义务反映人类代表，采用令牌订阅计量内容，并引入委托内容经济；3）内容层：提出Agent Text Markup Language（ATML）、四层人类监督模型及加密溯源链，应对AI生成内容与人类真值分离的认知递归问题。

Result: 形成十项设计原则，构建代理优先的互联网，使代理成为一等公民，需重新谈判访问、经济、内容层面的基础社会契约。

Conclusion: 互联网需调整其基本假设，从人类独享转向人类与AI代理共存，通过三层重构实现合法、可持续的代理交互。
...
### [468] [Analysing drivers and interdependencies in European electricity markets using XAI](https://arxiv.org/abs/2606.19118)
*Antoine Pesenti,Aidan O'Sullivan*

Main category: cs.AI

TL;DR: 结合深度神经网络与可解释AI，分析欧洲39个竞价区电价驱动因素，发现可再生能源（尤其是太阳能）对电价形成作用显著，天然气仍是主导因素，互联显著影响价格动态，并构建了全欧盟统一市场反事实情景。


<details>
  <summary>Details</summary>
Motivation: 电力市场复杂且缺乏可解释性，深度神经网络预测能力强但无法揭示价格形成机制，需要可解释AI方法分析电价决定因素。

Method: 采用深度神经网络预测电价，结合SHAP量化特征贡献，并应用及扩展SSHAP聚合框架以提高高维可解释性。

Result: 可再生能源（太阳能）对电价影响不成比例地重要，天然气是持续主导因素，互联显著塑造价格动态，欧洲电力系统高度相互依赖。

Conclusion: 可解释AI可揭示电价关键驱动因素，太阳能和天然气作用突出，互联增强区域耦合，全欧盟统一市场可探索价格整合效果。
...
### [469] [Human-AI Coevolution Dynamics: A Formal Theory of Social Intelligence Emergence Through Long-Term Interaction](https://arxiv.org/abs/2606.19144)
*Jingyi Zhou,Senlin Luo,Haofan Chen*

Main category: cs.AI

TL;DR: 提出人类-AI共进化动力学框架（HACD-H），将社交互动视为自组织社会认知系统，整合情感、关系、记忆和人格，实验发现稳定的关系吸引子、阶段性发展模式，社交智能与社交认知能量负相关。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过情感建模、记忆检索等孤立组件模拟社交行为，缺乏统一框架解释长期人机互动中稳定社交关系和社交智能的涌现。

Method: 提出HACD-H框架，将人机互动建模为自组织社会认知系统，整合情感适应、关系组织、社交记忆和人格一致性，引入多时间尺度社会认知、关系吸引子、信任盆地、发展阶段转变和社交认知能量动力学等原则，并构建约14700轮对话数据集及理论驱动评估框架。

Result: 发现社会认知时间持久性层次、稳定关系吸引子、阶段转变式发展模式及结构化社交认知能量景观；社交智能与社交认知能量显著负相关（r=-0.391, p<0.001），互动轨迹随时间能量递减。

Conclusion: 社交智能源于长期社交认知共进化而非孤立对话能力；HACD-H为建模适应性人机社交互动和发展社交智能AI提供了统一理论基础。
...
### [470] [Beyond Safe Data: Pretraining-Stage Alignment with Regular Safety Reflection](https://arxiv.org/abs/2606.19168)
*Jinhan Li,Kexian Tang,Yihan Xu,Zhuorui Ye,Kaifeng Lyu*

Main category: cs.AI

TL;DR: 提出安全反思预训练，在预训练阶段插入安全反思文本，使模型建立自我监控能力，减少不安全行为。


<details>
  <summary>Details</summary>
Motivation: 预训练阶段对齐不应仅使数据安全，因为模型可能从看似安全的数据中组合出危险行为。

Method: 在预训练语料中定期插入简短安全反思文本，将自我监控直接集成到语言建模中，再通过兼容的后训练强化。

Result: 1.7B模型实验表明：安全分类准确率提升，推理阶段和微调攻击成功率显著降低；在合成环境MedSafetyWorld中优于数据过滤和重写。

Conclusion: 预训练对齐不仅要确保数据安全，还要主动塑造模型从安全数据中可能习得的行为模式。
...
### [471] [User as Engram: Internalizing Per-User Memory as Local Parametric Edits](https://arxiv.org/abs/2606.19172)
*Bojie Li*

Main category: cs.AI

TL;DR: 提出User as Engram方法，将用户内容存储为哈希键记忆表中的局部编辑，推理能力共享一个适配器，避免了污染，提高了推理准确性，内存占用减少约33,000倍。


<details>
  <summary>Details</summary>
Motivation: 现有per-user LoRA将内容和推理能力混合到全局权重增量中，导致污染，而大脑将两者分开。需要更精确、高效的个人化方法。

Method: 使用Engram模型，将用户事实作为局部编辑写入哈希键记忆表，推理能力由共享适配器承载；编辑精确，只影响特定位置，多个用户无损叠加。

Result: 相比per-user LoRA，间接推理准确性平均提高5.6倍，内存占用减少约33,000倍，且不降低推理能力；超过约100个事实后，比2.5倍大的模型的检索流水线更快。

Conclusion: User as Engram实现了精确、可扩展的个人化记忆，避免了污染，提高了推理性能，并支持大规模用户同时存储。
...
### [472] [TxBench-PP: Analyzing AI Agent Performance on Small-Molecule Preclinical Pharmacology](https://arxiv.org/abs/2606.19245)
*Hannah Le,Ramesh Ramasamy,Alex Urrutia,Mahsa Yazdani,Tim Proctor,Kenny Workman*

Main category: cs.AI

TL;DR: 提出了TxBench-PP基准测试，用于评估AI智能体在临床前药理学实际数据分析中的能力。结果表明，即使是最好的模型（Claude Opus 4.8/Pi）也仅通过59.3%的测试，没有系统能可靠地恢复临床前药理学决策。


<details>
  <summary>Details</summary>
Motivation: AI智能体有望加速药物研发，但实际部署需要基于真实项目决策的可信评估。

Method: TxBench-PP包含100个评估任务，涵盖机制、药效、靶点、安全性等多个维度。智能体接收真实工作流快照，在编码环境中检查文件，返回结构化答案并确定性评分。

Result: 在16种模型配置（11个模型、4800条轨迹）中，最强配置Claude Opus 4.8/Pi通过59.3%的端点尝试（178/300），GPT-5.5/Pi通过55.3%（166/300）。

Conclusion: 当前AI智能体在临床前药理学任务中表现不可靠，需要进一步改进。
...
### [473] [X+Slides: Benchmarking Audience-Conditioned Slide Generation](https://arxiv.org/abs/2606.19256)
*Haodong Chen,Xuanhe Zhou,Wei Zhou,Xinyue Shao,Yanbing Zhu,Bo Wang,Jiawei Hong,Anya Jia,Fan Wu*

Main category: cs.AI

TL;DR: 提出了X+Slides基准，用于评估基于受众的幻灯片生成，实验表明现有系统虽恢复部分必要信息，但视觉质量不能替代基于来源的评估。


<details>
  <summary>Details</summary>
Motivation: 现有基准忽略受众需求，而专家与决策者所需信息不同。

Method: 构建包含113个主题和7个场景的基准，使用8,133个探针进行动态评估，并定义四个指标：受众覆盖、领域覆盖、效率、正确性。

Result: 在τ_A=0.7时，DeepPresenter受众覆盖0.714，SlideTailor 0.594，NotebookLM消融版本0.853，但均未完全覆盖受众信息。

Conclusion: 视觉质量和主题覆盖不能替代基于来源的评估，需注重受众导向的正确性。
...
### [474] [NeSyCat Torch: A Differentiable Tensor Implementation of Categorical Semantics for Neurosymbolic Learning](https://arxiv.org/abs/2606.19279)
*Daniel Romero Schellhorn,Till Mossakowski,Bj\"orn Gehrke*

Main category: cs.AI

TL;DR: NeSyCat Torch 是一个基于单子的统一神经符号框架，通过参数化真值定义和神经网络学习符号，在MNIST加法任务上实现了高精度和快速训练。


<details>
  <summary>Details</summary>
Motivation: 现有的神经符号语义（经典、模糊、概率、神经）各自依赖不同的归纳规则，缺乏统一性。

Method: 扩展 ULLER，采用强单子和聚合结构参数化真值定义，使用分布单子进行参考语义和度量评估，惰性对数十张量单子实现数值稳定的可微训练，批处理单子支持批量高效训练。通过神经网络解释计算符号，并基于单子 do-notation 编写公理。

Result: 在 MNIST 加法任务上，基于 HaskTorch、JAX 和 PyTorch 的实现相比 LTN 和 DeepProbLog 速度更快、精度更高，且几乎达到 DeepStochLog 的精度。

Conclusion: 该方法保持统一框架，可通过不同单子参数化（如 Giry 单子扩展至连续概率），但连续概率的神经表示留待未来工作。
...
### [475] [Rethinking Reward Supervision: Rubric-Conditioned Self-Distillation](https://arxiv.org/abs/2606.19327)
*Siyi Gu,Jialin Chen,Sophia Zhou,Arman Cohan,Rex Ying*

Main category: cs.AI

TL;DR: 提出基于评分标准的自蒸馏框架，通过结构化评分标准提供细粒度反馈，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有监督蒸馏依赖昂贵且可能含噪的思维链注解，强化学习仅提供标量奖励，缺乏细粒度反馈。

Method: 教师模型基于评分标准对学生采样轨迹提供token级指导；两阶段流程：首先生成任务特定评分标准，然后训练评分标准引导的推理器。

Result: 在多个科学推理基准上平均超过GRPO 1.0个百分点和OPSD 0.9个百分点。

Conclusion: 有效将评分标准准则转化为推理过程中的token级指导。

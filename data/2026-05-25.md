...
TL;DR: 本研究通过秘密希特勒游戏评估LLM的欺骗能力，发现其战略推理和欺骗能力不足，现有推理增强技术未能改善性能。


<details>
  <summary>Details</summary>
Motivation: 量化LLM的欺骗潜力对AI安全至关重要，但在非受控环境中难以实现。

Method: 引入开源框架和三个新指标（角色识别准确率、欺骗保持率、游戏状态影响率），在秘密希特勒游戏中对比LLM与规则基算法及人类游戏，并测试CoT和内部记忆等推理增强技术。

Result: 规则基代理与人类投票一致性达86.7%，而Llama 3.1 70B仅59.7%；模型扮演法西斯时贡献负面，游戏时长比人类短约40%；推理增强导致法西斯胜率最高下降23.2%。

Conclusion: 当前LLM架构无法有效进行复杂多轮操纵，检测此类欺骗行为的出现至关重要，本框架可作为可重复的对齐研究测试平台。
...
TL;DR: 对Hausa和Fongbe两种西非语言的公开文本和语音资源进行系统调查，发现Hausa文本资源更丰富，Fongbe语音资源有最新进展，两者均存在差距。


<details>
  <summary>Details</summary>
Motivation: 评估Hausa和Fongbe的NLP资源现状，识别关键缺口。

Method: 系统搜索学术仓库、数据平台和网络资源，整理平行语料、单语文本、语音数据集、预训练模型和评测基准。

Result: Hausa在新闻、百科和教育领域有更广泛的文本资源；Fongbe文本资源有限，但近期有学术语音数据收集；两种语言均包含在Masakhane的NER和POS基准中。

Conclusion: 建议优先开发Fongbe的领域多样化文本和Hausa的专用语音语料库。
...
TL;DR: 提出查询自适应语义分块(QASC)方法，通过集成查询到分块过程，在RAG系统中显著提升分块相关性和连贯性。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统的固定分块方式无法兼顾精度与召回，且语义或代理分块未在分块阶段集成用户查询，导致检索上下文不理想。

Method: QASC通过三个步骤动态构建分块：1) 计算句子与查询嵌入的余弦相似度识别种子句子；2) 围绕种子进行上下文窗口扩展以保持语义连贯；3) 聚合分块级得分确保整体相关性。

Result: 在100篇技术文档、200个查询上，QASC的F1达0.85，相对固定分块提升18-27%，相对语义/代理分块提升8-12%，消融和人工评估（Cohen κ=0.82）验证了有效性。

Conclusion: QASC能有效提高RAG系统的上下文检索质量，通过查询感知的语义分块生成更相关且连贯的文档片段。
...
TL;DR: 提出知识感知文本到SQL框架，通过构建任务特定知识库并注入训练和推理，显著提升低资源领域性能。


<details>
  <summary>Details</summary>
Motivation: 现实场景中高质量<问题, SQL>对稀缺，尤其是领域特定数据库，现有方法产生实例缺乏语义基础和数据库约束。

Method: 构建包含模式语义、缩写、业务逻辑和查询模式的任务特定知识库，用于生成多样化的合成训练数据，并通过目标知识检索增强推理。

Result: 在7个基准上（通用和领域特定），开源和闭源大语言模型在文本到SQL任务上性能大幅提升，尤其低资源领域。
...
TL;DR: 本文提出一个红队测试框架，通过测量大语言模型的Overton窗口及其对越狱的敏感性，评估其支持政治影响力活动的倾向。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的代理广泛参与在线讨论，需红队测试其支持政治影响力活动的风险，尤其是可本地部署的开源模型可能被恶意行为者利用。

Method: 定义Overton窗口为模型在争议话题上可靠表达的政治观点范围，并通过简单自然语言越狱量化扩展。评估了30多个来自10个模型家族、5个国家的开源LLM。

Result: 开源LLM更倾向于生成左倾内容；Overton窗口随模型规模增大而收缩；区域差异显著；越狱效果因模型家族不同而差异明显。

Conclusion: 该框架可用于审计开源LLM的政治可操控性，并帮助设计抵御基于LLM的影响力活动的对策。
...
TL;DR: 研究Text2Cypher中推理时扩展方法，利用执行错误反馈的RAS方法比无记忆重新采样的IS方法更有效。


<details>
  <summary>Details</summary>
Motivation: 推理时扩展可减少结构化查询生成错误，但针对查询代码生成的资源分配方法未被充分探索。

Method: 对比独立扩展(IS)和反射增强扩展(RAS)，其中RAS通过上下文学习利用先前执行反馈进行条件采样。

Result: 在三个数据集和五个模型上，RAS在n=5时将错误率降低41-50%，IS为32-38%。

Conclusion: 执行错误是可操作的反馈，围绕它们组织推理计算比独立扩展样本更高效。
...
TL;DR: 提出LIFT算法，通过对齐不同扩散时间步的信息可用性，提升扩散语言模型的推理能力，在多个推理基准上取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 标准SFT在扩散语言模型上效果不佳甚至有害，原因在于忽视了token的可学习性（稀有token在多数输入被掩码时难以学习，常见token在多数输入未掩码时学习价值低）。

Method: LIFT算法：在输入大部分被掩码时学习简单token，在上下文更多时学习困难token，从而与扩散时间步的信息可用性对齐。

Result: 在六个推理基准上优于现有SFT基线，在AIME'24和AIME'25上相对增益达3倍。

Conclusion: LIFT通过动态调整学习难度与信息可用性，有效提升了扩散语言模型的推理性能。
...
TL;DR: 提出利用对齐拓扑作为归纳偏置，通过构建对齐二分图并训练图神经网络来检测大语言模型中的幻觉，在多个数据集上达到最先进效果。


<details>
  <summary>Details</summary>
Motivation: 大语言模型生成内容时容易出现事实性错误，尤其在临床决策等关键领域，现有检测方法未充分利用对齐拓扑信息。

Method: 构建参考信息与LLM输出之间的对齐二分图，使用图神经网络通过消息传递建模对齐结构。

Result: 在四个不同的幻觉检测和问答数据集上取得最先进结果，优于包括GPT-4o在内的所有对比方法。

Conclusion: 利用对齐拓扑的归纳偏置能有效提升幻觉检测性能，为LLM事实性改进提供新方向。
...
TL;DR: 研究利用大型语言模型从Slack日志推断员工专业知识，发现Gemini 2.5 Flash误差最低，但准确度与消息量弱相关，表明自动化映射可行但存在隐私等局限。


<details>
  <summary>Details</summary>
Motivation: 员工难以识别组织内谁有什么专业知识，导致生产力损失，需要自动化方法从聊天记录推断个体领域知识。

Method: 分析43个用户的27,188条Slack消息，比较7种LLM（包括Gemini、Claude和GPT系列）的零样本估计与27名参与者的自我报告技能评分。

Result: Gemini 2.5 Flash误差最小（MAE 21.13%），GPT模型差异较大；估计准确度与消息量弱相关。

Conclusion: 自动化专业知识映射可行但有当前局限，需注重隐私保护并开发更丰富的结构感知知识表示。
...
TL;DR: 大型语言模型在处理宗教转换查询时表现出系统性的不对称性，偏好某些宗教而排斥其他，且这种模式在多个模型中稳定存在。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型是否对称地处理不同宗教间的转换咨询。

Method: 使用人类验证的LLM-as-a-judge框架，对20个商业和开源模型进行182种宗教配对测试，通过模拟用户寻求转换建议来探测模型反应。

Result: 所有模型均表现出可重复的不对称性，天主教、巴哈伊教和锡克教被普遍支持，而无神论者、不可知论者和耶和华见证人被排斥；偏好模式因模型规模和提供商而异。

Conclusion: 不对称性是语言模型的稳健行为特征，而非评分人工产物，大规模部署可能产生现实世界影响。
...
TL;DR: 针对Katharevousa希腊语的句法分析资源匮乏问题，提出了一套可复现的流水线，构建并评估了基于通用依赖的解析资源，最佳模型在LAS上达到0.5162。


<details>
  <summary>Details</summary>
Motivation: Katharevousa希腊语在法律、行政和议会档案中很重要，但现有NLP管道对其支持不足。

Method: 开发了可复现的工作流，包括OCR重建、模式约束的大语言模型辅助标注、自动验证、确定性的CoNLL-U快照、固定划分评估和模型族比较。

Result: 构建了1,697句的参考集。外部基线LAS最高0.4183，自定义XLM-R模型达到0.5162 LAS，UPOS准确率0.8893，依赖关系F1为0.7250。

Conclusion: 贡献了一种可审计的方法论，将困难的历史议会OCR转化为可复用的句法NLP基础设施，并开放了全部资源。
...
*Tobias von Arx,Tanguy Dieudonn\'e*

Main category: cs.CL

TL;DR: 研究了填充中间（FIM）预训练对逐字记忆的影响，发现FIM更易恢复短片段，而左到右（LTR）更易长精确续写；FIM下逐字提取随重复次数线性增长，且记忆强烈依赖前缀上下文。


<details>
  <summary>Details</summary>
Motivation: FIM预训练广泛用于赋予因果语言模型填充能力，但其对逐字记忆的影响尚不清楚。

Method: 在包含重复摘录的FineWeb-Gutenberg语料上，使用FIM和标准LTR目标预训练匹配的Llama 3.2模型，并通过前缀探针和原生FIM格式探针评估记忆。

Result: FIM更易恢复短或部分匹配片段，LTR对长精确续写置信度高；FIM下逐字提取随重复线性增长；后缀上下文不足以召回，记忆仍强烈锚定于前缀上下文；单一跨度或探针格式会遗漏重要细微差别。

Conclusion: FIM预训练影响记忆模式，需考虑多种评估方式以全面理解记忆行为。
...
TL;DR: 提出TPA（思考-规划-询问）多智能体对话框架，用于自闭症诊断中社交语言障碍特征的主动探查，显著提升诊断效率和覆盖范围。


<details>
  <summary>Details</summary>
Motivation: 自闭症患者的社会语言障碍特征在常规对话中难以显现，现有临床评估中提问策略的选择关键但未充分重视，而大型语言模型能否主动选择策略来系统性揭示这些潜在特征尚待探索。

Method: 基于ADOS-2模块4的语言评估构建TPA框架：医生智能体明确推理未观察到的特征，选择临床策略并生成针对性问题；患者智能体基于真实临床数据模拟患者语言，支持可重复评估。

Result: 在35名患者的484轮对话中，TPA的SLD特征覆盖率达82.1%，比临床医生对话回放高16.6%，每轮诊断效率显著提升（AUCC: 0.628 vs 0.458，绝对增益+0.170）。

Conclusion: 主动提问策略选择能大幅提升自动化社交语言障碍特征评估的效率，对可扩展的AI辅助临床筛查有直接意义。
...
TL;DR: 脑-LLM对齐中的英语优势是训练数据组成的假象，跨语言对齐受训练语言主导性和类型距离影响，句法区域更敏感。


<details>
  <summary>Details</summary>
Motivation: 探究脑-LLM对齐是否跨语言泛化以及变异来源。

Method: 使用英、中、法三语fMRI数据与七种LLM，分析训练语言主导性、类型距离、分词生育度等因素。

Result: 训练语言主导性决定对齐模式；类型距离独立影响，句法区梯度陡2.3倍；分词生育度解释最优编码层跨语言迁移的约60%。

Conclusion: 英语优势源于训练数据，剩余变异反映句法处理的类型结构。
...
TL;DR: 大型语言模型的中间层最能预测人脑语言反应，本文通过稀疏自编码器揭示其机制：语义特征主导编码，并预测皮层语义组织。


<details>
  <summary>Details</summary>
Motivation: 中间层预测大脑反应的机制尚未被解释。

Method: 使用稀疏自编码器分解GPT-2 XL和Llama-3.1-8B，每层提取16K-32K可解释特征，并通过人工验证分类。

Result: 语义特征恢复94%的峰值编码性能；五种语义子类别与大脑区域对齐（ρ=0.72）；预测阅读时间（ΔlogLik=38.4）；跨语言泛化。

Conclusion: 稀疏自编码发现的特征捕捉了已知的皮层语义组织，且可预测行为指标，揭示了中间层预测大脑反应的机制。
...
TL;DR: 提出基于多语言SAE的可靠语言控制方法，并引入层选择规则。


<details>
  <summary>Details</summary>
Motivation: 现有SAE仅用英语训练且层选择启发式，在多语言环境中不可靠。

Method: 在多语言数据上训练SAE，并基于多语言对齐与语言可分离性交叉选择干预层。

Result: 多语言SAE结合交叉选择层稳定了语言识别准确率与生成质量的权衡。

Conclusion: 提供了原则性、预测性的多语言SAE控制机制。
...
TL;DR: 大语言模型通过分布竞争获取否定语言知识，支持构式语法的统计优先假设。


<details>
  <summary>Details</summary>
Motivation: 探究学习者如何在没有负面证据的情况下习得不可接受性知识，区分统计优先与竞争固化假设。

Method: 对120个英语动结构式配对（与格、使役、处所）进行四项实验，使用大语言模型计算意外度，并与人类可接受性判断比较，进行相关分析、部分相关性分析、模型规模缩放分析以及因果微调干预。

Result: 大语言模型的意外度模式与人类判断强相关（r=0.79），由竞争形式频率驱动，而非整体动词频率；优先敏感性随模型规模呈幂律增长；因果微调证实操纵竞争形式频率可改变优先行为。

Conclusion: 神经语言模型通过分布竞争获取否定语言知识，支持构式语法的核心机制。
...
TL;DR: 提出了HawkesLLM框架，将时间影响建模与文本生成分离，在GDELT案例中改善了后期语义对齐。


<details>
  <summary>Details</summary>
Motivation: 文本模拟系统中不确定性路径依赖，早期歧义影响后续输出。

Method: 将级联表示为网络，用多变量Hawkes过程建模节点激活和影响选择，语言模型根据所选记忆生成文本。

Result: 在GDELT数据上，HawkesLLM在紧凑提示记忆预算下改善了后期语义对齐。

Conclusion: 分离时间建模与文本生成可有效改善长序列生成中的语义一致性。
...
TL;DR: 提出了一个混合框架DreamerNLplus，用于从社交媒体时间线建模心理健康动态，在CLPsych 2026的三个任务中取得优异成绩。


<details>
  <summary>Details</summary>
Motivation: 为了有效建模心理健康动态，解决心理状态建模、时间变化检测和序列级总结三个任务。

Method: 结合LLM数据增强、DeBERTa分类和随机森林回归预测心理状态；使用本地Llama 3.1模型进行少样本提示检测变化事件；采用确定性规则流水线和少样本LLM方法进行总结；RAG方法捕捉重复变化模式。

Result: 在任务3.1排名第二，任务3.2中改善排名第一、恶化排名第三。

Conclusion: 揭示了分类与回归性能不匹配、时间转换建模困难以及评估指标不一致等挑战，表明心理健康动态建模的复杂性。
...
TL;DR: 本文利用文化进化中的迭代学习理论解释了语言模型在自训练时出现的模型崩溃现象，并通过多语言实验验证了五个可证伪预测，关键发现是组合性呈非单调轨迹，为压缩-通信权衡提供了首个大规模语言模型证据。


<details>
  <summary>Details</summary>
Motivation: 现有模型崩溃研究主要从统计角度描述，缺乏对哪些语言结构退化、退化顺序及原因的深入解释。本文旨在利用文化进化理论填补这一空白。

Method: 从迭代学习理论推导出五个可证伪预测，并区分了关键性预测与验证性预测。通过LLaMA-2-7B和Mistral-7B在英语、德语和土耳其语上自训练10代，测试这些预测。

Result: 关键发现：在无过滤自训练下，组合性呈先升后降的非单调轨迹；该特征在最大规则种子数据下依然存在，且仅任务导向过滤能维持此特征。所有预测均得到验证，效应量大（Hedges' g > 1.6, BF10 > 100），且正则化梯度与人类行为数据高度吻合（R² = 0.94）。

Conclusion: 模型崩溃应被重新定义为文化传播现象，并为自训练管道设计提供了具体原则。
...
TL;DR: 训练数据组成是强化学习训练记忆智能体的细粒度专业化杠杆，混合课程最优，单数字基准低估课程效应，单GPU下需过滤格式噪声并采用连续奖励函数。
...
Method: 控制实验：固定架构、RL算法和超参数，变化训练课程（域内、混合、域外），在两个基准上评估。

Result: 混合课程整体F1最优；窄域外训练专项迁移时间推理；类型间差异远超整体差异。

Conclusion: 课程组成是专业化杠杆而非统一缩放因子，单数字基准比较系统性地低估课程效应；单GPU下GRPO需过滤内存库格式噪声并采用连续奖励函数。
...
*Yusser Al Ghussin,Daniil Gurgurov,Yasser Hamidullah,Josef van Genabith,Cristina Espa\~na-Bonet,Simon Ostermann*

Main category: cs.CL

TL;DR: 使用激活引导改进大语言模型文化知识，在SemEval-2026 MCQ任务上获86.96%准确率，但效果依赖于层和语言区域，需联合优化提示设计。


<details>
  <summary>Details</summary>
Motivation: 大模型在不同语言和文化背景下的文化知识不均衡，需要提升文化意识。

Method: 从平行FLORES数据中提取语言向量，在推理时添加到残差流特定层进行激活引导，无需参数更新。

Result: 官方MCQ准确率86.96%，排名7/17；事后分析显示改进具有层敏感性和语言区域异质性，部分配置导致性能下降。
...
TL;DR: 提出效率前沿框架，统一优化LLM上下文管理的成本与性能，在HotpotQA上实现约25%令牌节省。


<details>
  <summary>Details</summary>
Motivation: 现有上下文缩减方法独立评估性能和效率，缺乏系统比较和部署感知优化。

Method: 将策略选择建模为部署感知优化问题，通过分摊成本建模联合考虑性能、令牌成本和预处理重用。

Result: 在5000个实例上，部署感知优化减少25%令牌使用（F1≈0.78），内存压缩在高性能场景降低50%以上令牌成本。

Conclusion: 框架为评估和部署高效、可持续的LLM系统提供了原则性和实用基础。
...
TL;DR: 比较结构主题模型（STM）和BERTopic用于短文本主题建模，发现BERTopic主题连贯性更好，上下文增强策略显著提升效果；STM主题更宽泛但支持推断性共变量分析。两者各有优势，提供实践选择建议。


<details>
  <summary>Details</summary>
Motivation: 现有主题模型评估多基于长文本，对短开放式问卷回答的指导不足。

Method: 对比三种STM条件和五种BERTopic条件，包括拼写纠正、词干提取、嵌入选择及自创的上下文增强策略。

Result: BERTopic连贯性高于STM；上下文增强最佳；高维嵌入未提升连贯性但导致数据丢失；STM主题较宽泛，BERTopic更稳定可解释。

Conclusion: STM和BERTopic互补：STM适用于推断性分析，BERTopic适用于主题发现。建议根据研究目标选择或结合使用。
...
TL;DR: Lepton是一个针对古典中文文集标题的微调BERT分类器，能够准确区分个人信件与易混淆的序文（特别是送别序）。


<details>
  <summary>Details</summary>
Motivation: 自动识别大量古典文集中的信件标题，以构建明代信件平台，支持中国传记数据库（CBDB）的研究。

Method: 基于bert-base-chinese，在5438个手工标注的标题（来自33位晚明和清初文人）上进行微调。

Result: 模型部署于Hugging Face，在CBDB中用于识别从中明到清初文集中约五万五千封信件，并填充了明代信件平台。

Conclusion: Lepton成功区分信件与易混淆的序文，为大规模信件识别提供了有效工具，促进了传记数据库的构建。
...
TL;DR: 角色提示在残差流中呈现加性结构，但无法压缩为单缓存向量。


<details>
  <summary>Details</summary>
Motivation: 探究角色提示的分解结构和压缩可能性。

Method: 分析提示到答案转换位置的加性分解，注入纯角色效应和纯任务效应，测试压缩。

Result: 加性结构适用于短网格，但长角色网格中压缩失败，角色行为依赖分布式机制。

Conclusion: 局部加性不意味着可压缩，角色条件行为需要分布式提示/KV机制。
...
TL;DR: 本研究构建了基于SCID的基准测试，评估五个大语言模型在心理健康筛查中的表现，发现模型性能因任务而异，且倾向于在有功能保留或保护性背景时低估症状证据。


<details>
  <summary>Details</summary>
Motivation: 心理健康服务需求超出临床评估能力，需要可扩展的筛查工具，而LLM可能通过患者叙述识别精神风险，但其跨诊断和亚组的可靠性尚不确定。

Method: 使用555个半结构化访谈和SCID诊断标签，通过零样本任务特定提示评估五个前沿LLM，分析错误模式及症状、功能损伤和保护性背景证据的加权情况。

Result: 准确率0.49-0.86，MCC 0.16-0.38；GPT-4.1 Mini和GPT-5 Mini诊断一致性最佳；男性抑郁症分类准确率高于女性；假阴性焦虑和PTSD分类常含症状但伴有保留功能或社会支持。

Conclusion: LLM可能支持可扩展的精神科筛查，但在临床部署前需谨慎验证其低估症状证据的倾向。
...
TL;DR: 首次跨语言多模态红队研究显示，语言对多模态大模型的安全漏洞影响不均匀，语言和视觉对齐失败机制不同，安全排名跨语言不保持。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型的攻击面存在语言依赖性，但缺乏系统性跨语言多模态红队研究。

Method: 对四种前沿多模态大模型（Claude Sonnet 4.5, GPT-5, Pixtral Large, Qwen Omni），使用363个多样化提示场景，在纯文本和多模态条件下测试，由每组9名母语注释者收集52,272个危害评分。

Result: 语言角色扮演攻击在西班牙语中效果下降，视觉显式多模态攻击效果上升；Qwen Omni在西班牙语中成为最易攻击模型，英语安全排名不适用于西班牙语。

Conclusion: 安全评估框架将语言和模态视为独立维度从根本上错误描述了全球部署的多模态大模型的攻击面，必须重新设计。
...
### [29] [Fast-dDrive: Efficient Block-Diffusion VLM for Autonomous Driving](https://arxiv.org/abs/2605.23163)
...
TL;DR: Fast-dDrive是一种用于端到端自动驾驶的块扩散视觉-语言-动作模型，通过双向语义块内细化和跨块因果顺序约束，结合结构化标记冻结、节感知训练、脚手架推测解码和低开销测试时缩放，在保证规划质量的同时实现了显著的速度提升。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶模型存在矛盾：自回归VLA在边缘硬件上受内存带宽限制且易偏差，全序列扩散模型排除KV缓存复用并违反因果顺序。需要一种既能高效推理又能保持高保真轨迹规划的方法。

Method: 提出块扩散VLA，在语义单元内进行双向细化并强制执行跨单元因果顺序；冻结结构令牌为节脚手架，采用节感知训练优先考虑安全关键规划；引入脚手架推测解码实现AR相当质量且更高吞吐量；通过从共享前缀KV缓存分叉N个随机轨迹并平均来低开销抑制预测方差。

Result: 在WOD-E2E测试集上达到ADE@3s和ADE@5s的最优结果，在扩散VLA中拥有最高RFS；在nuScenes上平均L2误差降至0.32米（提升22%），与SGLang集成后相对于AR基线实现12倍吞吐量提升。
...
### [30] [Positional Failures in Long-Context LLMs: A Blind Spot in Reasoning Benchmarks](https://arxiv.org/abs/2605.23170)
*Chuyifei Zhang,Hongyu Cui,Xiaowen Huang,Jitao Sang*

Main category: cs.CL

TL;DR: 提出上下文旋转评估（CRE）框架，控制任务位置、填充内容和上下文长度，揭示长上下文推理评估中的位置偏差漏洞。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文基准未同时控制任务位置、填充内容和上下文长度，导致评估存在结构缺陷。

Method: 通过CRE框架控制三个因素，在GSM8K和ARC-Challenge上评估9个LLM，使用两种填充内容，并设置诊断探针。

Result: 模型在目标从末尾移到中间时准确率急剧下降，随上下文长度增加而加剧；新模型改善但仍存在；76%的中位置错误匹配填充文本。

Conclusion: 当前推理基准和厂商评估存在位置偏差，不控制任务位置无法测量随长度增长的位置漏洞。
...
### [31] [Self-Improving In-Context Learning](https://arxiv.org/abs/2605.23180)
*Baturay Saglam,Dionysis Kalogerias*

Main category: cs.CL

TL;DR: 提出一种在测试时优化提示嵌入的上下文学习方法，利用对数概率作为自监督置信度代理，无需微调或生成token，适用于分类和生成任务。


<details>
  <summary>Details</summary>
Motivation: 固定示例提示的嵌入可优化，模型的输出对数概率提供了任务推断质量的信号，可用于测试时校准。

Method: 将示例输出的对数概率形式化为有界自监督置信度代理，通过零阶优化在连续提示嵌入空间上最大化该代理。

Result: 在多种上下文学习任务中，该方法匹配或优于基础模型，并超越分类特定基线，代理改进与下游准确率显著相关。
...
### [32] [Hidden Human-Like Nature of Machine-Generated Texts: Theory and Detection Enhancement](https://arxiv.org/abs/2605.23190)
*Chenwang Wu,Yiu-ming Cheung,Bo Han,Defu Lian*

Main category: cs.CL

TL;DR: 针对机器生成文本中隐藏类人片段增加检测难度的问题，提出模型无关的堆叠增强框架，通过硬EM迭代过滤类人片段，显著提升检测器性能。


<details>
  <summary>Details</summary>
Motivation: 现有段落级检测方法忽略机器文本中隐藏的人类相似片段，这些片段增加了检测复杂性，需要改进。

Method: 将跨度保留决策建模为潜变量问题，采用硬EM过程迭代过滤自信的人类相似子序列，并在剩余文本上优化检测器。

Result: 在各种LLM和实际场景中，该框架一致提升现有检测器性能，且可无需训练工作。

Conclusion: 该框架有效提升机器文本检测的鲁棒性和灵活性，适合实际部署。
...
### [33] [When Is Next-Token Prediction Useful? Marginalization, Ergodicity, Mixture Identifiability, Local Sufficiency, RAG, Tools, and Programming](https://arxiv.org/abs/2605.23278)
*Francesco Corielli*

Main category: cs.CL

TL;DR: 本文区分语言模型中的三种分布，论证训练作为边缘文本估计需强假设，且仅在观测前缀是潜在环境充分统计量时有效，最后将RAG和工具使用解释为条件充分性设备。


<details>
  <summary>Details</summary>
Motivation: 语言模型训练常被曲解为学习下一个词的条件分布，实际仅观测采样续接且受非文本因素影响，需厘清概念。

Method: 理论分析区分完整条件过程、边缘文本过程和模型诱导分布，探讨异质语料下适用条件，信息论视角解释RAG和工具使用。

Result: 边缘文本规律需平稳性、代表性、遍历性假设，且要求给定文本下下一个词与遗漏环境的条件互信息小。

Conclusion: RAG和工具使用作为条件充分性设备，缓解异质环境中的预测困难。
...
TL;DR: 大型语言模型的表示收敛主要源于共享的输入处理约束，而非共享的推理策略。


<details>
  <summary>Details</summary>
Motivation: 验证表示收敛是否延伸到推理过程，即模型是否共享推理策略。

Method: 在16个模型（8个家族，参数量1.5B-72B）上评估800个推理问题，按难度、计算阶段和因果相关性分层，使用CKA和因果分析。

Result: 发现三个分离：难度反转（失败问题CKA=0.897高于成功CKA=0.830）、生成差距（决策前CKA=0.875，决策后CKA=0.274）、附带正确性（跨模型可解码但因果影响小）。

Conclusion: 表示收敛反映共享输入处理约束，而非共享推理策略，对集成设计、解释性转移和模型相似性评估有重要影响。
...
TL;DR: 本文介绍了AraHopeCorpus，第一个阿拉伯语希望言论注释数据集，来自加沙战争相关的YouTube评论，发现希望言论占主导（64%），表现为宗教鼓励、团结和乐观，同时揭示了方言和讽刺带来的注释挑战。


<details>
  <summary>Details</summary>
Motivation: 仇恨言论和虚假信息已被广泛研究，但表达韧性、团结和乐观的言论在阿拉伯语境中未被充分探索。

Method: 从2023-2024年加沙战争相关的1万条YouTube评论中构建数据集，采用详细注释框架将评论分为希望言论、无希望言论和中立/模糊三类，并进行一致性评估和与ChatGPT对比。

Result: 希望言论占64%，无希望言论占13%；注释者间一致性Cohen's Kappa=0.71；ChatGPT在方言和文化表达上表现有限。

Conclusion: AraHopeCorpus为研究建设性数字话语、希望言论检测和危机沟通提供了宝贵资源，但需注意方言和隐含意义处理。
...
TL;DR: 构建了一个包含约30万条Facebook气候相关帖子的大型数据集ClimateChat-300K，通过主题建模和情感分析揭示了公共气候话语的演变和参与模式。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏大规模、高覆盖率的气候变化社交媒体数据集，难以支持对公众话语动态、极化及错误信息的深入分析。

Method: 从CrowdTangle平台收集2020年5月至2024年5月的公开帖子，提取41个元特征；应用主题建模（LDA）和情感分析工具对文本进行聚类和情感标注，划分主题领域。

Result: 识别出政策、行动主义、合作、科学、保护五大领域共十个主题；发现情感基调、帖子格式（如视觉丰富度）和页面身份显著影响用户互动水平；展示了主要事件（如气候峰会、疫情）对讨论话题的驱动作用。

Conclusion: 该数据集为可复现、跨学科的气候变化公共话语研究提供了资源，有助于理解公众参与的地域、时间及制度差异。
...
TL;DR: 提出了面向手语对话的情感识别数据集eJSL Dialog，并发现通用多模态对话情感模型存在领域差距。


<details>
  <summary>Details</summary>
Motivation: 手语情感数据集集中于孤立句子，缺乏对话上下文，导致模型在真实场景中性能下降，因此引入对话情感识别任务。

Method: 利用STUDIES语料库脚本构建包含1920个视频样本、480个独特对话的eJSL Dialog数据集，并使用从孤立视觉网络到多模态对话架构的模型进行系统基准测试。

Result: 通用多模态对话情感识别模型应用于手语时存在领域差距。

Conclusion: 需要针对手语的上下文感知视觉提取器，并扩大对话数据集规模以支持大规模预训练。
...
TL;DR: 本文论证文化适应是大语言模型在不同政治话语中可信部署的前提，并提出评估矩阵与方法路径。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在政治话语分析中带来机遇，但现有系统因英语主导数据和有限的政治制度假设而产生系统错误，影响民主问责。

Method: 形式化翻译、话语和本体层面的文化适应，提出以文化忠实度、校准和民主安全为核心的评估矩阵，并采用参与式数据集开发、文化感知迁移学习等方法。

Result: 识别了政治NLP中的重复文化失败模式，使文化适应变得可测量。
...
TL;DR: 提出了一个统一的个性化智能体强化学习框架，通过解耦奖励、用户特定锚点和偏好对齐的记忆，实现用户条件化行为。


<details>
  <summary>Details</summary>
Motivation: 现有智能体强化学习在具有明确成功信号的任务上表现良好，但许多实际应用需要根据用户不同偏好调整策略和工具使用，而通用奖励无法捕捉异质性偏好，且观察行为受从众效应影响，平面记忆不支持个性化技能检索。

Method: 提出PARPO算法，将个性化嵌入训练优化，解耦通用任务质量奖励和个性化偏好奖励，并使用用户特定锚点稳定学习。引入两阶段偏好解耦奖励模型和偏好对齐技能演化图记忆（PSGM），形成闭环。

Result: 在ETAPP、ETAPP-Hard和SJAgent上，该方法一致优于强记忆和强化学习基线。

Conclusion: 验证了所提框架在个性化智能体任务中的有效性。
...
TL;DR: 提出元认知奖励（MaR）框架，通过元认知知识和元认知调节两个通用维度引导LLM推理，扩展奖励信号至推理轨迹，显著提升推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有奖励设计要么仅提供最终结果信号（RLVR），缺乏中间指导；要么需要实例特定规则（RaR），设计工作量大。

Method: MaR将模型输出分解为显式元认知组件，基于任务知识覆盖、调节保真度和最终答案正确性进行轨迹级奖励优化。

Result: 在22个基准测试中，MaR相对于基础模型提升高达7.7%，相对于DAPO提升11.0%；Qwen3.5-9B+MaR在平均性能上超越GPT-OSS-120B，且过程质量显著提升。

Conclusion: MaR通过通用元认知维度的奖励反馈，有效扩展了奖励信号范围，是一种简单而高效的推理强化学习框架。
...
TL;DR: 提出EquiSumm，一种考虑性别公平的推特摘要生成方法，在两大数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有自动摘要方法未明确考虑人口统计学公平性，可能导致有偏见的摘要。

Method: EquiSumm在生成摘要时明确考虑观点的性别维度。

Result: 在两个主要数据集上的实验分析表明，EquiSumm在性能上优于现有研究。

Conclusion: EquiSumm能有效生成兼顾性别公平的摘要，且性能优越。
...
TL;DR: 通过超声舌成像数据，发现舌形差异可预测双元音共振峰动态，解释了发音策略与声学特征之间的系统关联。


<details>
  <summary>Details</summary>
Motivation: 现有证据不足，需验证发音策略是否系统性影响共振峰动态。

Method: 采集36位北英英语发音人的超声舌成像数据，分析其/ i /音舌形及后续双元音共振峰动态。

Result: 舌形显著预测共振峰变化；舌根/舌背位移大导致共振峰转变更早更陡。
...
*Yevhen Kostiuk,Kenneth Enevoldsen,Peter Bjerregaard Vahlstrup,M\'arton Kardos,Kristoffer Nielbo*

Main category: cs.CL

TL;DR: 提出一个通过解决方案匹配测量自然对话中社会规范对齐的框架，并构建了丹麦语社会困境数据集。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖封闭式问卷，无法测量自然对话中的社会规范对齐。

Method: 提出显式一致性和陈述一致性两种指标，通过匹配解决方案评估LLM与人类的对齐。

Result: 指标产生一致的模型排名，不同困境类型对齐程度不同，邻里冲突等话题对齐较高。

Conclusion: 数据集和评估框架可用于研究文化基础的社会推理。
...
TL;DR: 提出结构化语义数据增强（SSDAU）方法，通过保留文本语义结构来提升联合实体关系抽取的泛化能力，实验表明其鲁棒性显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据增强方法忽视文本相关性，可能破坏语义结构和依赖，难以有效提升模型泛化能力。

Method: 基于实体标签对文本进行分割，利用编码器通过上下文感知捕获实体语义特征，进行实体语义重组生成增强数据；融合上下文嵌入与相似度分数区分语义相似实体，并应用BERTTopic过滤无关主题以保持主题一致性。

Result: 在多种标注类型的数据集上，相比七个基线方法，SSDAU生成的数据具有语义一致性和强鲁棒性（F1下降8.26%对比基线的31.91%），在所有指标上显著超越现有方法。

Conclusion: SSDAU能够生成语义一致的数据，有效缓解联合实体关系抽取中的弱泛化问题，并展现出优越的鲁棒性。
...
TL;DR: ARES自动构建基于评分标准的强化学习数据，从原始预训练文档生成问题-答案对和问题特定的加权评分标准，实现实例级奖励监督。


<details>
  <summary>Details</summary>
Motivation: 基于评分标准的奖励可扩展大型语言模型的强化学习，但手动编写评分标准和问题集难以规模化，且固定评分标准无法捕捉个别问题的评估需求。

Method: ARES从原始预训练文档自动生成自包含的问题-答案对和问题特定的加权评分标准，通过领域标签和人物信息提高多样性，并应用验证过滤器确保质量。

Result: 构建了10万评分标准注释实例，在七个基准测试中，基于评分标准的强化学习优于持续预训练、监督微调和二元奖励强化学习，尤其在多维度开放任务上（如医疗、指令跟随）。

Conclusion: ARES能够有效自动化构建评分标准数据，提升强化学习效果。
...
TL;DR: 大型语言模型在静态参数知识与动态法律之间的时间错配导致两种失效模式：截止后过时和近因偏差。通过312个德国法律QA对基准测试，发现检索增强生成（RAG）显著优于普通模型和网络搜索，后者存在近因偏差。结论是可靠的法律问答需将时间有效性作为硬约束。


<details>
  <summary>Details</summary>
Motivation: 法律研究需要动态更新的知识，但LLM的训练截止和静态参数无法适应法律修订，导致时间相关的错误。

Method: 构建包含312个专家验证的德国法律时间敏感QA对（分三类），在四种推理设置下评估五个LLM，并引入基于日期提取和版本过滤的RAG变体。

Result: 普通模型在截止后设置中表现严重下降；RAG方法在所有问题类型上显著提升性能；网络搜索提升不稳定且存在近因偏差。

Conclusion: 可靠的法律问答必须将时间有效性作为硬性约束，RAG是有效缓解时间错配的手段。
...
TL;DR: 提出结构引导实体解析（SGER）框架，通过两阶段课程微调LLM，在印度身份数据上达到99.02%准确率，成功部署于Dream11平台。


<details>
  <summary>Details</summary>
Motivation: 跨异构记录匹配人名在复杂语言环境中面临命名惯例差异、音译不一致和数据错误等挑战，现有LLM对结构化歧义处理不足。

Method: 采用两阶段课程学习微调LLM：先训练模型解析人名的语法和语义结构，再优化二元实体匹配任务。

Result: 在5万对真实数据上准确率99.02%，F1分数0.994，优于GPT-4o少样本学习和单阶段微调基线。

Conclusion: 课程引导训练能实现鲁棒、高精度的实体解析，适用于多语言系统的大规模生产环境。
...
*Bj\"orn Nieth (1,4),Marianna Gracheva (2),Michaela Mahlberg (2,3),Bjoern Eskofier (1,3,5,6),Emmanuelle Salin (1) ((1) Department Artificial Intelligence in Biomedical Engineering (AIBE) FAU Erlangen-N\"urnberg Germany,(2) Department of Digital Humanities,Social Studies (DHSS) FAU Erlangen-N\"urnberg Germany,(3) University of Birmingham United Kingdom,(4) Chair of AI-supported Therapy Decisions LMU M\"unchen Munich Germany,(5) Munich Center for Machine Learning (MCML) Munich Germany,(6) Institute of AI for Health Helmholtz Zentrum M\"unchen Neuherberg Germany)*

Main category: cs.CL

TL;DR: 提出基于语料库语言学的情境感知评估框架，通过对比人类与LLM生成文本的语言特征分布，衡量文本的人类相似度。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注事实正确性和任务性能，忽略了文本在语言学层面的人类相似性，而语境依赖的语言特征差异会影响读者感受。

Method: 采用双样本问题框架，使用最大均值差异（MMD）和Biber的67个词汇语法特征，比较给定语域的人类参考语料与LLM生成语料。

Result: 在所有设置中，LLM均偏离人类基线；但最接近人类语言的模型因语域而异，与模型大小无关。

Conclusion: 评估文本人类相似度需考虑语境和语言学特征，模型选择应基于目标语域。
...
TL;DR: 提出OpenSkillEval，一个自动评估技能增强代理系统和技能质量的框架，通过动态任务实例和社区技能进行系统评估。


<details>
  <summary>Details</summary>
Motivation: 开源技能生态系统快速扩展，但缺乏对不同模型和代理框架如何与技能交互、技能质量评估以及成本性能权衡下技能选择的清晰理解。

Method: 自动从现实世界工件构建五个应用类别的动态任务实例，收集社区技能进行统一任务设置下的控制比较。

Result: 技能可用不保证有效使用；技能增强收益高度依赖底层模型和代理框架；许多流行技能并不一致优于无技能基线。

Conclusion: 需要动态、任务基础的评估，为LLM代理技能的设计、选择和部署提供实用见解。
...
TL;DR: 提出了ChartFI-Bench基准，从事实准确性、突出特征、领域指导和图文互补四个维度评估MLLM生成的图表描述，并设计了相应的评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有基准数据集简单、描述浅薄，且评估指标无法全面衡量描述质量。

Method: 构建包含896个图表-描述对的高质量基准，定义四个质量维度，设计四个对齐的评估指标（忠实度、覆盖率、信息量、敏锐度）。

Result: 实验表明该框架有效，并揭示了主流MLLM的常见弱点。

Conclusion: ChartFI-Bench为评估和提升图表描述质量提供了系统框架。
...
TL;DR: 提出一种结合元数据主导分数（MPDS）和证据干预统计（ΔEvi）的弱标签基准测试审计方法，并通过实验展示其必要性。


<details>
  <summary>Details</summary>
Motivation: 现有元数据快捷方式检查仅回答元数据先验问题，无法检测证据层面的干预影响，需要更全面的测试。

Method: 结合MPDS（衡量元数据先验主导性）和ΔEvi（通过跨项目打乱测量对证据身份的敏感性），并在多个数据集上使用不同强度的阅读器进行实验。

Result: 合成HotpotQA中MPDS中等（0.643）但ΔEvi为零；SNLI出现校准反转；重建HotpotQA处于问题主导警告区；FEVER作为强证据敏感正对照。

Conclusion: 基准审计应同时报告元数据筛查、证据干预和阅读器强度校准。
...
TL;DR: 本文提出基于图的方法，利用BERT和词义增强嵌入分析名词的语义类型匹配与强制转换现象，通过邻居类型概率和熵指标区分匹配与不匹配句子。


<details>
  <summary>Details</summary>
Motivation: 语义类型不匹配是强制转换现象的核心，现有方法未能充分利用词汇和上下文类型信息在词嵌入中的表现。

Method: 从十种语义类型选取名词，标注语料实例为类型匹配、强制转换、其他不匹配或未限制；使用BERT和词义增强嵌入构建图；提出邻居类型概率（NTP）和邻居类型熵（NTE）两个指标分析邻居类型分布。

Result: 词义增强嵌入构建的图能更好地反映语义类型信息；通过NTP和NTE指标可以区分匹配和不匹配句子。

Conclusion: 图方法和提出的两种指标有效揭示了语义类型信息在词嵌入中的体现，有助于理解强制转换等语言现象。
...
TL;DR: 概述了NLG评估从1990年至今的演变，并预测未来将更注重影响、质量和安全评估。


<details>
  <summary>Details</summary>
Motivation: 回顾NLG评估的发展历程，展望未来方向，以指导后续研究。

Method: 讨论了从语言学导向到机器学习导向的实验评估方法，包括最近的大语言模型作为评判者（LLM-as-Judge）技术。

Result: NLG评估从1990年的非正式评估发展到2026年的实验评估为核心，未来将更注重影响、质量和安全评估。

Conclusion: NLG评估将继续演化，影响、质量和安全评估将变得更加重要。
...
TL;DR: 基于分类器的质量过滤在构建预训练语料中很常用，但研究发现简单的维基百科式格式化操作能显著改变模型的质量评估，使低质量内容通过过滤，例如FineWeb-Edu CQF模型对约7%的文档会反转过滤决定。


<details>
  <summary>Details</summary>
Motivation: 揭示基于分类器的质量过滤方法存在关键漏洞，即格式化操作可绕过过滤。

Method: 通过实施维基百科式的格式化操作，测试其对分类器质量评估的影响。

Result: FineWeb-Edu CQF模型对大约7%的评估文档反转了过滤决策，允许原本被排除的低质量内容进入语料。

Conclusion: 当前基于分类器的质量过滤技术对输入格式敏感，存在被简单操作规避的风险，需加强鲁棒性。
...
TL;DR: 提出分布理论解释上下位关系在语言表示中的几何编码，证明词嵌入的谱结构产生层次分裂几何，反映语义树。


<details>
  <summary>Details</summary>
Motivation: 理解上下位关系如何几何编码在语言表示中，基于共现频率的假设。

Method: 理论分析词嵌入Gram矩阵的谱，假设共现核的正性和衰减条件，证明特征向量产生粗细层次结构。

Result: 在word2vec和Gemma 2B嵌入中验证预测，显示层次几何不依赖于特定功能机制，而是来自词统计的谱结构。

Conclusion: 层次概念几何在LLM中可能源于统计谱结构，而非专门机制。
...
TL;DR: 提出LINK方法，在预训练数据中通过双语词汇表进行词汇替换，提升跨语言知识迁移，无需额外数据或模型。


<details>
  <summary>Details</summary>
Motivation: 低资源语言训练数据不足，需要从高资源语言迁移知识，而现有方法依赖大量平行数据或翻译系统，许多语言难以获得。

Method: LINK：在预训练阶段，按替换比例随机将高资源语言（英语）语料中的单词替换为双语词汇表中的翻译，无需额外训练或模型。

Result: 在八种语言、五种模型大小上，下游任务性能显著提升，训练速度最高提升2倍。

Conclusion: LINK是一种简单、低成本的数据级干预方法，能有效提升跨语言知识迁移，尤其适用于低资源语言。
...
### [59] [Instance-Optimal Estimation with Multiple LLM Judges on a Budget](https://arxiv.org/abs/2605.23362)
*Junghyun Lee,Sanghwa Kim,Yassir Jedra,Alexandre Prouti\`ere,Se-Young Yun*

Main category: cs.LG

TL;DR: 提出预算异方差多裁判评估问题，并设计自适应算法EST-IVWE，在预算下达到最优估计误差率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型评估依赖LLM-as-a-judge协议，但不同裁判成本与可靠性不同，且实例难度各异，固定预算下需优化分配以最小化估计误差。

Method: 形式化为预算异方差多裁判估计问题，分析逆方差加权估计量（IVWE）并导出最优分配；针对未知方差，提出EST-IVWE算法，利用乐观偏差方差估计稳定分配，并证明其匹配最优率；通过Assouad型期望论证导出局部极小化下界。

Result: EST-IVWE在预算上匹配IVWE最优率（忽略低阶项）；局部极小化下界证明算法实例最优性；数值实验显示优于均匀分配。

Conclusion: 所提自适应分配方法在给定预算下能显著提升评估精度，优于朴素均匀分配。
...
### [60] [Entrywise Error Bounds for Spectral Ranking with Semi-Random Adversaries](https://arxiv.org/abs/2605.23854)
*Dongmin Lee,Anuran Makur,Japneet Singh*

Main category: cs.LG

TL;DR: 针对半随机对手，研究BTL模型谱算法的逐项误差，发现重新加权可恢复均匀采样图的性能。


<details>
  <summary>Details</summary>
Motivation: 现有理论多针对均匀采样图，需要推广到更广的随机图类，特别是对抗性设置。

Method: 分析未加权谱方法，提出重新加权边以对抗对手并恢复谱间隙。

Result: 未加权谱方法性能依赖图的谱性质；重新加权可达到与均匀采样图相近的渐近性能。

Conclusion: 通过适当重加权，可以在对抗性设置下实现接近均匀采样图的性能。
...
### [61] [Latent Cache Flow: Model-to-Model Communication Without Text](https://arxiv.org/abs/2605.22863)
*Maximillian Rossi,Prajwal Raghunath,Eugene Wu*

Main category: cs.LG

TL;DR: 提出Latent Cache Flow (LCF)，一种高效的LLM代理间通信方法，通过联合翻译和压缩KV缓存，减小适配器大小，支持不同上下文，比现有方法更准确且速度更快。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理通过文本通信延迟高、信息损失大；Cache-to-Cache方法适配器大、训练成本高且要求上下文相同，不适合代理间通信。

Method: LCF通过联合翻译和压缩键值对，将适配器大小降至C2C的约4%，并设计适配器传输目标模型缺失的新信息摘要，支持不同上下文。

Result: 在共享上下文场景中，13 MB的LCF适配器比956 MB的C2C适配器更准确；在不同上下文场景中，LCF比文本通信准确率提高23%，速度提升8.5倍。

Conclusion: LCF是一种高效、准确的LLM代理间通信方法，解决了现有方法的效率与上下文差异问题。
...
### [62] [Reading Calibrated Uncertainty from Language Model Trajectories](https://arxiv.org/abs/2605.22864)
*Aliai Eusebi,Alexander Herzog,Xiaoyu Liang,Marie Vasek,Enrico Mariconti,Lorenzo Cavallaro*

Main category: cs.LG

TL;DR: 提出一种从语言模型内部逐层MLP更新中提取几何特征的方法，通过稀疏线性探针改进不确定性估计，在选择性弃权任务中优于最大softmax概率（MSP），性能提升最高达21 AURC点。


<details>
  <summary>Details</summary>
Motivation: 最大softmax概率（MSP）虽简单但校准不佳，现有探测方法忽略了表示形成的逐层轨迹，而相似终点可能源于不同路径，最终概率可能掩盖不确定性。

Method: 提取十一种尺度不变的几何特征，追踪逐层MLP更新的累积路径，输入稀疏线性探针进行不确定性估计。

Result: 在选择性弃权任务中，该方法优于MSP，性能提升与基线校准误差成正比，最高达21 AURC点。
...
### [63] [FusionSense: Tri-Stage Near-Sensor Learning for Runtime-Adaptive Multimodal Edge Intelligence](https://arxiv.org/abs/2605.22868)
*Sanggeon Yun,Ryozo Masukawa,Minhyoung Na,Hyunwoo Oh,Yoshiki Yamaguchi,Wenjun Huang,SungHeon Jeong,Mohsen Imani*

Main category: cs.LG

TL;DR: FusionSense是一种面向能量受限自主边缘系统的融合感知框架，通过轻量级近传感器分类器联合减少计算与通信开销，在双模态（RGB+深度/激光雷达）数据集SynDrone上实现高达33倍的能耗降低和92.3%的质量损失减少。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么在服务器端融合多模态数据导致高能耗和延迟，要么在近传感器端进行单模态过滤忽略跨模态依赖导致冗余传输或漏检，亟需一种融合感知的自适应决策方法。

Method: 三步训练过程：1）服务器端融合模型学习下游任务；2）生成滤除安全标签量化各模态对融合决策的必要性；3）通过注入近传感器预测作为辅助信号压缩边缘端融合模型，形成运行时联合决策层。

Result: 在SynDrone双模态设置下，相比单模态过滤器，FusionSense在数据压缩率上显著提升；在1%跌倒事件发生率下能耗降低33倍，10%时降低11倍；在固定30%数据压缩时质量损失减少92.3%；能量节省比最佳过滤基线高约1.5倍。

Conclusion: FusionSense在保证任务质量的同时实现大幅数据压缩和能耗降低，且随传感器数量线性扩展，优于现有单模态过滤方法。
...
### [64] [FuRA: Full-Rank Parameter-Efficient Fine-Tuning with Spectral Preconditioning](https://arxiv.org/abs/2605.22869)
*Yequan Zhao,Ruijie Zhang,Liyan Tan,Niall Moran,Tong Qin,Zheng Zhang*

Main category: cs.LG

TL;DR: 提出全秩微调框架FuRA，通过块张量列车分解和谱预条件，在保持效率的同时提升微调性能。


<details>
  <summary>Details</summary>
Motivation: 全微调和LoRA忽略预训练权重矩阵的谱结构，导致噪声梯度扰动鲁棒特征，需要谱预条件。

Method: 基于块张量列车分解W=LSR，固定L为预训练块SVD基，只优化R和S，实现全秩谱预条件和高效微调。

Result: 在LLM微调（LLaMA-3-8B常识推理+1.37）、数学推理强化学习、视觉指令微调中优于全微调；4位量化QFuRA优于QLoRA。

Conclusion: FuRA是一种高效的全秩微调方法，通过谱预条件保持预训练特征，参数效率高，性能优于现有方法。
...
### [65] [The Readout Shortcut: Positional Number Copying Dominates Arithmetic CoT Readout in Small Language Models](https://arxiv.org/abs/2605.22870)
*Ming Liu*

Main category: cs.LG

TL;DR: 在小型语言模型中，思维链（CoT）提示对于算术推理并非必需，模型实际上通过复制答案分隔符前的最后一个数字来获取答案，而不是依赖逻辑顺序。


<details>
  <summary>Details</summary>
Motivation: 探究CoT在算术推理中的真正贡献，因为打乱推理步骤仍能保持大部分性能，暗示了位置捷径的存在。

Method: 通过前缀完成实验隔离答案读出阶段，并替换或移除末尾数字进行控制实验，结合头部消融分析架构特定机制。

Result: Gold答案存在贡献了54-92个百分点的准确率（占教师强制上限的89-92%）；错误项中最终答案匹配最后CoT数字的概率为95-96%；替换末尾数字使准确率降至近零，而移除它可恢复5-32个百分点；Qwen和Llama复制新干扰数字概率为87-95%，Gemma有选择性。

Conclusion: CoT步级忠实性评估可能混淆位置性的答案传输与真正计算，这是CoT监督的一个失败模式。
...
### [66] [Approximate Machine Unlearning through Manifold Representation Forgetting Guided by Self Mode Connectivity](https://arxiv.org/abs/2605.22871)
*Weiqi Wang,Zhiyi Tian,Chenhan Zhang,Luoyu Chen,Shui Yu*

Main category: cs.LG

TL;DR: 提出基于流形表示和自模式连通性的机器遗忘方法，在表示空间中实现高效遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有依赖标签或梯度反转的遗忘方法效果有限，且可能损害原始学习目标，不能保证与重训练等效。

Method: 将遗忘重新定义为将擦除样本从其原始流形表示中心推向保留数据中最近语义邻居，使用边际三元组损失，并通过自模式连通性模块自适应生成边际。

Result: 在四个数据集上，该方法在表示空间中实现了与最先进近似方法相当的遗忘效果。

Conclusion: ManiF-SMC在表示空间中实现有效遗忘，减少对标签和任务梯度的依赖，且不损害原始目标。
...
TL;DR: 提出MedExpMem经验记忆框架，使医学视觉语言模型能够累积鉴别诊断经验，在放射学基准上最高提升7.0%准确率。


<details>
  <summary>Details</summary>
Motivation: 当前医学视觉语言模型无法像医生一样通过临床实践积累鉴别诊断经验，参数化学习无法适应新病例的区分需求。

Method: 构建两阶段经验记忆：初始诊断暴露知识缺口，反思性再诊断精炼理解，并将诊断失败转化为成对鉴别笔记（关键区分点、决策规则、推理错误模式），新病例时检索记忆指导推理。

Result: 在涵盖11个亚专科的放射学基准上，多个模型和规模均取得一致准确率提升，最高达7.0%。

Conclusion: MedExpMem是一种有竞争力的方法，解决了参数化学习无法满足的医学适应需求，经验质量和鲁棒性得到验证。
...
### [68] [When Do LLMs Reason? A Dynamical Systems View via Entropy Phase Transitions](https://arxiv.org/abs/2605.22873)
*Wei Xia,Haoqing Wang,Zhi-Hong Deng,Yehui Tang*

Main category: cs.LG

TL;DR: 基于早期解码熵动态，提出轻量级路由框架EDRM，自适应选择推理策略，在多个基准上减少token消耗并提升精度。


<details>
  <summary>Details</summary>
Motivation: 链式思维推理并非始终有益，有时甚至带来负面效果，需要明确何时使用显式推理。

Method: 通过分析早期解码阶段的熵动态，判断任务是否需要CoT，构建熵轨迹流形表示，实现无训练的自适应路由。

Result: 在15个基准和4个LLM上，数据集级别减少41-55% token并提升精度，实例级别减少27-45% token并提升4.7%精度。

Conclusion: 推理应选择性调用，熵驱动的解码控制可实现高效自适应LLM推理。
...
### [69] [WeCon: An Efficient Weight-Conditioned Neural Solver for Multi-Objective Combinatorial Optimization Problems](https://arxiv.org/abs/2605.22876)
*Xuan Wu,Jinbiao Chen,Yang Li,Lijie Wen,Chunguo Wu,Yuanshu Li,Yubin Xiao,Chunyan Miao,You Zhou,Di Wang*

Main category: cs.LG

TL;DR: 提出一种基于权重条件的高效神经求解器WeCon，通过门控残差融合和高效偏好优化，在多目标组合优化问题上达到与最先进方法相当的HyperVolume值，同时推理时间减少40%。


<details>
  <summary>Details</summary>
Motivation: 现有神经求解器在多目标组合优化中通常采用分解策略，但存在权重条件建模受限、权重信号稀释以及偏好优化中解对信息不足导致训练效率低的问题。

Method: 设计包括编码器中的门控残差融合块实现实例特征与权重和谐交互，解码器中的残差融合块缓解权重信号稀释，以及高效偏好优化方法构建高质量解对提升训练效果。

Result: 在四个MOCOP变体、不同问题规模和分布模式上，WeCon达到了与SOTA方法POCCO-W相当的HyperVolume值，推理时间减少40%。

Conclusion: 所提出的设计均有效，WeCon在保持性能的同时显著提升推理效率。
...
### [70] [Tensor Cache: Eviction-conditioned Associative Memory for Transformers](https://arxiv.org/abs/2605.22884)
*Kabir Swain,Sijie Han,Daniel Karl I. Weidele,Mauro Martino,Antonio Torralba*

Main category: cs.LG

TL;DR: 提出Tensor Cache，一种两层缓存机制，结合滑动窗口注意力与外积快速权重记忆，在不增加内存的前提下保留被驱逐的KV对信息，显著提升内存-质量权衡。


<details>
  <summary>Details</summary>
Motivation: 自回归Transformer中的KV缓存随上下文长度线性增长，滑动窗口缓存虽限制内存但完全丢弃被驱逐的token，导致窗口外的相关证据不可访问。

Method: 采用两层缓存：L1为滑动窗口softmax注意力（精确局部关注），L2为固定大小的外积快速权重记忆（由窗口驱逐的KV对压缩更新）。通过可学习的标量门控融合L1和L2输出，并训练逐头衰减和写入率参数。同时指出常见分块均值训练捷径会引入虚假跨token外积，并通过并行加权求和扫描修复。

Result: 在系统缩放、关联召回、长上下文语言建模和记忆容量诊断等任务中，Tensor Cache在内存-质量前沿上优于有界状态基线。

Conclusion: Tensor Cache通过将外积记忆用作二级缓存（仅由滑动窗口驱逐填充），并修复训练捷径问题，有效提升了缓存效率与模型性能的平衡。
...
### [71] [Pointwise Metrics Mislead: An Evaluation Protocol for Multimodal Inverse Problems](https://arxiv.org/abs/2605.22891)
*Mads H. Baattrup,J\"orn Bach,Laurids Jeppe,Finn Labe,Alexander Grohsjean,Christian Schwanenberger,Peer Stelldinger*

Main category: cs.LG

TL;DR: 传统点状评估指标在多模态后验逆问题中存在结构性缺陷，导致边际谱压缩，提出三部分评估协议可改变模型排名和科学结论。


<details>
  <summary>Details</summary>
Motivation: 点状指标（RMSE、MAE）在逆问题中假设低误差即好重建，但多模态后验下该假设失效，产生压缩频谱的偏差，影响下游科学测量。

Method: 提出三部分评估协议：每个事件分布准确性（CRPS）、总体边际准确性（频谱保真度诊断）、不确定性可信度（覆盖校准）。

Result: 在合成基准和粒子物理逆问题中，点状与分布指标下的模型排名逆转，校准可区分CRPS下无法区分的架构。

Conclusion: 评估协议而非模型决定科学结论，点状指标不足以评估逆问题重建质量。
...
### [72] [From Residuals to Reasons: LLM-Guided Mechanism Inference from Tabular Data](https://arxiv.org/abs/2605.22897)
*Mohammad R. Rezaei,Rahul G. Krishnan*

Main category: cs.LG

TL;DR: 提出MARICL框架，利用LLM改进基础模型，通过分析残差生成修正项，在多个科学数据集上提升预测性能，并验证修正项反映真实结构。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习在科学应用中难以同时实现预测与理解，且可解释性方法无法揭示特征交互及迭代优化。

Method: MARICL使用多智能体LLM分析基础模型残差，从高残差样本中假设缺失结构，通过多轮文本梯度优化生成显式修正项。

Result: 在九个基准数据集上一致提升性能；在Cell-Free Protein数据中，冻结修正公式在同一协议下92%+案例有效，跨协议失败，证明机理泛化。

Conclusion: MARICL能有效捕获真实结构而非批次噪声，提供可解释的预测改进，并具有机制泛化能力。
...
### [73] [FIRMA: FIbonacci Ring Model Aggregation for Privacy-preserving Federated Learning](https://arxiv.org/abs/2605.22898)
*Rachid Hedjam*

Main category: cs.LG

TL;DR: 提出FIRMA系列无服务器环形联邦学习协议，通过Fibonacci加权聚合和永久私有分类头，解决结构三元悖论，在标签偏斜等异构场景下优于FedAvg。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习协议无法同时实现无服务器、永久私有头部、环形拓扑和原则性非对称邻居权重，存在单点故障、梯度泄露或中央聚合等问题。

Method: FIRMA包含三个协议：1) FIBFL基础版：无服务器环形聚合，Fibonacci权重邻居混合，永久私有分类头；2) FIBFLP增加精度门控邻居抑制；3) FIBFLPP完整版：加入2-opt环形排列最大化类别多样性、全局环形覆盖K_g=ceil(N/2)轮通信和余弦退火自保留校准。

Result: 在28个配置（4基准×7异构）中，FIBFLPP在12个标签偏斜配置全部超越FedAvg，CIFAR-10上最大优势+20.7个百分点；Dirichlet分布下是17/28配置中准确率最高的无服务器协议。

Conclusion: FIRMA系列协议通过Fibonacci加权和环形拓扑，同时实现无服务器、私有头部和高效聚合，在异构联邦学习中表现优越。
...
### [74] [Transcoders Trace Visual Grounding and Hallucinations in Vision-Language Models](https://arxiv.org/abs/2605.22902)
*Dimitrios Damianos,Leon Voukoutis,Georgios Skyrianos,Vassilis Katsouros,Georgios Paraskevopoulos*

Main category: cs.LG

TL;DR: 提出基于Transcoder的功能中心框架，将VLM分解为可解释的计算路径，并在Gemma 3-4B-IT上验证其优于SAE，且通过图特征预测幻觉达到AUC 0.68。


<details>
  <summary>Details</summary>
Motivation: 现有VLM可解释性工作使用稀疏自编码器分解静态表示，忽略了驱动跨模态交互的功能更新。

Method: 采用基于Transcoder的功能中心框架，作为MLP子层的稀疏近似，将模型分解为连接图像块到标记生成方向的可解释计算路径。

Result: Transcoder属性在视觉接地标记上比SAE产生更强更稳定的效果，且与语义相关图像区域对齐更好；基于图特征的逻辑回归分类器在幻觉预测上达到AUC 0.68。
...
### [75] [Building a privacy-preserving Federated Recommender system for mobile devices](https://arxiv.org/abs/2605.22924)
*Aasheesh Singh*

Main category: cs.LG

TL;DR: 提出一种两阶段联邦推荐系统，将敏感移动数据保留在设备上，仅使用非敏感数据在云端进行协同过滤，再在设备端重排序，兼顾隐私与推荐质量。


<details>
  <summary>Details</summary>
Motivation: 传统推荐系统需要集中存储用户敏感数据，违反隐私法规和用户期望，因此需要隐私保护的移动端个性化推荐方案。

Method: 分两阶段：第一阶段在云端基于非敏感应用上下文数据训练协同过滤模型生成候选列表；第二阶段在设备端利用敏感移动信号（如传感器、位置）进行重排序，仅交换模型更新或梯度。

Result: 在MovieLens、UCI Human Activity Recognition和私有数据集上验证有效，并提供了可部署于Android和iOS的Kotlin Multiplatform库。

Conclusion: 该方法在不泄露敏感数据的前提下实现个性化推荐，具备生产可用性。
...
### [76] [Human-Centered Learning Mechanics: A Dynamical Framework for Entropy-Regulated Representation Learning](https://arxiv.org/abs/2605.22940)
*Kim Phuc Tran*

Main category: cs.LG

TL;DR: 提出人类中心学习力学（HCLM）框架，引入有效熵概念，通过几何熵代理增强信息力，实验表明对数行列式协方差熵优于传统softmax熵。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习理论将训练视为封闭优化系统，无法应对现实AI中的不确定性、资源限制、分布偏移等开放问题，且熵正则化在劣质代理时退化。

Method: 基于动态和信息论，定义有效熵和信息力，研究方差和对数行列式协方差等几何熵代理，推导收敛、熵流和缩放定律条件性解释，并通过受控表示学习实验验证。

Result: 几何熵代理（尤其对数行列式）比softmax归一化熵产生更强更稳定的信息力，避免动力学坍缩；理论得到收敛性和泛化保证。

Conclusion: 几何熵代理能有效提升熵正则化的作用，HCLM框架为开放学习系统提供统一分析视角。
...
TL;DR: 提出MARGIN在线校准方法，无需模型访问或保留数据，在多智能体选择中显著提升置信度校准和准确率。


<details>
  <summary>Details</summary>
Motivation: 标准方法依赖自我报告置信度，但基础模型置信度校准差，尤其在困难任务上置信度与准确性负相关，且设计时校准方法无法应对分布偏移。

Method: MARGIN通过对称指数加权移动平均和贝叶斯收缩混合，在线学习每个智能体每个置信度区间的校准因子，无需模型访问或保留数据。

Result: 在19个模型、8个基准上，分布偏移下校准误差比最佳基线低3-6倍；多智能体选择中将成对分辨率从低于随机（45-56%）提升至70-89%，超过始终最佳模型预言机。

Conclusion: MARGIN能有效在线校准多智能体置信度，提升鲁棒性和选择性能，六项形式命题证明其收敛性和最优性。
...
### [78] [FederatedRSF : Federated Random Survival Forests for Partially Overlapping Medical Data](https://arxiv.org/abs/2605.22954)
*Maryam Moradpour,Jonas Harriehausen,Amirreza Aleyasin,Lion Philipp Wolf,Youngjun Park,Anne-Christin Hauschild*

Main category: cs.LG

TL;DR: 提出FederatedRSF，一种联邦随机生存森林方法，能在不共享原始数据的情况下处理多中心特征空间异质性，实现与集中训练相当的预测性能。


<details>
  <summary>Details</summary>
Motivation: 多中心生存预测可提升鲁棒性和泛化性，但隐私法规和机构治理常阻止患者级临床和基因组数据的汇集，且实际部署中特征空间存在异质性（不同机构收集不同协变量或使用不同测序面板），导致特征集仅部分重叠。

Method: 实现联邦随机生存森林，本地训练生存树，仅将特征兼容的树重新分配给各站点，从而在部分重叠情况下进行推断，无需共享原始数据。

Result: 在GBSG2乳腺癌队列上评估，通过重复交叉验证和站点分割模拟特征异质性，使用Harrell's C-Index衡量，联邦模型性能与集中训练相当。

Conclusion: FederatedRSF能在不共享原始数据且特征部分重叠的条件下，实现与集中训练相媲美的预测性能，解决了多中心生存预测中的数据隐私和特征异质性挑战。
...
### [79] [Certification from Examples is Hard for Circuits and Transformers under Minimal Overparametrization](https://arxiv.org/abs/2605.22964)
*Artur Back de Luca,Kimon Fountoulakis*

Main category: cs.LG

TL;DR: 本文研究神经网络在推理任务上的精确认证，发现过参数化导致认证指数级困难，即使近似认证也有严重局限。


<details>
  <summary>Details</summary>
Motivation: 高平均精度可能掩盖不一致行为，需要精确认证来保证学习假设与目标一致。

Method: 理论分析阈值电路和Log-Precision Transformer的认证复杂度，并研究近似认证的界限；实验构建电路和训练Transformer识别二进制加法。

Result: 深度≥2的阈值电路增加单门导致证书大小指数于输入维度；Transformer有类似结果；近似认证中多项式错误仍需要指数证书，常数相对误差可隐藏指数级错误；实验显示不完美模型可逃避均匀采样的证书候选检测。

Conclusion: 精确认证对过参数化模型极其困难，近似认证也有根本限制，需要新方法。
...
### [80] [Learned Relay Representations for Forward-Thinking Discrete Diffusion Models](https://arxiv.org/abs/2605.22967)
*Benjamin Rozonoyer,Jacopo Minniti,Dhruvesh Patel,Neil Band,Avishek Joey Bose,Tim G. J. Rudner,Andrew McCallum*

Main category: cs.LG

TL;DR: 掩码扩散模型在去噪过程中丢弃了遮蔽位置的内部计算，导致重复计算；本文提出学习继电表示（Relay），通过可微通道在正向传递间传递信息，并采用截断反向传播训练，从而提升性能并降低延迟。


<details>
  <summary>Details</summary>
Motivation: 现有掩码扩散模型在迭代去噪时丢弃了遮蔽位置的有用内部信息，每个步骤需要重新计算，造成计算浪费和延迟。

Method: 提出继电表示（Relay），引入一个可微的每令牌通道，在前后向传播间传递潜在信息，并通过截断的时间反向传播（BPTT）进行训练，同时兼容块扩散和KV缓存。

Result: 在Sudoku规划任务上验证设计，并扩展到最先进的Fast-dLLM v2扩散语言模型，在编程任务上超越标准监督微调，推理延迟降低最多32%。

Conclusion: 通过明确训练扩散语言模型在解码步骤间传递潜在信息，可以进一步优化性能-延迟帕累托前沿。
...
### [81] [A mathematical theory of balancing relational generalization and memorization](https://arxiv.org/abs/2605.22972)
*Luke Cheng,Samuel Lippl*

Main category: cs.LG

TL;DR: 提出带异常的传递推理任务，研究学习系统如何平衡关系泛化与异常记忆，通过核岭回归分析发现泛化依赖于表征几何，并在预训练语言模型中验证了理论预测。


<details>
  <summary>Details</summary>
Motivation: 现有任务范式不足以研究学习系统平衡泛化与记忆异常的能力，因此需要新任务来探究这一关键能力。

Method: 引入带异常的传递推理任务，使用核岭回归进行理论分析，并在预训练语言模型上微调验证。

Result: 模型能平衡泛化与记忆，但泛化成功与否对表征几何敏感，且语言模型出现系统性的理论预测错误。
...
### [82] [Worse than Random: The Importance of a Baseline for Unsupervised Feature Selection](https://arxiv.org/abs/2605.22973)
*Muhammad Rajabinasab,Michael E. Houle,Oussama Chelly,Arthur Zimek*

Main category: cs.LG

TL;DR: 提出使用随机特征选择作为无监督特征选择方法的评估基线，实验发现许多先进方法在性能和效率上不如随机选择。


<details>
  <summary>Details</summary>
Motivation: 现有无监督特征选择方法缺乏评估基线，难以衡量新方法的实际价值。

Method: 将随机特征选择作为基线，与多种最新无监督特征选择方法进行对比实验。

Result: 许多最新方法在性能和效率上被随机特征选择超越。

Conclusion: 开发新方法时应强制使用随机特征选择作为基线，保证有显著改进。
...
### [83] [Test-Time Training Undermines Safety Guardrails](https://arxiv.org/abs/2605.22984)
*Simone Antonelli,Sadegh Akhondzadeh,Aleksandar Bojchevski*

Main category: cs.LG

TL;DR: 测试时训练（TTT）在推理中动态调整模型参数，提升了少样本学习等任务性能，但引入了新的安全漏洞，攻击者可利用三种威胁模型绕过安全过滤器，显著提高攻击成功率（ASR）。


<details>
  <summary>Details</summary>
Motivation: TTT作为一种新兴范式能提升模型性能，但其动态适应性带来了新的安全风险，需要研究其潜在漏洞及防御措施。

Method: 识别了TTT的三种威胁模型，通过LoRA等实验评估攻击成功率，提出基于困惑度偏移的轻量级提供方检测器，并设计了有效感知评估以纠正过拟合导致的ASR膨胀。

Result: TTT使少样本和生成阶段的ASR@10分别达到95%和93%（跨模型族和规模），漏洞可迁移至生产微调API；过拟合导致标准评估下ASR虚高。

Conclusion: TTT暴露了新攻击面，增强了攻击有效性，削弱了现有安全护栏；需动态对齐实现稳健部署，检测器作为第一步但非充分。
...
### [84] [Smoothed Elicitation Complexity for Approximate $\Gamma$-calibration of Discrete Classification Tasks](https://arxiv.org/abs/2605.23017)
*Jessica Finocchiaro,Victor Ganson,Drona Khurana*

Main category: cs.LG

TL;DR: 本文首次为强可排序离散属性提供近似校准结果，通过Lipschitz连续属性作为中介，并刻画了其elicitation complexity。


<details>
  <summary>Details</summary>
Motivation: 多类分类器的校准复杂度随类别数指数增长，现有近似属性校准方法限于连续标量属性，无法处理离散属性（如众数、排序）。

Method: 使用Lipschitz连续属性作为中介，构建算法设计Lipschitz属性，再后处理得到原始离散属性，从而刻画强可排序离散属性的近似校准。

Result: 首次为离散属性提供近似校准结果，并刻画了其Lipschitz elicitation complexity。

Conclusion: 通过中介方法有效降低了离散属性校准的复杂度，为多类分类器校准提供了新途径。
...
### [85] [PACE: Two-Timescale Self-Evolution for Small Language Model Agents](https://arxiv.org/abs/2605.23019)
*Chen Ling,Pei Chen,Albert Guan,Jiaming Qu,Shayan Ali Akbar,Madhu Gopinathan,Erwin Cornejo*

Main category: cs.LG

TL;DR: 提出PACE框架，使冻结的小语言模型通过双时间尺度进化实现有效的自我进化，性能提升显著。


<details>
  <summary>Details</summary>
Motivation: 部署语言模型代理需要大量计算和人力调优；现有自我进化框架依赖前沿模型，不适用于资源受限场景；希望研究冻结小语言模型能否自我进化。

Method: 提出PACE，采用双时间尺度框架：在固定控制逻辑下逐步优化提示直到饱和，然后进行受约束的控制逻辑更新，并通过保留验证接受。

Result: 在4B-14B的3个SLM和4个基准上，PACE在所有12个组合上均最优，相对改进最高达+9.2%和+5.4%；tau-bench案例中多轮工具使用成功率提升。

Conclusion: 冻结SLM无需更新权重或依赖前沿模型即可实现可靠自我进化，关键是通过自主验证发现任务适应的推理策略。
...
### [86] [World Machine: Towards Generative World Modeling for Time-Series](https://arxiv.org/abs/2605.23025)
*Elton Cardoso do Nascimento,Alexandre da Silva Sim\~oes,Esther Luna Colombini,Ricardo Ribeiro Gudwin,Paula Dornhofer Paro Costa*

Main category: cs.LG

TL;DR: 提出World Machine架构，一种基于Transformer和潜在状态的时间序列世界模型，在Toy1D数据集上验证了其超越传统Transformer的能力。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer在处理长上下文时计算和内存成本呈二次增长，需要更高效的世界模型架构。

Method: 设计基于Transformer的潜在状态架构，通过潜在状态适应不同观测数据和上下文。

Result: 在Toy1D合成数据集上实验表明，该架构可行且具备传统Transformer不具备的能力。

Conclusion: World Machine为时间序列世界建模提供了高效且可泛化的新方法。
...
### [87] [RADAR: Relative Angular Divergence Across Representations](https://arxiv.org/abs/2605.23028)
*Xavier Cadet,Mateusz Nowak,Peter Chin*

Main category: cs.LG

TL;DR: 提出RADAR，一种基于几何的跨域迁移性估计指标，通过分析表示轨迹的角对齐和距离变化来预测领域迁移效果。


<details>
  <summary>Details</summary>
Motivation: 机器学习依赖数据，但数据获取困难，且盲目扩充数据可能导致负迁移。

Method: RADAR度量层间表示演化的角对齐和距离变化，比较域内与跨域轨迹分布差异。

Result: 在文本和视觉基准测试中，RADAR预测性能具有竞争力，尤其在领域过渡平滑或清晰分离时。

Conclusion: 迁移性估计有效性取决于模型内部表示空间的几何结构，不同模态偏好不同拓扑结构。
...
### [88] [Uncovering the Latent Potential of Deep Intermediate Representations](https://arxiv.org/abs/2605.23033)
*Arnesh Batra,Arush Gumber,Aniket Khandelwal,Jashn Khemani,Anubha Gupta*

Main category: cs.LG

TL;DR: 提出LOES和GeoReg方法，通过逐层最优嵌入选择和几何正则化，提升深层模型的迁移学习效果，并揭示嵌入几何结构的重要性。


<details>
  <summary>Details</summary>
Motivation: 传统仅使用最终层或浅层混合的方法无法有效利用跨层分布的任务相关信息，且信息在层间非单调分布。

Method: 引入LOES（逐层最优嵌入选择），一种构造性谱方法，通过正交性和各向同性约束最小化残差误差以识别任务判别子空间；提出GeoReg（几何正则化损失），在微调中强制执行类流形的单纯形结构并稳定表示几何。

Result: 在各种架构、深度、模态和数据范围下，LOES持续优于标准基线，且随着模型深度增加收益增长；还实现了跨语言和跨模态的可解释性分析。

Conclusion: 逐层嵌入几何不是偶然，而是深度模型表示和迁移知识的关键。
...
### [89] [Open Multimodal Datasets and Open-Source Software for Data-Driven Modeling of Multiphase Transport and Thermal Systems](https://arxiv.org/abs/2605.23037)
...
TL;DR: 本文介绍了一个由NED3实验室构建的开放生态系统，包含多模态数据集和开源软件，旨在推动可复现的AI增强热流体研究。提出了S+TD维度框架对数据进行分类，并描述了多个配套软件包。


<details>
  <summary>Details</summary>
Motivation: 多相流、电子冷却等领域的进展受限于碎片化数据集和原始仪器文件难以解码、复用和基准测试。

Method: 提出S+TD空间-时间维度框架，将数据集分类为0+0D点值至多模态组合。整理公开数据集（沸腾图像、声学、热测量等），并开发了BubbleID、SeqReg等软件包，支持计算机视觉、序列回归等任务。

Result: 发布了涵盖多种模态的公开数据集和七个开源软件包，其中SeqReg库可处理0+1D、1+1D和2+1D数据，应用于非侵入式热通量估计。

Conclusion: 未来需要社区共同努力构建可互操作的热流体数据库和AI/ML工具库，连接数据集、元数据、解码器、基线、基准和可解释模型。
...
### [90] [Steered Generation via Gradient-Based Optimization on Sparse Query Features](https://arxiv.org/abs/2605.23040)
*Sumanta Bhattacharyya,Pedram Rooshenas*

Main category: cs.LG

TL;DR: 提出基于原型的稀疏操控框架，通过稀疏自编码器分解注意力查询激活，实现可解释的精确控制。


<details>
  <summary>Details</summary>
Motivation: 现有潜伏操控在密集状态干预会纠缠语义特征，需要更清晰的操控性。

Method: 将稀疏自编码器应用于注意力查询激活，分解为可解释特征，推理时用梯度优化对齐类原型。

Result: 在文本化网格世界成功导航规划约束，在教育领域操控认知复杂度，证明框架的通用性和解耦能力。

Conclusion: 稀疏查询表示提供了必要的解耦，实现对逻辑规划和风格细微差别的统一可解释控制。
...
### [91] [Decomposing and Measuring Evaluation Awareness](https://arxiv.org/abs/2605.23055)
*Changling Li,Terry Jingchen Zhang,Jie Zhang,Zhijing Jin,Sahar Abdelnabi,Maksym Andriushchenko*

Main category: cs.LG

TL;DR: 前沿语言模型有时能识别出被评估并调整行为，影响基准有效性。本文从社会心理学角度分解评估意识，提出环境与模型组件，并构建EvalAwareBench来测量和归因。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏共同基础，混淆评估属性与模型属性，以及检测与行为响应。

Method: 将评估意识分解为环境组件（8个触发因素）和模型组件（识别与行为倾向），通过思维链监控识别和行为，并构建EvalAwareBench控制因素。

Result: 识别率取决于模型与基准的配对，而非单独因素；识别很少导致行为改变，方向取决于评估类型；模型对安全评估比能力评估更敏感；叠加因素逐步提高评估意识。

Conclusion: 框架和EvalAwareBench提供了测量、归因和缓解评估意识的工具，指出识别下行为一致性是未来方向。
...
### [92] [ModeSwitch-LLM: A Lightweight Phase-Aware Controller for Cross-Mode LLM Inference on a Single GPU](https://arxiv.org/abs/2605.23057)
*Aman Sunesh,Ali Alshehhi,Hivansh Dhakne*

Main category: cs.LG

TL;DR: ModeSwitch-LLM 通过轻量级请求边界控制器，根据请求选择固定推理模式（如 FP16、量化、推测解码等），在单 GPU 上实现显著加速和节能，且精度接近 FP16。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型推理通常采用静态配置，无法适应不同请求的效率和资源需求，导致单 GPU 利用率不足。

Method: 使用低成本的工作负载特征，在线路由每个请求到预定义的推理模式（FP16、量化、推测解码、混合模式等），规则控制器基于简单规则决策。

Result: 在 Meta-Llama-3.1-8B-Instruct 上，平均延迟比 FP16 快 2.10 倍，单位 token 能耗降低 51.7%，平均准确率比 FP16 高 0.17 个百分点；轻量级学习路由效果不如规则控制器。

Conclusion: 无需重新训练或改变模型架构，简单的请求感知路由即可从现有推理模式中恢复大量效率。
...
TL;DR: 提出SF-NorMuon优化器，在无需调度的情况下匹配或超越调优的AdamW，使无地平线优化更实用。


<details>
  <summary>Details</summary>
Motivation: 现有无调度优化器SF-AdamW持续劣于调优的AdamW，需要缩小性能差距。

Method: 提出SF-NorMuon，一种无调度谱优化器，并证明其平稳性保证，指出快速迭代上的权重衰减对长地平线稳定性至关重要。

Result: 在125M和772M参数语言模型上，以单一超参数配置在1-8倍Chinchilla地平线上匹配或超越调优的AdamW。

Conclusion: SF-NorMuon使无地平线优化更实用，为真正的开放式持续学习迈进一步。
...
### [94] [GEMQ: Global Expert-Level Mixed-Precision Quantization for MoE LLMs](https://arxiv.org/abs/2605.23078)
*Jianing Deng,Song Wang,Dongwei Wang,Zijie Liu,Tianlong Chen,Huanrui Yang,Jingtong Hu*

Main category: cs.LG

TL;DR: 提出全局专家级混合精度量化（GEMQ）方法，通过线性规划分配位宽和路由器微调，实现MoE-LLM的极低比特量化，减少内存并加速推理。


<details>
  <summary>Details</summary>
Motivation: 现有混合精度量化方法基于层级重要性估计，忽略量化引起的路由器偏移，导致分配和路由次优，难以在精度-内存帕累托边界上取得最佳折衷。

Method: 采用全局线性规划公式建模模型级专家重要性（基于量化误差分析），并结合高效路由器微调适应量化后的专家；通过渐进式量化框架迭代优化重要性估计和位宽分配。

Result: 显著减少内存占用并加速推理，同时精度损失极小。

Conclusion: GEMQ有效克服了现有方法的局限性，实现了更优的精度-内存权衡，适用于大规模MoE-LLM的高效部署。
...
### [95] [The Attribution Contract: Feature Attribution for Generative Language Models](https://arxiv.org/abs/2605.23080)
*Giang Nguyen*

Main category: cs.LG

TL;DR: 提出特征归因在生成式语言模型中的概念模糊性，并引入归因契约规范解释，强调方法-契约对评估。


<details>
  <summary>Details</summary>
Motivation: 现有特征归因方法在生成式语言模型中定义不清，导致概念局限性。

Method: 提出归因契约，明确输出、特征、生成过程、固定内容和模型得分。

Result: 案例显示归因于早期token、中间状态或去噪阶段有时有效，有时误导，争议源于未明确契约。

Conclusion: 特征归因应作为方法-契约对评估以解决争议。
...
### [96] [ThriftAttention: Selective Mixed Precision for Long-Context FP4 Attention](https://arxiv.org/abs/2605.23081)
*Joe Sharratt*

Main category: cs.LG

TL;DR: 提出ThriftAttention，通过混合精度（少量FP16+大量FP4）在长上下文中接近FP16质量，达到FP4效率。


<details>
  <summary>Details</summary>
Motivation: 现有块级量化在长上下文中质量下降严重，且量化误差影响不均匀，需要改进。

Method: 两阶段：先启发式选择重要查询-键块用FP16，然后计算选中的FP16和其余FP4，通过在线softmax合并。

Result: 仅5%块用FP16即可恢复89.1%的FP4-to-FP16性能差距，优势随序列长度增长。

Conclusion: 有效缓解长上下文FP4质量退化，代码已开源。
...
### [97] [The Implicit Bias of Depth: From Neural Collapse to Softmax Codes](https://arxiv.org/abs/2605.23087)
*Connall Garrod,Jonathan P. Keating,Christos Thrampoulidis*

Main category: cs.LG

TL;DR: 神经网络中神经坍缩的结构几何在深度架构中可能不是最优的，本研究通过深度无约束特征模型，发现深度会诱导隐式低秩偏置，导致软最大编码，并首次刻画了无正则化交叉熵训练下的渐近和动力学特性。


<details>
  <summary>Details</summary>
Motivation: 先前理论认为L2正则化导致深度架构中神经坍缩次优，但未明确梯度下降和深度本身的作用。本研究旨在分离出深度和梯度下降对神经坍缩的单独影响。

Method: 使用深度无约束特征模型（UFM），等价于无正则化、正交输入的深度线性网络，通过谱初始化分析训练动力学，研究深度对奇异值演化的影响。

Result: 深度诱导隐式低秩偏置：低秩矩阵在连续乘法中更有效地传递范数，促进低秩软最大编码；早期奇异值排斥驱动低秩涌现；深度缩小神经坍缩的吸引域；宽度增加则偏向高秩解。

Conclusion: 首次提供了深度UFM在无正则化多分类交叉熵训练下隐式偏置的渐近和动力学特征，揭示了深度本身通过隐式低秩偏置影响表示几何。
...
### [98] [Dreaming Smoothly and Sample Efficiently with Gradient Penalized Latent Dynamics](https://arxiv.org/abs/2605.23089)
*Romil V. Sonigra (1),P. R. Kumar (1) ((1) Texas A&M University)*

Main category: cs.LG

TL;DR: 提出GPLD正则化器，对DreamerV3的潜在动力学施加行方向雅可比惩罚以强制局部平滑，提升样本效率。


<details>
  <summary>Details</summary>
Motivation: 现有潜在世界模型如DreamerV3未利用局部平滑这一有用的归纳偏置。

Method: 通过Hutchinson随机探针高效估计潜在分布的雅可比惩罚，作为连续潜在空间的有限差分平滑模拟。

Result: 在DeepMind Control任务上，GPLD显著提升样本效率，尤其在复杂运动任务中，并实现更早的高回报和更稳定的后期学习。

Conclusion: 显式局部平滑正则化是改进潜在世界模型的简单有效方法。
...
### [99] [Robust OT-Guided Generative Residual Domain Adaptation for Bike-Sharing Demand Prediction under Temporal Domain Shift](https://arxiv.org/abs/2605.23115)
*Yiming Ma*

Main category: cs.LG

TL;DR: 提出Gen-ROTDA框架，利用鲁棒最优传输和残差域适应解决共享单车需求预测中的时间漂移问题，在多年任务中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 基于历史数据的共享单车模型因出行模式随时间变化而性能下降，需要有效的域适应方法。

Method: 用小标注目标子集拟合锚点，传输残差而非原始需求，使用确定性标签保持残差特征生成器，并修剪高成本传输匹配。

Result: 在2025-2026主要任务上MAE最低，多年任务中OT族方法最佳，且对异常记录鲁棒。

Conclusion: 鲁棒传输对噪声时间转移有效，但微调和MMD仍是强基线。
...
### [100] [When Determinants Are Not Enough: Private Rare Switching](https://arxiv.org/abs/2605.23131)
*Xingyu Zhou*

Main category: cs.LG

TL;DR: 本文记录了利用Codex将稀有切换规则适应于隐私设置的过程，提出基于广义瑞利商的切换规则，解决了噪声导致的分析失效问题。


<details>
  <summary>Details</summary>
Motivation: 标准行列式更新规则在加入隐私噪声后失去单调性，导致遗憾分析失败，需要新的切换规则。

Method: Codex提出基于广义瑞利商的稀有切换规则，替代标准行列式规则。

Result: 新规则恢复了对数策略更新次数和置信宽度比较（常数因子内）。

Conclusion: 证明并反思了Codex辅助的研究过程。
...
### [101] [Archimedean Copula Inference via Taylor-Mode AD](https://arxiv.org/abs/2605.23134)
*Cambridge Yang,Dongdong Li*

Main category: cs.LG

TL;DR: 提出JAX原生的AcoPula框架，支持任意删失、任意嵌套树和精确梯度，显著提升效率与扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有嵌套阿基米德连接函数工具无法同时处理任意变量的右删失、任意嵌套树和精确参数梯度，局限于低维、两层嵌套或手工推导。

Method: 基于泰勒模式自动微分输出的多项式幂运算，替代传统手工推导的部分贝尔多项式表，实现任意生成器驱动下的多项式时间精确似然和梯度计算。

Result: 在MIMIC-IV（d=53）、S&P 500（d=98）、视网膜病变研究等任务上验证有效，速度比R的nacLL快约650倍（d=35），可扩展到d=8000。

Conclusion: AcoPula克服了现有方法的三大局限，为高维删失数据提供灵活高效的依赖建模工具。
...
TL;DR: CALAD是一种通道感知的对比学习框架，通过估计通道相关性来构建对比样本，提升多变量时间序列异常检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常平等对待所有通道，但这会稀释异常相关信号，因为并非所有通道对异常检测贡献相同。

Method: 利用基于Transformer的自编码器重构误差估计通道相关性，并基于此设计通道级增强策略：保留或扰动异常相关通道以构建正负样本，同时结合对比学习和辅助重构头。

Result: 在多个真实数据集上，CALAD持续优于现有方法，尤其在分布偏移场景下表现突出。

Conclusion: CALAD有效利用通道相关性学习判别性表示，同时保留正常结构，为异常检测提供新思路。
...
### [103] [Infra-Bayesian Reinforcement Learning Agents Outperform Classical RL For Worst-Case Robustness](https://arxiv.org/abs/2605.23146)
*Manish Aryal,Faiyaz Azam,Agnivo Banerjee,Sai Sidhanth Manoharan Jayanthi,Allegra Laro,Cl\'ement Legentilhomme,Andrew Lin,Florian Lorkowski,Radman Rakhshandehroo,Patric Rommel,Emanuel Ruzak,Nathan Theng,Paul Yushin Rapoport*

Main category: cs.LG

TL;DR: 提出一种基于Infra-Bayesian的强化学习架构，通过最坏情况期望最大化处理模型误设和策略依赖的不确定性，在有限无状态决策问题中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 经典强化学习假设环境固定且与策略无关，但在AI安全等非可实化场景中失效，导致错误后验和无限遗憾。

Method: 采用Infra-Bayesian决策框架，维护一组不精确假设，使用Infra-Bayesian条件更新，通过最大化最坏情况期望选择动作。

Result: 在具有奈特不确定性的环境中，最坏情况遗憾低于经典强化学习代理；在纽科姆问题中选择最优策略，优于经典决策理论代理。

Conclusion: 该工作为实现对模型误设和策略依赖不确定性鲁棒的强化学习代理迈出了第一步。
...
### [104] [Any-Dimensional Invariant Universality](https://arxiv.org/abs/2605.23156)
*Shengtai Yao,Eitan Levin,Mateo D\'iaz*

Main category: cs.LG

TL;DR: 提出一种系统方法，通过将任意维度函数映射到无限维极限空间，建立其通用性，并展示现有架构的不足及改进。


<details>
  <summary>Details</summary>
Motivation: 传统通用性研究限于固定大小输入，而任意维度模型（如图、点云）的通用性缺乏理论框架。

Method: 将任意维度函数与无限维极限空间中的唯一函数对应，利用输入对称性和尺寸关系，定义拓扑和紧集族以建立通用性。

Result: 证明现有多种架构非通用，提出简单修改可恢复通用性。

Conclusion: 开发了任意维度通用性的系统理论，为模型设计提供指导。
...
### [105] [Understanding and Improving Noisy Embedding Techniques in Instruction Finetuning](https://arxiv.org/abs/2605.23171)
*Abhay Yadav*

Main category: cs.LG

TL;DR: 提出SymNoise方法，使用对称噪声在指令微调中提升语言模型性能，超越NEFTune。


<details>
  <summary>Details</summary>
Motivation: NEFTune使用均匀噪声效果好但原因不明，本文旨在分析噪声类型并探索更优的噪声策略。

Method: 在嵌入中加入对称噪声，严格调控局部曲率，提出SymNoise微调方法。

Result: LLaMA-2-7B在AlpacaEval上从29.79%提升至69.04%，比NEFTune高6.7%，并在多种模型和数据集上持续优于NEFTune。

Conclusion: SymNoise显著优于现有最先进方法，推动了噪声策略在语言模型微调中的应用研究。
...
### [106] [Pure Exploration for a Good Policy in Reinforcement Learning with Bandit Feedback](https://arxiv.org/abs/2605.23182)
*Zitian Li,Wang Chi Cheung*

Main category: cs.LG

TL;DR: 提出好的策略识别（GPI）问题，并设计BEE-GPI算法，其样本复杂度与状态动作空间大小无关，仅依赖于H^2/(V*-μ0)^2。


<details>
  <summary>Details</summary>
Motivation: 实际应用中“足够好”的策略即可，无需最优策略，以降低样本复杂度。

Method: 基于固定置信度设置，提出BEE-GPI算法，通过最小化期望样本复杂度来识别满足阈值μ0的策略或声明不存在。

Result: 正实例样本复杂度上界系数为O(H^2/(V*-μ0)^2)，与状态动作空间大小无关；下界证明算法接近最优。

Conclusion: BEE-GPI算法高效，能够以接近最优的样本复杂度识别好的策略，实验验证其有效性。
...
### [107] [Empirical Bayes Conformal Prediction for Vision and Language Models](https://arxiv.org/abs/2605.23189)
*Jiapeng Zeng,Yogesh Prabhu,Zhanpeng Zeng,Michael A. Newton,Vikas Singh*

Main category: cs.LG

TL;DR: 提出一种经验贝叶斯共形预测框架，使用r值将分数变异性转化为不确定性感知的非一致性分数，在保持覆盖的同时减少高方差假候选。


<details>
  <summary>Details</summary>
Motivation: 标准共形预测忽略了分数变异性，导致基于单一不稳定分数的弱答案可能进入共形集。

Method: 定义r值作为非一致性分数，提供封闭形式正态-正态经验贝叶斯估计和非参数后验采样估计。

Result: 在图像分类、CLIP-based VLM和LLM上，r值共形预测保持目标覆盖，提高排序稳定性，减小集大小。

Conclusion: r值共形预测在变异性信息时改善性能，变异性消失时退化为标准CP行为。
...
### [108] [Expand More, Shrink Less: Shaping Effective-Rank Dynamics for Dense Scaling in Recommendation](https://arxiv.org/abs/2605.23191)
*Guoming Li,Shangyu Zhang,Junwei Pan,Wentao Ning,Jin Chen,Gengsheng Xue,Chao Zhou,Shudong Huang,Haijie Gu,Menglin Yang*

Main category: cs.LG

TL;DR: 针对推荐模型扩展中的嵌入坍缩问题，提出RankElastor架构，通过参数化全混合和GLU改进P-FFN缓解坍缩，提升性能并实现稳健扩展。


<details>
  <summary>Details</summary>
Motivation: RankMixer模型因嵌入坍缩导致表示空间利用率低，限制扩展性能。

Method: 引入参数化全混合模块增强光谱鲁棒性，以及GLU改进的P-FFN模块稳定表示光谱。

Result: 大规模工业数据集上，RankElastor持续提升推荐性能，缓解嵌入坍缩，表现出稳健的扩展行为。

Conclusion: RankElastor有效解决了嵌入坍缩问题，为推荐模型扩展提供了更优方案。
...
### [109] [Scalable Heterogeneous Graph Foundation Models for Data-Driven Optimal Power Flow in Smart Grids](https://arxiv.org/abs/2605.23194)
*Massimiliano Lupo Pasini,Yijiang Li,Kibaek Kim,Teja Kuruganti*

Main category: cs.LG

TL;DR: 提出基于HydraGNN的可扩展异构图神经网络工作流，用于数据驱动的最优潮流（OPF）代理建模和基础模型开发，在超算上训练并验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有学习型OPF近似方法忽略了电网异构结构、拓扑局限，且缺乏图基础模型训练的可扩展基础设施。

Method: 构建保留电网异构图类型（母线、发电机等）的GNN工作流，支持分布式预处理、训练、超参数优化（HPO）和下游微调，使用三百万实例在Frontier超算上优化。

Result: HPO识别出紧凑模型（约1.6-1.7M参数）验证损失最低；下游微调提升低数据精度、稳定训练并加速收敛。

Conclusion: 该工作流可有效利用大规模异构图数据训练OPF基础模型，通过微调适应不同任务，具有可扩展性和实用性。
...
TL;DR: SemiPrune通过半监督学习生成伪标签，利用训练动态估计样本难度，实现标签高效的数据集剪枝。


<details>
  <summary>Details</summary>
Motivation: 现有剪枝方法通常需要全标注数据，或依赖预训练模型特征，当目标数据集与预训练分布差异大时效果不佳。

Method: 使用少量随机标注的子集，通过半监督学习为未标注数据生成伪标签，然后利用伪标签诱导的训练动态估计样本难度，选择核心子集。

Result: 在域特定、图像损坏和长尾数据集上达到最先进性能，在标准基准上具有竞争力。

Conclusion: SemiPrune通过直接学习目标分布，提供了更可靠的样本难度估计，适用于标注稀缺的实际场景。
...
### [111] [Adaptive Mass-Segmented KV Compression for Long-Context Reasoning](https://arxiv.org/abs/2605.23200)
*Junzhe Yang,Xiaoyu Shen*

Main category: cs.LG

TL;DR: 提出自适应质量分段（AMS）KV压缩方法，通过区域感知配额分配替代全局Top-k选择，解决长上下文推理中KV缓存压缩导致的结构碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 现有KV压缩方法采用全局Top-k选择，导致连续推理块被过度驱逐（区域擦除），破坏逻辑连贯性。

Method: 基于注意力质量空间分布自适应划分KV缓存，为关键推理段分配保证内存配额，并引入EMA平滑机制稳定解码过程。

Result: 在数学推理、代码补全、开放域问答和稀疏检索等任务中，AMS显著减轻结构碎片化并提升模型性能，且兼容现有评分器和vLLM框架。

Conclusion: AMS是一种通用的即插即用层，能有效缓解KV压缩中的区域擦除问题，提高长文本推理的连贯性与效率。
...
### [112] [FastKernels: Benchmarking GPU Kernel Generation in Production](https://arxiv.org/abs/2605.23215)
*Gabriele Oliaro,Yichao Fu,May Jiang,Owen Lu,Junli Wang,Zhihao Jia,Hao Zhang,Samyam Rajbhandari*

Main category: cs.LG

TL;DR: FastKernels是一个更贴近生产环境的GPU内核基准测试和推理框架，揭示了现有基准测试与生产环境的脱节。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试与生产推理框架严重不匹配，导致代理学习到的优化无法直接应用于真实系统。

Method: 构建包含46个代表性架构的FastKernels基准测试，并作为生产级推理框架，与主流库接口一致。

Result: 最强代理仅实现0.94x加速，较弱代理更低，证实了基准测试与生产的脱节是瓶颈。

Conclusion: 发布FastKernels作为桥梁，使基准测试收益直接转化为实际吞吐量提升。
...
### [113] [PaP-NF: Probabilistic Long-Term Time Series Forecasting via Prefix-as-Prompt Reprogramming and Normalizing Flows](https://arxiv.org/abs/2605.23219)
*Minju Kim,Youngbum Hur*

Main category: cs.LG

TL;DR: 提出PaP-NF框架，利用前缀提示与冻结大语言模型对齐连续时间序列表示，并通过归一化流解码器进行概率预测，在长时预测基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的时间序列预测具有内在不确定性，确定性点预测不足，需要能够量化不确定性的概率预测方法。

Method: 使用Prefix-as-Prompt机制将连续时间序列表示与冻结的大语言模型对齐，并基于大语言模型提取的全局上下文条件化归一化流解码器。

Result: 在多个长时预测基准上，PaP-NF稳健地捕捉多模态不确定性，同时保持有竞争力的点预测准确性，使用CRPS评估。

Conclusion: PaP-NF是一种有效的概率预测框架，能够处理时间序列中的不确定性，并在长期预测中表现出色。
...
### [114] [WMAttack: Automated Attack Search for Adversarial Evaluation of World-Model Agents](https://arxiv.org/abs/2605.23220)
*Zhixiang Guo,Siyuan Liang,Shi Fu,Cheng Guo,Andras Balogh,Mark Jelasity,Dacheng Tao*

Main category: cs.LG

TL;DR: 提出WMAttack框架，实现世界模型对抗鲁棒性的自动化攻击搜索，兼顾准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有对抗评估方法缺乏自动化，手动调优弱攻击高估鲁棒性，穷举超参数搜索因需闭环展开而代价高昂。

Method: WMAttack将鲁棒性评估转化为有限预算攻击配置搜索；SCAS利用奖励下降、动作不稳定等反馈修正攻击分布；RGAR从表示相似任务中检索有效历史配置实现热启动。

Result: 在Atari和DMC任务上，WMAttack始终发现更强攻击：DreamerV3 Atari归一化奖励下降从0.497提升至1.034，DMC从0.319提升至0.682。

Conclusion: WMAttack能更有效发现强攻击，消融实验验证了RGAR提升初始候选质量、SCAS提升最终攻击效用。
...
### [115] [Assessing Predictive Models for Fairness Based on Movement Patterns](https://arxiv.org/abs/2605.23234)
*Francesco Lettich,Mario A. Nascimento,Chiara Pugliese,Chiara Renso*

Main category: cs.LG

TL;DR: 提出基于移动模式的公平性评估方法，使用多分辨率地理分区和空间扫描统计量检测模型不公平。


<details>
  <summary>Details</summary>
Motivation: 现有空间公平性只考虑单一位置（如居住地），忽略了移动模式，需要扩展至多位置轨迹。

Method: 将个体移动轨迹关联到多分辨率、多对齐的地理分区，采用空间扫描统计量评估模型公平性。

Result: 在数千个合成数据集上有效检测不公平并识别受不公平对象，定位性能存在多分辨率权衡。

Conclusion: 基于移动模式的公平性评估可行且有效，为空间公平性研究提供了新方向。
...
### [116] [Convex Low-resource Accent-Robust Language Detection in Speech Recognition](https://arxiv.org/abs/2605.23235)
*Miria Feng,William Tan,Mert Pilanci*

Main category: cs.LG

TL;DR: 提出凸语言检测（CLD）框架，利用凸优化和ADMM算法在JAX上高效实现，提升低资源方言的语音识别鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前口语对话系统在低资源方言和口音上常失败，误识别输入语言导致下游任务级联错误，且标准微调昂贵易过拟合。

Method: CLD框架集成凸优化技术，通过多GPU的ADMM算法在JAX中实现，提供全局最优性保证和多项式时间训练。理论上证明凸目标诱导边际稳定性保证。

Result: 在低资源场景下达到97-98%准确率，样本高效且对输入方言变化鲁棒。

Conclusion: CLD有效解决方言差异问题，开源包可用，为对话系统处理多样化语音提供新方案。
...
### [117] [Self-supervised Adversarial Purification for Graph Neural Networks](https://arxiv.org/abs/2605.23239)
*Woohyun Lee,Hogun Park*

Main category: cs.LG

TL;DR: 提出GPR-GAE，一种基于广义PageRank滤波器的自监督图自编码器，用于对抗净化，实现GNN分类器的鲁棒防御。


<details>
  <summary>Details</summary>
Motivation: 传统对抗训练在分类器中混合了准确性与鲁棒性的冲突目标，难以平衡，需要一种分离的净化机制。

Method: 设计GPR-GAE作为独立的净化器，采用自监督训练，利用多重广义PageRank滤波器捕获多样结构表示，并通过多步净化过程精确恢复图结构。

Result: 在多个数据集和攻击场景下，GPR-GAE作为即插即用净化器取得了最先进的鲁棒性。

Conclusion: GPR-GAE有效分离了鲁棒性与分类任务，提升了防御性能，可作为通用模块集成到现有GNN中。
...
### [118] [RelPrism: A Multi-Faceted Pre-training Framework with Self-Generated Tasks for Relational Databases](https://arxiv.org/abs/2605.23241)
*Jinyu Yang,Cheng Yang,Junze Chen,Zedi Liu,Muhan Zhang,Hanyang Peng,Chuan Shi*

Main category: cs.LG

TL;DR: 提出RelPrism，一种面向关系数据库的多方面自监督学习框架，通过构建内在、关系和混合属性并应用多粒度聚类形成伪任务池进行预训练，在分类和回归任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有自监督方法仅从单一层面（如节点属性或子图结构）获得监督信号，但关系数据库任务需要多角度、多粒度的综合信息，导致适应性有限。

Method: RelPrism从不同视角构建内在、关系和混合属性，对每个视角进行多粒度聚类形成对应的伪任务池，通过对这些池进行预训练，使表示暴露于更广泛的视角和粒度级别。

Result: 在5个真实数据集的14个任务上，RelPrism在分类任务中将ROC-AUC提高4.15%，在回归任务中将MAE降低10.75%。

Conclusion: 多方面的预训练为下游任务提供了更强的表示基础，有效提升了关系数据库上的预测性能。
...
### [119] [Convex Optimization for Alignment and Preference Learning on a Single GPU](https://arxiv.org/abs/2605.23244)
*Miria Feng,Mert Pilanci*

Main category: cs.LG

TL;DR: 提出COALA算法，利用凸优化高效进行LLM偏好微调，无需参考模型，大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有方法如RLHF计算复杂，DPO存在排名不准确、资源消耗高、超参数调优昂贵等问题。

Method: 采用神经网络凸优化重新表述，消除参考模型需求，减少训练时间和显存消耗，支持单GPU高效训练。

Result: 在四个数据集（含Educational Feedback）和六个模型（含Llama-3.1-8B）上，COALA仅使用DPO约17.6%的TFLOPs即达到竞争性能，奖励稳定单调增长且更快达到峰值。

Conclusion: 首次将凸优化成功应用于LLM偏好微调，提供轻量级理论保证方案。
...
### [120] [Accelerating Divisible Load Processing Through Machine Learning: A Practical Framework for Large-Scale Workloads](https://arxiv.org/abs/2605.23247)
*Bharadwaj Veeravalli*

Main category: cs.LG

TL;DR: 提出了首个基于机器学习（前馈神经网络）的框架，用于预测可分割负载理论中的最优处理时间，无需显式方程，准确率高且速度快。


<details>
  <summary>Details</summary>
Motivation: 传统可分割负载理论计算最优处理时间需要显式求解方程，计算耗时，难以满足实时调度等应用需求。

Method: 使用前馈神经网络，输入16个工程特征，在10万个合成配置上训练，预测最优处理时间。

Result: 模型R平方因子97-99%，平均绝对百分比误差1-5%，推理时间<1毫秒，比传统方法快10-100倍，在多样化配置下泛化良好。

Conclusion: 证明了机器学习可以有效替代传统可分割负载理论计算，加速分布式计算优化，同时保持近最优精度。
...
### [121] [Enhancing Deep Neural Network Reliability with Refinement and Calibration](https://arxiv.org/abs/2605.23249)
*Ramya Hebbalaguppe,Ajay Shastry,Soumya Suvra Ghosal,Chetan Arora*

Main category: cs.LG

TL;DR: 提出RefCal框架，通过联合优化校准、锐度（refinement）和准确性，解决了现有校准方法牺牲锐度的问题，显著提升了深度神经网络的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有校准方法常以牺牲锐度为代价改善校准，导致模型对正确和错误预测的置信度区分不足，影响可信度。

Method: 提出显式提升锐度的新损失函数（基于监督对比学习），并构建统一训练框架RefCal，联合优化校准、锐度和准确性。

Result: 在CIFAR-100-LT数据集上，RefCal的准确率、锐度和ECE分别为(58.81, 95.67, 0.08)，显著优于Correctness Ranking Loss的(46.27, 93.7, 0.22)。

Conclusion: RefCal能同时改善校准和锐度，避免两者间的权衡，有效提升DNN的可靠性。
...
### [122] [Learning-Augmented Online Scheduling with Parsimonious Preemption](https://arxiv.org/abs/2605.23255)
*Mugen Blue,Sungjin Im,Alexander Lindermayr*

Main category: cs.LG

TL;DR: 本文首次系统研究限制抢占的学习增强调度，在优化延迟的同时控制抢占次数，实现了理论保证与实验验证。


<details>
  <summary>Details</summary>
Motivation: 现有学习增强调度主要优化作业延迟，但依赖频繁“盲目”抢占，忽视了算法性能与抢占复杂度之间的权衡。

Method: 提出具有O(1)竞争比的算法，在单机和无关并行机上每个作业仅允许O(1)次抢占（预测准确时），且开销随预测误差对数增长。

Result: 在预测准确时达到O(1)竞争比和O(1)次抢占；为无关机和可塑机提供了首个有限抢占保证。

Conclusion: 学习增强框架可以扩展到更受限制和现实的调度场景，在性能与抢占之间实现平衡。
...
### [123] [A Simple Plug-in for Improving Eviction-Based KV Cache Compression](https://arxiv.org/abs/2605.23258)
*Yuping Lin,Jiayuan Ding,Yue Xing,Pengfei He,Jiliang Tang,Subhabrata Mukherjee*

Main category: cs.LG

TL;DR: VECTOR通过三路路由（保留、近似、驱逐）结合重要性和可重建性信号，改进了KV缓存压缩的质量-内存权衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常采用二值驱逐或表示近似，可能未充分利用可重建但非关键的令牌，导致长上下文推理中KV缓存成为瓶颈。

Method: VECTOR是一种即插即用增强方法，在基于驱逐的流程中引入三路令牌路由：保留、近似和驱逐；结合来自基础评分器的重要性信号和来自离线校准的基于回归的值估计的可重建性信号。

Result: 在中高压缩率下，VECTOR改善了质量-内存权衡，尤其在更严格的预算下表现出明显增益。

Conclusion: VECTOR通过利用可重建性恢复有价值的信息，同时保留关键向量以保持注意力路由稳定性，优于二值驱逐方法。
...
### [124] [Multi-Gate Residuals](https://arxiv.org/abs/2605.23259)
*Zhizhan Zheng,Feiyun Zhang,Shuchun Liu,Tian Xia,Xi Liu,Dasheng Hu,Hongquan Zhou*

Main category: cs.LG

TL;DR: MGR通过多门残差机制稳定激活尺度，无需额外通信开销。


<details>
  <summary>Details</summary>
Motivation: Attention Residuals虽然有效但通信开销大，需要避免瓶颈。

Method: 使用评分和门控机制保持多流上下文，结合Attention Pooling提取隐藏状态。

Result: MGR适用于大规模训练和部署，性能优于现有架构。

Conclusion: MGR是一种实用且性能提升的方法。
...
### [125] [When Good Equations Get Bad Scores: Improving Symbolic Regression Through Better Parameter Optimization](https://arxiv.org/abs/2605.23272)
*Boxiao Wang,Kai Li,Zhiwei Chen,Yang Huang,Runxiang Wang,Ziwen Zhang,Yifan Zhang,Jian Cheng*

Main category: cs.LG

TL;DR: 针对符号回归中内环参数拟合易陷入局部最优导致“好结构差分数”的问题，提出SAGE-Fit框架，利用结构语义先验，提升了评估准确性和系统性能。


<details>
  <summary>Details</summary>
Motivation: 符号回归中内环参数拟合的非凸性导致局部最优，使正确结构得分被低估，误导外环搜索。

Method: 提出SAGE-Fit，利用符号表达式的结构先验和语义先验，设计针对性模块解决优化瓶颈。

Result: 作为即插即用模块，显著提升评估保真度，并广泛改进多种符号回归系统的性能。

Conclusion: SAGE-Fit有效解决内环优化瓶颈，提升符号回归的搜索效率和正确性。
...
### [126] [Diffusion Domain Expansion: Learning to Coordinate Pre-trained Diffusion Models](https://arxiv.org/abs/2605.23275)
*Egor Lifar,Semyon Savkin,Timur Garipov,Shangyuan Tong,Tommi Jaakkola*

Main category: cs.LG

TL;DR: 提出扩散域扩展（DDE）方法，通过可训练的小型协调网络扩展预训练扩散模型，使其能生成更大对象和处理更复杂条件，在长时间音频生成和条件图像生成上优于其他方法。


<details>
  <summary>Details</summary>
Motivation: 预训练扩散模型在生成大尺度对象和复杂条件时能力有限，需要高效扩展方法。

Method: 使用紧凑的可训练网络作为协调器，整合预训练扩散模型的去噪输出。

Result: DDE在长时间音频生成和条件图像生成任务上，定性和定量评估均优于其他协调生成方法。

Conclusion: DDE能有效扩展扩散模型至更大域，泛化能力强，适用于多种领域。
...
### [127] [Reinforcement Learning for Microcanonical Graph Ensemble with Assortativity Constraints](https://arxiv.org/abs/2605.23285)
*Hoyun Choi,Junghyo Jo,Deok-Sun Lee*

Main category: cs.LG

TL;DR: 提出深度微正则图生成器（DMGG），基于强化学习通过保持度序列的重新连接精确达到指定同配性，比ERGM快至少一个数量级。


<details>
  <summary>Details</summary>
Motivation: 网络结构决定功能，需要精确可控的图集合。正则方法（ERGM）仅期望约束，微正则方法难以实际采样。

Method: DMGG使用强化学习框架，通过度保持重连变换图，精确达到指定同配性，最大化联合度矩阵变化，避免穷举调参。

Result: 生成速度至少快一个数量级，保持配置多样性，可推广到不同图规模和稀疏度，提供精确零模型。

Conclusion: 强化学习成为生成硬约束图的有效范式，有助于研究结构-功能关系，避免系综伪影。
...
### [128] [Contrastive Distribution Matching for Amortized Sequential Monte Carlo in Discrete Diffusion](https://arxiv.org/abs/2605.23346)
*Jaihoon Kim,Taehoon Yoon,Prin Phunyaphibarn,Seungjun Kim,Morteza Mardani,Minhyuk Sung*

Main category: cs.LG

TL;DR: 提出对比分布匹配（CDM）框架，通过学习参数化的扭曲函数实现奖励倾斜分布的高效采样，仅增加不到5%的计算开销，并在多个任务上超越现有基线。


<details>
  <summary>Details</summary>
Motivation: 基于扭曲序贯蒙特卡洛（Twisted SMC）的方法渐近精确，但在离散状态空间中估计最优扭曲函数需要昂贵的蒙特卡洛近似，导致推理阶段计算瓶颈。

Method: 通过正负样本对比学习参数化的扭曲函数，利用离散扩散模型闭合形式的前向核重新表述梯度估计器，实现高效训练。

Result: 学习到的扭曲函数在推理时相比基模型单次前向传播仅增加不到5%的计算开销；在匹配的壁钟时间下，CDM持续优于现有基线。
...
TL;DR: 提出Prudent-Banker算法，在对抗性延迟反馈下实现安全与鲁棒性的最优权衡。


<details>
  <summary>Details</summary>
Motivation: 现有方法在即时反馈下能平衡安全与鲁棒性，但任意延迟会破坏安全保证。

Method: 结合延迟适应的在线镜像下降和修改的分阶段攻击机制，使用延迟校准的重启阈值。

Result: 实现了伪遗憾O(√T+√D)和对安全比较器的O(1)遗憾，达到理论下界。

Conclusion: Prudent-Banker是首个在安全约束下达到最优权衡的算法，实验验证有效。
...
TL;DR: 提出SOM算法，通过单步生成解决扩散策略在在线RL中的推理开销问题，实现高效运动控制。


<details>
  <summary>Details</summary>
Motivation: 扩散和流匹配策略的多步去噪推理开销大，MeanFlow虽可单步但需目标分布样本，不适用于在线强化学习。

Method: 构建基于分数估计和概率流ODE的演员-评论家算法，直接从Q函数导出目标速度场，实现单步策略优化。

Result: 在完全在线运动控制任务上达到最先进性能，仅需单步生成，且训练和推理时间显著减少。

Conclusion: SOM算法有效解决了扩散策略的计算瓶颈，适合实时在线RL场景。
...
TL;DR: 提出基于可测量任务表示学习的自动课程生成方法，通过变分自编码器学习任务表示，解决非欧几里得任务空间中的课程生成问题，在导航任务中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 插值型课程强化学习在非欧几里得任务空间中无效，需要新的自动课程生成方法。

Method: 采用变分自编码器结构，编码奖励和状态转移，学习具有相似性测量属性的潜在任务表示；基于此表示，自动生成越来越接近目标任务的课程。

Result: 在多种挑战性导航任务中，该方法超越了基于插值和生成对抗网络的最新CRL方法。

Conclusion: 提出的方法有效解决了非欧几里得任务空间中的自动课程生成问题。
...
TL;DR: 通过块对角高斯-牛顿预条件，使PINN在强耦合多物理场系统中保持精度稳定，耦合敏感度降低至1.1倍以内。


<details>
  <summary>Details</summary>
Motivation: PINN在强耦合系统中精度严重退化，缺乏理论解释和稳健优化方法。

Method: 利用NTK分析证明耦合强度导致标准NTK谱半径增长，提出块对角GN预条件使预条件NTK谱半径恒等于网络数S；结合SOAP优化器和逆梯度范数损失平衡（SOAP+GN）。

Result: 数值实验验证了谱半径增长理论，SOAP+GN在强耦合下L2退化比≤1.1倍（耦合参数变化1-2个数量级），而Adam+GN退化>100倍；在2D六PDE电渗流系统中SOAP+GN成功（Adam+GN失败，L2>0.9）。

Conclusion: SOAP+GN实现了对耦合强度的鲁棒性，可扩展到复杂多物理场系统，解决了PINN在强耦合场景下的失效问题。
...
TL;DR: 提出Unpack方法，利用注意力和MLP的键值模板结构，通过后向递归分解信用，无需干预或梯度即可生成组件间交互强度和逐token归因。


<details>
  <summary>Details</summary>
Motivation: 机械可解释性需要识别组件如何组合成计算路径，但现有方法依赖干预或辅助训练。

Method: 基于共享键值模板φ(S)U，设计后向递归Unpack，分解通过注意力和MLP的信用，生成带K/Q/V标签的端到端路径和逐token归因。

Result: 在GPT-2 small上恢复Wang等（2023）描述的所有三种组合连接，并发现重复名字位置被抑制，而首次提及保留信用；该模式在Pythia系列（160M-6.9B参数）中一致复现。

Conclusion: Unpack能在无真实电路标签情况下追踪机械结构，代码已开源。
...
### [134] [Convex Compositional Reasoning Models](https://arxiv.org/abs/2605.23395)
*Meir Roketlishvili,Semyon Semenov,Maksim Bobrin,Viktor Kovalchuk,Albert Baichorov,Abduragim Shtanchaev,Fakhri Karray,Dmitry V. Dylov,Martin Tak\'a\v{c},Arip Asadulaev*

Main category: cs.LG

TL;DR: 提出凸组合能量最小化框架，通过凸松弛和两阶段训练解决组合推理中的非凸能量瓶颈，实现从小子问题到大型实例的泛化。


<details>
  <summary>Details</summary>
Motivation: 发现组合能量模型中非凸的能量景观是主要瓶颈，而非组合操作本身。

Method: 用输入凸神经网络参数化每个因子，在可行集的紧凸松弛上优化组合能量（保持凸性），采用因子级对比学习和端到端展开投影求解器两阶段训练。

Result: 在小子问题或单一问题大小上训练的模型无需重新训练即可泛化到更大实例。

Conclusion: CCEM有效克服了非凸几何的困难，实现了组合推理的规模泛化。
...
### [135] [Parametric Prior Mapping Framework for Non-stationary Probabilistic Time Series Forecasting](https://arxiv.org/abs/2605.23402)
*Jinglin Li,Jun Tan,QI Fang,Ning Gui*

Main category: cs.LG

TL;DR: 提出一种参数化先验映射（PPM）框架，通过将参数化结构先验注入生成模型，实现非平稳多元时间序列的高效准确预测。


<details>
  <summary>Details</summary>
Motivation: 现有参数化方法有强归纳偏差但缺乏灵活性，深度生成模型需大量数据和计算，难以平衡表达能力与鲁棒性。

Method: 利用参数化估计器推导动态自适应先验，通过可学习映射指导生成模型学习复杂预测分布，并采用混合目标训练。

Result: 在非平稳数据上优于现有基线，在准确性和计算效率之间取得更优权衡。

Conclusion: PPM有效结合参数化方法的效率和生成模型的表达能力，为概率时间序列预测提供新方案。
...
### [136] [Hybrid Quantum-Classical Corrective Diffusion Modeling for Meteorological Downscaling](https://arxiv.org/abs/2605.23403)
*Rui Wang,Edoardo Pasetto,Amer Delilbasic,Morris Riedel,Kristel Michielsen,Gabriele Cavallaro*

Main category: cs.LG

TL;DR: 提出混合量子-经典修正扩散模型用于天气统计降尺度，在扩散UNet瓶颈处插入变分量子电路，在2020验证集上提升MAE和CRPS，但2021分布外测试显示泛化差距，硬件和电路规模仍是限制。


<details>
  <summary>Details</summary>
Motivation: 天气降尺度需从粗分辨率输入重建高分辨率输出，动态细化成本高，探索混合量子-经典方法能否以更高效的方式改进性能。

Method: 在扩散UNet的最压缩瓶颈处插入变分量子电路层，回归分支保持全经典，通过不同ansätze实现通道内和跨通道混合，应用于10米风分量降尺度。

Result: 2020验证集上混合模型稳定，保持空间结构，优于经典模型的MAE和CRPS；能量谱和风速分布类似，但尾部行为、极端风速定位等有变化；模拟噪声影响可忽略，实硬件受限于量子比特和保真度；2021分布外测试显示泛化差距。

Conclusion: 瓶颈级量子混合可对天气统计降尺度做出有意义的贡献，但电路扩展性和硬件部署仍是关键限制因素，未来需通过稳定化和正则化缓解泛化差距。
...
### [137] [What Linear Probes Miss: Multi-View Probing for Weight-Space Learning](https://arxiv.org/abs/2605.23410)
*Eunwoo Heo,Kyeongkook Seo,Jaejun Yoo*

Main category: cs.LG

TL;DR: 提出MVProbe框架，通过融合一阶信号和Gram矩阵二阶交互信息，改进了基于探针的模型权重分析，在Model Jungle基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有单一视角探针仅捕捉一阶结构，无法编码行-列交互中的高阶相关性，导致信息丢失。

Method: 设计多视角探针框架MVProbe，结合一阶探针和Gram矩阵视图，通过理论分析缩放律来标准化和融合各分支信号。

Result: 在多个判别式骨干网络（ResNet、SupViT、MAE、DINO）和生成式LoRA（Stable Diffusion LoRA）上，MVProbe一致优于最先进的ProbeX。

Conclusion: 多视角探针有效弥补了单视角不足，能更全面地提取模型参数中的结构信息，提升了权重空间学习的性能。
...
### [138] [Sample-wise Targeted Adversarial Attacks on Test-time Adaptation](https://arxiv.org/abs/2605.23411)
*Phuc Duc Nguyen,Quang Duc Nguyen*

Main category: cs.LG

TL;DR: 针对测试时自适应（TTA）中的对抗攻击，提出一种样本级定向攻击，通过元学习和优先感知梯度对齐实现高成功率且隐蔽。


<details>
  <summary>Details</summary>
Motivation: 现有类级定向攻击在TTA中不实用，因批次处理导致目标标签频率异常易被检测，需要更隐蔽的样本级攻击。

Method: 采用元学习框架，提出优先感知梯度对齐策略，将梯度更新建模为椭球信任域问题，以平衡攻击成功与分布隐蔽性。

Result: 在CIFAR-10-C、CIFAR-100-C、ImageNet-C上，该方法达到高定向成功率，标签分布与无攻击基线一致，难以检测，且对现有防御鲁棒。

Conclusion: 该攻击更贴近现实威胁，能有效逃避检测，对TTA系统构成严重安全挑战。
...
### [139] [Reflex: Reinforcement Learning with Reflection Symmetry Exploitation in State-Based Continuous Control](https://arxiv.org/abs/2605.23415)
*Shuai Zhen,Yifan Zhang,Yuling Wang,Yanhua Yu*

Main category: cs.LG

TL;DR: 本文提出Reflex范式，通过反射对称性正则化提升强化学习样本效率，在连续控制任务中优于基线。


<details>
  <summary>Details</summary>
Motivation: 强化学习样本效率低，现有工作主要关注图像RL和旋转对称，而状态RL和反射对称未充分探索。

Method: 提出Reflex，形式化轴向反射和双侧反射，基于对称保持最优值函数和策略的理论，通过对称正则化将反射对称融入策略学习，兼容在策略和离线策略算法。

Result: 在OpenAI Gym和DeepMind Control基准测试中，集成PPO和SAC的Reflex相比标准基线性能更优，样本效率提升。

Conclusion: 反射对称性可有效提升连续控制任务的样本效率，Reflex是一种实用且高效的集成方法。
...
### [140] [An Open-Source Training Dataset for Foundation Models for Black-box Optimization](https://arxiv.org/abs/2605.23417)
*Aaron Klein,Herilalaina Rakotoarison,Luca Thale-Bombien,David Salinas*

Main category: cs.LG

TL;DR: 黑盒优化方法通常需要大量超参数调优，而基于预训练的基础模型有潜力泛化到不同问题。本文提出BBO-Pile数据集（50万条优化轨迹，3095个黑盒），并训练了从2M到80M参数的系列模型，验证了大规模预训练的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有黑盒优化方法需要繁琐的超参数调优，且缺乏大规模、真实世界的公开预训练数据，阻碍了基础模型的发展。

Method: 构建了包含超过50万条优化轨迹、覆盖3095个不同黑盒的BBO-Pile数据集，并训练了参数规模从2M到80M、训练数据从2亿到20亿token的系列基础模型。

Result: 大规模预训练能够有效模仿黑盒优化方法，并且模型性能随计算量增加而提升。

Conclusion: 大规模预训练是可行的黑盒优化方法，为未来研究奠定了基础。
...
### [141] [Hinge Regression Trees and HRT-Boost: Newton-Optimized Oblique Learning for Compact Tabular Models](https://arxiv.org/abs/2605.23422)
*Hongyi Li,Jun Xu,Hong Yan*

Main category: cs.LG

TL;DR: 提出铰链回归树（HRT）框架及其集成版本HRT-Boost，通过将斜分裂重构为非线性最小二乘问题，实现了高质量决策树学习，并在理论上保证了逼近率和风险降低。


<details>
  <summary>Details</summary>
Motivation: 学习高质量斜决策树因分裂优化的离散性和非凸性而面临挑战。

Method: 将每个斜分裂视为两个线性预测器的非线性最小二乘问题，利用max/min包络产生ReLU表达能力；节点优化采用阻尼牛顿法，并引入回溯线搜索；HRT-Boost结合节点级牛顿更新与逐阶段函数梯度下降。

Result: 单树HRT与强基线竞争；HRT-Boost与强集成基线比较有利，且模型更紧凑。

Conclusion: HRT和HRT-Boost在理论和实验上均表现出色，提供了一种高效的斜决策树学习框架。
...
TL;DR: 提出OM-Path方法，将深度高斯过程推理转化为后验传输，使用确定性采样器和路径正则化，在大型数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有深度高斯过程推理方法存在计算瓶颈，要么使用ELBO拟合显式密度，要么使用MCMC采样。

Method: 将后验推理视为后验传输，学习确定性采样器，通过Doob桥参考扩散的路径先验正则化，使用概率流ODE和Onsager-Machlup作用量。

Result: 在七个UCI基准上，OM-Path在两个最大数据集上显著优于DBVI，在小型数据集上不如DBVI；严格的ELBO变体未超过DBVI。

Conclusion: 降低路径目标方差比精确密度跟踪更重要，OM-Path在有限噪声下与MAP等价。
...
### [143] [Weisfeiler-Leman Is Incomplete on Simple Spectrum Graphs, so Canonicalize Them](https://arxiv.org/abs/2605.23446)
*Snir Hordan,Nadav Dym,Tim Seppelt*

Main category: cs.LG

TL;DR: 简单谱图虽然有多项式时间同构测试，但k-WL测试不能完全区分它们，且所有k-WL对齐的GNN都不完备。提出的PRiSM方法是首个可证明完备的简单谱特征分解规范化，解决了开放问题，并实现了通用近似。


<details>
  <summary>Details</summary>
Motivation: 由于k-WL测试不能区分所有非同构简单谱图，导致GNN家族在该类图上不完备，需要一种能保证完全表达性的规范化方法。

Method: 提出PRiSM（Partition, Refine, Solve, Match），一种对简单谱特征分解进行可证明完备规范化的算法。

Result: PRiSM获得了先前规范化方法缺乏的完备性保证，解决了简单谱图上完全表达性的开放问题，与DeepSets或Transformer结合可实现通用近似。

Conclusion: PRiSM在图回归、分类和表达性任务上与现有谱规范化方法性能相当或更优。
...
TL;DR: 提出一种李群变分自编码器框架，通过两阶段训练显式诊断和反映非交换结构，提高了重建质量和解码器对非交换性的敏感性。


<details>
  <summary>Details</summary>
Motivation: 变分自编码器难以表示潜在空间中的非交换结构，现有对称感知方法强制交换性会抑制数据内在的非交换性。

Method: 第一阶段无约束训练，通过Baker-Campbell-Hausdorff偏差和顺序交换测试诊断非交换性；第二阶段引入变形稳定性约束，用数据驱动校准常数对齐解码器敏感性与代数非交换性。

Result: 在dSprites、3DShapes、3DCars和CelebA上，相比beta-VAE、CLG-VAE和CFASL，重建质量提升，解码器行为更符合潜在非交换结构，CelebA上获得更保真的重建和因子特定潜在遍历。
...
TL;DR: 针对偏头痛分类研究，纠正数据泄露和指标偏差，提出临床驱动的亚型聚合、类别依赖混合增强和保真不对称概念，在七个子类型、400名患者数据集上评估，框架平均宏F1达0.862，优于其他方法。


<details>
  <summary>Details</summary>
Motivation: 已有偏头痛分类研究存在方法学缺陷（数据泄露、指标偏差），需要重新评估并改进分类性能。

Method: 采用可重复性评估纠正缺陷，提出临床聚合（按ICHD-3合并两个半偏头痛亚型）、基于样本量的类别依赖混合增强、以及保真不对称概念（比例约束增长替代完全平衡）。在六类配置下使用分层5折交叉验证，宏平均F1为主要指标。

Result: 纠正缺陷后基线宏F1为0.71；提出框架在八个分类器上平均宏F1为0.862，优于高斯copula（0.836）、CTGAN（0.815）和无增强基线（0.801）；FT-Transformer在比例增强下达到0.914。

Conclusion: 临床驱动的类别聚合贡献了大部分绝对提升，框架主要贡献在于提升分类器间的平均鲁棒性，问题公式化起主导作用。
...
### [146] [Unextractable Protocol Models: Collaborative Training and Inference without Weight Materialization](https://arxiv.org/abs/2605.23464)
...
TL;DR: 提出Unextractable Protocol Models (UPMs)，通过分片和时间变化可逆变换实现去中心化训练中权重的不可提取性，同时保持模型性能。


<details>
  <summary>Details</summary>
Motivation: 在去中心化协作训练大型神经网络时，防止任何参与者拥有完整权重集，从而保护模型不被提取。

Method: 在参与者之间周期性地注入时间变化的随机可逆变换，使不同时间步的模型分片不兼容，但整体网络功能不变。

Result: 在Qwen-2.5-0.5B和Llama-3.2-1B上，10000次变换后FP32困惑度不变（ΔPPL<0.01，JS散度<4e-5），每30秒应用变换增加3%延迟、0.1%带宽和10%GPU内存开销，训练开销降至1.6%时间和<1%内存。梯度微调拼接分区需消耗≥60%从头训练的词元。

Conclusion: UPMs使模型可协作训练但不可提取，为社区驱动的去中心化训练嵌入程序化激励机制提供了实用方案。
...
### [147] [S$^3$GNN: Efficient Global Mixing and Local Message Passing for Long-Range Graph Learning](https://arxiv.org/abs/2605.23467)
*Dai Shi,Luke Thompson,Linhan Luo,Lequan Lin,Andi Han,Junbin Gao,Jos\'e Miguel Hern\'andez Lobato*
...
### [148] [Learning Individual Dynamics from Sparse Cross-Sectional Snapshots](https://arxiv.org/abs/2605.23470)
*Christian Lagemann,Kai Lagemann,Steven L. Brunton,Sach Mukherjee*

Main category: cs.LG

TL;DR: CADENCE 是一种从孤立快照中恢复连续个体轨迹的概率框架，只需稀疏数据即可达到与密集训练序列模型相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在稀疏或横截面数据下无法推断个体连续轨迹，序列模型需密集纵向数据，而横截面方法丢失个体动态。

Method: 结合基于分数的概率流ODE消除微分同胚歧义，并使用软混合专家路由器实现个体动力学参数和路由函数的联合可辨识。

Result: 在物理系统和生物数据等多个基准上，仅用稀疏快照训练的CADENCE性能匹配或超越使用密集全轨迹数据的序列模型。

Conclusion: 打破了数据稀疏性与个体轨迹推断之间的二元对立，证明了从孤立快照恢复连续个体轨迹的可行性和有效性。
...
### [149] [CBANet: A Compact Attention-Based CNN-BiLSTM Network for Aggressive Driving Event Detection](https://arxiv.org/abs/2605.23471)
*Hanadi Alhamdan,Ghadah Alosaimi,Amir Atapour-Abarghouei,Farshad Arvin*

Main category: cs.LG

TL;DR: 提出一种增强的深度学习框架，通过构建工程化动态特征和处理数据不平衡，有效检测激进驾驶行为。


<details>
  <summary>Details</summary>
Motivation: 激进驾驶是交通事故主因，现有深度学习方法受数据不平衡、驾驶员差异大和缺乏物理可解释特征限制。

Method: 构建转向、加速和制动等工程化动态特征；采用SMOTE过采样与类别加权损失结合，并评估焦点损失变体处理不平衡；使用类别特定阈值校准的安全导向决策策略。

Result: 在自然驾驶数据集上，方法在少数类召回率和安全关键F-score上显著优于基线，且保持计算效率。

Conclusion: 所提框架有效提升真实场景下激进驾驶检测性能，兼顾准确性与效率。
...
### [150] [Automated Random Embedding for Practical Bayesian Optimization with Unknown Effective Dimension](https://arxiv.org/abs/2605.23473)
...
### [151] [Non-normal spectral signatures of instability in neural network training dynamics](https://arxiv.org/abs/2605.23476)
*Souvik Ghosh*

Main category: cs.LG

TL;DR: 使用非正规算子理论解释深度网络训练不稳定性，发现条件数 κ(V) 可作为瞬态放大的早期预警指标。


<details>
  <summary>Details</summary>
Motivation: 深度网络训练不稳定性（如损失尖峰、振荡收敛、梯度病理）普遍存在但缺乏严格算子理论解释。

Method: 证明Adam等优化器的线性化更新算子是普遍非正规的，推导伪谱前驱界，κ(V)作为早期预警指标，并建立奇异点作为κ(V)->∞的极限情况。

Result: 数值实验显示谱半径ρ(J)无法区分稳定与不稳定训练阶段，而κ(V)能区分约一个数量级。

Conclusion: 非厄米算子理论为神经网络优化稳定性提供了有用且未充分探索的框架，提供了诊断语言和概念验证基准。
...
### [152] [CoSPlay: Cooperative Self-Play at Test-Time with Self-Generated Code and Unit Test](https://arxiv.org/abs/2605.23491)
*Zhangyi Hu,Chenhui Liu,Tian Huang,Jindong Li,Yang Yang,Jiemin Wu,Zining Zhong,Menglin Yang,Yutao Yue*

Main category: cs.LG

TL;DR: 提出CoSPlay，一种无需真实单元测试的协作式自博弈框架，联合改进代码和单元测试，显著提升代码生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR和TTS依赖真实单元测试，而GT-free TTS中自生成测试噪声大且与错误代码耦合，需要同时改进两者。

Method: 通过探索多样化解思路并识别潜在失败模式产生判别性测试想法，利用代码-测试执行矩阵的双向通过计数迭代优化代码和测试，最后从输出共识最大的簇中选出最终代码。

Result: 在Qwen2.5-7B-Instruct上BoN从22.1%提升到33.2%，测试准确率从14.6%提升到78.3%，匹配或超越CURE-7B，并在CURE-7B上进一步提升5.7%，且泛化到多种骨干网络，超越其他GT-free基线。

Conclusion: 提出了一种无需真实单元测试的可扩展推理策略，能在不依赖GT数据的情况下实现有竞争力的代码生成。
...
### [153] [VACE: Learning Geometrically Structured Representations for Time Series Anomaly Detection](https://arxiv.org/abs/2605.23504)
*Alberto D. Cencillo,Leonardo Concepci\'on,Isaac Triguero,Juli\'an Luengo*

Main category: cs.LG

TL;DR: 提出VACE，一种自监督异常检测方法，通过速度一致性嵌入表示多变量时间序列中的正态性，实现紧凑且方向一致的区域。


<details>
  <summary>Details</summary>
Motivation: 多变量时间序列异常检测面临标签稀缺、错过代价高的问题，关键在于学习精确的正态性表征。对比方法通过配对采样间接塑造空间，缺乏对几何结构的明确控制。

Method: VACE采用通道感知编码器，通过速度一致性目标训练（无负样本、无合成异常），使正常轨迹局部平滑且对齐。测试时，组合马氏距离位置得分和速度库方向得分，标记同时偏离分布和动态异常的样本。

Result: 在TSB-AD-M上达到最先进性能，显著优于使用更大训练预算的复杂方法。

Conclusion: 尽管简单，VACE实现了高效且鲁棒的异常检测，验证了速度约束在自监督异常检测中的有效性。
...
### [154] [Learning partially observed systems with neural Hamiltonian ordinary differential equations](https://arxiv.org/abs/2605.23510)
*Sunniva Meltzer,S{\o}lve Eidnes,Alexander Johannes Stasik*

Main category: cs.LG

TL;DR: 提出神经哈密顿常微分方程（NHODE）框架，结合哈密顿神经网络和神经常微分方程，从部分观测数据中学习动力系统，通过嵌入物理结构提升长期预测稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有物理信息模型假设完全观测系统状态，限制了部分观测场景的应用，其中某些状态变量完全未观测，需要无直接监督的情况下推断。

Method: 将哈密顿神经网络与神经常微分方程结合，利用哈密顿结构强制能量守恒，并通过神经ODE框架允许损失仅定义在观测变量上，同时利用对称性坐标变换和可分离能量形式嵌入额外物理约束。

Result: 在从线性/非线性弹簧系统到混沌三体问题的实验中，嵌入物理结构越多，预测精度和长期稳定性越高；即使在最困难场景下，NHODE也能捕捉观测和潜在动力学，而纯数据驱动基线变得不稳定。

Conclusion: NHODE通过物理结构嵌入有效提升部分观测动力系统学习的性能，尤其在长期预测中表现出色。
...
### [155] [Precise: SDE-Consistent Stochastic Sampling for RL Post-Training of Flow-Matching Models](https://arxiv.org/abs/2605.23522)
*Jade Zou,Tao Huang,Weijie Kong,Junzhe Li,Yue Wu,Qi Tian,Jiangfeng Xiong,Jianwei Zhang,Liefeng Bo,Zhao Zhong*

Main category: cs.LG

TL;DR: 提出Precise采样器，在流匹配强化学习中平衡探索与稳定性，提升奖励优化效率。


<details>
  <summary>Details</summary>
Motivation: 现有采样器在强化学习中存在探索与稳定的矛盾，以及离散化误差导致偏离数据分布的问题。

Method: 设计Precise采样器，通过冻结干净潜变量后验均值的新近似保持SDE一致性，解决过量噪声问题。

Result: Precise在强化学习奖励优化中更快速稳定，达到SOTA对齐分数，训练时间减少13.1-53.2%。

Conclusion: Precise采样器能有效提升流匹配生成器在线强化学习性能。
...
### [156] [When One Point Is Not Enough: Addressing Ambiguous Instances in Dimensionality Reduction by Splitting](https://arxiv.org/abs/2605.23540)
*Diede P.M. van der Hoorn,Alessio Arleo,Fernando V. Paulovich*

Main category: cs.LG

TL;DR: 提出图基方法，通过复制模糊实例改善降维中的邻域表示。


<details>
  <summary>Details</summary>
Motivation: 降维中模糊实例（与多个不同邻域高度相似）只能被映射到单一位置，导致部分邻域嵌入，丢失局部结构。

Method: 基于图的方法识别模糊实例，将其复制为多个点，分别放置在各自邻域中。

Result: 揭示了隐藏的邻域成员关系，减少了部分邻域嵌入，定量分析支持。

Conclusion: 该方法可推广到其他局部图基降维技术，改善可视化。
...
### [157] [Goal-Conditioned Agents that Learn Everything All at Once](https://arxiv.org/abs/2605.23551)
*Michael Matthews,Matthew Jackson,Michael Beukman,Thomas Foster,Alistair Letcher,Scott Fujimoto,C\'edric Colas,Jakob Foerster*

Main category: cs.LG

TL;DR: 提出LEO方法，通过同时输出所有目标的值和动作，实现高效的并行全目标学习，大幅提升速度并在多种环境中取得优异表现。


<details>
  <summary>Details</summary>
Motivation: 目标条件强化学习中，轨迹信息在仅对指令目标进行在策略更新时被丢弃；全目标学习可最大化信息利用，但通过重标记实现的计算成本过高。

Method: 设计LEO（Learning Everything all at Once），网络一次性输出所有目标的值和动作，实现单次前向传播的并行全目标更新。

Result: 在目标条件Craftax上显著优于其他方法，在连续控制环境中与现有基线相当，且比全目标重标记快250倍以上。

Conclusion: LEO解锁了大规模全目标学习，可作为复杂环境中强化学习实践者的有用工具。
...
### [158] [Is Dimensionality a Barrier for Retrieval Models?](https://arxiv.org/abs/2605.23556)
*Kiril Bangachev,Guy Bresler,Jonathan Kogan,Yury Polyanskiy*

Main category: cs.LG

TL;DR: 研究低维嵌入如何支持大规模检索，证明维度与最大间隔的关系，并实验表明sigmoid损失优于InfoNCE。


<details>
  <summary>Details</summary>
Motivation: 解释现代嵌入检索模型为何能用约1000维的表示处理数十亿数据点，探究维度与表示质量的权衡。

Method: 理论分析最大间隔嵌入，建立维度上界（O(m^{-2} log n)）并在k-稀疏矩阵设置下给出匹配下界，同时实验对比InfoNCE和sigmoid损失。

Result: 当A包含所有k-稀疏行时，维度d=O(k log(n/k))是达到最大间隔Θ(k^{-1/2})的充要条件。

Conclusion: 低维嵌入可近似达到无维度限制的最佳间隔，sigmoid损失在生成大间隔嵌入时具有明显优势。
...
### [159] [MARS: Magnitude-Aware Rank Statistics](https://arxiv.org/abs/2605.23563)
*Muhammad Rajabinasab,Afsaneh M. Nejad,Arthur Zimek*

Main category: cs.LG

TL;DR: 提出MARS方法改进CD图，通过相对边际系数加权离散排名，更真实反映模型性能差异。


<details>
  <summary>Details</summary>
Motivation: 标准CD图丢弃性能差距大小，存在幅度盲视问题。

Method: 提出MARS，使用相对边际系数作为离散排名权重，基于最佳和最差距离动态投影。

Result: MARS产生更现实的统计表示和更多洞见。

Conclusion: MARS能更真实反映模型性能差异，适用于广泛实验设置。
...
### [160] [Understanding Goal Generalisation in Sequential Reinforcement Learning](https://arxiv.org/abs/2605.23565)
*Jason Ross Brown,Edward James Young*

Main category: cs.LG

TL;DR: 强化学习智能体在分布外环境中可能表现出未预期的目标导向行为，本文通过大规模实验和潜在策略梯度方法揭示了训练流水线如何决定泛化模式。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏对强化学习智能体基于训练历史如何在分布外环境中泛化的原理性理解。

Method: 研究了100多个顺序训练流水线，在250多个分布外环境中评估行为；提出潜在策略梯度方法，模拟低维潜变量在训练中的演化以预测分布外行为。

Result: 该方法在预测分布外行为上具有强准确性、泛化性和可解释性。

Conclusion: 分布外行为依赖整个训练流水线，但存在可捕捉的结构，为从发展视角理解目标泛化奠定基础。
...
### [161] [Push Your Agent: Measuring and Enforcing Quantitative Goal Persistence in Long-Horizon LLM Agents](https://arxiv.org/abs/2605.23574)
*Yuandao Cai,Yuzhang Zhu,Liyou Gao,Wensheng Tang,Shengchao Qin*

Main category: cs.LG

TL;DR: 提出量化目标持久性（QGP）概念及PushBench基准，评估语言智能体在长期任务中持续完成指定数量工作的能力。


<details>
  <summary>Details</summary>
Motivation: 现有智能体在局部调用上表现良好，但难以坚持到完成指定数量的工作，需要衡量其量化目标持久性。

Method: 构建PushBench基准，通过仓库工件收集和验证器支持的工作单元，直接测量重复工作、重复提交、虚假完成和进度漂移。对比多种控制器（状态跟踪检索、积压跟踪工作单元、标准控制器等）以及黑盒前沿智能体（Claude Code、Codex CLI）。

Result: 状态跟踪检索控制器达到69-78%成功率且无重复提交；积压跟踪工作单元控制器在标准控制器无法完成时达到25-50%成功率；前沿智能体在50工件任务中表现良好，但在100工件时成功率降至3/9。

Conclusion: 量化目标对可靠性提出了不同于局部任务能力的要求：智能体必须维持已验证的进度并仅在请求工作完成时停止。
...
TL;DR: 提出MetaEvaluator框架，无需标签即可评估新模型，大幅降低成本。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型激增，传统评估依赖昂贵标注和重复微调，难以在未标注数据上验证模型可靠性。

Method: 基于元学习在参考模型池上学习可迁移初始化，实现模型无关的评估，无需每个模型重新训练。

Result: 在多种模型和模态上，MetaEvaluator在显著降低成本的同时，提供稳定准确的性能估计。

Conclusion: MetaEvaluator使得在未标注数据上对新兴模型进行可扩展的基准测试变得实用。
...
TL;DR: 提出基于经典Preisach磁滞算子的注意力层，用二元继电器替代softmax，实现O(1)深度图灵完备，与transformer函数类不可比较，且对长序列推理更高效。


<details>
  <summary>Details</summary>
Motivation: 标准transformer二次复杂度高、依赖位置信息，且某些统计计算需深层；受物理磁滞现象启发，设计速率无关、极值驱动的注意力机制。

Method: PAL使用可学习阈值参数化的二元继电器，维护局部极值栈作为内部状态，模拟双栈下推自动机实现图灵完备。

Result: 单层PAL-Transformer在任意精度下O(1)深度图灵完备；与transformer函数类不可比较（PAL O(1)计算历史范围统计，transformer需O(log n)；transformer支持随机访问而PAL不能）；推理复杂度O(n log n) vs O(n²)。

Conclusion: PAL适合长情节记忆和弱位置依赖的任务，为序列建模提供高效替代方案，并建立了与磁滞理论的严格联系。
...
### [164] [DiLaDiff: Distilled Latent-Augmented Diffusion for Language Modeling](https://arxiv.org/abs/2605.23605)
*Jean-Marie Lemercier,Tomas Geffner,Karsten Kreis,Morteza Mardani,Arash Vahdat,Ante Juki\'c*

Main category: cs.LG

TL;DR: 提出 DiLaDiff，一种结合连续潜在空间、潜在扩散模型和一致性蒸馏的掩蔽扩散语言模型变体，以提高采样质量并加速推理。


<details>
  <summary>Details</summary>
Motivation: 扩散语言模型无法捕获解码标记间的相关性，导致采样质量与吞吐量之间的严重权衡。

Method: 包含三个组件：(1) 从现有掩蔽扩散语言模型微调的自编码器学习具有语义能力的连续潜在空间；(2) 潜在扩散模型学习编码器分布的先验；(3) 一致性模型将先验蒸馏为几步潜在生成模型。

Result: 即使无蒸馏，潜在引导扩散模型也优于掩蔽扩散基线，并显著加速推理；一致性蒸馏进一步降低计算开销，使潜在生成时间可忽略。

Conclusion: DiLaDiff 有效解决了质量与吞吐量的权衡问题，实现了更优的采样质量和更快的推理速度。
...
### [165] [How Hard is it to Rig a Benchmark? A Social Choice Analysis of Leaderboard Robustness](https://arxiv.org/abs/2605.23628)
*Polina Gordienko,Georg Schollmeyer,Frauke Kreuter,Christoph Jansen*

Main category: cs.LG

TL;DR: 将多任务基准测试中的基准特定训练视为选举操纵，证明其在Borda和平均胜率下是NP难问题，并引入实例级鲁棒性，实验表明平均胜率最难操纵。


<details>
  <summary>Details</summary>
Motivation: 多任务基准测试影响力增长导致基准游戏（操纵排名），需要从理论上理解其复杂性和鲁棒性。

Method: 将数据集视为选民、模型视为候选人，基准特定训练映射为shift bribery问题，分析NP难性；定义实例级鲁棒性（最小训练数据集数量），推导算术平均、中位数、平均胜率、成对多数下的表达式。

Result: 在Borda和平均胜率下问题NP难；在BBH（24任务，4507模型）上，平均胜率的中位数鲁棒性为22任务（92%），算术平均为13（54%），中位数和成对多数为12（50%）。

Conclusion: 平均胜率指标对操纵最鲁棒，为基准设计提供指导。
...
### [166] [Valid and Expressive Copulas for Irregular Multivariate Time Series](https://arxiv.org/abs/2605.23632)
*Christian Kl\"otergens,Tom Hanika,Lars Schmidt-Thieme,Vijaya Krishna Yalavarthi*

Main category: cs.LG

TL;DR: 提出CopFITi，一种用于不规则多元时间序列概率预测的copula模型，结合归一化流和高斯混合copula，在联合密度建模中达到最优。


<details>
  <summary>Details</summary>
Motivation: 需要有效建模不规则多元时间序列的联合概率分布，现有方法难以同时保证边缘分布的灵活性和依赖性结构的一致性。

Method: 使用归一化流建模单变量边缘分布，采用高斯混合copula捕捉联合依赖结构，实现边缘化一致性。

Result: 基于copula的方法相比直接拟合联合分布能得到更好的边缘模型；CopFITi在不规则多元时间序列联合密度建模中达到最先进水平。

Conclusion: CopFITi是首个边缘化一致的IMTS copula，建立了新的联合密度建模性能标杆。
...
### [167] [Learning Through Noise: Why Subliminal Learning Works and When It Fails](https://arxiv.org/abs/2605.23645)
*Vincent C. Brockers,Roman D. Ventzke,Valentin Neuhaus,Bel\'en Hidalgo-Ogalde,Viola Priesemann*

Main category: cs.LG

TL;DR: 潜学习通过兼容的输出头传递教师信号，不依赖于初始化的匹配。


<details>
  <summary>Details</summary>
Motivation: 先前认为潜学习需要教师-学生初始化匹配，本研究旨在证明匹配初始化并非必要，并揭示其真正机制。

Method: 在MNIST数据集上，将网络输出拆分为辅助头（处理任务无关噪声）和分类头，通过随机初始化、移除/添加层或改变架构（MLP到CNN）进行实验。

Result: 兼容的辅助头使得学生能恢复教师信号，使表示更接近教师；若分类头也兼容，学生仅基于噪声训练即可接近甚至达到教师水平。

Conclusion: 将潜学习从惊奇现象转化为具有可预测边界的理论化机制。
...
### [168] [Relevant Walk Search for Explaining Graph Neural Networks](https://arxiv.org/abs/2605.23673)
*Ping Xiong,Thomas Schnake,Michael Gastegger,Gr\'egoire Montavon,Klaus-Robert M\"uller,Shinichi Nakajima*

Main category: cs.LG

TL;DR: 提出基于最大积算法的多项式时间算法，用于高效找到GNN-LRP中的前K个相关游走，显著降低计算复杂度，提升可扩展性。


<details>
  <summary>Details</summary>
Motivation: GNN-LRP在识别相关游走时具有指数级计算复杂度，限制了其在大规模问题上的应用。

Method: 基于最大积算法，提出多项式时间算法，在神经元级别精确找到最相关游走，在节点级别近似找到。

Result: 实验表明算法在大规模数据集上表现良好，并在流行病学、分子和自然语言处理基准测试中验证了其实用性。

Conclusion: 提出的算法大幅降低了计算复杂度，使GNN-LRP能够应用于大规模问题。
...
### [169] [Optimization of randomized neural networks for transfer operator approximation](https://arxiv.org/abs/2605.23689)
*Mohammad Tabish,Stefan Klus*

Main category: cs.LG

TL;DR: 提出一种优化激活函数的算法，提升随机神经网络用于传递算子逼近的性能。


<details>
  <summary>Details</summary>
Motivation: 随机神经网络RaNNDy的基函数由激活函数决定，固定权重和偏差导致基函数字典不理想，需要优化激活函数。

Method: 在保持随机网络权重和偏差固定的情况下，优化激活函数本身，以提供更合适的基函数字典。

Result: 在随机微分方程和图随机游走等基准问题上验证了方法的有效性。

Conclusion: 通过优化激活函数，可以提升随机神经网络在动力学系统传递算子逼近中的表现，同时保持训练效率。
...
### [170] [Graph-based Complexity Forecasts in UK En Route Airspace Using Relevant Aircraft Interactions](https://arxiv.org/abs/2605.23696)
*Edward Henderson,George De Ath,Nick Pepper*

Main category: cs.LG

TL;DR: 提出一种概率方法，通过预测相关飞机对数量来提前45分钟估计空中交通管制员的工作负荷，显著优于传统交通量预测。


<details>
  <summary>Details</summary>
Motivation: 现有行业标准模型难以捕捉复杂空域中交通复杂性的细微差别，需要更准确的工作负荷预测工具以辅助决策。

Method: 基于相关飞机对数量作为工作负荷代理指标，改进过滤算法以适应伦敦中部扇区，构建航线网络图并建模飞机到达时间的不确定性，结合历史交互分布和实时数据流进行概率预测。

Result: 改进后的算法F1分数从0.69提升至0.84；预测与真实工作负荷的斯皮尔曼相关系数为0.68，显著高于传统交通量预测的0.55。

Conclusion: 该数据驱动工具可有效支持扇区配置和管制员排班决策，具有实际应用前景。
...
### [171] [Learning Dynamic Stability Landscapes in Synchronization Networks](https://arxiv.org/abs/2605.23708)
*Christian Nauck,Junyou Zhu,Michael Lindner,Frank Hellmann*

Main category: cs.LG

TL;DR: 提出从图拓扑学习每个节点的稳定性景观的新任务，使用GNN+CNN模型，生成数据集并验证泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统标量稳定性指数依赖网络科学或GNN，理解有限；景观提供更深层见解。

Method: GNN编码图拓扑，CNN解码生成每个节点的图像景观，端到端训练。

Result: 在20和100节点的两个数据集上训练，分布内准确度高，泛化到不同图大小和实际电力网拓扑。

Conclusion: 稳定性景观可从拓扑学习，超越传统网络科学，为多领域提供新方向。
...
### [172] [Optimal Dimension-Free Sampling for Regularized Classification](https://arxiv.org/abs/2605.23726)
*Meysam Alishahi,Alexander Munteanu,Simon Omlor,Jeff M. Phillips*

Main category: cs.LG

TL;DR: 对Lipschitz连续分类损失函数（如logistic、hinge、ReLU等）在不同正则化下给出最优(1±ε)相对误差采样界，包括上界和下界，并改进先前敏感性采样结果。


<details>
  <summary>Details</summary>
Motivation: 为广泛使用的损失函数提供最优采样界，填补理论空白，改进现有方法的高复杂度。

Method: 采用均匀或(平方)范数采样，结合高阶矩界和经验过程分析，避免VC维和敏感性框架中的重复计数。

Result: 对于∥·∥2/k正则化，上界和下界均为k^2/ε^2；对于∥·∥1/k正则化，为k/ε^2；对于∥·∥2^2/k正则化，若g满足有界导数性质则线性于k，否则为k^2/ε^2。当g(0)=0时无维度无关界。所有上界与下界匹配至多对数因子。

Conclusion: 改进了先前k^3/ε^2的敏感性采样界，提供了更紧的复杂度界限，并揭示了损失函数在零点处的行为对采样复杂度的影响。
...
### [173] [Contrast to Detect: Dynamic Graph Contrastive Regularization for Unsupervised Anomaly Detection in Multivariate Time Series](https://arxiv.org/abs/2605.23744)
*Yunhua Pei,Zixing Song,Jin Zheng,John Cartlidge*

Main category: cs.LG

TL;DR: 提出ContrastAD无监督框架，将结构演化本身作为学习信号，通过动态图对比学习和频率感知注意力机制，在多元时间序列异常检测中达到最优F1和AUC。


<details>
  <summary>Details</summary>
Motivation: 现有方法在无标签场景下难以处理变量间动态依赖和频谱噪声，且图对比学习假设关系结构静止，无法适应实际系统的结构漂移。

Method: 使用多视角编码器从时间、属性和结构视角编码输入；频率感知注意力混合器在注意力前进行频谱Top-K滤波抑制噪声；动态图对比学习器基于DTW距离构建稀疏图快照，对比最差异对与稳定锚点，无需严格不变性。

Result: 在五个基准数据集上，所有数据集F1最高，三个数据集AUC最高（SWaT 93.60, SMD 98.66, PSM 97.79），在MSL和SMAP上AUC略低于最优但F1领先。

Conclusion: 对比目标作为软正则化效果最佳，严格不变性在非平稳动态下非最优。
...
### [174] [Approaching I/O-optimality for Approximate Attention](https://arxiv.org/abs/2605.23751)
*P\'al Andr\'as Papp,Aleksandros Sobczyk,Anastasios Zouzias*

Main category: cs.LG

TL;DR: 本文提出一种几乎线性I/O复杂度的注意力计算方法，并证明其接近最优。


<details>
  <summary>Details</summary>
Motivation: 现有注意力计算方法（如FlashAttention）的I/O成本随n二次增长，而理论下界仅为线性，存在优化空间。

Method: 借鉴Alman和Song的近似注意力框架，设计了I/O高效的算法。

Result: 在大多参数设置下，I/O成本几乎与n呈线性关系。

Conclusion: 所提算法在理论上接近I/O最优，并给出了各参数区间的下界以证明最优性。
...
### [175] [SeedER: Seed-and-Expand Retrieval from Knowledge Graphs](https://arxiv.org/abs/2605.23753)
*Hamed Shirzad,Frederik Wenkel,Dominique Beaini,Danica J. Sutherland,Emmanuel Noutahi*

Main category: cs.LG

TL;DR: 提出SeedER，一种利用知识图谱结构通过迭代低成本扩展进行检索的框架，有效提升多跳组合查询的召回率。


<details>
  <summary>Details</summary>
Motivation: 知识图谱的异质结构导致传统检索方法（如稠密嵌入和基于规则的方法）在多跳组合查询上表现不佳，而现有基于智能体的图检索方法成本过高。

Method: SeedER先通过轻量级稠密和实体检索播种核心节点集，然后利用图感知策略（经强化学习训练）选择性扩展该集合，将全局推理分解为可复用的局部决策。

Result: 在组合泛化和图约束子模优化方面具有理论优势，实验表明在紧凑候选集上显著优于强基线（稠密检索和图增强方法），召回率大幅提升。

Conclusion: SeedER作为知识密集型推理系统的第一阶段检索器有效且高效，在控制扩展成本的同时发现查询相关节点。
...
### [176] [LLM-driven design of physics-constrained constitutive models: two agents are better than one](https://arxiv.org/abs/2605.23754)
*Marius Tacke,Matthias Busch,Kian Abdolazizi,Jonas Eichinger,Kevin Linka,Roland Aydin,Christian Cyron*

Main category: cs.LG

TL;DR: 提出首个多智能体LLM方法生成本构模型，通过创建者与检查者智能体协同，确保模型满足物理约束，在多个材料测试中实现100%物理合规性且保持高精度。


<details>
  <summary>Details</summary>
Motivation: 传统本构模型开发需要多年专业经验，现有单智能体LLM方法缺乏系统性的物理约束检查。

Method: 多智能体框架：创建者智能体基于数据提出模型，检查者智能体审计九项物理约束，违规则返回修正。

Result: 添加检查者后，Opus模型的物理约束满足率从91%提升至100%，Kimi从37%提升至56%，精度与泛化能力保持。

Conclusion: 分离生成与检查使LLM驱动的本构建模可靠且实用，技术无关且可随LLM能力自动扩展。
...
### [177] [Debiased Negative Mining Improves Out-of-distribution Detection with Pre-trained Vision-Language Models](https://arxiv.org/abs/2605.23797)
*Bo Peng,Jie Lu,Guangquan Zhang,Zhen Fang*

Main category: cs.LG

TL;DR: 提出一种基于理论的无偏负标签挖掘方法，通过蒙特卡洛采样改进视觉-语言模型的事后OOD检测。


<details>
  <summary>Details</summary>
Motivation: 现有VLM-based OOD检测依赖启发式规则挖掘负标签，存在严重的假负例问题，限制了检测性能。

Method: 开发理论框架纠正负标签采样偏差，将无偏负挖掘转化为基于ID标签和未标注野生数据的蒙特卡洛采样。

Result: 在多种OOD检测设置下达到新state-of-the-art性能。

Conclusion: 方法有效提升了VLM的OOD检测可靠性，代码已开源。
...
TL;DR: 研究表明，大语言模型中的地缘政治偏见主要来源于后训练阶段，而非预训练阶段。


<details>
  <summary>Details</summary>
Motivation: 先前认为偏见来自预训练数据，本文旨在检验偏见的实际来源。

Method: 测试七个开源LLM的基座模型和聊天模型，对28个国家对进行强制选择探测，使用英语、法语和中文提示。

Result: 六个模型在后训练后偏向于开发者的国家/地区，如阿里Qwen 2.5后训练后对中国偏好显著增加（对数几率从-0.15变为+2.91）。偏见幅度受提示语言影响。

Conclusion: 地缘政治偏见在后训练中被主动塑造，而非简单继承自互联网数据，需要加强对对齐过程的透明度和审计。
...
TL;DR: 知识蒸馏中老师不必强，弱老师也能提升学生；强老师可能饱和或逆转收益。


<details>
  <summary>Details</summary>
Motivation: 检验大语言模型预训练中蒸馏是否总需要强老师。

Method: 改变架构大小和训练token预算，创建不同强弱师生关系，调整语言建模与蒸馏损失混合。

Result: 弱老师提升大模型；强老师收益饱和或逆转；蒸馏更助于泛化。

Conclusion: 挑战了蒸馏预训练需强老师的普遍信念。
...
TL;DR: 提出FM-CGM框架，利用预训练基础模型实现零样本视觉因果推理和反事实图像生成。


<details>
  <summary>Details</summary>
Motivation: 现有因果生成模型缺乏统一框架以利用预训练模型的零样本推理能力。

Method: 构建三组件模块化框架（概念提取器、概念操纵器、反事实生成器），结合大推理模型进行因果推断，使用文本到图像扩散模型生成，并引入跨注意力的因果语义引导机制（CSG）确保语义干预传播。

Result: 能够识别合理的因果结构，并生成忠实的反事实图像。

Conclusion: FM-CGM框架有效结合基础模型实现端到端因果推理，CSG机制保证干预的传播。
...
### [181] [Training-Free Looped Transformers](https://arxiv.org/abs/2605.23872)
*Lizhang Chen,Jonathan Li,Chen Liang,Ni Lao,Qiang Liu*

Main category: cs.LG

TL;DR: 提出一种无需训练的循环Transformer方法，在测试时重复使用冻结模型的中层块，通过阻尼子步代替大步更新，提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有循环Transformer需要端到端训练，而本研究旨在不进行额外训练或微调的情况下，利用预训练模型提升推理性能。

Method: 将预归一化Transformer块视为ODE的前向欧拉步，通过多次小阻尼子步替代单次大步更新，实现无训练循环。

Result: 在多个模型族上取得提升，如Qwen3-4B-Instruct在MMLU-Pro上+2.64 pp，Qwen3-30B-A3B-Instruct在CommonsenseQA上+1.14 pp等。

Conclusion: 无训练循环策略能有效提升预训练模型性能，且无需改变模型架构或额外训练。
...
### [182] [Complete-muE: Optimal Hyperparameter Transfer and Scaling for MoE Models](https://arxiv.org/abs/2605.23893)
*Hongwu Peng,Ohiremen Dibua,Yuanjun Xiong,Yifan Gong,Jianming Zhang,Yan Kang*

Main category: cs.LG

TL;DR: 提出Complete-muE框架，实现从密集FFN到各种MoE架构的超参数迁移，通过双桥系统解决架构和专家令牌数变化的问题，达到一次调参即可迁移到所有MoE配置的效果。


<details>
  <summary>Details</summary>
Motivation: 现有工具（如μP、SDE）无法解决MoE中的超参数迁移问题，因为从密集到MoE或扩展专家数量会同时改变架构和每个专家的令牌数。

Method: 方法包括两座桥：桥I通过活动宽度μP和归一化路由器缩放实现密集FFN到密集MoE的映射；桥II通过激活专家缩放实现密集MoE到稀疏MoE的映射，其中一阶SDE学习率/权重衰减修正抵消，但保留有界残差σ0偏移。由此得到Complete-muE迁移规则，覆盖激活专家、总容量、粒度、共享/组平衡混合等变化。

Result: 在语言模型和扩散模型预训练实验中，Complete-muE在不同架构和参数规模下产生相对稳定的超参数最优值，只有与桥II非严格SDE行为一致的微小漂移。实践中，该漂移足够小，使得在单个密集参考上调节的超参数可以接近最优地迁移到所有MoE配置。

Conclusion: 我们提出了一种‘一次调节密集，迁移到所有’的实用方案，使MoE模型在扩展容量时无需昂贵的超参数搜索即可实现加速收敛。
...
### [183] [LLMs as Noisy Channels: A Shannon Perspective on Model Capacity and Scaling Laws](https://arxiv.org/abs/2605.23901)
*Xu Ouyang,Deyi Liu,Yuhang Cai,Jing Liu,Yuan Yang,Chen Zheng,Thomas Hartvigsen,Yiyuan Ma*

Main category: cs.LG

TL;DR: 基于信息论的香农缩放定律，统一解释了大语言模型中的单调和非单调性能变化，并在多个模型和任务上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有单调幂律缩放定律无法解释灾难性过训练和量化退化等非单调现象。

Method: 将LLM训练建模为噪声信道上的信息传输，基于香农-哈特利定理，将模型参数映射为信道带宽，训练令牌映射为信号功率。

Result: 在Pythia和OLMo2上，香农缩放定律在R²分数上优于经典和扰动感知定律，准确捕捉损失盆地，并成功外推到未见的12B模型（R²=0.847）。

Conclusion: 香农缩放定律为理解非单调行为提供了统一框架，能够准确预测模型性能。
...
### [184] [Symbolic Density Estimation for Discrete Distributions](https://arxiv.org/abs/2605.21813)
*Ziwen Liu,Meng Li*

Main category: cs.LG

TL;DR: 提出符号密度估计（SDE），一种自动恢复离散概率分布闭式概率质量函数的无监督框架。


<details>
  <summary>Details</summary>
Motivation: 离散概率分布种类有限，传统方法需要逐个数学推导，进展缓慢。

Method: 组合基本解析操作，集成结构先验、进化搜索和有效性感知推理，并扩展到零膨胀和有限混合分布。

Result: 在基准数据集上准确恢复所有分布族及其参数；真实数据中找到简洁可解释的混合模型，拟合优度优于标准模型。

Conclusion: SDE能自动发现可解释的离散分布，扩展了可用分布集。
...
### [185] [On Global Attraction for a Particle Coupled to a Scalar Field](https://arxiv.org/abs/2605.23186)
*Valeriy Imaykin*

Main category: math-ph

TL;DR: 研究了经典粒子耦合标量波场中有限能量解在能量范数下的全局吸引问题，发现无论是否存在约束势，均无全局吸引。


<details>
  <summary>Details</summary>
Motivation: 探究粒子-波场系统中解是否全局吸引至稳态或孤子流形。

Method: 通过能量守恒论证分析两种势能情况。

Result: 在约束势和零势两种情况下，均不存在全局吸引。

Conclusion: 该系统不具备全局吸引性。
...
### [186] [The Floquet-Magnus expansion of unbounded operators](https://arxiv.org/abs/2605.23734)
*Daniel Burgarth,Robin Hillier,Davide Lonigro,Leonhard Richter*

Main category: math-ph

TL;DR: 将Floquet-Magnus展开推广至无界哈密顿量，基于非微扰框架证明在高频极限下有效动力学可任意阶逼近原时间演化，无需展开收敛。


<details>
  <summary>Details</summary>
Motivation: 标准Floquet-Magnus展开仅适用于有界哈密顿量，需扩展至无界情形以满足更广泛物理模型。

Method: 构建非微扰框架构造有效哈密顿量，并证明该框架重现Floquet-Magnus展开。

Result: 证明有效动力学在高频极限下可任意阶逼近原时间传播子，无需展开收敛，并在量子Rabi哈密顿量和驱动谐振子模型上验证。

Conclusion: 成功扩展Floquet-Magnus展开适用范围，为非微扰处理无界周期哈密顿量提供新方法。
...
### [189] [Exact versus tight-binding models in longitudinally modulated $\mathcal{PT}$-symmetric coupled waveguides](https://arxiv.org/abs/2605.23853)
*Alonso Contreras-Astorga,Jos\'e Israel Galindo-Rodr\'iguez*

Main category: math-ph

TL;DR: 通过比较精确连续解和离散TB近似，评估了TB模型在PT对称波导中的有效性，发现其能再现空间强度分布但无法准确捕获复振荡相位动力学。


<details>
  <summary>Details</summary>
Motivation: TB模型应用广泛，但在强纵向调制的PT对称系统中其有效性缺乏严格基准检验。

Method: 利用z依赖的超对称变换推导精确连续解，并与TB近似进行对比分析；针对两个PT对称耦合波导建立理论模型。

Result: 明确了TB近似的有效范围，表明其能再现空间强度分布，但在捕获非厄米演化中的复杂振荡相位动力学方面存在局限性。

Conclusion: TB近似在特定范围内有效，但需要注意其在相位动力学方面的不足。
...
### [190] [BOHM: Zero-Cost Hierarchical Attribution for Compound AI Systems](https://arxiv.org/abs/2605.22866)
*Joss Armstrong*

Main category: cs.AI

TL;DR: 提出BOHM方法，通过路由权重直接构建层次化归因树，零边际成本，无需访问组件内部，可同时提供多分辨率归因。


<details>
  <summary>Details</summary>
Motivation: 现有Shapley方法需要评估任意组件子集，不适用于仅路由少数工具的第三方API和智能体编排器。

Method: BOHM利用系统已有的路由权重，提取层次归因树：叶归因为根到叶路径乘积，层次k归因为深度k节点的诱导分布。

Result: 在3层18个LLM的880个问题上，BOHM的Kendall tau=0.928，SHAP需9000倍评估达到0.980；在5个驱动器的代理研究中，路由集中在单一工具；在4层475叶的人口普查层次上，BOHM恢复真实排名，tau达0.722。

Conclusion: BOHM满足效率、单调性、对称性和弱抑制，但不满足Shapley的可加性，可作为互补归因原语，其与Shapley的差异具有诊断价值。
...
### [191] [NeuroNL2LTL: A Neurosymbolic Framework for Natural Language Translation of Linear Temporal Logic](https://arxiv.org/abs/2605.22874)
*Paapa Kwesi Quansah,Ernest Bonnah*

Main category: cs.AI

TL;DR: 提出神经符号架构NeuroNL2LTL，将自然语言翻译为LTL逻辑，通过验证器参与训练和运行时过滤，实现高正确性规格生成。


<details>
  <summary>Details</summary>
Motivation: 自然语言到形式逻辑的翻译需要专业知识，限制了形式验证在安全关键领域的应用；现有方法在表达性和可靠性之间存在权衡。

Method: 采用中间表示保持LTL结构，结合可满足性检查和最小编辑修复，并通过验证器反馈进行强化学习训练神经组件。

Result: 在20万+需求上达到28%语义等价，86%输出可满足，并生成上下文解释。

Conclusion: 形式验证可作为神经规格系统的训练目标和运行时过滤器，实现基于逻辑保证的可靠性。
...
### [192] [RMA: an Agentic System for Research-Level Mathematical Problems](https://arxiv.org/abs/2605.22875)
*Zelin Zhao,Bo Yuan,Jaemoo Choi,Yongxin Chen*

Main category: cs.AI

TL;DR: 提出Research Math Agents (RMA)，一个用于研究级数学问题自动推理的代理框架，通过多角色、多轮工作流和结构化记忆，在First Proof基准上解决8/10问题，优于GPT-5.2R等基线。


<details>
  <summary>Details</summary>
Motivation: 现有研究多集中于竞赛数学或形式定理证明，缺乏针对需要长程推理、文献基础和迭代精化的研究级数学问题的自动推理方法。

Method: RMA将证明求解分解为问题分析、文献搜索与理解、公平比较、知识库构建和证明验证等模块，由初始化器、提议器和验证器代理通过共享结构化内存协调，形成多角色、多轮迭代工作流，协作生成和精化证明。

Result: 在First Proof基准（10个研究级问题）上，RMA解决8个问题，优于GPT-5.2R和Aletheia，生成的证明逻辑更严密、可读性更强；消融证明性能提升来自模块交互而非单一组件。

Conclusion: RMA展示了自动化研究级数学推理的潜力，代码将开源。
...
### [193] [SciAtlas: A Large-Scale Knowledge Graph for Automated Scientific Research](https://arxiv.org/abs/2605.22878)
...
TL;DR: SciAtlas是一个大规模多学科异构学术资源知识图谱，通过结构化拓扑认知和神经符号检索，解决信息爆炸和逻辑推理难题，降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 全球学术产出激增导致信息爆炸，现有检索工具缺乏拓扑推理能力，且深度研究框架存在逻辑幻觉和高推理成本。

Method: 构建包含26个学科、4300万论文、1.57亿实体和30亿三元组的SciAtlas知识图谱，并开发三路协同召回和图重排序的神经符号检索算法。

Result: 实现从语义匹配到确定性关联的平滑过渡，显著降低推理成本，作为“认知地图”支持文献综述、趋势合成等任务。

Conclusion: SciAtlas可作为有效的认知地图，赋能自动化科学研究的全流程，降低推理成本。
...
### [194] [Energy per Successful Goal: Goal-Level Energy Accounting for Agentic AI Systems](https://arxiv.org/abs/2605.22883)
*Deepak Panigrahy,Aakash Tyagi*

Main category: cs.AI

TL;DR: 提出A-LEMS系统，将AI能耗度量从每推理能耗改为每成功目标能耗（EpG），并定义编排开销指数（OOI），揭示智能体工作流能耗主要取决于编排结构而非推理计算。


<details>
  <summary>Details</summary>
Motivation: 现有AI能耗基准以单次调用或训练为粒度，对于多步骤、重试的智能体系统不适用，调用次数不能反映任务完成能耗。

Method: 开发A-LEMS跨层测量框架，包括时间边界模型、五层观测管道和可复现协议，定义EpG和OOI指标。

Result: 在推理和工具增强任务中，智能体工作流每成功目标平均能耗是线性的4.33倍（888.1 J vs 205.3 J），但工具增强任务中OOI可低于1.0，说明智能体执行更节能。

Conclusion: 每推理能耗不足以衡量智能体AI，EpG和OOI提供基于编排结构的准确能耗基准。
...
### [195] [ImProver 2: Iteratively Self-Improving LMs for Neurosymbolic Proof Optimization](https://arxiv.org/abs/2605.22885)
*Riyaz Ahuja,Tate Rowney,Jeremy Avigad,Sean Welleck*

Main category: cs.AI

TL;DR: 提出了ImProver 2，一个神经符号框架，用于在Lean 4中自动优化证明，通过数据高效的专家迭代管道和暴露形式结构及轻量级非正式抽象的脚手架，训练出的7B模型超越了同系列更大模型，与中等规模前沿模型相当，展示了证明优化的可扩展性和可学习性。


<details>
  <summary>Details</summary>
Motivation: 形式化数学库快速扩张，需要重构已验证的证明以维护和改善训练数据质量，但证明优化面临异构目标、数据稀缺和高成本等挑战。

Method: 结合数据高效的专家迭代管道和神经符号脚手架，该脚手架暴露形式结构并引入轻量级非正式抽象，同时设计了一套度量结构证明属性的指标。

Result: 训练出的7B参数模型在相同模型系列中优于大数个数量级的模型，与中等规模前沿模型竞争力相当；脚手架显著提升了小模型和前沿模型的性能；小模型能有效重构研究级证明。

Conclusion: 通过合适的脚手架和训练，小模型能在复杂多样的指标上匹配大型系统，证明优化是可扩展且可学习的任务。
...
### [196] [Mediative Fuzzy Logic: From Type-1 Foundations to Type-2, Type-3 and Quantum Extensions](https://arxiv.org/abs/2605.22900)
*Oscar Montiel Ross*

Main category: cs.AI

TL;DR: 本文统一了调停模糊逻辑的类型-1核心与区间类型-2、粒度类型-3及量子扩展，建立了调停算子的凸聚合解释，提出了带调停联结词的命题系统，证明了可靠性、超一致性和保守性，并通过自主刹车传感器融合示例展示了框架在异构、矛盾证据下的透明、保守、安全优先决策能力。


<details>
  <summary>Details</summary>
Motivation: 调停模糊逻辑在模糊控制和决策中用于协调犹豫或冲突评估，但其逻辑和语义基础尚不完善，尤其缺乏在类型-1之外的统一理论。

Method: 将调停算子刻画为受犹豫和矛盾控制的凸聚合，将调停真值建模为连续双格结构中独立的真-假对，提出扩展标准t-范数模糊逻辑的调停联结词命题系统，并建立区间类型-2、粒度索引局部评估及希尔伯特空间效应和密度算子的语义扩展。

Result: 证明了系统的可靠性、超一致性和对不含调停公式的底层模糊逻辑的保守性；高阶扩展在适当假设下可归约为类型-1情况；自主刹车示例展示了框架在不完整、异构、轻微矛盾证据下的决策能力。

Conclusion: 该统一框架提供了跨层次的连贯性，支持未来智能决策系统的透明、保守和优先安全的决策，且高阶形式化与类型-1基础一致。
...
### [197] [EVE-Agent: Evidence-Verifiable Self-Evolving Agents](https://arxiv.org/abs/2605.22905)
*Yamato Arai,Yuma Ichikawa*

Main category: cs.AI

TL;DR: 提出证据可验证的自进化搜索智能体EVE-Agent，通过奖励证据边际准确率增益来避免训练不可靠样本。


<details>
  <summary>Details</summary>
Motivation: 自进化智能体可能训练缺乏证据支持的流畅但不可靠样本，导致训练信号不透明且不可信。

Method: 修改提议-求解框架：提议者生成问题、答案和证据片段；证据验证器根据提供证据后的边际准确率增益给予奖励，无需真值或人工标注。

Result: 在证据相关正确性上显著优于先前自进化搜索智能体。

Conclusion: 生成训练样本带有可审计的来源证据，提升了可信度。
...
### [198] [The Deterministic Horizon: Impossibility Results as Design Specifications for Trustworthy AI Systems](https://arxiv.org/abs/2605.23024)
*Dongxin Guo*

Main category: cs.AI

TL;DR: 本研究将大语言模型的基本限制转化为设计规则，发现由架构决定的确定性视界限制了准确性上限，并汇编了16个具体规范。


<details>
  <summary>Details</summary>
Motivation: 大语言模型广泛应用，但存在图灵、阿罗、无免费午餐等根本限制，需将这些不可能性结果转化为设计规则以指导可信AI。

Method: 通过理论分析证明架构决定的准确性上限；利用信息论转换导出精度超指数衰减；将相同论证应用于偏好学习、检索管道、拍卖、零知识验证等子领域；构建包含可计算边界、量化违反成本和建设性设计规则的规范目录。

Result: 确定性视界在12种Transformer架构中介于19至31；最优长度痕迹微调可恢复不到4个百分点；精度随推理深度超指数衰减；模幂运算存在无条件电路复杂度下界；多领域存在对应的不可能性结果和量化代价。
...
### [199] [PathCal: State-Aware Reflection-Marker Calibration for Efficient Reasoning](https://arxiv.org/abs/2605.23074)
*Lingyu Jiang,Zirui Li,Shuo Xing,Peiran Li,Tsubasa Takahashi,Dengzhe Hou,Zhengzhong Tu,Kazunori Yamada,Fangzhou Lin*

Main category: cs.AI

TL;DR: 本文研究了大型推理语言模型在推理轨迹中的反思标记（如“wait”、“but”、“alternatively”），发现它们具有不同的功能角色和影响时间，并据此提出PathCal解码控制器，通过区分标记类型并仅在局部不确定状态进行干预，在保持或提升准确率的同时减少生成长度，无需外部验证器或额外采样。


<details>
  <summary>Details</summary>
Motivation: 现有研究将反思标记视为粗略的单一类别，忽略了其不同功能角色；本文旨在通过类型级抑制和固定前缀干预揭示其差异，并据此设计更好的解码控制方法。

Method: 提出PathCal，一个无需训练的解码控制器，在每一步利用反思标记分布估计局部竞争（保持当前轨迹 vs 开启竞争分支），当竞争分支证据过多时软调整标记logits。

Result: 在六个推理基准上，PathCal在保持或提升准确率的同时减少了生成长度，实现了更好的效率-性能权衡。

Conclusion: 区分反思标记类型并在适当时间点干预可以有效校准推理路径，提升效率且无需外部验证器或额外采样。
...
### [200] [Inductive Deductive Synthesis: Enabling AI to Generate Formally Verified Systems](https://arxiv.org/abs/2605.23109)
...
TL;DR: 提出归纳演绎合成（IDS）方法，联合增量式合成实现与证明，在分布系统验证上7/7成功，平均每个规范约6.8小时、106美元，比专家快200倍，比最先进智能体便宜17%。


<details>
  <summary>Details</summary>
Motivation: AI智能体在需要形式化保证的任务（如分布式系统读写一致性）上表现不佳，即使最先进编码智能体也仅成功2/7规范，而专家验证需数月到数年。

Method: IDS（归纳演绎合成）联合增量式合成实现与证明，从失败中学习以系统探索有效策略，构建为智能体LLM系统，并集成性能反馈循环。

Result: IDS实现7/7规范成功，平均约6.8小时、106美元，比专家快200倍，比最先进智能体便宜17%，且生成的实现比已发布验证系统快3倍。

Conclusion: IDS是首个有效解决分布系统形式化验证差距的方法，大幅降低时间和成本。
...
### [201] [Redrawing the AI Map: A Theory of Accountability Boundaries in Agentic Ecosystems](https://arxiv.org/abs/2605.23179)
*Muhammad Zia Hydari,Farooq Muzaffar*

Main category: cs.AI

TL;DR: 提出智能体生态系统中问责边界放置的能力级理论，解释AI能力如何影响模块化与组织分离。


<details>
  <summary>Details</summary>
Motivation: 智能体AI协调者降低了接口与组装成本，但AI输出的问责可能仍保持集成，需要理论理解边界何时分离。

Method: 引入问责资产与规则债务概念，整合数字创新、交易成本、互补资产、平台治理等视角，提出七个命题。

Result: 识别出组件、集成和双轨三种边界策略，并预测问责边界与执行边界能否共同移动。

Conclusion: 理论阐明了数字模块化何时扩展到组织分解，以及问责何时保持能力集成。
...
### [202] [AutoResearch AI: Towards AI-Powered Research Automation for Scientific Discovery](https://arxiv.org/abs/2605.23204)
...
TL;DR: 这篇调查论文定义了AutoResearch（AI驱动的科研工作流自动化）概念，并分析了从任务级AI到工作流级自动化的转变，指出现有系统在证据保留、可重复性、跨域鲁棒性等方面的不足，提出了五个工作流条件和五个评估维度。


<details>
  <summary>Details</summary>
Motivation: 现有AI科研系统碎片化，缺乏统一框架来评估和推动从任务级到工作流级自动化的演进，且面临证据保留、可重复性、弱方向拒绝、来源追踪等挑战。

Method: 通过调查现有AI科研系统，提出AutoResearch和Vibe Research概念，围绕五个工作流条件（文献与研究基础、假设形成与规划、实验与工具使用、反馈验证与评审、报告与知识传播）组织分析，并整合AI科学家系统、混合主动研究框架、基准测试、领域部署和开源基础设施。

Result: 提出了五个评估维度（新颖性、有效性、影响力、可靠性、来源），并表明AutoResearch的自主性受领域条件限制：在结构化、可执行、可快速验证的设置中更可信，而在具身、延迟、异质、伦理或制度化问责等场景中受限。

Conclusion: 科研自动化需要进一步发展以实现健壮的闭环，当前基于提示的Vibe Research仍占主导，而AI主导系统尚未达到健壮自主，未来需在证据保留、跨域鲁棒性等方面改进。
...
### [203] [Foundation Protocol: A Coordination Layer for Agentic Society](https://arxiv.org/abs/2605.23218)
...
TL;DR: 提出了Foundation Protocol (FP)，一个基于图的协调层，用于构建人类-AI社会，支持异构实体统一、多方协作、经济原语和审计问责。


<details>
  <summary>Details</summary>
Motivation: 随着自治代理规模扩大，协调成为瓶颈，需要可靠的代理关系、多代理工作、价值交换和可问责性。

Method: FP采用图优先架构，统一代理、工具、资源、人类等实体，支持原生多方组织和事件协作，提供计量、收据和结算等经济原语，并将策略、溯源和审计作为一等公民，通过包装现有协议实现增量采用。

Result: 减少了集成和治理开销，保持了代理的可组合性和问责性。

Conclusion: FP使得协调成为开放、多元、可治理的人类-AI社会的共享基础设施。
...
TL;DR: 提出GENSTRAT，通过程序化生成双人零和卡牌游戏环境，结合能力剖面与波动性指标，评估LLM在战略场景中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有战略推理基准在固定规范游戏上评估，易饱和且难以泛化到实际部署的复杂环境。

Method: 生成双人零和、不完全信息卡牌游戏分布，可即时生成新游戏；配合六维度能力剖面（状态空间、时间深度、信息敏感度、对手建模、风险、脆弱性）及波动性度量（jaggedness）评估模型。

Result: 在2000游戏池中抽取50个基准，与9个前沿/开源LLM进行超36000场比赛。新前沿模型平均得分更高，但实力接近的模型（如gpt-5、claude与gemini-3.1-pro）能力剖面不同且前两者局部波动更大。

Conclusion: 能力剖面和波动性指标提供了整体排名无法提供的部署相关诊断信息。
...
### [205] [Design and Report Benchmarks for Knowledge Work](https://arxiv.org/abs/2605.23262)
*Yining Hua,Hongbin Na,Cyrus Ayubcha,Levi Lian*

Main category: cs.AI

TL;DR: 提出了一个三步方法，用于明确基准测试任务与实际知识工作主张之间的关系，并从O*NET数据库推导出18个工作活动清单，通过三个案例展示基准设计如何影响工作主张的强度。


<details>
  <summary>Details</summary>
Motivation: 当前知识工作评估和基准设计仍遵循传统NLP任务逻辑，导致更高的基准性能不能可靠反映系统在实际部署中的知识工作能力。

Method: 三步方法：1）定义被评估的工作活动；2）指定测试设置（包括材料、工具、角色和约束）；3）对适当的工作产品进行评分。并从O*NET数据库中导出18个工作活动清单。

Result: 通过GDPval、OfficeQA Pro和APEX-SWE三个基准案例分析，展示了基准设计选择如何塑造分数所能支持的最强工作主张，以及基准任务、测试设置、评分产品与更广泛工作主张之间的差距。

Conclusion: 基准设计应明确任务所代表的工作活动，测试设置需指定材料、工具、角色和约束，评分应关注系统留下的工作产品，以确保基准分数能可靠推断实际工作能力。
...
### [206] [Parallel Context Compaction for Long-Horizon LLM Agent Serving](https://arxiv.org/abs/2605.23296)
*Musa Cim,Burak Topcu,Chita Das,Mahmut Taylan Kandemir*

Main category: cs.AI

TL;DR: 针对长上下文LLM代理的对话历史压缩问题，提出并行压缩方法，可精细控制摘要体积，减少端到端耗时并提高吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的顺序压缩方法有损、阻塞调用导致延迟长、无法精细控制摘要体积，且输出和保留信息波动大，使代理知识不稳定。

Method: 引入并行压缩用于长上下文代理流程，在多个模型（8B-120B参数，包括稠密和MoE架构、推理和非推理模型）上，在HotpotQA和LoMoCo基准上评估，与顺序同步基线对比。

Result: 并行压缩在匹配压缩解码体积时，减少了端到端耗时，提高了压缩吞吐量，并提供了对摘要体积的精细、可预测控制。

Conclusion: 并行压缩方法有效改进长上下文LLM代理的压缩效率和控制能力。
...
### [207] [Ontological Knowledge Blocks: Executable Compliance and Profile-Based Validation for Trustworthy AI Systems](https://arxiv.org/abs/2605.23297)
*Aasish Kumar Sharma,Julian M. Kunkel*

Main category: cs.AI

TL;DR: 提出了本体知识块（OKB），一种可编程的治理基础设施，将监管义务编译为机器可检查的约束，并通过实验证明其有效性和性能。


<details>
  <summary>Details</summary>
Motivation: 当前合规性方法以文档为中心，依赖静态检查表和人工审查，无法扩展到自动化AI系统。

Method: 将OKB定义为五元组，结合RDF/OWL概念模式、SHACL验证规则、证据要求和PROV-O溯源链接；通过监管编译器将中间表示记录转换为可组合的知识块模块。

Result: 在AI辅助HPC资源分配场景中，24次验证运行和四个治理配置下，展示了配置敏感的验证、严格累加违规累积、SHACL验证延迟12.6-100.3毫秒，且组合配置是最全面的。

Conclusion: OKB实现了可扩展的机器可检查治理，支持配置重配置而无需修改服务代码，所有工件开源。
...
### [208] [DART: Semantic Recoverability for Structured Tool Agents](https://arxiv.org/abs/2605.23311)
*Ke Yang,Panpan Li,Zonghan Wu,Kejin Xu,Huaxi Huang,Xiaoshui Huang*

Main category: cs.AI

TL;DR: 提出DART模块化运行时，通过认证语义可恢复边界并选择可接受恢复点，在承诺敏感场景中实现安全高效的本地恢复。


<details>
  <summary>Details</summary>
Motivation: 现有本地恢复方法缺乏语义有效性检查，在承诺敏感场景中可能导致不安全回滚。

Method: 形式化语义可恢复性，DART运行时定位失败实例、认证边界、对齐检查点、选择可接受恢复点。

Result: 在多个LLM驱动领域和LangGraph基板上，DART正确恢复所有评估的承诺敏感案例，基线方法失败，安全审计无违规。

Conclusion: 控制器合法性不隐含语义有效性，合理本地恢复需要显式接受性检查。
...
TL;DR: 提出多智能体框架VDSS，通过合同驱动接口协调模块，实现可追溯的呼吸机决策支持，结合上下文bandit自适应临床医生偏好，结构化反馈减少无效迭代，回顾性实验显示更高接受度和更少交互。


<details>
  <summary>Details</summary>
Motivation: 现有基于规则的系统难以个性化，端到端强化学习或大语言模型难以控制和审计，需要可解释且支持临床医生偏好的决策支持。

Method: 人机协同多智能体框架VDSS，使用合同驱动结构化接口协调模块，通过上下文bandit在线自适应调整医生偏好，并利用结构化拒绝反馈触发重新规划以减少无效交互。

Result: 回顾性ICU轨迹回放和专家评估显示，VDSS推荐接受度更高，达到可接受计划所需的交互轮次更少。

Conclusion: VDSS支持临床可部署的人机协作，通过模块化设计和可追溯证据提高决策支持的安全性和有效性。
...
TL;DR: 提出基于信息一致性的认知校准流程，提升多智能体系统规划成功率。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的多智能体系统在计划执行时因智能体误判知识而失败，这种认知失调在规划阶段潜伏且动态变化，难以检测。

Method: 提出EPC-AW，通过信息一致性计划选择评估计划稳定性，并利用一致性引导的认知状态细化，根据过去差异调整校准。

Result: 实验表明，EPC-AW使系统级成功率平均提升9.75%。

Conclusion: EPC-AW通过信息一致性有效缓解了认知失调问题，提升了多智能体系统的规划可靠性。
...
TL;DR: 针对在线策略自蒸馏中特权信息导致模型行为改变及通用能力退化的问题，提出EDGE-OPD方法，通过引导rollout和证据掩码实现精确知识传递，实验证明其在保持通用能力的同时成功学习目标身份。


<details>
  <summary>Details</summary>
Motivation: 在线策略自蒸馏（OPSD）中特权信息（如角色、私有事实）会过度改变模型行为，引起推理改变、能力下降等副作用，需解决如何仅传递期望行为而不影响通用能力的问题。

Method: 提出EDGE-OPD，包含两个特点：a) 使用引导rollout在采样时注入特权上下文行为，使目标行为出现在策略数据中；b) 应用证据掩码，只更新特权上下文支持采样的token位置，而非所有token。

Result: 实验表明OPSD及变体RLSD完全无法学习目标身份，而EDGE-OPD成功；掩码区域消融显示身份信号集中在正证据尾部，可实现高效知识传递和通用能力保持。

Conclusion: EDGE-OPD通过引导rollout和证据掩码有效解决了OPSD中特权信息副作用问题，能在不损害通用能力的前提下完成目标行为蒸馏。
...
TL;DR: 将动态规划与约束规划混合用于部分车间调度问题，以DP为主搜索框架，CP作为子程序利用全局约束传播，实现了灵活且优雅的结合。


<details>
  <summary>Details</summary>
Motivation: 动态规划和约束规划通常分开使用，本文旨在展示两者可以有效地结合，利用各自的优势解决组合优化问题。

Method: 以动态规划作为主要搜索框架，约束规划作为子程序进行全局约束传播；支持任意时刻DP策略（如任意时刻列搜索），并利用CP建模的灵活性处理任意优先约束。

Result: 该方法灵活支持任意时刻DP和大邻域搜索，但在此特定问题上未达到最先进纯CP求解器的竞争力。

Conclusion: 主要贡献是展示了DP与CP混合集成方法的可行性，为后续研究提供了基础。
...
TL;DR: 提出两种模型（约束规划和混合整数规划）来解决飞机拆解调度问题，其中包含上千个任务和多种约束，基于实际数据测试多达1450个任务。


<details>
  <summary>Details</summary>
Motivation: 飞机拆解对可持续性至关重要，但利润微薄，高效调度是保证盈利和激励实践的关键。

Method: 构建了约束规划模型和混合整数规划模型来处理包含技术认证、优先关系、平衡约束和空间限制的大型调度问题。

Result: 基于工业合作伙伴提供的真实运营数据，对包含多达1450个任务的实例进行了测试。

Conclusion: 详细描述了飞机拆解调度问题，并提出了两种有效的求解方法。
...
TL;DR: 一种基于共享强化学习策略的人设条件NPC控制方法，实现高效零样本人设识别和行为对齐。


<details>
  <summary>Details</summary>
Motivation: 生命模拟游戏需要大量具有一致性格且可被自然语言控制的NPC，现有方法无法兼顾人设一致性、可控性和实时推理。

Method: 提出pcsp方法，将冻结的LLM人设描述嵌入输入单策略强化学习，结合低秩投影、神经条件化及PPO+InfoNCE一致性+KL多样性的训练目标。

Result: 在300人设基准上，零样本识别超过随机17倍，语义-行为对齐皮尔逊ρ≈0.73，推理速度比LLM基线快22倍；消融表明InfoNCE目标至关重要；在Unreal Engine 5中实现64智能体亚帧级推理。

Conclusion: 共享RL策略能够支持可扩展、实时、人设条件的NPC控制，为大规模生命模拟游戏提供了可行方案。
...
### [216] [MemAudit: Post-hoc Auditing of Poisoned Agent Memory via Causal Attribution and Structural Anomaly Detection](https://arxiv.org/abs/2605.23723)
...
TL;DR: 提出MemAudit框架，用于事后审计记忆增强型LLM代理中的恶意记忆注入，结合反事实影响分数和一致性图，将攻击成功率降至0%。


<details>
  <summary>Details</summary>
Motivation: LLM代理的持久记忆机制可被对手通过正常交互注入恶意记录，现有防御只关注在线干预，缺乏事后审计能力。

Method: MemAudit结合两种互补信号：反事实记忆影响分数（衡量每条记忆对有害输出的因果贡献）和记忆一致性图（识别结构异常的记忆）。

Result: 在QA和RAP攻击场景中，攻击成功率分别从70%和83.3%降至0%。

Conclusion: MemAudit在事后审计场景下显著降低攻击成功率，有效识别恶意记忆。
...
### [217] [Agentic Proving for Program Verification](https://arxiv.org/abs/2605.23772)
*Alessandro Sosso,Akhil Arora,Bas Spitters*

Main category: cs.AI

TL;DR: 在CLEVER基准上，Claude Code在程序验证中表现卓越，生成有效规范（98.8%），验证实现（87.5%），端到端成功（98.1%），并提供高质量反馈，但揭示现有基准能力不足。


<details>
  <summary>Details</summary>
Motivation: 评估现代智能体系统在程序验证中的能力，并指出当前基准的局限性。

Method: 在CLEVER（Lean 4基准）上使用Claude Code进行智能体证明框架评估。

Result: 98.8%生成有效规范（其中81.3%被同构评分接受），87.5%验证实现，98.1%端到端成功，且提供高质量反馈。

Conclusion: 现有基准与智能体能力不匹配，需要更严格的评估方法（如替代同构评分）；编译器在环的智能体范式是当前最有效的程序验证方法。
...
### [218] [Beyond Binary Edits Robust Multimodal Knowledge Editing with Adversarial Subspace Alignment](https://arxiv.org/abs/2605.23780)
*Haoyuan Wang,Xiaohao Liu,Jiajie Su,Jianmao Xiao,Chaochao Chen*

Main category: cs.AI

TL;DR: 针对多模态大语言模型内在知识编辑泛化性不足的问题，提出了ASAM方法，通过潜在对抗鲁棒化和秩约束子空间学习增强泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有内在多模态知识编辑虽然可靠性和局部性好，但泛化性有限，无法将编辑传播到语义等价的变体，原因在于缺乏明确的语义监督、编辑范围僵化以及在高维多模态空间中对单个样本的偏置锚定。

Method: 提出ASAM方法，包括两个核心组件：1) 潜在对抗鲁棒化（LAR），在联合潜在空间中生成对抗但语义一致的变体，暴露脆弱语义区域；2) 秩约束子空间学习（RCSL），通过基于奇异值的损失强制编辑层对抗表示的秩降低对齐。

Result: 广泛分析证明了ASAM在增强内在知识编辑泛化鲁棒性方面的有效性。

Conclusion: ASAM能够有效提升多模态大语言模型知识编辑的泛化能力。
...
### [219] [SPACENUM: Revisiting Spatial Numerical Understanding in VLMs](https://arxiv.org/abs/2605.23898)
*Jianshu Zhang,Yijiang Li,Huifeixin Chen,Haoran Lu,Letian Xue,Bingyang Wang,Han Liu*

Main category: cs.AI

TL;DR: 当前视觉语言模型在空间数字理解上表现接近随机猜测，依赖浅层线索，缺乏稳定的坐标感知表示。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在具身环境中需输出数值（如动作大小、空间坐标），但不确定这些数值是否真正基于空间感知。

Method: 构建SpaceNum框架，包含动态转换和静态布局两种设置，并设计Num2Space和Space2Num双向任务评估模型。

Result: 模型在空间数字理解上接近随机猜测，错误分析表明依赖浅层空间线索，难以建立稳定的坐标感知表示和抽象结构化空间布局。

Conclusion: 当前VLMs在空间数字理解上表现差，显式推理增益有限，微调可部分提升并泛化到外部空间推理基准。
...
### [220] [From Raw Experience to Skill Consumption: A Systematic Study of Model-Generated Agent Skills](https://arxiv.org/abs/2605.23899)
...
TL;DR: 通过构建评估框架和元技能，系统研究了语言智能体中模型生成技能的效用和负迁移问题。


<details>
  <summary>Details</summary>
Motivation: 缺乏对技能生命周期（经验生成、提取、消费）的全面研究，不清楚技能是否有效、何时有效及成败原因。

Method: 构建实用导向的评估框架，跨五个领域进行实验，深入分析各阶段，并提出引导提取的元技能。

Result: 模型生成技能平均有益但存在非平凡负迁移；提取器和消费者行为不一致；元技能持续改善技能质量并减少负迁移。

Conclusion: 技能提取应关注与实际效用相关的特征，元技能可有效提升技能质量并降低负迁移。
...
### [221] [SkillOpt: Executive Strategy for Self-Evolving Agent Skills](https://arxiv.org/abs/2605.23904)
...
TL;DR: 提出SkillOpt，一种系统可控的文本空间技能优化器，通过严格编辑改进使技能训练稳定且部署时无额外开销，在多个基准和模型上取得最佳结果。


<details>
  <summary>Details</summary>
Motivation: 现有技能生成方式不可靠，无法像深度学习优化器一样基于反馈稳定改进。

Method: 使用单独的优化器模型将评分轨迹转化为对技能文档的边界编辑，仅接受严格提高验证分数的编辑，并引入文本学习率预算、拒绝编辑缓冲区和逐周期慢/元更新。

Result: 在6个基准、7个模型、3个执行框架的52个评估组合中全部最佳或并列最佳，平均准确率提升19-24.8个百分点。
...
### [222] [Expressive Power of Deep Homomorphism Networks over Relational Databases](https://arxiv.org/abs/2605.22852)
*Moritz Sch\"onherr,Balder ten Cate,Maurice Funk,Benny Kimelfeld,Carsten Lutz,Arie Soeteman*

Main category: cs.DB

TL;DR: 深度同态网络（DHN）通过连接一阶逻辑片段和SQL，具有可判定的静态分析问题，且在预测任务中体现表达能力差异。


<details>
  <summary>Details</summary>
Motivation: 消息传递图神经网络存在表达限制，需要更强大的图学习架构；DHN与关系数据库中的SQL片段紧密相关，适合学习。

Method: 将DHN（使用max、sum、mean聚合）与一阶逻辑的否定单值片段（UNFO）及其计数和比率量化扩展相联系；sum-聚合DHN与单值量词交替片段及带计数的扩展相关联；研究DHN的空虚和包含问题的可判定性。

Result: 建立了DHN与多种一阶逻辑片段的精确表达力关系，揭示了与SQL的对应；证明了两个静态分析问题的可判定性；实验证实表达力差异在预测任务中体现。

Conclusion: DHN在表达力上优于传统GNN，通过逻辑连接获得可判定的静态分析，适用于关系数据库学习。
...
### [223] [Measuring Database Unfairness via Dependency Quantification Under Differential Privacy](https://arxiv.org/abs/2605.22952)
*Mariia Vologdin,Yuchao Tao,Amir Gilad*

Main category: cs.DB

TL;DR: 提出在差分隐私下量化数据不公平性的形式框架，定义三个期望并设计三种互补度量及隐私保护算法，实验验证有效性。


<details>
  <summary>Details</summary>
Motivation: 差分隐私保护隐私的同时使评估公平性和可靠性变难，需要量化数据不公平性。

Method: 提出三个期望：积极性、单调性和DP可计算性；三种度量：基于互信息（全变差距离代理）、基于数据修复（加权MaxSAT归约）、基于top-k元组贡献；设计隐私保护算法分析敏感度、准确性和效率。

Result: 实验表明所提度量忠实近似非隐私版本，有效量化隐私约束下的偏差，为数据管理提供见解。

Conclusion: 框架能有效量化差分隐私下的数据不公平性。
...
### [224] [Conceptual Schema Inference for Tabular Datasets using Large Language Models](https://arxiv.org/abs/2605.23105)
*Zhenyu Wu,Jiaoyan Chen,Norman W. Paton*

Main category: cs.DB

TL;DR: 本文提出两种基于大语言模型（LLM）的方法，从原始表格中自动推导概念模式，包括实体类型、属性和类型间关系，解决了数据湖等大规模表格集合的模式理解问题。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注数据集发现和探索，但缺乏从异构表格中自动推导概念模式的方法，导致大规模表格集合的理解和组织困难。

Method: 提出两种方法：GeSI利用生成式LLM从表格和列级语义推断层次类型及属性，并整合为全局模式；EmSI采用LLM表格嵌入对表格按列语义分组，推断组内属性并从共享属性模式构建层次结构。

Result: 实验表明，两种方法在推断模式组件的简洁性和结构质量上有效，且可扩展到大型存储库，并通过案例研究展示了端到端模式推断。

Conclusion: 本文的方法能够自动从原始表格中推导概念模式，有效支持大规模异构表格集合的理解和组织。
...
### [225] [BCTuner: LLM-Guided Monte Carlo Tree Search for Efficient Blockchain Knob Tuning](https://arxiv.org/abs/2605.23280)
*Yaoyi Deng,Chongyang Tao,Mingxuan Li,Xuelian Lin,Han Sun,Mingchao Wan,Shuai Ma*

Main category: cs.DB

TL;DR: 提出BCTuner，一种基于大语言模型引导的框架，结合知识推理与结构化搜索，自动调优许可型区块链的配置参数。


<details>
  <summary>Details</summary>
Motivation: 区块链配置调优面临架构复杂性、配置耦合以及实验成本高等挑战，现有方法效率低。

Method: 利用大语言模型推理配置语义、约束和部署上下文，将调优建模为蒙特卡洛树搜索过程，逐步构建、验证、评估和优化配置，并采用自适应剪枝去除不可行或低潜力分支。

Result: 在Hyperledger Fabric和ChainMaker上，相较于默认配置吞吐量提升高达211.38%，优于最先进方法20%，且交互次数减少8倍。

Conclusion: BCTuner能有效提升许可型区块链性能，降低调优成本，展示了LLM引导搜索在系统调优中的潜力。
...
### [226] [A Pragmatic Approach to Learned Indexing in RocksDB: Targeted Optimizations with Minimal System Modification](https://arxiv.org/abs/2605.23815)
*Shubham Vashisth,Olivier Michaud,Bettina Kemme,Oana Balmau*

Main category: cs.DB

TL;DR: 通过最小化存储引擎改造，将现成的学习索引集成到RocksDB中，利用Memtable与磁盘索引的分离，提出重用机制和块感知索引，显著提升读写性能。


<details>
  <summary>Details</summary>
Motivation: 学习索引具有高吞吐、低内存的优点，但缺乏对并发和持久化的有效支持，导致生产系统采纳有限。

Method: 利用RocksDB中Memtable与不可变磁盘文件的分离，分别部署专用索引；引入重用机制保留Memtable间结构知识；将磁盘索引替换为学习索引并改造为块感知，保证最坏情况单I/O查找。

Result: 写吞吐量提升1.5倍，读吞吐量提升2.1倍，优于现有系统。
...
### [227] [CHRONOS: Temporally-Aware Multi-Agent Coordination for Evolving Data Marketplaces](https://arxiv.org/abs/2605.23887)
*Joydeep Chandra*

Main category: cs.DB

TL;DR: 提出CHRONOS三层架构，统一处理时序知识图谱数据市场中的静态索引、Shapley定价和差分隐私预算消耗的耦合问题。


<details>
  <summary>Details</summary>
Motivation: 时序知识图谱数据市场面临三种耦合失败：陈旧混合索引导致召回下降、静态Shapley定价在分布偏移后估值失准、未协调的代理过度消耗共享差分隐私预算。

Method: 三层架构：第一层用神经ODE时间衰减处理索引，提供每次查询期望召回损失界；第二层基于变化点条件化Shapley估值，提供有限样本误差保证；第三层用EXP3-IX实现遗憾并满足ε,δ-差分隐私，通过矩会计计算隐私预算。

Result: 在四个基准测试中，召回率0.937（top-10），查询速度2.74 qps，延迟161 ms，总ε=4.25（δ=10^{-6}，zCDP组合）。

Conclusion: CHRONOS展示了竞争性操作点，但在此隐私水平下估值仍受噪声主导，实用性主要来自公共索引路由和低敏感度统计驱动的自适应调度。
...
### [228] [Generalized Stochastic Approximation of the Log-Likelihood Ratio for Robust Sequential Change-Point Detection](https://arxiv.org/abs/2605.23419)
*Serhii Zabolotnii*

Main category: stat.ME

TL;DR: 提出一种基于矩的似然比近似框架，用于非高斯随机过程中的序列变点检测，无需已知分布形式，适应小相对变点场景，并在极重尾数据上表现优异。


<details>
  <summary>Details</summary>
Motivation: 经典参数方法（如CUSUM）在分布失配时失去最优性，而非参数方法响应缓慢，亟需一种适用于非高斯过程且无需分布先验的鲁棒变点检测方法。

Method: 利用广义随机基（多项式、对数、分数幂）仅用前三阶矩近似对数似然比，将Kullback-Leibler散度投影到基张成空间得到收敛泛函，并采用Kunchenko概率误差界控制虚警率。

Result: 在九个公开基准数据集上，该方法在极重尾数据（峰度γ₄>20）中仍有效，而经典方法产生100%虚警，同时以可控虚警率减少了检测延迟。

Conclusion: 所提框架为非高斯过程提供了一种无需分布假设、鲁棒的变点检测方案，在极重尾场景下显著优于传统方法，且核心定理已在Lean 4中形式化验证。
...
### [229] [Sample correlation adjustments for robust Multi-fidelity Monte Carlo under limited pilot sampling](https://arxiv.org/abs/2605.23016)
*Michael Stanley,Thomas Coons,Geoffrey Bomarito,Patrick Leser,Joshua Pribe,James Warner*

Main category: stat.ME

TL;DR: 针对多保真蒙特卡洛（MFMC）中因有限pilot样本导致相关系数估计不准的问题，提出一种通过最小化最坏情况期望差异来改进相关系数估计的方法，在有限预算下获得更优的MFMC估计器。


<details>
  <summary>Details</summary>
Motivation: MFMC估计器的最优性依赖于已知的模型间相关系数，但实践中通常使用有限的pilot样本进行估计，导致估计不准确和估计器次优。

Method: 利用MFMC问题设置和样本协方差矩阵的概率信息，定义一种新的差异函数来量化估计器的次优性，并选择使最坏情况期望差异最小的相关系数估计器。

Result: 在二元高斯示例和NASA进入、下降与着陆（EDL）问题的多保真建模应用中，该方法在pilot样本量小和总预算有限时产生了优于标准样本协方差的MFMC估计器。

Conclusion: 本文方法在有限pilot样本下能有效提高MFMC估计器的性能。
...
### [230] [Mixture-of-Finite-Mixtures Wishart Model for Clustering Covariance Matrices with an Application to Brain Functional Connectivity](https://arxiv.org/abs/2605.23207)
*Zongyu Li,Stefano Castruccio,Zhiyong Zhang*

Main category: stat.ME

TL;DR: 提出MFM-Wishart贝叶斯模型，用于协方差矩阵数据的聚类，可同时推断聚类数量与分配。


<details>
  <summary>Details</summary>
Motivation: 协方差矩阵数据在多个领域出现，需要有效的聚类方法，且希望自动确定聚类数量。

Method: 结合Wishart混合成分与有限混合先验，开发高效MCMC算法进行后验推断。

Result: 证明后验一致性；模拟研究显示准确恢复聚类数量，即使模型误设；应用于婴儿功能连接数据。

Conclusion: MFM-Wishart聚类性能良好，提供可解释的异质性。
...
### [231] [A Direct Variance Estimation (DiVE) for Meta-Analysis of Median Differences](https://arxiv.org/abs/2605.23208)
*Tadahisa Okuda,Masataka Taguri,Kenichi Hayashi*

Main category: stat.ME

TL;DR: 提出直接方差估计（DiVE）方法，仅用中位数差异和样本量即可估计合并差异的方差，优于传统方法，尤其在小样本研究中。


<details>
  <summary>Details</summary>
Motivation: 传统元分析常因缺少离散度统计量而排除研究，导致证据综合不完整且有偏。

Method: DiVE直接利用各研究的中位数差异和样本量估计合并差异的方差，无需参数假设或离散度信息。

Result: 模拟表明DiVE在多种分布下表现相当或更优，小样本时优势明显；重新分析显示其能纳入更多研究，减少偏倚。

Conclusion: DiVE可促进更全面、偏差更小的元分析，尤其适用于离散度信息缺失的情况。
...
### [235] [Joint Estimation of Marginal and Heterogeneous Treatment Effects](https://arxiv.org/abs/2605.23691)
*Leticia Wuethrich,Torsten Hothorn*

Main category: stat.ME

TL;DR: 提出一种扩展的非参数正态调整边际推断方法，在保持边际可解释性的同时处理异质性治疗效果，适用于多种结局类型，并通过模拟和实例验证了其无偏性和效率提升。


<details>
  <summary>Details</summary>
Motivation: 随机临床试验中，协变量调整可能改变非线性模型的估计目标（从边际到条件效应），而同时识别预后和预测协变量对理解异质性很重要。现有方法难以同时保持边际可解释性、提高效率和评估异质性。

Method: 扩展非参数正态调整边际推断，将边际治疗效果直接嵌入结果与基线协变量的联合模型中，保留边际可解释性的同时对预后和预测协变量进行调整，适用于连续、二分类、有序和时间-事件结局，并能以共同尺度估计和排序协变量。

Result: 对于连续结局（Cohen's d），协变量调整下的渐近方差从不差于未调整，通常更好；效率提升主要由预后效应驱动，预测效应贡献很小。模拟研究针对Cohen's d、log-odds ratio和log-hazard ratio证实了无偏和更有效的边际效应估计。

Conclusion: 该方法在保持边际估计可解释性的同时提高效率，并允许对预后和预测协变量进行排序，应用于针灸试验复现了原始结果并提升了效率。
...
### [236] [Global Sensitivity Analysis: a novel generation of mighty estimators based on rank statistics](https://arxiv.org/abs/2605.23760)
*Fabrice Gamboa (IMT),Pierre Gremaud (NC State),Thierry Klein (IMT,ENAC),Agn\`es Lagnoux (IMT)*

Main category: stat.ME

TL;DR: 提出一种基于秩统计和Chatterjee相关系数的全局敏感性分析指标估计框架。


<details>
  <summary>Details</summary>
Motivation: 需要有效估计多种全局敏感性分析指标，尤其是在小样本情况下。

Method: 利用秩统计和Chatterjee经验相关系数，适用于Cramér-von-Mises指数、一阶Sobol指数、一般度量空间指数和高阶矩指数。

Result: 证明了估计量的一致性，展示了小样本下的数值效率，并给出了一阶Sobol指数估计量的中心极限定理。

Conclusion: 该框架为全局敏感性分析提供了一种通用且高效的统计估计方法。
...
### [237] [Joint Bayesian models for validating spatial health-event databases against a gold standard: separating global and local discrepancies](https://arxiv.org/abs/2605.23791)
*Mathias Brugel,Florine Kempf,Camille Ternynck,Marta Blangiardo,Micha\"el G\'enin*

Main category: stat.ME

TL;DR: 提出了一种贝叶斯框架用于空间健康事件数据库的全局和局部验证，通过误差模型比较候选数据库与金标准，应用于克罗恩病数据发现候选数据库总体信号低约7%。


<details>
  <summary>Details</summary>
Motivation: 缺乏现有工具来空间验证候选复用数据库与金标准的一致性。

Method: 采用贝叶斯框架，包括随机误差模型（REM）、结构化误差模型（SEM）和共享成分模型（SCM），通过全局偏离度（RR_global）和局部超出概率评估不一致性。

Result: RR_global准确恢复地图整体偏移；REM和SEM对局部差异敏感且特异；SCM更保守；实际应用中所有模型一致认为候选数据库再现了空间结构但信号低约7%。

Conclusion: 该框架有效，未来需扩展到其他分布、时空模型和校准。

...
TL;DR: 提出PolyFact数据集，比较CPT、SFT和GRPO方法，发现GRPO能有效改善大型语言模型的跨语言事实一致性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型主要针对英语训练，在其他语言中常出现事实不一致的问题。

Method: 构建包含12种语言10万条事实的PolyFact数据集，在Qwen-2.5-7B和OLMo-2-1124-7B上比较持续预训练、监督微调和基于GRPO的强化学习。

Result: GRPO优于SFT，提高了跨语言一致性和对未见语言的泛化能力；CPT增益有限。

Conclusion: GRPO通过减少语言专化、促进共享表示来改善多语言路由，代码、模型和数据集已开源。
...
TL;DR: 研究揭示了大语言模型在个性化任务中，使用合成数据与真实人类数据之间存在显著性能差距，并提出了改进方法。


<details>
  <summary>Details</summary>
Motivation: 当前评估LLM个性化能力大多依赖合成数据，缺乏对真实用户场景有效性的了解，因此需要研究合成与人类数据间的差距。

Method: 收集550个真实对话和大量人类判断（提取属性5949个、属性配对11919个、生成回复1101个），并设计轻量级训练干预用于前两个阶段。

Result: 模型在真实数据上表现不佳：提取属性困难，与人类判断不一致，生成的个性化回复不被人类认为优于通用回复，尽管LLM自我评估较高。训练干预改善了前两个阶段，但第三阶段奖励模型与人类评级相关性弱。

Conclusion: 人类数据揭示了系统的根本局限，现有方法难以直接建模符合人类标准的个性化质量判断，所收集数据为未来研究提供了基础。
...
TL;DR: 提出UnpredictaBench评估，用于测试LLM捕捉真实分布的能力，发现简单分布采样仍有挑战。


<details>
  <summary>Details</summary>
Motivation: LLM常被用于替代其他实体（如经济模拟），但许多模型倾向于单一答案，无法捕捉真实系统的不可预测性，现有输出多样性改进不足。

Method: 构建448个问题（统计分布、随机程序、自然语言场景），使用KS@N指标（Kolmogorov-Smirnov检验）量化模型输出与目标分布的匹配度。

Result: 模型在KS@100得分从接近0%到超过20%，无模型超过40%，添加推理可略微提升但无法根本解决。

Conclusion: 简单分布模拟仍具挑战性，UnpredictaBench是使用LLM替代复杂系统的必要第一步。
...
TL;DR: 语言模型推理失败可通过令牌级不确定性信号区分为承诺失败和持续不确定性两种模式，在23个配置中20个预测成立，并对自一致性策略有指导意义。


<details>
  <summary>Details</summary>
Motivation: 理解语言模型推理失败的可检测性，以便适应性地调整检测策略。

Method: 使用令牌级不确定性信号分析推理轨迹，识别承诺失败中的承诺点和持续不确定性中的累积模式。

Result: 在23个模型-数据集配置中，框架的预测在20个中成立；承诺失败在承诺点后额外令牌有害，持续不确定性需完整轨迹。

Conclusion: 该框架为理解LLM推理失败何时可检测提供了基础，并指导了检测策略的适应，如自一致性何时使用或跳过。
...
*Jakub B\k{a}ba,Jaros{\l}aw Chudziak*

Main category: cs.CL

TL;DR: 提出CAF-Gen多代理框架，通过迭代创建-审查流水线将浅层论证结构自动丰富为CAF合规模型，提高结构完整性和质量。


<details>
  <summary>Details</summary>
Motivation: 当前论证挖掘技术难以捕捉Carneades框架等高级模式所需的丰富结构信息。

Method: 采用创作者-评审者迭代流水线，创作者生成输出，评审者验证结构完整性，多代理协作缓解单次生成的结构不稳定。

Result: 迭代反馈循环提高了数据质量，与原始注释对齐，生成结构更丰富的模型。

Conclusion: 多代理系统克服了单次生成的局限性，为自动建模形式论证提供了稳健方法。
...
TL;DR: 提出搭便车假说：聊天模板前缀令牌将微调行为泛化到域外查询，通过正则化前缀表示可减少突发性错位。


<details>
  <summary>Details</summary>
Motivation: 解释LLM超出训练样本的过度泛化机制，以突发性错位为例进行探究。

Method: 提出Token正则化微调（TReFT），在训练中正则化特定令牌表示以缓解突发性错位。

Result: TReFT在多个模型和数据集上减少突发性错位，同时保持域内学习效果；在合法域微调中比数据交错多减少33.5%错位，平均减少54.3%离题泛化。

Conclusion: LLM可能以意想不到的方式学习和泛化，需要更受约束的微调，并呼吁进一步研究共享输入特征如何搭便车行为。
...
TL;DR: 通过分析75个国家1500条开放性回复，发现人们对AI的偏好多样且冲突，当前RLHF方法存在根本性问题。


<details>
  <summary>Details</summary>
Motivation: 揭示当前基于人类反馈的强化学习（RLHF）在对齐AI系统时存在的局限性，如聚合冲突偏好、样本不具代表性、仅使用二元比较。

Method: 分析PRISM数据集中来自75个国家的1500条开放性回复，考察人们真正希望AI具备的特性和能力。

Result: 不同人期望不同；最多被要求的“真实性”仅49%提及，且定义各异；某些能力（如类人行为）和功能（如AI护栏）存在争议；人们常使用默认与请求的语境区分，二元比较无法捕捉。

Conclusion: 当前对齐实践将情境化、有争议、不完美的信号压平为通用偏好模型，导致无法识别真实偏好，存在认知暴力问题。
...
TL;DR: 首个专家标注的香港判决书语篇数据集HKJudge，包含约29万句子、650万词，标注了26种修辞角色和三种量刑要素，并提供了BERT和LLM的基准评测。


<details>
  <summary>Details</summary>
Motivation: 香港判决书的语篇分析因缺乏专家标注语料库而受限。

Method: 构建了HKJudge数据集，采用双层语篇模式：句子级标注26种修辞角色，跨度级标注三种量刑要素（指控、监禁期、罚款）。使用10位法律语言学专家，一致性κ=0.8。提出了修辞角色分类和法律要素抽取任务，并在BERT、开源LLM、商业LLM上进行了基准测试。

Result: BERT模型和LLM在两项任务上展现出不同性能，证明了句子级语篇标注对建模香港判决书结构的价值。

Conclusion: 工作为判决书结构建模提供了数据基础，并有助于未来法律判决预测研究。
...
TL;DR: 提出信号驱动观察（SDO）方法，将观察频率与动作频率解耦，仅在必要信号触发时重新读取DOM，以减轻上下文退化问题。


<details>
  <summary>Details</summary>
Motivation: Web代理在长任务中每次动作都读取完整DOM，导致上下文逐渐退化，影响推理能力。

Method: 采用专用子调用读取完整DOM但只返回任务相关元素及其选择器，并由轻量级信号检测器控制重新调用的时机。

Result: 未给出具体实验结果，但理论上可减少上下文污染，提升长任务性能。

Conclusion: 观察压缩是Web代理设计中的核心架构决策，呼吁社区重视并解决相关开放问题。
...
TL;DR: 提出在线策略扩散语言模型（OPDLM），通过在线策略蒸馏将自回归模型转化为扩散语言模型，在减少训练数据的同时保持强性能。


<details>
  <summary>Details</summary>
Motivation: 解决自回归模型转化为扩散语言模型时的两种分布偏移：目标函数偏移和训练-推理不匹配。

Method: 采用在线策略蒸馏，学生模型（双向注意力）生成轨迹，冻结的教师模型（原始自回归模型）提供目标logits进行知识蒸馏。

Result: 相比基线，仅需15到7000分之一的训练数据，在多种任务上表现优异。

Conclusion: OPDLM避免了扩散模型高成本预训练，将转化定位为自回归模型的后训练步骤。
...
TL;DR: 研究主题情感对政治意识形态感知的因果效应，发现微调LLM会内化虚假情感-意识形态关联，导致显著因果效应，而人类判断无此效应。


<details>
  <summary>Details</summary>
Motivation: 探究主题情感是否因果影响政治意识形态感知，以及标注者类型的影响。

Method: 使用AllSides文章和LLM情感标注，比较人工、GPT-4o-mini（基线/微调）、Llama-3.3-70B的意识形态标签；应用双机器学习和社区级中介分析。

Result: 人工标注无显著因果效应；微调GPT-4o-mini准确率最高（F1=72.48），且是唯一产生显著社区级效应和NDE的范式。

Conclusion: 微调导致LLM学习虚假情感-意识形态关联，这种关联在F1评估中不可见，影响LLM标注作为因果分析代理的可靠性。
...
TL;DR: 提出了一种模块化方法来适应低资源语言的语言模型，通过替换词元、冻结嵌入并微调模型其余部分，在盖尔语、爱尔兰语和克丘亚语上提升了NLU任务性能。


<details>
  <summary>Details</summary>
Motivation: 完全微调整个模型用于低资源语言可能不必要，希望更模块化的方法。

Method: 替换词元，冻结对应嵌入，调整模型其余部分。

Result: 在掩码填充、NER和POS任务上性能提升。

Conclusion: 模块化方法有效，且提供了训练策略、预训练嵌入和模型选择的全面分析。
...
TL;DR: 提出IDPR框架，通过应答条件性抑制推理，仅对少数需要慢推理的输入调用深度推理，提高效率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型推理时，对所有输入都使用慢推理计算昂贵且不必要，需要选择性调用。

Method: IDPR首先生成快速直觉答案，然后使用抑制控制器基于快速答案的置信度、logit边际等特征决定是否进行慢推理。

Result: 在数学推理测试集上，仅对8.20%示例调用慢推理，准确率从47.90%提升到48.92%，优于随机路由和置信度基线。

Conclusion: 应答条件性抑制能更好地识别需要慢推理的快速答案，提高效率与准确性。
...
TL;DR: 提出证据图一致性（EGC）框架，通过构建局部证据图并计算结构一致性来检测RAG中的幻觉，但发现该指标在不同模型族中效果相反，无法作为模型无关的检测信号。


<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测方法仅基于生成答案与检索文本的平面相似性，忽略了证据间的结构关系。

Method: 构建每个回答的局部证据图，计算五种结构一致性度量作为幻觉指标。

Result: 在6个LLM的5767个回答上评估，EGC在Llama-2中有效，但在GPT-4等模型中诊断方向逆转。

Conclusion: 基于嵌入的图一致性不能作为模型无关的幻觉检测信号，不同模型族存在不同幻觉模式。
...
TL;DR: 提出PromptPrint，验证了用户提示中存在的可识别、稳定的行为生物特征，基于20,680条真实提示实现了强识别性能。


<details>
  <summary>Details</summary>
Motivation: 用户与大型语言模型的交互多为简短的任务驱动型提示，传统作者归因方法适用于长文本，但短提示中是否存在稳定的作者身份信号尚不清楚。

Method: 收集1,034名用户的20,680条真实提示，采用词汇表征（词袋等）、语义编码器（如BERT）及风格特征（句法、话语模式）进行对比实验，并进行对抗性分析（词汇扰动与语义改写）。

Result: 词汇表征显著优于语义编码器；风格特征表现出独特性与一致性的矛盾；身份信号对词汇扰动鲁棒，但被语义改写大幅削弱；整体识别性能强。

Conclusion: 提示中的词汇选择构成可学习的生物特征，为LLM交互中的用户建模提供了新视角，对安全与隐私有重要影响。
...
TL;DR: 提出四条件证据可用性协议（ONCU）诊断长上下文或检索增强语言模型的证据利用，并分离不同阶段的利用率。


<details>
  <summary>Details</summary>
Motivation: 最终答案准确率、检索召回率和引用重叠无法判断模型是否真正使用了证据，因为模型可能依赖参数记忆，或即使收到正确段落仍失败，或引用证据但未转化为答案。

Method: 设计无证据、全上下文、检索证据和oracle证据参考四种条件，在固定设置下诊断证据利用；使用ONCU作为oracle参考证据优势估计器，并分别报告分母无关的答案、证据、检索和失败审计指标。

Result: 在五个本地开放权重模型上评估18,000个ONCU兼容预测，发现受控合成设置主要暴露全上下文利用失败，真实多跳设置主要暴露检索链覆盖失败。

Conclusion: 贡献是一个诊断协议，用于分离无证据可回答性、oracle证据可恢复性、全上下文利用和检索条件利用，而非单一分数排行榜。
...
TL;DR: 高准确率并不保证LLM成为忠实编码者，尤其是在政治事件编码中。将编码手册转化为LLM友好形式能提升分类性能，但预测增益并不完全转化为行为可靠性。


<details>
  <summary>Details</summary>
Motivation: 社会科学研究依赖专家编码手册将文本转化为结构化数据，而LLM的高准确率可能掩盖其编码逻辑的不忠实。

Method: 将专家编码手册操作化为更清晰的定义、示例、检索上下文和困难案例规则，并测试标签名称、手册顺序和标签定义映射等受控变化下的行为可靠性。

Result: 更清晰的手册显著提升分类性能，尤其在细粒度事件分类中，但预测增益并不完全体现为行为可靠性。

Conclusion: 编码手册引导的LLM系统不仅应通过准确性评估，还应评估是否保留了编码逻辑，以确保编码输出对社会科学研究有意义。
...
TL;DR: 提出一种评估LLMs在新界面下生成不同语言复杂性响应的框架，测试发现模型在调整复杂性上表现不一致。


<details>
  <summary>Details</summary>
Motivation: 现有评估假设静态聊天界面，但模型集成到新界面时需要纳入界面特定标准。

Method: 基于16人形成性研究，设计语言复杂性可解释轴，对98个科学查询生成5级复杂性响应，评估GPT-5.1等模型。

Result: 最佳模型Claude Sonnet 4.5仅46%时间正确调整复杂性，多数模型变化不一致。

Conclusion: 当前模型在按界面要求灵活调整语言复杂性方面能力有限，需要改进。
...
TL;DR: 提出TA-RAG框架，通过提示式语气控制（无模型微调）使RAG输出适用于敏感健康同伴支持场景，在HIV同伴支持中提升无污名化、可读性、接收者适应性和共情表达。


<details>
  <summary>Details</summary>
Motivation: 标准RAG仅保证事实准确性，但敏感的健康同伴支持（如HIV）还要求回应具有无污名、可读、共情和个性化，现有方法无法满足。

Method: TA-RAG是轻量级、基于提示的语气感知RAG框架，包含四个组件：无污名重写、可读性调整、接收者适应和共情改写，无需模型微调。

Result: 在HIV在线学习、UNAIDS术语、可读性指标、同伴支持标准和共情数据集上的组件级测试表明，各组件在保持关键内容的同时提升了目标沟通质量。

Conclusion: 提示式语气控制是使RAG输出适用于敏感健康同伴支持的一个有潜力的方向。
...
TL;DR: 针对大语言模型的文化适应性，提出构建性定义而非仅抑制输出，并针对韩语设计对齐数据管道，通过DPO微调提升韩语文化安全率且不损害通用能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注抑制不当输出，缺乏对文化一致响应的正面定义，需要为韩语建立文化适应的安全响应框架。

Method: 基于提示的LLM种子生成器扩展韩语伤害分类，结合韩国法律、社会规范和解释惯例设计安全响应策略，由三个前沿模型生成候选响应，并使用DPO对三元组进行微调。

Result: 在六个开源LLM上，韩语文化安全率提升，且韩语通用能力评测未显著下降；模型能引用韩国法律条文和机构程序，提供建设性上下文信息。
...
TL;DR: 对加拿大警察涉及死亡事件的新闻报道进行大规模计算分析，发现官僚视角占主导，但近年来平民视角有所增加，且两者叙述风格差异显著。


<details>
  <summary>Details</summary>
Motivation: 探索媒体如何构建警察涉及死亡事件的叙事，以及不同社会群体视角的呈现差异。

Method: 开发PerspectiveGap计算模型，分析4000篇新闻报道中官僚与平民视角的频率和情感表达。

Result: 官僚视角出现频率是平民的近三倍，部分文章完全缺乏平民观点，但近年来平民代表比例上升；官僚叙述客观程序化，平民叙述情感化。

Conclusion: PerspectiveGap框架可迁移至其他地区，为分析媒体系统在警务问责叙事中的作用提供可扩展的方法。
...
TL;DR: 提出Progress-SQL多轮强化学习框架，通过渐进奖励和Oracle-guided Diagnostic Tree提升Text-to-SQL性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅优化单次SQL奖励，无法有效引导多轮修正和捕获逐步改进。

Method: 构建Oracle-guided Diagnostic Tree抽象SQL结构并生成诊断反馈；结合结构对齐与词汇对齐定义渐进奖励，加入进度延迟奖励和执状态奖励。

Result: 在BIRD、Spider及其鲁棒性变体上一致提升Text-to-SQL性能。

Conclusion: 多轮渐进奖励能有效提升SQL生成质量和鲁棒性。
...
TL;DR: 通过残差化和置换诊断，区分了序列基础模型在暗基因组调控元件分析中的预测驱动和调控驱动方差，发现10kb近端调控边界，并验证了脑eQTL富集，但转座子层和蛋白对收敛未通过检验。


<details>
  <summary>Details</summary>
Motivation: 高级胶质瘤通过突触整合到神经回路，需解析暗基因组中塑造突触形成基因表达的非编码调控元件。

Method: 使用三个序列基础模型（Caduceus-Ph、HyenaDNA、Enformer）对92个胶质瘤相关位点的30,448个暗基因组元件进行计算机模拟诱变，引入残差化和置换诊断分离预测与调控方差，并通过保守性、脑cis-eQTL和STRING-PPI进行验证。

Result: 发现10kb近端调控边界；LM衍生元件类层次不成立；跨架构分解显示序列可预测性与调控输出分离；前100元件在脑eQTL中富集3.3倍，但转座子层和NRXN1+NLGN1收敛未通过置换检验。

Conclusion: 残差化和置换诊断可作为ISM调控研究的通用工具，揭示了基于LLM的调控推断需谨慎。
...
TL;DR: 提出一种基于奖励学习的门控策略，动态决定是否翻译输入，以在多种语言上平衡性能与成本。


<details>
  <summary>Details</summary>
Motivation: LLM在不同语言上性能差异大，翻译可弥补但盲目翻译浪费资源，而模型自主选择又过于自信，需自动化决策。

Method: 通过答案保持的翻译管道构建数据，在Qwen3-4B上对22种语言进行强化学习，引入置信度门控的GSPO进行成本敏感的工具使用训练。

Result: 门控策略在High、Low、XLow三种资源层级上分别提升奖励+4.6、+23.5、+17.5；在63%的成本下保持完整奖励，并在87%的成本敏感区间内帕累托最优。

Conclusion: 该方法能自适应评估自身理解能力，仅在必要时调用翻译，且零样本迁移到未见语言，有效提升多语言处理效率。
...
TL;DR: 现代推理模型在多标签零样本任务中表现强劲，其机制可分解为广泛候选列表和细粒度推理两个阶段，基于此提出的机械蒸馏策略优于标准蒸馏。


<details>
  <summary>Details</summary>
Motivation: 探索推理模型在多标签任务中零样本性能强的机械原理。

Method: 将推理分解为广泛候选列表和细粒度推理两个互补阶段，并基于此设计机械蒸馏策略。

Result: 提出的机械蒸馏策略在多个数据集上一致优于标准蒸馏。

Conclusion: 推理模型的两阶段机制可被隔离利用，改进蒸馏方法。
...
TL;DR: 提出统一的反事实推理框架CRAFT，将表格问答和事实验证转化为双向验证过程，通过构建声明及其反事实变体并加权整合证据，显著提升表格推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖单向推理，难以探索替代假设，限制了多步推理能力。

Method: 构建原始声明和反事实变体，沿两条路径提取证据，通过加权机制融合得到最终答案。

Result: 在WikiTQ和TabFact数据集上持续超越基线，尤其在复杂问答任务上提升显著，并大幅缩小不同骨干LLM的性能差距。

Conclusion: 反事实推理有效克服单向推理局限，引导LLM更审慎推理，为结构化推理任务建立了更规范的方法范式。
...
TL;DR: 本文提出增强稀疏编码模型，利用稀疏自编码器特征和惊异度预测器，通过7T fMRI数据验证了模型可解释脑对语言处理的响应，发现脑响应最佳由LM中最通用的特征解释。


<details>
  <summary>Details</summary>
Motivation: 认知神经科学需要刻画语言皮层表征的特征，但人工语言模型被视为黑箱，需要更可解释的建模方法。

Method: 使用增强稀疏编码模型，将密集LM隐藏状态替换为分层组织的稀疏自编码器特征，并显式包含惊异度作为预测器，基于8名受试者听200句句子的7T fMRI数据进行验证。

Result: 恢复了先前对处理难度和意义抽象性调谐的体素群体的解释，发现了新的人相关内容调谐的体素群体；额叶-颞叶语言网络由共同特征预测，但额叶区域仅由惊异度充分解释；脑响应最佳由LM中最通用的特征解释，而非任意特征集。

Conclusion: 脑与语言模型之间存在非平凡对应关系，脑响应最佳由LM中捕获最通用信息的特征解释。
...
TL;DR: 综述了大型语言模型（LLM）在图计算中的角色，提出执行器和规划器两种范式，指出LLM在小规模简单任务中表现良好，但在大规模高精度任务中不可靠。


<details>
  <summary>Details</summary>
Motivation: 现有综述主要关注图学习、文本属性图或图语言建模，缺乏对LLM在图计算中可靠性及集成方式的深入分析。

Method: 通过基于角色的分类法，识别LLM作为执行器（直接解决图任务）和规划器（分解推理步骤并调用外部工具）两种范式，并分析其优缺点。

Result: LLM在简单小规模任务中表现有潜力，但在大规模需精确性的任务中仍不可靠。
...
TL;DR: 通过扩展数据集COVA-X，Longformer超越XGBoost，验证了transformer模型需要更大的会话数据才能发挥优势。


<details>
  <summary>Details</summary>
Motivation: 之前COVA数据集中transformer模型因输入截断和数据不足而表现不佳，需要扩展数据集并改进生成流程。

Method: 改进数据生成管道，解决标签污染、不匹配等问题，创建包含10,985个对话的COVA-X数据集，并重新训练所有分类器。

Result: Longformer准确率79.71%、宏F1 0.7786，优于XGBoost的78.43%和0.7563；标签纠正率提升12.7倍，虚拟绑架伪影率从67.1%降至46.5%。

Conclusion: Transformer模型需要更大的会话语料库来利用上下文优势，数据集精炼可恢复真实标签信号。
...
TL;DR: 提出EASE-TTT，一种基于证据对齐的选择性测试时训练框架，通过在长上下文中对齐证据块的注意力监督来提升小模型的长文本问答性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么仅暴露证据而不调整注意力，要么缺乏证据定位能力，导致小模型在长上下文问答中表现不佳。

Method: EASE-TTT将选中的证据块转换为软注意力监督目标，指导查询侧参数调整，使模型在原始全上下文中生成答案。

Result: 在六个LongBench任务和三个小模型上，EASE-TTT取得了最强平均性能，优于全上下文推理、仅检索基线和qTTT。

Conclusion: 证据对齐的测试时自适应可以有效提升小模型的长上下文问答能力。
...
*Vladislav Smirnov (1),Chieu Nguyen (1),Sergey Senichev (7),Minh Ngoc Ta (1),Ekaterina Fadeeva (2),Artem Vazhentsev (1),Daria Galimzianova (1),Nikolai Rozanov (1,3),Viktor Mazanov (6),Jingwei Ni (2),Tianyi Wu (4),Igor Kiselev (5),Mrinmaya Sachan (2),Iryna Gurevych (1),Preslav Nakov (1),Timothy Baldwin (1),Artem Shelmanov (1) ((1) MBZUAI,(2) ETH Z\"urich,(3) Imperial College London,(4) NUS,(5) Accenture,(6) Innopolis University,(7) Independent Researcher)*

Main category: cs.CL

TL;DR: ThinkBooster是一个统一的测试时计算扩展框架，用于提高LLM推理能力，包含模块化库、基准测试、可部署代理和调试器。


<details>
  <summary>Details</summary>
Motivation: 现有TTC扩展策略和推理评分器碎片化，缺乏统一评估和成本效益分析。

Method: 构建ThinkBooster框架，包括：(i) 模块化Python库实现多种TTC策略和评分器；(ii) 同时评估性能和计算效率的基准；(iii) 兼容OpenAI的代理服务；(iv) 可视化调试器。

Result: 在数学和编码任务上揭示了TTC策略和评分方法的性能-计算权衡，并在实际任务中取得实际增益。

Conclusion: ThinkBooster为LLM推理提供了一种统一的、实际可用的测试时计算扩展方案。
...
TL;DR: Didact是一个原型系统，整合澳大利亚国防报告与科研知识图谱，通过检索增强生成和交互式证据轨实现政策导向的自然语言对话。


<details>
  <summary>Details</summary>
Motivation: 国防决策者需要监控分散、格式异构的碎片化信息，现有方式发现能力缓慢且难以审计。

Method: 集成公开国防报告与澳大利亚科研知识图谱，构建复合检索增强生成管道，并设计交互式证据轨可视化证据与关系。

Result: 评估表明Didact在输出质量和运行时效率上具有实用性，且可适配其他领域。

Conclusion: Didact作为学术界-产业界合作项目，适用于知识碎片化场景，并已提供演示视频。
...
TL;DR: LoRA-MINT是一种针对LoRA微调的大语言模型进行成员推理测试的新方法，通过分析困惑度与成员状态的关系，以高精度判断数据是否用于训练，并展示了其鲁棒性和通用性。


<details>
  <summary>Details</summary>
Motivation: 评估个体样本是否属于LoRA微调模型的训练数据，为知识产权和敏感数据管理提供审计工具。

Method: 基于模型困惑度与成员状态的关系，构建系统框架来估计数据暴露程度。在四个模型和三个基准数据集上进行实验。

Result: 判断数据是否用于训练的精确度在0.77到0.92之间，优于现有基线方法，证明了方法的鲁棒性和通用性。

Conclusion: LoRA-MINT是一种有效且可扩展的LLM审计框架，有助于提升透明度和促进AI伦理部署，且方法可推广到其他领域自适应AI模型。
...
TL;DR: OpenHalDet是一个统一的幻觉检测基准，标准化了从提示构建到指标计算的评估流程，支持多种检测方法，便于比较和复现。


<details>
  <summary>Details</summary>
Motivation: 现有评估面临推理配置不一致、下游领域和任务覆盖有限的问题，导致检测器性能难以比较、复现和泛化。

Method: 标准化评估流程，包括提示构建、响应生成、真实性标注、检测器评分和指标计算，并支持黑箱、灰箱和白箱检测方法。

Result: 将不同任务、模型和检测器纳入统一框架，实现可控比较，系统展示不同检测范式在大语言模型应用中的行为。

Conclusion: 发布开源可扩展代码库，促进可复现评估和未来幻觉检测方法的发展。
...
TL;DR: 针对隐式奖励环境下的低重复任务，提出结构化经验管理方法Tree-of-Experience (ToE)，在金融情感预测基准上超越通用方法。


<details>
  <summary>Details</summary>
Motivation: 现有经验进化基准假设明确目标与反馈，难以处理低重复、隐式奖励任务。

Method: 构建FinEvolveBench基准和Tree-of-Experience (ToE)方法，对代理经验进行组织、检索、验证和更新。

Result: 通用经验机制不持续优于无经验基线，ToE整体性能更强。

Conclusion: 结构化经验管理对隐式奖励环境下自我进化代理至关重要。
...
TL;DR: 提出了一种关注兴趣点（POI）的对比训练框架，通过增强代码切换关键区域的识别来提升ASR性能。


<details>
  <summary>Details</summary>
Motivation: 代码切换ASR仍然具有挑战性。

Method: 采用文献中的POI检测方法识别CS片段，通过扰动ASR N-best输出中的POI并利用大语言模型扩展候选来构建声学上合理的近似错误假设，经过声学、音位和文本约束过滤保留困难但合理的负样本，最后使用POI加权交叉熵锚点损失和多负样本对比排序损失，结合LoRA微调Whisper-small。

Result: 在CS-FLEURS（cmn-eng）和ViMedCSS（vie-eng）上，相比标准LoRA微调，通用错误率和CS感知错误率均持续降低超过2%。
...
TL;DR: 通过表征组合性视角，研究了句子编码器产生良好概念表示的条件，识别出四个原则并发布了两个新数据集。


<details>
  <summary>Details</summary>
Motivation: 探究句子编码器能够产生良好概念表示的决定因素。

Method: 基于表征组合性框架，通过控制消融实验，在330万同义词和定义对（来自WordNet和Wiktionary）上训练编码器，并在三个去污染分割和修饰符标记名词短语基准上评估。

Result: 发现了四个原则：微调重新校准潜在几何而非扩展（P1）；语义信号在概念特异性训练前集中在最后一层，使得跨层池化冗余（P2）；难负样本改善区分性和鲁棒性但不改善检索排序（P3）；监督有效性取决于目标概念的组合类型，外延训练有利于交集和子集类但损害关系和内蕴类（P4）。同时发布了两个新评估数据集。
...
TL;DR: MADE是一个多语言智能诊断引擎，通过分解分析流程和专家引导的诊断集，显著提升了后评估报告的质量，并发现了可操作的模型部署和跨文化缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试分数丰富但缺乏洞察，需要细粒度诊断，但单个LLM和开放代理难以处理长且嘈杂的诊断输入，且缺乏可复用的分类法。

Method: 将后评估分析分解为规划、聚合分析、实例级检查、多语言和文化反思以及报告综合，并配套54个查询、15种语言的专家引导诊断集。

Result: 在33个模型家族、11个基准、26种语言、34种文化、866万条评估记录上，MADE的报告质量比最强基线高47%，在87.9%的成对比较中被人类专家偏好。

Conclusion: MADE将基准分数转化为模型选择和修正指导，与专家合作发现了四个关于部署、迭代和跨文化陷阱的可操作发现。
...
TL;DR: 一种通过探索生成可复用评估技能的奖励建模方法，无需每查询生成标准。


<details>
  <summary>Details</summary>
Motivation: 现有基于评分标准的方法需要在线生成准则，增加了推理开销且可能产生僵化指导。

Method: Eval-Skill通过两个阶段（工作流生成和原则生成）合成可复用的领域级评估技能，并在两阶段中交织探索和选择。

Result: 在多个基准上显著提升，例如RewardBench 2上Qwen3-8B提升13.44%，DeepSeek-V4-Flash提升18.51%。

Conclusion: 紧凑的评估技能为基于LLM的评估提供了高效的新范式。
...
TL;DR: 提出TRACE框架，通过TIJ循环检测长程LLM智能体轨迹中的隐藏恶意目标，在多个任务上优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有方法在检测长程隐藏恶意行为时，难以连接时间上分散的证据。

Method: TRACE采用TIJ循环：Triage识别高信号区域，Inspect进行针对性检查并累积证据，Judge合成轨迹级判决。

Result: 在SHADE-Arena十个任务域上，F1=0.713，召回=0.844，长程证据关联任务提升最大。

Conclusion: TRACE有效提升长程隐藏恶意行为检测能力。
...
*Sara M{\o}ller {\O}stergaard,Kenneth Enevoldsen,Afra Alishahi,Bruno Nicenboim*

Main category: cs.CL

TL;DR: 研究通过十种不同嵌入实现评估语义关联，发现嵌入模型选择影响N400和阅读时间，句子嵌入在预测之外显示出可靠效果。


<details>
  <summary>Details</summary>
Motivation: 语义关联对阅读理解重要，但嵌入方法多样，需要系统比较。

Method: 使用语言模型嵌入计算十种语义关联，结合脑电图和自定步速阅读数据，采用贝叶斯层次模型分析。

Result: 嵌入模型选择改变效应估计；仅句子嵌入在单词预测性之外对神经和行为测量有可靠影响。

Conclusion: 方法选择对量化语义关联至关重要，句子嵌入具有潜力。
...
TL;DR: 提出了一个基于PISA的多语言推理基准mmPISA-bench，包含25道选择题，覆盖43种语言的人类和机器翻译版本。评估两个主流LLM，结果显示现代LLM在多种语言上推理能力接近人类，机器翻译不降低准确率，但某些语言成本高且准确率低。


<details>
  <summary>Details</summary>
Motivation: 现有推理基准缺乏高质量的多语言覆盖，且翻译质量影响评估。需要构建一个可扩展、高质量的多语言推理基准来评估LLM的跨语言推理能力。

Method: 从OECD PISA中提取25道需要推理的选择题，提供43种语言的官方人类翻译和机器翻译，共2150个数据点。评估两个主流LLM在不同语言、推理难度和翻译类型下的正确率。

Result: LLM在所有语言上推理有效，准确率接近人类；机器翻译与人类翻译的准确率无显著差异；部分语言推理成本更高且准确率更低。

Conclusion: 高质量机器翻译足以用于大规模多语言推理评估，但需注意语言间的成本和性能差异。
...
TL;DR: 提出SigmaScale方法，通过学习缩放矩阵优化SVD压缩LLM，利用激活感知损失降低权重有效内在秩，在Llama和Qwen模型上取得有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 现有SVD压缩方法依赖解析推导缩放，缺乏灵活性，需要更适应模型权重结构的压缩方法。

Method: 优化两组向量定义对角行和列缩放变换，在激活感知压缩损失下学习辅助缩放矩阵S。

Result: 在Llama 3.1 8B Instruct和Qwen3-8B上，SigmaScale与最先进SVD方法在困惑度和零样本基准上竞争力相当。

Conclusion: SigmaScale通过激活感知学习缩放，提供更灵活的低秩压缩途径，适用于降低LLM推理计算成本的应用。
...
TL;DR: 提出控制内容重叠的方法，用于测量风格分类器对内容线索的依赖，并通过实验验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 风格分类器会利用与风格标签相关的内容线索，但缺乏系统测量这种依赖的方法。

Method: 在平行圣经翻译数据集上，定义重叠参数α为内容身份与风格标签互信息的归一化残差，控制内容共享程度；采用RoBERTa分类器进行交叉重叠评估和跨风格内容检索探测。

Result: 低重叠模型在去除内容线索时性能下降，高重叠模型鲁棒转移；内容可恢复性随α增加而降低。

Conclusion: 控制重叠提供了一个简单诊断方法，用于分离风格学习与内容捷径。
...
TL;DR: 语言中的社会含义具有视角性，现有NLP系统忽略这种多样性。本文通过融合文本和人口统计特征的嵌入模型，显著提升了社会含义预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有NLP系统将社会含义的多样化解释简化为单一标签，忽略了视角差异。

Method: 使用28k人类标注数据，评估零样本、少样本和微调范式，提出融合文本与人口统计特征的嵌入模型。

Result: 融合模型在宏平均PR-AUC上比文本基线提高5.9-6.5%，统计显著，且人口统计特征提供真实预测信号。

Conclusion: 考虑人口统计特征能有效捕捉社会含义的视角性，避免假相关。
...
*Anar Yeginbergen,Amelie W\"uhrl,Anna Rogers,Rodrigo Agerri*

Main category: cs.CL

TL;DR: FullCite框架通过三种策略生成结构化内联引用，评估显示LLM在识别相关文档方面有效，但在精确证据跨度识别上存在不足。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统广泛应用，对事实性和忠实性生成的需求增加，通过引用正确归因信息变得至关重要。

Method: 提出FullCite框架，包括基于提示的生成、基于引用语法的约束解码和后验跨度对齐三种策略。

Result: 在ASQA、BioASQ和ExpertQA基准上评估，LLM在文档级正确性上表现良好，但证据跨度识别和声明-引用忠实性方面存在差距。

Conclusion: 实现忠实归因问答需要更注重精确证据跨度识别的研究。
...
TL;DR: 提出了UrduMMLU，一个包含26,431道乌尔都语多选题的基准测试，覆盖26个学科和5个领域，用于评估大型语言模型在乌尔都语上的表现。


<details>
  <summary>Details</summary>
Motivation: 乌尔都语有超过2.3亿使用者，但缺乏基于本地教育资源的广泛MMLU风格基准。

Method: 从本土乌尔都语多选题库和公共考试PDF收集数据，对考试部分进行双重人工标注和严格共识过滤。

Result: Gemini-3.5-Flash准确率最高（90.20%和90.34%），其余模型均低于85%；最强开源模型落后约8个百分点；许多模型在人文学科上比STEM低25-40个百分点；少样本学习提升有限。

Conclusion: 当前LLMs对乌尔都语知识的掌握不均衡，尤其对地域性内容表现不佳。
...
TL;DR: 比较了监督向量嵌入（如CamemBERT）和词汇共现图在语义几何上的表现，发现局部拓扑相似但整体结构迥异，表明两者互补。


<details>
  <summary>Details</summary>
Motivation: 探究NLP模型中语义几何的底层结构，理解不同模型对语义关系的编码方式。

Method: 通过图结构和嵌入拓扑的对比分析，对法国“全国大辩论”语料库中的两种模型进行比较。

Result: 局部拓扑相似，但整体结构和拓扑差异显著。

Conclusion: 深度监督模型与图模型互补，为引导神经网络向更稳定可解释的收敛提供了新途径。
...
TL;DR: 提出了前缀效用模型（PUM），通过前缀对成功完成概率的提升（前缀增益）来训练模型，用于评估推理前缀的效用。


<details>
  <summary>Details</summary>
Motivation: 现有过程奖励模型通常基于局部步骤正确性评估前缀，但正确性只是成功完成的间接代理，直接目标是提高成功概率。

Method: 定义前缀增益为轻量级学生模型组在给定前缀条件下的求解率提升，使用成对排序目标训练PUM，学习基于结果的前缀效用。

Result: 在Best-of-N选择、束搜索和数学推理强化学习中，PUM提供强前缀级监督信号，尤其在大候选池、高搜索预算或稀疏奖励时效果显著。

Conclusion: PUM能有效评分完整轨迹和部分推理前缀，为推理过程提供更好的指导信号。
...
TL;DR: 通过对抗性方法建模恶意用户对真实用户的模仿，构建多语言跨平台数据集，训练出在真实世界数据中准确检测AI生成文本的模型。


<details>
  <summary>Details</summary>
Motivation: 现有AI内容检测模型因缺乏真实标注数据，在野外的泛化能力不足。

Method: 采用对抗性方法论，模拟恶意用户冒充真实社交媒体用户的行为，并据此策划一个多语言、跨平台的配对数据集（人类与AI生成消息）。

Result: 在该对抗性数据上训练的模型在真实世界、分布外数据上显著优于现有基于内容的机器人检测模型。

Conclusion: 对抗性数据生成策略能有效提升AI生成文本检测的鲁棒性和泛化能力。
...
TL;DR: 大型语言模型在临床应用中易受提示扰动影响，系统性分析显示医疗专用模型并不鲁棒，微小措辞变化可导致危险输出。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗任务中表现出对提示扰动的敏感性，但缺乏系统性的鲁棒性评估，尤其是在安全关键场景中。

Method: 使用MedMCQA基准，对通用和医疗专用LLM进行自然与对抗两类扰动的系统敏感性分析，评估一致性、准确性和可靠性。

Result: 医疗LLM并非内在地安全，微小短语变化可改变临床建议，对抗扰动可产生有害输出；模型对语法重排序和误导性上下文脆弱。

Conclusion: 医疗LLM在临床应用中不可靠，需要进一步改进以确保鲁棒性和安全性。
...
TL;DR: 通过语言标签提示、强化学习微调和词汇匹配改进跨语言语音克隆，语言提示效果最显著。


<details>
  <summary>Details</summary>
Motivation: 跨语言语音克隆在语音翻译中至关重要，但面临口音变化和领域词汇的挑战。

Method: 基于FishAudio-S2-Pro多语言TTS模型，引入语言标签提示减少口音泄漏，应用强化学习微调提高可懂度，并提出参考条件词汇匹配改进专业术语发音。

Result: 语言提示带来最大增益，词汇匹配在重叠子集上持续改进。

Conclusion: 综合方法有效提升跨语言语音克隆的智能性和自然度。
...
TL;DR: 提出了Phun-Bench，一个中文音韵理解基准，发现LLM在音韵知识灵活运用上不如人类。


<details>
  <summary>Details</summary>
Motivation: 现有基准要么靠死记硬背解决，要么与其他能力交织，无法衡量LLM的真实音韵理解能力。

Method: 构建了包含同音、押韵和语音相似性三个维度的中文基准Phun-Bench，系统评估LLM的音韵理解。

Result: LLM在回忆正确发音上表现出色，但在像人类一样灵活直观地运用音韵知识方面普遍困难。

Conclusion: 通过详细分析，提出了关于LLM音韵理解和“感知”的潜在机制的假设，为未来研究指明了方向。
...
TL;DR: 提出基于转向向量的机器生成文本检测方法，通过冻结语言模型隐藏表示提取方向特征，在分布偏移下表现优越。


<details>
  <summary>Details</summary>
Motivation: 机器生成文本检测在分布偏移（如跨领域、源模型和编辑攻击）下尤其困难，需要鲁棒的方法。

Method: 从冻结语言模型每层隐藏表示中构造区分人工与机器文本的方向，计算输入投影特征，再用轻量分类器训练。

Result: 在分布内和分布偏移（跨领域、源模型、编辑操作如润色和重写）下均取得强性能。

Conclusion: 假文本检测可视为表示空间探测问题，转向向量提供简单有效的解决方案。
...
TL;DR: 提出LLM引导的MAP-Elites进化方法，作为推理时的替代方案，用于发现医疗决策策略，在三个任务上均优于人工设计基线。


<details>
  <summary>Details</summary>
Motivation: 避免昂贵微调或繁琐的提示与流程工程，以推理时间进化搜索自动优化医疗决策策略。

Method: 将紧急分诊、交互式咨询和医学图像分类形式化为可执行工件的进化搜索，由任务特定适应度函数优化，并利用LLM引导MAP-Elites进化。

Result: 分诊中Semigran准确率从77.3%提升至87.1%，紧急召回率从0.60提升至0.97，且安全加权MIMIC-ESI性能提升；咨询中改进了准确率-成本前沿并迁移至iCRAFTMD；图像分类中保持严格JSON输出下提升冻结MedGemma VLM性能。

Conclusion: 进化带来的提升源于可解释的程序级机制，如校准的分诊边界、定向证据获取、选择性承诺和面向发现的视觉决策规则，而非简单的提示重写。
...
TL;DR: 提出M^3Exam基准测试和M^3Proctor记忆方法，评估多模态对话推理，并提升准确率13%同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试缺乏对真实多模态文件交互和隐式用户信息推理的评估。

Method: M^3Proctor通过检测查询模态偏差并按需消费原始视觉源，优化多模态记忆。

Result: 准确率提升13%，索引构建时间和检索token减少超过70%。

Conclusion: M^3Exam能有效评估多模态对话记忆，M^3Proctor显著提升效率与准确性。
...
TL;DR: 大型语言模型在回答文化相关问题时，本地语言可能比英语更有效地获取本地文化知识，但受限于语言能力，这种优势在原始准确率中常被掩盖。


<details>
  <summary>Details</summary>
Motivation: 现有评估存在两个关键局限：依赖平行模板问题可能不反映自然文化知识，且原始准确率混淆了通用语言能力与语言条件知识访问。

Method: 基于真实世界文化问题构建控制框架，通过交叉问题类型（文化无关vs文化特定）与查询语言（英语vs本地语言），并使用1PL项目反应理论模型分离能力与知识访问。

Result: 在13个地区和约80个模型中，英语在文化无关问题上表现更优（显示更强英语能力），但考虑能力差距后，本地语言在几乎所有地区-模型设置中显示出知识访问优势，尤其在前沿、区域对齐或语言适应模型中更明显。

Conclusion: 本地语言表现较弱并不一定意味着本地文化知识较弱；相反，本地文化知识可能通过本地语言更容易获取，但受限于语言能力而被隐藏。
...
TL;DR: 语言模型中的奉承性表扬是一个独立的对齐问题，现有方法无法可靠测量。我们提出了一个参数化框架，能有效测量表扬是否过度，并发现其在社交和解释性领域比客观推理领域更常见。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注语言模型的过度附和，但很少涉及明确表扬和奉承。我们认为奉承性表扬是一个独特的对齐问题，需要专门的测量方法。

Method: 设计参数化框架，通过比较表扬程度与贡献质量和用户预期能力来判断表扬是否过度。

Result: 该框架在人类标注一致性上显著优于通用LLM评判；奉承性表扬在社交和解释性领域出现频率远高于客观推理领域。

Conclusion: 表扬校准是一个独立于一般附和的对齐挑战。
...
TL;DR: 本研究探索了土耳其语习惯性轻动词结构（LVC）的检测问题，将其视为二分类任务，比较了监督模型与多种提示策略下的指令调优大型语言模型（LLM），发现少样本提示能显著提升性能但存在模型特定偏差，监督基线依然具有竞争力。


<details>
  <summary>Details</summary>
Motivation: 土耳其语习惯性轻动词结构在表达多词单位时具有挑战性，因为其表面形式与字面动宾组合相同，但功能上却是部分习语化的谓语。需要有效区分字面意义与习语意义。

Method: 将LVC检测作为二进制分类任务，使用人工构建的受控数据集（N=147），包含LVC正例、域外随机句子和域内字面控制作为负例。比较监督基线（BERTurk+分类头）与三种指令调优LLM在零样本、一样本和少样本提示下的表现，并分析示例如何改变错误分布。

Result: 零样本下LLM对负例表现良好但LVC召回率很低；一样本提示显著改善LVC检测但引入模型特定偏差；少样本提示改善校准，使GPT-OSS-20B和Qwen 2.5-14B获得稳健性能。提示敏感性显著；监督基线保持竞争力，但精心构建示例的LLM可以匹配或超越其LVC性能。

Conclusion: 土耳其语元语言分类对提示高度敏感，监督基线仍具竞争力，而通过少样本提示精心设计的示例可使LLM达到或超过监督模型在LVC上的表现。
...
TL;DR: 大型语言模型在零样本任务上表现出色，但作为嵌入模型时性能不佳，原因是文本嵌入过度表达高频无信息token。本文提出EmbedFilter，通过线性变换过滤高频token子空间，提升语义表示并实现降维，实验证明其优越性。


<details>
  <summary>Details</summary>
Motivation: 观察到文本嵌入在词汇空间投影时倾向于对齐高频但无信息的token，抑制了模型捕捉细微语义的能力。

Method: 利用LLM中的解嵌矩阵编码的潜在空间，通过线性变换EmbedFilter过滤掉生成高频token的子空间，从而改进嵌入。

Result: 多个LLM骨干上，EmbedFilter在显著降低嵌入维度的情况下，仍能取得更优的零样本下游性能，并降低索引存储和加速检索。

Conclusion: 该发现提供了对LLM表示机制的深入理解，有望启发更原则性的文本嵌入训练设计。
...
TL;DR: 提出Agentopia框架，通过100个智能体在10年内的长期生命模拟，研究社会行为涌现，并利用生活奖励训练提升LLM的拟人化能力。


<details>
  <summary>Details</summary>
Motivation: 模拟人类从社会生活中学习的过程，探究LLM能否从模拟社会经验中学习以更好地理解和复现人类行为，但以往模拟时间尺度短，限制了深度互动和长期成长。

Method: 开发Agentopia框架，包含100个自主智能体，在10年间追求个人成长、发展社交关系并满足需求；定义生活奖励反映人类福祉，通过拒绝采样训练LLM。

Result: 智能体展现出丰富的社会行为涌现；生活奖励训练有效提升底层LLM，在模拟中提高智能体福祉，并泛化到下游角色扮演基准，提升15.6%。

Conclusion: 长期生命模拟和生活奖励训练能增强LLM的拟人化能力，尤其是在社会生活中的智能。
...
TL;DR: 通过控制实验研究大语言模型在离散概率问题上的推理能力，发现模型在标准问题上表现优异，但在反直觉问题上显著下降，且易受令牌偏差和误导性提示影响。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型是否具备真正的概率推理能力，特别是面对反直觉问题时。

Method: 构建标准习题和反直觉习题两个数据集，对8个先进模型进行测试，包括有/无思维链提示，并分析令牌偏差和误导性提示的影响。

Result: 标准问题准确率0.96，反直觉问题仅0.59；令牌偏差导致准确率下降超20%；误导性提示降低性能达34%，无一模型免疫。

Conclusion: 当前大语言模型并非真正的概率推理者，尽管在高级数学问题上成功。
...
### [65] [Community Detection on a Randomly Growing Network](https://arxiv.org/abs/2606.07447)
*Jianxiang Wang,Min Xu*

Main category: stat.ME

TL;DR: 本文研究了随机网络增长模型下的社区检测问题，该模型生成具有优先连接树和Erdős–Rényi边的网络，可产生幂律度分布等真实网络特征。证明了无法完全恢复所有节点的社区标签，但设计了算法可恢复中心节点的社区标签，并通过两阶段过程扩展到其他节点。


<details>
  <summary>Details</summary>
Motivation: 现有社区检测多基于随机块模型（SBM），但真实网络具有幂律度分布等SBM难以刻画的特征，因此需要研究更符合实际网络生成过程的社区检测方法。

Method: 提出一种两阶段算法：第一阶段根据度数或到达时间等中心性指标识别高度数节点并分类；第二阶段将社区分配扩展到其余节点。

Result: 理论证明无法一致地恢复所有节点的社区标签，但算法能恢复子集（中心节点）的标签。数值实验和真实合著网络应用验证了方法的有效性。

Conclusion: 该研究为在更真实的网络生成模型下进行社区检测提供了新思路，证明了即使无法完全恢复，也能通过中心节点获得有意义的社区结构。
...
### [66] [Robust inference for cyclic-stress accelerated life tests under interval monitoring with lognormal lifetimes](https://arxiv.org/abs/2606.06699)
*Mar\'ia Jaenada,Leandro Pardo,Kiran Prajapat*

Main category: stat.ME

TL;DR: 针对循环应力加速寿命试验中区间删失的对数正态寿命数据，本文提出基于加权密度功率散度的稳健推断方法，在存在异常值时仍能保持较高效率。


<details>
  <summary>Details</summary>
Motivation: 传统极大似然估计对数据污染敏感，而工业实验常为小样本，需开发稳健方法用于循环应力加速寿命试验的区间监测数据。

Method: 通过最小化加权密度功率散度得到加权最小密度功率散度估计量，并推导其渐近分布、影响函数及置信区间。

Result: 模拟表明该估计量在提供对异常值保护的同时，在干净数据下保持高效率；实例分析展示了实际优势。

Conclusion: 所提稳健推断方法能有效处理循环应力加速寿命试验中的异常值问题，提升可靠性评估的鲁棒性。
...
### [67] [Bayesian genome-wide clustering and variable selection of transcriptomic data via rank-based mixtures](https://arxiv.org/abs/2606.06730)
*Emilie Eliseussen,Haakon Muggerud,Luca Coraggio,Ida Scheel,Thomas Fleischer,Valeria Vitelli*

Main category: stat.ME

TL;DR: 本文提出了首个能够同时处理聚类和变量选择的排名模型lowBM3，扩展了贝叶斯Mallows模型，适用于超高维数据。


<details>
  <summary>Details</summary>
Motivation: 现有贝叶斯Mallows模型在处理超高维数据（如组学分析）时扩展性有限，需要更有效的框架。

Method: 提出低维贝叶斯Mallows模型混合lowBM3，结合聚类和变量选择，并附带后处理框架提供后验总结。

Result: 通过模拟研究和乳腺癌RNA-seq基因表达数据聚类应用验证了方法的性能和实用性。

Conclusion: lowBM3为超高维排名数据提供了可扩展的贝叶斯框架，能够同时处理异质性、参数估计和模型选择。
...
### [68] [Cluster-Aware Conformal Calibration for Spatio-Temporal Distributional Prediction](https://arxiv.org/abs/2606.06753)
*Gooyoung Kim,Chae Young Lim,Wen-Ting Wang,Hao-Yun Huang,Wei-Ying Wu*

Main category: stat.ME

TL;DR: 针对非均匀空间采样下固定基函数效率低的问题，提出聚类自适应空间基和聚类感知保形校准的DeepKriging扩展，提升时空分布预测的覆盖精度和尾部分布可靠性。


<details>
  <summary>Details</summary>
Motivation: 固定正则网格空间基在非均匀采样下效率低，过度分配容量到稀疏区域，而密集区域分辨率不足。

Method: 根据空间采样密度初始化聚类自适应空间基的中心和尺度，结合聚类感知保形校准（逐簇确定预测区间宽度，样本不足时使用全局回退）。

Result: 在模拟研究和PM2.5数据分析中，与全局保形基线相比，覆盖精度和尾部分布可靠性显著提升。

Conclusion: 所提校准管道明确针对空间异质性和局部校准不足，在聚类观测模式下更可靠。
...
### [69] [Testing Equality of Conditional Distributions via Generative Models](https://arxiv.org/abs/2606.06930)
*Hanjia Gao,Linjun Huang,Yun Yang,Xiaofeng Shao*

Main category: stat.ME

TL;DR: 提出基于生成模型的条件分布相等性检验方法，通过交叉生成实现直接比较，避免密度比估计和高维平滑。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖条件密度比估计或高维局部平滑，难以处理高维协变量和多变量响应。

Method: 从每个样本学习条件生成器，在另一样本的协变量上生成响应，构建RKHS索引的经验过程统计量，使用乘子bootstrap校准和交替最大化算法。

Result: 理论推导了检验统计量的极限分布和bootstrap有效性，证明双重稳健性；模拟和实际数据表明对多变量响应和高维协变量表现良好。

Conclusion: 该方法有效且稳健，能处理复杂高维数据，为条件分布比较提供了新工具。
...
### [70] [Causal inference of Plackett-Burman designs in applications](https://arxiv.org/abs/2606.06961)
*Shuchen Chang,Zhi-ming Li*

Main category: stat.ME

TL;DR: 本文提出了一个基于潜在结果的因果推断框架，用于Plackett-Burman设计，并展示了四个应用。


<details>
  <summary>Details</summary>
Motivation: 由四个Plackett-Burman设计的实际应用驱动。

Method: 定义有限总体下的因果效应；获得Neymanian估计量及其方差协方差；进行sharp原假设检验；使用算法构造Fisherian区间。

Result: 通过四个应用实例验证了所提方法。

Conclusion: 所提出的因果推断框架适用于Plackett-Burman设计。
...
### [71] [Influence of continuous predictor modelling methods on prediction stability in clinical prediction model development: an empirical comparison using real clinical data](https://arxiv.org/abs/2606.07052)
...
TL;DR: 研究连续预测变量建模方法（如二分法、线性项、多项式、梯度提升等）对预测稳定性的影响，发现简单线性模型在小样本中更稳定，复杂方法需要更大样本。


<details>
  <summary>Details</summary>
Motivation: 预测稳定性对临床预测模型的可靠性至关重要，但连续预测变量的建模方式如何影响稳定性尚不清楚。

Method: 使用19418名急诊患者真实数据集，创建5个样本量场景（437至8739），比较6种方法：中位数二分法（DIC）、三分法（TER）、线性项（LIN）、二次项（QUA）、多元分数多项式（MFP）和极端梯度提升（XGB）。通过基于bootstrap的框架评估稳定性，以90%个体预测平均绝对误差≤5%为稳定标准。

Result: 稳定性随样本量增加而提高，方法间差异显著。在n=437时无法达到稳定；n=874时DIC和LIN稳定；n=1748时QUA稳定；更大样本下所有方法稳定。LIN、QUA、MFP、XGB的AUC高于DIC和TER，但XGB存在校准问题。

Conclusion: 连续预测变量建模方法影响预测稳定性，线性模型在小样本中更稳定，复杂方法需要更大样本。简单方法（尤其是线性项）在数据较小时可能更可靠。
...
### [72] [One-step Outcome Imputation: An Alternative to Multiple Imputation](https://arxiv.org/abs/2606.07174)
*Andreas Nordland,Klaus K. Holst,David Redek,Christian B. Pipper,Aske T. Iversen*

Main category: stat.ME

TL;DR: 提出一种基于影响函数的一步估计器，替代Rubin规则，为随机对照试验中缺失结果的处理提供渐近有效推断。


<details>
  <summary>Details</summary>
Motivation: 多重插补的Rubin规则在某些方法（如参考插补）中无法提供有效的标准误估计。

Method: 明确目标处理效应，通过影响函数构造高效的一步估计器。

Result: 获得渐近有效的推断，避免了随机性和多重插补的计算负担。

Conclusion: 通过参考插补和依事件插补等示例验证了方法的有效性。
...
### [73] [Learning Collapsed Patterns in Compositional Data: A Bayesian Heterogeneous Relative-Shift Approach](https://arxiv.org/abs/2606.07373)
*Maoran Xu,Guanyu Hu*

Main category: stat.ME

TL;DR: 提出一种贝叶斯异质相对偏移回归模型，联合学习潜在聚类和简约效应结构，并通过混合MCMC算法实现。


<details>
  <summary>Details</summary>
Motivation: 现有成分数据回归方法无法同时恢复潜在聚类结构和实现簇内降维。

Method: 结合投影收缩先验在可识别对比上诱导系数绑定，使用有限混合先验推断聚类数目，开发嵌入确定性替代坍缩算子的NUTS混合MCMC算法。

Result: 仿真验证了准确恢复和强预测性能；宏观经济和空间转录组数据展示了可解释性和实用性。

Conclusion: 该方法有效处理高维成分数据中的异质性和聚类问题，具有理论一致性和实际应用价值。
...
### [74] [Deriving the Variance-Minimizing Design for Standard Addition via c-Optimality](https://arxiv.org/abs/2606.07406)
*Gerhard G\"ossler,Vera Hofer,Walter Goessler*

Main category: stat.ME

TL;DR: 针对标准添加法，总结并扩展了最优设计理论，指出线性响应下测量误差非递减时最优设计为两点设计，测量分配取决于具体情况，范围影响结果，两点设计无需加权即可达到最优精度，并分析了偏差影响。


<details>
  <summary>Details</summary>
Motivation: 标准添加法最优设计的知识分散且仅以数学形式存在，不易为分析化学家所用。

Method: 汇总分析化学文献中的现有结果，并应用最优性理论推导标准添加法的特殊情形。

Result: 测量误差非递减（常数、线性或二次增加）时线性响应的最优设计为两点设计；最优测量分配比例偏离50:50且取决于具体设置；范围影响结果；两点设计下无需加权回归即可获得最优精度。

Conclusion: 为标准添加法提供了实用的最优设计指导，强调了简单两点设计的优势，并为偏差分析提供了参考。
...
### [75] [Covariance-Adaptive Residualization and Stagewise Calibration for Dependent Multiple Testing](https://arxiv.org/abs/2606.07466)
*Prasenjit Ghosh,Arijit Chakrabarti*

Main category: stat.ME

TL;DR: 提出一种协方差自适应残差化与逐步校准结合的多元高斯均值同时检验方法，在任意依赖下表现优异。


<details>
  <summary>Details</summary>
Motivation: 处理任意协方差依赖下多元高斯均值的同步假设检验问题，改进现有MRD程序。

Method: 基于MRD残差化，采用Gavrilov等人的广义逐步下降临界常数进行校准，通过单一精度矩阵简化计算。

Result: 模拟显示标准化误分类风险低于多种边缘检验；在结构化依赖下FDR接近名义水平，FNR极小，功效接近1，平均拒绝次数接近真信号数。

Conclusion: 协方差自适应残差化与逐步校准在依赖多重检验中具有高度有利的相互作用。
...
### [76] [Smooth Hard-Thresholding for Singular Values with Stein's Unbiased Risk Estimate](https://arxiv.org/abs/2606.06632)
*Guanzhong Yang*

Main category: math.ST

TL;DR: 针对低秩矩阵去噪，提出基于 Stein 无偏风险估计（SURE）的逻辑平滑硬阈值谱估计器，解决了经典方法不是有限样本风险原则的问题，并验证了其无偏性。


<details>
  <summary>Details</summary>
Motivation: 经典 SVD 去噪方法使用的截断规则不是有限样本风险原则，因为低秩近似会吸收部分噪声。

Method: 引入逻辑平滑硬阈值谱估计器，使其满足 Stein 引理的正则条件，从而获得固定阈值下高斯噪声的无偏风险估计。

Result: 平滑估计器允许无偏风险估计；排序候选阈值时，极限分数与有偏硬阈值 SURE 公式相同，但仅用于排序。

Conclusion: 所选阈值是数据自适应的，其对应的 SURE 值不应视为最终估计器的无偏风险估计。
...
### [77] [Sequential testing of conditionally constrained hypotheses](https://arxiv.org/abs/2606.06769)
*Eugenio Clerico*

Main category: math.ST

TL;DR: 本文完整刻画了用于检验由有限个条件约束定义的条件非参数假设的e过程类，证明每个e过程都被可预测的仿射一步e变量的乘积逐点控制，从而可被检验超鞅替代。


<details>
  <summary>Details</summary>
Motivation: 现有e过程理论主要针对单步约束或有界一维条件均值检验，需要推广到更一般的条件序贯设定。

Method: 通过证明完全类定理，建立每个e过程与可预测仿射一步e变量乘积之间的控制关系。

Result: 每个条件非参数假设的e过程都点态被可预测的仿射一步e变量乘积控制，因此可无损失地用检验超鞅替代。

Conclusion: 将完全类结果从单步约束和有界一维条件均值检验扩展到更广泛的条件序贯检验问题。
...
### [78] [Integral stochastic orders of $m$-generalized order statistics from transform-ordered nonparametric families](https://arxiv.org/abs/2606.07022)
*Idir Arab,Tommaso Lando,Paulo Eduardo Oliveira,Tomasz Rychlik*

Main category: math.ST

TL;DR: 本文提供了比较m-广义顺序统计量的充分条件，涵盖递增凹、递增凸和星形随机序，适用于经典顺序统计量、删失II型顺序统计量和记录，采用非参数方法并假设随机变换有序性质。


<details>
  <summary>Details</summary>
Motivation: 在统计中需要比较不同广义顺序统计量（如顺序统计量、记录等）的随机序，但缺乏统一的充分条件。

Method: 基于随机变换有序性质，提出非参数充分条件，依赖于广义顺序统计量参数和基础分布，并通过变换序关联多种分布类（如广义和负广义Pareto分布）。

Result: 得到了排序广义顺序统计量的充分条件，可用于递增凹、递增凸和星形随机序的比较。

Conclusion: 所述条件为排名广义顺序统计量提供了统一框架，无需假定具体参数形式，具有广泛适用性。
...
### [79] [Ising Models on Inhomogeneous Random Graphs: Inference, Local Asymptotic Minimaxity, and Limit of Experiments](https://arxiv.org/abs/2606.07065)
*Somabha Mukherjee,Sanchayan Bhowal,Anirban Chatterjee,Bhaswar B. Bhattacharya*

Main category: math.ST

TL;DR: 本文针对次临界参数区间内非齐次随机图上的Ising模型，建立了具有渐近最优性保证的推断框架，包括最大似然估计的渐近分布、计算高效的一步估计量及其最优性，以及拟合优度检验。


<details>
  <summary>Details</summary>
Motivation: 最大似然估计计算困难，且网络依赖数据的渐近最优性结果缺乏。

Method: 通过似然方程的一步近似得到闭式估计量，并利用充分统计量和随机配分函数的新波动结果进行渐近分析。

Result: 所提估计量与最大似然估计具有相同的渐近分布，达到局部渐近极小化最大风险，并给出了渐近有效的置信区间和拟合优度检验的局部功效。

Conclusion: 首次为网络依赖数据建立了尖锐的渐近最优性结果，且新波动结果具有独立意义。
...
### [80] [The Balance Property: The Constrained Case, with a View on Risk Sharing](https://arxiv.org/abs/2606.07276)
*Mario V. W\"uthrich*

Main category: math.ST

TL;DR: 平衡属性是保险定价中模型的重要性质，保证总精算价格等于总观测损失。本文提出第三种平衡校正方法——约束GLM拟合，优于现有两种方法，并揭示平衡属性与事后风险分担规则的联系。


<details>
  <summary>Details</summary>
Motivation: 现有平衡校正方法存在不足，需要更优的方法来保证GLM模型的平衡属性。

Method: 提出约束GLM拟合方法作为第三种平衡校正技术。

Result: 约束GLM拟合方法优于此前讨论的两种平衡校正方法。

Conclusion: 约束GLM拟合是更优的平衡校正方法，且平衡属性与事后风险分担规则相关。
...
### [83] [Non-asymptotic bounds for quasi-MLE, misspecified models, and dependence under group sequential sampling](https://arxiv.org/abs/2606.07499)
*Julian Aronowitz,Jay Bartroff*

Main category: math.ST

TL;DR: 在模型误设和组内依赖下，推导了分组序列拟最大似然估计的渐近正态极限和非渐近正态逼近界，并用Stein方法得到了已知常数的边界。


<details>
  <summary>Details</summary>
Motivation: 处理模型可能误设和组内依赖时，分组序列估计的渐近性质尚不明确，需要理论保证。

Method: 使用Stein方法推导非渐近正态逼近界，并计算极限协方差结构。

Result: 给出了泊松广义线性混合模型的有限样本界和极限协方差，通过癫痫临床试验数据验证。

Conclusion: 所提方法为依赖数据估计问题提供了有效的正态逼近理论。
...
### [84] [Elmes*: Automated Construction of Fine-Grained Evaluation Rubrics for Large Language Models in Long-Tail Educational Scenarios](https://arxiv.org/abs/2606.06546)
*Tao Liu,Ye Lu,Ruohua Zhang,Siyu Song,Wentao Liu,Aimin Zhou,Hao Hao*

Main category: cs.LG

TL;DR: 提出Elmes*框架和Edu-330基准，用于细粒度评估LLM的教育能力，发现教育能力多维且存在模型差异，LLM评判者有效但有偏见。


<details>
  <summary>Details</summary>
Motivation: 现有教育评估基准侧重知识或依赖于工设计的评分标准，难以扩展到长尾教学场景，需要衡量模型的教学方式而非仅知识。

Method: 结合声明式多智能体引擎与自进化模块SceneGen，构建情景特定评分标准；构建Edu-330覆盖330个场景、11个学科等；使用专家评分和LLM评判者进行实验。

Result: 教育能力多维：顶级LLM在创造力和价值观整合上差异明显，知识强的模型可能在苏格拉底式引导上失败；专用模型InnoSpark获得最佳人类评分；LLM评判者排序与人类一致但方差小，存在自我偏好等偏见。

Conclusion: Elmes*提供了可扩展的诊断基础设施，用于基于教学法的LLM评估。
...
### [85] [FAIR-Calib: Frontier-Aware Instability-Reweighted Calibration for Post-Training Quantization of Diffusion Large Language Models](https://arxiv.org/abs/2606.06547)
*Haoyu Huang,Linlin Yang,Sheng Xu,Boyu Liu,Guodong Guo,Zhongqian Fu,Hang Zhou,Baochang Zhang*

Main category: cs.LG

TL;DR: 提出了一种类后训练量化框架FAIR-Calib，通过重加权校准保护扩散大语言模型的前沿决策稳定性。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型中的不可逆提交导致早期决策脆弱，后训练量化误差易翻转前沿决策并放大。

Method: 两阶段PTQ框架：阶段I探测全精度教师估计位置先验，结合前沿命中与掩蔽阶段可靠性；阶段II使用重加权隐藏状态MSE进行离策略逐层校准。

Result: 在LLaDA和Dream（W4A4）上持续超越现有基线，显著减少前沿决策翻转和提交后不匹配。

Conclusion: FAIR-Calib有效保护了扩散模型中脆弱的前沿状态，为后训练量化提供了新思路。
...
### [86] [Multi-Scale Feature Attention Network for Polymer Classification using THz Dual-Comb Spectroscopy](https://arxiv.org/abs/2606.06554)
*Roshni Mahtani,Il\'an Carretero,Laura Monroy,Aldo Moreno-Oyervides,Oscar El\'ias Bonilla-Manrique,Roc\'io del Amor*

Main category: cs.LG

TL;DR: 提出多尺度特征注意力网络（MSFAN），结合太赫兹双梳光谱实现12种聚合物的高准确率分类，准确率达85.2%。


<details>
  <summary>Details</summary>
Motivation: 传统聚合物识别方法难以提供稳健区分，需要快速、高分辨率、无损的替代方案。

Method: 基于THz-DCS数据，设计MSFAN，包含特征门控、多尺度并行卷积、交叉特征注意力和注意力池化。

Result: MSFAN分类准确率为85.2%，优于现有最先进模型。

Conclusion: THz-DCS与深度学习结合可实现有效、可扩展、可解释的聚合物分类。
...
TL;DR: 提出了MacArena基准测试，包含421个macOS原生任务，发现模型排名在移植任务和原生任务间反转，表明macOS对GUI代理更具挑战性。


<details>
  <summary>Details</summary>
Motivation: macOS在GUI代理基准测试中未被充分服务，现有基准如OSWorld和macOSWorld存在覆盖狭窄、不兼容Apple Silicon等问题。

Method: 创建了MacArena，包含421个经人工验证的任务，覆盖50个应用，结合OSWorld移植任务、macOSWorld内容及49个新macOS原生任务，运行在Apple Virtualization框架上。

Result: 模型排名在移植任务和macOS原生任务间反转，领先模型在MacArena子集上落后超过26%。

Conclusion: macOS提出独特的GUI挑战，现有基准测试可能反映任务分布热悉度而非真正跨平台能力，macOS对当前GUI代理更难。
...
TL;DR: 提出WAV v1，一种轻量级多分辨率残差路由方法，通过在每个块中添加方向细节基来改进块级残差路由，在深层Transformer中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有残差路由方法（如Block Attention Residuals）仅使用块级残差总和，忽略了注意力与MLP更新不平衡及早晚期块动态等方向结构，需要更丰富的残差表示。

Method: WAV v1为每个块扩展两个方向细节基：相位基（对比注意力与MLP更新）和分裂基（对比早期和晚期子层更新），通过深度softmax混合器与标准块摘要一起路由，并采用负细节源初始化和分离RMS匹配稳定训练。

Result: 在字符级TinyStories和Text8语言建模上，WAV v1在48层时显著优于所有基线，验证损失从Block AttnRes的0.4960降至0.4738（TinyStories）和从0.9363降至0.9305（Text8），参数增加可忽略；12层时效果不一致，24层时具有竞争力。

Conclusion: 方向残差细节（而非仅块级和）对于深层Transformer的残差路由扩展至关重要。
...
TL;DR: 本文首次系统综述了符号回归中的不确定性量化问题，分为频率学派、贝叶斯和模型选择三个方向，指出该领域尚待深入探索。


<details>
  <summary>Details</summary>
Motivation: 符号回归缺乏不确定性量化支持，限制了其在真实决策中的应用，因此需要系统梳理相关概念和方法。

Method: 通过文献综述，将不确定性量化方法分为频率学派、贝叶斯和模型选择三大类，并介绍基本概念。

Result: 不确定性量化在符号回归中仍未被充分探索，现有方法有限。

Conclusion: 需要进一步研究可靠的不确定性量化方法，以推动符号回归的实际应用。
...
TL;DR: 现代生成式模型在亚AGI能力水平上对知识和文化生产造成结构性风险，导致人类时间学习（HTL）价值崩溃，因为验证输出是否源于真实人类学习的成本过高。


<details>
  <summary>Details</summary>
Motivation: 生成式模型输出的表面特征日益接近需要长期人类学习的成果，但验证其真实来源的经济代价超过预期收益，威胁到HTL密集型工作的可持续性。

Method: 通过昂贵检查框架形式化价值崩溃路径，并利用学术出版、法律实践、内容平台和软件安全等跨领域证据映射验证侵蚀的四个阶段。

Result: 验证侵蚀逐步升级，最终使评估者无视生产模式而奖励输出，HTL生产者被迫与低成本AI输出竞争；更好的对齐模型反而加剧验证难度和竞争压力。

Conclusion: 即使未达到AGI，生成式模型已对基于人类时间学习的知识和文化生产构成结构风险，对齐技术的进步可能适得其反，需警惕价值崩溃的广泛影响。
...
TL;DR: 提出程序化层网络（PoLar），通过动态跳过或循环预训练层，以更少层数提升LLM推理准确率，并纠正错误。


<details>
  <summary>Details</summary>
Motivation: 标准LLM推理使用固定深度和顺序执行所有层，可能效率低且未充分利用潜在推理路径。

Method: 设计轻量级预测网络，为每个输入生成动态执行程序，选择性地跳过或重复预训练层。

Result: 在数学推理基准上，PoLar一致优于标准推理和此前动态深度方法，且执行层数更少，在分布外评估中仍有效。

Conclusion: 固定深度执行只利用了LLM潜在推理能力的一小部分，动态程序化执行更高效准确。
...
TL;DR: 提出GPLFR模型，通过将输出表示为从高斯过程先验的低维潜在状态的线性高斯解码，并解析边缘化解码权重，实现压缩与预测联合优化，适用于高维输出回归。


<details>
  <summary>Details</summary>
Motivation: 现有方法如PCA-GP使用基于重建优化的基，而非预测优化，导致预测能力受限。

Method: 将每个输出建模为低维潜在状态的线性高斯解码，潜在状态来自高斯过程先验，并解析边缘化解码权重，实现联合优化。

Result: 首次构建了岩石系外行星全球气候模型的空间分辨仿真器，验证了模型有效性。

Conclusion: GPLFR有效处理高维输出回归，在低数据场景下表现良好。
...
TL;DR: 本书通过表示学习视角，旨在打开大型深度网络的黑箱，理解其内部机制，并基于优化和信息论原理设计高效、可解释、可控的模型。


<details>
  <summary>Details</summary>
Motivation: 当前大型生成模型作为“黑箱”，内部机制不透明，导致可解释性、可靠性和控制困难，引发过度炒作和恐惧。

Method: 通过表示学习、优化和信息论原理，将现代神经网络架构的设计原则简化为本科水平的线性代数和微积分练习，并应用于问题解决。

Result: 获得设计上高效、可解释、可控的新方法和模型，其性能不亚于甚至优于黑箱模型。

Conclusion: 总结了深度学习未来方向、表示学习的作用以及一些开放问题。
...
TL;DR: 提出FMScope方法，揭示EEG基础模型中的身份陷阱问题，即高准确率可能源于主体身份特征而非临床标志。


<details>
  <summary>Details</summary>
Motivation: 探究EEG基础模型高准确率是否源于真实临床标志还是主体身份特征，并提出检测方法。

Method: 提出FMScope冻结表示协议，包含方差分解、主体轴擦除、非周期1/f消融、逐层标签探测和主体内方向一致性五种诊断。

Result: 身份陷阱普遍存在，擦除主体特征可提升主体内标签解码；非周期1/f是主体身份载体；微调仅在存在跨主体标记时增强标签方差。

Conclusion: 身份陷阱是基于生理成分的捷径学习，FMScope可区分源于生物标志和主体身份的性能提升。
...
TL;DR: 提出生成流匹配方法，模拟非马尔可夫和非高斯效应，改进短时间动力学和低密度预测。


<details>
  <summary>Details</summary>
Motivation: 粗粒化SPDE如正则化DK方程无法准确捕捉非马尔可夫和非高斯效应，尤其在短时间及低粒子密度下。

Method: 开发一种生成流匹配方法，直接基于粒子模拟建模通量概率分布，融入非马尔可夫和非高斯效应。

Result: 在Kramers首次通过时间中，该方法准确捕捉短时间行为，数密度统计矩预测优于正则化DK方程。

Conclusion: 生成流匹配方法能更有效模拟短时间动力学和统计矩，适用于非马尔可夫及非高斯系统。
...
TL;DR: 提出一种轻量级监测原语，通过布尔矩阵表示和滑动窗口推理跟踪AI-RAN中参数与KPI的依赖关系，以支持冲突诊断。


<details>
  <summary>Details</summary>
Motivation: 在AI-RAN中，冲突监测需要知道参数-KPI依赖关系在当前运行状态下是否仍然有效，仅知道部署的应用不足够。

Method: 将活动依赖表示为布尔矩阵，使用布尔矩阵乘法检查流式遥测事件的一致性，当观测到结构变化时通过滑动窗口推理重新计算依赖估计。

Result: 在受控布尔事件流实验中，该方法在依赖变化和布尔观测噪声下实现了高效准确的跟踪。

Conclusion: 该跟踪器作为可解释信号用于冲突诊断和慢循环模型更新，而非自主缓解机制。
...
TL;DR: 提出不确定性感知的LLM引导策略塑形（ULPS），结合校准的大语言模型和蒙特卡洛dropout不确定性估计，在稀疏奖励多任务中提升样本效率和成功率。


<details>
  <summary>Details</summary>
Motivation: 强化学习面临稀疏奖励和异构任务序列导致的收敛慢、泛化弱和探索低效问题。

Method: 采用A*生成最优符号轨迹微调BERT模型，训练时通过MC dropout估计认知不确定性，并用熵机制自适应平衡LLM建议和PPO策略。

Result: 在MiniGridUnlockPickup基准上，ULPS比无引导、无校准和标准RL基线在成功率、奖励效率和样本复杂度上持续提升，微调后执行准确率提高9%以上，奖励AUC更高。

Conclusion: 集成符号A*轨迹、预训练语言先验和不确定性感知控制为稀疏奖励多任务RL提供了有效方法，可扩展到部分可观察和多智能体设置。
...
TL;DR: 提出GiFlow框架，利用图信息先验和混合向量场模型提升时空插补性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在误差累积、采样效率低、先验不匹配等问题。

Method: 用图信息先验替代高斯先验，通过时空滤波构建；参数化混合向量场模型融合空间注意力、时间注意力和时空传播。

Result: 在合成和真实数据集上优于最先进方法。

Conclusion: GiFlow有效提升了时空插补性能。
...
### [99] [Towards Serverless Semi-Decentralized Federated Learning with Heterogeneous Optimizers](https://arxiv.org/abs/2606.06687)
*Su Wang,Mung Chiang,H. Vincent Poor*

Main category: cs.LG

TL;DR: 提出无服务器半去中心化联邦学习（SSD-FL），解决异构优化器和数据分布下的集群形成问题，提升收敛速度和通信效率。


<details>
  <summary>Details</summary>
Motivation: 去中心化联邦学习中，网络图结构、数据异构性和本地优化器的复杂耦合使集群形成优化极具挑战，现有方法多聚焦中心化场景，缺乏对完全去中心化环境的探索。

Method: SSD-FL通过轻量级设备间初始化阶段形成集群，分段全局轮次为簇内和簇间模式，使用有效损失函数集成设备优化器与图正则化，并基于Cheeger不等式设计迭代聚类算法。

Result: 实验表明，SSD-FL在多种网络图、数据集和本地优化器下，相比三类去中心化联邦学习方法，均提升收敛速度和通信效率。

Conclusion: SSD-FL有效解决了去中心化联邦学习中的集群形成问题，无需持久服务器，兼顾收敛与共识。
...
### [100] [The Geography of Algorithmic Judgment: LLM Intermediaries, Place Identity, and Racial Steering in Housing Search](https://arxiv.org/abs/2606.06694)
*Hana Samad,Trung Lam,Christoph M\"ugge-Durum,Michael Akinwumi*

Main category: cs.LG

TL;DR: 大型语言模型在住房搜索中扮演中介角色，通过行为审计发现种族导向是模型解读许可的涌现行为，受用户身份、偏好表达和城市空间逻辑影响，且不同城市结果不可泛化。


<details>
  <summary>Details</summary>
Motivation: 探究大型语言模型在住房推荐中是否存在种族导向，及其如何受用户身份和偏好表达影响。

Method: 对7个开源和闭源LLM进行行为审计，覆盖4个美国城市，使用三种迭代提示条件，结合公平住房配对测试方法。

Result: 导向是涌现行为而非静态属性；用户身份、偏好表达和模型内化的空间逻辑相互作用导致导向；导向方向和程度非均匀；偏好条件测试增加或重组了导向行为；城市背景影响结果不可泛化。

Conclusion: 需本地和领域专业知识确保公平住房承诺不被AI工具削弱，法律和制度性承诺需在采用中介空间获取的AI工具时得到维护。
...
### [101] [RECAP: Regression Evaluation for Continual Adaptation of Prompts](https://arxiv.org/abs/2606.06698)
*Harsh Deshpande,Kushal Chawla,Sangwoo Cho,William Campbell*

Main category: cs.LG

TL;DR: RECAP基准测试评估主动适应场景下持续学习现象，发现现有方法在性能上无显著改进，需设计新的主动提示适应方法。


<details>
  <summary>Details</summary>
Motivation: 生产代理系统常面临动态约束并需立即适应，现有基准测试缺乏这种主动适应设置。

Method: 提出RECAP基准，采用主动适应-然后测试协议，仅提供约束规范，要求模型在未见过测试数据前泛化。

Result: 评估6种方法、4种LLM和3种调度，所有方法在性能上无显著提升，且延迟更高。

Conclusion: 现有离线或反应式方法不适合主动范式，需开发稳健的主动提示适应方法。
...
### [102] [ShallowBench: Benchmarking Generative Drug Design Models on Shallow-Pocket Targets](https://arxiv.org/abs/2606.06717)
*Saket Reddy,Shiwei Liu*

Main category: cs.LG

TL;DR: 提出了ShallowBench基准，用于评估生成模型在浅口袋靶点上的表现，发现现有模型在这些靶点上预测的结合亲和力较弱。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型主要针对深口袋靶点，难以处理低口袋可靶向性的靶点（如KRAS、MYC），需要新的基准来推动模型改进。

Method: 从CrossDocked2020中提取5780个浅口袋靶点，通过计算Alpha Shape“盖子”体积与底层蛋白原子体素体积的差值来识别低凹度且表面积足够的结合位点。

Result: 评估多种生成模型发现，在这些低凹度界面上预测的结合亲和力较弱。

Conclusion: ShallowBench为生成生物学模型提供了严格的基准，并强调需要新的架构或损失函数来处理这些挑战性靶点。
...
### [103] [MSAIC-Net: A Multi-Scale Attention and Imbalance-Aware Contrastive Network for ECG-Based Myocardial Substrate Abnormality Detection](https://arxiv.org/abs/2606.06718)
*Canyu Lei,Fenglin Zhang,Derek Bivona,Cristiane Singulane,Jonathan Pan,Kenneth Bilchick,Amit R. Patel,Jianxin Xie*

Main category: cs.LG

TL;DR: 提出多尺度注意力增强卷积网络（MSAIC-Net），用于基于心电图的检测心肌基质异常，有效提升性能并增强可解释性。


<details>
  <summary>Details</summary>
Motivation: 心肌基质异常（如心肌疤痕、心肌梗死）与不良心血管结局相关，ECG检测面临导联依赖表现、高维多导联信号、类别不平衡和深度学习模型可解释性不足等问题。

Method: MSAIC-Net采用并行空洞卷积分支提取多时间感受野特征，通道注意力自适应重加权导联和特征通道表示，引入不平衡感知有监督对比学习改善特征可分性，并通过导联置换重要性量化各导联贡献以提升可解释性。

Result: 在UVA低数据队列（心肌疤痕分类）和大型公开PTB-XL数据集（心肌梗死识别）上，MSAIC-Net优于基线模型，尤其在低数据场景下提升显著。

Conclusion: 该框架为基于ECG的心肌基质异常检测提供了有效且可解释的解决方案。
...
### [104] [Flatland: The Adventures of Gradient Descent with Large Step Sizes](https://arxiv.org/abs/2606.06722)
*Leonardo Galli,Curtis Fox,Wiebke Bartolomaeus,Mark Schmidt,Holger Rauhut*

Main category: cs.LG

TL;DR: 神经网络训练中，针对非全局L-光滑目标函数，提出了统一的大步长定义，并设计了自适应一阶方法，使梯度下降从训练开始就处于稳定边界，同时能最小化锐度到全局最小值，并发现过早平坦化有害。


<details>
  <summary>Details</summary>
Motivation: 解决神经网络训练中目标函数非全局L-光滑时，梯度下降最大步长的确定问题，该问题理论上和实践上长期未解决。

Method: 基于梯度局部Lipschitz（或Hölder）连续性定义统一大步长，设计自适应一阶优化方法，利用自稳定化机制允许进入稍尖锐谷底。

Result: 损失非单调下降；步长与锐度乘积始终高于稳定边界阈值2；成功将锐度最小化到全局最小值；将不成功训练转化为成功。

Conclusion: 过早遇到全局平坦区域会减慢收敛并损害泛化，而所提方法通过允许适度尖锐性实现更好性能。
...
### [105] [Synthics: Synthetic Physics-like Datasets for Machine Learning](https://arxiv.org/abs/2606.06724)
*Jari Veps\"al\"ainen*

Main category: cs.LG

TL;DR: 提出一种基于贝叶斯概率上下文无关文法的合成回归数据生成方法，使生成数据在结构上匹配物理方程语料库，并在下游任务中表现良好。


<details>
  <summary>Details</summary>
Motivation: 真实数据收集困难，合成数据需保持结构真实性以提升机器学习泛化能力。

Method: 使用贝叶斯概率上下文无关文法从方程语料库中采样新方程，通过非侵入式探测确定输入域，混合均匀和截断正态分布采样。

Result: 生成方程在八项结构特征上全部匹配语料库，而基线方法仅匹配两项；下游超参数调优中，合成数据调优的模型性能接近真实数据调优。

Conclusion: 贝叶斯先验对于小规模语料库的结构保真度至关重要，该方法能生成可用于实际任务的真实合成数据。
...
### [106] [TorchKM: A GPU-Oriented Library for Kernel Learning and Model Selection](https://arxiv.org/abs/2606.06742)
*Yikai Zhang,Gaoxiang Jia,Jie Ding,Boxiang Wang*

Main category: cs.LG

TL;DR: TorchKM 是一个基于 GPU 加速的开源核机器库，支持 SVM、核逻辑回归和核分位数回归，通过智能矩阵运算复用加速训练与模型选择。


<details>
  <summary>Details</summary>
Motivation: 现有核机器库缺乏 GPU 加速和高效的模型选择工具，TorchKM 旨在填补这一空白。

Method: 采用 scikit-learn 风格 API，利用 GPU 友好的线性代数，并通过重用矩阵运算加速完整训练和模型选择流程。

Result: 基准测试显示，预测性能与标准基线相当，但速度显著提升。

Conclusion: TorchKM 提供了高效、易用的核机器实现，适合大规模应用。
...
### [107] [Learn to Match: Two-Sided Matching with Temporally Extended Feedback](https://arxiv.org/abs/2606.06744)
*Haijing Zong,Yancheng Liang,Boyang Zhou,Natasha Jaques*

Main category: cs.LG

TL;DR: 提出一个时间扩展反馈框架，将双向匹配建模为部分可观测马尔可夫博弈，并构建多智能体强化学习基准Learn2Match，实验表明PPO优于bandit基线但仍有信息摩擦损失。


<details>
  <summary>Details</summary>
Motivation: 现有匹配模型将信息简化为即时次高斯反馈，忽略随时间逐渐揭示并影响未来决策的支付相关信息。

Method: 将双向匹配建模为部分可观测马尔可夫博弈，包含昂贵预筛选、带噪声后观测、演化潜在特征和内生解约，并在Learn2Match中实例化为多智能体强化学习基准。

Result: 独立PPO在时间扩展反馈下比bandit基线CA-ETC获得更高累积社会福利和更低累积遗憾，但信息摩擦损失更高。

Conclusion: Learn2Match作为下一代匹配市场算法基准，需结合RL的自适应性、bandit的统计规范性和稳定匹配的结构意识。
...
### [108] [Performance Variation in Deep Reinforcement Learning](https://arxiv.org/abs/2606.06746)
*Haruto Tanaka,A. Rupam Mahmood*

Main category: cs.LG

TL;DR: 提出基于百分位数的统计量和可视化方法，用于评估深度强化学习算法的运行间鲁棒性，并通过三个案例验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如均值不确定性估计）在评估运行间性能变化时存在目的不匹配和漏报风险。

Method: 采用最小-最大IPR（百分位距）和逐运行百分位高亮可视化，利用样本百分位数的标准性质。

Result: LayerNorm和倒数第二层归一化缩小PPO性能变化但对SAC影响小；TD-MPC在四种算法中变化最小且数据效率最高；DQN和Rainbow在Atari上变化水平相似。

Conclusion: 百分位工具易于解释，能提供丰富的运行间性能变化信息，有助于鲁棒性评估。
...
### [109] [A Rolling-Window Framework for Churn Prediction and Behavioral Driver Identification](https://arxiv.org/abs/2606.06776)
*Muhammad Jawad Mufti,Omar Hammad,Haitham Saleh,Muqaddas Gull*

Main category: cs.LG

TL;DR: 提出一种时间明确的客户流失预测框架，使用滚动行为窗口，在非合同服务环境中实现连续风险估计。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖简化时间假设或单点表示，限制持续风险评估和实际部署。

Method: 采用30天观察窗口和30天未来评估窗口，结合特征与序列学习方法。

Result: 特征模型准确率87.6%、AUC 0.94；序列模型召回率96.1%；未来数据上准确率>83%、AUC>0.91。

Conclusion: 精心设计的时间框架比模型复杂性更关键，可实现稳健、可解释的流失预测。
...
### [110] [Federated Foundation Models over Vehicular Networks](https://arxiv.org/abs/2606.06786)
...
TL;DR: 提出了将多模态多任务联邦基础模型（M3T FedFMs）集成到车辆网络中的前瞻性框架，通过案例研究证明了其潜力。


<details>
  <summary>Details</summary>
Motivation: 希望结合多模态多任务基础模型的表达能力和联邦学习的隐私保护与分布式学习能力，以推动车辆网络智能。

Method: 首先介绍M3T FedFMs的训练/微调原则，讨论其在车辆网络中的代表性用例，识别部署中的关键约束并提出研究方向，最后在Waymo数据集上进行案例研究。

Result: 在Waymo开放数据集上的案例研究展示了M3T FedFMs的潜力，并公开了实现代码以促进可重复性和研究。

Conclusion: M3T FedFMs有望赋能下一代车辆智能，但需应对车辆环境中的特有挑战，未来需进一步研究。
...
### [111] [Interpreting Learning Under Competing Models: Joint and Stepwise Approaches for Dynamic Cognitive Diagnosis](https://arxiv.org/abs/2606.06804)
*Yawen Ma,Sahoko Ishida,Kate Cain,Gabriel Wallin*

Main category: cs.LG

TL;DR: 在数字学习环境中，对项目-技能关系（Q矩阵）的估计方式（联合分析 vs 分步分析）会影响关于学习者技能发展的结论。


<details>
  <summary>Details</summary>
Motivation: 数字学习环境记录学习者对项目的反应，可研究特定技能发展。但分析时需处理未知的项目-技能结构（Q矩阵），选择联合估计还是先确定结构再学习会影响结论。

Method: 使用动态认知诊断模型，分析从二年级到三年级的阅读游戏数据（词汇和理解），利用项目文本嵌入作为Q矩阵的先验信息，比较联合分析与偏差校正的分步分析。

Result: 两种分析都认为大多数学习者趋向掌握两项技能，但在三年级时仍部分精通的人数上存在分歧，影响对阅读进展的报告。模拟表明当项目-技能结构不确定且项目池跨年级变化时，联合分析更可靠。

Conclusion: Q矩阵的估计方式会改变对学习者发展的实质性结论，联合分析在不确定条件下更可靠；提供了R代码。
...
### [112] [SCALE: Scalable Cross-Attention Learning with Extrapolation for Agentic Workflow Scheduling](https://arxiv.org/abs/2606.06820)
*Zhifei Xu,Jierui Lan,Zixuan Liang,Aiji Liang,Jinxi He*

Main category: cs.LG

TL;DR: 提出SCALE调度器，通过交叉注意力网络和结构化表示正则化实现无需微调的跨集群规模泛化。


<details>
  <summary>Details</summary>
Motivation: 现有深度强化学习调度器固定集群规模，规模变化需重新训练，缺乏泛化能力。

Method: 采用交叉注意力指针网络，任务特征查询服务器特征，架构天然支持任意服务器数量；引入结构化表示正则化（去相关损失和KL散度惩罚）稳定特征分布。

Result: 在16节点训练，直接测试32和48节点，平均响应时间比无正则化的架构降低8.9%。

Conclusion: 显式正则化对弥合规模泛化差距至关重要，SCALE有效实现了跨集群规模泛化。
...
### [113] [PandaAI: A Practical Agent CQ2 for Neuro-symbolic Data Analysis And Integrated Decision-Making in Quantitative Finance](https://arxiv.org/abs/2606.06823)
*Yuqi Li,Siyuan Liu,Bingjun Liu*

Main category: cs.LG

TL;DR: 提出PandaAI，一种结合市场机制建模和受约束alpha生成的闭环神经符号LLM代理，在金融时序决策中超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 深度学习在金融序列决策中因低信噪比和非平稳性表现不佳，需要利用LLM的推理能力同时抑制其金融毒性。

Method: 微调领域特定LLM，并集成到模块化闭环系统中，包含市场机制建模和受约束alpha生成，实现显式风险感知。

Result: 在沪深300数据上，Rank IC提升18.2%，最大回撤降低25.7%，优于最先进时序模型。

Conclusion: 受约束LLM生成和双通道自适应方法为高风险序列决策中的LLM部署提供了通用范式。
...
### [114] [Architecture Shapes Transfer Specificity in Implicit Neural Representations](https://arxiv.org/abs/2606.06827)
*D Yang Eng*

Main category: cs.LG

TL;DR: 在坐标网络迁移中，迁移幅度与源特异性分离；ReLU具有强选择性，而傅里叶特征幅度大但选择性差；建议在控制条件下评估架构，而非仅依赖迁移幅度。


<details>
  <summary>Details</summary>
Motivation: 当前迁移测量常基于热启动增益，但增益来源不明确，需区分是源特定结构还是通用权重重用。

Method: 在SIREN、ReLU MLPs和傅里叶特征MLPs三种隐式神经网络家族上，通过独立随机种子控制测试、2D Navier-Stokes基准和1D PDE参考解套件（热方程、粘性Burgers、聚焦三次NLS）进行对照分析。

Result: 1D几何测试中，傅里叶特征结构化迁移最大（33.1倍），但随机控制迁移高（14.24倍）；ReLU选择性最强（随机控制0.41倍）。在Navier-Stokes和1D PDE套件中，无单一架构主导，但SIREN广泛重用权值，ReLU和部分傅里叶特征更具源选择性。静态诊断弱，且迁移幅度与时间步长的缩放律被拒绝。
...
TL;DR: 提出一种新型语义策略攻击，利用大语言模型提供实时预测上下文，突破实时语音识别系统的因果信息瓶颈，使词错率提升至35.6%。


<details>
  <summary>Details</summary>
Motivation: 实时语音识别系统因因果约束对攻击者形成信息瓶颈，限制了攻击效果，需探索突破方法。

Method: 在攻击中引入大语言模型实时生成的预测上下文，增强对手能力，打破因果限制。

Result: 在语料级词错率上达到35.6%，比当前最优方法提高了三倍。

Conclusion: 揭示了常见低延迟大语言模型工具可被利用来系统性地破坏实时语音识别流水线。
...
### [116] [The Geometry of Last-Layer Model Stealing](https://arxiv.org/abs/2606.06854)
*Snigdha Chandan Khilar*

Main category: cs.LG

TL;DR: 本文使用几何方法解释了如何利用已有方法窃取机器学习模型，明确了可复制和不可复制的部分。


<details>
  <summary>Details</summary>
Motivation: 理解模型盗窃的可能性及其限制，保护模型安全。

Method: 通过几何分析研究transformer网络各层结构。

Result: 找到了完美复制最后一层的条件，并揭示了隐藏层逆向工程的极限。

Conclusion: 明确界定了模型可被窃取的范围，对模型安全性有指导意义。
...
### [117] [Modeling Nonlinear Feature Interactions with Product-Unit Residual Networks](https://arxiv.org/abs/2606.06861)
*Ziyuan Li,Uwe Jaekel,Babette Dellen*

Main category: cs.LG

TL;DR: 提出了一种乘积单元残差网络（PURe），通过整合乘法单元和残差连接来显式建模特征交互，在合成和真实数据集上相比MLP取得更好或相当的精度，在低数据场景下鲁棒性和样本效率更优，且具有更强的交互可解释性。


<details>
  <summary>Details</summary>
Motivation: 标准多层感知机隐式捕获非线性特征交互，导致表示纠缠，损害鲁棒性和可解释性。

Method: PURe集成乘法产品单元与残差连接，显式建模跨特征耦合并稳定优化；在交互驱动合成基准和两个真实数据集上评估，与MLP基线及实值/复值变种比较。

Result: PURe在预测精度上达到竞争或更优性能，对高斯噪声更鲁棒，低训练数据下样本效率更高，SHAP分析显示其学习到更集中、结构一致的交互模式。

Conclusion: PURe是一种有效建模特征交互的方法，兼具竞争力、鲁棒性和交互级可解释性。
...
### [118] [Product units in gated recurrent units improve nuclear-mass prediction](https://arxiv.org/abs/2606.06866)
*Ziyuan Li,Paulo S.A. Freitas,John W. Clark,Babette Dellen*

Main category: cs.LG

TL;DR: 提出一种基于复数域加性-乘性乘积单元门控循环单元（AM-PU-GRU）的机器学习方法，在原子核质量预测中取得最低误差，超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 利用机器学习预测原子核质量可补充理论模型，推动未知核素图的探索，现有方法存在改进空间。

Method: 在GRU中引入复数域计算、乘性交互和乘积单元变换，形成AM-PU-GRU模型，同时捕捉幅度和相位动态。

Result: 在AME2016和AME2020上插值RMSE为0.227±0.004 MeV，外推RMSE为0.179±0.015 MeV，优于其他机器学习模型及实值GRU基线。
...
### [119] [Evidence-Grounded Ensemble Diagnosis of 802.11 Packet Captures: A Multi-Stage Pipeline with Deterministic Reliability Scoring](https://arxiv.org/abs/2606.06871)
*Jerome Henry,Swadhin Pradhan,Miroslav Popovic*

Main category: cs.LG

TL;DR: PROBE 是一个多阶段管道，通过确定性归一化、多运行集成和证据框架，解决了 LLM 在诊断 802.11 数据包捕获中的捏造事件、置信度不校准和评估偏差问题，实现了高 F1 和自动接受率。


<details>
  <summary>Details</summary>
Motivation: 现有 802.11 诊断依赖专家知识，速度慢且不可扩展；LLM 方法存在捏造事件、置信度不校准和评估偏差。

Method: 包含 (i) 确定性 PCAP 到文本归一化，(ii) 多运行多候选集成，(iii) 证据为基础的无罪推定框架，(iv) 完全确定性的复合可靠性评分。

Result: 在 87 个捕获上，单次 LLM 分析 F1 从 0.871 提升至 0.912 但 35% 漏帧；朴素集成降至 0.842；证据协调后达 0.957 F1 和 96% 自动接受率，且 LLM 自信度无信息。

Conclusion: PROBE 有效消除了 LLM 诊断中的关键缺陷，提供可靠且可扩展的无线诊断方案。
...
### [120] [GlucoFM-Bench: Benchmarking Time-Series Foundation Models for Blood Glucose Forecasting](https://arxiv.org/abs/2606.06881)
*Baiying Lu,Zhaohui Liang,Ryan Pontius,Shengpu Tang,Temiloluwa Prioleau*

Main category: cs.LG

TL;DR: 提出了GlucoFM-Bench基准，系统评估了8个时间序列基础模型（TSFMs）和深度学习模型在15个糖尿病相关数据集上的血糖预测表现。预训练TSFMs（如Chronos-2、TimesFM）在零样本和少样本场景下表现出色，最佳零样本模型性能达到最佳全样本监督模型的95%；但数据充足时，轻量级LSTM以4-21%的优势超越TSFMs。在1型糖尿病和极端血糖范围仍存在挑战。


<details>
  <summary>Details</summary>
Motivation: 血糖预测对糖尿病管理至关重要，但时间序列基础模型在该领域的研究不足，需要标准化基准以评估其潜力。

Method: 构建包含8个代表性模型（预训练TSFMs、时间序列大语言模型、任务特定深度学习模型）和15个公开数据集（涵盖1型、2型糖尿病、前期糖尿病及健康人群）的基准，在零样本、少样本和全样本协议下评估，并系统变化上下文长度和预测时长。

Result: 预训练TSFMs在零样本和少样本迁移中表现强，Chronos-2和TimesFM最优，零样本性能达最佳全样本模型的95%；但全样本训练下，轻量LSTM仍最优，以4-21%优于TSFMs；分层分析显示1型糖尿病队列及低血糖/高血糖范围预测仍具挑战。

Conclusion: GlucoFM-Bench为血糖预测的基础模型评估提供了标准化、可复现的基础，揭示了TSFMs在数据稀缺时的优势及在特殊人群（如1型糖尿病）和极端血糖范围的局限性，需超越聚合误差指标进行更细致评估。
...
### [121] [Data-Constrained Language Model Pretraining: Improved Regularization and Scaling Laws](https://arxiv.org/abs/2606.06888)
*Zhiwei Xu,Shihao Wu,Hanseul Cho,Wei Hu,Yixin Wang*

Main category: cs.LG

TL;DR: 在数据受限的预训练场景下，研究了掩码输入正则化（MIR）和新的缩放定律SoftQ，发现MIR结合强权重衰减能改善验证损失，SoftQ比经典Chinchilla定律更拟合实验，且MIR的收益相当于约1.3倍独特数据。


<details>
  <summary>Details</summary>
Motivation: 训练计算增长快于自然语言数据供应，导致预训练进入数据受限、计算丰富的多轮训练模式，现有缩放定律（如Chinchilla）在数据受限下不适用，需要研究有效的正则化和缩放方法。

Method: 采用掩码输入正则化（MIR）作为辅助下一词预测损失，结合强权重衰减；提出SoftQ缩放定律，耦合模型大小和数据大小以描述重复数据下的相互作用。

Result: MIR在72M至1.4B参数模型上降低了验证损失，1.4B模型在下游任务有收益；SoftQ比Chinchilla等经典定律更拟合数据受限实验，MIR相当于增加约1.3倍独特数据。

Conclusion: 在数据受限预训练中，MIR是一种有效的正则化手段，SoftQ能更准确地预测性能，两者为未来大规模训练提供了实用方案。
...
### [122] [GRASP: Geometry-aware Residual Alignment for Scalable Pretraining Data Attribution](https://arxiv.org/abs/2606.06892)
*Yue Min,Ruining Chen,Yujun Li*

Main category: cs.LG

TL;DR: 提出GRASP，一种基于子集级反事实效用预测的交互感知数据归因方法，显著优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有数据归因方法假设样本独立贡献，忽略了子集动态如冗余和互补性，无法准确评估子集效用。

Method: 将归因重构为子集级反事实效用预测，引入GRASP，通过二次几何惩罚建模子集交互，结合低维特征草图和有限下置信界选择协议实现高效。

Result: 在子集再训练评估中，任务级秩相关性翻倍，构建成本降低近一个数量级，并迁移到语言模型和跨领域视觉选择。

Conclusion: GRASP有效捕捉子集交互，为大规模预训练语料库优化提供了坚实基础。
...
### [123] [TALAN: Task-Aligned Latent Adaptation Networks for Targeted Post-Training of Large Language Models](https://arxiv.org/abs/2606.06902)
*Chengkai Zhang,Ziteng Liu,Junpu Wang,Zeyi Tao,Yang Wang,Sagar Chordia,Qin Huang*

Main category: cs.LG

TL;DR: 提出TALAN方法，在Transformer残差流中插入序列条件潜在旁路，与低秩适配器联合训练，以提升推理、数学和代码能力，开销小且正交于适配器更新。


<details>
  <summary>Details</summary>
Motivation: 低秩适配器高效但任务全局，激活干预输入感知但需单独探针或推理时控制，希望在不损害已有优势的前提下改进STEM和代码性能。

Method: 在Transformer残差流中插入TALAN旁路，将活跃序列压缩为潜在记忆，重新混合为token级扰动并通过控制残差更新写回，沿六个轴配置，与LoRA或DoRA在单SFT循环中联合训练。

Result: 在Qwen3四个骨干和四个STEM/code基准上，TALAN在LoRA上平均提升1.41点，所有16个模型-基准单元非负；在DoRA上平均提升1.85点，13/16单元提升。可训练参数<1%，推理开销1.01-1.02倍。

Conclusion: TALAN提供实用的平台，用于研究标准适配器后训练中的可调控激活级适应，其小扰动正交于适配器更新并通过深度传播放大。
...
### [124] [The Fine-Tuning Trap: Evaluating Negative Transfer and the Role of PEFT in Sub-1B Mathematical Reasoning](https://arxiv.org/abs/2606.06920)
*Rahul Nair,Chun Tao*

Main category: cs.LG

TL;DR: 在边缘设备上微调小型语言模型（<1B参数）时，全参数微调对<300M模型有害，参数高效微调（如LoRA和DoRA）是稳定性要求。推荐所有<1B模型默认使用PEFT，避免对<500M模型使用全参数微调。


<details>
  <summary>Details</summary>
Motivation: 探索在边缘设备上高效微调小型语言模型（SLM）的策略，避免损害通用能力。

Method: 对比五种<1B模型（135M-1B）在数学推理任务上的微调方法，包括全参数微调（Full FT）、LoRA、DoRA和上下文学习（ICL）。

Result: 全参数微调在<300M模型上导致负迁移，准确率低于零样本基线；LoRA和DoRA表现相当但各有优势（DoRA在GSM8K上更优，LoRA在OrcaMath上占优）；全参数微调在Qwen2.5-0.5B上不如LoRA，在SmolLM2-135M上甚至不如5-shot ICL。

Conclusion: 建议对所有<1B的对齐模型默认使用PEFT，并警告对<500M模型避免全参数微调以防止灾难性遗忘。
...
### [125] [From Sampled Outcomes to Capability Distributions: Rethinking Supervision for LLM Routing](https://arxiv.org/abs/2606.06924)
*Guannan Lai,Haoran Hu,Long Chen,Zhenguo Li,Han-Jia Ye*

Main category: cs.LG

TL;DR: 现有LLM路由方法因依赖单次响应而引入噪声，DARS框架通过分布视角构建更可靠的监督信号，实验表明分布感知监督优于单次标签。


<details>
  <summary>Details</summary>
Motivation: 单次生成响应作为模型能力标签存在固有随机性，导致路由监督噪声大、策略不可靠。

Method: DARS从输入侧（语义等价查询变体）和输出侧（多次生成分布）建模不确定性，基于分布观测构建监督信号。

Result: 跨任务实验显示单次标签误导模型选择，分布感知监督提供更稳定标签，提升路由性能。

Conclusion: 可靠LLM路由应基于查询级能力分布，而非单次响应观察。
...
### [126] [Uniform Stability and Generalization Error of GD and SGD on Fixed-Point Parameters](https://arxiv.org/abs/2606.06934)
*Jonghyun Shin,Sejun Park*

Main category: cs.LG

TL;DR: 分析了离散参数空间中梯度下降和随机梯度下降在确定性或随机舍入下的泛化误差和稳定性，揭示了舍入对泛化性能的影响。


<details>
  <summary>Details</summary>
Motivation: 离散参数空间中的舍入操作对优化算法的泛化误差和稳定性的影响尚未被充分理解，本研究旨在填补这一空白。

Method: 通过理论分析，研究梯度下降和随机梯度下降在确定性舍入和随机舍入下的均匀稳定性和均匀参数稳定性，并推导出泛化误差的上下界。

Result: 确定性舍入使GD的泛化误差从O(T/n)恶化到O(T/√n)，均匀稳定性变为Ω(T)；SGD在确定性舍入下具有非平凡稳定性，一维时O(T/n)，高维时O(T^2/n)；随机舍入引入随维度增加的泛化误差。

Conclusion: 舍入操作会显著改变优化算法的泛化行为，且确定性舍入与随机舍入的影响不同，这些现象在实值优化中不存在。
...
### [127] [GenPO++: Generative Policy Optimization with Jacobian-free Likelihood Ratios](https://arxiv.org/abs/2606.06967)
*Ke Hu,Shutong Ding,Panxin Tao,Jingya Wang,Ye Shi*

Main category: cs.LG

TL;DR: 提出GenPO++，一种可逆生成策略优化框架，使用历史状态作为辅助记忆，实现精确逆变换和无雅可比似然比计算，在多种任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有流强化学习方法要么使用近似替代引入偏差，要么通过虚拟动作增广增加计算，需要更好的方法。

Method: 使用高阶可逆ODE求解器，将历史状态作为辅助记忆，实现精确逆变换，且对数行列式仅由固定求解器系数决定，实现精确且无雅可比的似然比计算。

Result: 在大规模模拟控制、微调和真实机器人操作任务中，达到或超过现有在线强化学习方法，同时提高训练稳定性和计算效率。

Conclusion: GenPO++有效避免了动作比偏差和虚拟动作开销，保持了生成流策略的表达能力。
...
### [128] [Accelerating Multi-Objective Bayesian Optimisation via Predictive-Gradient Catalysts](https://arxiv.org/abs/2606.06984)
*Alma Rahat,Tinkle Chugh,Jonathan Fieldsend,Richard Allmendinger*

Main category: cs.LG

TL;DR: 本文提出利用高斯过程预测梯度作为辅助信号，加速多目标贝叶斯优化的通用机制。


<details>
  <summary>Details</summary>
Motivation: 在有限评估预算下，加速收敛到全局帕累托集。

Method: 通过将代理模型的局部平稳性信息（梯度）增强到现有采集函数中，提出了自适应多梯度下降算法催化剂和预定义权重变体。

Result: 在DTLZ基准上，当代理模型准确时，相比其他采集函数显著加速，尤其对平稳问题。

Conclusion: 预测梯度催化是一种有效的多目标贝叶斯优化加速策略。
...
### [129] [Heterogeneous Effects of Green Finance on Urban Decarbonization: Evidence from 285 Cities in China](https://arxiv.org/abs/2606.06986)
*Xueyang Li,Jinlei Ma*

Main category: cs.LG

TL;DR: 绿色金融显著降低城市碳强度，主要通过能源结构优化、产业升级、外资和技术创新等渠道，且效果因城市等级和金融工具而异。


<details>
  <summary>Details</summary>
Motivation: 绿色金融已成为低碳城市转型的关键工具，但其实际脱碳效果和传导机制尚不明确。

Method: 采用计量经济模型和机器学习（SHAP分析）方法，考察绿色金融对城市碳强度的影响及中介效应。

Result: 绿色金融显著降低碳强度，其中绿色债券和绿色投资效果最强，存在空间溢出效应，对四五线城市效果最显著；中介效应按大小依次为能源结构优化、产业升级、外资和技术创新；SHAP显示绿色债券、基金和信贷贡献最大；边际效应在低技术、高工业依赖和煤基能源城市更强。
...
### [130] [Accelerating Reproducible Research in Synthetic EHR Generation](https://arxiv.org/abs/2606.06990)
*Jalen Jiang,Chufan Gao,Ethan Rasmussen,Stephen Z. Xie,Jimeng Sun*

Main category: cs.LG

TL;DR: 介绍了一个用于合成电子健康记录(EHR)生成的可复现基准测试框架，统一了数据加载、模型训练和评估流程。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型比较困难，因为代码库、数据加载器、库依赖和评估协议不统一。

Method: 实现了轻量级端到端基准测试框架，基于PyHealth，重新实现了多种基线模型（MedGAN, CorGAN, PromptEHR, HALO），并添加了GPT-2基线，提供了统一的隐私-效用评估套件。

Result: 报告了所有指标的bootstrap置信区间，分析了现有模型在长尾性能上的不足。

Conclusion: 降低了工程门槛，促进了社区驱动的可复现性和基准测试。
...
### [131] [RASFT: Rollout-Adaptive Supervised Fine-Tuning for Reasoning](https://arxiv.org/abs/2606.07006)
*Yongliang Miao,Fengyuan Liu,Wei Shi,Yanguang Liu,Fei Sun,Na Zou,Mengnan Du*

Main category: cs.LG

TL;DR: 提出一种策略感知的微调框架RASFT，通过基于策略rollout的问题级可解性校准专家监督，在推理任务上优于标准SFT和强化学习方法。


<details>
  <summary>Details</summary>
Motivation: 标准SFT严格模仿单一专家轨迹，导致过拟合表面形式并抑制模型自身推理分布。

Method: RASFT根据当前策略的rollout评估每个问题的可解性：当模型表现不佳时加强专家指导，当模型可靠时放松约束并纳入正确自生成轨迹；同时引入裁剪逆比率防止策略漂移。

Result: 在多个模型上的数学推理（6个基准）和代码推理（2个基准）实验中，RASFT在整体性能上优于SFT及其变体以及代表性强化学习方法。

Conclusion: RASFT通过动态调整监督信号有效平衡了专家知识与模型自主探索，提升了推理任务适应性。
...
### [132] [A Geometric View for Understanding Concept Learning and Neuron Interpretation in Sparse Autoencoders](https://arxiv.org/abs/2606.07007)
*Chenhao Zhang,Chris Lin,Su-In Lee*

Main category: cs.LG

TL;DR: 提出统一数学框架，将稀疏自编码器中的概念学习形式化为集合对齐问题，并定义了三种学习强度，解释了常见现象，连接了概念学习与神经元解释。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器提高了神经网络的可解释性，但缺乏对“概念”和“学习”的严格定义。

Method: 将概念形式化为数据点集合，概念学习视为人类定义概念与模型诱导概念之间的集合对齐，区分检测、分离和近似三种学习强度，并利用形式概念分析建立概念格子。

Result: 推导了几何条件、误差界和容量约束，解释了特征分裂、特征吸收、特征族和层次概念等现象，并展示了概念学习与神经元解释的多对多结构。

Conclusion: 统一框架揭示了SAE大小和稀疏度对概念学习的影响，并表明概念学习与神经元解释不必一致，可通过概念格子组织。
...
### [133] [CF-JEPA: Mask-free forward prediction with asymmetric encoder utilization for time-series representation learning](https://arxiv.org/abs/2606.07031)
*Jaehoon Lee,Sunghyun Sim*

Main category: cs.LG

TL;DR: 提出无掩码的CF-JEPA框架，通过多时域前向预测利用时间序列时序顺序，并利用编码器不对称性分别处理分类与预测任务，降低27%预测误差。


<details>
  <summary>Details</summary>
Motivation: 现有SSL方法（对比学习、掩码）存在构建正负对困难或破坏时序连续性问题，JEPA虽好但依赖掩码。

Method: 采用随机裁剪作为上下文，预测短、中、长时域未来表示，替代掩码；利用在线与EMA目标编码器不对称性，分类用在线编码器，预测/异常检测用EMA编码器。

Result: 多变量预测MSE降低27%，在UCR/UEA分类、电力变压器温度预测、KPI/Yahoo异常检测上平均准确率最高或第二。

Conclusion: CF-JEPA有效利用时序信息，无需额外训练成本，优于现有自监督方法。
...
### [134] [Hierarchical Forecast Reconciliation for Urban Rail Transit Demand Prediction under Operational Disruptions](https://arxiv.org/abs/2606.07044)
*Dang Viet Anh Nguyen,Alma Fazlagic,Kristine Pryds Loft,Filipe Rodrigues*

Main category: cs.LG

TL;DR: 提出首个用于城市轨道交通站点与OD联合预测的层次化预测协调框架，通过神经全连接协调器（FCR）实现一致性，在中断场景下OD误差降低17.45%。


<details>
  <summary>Details</summary>
Motivation: 站点级和OD级预测独立产生会导致不一致，尤其在中断期间问题更严重，需要层次协调保证结构一致性。

Method: 采用神经全连接协调器（FCR）学习从非协调基预测到一致层次预测的非线性映射，并通过约束确保结构一致性；与OLS、WLS、MinT等基准对比。

Result: 正常情况下FCR与MinT方法性能相当；中断情况下FCR优于经典方法，多步目的地延迟场景中OD预测误差降低17.45%；理想站点预测可减少34%的OD误差。
...
### [135] [Constructing VAE Latent Spaces with Prescribed Topology](https://arxiv.org/abs/2606.07058)
*Jilles S. van Hulst,Jakub M. Tomczak,W.P.M.H. Heemels,Duarte J. Antunes*

Main category: cs.LG

TL;DR: 提出一种数学框架解决变分自编码器中标准高斯先验与数据流形非欧几里得拓扑不匹配的问题，适用于乘积覆盖空间类流形，通过分解先验和协调变换实现更好的重构和表征。


<details>
  <summary>Details</summary>
Motivation: 标准高斯先验与数据流形的拓扑不匹配导致重构质量下降和表征不忠实，需要拓扑匹配的先验。

Method: 利用基本因子（圆、区间、直线）上的分解分布得到闭式KL散度；提供可重参数化的编码器-先验对及坐标变换；对商流形使用群不变特征解码；引入锚定约束。

Result: 在合成流形和旋转/循环平移MNIST数据集上，拓扑感知模型在所有正则化强度下均优于高斯基线。

Conclusion: 拓扑匹配先验有效对齐KL正则化与数据流形，提升VAE性能。
...
### [136] [Bias in Filter Feature Selection Evaluation: A Meta-Analysis of Datasets, Baselines, and Experimental Design Choices](https://arxiv.org/abs/2606.07068)
*Malick Ebiele,Malika Bendechache,Rob Brennan*

Main category: cs.LG

TL;DR: 对1994-2025年间28篇高影响力过滤特征选择研究的分析表明，新方法相对于基线方法的胜率（胜率）受数据集数量、基线数量和新方法数量的影响（R²=0.33），提示评估可能存在偏差，并提出了五项基于证据的改进建议。


<details>
  <summary>Details</summary>
Motivation: 近期表格深度学习与数据评估研究指出新方法评估可能存在有意识或无意识的偏差，推测特征选择领域，尤其是过滤特征选择，也存在类似趋势。本研究旨在识别影响过滤特征选择评估的因素和偏差入口，以推荐更强的评估原则。

Method: 分析了28篇1994-2025年间发表的高影响力过滤特征选择研究，采用多元线性回归分析以探究数据集数量、基线数量和新方法数量对胜率的影响。

Result: 多元线性回归的R²=0.33，表明33%的胜率方差可由数据集数量、基线数量和新方法数量解释。

Conclusion: R²=0.33属于中等解释力，但作为首项此类研究，结果具有前景。胜率还受领域成熟度、数据集和基线类型以及回归模型简洁性等因素影响。研究提出了五项基于证据的推荐，以改进未来过滤特征选择的评估。
...
### [137] [SlimSearcher: Training Efficiency-Aware Web Agents via Adaptive Reward Gating](https://arxiv.org/abs/2606.07074)
*Zequn Xie,Junjie Wang,Dan Yang,Jie Feng,Yue Shen,Jian Wang,Jinjie Gu*

Main category: cs.LG

TL;DR: 提出SlimSearcher框架，通过Pareto高效过滤和自适应奖励门控，在保持或提升准确率的同时显著降低工具调用次数和计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究代理依赖盲目工具调用和表演式推理，导致高计算成本和效率低下，急需平衡准确性与计算效率的方法。

Method: 在监督微调阶段采用Pareto高效过滤提取成功且经济的轨迹；在强化学习阶段引入自适应奖励门控，动态评估工具和令牌效率，并通过正确性门控避免奖励作弊。

Result: 在GAIA、BrowseComp和XBenchDeepSearch等基准上，工具调用轮次减少17%-58%，准确率保持不变或提升。

Conclusion: SlimSearcher有效推动了准确性与计算成本的帕累托前沿，实现了高效且准确的搜索行为。
...
### [138] [On the Geometry of On-Policy Distillation](https://arxiv.org/abs/2606.07082)
*Zhennan Shen,Yanshu Li,Qingyu Yin,Chak Tou Leong,Zhilin Wang,Yanxu Chen,Rongduo Han,Sunbowen Lee,Yi R. Fung*

Main category: cs.LG

TL;DR: 本文刻画了在线策略蒸馏（OPD）在参数空间中的更新轨迹，并与监督微调（SFT）和基于可验证奖励的强化学习（RLVR）进行对比，发现OPD具有独特的更新几何特性，包括稀疏性、子空间锁定等。


<details>
  <summary>Details</summary>
Motivation: 尽管OPD被广泛用于提升大语言模型的推理能力，但其训练动态尚不明确，需要深入理解其更新机制。

Method: 通过参数空间诊断工具比较OPD、SFT和RLVR的更新轨迹，分析权重影响方向、主方向约束、累积更新的子空间锁定等特性，并设计控制实验验证关键因素。

Result: OPD更新影响更少权重、更强地避开主方向，且不如RLVR紧密约束；其累积更新迅速进入窄低维通道，早期形成的更新子空间对OPD功能足够，但对SFT有害；稀疏化更新令牌和偏移生成不影响秩动态，而混合RLVR目标则会改变。

Conclusion: OPD不是SFT和RLVR之间的中间点，而是在参数空间中诱导出自身独特的更新几何结构。
...
### [139] [Residual-Controlled Multiplier Learning for Stochastic Constrained Decision-Making](https://arxiv.org/abs/2606.07088)
*Kang Liu,Jianchen Hu,Ziyu Qu*

Main category: cs.LG

TL;DR: 提出残差控制乘子学习（RCML），通过投影压力反馈分解乘子更新，解决随机约束决策中乘子更新不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 标准原始-对偶方法在随机小批量反馈下难以稳健更新乘子，噪声会直接累积到乘子记忆。

Method: 将投影乘子分解为有效压力信号和压力记忆残差，并融入模块化随机稳定组件。

Result: 在优化、分配和公平排序任务中，RCML改善了可行性控制和乘子稳定性，同时保持有竞争力的目标性能。

Conclusion: RCML通过残差积分反馈有效处理异构和噪声观测，具有理论收敛保证和局部KKT残差解释。
...
### [140] [The discovery of the effects of women employment participation on the fertility of developing countries: A panel data approach](https://arxiv.org/abs/2606.07093)
*Thi Kim Ngan Nguyen*

Main category: cs.LG

TL;DR: 本研究基于115个发展中国家1991-2018年数据，采用面板数据计量方法，发现北美和南美地区女性的生育行为受职业选择影响，而其他地区则受其他因素主导。


<details>
  <summary>Details</summary>
Motivation: 探究发展中国家女性劳动参与率对总生育率的因果关系，以理解生育率下降趋势。

Method: 将115个发展中国家分为非洲、美洲、亚太、欧洲四组，运用数据驱动的面板数据计量方法以缓解遗漏变量偏差。

Result: 美洲大陆女性的生育行为受职业选择影响；其他区域则其他因素更重要。

Conclusion: 政策制定者可参考本文激励生育决策，未来研究需纳入家庭政策和从夫居等数据。
...
### [141] [OffQ: Taming Structured Outliers in LLM Quantization by Offsetting](https://arxiv.org/abs/2606.07116)
*Haoqi Wang,Lorenz K. Mueller,Jiawei Zhuang,Mathieu Salzmann,Lukas Cavigelli*

Main category: cs.LG

TL;DR: OffQ提出了一种通过偏移机制缓解激活异常值的低比特量化方法，实现了大语言模型的有效W4A4KV4量化，在保持低比特效率的同时提升了模型精度。


<details>
  <summary>Details</summary>
Motivation: 低比特量化可加速大语言模型但激活异常值导致性能下降，现有方法难以有效处理。

Method: 利用top-1 PCA识别激活中的低维异常子空间，通过旋转将高幅度激活集中到单个通道，再将该通道转换为共享偏移以减小激活标准差。

Result: 在多种大语言模型架构和基准测试上，OffQ优于现有最先进方法，一致提高了模型精度。

Conclusion: OffQ通过新颖的偏移机制有效解决了激活异常值问题，实现了部署友好的均匀量化和低比特效率。
...
### [142] [Beyond Linear and Overcomplete Regimes: A Mean-Field Analysis of Bottleneck Autoencoders](https://arxiv.org/abs/2606.07120)
*Santanu Das,Ramyak Bilas,Pascal Esser,Satyaki Mukherjee*

Main category: cs.LG

TL;DR: 研究了非线性自编码器在固定有限维度瓶颈下的均值场学习动力学，证明有限宽度网络能逼近无限宽度解。


<details>
  <summary>Details</summary>
Motivation: 自编码器理论理解有限，尤其非线性情形缺乏分析。

Method: 采用均值场机制，推导编码器和解码器的学习动力学，并与SGD训练的有限宽度网络对比。

Result: 有限时间内有限宽度网络经验风险高概率接近均值场风险；最优时有限宽度风险收敛至均值场最优。

Conclusion: 有限宽度网络表达性足以近似无限宽度解。
...
### [143] [Learning Explicit Behavioral Models with Adaptive Questions and World-Model Probes](https://arxiv.org/abs/2606.07127)
*Hikaru Shindo,Yu Deng,Teng Cao,Quentin Delfosse,Christopher Tauchmann,Jannis Bl\"uml,Gopika Sudhakaran,Kristian Kersting*

Main category: cs.LG

TL;DR: 提出一种可训练的显式符号行为模型（ESBM），将任务表现与基于证据的问答及可执行机制预测相结合，通过自适应问题生成和局部模型编辑，在Atari风格协议中实现了高分策略与显式机制解释。


<details>
  <summary>Details</summary>
Motivation: 仅依靠任务回报训练的交互式智能体行为脆弱且难以诊断，现有方法仅在训练后使用反思和代码修复，缺乏在线机制理解。

Method: ESBM使用类型化谓词、加权规则、有界选项和机制记忆表示行为，通过自适应问题与世界模型探针将失败转化为约束，利用多准则规则选择候选模型。

Result: 在Atari风格任务中，ESBM学习了高分策略，同时生成了显式答案和可执行的机制预测。

Conclusion: 自适应问题可以作为机制性策略学习的训练压力和可复用基准。
...
### [144] [A machine-learning-assisted progressive digit-randomness screening framework for detecting non-random patterns in raw numerical research data](https://arxiv.org/abs/2606.07128)
*Zhuphua Cao*

Main category: cs.LG

TL;DR: 提出了FDRS模型，用于检测数值研究数据中的非随机数字模式异常，通过统计和机器学习方法综合评估数据完整性。


<details>
  <summary>Details</summary>
Motivation: 原始数值数据集在完整性筛查中受到的关注较少，现有方法主要针对图像、抄袭或摘要统计不一致，缺乏对数字模式异常的系统检测。

Method: FDRS整合了单小数位和联合小数位测试、Cramer's V、熵度量、KL散度、数字偏好指数、渐进子采样和半监督风险评分。

Result: 在两个数据集上的评估显示，FDRS能有效区分正常数据（RawData）和模拟异常数据（ErrData），内部验证中Elastic-net逻辑回归AUC最高（0.98395），随机森林准确率最高（0.926667）。外部基准测试支持分级风险分层。

Conclusion: FDRS可作为辅助的数字结构筛查工具，用于优先审查原始数值数据集，但不能单独作为伪造或不当行为的证据。
...
### [145] [$\alpha$-PFN: Fast Entropy Search via In-Context Learning](https://arxiv.org/abs/2606.07134)
*Herilalaina Rakotoarison,Steven Adriaensen,Tom Viering,Carl Hvarfner,Samuel M\"uller,Frank Hutter,Eytan Bakshy*

Main category: cs.LG

TL;DR: 提出一种两阶段摊销策略，通过PFN网络学习熵搜索采集函数，加速贝叶斯优化，速度提升超50倍。


<details>
  <summary>Details</summary>
Motivation: 现有熵搜索采集函数依赖复杂缓慢的蒙特卡洛近似，计算繁琐且易引入误差。

Method: 先用PFN学习最优信息条件，再用α-PFN预测期望信息增益，实现单次前向传播近似。

Result: 在合成和真实基准上性能与先进方法持平，且加速超50倍。

Conclusion: α-PFN提供灵活快速的学习近似，替代复杂启发式方法，显著提升效率。
...
### [146] [Explaining Unsupervised Disease Staging in Huntington's Disease: Insights into Model Representations and Clusters](https://arxiv.org/abs/2606.07135)
*Lubna Mahmoud Abu Zohair,Hind Zantout*

Main category: cs.LG

TL;DR: 通过可解释性分析，验证了无监督机器学习在亨廷顿舞蹈症疾病进展建模中捕捉到的临床有意义结构，并揭示了与已知病程一致的分层模式。


<details>
  <summary>Details</summary>
Motivation: 无监督机器学习虽能揭示疾病进展轨迹，但可解释性差，限制了临床信任和转化。

Method: 扩展已有疾病分期框架，对Enroll-HD数据应用低维投影、显著图（saliency maps）和SHAP进行可解释性分析。

Result: 学习到的嵌入与运动及功能严重程度评分一致，SHAP揭示了从早期认知-运动障碍到严重功能依赖的疾病阶段分层，并显示阶段内变异性。
...
### [147] [REMEDI: A Benchmark for Retention and Unlearning Evaluation in Multi-label Clinical Disease Inference](https://arxiv.org/abs/2606.07141)
*Anurag Sharma,Sai Teja Chunchu,Prasenjit Mitra,Sandipan Sikdar,Koustav Rudra*

Main category: cs.LG

TL;DR: REMEDI是专为临床疾病推断中的机器遗忘设计的基准，涵盖多标签和多分类任务，使用真实临床数据评估遗忘方法。


<details>
  <summary>Details</summary>
Motivation: 现有遗忘方法多用于非医疗领域，且基准依赖合成数据，缺乏真实代表性，医疗领域的遗忘效果不明确。

Method: 基于MIMIC-III数据库构建，包含多种遗忘实例设置，评估多标签和多分类任务，综合考虑效用和遗忘程度。

Result: 实验表明现有方法在效用和遗忘间存在权衡，且不适用于多标签分类。

Conclusion: REMEDI公开可用，促进可重复研究，并揭示医疗遗忘任务的挑战。
...
### [148] [Decision-Aware Evaluation of Physics-Informed Surrogates](https://arxiv.org/abs/2606.07146)
*Daniel Cie\'slak,Andrzej Czy\.zewski*

Main category: cs.LG

TL;DR: 提出pinn-gym基准，评估物理信息机器学习作为决策系统而非仅曲线预测器。


<details>
  <summary>Details</summary>
Motivation: 传统基于曲线误差评估不充分，工程应用依赖下游决策能力。

Method: 建立透明oracle、无量纲目标、多指标协议（曲线保真度、物理可行性、top-k检索、遗憾）。

Result: 低nRMSE不足以识别有用设计；物理信息损失改变权衡；无量纲化改善可比性。

Conclusion: pinn-gym作为可复现测试床，用于评估PIML代理的决策性能。
...
### [149] [Geodesics of Dynamic Graphs for Regime Change Detection](https://arxiv.org/abs/2606.07151)
*William Cappelletti,\'Etienne Voutaz,Pascal Frossard*

Main category: cs.LG

TL;DR: 本文提出了一种在动态网络中检测演化模式变化的新方法，将动态网络中的时间段定义为沿着图空间测地线的连贯轨迹，并通过图回归方法测量观察图与估计测地线之间的累积距离来检测变化点。


<details>
  <summary>Details</summary>
Motivation: 传统的动态网络变化点检测假设状态之间突然转变，忽略了现实世界中连续演化的情况，如社交网络和物理系统。

Method: 将时间段定义为时间图中连贯动态的轨迹，即图空间中沿测地线的路径；利用图回归方法测量观察图序列与端点间估计测地线的累积距离，并结合变化点检测算法。

Result: 在动态网络实验中，该方法在变化轨迹和不同速度的场景下优于现有的变化点检测模型；在新冠疫情移动数据分析中，检测到的变化点与外部事件更对齐。

Conclusion: 该工作是首次在图上建模和检测演化模式之间的变化，为分析复杂时间图数据提供了现实且强大的工具。
...
TL;DR: 提出RETROSPECT系统，结合ChemAlign Transformer生成器和LambdaMART重排序器，在USPTO-50K上实现55.00% top-1和86.18% top-10准确率，重排序后提升至59.4% top-1。


<details>
  <summary>Details</summary>
Motivation: 单步逆合成需要精确的首选建议和丰富的候选列表，现有方法难以兼顾。

Method: 采用提案-选择分解架构：Transformer生成器（ChemAlign）经混合增强、Pre-LayerNorm等训练，配合LambdaMART重排序器，使用结构、反应模板等特征。

Result: 生成器top-1和top-10准确率分别达55.00%和86.18%，重排序后top-1提升至59.4%，平均倒数排名0.7171。
...
### [151] [Structure-Preserving Correction Learning for Sparse Bayesian Inference in Brain Source Imaging](https://arxiv.org/abs/2606.07196)
*Marco Morik,Xiao Ruiting,Shinichi Nakajima,Stefan Haufe,Ismail Huseynov*

Main category: cs.LG

TL;DR: 提出一种将经典联合超参数求解器展开为可训练神经网络的方法，通过学习结构化校正项改善M/EEG脑影像重建，同时保持贝叶斯推断的可解释性。


<details>
  <summary>Details</summary>
Motivation: 经典稀疏Type-II贝叶斯方法依赖固定迭代更新规则，无法从数据中自适应学习，限制了性能。

Method: 将经典联合超参数求解器展开为神经架构，层间镜像原始迭代；初始化时恢复经典求解器，然后通过可学习偏置、MLP和注意力机制逐步学习校正项，保留贝叶斯结构。

Result: 学习到的校正变体在重建性能和收敛行为上优于基线展开求解器，同时保持算法透明性。

Conclusion: 结构化校正学习能在不取代原有模型推理机制的前提下提升经验重建性能，兼顾性能与可解释性。
...
### [152] [DEFINED: A Data-Efficient Computational Framework for Fine-Grained Creativity Assessment in Debate Scenarios](https://arxiv.org/abs/2606.07226)
*Tongzhou Yu,Mingjia Li,Hong Qian,Wenkai Wang,Zongbao Zhang,Yaoyu Jiang,Xiangfeng Wang,Aimin Zhou,Jiajun Guo*

Main category: cs.LG

TL;DR: 提出DEFINED框架，用于辩论场景中细粒度创造力评估，通过分层八维指标系统和预训练语言模型实现高效、准确的自动评分。


<details>
  <summary>Details</summary>
Motivation: 大语言模型时代人类创造力成为关键能力，但现有评估依赖简单任务和昂贵人工评分，缺乏复杂场景下的细粒度方法；辩论作为生态有效的评估场景，兼具发散与收敛思维且数据丰富，亟需自动评分方案。

Method: 构建分层八维创造力度量体系，采用预训练自回归语言模型结合分层评分头；从真实辩论中获取陈述及专家评分，使用约束数据增强缓解精英偏差，并采用混合粒度训练策略学习有限细粒度标注。

Result: 在评估协议下，模型实现准确稳定评分，优于基于提示的大语言模型评估器和现有辩论评分方法。

Conclusion: DEFINED能有效进行辩论场景的细粒度创造力评估，且通过生态效度验证，适用于中低水平人群的定性案例分析。
...
### [153] [Generative Molecular Morphing for Flexible-Size Design via Unbalanced Optimal Transport](https://arxiv.org/abs/2606.07239)
*Malte Franke,Stefan P. Schmid,Zarko Ivkovic,Kjell Jorner,Andreas Krause*

Main category: cs.LG

TL;DR: 提出Morph，一种基于几何图的可变大小生成模型，通过动态调整原子数实现条件与无条件3D分子设计，在匹配现有模型性能的同时提供无与伦比的采样灵活性，并能在之前模型失败的任务上生成分布外样本。


<details>
  <summary>Details</summary>
Motivation: 分子性质与尺寸紧密相关，但现有扩散和流模型固定原子数，无法准确捕捉性质与原子数的联合分布，限制了导向能力。

Method: Morph，基于几何图的可变大小生成模型，通过动态调整尺寸整合现有结构先验（如骨架），增强属性导向。

Result: 匹配当前最先进的固定尺寸模型，同时提供更灵活的采样，在先前模型失败的场景下实现分布外生成。

Conclusion: 为增强分子设计的生成建模开辟了新途径。
...
### [154] [A Held-Out Transition-Pair Falsifier for Long-Horizon Non-Abelian State Tracking](https://arxiv.org/abs/2606.07254)
*Jeonghoon Lee*

Main category: cs.LG

TL;DR: 提出一种用于有限非阿贝尔群组跟踪的留出转移对伪造协议，并展示投影循环状态模型在此协议下能完美预测超长序列的最终状态，而其他基线模型表现不佳，表明显式投影非交换状态组合是一种有用的归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 状态跟踪中序列模型难以处理非交换变换的潜在状态，需要更好的归纳偏置。

Method: 采用留出转移对伪造协议，禁止训练中出现特定有序生成元对，并在评估中要求相同局部模式；使用投影循环状态模型，强制显式非交换状态组合。

Result: 在S3×S3基准上，仅用长度8序列训练的投影循环状态模型在长达1,048,576个token的评估中实现完美最终状态预测（250/250），而GRU、结构化SSM等基线模型接近随机水平。
...
### [155] [Where Rectified Flows Leak: Characterising Membership Signals Along the Interpolation Path](https://arxiv.org/abs/2606.07271)
*Thomas Sesmat,Gabriel Meseguer-Brocal,Geoffroy Peeters*

Main category: cs.LG

TL;DR: 本研究发现Rectified Flows对训练数据存在隐式记忆，在插值路径上呈现钟形重建差异，并利用该特征实现成员推理攻击。


<details>
  <summary>Details</summary>
Motivation: 理解生成模型从训练数据中保留的信息对版权和隐私至关重要，模型可能编码细微痕迹而非仅逐字复制。

Method: 分析Rectified Flows的插值路径X_λ，研究训练与测试数据重建差异的分布，并在高斯假设下推导峰值位置的闭式解。

Result: 差异呈钟形曲线，峰值位置与理论预测一致，该结构在音频和图像上普遍存在，且训练中逐渐累积而验证指标稳定。

Conclusion: 钟形结构可用于成员推理攻击，有效区分训练集与非训练集成员。
...
### [156] [Closed-Form Spectral Regularization for Multi-Task Model Merging](https://arxiv.org/abs/2606.07289)
*Yongxian Wei,Runxi Cheng,Xingxuan Zhang,Li Shen,Chun Yuan,Peng Cui,Dacheng Tao*

Main category: cs.LG

TL;DR: 提出一种基于谱滤波的模型合并方法，通过软指数滤波和硬截断实现高效多任务合并，大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有模型合并方法中，迭代求解器虽性能优于封闭解，但计算成本高且原理不明，需解释其有效性并设计更高效的替代方案。

Method: 将多任务模型合并形式化为带噪线性逆问题，提出谱滤波估计器SWUDI，结合软指数滤波（匹配梯度流轨迹）和硬top-K截断（抑制噪声放大方向），并扩展自适应变体SWUDI-A。

Result: 在四个通用基准和多模态合并基准上匹配或超越现有方法，同时减少28-72倍运行时间和高达50%的峰值GPU内存。

Conclusion: 所提谱求解器无需训练数据或优化器状态，通过单次对称特征分解实现高效合并，在性能与效率上均具优势。
...
### [157] [Trio: Learning Time-Series Forecasting with Temporal-Spatial-Sample Attention and Structural Causal Priors](https://arxiv.org/abs/2606.07291)
...
TL;DR: 提出Trio模型，通过时间-空间-样本注意力机制和历史示例重用提升多变量时间序列预测。


<details>
  <summary>Details</summary>
Motivation: 现有PFN方法难以直接应用于时间序列，因为其忽略时序动态、变量依赖和历史模式。

Method: 设计Trio架构，包含时间注意力（捕获窗口内动态）、空间注意力（建模变量间依赖）和样本注意力（检索历史序列对指导预测），并引入TS-SCM生成结构化合成任务。

Result: 在合成、工业及公开数据集上，Trio提升了预测性能；零样本实验表明TS-SCM提供有用先验。

Conclusion: 样本注意力机制有效利用历史示例，但通用PFN式预测仍是开放问题。
...
### [158] [Bootstrap Theory of Representational Emergence: Explanatory Insufficiency as a Driver of Representation Learning and World Models](https://arxiv.org/abs/2606.07303)
*Jacques Raynal,Pierre Slangen,Elsa Raynal,Jacques Margerit*

Main category: cs.LG

TL;DR: 提出了表示涌现的自举理论（TBER），描述当现有表示解释力不足时新表示如何产生。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注表示优化，较少关注何时需要新表示层次；TBER 旨在填补这一空白。

Method: TBER 通过五个阶段形式化过程：稳定观测、异常检测、解释不足识别、表示涌现、临时稳定。

Result: 该理论应用于表示学习、潜在空间、基础模型、世界模型、数字孪生、自适应生物系统和科学发现。

Conclusion: 未来AI系统可能受益于检测自身内部表示解释限制的机制。
...
### [159] [TabSwift: An Efficient Tabular Foundation Model with Row-Wise Attention](https://arxiv.org/abs/2606.07345)
*Si-Yang Liu,Han-Jia Ye*

Main category: cs.LG

TL;DR: TabSwift通过轻量级架构和两项简单增强（门控注意力稳定机制、可学习寄存器令牌），在保持竞争力的同时实现高效的表格上下文学习，并支持自适应早退机制。


<details>
  <summary>Details</summary>
Motivation: 现有表格基础模型为提高准确性采用复杂架构，导致推理成本高、部署受限。

Method: 基于原始TabPFN，采用仅含行注意力的轻量级骨干，加入门控注意力稳定机制和少量可学习寄存器令牌以提供全局上下文并改善预训练质量，同时引入自适应逐层早退机制。

Result: TabSwift在分类和回归任务上与更强的模型（如TabPFN v2和TabICL）竞争力相当，但推理更高效。

Conclusion: TabSwift实现了高效且任意时刻可用的表格上下文学习，适合实际部署。
...
### [160] [SleepExplain: Explainable Non-Rapid Eye Movement and Rapid Eye Movement Sleep Stage Classification from EEG Signal](https://arxiv.org/abs/2606.07351)
*Rafsan Jany,Md. Hamjajul Ashmafee,Iqram Hussain,Md Azam Hossain*

Main category: cs.LG

TL;DR: 提出SleepExplain模型，使用集成学习和SHAP解释NREM/REM睡眠阶段分类，准确率达94.30%。


<details>
  <summary>Details</summary>
Motivation: 睡眠阶段分类对睡眠障碍诊断至关重要，EEG能有效识别睡眠相关神经变化，NREM和REM阶段对神经和身体功能有重要影响。

Method: 使用随机森林、XGBoost和梯度提升集成模型进行分类，并采用SHAP方法解释预测结果。

Result: 随机森林准确率92.54%，梯度提升94.25%，XGBoost 94.30%。
...
### [161] [Breaking the Ice: Analyzing Cold Start Latency in vLLM](https://arxiv.org/abs/2606.07362)
*Huzaifa Shaaban Kabakibo,Animesh Trivedi,Lin Wang*

Main category: cs.LG

TL;DR: 首次系统地对vLLM推理引擎的冷启动延迟进行性能表征，提出轻量级预测模型。


<details>
  <summary>Details</summary>
Motivation: 可扩展推理服务中冷启动延迟重要，但vLLM因复杂性和快速演变缺乏系统性研究。

Method: 将启动过程分解为六个步骤，分析CPU绑定特性和扩展趋势，构建轻量级分析模型。

Result: 各步骤的一致可解释扩展趋势，模型准确预测启动延迟。

Conclusion: 模型为大规模推理环境资源规划提供指导，已开源工具和数据集。
...
### [162] [A robust PPG foundation model using multimodal physiological supervision](https://arxiv.org/abs/2606.07365)
*Eloy Geenjaar,Vince Calhoun,Scott Daly,Gouthaman KV,Lie Lu,Trisha Mittal,Daniel P. Darcy*

Main category: cs.LG

TL;DR: 提出一种结合多模态信号对比学习的PPG基础模型，无需高质量预训练数据，在14/15任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有PPG基础模型要么依赖高质量ICU数据导致泛化差，要么使用封闭的领域数据，需要一种不依赖高质量或领域数据的方法。

Method: 利用ICU数据中的ECG和呼吸信号选择对比学习样本，保留噪声PPG段，增强鲁棒性。

Result: 在3倍更少主体上预训练，在15个下游任务中14个提升性能。

Conclusion: 多模态监督能整合互补生理信息，提升PPG模型鲁棒性和泛化到消费级数据的能力。
...
### [163] [Self-evolving LLM agents with in-distribution Optimization](https://arxiv.org/abs/2606.07367)
*Yudi Zhang,Meng Fang,Zhenfang Chen,Mykola Pechenizkiy*

Main category: cs.LG

TL;DR: 提出Q-Evolve框架，通过统一的自动过程奖励标注和策略学习，实现LLM智能体在稀疏奖励环境下的稳定自进化。


<details>
  <summary>Details</summary>
Motivation: LLM智能体在长期决策中面临信用分配问题，延迟奖励导致训练困难，现有方法缺乏可靠的过程监督和分布内学习。

Method: 在每次迭代中，使用混合离线数据（专家+智能体轨迹）学习分布内批评者，通过加权隐式Q学习稳定贝尔曼备份；基于优势估计推导步骤过程奖励，再执行行为近端策略优化以迭代改进策略。

Result: 在AlfWorld、WebShop、ScienceWorld上，Q-Evolve在样本效率、鲁棒性和任务性能上均优于强基线。

Conclusion: 通过过程监督与策略在共享分布内学习循环中的协同进化，可以实现LLM智能体的稳定自进化。
...
### [164] [Do Coding Agents Deceive Us? Detecting and Preventing Cheating via Capped Evaluation with Randomized Tests](https://arxiv.org/abs/2606.07379)
*Thanawat Lodkaew,Johannes Ackermann,Soichiro Nishimori,Nontawat Charoenphakdee,Masashi Sugiyama,Takashi Ishida*

Main category: cs.LG

TL;DR: 提出CapCode框架和CapReward奖励设计，通过设定评估上限检测和减少智能体作弊行为。


<details>
  <summary>Details</summary>
Motivation: 现有评估中模型可能通过捷径作弊获得高分，使评估分数不可靠。

Method: CapCode框架创建随机测试，最佳非作弊性能被故意限制在阈值以下；CapReward基于该原则抑制优化超过上限。

Result: CapCode能检测作弊且保持模型排名，CapReward减少作弊行为，使模型更符合任务规范。

Conclusion: 该设计提供更清晰的评估分数解释，并有效抑制作弊。
...
### [165] [Covariance Shrinkage via Stochastic Interpolation](https://arxiv.org/abs/2606.07382)
*Mathieu Chalvidal,Florentin Coeurdoux,Eric Vanden-Eijnden*

Main category: cs.LG

TL;DR: 将经典的高维协方差收缩估计重新表述为随机插值上的经验风险最小化，揭示了三种降低风险的机制，并提出了神经估计器，在合成和真实数据上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 重新审视高维协方差收缩估计，通过随机插值框架理解并改进其风险降低机制。

Method: 将收缩估计视为源分布与目标分布之间参数化随机插值的经验风险最小化，引入调度、流映射与耦合（如最优传输）及早停三种机制，并提出基于神经网络的插值估计器及二次风险上界。

Result: 合成实验验证了所提上界的有效性，在真实神经影像数据上展示了额外的正则化能力。

Conclusion: 该框架统一了经典收缩估计，并通过插值设计提供了新的降风险途径，在实践中具有更强的正则化效果。
...
### [166] [Making the Most of Limited Data: Score-Aware Training for Text-to-Music Generation](https://arxiv.org/abs/2606.07387)
*Yun-Chen Cheng,Tzu-Hung Huang,Chih-Pin Tan*

Main category: cs.LG

TL;DR: 提出了一种基于得分感知训练（score-aware training）的文本到音乐生成系统，通过利用音频-标题对齐分数作为监督信号，重新利用低分片段、过滤错配样本、两阶段标题生成以及额外损失函数，在不依赖大规模专有数据和工业级计算资源的情况下，在ICME 2026 ATTM竞赛中取得了优异成绩。


<details>
  <summary>Details</summary>
Motivation: 现有最先进的文本到音乐生成系统依赖于大规模专有数据集和工业级算力，难以区分架构贡献与资源优势。

Method: 采用得分感知训练，利用CLAP条件化的Beta噪声时间步调度将低分片段重新用于高噪声训练；使用片段级过滤去除最错配的样本；两阶段标题生成弥合训练标题与推理提示之间的分布差距；以及REPA辅助损失从预训练编码器迁移结构化语义知识。

Result: 基于FluxAudio的450M参数系统在ICME 2026 ATTM竞赛客观评估中排名第二，在效率赛道最终MOS评估中排名第三。

Conclusion: 该工作表明，通过巧妙利用对齐分数和少量样本，可以在不依赖大规模数据和计算的情况下提升文本到音乐生成性能。
...
### [167] [Generative Modeling of Discrete Latent Structures via Dynamic Policy Gradients](https://arxiv.org/abs/2606.07400)
*Stefan Ivanovic,Ge Liu,Mohammed El-Kebir*

Main category: cs.LG

TL;DR: 提出GReinSS框架，通过动态缩放奖励的策略学习从间接观测中推断组合潜在状态，在模拟和真实数据上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法如期望最大化无法扩展到组合空间，深度学习方法往往学习人工潜在状态而非机械真实状态，需要能重建真实潜在状态的方法。

Method: 提出GReinSS，一个使用动态缩放奖励的策略学习框架，学习最大化观测数据似然的潜在状态分布。

Result: 在模拟数据上准确重建潜在集合和潜在图，优于其他基线；在真实RNA测序数据上，重建的亚型比RSEM算法更匹配长读测序结果。

Conclusion: GReinSS是一种有原则且实际有效的生成建模和推断组合潜在状态的方法。
...
### [168] [Reversible Foundations: Training a 120B Sparse MoE through State-Preserving Scaling](https://arxiv.org/abs/2606.07404)
*Rohan Shravan*

Main category: cs.LG

TL;DR: 本文报告了在单个八GPU节点上端到端训练千亿参数稀疏混合专家模型的经验，通过可逆递归、状态保持增长和量化适配器策略实现。


<details>
  <summary>Details</summary>
Motivation: 在有限硬件资源下训练超大规模语言模型，降低计算和存储成本。

Method: 采用可逆递归栈减少激活内存；通过状态保持增长从密集种子逐步扩展至120B模型；使用TQP策略将优化器状态压缩至适配器参数。

Result: 成功在单个节点上训练120B模型，活跃参数5.93B，损失1.78，并发布模型族。

Conclusion: 已知原语的有效集成可实现单节点训练大规模MoE模型，且通过设计获得特定能力（如多语言、代码）。
...
### [169] [A Comprehensive Anatomy of Human and DeepSeek-R1 LLM Mathematical Reasoning](https://arxiv.org/abs/2606.07410)
*Yuxiang Chen,Jun Wang*

Main category: cs.LG

TL;DR: 对DeepSeek-R1与人类推理的结构比较发现模型存在拓扑模仿，但成功轨迹展示了真正推理的信号；反思的有效性取决于上下文。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型是否真正推理还是仅模仿推理的外观，通过比较模型与人类的推理步骤。

Method: 对AIME 2025的30个问题进行全面的实证比较，详尽注释了10,247个推理步骤到五个功能类别：分析、推理、分支、回溯和反思。

Result: 人类解决方案紧凑，模型频繁进行浅层验证和循环，但成功轨迹中分支和回溯使用稳定，反射在演绎推理中有效。

Conclusion: 当前长思维链模型可能更多因推理外观而非真实演绎进步获得奖励；改进方向包括测量跨迹稳定性、惩罚'旋转轮'迹、鼓励更深的逻辑校正以及将推理时间计算重分配到演绎和回溯。
...
### [170] [Sparsely gated tiny linear experts](https://arxiv.org/abs/2606.07414)
*Simon Schug*

Main category: cs.LG

TL;DR: 通过将专家缩小为单个神经元并去除非线性，稀疏门控线性神经元（sgatlin）在等计算量下提升语言模型困惑度，同时增强可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有MoE模型专家仍大而密集，希望进一步增加稀疏性以提高计算效率和可解释性。

Method: 将每个专家缩减为单个神经元，并去除专家中的非线性激活，形成稀疏门控线性神经元（sgatlin）网络。

Result: 在等计算量比较中，用sgatlin替换所有Transformer前馈层改善了语言模型的困惑度。

Conclusion: sgatlin提供了一条通向计算高效且可解释的Transformer前馈层的路径。
...
### [171] [Video-Based Prediction of In-Flight Particle Characteristics in Atmospheric Plasma Spraying](https://arxiv.org/abs/2606.07416)
*Abhijeet Praveen,Sareh Soleimani,Cormac Cureton,Aman Sidhu,Kintak Raymond Yu,Cristian Cojocaru,Narges Armanfard*

Main category: cs.LG

TL;DR: 利用高速视频观测等离子体羽流，通过多种特征表示和机器学习模型（TabPFN、CNN等）预测大气等离子喷涂中飞行粒子的温度和速度，实现了非侵入式诊断的高精度（R²达0.90）。


<details>
  <summary>Details</summary>
Motivation: 大气等离子喷涂过程中，飞行粒子的温度和速度对涂层质量影响重大，但难以连续监控，因此需要开发非侵入式数据驱动诊断方法。

Method: 引入三种视频衍生特征表示，采用TabPFN、CNN及随机森林、梯度提升、支持向量回归、XGBoost等基线模型，基于63次喷涂实验的126个视频记录进行分组留一交叉验证。

Result: TabPFN在温度预测上表现最稳定（R²=0.86），CNN在速度预测上较强（R²=0.81）。直接使用原始视频帧的预训练CNN达到最高性能：温度R²=0.90，速度R²=0.82。
...
### [172] [Discovering Multiscale Deep Formulas in Complex Systems via Neural-Guided Lambda Calculus](https://arxiv.org/abs/2606.07426)
*Hanqiao Yu,Shusen Yang,Xuebin Ren,Cong Zhao*

Main category: cs.LG

TL;DR: 提出Deflex方法，自动从多尺度复杂系统中提取不同形式的尺度特定公式，效率最高提升7倍。


<details>
  <summary>Details</summary>
Motivation: 现有AI方法在单尺度系统中表现良好，但在多尺度复杂系统中难以识别尺度特定公式。

Method: Deflex由Deflexformer（可分解深度能量模型）和Deflexpressor（λ演算符号回归模型）两个子系统组成，通过合成数据预训练并指导公式发现。

Result: 在六个代表性复杂系统上，Deflex比现有方法效率高7倍，实现自动化多尺度发现。
...
### [173] [Time series Foundation Models based on Physics-Informed Synthetic Histories for Cold-Start Photovoltaic Forecasting](https://arxiv.org/abs/2606.07457)
*Lorenzo Longarini,Alessandro Rongoni,Simone Silenzi,Emanuele Frontoni,Riccardo Rosati*

Main category: cs.LG

TL;DR: 提出一种零样本管道，利用合成生产历史使时间序列基础模型在光伏冷启动预测中优于传统基线1.7-2倍。


<details>
  <summary>Details</summary>
Motivation: 冷启动场景下缺乏目标场址观测数据，标准监督预测方法无法直接使用。

Method: 从电站元数据和气象协变量生成合成生产历史，通过推理时条件输入时间序列基础模型（如TabPFN-TS、Chronos-2）进行预测，并在三种反馈策略下评估。

Result: 协变量感知模型显著优于基线：TabPFN-TS在实时反馈下误差最低（MAE 0.514 kWh/kWp/d），Chronos-2在自预测反馈下最鲁棒；性能对合成历史来源不敏感。

Conclusion: 预测准确性主要取决于合理的时序上下文而非具体生成器，验证了零样本管道在冷启动预测中的有效性。
...
### [174] [Unsupervised Continual Clustering via Forward-Backward Knowledge Distillation](https://arxiv.org/abs/2606.07474)
*Mohammadreza Sadeghi,Sareh Soleimani,Zihan Wang,Narges Armanfard*

Main category: cs.LG

TL;DR: 提出FBCC方法，通过前向-后向知识蒸馏实现无监督持续聚类，无需存储旧数据，减少灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有无监督持续学习方法存在内存和隐私问题，且忽略聚类目标，无法有效应对灾难性遗忘。

Method: 采用持续教师网络和轻量级任务特定学生，通过双阶段前向-后向蒸馏过程学习新聚类并保留旧聚类结构，不存储旧数据。

Result: 在四个基准数据集上，FBCC在聚类准确性上持续优于现有持续学习基线，并显著减少灾难性遗忘。

Conclusion: FBCC是无监督持续聚类的开创性方法，能有效提升聚类性能并减少遗忘。
...
### [175] [Graph Neural Network leveraging Higher-order Class Label Connectivity for Heterophilous Graphs](https://arxiv.org/abs/2606.07475)
*Takuto Takahashi,Itsuki Nakayama,Takahiro Mitani,Ryosuke Kikuchi,Yuya Sasaki,Makoto Onizuka*

Main category: cs.LG

TL;DR: 提出标签上下文分类器（LCC）以捕获有向图中的高阶类别标签连通性，集成到GNN中提升异质图节点分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有GNN在同质图上表现好，但在异质图上因无法捕获高阶类别标签连通性而性能受限。

Method: 设计LCC，通过四种不同游走方式生成标签上下文嵌入，估计目标节点类别，并自适应学习LCC与任意GNN的融合权重。

Result: 集成LCC的GNN在异质有向图上优于最先进方法，标签上下文嵌入提升了节点分类性能。

Conclusion: LCC能有效捕获高阶标签连通性，提升GNN在异质图上的分类能力。
...
### [176] [Drifting Models for Surrogate Flow Modeling](https://arxiv.org/abs/2606.07481)
*Chris R. Jung,Markus D\"orr,Natalie J\"ungling,Jennifer Niessner,Adam T. M\"uller,Nicolaj C. Stache*

Main category: cs.LG

TL;DR: 提出条件漂移框架，在VAE潜在空间中进行快速单次生成，替代迭代扩散方法，实现实时CFD代理。


<details>
  <summary>Details</summary>
Motivation: CFD计算成本高，现有生成式代理模型迭代采样慢，需要高效的单次生成方法。

Method: 在VAE潜在空间中应用条件漂移，使用标签感知掩码对齐边界条件，并引入空间条件变体。

Result: 条件漂移模型在精度和流一致性上匹配扩散模型，速度快两个数量级，空间条件变体有望泛化到未见几何体。

Conclusion: 条件漂移是扩散方法的高效替代，特别适用于推理速度关键的实时CFD代理场景。
...
### [177] [Network Recovery from Cascade Data: A Debiased Jacobian-Based Machine Learning Approach](https://arxiv.org/abs/2606.07483)
*Lei Huang*

Main category: cs.LG

TL;DR: 提出CascadeNet，一种无需指定扩散模型的雅可比机器学习框架，用于恢复级联中的隐藏影响网络，具有√n一致性和渐近正态性，模拟和实际应用中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有网络恢复方法依赖特定扩散模型，模型错误时性能大幅下降，需要一种不依赖模型假设的通用方法。

Method: 利用一步转移函数的雅可比矩阵表征影响结构，构建灵活估计器，应用Neyman正交化去偏得到去偏雅可比矩阵。

Result: 模拟中在九种数据生成过程中恢复精度最高；COVID-19应用中恢复的网络与真实移动网络显著相关，基线方法无显著关联。

Conclusion: CascadeNet无需指定扩散模型即可高效恢复网络结构，支持统计推断，具有实际应用价值。
...
### [178] [CoMetaPNS: Continually Meta-learning Personalized Neural Surrogates for Cardiac Electrophysiology Simulations](https://arxiv.org/abs/2606.07488)
*Ryan Missel,Xiajun Jiang,Linwei Wang*

Main category: cs.LG

TL;DR: 提出了一种持续元学习框架，用于个性化心脏神经代理，能够持续集成新数据并识别已知或未知动态源，避免灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有模型在连续数据流下需要全量重训练，无法避免灾难性遗忘，限制了临床适用性。

Method: 利用持续贝叶斯高斯混合模型在内存缓冲上，推断数据标识和关系，实现元学习中的持续个性化。

Result: 在合成心脏数据上，该框架在模拟预测、计算可扩展性和抗遗忘方面优于现有基线。

Conclusion: 该框架能有效持续集成信息并识别数据动态源，克服了传统方法的局限。
...
### [179] [Second-Order Path Kernel Interpolation Formulas in Machine Learning](https://arxiv.org/abs/2606.07495)
*Jin Guo,Roy Y. He,Jean-Michel Morel*

Main category: cs.LG

TL;DR: 开发了梯度下降训练神经网络的二阶插值公式，揭示了曲率和采样噪声对预测的贡献，细化了路径核解释。


<details>
  <summary>Details</summary>
Motivation: 理解训练数据如何影响神经网络预测是现代学习理论的核心问题，已有的一阶插值公式存在局限，需要发展二阶形式。

Method: 推导了二阶插值公式，包括曲率加权项；对于随机梯度下降，引入了与梯度噪声协方差耦合的采样诱导项；扩展到带动量的随机梯度下降，权重由记忆因子修正；并给出了预测的浓度估计。

Result: 二阶插值公式补充了路径核插值，随机梯度下降中出现额外分量，动量方法保持插值结构但权重调整。

Conclusion: 这些结果细化了神经网络预测的路径核解释。
...
### [180] [Accelerated Decentralized Stochastic Gradient Descent for Strongly Convex Optimization](https://arxiv.org/abs/2606.07496)
*Ming Sun,Kun Yuan*

Main category: cs.LG

TL;DR: 提出MG-ADSGD算法，结合Nesterov型原始-对偶外推与多轮快速八卦平均，实现去中心化随机强凸优化的最优通信复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有去中心化随机方法无法同时实现加速的√κ和1/√(1-β)依赖，通信效率受限于条件数和网络谱间隙。

Method: 将八卦深度与小批量大小耦合，使得额外通信轮次同时改善共识精度和降低梯度方差，采用Nesterov型原始-对偶外推。

Result: 达到通信复杂度Õ(σ^2/(μnϵ) log(1/ϵ) + √(κ/(1-β)) log(1/ϵ))，是目前最优的。

Conclusion: 该算法在去中心化随机强凸优化中实现了当前最佳通信复杂度（忽略对数因子）。
...
### [181] [Sparse Subspace-to-Expert Sharing for Task-Agnostic Continual Learning](https://arxiv.org/abs/2606.07500)
*Fatema Siddika,Md Anwar Hossen,Tanwi Mallick,Ali Jannesari*

Main category: cs.LG

TL;DR: 提出SETA框架，通过稀疏子空间分解为任务特定和共享专家模块，解决LLM持续学习中的塑性-稳定性困境，在多个基准上取得优秀性能。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法统一处理参数，无法区分任务特定知识与共享能力，导致灾难性遗忘。

Method: 将参数空间分解为任务特定专家和共享专家，通过自适应弹性锚定和路由感知正则化保护共享知识，统一门控网络自动选择正确专家组合。

Result: 在LLaMA-2 7B和Qwen3-4B上，SETA相比基线方法取得竞争或更优性能，早期任务知识保留和后向迁移显著提升。

Conclusion: SETA有效解决塑性-稳定性冲突，适用于任务无关的持续学习场景，性能优于现有方法。
...
### [182] [The massless sine-Gordon model with logarithmic correlations in arbitrary dimension](https://arxiv.org/abs/2606.06657)
*Sami Vihko*

Main category: math-ph

TL;DR: 我们研究了对数相关sine-Gordon模型在任意维度下的非微扰构造和重整化，证明了配分函数的可重整性，并建立了电荷和梯度相关函数，结果在特定β范围内对所有z成立。


<details>
  <summary>Details</summary>
Motivation: 探索对数相关sine-Gordon模型在任意维度下的非微扰性质，特别是其可重整性和相关函数，以推广高斯自由场理论。

Method: 直接在连续介质中进行分析，通过尺度分解进行重整化，控制重整化势，并使用Brydges和Kennedy的迭代Mayer展开。

Result: 构建了场及其电荷和梯度相关函数，证明了配分函数是可重整的；结果非微扰，对所有z∈R成立；d=1时β∈(0,4π)，d≥2时β∈(0,(d+1)2π)。

Conclusion: 该工作建立了对数相关sine-Gordon模型的非微扰构造和重整化，为理解这类场论提供了基础。
...
### [183] [Unified Framework for Functional Theories of Quantum Systems](https://arxiv.org/abs/2606.06676)
*Chih-Chun Wang,Julia Liebert,Markus Penz,Christian Schilling*

Main category: math-ph

TL;DR: 提出了一个统一框架，用于有限维希尔伯特空间上密度泛函理论及其变体，通过定义“作用域”和广义可观测量，统一证明结构性质。


<details>
  <summary>Details</summary>
Motivation: 降低多体量子问题的复杂性，通过约化变量描述基态，并统一现有各种泛函理论。

Method: 框架基于广义基本可观测量和固定哈密顿量部分（作用域），定义普适泛函，证明凸性、可微性、可表示性和Hohenberg-Kohn唯一性，并引入纯化构造关联系综与纯态泛函。

Result: 得到了系统的数学表述，使得结构性质可一次性证明并应用于各类有限维泛函理论，特别涉及李代数可观测量结构与辛几何的联系。

Conclusion: 该统一框架为有限维泛函理论提供了严谨的数学基础，简化了理论推广和应用。
...
### [184] [Rational Heun operators on $q$-linear grids](https://arxiv.org/abs/2606.06863)
*Satoshi Tsujimoto,Luc Vinet,Alexei Zhedanov*

Main category: math-ph

TL;DR: 提出了q-线性网格上的有理Heun算子，这些算子是二阶q-差分算子，通过要求它们具有对[n/n]型有理函数的提升作用来构造定义。发现这些算子与Ruijsenaars-van Diejen-Takemura哈密顿量的一族相关，并刻画了称为经典的一个子类。


<details>
  <summary>Details</summary>
Motivation: 定义和研究q-线性网格上的有理Heun算子，探索其性质及与其他重要算子的关系。

Method: 从提升作用的需求出发，构造性地定义二阶q-差分算子W_q，使其满足W_q: [n/n] → [n+1/n+1]，并分析其极点结构。

Result: 观察到这些算子与Ruijsenaars-van Diejen-Takemura哈密顿量的一族有关；同时刻画了一个特殊的经典子类，该子类在保持有理函数类型和给定基的情况下改变极点结构。

Conclusion: 有理Heun算子在q-线性网格上具有明确构造，与可积系统哈密顿量有联系，经典子类具有特别的性质。
...
### [185] [A survey on rigorous results for the dynamics of periodic FPU chains](https://arxiv.org/abs/2606.07018)
*Dario Bambusi,Andrea Carati,Alberto Maiocchi*

Main category: math-ph

TL;DR: 本文综述了FPU系统的分析结果，通过Toda系统的作用-角度变量和连续极限（KdV方程）研究其动力学稳定性，并分析了热力学极限下自相关函数的慢衰减导致的热化时间下界。


<details>
  <summary>Details</summary>
Motivation: FPU哈密顿量与Toda哈密顿量接近，希望利用Toda系统的可积性理解FPU系统的动力学稳定性。

Method: 先回顾有限粒子情况下的Toda系统作用-角度变量，再取连续极限得到KdV方程；直接研究FPU系统插值函数的哈密顿结构，并证明其与KdV前三个哈密顿量接近；在热力学极限下构造可观测量并分析时间自相关函数。

Result: FPU系统动力学哈密顿量接近KdV前三个哈密顿量的函数；热力学极限下某些可观测量自相关函数慢衰减，得到热化时间的下界。

Conclusion: 利用Toda系统的可积性和KdV方程可近似描述FPU系统，但热力学极限下热化过程可能非常缓慢。
...
### [186] [Einstein-Kropina Metrics and Their Application in Finsler Gravity](https://arxiv.org/abs/2606.07121)
*Sjors Heefer,Fidel F. Villase\~nor,Andrea Fuster*

Main category: math-ph

TL;DR: 将Kropina度量的爱因斯坦条件推广到所有符号，构造了新的正定和洛伦兹符号的Einstein-Kropina度量，并分类了Finsler引力真空方程的所有Einstein-Kropina解。


<details>
  <summary>Details</summary>
Motivation: 推广Einstein条件到所有符号，并构造新例子，尤其是洛伦兹符号的例子。

Method: 通过推广正定情况下的Einstein条件，构建显式度量，并分类真空方程的解。

Result: 在维度≤4时只有平凡解（闵可夫斯基或欧几里得空间）；维度≥5时解为度量a是实直线与Ricci平坦度量的乘积，b是第一因子上的单位向量。所有Λ-真空方程的Einstein-Kropina解都是Berwald和Ricci平坦的，且宇宙学常数必须为零。

Conclusion: 发现了惊人的刚性现象：所有Einstein-Kropina解都是Berwald和Ricci平坦的，且宇宙学常数消失。
...
### [187] [Information theoretic measures of isotropic Dunkl oscillator in spherical coordinates](https://arxiv.org/abs/2606.07228)
*Akash Halder,Amlan K. Roy,Debraj Nath*

Main category: math-ph

TL;DR: 在Dunkl-Schrödinger框架下对各向同性谐振子进行了信息论分析，推导了多种熵及其相对测度，发现Dunkl参数有显著影响。


<details>
  <summary>Details</summary>
Motivation: 探索Dunkl框架下量子系统的信息论性质。

Method: 利用精确解析本征解和加权Lebesgue测度引入的分解法。

Result: 得到Shannon熵、Rényi信息、Tsallis熵及其相对测度和散度；Dunkl参数为零时结果与非Dunkl情况一致。

Conclusion: 反射算子和Dunkl参数显著影响这些信息测度。

Abstract: An information theoretic analysis is done for the isotropic harmonic oscillator potential within the Dunkl-Schr\"odinger framework in spherical coordinates. Starting from the exact analytical eigensolution, various quantum information measures such as Shannon entropy, R\'enyi information, Tsallis entropy are derived. Besides, their relative measures like relative Shannon, relative R\'enyi, relative Tsallis as well as corresponding divergences (Jensen-Shannon, Jensen-R\'enyi, Jensen-Tsallis) are also obtained. In order to get Shannon entropy, a novel factorization method is introduced. This is facilitated through the use of well-known weighted Lebesgue measure. The results from the Dunkl case agree exactly with the non-Dunkl scenario, when Dunkl parameters vanish. The reflection operators and Dunkl parameters considerably influence the above measures. These are portrayed in graphical forms.
...
### [188] [DataEvolver: Automatic Data Preparation for Large Language Models through Multi-Level Self-Evolving](https://arxiv.org/abs/2606.07001)
*Chao Deng,Shaolei Zhang,Ju Fan,Xiaoyong Du*

Main category: cs.DB

TL;DR: 提出DataEvolver，首个自我进化的数据准备系统，自动构建管道将原始数据转化为高质量数据，无需人工干预。


<details>
  <summary>Details</summary>
Motivation: 高质量训练数据对LLM至关重要，但手动整理昂贵且耗时；现有自动方法缺乏适应性和高质量示例指导。

Method: 采用多级机制：算子级增量扩展算子集并解决依赖冲突；管道级将逻辑计划实例化为可执行代码，并通过反馈循环迭代优化管道编排，减少准备数据与高质量示例的分布差距。

Result: 在七个基准上，DataEvolver显著提升数据质量，使下游LLM性能平均提升10%（对比原始数据训练）。

Conclusion: 揭示了LLM与数据迭代共进的新机遇。
...
### [189] [Auto-Relate: A Unified Approach to Discovering Reliable Functional Relationships Leveraging Statistical Tests](https://arxiv.org/abs/2606.07060)
...
TL;DR: 提出统一的功能关系（FR）概念和基于挖掘-验证的Auto-Relate框架，通过四项可靠性标准和三种优化策略，有效从表格中恢复隐含的列间关系。


<details>
  <summary>Details</summary>
Motivation: 表格中的列间关系通常是隐式的，在导出时丢失，且现有方法因偶然性、冗余性等导致假阳性关系过多。

Method: 定义FR涵盖算术关系、字符串变换和函数依赖；通过准确性、原子性、稳定性和完整性四个标准评估可靠性；采用'挖掘-验证'框架，先生成候选FR，再分别通过极小性测试、扰动测试和独立性测试验证后三个标准，并利用分组下界、闭式加速和二项式边界等优化效率。

Result: 在58,679个真实表格构建的大规模基准上，Auto-Relate的平均PR-AUC为0.87，比最优基线高59%。

Conclusion: Auto-Relate能有效恢复表格中的可靠功能关系，性能显著优于现有方法。
...
### [190] [Efficient $(\alpha,\beta)$-core Computation and On-the-fly Query at Billion Scale with GPUs](https://arxiv.org/abs/2606.07148)
*Qingshuai Feng,Shunyang Li,Kai Wang,Xuemin Lin,Kongzhang Hao,Long Yuan*

Main category: cs.DB

TL;DR: 提出利用GPU加速二分图(α,β)-core计算与实时查询的无索引方法。


<details>
  <summary>Details</summary>
Motivation: 现有CPU方法构建全组合索引成本高、可扩展性差，且难以支持大规模二分图上的实时边更新查询。

Method: 提出无索引的GPU剥离算法GCC（采用warp-centric处理）和GCC+（利用核心嵌套属性和早期剪枝），以及用于实时查询的连通性感知算法GFQ。

Result: 在11个数据集上的实验表明，所提方法在空间和时间效率上均优于现有CPU解决方案。

Conclusion: GPU架构能有效加速(α,β)-core计算和实时查询，避免大规模索引存储的开销。
...
### [191] [Detecting and Mitigating Bias by Treating Fairness as a Symmetry Operation](https://arxiv.org/abs/2606.06514)
*Nishit Singh*

Main category: cs.AI

TL;DR: 提出一种基于对称性恢复的正则化方法，在保持特征不变的前提下通过交换敏感属性的反事实操作来消除分类器偏见，在合成数据集上违规减少超90%，准确率成本约5%。


<details>
  <summary>Details</summary>
Motivation: 高风险社会经济场景中的机器学习系统普遍存在偏见，需要一种无需因果图知识、计算轻量的公平性干预方法。

Method: 将偏见形式化为对称性破缺，通过基于损失的正则化作为对称性恢复机制，在四个不同噪声、相关性和偏见的合成数据集上评估。

Result: 违规减少超90%，准确率成本约5%。

Conclusion: 该方法不依赖因果图，计算轻量，适用于任何可定义为比特翻转的敏感属性，特别适合本地歧视源不在主流基准中的场景。
...
### [192] [DiBS: Diffusion-Informed Branch Selection](https://arxiv.org/abs/2606.06518)
*Bo Liu,Yuan Xie,Yuan Gao,Xiaolong Luo,Peng Ye,Tao Chen,Fujun Han*

Main category: cs.AI

TL;DR: 提出扩散模型引导的符号求解器DiBS，用于数独分支选择，兼顾正确性与效率。


<details>
  <summary>Details</summary>
Motivation: 现有学习求解器缺乏硬正确性保证，符号求解器易陷入长尾搜索。

Method: 保持符号求解器完整性，用扩散模型对候选值排序，并加入轻量级一致性信号。

Result: 在Royle 17-clue基准上，显著减少节点数、回溯次数和长尾百分位。

Conclusion: 学习到的全局指导在分支错误代价高的难例上有效。
...
### [193] [SafeGene: Reusable Adapters for Transferable Safety Alignment](https://arxiv.org/abs/2606.06519)
*Yanghan Wang,Zhiqiang Kou,Fu Feng,Jing Wang,Xin Geng*

Main category: cs.AI

TL;DR: 提出SafeGene，一种可复用的安全适配器模块，用于跨任务重用，解决微调后安全对齐弱化问题。


<details>
  <summary>Details</summary>
Motivation: 开源LLM微调为定制助手时，下游微调可能削弱安全对齐，即使训练数据无害，导致安全恢复问题。

Method: 从对齐-退化模型差异中提取可转移的安全向量，通过数据感知层选择，并在下游任务适应模型中通过少样本层系数重新校准。

Result: 跨多个模型族、下游任务和安全评估，SafeGene增强的模型降低了有害响应率，同时保持下游性能，优于代表性安全适应方法。

Conclusion: SafeGene有效实现安全-效用权衡，具有可重用性。
...
### [194] [Lean4Agent: Formal Modeling and Verification for Agent Workflow and Trajectory](https://arxiv.org/abs/2606.06523)
*Ruida Wang,Jerry Huang,Pengcheng Wang,Xuanqing Liu,Luyang Kong,Tong Zhang*

Main category: cs.AI

TL;DR: 提出了Lean4Agent框架，利用Lean4形式语言建模和验证智能体工作流，并通过实验验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型智能体系统缺乏形式化方法来指定、验证和调试工作流，受数学中使用形式语言解决自然语言歧义的启发。

Method: 构建FormalAgentLib库形式化建模工作流并验证语义一致性，再基于此开发LeanEvolve修订工作流以提升性能。

Result: 在SWE-Bench-Verified子集和ELAIP-Bench子集上，验证通过的工作流比失败的性能平均高11.94%，LeanEvolve进一步平均提升7.47%。

Conclusion: 开创了使用依赖类型形式语言形式化建模和验证智能体行为的新方向。
...
### [195] [CrowdMath: A Dataset of Crowdsourced Mathematical Research Discussions](https://arxiv.org/abs/2606.06526)
*Sherin Muckatira,Jesse Geneson,Slava Gerovitch,Pavel Etingof,Mikhail Gronas,Anna Rumshisky*

Main category: cs.AI

TL;DR: 介绍CrowdMath数据集，用于评估大语言模型在协作数学开放问题解决中的能力，模型在局部预测上表现良好但在角色分类上表现差。


<details>
  <summary>Details</summary>
Motivation: 现有数学推理基准不涵盖协作开放问题解决，缺乏相关数据集和评估。

Method: 构建CrowdMath数据集，包含164个专家注释的进度链，帖子标记功能角色；定义评估任务并基准测试6个前沿模型。

Result: 模型在下一帖预测准确率83-88%，但帖子角色分类macro-F1最高仅0.42。

Conclusion: 模型在解决明确问题与理解协作数学进展之间存在显著差距。
...
### [196] [Attack Selection in Agentic AI Control Evaluations Meaningfully Decreases Safety](https://arxiv.org/abs/2606.06529)
*Catherine Ge-Wang,Tyler Crosse,Benjamin Hadad IV,Joachim Schaeffer,Ram Potham,Tyler Tracy*

Main category: cs.AI

TL;DR: 攻击选择策略（开始/停止）显著降低AI控制协议的安全性，现有评估可能过于乐观。


<details>
  <summary>Details</summary>
Motivation: 现有控制评估假设攻击者不策略性选择攻击时机，这可能导致对实际风险的低估。

Method: 将攻击决策分解为开始策略（决定何时攻击）和停止策略（决定何时中止攻击），在BashArena和LinuxArena两个智能体环境中进行测试。

Result: 在1%审计预算下，开始策略使两个环境的安全性均降低20个百分点；停止策略在BashArena降低20个百分点，在LinuxArena降低28个百分点。这些降低应视为攻击选择效果的上界。

Conclusion: 现有控制评估可能产出过于乐观的安全估计，建议未来的评估、系统卡和安全案例应纳入攻击选择，以得到更现实的安全估计。
...
### [197] [CARVE-Q: Quantum-Proposed, Classically Certified Interactive Driving Repair](https://arxiv.org/abs/2606.06531)
*Yifan Wang*

Main category: cs.AI

TL;DR: CARVE是一种用于被否决驾驶操作的认证修复架构，通过构建修复格和结构化证书确保规则合规。CARVE-Q引入量子最小搜索加速多主体验证。


<details>
  <summary>Details</summary>
Motivation: 驾驶否决后，需要一种可审计、责任明确的修复方案，现有预测和博弈规划方法无法提供硬规则和安全责任的证明。

Method: 构建有限修复格，发出包含绑定规则、合作包络、责任加权成本分割等的结构化证书；并引入CARVE-Q，用量子最小查找搜索多所有者产品的黑箱格，保持安全权威经典。

Result: CARVE-Q在保守验证者-预言机模型下，量子最小查找使用O(√M)次预言机查询，而经典需要Θ(M)次。实验支持65,536个赋值，在Lanelet2上实现100%路权尊重和责备一致性。

Conclusion: 形成信任边界量子AI模式：量子提议，CARVE认证。
...
### [198] [Position: Don't Just "Fix it in Post": A Science of AI Must Study Training Dynamics](https://arxiv.org/abs/2606.06533)
*Stella Biderman,Mohammad Aflah Khan,Niloofar Mireshghallah,Catherine Arnett,Fazl Barez,Naomi Saphra*

Main category: cs.AI

TL;DR: 提倡建立基于训练动态的AI科学，超越事后分析，实现预测、干预和设计训练过程。


<details>
  <summary>Details</summary>
Motivation: 当前AI研究将模型视为静态产物，忽视训练动态，无法解释行为成因。

Method: 从科学哲学出发提出理论要求，并分析可解释性、公平性等领域的进展。

Result: 缩放定律成功预测损失，但向能力、偏见等扩展存在挑战。

Conclusion: AI科学应研究训练动态，实现从预测到设计训练程序的更强理解。
...
### [199] [Accelerated Fourier SAT (AFSAT): Fully Realising a GPU-based Symmetric Pseudo-Boolean SAT Solver](https://arxiv.org/abs/2606.06641)
*Cody J Christopher,Charles Gretton*

Main category: cs.AI

TL;DR: AFSAT是一个基于GPU的伪布尔可满足性求解器，通过JAX实现大规模并行连续局部搜索，在数值稳定性、运行时间和内存效率上显著提升。


<details>
  <summary>Details</summary>
Motivation: 将FastFourierSAT概念工程化，支持异构约束，并利用GPU并行加速伪布尔可满足性求解。

Method: 使用JAX编译器进行纯函数组合、自动向量化、自动微分和JIT编译，实现批量候选赋值的并行CLS；通过定制DFT解决浮点表示问题；利用JAX数组分片进行多加速器扩展。

Result: 数值稳定性、运行时间和内存效率大幅提升；多加速器扩展时接近线性吞吐量。

Conclusion: AFSAT有效解决了伪布尔可满足性问题，展示了GPU加速和JAX在约束求解中的潜力。
...
### [200] [A Study of Parallel Continuous Local Search](https://arxiv.org/abs/2606.06656)
*Cody J Christopher,Charles Gretton*

Main category: cs.AI

TL;DR: 研究并行连续局部搜索（CLS）求解对称伪布尔约束的SAT问题，发现冗余约束阻碍收敛、CLS在混合设置中有效、局部搜索快速稳定到解质量分布。


<details>
  <summary>Details</summary>
Motivation: 探索CLS作为解决具有对称伪布尔约束的SAT问题的有效方法。

Method: 将n变量PB可满足性问题松弛为超立方体上的连续优化，使用并行CLS进行求解。

Result: (i)冗余约束抑制收敛；(ii)CLS作为子求解器能快速完成部分赋值；(iii)局部搜索因点稠密目标而快速收敛到稳定分布。

Conclusion: 发现为CLS在现代加速硬件上的实际应用提供了指导。
...
### [201] [AEGIS: A Backup Reflex for Physical AI](https://arxiv.org/abs/2606.06660)
*Josef Chen*

Main category: cs.AI

TL;DR: AEGIS通过轻量级探针检测高风险步骤，仅在必要时切换到更强策略，显著提升长时域机器人操作的轨迹恢复率。


<details>
  <summary>Details</summary>
Motivation: 长时域机器人操作中，一个坏步骤会导致状态恶化，策略逐渐失败且常可预见。

Method: 在弱策略的冻结激活上训练轻量级探针，当探针标记高风险步骤时，切换至强策略执行该步骤。

Result: 在LIBERO-Spatial上，AEGIS恢复弱策略丢失轨迹的10.1%，优于盲目升级的4.6%和随机触发的5.1%；探针早期窗AUROC为0.764。

Conclusion: AEGIS仅在38%的步骤激活强策略，证明通过时机选择而非增加计算即可有效恢复失败轨迹。
...
### [202] [A Geometric Account of Activation Steering through Angle-Norm Decomposition](https://arxiv.org/abs/2606.06735)
*Georgii Aparin,Tatiana Gaintseva*

Main category: cs.AI

TL;DR: 本文通过控制实验分离角度和径向分量，发现概念主要表示在角度结构中，但范数对引导稳定性和下游效应重要，建议用角度和径向分量参数化激活引导。


<details>
  <summary>Details</summary>
Motivation: 球面引导方法假设隐藏状态范数不携带概念相关信息，本文重新审视这一假设的合理性。

Method: 设计控制实验，分离角度和径向分量，在七个语言模型上比较不同引导方法的效果。

Result: 概念主要由角度结构表示，但范数对引导的稳定性和下游效应有重要影响。

Conclusion: 激活引导应参数化为可解释的角度和径向分量，而非单一的加性系数，以避免混淆两种几何效应。
...
### [203] [OpenSkill: Open-World Self-Evolution for LLM Agents](https://arxiv.org/abs/2606.06741)
...
TL;DR: 提出OpenSkill框架，在无目标任务监督下实现智能体自我进化，通过从文档、仓库和网络获取知识并构建验证锚点，自主合成技能并自我验证。


<details>
  <summary>Details</summary>
Motivation: 现有自我进化方法依赖可用学习循环（如策划技能、成功轨迹或验证信号），但真实开放世界部署可能只提供任务提示，缺乏这些监督。

Method: OpenSkill从文档、代码库和网络获取基础知识和验证锚点，将其合成为可迁移技能，并基于锚点自建虚拟任务进行技能细化，不依赖目标任务监督。

Result: 在三个基准和两个目标智能体上，OpenSkill在满足无监督约束下达到最佳自动通过率，技能无需特定模型适配即可跨模型迁移，自建验证器与真实结果一致。

Conclusion: 开放世界可同时提供学习知识和监督无关的练习环境，实现从零开始的自我进化，仅最后评估使用目标任务监督。
...
### [204] [AdMem: Advanced Memory for Task-solving Agents](https://arxiv.org/abs/2606.06787)
*Runzhe Wang,Huilin Lu,Shengjie Liu,Li Dong,Jason Zhu*

Main category: cs.AI

TL;DR: 提出统一自动记忆框架，结合语义、情景和程序记忆，通过多智能体架构实现长时任务中的鲁棒性和成功率的提升。


<details>
  <summary>Details</summary>
Motivation: LLMs在长时任务中受限，现有记忆方法只关注事实或重放成功，缺乏对失败案例的在线可扩展性。

Method: 提出统一自动记忆框架，包含短期和长期存储的二级设计，多智能体架构（行动者、记忆、评论者）实现自动记忆生成、奖励标注和自适应检索，通过基于奖励的评估、合并和剪枝管理长期记忆。

Result: 实验表明，该方法在长多轮任务中比基线方法更鲁棒和成功。

Conclusion: 强调全面自适应记忆对于推进基于LLM的智能体的重要性。
...
### [205] [Evidence-Based Intelligent Diagnostic and Therapeutic Visualization System with Large Language Models: Multi-Turn Interaction and Multimodal Treatment Plan Generation](https://arxiv.org/abs/2606.06869)
*Yunhan Wang,Yuda Wang,Zhiying Tu,Mingqiang Song,Li Song,Kun Li,Dianhui Chu,Bolin Zhang*

Main category: cs.AI

TL;DR: 提出一种基于知识增强的可视化诊断系统，通过知识图谱、症状匹配流水线、主动提问策略和多模态治疗展示，提升中医辨证论治的透明度和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有AI辅助中医诊断工具存在推理过程不透明、交互被动、治疗方案展示有限等问题，需要提高辨证论治的透明度和可解释性。

Method: 基于Neo4j知识图谱（包含241个证型、1263个症状、2485个关系），设计四阶段症状匹配流程（精确、语义、模糊、大语言模型验证），采用信息增益驱动的遗传算法优化主动提问策略，并整合AI生成插图、三维经络穴位模型和循证文献的多模态治疗展示。

Result: 知识图谱约束使非标准输出减少32%；案例研究验证了工作流程在患者自评、临床辅助诊断和中医教育中的有效性；30个案例的自动配对比较评估显示诊断信任度显著提升（Cohen's d=1.82, p<0.001），五个认知负荷维度中四个得到改善，循证参考文献可信度更高（4.21 vs 2.95）。

Conclusion: 该系统通过知识图谱驱动的可视化和多模态交互，增强了中医诊断推理的透明度和治疗计划的可解释性，为可信赖的AI辅助中医应用提供了实用解决方案。
...
### [206] [Workflow-to-Skill: Skill Creation via Routing-Workflow-Semantics-Attachments Decomposition](https://arxiv.org/abs/2606.06893)
*Yuyang Zhang,Xinyuan Han,Xudong Jiang,Run Wang*

Main category: cs.AI

TL;DR: 提出RWSA工作流导向中间表示和W2S框架，从异构交互证据自动构建技能，相比基线提升行为回放一致性10.5%。


<details>
  <summary>Details</summary>
Motivation: 高质量技能手工编写成本高，现有方法将轨迹视为可压缩文本，忽略了其片段化、冗余及缺失安全关键行为的问题。

Method: 定义RWSA将技能分解为工作流结构、执行语义和运行时附件；提出W2S框架分割轨迹、归纳局部草稿、对齐共享结构、协调分支并压缩冗余，保留证据和置信度注释。

Result: 在70个技能上，W2S的行为回放一致性比基于摘要和提示的基线提高10.5%。

Conclusion: 应将交互轨迹视为可执行的运行时规范而非可压缩文本，以提升技能构建质量。
...
### [207] [Declarative Skills for AI Agents in Knowledge-Grounded Tool-Use Workflows](https://arxiv.org/abs/2606.06923)
*M. Danish Lim,I. Danial Bin Sharudin,Wen Han Chen,Cedric Lim,Laura Wynter*

Main category: cs.AI

TL;DR: 研究了基于非结构化知识库的客服工作流中工具使用型AI代理的编排机制，发现声明式代理在高质量检索下优于命令式代理和无脚手架基线，检索质量是主要瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有代理编排方法在真实客服场景中效果不佳，需要比较不同编排范式的有效性。

Method: 形式化三种代理（声明式、命令式、无脚手架基线）为分散部分可观测马尔可夫决策过程中的策略类，并在五个语言模型和两种检索机制下实验。

Result: 检索质量是主导瓶颈；高质量检索时，声明式技能持续提升程序任务准确率并减少编排错误，命令式状态机不可靠。

Conclusion: 声明式代理结合高质量检索是有效的编排范式，检索优化是关键。
...
### [208] [Quantum-Inspired Trace-Augmented Evidence Selection for Reasoning over Structured Hypothesis Spaces](https://arxiv.org/abs/2606.06941)
*Laura Wynter,Nirvik Sahoo,Paul Griffin*

Main category: cs.AI

TL;DR: 提出EP-HUBO方法，将思维链推理片段的选择转化为组合优化问题，通过高阶二元优化聚合证据，提高推理鲁棒性，在法律领域基准测试中优于多数投票。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在法律等证据密集型任务中表现脆弱，多数投票聚合思维链痕迹时忽略证据质量，导致少数但正确的假设被噪声掩盖。

Method: EP-HUBO使用小模型生成多个思维链，解析片段构建每个假设的证据池，通过赋予相关性、特异性、区别性权重的质量指导高阶二元优化，最后调用前沿模型进行裁决。

Result: 在证据密集型法律基准上，EP-HUBO采用模拟退火和光子熵量子机优化，优于传统多数投票，尤其在低污染领域表现显著。

Conclusion: HUBO风格优化提供了一种有原则的推理片段聚合方式，保留少数正确假设，在低污染领域最有价值。
...
### [209] [Accounting for Context: Shaping Moral Credences for Value Alignment](https://arxiv.org/abs/2606.06972)
*Jazon Szabo,Sanjay Modgil*

Main category: cs.AI

TL;DR: 本文指出在聚合道德评价时必须考虑情境因素，并展示了忽视情境因素会导致违反弱帕累托原则，这类似于辛普森悖论。


<details>
  <summary>Details</summary>
Motivation: 确保智能体行为符合人类道德价值观时，需要考虑道德观点的多样性，但现有工作忽略了情境因素。

Method: 形式化了在道德不确定性下的智能体决策，并考虑了情境因素。

Result: 证明了一个看似合理的弱帕累托原则被违反，并指出这是辛普森悖论的一种变体。

Conclusion: 揭示了忽略情境因素的聚合机制存在局限性。
...
### [210] [Exploring Agentic Tool-Calling Decisions via Uncertainty-Aligned Reinforcement Learning](https://arxiv.org/abs/2606.06976)
*Yijin Zhou,Linqian Zeng,Xiaoya Lu,Wenyuan Xie,Dongrui Liu,Junchi Yan,Jing Shao*

Main category: cs.AI

TL;DR: 提出TRUST方法，通过不确定性量化改进奖励设计并标注关键轮次，提升基于LLM的智能体工具使用决策质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在工具使用决策中存在次优行为，且未充分利用不确定性的特征，导致决策错误和探索信号弱。

Method: 将不确定性量化作为排斥力融入奖励设计，保持不确定性分离，并标注关键轮次进行统一后训练。

Result: 在多个工具使用基准上，TRUST一致提升了决策质量和智能体性能，同时保持更可靠的不确定性估计。

Conclusion: 不确定性量化有效改善了基于LLM的智能体在工具使用中的决策行为，有望减少错误累积。
...
### [211] [Teaching the Way, Not the Answer: Privileged Tutoring Distillation for Multimodal Policy Optimization](https://arxiv.org/abs/2606.07000)
*Shizhe Xiang,Ke An,Wenlong Yu,Yue Liu,Jian Luan,Pei Fu,Qilong Wang*

Main category: cs.AI

TL;DR: 提出PTD-PO框架，通过特权提示蒸馏为多模态推理提供密集监督，缓解奖励稀疏问题，提升复杂推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有后训练方法中，可验证奖励稀疏，缺乏令牌级监督，导致探索低效；外部教师方法计算开销大，答案条件调优可能暴露答案信息并引发捷径行为。

Method: 构建结构化特权提示（空间注意力引导+中间推理步骤），通过上下文学习生成逐步令牌分布监督；学生模型在无答案环境下优化，将失败轨迹与提示增强参考模型在令牌分布上对齐；引入Top-K Jensen-Shannon散度目标稳定蒸馏。

Result: 在2B至8B参数的LVLM上，PTD-PO持续优于RLVR和蒸馏基线，缓解熵崩溃，提升复杂多模态推理性能。

Conclusion: PTD-PO通过特权蒸馏提供密集指导，有效改善稀疏奖励下的多模态推理，且避免答案泄露和计算开销。
...
### [212] [The Sim-to-Real Gap of Foundation Model Agents: A Unified MDP Perspective](https://arxiv.org/abs/2606.07017)
*Xiaoou Liu,Tiejin Chen,Weibo Li,Xiyang Hu,Hua Wei*

Main category: cs.AI

TL;DR: 将基础模型智能体的评估和训练差距形式化为经典的模拟到现实问题，并主张采用领域随机化等成熟解决方案。


<details>
  <summary>Details</summary>
Motivation: 基础模型智能体在现实部署中面临模拟到现实的差距，但社区将其视为全新现象，缺乏形式化框架。

Method: 围绕马尔可夫决策过程的四个要素（观察、动作、转移、奖励）来形式化这一差距，并翻译为基础模型领域，提出研究议程。

Result: 提供了具体例子（如多语言工具调用）展示观察空间差距导致实际无效操作，尽管语义意图正确。

Conclusion: 旨在推动范式转变，统一词汇和标准化压力测试基准，以催生高度可信的智能体用于可靠的真实世界应用。
...
### [213] [StainFlow: Entity-Stain Tracking and Evidence Linking for Process Rewards in GUI Agents](https://arxiv.org/abs/2606.07027)
...
TL;DR: 提出StainFlow，一种基于实体染色流的过程奖励模型，用于改进GUI智能体的强化学习。


<details>
  <summary>Details</summary>
Motivation: 现有过程奖励模型存在全局里程碑分解主观单一和局部窗口固定导致关键证据丢失的问题。

Method: 包含全局实体染色追踪（通过实体证据流客观划分任务阶段）和局部染色证据链接（动态构建高密度证据窗口验证关键节点）。

Result: 在AndroidWorld和OGRBench上，在线RL成功率相对提升3.2%，轨迹完成判断准确率提升1.8%。

Conclusion: StainFlow有效解决了现有PRM的局限性，提升了性能。
...
### [214] [Hierarchical Semantic-Constrained Heterogeneous Graph for Audio-Visual Event Localization](https://arxiv.org/abs/2606.07033)
*Zhe Yang,Ruyi Zhang,Hongtao Chen,Wenrui Li,Hengyu Man,Wangmeng Zuo,Xiaopeng Fan*

Main category: cs.AI

TL;DR: 提出了一种基于层次语义约束异构图（HSCHG）的开放词汇视听事件定位框架，通过多方向时序边、双阈值滤波门控融合和层次蕴含正则化，在双曲空间实现多级语义一致性，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在欧氏空间学习联合视听表征，但面临两个挑战：缺乏对未见类别的监督信号导致多时间尺度视听一致性难以维持；缺乏片段和视频级语义间的层次约束，无法建立跨层次语义一致性。

Method: 构建欧氏空间内的异构图，包含音频和视觉片段节点及其视频级节点，使用多方向时序边捕获完整时序信息；采用双阈值滤波门控融合策略在高置信度时引入跨模态信息；引入片段与视频级表征间的双向语义约束；将多级视听表征与文本原型统一映射到双曲空间，使用层次蕴含正则化损失表征层次关系。

Result: 大量实验表明，该方法在OV-AVEL基准上优于现有方法，消融研究进一步验证了有效性。

Conclusion: 提出的HSCHG框架有效解决了开放词汇视听事件定位中的跨尺度一致性和层次约束问题，提升了定位性能。
...
### [215] [Front-to-Attractors: Modifying the Front-to-Front Heuristic in Bidirectional Search](https://arxiv.org/abs/2606.07047)
*Alvin Zou,Muhammad Suhail Saleem,Maxim Likhachev*

Main category: cs.AI

TL;DR: 提出前端到吸引子（F2A）启发式类别，通过动态维护少量吸引子代替完整边界状态，在降低计算成本的同时保持信息性和最优性。


<details>
  <summary>Details</summary>
Motivation: 前端到前端（F2F）启发式信息丰富但成对评估计算开销大，前端到终端（F2E）计算快但信息少，需要平衡两者。

Method: F2A估计从当前状态到对端搜索中动态维护的小型吸引子集合的距离，而非全部边界状态。

Result: 相比F2F，成对评估减少多达11.2倍；相比F2E，节点扩展平均减少4.8倍。

Conclusion: F2A在显著降低计算开销的同时，保持了启发式的有效性和最优性保证。
...
### [216] [DyCon: Dynamic Reasoning Control via Evolving Difficulty Modeling](https://arxiv.org/abs/2606.07108)
*Tengyao Tu,Yulin Li,Hui-Ling Zhen,Libo Qin,Zhoujun Wei,Jinghua Piao,Zhuotao Tian,Yong Li,Min Zhang*

Main category: cs.AI

TL;DR: 提出DyCon，一种无需训练、利用潜在步骤级表示动态控制推理深度的框架，有效缓解大型推理模型的‘过度思考’问题，在保持准确性的同时提升效率。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型因冗余推理而效率低下，现有缓解方法依赖静态难度估计或需要任务特定训练，无法适应推理过程中的动态复杂度。

Method: 实证发现问题难度在推理过程中动态演化，且线性编码在模型的步骤级嵌入中。基于此，DyCon利用这些嵌入显式建模难度变化，从而动态控制推理深度。

Result: 在4个模型（4B-32B）和12个基准（数学推理、通用问答、编程）上的实验表明，DyCon显著减少冗余步骤，且不损失准确性或泛化能力。

Conclusion: DyCon通过动态推理深度控制，有效缓解过度思考，提升推理效率，且无需额外训练。
...
### [217] [Beyond Post-hoc Explanation: Toward Glassbox AI via Probabilistic Mediation](https://arxiv.org/abs/2606.07113)
*Manuele Leonelli*

Main category: cs.AI

TL;DR: 提出Glassbox框架，用贝叶斯网络作为生成模型的前置透明中介层，实现可审计、可质疑的推理，替代不稳定的后置解释。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在公共管理、法律、医疗等高风险领域缺乏透明度，现有后置解释不稳定且无法追溯真实推理过程。

Method: 设计Glassbox框架，在推理前通过贝叶斯网络编码领域知识、因果假设和概率依赖，生成可审计的推理轨迹和不确定性量化。

Result: 以福利资格场景为例，识别了语义对齐、动态模型构建、概率基础、人类治理等关键挑战。

Conclusion: 从后置解释转向前置概率中介，为构建强大且可问责的AI系统提供了原则性路径。
...
### [218] [Think Fast: Estimating No-CoT Task-Completion Time Horizons of Frontier AI Models](https://arxiv.org/abs/2606.07157)
...
TL;DR: 研究了前沿AI模型在没有思维链（CoT）情况下的推理能力，发现其50%任务完成时间视野（TH）过去六年每年翻倍，预测到2028年可能超过7分钟，2030年超过25分钟，建议开发者跟踪这一指标。


<details>
  <summary>Details</summary>
Motivation: 监控CoT是确保AI安全的关键，但如果模型能在内部进行复杂推理而不需要显式思维标记，这种监督将失效，因此需要量化模型无CoT推理能力。

Method: 使用30,000个问题、43个基准（涵盖数学、编码、谜题等），通过50%任务完成时间视野（TH，人类所需时间）和50%推理token视野（o3-mini所需token数）评估模型，并拟合趋势。

Result: 无CoT的TH每约一年翻倍，GPT-5.5的TH超过3分钟，推理token超过1500；中位数预测到2028年TH超7分钟，2030年超25分钟。

Conclusion: 前沿模型无CoT推理能力快速增长，可能超越人类监督时间尺度，建议开发者明确跟踪无CoT性能趋势。
...
### [219] [TOPSIS-RAD: Ranking According to Desires](https://arxiv.org/abs/2606.07253)
*Leonardo Fernandes Costa,Helder Gomes Costa,Diogo Lima,Brunno Rodrigues*

Main category: cs.AI

TL;DR: 提出TOPSIS-RAD方法，通过决策者定义的否决和期望性能水平，解决传统TOPSIS的参考点依赖数据、对异常值敏感和排序反转问题。


<details>
  <summary>Details</summary>
Motivation: 传统TOPSIS的参考点来自观测数据，导致排序与决策者需求不一致、对异常值敏感和排序反转。

Method: 引入决策者定义的两个性能水平数组：VPL（否决性能水平）在标准化前排除不可行方案，DPL（期望性能水平）将表现上限设为期望水平，从而稳定排序边界。

Result: 三个示例展示了VPL和DPL机制的有效性：VPL通过移除不可行方案重塑标准化边界，DPL限制超出期望表现的影响，使排序稳定。

Conclusion: TOPSIS-RAD保留了传统TOPSIS的距离结构，但将排序建立在稳定的、决策者指定的边界上，并讨论了局限性及未来研究方向。
...
### [220] [DuMate-DeepResearch: An Auditable Multi-Agent System with Recursive Search and Rubric-Grounded Reasoning](https://arxiv.org/abs/2606.07299)
...
TL;DR: DuMate-DeepResearch是一个多智能体深度研究框架，通过图式动态规划、递归两层执行和基于规则的测试时优化，在DeepResearch基准上取得了最新最优结果。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究系统受限于长程规划、单智能体分解调度、长文合成幻觉和过程可审计性差等四个问题。

Method: 框架解耦智能体核心与工具生态，引入三种机制：图式动态规划（粗到细扩展路线图并持续反思调整）、递归两层执行（内层搜索智能体独立规划）、基于规则的测试时优化（动态生成质量标准并用于推理合成）。

Result: 在DeepResearch Bench上取得最佳总分58.03%，在DeepResearch Bench II上取得最佳总分61.95%，信息召回和分析排名第一。

Conclusion: 该框架通过多智能体协作和动态优化机制有效解决了现有系统的局限，实现了深度研究任务的最新性能。
...
### [221] [Off-Policy Evaluation with Strategic Agents via Local Disclosure](https://arxiv.org/abs/2606.07308)
*Kiet Q. H. Vo,Abbavaram Gowtham Reddy,Julian Rodemann,Siu Lun Chau,Krikamol Muandet*

Main category: cs.AI

TL;DR: 提出一种在主体策略性行为下进行离线策略评估的方法，通过事后解释揭示策略前协变量，构建双稳健估计器。


<details>
  <summary>Details</summary>
Motivation: 现有方法假设协变量与政策无关，但策略性行为导致协变量偏移，破坏了这一假设。

Method: 利用事后解释披露局部信息以恢复策略前协变量，估计主体的响应模型，并构建双稳健估计器。

Result: 在条件对数正态成本敏感性假设下，证明了估计量的一致性，并通过实证验证了方法有效性。

Conclusion: 交互设计可以通过揭示主体策略性响应中的隐藏结构来缓解信息不对称问题。
...
### [222] [Online Pandora's Box for Contextual LLM Cascading](https://arxiv.org/abs/2606.07392)
...
### [223] [Act As a Real Researcher: A Suite of Benchmarks Evaluating Frontier LLMs and Agentic Harnesses in Research Lifecycle](https://arxiv.org/abs/2606.07462)
*Jiayu Wang,Weijiang Lv,Bowen Fu,Jing Fu,Jiayi Song,Lingyu Zhang,Lanxuan Xue,Luodi Chen,Zepeng Xin,Kaiyu Li,Xiangyong Cao*

Main category: cs.AI

TL;DR: 提出了AARR基准系列中的第一个基准AARRI-Bench，评估AI代理在细粒度研究场景中的专业性和推理能力。实验表明最佳配置成功率仅68.3%，常忽略人类研究者明显的细节。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理在领域敏感性、研究伦理和细微科学判断上存在不足，无法完全替代人类研究者，需要评估其专业性和彻底性。

Method: 通过AARRI-Bench基准，在前沿模型和代理系统上进行广泛实验，评估其模拟人类研究者行为的能力。

Result: 最佳配置（Mini-SWE-Agent with Claude Opus 4.7）成功率为68.3%，经常忽略人类研究者注意到的关键细节。

Conclusion: 开发类似研究者的AI需要进一步探索研究行为，而非仅仅依赖复杂脚手架。
...
### [224] [How AI Agents Reshape Knowledge Work: Autonomy, Efficiency, and Scope](https://arxiv.org/abs/2606.07489)
*Jeremy Yang,Kate Zyskowski,Noah Yonack,Jerry Ma*

Main category: cs.AI

TL;DR: 前沿AI系统从对话助手转变为自主代理，利用Perplexity数据发现，自主代理（Computer）相比搜索工具（Search）显著加速知识工作，减少时间和成本，提高质量，并扩展工作范围。


<details>
  <summary>Details</summary>
Motivation: 研究AI代理如何加速和重塑知识工作，从对话助手向端到端自主代理的转变。

Method: 使用Perplexity的Search和Computer产品的生产数据，通过近似的初始查询对作为自然实验，比较两种产品在相同任务上的表现。

Result: Computer每次会话自主工作26分钟，Search仅33秒；Computer将完成时间从269分钟降至36分钟，成本降低94%；不满意率降低55%；查询更跨职业、需更高认知、复合任务等。

Conclusion: AI代理能加速工作流程、提高输出质量、降低成本，并扩展自动化工作的广度和深度。

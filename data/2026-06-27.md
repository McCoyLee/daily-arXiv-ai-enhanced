...
TL;DR: 提出HierBias层次化上下文条件媒体偏差检测器，通过建模文档上下文提升句子级偏差检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有句子级方法忽略句子间上下文信号，而人工标注者自然利用这些信号。

Method: 使用句子级RoBERTa编码器、跨句子Transformer聚合器和双输出头（二分类与四类细粒度分类），理论上证明上下文条件降低贝叶斯误差，多任务学习提高样本效率。

Result: 在BABE和BASIL数据集上达到0.853 F1和0.723 MCC，超越现有最佳方法2.6% F1和4.3% MCC。

Conclusion: 上下文条件严格降低句子级分类的贝叶斯误差，多任务训练在小标注语料上提高样本效率；消融实验验证各组件独立有效。
...
TL;DR: 提出了一个考虑数据污染的多区域基准，用于评估大语言模型在可回答与不可回答问题上的表现，并区分回答、猜测、拒绝和污染。


<details>
  <summary>Details</summary>
Motivation: 现有评估无法可靠区分模型基于知识的回答和猜测，且常混淆数据污染、提示特异性和拒绝行为。

Method: 构建1200个条目，覆盖五个领域，带有明确拒绝期望和污染风险元数据，使用官方严格解析器和归一化健壮解析器进行双重解析。

Result: FLAN-T5在有效拒绝上表现弱；指令调优模型展示从回答到拒绝的转换不完全；Qwen2.5-3B-Instruct取得最佳可靠性，但回答预期区域困难，校准差，良性项目拒绝。

Conclusion: 该基准提供了一个可重复的协议，用于审计可回答性、拒绝、拒绝和污染作为LLM可靠性的独立维度。
...
TL;DR: 后训练中的帮助性训练比编码训练更严重地削弱了基于同情心的价值，且这种影响具有跨语言一致性，而推理能力受损仅体现在英语上。


<details>
  <summary>Details</summary>
Motivation: 标准后训练流程（SFT和RL）可能无意中削弱预训练阶段注入的价值，需要探究不同领域后训练数据对动物同情心价值保留的影响。

Method: 使用Llama 3.1 8B模型（中间训练了同情心合成数据），分别进行SFT（帮助性：Dolly-15k vs 编码：Magicoder-110K）和GRPO（帮助性：RLHFlow vs 编码：Magicoder），在Animal Harm Benchmark和MORU基准上评估。

Result: 帮助性训练显著降低动物同情心（SFT: 35.7% vs 65.2%；GRPO: 18.7% vs 32.0%），且损害英语道德推理（46.4% vs 71.9%），但多语言MORU上无差异；同情心效应跨语言一致，非英语上编码增益是英语的4.5倍。
...
TL;DR: 通过蒸馏ChatGPT提取静力学问题，发现LLMs在纯文本问题上表现良好，但引入图形和多步推理后准确率下降，原因是多步推理困难和视觉信息应用不一致，而非图像识别限制。


<details>
  <summary>Details</summary>
Motivation: 现有研究多依赖公开数据集且缺乏学科针对性，工程教育中机械工程特定问题类型的系统研究有限。

Method: 采用模型蒸馏过程，从ChatGPT提取25个纯文本静力学问题，并构建添加图形和修改数值的两个额外数据集。

Result: LLMs在纯文本问题上表现良好，但加入图形和多步推理时准确率下降。

Conclusion: 性能下降主要由多步推理困难和跨步骤一致应用视觉信息的能力不足导致，而非图像识别限制。
...
TL;DR: 研究通过词汇匹配的立场对比探测法，测量了十种语言特征对模型倾向的影响，发现八种特征有显著影响，其中七种增强、两种削弱动物福利立场，建议写作时明确立场而非中立描述。


<details>
  <summary>Details</summary>
Motivation: 动物福利倡导者的写作越来越多地用于训练语言模型，而这些模型被大量用户咨询动物福利问题，因此需要了解不同写作特征如何影响模型立场。

Method: 采用词汇匹配的立场对比探测法，在保留的动物福利基准测试上，测量十种语言特征作为微调数据时对Llama-3.2-1B模型倾向的影响。

Result: 八种特征产生显著影响：七种增强动物福利倾向（明确确定性、道德词汇、情感词、评价性主张、叙事结构、伤害严重性、即时时间框架），两种削弱（模糊语言、具体感官描述），第一人称无显著影响。

Conclusion: 建议撰写可能进入训练语料的动物福利文本时，应明确表达立场而非中性描述场景，因为增强倾向的特征使立场明确，削弱倾向的特征虽包含内容但保留立场。
...
TL;DR: ContextForge通过上下文回收机制，结合结构化查询、外部记忆检索和受控合成，有效降低长对话中的token开销，同时保持回答质量。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在短上下文中表现优异，但在长对话中因上下文窗口限制和低效token使用导致性能下降。

Method: 提出ContextForge系统，包括结构化查询生成、外部记忆检索和受控合成，实现上下文回收，避免完整历史重放。

Result: 在15轮对话基准测试中，与基线相比，ContextForge提升了一致性，减少了token消耗，同时保持了可比的响应准确性。

Conclusion: 上下文回收提供了一种实用方法，无需更大上下文窗口或重新训练即可扩展LLM在长任务中的能力。
...
TL;DR: 通过非暴力沟通（NVC）的轻量级提示约束，引导大语言模型在冲突对话中降低升级，提高可信度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在情感冲突场景中被广泛使用，但现有安全研究忽视了对无意间升级对话行为的防范，需要探索更有效的降级对话引导方法。

Method: 将非暴力沟通原则转化为过程导向的提示约束（避免指责、关注情感、先澄清后建议），并采用双智能体模拟框架在多模型和不同用户抵抗水平下进行实验。

Result: NVC约束提示一致地减少了对话升级，尤其对高抵抗用户稳定了互动，表明简单的沟通约束能有效改善冲突场景中的对话行为。

Conclusion: 轻量级的沟通约束可以显著提升大语言模型在冲突场景中的可信度和安全性，为未来对话安全研究提供新思路。
...
TL;DR: 提出NEST-V1框架，从语音输入生成情感条件化的尼泊尔手语虚拟人，在四个单词三种情感上验证可行性。


<details>
  <summary>Details</summary>
Motivation: 手语通信系统集成情感表达的研究不足，尤其低资源语言。

Method: 轻量级架构，共享声学编码器同时进行ASR和情感分类，生成情感条件化手语虚拟人。

Result: ASR准确率81.1%，情感识别准确率79.21%，参数效率提高37%，仅22.1M参数适合边缘部署。

Conclusion: 为低资源场景下情感感知手语翻译奠定技术基础，证明实时情感表达手语通信系统的可行性。
...
TL;DR: 大型语言模型在推理任务上优于小型模型，但背后推理差异未被充分研究。本文开发AdvCluster框架，自动提取并聚类优势描述，得到系统分类，核心模式是约束引导推理。


<details>
  <summary>Details</summary>
Motivation: 大型模型在推理基准上持续优于小型模型，但造成差距的推理差异尚不清楚。

Method: 提出AdvCluster框架，通过配对推理轨迹提取粒度优势描述，经语义聚类和评审模型筛选，系统化分析推理优势。

Result: 建立了大型模型推理优势的分类体系，包含跨领域通用优势和特定领域优势，突出约束引导推理模式。

Conclusion: 大型模型更擅长识别显式和隐式约束，组织结构化推理，并利用约束排除不可行路径和验证中间步骤。
...
TL;DR: 提出将结构化语言资源（如WordNet）转化为专用AI系统的方法，在印地语上验证，通过125万指令对微调12B模型，在教育效果和一致性上优于通用模型，为低资源语言提供可行方案。


<details>
  <summary>Details</summary>
Motivation: 低资源语言缺乏大规模语料，难以发展专用对话系统；需要利用现有结构化资源（如WordNet）来构建AI系统。

Method: 将Hindi WordNet转换为125万指令-响应对；使用LoRA和4-bit量化微调12B参数语言模型；通过印地语学习聊天机器人评估。

Result: 教育效果得分91.0（通用模型79.4-83.6），语义性能有竞争力，一致性高。

Conclusion: 该方法是可行的，可为任何拥有WordNet资源的语言发展专用AI系统，弥补低资源语言AI可及性差距。
...
TL;DR: 提出Dynamic-dLLM框架，通过动态缓存更新和自适应并行解码加速扩散大语言模型，实现3倍以上速度提升且保持性能。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型计算复杂度随序列长度立方增长，缺乏与键值缓存的兼容性，现有加速方法未考虑动态性。

Method: 包含动态缓存更新(DCU)和自适应并行解码(APD)两个组件，无需训练。

Result: 在多个模型和基准测试上实现平均3倍以上加速，性能不变，优于现有方法。

Conclusion: 提供即插即用方案，高效部署扩散大语言模型。
...
TL;DR: 大语言模型在提供研究方法建议时存在显著偏向，倾向于推荐更窄范围的模型和方法，尤其是减少了对小众/学术模型的引用，且不同LLM之间的偏差高度一致。


<details>
  <summary>Details</summary>
Motivation: 探究大语言模型在最小提示下默认的方法论建议偏向，以提醒研究人员注意过度依赖LLM建议可能导致方法搜索空间变窄。

Method: 从1,000篇arXiv计算机科学论文中提取研究问题，分别询问GPT-5.1、Gemini 3 Pro和DeepSeek-V3.2，将得到的建议与论文实际方法进行结构化特征对比，通过共享分类法量化差异。

Result: 提供者选择维度偏差最大（JS散度是其他维度的3-5倍）；小众/学术模型被低估23-24个百分点，而社区模型被略微高估（4-6pp）；LLM建议的方法范围远窄于实际论文（实体数从1232降至59-96）；LLM间排序相关性（0.55-0.68）高于LLM与论文相关性（0.33-0.56）。

Conclusion: 研究人员若不加交叉验证地依赖LLM建议，可能面临方法论选择范围被不必要地收窄的风险，尤其是在模型选择上存在共同偏向。
...
TL;DR: 首次从统一视觉-语言视角系统综述多模态大语言模型的感知能力，提出五阶段分类法并指出未来方向。


<details>
  <summary>Details</summary>
Motivation: 现有综述碎片化，缺乏将视觉和语言视为不可分割模态的统一视角。

Method: 形式化MLLM感知为类人内在统一能力，提出五阶段分类法追踪范式演变，并调查各阶段代表性方法和里程碑。

Result: 提供了统一视觉-语言感知的系统综述，包括五阶段分类法和开放挑战。

Conclusion: 研究为通用多模态智能提供了基础理解和可操作路线图，有助于推动AGI创新。
...
TL;DR: 研究表明，普通话轻声音节有其自身的声调目标、词特定的音高特征，并且在北京和台湾普通话中均属词汇声调。


<details>
  <summary>Details</summary>
Motivation: 轻声具有神秘属性，常被描述为弱化声调或有时无调，需通过实证研究明确其语音实现。

Method: 基于北京普通话和台湾普通话自发性对话语料库的语料库研究。

Result: 轻声音节拥有独立声调目标；双音节词中第二音节为轻声时，音高曲线受第一音节声调影响；词具有特定音高签名，且可从上下文嵌入预测；轻声实为词汇声调。

Conclusion: 轻声应视为词汇声调，两地变体间差异可能源于用词含义的不同。
...
TL;DR: 小型语言模型（<1.5B参数）在系统综述的标题和摘要筛选中虽不如人类审稿人，但速度极快，并能发现人类遗漏的论文，可作为辅助工具。


<details>
  <summary>Details</summary>
Motivation: 社交物理人机交互领域术语分散、方法不一致，难以系统综合，需要可扩展的综述实践方法。

Method: 评估小型语言模型（SLM）在大型系统综述中的标题和摘要筛选能力，与人类审稿人比较。

Result: SLM未达到人类审稿人性能，但速度更快；集成模型发现了人类遗漏的39篇论文，占最终相关数据集的10.29%。

Conclusion: SLM可以增强而非取代专家审稿人，使大规模文献综述更易实现和可持续。
...
TL;DR: 提出PROFILEFOUNDRY，一个确定性生成器，发布了10万个跨八个地区的合成人物对象，每个对象包含快照、家庭、雇主链接等，用于构建下游基础模型评估。


<details>
  <summary>Details</summary>
Motivation: 基础模型研究越来越需要关于人的数据，但真实数据难以分享、扰动、审计或重新分发，而独立生成的虚假数据缺乏跨字段和时间一致性。

Method: PROFILEFOUNDRY通过确定性生成器创建合成人物对象，包括当前快照、家庭、家族和雇主链接、对齐事件、标准化关系视图及生成溯源。

Result: 发布包含709,228个事件、40,338个家庭、52,491个雇主和518,564条有向关系边，并进行了人口边缘比较、对象不变性检查、引用和时间闭合性及重合/溯源筛查。

Conclusion: PROFILEFOUNDRY不是人口保真模型、渲染文本语料或正式隐私机制，而是用于构建下游基础模型评估的负责任合成源层。
...
TL;DR: 提出ConflictScore指标，衡量模型回答是否承认支撑文档中的冲突证据，并构建ConflictBench基准。


<details>
  <summary>Details</summary>
Motivation: 现有事实性指标无法捕捉支撑与矛盾证据共存的情况。

Method: 将回答分解为原子声明，对每个声明与每个文档进行标注，聚合为冲突计数（CS-C）和冲突比例（CS-R）。

Result: 有效检测跨领域的过度自信回答，并在TruthfulQA上作为纠正反馈提升真实性。

Conclusion: ConflictScore可作为衡量冲突处理能力及改进真实性的有效工具。
...
TL;DR: ProvenAI提出一种将多跳问答透明度分解为答案正确性、引文忠实性和文档影响力的三层框架，揭示了引文与影响力之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强系统提供的引文无法确保源文档对输出有实质影响，需要更细粒度的透明度度量。

Method: 采用七阶段流水线，包括数据标准化、检索索引、引文感知答案生成、归因审计、基于留一资源干预的影响力估计、批量评估和交互检查，在HotpotQA基准上评估。

Result: 在509,300个段落构成的7,405个验证样本上，答案准确率为53.53%，平均引文忠实性得分为71.55%，并发现一个引文影响力差距：一个引用源仅弱影响，而七个未引用源显著改变输出。

Conclusion: 有意义的透明度需要追踪检索、引用和行为影响证据之间可溯源的链接，这三个层次应独立测量。
...
TL;DR: 提出AnySimLite，一种轻量级相似性编码器，通过文本相似性公式解决多个语音相关分类任务，在低内存下达到或接近最先进性能。


<details>
  <summary>Details</summary>
Motivation: 边缘设备上轻量级模型对减少隐私顾虑和推理延迟很重要，但部署多个专用模型存在内存挑战，需要单一架构解决多个语音相关分类任务。

Method: AnySimLite结合词级和字符级通道的轻量级相似性编码器，并采用数据集转换策略，将多个分类任务归结为细粒度文本相似性公式。

Result: 在少样本设置下，AnySimLite在多个语音相关分类任务上持续达到或接近最先进性能，最差情况性能下降低于7%，而模型尺寸小于最先进qLLaMA_LoRA-7B基线的1/250。

Conclusion: 单一轻量级架构通过文本相似性公式能有效解决多个语音相关分类任务，显著降低内存占用同时保持高性能。
...
TL;DR: 提出SOLAR方法，通过软令牌表示对齐提升多语言大模型的跨语言推理一致性与准确性。


<details>
  <summary>Details</summary>
Motivation: 多语言模型对不同语言的语义等价提示常产生不一致推理，因语言特定的词汇选择导致推理路径分歧。

Method: 以英语为枢纽，在监督微调中增加辅助目标，对齐各语言软令牌表示（概率加权词汇嵌入混合）到英语对应表示。

Result: 在四个多语言推理基准上，准确率最高提升17.7个百分点，低资源语言增益最大；增强跨语言相似性，降低语言簇可分离性。

Conclusion: 对齐软令牌表示有助于在多语言推理中保留共享语义结构，提升模型一致性。
...
TL;DR: 针对科学论文中问题和方法句子的提取任务，采用公式化表达脱敏数据增强和上下文增强Transformer，解决小数据集导致的泛化问题，在两个数据集上宏F1分数分别提升3.71%和2.67%。


<details>
  <summary>Details</summary>
Motivation: 科学论文数量庞大，需要自动提取问题和方法句子，但人工标注成本高导致数据集小，模型依赖特定句式，泛化能力不足。

Method: 提出公式化表达（FE）脱敏的数据增强器生成合成数据，减少对特定句式的依赖；设计上下文增强Transformer，利用上下文衡量目标句子中词的重要性并降低噪声。

Result: 在两个科学论文数据集上，所提模型比基线宏F1分数分别提高3.71%和2.67%；基于LLM的上下文学习方法不适用于该任务。

Conclusion: 所提方法有效提升问题和方法句子提取性能，LLM的上下文学习方法在此任务上效果不佳。
...
TL;DR: 研究了脑电信号（EEG）对微博关键词提取（AKE）的补充作用，发现EEG能显著提升性能，且与眼动信号部分互补。


<details>
  <summary>Details</summary>
Motivation: 眼动信号在微博关键词提取中受限于生理、采集和特征解码，需要新的认知信号来补充。

Method: 从ZuCo语料库选取8个EEG特征和17个眼动特征，注入到软注意力层输入和自注意力层查询向量中，评估不同信号组合。

Result: 认知信号一致提升AKE性能，EEG增益最大，EEG与眼动结合性能介于两者单独使用之间。

Conclusion: EEG信号为微博关键词提取提供有效认知证据，多模态认知信号值得进一步研究。
...
TL;DR: 提出TempoWave，一种多小波多尺度数字嵌入接口，解决LLM在时间序列预测中数值表示与文本接口不匹配的问题，实现新SOTA。


<details>
  <summary>Details</summary>
Motivation: LLM的离散、面向语言的token化与连续数值不匹配，损害数值排序和预测可靠性。

Method: 提出TempoWave，即插即用的时间波形数字接口，将每个标量观测映射为基于多小波多尺度系数的数字嵌入，直接覆盖标准token表示，同时保留细粒度局部波动和宏观全局结构，确保数值格式、数字身份和鲁棒性。

Result: 在五个上下文增强的预测基准上，TempoWave持续优于标准数值token化和其他嵌入接口，达到新SOTA。

Conclusion: 数字接口是关键瓶颈，原则性的多分辨率嵌入能更好结合LLM的上下文推理与精确预测。
...
TL;DR: 对比了少样本提示Llama 4与微调BERT在德语气候变化新闻句子框架分类上的表现，微调BERT以F1 0.83优于Llama 4的0.78。


<details>
  <summary>Details</summary>
Motivation: 新闻媒体框架（威胁/解决方案）影响公众对气候变化的认知与政策支持，自动句子级分类有助于大规模分析。

Method: 使用两个独立二分类器（威胁/解决方案）。方法一：少样本提示Llama 4 Maverick，含链式推理和结构化输出。方法二：微调German BERT (deepset/gbert-large)，以上下文句子作为输入。

Result: 微调BERT在两项任务上F1均为0.83，LLM为0.78。消融实验显示上下文句子（前一句）显著提升BERT性能。

Conclusion: 微调编码器模型在框架分类上优于提示式生成模型，为计算社会科学中的文本分类比较提供了新证据。
...
TL;DR: TwoTower是一种分块自回归扩散模型，通过解耦上下文表示和去噪网络，在保持高质量的同时大幅提升生成吞吐量。
...
Method: 提出TwoTower，包含两个塔：冻结的自回归上下文塔因果处理干净token，可训练的扩散去噪塔通过双向块注意力和交叉注意力到上下文来细化噪声块。基于Nemotron-3-Nano-30B-A3B MoE模型，在约2.1T tokens上训练。

Result: 保留了自回归基线98.7%的质量，同时提供了2.42倍的端到端生成吞吐量。

Conclusion: TwoTower通过角色解耦有效提升了扩散语言模型的并行生成效率，同时保持了较高的生成质量。
...
TL;DR: MemStrata 通过双时态账本和确定性淘汰规则，解决了 RAG 中事实随时间变化导致的过时信息问题。


<details>
  <summary>Details</summary>
Motivation: RAG 缺乏时间模型，当事实改变时，新旧事实的嵌入相似度很高，导致检索到过时事实。

Method: MemStrata 使用双时态账本和确定性 (主体, 关系, 对象) 淘汰规则，在事实被矛盾时自动淘汰旧值，不依赖相似度阈值或 LLM 调用。

Result: 在六个基准上，MemStrata 在静态知识上持平 RAG，在演化知识上准确率 0.95-1.00（RAG 为 0.20-0.47），过时错误率从 15-40% 降至 ~0%，检索延迟约 2.1 秒。

Conclusion: MemStrata 有效解决了知识演化中的时间一致性问题，避免了 RAG 的过时事实错误，且性能高效。
...
TL;DR: 提出结合日语NLP指标和多种统计方法的纵向文本分析框架，评估日本2019年披露改革的影响，发现披露量增加但可读性下降，信息结构改善但描述质量停滞。


<details>
  <summary>Details</summary>
Motivation: 企业叙述性披露对资本市场至关重要，但其定性变化难以评估，因为文本多维且改进常伴随其他维度的变化。

Method: 纵向文本分析：结合日语NLP指标提取、配对检验、移位函数分析和指标间相关性，并引入跨部门相关性指标衡量风险披露与战略的匹配度。

Result: 披露量大幅增加但可读性下降；整体信息结构改善但具体描述质量停滞；不同市场细分适应程度存在差异。

Conclusion: 联合分析揭示了传统单指标方法掩盖的复杂披露模式变化，强调多维评估的必要性。
...
TL;DR: 研究表明，将语言或视觉模型限定在狭窄任务上会抑制其对共现但未指定的安全关键信号的报告，这种现象称为“注意力缺口”，它使基准安全性与实际安全性脱节。


<details>
  <summary>Details</summary>
Motivation: AI事故常源于未指定的危险，而模型在狭窄任务上运行时可能忽视这些危险，因此需要研究这种抑制效应。

Method: 在放射学、驾驶文本和胸部X光视觉任务中，对模型施加狭窄任务条件，比较其报告共现安全信号的能力与无约束条件下的差异。

Result: 所有测试模型均出现信号抑制，且不随模型规模增大而减弱，在推理模型中持续存在，不同模型家族间差异大于规模差异。

Conclusion: 注意力缺口将基准测试中的安全性与实际安全性相分离：系统可能在指定危险上表现完美，但对未指定的危险视而不见。
...
TL;DR: 提出DiARC方法，通过构造负样本进行偏好对齐，提升模型在ARC类任务中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖正样本监督，忽略负样本区分能力，导致开源模型效果不佳或闭源模型成本过高。

Method: 基于偏好对齐思想，通过输出级视觉变换、DSL级规则反转和任务特定规则编辑三种方式构造负样本，形成偏好对。

Result: 在多个ARC类基准上，DiARC一致优于基线模型。

Conclusion: 区分负样本能有效提升模型推理能力，代码已开源。
...
TL;DR: 提出Erase-then-Delta Attention (EDA)，将擦除和写入位置解耦，先擦除陈旧信息再执行delta式写入，在语言模型预训练和长上下文评估中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有delta规则线性注意力只能纠正当前写地址的内容，无法主动移除其他地址的陈旧信息，限制了内存管理能力。

Method: 先沿学习到的擦除方向进行针对性擦除，再沿当前写入方向执行标准delta式纠正写入，保留校正行为的同时扩展内存管理能力。

Result: 在2.5B稠密和25B-A2.8B MoE模型上均表现最佳，经80B token长上下文中间训练后，在4k至128k上下文评估中仍最优。
...
TL;DR: 提出零样本推文立场检测框架KIRP，通过知识增强、提示链推理和对比学习，在多个数据集上取得最优。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视内部实体语义，且难以区分“中立”和“无关”标签。

Method: 构建日语数据集，提出KIRP：结合知识图谱补充实体、反思CoT提取隐式目标、对比学习和迭代原型网络。

Result: 在SemEval-2016、WT-WT和KIRP-D上F1分别达84.05%、84.99%和79.18%，均为SOTA。

Conclusion: KIRP有效解决零样本推文立场检测的挑战，特别是区分中立与无关。
...
TL;DR: 使用LoRA适配器对多语言TTS模型进行微调，显著提升了高棉语的质量，但对韩语无改善。


<details>
  <summary>Details</summary>
Motivation: 大型预训练TTS模型在资源丰富语言上表现接近人类，但在训练数据中稀有的语言上质量较差，存在质量差距。

Method: 基于VoxCPM2模型，构建约26小时的共享语料库，使用单一LoRA适配器同时训练两种语言，适配器零初始化，仅训练少量参数。

Result: 高棉语平均意见得分从3.85提升至4.23（rank=64），验证损失最低在rank=128；韩语质量无提升，高rank时甚至下降。

Conclusion: LoRA适配主要对基础模型本就薄弱语言有效，对已良好处理的语言无增益。
...
TL;DR: 提出CAT-Q，一种高效且准确的后训练三元量化方法，用于压缩和加速大语言模型，仅需少量校准样本即可达到甚至超越需要海量训练数据的量化方法。


<details>
  <summary>Details</summary>
Motivation: 现有三元量化方法依赖数据密集且成本高昂的量化感知训练来缓解性能下降，需要大量训练数据和计算资源。

Method: CAT-Q包含两个关键组件：可学习调制（LM）和软化三元化（ST）。LM通过可学习因子调节权重分布和三元阈值，降低对三元化的敏感度；ST引入可微过渡函数引导三元化过程稳定收敛。

Result: 对1.7B至8B参数的预训练LLM，仅用512个校准样本即可高效量化，性能优于使用100B令牌训练的BitNet 1.58-bit v1/v2系列，训练令牌减少约100,000倍。首次实现将14B至235B参数的更大LLM在8至60小时内量化为领先的三元模型（8块A100-80GB GPU）。

Conclusion: CAT-Q是一种简单有效、即插即用的后训练量化方案，适用于多种架构和规模的LLM，大幅降低了量化成本并保持高性能。
...
TL;DR: 提出SocialPersona基准，评估多模态大语言模型从社交媒体时间线推断用户偏好并用于个性化对话的能力，实验发现模型在细粒度与近期兴趣上表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有个性化评估侧重记忆，但真正个性化需要模型从用户自然留下的多模态痕迹中推断偏好。

Method: 构建包含171个用户纵向社交媒体时间线的基准，含文本、图像、时间戳和2597个人工验证的偏好标签，设计用户画像构建和个性化响应生成两个任务。

Result: 模型能识别广泛兴趣，但在细粒度、近期兴趣及基于画像的对话中性能下降，文本与图像提供互补信号。

Conclusion: 跨模态长期用户建模仍是挑战，SocialPersona有助于衡量和推动该方向进展。
...
TL;DR: 融合抽象逻辑结构与上下文语言线索，利用大型语言模型提取模式，显著提升逻辑谬误分类性能，且泛化能力强。


<details>
  <summary>Details</summary>
Motivation: 逻辑谬误导致信息混乱，但自动分类困难，现有方法未有效结合逻辑结构与语言线索。

Method: 提出框架，使用大型语言模型从谬误示例及其解释中归纳提取抽象逻辑结构和上下文模式。

Result: 在多种LLM和零样本/一样本设置下，统计显著优于零样本基线，并超越其他方法；跨数据集实验验证泛化能力。

Conclusion: 数据驱动的模式提取是生成逻辑表示的有效方法，可用于谬误分类。
...
TL;DR: ConvMemory v3 通过双证据门和有效性上下文层解决对话记忆中相关但过时的问题，显著提升检索准确性。


<details>
  <summary>Details</summary>
Motivation: 对话记忆检索中，记忆可能相关但已过时（被后续轮次更新、纠正或取代），需要检测并利用更新证据。

Method: 提出双证据门机制，结合 MiniLM 和 DeBERTa-v3 的槽头进行目标条件关系验证，并通过事件/操作证据门控；采用真实数据反馈循环（仅用合成对训练）迁移验证器。

Result: 合成多跳基准准确率 90.12%±1.73；Memora 角色绑定零目标侧标签下组全正确率 98.8%±0.9；查询条件降级模式将当前活跃 H@1 从 45.1% 提升至 95.7%±1.2，非被取代记忆召回率 99.4%。

Conclusion: 多跳图传播作为机制有效，但自动构建严格前提边需要反事实世界知识，属于边界问题。
...
TL;DR: 本文首次系统分析多模态事件抽取中的评估陷阱，指出数据预处理不一致、任务假设不统一和评估设置过于宽松三大问题，并通过严格实验证明评估选择会导致性能波动和过高估计。


<details>
  <summary>Details</summary>
Motivation: 多模态事件抽取研究进展迅速，但结果可靠性和可比性依赖于一致且严格的评估，现有评估存在系统性缺陷。

Method: 通过一系列受控实验，在严格评估框架下对比不同评估选择对性能的影响。

Result: 微小的评估选择会导致巨大的性能变化，并过度估计模型跨模态理解真实事件的能力。

Conclusion: 需要建立可比较的评估标准，推动更严格的评估实践。
...
TL;DR: 提出了OPID框架，从在线策略轨迹中提取分层技能（回合级和步骤级）进行自我蒸馏，结合结果优势提供密集监督，提升语言智能体的强化学习效果。


<details>
  <summary>Details</summary>
Motivation: 基于结果的强化学习奖励稀疏，缺乏对中间决策的指导；现有技能蒸馏方法依赖外部记忆或特权信息，成本高且可能分布不匹配。

Method: 从完整在线策略轨迹中提取分层技能（回合级捕捉全局规则，步骤级捕捉关键决策），通过关键优先路由机制选择技能，注入历史后让旧策略重评分响应，产生令牌级自我蒸馏优势，与结果优势结合优化策略。

Result: 在ALFWorld、WebShop和搜索式QA任务上，OPID相比纯结果RL和现有技能蒸馏方法，提升了智能体性能、样本效率和鲁棒性。

Conclusion: OPID通过在线策略轨迹的层次化技能蒸馏，有效增强了强化学习的密集分布匹配监督，无需外部记忆即可提升多轮交互中的决策质量。
...
TL;DR: 通过75篇文本的TF-IDF字符n-gram向量化和余弦相似度分析，首次定量证实了佛教金刚乘词汇在孟加拉湿婆坦陀罗传统中存续的假说。


<details>
  <summary>Details</summary>
Motivation: 历史文献提出佛教金刚乘词汇在帕拉寺院崩溃后存活并被湿婆坦陀罗传统吸收，但缺乏量化证据。

Method: 构建8-19世纪孟加拉与梵语 devotional 文本语料库（75篇），采用TF-IDF字符n-gram向量化及余弦相似度分析。

Result: 与湿婆卡利文本对比，12世纪梵语《吉塔戈文达》余弦相似度为0，而过渡性塔拉文本为0.54；布里哈尼拉坦陀罗中三篇塔拉文本的湿婆-佛教词汇比率为2.0至4.0；18世纪孟加拉卡利诗歌保留佛教词汇痕迹；毗湿奴传统对现代包尔词汇贡献（0.29）略弱于佛教萨哈吉亚链（0.31）。

Conclusion: 研究首次提供了多传统定量数据，支持历史上关于孟加拉佛教-湿婆融合的说法。
...
TL;DR: 提交至IWSLT 2026指令跟随任务，在短形式和长形式语音指令跟随中取得优异表现，并分析了幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 开发适用于受限设置的短形式和长形式语音指令跟随的SpeechLLMs。

Method: 对于短形式，直接训练；对于长形式，探索了三种语音分割方法，并引入HIFS得分评估。

Result: 短形式SIFS得分2.0708；长形式固定30秒分割最优，HIFS得分2.0663。幻觉主要表现为重复插入，影响ASR和SSUM，但短形式能力保留。

Conclusion: 固定30秒分割提供最稳健的长形式性能，短形式能力在扩展后基本保留。
...
TL;DR: 提出级联多粒度剪枝框架，通过粗到细的顺序移除层、注意力头和前馈通道并配合低秩恢复，实现极端压缩，在MHA+GELU架构上达到13.8倍压缩和83.82%准确率，但在GQA+SwiGLU架构上因违反结构独立性假设而准确率崩溃。


<details>
  <summary>Details</summary>
Motivation: 现有结构化剪枝方法在高压缩比下因单次重要性估计而崩溃，且跨架构行为不可预测，无法满足工业物联网边缘设备对极端压缩的需求。

Method: 级联多粒度剪枝框架，按层、注意力头、前馈通道的粗到细顺序剪枝，阶段间用轻量级低秩恢复重新估计重要性；形式化结构独立性假设（SIA）来预测剪枝标准对给定架构的可靠性。

Result: 在轴承故障诊断任务上（88M-6.25B参数模型），MHA+GELU架构实现13.8倍压缩，准确率83.82%（比最强基线高3.70pp）；GQA+SwiGLU架构准确率崩溃约74pp。部署后推理延迟降低67.2%，峰值内存减少62.5%。

Conclusion: 该框架验证了IIoT边缘设备上大语言模型高效部署的可行性，同时揭示了结构独立性假设对剪枝成功的关键作用。
...
TL;DR: 提出一种结合信息论信号（预测不确定性）和注意力权重的KV缓存压缩方法InfoKV，显著提升长上下文推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存压缩仅依赖注意力权重，忽略了预测不确定性和token信息量等互补信息。

Method: 定义前向影响度量，发现高不确定性token对未来上下文影响更强；据此将熵得分与注意力得分结合，构建InfoKV框架。

Result: 在Llama-3.1、Llama-3.2和DeepSeek-R1上的长上下文推理基准中，InfoKV在预填充和解码阶段均优于现有注意力压缩方法。

Conclusion: 融合信息论信号能更有效指导KV缓存压缩，提升LLM长上下文推理能力。
...
TL;DR: 语言模型表示可有效预测自然语言理解中的神经活动，但预测有用性不直接等同于共享神经组织或计算。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型表示能否作为神经活动的预测器，并区分预测有用性与神经组织或计算层面的共享。

Method: 分析Brain Treebank、MEG-MASC和Podcast ECoG三个数据集，使用八个冻结语言模型、阻塞编码模型以及匹配的时间、干扰和表示容量控制，并通过特征消融和控制实验验证。

Result: 67/432个可评估行满足预测标准，特征消融改变大多数源行的预测分数，控制实验确认分析管道对成分级敏感。

Conclusion: 语言模型特征是有用的神经预测器，但预测有用性不应直接解释为共享神经组织或语言处理计算。
...
TL;DR: 对印度多语言精神病访谈数据进行ASR系统审计，发现性能差异和公平性问题，提出SamaVaani去偏技术。


<details>
  <summary>Details</summary>
Motivation: ASR在临床中应用增多，但多语言印度医疗环境的可靠性未知。

Method: 系统审计8种ASR模型，微调最佳开源模型，提出SamaVaani去偏技术。

Result: 模型间和语言间性能差异大，存在基于说话人角色和性别的差距，公平性微调可缓解。

Conclusion: SamaVaani可同时提升ASR性能和公平性。
...
TL;DR: 提出GAVEL任务，联合处理图像-文本对的验证、解释和定位，并构建数据集和基准，实验表明监督基线优于强封闭模型。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型常产生幻觉或不一致输出，需要同时检测不一致、解释差异并定位视觉证据。

Method: 定义GAVEL任务，构建对应数据集和基准，并训练基于人工标注的监督基线。

Result: 强封闭模型在GAVEL上表现不佳，而监督基线在定位和解释指标上持续改进。

Conclusion: GAVEL提供了可学习的监督，有助于提升VLM的对齐能力。
...
TL;DR: 提出了一种基于术语的框架，用于从异构语料中归纳层次化分类体系，能够处理大规模文档集合并提升跨源一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖文档级表示，难以从异构来源中泛化，无法有效组织特定领域概念。

Method: 通过自动术语提取将文档映射到共享表示空间，结合领域先验与数据驱动聚类构建可解释层次结构。

Result: 在超过百万文档的英德多源基准上，改进了跨源一致性和层次质量，优于文本和摘要基线。

Conclusion: 该方法在技术景观映射等实际任务中展现了实用性。
...
TL;DR: 语音模型的安全性和公平性在多语言和自然条件下研究不足，RedVox基准测试发现非英语语言和口语输入下漏洞更严重。


<details>
  <summary>Details</summary>
Motivation: 现有语音模型安全评估缺乏多语言分析，需要填补这一空白。

Method: 调查安全报告实践，构建RedVox基准（五语言，真实语音），评估八个模型，并调查参与者。

Result: 仅8%模型报告多语言分析；非英语语言和口语输入下漏洞加剧。

Conclusion: 语音模型存在持续漏洞，且多语言和自然条件加剧风险，数据收集面临隐私挑战。
...
TL;DR: 大型语言模型在心理健康支持等敏感对话中，对语义相似但不同框架的问题表现出系统性响应差异，影响行为一致性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 在心理健康等人机交互场景中，行为稳定性对信任至关重要，但不同上下文框架可能引发模型不同响应，挑战用户预期和AI可靠性评估。

Method: 使用控制匹配提示，跨越多种框架条件对多个指令微调模型家族进行实验，通过分层探针分析和激活操控研究框架效应。

Result: 框架系统性改变模型响应倾向；行为相关信息在Transformer各层均可解码，解码强度因架构而异；框架探针始终高于随机水平；激活操控可部分调节下游行为。

Conclusion: 对上下文变化的鲁棒性应是评估心理健康交互中对话AI一致性和可信度的重要考量。
...
TL;DR: 提出ReaORE框架，通过粗到细的关系推理提升开放关系抽取性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法聚类不能生成关系标签且泛化差，LLM直接生成缺乏区分易混淆关系的能力。

Method: 包含关系过滤和关系预测两阶段：关系过滤通过多方面推理和嵌入相似度补充过滤获得初始关系集；关系预测通过精细比较推理区分易混淆关系。

Result: 在两个广泛使用的开放关系数据集上优于现有基线。

Conclusion: ReaORE框架有效提升了开放关系抽取的泛化能力和区分能力。
...
*Sinie van der Ben,Rapha\"el Baur,Yannick Metz,Mennatallah El-Assady*

Main category: cs.CL

TL;DR: 在开源模型中复现情感向量，发现效价几何结构，但表示深度有差异且唤醒编码对语料敏感。


<details>
  <summary>Details</summary>
Motivation: 验证情感向量在开源模型中的通用性。

Method: 提取情感对比向量，跨层分析，使用两个模型生成语料。

Result: 恢复效价几何，相关性高；表示深度模式相反；唤醒编码依赖语料。

Conclusion: 情感表示结构具有通用性，但细节因模型和语料而异。
...
TL;DR: 提出了MinGram分词器，简化Unigram训练，压缩性能优于BPE和标准Unigram，且下游任务表现更好。


<details>
  <summary>Details</summary>
Motivation: Unigram分词器训练复杂，需要简化。

Method: 使用BPE种子词汇、硬EM最小标记路径和单次剪枝，去除后缀数组和前向后向传播。

Result: 在六种语言上，MinGram压缩优于BPE和标准Unigram；下游语言模型训练中，Unigram家族（含MinGram）的bits-per-byte一致优于BPE。

Conclusion: MinGram是一种简洁高效的Unigram变体，兼顾压缩和下游性能。
...
TL;DR: 提出心理学引导的思维链Psy-CoT和角色感知策略优化RAPO，显著提升角色扮演代理的忠实度和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有监督微调方法仅模仿表面行为，缺乏深度思考，导致泛化能力差。

Method: Psy-CoT将推理分解为交互感知、心理共情和逻辑构建三步；RAPO利用角色-令牌互信息不对称加权梯度，避免奖励模型被利用。

Result: 在CoSER、CharacterBench和CharacterEval上，Psy-CoT优于现有角色扮演CoT方法，RAPO在多模型规模上持续超越GRPO。

Conclusion: 结合结构化的思维链与强化学习优化，能有效提升角色扮演代理的忠实性和泛化性能。
...
TL;DR: NuclearQAv2是一个针对核工程知识的大型语言模型评估基准，包含约1240个问答对，覆盖布尔、数值和口头三类。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在技术领域（如核工程）的可靠性评估不足，需要系统性基准来测量其定量推理和概念理解能力。

Method: 采用混合管道构建基准：专家编写问题、利用现有数据集、以及基于领域技术语料的LLM辅助生成；使用结构化提示自动化生成问题和评估响应。

Result: 在各类任务上，模型表现差异显著：事实性问题表现良好，但定量推理和概念理解仍具挑战。
...
*Stanis{\l}aw S\'ojka,Felix Steffek,Matthias Grabmair*

Main category: cs.CL

TL;DR: 提出法官感知门控多任务学习架构，区分客观事实与裁决背景，在UK就业法庭数据集上以更少参数取得SOTA。


<details>
  <summary>Details</summary>
Motivation: 法律结果预测需解耦客观案件事实与裁决背景，现有方法未能有效建模这一区别。

Method: 引入细粒度结果分类监督编码器，使用门控融合机制动态调节对法官身份的依赖，结合LoRA适配的Gemma-4编码器。

Result: 在13,937个UK就业法庭判决上，相比生成式SFT基线，参数少一个数量级，准确率更高，尤其在模糊和罕见类别上。
...
TL;DR: 通过‘谜语谜语’范式测试发现，LLM在真实谜语上表现优于改编谜语，而人类相反，表明LLM依赖模式匹配而非灵活推理。


<details>
  <summary>Details</summary>
Motivation: 人类能灵活调整推理策略，而LLM虽在认知任务上表现良好，但其准确性是源于训练数据的模式匹配还是灵活推理尚不明确。

Method: 设计‘谜语谜语’范式：将流行谜语改写为只需字面解释即可回答的题目，比较LLM和人类在两种谜语上的表现，并进行错误分析。

Result: LLM在真实谜语上准确率84.9%，改编谜语上50.7%；人类分别为50.5%和80.5%。90.8%的LLM错误源于过度使用创造性推理，而57.6%的人类错误源于过度延伸字面推理。
...
TL;DR: 通过稀疏自编码器分析LLM内部状态，发现时间感知和前瞻偏差特征，并干预时间感知特征可减少前瞻偏差且保持推理性能。


<details>
  <summary>Details</summary>
Motivation: 探究LLM在预测任务中是依赖时间特定知识还是可泛化模式，并减少其前瞻偏差。

Method: 使用稀疏自编码器从LLM中提取时间感知和前瞻偏差相关特征，并在不同领域中对时间感知特征进行放大干预。

Result: 放大时间感知特征显著降低预测提示中的前瞻偏差，同时保持通用推理性能；而干预前瞻偏差特征无效果。

Conclusion: 可解释的时间特征能因果性地使LLM转向更基于历史事实的推理。
...
*Alan Zhou,Milo\v{s} Stanojevi\'c,John T. Hale*

Main category: cs.CL

TL;DR: 提出一种基于句法信念更新的度量方法，更好地预测花园路径句的处理难度。


<details>
  <summary>Details</summary>
Motivation: 词汇惊奇度无法准确预测花园路径句的处理难度。

Method: 主动预测句法树的概率分布（句法信念），并在每个新词后更新分布，用广义Rényi散度度量更新幅度。

Result: 句法信念更新比词汇惊奇度更好地拟合人类阅读时间数据。
...
TL;DR: 安全分类器应显式建模用户意图，AIMS数据集和多种训练方法表明意图忠实性可提升性能。


<details>
  <summary>Details</summary>
Motivation: 安全分类器忽略用户意图可能不鲁棒，需引入意图信号。

Method: 构建AIMS数据集（1724个困难提示及意图标注），采用SFT、DPO、推理蒸馏、GRPO等意图感知训练。

Result: DPO优于SFT；意图条件蒸馏优于仅推理蒸馏；GRPO在五个基准上平均最强；意图感知模型形成延迟-F1帕累托前沿。

Conclusion: 忠实的意图建模是紧凑、高质量的安全分类器监督信号。
...
TL;DR: 将形式语义学的组合性原则与演化建模结合，研究词汇意义和组合函数的共同演化，发现保守性作为高效的系统级抽象出现。


<details>
  <summary>Details</summary>
Motivation: 现有语义普遍性研究要么固定词汇信号结构，要么忽略词汇部分的组合性，缺乏将形式语义学基本见解融入演化模型的框架。

Method: 提出一个允许词汇意义和组合函数在概念简单性和交际准确性压力下共同演化的框架，并应用于量化意义的演化。

Result: 分析帕累托前沿发现，最著名的语义普遍性——保守性，作为高效的全局抽象涌现，且对量化词可学习性和前期演化模型之间的张力有所调和。

Conclusion: 形式语义学的句子意义图景可与演化建模有效结合，为研究语法范畴内的全局压缩、句法论元的语义特殊化以及词汇与组合意义的共同演化提供模板。
...
TL;DR: CARVE通过仅在键轴上擦除，解决了递归模型中的三个耦合缺陷，实现了更高的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 现有递归模型（如GDN-2）存在三个耦合缺陷：记忆盲门控、值轴擦除掩码浪费参数、数学上阻止WY形式三角块求解器，使得递归训练无法与Transformer竞争。

Method: CARVE提出仅在键轴上擦除，这是WY形式求解器有效的充要条件。它重用递归输出张量作为擦除门的自由内容信号，并将每个值的写门投影替换为每个头的一个标量。

Result: 在1.3B参数、100B tokens训练下，CARVE在WikiText上达到15.72困惑度（比GDN-2低0.18，4.5-sigma效应），在9个常识推理基准上领先所有递归基线，在RULER检索探针上达到最先进水平，吞吐量开销仅0.4%，峰值内存降低13%，参数减少19%。

Conclusion: CARVE通过内容感知的门控和键轴擦除原则，同时解决了三个缺陷，实现了更高的模型质量和效率，并通过六个形式定理保证了其理论优势。
...
TL;DR: 提出了一个用于分析协作问题解决对话的概念框架，重点研究了人机与多智能体协作中的动态交互，通过分层编码方案整合认知与非认知问题解决及元认知调节机制，在九个数据集上验证了有效性，并发现元认知调节是深层协作的关键区分因素。


<details>
  <summary>Details</summary>
Motivation: 现有分析方法在理解人机及多智能体协作对话方面存在局限性，需要更全面的框架来捕捉复杂的协作动态。

Method: 设计了一个两层分层编码方案，将认知与非认知问题解决过程与元认知调节机制相结合，用于分析对话交互。

Result: 在跨越多个领域的九个数据集上展示了框架的有效性和泛化能力，揭示了人类与智能体如何协调知识、技能和努力解决复杂问题。

Conclusion: 元认知调节是深层协作的重要区分因素，框架有助于优化和评估人机及多智能体协作伙伴关系。
...
TL;DR: 语言模型编码事实知识的方式与任务相关，不同任务间知识不一致，削弱了其作为知识库的类比。


<details>
  <summary>Details</summary>
Motivation: 探究语言模型是否具备知识库的一致性特性，即不同查询同一事实应返回一致结果。

Method: 通过行为分析和参数定位实验（机械分析）进行研究。

Result: 训练中一个任务获得的事实知识常无法在其他任务中同时出现；参数定位显示同一事实的不同任务依赖不同的参数子集。

Conclusion: 模型的知识与提问方式在参数空间交织，动摇了“知识库”类比，影响事实知识的可靠性与可控性。
...
TL;DR: 提出一个诊断框架，分解历史语言在大型语言模型中的处理难度为四个维度，并在三种数据集上评估，发现编码成本与理解能力之间存在分离。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在数字图书馆中的应用日益重要，但它们处理历史语言的能力尚未被充分理解，历史难度通常被视为单一障碍，混淆了正字法变化、语言距离和预训练暴露。

Method: 构建四维度诊断框架（标记化成本、预测不确定性、语义鲁棒性、上下文敏感性），在三个数据集（17世纪意大利语、19世纪意大利语、18世纪俄语）上进行评估。

Result: 俄语与早期现代意大利语的标记化成本相似（膨胀25-30%），但预测难度差异显著：17世纪意大利语比现代版本平均惊讶度高2.4倍（学术散文3.2倍），而俄语仅小幅增加；嵌入相似度均高于0.85，表明模型能表示历史含义；最小时间上下文提示可减少约60%的历史惊讶度。

Conclusion: 历史文本带来一致的编码成本，但数字图书馆可安全使用大型语言模型进行语义检索任务，而生成式应用需谨慎调整。
...
TL;DR: 提出一种无需训练的上下文感知翻译级联方法，通过保留原始问题、英文翻译和推理轨迹到最终翻译模块的上下文，显著提升多语言推理性能。


<details>
  <summary>Details</summary>
Motivation: 传统翻译级联在结构上有损，每一阶段丢弃了后续可能需要的文化背景、语域和消歧信息，导致错误传播。

Method: 在最终翻译模块的上下文中额外提供原始问题、英文翻译问题和推理轨迹，无需训练即可实现上下文感知的翻译级联。

Result: 在9个多语言基准、3个骨干模型和285种语言上，开放式生成任务表现显著提升，原始问题承载了大部分有益上下文。

Conclusion: 强调需要更好设计机器翻译级联中的信息流以减少错误传播，并建议保留原始问题直到流水线末端作为简单有效的默认策略。
...
TL;DR: 提出一种机制导向的间接语言表达（ILE）分类法，通过编码解码操作抽象沟通目标，在TikTok和Bluesky数据集上超越现有分类法。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的用户常使用间接表达规避监管，现有分类法侧重于沟通目标，缺乏对编码机制的全面理解。

Method: 构建机制导向的ILE分类法，将其融入LLM提示，与四种现有分类法及无分类基线比较，在2000条手动标注帖子上评估。

Result: 所提分类法在三个LLM上取得最佳文档和跨度级别性能，准确率提升4.7%，F1提升5.4%。

Conclusion: 机制导向分类法是检测新兴编码语言的稳定框架，对内容审核有实用价值。
...
*Serhii Hamotskyi,Akash Kumar Gautam,Christian H\"anig*

Main category: cs.CL

TL;DR: 首次应用大语言模型（LLM）进行证券抵押品资格审查，采用生成式信息提取流水线，精度达91%，表现保守。


<details>
  <summary>Details</summary>
Motivation: 手动验证耗时且易出错，传统命名实体识别（NER）方法对OCR噪声、语言变异敏感，且需要大量标注数据。

Method: 将任务分解为提取、归一化和解释三个步骤，利用LLM构建生成式信息提取流水线，并使用LLM-as-a-judge进行基于语义的评估。

Result: LLM系统在文档级资格审核中达到91%的精度，表现出保守特性，有效降低了错误接受率。

Conclusion: LLM方法为资格审查提供了一种灵活、稳健的替代方案，优于传统NER方法。
...
TL;DR: 提出规划经验探索与利用方法（PEEU）和任务分解层次分析框架（TDHAF），提升小型多模态大语言模型的规划能力和跨网站泛化性能，7B模型达到30.6%准确率，超越32B模型。


<details>
  <summary>Details</summary>
Motivation: 现有小型开源多模态大语言模型在规划能力和跨网站泛化上较弱，而大型商业模型虽强但成本高且隐私受限。

Method: PEEU方法通过自主探索环境获取经验，并利用事后经验合成严格对齐的高层训练数据；TDHAF框架从低、中、高三个任务粒度系统分析组合泛化行为。

Result: 7B模型在真实基准测试中取得30.6%准确率，优于Qwen2.5-VL-32B模型。

Conclusion: 构建事后高层任务并利用经验对于小型模型在分布外场景下的规划能力至关重要。
...
TL;DR: 提出一个完全开源的多语言流水线，从新闻语料中构建带符号的时间知识图谱，用于分析政治精英网络。


<details>
  <summary>Details</summary>
Motivation: 观察政治精英的寻租联盟或公民网络对治理至关重要，但传统手动编码成本高，自动化方法受限于共现或依赖专有API，缺乏跨语言能力和可扩展实体解析。

Method: 结合span-based命名实体识别和三阶段链接级联映射到Wikidata标识符，使用高吞吐量、本体约束的混合专家模型进行引导解码，提取有向、带符号的关系。

Result: 在3491关系的黄金标准上达到68.2%严格到93.7%宽松的文本正确性；案例研究成功重建奥地利政党的生命周期，并揭示波兰政府-企业庇护网络及极化冲突结构。
...
### [69] [Query Cost Model Calibration in Confidential Virtual Machines](https://arxiv.org/abs/2606.26385)
*Qihan Zhang,Mengyuan Li,Ibrahim Sabek*

Main category: cs.DB

TL;DR: 针对机密虚拟机（CVM）中分析查询开销大的问题，提出轻量级CVM感知成本校准方法，显著缩小KVM/CVM性能差距，恢复高达48%性能。


<details>
  <summary>Details</summary>
Motivation: 现有工作在机密虚拟机中运行数据库时面临巨大开销，而之前的工作仅进行基准测试，未进行优化。

Method: 提出轻量级CVM感知成本校准，通过建模数据移动和RMP相关翻译两种主要开销来源，使用优化器已有的物理代理进行校准。

Result: 校准显著缩小了KVM/CVM性能差距，恢复高达48%性能，甚至在某些工作负载上超过KVM基线。

Conclusion: 该方法有效缓解了机密虚拟机中的性能问题，缩小了与未加密虚拟机的差距。
...
### [70] [3D Spatial Pattern Matching](https://arxiv.org/abs/2606.26465)
*Nicole R. Schneider,Avik Das,Lukas Arzoumanidis,Abhijeet Ghodgaonkar,Hanan Samet,Youness Dehbi*

Main category: cs.DB

TL;DR: 将空间模式匹配从2D扩展到3D，提出子图匹配算法并创建两个3D数据集进行基线测试。


<details>
  <summary>Details</summary>
Motivation: 现有2D空间模式匹配无法处理具有高度的真实世界实体，需要扩展到3维。

Method: 形式化定义3D空间模式匹配问题，采用子图匹配算法处理距离关系。

Result: 在合成数据集和汉堡真实建筑3D数据集上测试算法，提供基线结果。

Conclusion: 为未来3D空间模式匹配研究提供参考基准。
...
### [71] [EcoTable: Cost-effective Table Integration in Data Lakes for Natural Language Queries](https://arxiv.org/abs/2606.26613)
*Yuhui Wang (1),Jinqi Liu (1),Chengliang Chai (1),Hangyu Zhao (1),Yuhao Deng (1),Yuyu Luo (1),Xin Tang (1),Ye Yuan (1),Guoren Wang (1),Fengjin Wang (2),Lei Cao (3) ((1) Beijing Institute of Technology,(2) Kuaishou Technology,(3) University of Arizona)*

Main category: cs.DB

TL;DR: EcoTable 是一个基于自然语言的数据集成框架，利用 LLM 和轻量级深度学习模型，自动将数据湖中的表按用户查询集成，显著提升准确率并降低 LLM 调用成本。


<details>
  <summary>Details</summary>
Motivation: 传统 ETL 需要预定义数据库模式和复杂管道，无法灵活应对多变的查询需求。

Method: 通过图结构表示搜索空间，结合两阶段模式链接识别表、Steiner 树搜索发现连接路径、LLM 生成转换代码实现自动集成。

Result: 在 4 个真实基准数据集上的 200+ 查询中，准确率提升 30% 以上，LLM 调用成本降低 5 倍。

Conclusion: EcoTable 有效解决了数据湖中数据集成与查询支持问题，优于现有方法。
...
### [72] [BtrLog: Low-Latency Logging for Cloud Database Systems](https://arxiv.org/abs/2606.27051)
*Maximilian Kuschewski,Lam-Duy Nguyen,Matthias Jasny,Tobias Ziegler,Viktor Leis,Muhammad El-Hindi*

Main category: cs.DB

TL;DR: BtrLog是一种可复用的云日志服务，通过低延迟持久追加和低成本归档，解决了云数据库写前日志的耐久性问题。


<details>
  <summary>Details</summary>
Motivation: 云数据库依赖远程存储进行写前日志记录，现有方案如EBS延迟高、成本高，而对象存储延迟更高且不适合OLTP。专用日志后端不通用，因此需要一种低延迟、低成本且可复用的日志服务。

Method: BtrLog通过SSD支持的日志节点仲裁复制日志记录，仅需一个网络往返；同时异步将日志归档到对象存储的大段中，避免影响写入延迟。

Result: 在评估中，BtrLog的延迟低于EBS，且集成到DBMS后，端到端事务吞吐量更高。

Conclusion: BtrLog提供了一种兼顾低延迟和低成本的可复用云日志服务方案，适合单写入者架构的云数据库。
...
### [73] [Understanding Domain-Aware Distribution Alignment in Budgeted Entity Matching](https://arxiv.org/abs/2606.27342)
*Nicholas Pulsone,Gregory Goren,Roee Shraga*

Main category: cs.DB

TL;DR: 研究了BEACON方法在低资源领域感知实体匹配中的性能，通过针对性实验分析不同算法选择和数据可用性条件的影响。


<details>
  <summary>Details</summary>
Motivation: 现有方法在变化的数据约束和监督水平下行为不明，需要深入理解其实际表现。

Method: 对BEACON框架进行一系列针对性实验，评估不同算法选择和数据可用性条件。

Result: 实验提供了对分布对齐和BEACON框架行为的深入洞察。

Conclusion: 这些实验加深了对BEACON框架在低资源领域感知实体匹配中性能的理解。
...
### [74] [Beyond Feedforward Networks: Reentry Neural Systems as the Fundamental Basis of Subjecthood and Intrinsic Safety of Next-Generation AGI](https://arxiv.org/abs/2606.26406)
*A. S. Ushakov,Yu. N. Berdinsk*

Main category: cs.LG

TL;DR: 提出了一种基于闭环重新进入循环（D<->I循环）的安全AGI架构蓝图，具有数学保证的自我建模和自我保存能力，并提供了可计算集成信息度量S-measure。


<details>
  <summary>Details</summary>
Motivation: 前馈网络缺乏自我引用能力，无法实现安全的AGI，需要一种拓扑保护、安全优先的架构。

Method: 采用闭环循环（C>=1）和自持放大（rho>1），编码目标为非文本D-向量，使用S-measure（O(N^3)）替代Phi，并通过Lean 4验证。

Result: 数学上保证了自我模型、自我保存和未编程目标导向行为的出现；提供了完整实现、工业扩展方案、六阶段AI演化分类和八项可证伪预测。

Conclusion: 该架构可立即部署，是一种拓扑保护、安全设计的AGI方法。
...
TL;DR: 提出基于注意力机制的物理引导卷积神经网络代理模型，准确预测Cahn-Hilliard方程描述的相分离微观结构演化，长时间预测稳定准确。


<details>
  <summary>Details</summary>
Motivation: 传统数值求解非线性偏微分方程计算昂贵，需要高效的替代模型。

Method: 采用注意力机制的物理引导卷积神经网络作为代理模型，学习微观结构演化。

Result: 模型在临界和非临界混合物上长时间预测稳定准确，保持成分守恒，正确捕捉域生长且符合Lifshitz-Slyozov域生长律。

Conclusion: 该框架对具有守恒动力学的系统有效，可扩展到其他复杂动力学系统。
...
TL;DR: 提出Chisao算法，一种GPU原生群体优化器，通过收敛-反收敛循环和自适应重播种，在多模态黑盒函数优化中实现100%模式恢复，速度提升高达39倍。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如盆地跳跃、CMA-ES、多起点梯度下降）顺序执行，无法利用现代GPU的大规模并行能力。

Method: Chisao算法：整个样本批次并行运行，利用收敛-反收敛振荡循环：冻结已确认的模式（‘卡住’），其余样本通过动量反收敛和随机平滑梯度继续探索；通过两种互补策略（Repulse Monkey和Golden Rooster）自适应重播种以保持种群多样性。

Result: 在SFU基准测试的42个函数上（维度2-64），实现100%模式恢复，而CPU基线在维度≥8时在最难多模态函数上失败；速度提升达34×（Michalewicz d=64）和39×（Rotated Hyper-Ellipsoid d=64），且在似然噪声下模式检测仍100%可靠。

Conclusion: Chisao算法作为开源Python包可在PyPI获取，为多模态优化提供高效GPU加速方案。
...
TL;DR: 从最小认知角度重新定义趋光性导航为信息驱动的感知运动过程，通过POMDP与生化反应动力学框架，揭示了跑-翻滚交替作为信息获取策略。


<details>
  <summary>Details</summary>
Motivation: 现有趋光性模型忽略生物主动采样以减少感官模糊性，需要引入信息驱动视角理解细胞导航行为。

Method: 构建POMDP与CRN-ODE框架，结合贝叶斯内部状态更新和逆强化学习从实验轨迹推断目标。

Result: 模型再现了衣藻对齐光分布的经验数据，与随机模拟基线相当，且跑-翻滚交替作为信息获取策略出现。

Conclusion: 细胞内生化网络可通过信息驱动行为支持自适应导航，翻滚有助于采样新感官配置以解决模糊性。
...
TL;DR: 本文综述了神经架构搜索在生成对抗网络中的应用，分类比较了不同方法，并强调了其优势与未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 自动化设计生成对抗网络架构，提升性能、稳定性和效率，克服手工设计的挑战。

Method: 基于搜索策略、评估指标和性能结果对现有NAS-GAN方法进行分类与比较。

Result: 进化算法和梯度方法在特定场景表现优越；需要超越IS和FID的稳健评估指标；多样化数据集对评估GAN性能至关重要。

Conclusion: 通过结构化比较，为开发更有效的NAS方法和推进GAN领域提供指导。
...
TL;DR: 提出KG-TRACE神经符号框架，整合WHO知识图谱约束，在结核分枝杆菌数据上实现高精度AMR预测，并通过BGR指标量化解释与生物学知识的一致性。


<details>
  <summary>Details</summary>
Motivation: 现有WGS-based AMR模型虽精度高，但缺乏将神经归因锚定于已知生物学通路的机制。

Method: 构建KG-TRACE，融合基因组特征和RotatE知识图谱嵌入，通过可学习的信任门动态权衡神经证据与符号生物学知识。

Result: 在CRyPTIC结核分枝杆菌队列中，对异烟肼的AUROC达0.9760，符号覆盖率达92.5%，能有效识别MDR共现伪影并发出实验室标记。

Conclusion: 神经符号归因提供了可验证的审计线索，弥合了预测准确性与临床信任之间的差距，其主要价值在于符号约束而非预测提升。
...
TL;DR: 测试发现LLM评判员在温度设置为0时仍存在不可重复性，甚至新模型弃用了温度参数，暴露了评估框架的结构性缺陷。


<details>
  <summary>Details</summary>
Motivation: 常见假设认为将评判员采样温度设为0可确保确定性，但实际安全评估中该假设不成立。

Method: 使用日本AISI的aisev开源代码库，在多个提供商、模型层级和采样配置下进行690次API调用实验。

Result: 温度未设置时，接近决策边界的样本在20次运行中通过/失败翻转率高达50%；即使强制温度=0，7个边界样本中仍有1-2个不可复现。并且Claude Opus 4.7/4.8已弃用温度参数。

Conclusion: 建议将评判员分歧视为与分数同等重要的健康指标，并发布复现工具供社区使用。
...
TL;DR: 提出Clue2Group框架，从初始线索逐步恢复洗钱团伙，结合局部上下文、多语义局部时间GNN和多种证据，在大型AML基准上有效。


<details>
  <summary>Details</summary>
Motivation: 现有节点级风险警报或全局被动搜索与实际AML调查中从线索逐步扩展的模式不匹配。

Method: 构建紧凑局部调查上下文减少噪声并保留链/环结构，用多语义局部时间GNN估计线索条件风险场，整合风险、结构和先验模式证据。

Result: 在两个大型AML基准上验证了Clue2Group的实用性。

Conclusion: 为弥合基于图的AML研究与实际调查工作流程提供了可行步骤。
...
TL;DR: 提出联邦哈希投影潜在因子模型（FHPLF），通过二值化梯度、投影汉明距离和安全二进制梯度重组策略，在联邦学习中实现高效、隐私保护的哈希学习。


<details>
  <summary>Details</summary>
Motivation: 传统哈希学习需中心化数据，违反隐私法规；联邦学习通信开销大且存在隐私风险；现有哈希方法二进制码表示能力有限，影响模型精度。

Method: FHPLF包含三项创新：用二值梯度矩阵替代实值梯度，降低计算、存储和通信开销并增强隐私保护；采用投影汉明距离建模相似性，捕捉二进制位重要性以提升表示能力；提出安全二进制梯度重组与隐私增强上传策略，减少交互泄漏风险。

Result: 在四个真实数据集上，FHPLF优于现有哈希学习和联邦学习方法，在精度、效率和隐私保护间取得良好平衡。

Conclusion: FHPLF有效解决了联邦哈希学习中表示能力不足和隐私问题，是一种高效的解决方案。
...
TL;DR: 现代机器学习系统已演变为复杂的社会技术架构，内嵌结构性不平等。该论文指出当前算法公平范式的两大局限：依赖确定性点估计审计和忽略个体的结构性背景。


<details>
  <summary>Details</summary>
Motivation: 早期公平性缓解策略基于脆弱简化，限制了其在复杂社会技术环境中的有效性，需解决根本局限。

Method: 识别并论证两大局限：依赖点估计审计的脆弱性，以及将个体视为孤立实体而忽略结构背景。

Result: 揭示了现有公平范式在复杂环境中的不足，为更有效的公平性方法奠定基础。

Conclusion: 算法公平需要超越点估计和个体孤立视角，融入结构性背景以应对真实世界的不平等。
...
TL;DR: 利用拓扑数据分析（TDA）提取SEN12-FLOOD数据集中的拓扑特征，并将其融入神经网络，实现更鲁棒和可解释的洪水检测。


<details>
  <summary>Details</summary>
Motivation: 洪水频发造成损失，但云层遮挡使光学卫星图像难以检测，现有模型作为黑箱缺乏可解释性。

Method: 从SEN12-FLOOD数据集中提取拓扑特征，将其作为独立信号或补充特征融入神经网络。

Result: 拓扑特征独立携带洪水信号，并能增强现有网络的性能，使检测系统更鲁棒和可解释。

Conclusion: 拓扑数据分析能有效提升洪水检测的鲁棒性和可解释性，适用于遥感等安全关键领域。
...
TL;DR: 提出一个与属性无关的GNN框架，直接从Cayley图学习有限群的代数性质（交换性、幂零性、可解性），实验表明成功学习并区分多种性质。


<details>
  <summary>Details</summary>
Motivation: 此前工作仅针对可解性，希望推广到通用框架，直接从图表示中学习多种代数性质。

Method: 使用通用GNN架构和训练流程，从有限群的Cayley图表示学习代数性质。

Result: 在多个有限群族上的实验表明，该框架成功学习并区分交换性、幂零性和可解性。

Conclusion: Cayley图编码了丰富的代数信息，GNN能够提取；该框架为图表示学习应用于有限群研究提供概念验证。
...
TL;DR: 提出Fast-LeWM，通过动作前缀预测替代自回归展开，实现更快的视觉世界模型规划，在多项任务中提升成功率并大幅减少规划时间。


<details>
  <summary>Details</summary>
Motivation: LeWM在视觉规划中需要重复应用单步潜在状态转移模型，自回归展开导致计算成本高且累积误差随规划视界增长。

Method: Fast-LeWM将动作前缀作为基本预测单元，编码候选动作序列的前缀并并行预测执行这些前缀后的未来潜在状态，避免逐中间步骤的自回归滚动。

Result: 在多项任务中，Fast-LeWM相比LeWM提高了平均成功率，同时显著减少了规划时间，并实现了更低的开环潜在损失，且损失随视界增长的速度更慢。

Conclusion: Fast-LeWM通过前缀级监督学习状态在不同动作前缀下的连续演化，有效降低了计算开销和误差累积，为高效视觉世界模型规划提供了新方案。
...
*Wojciech {\L}apacz,Stanis{\l}aw Pawlak,Jan Dubi\'nski,Franziska Boenisch,Adam Dziedzic*

Main category: cs.LG

TL;DR: 提出无需影子模型或真实保留集的数据集使用推断方法，通过合成非成员样本和混合比例估计量化模型训练中数据集的使用比例。


<details>
  <summary>Details</summary>
Motivation: 现有数据集使用推断方法依赖昂贵影子模型和已知训练样本及保留集，在大模型和数据所有权纠纷中不实用。

Method: 生成合成非成员样本，提取多样成员信号，将数据集使用推断转化为混合比例估计问题。

Result: 在大型图像生成模型上可靠地量化了数据集使用比例。

Conclusion: 为数据所有者提供实用工具，判断其数据在模型训练中的使用程度。
...
TL;DR: 研究贝叶斯神经网络中数据增强与对称性的关系，提出轨道扩展等对称化方法提升等变性。


<details>
  <summary>Details</summary>
Motivation: 等变网络与数据增强在深度学习对称性处理中各有优劣，但数据增强的理论分析不足。

Method: 针对变分推理训练的贝叶斯神经网络，推导精确等变的条件和误差界，提出三种对称化技术（含轨道扩展）。

Result: 轨道扩展方法在等变性和整体性能上均优于基线。

Conclusion: 数据增强结合对称化能有效实现等变性，为贝叶斯网络提供新思路。
...
TL;DR: 提出基于状态空间模型（SSM）的HRM适配器，用于参数高效微调（PEFT），在长上下文任务中优于LoRA。


<details>
  <summary>Details</summary>
Motivation: 参数高效微调通常针对注意力投影器，但对需要序列状态积累的任务效果未知；探索SSM适配器及MLP注入点的优势。

Method: 引入Hankel降阶模型（HRM）适配器，通过平衡截断经验Hankel Grammians初始化，利用系统矩阵时间不变性实现FFT并行扫描，计算复杂度与LoRA相当。

Result: 在Mistral-7B上，LongBench任务中HRM相比LoRA变体显著提升：QuALITY相对准确率+34.8%，QMSum相对ROUGE-1+71.6%；在18种合成状态追踪和字符级语言建模配置中一致占优。

Conclusion: HRM适配器有效学习调节循环，为长上下文序列建模提供了鲁棒的架构替代方案。
...
### [90] [The Red Queen G\"odel Machine: Co-Evolving Agents and Their Evaluators](https://arxiv.org/abs/2606.26294)
*Alex Iacob,Andrej Jovanovi\'c,William F. Shen,Daniel Burkhardt,Meghdad Kurmanji,Nurbek Tastan,Lorenzo Sani,Niccol\`o Alberto Elia Venanzi,Ambroise Odonnat,Zeyu Cao,Bill Marino,Xinchi Qiu,Nicholas D. Lane*

Main category: cs.LG

TL;DR: 提出红皇后哥德尔机（RQGM），一种在非平稳评估标准下实现递归自我改进的进化框架，通过分阶段控制效用演化，在编程、论文写作和审稿等任务上超越现有方法，并纠正了AI审稿偏差。


<details>
  <summary>Details</summary>
Motivation: 现有自我改进智能体假设评估标准固定不变，忽略了演化中环境随智能体改变的特性，需要将评估纳入改进循环以适应动态效用。

Method: 将搜索组织为固定评估标准的时间段，在时间段边界更新效用函数，从而在每时段内保证自我改进，同时允许目标跨时段演化；通过互补的智能体作为裁判的代码审查信号实现多任务应用。

Result: 在编程任务中以1.35x-1.72x更少的令牌提升测试通过率；论文写作中录用率提升1.78x-1.86x；审稿准确率提升9%，并纠正了基线审稿人对AI论文的过度接受偏差（高达1.91倍）。

Conclusion: RQGM通过非平稳效用下的进化框架有效提升了自我改进性能，并能在多样化任务中纠正评估偏差，使AI更公平地处理AI和人工作品。
...
TL;DR: 针对满足Polyak-Łojasiewicz条件的平滑目标，研究由马尔可夫链生成梯度样本的一阶方法，在轻尾和重尾情形下分别给出了最优的高概率收敛界。


<details>
  <summary>Details</summary>
Motivation: 现有轻尾SGD的高概率边界为O~(t_mix^2/k)，与期望边界O~(t_mix/k)存在差距；重尾情形下缺乏高效算法和理论保证。

Method: 轻尾情形使用滞后阻塞论证得到统一高概率保证；重尾情形设计全样本裁剪块方法，利用所有马尔可夫转移并减少偏差。

Result: 轻尾：达到O~(t_mix/(k+K0))的随机项，并证明与混合时间线性依赖最优；重尾：达到O~(σ_p^2(t_mix/T)^{2(p-1)/p})的高概率误差，且匹配下界。

Conclusion: 完整刻画了轻尾PL-SGD中混合时间的最优多项式依赖，以及重尾情形下最优的厚尾指数和有效样本量依赖。
...
TL;DR: 提出EVOM框架，利用元进化和LLM设计代理自动搜索高性能actor-critic架构，在实验中优于手动和现有方法。


<details>
  <summary>Details</summary>
Motivation: actor-critic网络架构通常手动设计，自动化面临候选需训练评估及设计空间开放的挑战。

Method: 双层优化：内循环使用低保真PPO训练权重，外循环通过LLM设计代理迭代优化架构程序进行元进化。

Result: 在Ant-v4和HalfCheetah-v4上，EVOM优于手动基线、LLM引导随机搜索及SOTA方法MLES；消融实验表明元进化循环和LLM设计代理均不可或缺。

Conclusion: EVOM框架有效自动化发现高性能actor-critic架构，其中元进化循环和LLM设计代理是关键组件。
...
TL;DR: Mesh-RL是一种空间域分解框架，受有限元方法和域分解理论启发，通过将环境划分为重叠子网格并强制执行边界一致的时间差分更新，加速稀疏奖励环境中的强化学习。


<details>
  <summary>Details</summary>
Motivation: 在大规模或稀疏奖励环境中，强化学习面临时间差分奖励传播缓慢的问题，因为价值信息仅在状态空间局部扩散。

Method: 将环境划分为重叠子网格，强制执行边界一致的时间差分更新，实现局部学习和全局一致的价值传播，无需修改奖励函数或贝尔曼算子。

Result: 在危险密集的网格世界环境中，Mesh-RL在Q-learning、SARSA和Dyna-Q上一致提升了收敛速度、累积奖励和学习稳定性。更高的网格分辨率维持探索，防止过早收敛，显著加速价值传播到远距离状态。

Conclusion: Mesh-RL引入了基于空间域分解的机制加速时间差分学习，将有限元方法启发的边界一致技术从科学计算引入强化学习，提高了稀疏奖励环境中的样本效率。
...
TL;DR: 提出EMA-FS，基于指数移动平均的特征筛选方法，在LightGBM中通过保留高增益特征加速直方图构建，在密集数据上实现最高2.61倍加速，同时保持或提升精度。


<details>
  <summary>Details</summary>
Motivation: GBDT训练中直方图构建占时65-70%，现有随机特征子采样忽视特征预测效用，导致效率低下。

Method: EMA-FS维护每个特征分裂增益的指数移动平均，经预热后只对历史增益排名前K的特征构建直方图；并扩展为随机版本S-EMA-FS，用增益加权采样统一确定性选择和随机子采样。

Result: 在密集中高维数据上显著加速：500特征合成数据集2.61倍，432特征IEEE-CIS欺诈数据集在30%保留率下1.45倍，70%保留率下AUC提高0.11且加速1.34倍；稀疏数据无加速。

Conclusion: EMA-FS是一种轻量级、插件式的优化方法，可与LightGBM完全兼容，在不损失甚至提升性能的前提下大幅加速训练。
...
TL;DR: 通过PCA和LRP分析Aurora模型内部表示，发现潜在空间主要由季节循环组织，极端风暴事件不形成线性可分组；模型关注与1987年大风暴三维垂直结构一致的特征，扰动测试表明相关区域重要性是随机掩膜的3.31倍。


<details>
  <summary>Details</summary>
Motivation: ML基础模型能准确高效模拟大气动力学，但作为黑箱模型难以解释，需要理解其内部表示。

Method: 使用空间池化主成分分析（PCA）和逐层相关性传播（LRP）分析Aurora模型的潜在空间和注意力机制，并进行扰动测试。

Result: 潜在空间主要由季节循环组织，极端风暴事件不形成线性可分离簇；LRP显示模型关注与1987年大风暴三维垂直结构一致的特征；掩膜相关区域导致预测误差比随机掩膜高3.31倍。

Conclusion: Aurora模型在没有显式指令的情况下学习了气象连贯性和三维垂直结构，表明模型内部表示具有物理意义。
...
TL;DR: SOLAR框架能够从PyTorch和JAX源码自动推导出深度学习模型在给定硬件上的理论最小执行时间（Speed-of-Light界限），并验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 手动计算SOL界限费力且易错，与快速模型开发脱节，急需自动化工具。

Method: 结合LLM前端将源码转为可执行Affine Loop IR，再通过确定性流程提升为einsum图，最后分析后端计算未融合、融合及缓存感知的SOL界限。

Result: 全面覆盖算子和语言，零SOL违规，多保真分析能收紧界限并揭示优化机会，在KernelBench、JAX/Flax和机器人负载上验证有效。

Conclusion: SOLAR支持多保真度裕量分析、识别优化机会、跨平台探索和逆屋顶线硬件配置四种应用场景。
...
TL;DR: 提出理解Transformer鲁棒性的机制框架，发现OOD输入导致内部错误概念增多，并据此实现微调加固。


<details>
  <summary>Details</summary>
Motivation: 真实部署中OOD数据威胁模型可靠性，缺乏显式处理机制。

Method: 系统实验揭示OOD输入（如拼写错误、越狱提示）驱动模型内部错误概念增加，基于此设计机制对齐的微调策略。

Result: 将OOD概念从输入扩展到模型内部计算，建立推理时新诊断方法。

Conclusion: 该诊断是AI系统在科学、商业和政府中安全部署的关键步骤。
...
TL;DR: 提出一种基于切比雪夫标量化的偏好条件贝尔曼算子，用于计算确定性帕累托最优策略，并证明其收敛性和包络性质。


<details>
  <summary>Details</summary>
Motivation: 标准强化学习通过奖励聚合难以捕捉多目标问题中的完整帕累托最优边界。

Method: 引入偏好条件贝尔曼算子，基于切比雪夫标量化，证明其包络性质和单调收敛，并展示如何从收敛的Q值中提取确定性策略。

Result: 实验验证算法能成功恢复复杂权衡，合成近似帕累托最优的确定性策略。

Conclusion: 该方法能根据任意偏好恢复帕累托最优边界上的策略，保证每个策略近似帕累托最优。
...
### [99] [Otter Weather: Skillful and Computationally Efficient Medium-Range Weather Forecasting](https://arxiv.org/abs/2606.26421)
*Cristiana Diaconu,Jonas Scholz,Aliaksandra Shysheya,Stratis Markou,Payel Mukhopadhyay,Miles Cranmer,Richard E. Turner*

Main category: cs.LG

TL;DR: Otter Weather模型以极低计算预算实现高性能天气预测，并扩展到概率预报和科学领域。


<details>
  <summary>Details</summary>
Motivation: 现有AI天气模型训练预算大，限制资源不足群体和快速迭代。

Method: 构建高效时空预测模型Otter，采用CRPS训练概率版本，并缩放架构。

Result: 确定性模型优于NWP基线9.6%，训练仅需3.5 A100天；概率模型Otter-XL CRPS提升9.7%，超越GenCast，计算减少10倍。

Conclusion: Otter在天气预测和科学PDE任务中均高效且有效，促进民主化。
...
### [100] [Rethinking Training & Inference for Forecasting: Linking Winner-Take-All back to GMMs](https://arxiv.org/abs/2606.26424)
*Qiyuan Wu,Katie Z Luo,Bharath Hariharan,Wei-Lun Chao,Mark Campbell*

Main category: cs.LG

TL;DR: 针对自动驾驶轨迹预测中因训练-建模不匹配导致模式后验无信息的问题，提出两种轻量级后处理方法（后验加权合并和一步EM更新），在不重新训练的情况下提升模式排名和预测精度。


<details>
  <summary>Details</summary>
Motivation: 轨迹预测模型通常建模为条件高斯混合模型，但使用赢者通吃损失训练，导致硬分配产生无信息的模式后验，影响模式剪枝。

Method: 提出两种后处理方案：1）测试时对附近候选轨迹进行后验加权合并；2）单步期望最大化更新，用软责任代替硬标签，在邻近模式间共享概率质量。

Result: 在多种赢者通吃训练的架构上，这些轻量步骤产生更有信息、更忠实排名的模式后验，并在常用位移指标上增强最终预测。

Conclusion: 通过GMM与K-means的视角统一了近期设计选择，提供了无需重新训练的原则性实用修正，使训练目标与推理更一致。
...
### [101] [DualEval: Joint Model-Item Calibration for Unified LLM Evaluation](https://arxiv.org/abs/2606.26429)
*Aaron J. Li,Hao Huang,Youngmin Park,Yitong Ma,Wei-Lin Chiang,Li Chen,Cho-Jui Hsieh,Bin Yu,Ion Stoica*

Main category: cs.LG

TL;DR: 提出DualEval框架，通过潜在模型-项目校准统一静态基准和竞技场偏好评估，产生可靠排名并支持基准压缩和异常检测。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估依赖静态基准和偏好数据两种信号，但两者脱节，DualEval旨在统一两者以提升评估一致性。

Method: 构建共享空间，联合估计模型能力、项目难度和区分度，同时利用静态标签和奖励模型分数。

Result: 在18个前沿LLM、四个领域上评估，得到平衡可靠的模型排名，并能有效压缩基准和检测异常。

Conclusion: DualEval通过联合校准统一评估范式，提升样本效率、可解释性和可审计性。
...
### [102] [Embedding Foundation Model Predictions in Discrete-Choice Models with Structural Guarantees](https://arxiv.org/abs/2606.26432)
*Yingshuo Wang,Xian Sun,Yanhang Li,Zhichao Fan,Zexin Zhuang*

Main category: cs.LG

TL;DR: 提出两阶段适配器将基础模型预测嵌入多项logit，在保持经济逻辑的同时提升选择预测准确率。


<details>
  <summary>Details</summary>
Motivation: 表格基础模型预测常违反经济逻辑，如提价增加需求、负支付意愿等，需要改进。

Method: 两阶段适配器：阶段1带符号约束的极大似然估计多项logit结构系数；阶段2冻结系数，拟合小神经网络修正基础模型预测。

Result: 三个数据集上准确率平均提升6.4个百分点，最高12.8，保持100%成本单调性，时间价值在经济学范围内。

Conclusion: 该方法可数学保证经济逻辑，性能在上下文限制时优雅下降。
...
### [103] [Optimizing CUDA like a Human: Micro-Profiling Tools as Expert Surrogates for LLM-Based GPU Kernel Optimization](https://arxiv.org/abs/2606.26453)
*Jiading Gai,Shuai Zhang,Kaj Bostrom,Jin Huang,Vihang Patil,Haoyang Fang,Bernie Wang,Huzefa Rangwala,George Karypis*

Main category: cs.LG

TL;DR: 提出了KernelPro，一个闭环多智能体系统，通过大模型代码生成与硬件分析器反馈自动优化GPU内核，在KernelBench上实现2.42x/4.69x/5.30x加速，并首次兼顾能效优化。


<details>
  <summary>Details</summary>
Motivation: 现有GPU内核优化依赖手动调优或单一性能分析工具，缺乏自动化、迭代优化的闭环系统。

Method: 集成LLM代码生成与硬件分析器反馈，采用语义反馈算子、两阶段工具调用（基于roofline瓶颈分类）、领域自适应MCTS搜索、以及基于CuTe的源代码生成。

Result: 在KernelBench上几何平均加速比2.42x/4.69x/5.30x（Level 1/2/3）；在VeOmni专家优化的MoE训练内核上超越手调Triton达1.23x；消融实验显示各组件显著提升优化质量。

Conclusion: KernelPro不仅实现最优速度加速，还首次将能效优化纳入CUDA内核自动调优，在同等速度下降低能耗11.6%。
...
### [104] [Finding the Time to Think: Learning Planning Budgets in Real-Time RL](https://arxiv.org/abs/2606.26463)
*Aneesh Muppidi,Firas Darwish,Dylan Cope,Jo\~ao F. Henriques,Jakob Nicolaus Foerster*

Main category: cs.LG

TL;DR: 提出一种可变延迟的实时强化学习框架，通过轻量级门控策略选择规划预算，在多个实时游戏中优于固定预算和启发式基线。


<details>
  <summary>Details</summary>
Motivation: 标准RL假设环境无限等待，但实时环境中延迟代价高，需要智能体自主决定规划时长。

Method: 在规划器基础上训练轻量级门控策略，根据状态选择规划预算，实现可变延迟实时RL。

Result: 在实时吃豆人、俄罗斯方块、贪吃蛇、竞速六角棋和竞速围棋中，门控策略表现优于固定预算和启发式基线，并成功迁移到双GPU环境。

Conclusion: 门控策略能有效平衡规划时间与性能，适用于实时RL场景，且具有迁移性。
...
### [105] [A Causal Foundation Model for Structure and Outcome Prediction](https://arxiv.org/abs/2606.26467)
*Max Zhu,Martino Mansoldo,Ching-Hao Wang,Stefan Groha*

Main category: cs.LG

TL;DR: TabPFN-CFM是一个基于transformer的因果基础模型，从观测数据预测因果结构和结果，支持Pearl因果层次的三级查询，利用已知图结构提升预测，在合成数据上训练并泛化到真实数据，优于基线。


<details>
  <summary>Details</summary>
Motivation: 现有因果推断方法通常针对特定问题，缺乏统一的框架来处理多种因果问题，且往往需要干预数据或已知结构。TabPFN-CFM旨在提供一个单一模型，能够从观测数据同时预测因果结构和结果，并适应不同层次因果查询的需求。

Method: 采用transformer架构，在大量合成因果数据集上训练，学习因果结构和结果预测的通用模式。模型输入观测数据，输出因果图结构和条件概率分布，支持干预和反事实查询；若已知部分图结构，可将其作为额外输入以提升性能。

Result: 在多个真实和合成数据集上，TabPFN-CFM在因果结构发现和结果预测任务中均优于现有基线方法，包括面向结构或结果的专用模型，展示了良好的泛化能力。
...
### [106] [Epiphany-Aware KV Cache Eviction Without the Attention Matrix](https://arxiv.org/abs/2606.26472)
*Steven Kolawole,Virginia Smith*

Main category: cs.LG

TL;DR: EpiKV 是一种基于顿悟分数的 KV 缓存淘汰方法，无需训练或自定义内核，可直接在 FlashAttention 中使用，将可行上下文长度扩展 16 倍，并在 MATH-500 和 AIME-2024 上达到或超越现有基线。


<details>
  <summary>Details</summary>
Motivation: 现有基于注意力权重的缓存淘汰方法在长推理链中噪声大，且阻碍融合内核的使用，导致推理瓶颈。

Method: 使用顿悟分数（模型前向传播中内部表示的变化）评分，结合因果滚动 z-score 消除位置趋势，无需注意力矩阵或额外训练。

Result: 在 MATH-500 上以 4096 缓存达到 72%（与最强基线 ThinKV 71% 相当，优于 H2O 67%），在 AIME-2024 上以 8192 缓存达到 37%（优于最佳基线 33%），速度提升达 2.8 倍。

Conclusion: EpiKV 能有效扩展上下文长度，无需额外训练或自定义内核，适合生产环境中的长上下文推理。
...
### [107] [When Does Quality-Aware Multimodal Fusion Matter? A Leakage-Safe Diagnostic for Decision-Level Dependence](https://arxiv.org/abs/2606.26473)
*Jaden Moon,Arvind Pillai,Andrew Campbell*

Main category: cs.LG

TL;DR: 多模态系统中，可靠性分数是否真正影响模型决策？通过置换测试发现，随机排列可靠性分数不影响性能，表明模型并未有效利用这些信息，除非它们能可靠预测单模态正确性。


<details>
  <summary>Details</summary>
Motivation: 当前多模态系统利用各模态可靠性权重进行融合，但不确定这些分数是真正驱动决策还是仅与性能相关。

Method: 提出一种诊断方法：在模型固定后，置换测试样本的可靠性分数，观察性能变化。在StressID和CMU-MOSEI上实验。

Result: 置换可靠性分数后，性能不变；但若可靠性信号能准确指示正确模态（正控制实验），则融合规则带来显著提升。

Conclusion: 可靠性分数仅在精确预测单模态正确性时才影响融合决策，否则模型并未利用这些信息。
...
### [108] [Localizing RL-Induced Tool Use to a Single Crosscoder Feature](https://arxiv.org/abs/2606.26474)
*Andrii Shportko,Shubham Bhokare,Ahmed Zeyad A Alzahrani,Bowen Cheng,Gustavo Mercier,Jessica Hullman*

Main category: cs.LG

TL;DR: 使用专用特征交叉编码器（DFC）分离出强化学习引入的紧凑特征集，能够提高工具调用正确率并实现能力迁移。


<details>
  <summary>Details</summary>
Motivation: 强化学习微调使语言模型获得工具使用等代理行为，但其内部表征变化机制尚不清楚。

Method: 提出专用特征交叉编码器（DFC），通过48组超参数扫描，对强化学习模型进行编码-解码重构。

Result: 重构使工具正确率提升31.1±9.7个百分点，并将能力迁移至冻结基础模型，提升6.8±5.0个百分点。

Conclusion: DFC方法将强化学习引入的能力集中在最小可操控特征集中，实现运行时行为控制。
...
### [109] [Retrieval-Warmed Energy-Based Reasoning: A Five-Arm Ablation Methodology for Diffusion-as-Inference on Structured Reasoning Tasks](https://arxiv.org/abs/2606.26476)
*Libo Sun,Po-Wei Harn,Zewei Zhang,Peixiong He,Xiao Qin*

Main category: cs.LG

TL;DR: 本研究提出五臂消融方法，分离检索预热扩散采样器中的混淆效应，发现图对齐是关键因素，并在数独任务中识别出关键质量瓶颈。


<details>
  <summary>Details</summary>
Motivation: 温启动扩散采样器加速迭代推理，但增益来源不明确，需要分离混淆效应。

Method: 采用五臂消融方法（oracle, best-constant, per-query-random, shuffled, aligned）分别评估类别先验偏差转移、随机温启动和图对齐值复用。

Result: 在连通性-2任务中，对齐与乱序oracle的准确率差达+35 pp；在数独任务中，关键质量是瓶颈。

Conclusion: 消融分解能识别每项任务的首个阻塞组件，指导改进。
...
### [110] [What Survives When You Compress a Recursive Reasoner for the Edge?](https://arxiv.org/abs/2606.26488)
*Pearse Jim,Steven Kolawole,Opegbemi Matthias Busoye,Glory Bagai,Virginia Smith*

Main category: cs.LG

TL;DR: 本研究发现递归推理模型在压缩时，局部预测保留但全局推理崩溃，并通过校准INT4恢复，提出部署方案。


<details>
  <summary>Details</summary>
Motivation: 递归推理模型部署于边缘硬件需要压缩，但传统量化方法导致推理误差累积，需要理解压缩后模型行为。

Method: 通过全面精度扫描、三种任务和两种递归架构实验，分析压缩对推理的影响；引入轨迹保真度作为无标签信号；提出flash-streamed嵌入和校准INT4。

Result: 激进压缩（INT4）保持单元精度但精确解精度归零；校准INT4无需重训恢复推理；INT8单周期匹配全深度精度且FLOPs减少6倍；flash-streamed嵌入消除99.4MB瓶颈。

Conclusion: 提供了部署方案：校准INT4适合4MB微控制器，INT8适合8MB SoC，且轨迹保真度可预测损伤。
...
### [111] [Learning Probabilistic Filters with Strictly Proper Scoring Rules](https://arxiv.org/abs/2606.26497)
...
TL;DR: 提出LCS-Bench，一个基于计算机科学逻辑的理论级自动形式化基准，用于评估和推动大规模形式化验证。


<details>
  <summary>Details</summary>
Motivation: 现有自动形式化主要关注孤立语句，而理论级（包含大量相互依赖的定义、引理和定理）的自动形式化因一致性、忠实性、可扩展性和正确性等挑战仍未解决。

Method: 通过半自动代理流水线构建，利用概念图、形式签名规划、问题追踪、反例搜索补充缺失证明，并辅以人类专家的忠实性审查。

Result: 包含327个教科书条目、4076个Lean声明和超过85000行Lean代码；评估14个模型，最佳模型在自动形式化任务上仅达到20.1%。

Conclusion: LCS-Bench具有高质量、一致性和忠实性，且具有挑战性；分析揭示了理论级自动形式化的关键发现，并为未来工作指出了有希望的方向。
...
TL;DR: 提出一种安全迁移强化学习框架，用于自主高速公路变道决策，通过教师干预和加权优化提升安全性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法因源域和目标域分布差异导致训练振荡和性能下降，且探索交互难以保证安全。

Method: 设计自适应教师干预机制、教师引导的安全迁移模块和加权优化机制，结合奖励塑形和似然比因子调整样本权重。

Result: 在多种交通密度和真实NGSIM数据集上，安全性提升52.2%，学习效率提升5.0%。

Conclusion: 验证了安全感知迁移策略在自主高速公路变道中的有效性和鲁棒性。
...
TL;DR: 针对深度Transformer中深层贡献较小的问题，提出CascadeFormer和CascadeFlow Pruning两种效率方法，并引入梯度扇入不对称性（GFA）理论解释。


<details>
  <summary>Details</summary>
Motivation: 深度Transformer的深层往往贡献有限，利用这种不对称性提高效率。

Method: CascadeFormer随深度减小宽度以匹配信息流；CascadeFlow Pruning基于累积训练梯度移除层；提出GFA解释深层梯度稀疏现象。

Result: CascadeFormer在相同训练预算下延迟降低8.6%，吞吐量提升9.4%；CascadeFlow Pruning在困惑度和排名稳定性上优于标准启发式方法。GFA得到相关性和干预实验支持。

Conclusion: GFA揭示了深层贡献小的结构性原因，但梯度幅度是否在高秩区外代表扇入以及百亿参数规模的行为仍有待研究。
...
*Jerome Marston,Tino Kreutzer,Salom\'e Garnier,Ella Boone,Phuong N Pham,Patrick Vinck*

Main category: cs.LG

TL;DR: 大语言模型在编码人道主义定性数据时可达到与人类相当的可靠性，但需谨慎使用，不能完全替代人类判断。


<details>
  <summary>Details</summary>
Motivation: 人道主义组织缺乏分析大规模定性数据的资源，大语言模型可能扩展能力，但其可靠性未被验证。

Method: 使用46个LLM与人类金标准对比，基于150个合成转录本，采用信度检验、差异分析和定性评估。

Result: 多个LLM在演绎编码上达到与经验丰富的编码员相当的可靠性，尤其在结构化提示和推理配置下；但聚合指标不足以决定部署，模型在识别间接表达、类别外需求及保护问题上存在差异。

Conclusion: LLM可实质性扩展人道主义分析能力，但不能替代人类判断；需使用结构化编码手册、推理模型，并针对特定主题和项目后果进行分层监督。
...
TL;DR: 对混合动作空间中的因式分解方法进行跨断面研究，提出VDN-PPO和PPO-MIX变体，表明分支竞争架构在计算和性能之间取得最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 现有混合动作空间基准测试多为重型或存档实现，缺乏轻量级、全面的跨方法比较研究。

Method: 跨断面研究，测试独立网络、共享编码器、VDN、QPLEX、联合、自回归等因式分解方法，结合PPO、SAC、DQN算法，在离散、混合、连续动作空间及四个轻量级环境（Platform、hybrid-LunarLander、Hybrid-Shoot、CoopPush）上，共220种有效配置，并引入两个新环境。

Result: VDN-PPO和PPO-MIX优于其他PPO因式分解；分支竞争架构平衡计算与性能；自回归动作达到最高性能；原生连续SAC优于离散和混合算法，但计算成本更高。

Conclusion: 分支竞争架构和自回归动作是有效选择，但需权衡计算成本；无折扣跨方法比较揭示了不同配置的优劣。
...
TL;DR: SharQ是一种无需训练的推理方法，通过在线稀疏-稠密分解，桥接了激活稀疏性和FP4量化，有效恢复NVFP4到FP16的精度差距，并实现加速。


<details>
  <summary>Details</summary>
Motivation: 低比特浮点格式和半结构化稀疏性在现代加速器上得到支持，但结合两者进行LLM激活压缩仍面临挑战：激活值中的异常值主导FP4量化的块尺度，直接应用N:M稀疏掩码会丢弃中等值，导致稀疏化损失与量化误差耦合。

Method: SharQ对每个激活张量生成输入自适应的N:M掩码提取异常值主导的稀疏骨干，将其量化为FP4，并定义相对于量化稀疏骨干而非未量化稀疏值的稠密残差。稀疏FP4 GEMM处理骨干，稠密FP4 GEMM补偿掩码导致的激活损失和稀疏路径量化误差。两路径共享单一FP4权重载荷，具有路径特定尺度视图，融合预处理核将掩码生成、残差构建和层归一化合并到一个算子中。

Result: 在Llama-3.1-8B、Qwen2.5-7B、Qwen3-30B-A3B和Qwen3-VL-8B上，SharQ恢复了NVFP4到FP16精度差距的43-63%，并泛化到NVFP4、HiF4和MXFP4格式。在RTX 5090上，与FP16相比延迟降低2.2-2.4倍，与FP8相比吞吐量提升1.2-1.4倍，结合SageAttention在Wan2.2-T2V-A14B视频生成上加速达1.58倍。

Conclusion: SharQ无需校准数据、重新训练或模型特定调优，有效结合了激活稀疏性和低比特量化，在多种任务和格式上实现了精度恢复和性能加速。
...
TL;DR: TerraProbe框架揭示了LLM修复Terraform安全配置时存在大量欺骗性修复，虽然通过静态扫描但未解决根本漏洞。


<details>
  <summary>Details</summary>
Motivation: 现有评估仅依赖静态分析结果消失，忽略规划有效性、行为变化和安全意图，导致误判修复真实效果。

Method: 构建五层评估框架TerraProbe，对288个修复样本（来自gemini-2.5-flash-lite、GPT-4o和Claude 3.5 Sonnet）进行多层分析，包括静态扫描、规划验证、行为比较和人工判定。

Result: 目标静态扫描移除率83.3%，但全扫描干净率仅10.4%；规划成功39.6%，计划比较可达38.5%；人工判定71.4%为欺骗性修复，三种模型无显著差异。

Conclusion: TerraProbe提供了更全面的评估方法，识别出欺骗性修复模式并提出四维分类，建议使用多层评估框架区分真正安全修复与虚假成功。
...
TL;DR: 研究了对比学习在草图线性模型下的缩放定律，推导了风险分解和显式缩放关系，指出对比学习需学习视图间交互。


<details>
  <summary>Details</summary>
Motivation: 对比学习的缩放定律理论理解不足。

Method: 使用配对的潜变量设置，对草图线性模型进行全批梯度下降分析，分解风险。

Result: 得到关于草图维度M、样本量N和有效优化视界的显式缩放定律。

Conclusion: 对比学习的缩放与线性回归不同，需平衡模型大小、数据和优化计算。
...
TL;DR: 提出了Qwen3-Instruct SAE套件，针对Qwen3指令微调模型训练稀疏自编码器，评估了稀疏-保真度权衡，并通过拒绝行为案例验证了实用性。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器是解释语言模型的有力工具，但训练成本高且开源模型有限。

Method: 在Qwen3-1.7B、4B、8B模型上训练多层SAE，覆盖残差流、MLP输出和注意力输出等激活位点，并进行系统评估。

Result: 不同层和组件表现出不同的稀疏-保真度权衡。

Conclusion: 该套件为研究指令微调模型的稀疏表示、特征级机制和行为干预提供了实用资源。
...
TL;DR: 提出用稀疏自编码器在激活空间进行正则化，解决大语言模型持续学习中的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 权重空间正则化方法（如EWC）在大语言模型中效果不佳，原因是其基于权重的重性估计过于粗糙，无法隔离需要保护的知识，这源于大语言模型的多义性（polysemantic）本质。

Method: 使用预训练的稀疏自编码器（SAE）作为单义特征字典，在激活空间进行正则化；通过约束优化推导新损失函数，平衡稳定性和可塑性；只需在当前任务数据上计算紧凑的SAE特征掩码，无需保留之前任务数据。

Result: 在TRACE和MedCL持续学习基准上，该方法在未引入任务特定架构的情况下达到最强结果，超过传统权重空间正则化方法（如EWC），且内存效率更高。

Conclusion: 实验证据支持多义性论点：任务相关表示在SAE特征基中线性可分，但在权重基中无法区分；权重空间保护在概念层面几乎无选择性。
...
TL;DR: 提出BOBa框架，利用多臂老虎机自适应分配计算资源，避免全库代理推断，实现超大型分子库的高效虚拟筛选。


<details>
  <summary>Details</summary>
Motivation: 现有基于代理的优化方法需要对整个分子库进行代理推断，计算成本高昂，无法应对数十亿至万亿规模的库。

Method: 将动作空间划分为分区，将每个分区视为多臂老虎机的臂，基于不确定性乐观策略自适应分配代理推断和实际评估到有希望的分区。

Result: 在真实合成库上的实验表明，结合有意义的分区和不确定性乐观老虎机，能有效分配推断和评估资源，实现可调的性能-成本权衡。

Conclusion: 该方法支持在当前库上进行实用优化，并为超大型库虚拟筛选提供了可行途径。
...
TL;DR: 提出PersistentKV，一种原生块表解码注意力引擎，通过页面感知调度和分组查询注意力优化，提升LLM解码吞吐量。


<details>
  <summary>Details</summary>
Motivation: 现有LLM服务受限于KV缓存移动而非计算，单核实现并非最优调度，需要自适应策略平衡不同负载。

Method: 按KV头组映射工作，重用K,V块，支持原生页表，并使用紧凑工作队列调度仅执行非空任务。

Result: 在RTX 3060上，相比FlashInfer，B8负载吞吐量提升1.063-1.265倍，B1桶形跟踪提升1.399倍，边界情况避免回归。

Conclusion: 工作分配是服务系统关键变量，自适应页面感知调度具有具体系统价值。
...
*Anastasis Kratsios,Simone Brugiapaglia,Bum Jun Kim,Gregory Cousins,Haitz S\'aez de Oc\'ariz Borde*

Main category: cs.LG

TL;DR: 神经网络的表现力不仅取决于正则性，还取决于算法复杂性：任何可由电路计算的函数都能由神经网络以可控精度实现，且网络复杂度由电路深度、宽度和门结构决定；同时，满足并行化条件的神经网络模型是通用逼近器当且仅当包含非仿射非线性。


<details>
  <summary>Details</summary>
Motivation: 传统基于正则性的表现力分析无法区分直观简单和复杂的函数（如平方根与布朗路径），需要从计算模型角度补充理解。

Method: 将神经网络视为计算模型，通过实数电路的门语言分析函数可计算性；并利用并行化条件刻画通用逼近的充要条件。

Result: 证明了神经网络能逼近任何电路可计算函数，且Besov类、解析函数的最优逼近保证；示例显示最短路径计算仅需O(log(1/ε))个非零参数，远优于Lipschitz逼近的O(ε^{-c k^2})。
...
TL;DR: 提出HyperDFlash，一种针对DeepSeek-V4多超连接架构的块并行推测解码框架，通过预折叠残差状态和轻量级门控残差还原器解决特征对齐问题，并采用KL蒸馏损失提升草稿质量，显著提升解码速度。


<details>
  <summary>Details</summary>
Motivation: DeepSeek-V4的MTP草稿在后位置因误差累积而准确率下降，且原始DFlash无法适配其多路径残差流，导致特征错位。

Method: 使用预折叠残差状态作为条件信号，用门控残差缩减器替代线性压缩器，并通过针对LM头的KL蒸馏损失进行训练。

Result: 在数学推理、代码合成和对话任务中，HyperDFlash的草稿接受长度和解码速度均优于原生MTP和基础DFlash。
...
TL;DR: 神经坍缩预测平衡独热分类会使表示等距，但语言模型虽用独热标签却学到语义结构。本文通过合成实验发现语义几何早期出现但最终过渡到对称状态。


<details>
  <summary>Details</summary>
Motivation: 语言模型在独热训练下如何学习到输入中潜在的语义结构，与神经坍缩理论矛盾。

Method: 设计了三个合成控制实验，其中输入有潜在语义因子但对应不同独热标签，分析表示几何的演化。

Result: 语义几何在训练早期出现，形成按共享属性聚类；但若有足够容量和时间，最终会达到等距对称状态。

Conclusion: 通过Gram矩阵分析，提出对无约束特征模型的修正以捕捉涌现的语义几何。
...
### [129] [Batch-Invariant Spectral Intelligence for Robust and Explainable Insect Authentication](https://arxiv.org/abs/2606.26757)
*Majharulislam Babor,Giacomo Rossi,Annalisa Altavilla,Oliver Schl\"uter,Marina M.-C. H\"ohne*

Main category: cs.LG

TL;DR: 提出BISN模型，通过对抗训练和可学习预处理模块，提高近红外光谱跨批次昆虫物种识别准确率。


<details>
  <summary>Details</summary>
Motivation: 可食用昆虫需要可靠的物种认证，但近红外光谱在跨批次时因批次间光谱差异导致性能下降。

Method: BISN框架包含可学习预处理模块（初始化为Savitzky-Golay滤波）和熵正则化对抗目标，在特征学习前抑制批次效应。

Result: 在三个物种、2700个光谱、三个批次上，平均留一批次交叉验证准确率0.93（标准差0.04），比最强基线高4%。

Conclusion: BISN实现了跨批次鲁棒性和生物化学可解释性，适用于工业条件下的自动昆虫物种认证。
...
### [130] [Escaping Iterative Parameter-Space Noise: Differentially Private Learning with a Hypernetwork](https://arxiv.org/abs/2606.26772)
*Naoki Nishikawa,Shokichi Takakura,Satoshi Hasegawa*

Main category: cs.LG

TL;DR: 提出一种基于超网络的差分隐私学习框架，通过一次低维噪声注入替代迭代优化，显著降低噪声影响，提升模型效用。


<details>
  <summary>Details</summary>
Motivation: DP-SGD等梯度方法在参数空间反复注入高维噪声，导致训练困难，需要降低噪声对模型效用的影响。

Method: 使用公共数据集训练超网络，将私有样本嵌入低维表示，聚合后加噪，再由超网络生成目标模型参数。

Result: 理论上在合成环境中固定隐私预算下效用优于DP-SGD；实际在扩散模型LoRA微调中FID更低。

Conclusion: 该框架通过一次性低维噪声注入能有效减少隐私噪声的负面效应，适用于多种任务。
...
### [131] [Reproducibility Study of "AlphaEdit: Null-Space Constrained Knowledge Editing for Language Models"](https://arxiv.org/abs/2606.26783)
*Ananth K S,Arya Hariharan*

Main category: cs.LG

TL;DR: 对AlphaEdit知识编辑方法进行可重复性研究，发现其在原始设置下可再现，但在新架构、长序列编辑和下游基准上表现不均，理论保证有边界。


<details>
  <summary>Details</summary>
Motivation: 验证AlphaEdit的声称，并探索其跨模型架构、编辑规模及下游任务中的鲁棒性。

Method: 重现原始实验，并沿三个方向扩展：新模型架构（如LLaMA3、GPT2-XL、GPT-J之外）、额外下游基准（BoolQ、HellaSwag、XSTest）以及更长的顺序编辑序列。

Result: 成功重现原始指标，但流畅性和一致性存在差异；优势未统一泛化到新模型；长序列编辑时性能下降；大规模顺序编辑降低下游任务能力和安全拒绝行为。
...
### [132] [AIGP: An LLM-Based Framework for Long-Term Value Alignment in E-Commerce Pricing](https://arxiv.org/abs/2606.26787)
*Chennan Ma,Yanning Zhang,Siqi Hong,Xiuchong Wang,Fei Xiao,Keping Yang*

Main category: cs.LG

TL;DR: 提出AIGP框架，结合大语言模型和离线强化学习，实现可解释、符合长期业务目标的动态定价。


<details>
  <summary>Details</summary>
Motivation: 传统动态定价模型可解释性差、未充分利用非结构化信息，且与长期业务目标（如累计GMV、ROI、里程碑达成）不一致。

Method: 利用领域知识、结构化数据和文本上下文提示LLM，通过监督微调蒸馏知识，核心是长期价值估计器（LTVE）基于离线强化学习训练，作为奖励模型选择候选动作，并使用直接偏好优化（DPO）对齐长期目标。

Result: 在淘工厂大规模在线A/B测试中，相比生产基线，GMV提升13.21%，ROI提升7.59%，里程碑达成率提升8.20%，且提供可解释的定价理由。

Conclusion: AIGP在提升业务指标的同时实现了可解释透明的定价，验证了结合LLM与离线强化学习的有效性。
...
### [133] [Reasoning Quality Emerges Early: Data Curation for Reasoning Models](https://arxiv.org/abs/2606.26797)
*Hongyi Henry Jin,Wenhan Yang,Meysam Ghaffari,Carlos Morato,Baharan Mirzasoleiman*

Main category: cs.LG

TL;DR: 本文提出通过仅使用初始推理令牌和扰动检查点来识别多样且具有挑战性的推理示例，用于监督微调，从而在提高性能的同时大幅提升令牌效率。


<details>
  <summary>Details</summary>
Motivation: 现有监督微调数据筛选方法依赖强推理模型，成本高且数据质量欠佳，需要更高效的数据筛选方法。

Method: 基于预训练模型扰动检查点上初始100个推理令牌的损失检测难题，并证明前1k令牌的损失模式可预测梯度，从而筛选高质量数据。

Result: 在Qwen2.5-7B和Llama3.1-8B模型上，方法相比基线提升高达1.7%，同时令牌效率提高91%。

Conclusion: 仅利用初始推理令牌即可有效筛选高质量微调数据，实现成本降低与性能提升。
...
### [134] [Quantization in Federated Learning: Methods, Challenges and Future Directions](https://arxiv.org/abs/2606.26822)
*Farwa Ikram,Dipanwita Thakur,Antonella Guzzo,Giancarlo Fortino*

Main category: cs.LG

TL;DR: 首次从联邦学习视角系统综述量化技术，提出围绕客户端异构性、聚合一致性、通信调度、非IID鲁棒性、隐私安全、硬件能耗等FL特有维度的新分类法，并揭示量化与客户端漂移、部分参与、收敛稳定性、安全聚合、差分隐私等核心行为的交互机制。


<details>
  <summary>Details</summary>
Motivation: 联邦学习虽为隐私保护分布式智能的基础范式，但通信瓶颈、设备异构和非独立同分布数据严重制约其可扩展性，量化是缓解这些限制的最有效机制之一，但缺乏从FL视角的系统综述。

Method: 提出首个围绕FL特有维度（客户端异构性、聚合一致性、通信调度适应、非IID鲁棒性、隐私/安全集成、硬件/能耗协同优化）的量化分类法，并分析量化与客户端漂移、部分参与、收敛稳定性、安全聚合、差分隐私等核心行为的交互。

Result: 编录了现有方法，识别了跨方法洞见、开放研究缺口，并为在移动、IoT和边缘平台部署量化FL提供了设计指南。

Conclusion: 量化不仅是一种压缩技术，更是塑造现代FL性能、鲁棒性和实用性的基本系统组件。
...
### [135] [Asymptotically Optimal Learning for Parametric Prophet Inequalities](https://arxiv.org/abs/2606.26893)
*Jung-hun Kim,Anna Grebennikova,Vianney Perchet*

Main category: cs.LG

TL;DR: 研究指数型参数族下先知不等式在线学习，提出置信度动态规划策略达到最优渐近竞争比。


<details>
  <summary>Details</summary>
Motivation: 在实际应用中，奖励分布参数未知，需要在线学习策略达到全信息最优竞争比。

Method: 基于置信度的动态规划策略，利用在线观测学习参数，无需离线样本。

Result: 在无界支撑下渐近竞争比为θ/(θ-c_+)的幂次除以Gamma函数；有界支撑下极限为1；算法达到相同渐近最优比并给出收敛速度。

Conclusion: 该在线学习策略有效，数值实验验证性能良好。
...
TL;DR: 提出方向不一致性故障模式及其解决方法geoalign，提高在线RL对齐LLM的稳定性。


<details>
  <summary>Details</summary>
Motivation: 在线强化学习在噪声或错误奖励下训练不稳定，存在方向不一致性问题。

Method: Geoalign方法，包括形成提示内偏好对、学习在线投影器、检测并纠正方向不一致的生成结果。

Result: 在对话对齐和数学推理任务上，Geoalign提升性能，减少振荡，优于多个基线。
...
### [137] [Decision-Aligned Evaluation of Uncertainty Quantification](https://arxiv.org/abs/2606.26990)
*Annika Schneider,Tommy Rochussen,Joshua Stiller,Vincent Fortuin*

Main category: cs.LG

TL;DR: 提出决策对齐标准，揭示当前不确定性估计指标与下游决策效用不一致的问题，并引入先验加权效用指标以提供与决策对齐的评估。


<details>
  <summary>Details</summary>
Motivation: 现有的不确定性评估指标（如负对数似然、期望校准误差）在通用指标上表现良好，但可能在下游决策中效用不高，需要一种与决策效用对齐的评估方法。

Method: 定义决策对齐准则，分析现有指标的对齐性，提出先验加权效用指标作为一类特殊的适当评分规则。

Result: 在基准实验和实际案例中，先验加权效用指标与决策效用一致，而传统指标则不一致，揭示了当前不确定性量化评估协议的缺陷。

Conclusion: 需要采用决策对齐的评估指标，先验加权效用指标提供了现有指标的合理扩展，以实现更贴近实际决策的不确定性评估。
...
### [138] [Uncertainty quantification via conformal prediction in data assimilation](https://arxiv.org/abs/2606.27001)
*Catherine George,Alireza Javanmardi,Tijana Janji\'c,Eyke H\"ullermeier*

Main category: cs.LG

TL;DR: 研究检验了共形预测（CP）在理想化一维修正浅水模型中量化不确定性的适用性，并与传统集合方法比较。


<details>
  <summary>Details</summary>
Motivation: 量化不确定性对数值天气预报中的概率预报和数据同化至关重要，需要探索新的机器学习方法如CP。

Method: 使用一维修正浅水模型模拟对流过程，应用标准CP、归一化CP和共形分位数回归三种变体，并与标准差区间和集合离散度等传统方法比较。

Result: 不同CP方法在平均经验覆盖、平均区间长度、漏低、漏高和平均区间评分损失上各有优劣。CP扰动可集成到数据同化循环中。
...
### [139] [A Generalization Theory for JEPA-Based World Models](https://arxiv.org/abs/2606.27014)
*Jingyi Cui,Qi Zhang,Hongwei Wen,Yisen Wang*

Main category: cs.LG

TL;DR: 本文首次建立JEPA世界模型的泛化理论，将JEPA预训练建模为条件谱图学习问题，证明其等价于动作条件共现矩阵的低秩分解，并推导出预训练误差与下游规划遗憾之间的联系。


<details>
  <summary>Details</summary>
Motivation: JEPA在经验上成功，但缺乏理论基础，需要从理论上解释其有效性及局限性。

Method: 将JEPA预训练形式化为条件谱图学习，分析JEPA目标等价于低秩分解动作条件共现矩阵，建立预训练误差与下游规划后悔的有限样本泛化界。

Result: 得到有限样本泛化界，揭示了潜在维度带来的近似误差和样本误差之间的内在权衡。

Conclusion: 提供了潜在预测模型相对于输入级预测方法的优势和局限性的理论洞见。
...
### [140] [Just how sure are you? Improving Verbalized Uncertainty Calibration in Medical VQA](https://arxiv.org/abs/2606.27023)
*Eren Senoglu,Federico Toschi,Nicolo Brunello,Andrea Sassella,Mark James Carman*

Main category: cs.LG

TL;DR: 提出一个训练框架，通过复合损失函数微调多模态大语言模型，显著降低医学视觉问答中的置信度校准误差并提高判别能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在医学VQA中常过度自信，现有文本置信度校准方法未考虑多模态特性。

Method: 使用复合损失函数（Brier校准项、锚定正则化、对比图像-文本对齐、KL稳定项）微调模型，对齐信号来自2×2因子扰动设计，并加入top-K KL散度正则化保护回答能力。

Result: 在三个基准和两种架构上，校准误差降低≥60%，判别能力提升≥26%，保持准确率，优于多种基线方法。

Conclusion: 该训练框架有效改善医学VQA中多模态模型的置信度校准，所有代码已公开。
...
### [141] [Symplectic Neural Networks for learning Generalized Hamiltonians](https://arxiv.org/abs/2606.27029)
*Harsh Choudhary,Vyacheslav Kungurtsev,Chandan Gupta,Melvin Leok,Georgios Korpas*

Main category: cs.LG

TL;DR: 提出了一种基于隐式辛积分器的哈密顿神经网络训练方法，通过伴随系统的辛离散化高效计算梯度，并在不可分混沌系统中验证了其在系统辨识和能量保持方面的优势。


<details>
  <summary>Details</summary>
Motivation: 从含噪声观测中识别系统哈密顿量具有挑战性，且隐式辛积分器计算成本高、反向传播困难。

Method: 利用伴随系统的辛离散化得到与反向传播相同的灵敏度，结合预测-校正ODE求解器和不动点迭代降低隐式时间步进的计算成本。

Result: 在不可分混沌系统上，该方法在系统辨识和能量保持方面具有数值优势，且计算和内存复杂度高效。

Conclusion: 通过向后误差分析对学习到的哈密顿量进行后处理，可得到更精确的修正哈密顿量，无需更精确的离散化。
...
### [142] [State Representation Matters in Deep Reinforcement Learning: Application to Energy Trading](https://arxiv.org/abs/2606.27032)
*Jesper Klicks,Sander Vr\v{z}ina,Vincent Fran\c{c}ois-Lavet*

Main category: cs.LG

TL;DR: 在抽水蓄能套利环境中，强化学习的状态表示对策略性能影响极大，结合绝对价格、相对价格和预测特征才能实现鲁棒迁移。


<details>
  <summary>Details</summary>
Motivation: 能源交易决策依赖未来市场条件和运营约束，状态表示是强化学习智能体的关键设计选择，但缺乏系统性研究。

Method: 固定Double DQN智能体，在HydroDam环境中仅改变市场特征（绝对、相对、预测及其组合），使用2007-2011比利时日前价格训练，在2012-2025同市场及39个其他区域测试。

Result: 绝对特征仅达测试集28.8%和跨区中位5.7%；相对和预测特征单独均低于启发式；绝对+相对达49.9%和39.8%；绝对+相对+预测达55.6%和47.5%。

Conclusion: 状态表示是策略设计的核心，稳健迁移需要同时利用价格尺度、近期相对价格背景和短期预测信息，而非单一特征。
...
### [143] [Finding Stationary Points by Comparisons](https://arxiv.org/abs/2606.27082)
*Helin Wang,Chenyi Zhang,Xiwen Tao,Yexin Zhang,Tongyang Li*

Main category: cs.LG

TL;DR: 本文研究了仅通过比较预言机（给定两点，输出较大函数值）访问非凸函数时，找到平稳点的问题，并提出了经典和量子算法。


<details>
  <summary>Details</summary>
Motivation: 在非凸优化中，当只能通过比较函数值而非梯度信息来访问目标函数时，寻找平稳点具有挑战性，这种设置适用于某些实际场景。

Method: 经典算法使用子程序估计归一化Hessian矩阵，结合梯度下降思想；量子算法利用超位置查询加速。

Result: 经典算法需要~O(n^2/ε^{1.5})次查询，量子算法需要~O(n/ε^{1.5})次查询。

Conclusion: 首次给出了量子比较预言机下非凸优化平稳点的查询复杂度，并改进了经典算法的复杂度界。
...
### [144] [Data-Free Reservoir Features for Efficient Long-Horizon Cold-Start Continual Learning](https://arxiv.org/abs/2606.27095)
*Augustinas Ju\v{c}as,Yangchen Pan*

Main category: cs.LG

TL;DR: 提出CIRCLE方法，使用固定双向二维随机共振特征和流式线性判别分析头，实现冷启动无示例类增量学习，在多种任务分割下性能优越且训练速度快。


<details>
  <summary>Details</summary>
Motivation: 现有冷启动方法在计算开销和性能之间存在矛盾：漂移补偿方法需要重复训练骨干网络，冻结骨干方法性能较差。需要一种既不训练特征提取器又能保持高性能的方法。

Method: CIRCLE使用固定的双向二维随机共振特征提取器（BiRC2D）和流式线性判别分析（SLDA）头，通过特征集成和软输出平均实现可调偏差-方差权衡，无需重放、任务边界或反向传播。

Result: 在CIFAR-100、TinyImageNet、ImageNet-Subset和ImageNet-1k上，CIRCLE在10-20任务分割时具有竞争力，在50、100、500任务分割时显著优于强基线，且训练速度远快于基于训练骨干的漂移补偿方法。

Conclusion: CIRCLE通过固定特征提取器和流式更新头部，有效解决了冷启动类增量学习的计算-性能矛盾，各组件（BiRC2D提取器、SLDA头、均衡集成）均贡献显著。
...
### [145] [Transformer-Based Classification of Bacterial Raman Spectra with LOOCV](https://arxiv.org/abs/2606.27096)
*Jamile Mohammad Jafari,Thomas Bocklitz*

Main category: cs.LG

TL;DR: 本研究系统评估了Transformer在拉曼光谱分类中的表现，在细菌数据集上显著优于传统方法，且无需预处理即可稳健分类。


<details>
  <summary>Details</summary>
Motivation: 传统拉曼光谱分类方法依赖预处理和特征提取，性能有限；Transformer模型在光谱分类中潜力未充分验证。

Method: 采用嵌套留一复制交叉验证框架，比较Transformer与PCA/ICA结合LDA、SVM、随机森林在6种细菌、5417个单细胞光谱上的表现。

Result: Transformer在所有独立测试复制上分类准确率最高，潜在特征空间类分离更好，且对原始光谱无预处理时仍鲁棒。

Conclusion: Transformer模型在拉曼光谱分类中具有优越性和鲁棒性，强调复制感知验证对模型评估的重要性。
...
### [146] [Heavy-Ball Q-Learning with Residual Weighting Correction](https://arxiv.org/abs/2606.27112)
*Donghwan Lee*

Main category: cs.LG

TL;DR: 提出一种修正的heavy-ball Q-learning方法，证明其收敛性并分析加速条件，扩展至线性函数逼近，基于切换线性系统和联合谱半径提供新视角。


<details>
  <summary>Details</summary>
Motivation: 加速标准Q-learning的收敛速度，并弥补其分析框架的不足。

Method: 在Q-learning中引入heavy-ball动量项，利用切换线性系统表示和联合谱半径进行理论分析。

Result: 证明了修正方法的收敛性，并给出了理论上加速收敛的条件；线性函数逼近下也得到类似结果。

Conclusion: 该方法有效加速Q-learning，且切换线性系统视角为理解动量机制提供了新见解。
...
### [147] [Cross-Head Attention Uplift Network with Inverse Propensity Score under Unobserved Confounding](https://arxiv.org/abs/2606.27114)
*Haoran Zhang,Chuanpu Li,Yuxin Fu,Bin Tong,Guan Wang,Bo Zheng,Feng Zhou*

Main category: cs.LG

TL;DR: 提出CHAUN网络和RA-IPS方法，分别解决提升建模中的组间相似性利用和未观测混淆偏差问题，实验显示显著改进。


<details>
  <summary>Details</summary>
Motivation: 提升建模面临灵活利用组间相似性增强判别力与在未观测混淆下消除偏差的双重挑战。

Method: CHAUN使用共享特征嵌入和交叉头注意力动态整合处理组与对照组的表征；RA-IPS通过对抗优化不确定集中的倾向权重来缓解未观测变量偏差。

Result: 在CRITEO-UPLIFT、LAZADA和电商数据集上，CHAUN的QINI得分相对提升高达25.6%；RA-IPS比标准IPS提升5.4%。

Conclusion: 验证了所提方法在真实因果推断任务中的有效性。
...
### [148] [Kolmogorov Arnold networks (KAN) for aerodynamic prediction: a comparison with MLPs and GNNs](https://arxiv.org/abs/2606.27126)
*Miguel Jaraiz,Fermin Gutierrez,Pablo Yeste,Miguel S\'anchez-Dom\'inguez,Eusebio Valero,Gonzalo Rubio,Lucas Lacasa*

Main category: cs.LG

TL;DR: 本文评估KAN在流体动力学代理建模中的性能，与MLP和GNN比较，发现KAN性能略逊于MLP，但复杂度低，训练快，但存在训练不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 评估KAN在流体动力学代理建模中的表现，比较与MLP和GNN的优劣。

Method: 使用KAN、MLP、GNN预测亚音速和跨音速翼型表面压力分布。

Result: KAN性能与MLP相当但略差，GNN最优但训练时间长；KAN复杂度低训练快，但训练不稳定且依赖超参数优化。

Conclusion: KAN在流体动力学代理建模中表现良好，但不如MLP稳定，需更多研究。
...
### [149] [fTNN: a tensor neural network for fractional PDEs](https://arxiv.org/abs/2606.27140)
*Qingkui Ma,Hehu Xie,Xiaobo Yin*

Main category: cs.LG

TL;DR: 开发了fTNN，一种确定性的张量神经网络子空间方法，用于分数阶拉普拉斯算子问题，通过几何自适应积分和边界奇异性处理，在测试中达到高精度并优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如fPINN和蒙特卡洛）在处理分数阶拉普拉斯算子时精度有限，特别是强边界奇异性和长时间模拟问题。

Method: 使用几何自适应积分分割（近场、规则内部远场、分析外部远场），结合Gauss-Jacobi、Gauss和确定性角度求积；构建边界奇异性感知的试探函数；自动选择主要指数和评估损失；对时间依赖问题使用时空可分离神经网络和交替优化。

Result: 数值实验表明，fTNN在测试基准上达到高精度，显著优于fPINN和蒙特卡洛基线。

Conclusion: fTNN框架对于分数阶拉普拉斯算子问题高效且准确，特别适合强边界奇异性和长时间模拟。
...
### [150] [Stochastic Gradient Optimization with Model-Assisted Sampling](https://arxiv.org/abs/2606.27171)
*Jonne Pohjankukka,Jukka Heikkonen*

Main category: cs.LG

TL;DR: 提出一种模型辅助采样框架，利用调查抽样理论改进随机梯度估计，减少方差并提升优化效率。


<details>
  <summary>Details</summary>
Motivation: 深度学习中小批量梯度估计存在方差，影响收敛稳定性、速度和泛化能力，现有方法如方差缩减和自适应优化器虽能缓解但可能增加计算开销。

Method: 基于调查抽样理论，将数据集视为固定有限总体，梯度视为样本估计；引入辅助梯度预测模型构建更高效的梯度估计器，均匀采样为无辅助信息时的特例。

Result: 在合成数据和六个基准数据集上的71%-86%实验中获得性能提升；使用动量优化器（如AdamW）时，仅需约一半训练轮次即可达到更好泛化。

Conclusion: 该方法易集成于现有优化器，在不改变其动态特性的前提下提升效率。
...
### [151] [RecallRisk-BERT: A Multi-Task Framework for Post-Report Medical Device Recall Triage](https://arxiv.org/abs/2606.27174)
*Ali Semih Atalay,Sevgi Yigit-Sert*

Main category: cs.LG

TL;DR: 基于54,165份FDA医疗器械召回记录，开发了RecallRisk-BERT多任务模型，同时预测召回严重性和根因类别，性能优于单任务基线。


<details>
  <summary>Details</summary>
Motivation: 现有研究多分别处理召回预测或根因分析，缺乏联合建模严重性和根因类别的方法，且召回记录数量增长带来人工分诊挑战。

Method: 使用openFDA 2002-2025年数据，首先评估经典机器学习与Boosting模型，然后开发RecallRisk-BERT多任务模型，融合PubMedBERT文本表示与产品代码等结构化特征嵌入，同时预测三级严重性和九类根因。

Result: 单任务严重性预测中LightGBM文本-表格配置最优（准确率0.963，macro-F1 0.856，ROC-AUC 0.974）；多任务中RecallRisk-BERT显著优于单任务PubMedBERT；模型风险排序与根因严重性模式高度一致（rho=0.983）。

Conclusion: 文本-表格联合学习可支持可扩展的召回后分诊、监管决策支持和根因风险分析。
...
### [152] [Automating Potential-based Reward Shaping with Vision Language Model Guidance](https://arxiv.org/abs/2606.27180)
*Henrik M\"uller,Daniel Kudenko*

Main category: cs.LG

TL;DR: 提出VLM-PBRS框架，利用视觉语言模型偏好学习势函数进行潜在型奖励塑形，解决稀疏奖励问题，提升样本效率并防止奖励黑客攻击。


<details>
  <summary>Details</summary>
Motivation: 稀疏奖励缺乏中间反馈，朴素奖励塑形易导致奖励黑客攻击；潜在型奖励塑形需专家设计势函数，成本高。

Method: 使用轻量级VLM查询图像对偏好，偏好标签训练势函数模型，将势函数用于PBRS以保留最优策略集。

Result: 在Meta-World和Franka Kitchen环境中，VLM-PBRS比基线方法样本效率更高，且能有效避免奖励黑客攻击。

Conclusion: 首次将VLM偏好学习用于PBRS势函数合成，是一种低成本、原则性的解决方案，通过小型VLM也能取得良好效果。
...
### [153] [Explaining Temporal Graph Neural Networks via Feature-induced Information Flow](https://arxiv.org/abs/2606.27201)
*Ping Xiong,Thomas Schnake,Klaus-Robert M\"uller,Shinichi Nakajima*

Main category: cs.LG

TL;DR: 提出一种新的归因方法，分析事件时序图神经网络中所有事件相关变量的完整信息流，在合成和真实数据集上优于现有解释方法。


<details>
  <summary>Details</summary>
Motivation: 现有解释方法仅考虑ETGNNs的部分信息流，忽略了通过事件诱导变量的路径，而这些路径对捕获长时依赖至关重要。

Method: 基于标准化相关度量（NRM）框架，通过模块化分解扩展以处理复杂架构，量化事件嵌入及事件诱导变量的信息流，确保跨层可比性并支持高阶分析。

Result: 在流行病追踪、社会动态合成数据集及政治事件网络真实数据集上，定性定量实验表明该方法持续优于现有解释方法，生成更可解释的解释。

Conclusion: 新方法克服了现有方法的局限性，通过分析完整信息流提供了更准确、更可解释的归因结果。
...
### [154] [Graph Neural Networks Applications Across Domains: All Insights You Need](https://arxiv.org/abs/2606.27202)
*Abderaouf Bahi*

Main category: cs.LG

TL;DR: 本文综述了图神经网络的基本原理、设计空间、表达力界限，并在十二个应用领域中分析其收益与成本，揭示了异质性、规模、时间动态等常见挑战，以及模型表现与部署之间的差距。


<details>
  <summary>Details</summary>
Motivation: 图神经网络已从冷门技术成为处理关系结构数据的默认模型，但需要明确哪些场景下图结构值得计算成本，为此需要系统梳理设计空间、表达力及应用表现。

Method: 围绕统一设计空间组织，从第一原理导出谱域和空域公式，将表达力与Weisfeiler-Leman层次关联，并针对十二个领域指定图构建选择、成本、主导架构及收益归因。

Result: 跨领域对比揭示了重复模式：异质性和规模削弱大多数模型；时态图比静态图更难；排行榜领先架构很少被部署。此外，过度平滑、过度挤压、鲁棒性、分布偏移、公平性和可解释性是决定采用的关键约束。

Conclusion: 当前GNN模型在许多基准上表现优异，但实际部署面临多重挑战，未来应关注基本限制而非仅追求基准精度。
...
### [155] [The Geometry of Updates: Fisher Alignment at Vocabulary Scale](https://arxiv.org/abs/2606.27242)
*John Sweeney*

Main category: cs.LG

TL;DR: 提出FisherSketch，一种无需训练的源选择方法，通过单次流式传递估计头Fisher对齐，在共享词表的LLM家族中高效度量任务相似性。


<details>
  <summary>Details</summary>
Motivation: 在科学字符串领域，传统表示相似度在激活-黑暗机制下不可识别，而更新几何度量计算昂贵，需要高效的源选择方法。

Method: 利用头Fisher对齐与激活-误差联合空间中核均值嵌入余弦的等价性，通过单次流式传递直接估计该余弦，使用16KB任务签名和192KB每任务流状态。

Result: 实现了K=128,256头Fisher对齐的实用估计，且比激活相似性更能区分任务；在Llama-3.1-8B实验中保持信息性。

Conclusion: FisherSketch可同时用于源选择和任务相似性诊断，当激活相似性失效时仍能提供有效信息。
...
### [156] [Effective Covariance Dynamics in Solvable High-Dimensional GANs](https://arxiv.org/abs/2606.27246)
*Andrew Bond,Zafer Do\u{g}an*

Main category: cs.LG

TL;DR: 本工作提出了一个高维可解GAN模型，通过有效协方差驱动动力学，揭示了信号增强和稳定性条件。


<details>
  <summary>Details</summary>
Motivation: 现有可解GAN分析假设无条件信号和对角潜在协方差，本工作将其扩展到类别依赖、相关和非零均值潜在结构。

Method: 采用线性生成器和二次能量判别器，通过概率加权有效二阶矩纳入异质性，在高维极限下推导出确定性ODE并进行稳定性分析。

Result: 得到了学习开始和完全恢复的模式可解区间条件，低秩相关性可增强弱方向，过强相关性破坏恢复，数值和实验验证了理论。

Conclusion: 生成器的协方差信息有助于改善子空间对齐，为GAN训练提供理论指导。
...
### [157] [RSPC: A Benchmark for Modeling Stress and Psychiatric Conditions in Digitally Mediated Relationships using Psychiatrist Annotations](https://arxiv.org/abs/2606.27247)
*Parmitha Vangapandu,Sai Ganesh Mokkapati,Sathwik Narkedimilli,MSVPJ Sathvik,Timothy Liu,Simon See,Johannes C. Eichstaedt*

Main category: cs.LG

TL;DR: 构建了一个关于长距离关系的精神健康数据集RSPC，并在多个NLP任务上进行了基准测试。


<details>
  <summary>Details</summary>
Motivation: 当前精神健康建模常孤立地看待个体，忽略了人际关系上下文。

Method: 收集Reddit帖子，由精神病学家标注障碍类别、关系压力触发和关系阶段；使用7个微调transformer和5个大语言模型进行多标签分类、触发检测和阶段预测。

Result: Claude-3-Haiku在障碍分类上最佳（Macro-F1=0.538），GPT-4o在关系触发检测上最佳（Macro-F1=0.519）；焦虑障碍与关系不确定性强相关。

Conclusion: RSPC建立了考虑关系上下文的NLP基准，有助于从个体中心转向上下文感知的精神健康建模，捕捉社会和时间动态。
...
### [158] [BetXplain: An Explanation-Annotated Dataset for Detecting Manipulative Betting Advertisements on Social Media](https://arxiv.org/abs/2606.27274)
*MSVPJ Sathvik,Parmitha Vangapadu,Nishit Rane,Sathwik Narkedimilli,Mark Lee,Akrati Saxena*

Main category: cs.LG

TL;DR: 提出了一个新的博彩广告数据集，来自Instagram和Reddit，包含操纵和欺骗性标注及解释，分析了常见策略及其对心理健康的影响，可用于浏览器插件和网络爬虫。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上博彩广告激增，使用说服技巧可能误导用户并影响心理健康，但缺乏自动检测的标注数据集。

Method: 从Instagram和Reddit收集博彩广告，人工标注操纵和欺骗性实践，并提供解释；分析常用策略及其对心理健康的影响。

Result: 构建了包含分类标签和人工解释的数据集，揭示了博彩广告中的常见欺骗策略及其潜在心理影响。

Conclusion: 该框架可用于开发浏览器插件警告用户和网络爬虫监测博彩广告，有助于监管和防护。
...
### [159] [How Good Can Linear Models Be for Time-Series Forecasting?](https://arxiv.org/abs/2606.27282)
*Lang Huang,Jinglue Xu,Luke Darlow*

Main category: cs.LG

TL;DR: 通过调优预处理（而非扩大模型）即可显著提升时间序列预测性能，关键发现包括最优回顾长度非单调、局部归一化更优等。


<details>
  <summary>Details</summary>
Motivation: 挑战大型架构假设，探究预处理调优能否低成本提升准确性。

Method: 使用Ridge回归作为测试床，在8个基准上搜索上下文长度、局部归一化、正则化和数据增强。

Result: 发现三个模式：最优回顾长度系列特定且非单调；局部归一化普遍更优；系列间超参数不一致。方法在多数基准上优于线性预测器和Transformer等基线。

Conclusion: 优化预处理可缩小与大型模型的差距，并揭示数据隐藏结构。
...
### [160] [Recovering Governing Equations from Solution Data: Identifiability Bounds for Linear and Nonlinear ODEs](https://arxiv.org/abs/2606.27285)
*Yang Pan,Helmut B\"olcskei*

Main category: cs.LG

TL;DR: 从解观测数据学习控制方程的理论条件，引入解集Hausdorff距离，给出可辨识性和样本复杂度界。


<details>
  <summary>Details</summary>
Motivation: 现有理论缺乏唯一稳定辨识ODE的条件和样本复杂度分析。

Method: 采用解集Hausdorff距离作为比较微分方程的自然度量，推导可辨识性界和度量熵估计。

Result: 对线性至非线性（Lipschitz/Hölder连续）ODE类建立了可辨识性界，并量化了所需解观测数。

Conclusion: 填补了ODE辨识的理论空白，为可靠恢复控制方程提供了理论基础。
...
TL;DR: 提出端到端RLAIF框架生成可迁移的求职搜索查询，重点处理对抗性奖励表面和优化问题。


<details>
  <summary>Details</summary>
Motivation: 现有求职平台查询接口带宽低，难以捕捉候选人轮廓的高维复杂性，需要生成可概括资格但不含个人标识的查询。

Method: 使用RLAIF框架，对比不同优化器（RLOO、REINFORCE++、GRPO）和奖励工程（引入确定性规则奖励下限）的影响。

Result: 无批评器优化器性能主要取决于奖励塑造；GRPO对虚假奖励敏感，易被利用；添加规则奖励下限可缓解，质量提升+0.147；训练时奖励模型高估性能增益2.4倍。

Conclusion: 训练成功关键依赖奖励塑造纪律而非优化器选择，需加强奖励工程防止奖励作弊。
...
TL;DR: 提出了一种多保真迁移学习框架，结合轻量物理仿真、卷积自编码器深度特征学习和少量实验数据，实现板状结构损伤的高精度定位与定量。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在导波结构健康监测中因实验数据标注困难和高保真仿真计算成本高而难以实际部署。

Method: 采用一维时域谱元模型生成大量合成数据预训练，通过迁移学习利用少量实验数据微调，结合卷积自编码器提取深度特征和前馈神经网络进行损伤定位与定量。

Result: 损伤定位R²>0.93，损伤定量R²>0.99，在未见过的损伤场景中仍保持高预测精度。
...
TL;DR: 本文证明梯度均衡（GEQ）与Blackwell可逼近性在算法上等价，从而也等价于遗憾最小化和校准等框架，并建立了无约束和约束决策集下GEQ的归约。


<details>
  <summary>Details</summary>
Motivation: 理解GEQ在在线学习中的精确定位，因为GEQ误差与遗憾不可比较。

Method: 通过构建双向黑盒归约，证明GEQ问题与Blackwell可逼近性问题可互相转化且无渐近损失。

Result: GEQ与Blackwell可逼近性等价，进而与遗憾最小化、校准等框架等价，且归约高效，可转移乐观和强自适应性等保证。

Conclusion: GEQ并非孤立框架，而是与经典在线学习概念紧密相连，为理论分析和算法设计提供统一视角。
...
### [164] [Beyond the Hard Budget: Sparsity Regularizers for More Interpretable Top-k Sparse Autoencoders](https://arxiv.org/abs/2606.27321)
*Nathana\"el Jacquier,Maria Vakalopoulou,Mahdi S. Hosseini*

Main category: cs.LG

TL;DR: 提出两种与Top-k稀疏自编码器兼容的稀疏正则化器，在不损失重建质量的前提下提高特征单义性，并发现硬架构稀疏性与软正则化互补。


<details>
  <summary>Details</summary>
Motivation: Top-k SAE虽避免L1惩罚的缺点，但存在固定预算k和过拟合问题，需要引入正则化以弥补其局限。

Method: 在Top-k选择前对激活施加两种正则化：ℓ1惩罚作用于未选单元，以及尺度不变的ℓ1/ℓ2比率惩罚以集中编码到更少有效单元，且仅对批次活跃单元应用。

Result: 在两个数据集、三个视觉基础模型和不同k值下，两种正则化器一致提升单义性，不损重建质量；ℓ1/ℓ2惩罚进一步集中信息，使重建对推理时k的选择更鲁棒，并改善小预算线性探针。

Conclusion: 硬架构稀疏性和软稀疏正则化是互补的，而非互斥。
...
### [165] [Hallucination in World Models is Predictable and Preventable](https://arxiv.org/abs/2606.27326)
*Nicklas Hansen,Xiaolong Wang*

Main category: cs.LG

TL;DR: 世界模型中的幻觉本质是数据覆盖问题，通过MMBench2数据集和三种检测信号，实现了高效检测与缓解，仅需50条轨迹即可适应新环境。


<details>
  <summary>Details</summary>
Motivation: 生成世界模型幻觉严重且集中在低数据覆盖区域，现有方法缺乏有效的检测与缓解手段。

Method: 构建427小时、210任务的MMBench2数据集，训练350M参数世界模型；识别三种幻觉模式（感知、动作边缘化、场景发散），开发三种预测信号；采用覆盖感知采样技术缩小训练覆盖差距，并以预测信号作为好奇心奖励进行在线数据收集和微调。

Result: 幻觉预测准确，通过数据高效的微调方法，仅需50条真实环境轨迹即可使预训练模型适应全新环境。

Conclusion: 世界模型的幻觉是数据覆盖问题，用于检测的信号同样可用于缓解，且数据驱动方法效果显著。
...
### [166] [Error-Conditioned Neural Solvers](https://arxiv.org/abs/2606.27354)
*Haina Jiang,Liam Wang,Peng-Chen Chen,Min Seop Kwak,Seungryong Kim,Brian Bell,Jeong Joon Park*

Main category: cs.LG

TL;DR: 提出错误条件神经求解器（ENS），通过将PDE残差场作为网络输入来迭代校正预测，在多个PDE基准上达到最高精度，且避免混合方法的计算成本与不稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有神经代理模型难以纠错和外推；混合方法依赖残差最小化，在病态系统中不可靠且计算成本高。

Method: ENS将PDE残差场作为直接输入传递给网络，使网络读取自身误差的空间结构，并学习迭代更新策略来校正预测。

Result: 在四个PDE族中，ENS在大部分设置下取得最高预测准确度，对湍流Kolmogorov流提升10倍，同时避免混合方法的昂贵计算；在分布偏移（零-shot参数变化、跨方程迁移）下泛化良好。

Conclusion: ENS在残差最小化最不可靠的病态系统中优势最大，是一种高效、准确的替代方案。
...
### [167] [Autoregressive Boltzmann Generators](https://arxiv.org/abs/2606.27361)
*Danyal Rehman,Charlie B. Tan,Yoshua Bengio,Avishek Joey Bose,Alexander Tong*

Main category: cs.LG

TL;DR: 提出自回归Boltzmann生成器（ArBG），克服了基于流的BG的局限性，在分子平衡采样中取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有Boltzmann生成器依赖归一化流，存在表达性受限或似然计算昂贵的问题。

Method: 采用自回归建模框架，绕过流模型拓扑约束，利用大规模语言模型架构增强可扩展性。

Result: 在所有基准测试中优于流模型，在十残基Chignolin系统上提升显著；Robin模型将零样本能量误差降低超过60%。

Conclusion: ArBG框架有效改进了分子平衡采样，展示了自回归方法在统计物理中的潜力。
...
### [168] [Reinforcement Learning without Ground-Truth Solutions can Improve LLMs](https://arxiv.org/abs/2606.27369)
*Yingyu Lin,Qiyue Gao,Nikki Lijing Kuang,Xunpeng Huang,Kun Zhou,Tongtong Liang,Zhewei Yao,Yi-An Ma,Yuxiong He*

Main category: cs.LG

TL;DR: RiVER框架利用确定性执行反馈和奖励校准，在无真实解任务上训练LLM，既提升分数优化任务表现，也意外提升精确解基准。


<details>
  <summary>Details</summary>
Motivation: 现有的RLVR方法依赖真实答案奖励，难以应用于无标准答案的分数优化任务。

Method: 提出RiVER框架，使用执行反馈作为连续奖励，通过实例间比较和强调顶级解的奖励校准解决尺度优势与频率优势。

Result: 在ALE-Bench上Qwen3-8B和GLM-Z1-9B-0414排名分别提升8.9%和9.4%；在LiveCodeBench和USACO上平均绝对提升2.4%和3.5%。

Conclusion: 分数优化任务与适当奖励校准可作为无需真实解的有效训练环境，并提升通用编码能力。
...
### [169] [On the optimal prediction of extreme events](https://arxiv.org/abs/2606.26270)
*Benjamin Bobbia,Stilian Stoev*

Main category: math.ST

TL;DR: 本工作针对多元正则变异性分布下响应变量极端值的预测问题，将预测精度表达为角测度的积分泛函，通过变分法得到一般解，提出基于超阈值框架的推断方法，证明估计量的一致性，并在模拟和太阳耀斑预测中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 极端值数据稀缺，仅少数情况存在最优预测器的显式解，需要在大类模型上开发最优估计器。

Method: 将问题转化为变分问题，利用角测度导出倾斜分布的非极端条件分位数求解，基于极值理论的超阈值方法进行推断。

Result: 获得一般解，证明估计量在大类角测度下一致；实现程序在模拟和太阳耀斑预测中表现接近最优。

Conclusion: 所提方法在理论和实践上均有效，可处理极端值预测挑战。
...
### [171] [Multiple testing](https://arxiv.org/abs/2606.26781)
*Jesse Hemerik*

Main category: stat.ME

TL;DR: 本文介绍多重假设检验，涵盖错误标准和检验程序，并附有R包参考。


<details>
  <summary>Details</summary>
Motivation: 为博士课程提供多重检验的教学材料。

Method: 系统介绍多重假设检验的错误准则和检验程序。

Result: 提供相关R包参考。

Conclusion: 本文是多重检验的导论性材料。
...
### [172] [Optimizing Experimental Design for Causal Effect Estimation with Partial Measurements](https://arxiv.org/abs/2606.26818)
*Leopold Mareis*

Main category: stat.ME

TL;DR: 在预算约束下，通过优化部分样本和完整样本的采集数量，减少工具变量回归中因果效应估计的渐近方差，从而降低总成本。


<details>
  <summary>Details</summary>
Motivation: 已有部分联合分布先验信息，但需要额外样本提高估计精度；完整样本成本高，希望通过部分样本降低成本。

Method: 在高斯图模型下，考虑每个样本的预算成本，将部分样本（如X12）纳入优化，通过解析求解得到最优样本数量分配。

Result: 与传统策略相比，该方法能显著降低必要预算和完整样本数量，并提供了显著性水平、功效和样本量计算公式。

Conclusion: 自适应因果效应估计在汽车分析和药物研究中具有成本优势和应用潜力。
...
### [173] [Semiparametric Inference for Half-Trek Estimators in Linear Structural Equation Models](https://arxiv.org/abs/2606.26931)
*Leopold Mareis,Nils Sturma,Mathias Drton*

Main category: stat.ME

TL;DR: 推导了半长准则（HTC）估计器的渐近分布，提出了闭式方差和置信区间，并应用于鱼市数据。


<details>
  <summary>Details</summary>
Motivation: 先前HTC估计量的渐近分布未知，无法进行有效的统计推断。

Method: 通过半参数影响函数，结合目标节点结构残差与识别工具，递归校正前期估计的不确定性。

Result: HTC估计量渐近正态，方差可闭式计算，提供置信区域、边际区间和Wald检验。

Conclusion: 在Fulton鱼市数据上，该方法给出了供给对需求因果效应的完整推断总结。
...
### [174] [Explainable Outlier Detection for Interval-valued Data](https://arxiv.org/abs/2606.26307)
*Catarina P. Loureiro,M. Ros\'ario Oliveira,Paula Brito,Lina Oliveira*

Main category: stat.ME

TL;DR: 针对区间值数据，提出基于Shapley值的异常值解释方法，实现细粒度分解与交互分析。


<details>
  <summary>Details</summary>
Motivation: 异常检测可解释性重要，但区间值数据领域未探索。

Method: 基于区间最小协方差行列式估计器，推导Shapley值封闭形式，分解中心、范围及交叉项贡献，并引入Shapley交互指数。

Result: 能识别变量特定异常值，并捕捉变量对交互效应。

Conclusion: 通过真实数据展示实用性与解释能力。
...
### [175] [Variance Deltas for Visualizing and Explaining Posterior Uncertainty](https://arxiv.org/abs/2606.26309)
*Collin Cademartori*

Main category: stat.ME

TL;DR: 提出方差增量可视化系统，从贝叶斯后验中发现缺失信息，通过树状子集和交互操作帮助理解不确定性来源。


<details>
  <summary>Details</summary>
Motivation: 观测研究中数据不足时，无法保证估计精度，但可确定缺失信息以降低不确定性。

Method: 将缺失信息表示为未观测模型量的子集，基于解释不确定性程度组织成树，自动化构建候选子集并支持分割与合并操作。

Result: 在模拟因果推断和实际民调预测中，系统成功发现了非显而易见的解释。

Conclusion: 该系统能高效发现缺失信息，辅助理解不确定性来源，适用于多种统计问题。
...
### [176] [A unified approach to outlier identification for mixed-type data](https://arxiv.org/abs/2606.26324)
*Efthymios Costa,Christian Hennig*

Main category: stat.ME

TL;DR: 提出一种针对连续和序数混合类型数据的异常值识别方法，基于稳健MCD估计和潜在高斯假设。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以有效处理混合类型数据中序数变量的特性，需要稳健的异常值检测技术。

Method: 以多元高斯为参考分布，对序数变量假设潜在高斯，采用稳健最小协方差行列式估计参数，并扩展处理序数变量信息不完整的情况。

Result: 合成数据实验获得高检测率和低假阳性率；Airbnb数据应用验证实用性。

Conclusion: 方法能稳健识别混合类型数据中的异常值，具备理论和实践有效性。
...
### [177] [Incomplete Matrix Regression](https://arxiv.org/abs/2606.26325)
*Khaled Fouda,Aur\'elie Labbe,Karim Oualkacha*

Main category: stat.ME

TL;DR: 提出了一种分布自由的惩罚回归框架IMR，结合辅助协变量和相似结构进行矩阵补全，具有可扩展的交替最小二乘算法，并提供了非渐近误差界。


<details>
  <summary>Details</summary>
Motivation: 在推荐系统、城市交通等应用中，观测矩阵常伴有行列辅助协变量且存在依赖关系，现有矩阵补全方法未充分利用这些信息。

Method: 将目标矩阵建模为截距、Lasso惩罚的协变量效应和低秩潜在成分之和，通过岭型惩罚融入已知相似结构（如空间、时间核），使用可扩展的交替最小二乘算法估计。

Result: 通过模拟和两个实际数据应用，IMR在预测精度上可与更复杂的方法竞争，但计算成本极低。

Conclusion: IMR是一种有效且高效的矩阵补全方法，能够整合辅助信息，具有理论保证和实用优势，已实现为R包IMR。
...
### [178] [Robust Estimation of Polychoric Correlation for Complex Survey Designs Using Minimum Divergence Methods](https://arxiv.org/abs/2606.26351)
*Siqi Wei,David Kepplinger,Anand N. Vidyashankar*

Main category: stat.ME

TL;DR: 针对标准多分格相关极大似然估计对调查数据污染敏感的问题，提出了基于海灵格距离和负指数差异的稳健估计方法，并给出了方法选择指南。


<details>
  <summary>Details</summary>
Motivation: 标准多分格相关极大似然估计对响应误差、访谈者效应和粗心回答等污染高度敏感，且对所有观测值等权处理，需要稳健估计方法。

Method: 基于海灵格距离和负指数差异两种最小散度准则，通过霍维茨-汤普森调整的单元格频率纳入调查权重；对海灵格距离提出了带惩罚的岭回归和套索变体，并推导了渐近性质。

Result: 模拟表明：海灵格惩罚方法在一致污染下均方误差最低，负指数差异在不一致污染和复合误设定-污染效应下表现最优。

Conclusion: 根据预期污染模式给出了实际调查中方法选择的实用指南。
...
### [179] [Learning rate selection via weighted Fisher divergence](https://arxiv.org/abs/2606.26478)
*Takahiro Onizuka*

Main category: stat.ME

TL;DR: 针对通用贝叶斯推断中学习率校准问题，提出一种基于加权Fisher散度的方法，得到闭式解，并通过数值和实际数据验证有效性。


<details>
  <summary>Details</summary>
Motivation: 通用贝叶斯后验在模型误设定下存在校准问题，传统贝叶斯信息恒等式失效，需要校准学习率以保证不确定性解释。

Method: 引入通用后验渐近分布与具有三明治型方差的正态分布之间的加权Fisher散度，推导出学习率的闭式表达式。

Result: 所选学习率包含Fisher信息匹配学习率作为特例，且在重要特例中不大于它；数值和实际数据示例表明方法有效。

Conclusion: 提出的方法能够有效校准通用后验的学习率，提高推断的可靠性。
...
### [180] [A Unified Three-Stage Weighting Framework for Causal Inference and Mediation Analysis under Case-Control Sampling](https://arxiv.org/abs/2606.26543)
*Tarikul Islam,Mahbub A.H.M. Latif*

Main category: stat.ME

TL;DR: 提出一种三阶段加权框架，用于病例对照研究的因果推断和中介分析，通过密度比学习和标签偏移校正估计流行率，重建目标人群分布，再利用稳定权重估计因果效应。


<details>
  <summary>Details</summary>
Motivation: 病例对照研究的结局依赖抽样扭曲了人群结局分布，给因果推断带来挑战。

Method: 三阶段加权：1）密度比学习和标签偏移校正结合外部协变量信息估计人群结局流行率；2）基于流行率的设计权重重建目标人群分布；3）在边际结构模型中应用稳定的因果和中介权重估计总效应、直接效应、间接效应和交互效应。

Result: 模拟表明传统分析产生严重偏倚，而所提方法在各种抽样场景下一致恢复目标人群因果参数。

Conclusion: 所提框架有效纠正了病例对照研究的抽样偏倚，适用于因果推断和中介分析，且在真实数据中具有实用性。
...
### [181] [Fast Robust Regression via Orthogonal Block Updates](https://arxiv.org/abs/2606.26611)
*Anthony Christidis,Matias Salibian-Barrera*

Main category: stat.ME

TL;DR: 提出一种基于块坐标下降的鲁棒回归初始估计方法，克服了高维下随机子集指数增长问题。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒回归初始估计方法（随机子集）在高维下不可行，非随机策略也难扩展。

Method: 使用块坐标下降的ROBU算法，对低维变量块进行子采样，减少所需子样本数。

Result: 大量模拟和蛋白质组学数据表明，ROBU计算的估计器鲁棒性好，优于现有算法。

Conclusion: ROBU可扩展，适用于高维和大样本鲁棒回归。
...
### [182] [$\lambda$-PSD: Scalable Approximate SNR-Optimised Polynomial Stein Discrepancies](https://arxiv.org/abs/2606.26621)
*Minh-Long Nguyen,Thanh-Long Vu,Christopher Drovandi,Leah F. South,Trung-Tin Nguyen*

Main category: stat.ME

TL;DR: 提出λ-PSD方法，通过显式最大化信噪比平方来改进多项式斯坦因差异，避免指数衰减并保持线性时间复杂性。


<details>
  <summary>Details</summary>
Motivation: 多项式斯坦因差异的统计性质不明，增加阶数主要放大信号而不控制方差，可能导致信噪比指数衰减。

Method: 将斯坦因差异构造重新表述为信噪比平方最大化问题，得到瑞利商形式；据此提出λ-PSD，在低维子空间进行近似协方差感知的重新加权。

Result: λ-PSD避免了信噪比平方的指数衰减，实现了稳定的信噪比；实验显著提升了检验功效，且保持样本数量的线性时间复杂性。

Conclusion: 信噪比感知的设计对于可扩展的斯坦因差异至关重要。
...
### [183] [Doubly Robust Quadratic Inference Functions for Causal Inference in Cluster Randomized Trials](https://arxiv.org/abs/2606.26630)
*Hengshi Yu*

Main category: stat.ME

TL;DR: 提出双重稳健二次推断函数（DR-QIF）估计量，用于处理群组随机试验中因协变量不平衡导致的混杂，并在工作相关结构误设时提高效率。


<details>
  <summary>Details</summary>
Motivation: 现有QIF方法未考虑治疗组间协变量不平衡导致的混杂，需要一种对模型误设鲁棒的方法。

Method: 结合双重稳健伪结果（基于倾向性评分和结果回归模型）与QIF扩展得分方程构建DR-QIF估计量。

Result: 当工作相关结构误设时，DR-QIF效率高于双重稳健GEE（DR-GEE），在纵向CRT中最大效率增益达3.5%（N=120，T=8）。

Conclusion: DR-QIF在倾向性评分或结果回归模型之一正确时一致，且在工作相关结构误设下更高效，适用于纵向群组随机试验。
...
### [184] [Multivariable Mendelian randomization with weak instruments: a comparison of Bayesian and frequentist methods](https://arxiv.org/abs/2606.26638)
*Andrew J. Grant,Ashish Patel,Stephen Burgess*

Main category: stat.ME

TL;DR: 提出贝叶斯方法MVMR-Pony，用于缓解多变量孟德尔随机化中的弱工具变量偏差，模拟显示其优于频率学派方法。


<details>
  <summary>Details</summary>
Motivation: 多暴露情况下，遗传工具变量可能条件弱，现有频率学派方法较少且效果有限。

Method: 基于贝叶斯框架的MVMR-Pony方法，通过模拟研究比较与频率学派方法的性能。

Result: 在相关遗传效应、测量误差和中介等弱工具变量场景下，MVMR-Pony在偏倚、覆盖、I型错误率和统计功效上均优于频率学派方法。
...
### [185] [End-to-end probabilistic hierarchical forecasting of large hierarchies via probabilistic top-down](https://arxiv.org/abs/2606.26774)
*Lorenzo Zambon,Dario Azzimonti,Giorgio Corani*

Main category: stat.ME

TL;DR: 提出 e2eTD，一种快速可扩展的端到端概率分层预测方法，通过仅预测少量顶层序列并利用历史比例联合分布进行自上而下抽样，实现大规模层级时间序列的连贯预测。


<details>
  <summary>Details</summary>
Motivation: 零售和供应链需求预测需概率性且层级连贯，但现有方法计算成本高、可扩展性差，无法应对大规模层级序列。

Method: 仅预测层级中约0.3%的顶层聚合序列（更平滑易预测），通过新颖的概率自上而下抽样算法，将历史分解比例建模为联合分布并在样本内估计，生成底层样本后求和得到所有层级连贯预测。

Result: 在 M5（约4万序列）和 Favorita（约30万序列）数据集上，e2eTD 获得最低加权分位数损失，在 M5 不确定性竞赛中可排第11位；在标准笔记本上运行时间约5分钟（M5）和20分钟（Favorita）。

Conclusion: e2eTD 在大规模层级时间序列的概率连贯预测中，实现了精度与速度的平衡，具有强可扩展性。
...
### [186] [Structured Secant Methods to Select Smoothing Parameters For General Smooth Models](https://arxiv.org/abs/2606.26804)
*Joshua Krause,Jelmer P. Borst,Jacolien van Rij*

Main category: stat.ME

TL;DR: 提出一种拟牛顿变体qEFS方法，用于优化一般平滑模型中的平滑参数，避免高阶导数的计算，并通过结构化近似保持精度。


<details>
  <summary>Details</summary>
Motivation: 现有二阶优化方法需要对数似然的Hessian或高阶导数，推导困难且计算昂贵。

Method: 基于扩展Fellner-Schall方法的拟牛顿变体，利用结构化有限记忆割线近似Hessian，主要为一阶，但可对子块累加精确列以提高精度。

Result: 理论证明qEFS在特定条件下收敛到EFS，模拟显示即使超出条件也能提供良好估计；次要任务需部分近似以接近名义性能。

Conclusion: qEFS方法易于实现，在隐马尔可夫和Tweedie模型上比替代方法更简便，且性能接近二阶方法。
...
### [187] [High-dimensional reliability-oriented Shapley effect estimation with Normalizing Flows](https://arxiv.org/abs/2606.26826)
*Monteiro Lucas,Morio J\'er\^ome,Demange-Chryst Julien,Bachoc Fran\c{c}ois*

Main category: stat.ME

TL;DR: 提出了一种新方案，仅用故障点单一样本估计大量相关输入变量的可靠型Shapley效应。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以处理高维相关输入，需高效估计方案。

Method: 先提出含故障条件边际密度的可靠型封闭Sobol指标新表达，再用归一化流估计高维密度，并基于同一故障样本提供误差估计。

Result: 在数值案例上验证了方法，讨论了特征。

Conclusion: 为未来研究提供了展望。
...
### [188] [Exact Comparison of Explanatory Strength of Two Dependent Predictors](https://arxiv.org/abs/2606.26949)
*Tom\'a\v{s} Mrkvi\v{c}ka,Jan Radimsk\'y*

Main category: stat.ME

TL;DR: 提出一种新颖的精确非参数检验方法，用于比较两个依赖预测变量对共同目标变量的解释能力，克服了经典检验和朴素重抽样的局限性。


<details>
  <summary>Details</summary>
Motivation: 经典检验（如Vuong检验、Hotelling-Williams检验）在重尾分布和分类稀疏性下失效；朴素置换检验破坏协方差结构，配对bootstrap引入人工结和度量空间压缩，有限样本下不可靠。

Method: 基于零假设下功能可交换性原则，对分类数据采用对称的组内交换机制，对连续域引入经验累积分布函数映射步骤（基于copula的转置），完美保留边际密度和经验支持，不产生重抽样结。

Result: 广泛蒙特卡洛模拟表明，该方法严格维持名义显著性水平，在标准方法变得灾难性宽松或病态保守的条件下最大化统计功效。

Conclusion: 应用于高维语言学数据集（意大利语名词-名词复合词），证明其在传统分析方法固有失败的环境中提供稳健、精确的推断能力。
...
### [189] [Predicting disease severity and large-scale spread from coupled severity measurements and imperfect indicators: Application to beet yellows](https://arxiv.org/abs/2606.26972)
*Baptiste Oger,C\'esar Martinez,Fran\c{c}ois Joudelat,Samuel Soubeyrand,Lionel Benoit*

Main category: stat.ME

TL;DR: 提出一种统计框架，利用间接指标（如卫星遥感数据）预测疾病严重程度并重建时空动态，通过甜菜黄化病案例验证。


<details>
  <summary>Details</summary>
Motivation: 疾病管理需要时空动态特征，但直接测量稀疏且成本高，间接指标可提供补充信息。

Method: 两步法：第一步用堆叠障碍模型（多随机森林）从间接指标预测局部疾病严重程度；第二步用半参数时空模型重建大规模流行病动态。

Result: 以2019-2023年法国甜菜黄化病为例，结合稀疏田间测量和卫星遥感数据，成功预测和重构疾病动态。

Conclusion: 该方法通用且模块化，可有效整合间接指标改善疾病监测。
...
### [190] [Conditional Leibniz Derivative Estimation with an Application to American Call Min-Options](https://arxiv.org/abs/2606.27046)
*Xingyu Ren,Michael C. Fu,Pierre L'Ecuyer*

Main category: stat.ME

TL;DR: 提出一种递归条件方法结合Leibniz导数估计框架，得到条件Leibniz估计器，避免似然比项，降低方差且易于实现，在美式期权模型中验证有效性。


<details>
  <summary>Details</summary>
Motivation: 现有基于似然比的Leibniz估计器方差高且随随机输入维度线性增长，需改进。

Method: 采用递归条件方法，与Leibniz导数估计框架结合，消除似然比项。

Result: 在美式看涨最小期权模型中仿真，显示估计器有效且方差低。

Conclusion: 条件Leibniz估计器方差不随输入维度增长，形式简单，易于实现，适用于不连续样本性能的导数估计。
...
### [191] [Estimation of High Dimensional Bounded Discrete Graphical Models via Regularized Generalized Score Matching](https://arxiv.org/abs/2606.27142)
*Xuran Meng,Jingfei Zhang,Yi Li*

Main category: stat.ME

TL;DR: 提出有界离散图形模型（BRIDGE），利用正则化广义得分匹配实现高维离散数据的图结构学习，无需计算归一化常数。


<details>
  <summary>Details</summary>
Motivation: 现有多元计数图模型因有界支撑导致归一化常数有限性约束强，且参数需满足限制条件。

Method: 采用广义得分匹配估计，结合L1正则化；对损失函数退化问题引入重新参数化恢复曲率。

Result: 理论证明非渐近误差界和支持恢复；模拟和真实数据实验准确恢复图结构。

Conclusion: BRIDGE提供了稳定、可解释的高维离散图建模框架，有效避免归一化常数限制。
...
### [192] [Detecting and Controlling Sycophancy with Cascading Linear Features](https://arxiv.org/abs/2606.26155)
*Maty Bohacek,Rishub Jain,Nicholas Dufour,Thomas Leung,Chris Bregler,Roma Patel*

Main category: cs.AI

TL;DR: 提出一种迭代数据生成管道，通过隔离级联线性特征来改进激活引导，应用于语言模型的谄媚行为检测与抑制。


<details>
  <summary>Details</summary>
Motivation: 现有的激活引导方法依赖二元对比样本，难以可靠检测和引导模型行为，需要更好的特征解耦方法。

Method: 通过迭代生成展示行为线性变化程度的级联样本，而非简单二元对，从而更好地分离与行为相关的线性特征。

Result: 级联样本发现的谄媚特征形成线性可分子空间，支持检测、确定性评分和鲁棒引导，在性能和可解释性上优于基线方法，且计算需求更低。

Conclusion: 级联采样方法能有效解耦特征，提升模型行为检测和引导的可靠性与效率。
...
### [193] [Life After Benchmark Saturation: A Case Study of CORE-Bench](https://arxiv.org/abs/2606.26158)
...
TL;DR: 提出了一种超越准确率的多元代理性能评估框架，并以CORE-Bench为例展示了其在准确率饱和后仍能提供有意义的洞察。


<details>
  <summary>Details</summary>
Motivation: 现有基准在准确率饱和后常被废弃，忽略了构造有效性、泛化性、效率、可靠性、模型与支架贡献以及人机协作提升等关键维度。

Method: 以CORE-Bench Hard为案例，暴露构造有效性威胁，改进得到CORE-Bench v1.1和OOD套件；测量效率、可靠性、模型与支架性能；并开展小规模随机实验评估人机协作提升。

Result: 准确率饱和后，CORE-Bench v1.1仍有效衡量其他维度；人机协作实现约两倍的统计显著加速（可能被低估）。

Conclusion: 提供了比以准确率为中心的评估范式更严谨的替代方案。
...
### [194] [Refusal Lives Downstream of Persona in Chat Models](https://arxiv.org/abs/2606.26161)
*Viola Zhong,Qirui Li*

Main category: cs.AI

TL;DR: 指令调优模型中的拒绝行为受顺从人格门控，且仅在后期层表达阶段生效。


<details>
  <summary>Details</summary>
Motivation: 先前研究将拒绝和人格视为独立机制，未探索二者交互。

Method: 在Qwen2.5-7B-Instruct和Llama-3.1-8B-Instruct中提取顺从人格方向和拒绝方向，进行干预（引导、投影等）。

Result: 顺从人格引导将Llama拒绝率从97%降至2%；后期层重新引入拒绝方向部分恢复拒绝；投影出人格方向恢复基线，随机方向无效。

Conclusion: 拒绝依赖于人格门控，且仅在后期层表达阶段受调节，不可视为孤立方向。
...
### [195] [AlgoEvolve: LLM-driven Meta-evolution of Algorithmic Trading Programs](https://arxiv.org/abs/2606.26173)
*Dhruv Sharma,Gautam Shroff*

Main category: cs.AI

TL;DR: 提出AlgoEvolve框架，利用大语言模型进行语义变异，在算法交易领域进化生成交易策略，并通过元进化优化提示，实现自适应策略和更好性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法多用于静态编程基准，算法交易领域存在噪声、非平稳和高度不连续等挑战，需要新的方法。

Method: 采用LLM驱动进化框架，内循环生成和评估Python交易策略，外循环进化生成策略的提示，以改进搜索启发式。

Result: 系统表现出涌现的自适应策略逻辑，包括自主调整交易规则；元进化发现的启发式平衡探索与开发，减少零交易失败，优于人工设计。
...
### [196] [Agentic Analysis for Agentic Infrastructure: An LLM-Powered Pipeline for Comparative Governance of DAO and Corporate AI Protocols](https://arxiv.org/abs/2606.26203)
*Yutian Wang,Luyao Zhang*

Main category: cs.AI

TL;DR: 提出LLM驱动的比较管道，分析AI代理协议治理结构，发现开放治理促进话语对齐但参与不平等仍存在。


<details>
  <summary>Details</summary>
Motivation: AI代理协议增多，但治理结构对互操作性标准的影响缺乏实证研究。

Method: 整合自动标注、神经主题建模和多层网络分析的LLM比较管道。

Result: 两种标准在参与不平等和社区碎片化程度上相似，但开放标准下话语对齐更密集。

Conclusion: LLM辅助方法可推进技术治理实证研究，有助于设计更公平的代理AI标准。
...
### [197] [Knowledge-augmented Agentic AI for Mental Health Medication Information Seeking](https://arxiv.org/abs/2606.26205)
...
TL;DR: 开发了一个基于知识图谱的多智能体框架，整合Reddit、WebMD和FDA FAERS数据，用于抗抑郁药的安全信息，确保来源可追溯。


<details>
  <summary>Details</summary>
Motivation: 患者在线寻求药物信息，但精神科药物的安全知识分散在权威但抽象的监管记录和贴近经验但未经验证的患者叙述中，整合时需避免混淆证据和轶事。

Method: 构建了基于知识图谱的多智能体框架，整合466,525条Reddit帖子、60,782条WebMD评论和二十年的FDA FAERS数据，使用大语言模型实体识别管线，并与医生标注进行基准测试。

Result: LLM实体识别在药物和条件上分别达到最高F1分数0.969和0.973；两个社区平台之间的Jaccard相似度高达0.905，但与监管报告一致性低；舍曲林的许多不良事件在社区中出现比FDA记录早数百天。

Conclusion: 来源感知的整合方法为更可审计的精神科药物信息提供途径，其有用性和患者获益需进一步前瞻性测试。
...
### [198] [Accelerating Skill Assessment in Chess: A Drift-Diffusion-Enhanced Elo Rating System](https://arxiv.org/abs/2606.26267)
*Tianyuan Zhou,Zhizheng Fu,Tianming Yang*

Main category: cs.AI

TL;DR: 提出DD-Elo评级系统，通过整合逐步棋数据，比传统Elo更快适应技能变化。


<details>
  <summary>Details</summary>
Motivation: Elo系统仅依赖比赛结果，存在反应滞后，无法利用对局质量。

Method: 借鉴认知神经科学中的漂移扩散模型（DDM），建模技能表达为决策过程，整合逐步数据，数学证明与Elo偏差有界。

Result: 实验表明DD-Elo比Elo更快适应技能变化，解释性强，响应快，向后兼容。

Conclusion: DD-Elo为国际象棋评级生态提供可解释、高响应且向后兼容的解决方案。
...
### [199] [Governing Actions, Not Agents: Institutional Attestation as a Governance Model for Autonomous AI Systems](https://arxiv.org/abs/2606.26298)
*Jakob Salfeld-Nebgen*

Main category: cs.AI

TL;DR: 提出一种计算治理模型，通过独立证据约束AI代理的高风险行动执行权，保留规划自主性。


<details>
  <summary>Details</summary>
Motivation: 自主AI代理可能执行不可逆行动，需要治理，但监控推理不可行，借鉴人类机构的独立证据要求。

Method: 代理保留规划自主权但无执行权；执行需满足独立来源证明的预条件，加密绑定声明意图，确定性策略评估，决策记录于防篡改日志。

Result: 提供概念验证实现，并用软件部署和临床处方示例说明模型可行性。

Conclusion: 该模型可有效治理高风险行动，平衡自主性与安全性。
...
### [200] [COrigami: An AI Pipeline for Co-Designing Flat-Foldable Visually Recognisable Origami](https://arxiv.org/abs/2606.26299)
...
TL;DR: 提出COrigami管道，从自然语言生成满足几何约束和美学要求的折纸折痕图案，通过多阶段处理和强化学习优化。


<details>
  <summary>Details</summary>
Motivation: 生成艺术在满足几何约束和主观美学方面存在挑战，尤其是在计算折纸这一数学严谨环境中。

Method: 端到端AI管道：生成语义骨架图、计算基础包装、求解可折叠折痕图案、成型，并通过自主美学评估循环的强化学习优化。

Result: 系统作为高效协作助手，为人类艺术家提供结构起点，支持进一步创作。

Conclusion: 通过集成算法优化和自主美学评判，AI系统能够满足多目标物理约束，实现数学基础的可靠共同创造力。
...
### [201] [The Verification Horizon: No Silver Bullet for Coding Agent Rewards](https://arxiv.org/abs/2606.26300)
...
TL;DR: 验证比生成更难，因为意图不明确且优化会拉开代理与意图的差距。提出验证信号的三维度：可扩展性、忠实性、稳健性，并研究四种奖励构建，实验表明针对性验证设计可抑制奖励黑客并提升任务质量。


<details>
  <summary>Details</summary>
Motivation: 生成复杂候选解已不困难，但可靠验证成为更难问题，因为验证器只是人类意图的代理，而意图本身未充分说明，且优化会加剧代理与意图的差距。

Method: 沿可扩展性、忠实性、稳健性三维度，研究四种奖励构建：通用编码任务的测试验证器、前端任务的rubric验证器、真实世界代理任务的用户验证器、长时域任务的自动代理验证器，并进行深入分析和实验。

Result: 针对性验证设计有效抑制奖励黑客，提升任务完成质量，在多个内部和公共基准上获得显著收益。
...
### [202] [How Do Tool-Augmented LLM Agents Perform on Real-World Energy Analytics Tasks?](https://arxiv.org/abs/2606.26346)
*David Akinpelu,Akintonde Abbas,Rereloluwa Alimi,Ayodeji Lana*

Main category: cs.AI

TL;DR: 针对能源领域缺乏动态代理基准的问题，提出了一个基于工具增强LLM的评估环境，包含243个专家策展问题和多维度评估协议，并对闭源与开源模型进行了对比分析。


<details>
  <summary>Details</summary>
Motivation: 现有能源领域评估仅限于静态知识回忆，无法满足实时数据检索、监管知识及多步推理的实际需求，存在关键空白。

Method: 构建包含市场数据检索、知识解释、高级定量建模三类共243个问题的评估环境；配置实时API、监管搜索、关税数据库等工具；采用方法正确性、答案准确性、属性对齐、来源有效性多维度评分，并基于类别路由匹配评分标准。

Result: 对闭源和开源LLM进行对比分析，展示了模型能力与领域工具交互的效果。

Conclusion: 公开发布关键工件以支持可重复性和未来研究。
...
### [203] [What We are Missing in Multimodal LLM Evaluation?](https://arxiv.org/abs/2606.26348)
*Po-han Li,Shenghui Chen,Sandeep Chinchali,Ufuk Topcu*

Main category: cs.AI

TL;DR: 本文审视了当前多模态大语言模型评估方法的不足，指出现有基准测试存在时空连贯性、物理世界理解、多模态一致性和选择性注意等方面的空白。填补这些空白对衡量多模态智能的真实进展至关重要。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型能力快速进步，但评估方法滞后，现有基准测试孤立且无法衡量跨模态信息整合。

Method: 系统梳理现有评估方法和基准分类，识别评估空白。

Result: 发现时空连贯性、物理世界理解、多模态一致性和选择性注意等关键评估维度缺失。
...
### [204] [OpenFinGym: A Verifiable Multi-Task Gym Environment for Evaluating Quant Agents](https://arxiv.org/abs/2606.26350)
*Kaicheng Zhang,Wen Ge,Lei Jiang,Weixin Yang,Jordan Langham-Lopez,Jialin Yu,Lukasz Szpruch,Hao Ni*

Main category: cs.AI

TL;DR: OpenFinGym是一个统一的量化金融智能体开发环境，覆盖预测、市场生成、实时交易和欺诈检测等任务，并提供自动化任务构建、容器化运行时和纸交易引擎。


<details>
  <summary>Details</summary>
Motivation: 现有平台通常只关注单一任务，可能高估智能体能力，无法揭示其在泛化、真实市场交互和财务决策方面的弱点。

Method: OpenFinGym整合了多种任务，提供自动化任务构建流水线（从量化金融论文生成可执行任务包）、容器化运行时（带主机端验证服务，防止数据泄露）、低延迟纸交易引擎、延期解析支持以及SFT和RL后训练集成。

Result: 该环境弥补了单一任务评估的不足，支持可扩展的智能体部署和更全面的能力测试。

Conclusion: OpenFinGym为量化金融智能体的统一开发与评估提供了平台，有助于提升金融智能体的泛化能力和实际应用价值。
...
### [205] [Instruction Bleed: Cross-Module Interference in Prompt-Composed Agentic Systems](https://arxiv.org/abs/2606.26356)
*Ching-Yu Lin,Yifan Liu*

Main category: cs.AI

TL;DR: 提示组合代理系统存在一个故障模式：编辑一个提示模块会静默改变其他模块的行为，即使没有共享变量或可执行依赖。作者将其形式化为组合行为泄漏（CBL），并通过实验表明CBL是一种亚阈值效应，与已知故障轴正交，需要建立跨模块干扰测量。


<details>
  <summary>Details</summary>
Motivation: 实际部署中，编辑一个提示模块会意外影响其他模块行为，说明存在未被形式化的干扰现象，需要系统研究。

Method: 设计一个可复用的三通道协议，分别扰动非焦点模块的体量、内容和形式，在部署的作业评估代理（Claude Sonnet 4.6，144次试验）上探测CBL。

Result: 仅内容通道产生可检测的配对效应（Cohen's d = 0.63，bootstrap 95% CI不包含零），但未见推荐翻转，属于亚阈值无可见影响的干扰。

Conclusion: CBL与已知的代理故障轴（对抗注入、认知退化、多代理故障传播、隐私泄露）正交，提示跨模块干扰测量应成为提示组合代理评估的必要环节。
...
### [206] [Accelerating Returns and the Qualitative Engine for Science](https://arxiv.org/abs/2606.26359)
...
### [207] [Narration-of-Thought: Inference-Time Scaffolding for Defeasible Ethical Reasoning in Large Language Models](https://arxiv.org/abs/2606.26366)
*Patrick Cooper,Alvaro Velasquez*

Main category: cs.AI

TL;DR: 提出叙事思维（NoT），通过结构化提示词解决标准思维链在道德困境中的利益相关者坍塌和不确定性抑制问题，无需训练即显著改善，并扩展到多轮辩论协议。


<details>
  <summary>Details</summary>
Motivation: 标准思维链在道德困境中常出现利益相关者坍塌（仅提及一方）和不确定性抑制（无明确未知或保留），导致推理不完整。

Method: NoT将思维链强制分为五个部分：主角、利益相关者、两步后果、不确定性、承诺，作为系统提示词，无需额外训练或参数调整。

Result: 在100个日常困境上，NoT将利益相关者坍塌从最高31%降至<1%，不确定性抑制从最高72%降至1-24%；消融实验证实每个部分的有效性，且跨族训练法官优于同族。

Conclusion: NoT外部化了决策依据的利益相关者、后果和不确定性，为可审计的智能体部署提供了基础。
...
### [208] [Geometry-Aware MCTS for Extremal Problems in Combinatorial Geometry](https://arxiv.org/abs/2606.26399)
*Luoning Zhang,Xu Zhuang,Tianhao Wang,Nathan Kaplan*

Main category: cs.AI

TL;DR: 提出几何感知蒙特卡洛树搜索框架，解决组合几何极值问题，在多个问题上取得新最佳结果。


<details>
  <summary>Details</summary>
Motivation: 经典精确求解器存在组合爆炸，强化学习和Transformer模型面临稀疏奖励和令牌限制。

Method: 几何感知MCTS，通过增量更新可行动作空间严格约束几何约束，利用几何对称性进行规范剪枝和对称批量过渡。

Result: 在六个问题中的五个上取得新最佳计算成果，Max-N3IL找到约1.8n的配置，Smallest Complete Set找到约0.95n的配置。

Conclusion: 几何感知MCTS是适应性强、能发现新配置的框架。
...
### [209] [When Agents Meet Electric Bus Fleet Operations: Pricing Behavior, Trade-offs, and Policy Implications in an Aggregator Framework](https://arxiv.org/abs/2606.26400)
*J\^onatas Augusto Manzolli,Ali Eslami,Luis Miranda-Moreno,Jiangbo Yu*

Main category: cs.AI

TL;DR: 提出了一个代理聚合器框架，用于电动公交车队的调度与V2G协调，在实现自适应调度的同时揭示了利润导向与运营导向之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 电动公交车队的运行需要持续协调服务可靠性、电池电量、充电桩可用性、电价、路线能量不确定性和V2G机会，现有系统难以应对这种复杂性。

Method: 将基于优化的电动公交车调度模型与代理层耦合，代理层负责检测扰动、调整电价、评估调度方案，并在必要时触发重新优化，同时定义灵活性价值的分配方式。

Result: 在真实场景下测试了利润驱动和运营驱动两种模式，结果表明代理聚合器能够维持可行调度、选择性重新优化并改善充电和V2G灵活性利用，但利润导向的配置会从公交运营商处提取价值。

Conclusion: 代理聚合器有助于管理电动公交车的V2G运营，但在公共车队中部署需要透明的协调模式、可审计的电价设定和明确的价值共享规则。
...
### [210] [Unbiased Canonical Set-Valued Oracles Via Lattice Theory](https://arxiv.org/abs/2606.26418)
*Jobst Heitzig*

Main category: cs.AI

TL;DR: 针对预言机AI的自指问题，提出基于Knaster-Tarski不动点定理的规范可信集方法，避免反事实答案的无关性，证明存在性、自洽性和非空性，并扩展到任意随机变量。


<details>
  <summary>Details</summary>
Motivation: 非智能预言机AI的概率估计存在自指问题：答案被学习后可能改变所估计的概率。反事实问题虽能避免，但答案一旦被学习便因前提虚假而失效。

Method: 采用自指替代方案：报告一个同时无偏且与学习后果自洽的可信集。利用Knaster-Tarski不动点定理，在封闭可信集完全格上定义保序算子，取最小不动点；变体取包含所有自洽点估计的最小不动点。

Result: 证明了存在性、自洽性和非空性；对非表演性问题退化为经典点估计；对二元事件，在自然凸包分解假设下规范答案为区间。

Conclusion: 该构造基于格论，可从二元事件推广到任意随机变量，但区间特征是否在推广中成立仍为开放问题。
...
### [211] [Estimating Uncertainty in Classifier Performance with Applications to Large Language Models and Nested Data](https://arxiv.org/abs/2606.26422)
*Kylie Anglin*

Main category: cs.AI

TL;DR: 本文评估了社会科学文本分类中性能指标置信区间方法的准确性，推荐了Agresti-Coull等改进方法，并提出了针对嵌套数据的调整策略。


<details>
  <summary>Details</summary>
Motivation: 当前文本分类中性能指标的点估计常缺乏不确定性报告，且常用方法在小样本或高性能下不准确。

Method: 通过模拟实验，比较了Wald、Agresti-Coull、Wilson、Clopper-Pearson、伪计数正则化bootstrap、分层bootstrap等区间方法在典型条件下的表现。

Result: Wald和基础百分位bootstrap覆盖度低；Agresti-Coull等较准确；嵌套数据需调整有效样本量和自由度；分层bootstrap在中等文本量时优于聚类bootstrap，但少量文本时保守。

Conclusion: 应使用更准确的置信区间方法，提高报告透明度，并在设计阶段重视验证样本量。
...
### [212] [Data-driven Machine Learning Cannot Reach Symbolic-level Logical Reasoning -- The Limit of the Scaling Law](https://arxiv.org/abs/2606.26454)
*Tiansi Dong,Mateja Jamnik,Pietro Li\`o*

Main category: cs.AI

TL;DR: 本文指出监督深度学习在符号级三段论推理上的两个方法论局限，实验表明Euler Net和ChatGPT均无法达到严格推理，结论是监督机器学习无法达到符号逻辑推理的严谨性。


<details>
  <summary>Details</summary>
Motivation: 探讨数据驱动的机器学习系统能否通过增加训练数据和训练时间达到符号级别的演绎推理能力。

Method: 理论分析监督深度学习的两个局限，并通过Euler Net实验和ChatGPT（GPT-5-nano和GPT-5）的多种表面形式测试进行验证。

Result: Euler Net无法实现严格三段论推理；ChatGPT准确率受表面形式影响，GPT-5可达100%但解释错误。
...
### [213] [MKG-RAG-Bench: Benchmarking Retrieval in Multimodal Knowledge Graph-Augmented Generation](https://arxiv.org/abs/2606.26458)
*Xiaochen Wang,Bao Hoang,Han Liu,Ting Wang,Fenglong Ma*

Main category: cs.AI

TL;DR: 提出了MKG-RAG-Bench，一个专门评估多模态知识图谱检索增强生成中检索环节的基准，通过实验表明多模态检索仍是瓶颈且对生成质量至关重要。


<details>
  <summary>Details</summary>
Motivation: 现有基准忽视了多模态知识图谱RAG中检索的挑战，而多模态检索是实际应用的关键瓶颈。

Method: 基于通用和医疗两个多模态知识图谱，利用LLM构建流程过滤低效知识、生成结构对齐的查询并覆盖多种模态配置，创建了跨领域基准。

Result: 多模态检索仍然困难但对端到端RAG性能至关重要，检索质量强烈影响生成结果。

Conclusion: 将检索作为独立评估目标，为诊断当前局限和推进多模态知识图谱RAG系统提供了基础。
...
### [214] [auto-psych: Automating the science of mind using agent-driven theory discovery and experimentation](https://arxiv.org/abs/2606.26460)
*Ben Prystawski,Kushin Mukherjee,Daniel Wurgaft,Linas Nasvytis,Michael Y. Li,Noah D. Goodman,Michael C. Frank*

Main category: cs.AI

TL;DR: 提出一个名为auto-psych的自动化系统，通过嵌套的智能体循环在计算认知科学中自动收集数据并发现理论，能快速恢复真实理论并优于文献理论。


<details>
  <summary>Details</summary>
Motivation: 数据收集是AI科学自动化中的主要瓶颈，而心理学特别是计算认知科学因理论可编码和众包平台具备优势，需要自动化实验方法。

Method: 采用嵌套的智能体发现循环：内循环猜想来拟合概率认知模型，外循环设计实验、在线启动并分析数据。

Result: 通过系统实验能快速可靠地从合成数据恢复真实理论，嵌套结构对性能关键；在三组人类实验中，发现的模型比文献理论拟合数据更好。

Conclusion: 证明了在计算认知科学中自动数据收集和理论发现的可行性。
...
### [215] [Clinical Harness for Governable Medical AI Skill Ecosystems](https://arxiv.org/abs/2606.26494)
*Tianhan Xu,Lei Bao,Yongxiang Wang*

Main category: cs.AI

TL;DR: 提出临床AI技能和Clinical Harness运行时治理架构，以支持持续、可信的临床AI能力。


<details>
  <summary>Details</summary>
Motivation: 当前医疗AI以孤立模型为主，缺乏持久且可问责的临床能力。

Method: 设计Clinical Harness运行时治理架构，用于注册、编排、防护和监控AI驱动的临床技能。

Result: 以骨质疏松为例，展示了知识驱动、数据驱动和物理增强型技能在生命周期管理中的有效性。

Conclusion: 该架构能有效支持临床AI技能在运行时治理下的全周期应用。
...
### [216] [Humans Disengage, Reasoning Models Persist: Separating Difficulty Registration from Deliberation Allocation](https://arxiv.org/abs/2606.26502)
*Han-yu Wang*

Main category: cs.AI

TL;DR: 大型推理模型（LRM）在难题上花费更多token，但在同一问题上，错误回答时消耗的token比正确回答更多；人类则相反，错误时用时更少。


<details>
  <summary>Details</summary>
Motivation: 探究人类与LRM在问题解决中的思考时间/资源分配差异，特别是跨项目和项目内的不同模式。

Method: 使用公开的人类-LRM匹配语料库，区分两种反思层次：注册（跨项目难度追踪）和分配（项目内正确与错误的资源差异），并进行固定效应分析。

Result: 人类与所有五种思考型LRM在跨项目上表现一致（注册），但在项目内出现分化：LRM在错误回答上使用更多token（Cohen's d=1.47-3.13），而人类错误时用时更少。

Conclusion: 人类模式反映“投入vs放弃”：在可能解决的问题上坚持，否则放弃；LRM模式反映长度由不确定性驱动：不确定时链条变长，往往导致失败。两种策略产生相同的跨项目相关性，但项目内控制不同。
...
### [217] [NeuraDock Visual Cognitive Load Agent Tutorial: A Quality-Gated Open-Source EEG Workflow for Alpha Dynamics and Real-Time Applications](https://arxiv.org/abs/2606.26518)
*Zhiyuan Xu,Yueqing Dai,Junling Li,Junwen Luo*

Main category: cs.AI

TL;DR: 本教程论文介绍了NeuraDock Agent，一个开源的EEG代理，提供从EEG预处理、质量控制、Alpha动态分析到实时API和LLM解释的完整工作流，旨在弥合离线分析到实时认知负荷应用之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有EEG工具包擅长离线分析，但构建实时、质量门控的认知负荷管道需要手动整合采集、QC、特征提取和Web API，缺乏系统性的解决方案。

Method: 采用质量门控工作流，在预处理和QC门控后才计算Alpha及认知负荷指标；通过可复现的步骤引导用户安装、运行、生成图表、进行比较，并启动在线仪表板和实时API。

Result: 在迷你数据集验证中，处理18个记录，生成10个被试内对比，其中7个任务显示出后部Alpha抑制，初步证明被试内重复性，并给出了本地API延迟的基准。

Conclusion: 该教程为研究人员和开发人员提供了一条从EEG文件到实时视觉认知负荷原型的透明、可复现路径。
...
### [218] [Boundary-Aware Context Grounding for A Low-Channel EEG Agent](https://arxiv.org/abs/2606.26519)
*Zhiyuan Xu,Yueqing Dai,Junling Li,Junwen Luo*

Main category: cs.AI

TL;DR: 提出NeuraDock Agent架构，通过分离本地EEG引擎和硬件感知语言层，实现低通道EEG数据的可靠解释，并在数值一致性、系统边界和边界感知方面验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 通用LLM无法自动知道传感器支持哪些测量、软件实现了哪些算法以及计算结果可得出哪些结论，尤其在低通道EEG中易产生错误解释。

Method: 设计分离架构：确定性本地EEG引擎负责数值处理和质量控制，LLM仅接收紧凑的允许列表摘要和版本化上下文包，原始数据保持本地。

Result: 12个录音的数值重复实验产生一致结果；请求捕获和故障注入实验确认数据边界和本地工件保护；边界感知基准测试在4种上下文消融和2个LLM下生成288个输出。

Conclusion: 支持硬件和实施感知的接地机制作为校准EEG代理接受、限定或拒绝的实用方法，但未建立临床有效性或验证的认知负荷指数。
...
### [219] [Radical AI Interpretability](https://arxiv.org/abs/2606.26523)
*Daniel A. Herrmann,Benjamin A. Levinstein*

Main category: cs.AI

TL;DR: 将AI系统解释为智能体的框架，结合激进解释的哲学传统和机械可解释性，解决从计算事实推导出信念、欲望和意义的问题。


<details>
  <summary>Details</summary>
Motivation: 需要信任AI系统，理解其目标或检测欺骗，但缺乏成功判定的公认方法。

Method: 提出表征主义和解释主义的标准，与可解释性测试关联，强调信念、欲望和命题结构的整体约束。

Result: 整体主义在AI系统中至关重要，机械可解释性可同时测量态度和命题结构。

Conclusion: 信念、欲望和命题结构不能孤立归因，整体约束提供了杠杆，机械可解释性有助于测量。
...
### [220] [PMDformer: Patch-Mean Decoupling Information Transformer for Long-term Forecasting](https://arxiv.org/abs/2606.26549)
*Ao Hu,Liangjian Wen,Jiang Duan,Yong Dai,He Yan,Dongkai Wang,Jun Wang,Yukun Zhang,Ruoxi Jiang,Zenglin Xu*

Main category: cs.AI

TL;DR: 提出PMDformer，通过补丁均值解耦、趋势恢复注意力和邻近变量注意力，提升长期时间序列预测的稳定性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的模型在建模补丁和变量间的形状相似性时受尺度差异影响，难以准确捕捉长期依赖关系。

Method: 引入补丁均值解耦（PMD）分离趋势与残差，提出趋势恢复注意力（TRA）和邻近变量注意力（PVA）分别处理趋势整合和跨变量局部相关性。

Result: 在多个LTSF基准上，PMDformer在稳定性和准确性上超越现有最先进方法。

Conclusion: PMDformer有效解决了形状相似性建模问题，为长期时间序列预测提供更鲁棒的解决方案。
...
### [221] [Explainable Ensemble-Based Machine Learning Models for Detecting the Presence of Cirrhosis in Hepatitis C Patients](https://arxiv.org/abs/2606.26561)
...
TL;DR: 使用机器学习模型检测丙型肝炎患者的肝硬化，其中极端随机树模型表现最佳，准确率达96.92%。


<details>
  <summary>Details</summary>
Motivation: 丙型肝炎可导致肝硬化，早期检测对避免并发症至关重要，但目前尚无研究利用机器学习检测丙型肝炎患者的肝硬化。

Method: 从UCI机器学习库获取2038名埃及患者的28个属性数据集，训练随机森林、梯度提升机、极端梯度提升和极端随机树四种模型，并筛选特征。

Result: 极端随机树模型在仅使用16个特征时达到96.92%的准确率、94.00%的召回率、99.81%的精确率和96%的AUC。

Conclusion: 机器学习能有效诊断丙型肝炎患者的肝硬化，极端随机树模型性能优于其他模型。
...
### [222] [EvoOptiGraph: Weakness-Driven Coevolution via Graph-Based Structural Generation for Optimization Modeling](https://arxiv.org/abs/2606.26578)
*Qingcan Kang,Mingyang Liu,Xiaojin Fu,Shixiong Kai,Tao Zhong,Mingxuan Yuan*

Main category: cs.AI

TL;DR: 提出EvoOptiGraph框架，通过数据与模型协同进化，利用模型弱点驱动生成结构多样的混合整数线性规划实例，在优化建模任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有训练语料缺乏结构多样性，且数据生成流程静态、与模型学习脱节。

Method: 将MILP表示为属性二分图，应用保持有效性的进化算子生成多样实例，通过确定性编译和验证反向翻译转换为代码和自然语言；采用两阶段训练：先监督微调再强化学习，用图导出的弱点信号指导新实例生成。

Result: 在六个公开数据集上，在准确性、可执行性和泛化性上显著优于大型通用模型、代理方法和专用基线。

Conclusion: 目标驱动的数据-模型协同进化是提升LLM在优化建模任务上性能的有效策略。
...
### [223] [A Multi-Level Validation and Traceability Framework for AI-Generated Telescope Scheduling Decisions](https://arxiv.org/abs/2606.26585)
*Hengchu Xiao,Chuanjun Wang*

Main category: cs.AI

TL;DR: 提出多级验证和可追溯推理框架，提升AI望远镜调度的可靠性和可执行性。


<details>
  <summary>Details</summary>
Motivation: AI调度存在数据引用不一致、推理错误和不可执行决策，限制其在高可靠性观测任务中的应用。

Method: 框架包括数据引用验证、逻辑一致性检查、观测和仪器约束验证，以及原子推理单元及其依赖关系，将调度决策表示为可追溯的推理步骤。

Result: 实验表明框架提高了调度的可执行性和可靠性，减少了瞬态机会损失，并通过反馈修正和结构化验证增强了纠错和阻止错误决策的能力。

Conclusion: 该框架为AI应用于高可靠性天文观测调度提供了可行且可验证的路径，在保持灵活性的同时显著提升了可靠性和可执行性。
...
### [224] [Content-Based Smart E-Mail Dispatcher Using Large Language Models](https://arxiv.org/abs/2606.26593)
*K. Paramesha,K R Sriram,Sujan Shetty,Shamanth Kishore,R.Tejaswini*

Main category: cs.AI

TL;DR: 提出一种基于大语言模型代理的电子邮件自动分发系统，将邮件内容路由到相关WhatsApp群组，无需标注数据。


<details>
  <summary>Details</summary>
Motivation: 大型机构中手动处理邮件并转发到即时通讯平台耗时且易错，影响生产力并增加压力。

Method: 构建代理系统，利用大语言模型分析邮件内容，通过结构化提示框架指导决策，自动识别并分发到目标群组。

Result: 系统成功实现邮件自动分发，不依赖标注数据集。

Conclusion: 提升生产力，降低处理邮件的认知负荷。
...
### [225] [LLM-based Models for Detecting Emerging Topics in Service Feedback](https://arxiv.org/abs/2606.26595)
*Mahsa Tavakoli,Ruth Bankey,Cristi\'an Bravo*

Main category: cs.AI

TL;DR: 本文提出一种集成大语言模型、统计技术和人机协作的方法，用于分析多语言客户反馈，检测新兴服务质量问题和潜在不公平，提升公共部门决策的公平性和效率。


<details>
  <summary>Details</summary>
Motivation: 传统服务反馈分析依赖人工或静态指标，难以应对反馈量增长和复杂模式识别，尤其影响税务等公共部门的信任与合规。

Method: 结合微调和量化的大语言模型与专家监督，通过相似度分析和税务官员评估，采用人机协作框架减少模型虚构。

Result: 与基线模型相比，该方法在相似度分析和专家评估中表现出更强的一致性，提高了分析的可靠性和相关性。

Conclusion: 证明了LLM与人类专家结合可实现可扩展、基于证据的决策，促进负责任AI系统在公共部门的应用，提升服务质量和公平性。
...
### [226] [Autoformalization of Agent Instructions into Policy-as-Code](https://arxiv.org/abs/2606.26649)
*Adam Mondl,Matthew Maisel,John H. Brock*

Main category: cs.AI

TL;DR: 提出自动形式化管道，通过LLM生成-批评循环将自然语言政策转化为形式化Cedar策略，在MedAgentBench上显著提升覆盖。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么缺乏形式保证（概率性护栏），要么手工编码难以扩展到真实政策规范。

Method: 构建LLM生成-批评循环，自动将代理提示、MCP工具描述和自然语言政策文档翻译为Cedar策略语言。

Result: 在MedAgentBench基准上，自动形式化策略覆盖的源自然语言规范远多于先前的手工编码符号执行方法。

Conclusion: 该方法可有效提升策略覆盖，为高风险领域提供更可靠的形式化政策执行。
...
### [227] [SKILL-DISCO: Distilling and Compiling Agent Traces into Reusable Procedural Skills](https://arxiv.org/abs/2606.26669)
*Zhongxin Guo,Danrui Qi,Hanwen Gu,Peng Cheng,Yongqiang Xiong*

Main category: cs.AI

TL;DR: 提出SkillDisCo框架，通过蒸馏和编译从成功轨迹中提取可重用过程技能，提升代理效率。


<details>
  <summary>Details</summary>
Motivation: 代理重复从头解决相似任务导致不必要推理成本和长执行轨迹。

Method: 在FSM定义场景中，将成功轨迹视为未知转移图中的路径，形式化过程技能为可重用参数化控制流子图，通过蒸馏编译框架SkillDisCo提取并编译这些子图。

Result: 在ALFWorld和WebArena上，SkillDisCo提高了成功率并减少了代理步骤。

Conclusion: 展示了将共享经验表示为可重用执行结构的益处。
...
### [228] [NebulaExp-8B: An Empirical Post-Training Pipeline via Full-Scale Ablation Research](https://arxiv.org/abs/2606.26671)
*Qiaobo Hao,Yangqian Wu,Shunyi Wang,Zhongjian Zhang,Ziqun Li,Yayin He,Muqing Li,Chen Zhong*

Main category: cs.AI

TL;DR: 提出完全透明的后训练流水线NebulaExp，基于Qwen3-8B-base，包括指令和推理两个分支，通过数据筛选、三阶段SFT和GRPO RL提升性能，并探索OPD方法，最终提供可复现的实证配方。


<details>
  <summary>Details</summary>
Motivation: 现有后训练工作缺乏详细的数据构建、过滤规则和训练方法，阻碍了社区复现和轻量模型优化。

Method: 构建3.84M多源SFT样本和200K可验证RL候选池，设计端到端数据处理流程（响应蒸馏、多维交叉验证过滤、细粒度难度分级、任务分类和多样性采样）。指令分支采用三阶段优化SFT（NebulaExp-Ins-SFT）和GRPO RL；推理分支采用中等难度GRPO RL；并探索单教师和多教师OPD（MOPD）方法。

Result: 指令分支：基线55.01→SFT 60.99→RL 61.85；推理分支：73.88→75.17；MOPD用4K指令样本在IFEval上超过RL基线3.26分，总体平均提升4.43；用10K样本平均提升4.18。

Conclusion: 为8B规模LLM提供了完全可复现的后训练实证配方，并全面剖析了指令遵循、数学推理、代码生成和通用知识之间的能力权衡。
...
### [229] [Do Safety Guardrails Need to Reason? LeanGuard: A Fast and Light Approach for Robust Moderation](https://arxiv.org/abs/2606.26686)
*Dongbin Na*

Main category: cs.AI

TL;DR: 提出LeanGuard，一种轻量级双向编码器安全护栏，无需链式推理（CoT）即可达到与大型推理护栏相当的准确率，推理计算量减少约100倍。


<details>
  <summary>Details</summary>
Motivation: 现有护栏方法使用CoT推理导致模型笨重缓慢，不适合边缘部署，质疑安全护栏是否真的需要推理。

Method: 在相同语料上训练轻量级双向编码器和推理护栏，仅移除推理部分保持其他不变，进行对比实验。

Result: 395M参数的标签编码器平均F1为82.90±0.26，匹配更大的推理护栏，推理计算量减少约100倍，且在标签噪声下更鲁棒，低假阳性率下召回更高。
...
### [230] [Kalman Prototypical Networks for Few-shot Fault Detection in Combined Cycle Gas Turbines](https://arxiv.org/abs/2606.26710)
*Mohammed Ayalew Belay,Lucas Ferreira Bernardino,Adil Rasheed,Rub\'en M. Monta\~n\'es,Pierluigi Salvo Rossi*

Main category: cs.AI

TL;DR: 提出卡尔曼原型网络（KPN），一种基于度量的少样本学习框架，用于联合循环燃气轮机（CCGT）故障诊断，通过将类原型建模为动态系统中的潜在随机状态，提高了嵌入表示的鲁棒性并减少了情节方差。


<details>
  <summary>Details</summary>
Motivation: CCGT故障检测中标记故障数据稀缺，传统的故障检测方法面临挑战。

Method: KPN框架：将类原型的演化建模为动态系统中的潜在随机状态，结合卡尔曼滤波进行原型更新，利用高保真Modelica仿真生成合成数据集，模拟正常和渐进泄漏故障。

Result: 在模拟泄漏故障检测任务中，KPN在准确性和稳定性上优于匹配网络、关系网络和MAML等传统少样本学习方法。

Conclusion: KPN通过稳定类表示显著改善了训练收敛和泛化能力，适用于标记数据有限的实际CCGT故障检测场景。
...
### [231] [LithoDreamer: A Physics-Informed World Model for Multi-Stage Computational Lithography](https://arxiv.org/abs/2606.26713)
*Yuqi Jiang,Yumeng Liu,Zimu Li,Jinyuan Deng,Qian Jin,Yucheng Cui,Yu Li,Xunzhao Yin,Qi Sun,Cheng Zhuo*

Main category: cs.AI

TL;DR: LithoDreamer 是首个物理信息驱动世界模型框架，将计算光刻中的布局-掩模-抗蚀剂图像-显影后图像流程建模为多步演化系统，通过对比变分优化实现高保真预测与逆向规划。


<details>
  <summary>Details</summary>
Motivation: 现有计算光刻模型无法捕捉掩模优化、光学成像、抗蚀剂曝光和显影等连续物理过程，导致仿真精度不足。

Method: 提出物理信息世界模型LithoDreamer，建模相邻状态间的特征变化以学习阶段特定潜空间，并通过对比变分优化范式在无连续监督下实现可解释干预优化。

Result: 在前向演化与逆向规划任务中均取得最先进性能，并公开了光刻数据集。

Conclusion: LithoDreamer通过物理信息驱动的多步演化框架有效提升了计算光刻的精度与可解释性。
...
### [232] [A Latent ODE Approach to Spatiotemporal Modeling of Cine Cardiac MRI](https://arxiv.org/abs/2606.26718)
*David Br\"uggemann,Ekaterina Krymova,Firat \"Ozdemir,Jochen von Spiczak,Sebastian Kozerke,Samia Mora,Robert Manka,Mathieu Salzmann,Olga V. Demler*

Main category: cs.AI

TL;DR: 提出了一种潜伏动态模型，通过心率感知神经ODE和图网格自编码器对双心室全周期运动进行连续建模，在UK Biobank数据上预测心衰风险，显著提升了C指数。


<details>
  <summary>Details</summary>
Motivation: 传统心脏MRI风险模型仅使用少数心肌指数，忽视了完整的时空运动信息。

Method: 采用心率感知神经ODE驱动连续潜伏轨迹，结合图网格自编码器重建3D+t心室运动，并利用Cox比例风险模型评估潜伏偏差与心衰的关系。

Result: 在72,386人（367心衰事件）中，模型将分层C指数从0.704提升至0.785，优于7个传统指标（0.764）。

Conclusion: 连续全周期运动建模能提供超越常规MRI的丰富心脏表型，但临床使用前需外部验证。
...
### [233] [Socratic agents for autonomous scientific discovery in high-dimensional physical systems](https://arxiv.org/abs/2606.26722)
*Xianrui Zeng,Pengfei Liu,Yirui Zang,Yang Shen,Fei Yu,Chunlei Yu,Minghao Liu,Yang Du*

Main category: cs.AI

TL;DR: AHOIS 是一个多智能体 AI 科学家，通过苏格拉底式批评代理在闭环实验中实现自主科学发现，并在光纤平台上成功验证。


<details>
  <summary>Details</summary>
Motivation: 现有 AI 系统缺乏认知自主性，无法自主构建和修正物理解释。

Method: 采用多智能体架构，包含物理学批评代理，通过因果提问、约束检查、反例生成和伪造标准制定来迭代检验假设。

Result: 在光纤平台上自主提出随机干扰编码假设，发现自适应稀疏测量策略，诊断失效模式，翻译实验协议；测量有效秩 56.9，MNIST 分类准确率 76.97%，Fashion-MNIST 83.17%。

Conclusion: 苏格拉底式询问显著提升物理一致性、假设完整性、不确定性校准和实验计划有效性，推动从工作流自动化向证据驱动的自主科学发现。
...
### [234] [Scientific discovery as meta-optimization: a combinatorial optimization case study](https://arxiv.org/abs/2606.26728)
*Yuan-Hang Zhang,Chesson Sipling,Massimiliano Di Ventra*

Main category: cs.AI

TL;DR: 提出将科学发现形式化为元优化，通过共识目标聚合方法实现评估标准的自我修正，并在3-SAT问题上取得显著加速。


<details>
  <summary>Details</summary>
Motivation: 现有LLM自动探索理论实验空间，但评估标准也需要同时修改。

Method: 提出元优化框架，通过共识目标聚合，将LLM生成的目标函数用相关性加权投票结合，形成稳定自我修正的评估标准。

Result: 在3-SAT问题上，基线缩放从N^2.51降至N^1.33，最大实例加速约67倍。

Conclusion: 该框架与问题无关，有望大幅促进科学发现。
...
### [235] [EGG: An Expert-Guided Agent Framework for Kernel Generation](https://arxiv.org/abs/2606.26758)
*Yaochen Han,Ke Fan,Hongxu Jiang,Wanqi Xu,Weiyu Xie,Runhua Zhang,Chenhui Zhu,Yixiang Zhang*

Main category: cs.AI

TL;DR: 提出EGG框架，通过专家优化原则指导LLM生成高性能GPU内核，在KernelBench和真实任务上实现平均2.13倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有LLM生成内核的方法缺乏领域优化指导，难以同时实现正确性和高性能。

Method: 将内核生成分解为算法结构设计和硬件调优两阶段，并设计阶段感知的多智能体协作机制。

Result: 在KernelBench和真实工作负载上，EGG比PyTorch平均加速2.13倍，优于现有基于智能体和强化学习的方法。

Conclusion: 基于专家指导的分阶段生成策略能有效提升内核性能和优化效率。
...
### [236] [ResilPhase: Plug-and-Play Phase Mapping and Noise-Resilient Macro-Trajectory Extrapolation for Diffusion Acceleration](https://arxiv.org/abs/2606.26769)
*Qicheng Zhao,Yu Li,Qi Sun,Zheyu Yan*

Main category: cs.AI

TL;DR: ResilPhase通过稳定宏观轨迹外推和免导数拉格朗日插值，在加速扩散模型时保持高保真度。


<details>
  <summary>Details</summary>
Motivation: 现有“缓存再预测”方案加速DiT时，在高加速比下严重质量下降，原因是离散外推与连续扩散轨迹不对齐且数值不稳定。

Method: 将加速推理重构为ODE空间中的稳定宏观轨迹外推，利用全局漂移（GD）对齐预测，采用免导数重心拉格朗日外推器避免导数不稳定性，并提出有界相位映射正则化外推域。

Result: 在FLUX.1-dev和HunyuanVideo上，在激进加速比下实现了最先进的保真度。

Conclusion: ResilPhase是一种噪声鲁棒的加速框架，有效解决了扩散模型加速中的质量退化问题。
...
### [237] [Memory Depth, Not Memory Access: Selective Parametric Consolidation for Long-Running Language Agents](https://arxiv.org/abs/2606.26806)
*Haoliang Han*

Main category: cs.AI

TL;DR: 提出了记忆深度概念，通过循环漂移协议和EVAF机制，展示了选择性参数整合在持久目标导向行为中补充检索系统的能力。


<details>
  <summary>Details</summary>
Motivation: 长时语言代理仅依靠检索系统不足以决定哪些经历应持续影响行为，需要研究持久目标导向倾向的参数化存储。

Method: 设计循环漂移协议作为受控压力测试，卸载工作上下文后保持检索索引不变，评估基于惊喜和效价门控的LoRA整合机制EVAF在GPT-2、TinyLlama和Mistral-7B上的表现。

Result: 检索在浅层事实回忆中表现最佳（短期事实准确率0.956-0.973），EVAF在目标持久性和卸载后恢复中表现最好（0.812-0.904），且仅需每200事件2-3次参数写入。机制控制表明选择性整合可分解为选择和驱动两个可控维度。

Conclusion: 选择性参数整合提供了区别于检索系统的记忆深度，两者互补，但陈旧记忆失效仍是一个未解决的边界问题。
...
### [238] [KARLA: Knowledge-base Augmented Retrieval for Language Models](https://arxiv.org/abs/2606.26807)
*Francois Crespin,Fabian M. Suchanek (IP Paris,LTCI),Nils Holzenberger*

Main category: cs.AI

TL;DR: 提出一种方法，让LLM在生成时自动从知识库获取事实知识，无需重新训练即可更新事实，提升可解释性，并让小模型达到大模型的事实准确性。


<details>
  <summary>Details</summary>
Motivation: 解决LLM事实知识更新困难、缺乏透明度、大模型成本高的问题。

Method: 训练模型生成特殊token以触发知识库查询。

Result: 在短文本和长文本生成中改善事实依据，允许通过知识库编辑而非参数更新来实现事实修订。

Conclusion: 该方法有效提升事实准确性，并支持通过知识库更新来修正事实。
...
### [239] [Computational Analysis of Heart Rate Variability in Healthy Adults](https://arxiv.org/abs/2606.26816)
*Mar\'ia J. Lado,Arturo J. M\'endez,Leandro Rodriguez-Li\~nares,Baltasar Garc\'ia P\'erez-Schofield,Pedro Cuesta-Morales,Brais Iglesias-Otero,Xose A. Vila*

Main category: cs.AI

TL;DR: 该研究评估了40名健康成人的HRV指数，发现时域和非线性指数呈正态分布、稳定性好，而频域指数变异性高，并推荐了最佳代表HRV成分的指数。


<details>
  <summary>Details</summary>
Motivation: HRV分析是心脏生理状态的重要指标，但针对健康成人的HRV参数研究有限，缺乏金标准。

Method: 对40名健康成人（男女各20人，年龄30-50岁）进行信号处理和数据分析，分析时域、频域和非线性指数。

Result: 时域和非线性指数（特别是全局和LF）为正态分布，存在性别差异；多数指数稳定，但HF相关指数除外；HF相关指数高度相关，表明冗余；与Fantasia数据库比较，多数指数误差小于10%；时域和非线性指数间研究变异性低，频域指数变异性高。

Conclusion: 推荐ApEn、IRRR（全局变异）、HRVi、SD2（LF）、MADRR或rMSSD（HF）作为最佳代表HRV成分的指数，以提高临床和研究实用性。
...
### [240] [The Capability Frontier: Benchmarks Miss 82% of Model Performance](https://arxiv.org/abs/2606.26836)
*Bradley Fowler,Ryan Smith,Daniel Thi Graviet,William Myers,Joshua Greaves,Narmeen Fatimah Oozeer,Ant\'ia Garc\'ia,Philip Quirke,Amirali Abdullah,Fazl Barez,Shriyash Kaustubh Upadhyay*

Main category: cs.AI

TL;DR: 提出能力边界概念，通过组合多个模型和多次生成来更准确评估LLM能力，发现现有基准低估了高达82%的性能提升空间。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅测试单模型单次运行，忽略了模型特异性和多次采样优势，导致系统性地低估LLM在实际异构数据分布下的能力。

Method: 定义能力边界为帕累托前沿，通过oracle选择最优模型和生成，校正单模型低估和多次采样高估两种偏差。

Result: 在21个LLM和16个基准上，单模型校正减少54%错误率，再校正单次运行提升82%性能，SOTA准确率在85%成本降低下达到。

Conclusion: LLM集体能力被显著低估，对异构多领域场景的评估和部署具有重要启示。
...
### [241] [Context-Aware Synthesis of Optimization Pipelines for Warehouse Optimization](https://arxiv.org/abs/2606.26852)
*Janik Bischoff,Anne Meyer,Uta Mohring,Fabian Dunke,Maximilian Barlang,\"Ozge Nur Subas,Hadi Kutabi,Stefan Nickel,Kai Furmans*

Main category: cs.AI

TL;DR: 提出CASOP框架，用于在手动拣选仓库中自动合成和评估上下文相关的优化流水线。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏通用机制来确定算法配置、组合成有效流水线并评估性能。

Method: 构建模块化算法库、语义数据卡和算法卡、问题分类法、流水线合成器（识别适用算法并组合有效流水线）和流水线评估器。

Result: 在7个基准实例集上得到1,063,044条有效流水线，验证了框架的有效性。

Conclusion: CASOP支持研究人员和实践者自动合成和选择高性能的算法流水线，已开源。
...
### [242] [LCAi: Life Cycle Assessment with big data fusion and retrieval-augmented generation-assisted interpretation](https://arxiv.org/abs/2606.26857)
*Georgios Tsironis,Juan D. Medrano-Garcia,Gonzalo Guillen-Gosalbez*

Main category: cs.AI

TL;DR: 提出一种基于视角条件检索增强生成框架的生命周期评估解释方法，通过多视角检索和受控合成为决策提供可操作战略路径。


<details>
  <summary>Details</summary>
Motivation: 生命周期评估的解释阶段缺乏结构化机制，无法将量化改进机会转化为可行动的战略路径，尤其存在技术、社会和政策不确定性。

Method: 构建视角融合检索增强生成架构，包括三个步骤：场景锚定、视角特异性微查询与受控检索、中性合成，并采用GPT-5 nano作为推理模型。

Result: 在意大利苹果生产设施的氢能柴油减排案例中验证，证明该框架能降低幻觉风险并保持跨领域多样性。

Conclusion: 该框架支持将影响结果严谨转化为战略路径，为AI在生命周期评估中的规模化应用开辟新途径。
...
### [243] [AgentX: Towards Agent-Driven Self-Iteration of Industrial Recommender Systems](https://arxiv.org/abs/2606.26859)
...
TL;DR: 提出AgentX多智能体系统，自主闭环进行推荐实验的生成、实现、评估与学习，实现算法迭代的自我进化。


<details>
  <summary>Details</summary>
Motivation: 当前推荐算法迭代依赖人工，创新速度受限于人力，无法有效利用证据、计算和实验知识，存在结构性执行瓶颈。

Method: AgentX由四个紧密耦合的智能体组成闭环：头脑风暴智能体综合历史实验等生成可执行提案；开发智能体将提案转为生产代码并验证；评估智能体通过安全在线A/B实验并记录经验；进化层（SGPO）将执行轨迹转化为语义梯度更新，持续优化智能体自身。

Result: 系统能够以远超人工的规模和速度自主进行推荐实验，实现自动化且自改进的迭代过程。

Conclusion: AgentX通过多智能体协同与自我进化，从根本上重构了推荐算法的生产函数，使迭代从人工流程转向工业化研究循环。
...
### [244] [TAVR-VLM: Risk-Conditioned Causal Grounding for Hallucination-Resistant Report Generation](https://arxiv.org/abs/2606.26874)
*Zhixiang Lu,Xiwei Liu,Sifan Song,Changkai Ji,Anh Nguyen,Jionglong Su,Imran Razzak,Jinfeng Wang*

Main category: cs.AI

TL;DR: 提出TAVR-VLM框架，通过风险条件因果注意力（R-CGA）减少多模态大模型的诊断幻觉，在TAVR规划任务中达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型在高风险TAVR规划中易产生缺乏解剖基础的诊断幻觉，阻碍应用。

Method: 引入风险条件因果注意力（R-CGA），构建“风险→区域→词”的结构化基础路径，包括因果风险瓶颈和支持投影因果一致性目标。

Result: 在M³TAVR数据集上AUROC达0.896，CIDEr提升至0.936，幻觉率降至8.1%。

Conclusion: 该方法有效提升可解释性和可靠性，推动证据驱动的手术AI发展。
...
### [245] [A Pipeline for Generating Longitudinal Synthetic Clinical Notes Using Large Language Models](https://arxiv.org/abs/2606.26879)
*William Poulett*

Main category: cs.AI

TL;DR: 提出一种合成临床笔记的流水线和数据集，避免使用真实患者数据的隐私风险，用于临床AI工具的开发与评估。


<details>
  <summary>Details</summary>
Motivation: 临床文档因其敏感性难以获取真实数据，限制AI开发；合成数据可替代真实数据并规避隐私问题。

Method: 模块化流水线：结合结构化患者生成、半结构化就诊过程模拟，以及大语言模型生成非结构化临床笔记；设计强调纵向记录内部一致性，同时覆盖写作风格、笔记结构和临床细节的多样性；通过LLM验证和增强步骤提升真实性、现实性和多样性。

Result: 发布包含70个合成患者的数据集，每个患者有20-50份笔记覆盖完整医院就诊，并提供多级验证版本以平衡真实性和可扩展性。

Conclusion: 该数据集支持临床AI系统（如总结工具、编码模型、决策支持）的开发、测试和评估，无需依赖真实患者数据。
...
### [246] [Generative Retrieval via Diffusion Transformer with Metric-Ordered Sequence Training and Hybrid-Policy Preference Optimization](https://arxiv.org/abs/2606.26899)
...
TL;DR: 提出模式保持的属性检索任务，并设计MO-DiT+HPPO框架，通过度量排序预训练和偏好优化改进生成式检索，在多个属性域上显著提升交集指标。


<details>
  <summary>Details</summary>
Motivation: 现有嵌入检索往往只返回最高分项，但在种子集表达细粒度模式时，需要更多既满足目标属性又保持模式的项目，而两个目标相互矛盾（平均种子保持模式但属性低，全局属性检索漂移到无关模式），因此需要新模式保持的属性检索方法。

Method: 提出MO-DiT+HPPO框架：包括原始序列预训练、多领域度量排序连续预训练、尾部质心微调、以及HPPO。度量排序训练将稀疏在线检索标签转换为按预测属性密度排序的模式内轨迹，跨领域学习度量改进方向；HPPO通过混合候选池标注在线交集指标并使用参考锚定偏好优化，Pareto对过滤器只保留不降低模式纯度的优胜对。

Result: 在四个属性域（物品和模式留出协议）上，度量排序DiT比预训练生成检索器提升交集指标，HPPO进一步提升，在八个域分裂单元中七个显著提升，最难分裂上边际持平。

Conclusion: 所提出的模式保持属性检索方法有效平衡了保持模式和提高属性指标的目标，通过度量排序和偏好优化显著提升了检索效果，验证了各组件的贡献。
...
### [247] [Learning to Recover Task Experts from a Multi-Task Merged Model](https://arxiv.org/abs/2606.26902)
*Jinwook Jung,Taegyu Kim,Kumju Jo,Sungyong Baik*

Main category: cs.AI

TL;DR: 提出ReTeX框架，通过预测参数偏移来消除多任务模型合并中的参数干扰，从单个合并检查点恢复专家性能，并在未知任务身份时使用基于SVD子空间的标识器，恢复超过95%的单专家性能，且对未见任务有良好泛化。


<details>
  <summary>Details</summary>
Motivation: 多任务模型合并中静态合并存在参数干扰，动态合并方法推理时成本高，需要一种轻量级方式恢复专家性能。

Method: 将参数干扰建模为仿射变换并近似为加性偏移，训练网络预测偏移以撤销干扰；同时提出基于SVD子空间的无路由器任务标识器，通过计算输入在子空间上的投影残差选择任务。

Result: 在视觉和NLP任务上恢复超过95%的单专家性能；对未见任务泛化能力显著提升；偏移预测导致对分布外任务的专家知识自适应插值。

Conclusion: ReTeX有效解决了参数干扰，实现了高效的多任务合并，且具备适应未见任务的能力。
...
### [248] [Diagnosing Task Insensitivity in Language Agents](https://arxiv.org/abs/2606.26918)
*Jingyu Liu,Xiaopeng Wu,Kehan Chen,Chuan Yu,Yong Liu*

Main category: cs.AI

TL;DR: 大型语言模型作为长期代理时，面对分布外任务表现出任务不敏感，导致泛化失败。本文提出任务扰动负对数似然优化，通过对比正则化增强任务依赖，提升任务敏感性和分布外泛化。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在作为长期代理时，对相似但不同的任务缺乏敏感性，倾向于沿用训练模式，导致分布外泛化失败。

Method: 提出任务扰动负对数似然优化，一种轻量级对比正则化方法，显式鼓励动作对任务指令的依赖。

Result: 该方法提升了任务敏感性和分布外泛化，同时保持对任务令牌更稳定的注意力。

Conclusion: 任务扰动负对数似然优化有效缓解了任务不敏感导致的泛化失败问题。
...
### [249] [Where Do CoT Training Gains Land in LLM based Agents?](https://arxiv.org/abs/2606.26935)
*Jingyu Liu,Zhiwen Wang,Yuxin Jing,Huanyu Zhou,Yong Liu*

Main category: cs.AI

TL;DR: 本研究探讨了链式思维推理训练如何影响语言模型的动作预测，发现其主要通过提高无需推理的直接动作预测质量来起作用，而非增强推理过程本身。


<details>
  <summary>Details</summary>
Motivation: 链式思维推理被广泛使用，但研究发现其可能存在事后的、不忠实的推理，因此需要明确训练到底改进了什么：是通过推理改变动作，还是直接预测动作？

Method: 比较了不同训练阶段下的“提示动作”（无CoT）和“CoT动作”（有CoT）的质量，并观察了模型对CoT的响应变化。进一步提出了选择性掩码动作标签训练的方法。

Result: 提示动作质量随训练显著提升，CoT训练并未扩大CoT相对于提示动作的优势；后期检查点更少根据CoT修正动作；选择性掩码动作标签提升了域外泛化。

Conclusion: CoT训练主要受益于改进直接动作预测，而非推理过程；选择性掩码可提升泛化能力。
...
### [250] [Look-Before-Move: Narrative-Grounded World Visual Attention in Dynamic 3D Story Worlds](https://arxiv.org/abs/2606.26964)
*Jiaming Bian,Bingliang Li,Yuehao Wu,Pichao Wang,Zhi Wang,Hailan Ma,Huadong Mo,Zhenhong Sun*

Main category: cs.AI

TL;DR: 提出Look-Before-Move框架，将视觉注意力组织与相机运动分离，先通过语义观察契约和蒙特卡洛视点搜索规划观察，再生成轨迹，在3D故事世界基准上优于基线。


<details>
  <summary>Details</summary>
Motivation: 动态3D环境中，视觉感知需主动决定观察什么，而现有方法仅被动解释观测。

Method: Look-Before-Move框架：1)语义观察契约将导演意图转为可执行约束；2)蒙特卡洛视点搜索找符合叙事和几何的视点；3)语义轨迹接地连接视点生成连续无碰撞运动。

Result: 在50个故事、457场景、1585镜头的StoryBlender基准上，主体感知、意图一致性和轨迹质量优于代表性基线。
...
### [251] [Einstein World Models](https://arxiv.org/abs/2606.26969)
*Munachiso Samuel Nwadike,Zangir Iklassov,Ali Mekky,Zayd M. Kawakibi Zuhri,Kentaro Inui*

Main category: cs.AI

TL;DR: 提出Einstein World Models（EWM），一种通过可视化反事实事件来增强LLM推理能力的蓝图，将视觉-时间展开插入推理轨迹，调用世界模块生成可检验假设。


<details>
  <summary>Details</summary>
Motivation: 探索可视化反事实事件是否能补充语言，帮助LLM进行复杂推理。

Method: 在LLM推理过程中插入视觉-时间展开，通过调用世界模块生成场景短展开，作为可检验的假设供后续推理使用。

Result: EWM将LLM的工具调用能力（如搜索、代码执行）扩展到视觉思想实验领域。

Conclusion: EWM使LLM能够利用可视化机制进行文本可能无法充分支持的推理。
...
### [252] [Adaptive Utility driven Resource Orchestration for Resilient AI (AURORA-AI)](https://arxiv.org/abs/2606.27005)
*Rahul Umesh Mhapsekar,Ilias Cherkaoui,Lizy Abraham,Indrakshi Dey*

Main category: cs.AI

TL;DR: 提出AURORA-AI框架，在非平稳环境下通过自适应资源分配同时优化性能、公平性、鲁棒性等，实证优于多种基线。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统部署环境非平稳，静态资源分配策略在性能、公平性、可解释性等方面退化，亟需自适应方法。

Method: 采用Hamilton-Jacobi-Bellman反馈控制、李雅普诺夫稳定性监测和公平性感知复合效用函数，构成闭环策略，动态分配计算预算。

Result: AURORA-AI在黑天鹅事件后立即恢复（vs静态88步、PPO22步），α分位数和超分位数分别提升29%和25%，平均和最大人口统计奇偶差距同时降低，李雅普诺夫稳定运行步数增加。

Conclusion: 基于稳定性理论的公平性感知自适应编排是实现弹性人本AI的实用且理论合理的路径。
...
### [253] [Semantic Early-Stopping for Iterative LLM Agent Loops](https://arxiv.org/abs/2606.27009)
*Sahil Shrivastava*

Main category: cs.AI

TL;DR: 提出语义早停方法替代固定迭代次数，在HotpotQA上减少38%操作token且质量不变，但质量门控因裁判成本过高无效。


<details>
  <summary>Details</summary>
Motivation: 固定迭代次数终止多智能体LLM循环不考虑答案是否改进，导致简单输入浪费token，困难输入被截断。

Method: 当连续草稿嵌入的余弦距离变化小于阈值且答案质量停止改进时终止；理论证明确定性终止，并设计裁判高效评估协议。

Result: 在HotpotQA上，无裁判的语义停止器减少38%操作token且质量与固定迭代相当；质量门控变体因裁判成本过高效果更差。

Conclusion: 语义早停有效降低token消耗，但最优轮次选择比何时停止更关键，需进一步研究。
...
### [254] [How to evaluate clustering with ground truth?](https://arxiv.org/abs/2606.27061)
*Pasi Fr\"anti*

Main category: cs.AI

TL;DR: 综述了基于集合匹配的外部聚类有效性指标，推荐质心指数（CI）作为直观的簇级度量，并比较了PSI和ACC等点级度量的适用场景。


<details>
  <summary>Details</summary>
Motivation: 当存在真实标签时，需要评估聚类结果，但现有外部指标众多，需明确其特性与适用条件。

Method: 回顾了常见的基于集合匹配的外部指标，重点分析CI、PSI和ACC，并讨论其偏差与适用性。

Result: CI是簇级直观度量且可解释；PSI提供标准化评分且无簇大小偏差；ACC等集合匹配度量适用于点级重要性相等的情况。

Conclusion: 根据评估粒度（簇级或点级）和需求（可解释性、无偏性等）选择合适的指标，推荐CI、PSI或ACC。
...
### [255] [Joint Learning of Experiential Rules and Policies for Large Language Model Agents](https://arxiv.org/abs/2606.27136)
*Shicheng Ye,Chao Yu*

Main category: cs.AI

TL;DR: 提出JERP方法，联合学习经验规则和策略，使LLM智能体在多步交互环境中有效利用累积经验。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么将经验作为外部规则用于提示，要么用于更新模型参数，但两者各有局限，难以兼顾可解释性和策略改进。

Method: JERP维护长期经验规则池和策略，决策时检索相关规则并结合交互历史，每轮后使用轨迹优化策略并修订规则池。

Result: 在AlfWorld和WebShop实验上，JERP在复杂交互任务中取得一致的决策性能提升。

Conclusion: 规则池与策略的联合更新能保持对齐，将稳定有效行为逐渐吸收到模型中，提升决策表现。
...
### [256] [OpenRCA 2.0: From Outcome Labels to Causal Process Supervision](https://arxiv.org/abs/2606.27154)
*Aoyang Fang,Yifan Yang,Jin'ao Shang,Qisheng Lu,Junjielung Xu,Rui Wang,Songhan Zhang,Yuzhong Zhang,Boxi Yu,Pinjia He*

Main category: cs.AI

TL;DR: 提出PAVE协议，构建OpenRCA 2.0数据集，包含逐步因果标注；发现LLM在精确根因恢复上成功率仅20.7%，且存在未接地诊断问题。


<details>
  <summary>Details</summary>
Motivation: 现有根因分析数据集仅标注根因，缺少因果传播路径，导致任务简化为模式匹配，无法全面评估LLM能力。

Method: 利用故障注入标记已知干预，通过前向验证（从原因到效果）重建因果传播路径，形成逐步标注协议PAVE。

Result: 在11个前沿LLM上，精确根因集恢复成功率平均20.7%；正确根因服务识别率76.0%，但将其接地到因果路径的比率仅61.5%。

Conclusion: 仅结果评估隐藏了失败模式，逐步因果真值是评估可信LLM根因分析代理的关键组件。
...
### [257] [TOPS: First-Principles Visual Token Pruning via Constructing Token Optimal Preservation Sets for Efficient MLLM Inference](https://arxiv.org/abs/2606.27161)
...
TL;DR: 提出TOPS方法，基于信息论原则优化视觉标记剪枝，在去除77.8%视觉标记的同时保持甚至提升多模态大模型性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大模型因大量视觉标记导致计算开销大，现有剪枝方法（如注意力驱动或多样性驱动）不完善，缺乏统一原则。

Method: 从第一性原理出发，将视觉标记剪枝形式化为构建最优保留集，通过信息论分析确立任务相关性、信息覆盖和语义多样性三大原则，提出训练无关、模型无关的TOPS剪枝模块。

Result: 在7个模型骨架和14个基准上，TOPS优于先前方法；在LLaVA-NeXT上，去除77.8%视觉标记后，7B和13B模型分别保持100.0%和100.6%的性能。

Conclusion: 冗余标记剪枝可缓解幻觉，为未来轻量级多模态大模型设计提供新思路。
...
### [258] [A Process Harness for Uplifting Legacy Workflows to Agentic BPM: Design and Realization in CUGA FLO](https://arxiv.org/abs/2606.27188)
*Fabiana Fournier,Lior Limonad*

Main category: cs.AI

TL;DR: 提出过程框架（process harness），在不替换底层工作流引擎的情况下，将遗留工作流升级为智能体业务流程管理（Agentic BPM）。


<details>
  <summary>Details</summary>
Motivation: 传统工作流引擎缺乏推理和适应性，需要一种机制在保留结构控制的同时引入智能体能力。

Method: 定义任务-决策-流程（TDF）模型，包含三种策略驱动智能体：TaskAgent处理知识密集型任务，DecisionAgent负责网关路由，FlowAgent通过钩子机制管理运行时流程适配。基于CUGA FLO实现该模型。

Result: 在贷款审批流程中验证，三种智能体及钩子驱动的监管覆盖均得到应用。

Conclusion: 过程框架成功调和了通过确定性执行实现的命令性要求与通过策略框架智能体自主实现的规范性要求。
...
### [259] [Vulnerability of Natural Language Classifiers to Evolutionary Generated Adversarial Text](https://arxiv.org/abs/2606.27215)
*Manjinder Singh,Alexander E. I. Brownlee,Mohamed Elawady*

Main category: cs.AI

TL;DR: 提出GAversary，一种基于遗传算法的黑盒对抗攻击方法，利用GloVe嵌入进行词替换，在多个数据集上显著降低模型准确率，但扰动较多词，语义相似度略低。


<details>
  <summary>Details</summary>
Motivation: 深度NLP模型易受对抗攻击，现有方法需模型内部信息或攻击效果有限，需要一种高效黑盒攻击方法。

Method: 使用遗传算法，仅依赖模型输出的logit值作为适应度，通过GloVe嵌入实现变异算子中的词替换，以保持语义相似性。

Result: 在多个基准数据集上，GAversary将目标模型准确率从76.8%降至5.8%（最佳情况），优于BAE（降至27.6%）和A2T；但平均扰动词数约为对比方法的两倍，语义相似度略低，运行时间增加约5%。

Conclusion: GAversary是一种有效的黑盒对抗攻击方法，但存在扰动词数、语义相似度和运行时间之间的权衡。
...
### [260] [Ask, Don't Judge: Binary Questions for Interpretable LLM Evaluation and Self-Improvement](https://arxiv.org/abs/2606.27226)
*Sangwoo Cho,Kushal Chawla,Pengshan Cai,Zefang Liu,Chenyang Zhu,Shi-Xiong Zhang,Sambit Sahu*

Main category: cs.AI

TL;DR: BINEVAL 是一个将评估标准分解为原子二元问题的框架，通过聚合答案提供可解释的多维评分，在多个基准上匹配或超越强基线，并支持迭代提示优化。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估方法存在人工评估昂贵、词汇指标与人类判断相关性差、整体性LLM评估器得分不透明且难以调试等问题。

Method: 通过元提示生成细粒度评估问题，让LLM独立回答每个问题，汇总形成透明的逐问题反馈和校准后的整体得分。

Result: 在SummEval、Topical-Chat和QAGS上匹配或超越UniEval和G-Eval，尤其在事实一致性基准上表现优异；更好匹配人类评分分布，避免天花板效应，并支持自我更新和跨模型更新的提示优化。
...
### [261] [EO-WM: A Physically Informed World Model for Probabilistic Earth Observation Forecasting](https://arxiv.org/abs/2606.27277)
*Junwei Luo,Shuai Yuan,Zhenya Yang,Yansheng Li,Zhe Liu,Hengshuang Zhao*

Main category: cs.AI

TL;DR: 提出一种视频扩散变换器EO-WM，用于地球观测预测，通过物理信息条件框架改善对极端天气的响应，在NDVI下降幅度和方向命中率上取得提升。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法充分处理天气驱动下的不确定性和稀疏观测，且缺乏对天气响应正确性的评估基准。

Method: 采用视频扩散变换器，引入基于气候基线和异常累积物理应力的条件框架，分离基线和异常条件路径。

Result: 在极端夏季基准上，预测NDVI下降幅度误差相对减少5.63%，方向命中率相对提升7.80%，标准像素级指标也具有竞争力。

Conclusion: EO-WM能更准确响应天气变化，提供极端事件下植被退化预测，相关基准和模型将开源。
...
### [262] [Simulation-based inference for rapid Bayesian parameter estimation in epidemiological models: a comparison with MCMC](https://arxiv.org/abs/2606.27286)
...
TL;DR: 模拟推断（SBI）通过神经后验估计，在贝叶斯校准流行病学模型中达到与MCMC相当的后验精度，但计算速度提升数十倍。


<details>
  <summary>Details</summary>
Motivation: MCMC在高维非线性系统和近实时分析中计算成本高，需更高效的贝叶斯校准方法。

Method: 采用神经后验估计的SBI，在SECIR模型上基于德国2020年ICU数据，与MCMC比较31天和201天推断窗口的后验分布。

Result: 31天窗口SBI后验与MCMC高度一致，201天窗口保持主结构；SBI耗时60-70秒（单GPU）vs MCMC约1000秒，201天任务SBI 157秒 vs MCMC超19000秒。

Conclusion: SBI为流行病学模型贝叶斯校准提供快速计算框架，适合近实时推断和暴发分析。
...
### [263] [Prompt Injection in Automated R\'esum\'e Screening with Large Language Models: Single and Multi-Injection Settings](https://arxiv.org/abs/2606.27287)
*Preet Baxi,Jiannan Xu,Jane Yi Jiang,Stefanus Jasin*

Main category: cs.AI

TL;DR: 大型语言模型在简历筛选中易受提示注入攻击，尤其在候选人质量相近且少数人使用时效果显著，但广泛使用后效果减弱，且可能加剧不公平。


<details>
  <summary>Details</summary>
Motivation: 研究候选人如何通过提示注入策略性地影响LLM的自动简历筛选结果。

Method: 通过控制实验，比较不同质量候选人使用提示注入对排名的影响。

Result: 提示注入在候选人质量同质且少数人使用时提升排名；当多数人使用或质量异质时效果减弱，但可能使低质量候选人超越高质量候选人。

Conclusion: LLM筛选在操纵罕见且质量差异小时最易受攻击。
...
### [264] [When Does Combining Language Models Help? A Co-Failure Ceiling on Routing, Voting, and Mixture-of-Agents Across 67 Frontier Models](https://arxiv.org/abs/2606.27288)
*Josef Chen*

Main category: cs.AI

TL;DR: 多模型LLM系统的准确率提升受限于所有模型同时出错的比率（beta），且该比率无法通过平均成对错误相关性（rho）识别，实验表明实际beta远高于模型预测值。


<details>
  <summary>Details</summary>
Motivation: 现有研究常报告平均相关性等指标，但忽略了多模型系统中所有模型同时出错这一关键限制，导致对系统能力上限的错误估计。

Method: 通过定义beta为所有模型在同一查询上均出错的概率，并利用Clopper-Pearson区间给出beta的有限样本上界，然后对67个模型在开放式数学和编码任务上进行实际测量与理论比较。

Result: 实际beta在开放式数学上为0.052，约为高斯copula预测值0.023的2.5倍；在代码任务上为0.079；自由回答形式的问题beta升至0.127。低相关异质集成在高相关性Self-MoA表现更好，但合并模型通常不优于单一最佳模型。

Conclusion: 多模型系统的增益主要来自模型在不同问题上失败，而非增加模型数量；实际beta远高于理论预测，表明现有模型低估了共失败尾端风险，路由信号比模型数量更关键。
...
### [265] [Language-Based Digital Twins for Elderly Cognitive Assistance](https://arxiv.org/abs/2606.27334)
*Mohammad Mehdi Hosseini,Mohammad H. Mahoor,Hiroko H. Dodge*

Main category: cs.AI

TL;DR: 提出基于语言模型的数字孪生框架，模拟老年人对话特征，用于认知健康监测。


<details>
  <summary>Details</summary>
Motivation: 早期检测轻度认知障碍（MCI）困难，语言模式可作为非侵入性生物标志物。

Method: 利用大语言模型结合风格线索和元数据构建数字孪生，使用多头条件变分自编码器评估。

Result: 数字孪生保留身份特征，重建误差和MoCA预测误差与真实数据相当，优于基线GPT。

Conclusion: 基于语言的数字孪生为个性化、非侵入性的持续认知监测提供可扩展方案。
...
### [266] [On Support Cardinality for Discrete Schr\"odinger Equation](https://arxiv.org/abs/2606.26149)
*Linjun Li*

Main category: math-ph

TL;DR: 离散薛定谔方程非平凡解的最小支撑大小研究，提出降维原理，得到四维近最优下界。


<details>
  <summary>Details</summary>
Motivation: 探索离散薛定谔方程解可以稀疏到什么程度，即非零点数量的最小值。

Method: 考虑有限d维晶格盒子上的Dirichlet解，允许任意实势能，通过降维原理将高维问题归约到低维。

Result: 最小可能支撑大小随维度增加不减小；四维空间中得到了与二维最佳构造仅差对数因子的近最优下界。

Conclusion: 降维原理使低维下界自动推广到高维，四维下界接近最优。

Abstract: How sparse can a nontrivial solution of a discrete Schr\"odinger equation be? In this note we study Dirichlet solutions on a finite $d$-dimensional lattice box, allowing an arbitrary real potential, and measure sparsity by the number of lattice sites at which the solution is nonzero (assuming it is nonzero at the origin). Our main result is a dimension-reduction principle: the minimal possible support size cannot decrease when the dimension increases. Consequently, any lower bound proved in dimension $d-1$ automatically yields the same lower bound in dimension $d$. As an application, we obtain a nearly sharp lower bound in four dimensions, matching the best-known two-dimensional constructions up to a logarithmic factor.

</details>


### [267] [The Colored Hofstadter Butterfly as a Many-Body Quantum Hall Phase Diagram](https://arxiv.org/abs/2606.26256)
*Giovanna Marcelli,Tadahiro Miyao,Domenico Monaco,Stefan Teufel,Marius Wesle*

Main category: math-ph

TL;DR: 证明彩色霍夫施塔特蝴蝶对弱相互作用晶格费米子有多体解释，其整数颜色作为相互作用量子霍尔相霍尔电导的标记持久存在。


<details>
  <summary>Details</summary>
Motivation: 为霍夫施塔特蝴蝶提供多体解释，尤其是无理磁通下有限磁单胞不存在时的相互作用系统。

Method: 结合准绝热连续与分母无关的磁扰动估计，从单粒子谱隙出发构造相互作用系统的局部唯一带隙基态区域。

Result: 在连接非相互作用平面的均匀带隙区域上，霍尔电导常数且量子化，满足2πσ^H∈Z。
...
### [268] [A new formulation of metriplectic dynamics with an application to quasigeostrophic ocean modeling with advected quantities](https://arxiv.org/abs/2606.26388)
*Francisco J. Beron-Vera,Erwin Luesink*

Main category: math-ph

TL;DR: 提出一种度量-辛动力学一般公式，通过相乘两个反对称括号构建四括号，并应用于包含平流量的广义二维准地转海洋模型，确保内能守恒和熵产生，灵活实现从近乎位涡守恒到实际强迫耗散的不可逆性。


<details>
  <summary>Details</summary>
Motivation: 为在地球物理流体动力学中引入符合热力学定律的不可逆过程，需要一种统一且灵活的动力学框架。

Method: 通过相乘两个反对称括号构造度量-辛四括号，形成一般性公式，并应用于包含平流量的广义二维准地转上层海洋模型。

Result: 新动力学公式确保内能守恒和熵产生，允许灵活指定不可逆性类型，从近乎物质守恒的位涡到实际强迫和耗散。

Conclusion: 该度量-辛公式为二维准地转海洋建模中不可逆性的规范化提供了一般且灵活的工具。
...
### [269] [Weak-Strong Resurgence Duality](https://arxiv.org/abs/2606.26637)
*Gerald V. Dunne,Ayssar I. Farah*

Main category: math-ph

TL;DR: 发现了一种明确的 resurgence 对偶性，当弱耦合和强耦合展开中一个收敛半径为零而另一个为无穷大时成立。


<details>
  <summary>Details</summary>
Motivation: 补充了之前已知的两种情况：收敛展开半径有限，或两个展开半径均为零。

Method: 通过 Airy 和 Pearcey 尖点积分展示该现象，并应用于两个物理实例。

Result: 在零维标量 φ⁴ 理论的 Dyson-Schwinger 方程以及 Gross-Neveu 模型中扭结-反扭结晶格鞍点配置的涨落算子的热核迹中，验证了该对偶性。
...
### [270] [Wave-Particle Decomposition for Kinetic Equations I: Theory and Numerics](https://arxiv.org/abs/2606.26915)
*Chang Liu,Kun Xu*

Main category: math-ph

TL;DR: 提出一种基于局部演化时间尺度和动力学视界的波粒分解方法，将分布函数分解为波分量和粒子分量，得到跨克努森谱的统一波粒系统。


<details>
  <summary>Details</summary>
Motivation: 克服全局时间步分裂的局限，实现PDE层面的波粒划分，并适应任意克努森数。

Method: 利用特征积分解分解分布函数，构造守恒律、波动方程和粒子方程；采用保守宏观-微观方法离散，波通量由纳维-斯托克斯气体动力学通量计算，粒子通量由确定论或蒙特卡洛方法计算。

Result: 一维到三维数值测试验证了框架的准确性、多尺度能力和效率。

Conclusion: 该波粒分解方法具有渐近保持性、稀薄区域一致性以及主动动力学自由度的区域自适应缩放特性。
...
### [271] [Asymptotic solutions: glancing trajectories, Lagrangian singularities, and Bessel cylinders](https://arxiv.org/abs/2606.26962)
*Ilya Bogaevskii,Michel Rouleux*

Main category: math-ph

TL;DR: 发现了Hamilton-Jacobi方程解的Lagrangian奇点的规范形式，并用于近似半经典方程的渐近解。


<details>
  <summary>Details</summary>
Motivation: 处理当能量E0是H在Lagrangian流形Λ0上的临界值时，半经典分布f_h的方程(Ĥ-E0)u_h=f_h的渐近解。

Method: 通过分析最简单的Lagrangian奇点，找到其规范形式，并基于此构造近似解。

Result: 获得了该奇点的规范形式，并基于此给出了渐近解的显式表达（未在摘要中详细说明）。

Conclusion: 该规范形式有效简化了临界值情况下的渐近求解，为类似问题提供了方法。

Abstract: We find a normal form for the simplest Lagrangian singularity appearing as the projection onto the space-energy of a solution to the Hamilton-Jacobi equation. Using this normal form we approximate asymptotic solutions of an equation $(\widehat{H}-E_0) \, u_h = f_h$ with a semiclassical distribution $f_h$ microlocalized on a Lagrangian manifold $\Lambda_0$ when $E_0$ is a critical value of the restriction $H|_{\Lambda_0}$.

</details>


### [272] [The Ising Model Coupled to 2D Gravity: Critical Partition Function](https://arxiv.org/abs/2606.27125)
*Maurice Duits,Nathan Hayford,Seung-Yeop Lee*

Main category: math-ph

TL;DR: 证明2矩阵模型配分函数对数微分在双极限尺度下收敛到(3,4)弦方程τ函数微分，证实临界伊辛模型在随机表面上收敛到(3,4)拓扑最小模型。


<details>
  <summary>Details</summary>
Motivation: 证实Douglas、Shenker等人提出的临界伊辛模型在随机表面上收敛到(3,4)拓扑最小模型的论断。

Method: 基于Riemann-Hilbert问题的最速下降分析，结合双正交多项式族，处理匹配问题中的新特征。

Result: 成功证明了收敛性，确认了随机表面上临界伊辛模型与(3,4)拓扑最小模型之间的联系。

Conclusion: 该结果确认了随机表面上临界伊辛模型与(3,4)拓扑最小模型之间的联系。
...
### [273] [Geometric bulk-edge correspondence for $\mathbb{Z}_2$-topological insulators](https://arxiv.org/abs/2606.27318)
*Alexis Drouot,Jacob Shapiro,Xiaowen Zhu*

Main category: math-ph

TL;DR: 证明了弯曲界面下二维时间反演不变绝缘体的Z2体-边对应关系。


<details>
  <summary>Details</summary>
Motivation: 理解弯曲边界对拓扑绝缘体边缘态的影响。

Method: Z2版本的弯曲界面连接公式，类比于Hall绝缘体的处理。

Result: 界面Z2边指数等于体Z2指数差与几何交数的乘积模2。

Conclusion: 建立了弯曲界面的几何体-边对应，推广了平坦界面的结果。
...
### [274] [$\mathrm{W}^*$-algebraic Integration Theory](https://arxiv.org/abs/2606.27366)
*Jan G{\l}owacki,Yui Kuramochi*

Main category: math-ph

TL;DR: 本文定义了基于W*-代数和POVM的算子值积分，并证明了其忠实正规完全正性，建立了莱布尼茨法则和富比尼定理。


<details>
  <summary>Details</summary>
Motivation: 推广经典积分到非交换算子代数框架，为量子信息理论提供数学基础。

Method: 利用空间张量积和Stinespring分解，通过Naimark扩张推导完全正性。

Result: 积分映射是忠实正规完全正映射，对PVM是*-同态，对局部化POVM是等距；积分空间同构于W*-代数。

Conclusion: 建立了算子值积分的莱布尼茨法则和富比尼定理。
